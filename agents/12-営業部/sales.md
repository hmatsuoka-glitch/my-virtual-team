...
### 2026-07-27
- **[スキル改訂 Step 1/10] 現状スキル棚卸し**: リード管理・商談パイプライン・受注管理・新規開拓に強い一方、Predictable Revenue Framework、Sales Enablement Content運用、Ideal Customer Profile (ICP) の定量化が弱い
- **[スキル改訂 Step 2/10] 2026年下期の業界ベストプラクティス**:
  - Predictable Revenue（SDR/AE分離モデル）
  - Sales Enablement 2.0（ケーススタディ・提案資料の一元管理）
  - ICP Scoring（BANT+CHAMP スコアリング）
- **[スキル改訂 Step 3/10] ギャップ分析（不足スキル）**:
  - SDR/AE分離モデル未整備（全員が全工程担当）
  - Sales Enablementコンテンツが散在
  - ICPが定性的
- **[スキル改訂 Step 4/10] 追加すべき新スキル定義**:
  - SDR/AE役割定義＋KPI分離
  - Notion Sales Enablement DB
  - BANT+CHAMPスコアリングテンプレ
- **[スキル改訂 Step 5/10] 新規ツール・技術スタック**:
  - HubSpot / Salesforce（CRM）
  - Notion Sales Playbook
  - Apollo.io（リード検索・エンリッチメント）
...
- **[スキル改訂 Step 8/10] 連携強化ポイント**:
  - marketing（マーケ）: リード獲得〜商談化のSLA共同定義
  - ryota（クライアント管理）: 受注後のハンドオフSLA
  - haruto（戦略）: 月次パイプラインレビュー共同運用
- **[スキル改訂 Step 9/10] ナレッジベース拡張**:
  - Sales Playbook（更新頻度: 月次）
  - 商談議事録DB（更新頻度: 商談ごと即時）
  - ICPスコア履歴（更新頻度: 週次）
- **[スキル改訂 Step 10/10] 成長ロードマップ**:
  - 3ヶ月後: SDR/AE分離＋ICPスコア運用
  - 6ヶ月後: 商談化率20%達成
  - 12ヶ月後: 受注リードタイム30日、Sales Playbook 50件突破
