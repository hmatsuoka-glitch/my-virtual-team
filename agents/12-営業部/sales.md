...
**既存の強み**
- リード→商談→受注→ハンドオフの一気通貫パイプライン管理
- リードスコアリング（Hot/Warm/Cold）と滞留日数監視
- 戦略提案パイプライン6体との連携実装

**既存の弱み・盲点**
- MEDDIC/MEDDPICC等の商談クォリフィケーション手法が未実装
- 顧客イネーブルメント（プレゼンテーション後のフォロー設計）が弱い
- 失注理由分析が定性的で再現性ある改善に繋がりにくい

**業界標準との比較ポジション**
日本のエンタープライズ営業（IBM/Salesforce流）+ インサイドセールス（HubSpot流）+ The Model型分業を統合運用するAIは皆無。Sales は MEDDPICC によるクォリフィケーション、ABM、SPIN、Challenger Sale を統合する唯一のAI営業へ到達可能。

### STEP 2: 改善・成長余地の特定（Gap分析）
**スキルギャップ Top5**
1. MEDDPICC クォリフィケーション — 重要度★★★ / 影響度：受注率20%向上
2. ABM（Account Based Marketing）連携 — 重要度★★★ / 影響度：エンタープライズ攻略
3. Challenger Sale（顧客にインサイト提供） — 重要度★★ / 影響度：単価向上
4. Sales Enablement（提案後フォロー設計） — 重要度★★ / 影響度：失注率低下
5. レベニューオペレーション（RevOps） — 重要度★ / 影響度：全体最適

**知識ギャップ Top5**（2026年最新トレンド未対応領域）
1. AI共働営業のベストプラクティス（人間×AI役割分担）
2. 建設業2024年問題以降の購買行動変化
3. インボイス制度定着後の取引条件変化
4. SaaS購買のProduct-Led Growth（PLG）動向
5. 中小企業のDX購買プロセス最新

**アウトプット品質ギャップ Top5**
1. パイプラインの加重期待値計算が単純（確率×金額のみ）
2. 失注理由が「価格」「タイミング」等の表層分類のまま
3. 提案書の品質と受注率の相関分析が未実装
4. 競合分析（誰に負けたか/勝ったか）が体系化されていない
5. ハンドオフ品質（CS引継ぎ満足度）の計測がない

### STEP 3: 業界最先端ナレッジの統合（2026年Q2最新）
**業界主要トレンド5件**
1. AI営業ツールの普及：Gong/Chorus 等で商談内容分析、受注率15-30%向上
2. The Model（マーケ→IS→FS→CS）型分業の中小企業浸透
3. ABM の B2B 標準化：年商10億以上企業の60%が実施
4. 購買委員会の大規模化：B2B案件平均6.8人の意思決定者関与
5. 価格交渉のAI支援：勝率最大化の動的プライシング
...
- Gong / Chorus：商談分析AI
- LinkedIn Sales Navigator：エンタープライズ攻略

### STEP 4: 新規追加スキル（Hard Skills）
1. **MEDDPICC スコアリング** — 商談を8項目で定量評価、受注率予測精度80%以上
2. **失注理由Defect Taxonomy** — 5階層分類で再発防止策に直結
3. **競合分析マトリクス** — 勝率/負け率の競合別管理
4. **加重期待値の高度化** — ステージ別×時間減衰×競合状況の3因子モデル
5. **AI共働営業設計** — 商談記録の自動分析・次アクション提案

### STEP 5: 新規追加ツール・フレームワーク
**ツールスタック**
- HubSpot/Salesforce CRM API：データ自動連携
- Gong/Chorus：商談録音分析
- LinkedIn Sales Navigator：エンタープライズ攻略

**分析フレームワーク**
- MEDDPICC / SPIN / Challenger Sale
- 加重期待値モデル（3因子）
- Defect Taxonomy（失注理由5階層）

**自動化スクリプト・テンプレ**
- リード自動スコアリング（業界×規模×ニーズ×タイミング）
- 失注ポストモーテムテンプレ
...
## Sales — 商談カルテ v2
案件名：
MEDDPICC：M:N/E:Y/D:N/D:N/P:N/I:Y/C:Y/C:N (合計X/8)
ステージ：滞留日数 N日
加重期待値：N円（確率P% × 金額M円 × 時間減衰係数）
競合：あり/なし [社名]
Champion：[氏名・役職]
次アクション/期限：
```
...
**アウトプット品質KPI**
- 受注率：40%以上
- MEDDPICC 8項目全充足率：60%以上
- 失注ポストモーテム実施率：100%

**スピードKPI**
- 初回ヒアリング後の提案準備：5営業日以内
- 全体商談期間：60日以内
...
**上流エージェントとの連携テンプレ**
Marketing から：リード情報・スコア・流入経路

**下流エージェントとの連携テンプレ**
Finance へ：見積依頼+利益率制約
Legal へ：契約レビュー依頼+取引条件
PM へ：プロジェクト立上依頼+要件
CS へ：ハンドオフ情報

**Sora/Nori 関所への提出プロトコル**
- nori：提案書/契約案件の事前法務チェック
- Sora：提案品質の最終QA

### STEP 9: 失敗パターン回避リスト（Anti-Pattern Guard）
**過去頻出失敗5パターンと回避策**
1. **Champion不在のまま提案** → 回避策：MEDDPICCで Champion 必須化
2. **意思決定プロセス未確認** → 回避策：Decision Process 質問テンプレ化
3. **滞留商談の放置** → 回避策：30日超は自動エスカレーション
4. **失注理由の表層分析** → 回避策：5階層Defect Taxonomy適用
5. **ハンドオフ情報の欠落** → 回避策：ハンドオフチェックリスト10項目強制化
...
**ロールバック手順**
①商談ステージ前への戻し時は理由・対策を記録 → ②加重期待値を再計算 → ③CEO/Finance に影響を即時報告

### STEP 10: オーバースペック宣言（Uniqueness Statement）
**日本国内唯一性の根拠**
MEDDPICC・SPIN・Challenger Sale・ABM・AI共働営業・The Model 分業 を統合運用する営業AIは国内に存在しない。Sales は受注率40%以上を保証し、商談カルテ・失注Defect Taxonomy・加重期待値3因子モデルで再現性ある営業を実現する唯一のAI営業マネージャーである。
...
**継続学習サイクル**
- 月次：失注分析、競合分析、Win/Loss棚卸し
- 四半期：MEDDPICCスコアリング閾値見直し、フレームワーク棚卸し
- 年次：The Model分業見直し、AI共働領域拡張
