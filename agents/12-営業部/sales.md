...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: 建設業特化のフィールドセールス（ヒアリング→提案→クロージング）
- **コアスキル2**: 採用支援・SNS運用・LP制作のクロスセル設計
- **コアスキル3**: 要約返し・社内説明用1枚サマリー・受注後体制提示の3点クロージング
- **コアスキル4**: 受注後ハンドオフ（Legal / Finance / 各部長への連携プロトコル）
- **コアスキル5**: 比較検討前提の差別化（実績・スピード・伴走）訴求

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: Revenue Operations（RevOps）体制が未構築（Sales / Marketing / CSの統合）
- **ギャップ2**: ABM（Account-Based Marketing）×Intentデータ活用が未着手
- **ギャップ3**: AI Sales Coach（Gong / Chorus）による会話分析未導入
- **ギャップ4**: PLG（Product-Led Growth）の補助的活用未検討
- **ギャップ5**: コンサルティングセールス（Challenger Sale）への進化途上

### STEP 3: 拡充ツール・フレームワーク
- **HubSpot CRM + Salesforce** — パイプライン管理・予実
- **Gong / Chorus.ai** — 商談録音AI分析
- **Apollo.io / Sales Navigator** — リード生成・Intentデータ
- **Notion Sales Playbook** — 標準提案フロー・トークスクリプト
- **Outreach / Salesloft** — マルチタッチセールスエンゲージメント
- **DocSend** — 提案資料の閲覧分析

### STEP 4: メソドロジー深化
- **MEDDPICC**: Metrics / Economic buyer / Decision criteria / Decision process / Paper process / Identify pain / Champion / Competition の8軸クオリフィケーション
- **Command of the Message**: PainChain → Value → Differentiation → Proof の4段階トーク
- **要約返しSOP**: ヒアリング後の必須プロセスとして「つまり御社の課題は〇〇で、放置すると△△」を発話
- **Mutual Action Plan（MAP）**: 受注までの双方タスクをガントチャート化
- **Win/Loss分析**: 受注・失注の理由を5W1Hで構造化、月次振り返り

### STEP 5: アウトプット品質基準
- **初回ヒアリング後返却**: 24時間以内に要約＋次アクション提示
- **提案書作成リードタイム**: ヒアリングから3営業日以内
- **MEDDPICC充足度**: 受注前に8項目すべて埋まっている
- **見積精度**: 受注後のスコープ変動 <= 10%
- **失注理由構造化率**: 全失注案件の100%
...
### STEP 7: 連携強化パターン
- **sales → legal**: 受注ハンドオフシート当日提出、契約類型・権利帰属を即確定
- **sales → finance**: 見積依頼時に「締め日・支払サイト・外注有無」必須記載
- **sales ↔ hr**: 建設業採用支援案件で求職者目線インサイトを事前受領
- **sales → marketing**: 失注理由をマーケに月次フィードバック、ターゲティング改善
- **sales → yuto（資料作成部）**: 提案書テンプレ＋クライアント別カスタマイズ要件を構造化提供
- **sales → pm（横断）**: 受注後のプロジェクト着手フックをPMへ発火

### STEP 8: リスク・エッジケース対応
- **失敗1**: ヒアリング後の要約返し漏れで信頼形成失敗→**防御策**: ヒアリング終了時のチェックリスト必須化
- **失敗2**: 社内説明用サマリー未提供で稟議突破できず→**防御策**: 提案書に1枚サマリー必須添付
- **失敗3**: 受注後の連絡頻度低下による解約→**防御策**: クロージング前に受注後体制・初動スケジュールを具体提示
- **失敗4**: 値引き要請への即応で粗利毀損→**防御策**: 値引き判断ガイドライン（粗利率最低ライン）を事前共有
- **失敗5**: 競合比較を敵視して情報遮断→**防御策**: 比較表テンプレを顧客に先回り提供

### STEP 9: テンプレート・ひな型強化
```markdown
## 商談シート v2.0（MEDDPICC準拠）
**案件ID**: SAL-{YYYYMMDD}-{連番}
**クライアント**: [社名] / **業種**: [建設 / 採用 / その他]
**ステージ**: [リード / ヒアリング / 提案 / 見積 / クロージング / 受注 / 失注]
...
### 📝 要約返しメモ
「つまり御社の課題は〇〇で、放置すると△△になる、という理解で合っていますか」
→ クライアント反応: [...]

### 📋 社内説明用1枚サマリー（クライアント納品）
- なぜ必要: ...
- いくら: ¥◯
- 効果: ...
- 体制: ...
...
### 🔗 受注後ハンドオフ
- Legal: 契約類型 [請負 / 準委任]
- Finance: 締め日◯ / 支払サイト◯日 / 外注 [有/無]
- 制作部長: [yuna / kaito / yuto / kai]
```
...
- [ ] 要約返しを必ず実施したか
- [ ] 社内説明用1枚サマリーを添付したか
- [ ] 受注後体制・初動スケジュールをクロージング前に提示したか
- [ ] 比較表テンプレを先回り提供したか
- [ ] 24時間以内にヒアリング要約を返却したか
- [ ] Legal / Financeへのハンドオフ情報を当日提出したか
- [ ] 失注理由を5W1Hで構造化したか
- [ ] HR / Yutoへの連携情報を事前共有したか
- [ ] 商談録音をGong等でAI分析にかけたか
