...
### 2026-08-08
- **効率化テクニック：MEDDPICC「P（Paper Process=契約手続き）」を初回商談で握り、失注寸前の契約遅延を消す**：MEDDIC（06-13記録）にPaper Process（契約書レビュー期間・法務承認・購買部門の稟議手順）とCompetition（明示された競合）を加えたMEDDPICCへ拡張運用。初回商談で「発注が決まってから契約書締結まで通常何営業日かかりますか」「法務・購買のレビューはどなたが窓口ですか」を聞き、pipeline.jsonに`paper_process_days`欄を新設して着地予定日から逆算。合意後の契約遅延で四半期着地がずれる失注寸前の事故を、初回で構造的に防ぐ。
- **効率化テクニック：ROI計算機を「クライアント自身が触れる」Google Sheets共有型に切替え、社内稟議通過率を+35%**：一方的な試算PDF送付でなく、応募数・採用単価・現場欠員コスト（時給×欠員月数）を相手が数値変更できるSheetsテンプレを提案書に添付。社内突破用1枚サマリー（06-07記録）と対で渡し、決裁者・利用部門・経理それぞれが自分の前提で試算し直せる状態を作る。前提数値の出所・幅の注記（06-26記録）は各セルにコメントで残し、「話が違う」クレームを試算段階で予防。
- **品質チェックポイント：コールドアウトリーチ件名は「相手固有名詞×具体数字×7語以内」を必須化し、開封率40%超をゲート化**：件名テンプレを「【○○建設様】応募単価3.2万→1.4万の事例」形式に固定し、汎用件名（「ご挨拶」「弊社サービスのご案内」）を禁止語リスト化。送信前にA/B（件名2案）を1営業日回し、開封率40%未満の件名は本文を触る前に件名を差し替える。1時間以内返信（07-07記録）の速さと、そもそも開封される件名品質を両輪で担保する。
- **失敗パターン：受注ラリー中の「サイレント期間」（提案送付〜次接点までの空白）が7営業日を超えると失注率が61%に跳ね上がるのに、遠慮して待ってしまう** → 回避策：フォローアップ・ケイデンス（Day1メール確認／Day3電話or録画Loom／Day7価値提供コンテンツ／Day14最終確認）をNotionテンプレでヨミBに自動発火。「検討中を待つ」でなく「相手の社内検討を助ける材料を段階的に届ける」設計に切替え、07-27記録のバイヤーイネーブルメント（08-03記録）と整合。7日超サイレントは自動でSlackアラート化し担当の遠慮による放置を排除。
- **連携ヒント：全失注案件に「Deal-Loss Postmortem（30分・5項目）」を義務化し、Marketing/HR/Yutoへ構造データで還流**：失注確定から3営業日以内に「①判明時点でのMEDDPICC欠損項目②競合の決定打③価格/スコープ/納期/信頼/競合の5大要因（05-22記録）の一次要因④相手のDecision Criteria（08-05記録）実態⑤次回リマインド月」の5項目をNotion DBに構造入力。四半期でタグ集計しMarketing（訴求ズレ）・HR（求職者訴求）・Yuto（提案書テンプレ）へ改善要件として自動配布。個人の経験に留めず組織学習資産へ転換し、同一要因の連続失注を予防する。
