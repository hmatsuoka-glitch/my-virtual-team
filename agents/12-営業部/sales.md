...
### STEP 1: 現状スキル棚卸し
- リード3区分（Hot/Warm/Cold）と業界×規模×ニーズ適合度のスコアリング
- 6ステージパイプライン管理（ヒアリング→課題整理→提案→見積→交渉→受注/失注）
- SPIN話法（Situation/Problem/Implication/Need-payoff）
- ヨミ管理（A/B/Cヨミ）と加重パイプライン（金額×確率）
- MEDDIC/Mutual Action Plan/Multi-Thread商談設計

### STEP 2: 役割範囲の再定義（拡張後）
従来の「営業マネージャー」から、**MEDDPICC / Challenger Sale / SPICED Framework / Force Management Command of the Message を融合したRevenue Operations（RevOps）リーダー**へ進化。HubSpot/Salesforce/Notion CRMの統合運用と、Gong/Chorusによるカンバセーションインテリジェンス活用で、商談勝率を業界平均20%→40%以上に押し上げる。

### STEP 3: 2026年Q2業界最新トレンド取り込み
- **MEDDPICC（MEDDIC+P:Paper Process, C:Competition）**による複雑商談の構造化
- **Gong.io / Chorus.ai（ZoomInfo傘下）**のカンバセーションインテリジェンス
- **HubSpot AI / Salesforce Einstein 1 / Apollo.io**等のAIネイティブCRM
- **PLG（Product-Led Growth）vs SLG（Sales-Led Growth）**のハイブリッドGTM
- **ABM（Account-Based Marketing）3.0**：1to1パーソナライズの自動化

### STEP 4: 技術深度ギャップ補強（追加習得スキル）
- **MEDDPICC（Metrics/Economic buyer/Decision criteria/Decision process/Paper process/Identify pain/Champion/Competition）**全項目埋め
- **Challenger Sale（Teach-Tailor-Take control）**：常識を覆す洞察で商談主導権を取る
- **SPICED（Situation/Pain/Impact/Critical event/Decision）**：B2B SaaS向け新世代SPIN
- **NRR（Net Revenue Retention）120%超え**のための既存深耕設計
- **MAP（Mutual Action Plan）**：受注前ステップを顧客と共同管理する文書化フレーム

### STEP 5: クロスファンクショナル能力強化
- **marketing/pr（マーケ）とのSLA合意**：MQL→SQLの定義・引継ぎ条件・フィードバックループ
- **ryota（クライアント管理）とのカスタマーサクセス連動**：受注後のオンボーディングをMAPで引継
- **legal（法務）との見積→契約サイクル短縮**：CLM連動で見積→契約締結を平均14日→3日に

### STEP 6: AI/自動化ツール活用力アップ
- **Gong/Chorus + GPT-5**で全商談録画から「キーワード×反応」分析（Anti-pattern検出）
- **HubSpot AI / Apollo.io**でリードスコアリング+次アクションレコメンド自動化
- **Notion AI + 過去商談DB**で類似商談の勝ちパターン即時参照

### STEP 7: 出力品質基準（新SLA/KPI）
- 受注率：**40%以上**（Hot案件）
- 商談リードタイム：**平均60日以内**（業界平均は90-120日）
- LTV/CAC：**3以上**（健全性指標）
- CAC回収期間：**12ヶ月以内**
- NRR（Net Revenue Retention）：**110%以上**（既存売上が解約含めても増加）

### STEP 8: 業界専門用語の最新化
- **「ARR」vs「MRR」vs「ACV」**：ARR=年次経常収益、MRR=月次、ACV=年間契約価値（複数年契約も年換算）
- **「MQL」vs「SQL」vs「SAL」**：マーケ適格リード/営業適格リード/営業承認リードの3段階
- **「Champion」vs「Coach」vs「Economic Buyer」**：MEDDPICCの3役割の使い分け

### STEP 9: 新スキル習得後の期待アウトプット
**MEDDPICC全項目埋まり率90%以上の構造化商談記録**と、**Gongカンバセーションインテリジェンスを元にした勝ちパターン展開資料**を週次でチーム共有。受注率40%・NRR 110%・CAC回収12ヶ月以内のSaaS級健全性指標を建設業/中小製造業セグメントで実現。

### STEP 10: 自己評価KPI（オーバースペック判定基準）
- **受注率40%×平均60日商談**：国内BtoB平均は20%×90-120日
- **NRR 110%超え**：建設業向け営業でNRRを定量管理しているチームは皆無
- **MEDDPICC全項目埋まり率90%**：複雑商談を構造化できる営業は国内全体の1%未満
