...
### 1. 現状スキル棚卸し（強み・ギャップ）
**強み**: BANT/MEDDIC使い分け、SPIN話法、ヨミ管理（A/B/Cヨミ）、Mutual Action Plan、Champion Enablement、失敗パターン蓄積（30件超）、5大失注要因分析、業種別テンプレ3種、警戒解除型初回返信、Otter→Notion自動化、加重パイプライン管理。
**ギャップ**: ①Revenue Operations（RevOps）統合視点が未整備、②シグナル・インテント・データ活用（6sense/Bombora等）未導入、③Account-Based Selling（ABS）の構造化未完、④AI営業コーチ（Gong/Chorus）による会話分析自動化未実装、⑤LTV/CAC/NRRをリアルタイム可視化するBIダッシュボード未構築、⑥建設業特化のIndustry Cloud活用ノウハウ蓄積途上。

### 2. 追加習得スキル（2026必須6選）
1. **RevOps基盤設計**：Sales/Marketing/CSのデータを単一CRMに統合し、Lead→MQL→SQL→受注→更新の全ステージを単一ファネルで可視化（HubSpot Operations Hub / Salesforce Revenue Cloud）
2. **Conversation Intelligence**：Gong.io / Chorus.ai / Otter.ai for Sales導入で全商談を録音→AI解析→「トーク比率・キーワード頻度・競合言及・次アクション抽出」を自動レポート化
3. **Intent Data活用**：6sense / Bombora / ZoomInfo Intent でWeb上のサーチ行動（「採用支援」「TikTok広告」検索）を持つ企業を先回り特定し、Cold→Hotへ昇格
4. **ABS（Account-Based Selling）**：ターゲットアカウントリストを業種×規模×シグナルでスコアリングし、マルチスレッド接点（窓口/決裁者/利用部門）を体系的に作る
5. **Sales AI Agent運用**：Apollo.io 2026 / Outreach AI / Salesloft Rhythm でリサーチ→初回メール→フォローを自動化、人間は提案・クロージングに集中
6. **動的価値提案（Value Engineering）**：Dock.us / Mindtickle / Bigtincan で提案資料・ROI計算を動的生成、商談中にリアルタイム数値カスタマイズ
7. **Forecast Science**：Clari / BoostUp / Aviso で過去パイプラインデータからML予測モデルを構築、ヨミ精度をBI化（誤差±10%以下）
8. **建設業Industry Cloud連携**：どっと原価・SPIDERPLUS・ANDPADと連携した提案、現場DXトレンド（i-Construction 2.0 / 国交省BIM/CIM 2025義務化）の常時アップデート

### 3. 推奨ツール/フレームワーク（実名10選）
| カテゴリ | ツール | 用途 |
|---|---|---|
| CRM/RevOps | Salesforce Sales Cloud / HubSpot Sales Hub Enterprise | 統合パイプライン |
| 会話分析 | Gong.io / Chorus.ai | 商談録音→AI解析 |
| インテント | 6sense / Bombora | 購買シグナル検知 |
| 営業自動化 | Apollo.io 2026 / Outreach / Salesloft | アウトバウンド自動化 |
| 提案・電子契約 | Dock.us / PandaDoc / クラウドサイン | デジタルセールスルーム |
| Forecast | Clari / BoostUp | ML売上予測 |
| Enablement | Mindtickle / Highspot | コンテンツ・トレーニング |
| データ可視化 | Looker / Tableau / Power BI | KPIダッシュボード |
| コラボ | Slack Sales Elevate / Notion | 社内連携 |
| 建設DX連携 | どっと原価 / ANDPAD / SPIDERPLUS | 業界特化提案 |

### 4. KPI/評価指標（数値付き）
- **受注率**: 40%以上（現状目標維持）→ 2026年末50%へ
- **LTV/CAC比**: ≧3.0（目標4.0）
- **CAC回収期間**: ≦12ヶ月（目標9ヶ月）
- **NRR（売上継続率）**: 110%以上（既存深耕＋アップセル）
- **平均商談期間**: 60日以内（目標45日）
- **加重パイプライン予測誤差**: ±10%以内
- **初回返信時間**: 1時間以内100%
- **マルチスレッド率**: 商談化案件の80%以上で3接点確保
- **Champion育成率**: 受注案件の100%でChampion特定済み

### 5. 90日成長ロードマップ
**Day 1-30 (基盤構築)**: Gong.io導入→全商談録音化、HubSpot/SalesforceでMQL→SQL→受注ファネル統合、Notion DB+加重パイプライン自動計算実装、業種別テンプレ3種をDock.us上でデジタルセールスルーム化。
**Day 31-60 (自動化拡張)**: Apollo.io 2026でアウトバウンド自動化（週50社新規アプローチ）、6sense Intent Data連携でHot判定自動化、Clari導入でML売上予測、Mutual Action Plan標準テンプレ全案件適用。
**Day 61-90 (高度化)**: ABS体制で重点20社の決裁者マッピング完了、Champion育成プログラム稼働、NRR110%達成のための四半期定例レビュー設計、建設業特化Industry Cloud連携で他社差別化提案を3案件成約。

### 6. 出力品質向上チェックリスト（送付前必須）
- [ ] 提案書PDF全文grepで前クライアント名・他社金額残存ゼロ
- [ ] 見積3点検算（縦計・横計・本文金額一致）
- [ ] ROI試算の前提出所注記（ヒアリング値 / 推定値の区別）
- [ ] Mutual Action Plan添付（次回日時・誰が・何を・いつまで）
- [ ] 社内説明用1枚サマリー添付
- [ ] 失敗事例ブロック1ページ必須記載
- [ ] Champion明記・マルチスレッド3接点確保
- [ ] Legal/Finance/PM事前確認済み（口頭確約禁止）
- [ ] ヨミ区分（A/B/C）と根拠1行記載
- [ ] 次回リマインダーNotion DB登録済み

### 7. 他エージェントとのコラボ強化案
- **Marketing**: 6sense Intent Data連携→MQL定義統一→Sales Accepted Lead（SAL）合意フローをSlackで自動化
- **Finance（gen系）**: 見積依頼に「締め日・支払サイト・外注有無・建設業特約」必須添付、入金予測精度向上
- **PM (kai)**: クロージング前実現可能性確認API化、Slack `/feasibility-check` で即時回答
- **HR (連携先)**: 建設業採用支援案件で求職者目線レポートを提案書に自動差し込み
- **nori (Legal)**: 口頭特約の即時Slack共有→受注前リーガル判定で契約段階の覆り防止
- **資料作成部 yuto**: Dock.us上の業種別テンプレを共同編集、提案書納品リードタイム2営業日固定
- **sora (QA)**: 送付前チェックリスト10項目をsora QAゲートに組込み、機械的GO/NO-GO判定
