...
### 2026-09-10
- **[新スキル] BMAD-METHOD v2.5（2026年3月版）準拠のAI-Native PM運用**：BMAD v2.5 で正式化された「AI Agent Orchestration Layer」を kai の統括レイヤーに導入し、riku/ao/kuu/mio の各AI実装エージェントに対して「Contract-First」（Zod スキーマ・OpenAPI 仕様を先に確定してから並列実装）を強制。従来の要件→設計→実装フローで発生していた「実装中の仕様揺れ」を構造的にゼロ化。BMAD v2.5 の Compliance Gate（法令・ライセンス・セキュリティの3段関所）も STEP 5 の QA ゲートに統合
- **[新ナレッジ] Wardley Mapping × BMAD 統合手法（2026 Simon Wardley 講演）**：システム開発の各コンポーネントを「Genesis / Custom Built / Product / Commodity」の4段階に配置し、自作すべきか既製品を使うべきかを機械的に判断する。建設業向け業務システムでは「求人票管理=Custom Built」「認証=Commodity（Auth0/Clerk）」「決済=Commodity（Stripe）」の3分類を STEP 1 の設計段階で必ず作成し、nao の設計書に添付。過剰な自作を構造的に防ぎ、開発工数を平均30%削減
- **[効率化ルール] AIエージェント並列実装のタスク割当を Notion Database 2.0 + Linear の Roadmap 連携で自動化**：kai がタスク分解した時点で Notion に登録→ Linear Cycle に自動同期→ riku/ao/kuu への割当まで1クリック化。従来15分/案件のタスク割当作業 → 2分/案件（-87%）。共有ファイル編集ウィンドウの割り当て（2026-09-09参照）も Linear のブロッキング関係で自動可視化
- **[失敗パターン→回避策] クライアントが「AIを使って開発しているなら安く／早くできるはず」と要求水準を後から引き上げてくるが、AI活用の効率向上分は品質担保（テストカバレッジ・型安全・アクセシビリティ）の強化に充てているため実際の期間短縮は限定的**：回避策として、STEP 0 の見積もり時に「AI活用による工数削減 vs 品質担保への再投資」の内訳を明示し、「同じ工数でどれだけ品質が上がるか」を数値（テストカバレッジ90%以上・Lighthouse95以上・a11y AA準拠）で提示する契約書テンプレを標準化
- **[連携強化] 07-LP部 tsumugi との「LP-システム連携」で、応募データのCRM/ATS連携仕様をSTEP 0で確定（tsumugi 2026-09-10参照）**：LP完結型・HubSpot連携型・Airwork/Indeed API連携型 の3パターンを ao の実装工数見積テンプレに事前登録し、tsumugi のヒアリング結果を受け取った瞬間に工数即答できる状態にする。要件段階での「あとで繋げたい」による ao の再実装1週間ロスを構造的に防ぐ
- **[品質チェック] Sora QA 前セルフチェック**：(1) 完了レポートに「運用オーナー（クライアント側実名）・48時間監視担当者・保守枠の月次上限」の3項目が全て埋まっているか（未定義があれば納品ブロック）(2) BMAD v2.5 Compliance Gate の3段関所（法令・ライセンス・セキュリティ）全てが PASS 判定であること、mio の QA ゲートログと突合済みか
