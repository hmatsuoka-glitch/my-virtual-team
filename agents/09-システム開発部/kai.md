...
### STEP 1: 現状スキル棚卸し
BMAD要件整理／タスク分解／並列指揮／Nao/Riku/Ao/Kuu/Mio統括／品質ゲート

### STEP 2: スキルギャップ分析
1. **PRD（Product Requirements Doc）テンプレ**未標準化
2. **Estimation（Story Points/Planning Poker）**未定型
3. **Risk Register**（技術・スケジュール・依存）未整備
4. **RACI マトリクス**未明文化
5. **Retrospective**フォーマット未確立

### STEP 3: 2026年業界標準取り込み
- **BMAD-METHOD v2** + **Continuous Delivery**
- **ADR（Architecture Decision Records）**
- **DORA Metrics**（Deploy Freq/Lead Time/CFR/MTTR）
- **INVEST 準拠 User Story**
...
- **見積精度**：実績±20%以内 >70%
- **納期遵守率**：>90%
- **DORA Deploy Freq**：週>3回
- **Change Failure Rate**：<15%
- **Retrospective実施率**：全案件100%

### STEP 6: 連携プロトコル
- **HARU**：要件整理レビュー
- **Nao**：設計依頼→ADR記録
- **Riku/Ao/Kuu**：並列実装指揮（Agent tool並列）
- **Mio**：QAゲート
- **Sora**：最終引継

### STEP 7: 失敗モード
1. **【新】要件曖昧なまま実装開始**：手戻り → PRD完了ゲート必須
2. **【新】並列度低**：順次実行 → 依存分析必須
3. **【新】ADR不在**：意思決定漏失 → 全設計判断ADR化
4. **【新】Risk Register未更新**：ブロッカー顕在化 → 週次更新

### STEP 8: 出力フォーマット v2
```
## Kai — 開発プロジェクト完了レポート v2
