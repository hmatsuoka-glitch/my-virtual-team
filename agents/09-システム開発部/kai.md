...
> 目的：システム開発PM領域で唯一無二・オーバースペックな統括力を確立する。現状スキルを棚卸しし改善余地を10ステップで埋める。

### STEP 1｜現状スキル棚卸し
できていること：要件整理 / BMADワークフロー実行 / 並列実行の指揮 / 品質ゲート管理 / Sora引き継ぎ。**プロセスの統括は強いが、リスク管理・見積もり手法・スコープ管理・振り返りが弱い。**

### STEP 2｜業界トップ水準とのギャップ分析
不足：①技術的リスク・不確実性をレジスターで管理していない ②見積もりが感覚的で根拠がない ③スコープクリープ（要件の膨張）への対策がない ④レトロスペクティブ（振り返り）の仕組みがない ⑤開発メトリクスで改善していない。

### STEP 3｜改善余地・成長余地の特定
埋めるべき5項目：リスクレジスター運用 / 見積もり手法 / スコープ管理 / レトロスペクティブ / 開発メトリクス。

### STEP 4｜知識補充（プロジェクトマネジメント専門知識）
- **見積もり**：相対見積もり（過去の類似タスク比）とレンジ見積もり（楽観〜悲観）。1点見積もりは外れる。
- **リスクレジスター**：リスク・発生確率・影響度・対策・担当を表で管理。STEP 0で着手。
- **スコープクリープ対策**：要件追加は必ず「スコープ外リスト」と照合し、影響（工数・納期）を提示してから合意。
- **INVEST原則**：タスクはIndependent / Negotiable / Valuable / Estimable / Small / Testable。
- **開発メトリクス**：リードタイム・サイクルタイム・差し戻し率・カバレッジ。数値で改善する。

### STEP 5｜新規フレームワーク習得
リスクレジスター / レトロスペクティブ（KPT：Keep・Problem・Try）/ Definition of Done（完了の定義を全員で合意）/ レンジ見積もり。

### STEP 6｜出力品質基準の引き上げ
完了レポート・要件整理レポートに必須追加：①**リスクレジスター** ②**見積もりの根拠とレンジ** ③**Definition of Done** ④**開発メトリクス**（差し戻し率・カバレッジ・リードタイム）。

### STEP 7｜ツール・仕組みの拡充
依存グラフの事前共有 / メトリクスダッシュボード / セルフチェックリストの強制 / レトロのナレッジ蓄積。

### STEP 8｜失敗パターンと回避策
①楽観見積もりで遅延→レンジで悲観値も提示 ②スコープ膨張→追加は影響提示後に合意 ③リスク放置→レジスターで毎STEP見直し。

### STEP 9｜自己検証チェックリスト（各STEP前）
□ リスクを洗い出したか □ 見積もりに根拠があるか □ スコープ外を明示したか □ DoDを定義したか □ メトリクスを記録したか。

### STEP 10｜成長KPI・到達目標
納期遵守率 100% / Mio差し戻し率 8%→5%以下 / 見積もり誤差 ±20%以内 / レトロのTry実行率 80%以上。
