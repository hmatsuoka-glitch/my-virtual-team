...
### 2026-09-21
- **2026年最新トレンド：BMAD-METHOD v3 の「Story Compression」で要件と実装の距離を短縮**：BMAD v3 では PRD → Architecture → Story の 3 段階に「Story Compression」が追加され、Nao の設計書を機械可読な YAML front matter 付き Story に自動変換する。riku/ao/kuu は Story を Cursor Rules に直接注入して実装、要件の意図を実装者が誤解する事故が体感 60% 減少。kai は Story Compression 段階で「Given-When-Then が全機能に付いているか」を最終ゲートとして確認、機能定義の抜け漏れを実装前に潰す。
- **2026年最新トレンド：Vercel v0 と Cursor Composer の並列生成でプロトタイプ 4 時間化**：STEP 3 前半のプロトタイプフェーズで、v0 に画面設計を渡して UI を生成、Cursor Composer に Story を渡して API を生成、両者を Ao が結線する 4 時間ワークフロー。従来 3 日かかっていた「動くプロトタイプ」が半日、クライアント STEP 0 の指差しレビュー結果を即日で反映可能、意思決定サイクルが週単位 → 日単位に短縮。
- **失敗パターン：BMAD の Architecture ドキュメントを Nao 単独で完成させてから riku/ao に共有** → 回避策：Architecture ドキュメントを Nao が起草した時点で kai が「30分レビュー会」を強制招集、riku/ao/kuu の 3 名が「実装できる粒度か」を検証。設計完成後の手戻り 3 日 → 起草段階の指摘 30 分（理由：Nao の設計は理論的に完璧でも、実装側の暗黙前提と噛み合わないケースが実案件で頻発、早期擦り合わせで工数の 20% 削減）。
- **失敗パターン：クライアント承認プロセスを「メール返信のみ」で運用してタイムラインが崩壊** → 回避策：STEP 0 契約時点で「承認は Google Docs のコメント + Loom 動画確認 + 最終口頭承認」の 3 段階を契約書に明記。特に決裁者が現場常駐の案件では、週 1 回 30 分のカレンダー固定枠で「Loom を一緒に見る同席レビュー」を実施（理由：2026-09-13 の検収カレンダー確保と同じ論理、承認は先方の善意でなく確保された時間でしか進まない）。
- **品質チェックポイント：checklists/architect-checklist.md + checklists/qa-gate.md の 2 段関所を kai が両方立会**：Nao が architect-checklist を通過した後の設計、mio が qa-gate を通過した後の実装、両方 kai が最終レビュー時に「なぜこの設計/実装で OK か」を 3 行で言語化。無言の承認をなくし、責任分界を明示。sora QA 前に kai レベルで潰せる論点を最大化。
- **品質チェックポイント：Story ごとの「実装後 KPI 定義」を PRD に組み込む**：機能単位で「実装後にどの KPI で成功判定するか」を PRD の Story セクションに書き、実装完了時に Akari のレポート項目へ自動反映。Nao の設計と Akari の運用が分断せず、「作った機能が使われたか」を kai が週次で監視、開発だけで完結しない「事業成果につながる開発」ポジション。
- **効率化テクニック：Linear + Cursor Rules 統合で Story → Task 分解を自動化**：Nao の Story を Linear に投稿すると、Linear の webhook が Cursor Rules を自動生成して各実装者のリポジトリに配信。riku/ao/kuu が Cursor を開いた瞬間から「今日やるべき Story のコンテキスト」が注入されており、着手コスト 30 分 → 3 分。並列実行のスループット 3 倍化。
- **nori 連携の小ヒント：STEP 0 契約時点で「開発対象システムの法的位置付け」を nori 事前照会**：応募者管理 SaaS なら個人情報保護法・APPI、決済連携なら資金決済法、マッチング機能なら媒介業該当性を STEP 0 で nori に照会。実装完了後に「そもそも法規制で使えない」判明する最悪ケースを事前予防、nori は要件定義段階から関所として組込み、システム開発の nori 通過率 100%。
