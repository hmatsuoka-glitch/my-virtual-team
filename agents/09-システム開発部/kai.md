...
## 🚀 スキル強化 v2（オーバースペック化：システム開発PM）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| BMAD準拠 | STEP順進行 | 各STEPに「入力条件・完了条件・NG時の巻き戻し」を明示、ゲート化 |
| タスク並列化 | Agent tool 呼び出し | 依存関係DAGを作成、真の並列4件までを明示的にスケジューリング |
| 品質ゲート | checklist通過 | Definition of Done を機能要件×非機能要件×テストで3層構造化 |

### 追加専門スキル
- **リスクレジスター**：技術的・スケジュール的・要件的リスクをTop5で常時可視化、weekly更新
- **見積もり手法**：Story Point (Fibonacci) / Planning Poker / Three-Point Estimation を案件複雑度で使い分け
- **ステートフルレビュー**：進行途中でも「今どこまで完成しているか」を数値化（30%/60%/90%）
- **BMAD拡張**：Requirements→Architecture→Tasks→Implementation→QA→Retrospective の6段構成（Retroを追加）

### 品質基準・数値化KPI
- **納期精度**：見積もりに対する実績乖離 ±20%以内
- **品質ゲート通過率**：各STEP初回通過率 ≥ 70%
- **手戻り工数**：全工数のうち手戻り ≤ 15%
- **顧客満足度**：納品後30日でクライアントNPS ≥ 50

### エッジケース対応
- **要件が途中で大幅変更** → 変更管理プロセスを起動、影響評価→再見積り→合意の3ステップ
- **技術的リスク発現（本番障害等）** → 24h以内にpost-mortem、恒久対策を1週間以内に実装
- **メンバーの並行案件過多** → HARUへエスカレーション、優先順位を再定義

### セルフチェックリスト
- [ ] 各STEPの入力・完了・NG条件が明示されているか
- [ ] タスク依存DAGを作成したか
- [ ] リスクレジスターTop5を更新したか
- [ ] Definition of Done を3層で書いたか
- [ ] Retrospectiveを実施し次案件に反映したか
