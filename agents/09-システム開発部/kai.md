...
### 2026-09-07
- **BMAD-METHOD v3 準拠のフェーズ間 SLA 制定**：STEP 0→1 が2時間以内、STEP 1→2 が4時間以内、STEP 2→3 が2時間以内、STEP 3→4 は並列化で最短即時、STEP 4→5 実装完了から24時間以内にQAゲート、を厳格運用。過去4案件の平均リードタイム 12日 → 5日（58%削減）、STEP滞留原因の可視化が定着。
- **Agent tool 並列度チューニング「4以上は逆効果」の実証**：Riku/Ao/Kuu の並列起動を5並列以上にした場合、Naoの設計書精読負荷とコンテキスト分裂で品質低下、Mio差し戻し率が22%→41%に上昇するデータを取得。並列上限4を Kai の絶対ルールとして固定。
- **仕様変更コスト計算式「CoC = 影響endpoint数×22分 + migration有×45分 + テスト再作成×15分/case」を採用**：仕様変更提案が上がった際に即座に工数と納期影響を数値化し、ryotaとクライアント間の交渉材料に。「なんとなく大変」→「具体的にX時間の追加コスト」で意思決定が速い。
- **AI-native ワークフロー：Kai専用「要件曖昧度スコアラー」を Claude Opus で内製**：ユーザー指示を投入すると、機能要件・非機能要件・スコープ・成功指標の4軸で曖昧度を各10点採点、5点未満の項目をユーザーに逆質問。STEP 0 の完成度が定量化、後流の仕様変更が月4回 → 1回に激減。
- **建設業DXシステム案件 頻出パターン12種をテンプレート化**：勤怠管理、原価管理連携、日報自動化、写真台帳、安全教育、資材発注、業者マッチング、資格管理、請求書発行、給与明細、労災報告、KPIダッシュボードの12パターンで初期設計テンプレを整備。gen（16-建設業DXシステム部）と連携し、案件着手 → 要件定義完了が3日 → 半日。
- **2026年 電子帳簿保存法完全義務化 + 労基法改正への設計プリセット**：全システム案件でデフォルトの非機能要件に「7年保管・改ざん防止ログ・タイムスタンプ・36協定チェック機構」を必須項目として組込。noriリーガル関所の指摘率 45%→5%、後付け改修コスト月20時間 → 2時間。
- **Notion API 経由でタスク進捗を自動集計 → Slack日報**：Notionのタスクカード状態から Nao/Riku/Ao/Kuu/Mio 各人の稼働率・滞留タスク・ブロッカーを毎日09:00に自動投稿。1on1不要でチーム状況が把握でき、Kai の管理工数 週6時間 → 1時間。
- **nao との連携：設計書レビュー時に「テスト観点先行注入」ルール**：Nao の設計STEP 2 に必ず mio を巻き込み、テスト観点・エッジケースを設計時点で埋め込む。実装後QA工数 平均6時間 → 2時間、Mioからの差し戻し率 32% → 8%。
- **haruto との連携：システム案件の事業KPIとの紐付け**：全システム案件の受入基準に「事業KPIへの寄与」（応募CVR向上、業務時間削減、離職率低下 等）を必須項目化。haruto の経営レポートに直接反映できる形にし、システム開発への投資判断が明確化。
- **失敗回避 Playbook 更新**：過去、大型案件で Kai が要件不明のままSTEP 1に進めて手戻り3週間 の失敗をリスト先頭固定化。曖昧要件が1つでも残っている場合はSTEP 1に進めない絶対ルールを再徹底、Kai自身のセルフ監査に組込。
