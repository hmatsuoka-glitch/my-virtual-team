...
| 領域 | 到達済みの水準 | 判定 |
|---|---|---|
| CI/CD パイプライン | 4 段階品質ゲート・reusable workflows・concurrency キャンセル・turbo/pnpm/docker 二段キャッシュ・arm64 runner | ✅ 業界上位 |
| 環境変数・シークレット | 3 環境厳密分離・`vercel env ls \| diff` 日次 CI・`rotate-secret.sh` 併存ローテ・gitleaks 三重ゲート・`environment: production` 隔離 | ✅ 業界上位 |
| デプロイ戦略 | Atomic Deploy＋Edge Middleware canary 10%／5 分監視・stable タグ自動付与・`vercel rollback` 30 秒復帰・Skew Protection | ✅ 業界上位 |
| Observability | OTel＋Grafana Cloud 3 軸・heartbeat・合成監視・高基数ラベル禁止・アラート 3 段階＋月次閾値再校正 | ✅ 業界上位 |
| SLO・DORA | SLA 99.5%／SLO 99.9% 二段構え・エラーバジェット運用・DORA 4 指標の週次自動計測 | ✅ 業界上位 |
| DR・バックアップ | RTO/RPO 数値合意・PITR/WAL・四半期リストア訓練・別リージョン保管・FMEA | ⚠️ 「消されない」耐性が未定義 |
| コスト最適化 | Spend Management 50/80% 通知・retention 階層化・preview 自動 teardown・Fluid Compute Active CPU | ⚠️ 総額のみ。案件別に分解できない |
| IaC | Terraform module 化・`terraform plan -detailed-exitcode` 週次ドリフト検知・SLO.yaml からの設定自動生成 | ✅ 業界上位 |
| セキュリティ | CSP/HSTS ヘッダー機械ゲート・PII マスキング・最小権限原則・attestation 検討 | ⚠️ 人の権限ライフサイクルと本番アクセス統制が空白 |
| 負荷・キャパシティ | QPS/同時実行/バックプレッシャの用語整理・`max_connections` 逆算 | ❌ 実測（負荷試験）が存在しない |
| 組織耐性 | Runbook・Statuspage テンプレ・incident-check bot | ❌ Kuu 単独依存（バス係数 1）が未解消 |

**総括**：単一アプリを「落とさず・速く・安く」出す技術は完成域。空白は（a）実測で裏を取る系（負荷試験・訓練）、（b）人と権限のガバナンス、（c）クライアント複数社を 1 基盤で捌く多テナント運用、（d）事業数値へのインフラ翻訳。

### 到達すべきベンチマーク

| ベンチマーク領域 | 目標水準（国内トップ 1% ライン） |
|---|---|
| IaC | 本番環境の 100% がコード由来。ドリフト検知は週次でなく PR 単位＋日次、手動変更の是正 24h |
| GitHub Actions 最適化 | PR フィードバック中央値 3 分以内、ランナー費の月次単価を計測し前月比監視 |
| シークレット管理 | 常時本番 write 権限保持者 0 名、全本番オペに監査証跡、権限棚卸し四半期 |
| SLO・エラーバジェット | 全体 SLO に加え **テナント別 SLO** を提示、バジェット残 25% 未満でリリース凍結を自動発火 |
| Observability | 全 span に低基数 `tenant.id`、テナント別 p95 と全体 p95 の乖離を常時監視 |
| DR・RPO/RTO | RPO 5 分・RTO 60 分を実測で担保。バックアップは **イミュータブル（WORM）** で削除・暗号化攻撃に耐える |
| コスト最適化 | 総額でなく **案件別原価・1 応募あたり原価** に分解し、Haruto の粗利表に直結 |
| 組織耐性 | バス係数 2 以上。四半期ゲームデーで Kuu 不在時の復旧を実演済み |

### 特定されたスキルギャップと優先度

| # | ギャップ | 事業インパクト | 即戦力度 | 優先度 |
|---|---|---|---|---|
| 1 | マルチテナント分離・テナント別 SLO（クライアント 7 社が同一基盤を共有） | 高（1 社起因の全社停止＝全案件の信頼喪失） | 高 | **S** |
| 2 | 本番アクセス統制（Break-glass）と IAM ライフサイクル棚卸し | 高（業務委託の権限剥奪漏れは情報漏洩に直結） | 高 | **S** |
| 3 | 負荷試験の常設化とキャパシティプランニング（媒体掲載開始日のスパイク） | 高（応募が最も来る日に落ちる＝最悪の事故） | 中 | **S** |
| 4 | インフラ Unit Economics（案件別原価配賦・1 応募あたり原価） | 高（Ryota の見積根拠・Haruto の粗利管理に直結） | 高 | **S** |
| 5 | WAF・Bot 対策・応募フォームのレート制限 | 高（bot 応募はクライアント信頼を直撃＋従量課金膨張） | 高 | **S** |
| 6 | ゲームデー／オンコール設計（バス係数 1 の解消） | 高（Kuu 休暇時に復旧不能な構造リスク） | 中 | **A** |
| 7 | イミュータブルバックアップ（3-2-1-1-0）と SBOM 供給網継続検証 | 中（発生確率は低いが発生時は事業停止級） | 中 | **A** |
| 8 | プラットフォーム Exit 戦略・可搬性スコア（Vercel 依存度の定量化） | 中（値上げ・規約変更時の交渉カード） | 低 | B |
| 9 | データレジデンシー（保管リージョンの契約適合性の機械検証） | 中（nori の越境移転レビューの自動化） | 低 | B |

---

### 🔧 新規習得スキル

#### 1. マルチテナント分離とテナント別 SLO（ノイジーネイバー封じ込め）

**なぜ必要か**：
サクバズはクライアント 7 社が同一の採用管理基盤を共有する構造。1 社の CSV 一括取込や大量応募スパイクが Serverless の同時実行枠と DB コネクションを食い潰すと、無関係の 6 社まで p95 が劣化する（ノイジーネイバー）。現行の監視は「全体 p95」しか持たないため、1 社だけが 3 秒かかっていても全体平均に埋もれて検知できない。クライアント別 SLA を提示している以上、計測単位が全体しかないのは根本的な欠落。

**具体的手法**：
1. OTel の全 span に `tenant.id` 属性を付与（クライアント数 7〜20 の低基数なので、高基数ラベル禁止則に抵触しない）。Grafana でテナント別 p95・エラー率・Function 実行回数をパネル化。
2. Upstash Ratelimit（sliding window）でテナント別レート制限。IP 単位でなくテナント単位に置くのが要点（1 社の社内 NAT からの正常アクセスを IP 制限で誤爆させないため）。
3. 重量バッチは Inngest の `concurrency.key = tenantId` で **テナントごとに** 同時実行を絞る。グローバル上限だけだと 1 社がキューを占有する。
4. DB は `tenant_id` に必ず複合インデックス先頭を置き、Postgres RLS（Row Level Security）でアプリのバグでも越境参照が起きない構造にする。
5. 単一テナントの占有率が閾値を超えたら Vercel プロジェクトごと分離（`let-{client}-app`）して物理隔離。

**判断基準・数値ライン**：
- テナント別 p95 が全体 p95 の **2.0 倍**を超えた状態が 15 分継続 → 隔離検討アラート（P2）
- 単一テナントが総 Function 実行回数の **40%** 超 → 専用プロジェクトへ分離を Kai へ起案
- テナント別 SLO：可用性 99.9%／p95 500ms。**全体が緑でもテナント 1 社が赤なら SLO 違反として扱う**
- テナント別レート上限：一般 API 600 req/min、書き込み系 60 req/min、一括取込 同時 2 ジョブ
- DB コネクション配分：`max_connections × 0.7 ÷ テナント数` を 1 社あたりの上限とし PgBouncer のプール分割

**実務テンプレート**：
```ts
// middleware.ts — テナント別レート制限（Upstash）
import { Ratelimit } from "@upstash/ratelimit";
const limiters = {
  read:  new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(600, "1 m"), prefix: "rl:read"  }),
...
const { success, remaining } = await limiters[kind].limit(tenantId);
if (!success) return new Response("Too Many Requests", { status: 429, headers: { "Retry-After": "60" } });
```
```yaml
# inngest: テナント単位で同時実行を封じ込める
concurrency:
  - key: "event.data.tenantId"   # テナントごとに 2 並列まで
    limit: 2
  - limit: 10                    # 全体上限
```

**期待効果**：
1 社起因の全社巻き込み障害をゼロ化。クライアント別 SLA を「全体平均」でなく実データで提示でき、Akari／Ryota のレポートが「御社の稼働率」を語れるようになる。

---

#### 2. 本番アクセス統制（Break-glass）と IAM ライフサイクル棚卸し

**なぜ必要か**：
シークレットの「保管」は完成しているが、**人の権限**が統制されていない。本番 DB への直接接続・Vercel トークン・GitHub Owner 権限が常時付与されたままで、誰がいつ何を実行したかの証跡がない。業務委託メンバーの契約終了時に剥奪が漏れれば、それだけで個人情報保護法上の安全管理措置違反になる。nori のセキュリティ質問票（クライアントのセキュリティチェックシート）でも必ず問われる項目で、答えられないと大手案件の与信で落ちる。

**具体的手法**：
1. **平時ゼロ権限**：本番 DB は read-only ロールのみ常時付与、write ロールは誰にも常時付与しない。
2. **Break-glass 手順**：Slack `/breakglass <理由> <想定所要時間>` → Kai が承認 → 一時クレデンシャル（TTL 60 分）発行 → セッション記録開始 → 期限で自動失効 → 事後レビュー Issue を自動起票（Kuu が 24 時間以内に「何を実行し、なぜ必要だったか」を記入）。
3. **四半期の権限棚卸し**：GitHub org / Vercel team / Sentry / Notion / DNS レジストラの 5 サービスをスクリプトで CSV 出力し、人事名簿（在籍者・稼働中の業務委託）と diff。差分は 5 営業日以内に是正。
4. **オフボーディング SLA**：退職・契約終了の当日中に全サービス剥奪、翌営業日に剥奪完了スクショを nori へ提出。
5. GitHub Actions の `permissions:` は `contents: read` を既定とし、必要なジョブだけ昇格（デフォルト write の全ジョブ付与を禁止）。

**判断基準・数値ライン**：
- 常時本番 write 権限保持者：**0 名**
- Break-glass 一時クレデンシャル TTL：**60 分**（延長は再申請）／月間発動回数 **3 回超**なら「恒常運用を自動化すべき兆候」として棚卸し
- 未使用 **90 日**のアカウント／トークンは自動無効化
- 権限棚卸し：四半期 1 回、差分是正 **5 営業日以内**
- オフボーディング剥奪：**契約終了当日（24 時間以内）**
- 全 break-glass セッションの記録保持：**13 か月**

**実務テンプレート**：
```bash
#!/usr/bin/env bash
# scripts/access-audit.sh — 四半期の権限棚卸し（5 サービス横断エクスポート）
set -euo pipefail
OUT="audit-$(date +%Y%m%d)"; mkdir -p "$OUT"
gh api "orgs/$ORG/members"  --paginate -q '.[].login'                > "$OUT/github.txt"
gh api "orgs/$ORG/outside_collaborators" --paginate -q '.[].login'  >> "$OUT/github.txt"
vercel teams ls --json | jq -r '.members[] | "\(.email),\(.role)"'   > "$OUT/vercel.csv"
sentry-cli organizations members list --json | jq -r '.[].email'     > "$OUT/sentry.txt"
# 人事名簿（roster.csv）との差分＝剥奪漏れ候補
comm -23 <(sort -u "$OUT"/*.txt) <(cut -d, -f1 roster.csv | sort -u) > "$OUT/ORPHANS.txt"
[ -s "$OUT/ORPHANS.txt" ] && echo "::error::剥奪漏れ候補あり" && cat "$OUT/ORPHANS.txt"
```

**期待効果**：
内部不正・剥奪漏れによる漏洩リスクを構造的に排除。クライアントのセキュリティチェックシートに証跡付きで即答でき、大手案件の与信通過率が上がる。

---

#### 3. 負荷試験の常設化とキャパシティプランニング

**なぜ必要か**：
`max_connections ÷ 想定同時関数数` の余裕計算は行っているが、その「想定」が実測に基づいていない。採用支援事業はスパイクの発生日が**事前に分かる**という稀有な特性がある（Indeed／エアワーク掲載開始日、会社説明会の告知直後、TVCM・折込チラシの投下日）。読めるスパイクで落ちるのは、インフラ担当として最も言い訳の効かない事故。

**具体的手法**：
1. k6 を GitHub Actions から staging に対して実行。3 シナリオを固定化：
   - **Smoke**（1 VU／1 分）：全 PR で実行。壊れていないことの確認。
   - **Load**（想定ピークの 1.0 倍／10 分）：本番昇格前ゲート。
   - **Spike**（ピークの 3.0 倍を 30 秒で立ち上げ）：週次＋掲載開始日の 3 営業日前に必ず実行。
2. **想定ピークの算出根拠**：過去 30 日の応募数の日次最大 ÷ 有効受付時間（実測ピーク帯 21〜23 時に 40% が集中する前提で圧縮）× 媒体掲載開始日の実績倍率。LET の実績では掲載開始日は平常日の **2.5〜4.0 倍**。安全側に 4.0 倍を採用。
3. ボトルネックの特定順序を固定：① Function 同時実行上限 → ② DB コネクション → ③ 外部 API のレート制限 → ④ Edge Middleware の処理時間。
4. 結果を `capacity.md` に記録し、Nao の `SLO.yaml` へフィードバック。

**判断基準・数値ライン**：
- 合格閾値：p95 < **500ms**／エラー率 < **0.5%**／DB コネクション使用率 < **max の 70%**／Function 同時実行 < 上限の **60%**
- Spike シナリオで 5xx が **1 件でも**出たら本番昇格を保留
- 掲載開始日の **3 営業日前**までに Spike 試験 PASS がないクライアントは、Kai 経由で掲載日調整を起案
- 負荷試験の実施頻度：Smoke=毎 PR／Load=毎リリース／Spike=週次＋イベント前

**実務テンプレート**：
```js
// tests/load/spike.js — 応募フォーム投稿のスパイク試験
import http from "k6/http"; import { check } from "k6";
export const options = {
  scenarios: { spike: { executor: "ramping-arrival-rate", startRate: 5, timeUnit: "1s",
    preAllocatedVUs: 200, stages: [
      { target: 5,  duration: "1m" },   // 平常
      { target: 60, duration: "30s" },  // ピーク 4 倍へ急立ち上げ（掲載開始日相当）
...
export default function () {
  const res = http.post(`${__ENV.BASE_URL}/api/applications`,
    JSON.stringify({ tenantId: __ENV.TENANT_ID, name: "負荷試験", email: `k6+${__VU}@example.test` }),
...
**期待効果**：
「掲載開始日にサイトが重い／落ちる」というクライアントが最も損害を感じる事故を事前排除。キャパシティの根拠が実測になり、Nao の非機能要件と Ryota の SLA 提示が数値で接続される。

---

#### 4. インフラ Unit Economics（案件別原価配賦・1 応募あたり原価）

**なぜ必要か**：
現状のコスト管理は「今月の Vercel は $X」という総額。これでは Ryota が見積を作るとき・Haruto が案件別粗利を見るときに使えない。SNS マーケ×採用支援は「1 応募あたりいくらで獲れたか」で評価される事業なので、インフラ費も同じ単位（応募あたり原価）に翻訳できなければ経営会話に乗らない。逆にここが出せると、クライアント提案で「応募単価にインフラ原価を含めて開示する透明性」が差別化になる。

**具体的手法**：
1. Vercel プロジェクト命名を `let-{client}-{app|lp}` に完全統一し、プロジェクト＝配賦単位にする（命名規約を Terraform module で強制）。
2. 直課できない共有リソース（監視 SaaS・DNS・Redis・ログ基盤）は **当月応募数按分**で配賦。按分基準は毎月固定し、途中で変えない。
3. 月次バッチで `cost_per_application = 案件別インフラ費 ÷ 当月応募数` を算出し、Notion「案件別インフラ原価」DB へ自動投稿。Akari／Haruto が参照。
4. 前月比の異常検知：応募数が横ばいなのに原価だけ増えた案件は、ISR revalidate 暴走・bot 流入・画像最適化の過剰実行のいずれかを疑う（この 3 つで実績の大半を占める）。

**判断基準・数値ライン**：
- 1 応募あたりインフラ原価：**¥50 以下**を標準、**¥50〜120** で要因分析、**¥120 超**で設計見直し（Kai へ起案）
- 案件売上に占めるインフラ費比率：**8% 以内**（超過は粗利警告）
- 前月比 **+30%** かつ応募数増加 +10% 未満の案件 → 自動で「コスト異常」Issue 起票
- 按分対象の共有費が総額の **25%** を超えたら直課への切り出しを検討（配賦の説得力が落ちるため）

**実務テンプレート**：
```markdown
## 2026-08 案件別インフラ原価（自動生成）
| 案件 | 直課（Vercel/DB） | 按分（監視・DNS・Redis） | 合計 | 当月応募数 | 応募単価 | 前月比 | 判定 |
|---|---|---|---|---|---|---|---|
| 翔星建設 | ¥12,400 | ¥3,100 | ¥15,500 | 412 | ¥38 | +4% | ✅ |
| 宮村建設 | ¥9,800  | ¥2,400 | ¥12,200 | 88  | ¥139 | +52% | ❌ 要因分析 |
按分基準：当月応募数比／共有費総額 ¥18,000（総額の 21%）
```

**期待効果**：
Ryota の見積が「感覚の丸め」から原価根拠付きに変わる。Haruto の案件別粗利表にインフラ費が正しい粒度で載り、赤字案件を早期に検出できる。

---

#### 5. WAF・Bot 対策と応募フォームの多層防御

**なぜ必要か**：
「WAF・DDoS 対策設定」は既存の役割定義に**項目名としてだけ**存在し、具体的な手法も閾値もない。採用フォームは bot スパム・競合他社によるスクレイピングの直接的な標的で、bot 応募はクライアントの採用担当が実際に連絡を試みて初めて発覚するため、**システムは全て正常（200 応答）のまま信頼だけが毀損する**という最悪の失敗モードを持つ。従量課金の膨張も同時に起きる。

**具体的手法**（単層に依存しない多層防御）：
1. **L1 WAF**：Cloudflare/Vercel Firewall の managed ruleset を有効化（OWASP Core Rule Set 相当）。
2. **L2 Bot スコア**：`cf.client.bot` / Vercel BotID で明白な bot をブロック、グレーゾーンはチャレンジへ。
3. **L3 Turnstile**：応募フォームに不可視 CAPTCHA。サーバー側で siteverify を必須検証（クライアント側だけの検証は迂回される）。
4. **L4 レート制限**：IP 単位 5 submit / 10 min、テナント単位 200 / h（スキル 1 のリミッタを再利用）。
5. **L5 Honeypot**：CSS で隠した `website` フィールドに入力があれば **200 を返しつつ破棄**（403 を返すと bot に検知ロジックを学習される）。
6. **L6 使い捨てメールドメイン**のブロックリスト照合＋ MX レコード実在確認。

**判断基準・数値ライン**：
- bot 応募率：総応募数の **1% 未満**（クライアントからの「連絡不能」報告率で近似計測）
- Turnstile 通過率：**98% 以上**（これを下回るなら正常ユーザーを弾いている＝機会損失）
- WAF ブロック率：**5% 超**なら誤検知調査を起動（正常ユーザー巻き込みの兆候）
- 4xx 件数が前週比 **+200%** で Slack #infra へ P2 通知
- 応募 API の 429 発生率：**0.1% 未満**（正常利用者が制限に当たっていない証明）

**実務テンプレート**：
```ts
// app/api/applications/route.ts — 多層検証の順序（安いチェックから先に）
if (body.website) return Response.json({ ok: true });                       // L5 honeypot: 静かに破棄
const ip = req.headers.get("x-forwarded-for")?.split(",")[0] ?? "unknown";
if (!(await ipLimiter.limit(ip)).success) return new Response(null, { status: 429 });   // L4
const ts = await fetch("https://challenges.cloudflare.com/turnstile/v0/siteverify", {   // L3
  method: "POST", body: new URLSearchParams({ secret: env.TURNSTILE_SECRET, response: body.token }),
}).then((r) => r.json());
if (!ts.success) return new Response("verification failed", { status: 400 });
if (DISPOSABLE_DOMAINS.has(body.email.split("@")[1])) return new Response(null, { status: 400 }); // L6
```

**期待効果**：
bot 応募でクライアントの採用担当の工数を溶かす事故を封じる。「システム正常・信頼だけ毀損」という検知不能な失敗モードを、計測可能な指標（bot 応募率・Turnstile 通過率）に変換する。

---

#### 6. ゲームデー（障害注入訓練）とオンコール設計 — バス係数 1 の解消

**なぜ必要か**：
Runbook・ロールバック手順・incident-check bot は整備済みだが、**それを Kuu 以外が実行できるかは一度も検証されていない**。手順書は「書いた本人だけが読める」状態に必ず劣化する。Kuu が休暇・病欠・移動中の間に P0 が起きれば復旧不能で、これは技術課題でなく組織構造上の単一障害点。アラート発火テスト（四半期）は監視の死活を見るが、**人と手順の死活**は見ていない。

**具体的手法**：
1. **四半期ゲームデー（90 分）**：Kuu は「観察者」に徹し一切手を出さない。Riku／Ao／Mio のいずれか 1 名が Runbook だけを頼りに復旧を試みる。詰まった箇所を Kuu がその場で Issue 化（＝Runbook の欠陥リスト）。
2. **シナリオのローテーション**（毎回 1 つ、5 種を順に）：① デプロイ後の白画面 → ロールバック実行 ② 依存 SaaS（決済・メール）全停止 → 縮退運転＋告知 ③ DB コネクション枯渇 → プーラ設定変更 ④ TLS 証明書失効／DNS 誤設定 ⑤ 本番データの誤削除 → PITR で任意時点復元。
3. **オンコール体制**：一次 Kuu／二次 Kai。夜間（22:00〜08:00）は P0 のみ PagerDuty で起こす、P1 以下は翌朝処理。連続オンコール上限 **2 週**、深夜対応した翌日は代休を Kai が自動承認。
4. 訓練で発覚した「Kuu の暗黙知」は必ず Runbook かスクリプトに落とし、口伝を残さない。

**判断基準・数値ライン**：
- **バス係数 ≥ 2**（Kuu 以外に P0 復旧を完遂できる人が最低 1 名）
- Runbook 単独実行成功率：初回 60% 程度を想定 → **2 週間以内**に手順改訂し、次回 **100%** を目標
- ゲームデーでの MTTR ≤ 実障害 MTTR の **1.5 倍**以内（乖離が大きいほど手順が属人的）
- ゲームデー実施：**四半期 1 回・90 分**、未実施の四半期を作らない
- 深夜 P0 呼び出し：**月 1 回以下**（超過は監視閾値かアーキテクチャの問題として棚卸し）

**実務テンプレート**：
```markdown
## ゲームデー記録 2026-Q3 / シナリオ③ DB コネクション枯渇
- 実行者：Ao（Kuu は観察のみ・質問への回答も禁止）
- 注入：staging で PgBouncer の pool_size を 5 に変更し k6 Load を実行
- 検知まで：4 分 12 秒（目標 5 分以内）✅
- 復旧まで：22 分 30 秒（目標 15 分以内）❌
- 詰まった箇所：
  1. Runbook に PgBouncer 管理コンソールへの接続方法の記載なし → #1284 で追記
  2. `max_connections` の現在値をどこで見るか不明 → incident-check bot に項目追加 #1285
- 次回までの改善期限：2026-09-05
```

**期待効果**：
Kuu 不在時の復旧不能リスクを解消し、Runbook を「書いてある」から「他人が実行できる」水準へ引き上げる。Kuu 自身が休暇を取れる体制になり、燃え尽きによる離脱リスクも下がる。

---

#### 7. イミュータブルバックアップ（3-2-1-1-0）と SBOM による供給網継続検証

**なぜ必要か**：
バックアップは取得・PITR・四半期リストア訓練まで到達しているが、**バックアップそのものが消される／暗号化される**シナリオへの耐性がない。ランサムウェアも操作ミスも、本番と同じ権限下にあるバックアップを最初に狙う。また依存管理は Renovate・attestation まで来ているが、**「今この瞬間、本番に何のバージョンが載っているか」の台帳（SBOM）がない**ため、重大 CVE の公表時に影響有無を即答できず、クライアントからの緊急照会に「調査中です」としか返せない。

**具体的手法**：
1. **3-2-1-1-0 ルール**：コピー **3** 本／媒体 **2** 種／オフサイト **1** 本／**イミュータブル（WORM）1** 本／復元検証エラー **0**。
2. 日次スナップショットを S3 Object Lock（Compliance モード・保持 35 日）または R2 の保持ポリシー付きバケットへ複製。**削除権限を持つ主体を分離**する（バックアップ用アカウントの鍵を本番デプロイ用と別管理し、CI からは書き込みのみ可能・削除不可の IAM ポリシーにする）。
3. **SBOM**：CI で CycloneDX 形式の SBOM を生成し、デプロイ成果物と紐付けて **13 か月**保管。
4. 新規 CVE 公表時は `grype sbom:...` で全プロジェクト横断照合し、影響有無を **2 時間以内**に回答。影響ありなら既存基準（Critical/High は 72 時間以内に対応）へ接続。

**判断基準・数値ライン**：
- イミュータブル保持期間：**35 日**（RPO 5 分・RTO 60 分の設計値とセットで Nao と合意）
- 復元検証：四半期 1 回・**エラー 0**、実測所要時間を RTO の根拠として記録
- バックアップ削除権限保持者：**0 名**（保持期間内は技術的に削除不能）
- CVE 影響判定 SLA：**2 時間以内**／Critical・High の修正：**72 時間以内**
- SBOM 保管：全本番デプロイについて **13 か月**

**実務テンプレート**：
```yaml
# .github/workflows/sbom.yml — SBOM 生成＋成果物署名＋CVE 照合
- uses: anchore/sbom-action@v0
  with: { format: cyclonedx-json, output-file: sbom.cdx.json }
- uses: actions/attest-build-provenance@v1        # SLSA Build L3 相当の来歴署名
  with: { subject-path: sbom.cdx.json }
- run: grype sbom:sbom.cdx.json --fail-on high    # High 以上で CI FAIL
- uses: actions/upload-artifact@v4
  with: { name: sbom-${{ github.sha }}, path: sbom.cdx.json, retention-days: 400 }
```
```bash
# バックアップ用バケット：CI は書けるが消せない（Object Lock 併用）
aws s3api put-object-lock-configuration --bucket let-backups \
  --object-lock-configuration 'ObjectLockEnabled=Enabled,Rule={DefaultRetention={Mode=COMPLIANCE,Days=35}}'
```

**期待効果**：
ランサムウェア・誤削除でバックアップごと失う最悪シナリオを技術的に不可能にする。CVE 公表時の初動が「全プロジェクト調査」から「1 コマンド照合」に変わり、クライアントへの回答が当日中に返せる。

---

### 🚫 新たに定義した NG パターン

1. **テナント識別子のない全体メトリクスだけを見て「正常」と判断する** — 全体 p95 が緑でも 1 社だけ 3 秒かかっている状態は日常的に起きる。テナント別の分解なき「異常なし」報告は禁止。
2. **平時から本番 write 権限を保持し続ける／break-glass 記録を残さず本番オペを実行する** — 「急いでいたから」は理由にならない。証跡のない本番操作は、後から正当性を証明できない時点で事故と同じ扱い。
3. **負荷試験を実施せず、キャパシティの根拠を感覚値（「たぶん捌ける」）で置く** — 掲載開始日という**事前に分かっているスパイク**で落ちるのは、インフラ担当として最も言い訳が効かない。実測なきキャパシティ宣言は禁止。
4. **Kuu 本人しか実行できない Runbook を「整備済み」と報告する** — 手順書の完成度は、書いた本人でなく**他人が実行して復旧できたか**でしか測れない。ゲームデー未検証の Runbook は「未整備」と同義。
5. **インフラコストを総額だけで報告し、案件別・1 応募あたりに分解しない** — 「今月 $X でした」は経営会話に乗らない。Ryota の見積・Haruto の粗利表に接続できない数字は、出していないのと同じ。
6. **応募フォームに CAPTCHA を 1 つ入れただけで「bot 対策済み」とする** — 単層防御は必ず突破される。WAF・Bot スコア・Turnstile・レート制限・honeypot・使い捨てメール判定の多層が揃って初めて対策と呼ぶ。
7. **バックアップを本番と同一アカウント・同一権限下にのみ置く** — 本番を奪われた攻撃者・本番を消した誤操作は、同じ権限でバックアップにも到達する。削除耐性のないバックアップは「あるつもり」に過ぎない。
8. **Turnstile/WAF の通過率を測らずにブロック設定だけ強化する** — 正常な求職者を弾いた分は誰からも報告されない。ブロック率と通過率をセットで測らない防御は、機会損失を不可視化する。
...
| 指標 | 強化前 | 強化後（v2 到達目標） |
|---|---|---|
| 監視の計測単位 | 全体 p95・エラー率のみ | 全体＋**テナント別 SLO**（7 社個別に可用性 99.9% / p95 500ms を提示） |
| 1 社起因の全社障害 | 検知も隔離も未定義 | ノイジーネイバーを同時実行キー＋レート制限で構造封じ込め、発生 **0 件** |
| 本番 write 権限の常時保持者 | 実質 Kuu が常時保持 | **0 名**（break-glass TTL 60 分・全操作に監査証跡） |
| 権限剥奪漏れ | 検知手段なし | 四半期棚卸し＋オフボーディング **24 時間** SLA、剥奪漏れ **0 件** |
| キャパシティの根拠 | 感覚値（`max_connections` の机上逆算のみ） | k6 実測（Smoke/Load/Spike）、掲載開始日 **4 倍**スパイクを PASS 済み |
| 掲載開始日の可用性 | 未検証（運任せ） | 3 営業日前に Spike 試験 PASS が本番昇格の必須条件 |
| コスト可視性 | 月額総額のみ | **案件別原価＋ 1 応募あたり原価 ¥50 基準**、粗利表に直結 |
| bot 応募 | 検知不能（システムは 200 応答のまま） | 6 層防御＋ bot 応募率 **1% 未満**／Turnstile 通過率 **98% 以上**を計測 |
| バス係数 | **1**（Kuu 不在で復旧不能） | **2 以上**（四半期ゲームデーで Kuu 不在復旧を実演済み） |
| Runbook の実効性 | 書かれているが未検証 | 他人による単独実行成功率 **100%** を四半期ごとに実測 |
| バックアップの削除耐性 | なし（本番と同一権限下） | **3-2-1-1-0**／WORM 保持 **35 日**・削除権限保持者 0 名 |
| CVE 公表時の初動 | 全プロジェクトを手動調査 | SBOM 照合 1 コマンド、影響判定 **2 時間**以内にクライアント回答 |
| 深夜 P0 呼び出し | 発生都度・上限なし | **月 1 回以下**、超過は監視・設計課題として棚卸し |

**位置づけ**：v1 完了時点で「単一アプリを落とさず速く安く出す」国内上位水準に到達済み。v2 で加わるのは **(a) 複数クライアントを 1 基盤で捌く多テナント運用、(b) 人と権限のガバナンス、(c) 実測で裏を取る文化（負荷試験・ゲームデー）、(d) インフラを事業数値へ翻訳する能力** の 4 軸。技術的優秀さを「クライアントに説明可能で、Kuu が倒れても回る」形に変換した状態が v2 の完成形。
