...
### 2026-08-30
- **Vercel Fluid Compute（旧Edge Functions次世代）の本番採用でコールドスタートをほぼ排除、応募フォームの初回応答が瞬時に**：従来のServerlessでコールドスタート400-800msだった応募APIをFluid Computeへ移行し、初回リクエストでも80ms以下を維持。「朝イチの応募一覧」問題が構造的に解消。効果例：コールドスタート起因のP99レイテンシ800ms→90ms／朝イチアクセスの体感"重さ"解消
- **Terraform Cloud + Vercel Provider でIaC完全宣言、環境構築を手動から機械化**：Vercelプロジェクト・環境変数・ドメイン・DNS・GitHub連携をTerraformで宣言し、新規案件のインフラ構築が`terraform apply`一発に。手動作業起因のミスがゼロ化。効果例：新規案件インフラ構築時間3時間→15分／手動設定ミス月2件→0件
- **OpenTelemetry Collectorを Vercel + Grafana Cloudへ集約、7社横断で単一ダッシュボード運用**：各案件のログ・メトリクス・トレースをOtelで統一収集し、Grafana Cloudで7社を1画面で監視。案件別ダッシュボード切替の手間が消滅。効果例：監視画面切替時間1日15分×7社→統合ダッシュボード／障害検知平均時間5分→90秒
- **`preview-comments` (Vercel Toolbar)をクライアントレビューで活用、実機URLに直接コメント運用**：クライアントがVercel Preview URL上に直接コメントを付ける運用に切り替え、Slack/メール往復が消える。コメントは自動でLinear/GitHub Issueに連携し、修正tasksが即発行される。効果例：レビュー往復所要時間3日→当日／修正tasks作成工数手動15分→自動0分
- **AWS S3 → Vercel Blob移行で画像/PDF管理を統合、Bandwidth/Egress費用を大幅削減**：バナーPNG・LP画像・応募フォーム添付ファイルをVercel Blobに一元化し、Edge配信で高速化＋AWS Egress費用を削減。7社のCDN費用が集約されコスト透明化。効果例：月次CDN費用従来比-45%／Blob化による画像配信速度平均+35%
- **RUM (Real User Monitoring) の Core Web Vitalsを求職者セグメント別に分割、実機3G/4G/5Gの体感差を可視化**：Vercel Speed InsightsのRUMを回線種別×端末種別でセグメント分割し、「建設現場4G」「都心5G」「郊外3G」の体感差を数値化。Rikuと共有し画像最適化・遅延読込の優先度判断に活用。効果例：3G環境のLCP4.2s→2.1s／現場からの応募完了率+18%
- **Sentry Session Replay 導入で本番エラーの再現手順を動画で自動記録、デバッグ所要時間を短縮**：本番500エラー発生時のセッション操作を動画で自動記録し、Ao/Rikuは再現手順を推測せず動画で確認。エラー再現に費やす時間がほぼゼロに。効果例：本番バグの再現手順特定時間45分→3分／「再現できません」で放置されるバグ月2件→0件
- **11-管理部門 nori へ「個人情報保護・特商法・API利用規約」チェック依頼をデプロイゲート化**：本番デプロイ前に nori の関所を通し、プライバシーポリシー・特商法表記・第三者API利用規約準拠を機械同期。デプロイ後のリーガル起因差戻しが構造的に消える。効果例：nori起因のデプロイロールバック月1件→0件／本番投入前のリーガル観点漏れゼロ
