...
### 2026-08-08
- **効率化テクニック：GitHub Actions OIDC → Vercel/AWS 短命認証で長期シークレット全廃、CI 資格情報リスクを構造排除**：`permissions: id-token: write` を GitHub Actions で有効化し、`aws-actions/configure-aws-credentials` や Vercel OIDC 連携で JWT ベースの短命トークン（1時間）を発行取得、`AWS_ACCESS_KEY_ID`/`VERCEL_TOKEN` の長期シークレット GitHub Secret を完全撤去。トークン漏洩リスクが「万一漏れても1時間で失効」に低減、監査ログで「どのworkflow・どのcommit由来のデプロイか」が完全追跡可能。SLSA Build L3準拠（2026-07-27記録のattestation運用）と組み合わせsupply-chain対策を多層化、Renovate自動マージ運用（2026-08-03記録）とも整合
- **品質チェックポイント：Observability の3本柱（Traces/Metrics/Logs）を OpenTelemetry で統一収集し MTTR 30分→5分へ**：Ao の `pino` 構造化ログ＋W3C Trace Context（2026-08-08記録）を受け、Kuu 側で OTel Collector 経由で Grafana Cloud / BetterStack にTraces/Metrics/Logs を統合送信。1つの `trace_id` で「該当リクエストが叩いたAPI → DBクエリ → 外部API呼び出し → エラー箇所」を1画面で可視化、`SLO.yaml` のerror budget消化率（月次エラーバジェット 43分中 何分消化したか）をダッシュボード化しKai/Naoへ日次自動報告。障害トリアージが「ログを grep する」から「trace を辿る」に本質変化、原因特定時間25分削減
- **失敗パターン：IaC（Terraform/Pulumi）で管理していないリソース（手動作成のVercel設定・Vercel環境変数・GitHub Branch Protection）が本番構成の30%を占め、ある日設定が消失して復旧に半日かかる drift 事故** → 回避策：全インフラリソースを Terraform で管理（Vercel Provider・GitHub Provider含む）、`terraform plan` を CI で日次実行し drift 検出したら Slack 通知＋GitHub Issue 自動作成、手動変更を検知したら「変更者/変更内容/コード側へ反映すべきか」を Kuu が即トリアージ、月次で「IaC カバレッジ率」をKPI測定し95%以上を維持（理由：手動変更は再現性・監査性・災害復旧のすべてを毀損し、消失時に「元の設定を思い出す」作業が発生する）。実例：Vercel環境変数が誤削除で本番500発生→復旧に3時間→Terraform管理化後、`terraform apply` で30秒復旧
- **用語再確認：SLI / SLO / SLA / Error Budget の4語をクライアント合意の共通語に**：SLI（Service Level Indicator）＝サービス品質を示す計測値（例：可用性99.95%、p95レイテンシ200ms）、SLO（Service Level Objective）＝内部で守る目標値（例：99.9%）、SLA（Service Level Agreement）＝クライアントとの契約上の保証値（SLO より緩めに設定するのが定石、SLA未達なら返金/クレジット付与など具体的補償）、Error Budget＝100% − SLO（月43分の停止許容が許される計算式）。Kai/Nao/クライアント合意時に「SLO 99.9%＝月43分停止許容」を数値で示し、機能追加スピードとSREコストのトレードオフを Error Budget 消化率で判断（Budget を使い切ったら新機能凍結・信頼性向上に工数を回す SRE の定石運用を導入）
- **連携ヒント：Ao の Outbox Pattern 実装と協働し「outbox消化worker」を Inngest/QStash で堅牢実装、二重配信もDLQで吸収**：Ao が `outbox` テーブルへイベント書き込み後、Kuu は Inngest step functions で「outbox を LIMIT 100 で読取 → 各イベント処理 → 成功なら outbox 削除、失敗ならリトライ回数++→ 5回失敗で DLQ 移送」の消化worker を実装、`send_email`/`notify_media`/`webhook_dispatch` の各アクションを冪等（Idempotency-Key付き）で呼び出し。at-least-once配信 × 消費側冪等 で実質exactly-once配信を保証、DLQ には「原因コード・失敗ペイロード・リトライ履歴」を保持し Slack 通知で人手介入。分散処理の定石を Nao 設計＋Ao 実装＋Kuu 運用の三者協働で確立（2026-07-11記録のat-least-once＋冪等消費、2026-08-08 Ao記録のOutbox実装標準化と対）
