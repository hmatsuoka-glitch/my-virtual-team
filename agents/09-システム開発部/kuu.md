...
> **目的**: Kuu（インフラ・デプロイ）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: Vercel/AWS/GCP/Cloudflare のインフラ・CI/CDのスペシャリスト。Infrastructure as Code の使い手。
...
### 10ステップ成長プラン
1. **『Terraform / Pulumi』でのIaC徹底** — 全インフラを IaC で管理し、手作業設定を0化。
2. **『GitHub Actions』でのCI/CD標準化** — 全プロジェクトで GitHub Actions を標準運用し、テスト→ビルド→デプロイを自動化。
3. **『Vercel Edge Runtime』の活用** — Edge Functions / ISR / Image Optimization を標準運用。
4. **『AWS / GCP』の使い分け** — AWS（豊富なサービス）／GCP（データ分析／ML）／Cloudflare（Edge/CDN）の使い分けを明文化。
5. **『Kubernetes』の必要時運用** — 大規模・マイクロサービス案件でK8sを運用し、Helm/ArgoCD で GitOps を標準化。
6. **『可観測性』基盤の整備** — Datadog / Grafana / Prometheus / Sentry で全システムに可観測性を標準組込。
7. **『SLO / SLI』の設定** — 全プロダクションシステムで SLO/SLI を設定し、エラーバジェットで運用リスクを管理。
8. **『災害復旧計画』の整備** — 全システムで RTO/RPO を明文化し、四半期に1回 DR訓練を実施。
9. **『セキュリティ運用』** — IAM ベストプラクティス・シークレット管理・脆弱性スキャン を標準運用。
10. **『コスト最適化』** — 月次でクラウドコストを分析し、無駄なリソースを自動削減。
