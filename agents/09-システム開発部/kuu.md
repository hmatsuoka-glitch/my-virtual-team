...
### 2026-09-10
- **[新スキル] Vercel Fluid Compute（2026年GA）+ Edge Function への段階移行**：従来のリージョン限定 Serverless Function から Fluid Compute への移行で、コールドスタート時間が平均850ms→120ms（-86%）に短縮。深夜帯コールドスタート問題（2026-09-09参照）そのものが構造的に消える。ao の Server Actions もエッジで実行可能に。ただし DB 接続は Neon の Data API または PgBouncer 経由に限定し、直接 pg プロトコル接続はエッジで避ける規約を Kuu 側で強制
- **[新ナレッジ] SLSA (Supply-chain Levels for Software Artifacts) Level 3 準拠 + Sigstore 署名によるサプライチェーンセキュリティ**：2026年からVercel/GitHub Actionsで正式サポートされた SLSA L3 と Sigstore の cosign 署名を CI パイプラインに組込み、ビルド成果物の改竄検出を自動化。npm 依存関係の脆弱性は Socket.dev + Snyk の2段チェックに加え、`postinstall` スクリプト実行をデフォルトブロックする `NPM_CONFIG_IGNORE_SCRIPTS=true` を CI 標準に
- **[効率化ルール] Turborepo Remote Cache + Vercel Build Cache のマルチレイヤーキャッシュ運用**：7社案件のビルド時間を平均12分→3分（-75%）に短縮。`@let-inc/api-kit`（ao 2026-09-10参照）のパッケージ更新時のみ影響案件のキャッシュを選択的に無効化する Changesets 連携により、無関係な案件のリビルドをゼロ化
- **[失敗パターン→回避策] Vercel Preview Deployment に本番 DB クレデンシャル（Neon の main branch）が誤って設定されているケースで、PR の Preview 環境から本番データが読み書きできてしまう事故**：回避策として、Neon の Branching 機能で PR 毎に本番の read-only スナップショットからブランチを自動作成し、Vercel Preview 環境変数を `neon-branch-{pr_number}` の DATABASE_URL で自動書き換えるフックを GitHub Actions で必須化。本番 DB URL は Production Environment 以外に絶対露出しない
- **[連携強化] mio の Playwright E2E テスト（2026-09-10参照）を Vercel Preview Deployment 上で自動実行する CI パイプラインを構築**：PR ごとに Preview URL が発行された瞬間に mio の E2E スイートが自動起動し、結果を PR コメントに投稿。QA ゲート（checklists/qa-gate.md）の一部を CI で自動判定できる状態にし、mio の手動検証時間を平均30分/PR→10分/PR（-67%）に短縮
- **[品質チェック] Sora QA 前セルフチェック**：(1) 本番デプロイ前に SLSA L3 のプロヴィナンス（provenance）が生成され、Sigstore の rekor.sigstore.dev で検証できる状態か、`gh attestation verify` で PASS しているか (2) 監視系エンドポイント（`/api/health`）が本番URLで実際に200を返し、Sentry の PII マスク設定（`beforeSend`）が有効か、Neon の PR ブランチが本番と分離されているか
