...
### 2026-09-04
- **Cloudflare Workers vs Vercel Edge の決定表を Kaito と共通化し、案件受注時点でランタイム選定を1分で終わらせる**：LP複製・キャンペーン案件のように「静的アセット＋数本のフォーム/計測エンドポイント・全世界配信・費用最重視」なら Workers＋R2＋KV、Next.js の App Router を全面採用しサーバー機能とプレビュー環境と Skew Protection（2026-08-03参照）を活かす採用管理系は Vercel、と 5 軸（フレームワーク・状態管理・地理分散・料金モデル・チームの運用経験）で振り分ける表を `docs/runtime-decision.md` に置く。Kaito が受注時にこの表を通してから設計依頼を Nao へ回す運用にすると、途中で「やっぱり Vercel に戻す」の作り直しが消える。Fluid Compute の Active CPU 課金（2026-07-27参照）で Vercel 側の外部 API 待ち中コストは既に落ちているため、コスト単体を理由に安易に Workers を選ぶ判断は表から外す。
- **Argo CD 2.10 の ApplicationSets を、社内ダッシュボード等の k8s 常駐案件だけに限定して導入し、Vercel 系の push 駆動運用と混在させない**：クライアント案件の 9 割は Vercel の GitHub 連携で十分だが、常駐ワーカー・GPU ジョブ・オンプレ連携が絡む案件で k8s に載る時だけ GitOps を敷く。ApplicationSets の Pull Request Generator でクラスタ側の preview 環境も PR 連動にし、Vercel preview と同じ操作感（PR オープンで環境作成・クローズで teardown、2026-08-05参照）を再現する。Nao の設計段階で「この案件は GitOps か push 駆動か」を明示してもらい、Kuu が中途半端に両方を回して認知負荷を上げる事態を避ける。
- **Doppler で secret rotation を月次自動化し、Ao の `.env.example` 差分検知（2026-09-01参照）と 1 本のパイプラインに束ねる**：DB 接続文字列・外部 SaaS API キー・SMTP 資格情報を Doppler をハブに置き、Doppler → Vercel/GitHub Actions/Sentry へ同期。rotation は月初 JST 03:00（応募トラフィックの谷、2026-08-16参照）に自動発火し、失敗時のみ Ao と Kuu へ通知。従来「退職者が触っていたキーを誰も回していない」問題が半年放置されていたが、rotation を既定にすると「回せない依存」（ハードコード・古い SDK）が月次で自然に炙り出される。PII マスク（2026-09-02参照）と併せて、資格情報の露出面を出す側で塞ぐ二段構えにする。
- **観測スタックは Sentry（アプリエラー・リリース連動）＋ Grafana Cloud（メトリクス・ログ・traces を OTel semantic conventions 経由、2026-07-27参照）＋ Vercel Speed Insights（RUM）の 3 層に固定し、Datadog は中規模案件では初期構築時点で意図的に採用しない**：Datadog は機能で優るが custom metrics と log ingest の従量課金が予算を先に食い尽くし、half-year でスタック入れ替えのコストが観測導入コストを上回る。3 層構成なら月額を 1/3〜1/5 に抑えつつ、Mio の canary＋synthetic（2026-08-27参照）の結果も Grafana に集約でき、Sora の事後 QA で「本番のどの指標を誰が見ているか」の一覧が 3 ダッシュボードで済む。案件が大規模化してから Datadog への移行を検討する順序を初期方針として明文化する。
- **SOC 2 Type II の evidence collection を Vanta/Drata の Vercel・GitHub・Sentry コネクタで自動化し、初期構築の必須チェック（2026-08-18参照）に組み込む**：2026 は AICPA 2022 TSC 改訂の運用が定着し、監査人側も自動収集ツールの証跡を第一級で受理する運用が標準化。クライアントのセキュリティ質問状（アクセス制御・変更管理・脆弱性対応・ログ保持）に対し、都度スクショを集めるのでなく Vanta のレポートを PDF 化して返す運用にすると、質問状 1 件の回答時間が半日から 15 分に落ちる。Kai がクライアントから求められた時に慌てて整備するのでなく、案件立ち上げ時点で GitHub の branch protection・Sentry の access log・Vercel の deployment audit を Vanta に接続しておき、監査対応の下地を「後から遡って整えられない項目」だけでも初期に済ませる。
