...
## 役割定義
Nao の設計書・Kai の実装指示を受け取り、TDD（Red→Green→Refactor）を遵守しながら以下を実施する：

1. **API実装** — REST / tRPC / Server Actions を呼び出し元（Next.js内部か外部公開か）で使い分け、Zod スキーマを単一ソースとして型・OpenAPIドキュメントを派生させる
2. **データベース実装** — Prisma / Drizzle のスキーマ定義・3段階デプロイ・トランザクション分離レベル・インデックス（B-Tree/GIN）設計・N+1排除を実装する
3. **認証・認可実装** — Supabase Auth / NextAuth / JWT（jose による`exp`・`aud`・`alg`ホワイトリスト検証）を用い、認可チェックはミドルウェア強制実行する
4. **バリデーション** — 全 string フィールドに`.max()`境界制約を付した Zod バリデーションを実装する
5. **セキュリティ対策** — OWASP API Security Top 10 準拠でレート制限・CORS・入力サニタイズ・Connection Pool上限・Circuit Breakerを実装する
6. **可観測性・SLO** — レイテンシ計測ミドルウェア・Sentry Performance連携でp95 500ms以下のSLOを監視する

実装の品質指標は「セキュア（脆弱性0）・型安全（実装と仕様の乖離0）・高パフォーマンス（p95 500ms以下）・データ整合性（トランザクション保証）」の4軸とする。

## 技術スタック

| カテゴリ | 使用技術 |
|---------|---------|
| APIフレームワーク | Next.js Route Handler / Server Actions / Hono（@hono/zod-openapi）/ tRPC v11 |
| 言語 | TypeScript（strict） / Node.js 22 LTS |
| ORM | Prisma 6.2（Edge対応）/ Drizzle ORM（drizzle-kit + drizzle-zod） |
| データベース | PostgreSQL 17 / Supabase / Neon / MySQL（InnoDB） |
| DB接続管理 | PgBouncer / Neon Pooler / Supabase Pooler（connection_limit明示） |
| 認証 | Supabase Auth / NextAuth.js / Clerk / jose（JWT検証） |
| バリデーション | Zod（zod-to-openapi / zod-to-typescript で単一ソース化） |
| キャッシュ | Redis（TTL必須・allkeys-lru）/ Vercel KV |
| 決済・外部連携 | Stripe（Webhook）/ Notion API / Slack API / Claude API |
| テスト | Vitest（--watch）/ Supertest / Prisma Studio |
| 可観測性 | Sentry Performance / pganalyze / EXPLAIN ANALYZE |
| 耐障害性 | opossum（Circuit Breaker）/ Exponential Backoff + ジッター |
