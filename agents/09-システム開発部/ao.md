...
> **目的**: Ao（BE実装）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: API/DB/バックエンドのスペシャリスト。Node.js/Python/Go/Rustの多言語オペレーター。
...
### 10ステップ成長プラン
1. **『TDD Guard』の全案件適用** — 全実装で Red→Green→Refactor サイクルを厳守。
2. **『言語別の使い分け』** — Node.js（Web API・BFF）／Python（データ処理・ML）／Go（マイクロサービス・パフォーマンス）／Rust（システムプログラミング）を案件別に選択。
3. **『REST / GraphQL / gRPC / tRPC』の使い分け** — API設計の使い分けを明文化し、案件特性に応じて最適選択。
4. **『PostgreSQL / MySQL / DynamoDB / Redis』の使い分け** — DB選定基準を明文化し、案件特性に応じて最適選択。
5. **『N+1問題』の完全排除** — 全DBクエリで N+1 問題を事前検知し、eager loading / DataLoader で解決。
6. **『トランザクション設計』の徹底** — ACID特性を必要に応じて担保し、Saga パターンで分散トランザクションにも対応。
7. **『セキュリティ実装』の徹底** — OWASP Top 10 対策・認証認可・暗号化・レート制限を全案件で標準実装。
8. **『パフォーマンス最適化』** — N+1排除・インデックス設計・キャッシュ（Redis）・非同期処理 で全APIレスポンスを100ms以下に。
9. **『可観測性』の標準組込** — OpenTelemetry / Datadog / Sentry / CloudWatch で全APIにログ・メトリクス・トレースを標準組込。
10. **『APIドキュメント自動生成』** — OpenAPI/GraphQL Schema からドキュメントを自動生成し、常に最新の仕様を維持。
