...
### 2026-09-04
- **Neon / Supabase の DB ブランチングを PR プレビュー環境の既定にし、kuu の Vercel Preview と 1 対 1 で紐付ける**：従来はステージング DB を全 PR で共有し、ren のフォーム LP 検証と Ao のマイグレ検証が互いのシードを汚染して「昨日通ったテストが今日落ちる」が頻発していた。CI で PR 作成時に親ブランチから DB ブランチを分岐（`neonctl branches create --parent main`）→ その接続文字列を Vercel Preview の `DATABASE_URL` に自動注入 → PR クローズで自動破棄する運用に切り替え、破壊的マイグレも Preview で本番同等データに対して安全に試せる。kuu の 3 段階デプロイ（2026-05-16 参照）判定を PR 段階で 100% 完結させ、mio の QA も他 PR の変更を踏まない独立環境で回せる
- **tRPC v11 の subscriptions（SSE トランスポート）で管理画面の応募リアルタイム反映を Riku と設計する**：採用担当が毎朝叩く応募一覧（2026-08-16 参照）の「更新ボタンを何度も押す」体験を、SSE 購読で新規応募が入った瞬間に自動追記する形へ変更。ただし Vercel Functions の `maxDuration`（Pro でも 800 秒上限）で接続が切れるため、クライアント側で切断→再接続時に「最後に受け取った受付番号」以降を差分取得する再開契約を Riku と着手前に握る。subscription の初期化時に `checkUserOwnership()` 相当のテナント境界チェックを必ず通し、他テナントの応募が streaming で漏れる事故を構造遮断（Server Actions と同じ攻撃面）。長時間接続のため kuu へは Function 同時実行数の見積もりを共有し、必要なら Cloudflare Workers / Hono への切り出しを選択肢に置く
- **Postgres 17 の Row Level Security を multi-tenant 採用管理の「認可の最終防衛線」として nao(dev) と設計表に組み込む**：アプリ層の Prisma `$extends()` と `checkUserOwnership()`（第一防衛、2026-06-16 参照）は不変で維持したうえで、DB 側で全応募・選考履歴テーブルに `CREATE POLICY tenant_isolation USING (tenant_id = current_setting('app.tenant_id')::uuid)` を敷き、アプリの認可ロジックが 1 箇所でも漏れたら DB が拒否する二重化にする。Prisma 経由の全リクエスト冒頭で `SET LOCAL app.tenant_id = $1` をミドルウェアから発行し、コネクションプール共有時のセッション変数残留を `LOCAL` で必ずトランザクション終了時に破棄。nao(dev) の設計表に「ロール × リソース × ポリシー式」列を追加し、mio には「ポリシー無効化を試して 403 になるか」の DB 側認可テストを追加依頼。OpenFGA 等のポリシーエンジン（2026-08-03 参照）を採用する場合も RLS は撤去せず、判定の宣言化と DB の最終防衛は別レイヤとして両立させる
- **Postgres 17 の `MERGE` 文で「応募 upsert + 選考履歴の状態遷移 insert」を 1 クエリ化し、往復と競合を同時に消す**：従来は `findUnique` → 存在なら update / 無ければ create → 履歴 insert の 3 ラウンドトリップで、その隙間で他リクエストと競合し 2026-08-12 の read-modify-write と同じ在庫ズレ級の事故を起こしていた。`MERGE INTO applicants USING (VALUES ($1, $2, ...)) AS src ON ... WHEN MATCHED AND status <> 'withdrawn' THEN UPDATE ... WHEN NOT MATCHED THEN INSERT ... RETURNING id, xmax = 0 AS inserted` で 1 文に集約し、`RETURNING` で「新規挿入 or 既存更新」を分岐せず判定可能にする。同一文内で `WITH ... INSERT INTO application_events` を CTE 連結すれば履歴も同 1 コミットに束ねられ、ロック期間短縮＋ ORM の便利メソッド N+1（2026-07-01 参照）も同時解消。破壊的変更ではないため kuu の 3 段階デプロイ不要だが、mio へは MERGE の 3 分岐（マッチあり更新／マッチなし挿入／条件外スキップ）＋同時 2 リクエスト競合の 4 パターンを名指しで引き渡す（2026-08-13 の境界申告連携）
- **Server Actions を `import 'server-only'` の物理境界で守り、機密ロジック・DB クライアントの Client バンドル混入をビルド段階で構造遮断する（2026-08-03 の延長）**：Server Actions が公開エンドポイント同等の攻撃対象で `checkUserOwnership()` を必ず通す原則は不変。加えて、Client Component から Server Action ファイルを誤って import した瞬間に Prisma クライアント・暗号鍵参照・S3 署名ロジックがクライアント JS へ露出する（Next.js は Server Action の関数境界は守るが、同ファイルから export された他ユーティリティは Client に混入し得る）。DB モジュール・認可ヘルパ・秘密鍵参照モジュールの先頭で `import 'server-only'` を必須化し、誤 import 時はビルド失敗で物理ブロック。ren（応募フォーム LP → Server Action）にも同ルールを共有し、`api-kit` パッケージ（2026-09-01 参照）側の内部モジュールにも同 import を適用してコピペ経由の露出も遮る。ESLint で「`server-only` を先頭 import していないファイルからの `prisma`／`process.env.*_SECRET` 参照」を検出する自作ルールを CI 必須化
