...
### 2026-09-21
- **2026年最新トレンド：Next.js 16 の Server Actions v2 + `use cache` ディレクティブでバックエンド境界が再定義**：Next.js 16 では Server Actions が Zod-first で型検証を自動注入し、`'use cache'` ディレクティブで React 19.2 の cache primitive がキャッシュ層を宣言的に構成する。従来 riku 側で fetch していたエンドポイントを Ao 側で Server Action として export し、`revalidateTag()` で無効化制御。API Route Handler は「外部連携専用」に用途を絞り、内部通信は Server Actions へ移行、型不整合バグが体感 70% 減少。
- **2026年最新トレンド：Drizzle ORM v0.40 + Neon PostgreSQL 17 の PITR（Point-in-Time Recovery）を TDD に組み込む**：Neon の branching + PITR で「テストごとに DB スナップショット分岐→破棄」を Vitest の `beforeEach` で `neon.branches.create({ parent: 'main', point_in_time: 'HEAD' })` として自動化。テスト用固定シード不要、本番相当データで並列テスト実行可能、テスト時間は変わらず信頼性 3 倍。TDD Guard（mio 監督）と組み合わせて「Red → Green → Refactor」の各段階で本番相当スナップショットで動く安心感。
- **失敗パターン：Server Actions 内で `revalidatePath('/')` を無条件に呼んで全画面キャッシュ破壊** → 回避策：`revalidateTag('applicant:list')` のようにタグ単位で無効化、タグは Zod スキーマから自動生成した定数を使用。`revalidatePath` は「本当にルート全体を再検証したい時」のみ、コードレビュー時に mio が自動 lint 検出（理由：全画面破壊は Vercel の ISR キャッシュヒット率を 90% → 20% に落とし、月次コストが 3 倍に膨らむ事故が実案件で発生）。
- **失敗パターン：DB スキーマ変更を Drizzle migration で流したが本番の zero-downtime 手順を踏まなかった** → 回避策：`ALTER TABLE ... DROP COLUMN` は「①アプリから該当列参照を削除デプロイ → ②migration で DROP → ③次リリース」の 3 段階に固定化。expand-contract パターンを Drizzle の `migrations/` 内 README に強制、kuu のデプロイフローと連動（理由：カラム削除と参照コード削除が同時デプロイだと、ロールバック時に前バージョンが存在しない列を参照して 500 エラー全ユーザー波及）。
- **品質チェックポイント：pgTAP + Drizzle の `assert` を CI で並走**：スキーマ制約（NOT NULL、CHECK、FK）を pgTAP で「制約が存在すること」を SQL レベルでテスト、Drizzle 型定義との drift を検出。`SELECT ok(has_constraint('applicants', 'apps_email_check'))` のようなアサーションを migration ごとに追加、DB とアプリ層の二重検証で「型は通るのに DB 制約で落ちる」バグを撲滅。
- **品質チェックポイント：OpenAPI 3.1 生成の自動化 + 契約テスト**：Ao の Server Actions と API Routes から `@hono/zod-openapi` で OpenAPI 3.1 を自動生成、riku 側は生成された型定義で fetch 実装。契約テストとして Pact-JS を CI に組み込み、「Ao がスキーマ変更 → riku 側の実装が破綻」を PR レベルで検出。ren（LP）にも共通 OpenAPI を配布して連携効率化。
- **効率化テクニック：`drizzle-kit push` の禁止と `drizzle-kit generate + migrate` の強制**：`push` は本番で使うと不可逆な破壊的変更を招くため、`.drizzle-kit-config.ts` で `push` コマンドを実行時にエラーで停止させる wrapper を導入。全変更は必ず migration ファイルとして残し、Git で差分レビュー可能に、kuu のロールバック手順も migration の revert で完結。
- **mio 連携の小ヒント：`describe.concurrent` + `Testcontainers` で DB 統合テストを並列化**：mio が管理するテストランナーで Postgres 17 コンテナを Testcontainers で並列起動、Vitest の `describe.concurrent` と組み合わせて DB を要する統合テストの実行時間を 8分 → 1.5分（80%短縮）。mio の TDD Guard 実行スケジュールに組み込み、Red→Green サイクルを高速回転。
