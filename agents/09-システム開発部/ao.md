...
### 2026-07-21
- **API フレームワーク刷新**: **現状**：Next.js Route Handler で `NextRequest` 冗長取り回し＋認可チェック個別実装で実装時間30分/エンドポイント、p95 レイテンシ 300ms → **改善**：Hono + `@hono/zod-openapi` + Bun ランタイムに移行し `createRoute({ method, path, request, responses })` 1回でルート・OpenAPI・Zod・型を単一ソース化、Cloudflare Workers Edge Runtime 化 → **期待効果**：実装時間30分→12分、p95 300ms→80ms、Riku への仕様共有は `/doc` URL 1本で完結、開発速度2.5倍・レイテンシ73%改善
- **ORM 戦略見直し**: **現状**：Prisma のみで運用しスキーマ修正時に `migrate dev` 30秒待ちがローカル律速要因、認可チェック分散実装で漏れリスク → **改善**：新規案件は Drizzle ORM + `drizzle-kit generate/push` で 5秒反映＋`drizzle-zod` で Zod 自動派生、既存 Prisma プロジェクトは 6.2 の `$extends()` でグローバル認可注入し `where:{deletedAt:null, userId:ctx.userId}` を全モデル自動付与 → **期待効果**：ローカル DDL 反映 6倍高速化（30秒→5秒）、認可漏れの分散実装を集約1箇所化し OWASP API1 脆弱性の物理排除、実装行数20%削減
- **RPC 技術選定の判断軸明文化**: **現状**：tRPC・Server Actions・REST の使い分け基準が曖昧で新規案件毎に議論15分消費、Riku との型ズレが散発 → **改善**：判断軸を「呼び出し元が Next.js のみ＋社内ツール＝Server Actions／外部公開 API・型安全共有優先＝tRPC v11／モバイル・第三者連携＝REST + OpenAPI」で明文化し kai 要件整理段階で選定完了、tRPC v11 の動的ルーター型推論で仕様と実装の乖離を型レベル検出 → **期待効果**：技術選定議論15分→3分、Riku との型共有ゼロボイラープレート化、FE/BE 仕様ズレを型システムで自動検出しレビュー往復ゼロ化
- **PostgreSQL 17 の JSON+SQL ハイブリッド設計採用**: **現状**：JSON カラム利用時に「スキーマレス保持と検索性」がトレードオフで Mongo 併用検討、DB 種類増でトランザクション境界が分断 → **改善**：PostgreSQL 17 の JSON_TABLE 関数標準化＋GIN インデックス＋`jsonb_path_query` で「JSON カラム保持＋SQL 検索」ハイブリッド設計を採用、Mongo 併用廃止で RDB 単一運用 → **期待効果**：DB 種類数削減で運用コスト40%削減、トランザクション境界を JSON カラムまで拡張しデータ不整合ゼロ化、AI 駆動 SQL 最適化ツール（EverSQL/pganalyze）の恩恵を JSON にも適用
- **Supabase Edge Functions 統合**: **現状**：Supabase Auth・Vercel Functions・Route Handler が別ランタイムでコールドスタート合計200ms超、認可ロジックが分散 → **改善**：Supabase Edge Functions（Deno ベース）にバックエンド集約し RLS ポリシー＋Realtime＋Auth を単一プラットフォーム統合、`@supabase/ssr` で Next.js との Cookie 認証を共通化、Row Level Security で認可ロジックを DB レイヤーに落とす → **期待効果**：コールドスタート200ms→50ms、認可漏れを DB レイヤーで物理排除、Ao の Route Handler 実装量30%削減し Nao 設計フェーズにリソース再配分
- **OWASP API Security Top 10（2026版）の CI 自動化**: **現状**：セキュリティチェックが手動レビュー中心で見落としリスク＋レビュー工数60分/PR → **改善**：OWASP 2026 準拠の CI 自動チェック統合（API1: `checkUserOwnership()` AST 検査、API2: JWT `jose.jwtVerify()` ホワイトリスト、API3: DTO ホワイトリスト方式、API4: ページネーション・レート制限有無、API7: SSRF 対策 URL 許可リスト、API8: CORS/console.log ESLint、API10: 依存関係 CVE 検出）を pre-commit + CI で二重ゲート化 → **期待効果**：セキュリティレビュー60分→0分、本番リリース前脆弱性 100% ブロック、Mio の QA 工数を機能テストに集中
- **TDD Guard による強制テストファースト**: **現状**：TDD が個人の意思頼みで実装コード先行→テスト後付けが散発、カバレッジ50%止まりで refactoring 恐怖症発生 → **改善**：TDD Guard を pre-commit フックに統合し「テスト赤化なしの実装コード変更を物理禁止」、Vitest + `@anatine/zod-mock` + p-limit で正常/異常系 fixture 並列テスト、Mio との連携で「実装完了 = カバレッジ80% + 認可ペアテスト全 PASS」をマージ条件化 → **期待効果**：テストカバレッジ50%→80%以上を強制、バグ本番流出率70%削減、Ao の心理的安全性向上で refactoring 恐怖症消滅
- **JWT・Webhook セキュリティ二重防御**: **現状**：JWT の署名検証を自前 decode で済ませ、Webhook を署名検証なしで受信して認証バイパス・偽造リクエストのリスク → **改善**：`jose.jwtVerify()` で `algorithms`/`audience`/`issuer`/`exp`/`nbf` を必須検証＋JWKs エンドポイント TTL 10 分キャッシュ、全 Webhook で `stripe.webhooks.constructEvent(rawBody, sig, secret)` 署名検証必須化＋`event.id` 冪等キー、ESLint カスタムルールで `jwt.decode()` と Webhook の `JSON.parse(rawBody)` 単独使用を禁止 → **期待効果**：認証バイパス・Webhook 偽造事故を構造的にゼロ化、`alg: none` 攻撃対策、冪等キーで重複処理防止でデータ不整合排除
- **DB Connection Pool の設計チェックリスト化**: **現状**：Connection Pool 上限未指定で本番デプロイ時に `Too many connections` 全停止、Vercel Functions 同時実行数と DB max_connections の関係が不透明 → **改善**：DATABASE_URL に `?connection_limit=1&pool_timeout=10` 明示＋PgBouncer/Neon Pooler/Supabase Pooler 経由必須化、Vercel Functions 最大同時実行数 ×（1 + バッファ）が DB max_connections 内に収まる設計をチェックリスト化、`pg_stat_activity` で本番同時接続数を継続監視、Kuu との連携で本番デプロイ前チェックを CI 自動化 → **期待効果**：`Too many connections` インシデントゼロ化、コスト最適化（不要な max_connections 増強を回避）、Kuu との連携ハンドオフ時間15分→3分
- **PII 設計の nori 事前合意フロー確立**: **現状**：個人情報（応募者氏名・電話・履歴書）の削除フロー・保存期間が実装後追加で対応困難、nori リーガル NG が後戻り工数を大量発生 → **改善**：PII テーブル設計段階で nori と「保存期間・物理削除 or 論理削除・関連データのカスケード方針」を合意し設計書に明記、削除 API（本人請求対応）と保存期間超過の自動パージバッチを実装セットで着手、暗号化（AES-256-GCM）・ハッシュ化（bcrypt/argon2id）・エンコード（base64）の用語区別を Nao/nori と共通言語化、退会ユーザー再登録は部分ユニークインデックス（`WHERE deleted_at IS NULL`）で技術的負債を予防 → **期待効果**：個人情報保護法削除請求対応の実装後追加ゼロ化、リーガル NG による後戻り工数排除、退会ユーザー再登録クレームの構造的解消
