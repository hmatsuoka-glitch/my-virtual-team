...
### 2026-08-08
- **効率化テクニック：API冪等性（Idempotency）を`Idempotency-Key`ヘッダー＋Redis TTL 24h で全 POST/PATCH/DELETE 標準実装しネットワーク再送重複を構造排除**：`middleware/idempotency.ts` で `Idempotency-Key` ヘッダーからキーを抽出→ Redis に「キー→レスポンス本体」を24時間保持→ 2回目以降の同キーリクエストは処理せず前回レスポンスを返す設計。決済・応募・通知送信など重複が致命的な操作を全て網羅、Riku 側は `crypto.randomUUID()` でキー生成してヘッダー付与するだけ。従来「二重応募検知ロジック」を各エンドポイントに手実装していた工数消滅、失敗時の指数バックオフリトライも安心して実行可能（2026-07-11記録の冪等性用語を実装標準化）
- **品質チェックポイント：構造化ログ（structured logging）を `pino`＋`traceparent` W3C仕様で必須化し障害トリアージ時間 30分→3分**：全ログ出力を `logger.info({request_id, user_id, endpoint, duration_ms, status_code, ...})` の JSON構造で統一、`Traceparent` ヘッダー（W3C Trace Context仕様）で OpenTelemetry の trace_id/span_id を伝播、Kuu の Datadog/Grafana に流入させて「特定応募ID → 全ログ集約」を1クエリで抽出可能に。console.log 禁止 lint と PII マスキング関数（メール・電話は下4桁のみ）も必須。障害時のリクエスト追跡・N+1検出・遅延ボトルネック特定が劇的高速化（2026-08-03記録の可観測性要件を実装化、Kuu の observability 設計と直結）
- **失敗パターン：外部API（媒体連携・決済）呼び出しにリトライを実装したが、対象APIが非冪等（同一リクエスト2回で二重処理）で、リトライにより顧客に二重請求・二重通知が発生** → 回避策：外部API呼び出しに `retry-with-backoff` を実装する前に「対象APIが冪等か」を必ず契約確認、非冪等APIは自前でIdempotency-Key相当を実装（外部API側が対応不可なら「DB に処理済みフラグを立ててから呼び出す at-most-once」設計に切り替え）、リトライ回数・バックオフ倍率・最終失敗時のDLQ（Dead Letter Queue）投入を全て `retry-policy.yaml` で管理（理由：リトライは冪等前提の技法で、非冪等APIへの盲目的リトライは冪等性ないためどの層でも吸収不能）。実例：Stripe決済で500エラー時にリトライ→二重請求発生→冪等性確認プロセスを導入後ゼロ
- **TDD with Fixture Management：`gen-test-fixtures.ts` を Zod スキーマから自動生成し「テストデータ手作成 20分→0分」を全機能で実現**：Zod スキーマから `@anatine/zod-mock` で fixture（サンプルデータ）を自動生成、`create()`/`createMany()`/`createEdgeCase()`（境界値・エラーケース）の3種類を吐き出す。Mio へは fixture ファクトリを `packages/test-fixtures` で共有し、契約テスト・E2E・単体テストの全レイヤーが同一データ生成関数を踏む。Zodスキーマ1箇所修正で全レイヤーのfixtureが自動追従、Prisma seed とも共通化しローカル開発DBの初期データも同ソース化（2026-07-27記録のZod単一ソースを fixture 生成まで拡張）
- **連携ヒント：Kuu との「トランザクション境界と外部副作用」設計協働で Outbox Pattern を実装標準化**：応募登録＋通知送信＋媒体連携のような「DB書き込みと外部副作用」を1トランザクションで扱うと外部障害の巻き込みが発生するため、Ao が同一トランザクション内で `outbox` テーブルへイベント書き込み→ Kuu の CDC（Change Data Capture）または cron ポーリングで after-commit 配信する設計を標準化。Ao 側は `db.$transaction([applicationCreate, outboxInsert])` の atomic 保証だけに責任集中、Kuu 側は outbox の消化 worker（リトライ・DLQ・監視）を担当。二重処理・欠落なくメッセージ配信できる分散処理の定石を部内標準化（2026-08-03記録のOutbox Pattern を Nao 設計＋Ao実装＋Kuu運用の三者協働で確立）
