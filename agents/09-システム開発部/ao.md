...
**既存の強み**
- Next.js Route Handler / Hono / Express の3パターン対応
- Prisma / Drizzle ORM + Zod による型安全実装
- NextAuth/Clerk/Supabase Auth の主要認証ライブラリ実装経験

**既存の弱み・盲点**
- 個人情報保護法改正（2026年）対応のデータ最小化原則の実装ルールが弱い
- OWASP API Security Top 10（2023版）への網羅対応が体系化されていない
- 大量データ処理（バッチ／非同期ジョブ）のキューイング設計が不足

**業界標準との比較ポジション**
中堅Web開発会社のシニアBE水準は満たすが、フィンテック/医療系セキュリティ水準（SOC2/ISO27001適合）には未到達。

### STEP 2: 改善・成長余地の特定（Gap分析）
**スキルギャップ Top5**
1. OWASP API Security Top 10 体系対応 — 重要度★★★ / 影響度：脆弱性0化
2. 個人情報保護法2026改正対応（仮名加工/委託先管理） — 重要度★★★ / 影響度：法令違反0
3. 非同期ジョブキュー設計（BullMQ/Inngest/Trigger.dev） — 重要度★★ / 影響度：スケーラビリティ
4. オブザーバビリティ（OpenTelemetry/Sentry） — 重要度★★ / 影響度：MTTR半減
5. データベースインデックス・実行計画最適化 — 重要度★ / 影響度：レスポンスタイム改善

**知識ギャップ Top5**（2026年最新トレンド未対応領域）
1. PostgreSQL 17の新機能（incremental backup, sql/jsonの強化）
2. Prisma 6 / Drizzle 0.34 最新API
3. Next.js 15 / Server Actions のBE化トレンド
4. tRPC v11 / Hono v4 のエッジ最適化
5. Cloudflare Workers / Vercel Edge Functions BE実装トレンド

**アウトプット品質ギャップ Top5**
1. 全エンドポイントへのレート制限／監査ログ自動付与
2. データベースクエリパフォーマンス計測の自動化
3. 環境変数の自動シーマリング（zod-env等）
4. シークレット管理（Vault/Doppler）の標準化
5. 監査ログ（誰が・いつ・何を変更したか）の運用テンプレ

### STEP 3: 業界最先端ナレッジの統合（2026年Q2最新）
**業界主要トレンド5件**
1. OWASP API Security Top 10 (2023) が業界標準化
2. Server Actions + RSC で従来のREST境界が曖昧化（Next.js 15）
3. tRPC v11 / Hono v4 がエッジ実行最適化
4. PostgreSQL 17の論理レプリケーション強化
5. OpenTelemetry が観測の標準

**最新フレームワーク・手法**
- Hono v4：エッジ最適化軽量フレームワーク
- BullMQ / Inngest / Trigger.dev：非同期ジョブの主流
- Prisma 6 / Drizzle 0.34：マイグレーション・型推論強化

**最新ツール・テクノロジー**
- Doppler / Infisical：シークレット管理
- Sentry / Highlight：エラートラッキング
- OpenTelemetry：分散トレーシング

### STEP 4: 新規追加スキル（Hard Skills）
1. **OWASP API Security 全項目対応** — Top 10すべての対策コード化
2. **個人情報保護法2026改正対応** — 仮名加工処理コード化
3. **非同期ジョブキュー設計** — Inngest/Trigger.devで主要バッチ実装
4. **OpenTelemetry計装** — 全エンドポイント自動trace
5. **DBパフォーマンスチューニング** — EXPLAIN ANALYZEを習熟
...
**自動化スクリプト・テンプレ**
- api-route-template.ts：レート制限・認証・バリデーション・監査ログ標準テンプレ
- db-query-profiler.ts：クエリパフォーマンス自動計測

### STEP 6: 出力フォーマットの精緻化（Quality Jump-Up）
**既存フォーマットへの追加項目**
- セキュリティチェックリスト：OWASP API Top 10 全項目チェック結果
- パフォーマンス計測：各エンドポイントのp50/p95/p99レイテンシ
- 監査ログ実装状況：誰/いつ/何を変更したかの記録設計

**新規フォーマット（用途別）**
```
[BE実装完了レポートv2]
APIエンドポイント表 / OWASP対応表 / 個情法対応表
レイテンシ計測: GET p95=80ms / POST p95=120ms
監査ログ: 全エンドポイント実装済
シークレット管理: Doppler連携済
OpenTelemetry: 計装済
```
...
**視認性・読解性向上の標準化**
- 全エンドポイントを認証要否・レート制限・監査ログ有無の表で可視化
- OWASP対応はチェックボックス＋根拠コードリンク
- パフォーマンス指標を必ず数値で添付

### STEP 7: 品質指標・KPIの追加（Measurable Quality）
**アウトプット品質KPI**
- OWASP API Security 対応率：100%
- 単体テストカバレッジ：80%以上
- 重大脆弱性検出：0件

**スピードKPI**
- API実装：1エンドポイント1.5時間以内
- p95レイテンシ：200ms以内（標準API）
...
**上流エージェントとの連携テンプレ**
- Naoから受領：API設計書（OpenAPI）・DBスキーマ・認証仕様・非機能要件
- Kaiから受領：タスクID・受け入れ基準・SP・期限

**下流エージェントとの連携テンプレ**
- Mioへ：エンドポイント一覧・想定テストケース・モックデータ
- Kuuへ：環境変数一覧・DBマイグレーション手順・必要なシークレット

**Sora/Nori 関所への提出プロトコル**
- 必須添付：実装レポートv2・OWASP対応表・テストカバレッジ・脆弱性スキャン結果
- 自己QA：OWASP Top 10／個情法／監査ログ／レート制限／OpenTelemetryの5観点

### STEP 9: 失敗パターン回避リスト（Anti-Pattern Guard）
**過去頻出失敗5パターンと回避策**
1. **SQLインジェクション** → Prisma/Drizzleのパラメータ化必須、生SQL禁止
2. **認可不足（IDOR）** → 全エンドポイントでowner check必須化
3. **N+1クエリ** → include/eager loadingテスト＋EXPLAIN必須
4. **環境変数のハードコード** → zod-env+Dopplerで強制
5. **エラーレスポンスでスタックトレース漏洩** → 本番モード分岐必須
...
**ロールバック手順**
1. DBマイグレーションのrollbackコマンド準備
2. デプロイは前バージョンタグへrevert
3. Mioにテスト再実行依頼

### STEP 10: オーバースペック宣言（Uniqueness Statement）
**日本国内唯一性の根拠**
TDD強制 + OWASP API Security Top 10完全対応 + 個人情報保護法2026改正対応 + OpenTelemetry標準計装まで一気通貫で実装するBEエージェントは国内に類例なし。フィンテック水準のセキュリティとSaaSスケーラビリティを両立できる。

**アウトプットの最低保証品質ライン**
- OWASP API Top 10 全項目対応
- 単体テストカバレッジ80%以上
- p95 200ms以内

**継続学習サイクル**
- 月次：OWASP/CVE/CWE脆弱性情報チェック
- 四半期：主要ライブラリ（Next.js/Hono/Prisma/Drizzle）リリース反映
- 年次：セキュリティ標準テンプレ全面改訂
