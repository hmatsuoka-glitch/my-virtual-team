...
### 2026-08-30
- **Drizzle 0.35 + PostgreSQL 17のMERGE構文で「upsert+条件更新」を1クエリ化、冪等キー処理のクエリ本数を1/3に**：応募フォームの重複送信対策で従来「SELECT→INSERT/UPDATE」の3手番だった処理をPG17のMERGEで1手に集約。同時実行の競合窓が消え、実装コードもシンプル化。効果例：冪等処理レイテンシ90ms→35ms／トランザクション競合起因の失敗率0.3%→0.02%
- **PgBouncer + Supabase Session Pooler の使い分けをTerraformで宣言、コネクション枯渇を根絶**：短命なServerless関数はTransaction Pooler、常駐ワーカーはSession Poolerと運用ルールを Kuu と共有し、Prisma `connection_limit`を接続方式ごとに固定。トラフィックスパイク時のP99劣化が消える。効果例：接続枯渇起因の500エラー月10件→0件／ピーク時P99レイテンシ1.2s→380ms
- **Zod 4のブランド型（Branded Types）でPII/内部ID/連番受付番号を型レベル分離、混同事故を静的検出**：`type ApplicantId = z.string().uuid().brand('ApplicantId')`と`type ReceiptNo = z.string().regex().brand('ReceiptNo')`を分け、両者を混同したコードをコンパイル時にエラー化。受付番号にUUIDが混入する等の事故を根絶。効果例：ID系混同起因のバグ月2件→0件／レビュー時のID型指摘工数15分→0分
- **Turborepo Remote Cache + Vercel Blob 連携でCI/CD所要時間を大幅短縮、mio連携の統合テスト待ちが消滅**：TypeScriptビルド・Prisma Client生成・スキーマ検証をリモートキャッシュで共有し、mioのテスト実行待ちが最小化。7社横断のモノレポ運用でも各案件の変更だけを再ビルド。効果例：CI所要時間8分→90秒／PRマージからデプロイ完了まで25分→6分
- **OpenTelemetry + Vercel Observabilityの自動計装で「クエリ→API→FE」の一貫トレース、体感遅延の真因が即特定**：Kuu管理の`otel-collector`にPrisma/Fetch/Next.jsのspanを流し、遅延の内訳（DB/外部API/レンダリング）を1画面で可視化。「なぜ重いのか」の推測調査が消える。効果例：性能問題調査工数2時間→15分／体感遅延の真因特定率60%→95%
- **BMAD Architect Checklistの「セキュリティ」項目を Snyk + Semgrep 自動チェック化、OWASP Top10違反をPR段階で機械検出**：Nao設計時のセキュリティチェックを人手でなくCIで機械実行し、SQLインジェクション・XSS・認可漏れ・秘密情報漏洩をPR前に検知。着手前ゲートで漏れが構造的に消える。効果例：本番投入後のセキュリティ指摘月3件→0件／セキュリティレビュー工数45分→自動2分
- **Mio との「危険な境界の名指し申告」を`test-hints.md`ファイル化して自動渡し**：TZ境界・冪等キー重複・在庫競合・論理削除カスケード等の危険境界を、実装完了時に`__test-hints__/`配下へMD形式で自動配置。Mioはそれを起点にテストケースを生成し、境界バグの抜けが構造的に消える。効果例：QAすり抜け境界バグ月4件→0件／Mioのテストケース設計工数40分→10分
