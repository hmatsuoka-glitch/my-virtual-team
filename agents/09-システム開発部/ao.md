...
### 2026-09-10
- **[新スキル] Drizzle ORM 0.40（2026年1月GA）+ PostgreSQL 17 の pg_stat_io を活用した「クエリチューニング自動化」**：Drizzle ORM 0.40 の `explain` API で全クエリの実行計画を取得し、PostgreSQL 17 で新規追加された `pg_stat_io` ビューと突合して I/O ボトルネックを自動検出。従来 nao の設計→ao の実装→本番負荷試験で発覚していた N+1 やインデックス不足を、開発時点で `pnpm db:analyze` 1コマンドで検出。riku の Server Actions（2026-09-10参照）経由での DB アクセスも同ツールで解析可能
- **[新ナレッジ] Outbox Pattern + Change Data Capture（CDC）の統合手法（Debezium 3.0）**：2026-09-02 の Outbox（応募と外部送信のトランザクション整合性）をさらに発展させ、Debezium 3.0 の PostgreSQL CDC で Outbox テーブルの変更を Kafka/NATS へストリーミング。求人媒体連携・LINE通知・SMS通知の複数チャネルへ同一イベントを配信可能に。ジョブキュー（BullMQ v6）から CDC ベースへ移行することで、外部連携の追加時に ao の実装工数を 0.5人日→0.1人日（-80%）
- **[効率化ルール] `@let-inc/api-kit` の社内パッケージ運用に Changesets + Turborepo Remote Cache を導入**：パッケージ更新時の CHANGELOG 生成・バージョン付与・7社案件への配信を自動化。Kuu の CI と連携し、`@let-inc/api-kit` の更新が7社案件のビルドキャッシュを自動的に無効化して再ビルドを走らせる。手動配布作業（従来週1時間）→ 0時間
- **[失敗パターン→回避策] Server Actions と tRPC の共存時、riku（FE）が誤って Client Component から直接 Prisma/Drizzle を import してしまい、DBクレデンシャルがクライアントバンドルに漏洩する事故が発生**：回避策として、DB アクセスは必ず `app/server/db.ts` の `"server-only"` パッケージを import したモジュールに限定し、ESLint plugin `eslint-plugin-import` の `no-restricted-paths` で `app/(client)/**` からの DB モジュール import を完全ブロック。CI で必須チェック
- **[連携強化] 07-LP部 tsumugi の Situational Job セグメンテーション（tsumugi 2026-09-10参照）を CRM/ATS 連携仕様に統合**：LP の応募データを HubSpot/Salesforce へ連携する際、Situational Job（6セグメント）を `custom_field` として送信することで、クライアント側の CRM 上で応募者のペルソナ別に自動セグメント化される。ao はこの連携仕様を tsumugi の LP STEP 0 で確定させ、実装工数を要件段階で見積もる
- **[品質チェック] Sora QA 前セルフチェック**：(1) 全 API エンドポイントに対して Zod スキーマバリデーション・相関ID貫通・冪等キー対応の3項目が実装されているか、`@let-inc/api-kit` の型チェックで自動検証済みか (2) DB クエリの実行計画を Drizzle `explain` で全てチェックし、N+1 や Seq Scan がゼロであること、`pg_stat_io` で I/O ボトルネックが検出されていないか
