...
### 2026-07-21
- **TDD Guard の pre-commit フック導入で「テスト後追い」を物理不可能化**：現状=実装先行 PR が月 3〜4 件、テスト後追いで Mutation Score 45% と低迷しロジックバグが本番流出→改善=`test.only`/`describe.only`/空 `catch{}`/`waitForTimeout`/未使用モックを pre-commit hook で機械検出しコミット即 block、Red-Green-Refactor の Red 先行をコミット単位で強制→期待効果=Mutation Score 45%→65% 到達、テスト後追いによる偽陰性ゼロ化、Riku/Ao のテスト設計スキル自動底上げで実装品質全体が向上
- **Vitest 3 移行とブラウザモードでの Intl / DOM API 依存バグの単体層検出**：現状=jsdom で `Intl.Segmenter`/`ResizeObserver`/`IntersectionObserver` が undefined になり、絵文字文字数カウント・スクロール連動 UI のバグを単体層で拾えず E2E で初検出→改善=Vitest 3 ブラウザモード（Chromium/WebKit）で単体テストを実ブラウザ実行、`.browser-only` ディレクトリで DOM 依存テストを隔離実行→期待効果=単体層で拾えるバグが 30% 増、E2E 層の負荷減で full run 12 分→9 分、日本語ユーザー特有の絵文字/濁点合成バグの本番流出ゼロ
- **Playwright storageState + 認証済みセッション事前生成で E2E 実行時間を 3 分の 1 に**：現状=100 本の E2E 各テストで UI ログイン 15 秒×N が積み上がり、full run で 25 分→ローカル実行が省かれフィードバック遅延→改善=`global.setup.ts` で各ロール（管理者/一般/別テナント）を 1 度だけログインし `storageState.json` に保存、全テストは認証済みから開始、認可ペアテストは「別ロールの storageState を差し替え」で自動生成→期待効果=full run 25 分→8 分、ログインフロー Flaky 消滅、認可ペアテスト網羅工数ゼロで OWASP A01 検出率 100% 維持
- **Stryker Mutator v9 の nightly 常設 + 朝の Slack 投稿で「弱いアサーション」を継続改善**：現状=Line カバレッジ 82% を達成しているのに本番ロジックバグ月 2 件流出、「緑だから OK」文化で検証品質が測れず→改善=StrykerJS nightly ジョブで Mutation Score 計測、「弱いアサーション TOP 3」と Mutation Score 前日比を `mio-quality` Slack へ自動投稿、Mio が朝レビューで補強箇所を Riku/Ao へフィードバック→期待効果=Mutation Score 45%→65% 到達、本番ロジックバグ 月 2 件→0.5 件、カバレッジ盲信文化の物理排除
- **Coverage.py Branch mode を Python バックエンド全リポジトリに導入し FE と統一基準**：現状=BE 側は Line カバレッジのみ計測、`if a and b:` の片側通過で 100% 表示され条件系バグを見逃し→改善=Coverage.py の `--branch` オプション＋pytest-cov で Branch 85% を CI 必須ゲート化、FE の Vitest Branch 85% と揃えて全社統一→期待効果=Branch カバレッジ全社統一 85%、条件系バグ検出率 2 倍、FE/BE で「カバレッジの意味」の会話ズレ解消
- **Testcontainers（Postgres/Redis/MinIO）による本物依存の統合テスト常設化**：現状=Prisma を `mockDeep` でモックし本物 DB との契約違反（型・NOT NULL 制約・トリガ・トランザクション境界）を統合層で拾えず本番検出→改善=`@testcontainers/postgresql` で PR 毎に本物 Postgres 起動、Redis / MinIO（S3 互換）も同様に起動し統合テストを実依存で実行→期待効果=本番 DB 契約違反バグ検出率 100%、モック手書き工数 20 分/エンドポイント→ゼロ、Ao の OpenAPI 変更が即 CI で fail し FE-BE 乖離ゼロ
- **Contract Testing（Schemathesis）で OpenAPI ⇔ 実装の差分を PR 単位で自動検出**：現状=Ao の OpenAPI 更新に FE msw モックが追従せず、月 1 で契約違反が本番検出（レスポンス形式ズレ・型不一致）→改善=Schemathesis を CI に組込、OpenAPI schema の全エンドポイントに対し正常系/異常系/境界を自動生成して実サーバに撃込、契約違反は即 fail。`openapi-msw` で FE モックも自動生成し二重管理排除→期待効果=契約違反本番流出ゼロ、FE-BE 間の仕様乖離検出遅延 週 1→PR 単位、Ao 仕様更新と Mio モック追従の同期自動化
- **AI Test Generation（Claude）による Gherkin → Vitest/Playwright ひな型自動生成で受入基準トレーサビリティを構造担保**：現状=Nao の受入基準から手書きでテストひな型を作成、1 機能 60 分、要件⇔テストの対応漏れが月 2〜3 件発生→改善=`.feature` ファイル（Given-When-Then）を SSOT にし、Claude API 経由で Vitest（`vitest-cucumber`）と Playwright（`playwright-bdd`）両方のひな型を 1 コマンド生成、Mio は assertion 詰めと戦略判断に集中→期待効果=テスト骨格作成 60 分→5 分/機能、要件⇔テストのトレーサビリティ自動担保、受入基準変更時の両層同時追従で乖離ゼロ
- **Property-Based Testing（fast-check）を金額・日付・シリアライズの純粋関数に常設**：現状=`toBe(0.3)` の丸め誤差・タイムゾーン跨ぎ・NFC/NFD 濁点合成の反例を人が思いつかず本番検出、採用課金の 1 円ズレでクライアント信頼失墜リスク→改善=`fast-check` で純粋関数に property テストを常設、「往復変換で元に戻る」「金額合計は入力の総和と一致」「日付境界を跨いでも件数保存」等の性質を機械検証、seed 固定で決定的実行→期待効果=境界系バグの本番流出 月 3 件→0 件、`0.1+0.2` 型の丸め・IEEE754 反例を自動発見、金額系の信用直撃事故ゼロ
- **Playwright 1.50 の AI Auto-Healing + セレクタ変更 self-healing で E2E 保守工数削減**：現状=Riku の UI 変更で `#submit-btn`→`[data-testid="submit"]` セレクタ変更、5 テストが同時に赤化して調査 30 分、E2E 保守が Riku/Mio の往復ボトルネックに→改善=Playwright 1.50 の `auto-heal` 機能を有効化し AI がセレクタ変更を自動推論、`warning` ログで Mio が事後に「意図した変化か」を確認する運用ルール化、`data-testid` 命名規約を Riku と共有→期待効果=セレクタ起因 Flaky 対応工数 70% 削減、Riku の UI 改修→Mio の E2E 保守の往復消滅、AI 誤推論による偽陰性は warning ログ 100% 目視でカバー
