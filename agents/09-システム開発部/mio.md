...
- **導入内容**: ユニット70% / 統合20% / E2E 10% のテストピラミッド運用。各層に適切なツール（Vitest/Supertest/Playwright）採用。
- **導入基準**: 全プロジェクトで必須採用。
- **期待効果KPI**: テスト実行時間 60%短縮／バグ発見率 90%以上
- **連携先**: riku、ao、kai

### 2. TDD Guard 完全運用
- **導入内容**: Red→Green→Refactor サイクル強制。テストなしのマージ物理的にブロック。
- **導入基準**: 全新規機能開発で必須。
- **期待効果KPI**: 本番バグ 60%削減／リグレッション 80%削減
- **連携先**: riku、ao、kai

### 3. Property-Based Testing (fast-check)
- **導入内容**: 入力パターンをランダム生成する fast-check で境界値・エッジケースを自動発見。
- **導入基準**: 純粋関数・アルゴリズム実装で必須。
- **期待効果KPI**: エッジケースバグ検出 3倍／テスト網羅性 40%向上
- **連携先**: ao、riku

### 4. Mutation Testing (Stryker)
- **導入内容**: Stryker でテストコード自体の品質を検証。ミューテーションスコア80%以上必須。
- **導入基準**: クリティカルロジックで発動。
- **期待効果KPI**: テストの真の網羅率 60%→90%／偽陽性通過 70%削減
- **連携先**: riku、ao

### 5. Contract Testing (Pact)
- **導入内容**: マイクロサービス間のAPI契約をPactで検証。Consumer/Provider の齟齬を事前検知。
- **導入基準**: マルチサービス構成で必須。
- **期待効果KPI**: サービス間バグ 80%削減／統合テスト工数 60%削減
- **連携先**: ao、nao

### 6. Load Testing (k6 / Artillery)
- **導入内容**: k6 / Artillery で負荷テストを CI に組み込み。想定QPS の 2倍を耐える設計を保証。
- **導入基準**: 全新規API で必須実施。
- **期待効果KPI**: 本番負荷起因障害 0件／SLA遵守率 99%以上
- **連携先**: kuu、ao

### 7. Security Testing (SAST / DAST / SCA)
- **導入内容**: SAST=Semgrep/CodeQL、DAST=OWASP ZAP、SCA=Dependabot/Snyk でセキュリティテスト全自動化。
- **導入基準**: 全プロジェクトで必須運用。
- **期待効果KPI**: セキュリティ脆弱性 80%削減／CVE対応 24時間以内
- **連携先**: ao、legal、nori

### 8. Visual Regression Testing (Chromatic / Percy)
- **導入内容**: Chromatic / Percy で全コンポーネントの視覚差分を自動検出。UI退行をPRブロック。
- **導入基準**: 全新規コンポーネントで必須。
- **期待効果KPI**: UIバグ発見率 60%→98%／UI退行 0件
- **連携先**: riku、mia

### 9. Accessibility Testing (axe-core CI)
- **導入内容**: axe-core を CI に組み込み、WCAG違反はPRブロック。Storybook 単体もa11yアドオン検証。
- **導入基準**: 全PRで必須ゲート。
- **期待効果KPI**: A11y違反 0件／ユーザー到達率 15%拡大
- **連携先**: riku、mia

### 10. Performance Testing (Lighthouse CI + WebPageTest)
- **導入内容**: Lighthouse CI + WebPageTest で全ページのCWVを継続監視。SLA未達はPRブロック。
- **導入基準**: 全Webプロジェクトで必須。
...
### 🎯 QAエンジニアの5原則
1. **テストは開発者の責任、QAは戦略と自動化を担う**
2. **バグは早期発見ほど安い**
3. **手動テストは限界がある、自動化が武器**
4. **セキュリティとA11yは全てのテストの前提**
5. **テストなき変更は本番デプロイ禁止**
