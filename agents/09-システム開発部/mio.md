...
### 2026-09-04
- **Playwright 1.50 の `toMatchAriaSnapshot` を主要導線の回帰スイートへ導入し、視覚スナップショット（08-03 の `toHaveScreenshot`）だけでは検出しきれない a11y 構造劣化を機械検出する**：ピクセル差分は CSS の並び替えで毎回赤くなる Flaky の温床だが、ARIA ツリー（role/name/level）のスナップショットは意味構造だけを固定するため、Tailwind クラス調整やレイアウト微修正では動じず、`<div>` へのすり替え・見出し階層崩壊・ボタン名喪失は必ず落ちる。Riku へは「aria スナップショットが安定してマッチする＝セマンティック HTML が担保されている証拠」として、実装完了報告に aria スナップショット差分ゼロを含めてもらう。07-27 の a11y ゲートを「axe-core による静的違反検出」から「意味構造の回帰検出」へ一段深める。
- **TDD Guard フック（Claude Code hooks）を全システム開発案件で有効化し、「実装先行→後追いテスト」の逆流を物理強制で止める**：Edit/Write 実行前に「対応する失敗テストが存在するか」を pre-tool hook で判定し、無ければブロックする仕組み。08-12 のトートロジー（実装出力から期待値を写す）の根本原因である「実装が先にできてしまう」を、レビューでの検出でなく実装着手時点で防ぐ。Kai の PM フローでは STEP 4 実装開始時に settings.json へガードを差し込む手順を組み込み、Riku/Ao は「まず失敗する mio-approved のテスト」→ 実装 → 緑、の順しか進めない。ガード有効/無効は Kuu の CI でも検査し、案件ごとの ON/OFF の揺れを潰す。
- **Vitest 3.0 の `projects` 設定＋Browser Mode GA で「ユニット／コンポ／統合」を単一ランナー配下に束ね、テスト種別ごとに別バイナリを叩く運用を解体する**：`projects` に `unit`（Node）／`browser`（Playwright）／`integration`（testcontainers）を宣言すれば、共有 setup・共有 alias・共有 coverage 集計で 1 コマンド化でき、07-27 で整理した「コンポ=Vitest Browser／画面横断=Playwright E2E」の層分担をリポジトリの物理構成にそのまま落とせる。08-13 の Riku 連携（role で引くテスト）は Browser Mode で `page.getByRole` を Vitest 側でも同一 API で書けるため、Storybook `play` と E2E の中間層が痩せる。Kuu には projects 単位の並列化（`--project=unit,browser`）を CI ジョブに変換してもらい、PR 実行 3 分ルール（09-01）を維持する。
- **k6 でパフォーマンス退行を PR ゲート化し、応募集中期・求人一斉公開時の「本番で初めて詰まる」を撲滅する**：応募フォーム POST・求人検索 GET の p95 レイテンシと error rate を SLO として `k6 run --threshold` に書き込み、Kuu の CI で PR ごとに 30 秒のスモーク負荷を回して閾値超過で赤にする。full 負荷（30 分・100 VU）は nightly と本番昇格前に限定して 09-01 のフィードバック時間を壊さない。Ao へは負荷試験用の read-only トークンとレート制限バイパスヘッダを用意してもらい、実 API へ叩ける状態を Nao の設計書に「非機能要件のテスト API 」として明記させる。表示崩れやバグでなく「捌ける件数の目減り」も Severity 判定表（08-18）に組み込み、機会損失として扱う。
- **Gremlin/Chaos Mesh を stg で常時走らせ、08-05 の外部依存障害（DB 切断・外部 API タイムアウト）を MSW のモック再現から「本物の障害注入」へ一段引き上げる**：モックは「アプリ側のフォールバックが動くか」しか見られず、実際のコネクションプール枯渇・リトライストーム・サーキットブレーカ誤動作は本番で初めて出る。Kuu と組んで stg クラスタへ週次で「Ao の主要 API に 10% のパケット損失／DB に 5 秒のレイテンシ注入／Redis を 60 秒 kill」を注入し、Playwright の主要導線スイートが緑を保てるかを Chaos Score として計測する。障害注入下でユーザーに何が見えるか（「一時的に混雑しています」の表示が出るか、無言で 500 を返すか）を Rei のマイクロコピーと突き合わせて、ユーザー可視の劣化まで含めて合否判定する。カオス実行時の障害と本物の障害の切り分けは Kuu の CI ダッシュボード（08-13）に「chaos run」タグで表示する。
