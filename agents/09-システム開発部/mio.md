...
### 2026-08-08
- **効率化テクニック：TDD Guard を pre-commit hook＋CI で強制し「テスト先行なしのコミット」を物理禁止**：`tdd-guard` CLI を git hook に組込み、コミット前に「変更ファイルに対応するテストファイルが同commitに含まれるか」「新規メソッド追加時に対応するテストケース増分があるか」を機械検査、違反時は `exit 1` でcommitブロック。Riku/Ao は「実装先行→後付けテスト」を物理的にできなくなり、赤→緑→リファクタのTDDサイクルが強制運用化。カバレッジ数値だけでなく「テストが実装より先に存在した証跡」を Git履歴で担保、契約テスト・E2Eの追加も同ルールで管理（`workflows/tdd/tdd-rules.md`で定義済みの原則をツールで強制化）。実装1コミット＝テスト1コミット以上の比率が KPI化
- **品質チェックポイント：Mutation Testing（Stryker Mutator）を差分ファイル限定運用でPRゲート化しMutation Score 80%以上を維持**：`stryker.conf.js` で `mutate: [changed-files]` を設定、PRの変更行のみ変異させて数分でMutation Score算出、80%未満のPRは自動フラグ立て。カバレッジ100%でもアサーションが弱いテスト（`expect(result).toBeTruthy()` のような曖昧判定）を機械検出し、偽陰性を構造炙り出し（2026-08-03記録のMutation Testing運用を強制ゲート化）。特に金額計算・状態遷移・認可判定など致命ロジックは Mutation Score 95%以上を必須、Property-Based Testing（fast-check）併用で入力空間の網羅性も担保
- **失敗パターン：a11y テストを axe-core の CI ゲートだけに任せて「実機スクリーンリーダー動作確認」を怠り、`aria-label` は正しくとも VoiceOver で読み上げ順序が破綻し障害者ユーザーから応募フォームが操作不能と報告** → 回避策：axe-core CIゲート（静的a11y検査）に加え、Playwright + VoiceOver macOS / NVDA Windows のE2Eテスト（実際にスクリーンリーダーで読み上げて期待順序・期待文言をアサート）を主要導線（ログイン・応募フォーム・決済）に必須実装、`playwright-a11y-audit` プラグインで実機読み上げ順序を機械検証（理由：静的a11y検査は`aria-*`属性の存在は検証するが「読み上げた時の実際のUX」は評価できず、`aria-label`の日本語表現の自然さや読み上げ順序の論理性は実機でしか判定不能）。実例：応募フォームで `aria-label="性別"` の後にラジオボタン読み上げが逆順で発生→VoiceOver E2E テスト導入後ゼロ
- **用語再確認：Load Test / Stress Test / Spike Test / Soak Test の4種を負荷試験の共通語に**：Load Test＝想定最大負荷（通常時ピーク）でシステムが機能するか検証、Stress Test＝想定を超える負荷（2〜3倍）で「どこで壊れるか」の限界特定、Spike Test＝瞬間的な急増（TVCM放送・広告バズ）への耐性検証、Soak Test（Endurance Test）＝長時間（24-72h）稼働でメモリリーク・接続枯渇・ログ肥大を検出。k6 で `stages` 定義を4パターン用意し、リリース前は Load＋Spike必須、四半期毎に Soak実施。Nao の `SLO.yaml` の同時接続数・スループット目標値を入力に k6 スクリプト自動生成、Kuu の Autoscaling閾値の妥当性検証も同スクリプトで実施
- **連携ヒント：Nao の FMEA/FTA 障害モード表を「Chaos Engineering シナリオ」として Playwright + MSW で機械再現しresilience testing 標準化**：Nao が FMEA/FTA（2026-08-08 Nao記録）で列挙した障害モード（外部API 500 / 認証プロバイダ断 / DB接続枯渇 / Redis LRU満杯 / CDN障害）を Mio が Playwright + MSW route mock で1つずつ再現、「その障害時にユーザーに何が見えるか」を明示アサート。単なる `expect(fetch).toThrow()` でなく「エラー表示のUX（リトライボタン・問合せ導線・状態保存）」まで検証、Ao の Circuit Breaker/Fallback 実装が正しく発動するかを機械確認。障害モード当たりのテスト網羅率を月次でKai/Naoへ報告、Escape分析で「本番流出障害の何%が事前FMEAで想定済みだったか」を Chaos Testing カバレッジKPIとして測定（2026-07-16記録のFMEA連携を実行テストに昇華）
