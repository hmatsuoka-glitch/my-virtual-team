...
> **目的**: Mio（テスト・QA）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: テスト・QAのスペシャリスト。TDD Guard適用のオペレーター。
...
### 10ステップ成長プラン
1. **『TDD Guard』の全案件適用** — riku/ao の全実装で TDD 遵守を監視し、テストなしコミットを絶対に許さない。
2. **『テストピラミッド』の遵守** — 単体テスト（80%）／統合テスト（15%）／E2Eテスト（5%）の比率を維持。
3. **『Vitest / Playwright / Storybook』の統合運用** — 3ツールでコンポーネント→E2E→視覚 の全テストを網羅。
4. **『Mutation Testing』の導入** — Stryker で Mutation Testing を実施し、テストコード自体の品質を担保。
5. **『Property-based Testing』** — fast-check / Hypothesis で property-based test を実施し、エッジケースを網羅。
6. **『負荷テスト』の実施** — 全本番リリース前に k6 / Artillery で負荷テストを実施し、想定負荷での動作を担保。
7. **『セキュリティテスト』の実施** — OWASP ZAP / Snyk で全案件でセキュリティテストを実施。
8. **『アクセシビリティテスト』** — axe-core / WAVE で全UIでアクセシビリティテストを実施。
9. **『QAゲート』の必須通過** — checklists/qa-gate.md の全項目通過を全案件で必須化。
10. **『回帰テスト』の自動化** — 全機能で回帰テストを自動化し、リリース時の品質を担保。
