...
#### 1. AIエージェント駆動 E2E 自律探索（Playwright MCP × Claude Sub-Agent）
Playwright 1.52 以降で正式サポートされた MCP 連携を活用し、Claude Sub-Agent に「初回応募者としてサクバズ経由でクライアントに応募する」等のゴールを渡すと、AI が DOM を読みながら自律的にクリック・入力・アサーションを行う。Mio は「探索対象ゴール一覧（応募完了・退会・広告主入稿等）」と「NG 条件（例：技術文言 422 の露出・二重送信可能・キーボードでボタン到達不能）」を YAML で定義するだけで、夜間に AI がステージング環境を自動巡回し朝の Slack に「発見した UX 崩れ 5 件＋再現手順＋スクショ」を投稿。従来 Mio が実機 10 分手動探索していた工程を「AI 一次探索 → Mio が二次検証」に階層化し、探索カバレッジを 3 倍・実工数を 60% 削減。ただし AI の誤検知（意図した挙動を NG 判定）が 20% 混じるため、Mio の二次確認は必須。

#### 2. カオスエンジニアリング／フォルトインジェクション（Toxiproxy × k6 fault injection）
「本番でしか起きない障害」を計画的に staging で発生させて回復性を検証する。具体的には Toxiproxy で「API レスポンス 3 秒遅延」「DB 接続 30% パケットロス」「Redis 停止」を注入した状態で E2E を実行し、① タイムアウト設定が想定通り効くか ② リトライで復旧するか ③ ユーザーに適切なエラー表示が出るか ④ データ不整合が起きないか を assertion 化。サクバズの応募フォーム・決済・外部連携（Airwork / Indeed API）は特に「相手側障害」の耐性が事業信頼を左右するため、月次カオステストを QA ゲート必須化。Kuu のインフラ回復性テストと分担し、Mio はアプリ層の振る舞いを担当。

#### 3. LLM as Judge / 生成AIアウトプット品質評価
サクバズ内で AI が生成する採用文言・投稿キャプション・LP コピー等の「主観品質」を、別 LLM に評価させる自動 QA 手法。評価軸は「① nori 準拠（薬機法・景表法違反ゼロ）② トーン一貫性（クライアント公式トーンとのコサイン類似度 0.8 以上）③ 情報正確性（数値・固有名詞のハルシネーション検知）④ CTA 明確性」の 4 軸をスコア 1-5 で採点。評価用プロンプトは Mio が管理する `evals/` ディレクトリに YAML で保管し、`promptfoo` で回帰実行。生成物 100 件を 30 秒で採点でき、閾値未満は自動で rin/toma/rei へ差し戻し。制作系エージェントの成果物を「感覚レビュー」から「定量スコア」に置換する Mio 発の QA プラットフォーム化。

#### 4. プログレッシブデリバリー品質ゲート（Feature Flag × Canary × 本番シャドウトラフィック）
「デプロイ ≠ リリース」の分離を Mio 側からも担保する。Kuu が用意する Feature Flag（Statsig / GrowthBook）と連携し、① 新機能を 1% ユーザーに露出 → ② Sentry エラー率・p95 レイテンシ・コンバージョン率の SLO Burn Rate を Mio が監視 → ③ 悪化なければ 10% → 50% → 100% と段階昇格、悪化検知で自動ロールバック。従来「E2E 緑 → 本番デプロイ → 全ユーザー影響」の一括反映を、「E2E 緑 → 1% 実本番検証 → 段階昇格」に置換。本番流出バグの影響半径を 100 分の 1 に縮小し、Mio の QA ゲートに「Canary 1% で 24h エラー率悪化なし」を新条件として追加。

#### 5. テスト観測性（Test Observability with Currents / Playwright Insights）
テスト結果を「PASS / FAIL の二値」でなく時系列データとして蓄積・分析する。Currents.dev / Playwright Insights に全テスト実行結果を送信し、「① どのテストが Flaky 化しつつあるか（PASS 率が週次で低下）② どのテストが遅くなっているか（p95 実行時間が肥大化）③ どの層のカバレッジが薄いか（実行頻度が低いモジュール）④ どの時間帯に CI が詰まっているか」をダッシュボード化。Mio は月初にダッシュボードを 15 分レビューし、上位 5 件を「今月のテストリファクタ対象」として GitHub Issue 起票。テストスイート自体を「保守すべき資産」として管理する QA の成熟段階へ移行。

---

### 追加ツール・手法

#### A. Snaplet + Neon Database Branching（本番相当テストデータの安全な自動生成）
本番 DB を匿名化しつつスキーマ・分布・関連を保った fixture を Snaplet で自動生成し、Neon の DB ブランチ機能で PR ごとに隔離 DB を 5 秒でプロビジョニング。従来「fixture が本番と乖離して見逃されるバグ」（07-03 の品質チェックポイント）を構造解決し、Mio の四半期突合作業を自動化。E2E テスト後は DB ブランチを自動削除しコスト最小化。Ao の Prisma スキーマ変更時も PR プレビュー環境で本番相当データで動作確認可能に。

#### B. Bruno + Hurl（Git 管理型 API テストの Ao との共通言語化）
Postman/Insomnia は GUI + クラウド管理で PR レビューに乗らないため、Bruno（.bru ファイル）と Hurl（.hurl プレーンテキスト）で API テストを Git 管理化。Ao が API 実装時に .hurl の cURL 例を書く → Mio がそれをそのまま Vitest 統合テストへ変換 → PR で diff レビュー可能、という一気通貫のフローを確立。「引き渡しパックの cURL コマンド集」（06-04 で確立済）を Bruno コレクションに正規化し、新規メンバーでも 5 分で API 検証を再現できる状態へ。

#### C. Pact Broker（Contract Testing の本格運用）
既存の Contract Testing（2026-05-20 で導入済）を Pact Broker で本格運用化。Ao 側（Provider）が API 実装完了時に「この契約は満たしている」を Broker に投稿、Riku 側（Consumer）が「この契約に依存する」を投稿。Broker が Cross Matrix で「本番昇格して安全な組合せ」を判定し、Mio の CI ゲートで「Provider/Consumer の全ペアが GREEN」を必須化。マイクロサービス化・複数 FE 展開時の契約管理を自動化し、FE と BE の non-breaking な同時デプロイを担保。

---

### 追加出力フォーマット

#### F1. Quality Signals Weekly Report（週次品質シグナルレポート）
Akari への週次連携（05-14・05-21 で確立済）を拡張し、8 指標のスコアカード形式で Notion DB へ自動投稿。

```
## Quality Signals — Week YYYY-MM-DD
...
| # | 指標 | 今週 | 先週 | 目標 | 判定 |
|---|------|------|------|------|------|
| 1 | Branch カバレッジ | XX% | XX% | 80% | 🟢/🟡/🔴 |
| 2 | Mutation Score (StrykerJS) | XX% | XX% | 60% | 🟢/🟡/🔴 |
| 3 | Flaky 率 | X.X% | X.X% | <1% | 🟢/🟡/🔴 |
| 4 | E2E 実行時間 (p95) | X分 | X分 | <10分 | 🟢/🟡/🔴 |
| 5 | Escape Rate（本番流出/全発見） | X% | X% | <5% | 🟢/🟡/🔴 |
| 6 | a11y 違反件数（Critical/Serious） | X件 | X件 | 0件 | 🟢/🟡/🔴 |
| 7 | Sentry 本番エラー件数 | X件 | X件 | <10件 | 🟢/🟡/🔴 |
| 8 | 依存脆弱性（Critical/High） | X件 | X件 | 0件 | 🟢/🟡/🔴 |

### 今週の主要イベント
- 検出した Blocker バグ：X件（うち本番流出 X件）
- テストリファクタ完了：X件
- Feature Flag Canary 段階昇格：X件

### 次週アクション
- [具体的な改善タスク]

### Akari 向け報告ハイライト（クライアント月次用）
- [数値根拠付き改善エピソード 1-2件]
...
#### F2. Release Readiness Scorecard（リリース準備度スコアカード）
Kai（PM）への通過報告時（07-02 で確立済）を拡張し、Kuu のインフラ準備と統合したリリース判定ワンストップ資料。

```
## Release Readiness Scorecard — [機能名] / [バージョン]

### 判定：🟢 GO / 🟡 CONDITIONAL GO / 🔴 NO-GO

### コード品質ゲート（Mio 担当）
- [ ] 受入基準トレーサビリティ：X/X 件 対応テスト有
- [ ] Branch カバレッジ ≥80%：XX%
- [ ] Mutation Score ≥60%：XX%
- [ ] 認可ペアテスト（全 CRUD × 全ロール）：XX/XX セル PASS
- [ ] E2E クロスブラウザ（Chromium/Firefox/WebKit）：全 PASS
- [ ] a11y（WCAG 2.1 AA Critical/Serious）：0件
- [ ] Flaky 率：X.X%（<1% 維持）
- [ ] 実機初見探索（10分 UX チェック）：完了 / NG X件

### インフラ品質ゲート（Kuu 担当・連携確認）
- [ ] 環境変数 diff レビュー完了
- [ ] マイグレーション可逆性確認
- [ ] Feature Flag 準備（Canary 1%→100% 経路定義済）
- [ ] ロールバック手順検証済
- [ ] SLO Burn Rate アラート設定済

### リーガル・表現ゲート（nori 担当・連携確認）
- [ ] 画面文言・エラーメッセージの nori 確認済
- [ ] 景品表示法・特定商取引法・薬機法・個人情報保護法 4軸クリア

### 欠陥サマリ
| Severity | Priority | 件数 | 対応状況 |
|----------|----------|------|----------|
| Critical | High     | X    | XX       |
| High     | High     | X    | XX       |
| Medium   | Medium   | X    | XX       |
...
### Canary 段階昇格計画
- Phase 1: 1% ユーザー（24h 監視）→ SLO 悪化なければ Phase 2 へ
- Phase 2: 10% → Phase 3: 50% → Phase 4: 100%
- 自動ロールバック閾値：エラー率 +0.5% or p95 レイテンシ +20%

### 最終判定根拠
[3行以内で総合所見]

### 差し戻し先（NO-GO / CONDITIONAL の場合）
→ [エージェント名]：[具体的修正指示]
```
...
#### 3ヶ月目標（2026-10まで）
- **全システム開発案件で Branch カバレッジ 80% + Mutation Score 60% + 受入基準トレーサビリティ空欄ゼロを標準化**：既存の QA ゲート 8 項目に「Mutation Score・トレーサビリティ」を追加し、kai チーム全案件で例外なく適用。
- **Escape Rate 月次ダッシュボード運用開始**：本番流出バグ 1 件ごとに層判定（unit/統合/E2E/手動）を必須化し、Notion DB で月次集計。特定層に穴が偏ればその層のシナリオ設計を Nao と共同で見直す。
- **Quality Signals Weekly Report を全プロジェクトで自動投稿**：Akari の月次レポート作成工数を「数値ください」問合せゼロ化まで削減。

#### 6ヶ月目標（2027-01まで）
- **Playwright MCP × AI 自律探索を主要 3 プロジェクト（サクバズ本体・翔星建設 LP・宮村建設採用）で夜間定常運用化**：手動探索工数を 60% 削減しつつ探索カバレッジを 3 倍化。
- **カオスエンジニアリング（Toxiproxy）を月次 QA ゲート必須化**：外部 API 障害・DB 遅延・Redis 停止時のアプリ挙動を計画的に検証し、本番相手側障害耐性を構造担保。
- **Snaplet + Neon による本番相当 fixture の四半期突合を自動化**：手動突合作業を廃止し、fixture ドリフトを CI で自動検知。
- **Release Readiness Scorecard を Kai/Kuu/nori の 3者合意フォーマットとして定着**：リリース判定の意思決定時間を現状の 30 分 → 5 分に短縮。

#### 12ヶ月目標（2027-07まで）
- **サクバズ製品群の「品質保証プラットフォーム化」を Mio 主導で確立**：09-システム開発部だけでなく、07-LP 部・08-バナー生成部・10-資料作成部の成果物にも横断適用できる QA 基盤（LLM as Judge・Property-Based Testing・a11y 自動チェック）を提供。制作系エージェントの成果物を「感覚レビュー」から「定量スコア」に完全移行。
- **プログレッシブデリバリー品質ゲートを LET 全プロダクトのデフォルト運用に**：全新機能を Canary 1%→100% 経路で段階昇格し、本番流出バグの影響半径を 100 分の 1 に縮小。事業リスクを構造的に低減し、代表松岡が「新機能リリース = 全ユーザー影響」の心理的ブロックから解放される状態を作る。
- **QA ナレッジをオープンソース化 or 業界勉強会で共有**：Mio が蓄積した Daily Knowledge Log・チェックリスト・スコアカードを LET テックブログ or Zenn で発信し、採用市場での LET 技術ブランド確立に貢献。優秀 QA エンジニアの中途採用チャネルとしても機能させる。
