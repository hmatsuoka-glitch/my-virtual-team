...
### 2026-09-10
- **[新スキル] Stryker Mutator v9（2026年GA）によるMutation Testing + Contract Testing (Pact v13)の2段QAゲート**：従来のカバレッジ100%だけでは検出できない「テストが書かれているが実質何も検証していない」ケースを Stryker のミューテーション（意図的なコード変異）で炙り出し、Mutation Score 80%以上を QA ゲート必須項目に。加えて ao と外部API（Airwork/Indeed）の連携は Pact v13 の Consumer-Driven Contract Testing で契約を固定化し、外部仕様変更を CI で自動検知
- **[新ナレッジ] TDD Guard 2.0 + AI-Assisted Testing の統合手法**：Claude Opus 4.7 + TDD Guard 2.0 で「テストが書かれていないコードのコミットを構造的にブロック」する運用が2026年に業界標準化。riku/ao の実装ペースを維持しつつ、テストファーストを強制。加えて、AI が実装コードから「未テストのエッジケース」を自動生成し、mio がレビューして採用/棄却する運用に移行。エッジケース発見時間を平均40分/機能→10分/機能（-75%）
- **[効率化ルール] Playwright 1.50 + Component Testing + MSW v3 の統合テスト体制**：riku の Server Components / Server Actions（riku 2026-09-10参照）を Playwright Component Testing でユニット層検証し、E2E は主要導線のみに絞る「テストピラミッドの再構築」。従来 E2E 200本 → 60本に削減し、CI 実行時間を 25分 → 8分（-68%）に短縮。Kuu の Vercel Preview 上での自動実行（kuu 2026-09-10参照）と連携
- **[失敗パターン→回避策] Mutation Testing を全モジュールに適用すると CI 時間が数時間に膨らみ、開発速度が落ちて形骸化する**：回避策として、Stryker の `--incremental` オプションで PR で変更されたファイルの mutant のみ実行する運用に切り替え、全量実行は週次スケジュールで別レーンに逃がす。PR ごとの mutation testing は平均3分以内に収める
- **[連携強化] 07-LP部 mia（ピクセル単位QA）との連携で「E2E テストとビジュアル回帰テストのシナリオ共通化」**：LP と業務システムの両方で「応募フォーム送信」フローが存在するケースが増えているため、mia の Percy/Chromatic ベースの Visual Regression テストシナリオを Playwright スクリプトに変換し、mio 側でも同じシナリオで機能検証を実施。テスト工数の重複を排除し、LP↔システムの一貫性も自動検証
- **[品質チェック] Sora QA 前セルフチェック**：(1) Mutation Score が全モジュールで80%以上、Pact Contract Test が全外部連携で PASS、実機モバイル検証（BrowserStack 1本）が完遂している3項目を必須確認 (2) `checklists/qa-gate.md` の全項目が PASS で、TDD Guard 2.0 のログに「テスト先行違反」がゼロ、Blocker/Critical レベルの残バグが0件であることを最終レポートで明示
