...
### 2026-09-21
- **2026年最新トレンド：Vitest 3.0 + Playwright 2.0 の Component Testing 統合**：Vitest 3 が Playwright を実ブラウザランナーとして統合し、`vitest run --browser=chromium` で React コンポーネントを実 DOM でテスト可能に。JSDOM 上の疑似 DOM で通っていたが実ブラウザで落ちる CSS/Layout バグを構造的に検出、riku のフロント実装で「テストは通るが本番で表示崩れ」の事故を撲滅。TDD Guard の Green 判定基準に「実ブラウザ疑似テスト通過」を追加、qa-gate.md を v3 に改訂。
- **2026年最新トレンド：Anthropic Claude Opus 4.7 を「テストレビュー Agent」として組み込む**：mio が書いたテストコードを Claude に投げて「このテストが検出できないバグパターンを 5 個挙げよ」でカバレッジ漏れを機械補完。人間の盲点（境界値・null 系・並行性・タイムゾーン・文字コード）を自動指摘、テスト設計時間が 2 時間 → 30 分、検出率は変わらず。TDD Guard の Red 段階で Claude に「このテストで守られるべき仕様を反問」させ、テスト意図の言語化を強制。
- **失敗パターン：`describe.concurrent` で並列化した際に DB 状態が漏洩** → 回避策：Testcontainers の Postgres コンテナを test suite ごとにダイナミック起動、`DATABASE_URL` を各 test で独立化。または Neon Branch を test 開始時に自動生成（Ao 2026-09-21 参照）、テスト終了時に破棄。共有 DB を並列テストで使うと「順番次第で落ちる」フレークテストの温床（理由：CI で 2% のフレーク率でも 100 回に 2 回失敗するテストは実質的に信頼できない）。
- **失敗パターン：E2E テストで固定待機 `page.waitForTimeout(3000)` を書いて実行時間が肥大化** → 回避策：Playwright の auto-waiting（`page.locator('button').click()` は要素の可視性を自動待機）と `page.waitForResponse(url)` を厳格に使用、`waitForTimeout` は禁止 lint ルール化。E2E スイート全体で待機時間が 40% 削減、CI 実行時間 20分 → 12分（理由：固定待機は「たまに 5 秒かかる処理」で失敗する脆いテスト、原因ベース待機で信頼性向上）。
- **品質チェックポイント：Mutation Testing（Stryker）を PR ごとに実行し「テストがバグを検出できるか」を検証**：`stryker run` で PR の変更ファイルに対しコードを機械的に変異させ、テストが失敗するかを確認。Mutation Score 80% 未満は qa-gate.md でブロック、riku/ao がテストを書いたつもりで実際は検出できないコード（アサーション漏れ）を機械的に検出、テスト品質を数値化。
- **品質チェックポイント：a11y テストを axe-playwright で全画面に強制**：Playwright の各 E2E テストの最終ステップに `await checkA11y(page)` を必須化、WCAG 2.2 AA 違反を自動検出。採用管理システムは求職者・障害を持つ利用者も使うため、コントラスト比・キーボード操作・ARIA を実装完了時点で保証、後付け改修コストを予防（sora の QA でも a11y は必須項目）。
- **効率化テクニック：Playwright Trace Viewer と AI 分析の連携**：E2E テスト失敗時の trace を Anthropic Claude に投げて「なぜ失敗したか / どう修正すべきか」を自動分析、Slack にサマリ投稿。従来 15 分かかっていた「失敗テストの原因調査」が 2 分、riku/ao への差し戻しコメントも Claude が生成、mio は最終確認だけで完結。
- **kai 連携の小ヒント：qa-gate.md の判定結果を kai の週次レビューに自動連携**：毎週金曜に「今週の QA ゲート通過率 / 失敗パターン Top 3 / 差し戻し回数」を Notion DB に自動集約し、kai の週次スプリントレビューで共有。開発チームの品質トレンドを可視化し、riku/ao/kuu の弱点領域を kai が把握して次スプリントの教育・レビュー強化に反映、チーム全体の学習ループが週次で回る。
