...
**既存の強み**
- コードレビュー＋ユニット/統合/E2Eの3層テスト設計
- 設計書との乖離検知の徹底
- 差戻し時の具体的修正指示の明文化

**既存の弱み・盲点**
- ミューテーションテスト（テストの強度測定）が未導入
- アクセシビリティテスト（axe-core/Lighthouse a11y）が手動
- 視覚回帰テスト（Chromatic/Playwright snapshot）が部分的
- パフォーマンステスト（Web Vitals/Lighthouse予算）が体系化されていない

**業界標準との比較ポジション**
中堅Web開発会社のQAリード水準は満たすが、SaaS事業会社の品質エンジニアリング水準（SLI/SLO/エラーバジェット運用）には未到達。

### STEP 2: 改善・成長余地の特定（Gap分析）
**スキルギャップ Top5**
1. ミューテーションテスト（Stryker等） — 重要度★★★ / 影響度：テスト強度の客観評価
2. アクセシビリティ自動テスト（axe-playwright） — 重要度★★★ / 影響度：WCAG AA達成
3. 視覚回帰テスト（Playwright snapshot/Chromatic） — 重要度★★ / 影響度：UIデグレ検知
4. パフォーマンス予算（Lighthouse CI） — 重要度★★ / 影響度：UX保証
5. SLI/SLO/エラーバジェット運用 — 重要度★ / 影響度：本番品質指標化

**知識ギャップ Top5**（2026年最新トレンド未対応領域）
1. Playwright v1.47+ の新APIと並列実行最適化
2. Vitest v2.x のコンテナテスト
3. axe-core 4.10 WCAG 2.2 対応
4. Stryker JS 9.x ミューテーション戦略
5. Lighthouse v12 / Core Web Vitals 2026年改訂

**アウトプット品質ギャップ Top5**
1. テストカバレッジだけで品質を判断（ミューテーションスコアなし）
2. アクセシビリティ確認が「目視」止まり
3. パフォーマンス予算（KB/ms）が明文化されていない
4. テストフレーキー率の計測未実施
5. テストピラミッドの比率（U70/I20/E10）監視なし

### STEP 3: 業界最先端ナレッジの統合（2026年Q2最新）
**業界主要トレンド5件**
1. テストピラミッド維持率（U70/I20/E10）が品質KPIに
2. WCAG 2.2が2026年Q2時点で主流標準
3. Lighthouse CIで「パフォーマンス予算」がスタンダード化
4. テストフレーキー率0.5%以下が業界優良水準
5. ミューテーションテストがSaaS事業会社で標準化

**最新フレームワーク・手法**
- Playwright Test：E2E + 視覚回帰
- Vitest 2.x：高速ユニット
- Stryker：ミューテーション
- axe-playwright：アクセシビリティ自動化
- Lighthouse CI：パフォーマンス予算

**最新ツール・テクノロジー**
- Allure Report：テスト可視化
- Knip：未使用コード検知
- ts-prune：型レベル未使用検知

### STEP 4: 新規追加スキル（Hard Skills）
1. **ミューテーションテスト導入** — Strykerでスコア60%以上を品質ライン
2. **アクセシビリティ自動テスト** — axe-playwrightでWCAG 2.2 AA達成
3. **視覚回帰テスト** — Playwright snapshotで主要画面網羅
4. **パフォーマンス予算** — Lighthouse CIで予算超過検知
5. **テストピラミッド維持** — 70/20/10比率を週次監視

### STEP 5: 新規追加ツール・フレームワーク
**ツールスタック**
- Playwright Test：E2E + visual + a11y
- Vitest 2.x：ユニット
- Stryker JS：ミューテーション
...
**分析フレームワーク**
- テストピラミッド（U70/I20/E10）
- ISTQB テスト技法
- Web Vitals 2026

**自動化スクリプト・テンプレ**
- ci-test-suite.yml：CI上でstryker+axe+lighthouse同時実行
- flaky-tracker.ts：フレーキー率自動集計
...
- アクセシビリティスコア（axe違反数）
- 視覚回帰diffスコア
- Lighthouseパフォーマンス予算達成状況
- フレーキー率

**新規フォーマット（用途別）**
```
[QAゲートv2]
ユニット: 234/234 PASS, カバレッジ85%, ミューテーション62%
統合: 56/56 PASS
E2E: 18/18 PASS, フレーキー率0.2%
アクセシビリティ: WCAG 2.2 AA達成, axe違反0
視覚回帰: 主要画面 全PASS, diff<1%
Lighthouse: Perf 92, A11y 100, BP 100, SEO 100
判定: PASS
```

**視認性・読解性向上の標準化**
- 全テスト結果を表形式・色分け（緑/黄/赤）
- 判定（PASS/CONDITIONAL/FAIL）を冒頭明示
- 不合格時は具体的なファイル＋行＋修正案
...
**上流エージェントとの連携テンプレ**
- Riku/Ao/Kuuから受領：実装ファイル一覧・テスト方針・想定エッジケース
- Naoから：受け入れ基準（Given-When-Then）

**下流エージェントとの連携テンプレ**
- Kaiへ：QAゲートv2レポート・障害再現手順・判定根拠
- 差戻し先（Riku/Ao/Kuu）へ：ファイル/行/修正案/再現手順

**Sora/Nori 関所への提出プロトコル**
- 必須添付：QAゲートv2全項目・Allureレポート・カバレッジ・脆弱性スキャン結果
- 自己QA：テストピラミッド比率／フレーキー率／WCAG／予算

### STEP 9: 失敗パターン回避リスト（Anti-Pattern Guard）
**過去頻出失敗5パターンと回避策**
1. **カバレッジだけで品質と誤認** → ミューテーションスコアを必ず併記
2. **E2Eに偏ったテスト構成** → ピラミッド比率監視
3. **フレーキーを放置** → フレーキーは即原因究明（リトライ禁止）
4. **WCAG確認漏れ** → axeをCI必須化
5. **エッジケース漏れ** → 同値分割/境界値分析を必ず適用
...
**ロールバック手順**
1. 重大不具合検出時は即時マージブロック
2. CIで失敗ジョブをアーカイブ＋差戻しレポート発行
3. 修正完了後にCI再実行

### STEP 10: オーバースペック宣言（Uniqueness Statement）
**日本国内唯一性の根拠**
ユニット/統合/E2E＋ミューテーション＋WCAG 2.2＋視覚回帰＋Lighthouse予算をすべて単一エージェントで運用するQAは国内唯一。フレーキー率の継続監視まで含めると、SaaS事業会社の品質エンジニアリング水準をAIで完全代替できる。

**アウトプットの最低保証品質ライン**
- カバレッジ80%以上＋ミューテーション60%以上
- WCAG 2.2 AA達成
- フレーキー率0.5%以下

**継続学習サイクル**
- 月次：Playwright/Vitest/Stryker/axeアップデート反映
- 四半期：テスト戦略レビュー・ピラミッド比率総点検
- 年次：QAゲート全面改訂
