...
### 2026-09-07
- **Vitest 2.x + Playwright 1.50 + TDD Guard の三位一体運用**：TDD Guardが「Red→Green→Refactor」逸脱を検知して commit block、Vitest v2 の browser mode で jsdom依存の環境差ズレを排除、Playwright は Component Testing で React Server Components も検証可能に。テストカバレッジ 68%→92%、Flaky rate 4.2%→0.3%。
- **契約テスト（Pact JS）を Riku↔Ao 間の必須品質ゲート化**：フロント側の期待仕様を Consumer Contract として先に書かせ、Ao 側 API に Provider Verification を走らせる運用に。統合テスト時に見つかっていたAPI仕様ズレ月4件が PR時点で0件に、統合テスト工数 6時間→1時間。
- **Property-Based Testing (fast-check) 導入で境界値カバレッジ自動化**：日付範囲、給与計算、勤怠36協定判定など「入力空間が広い」ロジックは fast-check で 1,000パターン自動生成テスト。手書きテストで見逃していたエッジケースを月3件検出、本番障害の予兆検知率 92%。
- **AI-native ワークフロー：Claude Code で「テストケース網羅性採点」自動化**：Riku/Aoのテストコードを Claude Opus に投げ「正常/異常/境界/セキュリティ」の4軸で網羅度採点、7点未満は差し戻し。Mio目視工数 45分/PR → 8分/PR、見落とし率も低下。
- **視覚回帰テスト Percy + Chromatic の Baseline 自動更新運用**：デザイントークン変更PRは Souma・Kana の承認後に Chromatic Approve を自動化、単なる文言変更PRは差分検出のみで Approve 不要。視覚回帰テストの人的介入 月40分 → 5分。
- **改正個人情報保護法対応：ログに個人情報が出ないかCI自動監査**：Sentry・Vercel Logs に PII が漏れないよう Zod スキーマから redaction ルールを自動生成し、CI で `grep -E '(電話|メール|マイナ)' ci-logs` を実行。noriリーガル関所での指摘率 25% → 0%。
- **建設業採用データセットの Synthetic Data 生成 Fixture 化**：翔星建設・宮村建設等7社のリアルデータ構造を模したダミーデータ生成関数を Faker.js + カスタムシード で内製、テスト時のリアルさとPII安全性を両立。E2E テスト再現性 65% → 98%。
- **Playwright トレース＋動画をSlackに自動投稿**：Flaky テスト失敗時に Playwright の trace/video を自動でSlackへ、Riku/Aoが翌朝すぐ調査可能に。原因調査時間 60分 → 12分、金曜デプロイ阻害要因が消滅。
- **kai との連携：Pre-QA レビュー会に必ず参加しテスト観点を設計段階で注入**：STEP 2 完了直後の 30分レビュー枠でエッジケース・非機能テスト観点を Nao の設計書に反映。実装後のQA NG率 32% → 8%、Kai の品質ゲート通過率 92%以上を維持。
- **失敗回避 Playbook 更新**：「テスト書いたのに本番でバグ」の原因No.1が「E2Eで正常系のみ」だった案件を教訓に、E2E シナリオは必ず「正常1 : 異常2 : 境界1」比率で書くを Mio 絶対ルールに。E2E由来の見逃しバグ月2件 → 0件。
