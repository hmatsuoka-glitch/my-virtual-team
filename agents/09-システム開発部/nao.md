...
### 2026-09-21
- **2026年最新トレンド：BMAD-METHOD v3 の PRD テンプレートに「Non-Goals セクション」が必須化**：v3 では「作らない機能」を PRD に明文化する Non-Goals セクションが標準に。「応募者管理システム v1 では英語版を作らない／給与計算連携をしない／モバイルアプリは作らない」等を STEP 1 で明示、後の要件膨張を構造的に予防。kai の見積根拠にも直結し、クライアントとの期待値ミスマッチを 30% 削減、スコープクリープによる納期遅延を撲滅。
- **2026年最新トレンド：Event Storming + C4 Model のハイブリッド設計手法**：Miro で Event Storming（付箋で「◯◯した」イベントを時系列並べ）を実施 → C4 Model（Context/Container/Component/Code の 4 階層図）に落とし込む 2 段構え。建設業クライアントの業務フロー（応募→書類選考→現場見学→内定）が「誰が何をトリガーするか」で可視化され、Nao 単独の脳内モデルではなく「クライアントも読める設計書」に。architect-checklist.md の必須成果物として追加提案。
- **失敗パターン：ER 図を「テーブル一覧＋外部キー」だけで完成させて業務ルールが漏れる** → 回避策：ER 図の各テーブルに「1 行の存在理由（誰が何のために作るか）」と「削除ポリシー（論理/物理/カスケード）」を必ず注釈。テーブル存在理由が言語化できないカラムは削除候補、削除ポリシー未定義のテーブルは Ao 実装前に kai と nori に照会（理由：Ao 2026-09-13 の「UI 削除 vs 本人請求削除」問題を設計段階で予防、後付けカスケード変更は本番データ破損リスク）。
- **失敗パターン：非機能要件（性能・可用性・セキュリティ）を「一般的な水準で」と曖昧にする** → 回避策：ISO/IEC 25010 の品質特性を STEP 1 で 10 項目分ヒアリング、各項目に「必須/推奨/不要」の 3 段階と定量目標（p95 レスポンス 2 秒、月間稼働率 99.5%、平均復旧時間 4 時間等）を明記。定量目標が書けない項目は Nao 側で「業界ベンチマーク」として提案し、クライアント承認を取る（理由：非機能を数字で書かないと mio の qa-gate で判定不能、kuu のインフラ設計も曖昧化）。
- **品質チェックポイント：architect-checklist.md 通過前に「30分同席レビュー」を kai 主催で強制**：Nao の設計を riku/ao/kuu の 3 名が「実装できる粒度か」を検証（kai 2026-09-21 参照）、設計完了後の手戻りを起草段階で潰す。Nao は「なぜこの設計にしたか」を 3 分プレゼン、実装側の質問を全て記録して設計書に FAQ セクションとして追記。
- **品質チェックポイント：セキュリティ要件を OWASP Top 10 + ASVS Level 2 で網羅する**：応募者管理システムは個人情報を扱うため、ASVS Level 2（Application Security Verification Standard）の 200 項目チェックリストを設計時に通す。特に「V3: セッション管理」「V7: 暗号化」「V10: マルウェア対策」を Nao の設計書に必須反映、nori のリーガルチェックと連動、実装後の脆弱性対応コストを予防。
- **効率化テクニック：ChatGPT o1-preview で「設計書レビュー Agent」を並走**：Nao の設計書 Markdown を o1 に投げて「この設計で発生する可能性のある不具合を 10 個列挙し、優先度順に並べよ」で機械レビュー。人間の盲点（並行性・障害時のフェイルオーバー・データ整合性）を自動指摘、Nao の脳内検証を機械で補完、設計品質を体感 30% 向上。
- **kai 連携の小ヒント：Story Compression 時点で「実装工数の見積レンジ」を Nao から先出し**：BMAD v3 の Story Compression（kai 2026-09-21 参照）で Nao が各 Story に「実装 XX 時間 ± YY 時間」を先付け、kai の見積根拠に直結。riku/ao/kuu の実績データを Nao が Notion DB で管理し、Story 種別（CRUD 画面 / 認証 / 通知 / 帳票）ごとの平均工数を蓄積、見積精度が過去比 40% 向上。
