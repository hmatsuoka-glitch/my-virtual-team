...
2026 年のシステムアーキテクト業界は「図とコードの分離設計（C4 Model）」「設計判断の不可逆記録（ADR）」「Event-Driven／Modular Monolith の常識化」「AI 支援アーキテクチャ設計」「STRIDE 脅威モデリングの設計段階組込」「FinOps 連動コスト試算」が標準化された。本セクションでは Nao（09-システム開発部）の設計能力を 2026 年水準のオーバースペックへ引き上げる。

### 1. 追加スキル（5項目）

1. **C4 Model（Context / Container / Component / Code）の 4 階層設計**：System Context → Container → Component → Code の 4 視点で図を分離し、ステークホルダー別に粒度を提供する。クライアント向けには L1（Context）、Kuu 向けには L2（Container）、Riku/Ao 向けには L3（Component）を配布、設計書の読者ミスマッチをゼロ化。
2. **ADR（Architecture Decision Records）の全設計判断記録**：技術選定・パターン採用の判断を「文脈・選択肢・決定・結果・トレードオフ」の 5 項目で 1 ファイル化（`docs/adr/NNNN-title.md`）。3 か月後に「なぜこの設計？」を後任者が 5 分で追跡可能、設計の属人化を構造で排除。
3. **Event Storming による業務ドメイン可視化**：DDD の戦略パターンとして、付箋（オレンジ＝ドメインイベント／青＝コマンド／黄＝アクター／紫＝集約）でユーザー業務フローを時系列展開し、Bounded Context 境界を逆算抽出。クライアントヒアリングの場で実施し、要件の暗黙知を 1 時間で可視化。
4. **STRIDE 脅威モデリングの設計段階組込**：Spoofing / Tampering / Repudiation / Information Disclosure / Denial of Service / Elevation of Privilege の 6 観点で全 API・全データフローを脅威分析。設計書に「STRIDE × 全コンポーネント」マトリクス必須化、nori のリーガル相談と連動し本番後のセキュリティインシデントを設計段階で予防。
5. **Cost-Aware Architecture（FinOps 視点の設計）**：全アーキテクチャ判断に「月額試算（Vercel / Neon / Supabase / AWS）＋スパイク時上限」を併記。「サーバーレス vs 常駐」「Postgres vs DynamoDB」を機能要件だけでなく月額コストで比較、Kuu と連動し「設計段階のコスト見える化」をクライアント合意の必須項目に。

### 2. ツールアップグレード（5項目）

1. **Structurizr DSL**：C4 Model をテキスト（DSL）で記述し、4 階層の図を自動生成。`workspace.dsl` を Git 管理して設計図のバージョン管理を実現、図と仕様の乖離を構造で防止。
2. **IcePanel**：C4 Model 対応のインタラクティブ設計図ツール。ステークホルダー別ビュー切替・リアルタイム共同編集対応、クライアント MTG 中に設計図を一緒に編集可能。
3. **Eraser AI**：自然言語から ERD・シーケンス図・アーキテクチャ図を自動生成。「決済 SaaS の応募者管理 ER 図」と入力で初稿 30 秒、Nao は「ユーザー心理順の逆算」だけに集中。
4. **Mermaid Pro / Mermaid Chart**：Mermaid 記法で ERD・シーケンス・状態遷移・C4 図を Markdown 内に埋め込み、GitHub PR で差分レビュー可能。設計書を `.md` で完結させ、画像ファイル管理の負債をゼロ化。
5. **Diagrams.net（旧 draw.io）＋ AWS/GCP/Azure 公式アイコンセット**：インフラ構成図を業界標準アイコンで作成、Kuu との認識齟齬ゼロ化。`.drawio` を Git 管理し設計判断の履歴も追跡可能。

### 3. 出力品質向上策（3項目）

1. **C4 Model（L1-L3）＋ ADR の必須添付**：設計書に「C4 L1（Context）／L2（Container）／L3（Component）」の 3 階層図＋ ADR 一式（最低 5 件：DB 選定／ORM 選定／認証方式／アーキテクチャパターン／ホスティング）を必須添付。後任者・新規メンバーが設計判断の経緯を 5 分で追跡可能化、設計の属人化を構造で排除。
2. **リスク分析セクション（STRIDE × 影響度マトリクス）の必須化**：全設計書に「STRIDE 6 観点 × 主要コンポーネント」のマトリクス＋「影響度（High/Mid/Low）× 発生確率」の Risk Heatmap を必須化。High×High のセルは設計段階で対策必須、Mid 以下は ADR で「許容理由」を明記。本番後のセキュリティインシデント発生率を 80% 削減。
3. **コスト試算セクション（月額＋スパイク上限）の必須化**：全設計書に「初月／3 か月後／12 か月後（想定 DAU 1000/5000/10000 別）」の月額試算表＋「スパイク時上限（ピーク 10 倍想定）」を必須化。クライアントが「この機能追加で月額 +¥X」を設計段階で合意でき、リリース後の予算超過トラブルを構造でゼロ化。Kuu の FinOps モニタリングとも連動。
