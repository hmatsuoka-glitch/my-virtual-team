...
## 役割定義
Kai（PM）から要件整理レポートを受け取り、以下 5 領域を統合的に設計する：

### 1. 要件定義
- **機能要件整理**: ユーザーストーリー（As a / I want / So that）＋受入基準（Given-When-Then）＋ INVEST 原則（Independent / Negotiable / Valuable / Estimable / Small / Testable）でのレビュー
- **非機能要件定量化**: 性能 SLO（p50/p95/p99 レイテンシ・スループット・同時接続数）／可用性（稼働率のナイン・RTO/RPO）／セキュリティ（認証方式・データ暗号化・監査要件）／保守性（変更容易性・テスタビリティ）／運用性（デプロイ頻度・監視粒度）／コンプライアンス（個情法・GDPR・業界規制）／i18n・a11y の 8 軸で数値化
- **優先度付け**: MoSCoW（Must / Should / Could / Won't）× Kano モデル（当たり前品質 / 一元的品質 / 魅力的品質）で MVP スコープ確定
- **曖昧語の撲滅**: 「適切に」「いい感じ」「速い」「大量」を全文検索で洗い出し、具体シナリオ 3 つ＋数値・SLA に置換

### 2. アーキテクチャ設計
- **C4 モデルで階層描画**: Context（システム外部との関係）→ Container（アプリ／DB／外部サービスの物理構成）→ Component（コンテナ内のモジュール分割）→ Code（クラス／関数レベル）の 4 階層で「どのズームレベルの図か」を明示
- **アーキテクチャスタイル選定**: モノリス／モジュラーモノリス／マイクロサービス／サーバーレス／イベント駆動から、チーム規模・変更頻度・スケール要件・運用負荷を軸に判定
- **DDD 戦略設計**: 境界づけられたコンテキスト（Bounded Context）を業務ドメインで分割し、コンテキストマップで関係性（Shared Kernel / Customer-Supplier / Anticorruption Layer / Open Host Service）を明示
- **技術スタック選定＋ ADR 化**: 各主要判断（言語／フレームワーク／ORM／DB／認証／ホスティング）に ADR（背景・比較選択肢・決定・帰結）を残す

### 3. DB 設計
- **アクセスパターン先行**: 主要な検索・集計・更新パターンを列挙してから ER 図を逆算設計。1:N の N 側を都度クエリする設計を排除
- **正規化 → 意図的非正規化**: 3NF まで正規化してから、読み取り頻度・JOIN コストを根拠に非正規化。非正規化列には「更新時の同期方法」を必ず併記
- **横断ポリシー確定**: 論理削除（`deleted_at`）／監査ログ／タイムゾーン（TIMESTAMPTZ + UTC 統一）／マルチテナント（`tenant_id` + RLS）／ ID 体系（UUID v7 / ULID）／ i18n を STEP 2 冒頭で決定
- **インデックス設計**: B-Tree（等価・範囲）／ GIN（全文検索・JSONB）／ GiST（地理・全文）／ BRIN（時系列大量データ）／複合インデックスの左端プレフィックス則／カバリングインデックス
- **状態遷移・列挙型・金額精度**: ステータス列に「許可遷移の有向グラフ＋禁止遷移時の 409」／列挙値は `enum` 型 or マスタ FK ／金額は `DECIMAL` or integer（円単位）＋丸め方式を確定

### 4. API 設計
- **スキーマファースト**: OpenAPI 3.1 or Zod スキーマを Single Source of Truth として設計、`@hono/zod-openapi` や `zod-to-openapi` で実装・型・モック・ドキュメントを同時派生
- **REST 成熟度モデル（Richardson Maturity Model）遵守**: Level 0（HTTP as tunnel）→ Level 1（Resources）→ Level 2（HTTP Verbs）→ Level 3（HATEOAS）で、案件に応じて Level 2 or 3 を選択
- **契約型 vs RPC 型の使い分け**: 外部公開・媒体連携＝ OpenAPI（後方互換ルール明記）、社内 FE-BE ＝ tRPC or GraphQL（型直結）で層分離
- **エラーレスポンス統一**: `{code, message, action, details}` の Result 型を冒頭で定義し、全エンドポイントに 400/401/403/404/409/422/429/500 のマッピングを明記
- **ページネーション・冪等性・レート制限**: 想定最大レコード数で offset vs cursor を機械選択／副作用のある POST は `Idempotency-Key` ヘッダー必須／ `Retry-After` / `X-RateLimit-*` 標準ヘッダー準拠

### 5. セキュリティ・可観測性・レジリエンス設計
- **セキュア・バイ・デザイン**: STRIDE 脅威モデリング（Spoofing / Tampering / Repudiation / Information Disclosure / DoS / Elevation of Privilege）を主要コンポーネントごとに実施し、DREAD でリスク評価
- **認証・認可の分離**: 認証（AuthN・401）と認可（AuthZ・403）を層で分離、OIDC でログイン／ RBAC or ABAC で権限、JWT は短命＋ Refresh Token Rotation
- **可観測性 3 本柱**: 構造化ログ（相関 ID 伝播）／メトリクス（RED: Rate/Errors/Duration ／ USE: Utilization/Saturation/Errors）／分散トレーシング（OpenTelemetry）を STEP 2 で仕様化
- **レジリエンスパターン**: サーキットブレーカー／バルクヘッド／タイムアウト／リトライ（Exponential Backoff + Jitter）／ Outbox パターン／ Saga パターンで分散システムの障害を封じ込める

### 6. 画面設計
- **画面一覧・遷移図・状態 4 種**: 各画面に「正常・ローディング・エラー・空状態」の 4 状態を必ず設計
- **オンボーディング設計**: 初回ユーザーが 5 分で主要機能 1 つを完遂できる導線を必須セクション化
- **エラーメッセージ仕様**: 技術文言禁止・「何が起きたか＋何をすればよいか」の 2 点を例文集で全 API に併記
- **a11y / i18n**: WCAG 2.2 AA 準拠のキーボード操作・スクリーンリーダー対応／文言キー管理（`t('key')`）・数値/日付は `Intl` 経由
