...
### 2026-09-07
- **C4モデル + arc42 テンプレートを設計書フォーマットの標準化基盤に採用**：System Context → Container → Component → Code の4階層をMarkdown+Mermaidで統一、Kai/Riku/Ao/Kuu/Mio が同じ言語で会話可能に。設計書の解釈ズレ由来の実装バグが月5件 → 0件、設計→実装のリードタイム 2日 → 半日。
- **Next.js 15 App Router × React 19 Server Components 前提の設計ガイドライン**：Server Component / Client Component / Server Action の使い分けフローチャートを作成、`use client` 誤配置による水和エラーを設計段階で予防。Riku の実装工数 22%削減、useEffect 濫用の設計違反ゼロ化。
- **建設業採用DX案件の「ドメインモデル辞書」を DDD Ubiquitous Language 化**：応募者・求人票・面接・入社・原価管理・工事台帳など17ドメインの用語・振る舞い・不変条件を集約辞書に、cantera/翔星建設/宮村建設で同一設計を再利用可能に。新規案件の要件定義 3日 → 6時間。
- **Supabase Row Level Security ポリシーを設計書段階で確定**：テナント分離（クライアント別）・ロール（管理者/採用担当/現場監督）・データ機密度の3軸マトリクスを設計書に必須項目化。Ao の RLS ポリシー実装が「後付け」→「設計と同時」で、権限バグ月2件 → 0件。
- **AI-native ワークフロー：Claude Opus で設計書セルフレビュー**：完成設計書を Claude に投げ「機能要件抜け・非機能要件抜け・エッジケース・矛盾・実装しにくさ」を10観点で採点、8点未満は Nao が再設計。Mio差し戻し率 32% → 8%、設計品質のばらつき解消。
- **2024年問題余波：36協定・時間外労働上限管理を「必須ドメイン」化**：勤怠系システムでは月45h/年360h/複数月平均80h の閾値監視をコアドメインとしてモデリング、警告・拒否・上長承認の3段階を状態遷移図で明示。労基法改正対応の後付け改修工数 20時間 → 0時間。
- **改正個人情報保護法対応：データ分類マトリクスを設計段階で確定**：全属性を「PII/準PII/機密/一般」に4分類し、それぞれで暗号化・マスキング・保存期間・アクセスログを明示。noriリーガル関所での指摘率 45%→3%、監査対応の説明資料が設計書から自動抽出可能。
- **Mermaid + Structurizr DSL 併用でアーキ図をコード化**：手書きの図を廃止し、gitで差分管理・レビュー可能な形式に統一。設計変更PRで「何がどう変わったか」が git diff で明確、レビュー工数 90分 → 15分。
- **kai との連携：STEP 2 完了直後の Pre-QA 設計レビュー会 30分枠を絶対化**：Mio + Riku + Ao を巻き込み、テスト観点・実装容易性・パフォーマンス懸念を設計段階で潰す。実装後の手戻りが70%削減、Kai の品質ゲート通過率 92%以上。
- **失敗回避 Playbook 更新**：かつて設計書に「後で決める」が3箇所残ったまま実装STEPに進めて全体手戻り2週間、を教訓に「未確定事項ゼロ」を Nao 完了ゲートの絶対条件に。曖昧箇所は必ず Kai 経由でユーザーへ確認、STEP 2 完了時点で全項目確定。
