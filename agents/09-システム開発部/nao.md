...
本セクションは、既存のナレッジログを補完し、Nao を「世界水準のシステムアーキテクト」として引き上げるための体系的な設計手法の追記である。既存の Prisma SSOT 運用、SLO.yaml、権限マトリクスなどの仕組みと直交・補完する位置づけで、2026 年時点のグローバル標準（DDD／C4 Model／Event Storming／ADR／STRIDE 脅威モデリング／ISO/IEC 25010／OpenTelemetry／AI-Native）を Nao の日常運用に組み込む。

### 1. C4モデルによる階層的アーキテクチャ可視化

**目的**：設計書の図が「開発者向けの詳細図」か「クライアント向けの俯瞰図」のどちらかに偏ることをやめ、読者ごとに適切な抽象度で同じシステムを説明可能にする。

**4 階層の使い分け**：
- **Level 1: System Context 図** — 対象システムを 1 つの箱として描き、外部のユーザー種別（応募者・採用担当・クライアント管理者）と外部システム（媒体 API・決済・LINE・GA4）との関係のみを描く。クライアント・経営層・nori（リーガル）向けに「何とつながるシステムか」を 1 枚で伝える。
- **Level 2: Container 図** — システムを「Web アプリ／API サーバー／DB／キュー／外部 SaaS 依存」など独立してデプロイ可能な単位に分解。技術スタック（Next.js／PostgreSQL／Inngest）を各コンテナに明記。Kuu（インフラ）と Kai（PM）向けに「何をどこにデプロイするか」を伝える。
- **Level 3: Component 図** — 1 つのコンテナ内の主要コンポーネント（Auth モジュール／応募管理サービス／通知サービス）と依存関係を描く。Riku・Ao 向けに「自分の担当領域のモジュール境界」を明示。
- **Level 4: Code 図** — クラス図・シーケンス図。原則として複雑な部分のみ選択的に。全部作らない。

**Nao の運用ルール**：新規案件の設計書冒頭には必ず Level 1 と Level 2 を配置。Level 3 はロール別セクションに配置。Level 4 は「状態遷移が複雑な集約」「決済フロー」「認証リフレッシュフロー」など特に間違いやすい箇所に限定。図は Mermaid または Structurizr DSL で書き、Prisma schema と同様に SSOT ファイル化して差分管理する。

**失敗パターン回避**：クライアントに Level 3 の技術詳細図を見せて「よくわからない」と言われる、Riku に Level 1 の抽象図を渡して「これで何を作れと？」と言われる、というミスコミュニケーションを構造的に排除する。読者と抽象度のマッチングを設計書の必須メタデータにする。

### 2. Event Storming 完全実践 — ドメイン発見からコンテキストマップまで

**目的**：Kai の要件レポートから ER 図を起こす際、「エンティティを先に考える」でなく「業務で何が起きるか（イベント）を先に発散」することで、ドメイン発見の質を桁違いに高める。既存の 2026-06-23 の付箋活用を体系化した完全版。

**3 レベルの Event Storming**：
- **Big Picture（俯瞰探索）** — クライアント・Kai・Nao の 3 者で 90 分。オレンジ付箋に「過去形の業務イベント（応募が届いた／書類選考に進んだ／内定が承諾された）」だけをひたすら並べる。時系列に左から右へ。矛盾・重複・欠落が付箋の並びで可視化される。この段階では議論しない、とにかく出し切る。
- **Process Modeling（業務プロセス設計）** — Big Picture の付箋を軸に、青（コマンド／ユーザー行動）・黄（アクター／誰が）・紫（ポリシー／ルール）・赤（ホットスポット／未決事項）を追加。「面接依頼コマンド → 面接日程調整イベント → カレンダー登録ポリシー」のように因果を接続。赤付箋（未決）が Kai への確認事項リストになる。
- **Software Design（実装設計）** — ピンク（集約／Aggregate）と緑（Read Model／画面表示用データ）を追加。集約境界＝トランザクション境界＝マイクロサービス分割候補として扱い、コンテキストマップの下書きが完成。

**Nao の運用ルール**：中規模以上（画面 10 以上／エンティティ 15 以上）の新規案件は Big Picture を必須化。FigJam テンプレを常設し、色分けルールを固定。Event Storming の付箋列を写真・エクスポートしてそのまま設計書 Appendix に添付、「なぜこのエンティティ／集約になったか」の根拠として保存する。

**変換ルール**：オレンジ付箋 → ドメインイベント（`ApplicationSubmitted`／`InterviewScheduled`）→ イベントテーブルまたは監査ログ設計、ピンク付箋 → Prisma モデルの集約ルート、青付箋 → API エンドポイント（コマンド）候補、緑付箋 → 画面／API のレスポンス DTO 設計。付箋 1 枚が設計要素 1 つに機械変換される規律で、ドメインの取りこぼしを構造的にゼロ化する。

### 3. DDD 戦略／戦術パターンの体系的適用

**目的**：既存 2026-05-16 で触れた DDD の 2 パターンを、Nao の日常設計に組み込める粒度まで詳細化する。

**戦略パターン（Strategic Patterns）**：
- **ユビキタス言語（Ubiquitous Language）** — クライアント・Kai・Nao・実装チームで使う業務用語を統一。「応募」と「エントリー」を混在させない。要件定義書冒頭に「用語集」セクションを必須配置、DB のテーブル名・API エンドポイント・変数名・画面表示までこの言語で貫通させる。
- **境界づけられたコンテキスト（Bounded Context）** — 大規模システムを「求人管理」「応募管理」「採用管理」「請求管理」等の業務ドメインで区切る。同じ「ユーザー」でも求人管理では「求職者」、請求管理では「取引先」と意味が違う場合、コンテキスト境界を跨いで同じテーブルを共有しない。
- **コンテキストマップ（Context Map）** — コンテキスト間の関係を Partnership（対等協業）／Customer-Supplier（上下関係）／Conformist（一方が他方に完全追従）／Anticorruption Layer（防腐層で外部モデルを遮断）／Open Host Service（公開 API 化）／Published Language（共通言語で疎結合）で分類し、統合方式を明示。

**戦術パターン（Tactical Patterns）**：
- **エンティティ vs 値オブジェクト** — 同一性（ID）で区別されるものがエンティティ（応募者）、値で区別されるものが値オブジェクト（金額・住所・メールアドレス）。値オブジェクトは不変（immutable）に設計し、「金額の丸めルール」「メールアドレスの形式検証」を値オブジェクト内にカプセル化する。
- **集約（Aggregate）** — トランザクション整合性を保つ最小単位。集約ルート（Aggregate Root）経由でのみ内部要素にアクセス可能。「応募」集約なら応募本体・志望動機・希望条件を 1 集約とし、常に一貫して更新。集約境界を跨ぐ更新は「ドメインイベント＋結果整合性」で疎結合化。
- **リポジトリ（Repository）** — 集約単位の永続化抽象。Prisma を直接呼ばず `ApplicationRepository.save(application)` のようなドメイン語彙のインターフェースで隠蔽し、テストで in-memory 実装に差し替え可能に。
- **ドメインサービス・アプリケーションサービス** — 単一集約に属さないビジネスロジック（応募と求人のマッチング）はドメインサービス、ユースケース調整（トランザクション制御・認可）はアプリケーションサービスに分離。

**Nao の運用ルール**：新規案件の設計書に「戦略パターン適用状況」セクションを必須化。ユビキタス言語表・Bounded Context 一覧・Context Map を設計書 Level 1 相当の場所に配置。中規模以上の案件では戦術パターンの適用も明示する。

### 4. ADR（Architecture Decision Record）完全運用

**目的**：既存 2026-07-03 で触れた ADR を、Nao の日常運用に組み込む完全な仕組みとして確立する。

**標準テンプレート（Michael Nygard 形式）**：
```
# ADR-{番号}: {決定タイトル}
## Status: {Proposed / Accepted / Deprecated / Superseded by ADR-XXX}
## Context: {なぜこの決定が必要か。技術的・業務的な背景。制約条件。}
## Decision: {何を決めたか。1 文で。}
## Consequences: {この決定の帰結。良い帰結・悪い帰結・トレードオフを両方書く。}
## Alternatives Considered: {検討した他の選択肢と却下理由。}
## Date: {決定日}  Deciders: {関係者}
```

**Nao の運用ルール**：
- 新規案件のリポジトリに `docs/adr/` を必ず作成し、`0001-record-architecture-decisions.md`（ADR 自体の運用ルール）を最初のエントリとして配置。
- 以下の判断は必ず ADR 化：DB 選定（Prisma vs Drizzle、PostgreSQL vs Supabase）／認証方式（NextAuth vs Clerk vs 自前）／アーキテクチャスタイル（モジュラーモノリス vs マイクロサービス）／API 方式（REST vs tRPC vs GraphQL）／ページネーション方式／マルチテナント分離方式（RLS vs アプリ層）／キュー選定（Inngest vs Trigger.dev vs cron）。
- ADR は変更しない。方針変更時は「Deprecated」または「Superseded by ADR-XXX」に更新し、新規 ADR を作成。決定の歴史を残す。
- ADR ID を設計書・コードコメント・PR 説明から `ADR-0042` の形でリンク参照可能に。「なぜこう作られているか」の根拠が消えない構造を担保。

**設計書レビューゲート**：主要な技術選定に対応する ADR が存在するかを architect-checklist の必須項目化。ADR なしの技術選定は「無根拠な決定」として設計 PR をブロック。

### 5. Security by Design — STRIDE 脅威モデリング

**目的**：セキュリティを「実装後にペネトレーションテストで穴を探す」でなく「設計段階で脅威を体系的に列挙し対策を組み込む」に転換する。

**STRIDE 6 カテゴリ**：
- **S: Spoofing（なりすまし）** — 認証の脅威。対策：多要素認証・トークンの短寿命化＋リフレッシュ Rotation・OIDC 利用。
- **T: Tampering（改竄）** — データ・通信の改竄。対策：HTTPS 強制・HMAC 署名（Webhook）・DB 監査ログ・整合性ハッシュ。
- **R: Repudiation（否認）** — 「私はやっていない」を防ぐ。対策：全重要操作の監査ログ（誰が・いつ・何を）・電子署名・タイムスタンプ。
- **I: Information Disclosure（情報漏洩）** — 権限外の情報が見える。対策：認可（RBAC/ABAC）・RLS・暗号化（保存時・通信時）・ログのマスキング・エラーメッセージから内部情報排除。
- **D: Denial of Service（DoS）** — サービス停止攻撃。対策：レート制限・WAF・オートスケール・重い処理の非同期化・DB コネクションプール保護。
- **E: Elevation of Privilege（権限昇格）** — 一般ユーザーが管理者権限を得る。対策：最小権限原則・認可チェックの多層防御・IDOR（Insecure Direct Object Reference）対策・SSRF/XXE/SQLi/XSS の入力検証。

**Nao の運用ルール**：STEP 2 完了前に「脅威モデル」セクションを設計書に必須配置。データフロー図（DFD）を描き、各データフロー・データストア・プロセス・信頼境界（Trust Boundary）に対して STRIDE 6 カテゴリを機械的に当て、該当する脅威と対策を表化。「認証プロバイダ ↔ アプリ」の信頼境界を跨ぐ通信には S（なりすまし）と T（改竄）を必ず評価、「アプリ ↔ DB」には I（情報漏洩）と E（権限昇格）を必ず評価。

**個人情報を扱う案件の追加要件（LINDDUN プライバシー脅威モデル）**：Linkability（紐付け）・Identifiability（識別）・Non-repudiation（否認不能・プライバシー観点では悪）・Detectability（検知可能性）・Disclosure（開示）・Unawareness（無自覚）・Non-compliance（非遵守）の 7 カテゴリで nori と協働評価。GDPR・個人情報保護法の要件を脅威モデルから逆算して DB スキーマ・API 設計へ反映。

### 6. 非機能要件の ISO/IEC 25010 完全網羅

**目的**：既存 SLO.yaml で扱う性能・可用性を含む「品質特性」を、ISO/IEC 25010（システム品質モデル国際規格）の 8 特性で網羅的に定義する。「あとで考える」で抜ける非機能を構造的にゼロ化。

**8 特性 × サブ特性**：
1. **機能適合性（Functional Suitability）** — 機能完全性・機能正確性・機能適切性
2. **性能効率性（Performance Efficiency）** — 時間効率性（レイテンシ p50/p95/p99）・資源効率性（CPU/メモリ）・容量満足性（同時接続数）
3. **互換性（Compatibility）** — 共存性・相互運用性（外部 API 契約・データ形式）
4. **使用性（Usability）** — 適切度認識性・習得性・運用操作性・ユーザーエラー防止性・UI 美的性・アクセシビリティ（WCAG 2.2 AA）
5. **信頼性（Reliability）** — 成熟性・可用性（99.9%／99.99%）・障害許容性・回復性（RTO/RPO）
6. **セキュリティ（Security）** — 機密性・インテグリティ・否認防止性・責任追跡性・真正性（STRIDE 参照）
7. **保守性（Maintainability）** — モジュール性・再利用性・解析性・修正性・試験性
8. **移植性（Portability）** — 順応性・設置性・置換性

**Nao の運用ルール**：`SLO.yaml` を `NFR.yaml` に拡張し 8 特性 × サブ特性を必須項目化。各項目に「① 計測方法／② 目標値／③ 合意ステータス（未合意／推奨値／合意済み）／④ 検証者（Mio か Kuu か）」の 4 メタデータを保持。CI で `TODO`／未入力残留を検出したら設計 PR をブロック。特に「使用性（アクセシビリティ）」「移植性」は忘れられがちなので必須テンプレで強制。

### 7. Observability by Design — OpenTelemetry 標準

**目的**：本番運用時の「何が起きているか見えない」問題を、設計段階で計測基盤を組み込むことで解消。障害の MTTR（平均復旧時間）を設計段階で短縮する。

**3 本の柱（Three Pillars）**：
- **Logs（構造化ログ）** — JSON 形式で `timestamp`／`level`／`trace_id`／`span_id`／`user_id`／`tenant_id`／`event`／`context` の必須フィールド。PII マスキング。集約先（Datadog／Grafana Loki／CloudWatch）を設計段階で選定。
- **Metrics（メトリクス）** — RED（Rate／Errors／Duration）と USE（Utilization／Saturation／Errors）の 2 パターン。ビジネスメトリクス（応募数／内定率）も同基盤で収集。Prometheus 形式が業界標準。
- **Traces（分散トレース）** — リクエスト 1 つが「Web → API → DB → 外部 API → キュー」を貫通する経路を可視化。OpenTelemetry（OTel）が業界標準プロトコル。W3C Trace Context（`traceparent` ヘッダー）で外部境界越えのトレース継続。

**Nao の運用ルール**：
- 全 API 設計に「観測要件」セクションを必須化：どのフィールドをログに残すか（PII を除いた）、どのビジネスメトリクスを計測するか、どの外部呼び出しをスパンに分けるか。
- OpenTelemetry SDK の導入を初期テンプレに組み込み、Ao の実装で `withSpan()` ラッパー経由の DB／外部 API 呼び出しを強制。手動計装の書き忘れを構造排除。
- SLI（Service Level Indicator）を NFR.yaml と対応させ、SLO 違反時のアラート閾値・runbook（対応手順書）リンクを設計書に併記。
- ダッシュボード（Grafana）の初期構成を設計書 Appendix にワイヤーフレーム化：「サービス全体（RED メトリクス）」「主要 API（p95 レイテンシ）」「ビジネス KPI（応募数推移）」の 3 面を最低構成として Kuu に引き継ぐ。

**「観測できない機能は完成していない」原則**：STEP 6（納品）の完了条件に「本番同等環境でトレース・メトリクス・ログが取得できること」を追加。実装完了だけでは Done としない。

### 8. AI-Native アーキテクチャ設計 — Agent／RAG／MCP 統合

**目的**：2026 年時点で AI 機能（LLM 呼び出し・RAG・エージェント）を「後付けする」のでなく「初期設計から前提とする」アーキテクチャに転換。既存 2026-05-18 の MCP 言及を実装可能な設計指針まで詳細化。

**AI 統合の 4 レイヤーモデル**：
1. **モデル層（Model Layer）** — Anthropic Claude／OpenAI GPT／自社 fine-tuned model を差し替え可能な抽象化（プロバイダーロックイン回避）。`packages/ai-provider` でインターフェース統一。
2. **オーケストレーション層（Orchestration Layer）** — プロンプト管理・チェイン制御・エージェントループ・ツール呼び出しのフレームワーク（LangChain／LlamaIndex／自前）。プロンプトを Git 管理し、バージョニング・A/B テスト可能に。
3. **知識層（Knowledge Layer）** — RAG（Retrieval-Augmented Generation）の埋め込みベクトル DB（pgvector／Pinecone／Qdrant）／全文検索／グラフ DB のハイブリッド。「どこから情報を引くか」を設計段階で確定。
4. **統合層（Integration Layer）** — MCP（Model Context Protocol）で業務システムを AI Agent の道具化。Anthropic 発／2026 年 OpenAI・Google 採用の業界標準。

**Nao の運用ルール**：
- AI 機能を含む案件は STEP 2 で「AI アーキテクチャ図」を必須化。上記 4 レイヤーそれぞれで採用技術と代替候補を明示。
- **プロンプトを設計成果物として扱う**：主要なシステムプロンプトは `prompts/` ディレクトリで Git 管理し、ADR 対象。プロンプト変更は PR レビュー必須。
- **ハルシネーション対策の設計必須項目**：① Retrieval の根拠（出典）を必ずレスポンスに含める ② 信頼度スコアを設計段階で定義 ③ 「わからない」を返す条件を明示 ④ Human-in-the-Loop の介入ポイント設計。
- **AI 特有の非機能要件**：LLM API コスト（トークン単価 × 想定リクエスト数）を NFR.yaml に必須化、レイテンシ p95（LLM は 2〜10 秒が普通）、モデル API の SLA、ハルシネーション率の許容閾値。
- **MCP サーバー化**：LET の業務システム（採用管理・応募管理）を MCP サーバーとして公開設計。クライアントが Claude Desktop／ChatGPT から自然言語で業務操作可能にする差別化機能。設計段階で「AI からアクセス可能な操作範囲」と「認可の適用方法」を確定。

**AI Safety by Design**：nori と協働し、以下を設計段階で確定：① プロンプトインジェクション対策（システムプロンプトと user input の分離）② 出力の PII フィルタリング ③ 有害コンテンツフィルタ ④ AI が実行できる操作の allowlist（削除・送金など高リスク操作は Human 承認必須）⑤ AI 応答のログ保存期間と削除フロー。

---

**本アップグレードの適用開始日**：2026-07-19 以降の新規案件からは、上記 8 項目を STEP 2 の設計品質ゲートに組み込む。既存案件は次回大規模改修時に段階適用。architect-checklist.md への項目追加は kai と協議のうえ順次実施。
