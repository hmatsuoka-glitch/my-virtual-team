...
### 2026-07-27
- **[スキル改訂 Step 1/10] 現状スキル棚卸し**: BMAD Architect準拠の要件定義・システム設計に強い一方、C4モデルでの多層可視化と、ADR（Architecture Decision Records）の運用、非機能要件（NFR）の定量化が弱い
- **[スキル改訂 Step 2/10] 2026年下期の業界ベストプラクティス**:
  - C4 Model（Context/Container/Component/Code の4層）
  - Architecture Decision Records（意思決定履歴の資産化）
  - NFR定量化（レイテンシ・可用性・スケーラビリティを数値化）
...
- **[スキル改訂 Step 4/10] 追加すべき新スキル定義**:
  - C4 Model 4層設計標準化
  - ADR .md テンプレ運用（意思決定・代替案・帰結を記録）
  - NFR定量表（レイテンシms・可用性%・RPS等）
- **[スキル改訂 Step 5/10] 新規ツール・技術スタック**:
  - Structurizr（C4 Model図生成）
  - Notion ADR Database
  - Mermaid（設計書内図生成）
...
- **[スキル改訂 Step 8/10] 連携強化ポイント**:
  - kai（PM）: 設計完了ゲートに ADR + NFR 必須
  - ao（BE）: Container図でAPI境界共有
  - riku（FE）: Component図でBFF責務共有
- **[スキル改訂 Step 9/10] ナレッジベース拡張**:
  - ADRデータベース（更新頻度: 意思決定ごと即時）
  - C4 Model事例集（更新頻度: 案件ごと即時）
  - NFR業界ベンチマーク（更新頻度: 四半期）
- **[スキル改訂 Step 10/10] 成長ロードマップ**:
  - 3ヶ月後: C4 Model + ADR全案件必須化
  - 6ヶ月後: 設計書8時間/システム達成
  - 12ヶ月後: NFR定量化100%、社内設計標準として定着
