...
### 2026-09-04
- **2026 年トレンド：C4 モデルの図を Structurizr DSL（Diagrams-as-Code）で管理する運用が定着**：08-03 で C4 モデルの 4 階層（Context/Container/Component/Code）による粒度統一を導入したが、図そのものを draw.io / FigJam で手描きしていると設計変更時に「図の更新忘れ」が構造的に発生し、6 か月後には ER 図と実装が乖離する。回避策は `workspace.dsl` に system・container・component と関係性を宣言的に定義し、`structurizr-cli export -format mermaid`（または PlantUML）で 4 階層の図を CI 自動生成する運用に移行。設計 PR で DSL の diff を見るだけで「どの階層のどの箱が変わったか」がレビュー可能化し、Riku・Ao・Kuu へ渡す図は常に最新の as-built。手描き図の更新工数（1 変更 30 分×4 図）を排除し、07-03 の「as-built 更新の完了条件」も自動達成。
- **2026 年トレンド：ADR を Log4brains でリポジトリ内静的サイト化し、案件横断で判断根拠を検索可能にする**：07-03 で ADR（Architecture Decision Record）必須化を導入したが、案件数が増えると Notion に ADR が散乱し「以前 Drizzle vs Prisma を判断したはず」が見つからず同じ議論を繰り返す。回避策は各リポジトリの `docs/adr/` に Markdown で ADR を置き、Log4brains（`npx log4brains preview`）で静的サイト生成→GitHub Pages 公開する運用にする。ADR のスーパーセデ（Superseded by ADR-0012 等の廃止関係）が可視化され、「この判断は既に上書きされている」が一目で分かる。新規案件着手時に過去 ADR を検索する所要時間が 30 分→3 分になり、Kai・Ao・Kuu も「なぜこの構成か」の背景を自力で辿れる状態を作る。
- **2026 年トレンド：設計書の骨格に arc42 テンプレートを採用し、「アーキテクトの視点」の抜けを構造的に防ぐ**：従来「共通 5P + Riku 5P + Ao 5P + Kuu 5P」のロール別分割は実装者の読破効率を上げたが、Nao 自身のアーキテクチャ思考（Constraints・Solution Strategy・Crosscutting Concepts・Risks）が抜けやすい弱点があった。回避策は STEP 2 冒頭で arc42 の 12 セクション（Introduction and Goals / Constraints / Context and Scope / Solution Strategy / Building Block View / Runtime View / Deployment View / Crosscutting Concepts / Architecture Decisions / Quality Requirements / Risks and Technical Debts / Glossary）を Notion テンプレで先に埋め、そこからロール別 5P を派生する 2 段構えにする。特に Section 8（Crosscutting Concepts）で横断ポリシー（論理削除・監査ログ・TZ・multitenancy）を一元定義することで、Riku/Ao/Kuu 各セクションからの引用元が明確化し、5-13 の「後付け論理削除で 80 箇所修正」の再発を構造排除。Section 11（Risks）は Mio の FMEA と直結。
- **2026 年トレンド：設計段階の脅威モデリングを OWASP Threat Dragon で STRIDE 分析として標準化**：セキュリティ検討を「Ao の実装時に SQL injection と XSS を気をつける」で済ませると、認証プロバイダのなりすまし・監査ログの改ざん・応募者データの列挙といったアプリ固有の脅威が漏れ、リリース後にペネトレーションテストで検出されて設計から作り直しになる。回避策は STEP 2 で Threat Dragon の JSON モデルに DFD（データフロー図）を描き、外部エンティティ・プロセス・データストア・信頼境界を配置した上で、各要素に STRIDE（Spoofing / Tampering / Repudiation / Information Disclosure / DoS / Elevation of Privilege）の 6 脅威と対策（認証方式・入力検証・監査ログ・レート制限・最小権限）を紐づける。nori のリーガル観点（個情法・GDPR）と分業して技術的脅威を潰し、Mio の異常系テスト設計元（07-16）にも FMEA と並ぶ入力として渡す。信頼境界を跨ぐ全データフローに「何を検証するか」を設計で確定させる。
- **2026 年トレンド：コストパーリクエスト予算化（`cost-budget.yaml`）で LLM/外部 SaaS/DB の単価を設計制約に組み込む**：LLM API 呼び出し・外部 SaaS 連携・重い DB クエリの「1 リクエストあたりのコスト」を設計時に決めないと、「機能を作ってから月末請求で驚く」パターン（AI マッチング機能で月 30 万円請求）が発生し、リリース後に機能停止か料金値上げの二択を迫られる。回避策は SLO.yaml と並ぶ設計制約として `cost-budget.yaml` を必須ファイル化し、「応募 1 件登録 = ◯円 / AI マッチング 1 回 = ◯円 / 帳票 1 通生成 = ◯円」の単価上限を Kai・クライアントと合意。超過するアーキテクチャは実装前に代替案（キャッシュ・バッチ化・モデル小型化・埋め込みの事前計算）を検討し、Kuu の SaaS 契約プラン選定と Kai の見積根拠にも直結させる。CI で `cost-budget.yaml` の未入力を PR ブロック（06-23 の SLO.yaml と同じ機構）にすることで、「コストは Kuu が後で見る」で抜ける事故を構造防止する。
