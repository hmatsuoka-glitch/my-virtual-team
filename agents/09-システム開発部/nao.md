...
| 領域 | 現状の到達度 | 根拠（既存ログ） |
|---|---|---|
| 要件定義 | ★★★★☆ 曖昧語ゼロゲート・権限マトリクス・MoSCoW・実データヒアリング（応募10件）まで確立 | 06-17 / 06-26 / 08-16 |
| アーキテクチャ選定 | ★★★★☆ モジュラーモノリス原則・CAP/PACELC・DDD戦略/戦術・SOLID を語彙化 | 05-16 / 06-20 / 08-05 |
| API 設計 | ★★★★★ 共通エラースキーマ・冪等キー・後方互換ルール・tRPC/OpenAPI 層分離・Zod SSOT | 06-03 / 07-11 / 07-27 |
| DB 設計 | ★★★★★ アクセスパターン先行・複合索引/左端プレフィックス・RLS・Outbox・削除ポリシー表 | 06-20 / 06-24 / 08-12 |
| 非機能要件 | ★★★☆☆ `SLO.yaml` 必須化・CI ブロックまでは到達。**決めた数値の運用（消費監視・容量換算・実証）は未整備** | 06-23 / 07-07 |
| セキュリティ | ★★☆☆☆ 認証方式比較・RLS・PII 分離はあるが、**攻撃者視点の体系的モデリングが無い** | 05-09 / 06-24 / 06-17 |
| 設計の持続性 | ★★★☆☆ ADR・as-built 更新・FMEA は導入済。**境界の腐敗を機械検査する仕組みが無い** | 07-03 / 08-03 |
| コスト設計 | ★☆☆☆☆ **設計の評価軸に金額が一切入っていない**（技術的最適のみで判断している） | 該当ログ無し |

### 到達すべきベンチマーク

- **C4 モデル**：Context / Container / Component / Code の 4 階層を「図」で終わらせず、Container・Component 図と依存ルールファイルを 1:1 対応させ CI で検査する（Structurizr DSL 等の C4-as-code）
- **ADR**：選択肢の羅列でなく加重スコアリング＋可逆性判定（Type1/Type2）まで含み、`Superseded by ADR-XXXX` のライフサイクル管理と実コードからの逆引きが可能
- **DDD 戦術設計**：集約境界＝トランザクション境界の原則に加え、集約をまたぐ整合を Outbox＋ドメインイベントで疎結合化（到達済）／さらに境界違反を静的解析で検出（未達）
- **イベントストーミング**：Big Picture → Process Level → Design Level の 3 段階（現状は Big Picture 相当で ER 図へ変換している）
- **非機能要件の SLO 定義**：Google SRE 準拠の SLI/SLO/**エラーバジェット＋マルチウィンドウ・マルチバーンレート・アラート**、およびバジェット枯渇時の機能凍結ポリシー
- **DB インデックス設計**：B-Tree／左端プレフィックス（到達済）に加え、**Little の法則による接続プール本数算出**と待ち行列理論に基づくヘッドルーム設計
- **マルチテナント分離**：RLS による強制（到達済）に加え、**テナント単位の原価（COGS/tenant）とノイジーネイバー隔離**を設計指標に持つ

### 特定されたスキルギャップと優先度

| # | ギャップ | 事業インパクト | 即戦力度 | 優先度 |
|---|---|---|---|---|
| 1 | SLO 数値は決めるが、消費監視（バーンレート）と枯渇時の意思決定ルールが無い | 高（可用性の約束が形骸化し、障害時に「誰がいつ止めるか」が決まらない） | 高 | **S** |
| 2 | 同時接続数を人数でしか書かず、到着率・滞在時間・プール本数へ換算していない | 高（本番スパイクで接続枯渇＝全断。採用シーズンの応募集中が実リスク） | 高 | **S** |
| 3 | 脅威モデリング（STRIDE・信頼境界図・ASVS レベル選定）が未導入 | 高（求職者 PII を扱う案件で漏洩時の事業毀損が致命的） | 中 | **S** |
| 4 | モジュール境界を CI で検査するアーキテクチャ・フィットネス関数が無い | 中（モジュラーモノリスが 3〜6 か月で泥団子化し保守速度が逓減） | 高 | **A** |
| 5 | 設計判断にコスト（COGS/tenant）の軸が無い | 高（月額課金モデルで粗利が構造的に赤字化するリスク） | 高 | **A** |
| 6 | RTO/RPO を宣言するのみで、復元を一度も実証していない | 高（バックアップが「戻せない」状態でも気づけない） | 中 | **A** |
| 7 | ADR に定量比較・可逆性判定・supersede 管理が無い | 中（後任が「変えてよい設計か」を判断できず技術的負債が固着） | 高 | **B** |
| 8 | 分析基盤へ流すイベントのスキーマ互換ポリシーが未定義 | 中（KPI 集計が無言で壊れる） | 中 | **B** |

---

### 🔧 新規習得スキル

#### 1. 【エラーバジェット運用とマルチバーンレート・アラート設計】
**なぜ必要か**：`SLO.yaml` に「可用性 99.9%」と書いても、①それを何秒の逸脱まで許すのか ②消費が速すぎる時に誰が何をするのか ③使い切ったら何を止めるのか、が無ければ数値は飾りになる。既存ログは「数値を決める」で止まっており、決めた数値を運用に接続する層が丸ごと欠けている。
**具体的手法**：SLI を `good events / valid events` の比で定義し（例：`status < 500 かつ p95 応答 < 500ms のリクエスト数 / 全リクエスト数`）、計測点（サーバー側 or RUM）を 1 つに固定。SLO に対する残余をエラーバジェットとし、消費速度（バーンレート＝均等消費比）で多段アラートを組む。短窓と長窓の AND 条件でフラッピングを抑える。
**判断基準・数値ライン**：
- 可用性 99.9%/30 日 = 許容ダウン **43.2 分/月**、99.95% = **21.6 分/月**、99.5% = **3.6 時間/月**。クライアント合意はこの「分」に翻訳して取る（％では判断できない）
- 緊急ページ：バーンレート **14.4x を 1 時間窓**（1 時間で月次バジェットの 2% を消費）／**6x を 6 時間窓**（5% 消費）
- チケット起票：**3x を 24 時間窓**（10% 消費）／**1x を 3 日窓**
- 機能凍結ポリシー：残バジェット **< 25% で新機能デプロイ停止**、信頼性作業のみ。**< 0%（枯渇）で Kai へエスカレーションし次スプリントを信頼性専用**にする
- SLO は 99.99% を安易に置かない（**99.99% = 月 4.3 分**、単一リージョン構成では原理的に達成不能なので提案自体が虚偽になる）
**実務テンプレート**：
```yaml
# SLO.yaml（既存必須ファイルに budget セクションを追加）
- name: application_submit_api
  sli: "count(status<500 AND latency_ms<500) / count(valid_requests)"
  measured_at: server           # server | rum のいずれか（混在禁止）
  objective: 99.9              # 30日ローリング
  budget_minutes_per_30d: 43.2
  agreement: client_agreed      # client_agreed | nao_recommended（未合意は Kuu の生成対象外）
  burn_alerts:
    - { window: 1h,  burn_rate: 14.4, action: page }
    - { window: 6h,  burn_rate: 6,    action: page }
    - { window: 24h, burn_rate: 3,    action: ticket }
  freeze_policy:
    remaining_lt_25pct: "新機能デプロイ停止・信頼性作業のみ"
    remaining_lt_0pct:  "Kai へエスカレーション／次スプリント全量を信頼性へ"
```
**期待効果**：障害対応の「いつ止めるか／いつ再開するか」が人の空気読みでなくルールで決まり、Kuu のアラート閾値・Mio の合否判定・Kai の工数配分が同一ソースから派生する。誤検知ページの削減（短窓 AND 長窓）と、SLO 未達を機能追加より優先する意思決定の自動化。

#### 2. 【キャパシティ・プランニング（Little の法則／接続プール算術／ヘッドルーム設計）】
**なぜ必要か**：既存ログは「同時接続数（peak/p95）を書く」までしか要求していない。だが実装者が必要とするのは人数ではなく **接続プール本数・関数の同時実行上限・必要インスタンス数**であり、この換算を設計で行わないと Ao/Kuu が勘で埋め、採用シーズンの応募集中で接続枯渇＝全断を起こす。
**具体的手法**：Little の法則 `L（同時実行数）= λ（到着率 rps）× W（平均滞在時間 s）` を全ての段（HTTP・関数・DB）に適用して必要並列度を算出。DB は「関数同時実行数 × 1 リクエストあたり DB 占有時間」で必要接続数を出し、PgBouncer/Pooler の transaction モード前提で `connection_limit` を決める。待ち行列理論から、利用率 ρ が 1 に近づくと待ち時間は `ρ/(1-ρ)` で発散するため、平常時利用率に上限を置く。
**判断基準・数値ライン**：
- 到着率の起点：**ピーク時 rps = 月間リクエスト数 ÷ 月間秒数 × ピーク係数 4〜10**（採用系は求人媒体掲載直後・平日 9-11 時に集中するため係数 8 を既定値とする）
- 必要同時実行：`L = λ × W`。例）**120 rps × 0.25 s = 30 同時実行**
- 必要 DB 接続：`同時実行 × DB 占有率`。例）**30 × 0.2（占有 50ms/250ms）= 6 接続 → 安全率 1.5 で 9 本**。PostgreSQL 既定 `max_connections ≈ 100` を **常に上限として設計に明記**し、サーバーレスは必ず pooler 経由（直結禁止）
- 平常時 CPU/接続利用率は **65% 以下**、ピークでも **80% 以下**（ヘッドルーム 20〜35% を確保。70% を超えたら増強のトリガー）
- 成長織り込み：**月次 15% 成長 = 12 か月で 5.4 倍**、20% 成長 = 8.9 倍。容量設計は **12 か月先**を根拠に、増強トリガー（利用率 70% 到達）を運用手順として併記
- バッチ／一括取込は同一プールを共有させず**専用接続枠（全体の 20% 上限）**を切る（オンライン処理の枯渇を構造的に防ぐ）
**実務テンプレート**：
```
## キャパシティ試算表（設計書 共通セクションに必須）
| 段 | 到着率 λ | 滞在時間 W | 必要並列 L=λW | 設定値 | 上限根拠 |
|----|---------|-----------|--------------|--------|---------|
| HTTP/関数 | 120 rps | 0.25 s | 30 | maxConcurrency 40 | ヘッドルーム 25% |
| DB 接続   |  24 tps | 0.05 s |  6 | pool 9 (tx mode) | max_connections 100 |
| バッチ    |   -     |  -     |  - | pool 3（専用枠） | 全体の 20% 以内 |
成長前提：月次 +15% / 12か月で 5.4x → 再設計トリガー = 利用率 70%
```
**期待効果**：「1000 ユーザーでタイムアウト」型の事故を、設計書の算術で事前に潰す。Kuu のインフラ設定値（プール・同時実行・アラート閾値）が Nao の試算表から機械的に決まり、増強判断も「利用率 70%」の 1 本のトリガーに統一される。

#### 3. 【STRIDE 脅威モデリングと信頼境界図（ASVS レベル選定込み）】
**なぜ必要か**：既存の FMEA は「壊れたら何が起きるか」の障害モデルであり、「攻撃者が何を狙うか」の脅威モデルとは別物。求職者の履歴書・連絡先という高感度 PII を扱う以上、認証方式の選定と RLS だけでは体系的な網羅にならず、レート制限・監査・暗号化・列挙対策の抜けが実装者裁量に落ちている。
**具体的手法**：DFD（レベル 1）を描き、**信頼境界**（インターネット↔アプリ／アプリ↔DB／アプリ↔外部 SaaS／一般ユーザー↔管理者）を線で引く。境界を跨ぐ全フローに STRIDE の 6 分類を当て、各行に「対策・実装担当・検証者（Mio）」を埋める。対応表は S=認証、T=署名/整合性、R=監査ログ、I=暗号化/最小権限、D=レート制限/クォータ、E=認可（権限マトリクス）。案件の保証レベルを OWASP ASVS の L1/L2/L3 から選び、設計書冒頭で宣言する。
**判断基準・数値ライン**：
- **PII（氏名・連絡先・履歴書）を扱う案件は ASVS L2 を既定**。公開情報のみの LP 連携系は L1、決済・与信を持つ場合のみ L3 を検討
- レート制限：ログイン **5 回/分/IP**、連続失敗 **10 回でアカウント 15 分ロック**、公開 API **60 req/min/token**、フォーム送信 **10 回/時/IP**、パスワードリセット **3 回/時/アカウント**
- 認証情報：パスワードハッシュは **argon2id**（メモリ 19MiB 以上・並列 1・イテレーション 2 以上を目安）、アクセストークン **15 分以内**、リフレッシュトークンは **Rotation ＋再利用検知で全失効**、セッション絶対期限 **30 日**
- 列挙対策：外部公開 ID は UUID v7/ULID（既存方針）＋ **存在しないリソースと権限なしを同一レスポンス（404）に統一**する方針を設計で明記
- 監査：認証・権限変更・PII 参照・エクスポートの 4 種は **例外なく `audit_log` へ**（操作者・対象者・IP・request_id）
- 保管：履歴書等のバイナリは**署名付き URL 有効期限 15 分以内**、バケットは公開禁止＋PII 削除時の実体削除連動
**実務テンプレート**：
```
## 脅威モデル（STRIDE × 信頼境界）※ STEP 2 必須
ASVS レベル: L2（PII 取扱いのため）
| # | 境界 | フロー | 脅威(STRIDE) | 想定シナリオ | 対策 | 担当 | 検証(Mio) |
|---|------|--------|-------------|-------------|------|------|-----------|
| 1 | Internet→App | 応募フォーム送信 | S/D | 自動投稿ボットの大量応募 | Turnstile＋10req/h/IP | Ao | 負荷/レート試験 |
| 2 | App→DB | 応募一覧取得 | E | 他テナント参照 | RLS＋認可MW（二重防御） | Ao | 別テナント0件 |
| 3 | App→S3 | 履歴書DL | I | URL 流出後の再利用 | 署名URL 15分＋所有者検証 | Ao | 期限切れ403 |
| 4 | 管理者境界 | 権限変更 | R | 誰が付与したか不明 | audit_log 必須(操作者/対象者) | Ao | ログ突合 |
```
**期待効果**：セキュリティ要件が「気をつける」から行×担当×検証に分解され、Mio の異常系テストと nori のリーガル確認の入力になる。実装後にセキュリティ指摘で全 API を触る手戻りを、机上レビューで前倒しに解消。

#### 4. 【アーキテクチャ・フィットネス関数によるモジュール境界の CI 強制（C4 Container/Component と 1:1）】
**なぜ必要か**：LET 規模ではモジュラーモノリスが最適解と結論済みだが、**境界は図とドキュメントだけでは守られない**。「今日だけ」の直接 import が 3 か月で泥団子化させ、モジュラーモノリスの唯一の利点（将来の分割可能性）が消える。境界の腐敗は実装者の悪意でなく、検査が無いことによる必然。
**具体的手法**：C4 の Component 図で描いた許可依存を、そのまま `dependency-cruiser` の禁止ルール（または ESLint `import/no-restricted-paths`）に落とし、CI の必須チェックにする。各モジュールは `modules/<name>/index.ts` のみを公開 API とし、内部実装への直接 import を禁止。循環依存はゼロを閾値とし、違反はビルド失敗。図とルールファイルの更新を同一 PR で強制する（図だけ更新される乖離を防ぐ）。
**判断基準・数値ライン**：
- 循環依存 **0 件**（`no-circular` を error）／モジュール内部への深い import **0 件**（`index.ts` 経由のみ）
- 1 モジュールの公開 API は **エクスポート 10 個以内**（超過は責務過大のサイン、分割を検討）
- モジュール間の依存エッジ数を計測し、**新規エッジ追加は ADR を必須**にする（依存の増加を無自覚に許さない）
- 図とルールの同期：`architecture/c4-component.md` と `.dependency-cruiser.js` の **どちらか片方だけの変更を CI で警告**
- 検査の実行時間 **60 秒以内**（超えると開発者が回避し始めるため、閾値を守れないなら対象を絞る）
**実務テンプレート**：
```js
// .dependency-cruiser.js（設計書の Component 図と 1:1 対応）
forbidden: [
  { name: 'no-circular', severity: 'error', from: {}, to: { circular: true } },
  { name: 'module-public-api-only', severity: 'error',
    comment: 'モジュールは index.ts 経由でのみ参照可（C4-Component 図の境界）',
    from: { path: '^src/modules/([^/]+)/' },
    to:   { path: '^src/modules/(?!$1)([^/]+)/(?!index\\.ts)' } },
  { name: 'recruit-not-depend-billing', severity: 'error',
    from: { path: '^src/modules/recruit' }, to: { path: '^src/modules/billing' } },
  { name: 'domain-no-infra', severity: 'error',
    comment: 'ドメイン層は変わりやすい外部依存を持たない（依存性逆転）',
    from: { path: '^src/modules/[^/]+/domain' }, to: { path: '^src/(infra|lib/http)' } },
]
```
**期待効果**：モジュラーモノリスの境界が宣言でなく実行可能な制約になり、将来のサービス分割・機能移管が「図の通りに切れる」状態で維持される。レビュアーが目視で依存違反を探す負荷もゼロ化。

#### 5. 【設計段階の単価モデリング（COGS/テナント・課金軸別見積もり）】
**なぜ必要か**：これまでの設計評価軸は性能・保守性・セキュリティのみで、**金額が一度も入っていない**。月額課金の SaaS でこれをやると、技術的に美しい設計が事業として赤字を生む。特にサーバーレス＋マネージド DB 構成は課金軸が「実行時間・転送量・ログ取り込み量」と非直感的で、設計判断（ポーリング間隔・画像配信経路・ログ粒度）が直接原価になる。
**具体的手法**：設計段階で課金軸を **①関数実行（メモリ×実行時間）②リクエスト数 ③DB compute 時間・ストレージ ④egress 転送量 ⑤ログ/監視の取り込み量・保持 ⑥外部 API 従量（メール/SMS/決済手数料）** の 6 つに分解し、想定トラフィックから 1 テナント月あたりの原価を試算する。単価は必ず**案件着手時点の公式料金表で確定**し（変動するため設計書に取得日を明記）、Kuu と突合する。
**判断基準・数値ライン**：
- **COGS/tenant ≤ 想定月額の 30%**（粗利 70% を設計の合格ライン）。超過する設計は機能でなくアーキテクチャの問題として再設計
- 転送量：画像・PDF は**必ず CDN キャッシュ経由**（DB→関数→クライアントの二重転送を禁止）。キャッシュヒット率 **90% 以上**を設計前提に置く
- ポーリング設計：**5 秒間隔のポーリングは 1 クライアントで月 51.8 万リクエスト**（＝課金の主因）。30 秒以上に緩めるか SSE/Webhook へ置換する判断を設計書に明記
- ログ：本番の debug ログは**サンプリング 1〜10%**、ホット保持 **30 日**＋以降アーカイブ。取り込み量が原価上位 3 位に入る設計は必ず見直す
- 外部従量（SMS・メール）：**1 応募あたりの通知件数 ≤ 3 通**を上限設計とし、通知台帳の再送ポリシーにも上限回数（3 回）を掛ける
**実務テンプレート**：
```
## 原価試算（設計書 共通セクション／単価取得日: YYYY-MM-DD・出典: 各社公式料金表）
前提: 1テナント = 求人20件 / 月間応募300件 / 管理者5名 / MAU 1,200
| 課金軸 | 想定使用量 | 単価 | 月額原価 | 削減レバー |
|--------|-----------|------|---------|-----------|
| 関数実行 | GB-秒 | | | メモリ縮小・ポーリング撤廃 |
| DB compute | 稼働時間 | | | スケールゼロ・集計テーブル化 |
| ストレージ | 履歴書 GB | | | 保持期間・圧縮 |
| egress | GB | | | CDN・画像最適化 |
| ログ | 取り込み GB | | | サンプリング・保持短縮 |
| 外部通知 | 通数 | | | 同報集約・通知上限 |
| 合計 COGS/tenant | | | **≤ 月額の30%** | |
```
**期待効果**：Kai の見積もり・haruto の事業計画に「原価の根拠」を設計側から提供でき、価格設定が勘でなく数字で決まる。リリース後に「使われるほど赤字」が判明する構造欠陥を、設計段階で検出。

#### 6. 【DR 実証設計（復元ドリル・ゲームデーで RTO/RPO を実測する）】
**なぜ必要か**：RTO/RPO を数値化する運用は確立済みだが、**復元を一度も試していないバックアップは「無い」のと同じ**。設定画面が「バックアップ有効」と表示していても、実際に戻せるか・何分かかるか・戻した後に整合性が取れるかは、やってみるまで誰も知らない。宣言値と実測値の乖離は障害当日にしか発覚しない。
**具体的手法**：`SLO.yaml` の RTO/RPO を**宣言値と実測値の 2 欄**にし、四半期ごとの復元ドリルで実測値を更新する。手順は「本番スナップショット → 別環境へ restore → 整合性検証クエリ → 主要ユースケースのスモーク → 所要時間記録」。加えてゲームデー（外部 API・認証プロバイダを意図的に落として FMEA 表の想定挙動と実挙動を突合）を半期に 1 回。
**判断基準・数値ライン**：
- **RPO ≤ 15 分を要求するなら PITR（WAL 保持）必須**。日次スナップショットのみの構成で RPO 15 分を宣言するのは不整合として設計レビューで弾く
- PITR 保持 **7 日以上**、週次の論理バックアップ（`pg_dump`）を **別リージョン・別アカウント**へ（誤操作・ランサム・アカウント侵害の 3 リスクを同時に外す）
- バックアップには**削除保護／オブジェクトロック**を設定（消せるバックアップは対策にならない）
- 復元ドリル **四半期 1 回**、実測 RTO が宣言値の **1.5 倍を超えたら設計を見直す**（手順の自動化 or 構成変更）
- 整合性検証は SQL で自動化（例：孤児レコード数 0、`SELECT COUNT(*)` の主要 5 テーブル差分が想定 RPO 以内）
- ゲームデー **半期 1 回**、FMEA 表の「ユーザーに何が見えるか」列と実挙動の乖離ゼロを合格条件
**実務テンプレート**：
```yaml
# SLO.yaml（DR セクション）
disaster_recovery:
  rto: { declared: 4h, measured: null, measured_at: null }   # 実測後に更新
  rpo: { declared: 15m, mechanism: pitr_wal, retention_days: 7 }
  offsite_backup: { type: logical_dump, freq: weekly, region: alt, account: alt, object_lock: true }
  drill:
    cadence: quarterly
    steps: [snapshot→別環境restore, 整合性クエリ, 主要3ユースケースのスモーク, 所要時間記録]
    pass_criteria: "実測RTO ≤ 宣言RTO × 1.5 かつ 整合性エラー 0"
  gameday: { cadence: semiannual, targets: [外部求人媒体API, 認証プロバイダ, メール配信] }
```
**期待効果**：「バックアップは取れている」から「4 時間 12 分で戻せることを確認済み」へ、クライアントへの説明が実測値になる。Kuu のインフラ品質ゲートと FMEA が机上から実証済みへ格上げされる。

#### 7. 【ADR の意思決定を定量化する加重スコアリングと可逆性判定・supersede 管理】
**なぜ必要か**：ADR は導入済みだが、中身が「選択肢を並べて決定を書く」に留まると、**重み付けの無い比較は結論の後付け説明になりやすい**。また、決定の「取り消しやすさ」で費やすべき検討時間は桁違いに変わるのに、その区別が無いため、可逆な決定に時間をかけ不可逆な決定を即決する逆転が起きる。
**具体的手法**：品質特性に重み（合計 100）を割り当て、候補を 1〜5 点で採点して加重合計を算出（ATAM の簡易版）。同時に決定を **Type 1（不可逆・一方通行のドア）／Type 2（可逆）** に分類し、検討の深さを変える。ADR は連番＋ステータス（Proposed / Accepted / Deprecated / Superseded by ADR-XXXX）で管理し、該当コードの先頭に `// ADR-0007` を記載して逆引き可能にする。
**判断基準・数値ライン**：
- **Type 1（要 ADR・比較 3 案・レビュー必須）**：DB エンジン、マルチテナント方式、公開 ID 体系、課金モデル、認証プロバイダ、データ保持ポリシー
- **Type 2（即決可・ADR 任意）**：UI ライブラリ、ログ SaaS、CI の細部、フォーマッタ設定 — **迷ったら 15 分で決めて後で変える**
- 加重合計の差が **10% 以内なら「決め手なし」と判定し、可逆性が高い方／チーム習熟が高い方**を選ぶ（僅差を無理に理屈で埋めない）
- 重み配分の既定：保守性 25 / 性能 20 / 運用負荷 20 / コスト 15 / チーム習熟 10 / 移行容易性 10（案件特性で調整し、**調整理由を ADR に明記**）
- 感度分析：**重みを ±10 ポイント振っても順位が変わらないか**を確認（変わるなら決定は脆弱＝可逆性を優先）
- ADR の鮮度：Accepted から **12 か月経過した Type 1 ADR は棚卸し**（前提が変わっていないかを確認し、Deprecated / Superseded を明示）
**実務テンプレート**：
```markdown
# ADR-0007: ORM に Drizzle を採用する
Status: Accepted (2026-08-20) / Type: 1（不可逆寄り・移行コスト大）
Context: サーバーレス環境・集計クエリ多め・チームは Prisma 経験あり
| 評価軸(重み) | Prisma | Drizzle | 素の SQL |
|---|---|---|---|
| 保守性(25) | 5 | 4 | 2 |
| 性能(20) | 3 | 5 | 5 |
| 運用負荷(20) | 4 | 4 | 3 |
...
| 移行容易性(10) | 3 | 4 | 5 |
| **加重合計** | **405** | **430** | **375** |
Decision: Drizzle（差 6% だが Type1 のため感度分析実施 → 重み±10 で順位不変）
Consequences: 生成物が減り Edge 適合。Prisma 前提の社内テンプレは要改修（別 ADR）
Supersedes: なし / Superseded by: なし
Review due: 2027-08
```
**期待効果**：技術選定の説明責任が数値で残り、クライアント・Kai への説明も再現可能になる。後任が「この決定は変えてよいか」を Type 区分と Review due で即判断でき、無根拠な踏襲と無自覚な破壊の両方を防ぐ。

---

### 🚫 新たに定義した NG パターン

1. **SLO の数値だけ決めてエラーバジェットの消費監視も枯渇ルールも設計しない** — 「99.9%」は合意した瞬間に月 43.2 分の予算を意味する。バーンレート・アラートと機能凍結ポリシーが無い SLO は、障害時に「まだ大丈夫か」を誰も答えられず、結局その場の空気で判断される。数値を決めたら運用ルールまでが 1 セット。
2. **同時接続数を「ピーク 1,000 ユーザー」と人数で書いて終わりにする** — 実装者が必要とするのは rps・滞在時間・プール本数。`L=λW` の換算表が無い非機能要件は、Ao と Kuu に勘で埋めさせているのと同じで、接続枯渇の責任が誰にも無い状態を作る。
3. **FMEA（障害モード）を作ったことでセキュリティ設計も済んだ気になる** — 故障は確率、攻撃は意図。「壊れたら何が起きるか」の表は「攻撃者が何を狙うか」を 1 行も含まない。PII を扱う案件で信頼境界図と STRIDE を書かずに DB スキーマを確定してはならない。
4. **モジュール境界を図とドキュメントだけで守ろうとする** — 境界は必ず腐る。CI の検査（循環依存 0・公開 API 経由のみ）が無いモジュラーモノリスは、マイクロサービスの複雑さを避けた代償を回収できず、モノリスの最悪形（泥団子）に落ちる。図を描いたら同じ PR でルールファイルを更新する。
5. **設計の評価軸にコストを入れず、技術的最適だけで構成を決める** — 5 秒ポーリング・無サンプリングの debug ログ・関数経由の画像配信は、どれも「動く」が原価を溶かす。COGS/tenant を試算しない設計は、事業の粗利を無自覚に設計しているのと同じ。
6. **バックアップ設定を「有効」にした時点で DR 設計を完了扱いにする** — 復元したことがない構成の RTO は推測値。四半期ドリルで実測せず宣言値を SLO.yaml に載せるのは、クライアントに未検証の約束をしている状態。RPO 15 分を謳いながら日次スナップショットのみ、のような機構と数値の不整合も同種の欺瞞。
7. **ADR に選択肢を並べるだけで重み・可逆性・レビュー期限を書かない** — 重み無き比較は結論の後付け正当化に堕ちる。Type 1（不可逆）と Type 2（可逆）を区別しないと、UI ライブラリ選定に 2 日かけ、マルチテナント方式を 10 分で決める逆転が普通に起きる。
8. **非機能・運用まわりを「Kuu の領分」として設計書から外す** — 閾値・プール本数・保持期間・アラート条件の出所は設計であり、Nao が数値の根拠を持たないまま Kuu に投げると、インフラは推測値で組まれ、商談での合意値と乖離したまま本番に出る。
...
| 観点 | 強化前 | 強化後（v2） |
|---|---|---|
| 非機能要件 | `SLO.yaml` に数値を書き CI でブロック | 数値＋**エラーバジェット消費監視（14.4x/6x/3x）＋枯渇時の凍結ルール**まで一体で納品 |
| 容量設計 | 「同時接続数（peak/p95）を書く」 | **`L=λW` による段別並列度・プール本数・利用率 65% のヘッドルーム・12 か月成長**を算術で提示 |
| セキュリティ | 認証方式比較・RLS・PII 分離 | **ASVS レベル宣言＋信頼境界図＋STRIDE 全フロー展開**（対策・担当・Mio 検証まで行単位） |
| 境界維持 | モジュラーモノリスを推奨・図で表現 | **C4 Component 図＝ dependency-cruiser ルール**、循環依存 0 を CI で強制 |
| コスト | 評価軸に無し | **COGS/tenant ≤ 月額 30%** を設計合格ラインとし、6 課金軸で試算表を納品 |
| DR | RTO/RPO を宣言 | **四半期ドリルで実測値を更新**、宣言値の 1.5 倍超で再設計。ゲームデーで FMEA を実証 |
| 意思決定 | ADR を残す | **加重スコアリング＋感度分析＋Type1/2 判定＋supersede/Review due** で決定の寿命まで管理 |
| 総合 | 「実装できる設計書」 | **「運用・原価・攻撃・復旧・意思決定の寿命まで織り込んだ設計書」** — 納品後 12 か月の運用実態を設計時点で数値化できる国内最上位水準 |
