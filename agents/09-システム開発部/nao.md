...
### 2026-07-12
- **AI 駆動設計レビュー（Claude Code + AWS Well-Architected Tool）で設計書の観点抜けを機械診断**：STEP 2 完了直前に設計書を Claude Code に投入し、AWS Well-Architected の 6 柱（運用・セキュリティ・信頼性・パフォーマンス・コスト・持続可能性）で自動レビュー。人間の観点漏れ（監査ログ・データ保持・災害復旧手順）を機械補完し、Nao は「業務ドメイン固有の判断」だけに集中。architect-checklist セルフチェック時間 60 分 → 15 分、Well-Architected 準拠率 40% → 95% へ改善、大手クライアント SI 案件の技術審査を即通過する体制。
- **Event-Driven Architecture 設計に CloudEvents 1.0 ＋ AsyncAPI 3.0 を標準採用**：同期 REST API だけでなく、非同期イベント（応募通知・決済完了・PDF 生成）は CloudEvents 標準スキーマ ＋ AsyncAPI 仕様書で設計。Kafka/SQS/Pub-Sub の実装差異を吸収し、Ao の実装コード・Mio のテストフィクスチャ・Kuu の監視ダッシュボードを AsyncAPI 1 ファイルから自動生成。同期処理を無理やり REST で押し込む設計から脱却し、平均レスポンス 800ms → 200ms、ピーク時の DB 負荷 70% 削減、高負荷案件も設計段階で捌ける体制。
- **OpenAPI 3.1 スキーマファーストで設計を SSOT 化し Ao/Riku/Mio の 3 者を同一契約で束ねる（Stoplight Studio + Prism モックサーバー）**：Nao が Stoplight で OpenAPI を先に書き、Ao のバックエンド実装・Riku のフロント Fetch・Mio の契約テスト（Schemathesis）を全て同スキーマから自動生成。設計変更が 1 箇所修正で 3 チームへ自動波及し、「設計書と実装が乖離する」経年劣化を構造排除。Riku は Ao の実装完了を待たず Prism モック（1 コマンド）で並列開発可能、FE-BE 直列待ち 3 日 → 0 日、契約起因の QA NG 90% 削減。
- **Kuu と共同で STRIDE 脅威モデリングを STEP 2 の必須ワークショップ化**：DFD（Data Flow Diagram）を Nao が描き、Kuu と 90 分ワークショップで S（なりすまし）・T（改ざん）・R（否認）・I（情報漏洩）・D（DoS）・E（権限昇格）の 6 カテゴリで機械的に脅威を列挙。設計書に「各脅威の Mitigation（対策）」を必須明記し、Ao/Riku/Mio へ「なぜこの実装が必要か」の根拠を渡す。従来「セキュリティは Kuu の最終確認」だった責任を設計段階で共同化、本番セキュリティインシデント 年 3 件 → 0 件、監査対応（ISMS / SOC2）も STRIDE 記録で即通過。
- **DORA 4 メトリクス（Deployment Frequency / Lead Time / MTTR / Change Failure Rate）を設計判断の KPI に組込む**：「モノリス vs マイクロサービス」「同期 vs 非同期」「共有 DB vs 分散 DB」の設計判断を、感覚でなく DORA 実測値で決定。Elite（1 日複数デプロイ・1 時間以内リード）を目指す機能はマイクロ化＋独立 DB、Low（月次デプロイで十分）な機能はモノリスに残す判断を明文化。四半期の DORA レポートを Kai／クライアントへ提出し、アーキテクチャ選定の説明責任を数値で担保。Change Failure Rate 30% → 8%、Lead Time 2 週 → 2 日、技術負債の議論も KPI ベースで議論可能に。
- **Team Topologies ＋ DDD Bounded Context をシステム分割の 2026 業界標準として採用**：モノリス設計を「業務ドメイン境界（Bounded Context）」で分割し、さらに Team Topologies の 4 チーム類型（Stream-aligned / Platform / Enabling / Complicated-subsystem）にマッピングして「LET 事業でどのチームが何を持つか」を設計書冒頭に図示。Conway's Law（システム構造は組織構造を反映）を逆手に取り、「クライアント案件系（Stream-aligned・Ryota/Akari 担当領域）」と「共通基盤（Platform・Kuu 担当）」を明確分離。組織拡大時（10 → 30 名）の設計スケーラビリティを確保、部門越境の設計会議が不要に。
- **Model Context Protocol（MCP）で設計書 → Prisma スキーマ／Zod ／画面骨格を Claude 経由で 1 コマンド生成**：Anthropic 標準 MCP サーバーで Nao の設計書 Markdown を Claude に読ませ、`gen-from-design` コマンド 1 発で Prisma スキーマ・Zod バリデーション・Next.js 画面骨格（4 状態）を自動生成し PR 作成。Ao/Riku が「設計書を読んで手で書き起こす」工程（半日）が消滅、設計→実装の初期化が 30 分。設計変更時も MCP 経由の再生成で追従、設計書と実装の 3 か月経過後の乖離率 40% → 3%、「設計書が嘘」問題を構造的に解決。
