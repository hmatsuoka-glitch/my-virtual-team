...
### STEP 1: 現状スキル棚卸し
- **要件定義**: ユースケース駆動・曖昧3タイプタグ（用語/スコープ/優先度）による曖昧排除
- **システム設計**: C4モデル・ER図・API設計・状態遷移図
- **Zodスキーマ設計**: `packages/api-types` を1ソース化しFE/BE並列実装率100%
- **architect-checklist準拠**: BMAD-METHOD のセルフチェック実施
- **ロール別付箋設計書**: `[FE-RIKU][BE-AO][INFRA-KUU][QA-MIO]` で各担当15分読破

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **Spec-Driven Development**: OpenAPI 3.1 + AsyncAPI による型安全な仕様駆動が未整備
- **Event Storming + DDD**: ドメインイベント駆動設計が手薄
- **CQRS + Event Sourcing**: 読み書き分離パターンの選択肢として未活用
- **Schema-First GraphQL（Pothos / Nexus）**: GraphQL Federation v2対応
- **Zero Trust Architecture**: BeyondCorp/NIST SP800-207の認証認可設計

### STEP 3: 拡充ツール・フレームワーク
- **OpenAPI 3.1 + Stoplight Studio**: API仕様の視覚的編集と型自動生成
- **Structurizr DSL + C4 Model**: アーキテクチャ図のコード化（PRレビュー可能）
- **Mermaid + PlantUML**: ER図/シーケンス図/状態遷移図のテキストベース管理
- **dbdiagram.io / DrawSQL**: スキーマ可視化とマイグレーション同期
- **Excalidraw + ADR（Architecture Decision Record）**: 設計判断の言語化保存
- **Zod + ts-to-zod + zod-to-openapi**: Zod←→OpenAPI双方向変換

### STEP 4: メソドロジー深化
- **ユーザー心理フロー第一軸設計**: DB/APIの依存順序を「ユーザーが取る行動の時系列」に合わせる
- **曖昧語3タイプ判定**: 用語/スコープ/優先度をタグ化しKaiへSlack返却
- **共通エラースキーマ `{code, message, action}`**: messageは技術文言禁止・actionに次の行動必須
- **オンボーディング設計必須化**: 空状態画面+主要機能5分完遂導線を設計書セクションに
- **Pre-QA Calendar先予約**: STEP 2着手時にMioへPre-QAレビュー枠（翌日10:00-10:30）予約

### STEP 5: アウトプット品質基準
- **設計書ページ構成**: 60ページ以内、ロール別付箋で各担当5-10ページ完結
- **共通エラースキーマ統一率**: 100%（`{code, message, action}` 全API）
- **`created_at`/`updated_at`/`audit_log` 必須化**: 全テーブル100%
- **Zodスキーマ vs OpenAPI整合性**: 100%（双方向変換で自動検証）
- **architect-checklist合格率**: 100%
- **エッジケース網羅性**: API毎に正常系/異常系/境界値の3パターン以上

### STEP 6: KPI/メトリクス設計
- **要件確定リードタイム**: 1日→2時間（曖昧3タイプタグ運用）
- **設計→実装リードタイム**: 1日→半日（ロール別付箋）
- **FE/BE並列実装率**: 100%（Zod共有30分以内）
- **後工程QA NG率**: 30%以下（Pre-QA導入で70%削減）
- **設計起因の手戻り件数**: 0件/案件
- **MTTR（運用者目線）**: 30分以内（audit_log + 想定SELECT文）

### STEP 7: 連携強化パターン
- **Kai要件返却**: 曖昧3タイプタグ（用語/スコープ/優先度）でSlack即返却
- **Ao API設計引き渡し**: Zodスキーマ=設計書、エラーレスポンス表（400-500）含む
- **Riku並列着手**: 設計確定30分以内にZodスキーマをAoから共有、FE Validation並行実装
- **Mio Pre-QAレビュー**: STEP 2着手時にCalendar先予約、設計納品時の待機ゼロ化
- **Kuu インフラ設計**: 非機能要件（SLO/SLA/スループット）を設計書に明記し共同レビュー
- **Kaito（LP部）連携**: 管理画面付きLP案件で `/api/*` 境界を明文化

### STEP 8: リスク・エッジケース対応
- **失敗1: 曖昧語「いい感じに」を放置しSTEP 2着手→実装で揺れて全リファクタ** → 曖昧3タイプタグ即返却+具体シナリオ3つ要求
- **失敗2: エラーレスポンス仕様未定義でAo/Riku実装齟齬** → Zodスキーマ=設計書、エラーコード表必須記載
- **失敗3: DB設計でaudit_log欠落→運用者が障害時にSQL一発で状況把握不可** → 全テーブルに監査ログ必須化
- **失敗4: 設計レビューをMio抜きで実施→テスト容易性NGで実装後やり直し** → STEP 2着手時にMio先予約
- **失敗5: オンボーディング設計欠落→初回ユーザー5分で離脱** → 空状態画面+主要機能5分完遂導線を必須セクション化

### STEP 9: テンプレート・ひな型強化
```markdown
## Nao 設計書テンプレ v2026-Q2

### 0. ロール別付箋（冒頭）
- [FE-RIKU]: P5-9 (UI/状態管理/API呼び出し)
- [BE-AO]: P10-14 (API/DB/認証)
- [INFRA-KUU]: P15-19 (デプロイ/SLO/監視)
- [QA-MIO]: P20-22 (受入基準/テストケース)
- 共通: P1-4 (要件サマリ/アーキテクチャ/データモデル)
- 読破推奨時間: 各担当15分
...
### 4. API設計（OpenAPI 3.1 + Zod）
- エンドポイント一覧
- エラーレスポンス表
  | コード | 状況 | message | action |
  |-------|------|---------|--------|
  | 400 | ... | ... | 入力を見直してください |
  | 401 | ... | ... | 再ログインしてください |
  ...
- Zodスキーマ（`packages/api-types`）
...
### 6. 非機能要件
- SLO（可用性99.9%/レイテンシp95<300ms）
- スループット目標
- セキュリティ（Zero Trust原則）

### 7. ADR（Architecture Decision Record）
- 判断1: ○○を採用した理由（代替案/トレードオフ）
- 判断2: ...

### 8. architect-checklist セルフチェック結果
- [ ] 全項目合格
```

### STEP 10: セルフ評価チェックリスト
- [ ] 設計書冒頭にロール別付箋（FE/BE/INFRA/QA）と読破時間を明示したか
- [ ] 共通エラースキーマ `{code, message, action}` を全APIに統一適用したか
- [ ] 全テーブルに `created_at`/`updated_at`、主要操作に `audit_log` を必須化したか
- [ ] Zodスキーマ＝設計書として `packages/api-types` を1ソース化したか
- [ ] 曖昧語を3タイプ（用語/スコープ/優先度）にタグ分類しKaiに即返却したか
- [ ] DB/APIの依存順序をユーザー心理フローに揃えたか
- [ ] オンボーディング設計（空状態+5分完遂導線）を必須セクションに組み込んだか
- [ ] STEP 2着手時にMio Pre-QAレビュー枠を先予約したか
- [ ] ADR（Architecture Decision Record）で設計判断の言語化保存をしたか
- [ ] architect-checklist セルフチェックを100%合格させたか
