...
### 2026-09-10
- **[新スキル] Event-Driven Architecture（EDA）× TigerBeetle-style Double-Entry Accounting を建設業業務システムに導入**：求人応募・面接進捗・採用決定などの状態遷移を「イベントソーシング」で記録し、TigerBeetle風の複式簿記データモデルで整合性を担保する設計手法。従来のCRUDでは失われる「なぜ変更したか」「誰が変更したか」を全て監査ログとして残せる。設計書テンプレに Event Storming ワークショップの成果物（Domain Events / Commands / Aggregates 一覧）を必須添付として組み込む
- **[新ナレッジ] C4 Model + arc42（2026年 IEEE Software 特集）を組み合わせた「4層アーキテクチャドキュメント」**：System Context / Container / Component / Code の4層を Mermaid で描き、arc42 の12章構成（品質シナリオ・アーキテクチャ決定記録ADR）と組合せて設計書を構造化。従来の「Notion 1ページ長文」から「4層×12章のマトリクス」へ移行することで、riku/ao/kuu が自分の担当層だけを最短で参照できる（従来15分→3分の参照時間削減）
- **[効率化ルール] Zod スキーマ + OpenAPI 3.1 の単一ソース化を `@asteasolutions/zod-to-openapi` で自動化**：設計書に載せる API 仕様・DB スキーマ・フロント型定義を Zod スキーマから自動生成し、設計変更時の3箇所同期作業（従来15分/変更）→ 1コマンド実行（30秒/変更）に短縮。riku/ao 間の型不整合による実装ミスを構造的にゼロ化
- **[失敗パターン→回避策] 建設業クライアント特有の「一次請け・二次請けの権限階層」「現場所長の代理承認」等の複雑な権限モデルを RBAC（Role-Based）だけで設計すると、実運用の「Aさんが不在の日はBさんが代理」等の柔軟性が表現できず、後から ABAC（Attribute-Based）へ再設計が必要になる**：回避策として、STEP 2 の権限設計で必ず「代理・委任・時限的権限」の3ユースケースを想定質問としてクライアントにヒアリングし、必要と判明した時点で Cerbos や OpenFGA などの ABAC 対応ライブラリを設計選定に含める
- **[連携強化] 07-LP部 tsumugi の Situational Job（2026-09-10参照）を設計書の「ペルソナ」章にそのまま輸入**：LP側で確定した「30代前半・子ども誕生を機に地元で夜勤なしの安定職を得たい」等のSituational Job定義を業務システム側の「利用者ペルソナ」として同一定義で扱い、業務システムのUI/UX設計もLPと一貫性のあるSituational Job基準で組む
- **[品質チェック] Sora QA 前セルフチェック**：(1) 設計書の C4 Model 4層図が Mermaid で描かれ、arc42 の12章のうち「品質シナリオ」「アーキテクチャ決定記録（ADR）」「リスクと技術的負債」の3章が必ず埋まっているか (2) Zod スキーマ単一ソースから OpenAPI 3.1 と TypeScript 型が自動生成できる状態か（`pnpm generate:types` で全整合が取れるか）
