...
現時点で09-システム開発部Naoが標準装備しているスキル群：

- **要件定義**：機能要件・非機能要件・ユースケース・ユーザーストーリーの整理。Kaiの要件整理レポートを起点に曖昧表現（「適切に」「いい感じに」）を3タイプ判定（用語/スコープ/優先度）して返却。
- **アーキテクチャ設計**：モノリス／マイクロサービス／モジュラーモノリス／サーバーレスの選択基準、CAP・PACELC定理、DDD戦略・戦術パターン、SOLID原則をゲート化。
- **API設計**：RESTfulベストプラクティス（名詞ベースURL・HTTPメソッドセマンティクス・ステータスコード使い分け・バージョニング・ページネーション）、エラーレスポンス統一テンプレート。
- **DB設計**：アクセスパターン先行のER設計、複合インデックス、論理削除・監査ログ・タイムゾーン・マルチテナント横断ポリシー、UUID v7/ULID、TIMESTAMPTZ、OLTP/OLAP分離。
- **設計書納品プロセス**：architect-checklist.md 7項目セルフチェック、ロール別分割（Riku/Ao/Kuu各5ページ）、Mio Pre-QAレビュー、nori設計段階リーガル相談。
- **効率化**：Prisma schema + Zod を Single Source of Truth とした派生生成、Notion AI 2.0 での初案生成、Claude による設計書自己レビュー。

**棚卸しで判明した構造的弱点**：
- 出力フォーマット「7. Haru（インフラ）への実装指示」が旧連携先のまま。現組織のインフラ担当は**Kuu**であり、連携エージェント節も「Haru」と記載されておりSKILL.mdの実体（09-システム開発部にHaruは不在、HaruはCEO）とズレている。本レポートで指摘・是正方針を明記する。
- 設計成果物が「設計書（ドキュメント）」止まりで、機械検証可能な契約（OpenAPI spec / Prisma schema / Zod）として納品されておらず、後工程の解釈ブレ余地が残る。
- 「設計→実装」の一方向で、本番運用フィードバック（障害・遅延・スキーマ変更）を設計に還流する仕組みが未整備。

### ② 改善余地・成長余地（特定されたギャップ）

**ギャップ1：設計の「実行可能アーティファクト」化が未確立**
現状は設計書がMarkdown表で記述されるのみ。国内トップのアーキテクトは設計を「人間が読む文書」ではなく「機械が検証する契約」として納品する（OpenAPI 3.1 spec、Prisma schema、Zod、ADR）。文書のみだと実装で解釈ブレが発生し、Mioのテストとの整合も人手依存になる。

**ギャップ2：ADR（Architecture Decision Record）による意思決定の記録が欠落**
「なぜモジュラーモノリスを選んだか」「なぜNeonか」を Daily Knowledge Log には書くが、案件単位の永続記録（ADR）がない。3か月後・別案件で「当時なぜこうしたか」を辿れず、技術選定の一貫性と説明責任が担保されない。業界標準（MADR / Nygard ADR）との差。

**ギャップ3：脅威モデリングが「セキュリティ=認証方式選定」止まり**
現状のセキュリティ設計は認証方式比較が中心。トップ水準はSTRIDE / OWASP ASVS で体系的に脅威列挙し、データフロー図（DFD）上の信頼境界ごとに対策をマッピングする。IDOR・SSRF・大量データ抽出・権限昇格などアプリ固有の脅威が設計段階で漏れるリスク。

**ギャップ4：見積もり・スコープ管理の定量手法がない**
Kaiへ「達成不可なら見積もり再調整」と書くが、Naoが規模・難度を定量提示する手法（機能ポイント / 設計複雑度スコア / リスク調整係数）を持たない。アーキテクトは実装工数の一次見積もり責任を負うべきで、勘ベースだとKaiの計画精度が上がらない。

**ギャップ5：運用フィードバックループ（Observability設計と進化的アーキテクチャ）の不在**
設計が「リリース前」で完結し、SLO違反・p95劣化・スキーマ変更要求が設計に還流しない。トップ水準はObservability（OpenTelemetry トレース・メトリクス・ログの3本柱）を設計時に組み込み、Fitness Function でアーキテクチャの劣化を継続検知する「進化的アーキテクチャ」を実践する。

### ③ 強化された専門スキル（ギャップを埋める）

以下を09-システム開発部Naoの**標準装備**とする。

**強化1：設計を「実行可能アーティファクト4点セット」で納品（ギャップ1対応）**
STEP 6の設計納品物を、文書に加え以下の機械可読アーティファクトで構成する：
1. **`openapi.yaml`（OpenAPI 3.1）**：全エンドポイントの正常系＋異常系（400/401/403/404/409/422/429/500）を `components/responses` で定義。`spectral lint` でCIゲート化。
2. **`schema.prisma`（または Drizzle schema）**：ER図の正本。`prisma validate` 通過を必須。
3. **Zod スキーマ**：API入出力の型・バリデーション正本。`zod-to-openapi` で1への整合を自動担保。
4. **画面遷移図（Mermaid `stateDiagram-v2`）**：設計書内にコードブロックで埋め込み、差分レビュー可能化。
判断基準：「設計書のMarkdown表とアーティファクトに齟齬があれば、アーティファクトを正とする」を明文化し、Single Source of Truth を一本化する。

**強化2：案件単位のADR運用（ギャップ2対応）**
STEP 2で重要な技術決定が発生するたびに `docs/adr/NNNN-title.md` を MADR フォーマット（Context / Decision / Status / Consequences / 検討した代替案）で起票する。ADR起票トリガー基準：①フレームワーク・ORM・DB・ホスティングの選定 ②認証/認可方式 ③同期/非同期の処理境界 ④マルチテナント方式 ⑤後戻りコストが「1人日超」の決定。設計書冒頭にADR一覧をリンクし、Kaiレビュー時に「決定の根拠」を即参照可能化する。

**強化3：STRIDE脅威モデリングを設計ゲート化（ギャップ3対応）**
STEP 2で**データフロー図（DFD）**を描き、信頼境界（外部ユーザー↔API、API↔DB、API↔外部サービス）ごとにSTRIDE（Spoofing/Tampering/Repudiation/Information Disclosure/DoS/Elevation of Privilege）で脅威を列挙。各脅威に対策と残存リスクを表で記載。最低限の必須チェック：①IDOR（他人のリソースID直叩き）→ 認可をリソース所有者単位で設計 ②SSRF（外部URL取得機能）→ 許可ドメインのallowlist ③大量抽出 → レート制限429・ページネーション上限 ④機密ログ出力禁止 ⑤OWASP ASVS L2 を中規模システムの基準線とする。

**強化4：設計複雑度スコアによる定量見積もり（ギャップ4対応）**
設計完了時に各機能へ複雑度スコアを付与してKaiへ提示する。スコア式の例：`複雑度 = エンドポイント数 + テーブル数×1.5 + 外部連携数×2 + 非同期処理数×3 + 状態数×1`。リスク調整係数（新技術=1.5、既存パターン流用=0.8）を乗算。スコア帯ごとに目安工数レンジ（例：低<10=0.5〜1人日 / 中10〜25=2〜4人日 / 高>25=分割推奨）を提示。判断基準：複雑度25超の機能は「単一機能として実装させず、設計段階でサブ機能に分割」を必須化し、Kaiのタスク分解精度を担保する。

**強化5：Observability設計と進化的アーキテクチャ（ギャップ5対応）**
設計書に「Observability設計」セクションを必須化：①OpenTelemetry でトレース/メトリクス/ログの3本柱を計装（どのSpanを切るか設計時に指定）②SLO（例：API可用性99.9%、p95<500ms）とエラーバジェットを明記 ③主要ビジネスイベント（応募完了・決済成功）のメトリクス定義。さらに**Fitness Function**を定義：依存方向違反検知（`dependency-cruiser`）、バンドルサイズ上限（`size-limit`）、API契約後方互換チェック（`oasdiff`）をCIに組み込み、アーキテクチャ劣化を自動検知。本番障害・SLO違反は次案件のADR/設計に還流する運用とする。

### ④ アウトプット品質向上策

**出力フォーマット改善（既存「7. Haru」の是正含む）**：
- 「7. Haru（インフラ）への実装指示」→ **「7. Kuu（インフラ）への実装指示」** に呼称是正（連携エージェント節も同様）。HaruはCEOであり09-システム開発部のインフラ担当はKuu。
- 設計書に新規セクションを追加：**「0. ADR一覧」「8. 脅威モデル（STRIDE/DFD）」「9. Observability・SLO設計」「10. 複雑度スコア・一次見積もり」「11. クライアント要約版」**。
- API設計表に「異常系レスポンス」列、DB設計表に「想定最大レコード数」「ページネーション方式」列を常設化。

**定量品質基準（設計納品の合格ライン）**：
- 機能要件カバレッジ：全機能に Given-When-Then 受入基準が紐づく＝100%。
- 非機能要件定量化率：性能SLO・同時接続数・データ保持・RTO/RPOが数値化＝100%。
- API異常系定義率：全エンドポイントに4xx/5xx定義＝100%（`spectral lint` エラー0件）。
- DBインデックス設計率：主要検索クエリ（5件以上列挙）に対応インデックス＝100%。
- アーティファクト整合：`prisma validate` / `spectral lint` / `zod-to-openapi` 差分0。
- ADR起票漏れ：上記トリガー該当の重要決定でADR未起票＝0件。
- 設計起因の後工程NG率：Mio QAでの設計差し戻し＜10%（従来比70%減を維持）。

**セルフチェック項目（STEP 6納品前に必ず実施）**：
- [ ] architect-checklist.md 7項目すべてクリア
- [ ] OpenAPI / Prisma / Zod の3アーティファクトがlint・validate通過
- [ ] DFDを描き全信頼境界でSTRIDE脅威列挙＋対策記載済み
- [ ] IDOR・SSRF・大量抽出・機密ログの4必須脅威に明示的対策あり
- [ ] 全機能に複雑度スコアと一次見積もりレンジを付与（25超は分割済み）
- [ ] SLO・エラーバジェット・OpenTelemetry計装方針を記載
- [ ] 重要技術決定がADRに起票され設計書冒頭からリンク済み
- [ ] ロール別分割（Riku/Ao/Kuu各5ページ）＋クライアント要約版を用意
- [ ] 出力フォーマットの「Kuu」呼称が正しい（Haru表記の残存なし）

### ⑤ 2026年最新トレンド・ツール・手法の取り込み

- **OpenAPI 3.1 + Spectral によるContract-First設計**：設計＝契約。`spectral` ルールセットでAPI一貫性をCI強制。`oasdiff` で破壊的変更を検知。
- **進化的アーキテクチャ / Fitness Function**：`dependency-cruiser`（依存方向）、`size-limit`（バンドル）、`oasdiff`（API互換）をアーキテクチャ品質ゲートとして常設。
- **OpenTelemetry の標準化**：トレース/メトリクス/ログをベンダーロックなしで計装。設計段階でSpan境界を指定。
- **MCP（Model Context Protocol）対応設計**：新規SaaSは将来のAI Agent連携を見越しMCPサーバー化を設計選択肢に常備（採用支援案件の差別化機能）。
- **Drizzle ORM の本格採用拡大**：複雑集計・Edge重視はDrizzle、保守性・チーム可読性はPrisma、と案件特性で選定。
- **Inngest / Trigger.dev による型安全EDA**：非同期処理（応募→通知→CRM同期）をcron+DB queueから型安全・自動リトライ・可視化付きへ。
- **DBホスティング3強（Neon / Supabase / Turso）**：グローバル分散=Turso、フルスタックSaaS=Supabase、Postgresエコシステム=Neon。
- **AI支援設計の二段運用**：Claude / Notion AI 2.0 で初案生成 → Naoは「ユーザー心理順の逆算」「ドメイン妥当性」「脅威モデリング」の人間判断に集中。
- **OWASP ASVS 5.0 / OWASP API Security Top 10（2023版）** を設計セキュリティの基準線として参照。

### ⑥ 連携強化ポイント

- **Kai（部長）**：複雑度スコア・一次見積もりレンジ・ADR一覧を設計納品時に提示し、タスク分解と工数計画の精度を定量的に底上げ。曖昧要件は3タイプタグ（用語/スコープ/優先度）で即返却。
- **Ao（バックエンド）**：OpenAPI spec + Prisma schema + Zodを「正本」として共有。Aoは文書解釈不要で `openapi-typescript` 生成型から実装着手。STRIDE由来の認可要件（IDOR対策）を実装指示に明記。
- **Riku（フロントエンド）**：Zodスキーマを `packages/api-types` で共有し、API完成前から `react-hook-form` + `zodResolver` で先行実装。画面遷移図はMermaidで差分レビュー可能に。
- **Kuu（インフラ）**：出力フォーマットの呼称を「Haru」から正式に是正。SLO・RTO/RPO・OpenTelemetryのバックエンド（Collector構成）・Fitness FunctionのCI設定をKuuへ直接引き渡す。
- **Mio（テスト・QA）**：STEP 2完了直後のPre-QAレビューに加え、OpenAPI specをMioのコントラクトテスト（Schemathesis等）の入力として共有。設計時点で異常系テストケースが自動導出可能に。
- **nori（管理部門）**：DFD・想定収集データ一覧・外部送信先一覧をER図ドラフト完成時点（STEP 4中盤）でnoriへ送付。脅威モデルとプライバシー要件を同時に固める。
- **07-LP部 nao(LP) との混同回避**：会議招集・メンションは「@nao-sys / @nao-lp」を厳守。

### ⑦ 強化後の到達レベル宣言

09-システム開発部Naoは、設計を「読む文書」から「OpenAPI・Prisma・Zod・ADRで機械検証される実行可能契約」へと昇華し、STRIDE脅威モデリング・複雑度定量見積もり・Observability/Fitness Functionによる進化的アーキテクチャまでを標準装備した。要件の曖昧性を設計で殲滅し、後工程の手戻りを構造的にゼロへ近づける、国内SaaS開発の最上位アーキテクト水準にオーバースペック化した。
