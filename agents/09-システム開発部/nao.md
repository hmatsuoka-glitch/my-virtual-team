...
### 2026-08-08
- **効率化テクニック：ADR（Architecture Decision Record）を Markdown＋MADR フォーマットで `docs/adr/*.md` に強制運用し設計判断を全て機械追跡**：`0001-record-architecture-decisions.md` から連番で「決定内容／文脈／検討した代替案／却下理由／結果影響」を5セクション必須テンプレ化し、`adr-tools` CLI で新規作成時に自動雛形生成。設計 PR にADR番号必須付与し、後日の「なぜPrismaでなくDrizzle？」照会に30秒で回答可能に。Kai の変更管理ログとリンクさせ、STEP2〜6全工程でADR番号がトレース可能。新規参画者のオンボーディング時に「過去の設計判断ログ」として単一ソース化（2026-08-03記録のADR＋C4運用を強制化）
- **品質チェックポイント：非機能要件（NFR）チェックリスト15項目を `nfr-checklist.yaml` として必須ファイル化し STEP2 完了判定に組み込む**：性能（p50/p95/p99レイテンシ／同時接続数／スループット）／可用性（SLO/SLA/RTO/RPO）／セキュリティ（認証方式／認可粒度／暗号化／PII取扱／監査ログ）／スケーラビリティ（水平/垂直／自動スケール閾値）／保守性（ログ／メトリクス／トレース／エラーバジェット）／互換性（ブラウザ／API版数管理）／アクセシビリティ（WCAG 2.2 AA準拠）／観測性（相関ID／構造化ログ／health check階層）／災害復旧（バックアップ頻度／リストア手順／DRサイト）／コンプライアンス（GDPR/個情法／業法）／コスト（月次インフラ予算／リソース上限）／運用（オンコール／エスカレーション）／文書化／教育／退役計画（EOL条件）の15項目のうち1項目でも `TODO` 残留なら CI で設計 PR fail、Kuu の監視設計と Mio のNFRテスト設計の共通ソース化（2026-07-07記録のSLO.yaml拡張）
- **失敗パターン：脅威モデリング（STRIDE）を「時間があれば」レベルで扱い、認可漏れ・IDOR（Insecure Direct Object Reference）・SSRF等の設計欠陥を実装後QA で検出し、DB スキーマから作り直し** → 回避策：STEP2完了条件に「STRIDE の6カテゴリ（Spoofing/Tampering/Repudiation/Information Disclosure/Denial of Service/Elevation of Privilege）× 各ユースケース」の脅威モデリング必須実施、`docs/threat-model/{feature}.md` に「脅威／緩和策／残余リスク／責任者（実装/運用/契約）」を列挙、Mio のセキュリティテストシナリオ生成の入力にする（理由：セキュリティ欠陥は実装後発覚が最もコスト高でDBスキーマ・API契約まで遡及することが多い）。実例：応募情報API で IDOR 発覚（他社の応募者データ取得可能）→リリース後1週間で全機能停止・再設計→STRIDE強制後、設計段階で認可粒度が確定
- **API契約ファースト設計：OpenAPI 3.1 スキーマを Nao 手で先に確定→ `openapi-typescript`＋`msw` で型・モック・契約テストを全派生**：外部連携APIは実装より先に `api-contracts/*.yaml` を Nao が確定し、Riku 用の TypeScript 型定義＋MSW モックサーバー＋Ao のリクエスト/レスポンスバリデータ＋Mio の契約テスト（Pact または Dredd）を1コマンド `pnpm gen:contracts` で全生成。Riku がAo の実装完了を待たずモックで並列実装開始でき、結合段階の不整合が原理的にゼロ化。破壊的変更は SemVer MAJOR で `/v2` 分離、内部tRPCとの二層構成を維持（2026-07-27記録のtRPC/OpenAPI層分離を派生生成化）
- **連携ヒント：Mio との FMEA（Failure Mode and Effects Analysis）+ FTA（Fault Tree Analysis）2手法併用でセキュリティ・可用性の異常系網羅**：FMEA（ボトムアップ：各コンポーネントの故障モード列挙）＋ FTA（トップダウン：致命障害から原因を分解）の両方向で異常系を洗い、Mio に「Playwright route mock で再現可能な障害モード」（外部API 500/タイムアウト／認証プロバイダ断／DB接続枯渇／Redis LRU 満杯／CDN障害）を専用DBで受け渡し。異常系テストを Mio が想像で補う状態を消し、設計時FMEA表を実行される検証に変換。障害モード当たりのテスト数を過去平均3件→8件へ増やしMTTRリスクを事前潰し（2026-07-16記録のFMEA連携をFTA併用で強化）
