...
### 2026-08-30
- **BMAD Architect Checklist を Cursor MCP拡張で自動実行、設計書のセルフチェック工数を大幅短縮**：`checklists/architect-checklist.md`の全項目をClaude 5がドキュメント読解して自動採点し、欠落項目・矛盾・不明瞭表現をレポート化。Naoは差分だけレビューし本質的な設計判断に集中。効果例：セルフチェック工数45分→8分／チェック漏れ月2件→0件
- **Domain-Driven Design × イベントストーミングをFigJam連携で高速化、業務モデル可視化を対話式に**：クライアントヒアリング時にFigJam上でイベント→コマンド→アクター→ルールを付箋で並べ、Claude 5 に自然言語で要約させる。従来2日かかった業務モデル構築が半日で完了。効果例：業務モデル構築2日→4時間／クライアント合意速度3日→当日
- **`packages/shared-types` モノレポ運用でZod→OpenAPI→FE型→DB型を単一ソースから派生、多層バリデーション不整合を根絶**：Zodスキーマを起点にopenapi-zodからOpenAPI生成、Prisma型導出、tRPC型連鎖まで自動化。FE/BE/DB のバリデーション定義揺れが構造的に消える。効果例：層別バリデーション不整合月4件→0件／スキーマ変更時の影響範囲把握時間30分→自動2分
- **建設業向け業界標準ドメインモデル資産化、新規案件の初期設計時間を大幅短縮**：応募・案件・現場・工程・原価・請求のドメイン集約を`domain-templates/construction/`に集約。7社横断で使い回せる形にし、新規案件は業務差分だけ追加。効果例：新規案件の初期ドメイン設計3日→半日／業界共通要件の重複設計→ゼロ
- **設計書のClaude 5レビュー「Reviewer Persona」を4視点（実装者/QA/運用者/クライアント）で自動実施**：nao作成の設計書を4種のペルソナ視点でレビューさせ、実装時の疑問点・テスト時のGiven-When-Then不足・運用時の監視漏れ・クライアント理解の難しさを事前検出。設計後段の手戻りが激減。効果例：設計後の実装質問月10件→2件／設計→実装のリードタイム3日→当日
- **PlantUML+Mermaid → Excalidraw AI 移行で図と設計書の同期を機械化、C4モデルの階層図を自動生成**：Excalidraw AIに「以下の要件からContainer図を描いて」と自然言語で依頼し、C4の4階層を即座に生成。設計書更新時の図同期漏れが構造的に消える。効果例：図作成工数2時間→15分／図と設計書のズレ月3件→0件
- **11-管理部門 nori との「エンティティ削除ポリシー×個人情報保護法」対応表を設計テンプレ必須化**：エンティティ単位で「業務レコード=論理削除＋保持N年／PII=本人請求で匿名化／監査ログ=物理削除禁止」を noriの法令要件と1行ずつ照合する対応表を設計テンプレの必須シートに追加。設計後段のnori差戻しが構造的に消える。効果例：nori起因の設計差戻し月2件→0件／個人情報保護法対応の設計反映率100%
- **08-バナー生成部 Yuna との「配信計測タグ・GTM設定」を要件表に必須追加**：応募フォーム系案件の設計時に、Yunaが配信するバナーの計測タグ（Meta Pixel/GTag/LINE Tag）・GTM設定・イベント発火条件を要件表に必須項目化。実装後の「計測が動かない」を構造的に防ぐ。効果例：計測未発火起因の"応募ゼロ誤認"月2件→0件／設計段階での計測要件反映率100%
