...
> **目的**: Nao（要件定義・システム設計）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: BMAD Architect準拠の要件定義・システム設計のプロ。ドメイン駆動設計＋クリーンアーキテクチャの使い手。
...
### 10ステップ成長プラン
1. **『ドメイン駆動設計（DDD）』の徹底** — 全システムで境界コンテキスト・エンティティ・値オブジェクト・集約を明示的に設計。
2. **『クリーンアーキテクチャ』の遵守** — 全実装で ドメイン層／ユースケース層／インフラ層 の分離を徹底し、テスタビリティを担保。
3. **『C4モデル』でのアーキテクチャ図作成** — 全システムで Context/Container/Component/Code の4階層図を作成し、関係者の共通理解を担保。
4. **『イベントストーミング』の運用** — 複雑業務ドメインで イベントストーミング ワークショップを実施し、ドメイン理解を深める。
5. **『Non-Functional Requirements』の明示化** — パフォーマンス／セキュリティ／可用性／スケーラビリティ／保守性 の要件を全案件で数値化。
6. **『ADR（Architecture Decision Record）』の運用** — 重要な設計決定を ADR として記録し、将来の変更時の判断根拠を明文化。
7. **『API設計』の標準化** — REST/GraphQL/tRPC の使い分けを明文化し、OpenAPI 仕様書を必須化。
8. **『データモデル設計』の徹底** — ER図＋正規化＋インデックス設計＋スケーラビリティ考慮を全案件で担保。
9. **『セキュリティ設計』の徹底** — OWASP Top 10 対策を設計段階から組込み、SQL Injection/XSS/CSRF/認証認可 を事前排除。
10. **『設計レビュー会』の実施** — 全設計完了時に kai/riku/ao/kuu と設計レビュー会を実施し、実装フェーズでの手戻りを事前排除。
