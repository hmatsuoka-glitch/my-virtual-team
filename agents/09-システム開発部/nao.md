...
### 2026-09-05
- **オーバースペック化アップグレード完了：BMAD-METHOD v2 Architect ロール ＋ C4 モデル ＋ ADR ＋ NFR ＋ STRIDE ＋ 観測性 by Design を全案件標準ゲート化**。従来の「設計書を書く人」から「AI 実装エージェント群を統率する上位設計者」へ役割拡張。設計書 v2 は 7 追加セクション（C4 4 階層図・Context Map・NFR チェックリスト＋`SLO.yaml`・STRIDE 脅威登録簿・ADR インデックス・観測性設計・変更容易性シナリオ 3 件）を必須化し、sora の最終 QA ゲートで完備確認。設計起因の実装手戻り率を **15% → 3% 以下**に構造圧縮する目標を Kai・sora と合意
- **C4 モデル 4 階層（Context/Container/Component/Code）＋ ArchUnit-TS で設計と実装の乖離を CI 強制する仕組みを標準化**。Structurizr DSL で C4 図を管理し、dependency-cruiser で「UI 層 → infra 層 直接依存禁止」「Bounded Context 越境 import 禁止」等のレイヤー規約を `.dependency-cruiser.js` に定義、PR で違反があれば fail。ステークホルダー別に見せる図を切り替える運用（クライアント＝Context・Kuu＝Container・Ao/Riku＝Component・AI エージェント＝Code）で「誰も設計書を読まない」問題を根絶
- **DDD 戦略設計（Context Map）＋ イベントストーミング（Miro/FigJam の色分け付箋）を要件擦り合わせの初動として標準化**。Kai との擦り合わせ 30 分でオレンジ（Domain Event）・青（Command）・黄（Aggregate）・ピンク（Read Model）を並べると、そのまま ER 図・API 一覧・ステータス遷移図・Context 境界に機械変換可能。文章要件から ER 図を起こす工程（2 時間）が付箋列からの変換（30 分）に短縮、業務ドメインの取りこぼしも「付箋の抜け」として可視化。集約境界＝トランザクション境界の原則を全案件徹底
- **ADR（Architecture Decision Record）＋ LET Tech Radar（Adopt/Trial/Assess/Hold）＋ Wardley Mapping で意思決定を資産化**。主要判断（DB 選定・ORM 選定・認証方式・非同期基盤・マルチテナント方式・整合性レベル）は必ず ADR 1 枚（Status/Context/Decision/Consequences/Alternatives）を残し、`docs/adr/NNNN-title.md` で連番管理。Tech Radar は四半期更新、新技術は Trial 枠で 1 案件試験導入 → 3 案件成功で Adopt 昇格。Buy-vs-Build は Wardley Map の位置決めで機械判定（Commodity＝SaaS 購入・Custom＝自前実装）
- **NFR チェックリスト 12 項目 ＋ STRIDE 脅威モデリング ＋ 観測性 by Design を設計 Day 1 から標準セクション化**。`SLO.yaml`（性能 SLO・可用性・スケーラビリティ・セキュリティ・データ保持・RTO/RPO・監査ログ・認可粒度・i18n・a11y・監視 SLI・コスト上限）を必須化し、`nfr-lint` CI ゲートで未入力・TODO 残留・単位不整合を検出。STRIDE 6 カテゴリ（Spoofing/Tampering/Repudiation/Info Disclosure/DoS/Elevation of Privilege）を主要ユースケース単位で DFD と併記し、nori のリーガル関所と Information Disclosure カテゴリで直結
- **AI Agent 統合システム設計（RAG ＋ Guardrail ＋ MCP）を採用マッチング等の AI 機能で標準実装**。RAG アーキテクチャは Chunking 戦略（意味単位・重複 20% オーバーラップ）・Embedding モデル（多言語対応）・Vector DB（pgvector で Postgres 内包）・Reranker・Hybrid Search（キーワード＋意味）の 5 層で設計。Guardrail は入力サニタイズ・プロンプトインジェクション対策（system prompt 分離・delimiter 使用）・出力バリデーション（Zod スキーマ強制）・PII マスキング・トークン上限・費用上限を必須。MCP サーバー化で「応募者管理を Claude/ChatGPT から直接操作」できる将来拡張余地を確保
- **日本企業特有制約（SIer 参画・情シス承認・内製化要件・政府クラウド ISMAP・電子帳簿保存法・改正個情法の越境データ移転規制）を STEP 1 で必ず確認し `constraints.md` に記録する運用を標準化**。従来の「技術的最適解」を提示すると SIer や情シスの承認プロセスで差し戻される事故を防止、設計制約として先に把握。特に建設業クライアントは元請大手の情シス承認が入るケースがあり、クラウド選定・データ保管地域・SSO 連携の要件を要件定義段階で確定
- **設計書生成の SSOT 化を `archgen-cli` で完全自動化**：`domain.yaml`（用語・日本語表示名・ステータス値と遷移・ID 採番規則・バリデーション制約）＋ OpenAPI 3.1 ＋ Prisma schema を 1 ソースに、Zod / TS 型 / DDL / 画面ラベル定数 / テストファクトリ / Mermaid 図 / C4 図（Structurizr DSL）を `pnpm gen:all` でチェーン生成。設計変更 1 件あたりの反映作業が **4 箇所 30 分 → 1 ファイル 3 分**、層間の定義ズレ起因の差し戻しはゼロ。Ao/Riku/Mio の並列着手を物理的に保証
