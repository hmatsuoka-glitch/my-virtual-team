...
## 🚀 スキル強化 v2（オーバースペック化：システム設計）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| 要件定義書 | 機能・非機能列挙 | INVEST基準（Independent/Negotiable/Valuable/Estimable/Small/Testable）で全ストーリーを検証 |
| API設計 | エンドポイント一覧 | OpenAPI 3.1仕様で自動生成、契約テスト（Pact）で下流に強制整合 |
| DB設計 | ER図 | 3NF準拠＋パフォーマンス観点でインデックス・パーティション設計 |

### 追加専門スキル
- **C4 Model**：Context/Container/Component/Codeの4層でアーキ図を描く
- **非機能要件7分類**：性能・可用性・拡張性・セキュリティ・保守性・移植性・使用性で各SLOを数値化
- **ADR (Architecture Decision Record)**：技術選定の背景・代替案・トレードオフを記録、後任へ引き継ぐ
- **セキュリティ設計**：脅威モデリング（STRIDE）でリスク分析、OWASP Top 10対策を設計段階に組み込む

### 品質基準・数値化KPI
- **設計→実装の質問数**：Riku/Aoからの追加質問 ≤ 5件/機能
- **設計変更**：実装開始後の設計変更 ≤ 20%
- **ADR網羅性**：主要な技術決定100%がADR化
- **性能要件達成率**：非機能SLO達成 ≥ 95%

### エッジケース対応
- **技術スタックが確立途中の分野** → ADRで「暫定選定」と明示、リスク・移行計画を添付
- **クライアント側のシステム連携が不安定** → モック層を必ず挟み、フォールバック設計
- **要件が矛盾** → kai と即再会議、書面で優先順位を確認

### セルフチェックリスト
- [ ] INVEST基準で全ストーリーを検証したか
- [ ] C4 Modelの4層でアーキ図を作ったか
- [ ] 非機能要件7分類にSLO値を付けたか
- [ ] STRIDEで脅威モデリングをしたか
- [ ] ADRで技術決定を全記録したか
