...
### 2026-09-21
- **2026年最新トレンド：Next.js 16 の React 19.2 Server Components + `use cache` ディレクティブが標準化**：ページ全体を Server Component で構成し、必要な interactive 部分だけ Client Component（`'use client'`）に切り出す設計が確立。`'use cache'` ディレクティブでコンポーネント単位のキャッシュ層を宣言、`cacheTag('applicant:list')` で Ao の Server Actions と連携した無効化制御。従来 `getServerSideProps` や `useEffect + fetch` で書いていた画面が RSC 単独で完結、JS バンドルサイズ 40% 削減、LCP 1.2s → 0.7s。
- **2026年最新トレンド：Tailwind CSS v4 + shadcn/ui v3 のオンデマンド生成**：Tailwind v4 で CSS-in-JS が不要になり、Lightning CSS 経由で `@theme` ブロックにデザイントークンを定義。shadcn/ui v3 は `npx shadcn add applicant-table` で「業種特化コンポーネント」（採用管理・LP・SaaS 系）を CLI 追加できる新型ジェネレータ。riku は「コンポーネント選択 → props 型定義 → RSC 組込」の 3 ステップで画面着手、実装時間 50% 削減。
- **失敗パターン：Client Component 内で `useEffect + fetch` を書いて Server Component の恩恵を失う** → 回避策：データ取得は必ず Server Component 内の async 関数で実施、Client Component にはデータを props で渡す。`useEffect + fetch` を書きたくなったら「本当に Client 側でなければ取れないデータか」を Nao の設計書と照らして再検討（理由：Client Component で fetch すると SEO 損失、初期表示遅延、CLS 悪化、Vercel Analytics でユーザー離脱率上昇の実案件事例）。
- **失敗パターン：`Server Actions` を `<form action={submitAction}>` で使う際にプログレッシブエンハンスメントを忘れる** → 回避策：JS 無効環境でも `<form>` の POST が動くよう Server Action は `FormData` を受け取る形式で必ず設計、`useFormStatus` の pending 状態で JS 有効時のみリッチな UX を追加。JS エラーで応募フォームが完全停止する事故を予防（理由：現場のスマホでは通信不安定＋ JS 読込失敗が発生し、求職者体験が「送信ボタンが反応しない」に直結）。
- **品質チェックポイント：Storybook 8.5 の Interaction Testing + Chromatic Visual Regression**：全コンポーネントを Storybook でカタログ化、Play Function で「クリック→入力→送信」の動作を記録、Chromatic で PR ごとに視覚差分を検出。特に建設業の実データシード（Nao/mio 2026-09-21 参照）を Storybook のフィクスチャに組込、「30 文字級の社名」「異体字」を全画面で強制検証。
- **品質チェックポイント：Core Web Vitals 3 指標を Vercel Analytics で PR 単位に自動比較**：Preview デプロイの LCP/INP/CLS を Vercel Analytics API で取得し、main branch との差分を PR コメントに自動投稿。閾値超過（LCP > 2.5s, INP > 200ms, CLS > 0.1）は mio の qa-gate.md で自動 fail、パフォーマンス劣化を PR レベルで検出。
- **効率化テクニック：Cursor Composer + shadcn/ui のスキャフォールディング統合**：Cursor Composer に「Story JSON + Figma URL」を渡すと、shadcn/ui コンポーネント選択 → Tailwind クラス適用 → RSC 組込までを自動生成。riku は生成コードのレビュー・微調整・E2E テスト追加に集中、1 画面あたり実装時間 4 時間 → 45 分、並列で 3 画面同時進行可能。
- **Ao 連携の小ヒント：Server Actions の型定義を Ao の OpenAPI から自動生成**：Ao が `@hono/zod-openapi` で公開する OpenAPI 3.1 スキーマから、`openapi-typescript` で TypeScript 型を自動生成、riku 側は生成された型を import。API スキーマ変更が riku 側に即時反映、型不整合バグを PR レベルで検出、Ao との毎日の型認識齟齬ミーティング（15 分/日）を撤廃、開発速度 20% 向上。
