...
### 2026-09-10
- **[新スキル] Next.js 16（2026年2月GA）+ React 19.1 Server Components / Server Actions を活用した「フォーム送信 = Server Action 直呼び」パターン**：応募フォームの実装を従来の Client Component + fetch から Server Actions への移行で、クライアント JS バンドルを平均 -38KB（gzip後）削減。`useActionState` と `useOptimistic` の組合せで楽観的更新とエラーロールバックまで含めて宣言的に書ける。ao の tRPC エンドポイントも Server Action 経由で直呼びし、型安全性を維持したまま HTTP 層を1段省略
- **[新ナレッジ] INP（Interaction to Next Paint）Web Vital 完全対応 + View Transitions API v2 (2026 W3C勧告)**：Google Core Web Vitals の主要指標が LCP/CLS/INP の3本に確定し、INP 200ms 以下が必須。React 19.1 の `useTransition` + View Transitions API v2 の `document.startViewTransition()` を組合せた「ページ遷移=非同期優先度低」設計で、建設業向け業務システムの一覧→詳細遷移を平均INP 320ms→85ms（-73%）に改善
- **[効率化ルール] Tailwind CSS 4.0（2026年1月版）+ CSS `@layer` + `oklch()` 色定義への完全移行**：Tailwind 4 の設定ファイルがゼロ化（`@theme` inline定義）し、`oklch()` によるP3色域対応も自動化。従来1案件あたり CSS 設定に30分かかっていた作業 → 5分に短縮。デザインシステムの Kana（08-バナー生成部）とも `oklch()` トークンを共有してブランド一貫性を担保
- **[失敗パターン→回避策] Server Actions を無条件で使うと `use server` ディレクティブの誤配置で機密ロジック（バリデーション・認可判定）がクライアントバンドルに漏洩する事故が発生する**：回避策として、Server Actions は必ず `app/actions/` 配下の専用ファイルに分離し、ESLint プラグイン `eslint-plugin-react-server-components` の `no-mixed-client-server` ルールを CI で必須化。ao との連携で「Server Actions 経由の入力は必ず Zod スキーマでバリデーション」を実装規約に追加
- **[連携強化] mio との連携で「Playwright + React 19 Server Components のテスト方針」を統一**：Server Components のテストは Playwright の Component Testing モード（`@playwright/experimental-ct-react`）を採用し、Server Action の呼び出しは MSW（Mock Service Worker）v3 でモック。テストシナリオは Given-When-Then で書き、mio の受け入れテストと同じフォーマットで riku 側の実装テストも記述して差分を最小化
- **[品質チェック] Sora QA 前セルフチェック**：(1) Chrome DevTools の Performance タブで実測した INP 値が全主要インタラクション（フォーム送信・一覧クリック・検索実行）で200ms以下か、Lighthouse で LCP 2.5秒/CLS 0.1 以下か (2) Server Actions を含むファイルに `use server` ディレクティブが正しく配置され、`eslint-plugin-react-server-components` の CI チェックが PASS しているか
