...
### 2026-09-04
- **React 19.3 の `use()` フックが安定期に入り、Server Component から Promise を Client に props で渡して Client 側で `use(promise)` で await する非同期データ取得が定番化する**：従来の `useEffect`＋`useState` フェッチや `useSWR`/`useQuery` を経由せず、Server 側で `const promise = fetch(...)` を起票し Client の葉コンポーネントで `const data = use(promise)` と受ける形が RSC ペイロードで直列化可能な扱いになった。`<Suspense fallback>` 境界と組み合わせれば「骨組みは即描画・データは後追いストリーム」が素の React 機能だけで書け、TanStack Query は「キャッシュ・再検証・楽観的更新・invalidate が要る箇所だけ」に用途を絞る棲み分けが定石に。Nao の DTO 設計で「境界を越える値はプレーン化・日時は ISO 文字列」（08-13）と揃え、Promise を渡した先で Date/Map/関数を含めない前提を握ってから採用する。
- **React Compiler が 1.0 GA、`eslint-plugin-react-compiler` の警告ゼロを確認してから babel/swc プラグインを有効化する 2 段移行を新規/既存で切り替える**：07-27 の stable から進み、公式指針が「新規は Compiler デフォルト ON・既存は診断→段階導入」で固まった。既存改修時の踏み抜きパターンは、① 手動メモ化前提で依存配列を絞っていた箇所が二重最適化で挙動が変わる、② ミュータブルな `useRef` 経由の副作用が「フックのルール違反」で検出される、③ 条件分岐内の Hook 呼び出しが今まで通っていたが Compiler で赤くなる、の 3 種類。Riku は新規リポは `useMemo`/`useCallback`/`React.memo` を最初から書かず、既存改修は eslint プラグインの警告解消 → plugin 有効化 → PR ゲートで再計測、の順で進める。Mio への PR テンプレに「Compiler ON/OFF」チェック欄を追加してもらい、レビュー時に手動メモ化と Compiler 期待の書き方の混在を検出可能化する。
- **WCAG 2.2 の新規基準を実装標準に格上げし、`axe-core` v4.10+ の WCAG 2.2 ルール群を PR ゲートへ追加する**：2.1 から 2.2 で追加された基準のうち FE 日常業務に効くのは、① Focus Not Obscured (Minimum)（2.4.11）＝ フォーカス中要素が固定ヘッダー等に完全に隠されない（`scroll-padding-top` と `scroll-margin` の指定が実装対応）、② Target Size (Minimum)（2.5.8）＝ タップターゲット 24×24 CSS px 以上（建設業現場向け 44px 要件（08-16）はさらに厳しい上位で継続適用）、③ Dragging Movements（2.5.7）＝ ドラッグ操作には代替を必ず用意（並べ替えリストなら「上へ／下へ」ボタン併設・kanban なら「別カラムへ移動」プルダウン）、④ Consistent Help（3.2.6）＝ ヘルプ導線位置のページ間一貫、⑤ Accessible Authentication（3.3.8）＝ CAPTCHA など認知テストに代替手段を用意。Riku はこれらのコンポーネント単位チェックリストを `packages/ui/A11Y.md` に固定し、Sora の QA チェックで a11y 深度を問われた際の判断根拠にする。建設業クライアントの現場ユーザー要件と重なるため業務システム案件では 2.2 準拠を提案書ベース要件に含める（Ryota との商談用にも同ファイルを共有）。
- **Conform（Progressive Enhancement 型フォームライブラリ）＋ Server Actions の組合せが RHF 代替として台頭、JS ロード前・実行失敗時にも submit が通る設計を採用サイト系で標準化**：`useActionState`（07-27）で Server Actions を扱う流れが定着した結果、フォーム側も「JS 不要でも submit が届く」progressive enhancement が求められる。Conform は `<form action={serverAction}>` の素の HTML submit を残しつつ、Ao の Zod スキーマから同期的にクライアント側バリデーション・フィールドエラー・aria 属性を派生させ、送信失敗時の「入力を保持したまま該当フィールドへフォーカス」（07-01）も型経由で自動配線される。Riku の判断軸は「複雑な連動 UI・ステップフォーム・リアルタイム連動＝ RHF／単純な CRUD・SEO 重視・LP 応募フォーム＝ Conform」の用途分け。Ren（07-LP 部）が担当する採用 LP の応募フォームは Conform で組み「JS が落ちても submit が通る」を Ren と合意する住み分けを追加（05-14／06-04／08-27 の境界ルールを更新）。Ao との 422 フィールドエラーマッピング契約（07-02）は Conform の `parseWithZod` に直接乗るため、既存の `handleResult` ヘルパー（07-07）と互換性のあるアダプタを `packages/ui` に用意する。
- **React Aria Components v1 GA、Radix / shadcn の代替候補として選定基準を `packages/ui/COMPONENTS.md` に固定化する**：Adobe の React Aria が Headless UI プリミティブとして v1 に到達し、キーボード操作・スクリーンリーダー・国際化（RTL・IME 対応・複数ロケール）の実装深度で shadcn/ui（Radix ベース）と直接競合する立ち位置に。両者の棲み分けは「shadcn/ui＝ソースコピー式・Tailwind v4 `@theme`（07-27）と直結・案件ごとカスタマイズ自由・国内業務システム向き／React Aria＝npm 依存・a11y と i18n が最上級・複数言語 SaaS 向き」。Riku の選定基準は「国内建設業クライアントの業務システム＝ shadcn/ui（09-01 の `packages/ui` 集約に統一）／多言語対応が仕様に入る採用 SaaS＝ React Aria」を機械決定し、Kai の要件整理段階で言語要件を必ず確認してからライブラリを選ぶ。Kuu の CI で axe-core を回す前提と併せ、a11y 品質を「ライブラリ選定＋自動チェック」の二段で担保する。
