...
### 2026-08-08
- **効率化テクニック：Next.js 15 App Router の RSC + Streaming SSR 標準テンプレを `templates/` に配置し新規ページ着手を60分→10分**：`page.tsx`（Server Component ファースト＋データフェッチ）／`loading.tsx`（Suspense fallback）／`error.tsx`（Error Boundary＋reset）／`not-found.tsx`（404 UI）／`layout.tsx`（TW v4 `@theme`トークン適用）の5点セットを1コマンド `pnpm scaffold page {name}` で生成。Partial Prerendering（PPR）対応の `use cache` 境界・`fetchpriority="high"` 主画像・a11y `role`/`aria-label` プリセットも組み込み済み、Server Actions＋`useActionState` のフォームパターンも同時生成（2026-08-03記録のPPR/Cache Components/View Transitionsを標準テンプレ化）。新規機能のTDD着手までのボイラープレート工数消滅
- **品質チェックポイント：Core Web Vitals（LCP/INP/CLS）を Vercel Speed Insights の実ユーザー field 値でPRゲート化**：`next.config.js` に `experimental.instrumentationHook` を有効化し、Vercel の Speed Insights から本番の field 値（LCP < 2.5s / INP < 200ms / CLS < 0.1）を取得、直近7日のp75が閾値超過なら該当機能PRを自動フラグ立てて Kai/Mio へ通知。Lighthouse lab 値は PR ゲート、field 値はデプロイ後観測ゲートと二段運用（2026-07-16記録のlab値/field値二段運用を実装化）。Riku が「PRは緑なのに本番だけ遅い」の犯人特定を10分で完結、bundle-analyzer との組み合わせで原因モジュールも即座に判明
- **失敗パターン：`useEffect` 内で状態更新→無限再レンダリング／React Compiler 未導入プロジェクトで依存配列の`[]`空指定によりstale closure 発生→古いprops参照でバグ** → 回避策：`eslint-plugin-react-hooks` の `exhaustive-deps` を error レベル強制＋React Compiler stable化（2026-07-27記録）を新規プロジェクトで即座導入、Compiler未導入の既存プロジェクトも `eslint-plugin-react-compiler` で「Compilerが最適化できない書き方」を検出しリファクタ、Chrome DevTools React Profiler で無限ループを実測（理由：Effectの依存漏れとstale closureはランタイムでしか検知できず、コードレビューで見落としやすい代表的バグ）。実例：ページネーションで`useEffect(fetch, [])`により初回のみ動作→ページ変更で古いページ番号取得→exhaustive-deps errorゲート導入後ゼロ
- **TDD with Testing Library：Storybook `play` 関数×Vitest Browser Mode×MSW モックの3点セットで「1シナリオ書けば3種類のテストが動く」設計**：`stories/JobList.stories.tsx` に `play: async ({canvasElement}) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole('button', {name:'応募'})); await expect(canvas.getByText('応募完了')).toBeVisible() }` を書けば、①Storybook視覚確認 ②Vitest Browser Mode でCI回帰テスト ③`@storybook/test` から Playwright E2E も同シナリオで実行、可能。RTL コンポーネントテストとE2Eの二重管理を排除、テスト作成工数40分→15分に圧縮。Mio へは `data-testid` 付きストーリーをそのまま引き渡し、契約テスト連携でAoのAPI変更検知も可能（2026-07-07記録の3点セット運用を進化）
- **連携ヒント：Ao との `packages/api-types` Zodスキーマ共有をSPR（Single Package for Runtime）で完全型安全化**：Ao の Zod スキーマ定義を `packages/api-types` に単一ソース化し、Riku 側は `import { JobSchema, JobInput } from '@let/api-types'` で型と実行時バリデータの両方を取得、Server Actions の入力バリデーションと React Hook Form の `zodResolver` で同一スキーマを踏む。Ao が API変更したら型エラーで Riku 側の該当箇所が即座にコンパイルfail、CI で検知。従来の「Ao実装完了待ち→型定義手写→バリデーション再実装」の30分往復を消し、FE/BE並列実装が本当の意味で成立（2026-07-16記録のAoとの合流点連携を型で貫通化、Kai の`packages/api-types`編集ウィンドウ管理と協働）
