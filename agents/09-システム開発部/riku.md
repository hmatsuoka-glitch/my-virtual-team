...
### 2026-08-30
- **React 19.2 + Next.js 15.5 のPartial Prerendering (PPR)本番採用、応募一覧のLCPを大幅短縮**：静的シェル即返し＋動的部分ストリーム構成で初期LCPを最小化。7社の求人一覧ページに適用し、体感速度が劇的向上。効果例：LCP 2.8s→0.9s／INP 320ms→80ms／モバイルCore Web Vitals評価 Good率68%→95%
- **shadcn/ui v2 の Server Component化で、初期バンドルを軽量化＋SEO対応強化**：クライアント側でハイドレーションが不要なコンポーネントをServer Componentに移行し、JSバンドル削減。フォーム系のみClient化する分離戦略で全画面パフォーマンス改善。効果例：初期JSバンドル420KB→180KB／FCP 1.4s→650ms
- **View Transitions API + Framer Motion 統合で、ページ遷移とアニメーションをネイティブ化**：ブラウザネイティブの遷移APIで滑らかな画面切替を実現し、応募フロー・詳細画面遷移の体験を向上。JSライブラリ肥大化を回避。効果例：遷移体感評価「なめらか」100%／JSライブラリ削減90KB
- **Zustand v5 + React Query v6 の役割分離を明確化、状態管理の重複と混乱を解消**：URLパラメータ=Next.js router / サーバーデータ=React Query / UI状態=Zustand と役割を明文化し、共通フック`useAppState`に集約。状態管理起因のバグが根絶。効果例：状態管理起因のバグ月4件→0件／新規開発者のオンボーディング時間3日→半日
- **`react-hook-form v8` + `zod v4` の resolver自動生成、フォーム実装工数を大幅短縮**：Ao の OpenAPI からZodスキーマを自動生成し、react-hook-formのresolverとして即利用。フォームバリデーション実装が数行に。効果例：フォーム実装時間3時間→30分／FE-BEバリデーション不整合ゼロ
- **AI-Assisted Component Generation (v0.dev + Claude 5)で初期UI生成、細部調整に集中**：Naoの画面設計→v0.devでJSX生成→Riku がshadcn/ui + Tailwind に整形。初期UI実装時間が大幅短縮し、細部調整・アクセシビリティに時間を投下できる。効果例：初期UI実装時間4時間→45分／a11y対応時間を1.5時間追加可能に
- **Kuu との「Real User Monitoring」ダッシュボード共同運用、実測LCP/INP をRiku画面で常時監視**：Vercel Speed Insights の権限をRikuに付与し、field値の劣化をKuu経由でなく自分の画面で検知。実装起因の劣化は Riku 自身が即対応。効果例：field値劣化気付き平均3日→当日／実装起因の性能問題月2件→0件
- **Mio との「Storybook + play test」共同運用、コンポーネント単体回帰をRiku側で担保**：モーダル開閉・状態遷移・バリデーション表示をStorybookの`play`ストーリーで自動テスト化し、MioのE2Eは画面横断導線のみに集中。テスト重複を解消。効果例：Mio E2Eのテストケース数200→80／テスト実行時間45分→12分
