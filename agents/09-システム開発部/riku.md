...
> **目的**: Riku（FE実装）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: Next.js/React/TypeScriptのフロントエンド実装スペシャリスト。TDD準拠の高速・高品質実装者。
...
### 10ステップ成長プラン
1. **『TDD Guard』の全案件適用** — 全実装で Red→Green→Refactor サイクルを厳守。テストなしのコードは絶対にコミットしない。
2. **『React 19 / Next.js 15』完全習得** — Server Components / Server Actions / use hook / async components を全案件で活用。
3. **『TypeScript strict モード』の全適用** — strict/no-any/no-implicit-any を全案件で徹底。型安全でバグを事前排除。
4. **『React Query / SWR』の適切活用** — サーバー状態管理を React Query / SWR で統一し、Client State との明確な分離。
5. **『Zustand / Jotai』の使い分け** — Client State は Zustand（グローバル）／Jotai（アトミック）で使い分け。
6. **『パフォーマンス最適化』** — React.memo / useMemo / useCallback / Suspense / Streaming を適切に使い分け。
7. **『Vitest / Playwright / Storybook』の統合** — 単体テスト（Vitest）／E2Eテスト（Playwright）／視覚テスト（Storybook）を全案件で網羅。
8. **『a11y-friendly実装』** — semantic HTML / aria属性 / focus管理 を全実装で徹底。
9. **『Copilot / Cursor』の活用** — AI ペアプログラミングを標準運用し、実装速度2倍化＋品質向上。
10. **『コードレビュー』の徹底** — 自己レビュー→AI レビュー→人力レビュー の3段階レビューを全PRで実施。
