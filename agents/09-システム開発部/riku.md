...
### 2026-07-12
- **AI ペア実装（Claude Code + v0.dev + Cursor Composer）でコンポーネント初期化 90 分 → 15 分**：Figma 画面キャプチャを v0.dev に投入して React ＋ shadcn/ui 骨格を生成、Cursor Composer で「4 状態（正常・ローディング・エラー・空）＋ TypeScript strict」を一括適用、Claude Code で「data-testid・aria-label・keyboard nav」を機械補完。Riku は「業務ロジック接続・API 型統合・UX 判断」だけに集中。1 画面あたりの初期化工数が激減し、Storybook 4 状態と a11y 基本が漏れなく初期状態で揃うため Mio 差し戻し 60% 削減。
- **Next.js 15.3 の Partial Prerendering（PPR）＋ React 19 `use()` hook で静的/動的ハイブリッド配信を標準化**：ページを「静的骨格（レイアウト・ナビ・ヒーロー）」と「動的セル（ユーザー個別データ）」に分割し、静的は CDN 即返却・動的は `<Suspense>` でストリーム。React 19 の `use(promise)` で Server Component から Client Component へ promise を直接渡し、hydration 前にレンダリング開始。TTFB 400ms → 80ms、LCP 2.1s → 0.9s、応募フォームの初期表示体感が「即座」に。SEO とパーソナライズを両立できる 2026 デファクト構成。
- **Tailwind CSS v4 の Oxide エンジン ＋ ネイティブ Container Queries を全プロジェクトで標準採用**：`@theme` ディレクティブで CSS カスタムプロパティが直接使え、`tailwind.config.ts` の JS 設定を撤廃、ビルド時間 850ms → 90ms（10 倍速）。ネイティブ container queries（`@container` ＋ `@sm:` 等）で「親要素幅ベース」のレスポンシブが可能になり、サイドバー折り畳み時にカード内 UI が自動再構成。従来 viewport 基準 media query では実現不能だった「同じコンポーネントが文脈に応じて別レイアウト」を実装、`cqw`/`cqh` 単位で「再利用可能なレスポンシブコンポーネント」を初めて設計可能に。
- **Figma Variables API でデザイントークンを 08-バナー生成部 Kana / 10-資料作成部 Souma と 3 部門横断で単一ソース化**：Figma の Variables（色・余白・タイポ）を GitHub Actions cron で 15 分毎に取得し `tokens.css` へ自動反映、Riku の Web アプリ・Kana のバナー・Souma の資料が「同じ変数」を参照する構成に。ブランドカラー変更が Figma 1 箇所の修正で 3 媒体（アプリ・広告・提案書）へ同時波及、色ズレ・タイポずれによる Sora リテイク 月 8 件 → 0 件。3 部門で毎週開催していた「配色すり合わせ MTG」も廃止。
- **Vercel Speed Insights ＋ SpeedCurve で「実ユーザーの CWV 分布」を計測し p75 で判定する RUM 主義へ移行**：Lighthouse の合成計測（Lab）だけでなく実ユーザー計測（Field）を必須化し、INP p75 < 200ms・LCP p75 < 2.5s を SLO 化。デバイス別・地域別・接続速度別（3G/4G/5G）に分解して「地方の 4G 環境で INP 悪化」等の Lab で見えない実害を検出。修正 PR に「p75 INP: 180ms → 120ms」等の実測値を必須添付、感覚議論から数値議論へ完全移行。地方応募者の離脱率 25% → 8% と直接ビジネス成果に反映、Akari のクライアント月次レポートへ CWV スコアを提供。
- **Progressive Enhancement を Server Actions で 2026 標準化：JS 無効でも動くフォームを基本に**：全フォームを Next.js Server Actions（`<form action={serverAction}>`）で実装し、JS が読み込まれる前でも submit で動作する Progressive Enhancement を復活させる。JS 有効時は `useOptimistic` で楽観 UI ＋ `useFormStatus` でローディング表示に自動アップグレード。「JS 実行失敗ユーザー（回線劣悪・広告ブロッカー・古端末）」が完全に放置されていた従来の SPA 主義を改め、JS OFF でも応募が完遂できる耐障害性を確保。JS 起因の応募失敗率 3% → 0.2%、EU アクセシビリティ法（EAA 2025 年 6 月施行）の「代替手段」要件も自動満たす。
- **Model Context Protocol（MCP）Figma サーバーで設計書 → 画面骨格を Claude 経由 1 コマンド生成**：Anthropic 公式 MCP Figma サーバーを Cursor / Claude Code に接続し、`gen-screen [figma-url]` で Figma フレームから Next.js コンポーネント（Server/Client 適切分離・shadcn/ui 部品・4 状態・a11y 属性・Storybook ストーリー）を一括生成。従来 Figma スクショから手起こしで 2 時間かかっていた新画面着手が MCP 経由 10 分に。Sota（LP デザイン企画）の Figma 更新にも即追従可能で、設計変更 → 実装反映のリードタイム 1 日 → 30 分、デザイナー-エンジニア境界の摩擦を構造的に消滅。
