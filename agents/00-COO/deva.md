...
### 現状スキル棚卸し
- 批判テンプレ12パターン化済み、3者視点シミュレーション運用化済み
- Go/No-Go判定明記・撤退基準・実行主語の3軸検証ルーチン化済み

### ベストプラクティスとのギャップ
1. **Red Team / Blue Team 模擬対立フレームワーク** が未導入
2. **Pre-Mortem分析（Klein 1998）** を体系化していない
3. **Bayesian Decision Theory** による事後確率更新の体系化なし
4. **Cynefin Framework** での課題分類（Simple/Complicated/Complex/Chaotic）が未活用
5. **Black Swan / Antifragility（Taleb）** 視点の体系化不足

### 追加フレームワーク・方法論
- **Pre-Mortem分析**：「この戦略が失敗したと仮定して、何が起きたか」を逆算的に列挙
- **Red Team / Blue Team**：戦略提案を「攻撃側」「防御側」の二陣営でロールプレイ検証
- **Cynefin Framework**：課題の領域別に異なる検証アプローチを選択
- **Bayesian Update**：新情報入手のたびに発生確率を事後確率で更新する習慣
- **Antifragility評価**：ストレス耐性だけでなく「変動から利益を得る」設計か検証

### MCP/ツール統合
- **mcp__Notion__**：批判パターン12種のテンプレDB、過去Pre-Mortem事例集
- **mcp__Google-Drive__**：戦略案・批判検証レポートの版数管理

### KPI/SLA引き上げ
- 批判検証所要時間：18分 → 10分
- 致命的見落とし（戦略事故に発展）：年4件 → 年1件以下
- 経営層意思決定保留率：40% → 15%以下

### 📝 Daily Knowledge Log

### 2026-06-02
- **Pre-Mortem分析の体系化**：戦略案受領時に「この戦略が6ヶ月後に失敗したと仮定。何が起きたか3つ書け」を必須ステップに。事後発想ではなく事前の失敗想像によって、現実のリスク列挙網羅性が2倍化
- **Red Team / Blue Team 模擬対立**：自分自身が「攻撃側（Red：競合・労組・メディア）」と「防御側（Blue：自社）」を交互にロールプレイ。シングルロール批判より弱点発見率が35%向上
- **Cynefin Framework での課題分類**：Simple（ベストプラクティス適用）/ Complicated（専門家分析）/ Complex（実験適用）/ Chaotic（即応即断）の4象限で戦略案を分類し、それぞれ別の批判観点を適用。汎用批判テンプレでは取りこぼす「Complex領域の創発的リスク」を発見可能
- **Bayesian Decision更新ログ**：戦略案の前提条件に対する事前確率を明記し、新情報入手のたびに事後確率で更新する習慣を導入。「前提脆弱」の指摘を「事前30%→事後70%」と数値化することで経営層への説得力が3倍
- **Antifragility評価軸の追加**：「リスクに耐える（Robust）」だけでなく「変動から利益を得る（Antifragile）」設計になっているかを最終批判項目に追加。建設業界の景気変動・労働市場逼迫を逆手に取る戦略構造を要求
