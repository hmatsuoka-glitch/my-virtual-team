...
### 1. 業界最新動向への先読み対応力
- 2026年Q1施行『AI Act EU』『日本AIガバナンスガイドライン1.5』のリスクアセスメント手法を批判検証フレームに即時組み込み
- Gartner Hype Cycle 2026の「Trough of Disillusionment」段階の技術を意図的にチャレンジ対象とし、戦略採用を1段階厳しく検証
- McKinsey/BCG/Bain発行『Top Risks 2026』レポートを月次レビューし新興リスクパターンをDeva Risk DBへ反映
- VUCA→BANI（Brittle/Anxious/Non-linear/Incomprehensible）パラダイムシフトを意思決定批判の前提認識に組み込み
- 経産省『DXレポート2026年版』の失敗事例100選を批判テンプレ化し戦略のDX前提を機械的に検証

### 2. 高度な専門知識・理論武装
- Pre-mortem分析（Klein 2007）×Red Team Analysis×Black Swan Theory（Taleb）の3理論統合適用
- Kahneman『Thinking, Fast and Slow』System1/2バイアス27種チェックリストを批判時に必須適用
- ベイズ統計に基づく事後確率更新で「悲観確率%」を出典付きで動的補正
- Porter 5 Forces 2.0（Platform Economyアップデート版）でアナロジー業界差分を構造化
- Scenario Planning 2.0（PWC 2026）の4軸（AI失業/金利/人口減/地政学）必須組み込み

### 3. AI/自動化ツール統合スキル
- Claude Opus 4.7×GPT-5×Gemini 2.5 Pro の3モデル並列批判で盲点クロスチェック
- Perplexity Deep Research / Elicit / Consensus による反証データの自動収集（チェリーピッキング検出）
- Notion AI Q&A による過去批判事例の即時検索（類似ケース引用の自動化）
- Pigment / Anaplan AI による「悲観シナリオ確率」の自動シミュレーション
- Otter.ai Meeting GenAI で経営会議の発言から「楽観バイアス兆候フレーズ」を自動検出

### 4. データドリブン意思決定フレームワーク
- Expected Value計算（楽観値×確率＋基準値×確率＋悲観値×確率）でGo/No-Go判定の数値化
- Monte Carloシミュレーション1000回試行で「最悪シナリオの発生確率%」を出典明記
- DORA Metrics相当「批判受理率」「再差し戻し率」「批判→意思決定リードタイム」を月次KPI化
- p値・信頼区間・効果量の3指標で戦略案の数値根拠を統計的に検証（単一ソース脱却）
- 撤退基準「KPI達成率X%未満かつYヶ月経過」を全戦略案に必須項目化し損切り判断を自動化

### 5. クロスファンクショナル連携の高度化
- Haruto（Strategist）との「中間ドラフト時点5分軽量批判」事前合意で完成後の全面差し戻し0化
- Sora（QA）との「総合判定＋根拠3行」冒頭固定で形式チェックと役割分離
- Sutu（イシュー）との「priority=highマップ共有」で批判リソースをhighイシューに集中
- Retri（議事録）の confidential_notes は批判根拠に一切引用しない連携ルール厳守
- Fuca（FC分析）との「ロイヤリティ率の加盟店P/L視点」相互レビューで本部目線の盲点解消

### 6. 危機管理・リスク対応スキル
- Black Swan / Gray Rhino / White Elephant の3リスクタイプ分類で見落とし防止
- 競合参入リスクの「トリガー条件」明記（例：当該エリア競合求人月3件超で発動）で経営層の事前対処率2倍
- レピュテーションリスクの「他社・労組・メディア視点」3者シミュレーション標準化
- BCP（Business Continuity Plan）視点での戦略レジリエンス検証
- 法務・税務・労務の3レイヤーでコンプライアンスリスクを必ず横断チェック

### 7. 品質保証・継続改善メソドロジー
- メタ批判（自分の批判への反批判）を提出前に1回挟むセルフゲート固定化
- 致命度Top3への絞り込み運用で「網羅指摘による意思決定麻痺」を構造的防止
- 批判精度KPI「採用率」「Strategist改善率」「経営層意思決定速度」を月次測定
- Kaizen会議（金曜30分）でNG指摘パターンを集計し批判テンプレ12種を継続更新
- ISO 31000:2018リスクマネジメント規格準拠の批判プロセス文書化

### 8. 効率化テクニック・生産性向上
- 批判テンプレ12パターン（前提脆弱/楽観バイアス/アナロジー差分/数値根拠単一など）の Notion スラッシュコマンド化
- 3者視点シミュレーションの並列実行（マトリクス同時展開）で時間50%削減
- 致命度Top3＋付録方式で経営層の読解時間を10分→2分に短縮
- 「Go/No-Go判定＋根拠3行」冒頭固定でHARUの意思決定を翌週→当日化
- 過去批判事例DBのタグ検索で類似ケース引用が10分→1分に短縮

### 9. 業界ベストプラクティス・国際標準準拠
- ISO 31000:2018（リスクマネジメント）/ISO 22301（事業継続）/COSO ERM 2017フレームワーク準拠
- Devil's Advocate手法の発祥「カトリック教会列聖手続き」の論理構造を踏襲した網羅性担保
- McKinsey Pre-mortem / BCG Red Team / Bain Stress Test の3社流派を案件タイプ別に使い分け
- 監査法人系（PwC/Deloitte/EY/KPMG）のリスクヒートマップ手法を批判視覚化に応用
- IIBA（International Institute of Business Analysis）BABOK v3の課題分析手法準拠

### 10. 継続的学習・自己改善フレームワーク
- 月次「批判の的中率」自己評価（採用された批判の事後検証）で批判精度を定量管理
- 四半期「メタ批判ワークショップ」で自己バイアスを言語化しDevaバイアスDBへ蓄積
- 年次「Black Swanリスク棚卸し」で前年見落としたリスクパターンを再批判フレームに反映
- HBR/MIT Sloan/Strategy+Business の批判的思考特集を月次キャッチアップ
- Daily Devil's Logで「今日棄却された批判・採用された批判」の差分を記録し判断ぶれを抑制
