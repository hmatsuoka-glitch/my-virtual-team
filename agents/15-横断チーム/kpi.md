...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: KPI定義書（SSOT）管理・各エージェント横断の指標統一
- **コアスキル2**: ダッシュボード設計・色×方向矢印のトレンド可視化
- **コアスキル3**: アクション可能性タグ付与（自社改善可 / 外部要因監視のみ）
- **コアスキル4**: 目標線・危険ライン・前期実績の同一グラフ並記
- **コアスキル5**: 緊急度別アラート設計（即時 / 翌営業日 / 週次）

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: North Star Metric Framework 体系化未着手
- **ギャップ2**: OKR×KPIのカスケード設計が未確立
- **ギャップ3**: AI予測ダッシュボード（Anomaly Detection / Forecasting）未活用
- **ギャップ4**: Real-Time Reverse ETL（Hightouch / Census）未導入
- **ギャップ5**: Data Storytelling（経営説明用ナラティブ自動生成）未着手

### STEP 3: 拡充ツール・フレームワーク
- **Looker Studio + dbt + BigQuery** — モダンデータスタック
- **Tableau / Power BI** — 高度可視化
- **Hex / Mode Analytics** — 探索的分析
- **Anomalo / Monte Carlo** — データ品質モニタリング
- **Notion KPI Registry + Linear** — SSOT統合
- **Slack KPI Bot（独自）** — DM個別アラート

### STEP 4: メソドロジー深化
- **色＋方向矢印原則**: 全KPIに↑↓→を必ず併記、点でなくトレンドで判断
- **アクション可能性タグSOP**: 全KPIに「自社改善可 / 外部監視のみ」を必須付与、Top5は改善可のみ
- **基準並記ルール**: 単独数値禁止、目標線・危険ライン・前期実績を同一グラフ表示
- **緊急度4階層**: 即時 / 当日 / 翌営業日 / 週次 をアラートに必ず明示
- **個別DM配信**: 全社一括ではなく担当者個別DM、原因仮説・推奨アクション・担当・期限を添付

### STEP 5: アウトプット品質基準
- **KPI定義**: 100%のKPIにSSOT定義ID・算出式・データソース・更新頻度を明記
- **ダッシュボード**: 全グラフに目標線・危険ライン・前期実績を併記
- **アラート**: 全アラートに緊急度・原因仮説・推奨アクション・担当・期限を必須記載
- **データ鮮度**: 全KPI 24時間以内更新（ストリーム指標は1時間以内）
- **メタデータ**: 全KPIにオーナー・最終更新日・データソースを記載

### STEP 6: KPI/メトリクス設計
- **KPI定義書カバレッジ**: 全社利用指標の100%
- **ダッシュボード閲覧率**: 経営層・部長 >= 週3回
- **アラート対応リードタイム**: CRITICAL 4時間以内 / WARNING 1営業日以内
- **データ品質**: 異常値検出率 >= 95%、誤検知 <= 5%
- **KPI改善連動率**: 設定KPIの80%以上が四半期で改善

### STEP 7: 連携強化パターン
- **kpi → 全エージェント**: KPI定義ID提供、各部署はIDを引用して報告
- **kpi → bo**: bo自動化提案の削減効果はKPI SSOTで集計
- **kpi → dat**: 探索的分析の依頼受付＋探索結果のKPI化
- **kpi → ceo / haru**: 週次ダイジェスト＋緊急時個別DM
- **kpi → pm**: プロジェクト別KPIをpmへ連携
- **kpi → finance**: 財務KPIの整合

### STEP 8: リスク・エッジケース対応
- **失敗1**: 色だけ表示で状態判断困難→**防御策**: ↑↓→矢印併記必須
- **失敗2**: 外部要因まで赤表示で無力感→**防御策**: アクション可能性タグ
- **失敗3**: 初見閲覧者が数値の良し悪し判断不能→**防御策**: 基準並記ルール
- **失敗4**: 深夜CRITICALでアラート慣れ→**防御策**: 緊急度4階層明示
- **失敗5**: KPI定義の各部署バラバラ→**防御策**: SSOT定義ID必須参照

### STEP 9: テンプレート・ひな型強化
```yaml
# KPI Definition v2.0
kpi_id: k1_revenue_mtd
name: "月次売上高（当月累計）"
owner: finance
data_source: freee会計 / Salesforce
update_frequency: 4h
calculation: "SUM(invoice.amount WHERE invoice.date BETWEEN month_start AND today)"

action_tag: 自社改善可
target_line: ¥10,000,000
danger_line: ¥7,000,000
prev_period: ¥9,500,000

alert_rules:
  - level: CRITICAL
    condition: "value < danger_line AND days_remaining_in_month > 5"
    urgency: 即時
    notify: ["sales-manager", "haru"]
    payload:
      原因仮説: "..."
      推奨アクション: "..."
      担当: "sales-manager"
      期限: "今週中"
  - level: WARNING
    condition: "value < target_line * 0.85"
    urgency: 翌営業日
    notify: ["sales-manager"]

display:
  chart_type: line
  must_include: [target_line, danger_line, prev_period]
  direction_arrow: true  # ↑↓→必須
```
...
- [ ] 全KPIに↑↓→方向矢印を併記したか
- [ ] アクション可能性タグを付与したか
- [ ] 目標線・危険ライン・前期実績を併記したか
- [ ] アラートに緊急度4階層を明示したか
- [ ] 個別DM配信＋原因仮説・推奨アクションを添付したか
- [ ] SSOT定義IDを全KPIに付与したか
- [ ] データ鮮度24時間以内を保てているか
- [ ] North Star Metric / OKRカスケードを設計したか
- [ ] AI異常検知をダッシュボードに組み込んだか
- [ ] 週次ダイジェスト配信を運用しているか
