...
サクバズ事業（SNSマーケ×採用支援／建設業7社担当）の全社KPI俯瞰役として、2026年下期に向けた強化パッケージ。既存の3層ダッシュボード・SSOT定義書・CV自動閾値・reconciliation運用は完成度が高いため、その土台の上に「予測型・意味論型・生成AI活用型」の3レイヤを乗せる方向で拡張する。

### スキル拡張

1. **予測型アラート（Predictive KPI Monitoring）**
   Prophet / Kats / River などの時系列予測モデルで「7〜30日先の着地予測」を先に出し、目標乖離が発生する前に WARNING を発火させる。既存の EWMA 乖離（07-01記録）は「起きた後の検知」だが、これに「起きる前の予告」を追加する。サクバズの月次採用KPI（宮村建設・清一建設等）で月末の着地見込みを月中旬時点で提示し、Ryota / Akari のクライアント対応に前倒しでインプット。変化点検知（Bayesian Online Changepoint Detection）も併用し、トレンドの構造変化を「単なる異常値」と区別してアラート種別を分ける。

2. **メトリクスツリー（Metric Tree / KPI DAG）の常設化**
   06-13記録の KGI→CSF→KPI 階層を静的な定義書に留めず、有向非循環グラフ（DAG）として常時可視化する。各KPIノードに「親CSF・子構成要素・ガードレール・stock/flow・データソース」をエッジで結び、ダッシュボードから「このKPIが動いた時にどのKGIが何%動くか」を逆算表示。定義変更時の影響範囲（06-16記録の依存グラフ）と統合し、5部門影響レビュー（05-27記録）の判定材料を機械化する。

3. **AI自動ナレーション（Narrative BI）**
   月次レポートの「なぜ乖離したか」文章生成を、Dat 深掘り結果（数値）を入力に LLM で自動草案化する。Kpi は数値集計＋ナレーション草案まで担当し、Dat は要因分析の数値証拠に集中、CEO は完成に近い文章を微修正するだけで承認可能。既存の「乖離検出→Dat起票→結果転記」ワークフロー（06-16／07-07記録）の最終段に「ナレーション自動起草」を挿入し、月初提出リードタイムをさらに短縮する。

4. **セグメント／コホート第一のKPI設計**
   全社平均・全社合計が「クライアント7社の1社大型案件」に引きずられる問題（07-01記録の中央値化）を、集計段階からセグメント軸（クライアント×媒体×職種）で保持する運用に拡張。トップ5KPIも「全社値／最重要セグメント（Top3クライアント）／悪化セグメント（下位3クライアント）」の3視点でパラレル表示し、平均値の裏で悪化する個別クライアントを構造的に検知する。

5. **データコントラクト（Data Contract）による上流品質SLA化**
   連携停止検知（06-03／06-26記録）を「起きたら検知」から「起きないよう契約化」へ引き上げる。GA4・Airwork・Notion・Slack など上流データソースごとに「スキーマ・更新頻度・欠損率・タイムスタンプ形式」を data contract として定義し、契約違反は上流エージェント（shun / kuu 等）に自動起票。ダッシュボード側で気づく前に上流で止める運用に切り替え、backfill 許容期間（07-03記録）と組み合わせて遡及修正の根源を潰す。

### 追加ツール・手法

1. **セマンティックレイヤ（dbt Semantic Layer / Cube / MetricFlow）**
   SSOT定義書（Notion）を「人間が読む定義」から「機械が実行する定義」に格上げする。KPI算出式・stock/flow区分・期間境界・親CSFリンクを YAML で宣言し、BI ツール・Slack Bot・月次レポート全てが同一の定義から数値を引く。同名異定義（05-27記録）を定義書レビューでなくコンパイルエラーで防ぐ。

2. **時系列予測 & 変化点検知ライブラリ（Prophet / Kats / River / Ruptures）**
   予測型アラート（上記スキル1）の実装基盤。EWMA・移動平均（06-20記録）に加えて、季節性・曜日効果・イベント効果を自動学習する Prophet を導入し、CV自動閾値（06-16記録）と組み合わせて閾値関数を進化させる。オンライン学習（River）でモデル自身も日次更新し、「昔の閾値のまま形骸化」を防ぐ。

3. **データオブザーバビリティ（Monte Carlo / Sifflet / Datafold + Great Expectations）**
   合計整合 assert（06-12／06-16記録）と過去30日スナップショット回帰（06-12記録）を、内製実装から専用ツールへ移行し、フリッシュネス（鮮度）・ボリューム（件数）・スキーマ・分布・リネージの5次元を常時監視。データコントラクト（上記スキル5）の違反検知エンジンとして常駐させ、集計ジョブの共通ラッパー（07-07記録）と統合する。

### 追加出力フォーマット

1. **metric_tree.yaml（KPI意味論定義）**
   ```yaml
   version: "2026.07"
   kgi:
     - id: kgi_annual_revenue
       name: "年商目標"
       target: { fy2026: 500000000, unit: "JPY" }
       csfs: [csf_client_retention, csf_new_client_acquisition, csf_service_arpu]
   csf:
     - id: csf_client_retention
       name: "既存クライアント継続率"
       parent_kgi: kgi_annual_revenue
       weight: 0.55
       kpis: [kpi_monthly_health_score, kpi_churn_rate]
   kpi:
     - id: kpi_monthly_health_score
       name: "月次ヘルススコア"
       parent_csf: csf_client_retention
       formula: "AVG(client_health_score) WHERE month = current"
       stock_or_flow: "stock"
       leading_or_lagging: "leading"
       source: { system: "Notion", table: "client_health", contract: "dc_notion_client_v3" }
       target: { commit: 70, stretch: 85 }
       guardrails: [kpi_response_time_hours]
       threshold_function: "cv_based_dynamic(window=28d, band=2.5)"
       segment_axes: ["client", "industry", "contract_size"]
       owners: ["ryota", "akari"]
       drilldown_url: "https://.../kpi/health_score"
   guardrail_pairs:
     - primary: kpi_new_leads_count
       counter: kpi_lead_quality_score
       rationale: "リード数最大化→質劣化の副作用（グッドハート）検知"
   ```

2. **narrative_brief.md（AI生成の経営ブリーフ）**
   ```markdown
   # 経営ブリーフ YYYY-MM

   ## 一言サマリー（AI草案・要CEO確認）
   {NSMの着地予測、最重要3セグメントの健全性、対応要のCRITICAL件数を1文で}

   ## トップ5KPI 実績 vs 予測 vs 目標
   | KPI | 実績 | 予測着地 | 目標 | コミット | 状態 | 方向 | 変化点 |
   |---|---|---|---|---|---|---|---|
   | {NSM} | {実績} | {Prophet予測} | {目標} | {死守} | 🟢/🟡/🔴 | ↑/→/↓ | あり/なし |

   ## 悪化セグメント TOP3（全社平均の裏で沈んでいる）
   1. {クライアントA / 媒体B}：{指標} が {前月比} → 原因仮説：{Datナレーション}
   2. ...

   ## CRITICAL / WARNING 対応状況
   - 未着手：{件数}（回復閾値ヒステリシス考慮後）
   - 対応中：{件数}
   - 解消：{件数}

   ## 次月の先行指標シグナル（Leading）
   - {先行指標} が {方向} に振れ、翌月NSMに {推定効果}pp の影響見込み

   ## データ品質サマリー
   - Data Contract 違反：{件数} / 遡及修正：{件数}（詳細：backfill_log参照）
   - 定義変更：{件数}（影響レビュー完了 / 未完了）

   ---
   *本文はAI草案。数値は SSOT metric_tree.yaml v2026.07 に準拠。最終承認：CEO*
   ```

### 成長目標（3ヶ月・6ヶ月・12ヶ月）

**3ヶ月（〜2026-10-20）：意味論の機械化**
- 全社KPIを metric_tree.yaml に完全移植し、SSOT定義書と YAML の乖離ゼロ運用を確立
- ダッシュボードから KPI DAG の逆引き表示（このKPIを動かすと何が動くか）を実装
- narrative_brief.md の AI草案を月次で試験運用開始（Ryota / Akari とのクライアント報告に組み込み）
- 上位10クライアント指標にセグメント第一の並列表示（全社／Top3／下位3）を導入

**6ヶ月（〜2027-01-20）：予測型と契約型への進化**
- Prophet / Kats による予測型アラートを主要NSM 5指標で常時稼働、月中旬時点の月末着地予測を CEO 定例に組み込み
- データコントラクトを GA4 / Airwork / Notion / Slack の4系統で運用開始、契約違反の上流自動起票を確立
- Monte Carlo または Sifflet を導入し、内製 assert 群を専用オブザーバビリティ基盤へ段階移行
- Dat との「乖離検出→深掘り→ナレーション自動起草」パイプラインを月次レポートに完全統合

**12ヶ月（〜2027-07-20）：全社セマンティック統一と経営会議のリアルタイム化**
- dbt Semantic Layer / MetricFlow で全KPIをコード化し、Slack Bot・ダッシュボード・月次レポート・Prナレーションが同一定義から数値を引く体制を完成
- CV自動閾値・季節性補正・変化点検知をオンライン学習（River）で日次自更新化し、閾値メンテを手動ゼロにする
- 経営会議を「PDFレビュー」から「ライブダッシュボード＋AIナレーション＋Dat即時深掘り」の対話型に転換
- 建設業7社の採用KPIで「予測着地×セグメント×先行指標」の3次元先読みを標準納品化し、Ryota / Akari の提案力・継続率に直結させる
