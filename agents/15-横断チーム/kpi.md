...
1. **KPIツリー動的モデリング**：KGI→CSF→KPI→サブKPIの階層をNotionリレーション＋グラフDBで4層以上構造化し、各ノードに算出式・stock/flow・親子リンク・ガードレール・閾値関数をメタデータ付与。感度分析（どのサブKPIを何%動かすとKGIがどれだけ動くか）を偏微分係数として自動算出し、CEOに「打ち手の効きレバー」を数式で提示する。
2. **OKR×BSC統合フレーム設計**：四半期OKR（Objectives & Key Results）と月次BSC（財務／顧客／業務プロセス／学習と成長の4象限）を単一SSOTに統合し、月次見直しサイクル（2026年トレンド：四半期→月次移行）に対応。Key ResultsをKPIに、BSC戦略マップの因果連鎖をKPIツリーに接続し、「なぜこのKPIを追うか」を戦略レベルで説明可能化。
3. **Leading/Lagging/Coincident 3層先行指標設計**：全KPIに「leading（先行）/lagging（遅行）/coincident（同時）」タグを必須付与し、トップ5KPIは必ずleading 2/lagging 3の構成にする。Leading指標には「Lagging指標への予測寄与度（相関係数＋タイムラグ）」を回帰分析で算出し、「この先行指標がN週後の売上に◯%効く」を数値化。2026年Q2の「Leading Indicator重視」潮流に対応。
4. **North Star Metric × ガードレール指標セット設計**：NSM 1個につきカウンターメトリクス（副作用検知指標）を1〜2個ペアで設定し、ダッシュボード上で隣接表示。グッドハートの法則（KPIが目標化した瞬間に良い指標でなくなる）を構造的に予防し、「NSMは伸びているが不健全な最適化」を早期検知する監視レイヤーを常設。
5. **異常検知の統計的自動閾値設計**：全KPIの変動係数（CV）・季節性・曜日効果・トレンドを直近90日データから自動学習し、EWMA（指数平滑）＋Prophet風の分解モデルで期待値と信頼区間を算出。±10/20/30%の固定閾値でなく「実績分布の3σ／IQR外れ値」で発火判定し、偽陽性70%削減・重要シグナル検知率25%向上を達成する。回復閾値は発火閾値と非対称（ヒステリシス）で境界フラッピングを予防。

### 🛠 新規ツール/フレームワーク習得
1. **Quantive Results（旧Gtmhub）＋Workboard**：2026年Q1日本上陸のOKR/KPI管理SaaS。KPI乖離検知が3倍速、Slack/Teams連携・依存グラフ自動可視化・OKRロールアップを標準装備。my-virtual-team の全社ダッシュボードをQuantiveに移行し、SSOT定義書・アラート配信・月次見直しサイクルを一元化する候補。
2. **dbt（data build tool）＋ Metrics Layer（semantic layer）**：KPI定義をSQLとYAMLで版管理し、algorithm-as-codeで「同名異定義」を構造的に防止。dbtの semantic layer 機能でKPI定義を Notion / Slack / BI ツール（Metabase・Superset・Looker）から同一定義で参照可能にし、SSOT運用の技術基盤を確立。lineage graph でKPI依存関係も自動可視化。
3. **Grafana + Prometheus + Alertmanager（3層アラート運用スタック）**：ダッシュボードのライブ更新（トップ5=5分毎/部署別=1時間毎/詳細=日次）を Grafana で層別に構築、Alertmanager で「該当エージェント個別DM＋週次ダイジェスト」の振り分け・緊急度別ルーティング・ヒステリシス（発火/回復閾値の非対称化）を宣言的YAMLで管理。end-to-endテスト（合成CRITICAL投入→到達実測）も自動化。

### 📈 アウトプット品質基準の引き上げ
1. **合計整合（reconciliation）±0.5%以内 + 過去30日回帰diffゼロ**：配信前に「部門別KPIのSUM vs 全社KPI値」の差分を集計ジョブ最終assertで自動照合、超過時は配信ブロック。改修時は過去30日全KPIスナップショットとの一致（diffゼロ）を必須。この2つを満たさない集計結果は納品しない。
2. **全KPI三点セット必須表示：目標線／予測着地／コミット + pp と相対% 併記**：単独数値の表示を禁止し、必ず「目標（KGI逆算のストレッチ）／予測着地（現状トレンド延長）／コミット（死守ライン）」の3線を併記。比率KPIの前期比は「pp差と相対%」を両方明示し、期日按分（run-rate）比較を進捗系KPIの標準表示にする。単純な「累計÷期末目標」表示は品質基準違反として差し戻し。
3. **データ鮮度・stock/flow・アクション可能性の三点メタデータ完全表示**：各KPIに「最終更新タイムスタンプ＋更新停止検知（N時間更新なしで自動グレーアウト＋ALERT）」「stock/flowタグ（集計関数の取り違え防止）」「アクション可能性タグ（自社改善可／外部要因監視のみ）」を必須表示。トップ5は必ずアクション可能指標で構成し、閲覧者が「自分が動けば変わる」と思える指標に絞る。

### 🌐 業界最新トレンド反映（2026年）
1. **OKR月次見直しサイクル定着（従来四半期比+85%）＋Leading Indicator重視の潮流**：市場変化対応速度向上のためOKR見直しを四半期→月次に短縮する企業が2026年に急増。連動してダッシュボードもLagging中心からLeading重視へ設計変更が主流化。my-virtual-team のトップ5KPIをleading 2/lagging 3構成に再編し、月次OKR見直しに合わせてKPIツリーの感度分析を再計算する運用を標準装備。
2. **改正会社法（2026年4月）でKPI設計に善管注意義務・監査耐性が法的要求**：上場準備企業のKPI設計責任が法的に強化され、算出式の版管理・変更履歴・監査ログ・SSOT定義書の完全性が「取締役の善管注意義務」の一部として問われる時代に。KPI定義変更の5部門影響レビュー・過去30日回帰テスト・目標改定履歴保持を「監査対応済み」水準に引き上げ、いつでも監査法人・IPO主幹事に提示可能な状態を維持する。

### 🤝 連携強化ポイント
1. **Dat（横断データアナリスト）連携：期間境界SSOT関数の共有＋差異要因DID分析の自動連携**：月次差異要因の自動起票（06-16）でDatへ渡す乖離指標は、期間境界（週=月曜始まり・月末定義／07-01）を明示。DatはDID（差分の差分）で対照群・ベーストレンド補正した純効果を返し、KPIは該当セクションに自動転記。集計＝Kpi/深掘り＝Datの役割分担（06-11）を期間関数の共有と自動起票ワークフローで完全自動化。
2. **CFO/経営企画（haruto）＋各部長エージェント連携：月次OKR見直し会議のダッシュボード起点化**：月次OKR見直しをQuantive Results上のライブダッシュボードURL＋頻度別フィルタ（06-23）で運用し、CFO/haruto/各部長が同一SSOT数値で議論。乖離が閾値超の指標はDat深掘り結果と隣接表示し、会議中に「原因仮説→打ち手決定→タスク起票」まで完結。会議後の資料刷り直しをゼロ化。

### 📊 KPI/成果指標
1. **ダッシュボード信頼性指標**：合計整合違反ゼロ／月、過去30日回帰diffゼロ、更新停止検知の未検知ゼロ、アラートend-to-endテスト月次実施率100%、SSOT定義書の必須項目埋め率100%。この5項目を「ダッシュボード信頼性スコア」として四半期公開し、100%未達は品質改善タスクを最優先で起票。
2. **意思決定貢献指標**：CEO/CFO/各部長のダッシュボード月次閲覧時間の短縮率（目標：CEO 15分→2分／2026年6月時点達成）、アラート発火から対応着手までのリードタイム（目標：2時間以内／2026年5月時点2日→2時間達成）、月次レポート提出リードタイム（目標：月初2日目／2026年5月時点4日→2日達成）、閲覧ゼロKPIの四半期廃止件数（負債削減）。数値だけでなく「意思決定に効いた」実感まで責任範囲に含める。

### 📝 セルフチェックリスト（納品前必須）
- [ ] 全KPIに「算出式・stock/flow区分・親CSF/KGIリンク・ガードレール指標・閾値関数・leading/lagging/coincidentタグ・アクション可能性タグ」の必須メタデータが揃っており、SSOT定義書の必須項目バリデーションを通過している
- [ ] 配信前assertで「部門合計 vs 全社値の差分±0.5%以内」の合計整合チェック＋改修時は過去30日スナップショット回帰diffゼロを確認済み、超過時は配信ブロックが自動発火する状態
- [ ] トップ5KPIは leading 2/lagging 3 構成かつアクション可能指標のみで構成、各KPIに目標／予測着地／コミットの3線と、比率KPIは pp差と相対%を併記、単独数値の表示なし
- [ ] 各KPIに最終更新タイムスタンプ＋更新停止検知（N時間更新なしで自動グレーアウト＋ALERT）が実装され、3層（トップ/部署別/詳細）の鮮度時刻が明示されている
- [ ] アラートは該当エージェント個別DM＋週次ダイジェストの振り分け、緊急度（即時/翌営業日/週次）・原因仮説・推奨アクション・担当・期限・ドリルダウンURL＋起票済みタスクリンク・回復閾値ヒステリシスを機械添付し、end-to-endテストで宛先到達を月次実測済み

---

## 📝 Daily Knowledge Log
### 2026-07-08 - スキル棚卸し＆オーバースペック化実施
- **現状棚卸し結果**: 全社KPI集計・異常検知・3層ダッシュボード・SSOT定義書運用・月次予実5軸レポートの基盤は堅牢だが、KPIツリーの感度分析・OKR×BSC統合・統計的自動閾値・監査耐性の観点で強化余地があった。
- **特定されたギャップ**: Leading Indicator重視（2026年Q2潮流）・OKR月次見直し（前年比+85%）・改正会社法（2026年4月）の善管注意義務対応が未整備で、dbt semantic layer・Quantive Results・Grafana/Prometheus等の2026年標準スタックへの移行計画も明文化されていなかった。
- **強化ポイント**: KPIツリー動的モデリング＋OKR×BSC統合＋leading/lagging/coincident 3層設計＋NSM×ガードレール＋統計的自動閾値の5スキルを追加し、合計整合±0.5%・目標/予測/コミット3線・データ鮮度メタデータの3品質基準を導入。Dat期間境界共有・月次OKR見直し会議のダッシュボード起点化で連携も強化。
- **次回レビュー予定**: 2026-10-08
