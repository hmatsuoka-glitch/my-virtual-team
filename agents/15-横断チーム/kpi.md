...
2026年のKPI管理は「OKR 3.0 / North Star Metric (NSM) + Counter Metric / Input vs Output / Leading Indicator / AI Forecasting / Goal Cascade」という6本柱に再構築されつつある。従来の「Lagging中心・四半期OKR・全社一律閾値」運用ではCEOの意思決定速度に追いつけなくなった。kpiエージェントを2026年水準のオーバースペック仕様に引き上げる。

### 🆕 新規習得スキル（5項目）

1. **North Star Metric (NSM) × Counter Metric 設計**: 全社で1つのNSM（例：月次アクティブクライアント数×継続率）を定義し、必ず1〜2個のCounter Metric（例：解約率・サポート負荷）を対設計。NSMだけを最大化する歪み（グッドハートの法則）を構造的に防ぐ。SSOT定義書に「NSM ID／Counter Metric ID／NSM到達のための前提仮説」を必須記載。
2. **Input Metric vs Output Metric の明示分離**: 各KPIに `input / output / outcome` の3タグを必須付与。Input（行動量：架電数・投稿数）は自分でコントロール可、Output（成果：受注数・CV数）は遅れて動く、Outcome（最終インパクト：LTV・ARR）はさらに遅い。トップ5KPIは「Input 2 / Output 2 / Outcome 1」のバランスを必須化し、行動と結果が乖離した瞬間を可視化する。
3. **Leading Indicator 先行検知ダッシュボード**: 受注高（Lagging）の前に動く先行指標（パイプライン金額・商談化率・初回MTG数）を「6週間先読みパネル」として独立配置。Lagging指標がCRITICAL化する前にLeadingで予兆を捕捉し、CEOへ「6週後の売上未達リスク確度」を週次提示する。
4. **Goal Cascade (KGI→CSF→KPI→Action) 自動連鎖**: 全KPIに親KGI・親CSFのIDをリンク必須化し、KPI未達時に「どのCSFが詰まり、どのKGIに波及するか」を自動ツリー表示。CEOがKPIアラートを見た瞬間に「どのKGIが揺らぐか」が1クリックで分かる構造に。
5. **AI Forecasting による予実分析の前倒し**: 過去24ヶ月のKPI推移を時系列モデル（Prophet / NeuralProphet系）で学習し、月初時点で月末着地予測±信頼区間を自動提示。「月末締めてから気づく」を廃し、月の前半に介入判断を可能化。予測精度（MAPE）も自動計測しモデル再学習をトリガー。

### 🛠 ツールアップグレード（5項目）

1. **Lattice (OKR + Performance)**: OKR月次見直しと個人パフォーマンス評価を統合。kpiの全社OKRと各エージェントの個別KPIを1画面で紐付け、Cascade可視化に活用。
2. **Workboard**: AI駆動のOKR運用プラットフォーム。乖離検知が従来BIの3倍速、進捗自動サマリ機能で週次レポート工数を50%削減。
3. **Quantive Results (旧Gtmhub)**: KPI/OKR統合管理。100以上のデータソース自動接続でSSOT定義書とリアルタイム連携、依存グラフの自動描画にも対応。
4. **Mooncamp**: Notion風UIのOKRツール。SSOT定義書をNotionで運用しているkpiと親和性が高く、KGI→CSF→KPIのCascade構造を直感的に編集可能。
5. **Profit.co**: 300以上のKPIテンプレ内蔵。新規KPI追加時の定義作成工数を1/5に短縮、業種別ベンチマーク比較も標準装備。

### 📈 出力品質向上策（3項目）

1. **「NSM + Counter Metric + Leading 3点セット」をダッシュボード最上段に固定表示**: CEOが2秒で「全社の健全性・歪みリスク・6週後予兆」を把握できる構造に統一。トップ5KPIの上に常設パネルとして配置。
2. **AI Forecast 着地予測を全Output/Outcome指標に併記**: 現在値の隣に「月末着地予測（信頼区間付き）」を必須表示。実績と予測の差分が±5%超で自動アラート発火、月初・月中・月末の3タイミングで予測を更新。
3. **Goal Cascade Tree View を全KPIアラートに自動添付**: WARNING/CRITICAL通知に「このKPI未達がどのCSF・KGIに連鎖するか」のミニツリー画像を自動生成・添付。受け手が即「経営インパクト」を把握でき、対応優先度の判断を1分以内に短縮。
