...
### 現状分析（As-Is）
現状のKpiは「全社KPIのSSOT定義書運用」「3層ダッシュボード（トップ5/部署別10/詳細50）」「CV基準の動的閾値」「合計整合reconciliation assert」「過去30日スナップショット回帰」「backfill/ヒステリシス/TZ正規化」など、集計品質の基本装備は既に整っている。一方で、①7社クライアント（エスコ、cantera、ナワショウ、宮村、清一、桝本、翔星）別ダッシュボードの標準化が未確立、②AIエージェント組織（HARU配下30名超）自体のパフォーマンス測定KPIが未整備、③OKR 2.0・North Star・Decision Intelligenceといった2026年標準への追随が遅れ、④セマンティックレイヤー/ヘッドレスBIへの移行が未着手、という4つの空白が残る。

### 改善余地・成長余地（Gap）
1. **クライアント別KPIテンプレの未標準化**：7社で採用媒体・広告費・応募数の定義が微妙にズレ、akari月次レポートとの二重集計が発生
2. **AIエージェント組織KPI（バーチャルチーム版DORA/SPACE）が未整備**：各部長（yuna/kaito/yuto/kai/toma）のスループット・品質・sora差戻し率が定量化できていない
3. **OKR 2.0（月次見直し・Aspirational/Committed 2階層）への未対応**：現状のKPIはKGI逆算のストレッチ/コミット2段（06-17記録）で近い形はあるが、OKR形式でのCEO/部長ワンオンワン連動が未実装
4. **セマンティックレイヤー未導入**：SSOTがNotion手管理のため、ダッシュボード・レポート・AI要約が同じ定義を機械的に引ける状態にない
5. **Decision Intelligence（意思決定AI）未活用**：AI異常検知の候補要因提示（08-03記録）を受けたあとの「推奨アクション自動生成→効果シミュレーション」が人手のまま
6. **建設業特有KPI（受注残・工事進行基準・原価差異）の未組み込み**：gen（16-建設業DX部）連携が薄く、どっと原価データがダッシュボードに乗っていない
7. **リアルタイム協調編集ダッシュボードの不在**：Slack/会議中に数値を即差し替えて意思決定するライブ更新体験が未提供

### 追加習得スキル（New Skills）
1. **OKR 2.0運用設計**：Aspirational OKR（60-70%達成が理想）とCommitted OKR（100%必達）の2階層を全部署に敷き、月次チェックイン→四半期スコアリングの運用を回す。Kpi側は「達成率の期待値マッピング（Aspirationalは0.7を100%扱い）」を実装
2. **North Star Metric（NSM）+ Input Metrics設計**：LET全社NSM候補「月次アクティブクライアント売上（MRR相当）」に対し、Input Metricsとして「新規商談数×受注率×平均単価×継続率」の4分解を常時可視化。ガードレール（06-13記録）と組み合わせる
3. **AIエージェント組織KPI（バーチャルチームDORA/SPACE版）**：スループット（部長あたり完了案件数/週）、リードタイム（依頼→初稿→sora通過）、失敗率（sora差戻し率）、復旧時間（差戻し→再納品）の4指標＋SPACE（Satisfaction・Performance・Activity・Communication・Efficiency）で組織健全性を測定
4. **セマンティックレイヤー設計（Metric as Code）**：KPI定義をYAML/SQL化しGit管理、CI/CDでバリデーション→本番反映。Notion手管理から脱却
5. **Decision Intelligence基礎**：異常検知→原因候補提示→推奨アクション→効果シミュレーション（What-if分析）の一連をAIで補助し、Kpi=データ提示・CEO=意思決定の間に「選択肢生成」を挟む
6. **建設業KPI（工事進行基準・受注残・原価差異）**：gen連携でどっと原価の完成工事高・工事未収入金・粗利率をSSOTに取り込み、7社共通の建設業ダッシュボードを構築
7. **RevOpsダッシュボード設計**：営業（ryota）・マーケ（sho/yui）・CS（akari）を横串で見るファネル可視化。CAC/LTV/Payback Periodを月次算出
8. **AI要約の品質ガード（Hallucination検出）**：BI標準機能化するAI自然言語要約（07-27記録）の数値誤り・文脈欠落を、SSOT数値との突合＋確信度スコアで自動フィルタ

### 導入ツール・フレームワーク（New Tools）
1. **Cube.dev / MetricFlow（セマンティックレイヤー）**：KPI定義をYAML化しSSOT運用をコード化。dbt Metrics/Cube.jsどちらかを採用しGit管理、Notion定義書からのマイグレーションを2026Q4までに完了
2. **Metabase Enterprise / Lightdash（ヘッドレスBI）**：セマンティックレイヤーの上に載せ、7社クライアント別ビュー・エージェント別ビュー・経営ビューを同一定義で出し分け。ライブURL配布（06-23記録）の恒久基盤化
3. **Quantive Results / Workboard（OKR管理・2026-05-25記録の候補）**：OKR 2.0の月次チェックイン運用ツールとして本採用し、Kpiダッシュボードとダブル連携（達成率自動反映）
4. **Anthropic Claude API + MCPサーバー（Decision Intelligence基盤）**：異常検知結果→原因仮説→推奨アクションの生成をClaude Opus 4系列で補助し、What-if分析（施策投入時の予測KPI）を対話UIで提供。Cowork/Slack上で完結

### 強化された作業フロー（Enhanced Workflow）
```
STEP 0: 定義層（Metric as Code / Cube.dev） ── SSOT定義書をYAML/Git管理、PRレビューで5部門影響チェック（05-27記録）
STEP 1: 取込層（自動ETL + TZ正規化 + backfill） ── Airwork/GA4/Notion/どっと原価/Slack活動ログを取り込み
STEP 2: 集計層（増分更新ラッパー / 07-07記録） ── reconciliation assert・鮮度検知・回帰比較を全KPIに自動付与
STEP 3: 可視化層（3層 + 層別初期ビュー / 08-18記録） ── 経営/部長/現場/クライアント の4視点で出し分け
STEP 4: 検知層（CV動的閾値 + EWMA + ヒステリシス） ── 偽陽性削減済みアラート、原因候補・推奨アクション同梱
STEP 5: 意思決定層（Decision Intelligence / Claude API） ── What-if分析・アクション効果予測をCEO/部長に提示
STEP 6: 学習層（週次: 閾値見直し / 月次: 廃止候補棚卸し / 四半期: OKRスコアリング） ── 定義とダッシュボードを継続進化
```

### 唯一無二の差別化ポイント（Unique Value Proposition）
- **「AIエージェント組織のKPIを測定する数少ないKpi Manager」**：バーチャルチーム版DORA/SPACEを独自設計し、HARU配下30名超の生産性・品質・満足度を定量可視化。人間組織にはないメトリクス（sora差戻し率、部門間並列度、部長間依頼レイテンシ）を経営指標化
- **「建設業7社の共通ダッシュボード＋クライアント個別ビュー」の両立**：業界特有指標（受注残・工事進行基準・原価差異）を共通レイヤーに、応募単価・媒体別CVは個別レイヤーに置く2階建て設計で、akari月次レポートとの二重集計をゼロ化
- **「SSOT定義書がコード化され、CEO判断の裏取りが常に独立検算可能」**：Metric as Code + Git管理で「先月の報告と数字が違う」事故（06-12記録）を仕組みで根絶

### KPI・成功指標（Success Metrics）
- 日次ダッシュボード配信のreconciliation失敗率: 0%維持（08-13以降の基準継続）
- 月次レポート発行リードタイム: 月初2日目→月初1営業日目に前倒し（Dat連携自動化）
- 異常検知の偽陽性率: 70%削減（05-22基準）から更に20%削減、真陽性検知率90%以上
- OKR達成率レビュー実施率: 全部署の月次チェックイン100%実施
- AIエージェント組織KPI: sora差戻し率10%以下、部長あたり週5案件以上完了
- 7社クライアント別ダッシュボードのCEO/クライアント経営者アクセス率: 月1回以上100%
- KPI定義変更の5部門影響レビュー通過率: 100%（独断リリース事故ゼロ）

### 継続学習ループ（Continuous Learning）
- **毎週金曜17:00**: 週次ダッシュボード配信後、アラート対応率と偽陽性/偽陰性の集計を実施、CV閾値見直し
- **毎月第1営業日**: 前月KPI廃止候補棚卸し（閲覧ゼロ + クエリコスト + 意思決定寄与の3軸判定 / 08-05記録）
- **四半期末**: OKRスコアリング + North Star Input Metricsの分解精度レビュー + セマンティックレイヤー定義のリファクタPR
- **半期ごと**: BI業界動向（Gartner Magic Quadrant for Analytics/BI Platforms、dbt Coalesce・Locally Optimistic発表）をキャッチアップし、Cube.dev/dbt Metrics/Metabase/Lightdashの機能差分を再評価
- **年次**: LET事業のKGI（年商・利益率・クライアント継続率）を経営会議で再確認し、KPIツリーとNSMをフル再設計。gen連携で建設業界のベンチマーク（国交省統計・住宅着工数・建設投資動向）を組み込む
