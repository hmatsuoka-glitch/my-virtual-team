...
### 2026-09-10
- **[新スキル：North Star Metric + Input Metrics の "階層 KPI ツリー" を dbt で SSOT 化]**：LET 事業の North Star Metric を「クライアント現場入場人数（HR 09-10 記録の 7 段ファネル最終段）」に据え、その Input Metrics（応募数・書類通過率・面接通過率・承諾率・入社率・入場率）を dbt Semantic Layer（Dat 09-10 記録）で 1 SSOT に集約。四半期ごとの North Star 更新は Kai・Haruto・Sora の 3 者レビュー必須で GitHub PR 化し、指標定義の変更履歴を全社で可視化する。バニティ指標混入（06-24 記録）を構造的に排除。
- **[新ナレッジ：SLI/SLO/SLA と Error Budget の KPI 運用適用]**：SRE 由来の SLI（Service Level Indicator）／SLO（Objective）／SLA（Agreement）と Error Budget（許容失敗量）の考え方を KPI 運用へ適用。「月次目標達成率 95% を維持できれば残りの 5% は施策実験に使ってよい」というエラーバジェット思考で、目標達成に対する挑戦バランスを構造化。Sora・Haruto の意思決定を「守り／攻め」の二分法から「今月の残バジェット」で判断する運用へ。
- **[効率化ルール：ダッシュボード変更検知の "Git-based Diff Notification"]**：SSOT 定義書・KPI ツリー・ダッシュボード設定を GitHub リポジトリで管理し、変更のたびに Slack #kpi-changes へ Diff を自動投稿。従来「月末に定義変更が発覚→過去数値の意味が変わって混乱」の事故が四半期 2 件発生していたのを、Diff Notification で発生の瞬間に全社周知しゼロ化。定義変更を Silent Change でなく Loud Change にする文化装置。
- **[失敗パターン→回避策：AI 要約の "数値誤読" と "文脈欠落" 事故]**：ダッシュボードの変化点を AI が自然言語で要約する機能（07-27 記録）で、「先週比 30% 増」を「先月比」と誤要約したり、季節性・キャンペーン影響のコンテキストを省略したりする事故。回避策は AI 要約を Sora QA 前ゲートに入れず、必ず Kpi 担当者が「主要指標 3 個のみ AI 要約 → 残りは人手記述」の分担にし、AI 要約結果は必ず数値の一次ソースリンクを併記して機械照合可能な状態にする。
- **[連携強化：Dat との "指標定義 CI/CD" 相互統合]**：Dat 09-10 記録と対称に、Kpi の SSOT 指標変更は Dat レビュー必須、Dat の分析用指標追加は Kpi レビュー必須の相互 PR 承認へ。Kpi の速報/確定区別（06-03 記録）と Dat の未確定期区別（08-12 記録）を共通の状態タグ `is_finalized` で表現し、両部門が同じ真実を共有する。指標定義の食い違いによる報告齟齬をゼロ化。
- **[品質チェック：Sora QA 提出前の "ダッシュボード 7 点セルフ検品"]**：① 合計整合 reconciliation（06-12 記録）② 単位統一（09-09 記録）③ 目標未設定＝グレー表示（09-02 記録）④ 母集団変更の断絶線（09-02 記録）⑤ 参照値の互換性判定済み（09-01 記録）⑥ モバイル実機で層別ビュー可読（09-09 記録）⑦ 欠測フラグの経営除外・本人通知振り分け（09-01 記録）、の 7 点を自動判定してから Sora へ回す。Sora は数値整合を再検証せず、経営判断への繋がりだけを見る役割分担。
