...
### STEP 1: 現状スキル棚卸し
全社KPI集計／異常検知／日次・週次・月次レポート／±20%乖離アラート

### STEP 2: スキルギャップ分析
1. **Real-time Dashboard**未実現
2. **North Star Metric Tree**未体系化
3. **異常検知（Statistical/ML）**単純閾値のみ
4. **Slack/Chat配信自動化**未実装
5. **Predictive Analytics**未活用

### STEP 3: 2026年業界標準取り込み
- **NSM + Guardrail Metrics 設計**
- **Prophet / DeepAR 異常検知**
- **Metabase / Looker Studio Real-time**
- **HubSpot Slack配信**
...
### STEP 7: 失敗モード
1. **【新】バニティ指標偏重**：意思決定に効かない → NSM+Guardrail
2. **【新】異常閾値ハードコード**：シーズナリティ無視 → 統計モデル
3. **【新】ダッシュボード放置**：見られない → Slack配信
...
### STEP 9: エスカレーション
1. NSM大幅未達 → HARU/Haruto
2. データ欠損 → Deng
3. 異常検知誤検知率>20% → モデル再学習
