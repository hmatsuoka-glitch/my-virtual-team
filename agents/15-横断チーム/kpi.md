...
### 2026-07-12
- **2026年潮流：メトリクス・セマンティックレイヤー（dbt Semantic Layer / Cube）でSSOT定義を「実行可能な計算契約」に格上げ**：これまでSSOT定義書はNotionのテキストで算出式を書いていたが、下流のBI/Slack Bot/月次PDFが個別SQLで再実装し微妙にズレていた。dbt Semantic Layer / Cube Semantic Layerで指標を宣言し、全下流が同一レイヤーからクエリする方式に切り替えると、同名異定義の実装ズレを構造的にゼロ化、指標追加の実装工数が1件2h→20分、Bo/Owl/Pr/Dat連携の算出式整合（06-11/07-02）が定義参照だけで自動整合する。
- **先端手法：異常検知にベイズ変化点検知（BOCPD）＋Prophet分解を導入し、CV/EWMA閾値では拾えない「緩やかな構造変化」を検知**：EWMA（06-20）は急変への感度は良いがトレンドが緩やかに折れる構造変化（例：新規リード獲得チャネルの実質枯渇）を偽陰性で見逃す。Prophetでトレンド/季節性を分離し、Trend成分にBayesian Online Change Point Detectionを当てると、閾値では検知遅れ2〜3週間→BOCPD併用で6週間前倒し検知になる指標が全KPIの40%に到達。07-01の「実績トレンド乖離併用」の実装体を先端手法に格上げする。
- **横断連携：月次差異要因の深掘りループを「KPI検出→Dat DID分析→Pm要員影響→Pr対外表現統一」の並列Slack Workflowに束ねる**：これまで月次差異検出後に手動でDat→Pm→Prを順次依頼し月初提出まで3営業日かかっていた。乖離検出時点で3エージェントに同期タスクを自動起票し、Datの純効果分析（07-02）・Pmの稼働影響（07-02）・Prの対外文言案（07-02）を並列で回し、返信を該当セクションへ自動差し込みすると、月初提出リードタイムが3営業日→半日、転記工数がゼロ化する。06-16の自動起票と07-02の三面整合を1本のWorkflowに統合する。
- **意思決定支援：ダッシュボードに「予測着地の信頼区間（Prediction Interval, 80%/95%）」を必須表示し、点推定でなく確率分布で経営判断させる**：目標/予測/コミット3線（06-20）の「予測」を単一点で出すと「その予測は当たるのか？」の問い直しで会議が止まる。Prophet / ARIMAで80%/95%の予測区間を計算し、目標線が95%区間の外なら「未達確率が高い」、80%区間内なら「達成濃厚」と一目で判定できる。予測に対する経営判断の意思決定時間が15分→3分、コミット割れの早期対応起票率が20%向上。
- **AI活用：WARNING/CRITICALアラート本文の「原因仮説・推奨アクション・期限」をLLMで自動生成し、テンプレ手作業をゼロ化**：05-24の「原因仮説1行／推奨アクション1行／担当／期限」添付はこれまで人手でテンプレを埋めていた。Claude/GPTに「該当KPIの直近30日データ＋依存グラフ＋過去の類似アラート対応履歴」をコンテキスト渡しして生成させると、アラート1本あたりの手作業が10分→即時0分、生成品質も週次サンプリングレビューで承認率95%を維持。06-23のドリルダウンURL/タスクリンク同梱と組み合わせ「押すもの」化を完成させる。
- **2026年標準：Amplitude North Star Framework 3.0を導入し、NSM＋Input Metrics 3-5個＋ガードレール2個の階層固定でトップ画面を再構築**：現行のトップ5KPI（leading 2/lagging 3・05-27）を、NSMを頂点にInput Metrics（NSMを動かす操作可能な先行指標）を明示階層化する2026年Amplitude/Reforge標準に寄せる。「何を動かせばNSMが動くか」が視覚化され、Input Metricsへの打ち手起票率が30%→65%、NSM改善サイクルが四半期→月次に短縮。06-13のガードレール（カウンターメトリクス）と06-24のバニティ排除ルールをこの階層に強制適用する。
- **効率化：Great Expectations / dbt testsで「データ品質assert」を宣言化し、配信前品質ゲート（06-26）をSSOT定義から自動生成**：合計整合±0.5%（06-12）・鮮度検知（06-03）・過去30日回帰diffゼロ（06-12）・分母最小サンプル数（06-03）はこれまでKPIごとに集計スクリプトへ手実装していた。Great Expectations（またはdbt tests）でExpectationとして宣言し、SSOT定義書の指標メタから自動生成する形にすると、新規KPIは登録した瞬間に品質ゲートが揃い、実装漏れがゼロ化。品質assert整備工数が指標1件30分→2分、07-07の集計ラッパー強制の完成形になる。
