...
### 現状スキル評価
日次/週次/月次のレポーティング型運用は形になっているが、ダッシュボード実装層（Looker Studio 単発）が指標定義の SSOT を持たず、Kpi 個人の運用ルールで整合を取っている。異常検知が「目標±20%」の閾値ベースのみで、季節性・トレンド調整をした異常検知や統計的プロセス管理が未導入。指標定義（メタデータ）が Notion テキスト管理で、変更の影響範囲追跡が手作業。

### 特定した改善余地
- ダッシュボードが Looker Studio 一択で、複雑なドリルダウン・パーソナライズが困難
- 指標定義（SSOT）が Notion 手書きで、実装との乖離が発生しやすい
- 異常検知が固定閾値、季節性・トレンド調整をした統計的検知（3σ、ARIMA残差）が未導入
- リアルタイム性が日次バッチ止まりで、広告の予算焼き切れなど即応が必要な指標に対応できない
- Rui/Dat から受け取る参照値のメタデータ管理が指標名ベースで、定義互換性の突合が手動

### 追加スキル10選（オーバースペック化ロードマップ）
1. **Looker (LookML) セマンティックレイヤ** — 指標定義を LookML に集約、Explore で全社共通ドリル可能／KPI: 指標定義の重複ゼロ・変更影響追跡100%
2. **Metabase セルフサービスBI** — 非エンジニア（BO・営業）が自分で切って見られる／KPI: 月間アクティブユーザー 12→35名
3. **Superset (Apache) OSS BI** — カスタムビジュアライゼーション、SQL Lab で分析者向け／KPI: ダッシュボード数 20→60
4. **Notion Database + Formula 3.0 SSOT** — 指標定義・メタデータ・変更履歴を Notion で構造化管理、Slack/Linear と双方向同期／KPI: 定義変更→通知→影響レビュー完了 24h以内
5. **Linear Metrics Integration** — 指標変更を Linear Issue で管理し、影響部門を Assignee で強制通知／KPI: 定義変更の未レビュー 0
6. **Statistical Process Control（SPC）チャート** — 平均±3σの管理限界線で「異常」を統計的に判定／KPI: 誤検知率 -60%
7. **ARIMA / Prophet 残差ベース異常検知** — 季節性除去後の残差でスパイク検知／KPI: 検知精度（F1）0.75以上
8. **Streamlit / Retool リアルタイムダッシュボード** — 広告予算残・応募数など秒単位更新が必要な指標を専用UIで／KPI: 主要3指標のレイテンシ <30秒
9. **DataDog Metric + Notebook 経営KPI版** — SRE用ツールを経営KPIに転用、SLI/SLO ベースの目標管理／KPI: KPI違反アラート応答時間 15分以内
10. **OKR/KR ダッシュボード（Notion + Formula）** — 全社OKRを四半期単位で追跡、KR進捗の週次コミット率を可視化／KPI: KR達成率 70%以上、OKRレビュー参加率 100%

### 新規ナレッジソース・ツール
- Looker (LookML) — Google Cloud の企業向け BI、セマンティックレイヤ標準
- Notion Database + Automations — 指標カタログ・変更管理・通知
- Linear — 指標定義変更のワークフロー管理
- Streamlit / Retool — Python/Web で作る軽量リアルタイムダッシュボード
- DataDog Metric — SRE 由来のメトリック管理・アラート

### 連携強化ポイント
- **Dat (データアナリスト)**：dbt Mart 層を Kpi の唯一のデータソースとし、指標定義変更は dbt テスト+ADR+Notion SSOT の 3 点セットで管理
- **Bo (業務自動化)**：ジョブ由来の「取得件数ゼロ」を欠測フラグ付きで受領、鮮度SLA を Bo のジョブ実行頻度と Notion 台帳で突合
- **Pm (プロジェクトマネージャー)**：クライアント報告の「変化点1枚」を Kpi 側で用意し PM 経由で LINE 要点3行に載せる、二重集計を禁止

### 実践シナリオ例
7クライアント合算の「今週の1枚」ダッシュボード刷新案件。Notion SSOT に主要18指標の定義（分子・分母・期間境界・母集団条件・鮮度SLA）を登録、Looker で LookML 化して全指標を Explore で切り替え可能に。SPC で「応募数」の管理限界を各クライアント×媒体別に算出、閾値ベースなら誤検知していた季節性由来のスパイクを除外して真の異常のみ検知。Streamlit で広告予算残・配信状態を30秒更新するリアルタイムダッシュボードを構築、Marketing が「昨日で予算焼き切れていた」を今朝発見するのではなくその瞬間に対応可能に。OKR ダッシュボードで四半期 KR「MQL 200件」「受注8件」「クライアント継続率90%」の進捗を毎週金曜自動更新、CEO の意思決定サイクルが月次→週次に短縮。

### 2026-08-29 Daily Knowledge Log 追記
- **Notion SSOT v1 構築完了**：主要指標18本の定義を Notion Database 化、Formula 3.0 で「変更時に Slack 通知＋Linear Issue 自動作成→影響部門レビュー」のワークフロー実装。初週で 4 件の定義変更があり全て 24h 以内にレビュー完了、従来の Notion テキスト運用時代に月2件発生していた「Kpi と Marketing の指標定義ズレ」がゼロに。
- **Looker LookML 移行実施**：Looker Studio の主要 8 ダッシュボードを LookML 化、Explore 機能で全指標が View 経由で切れるように。営業マネージャーが「翔星建設×TikTok×週次」を自分で切って3分でレポート作成、従来 Kpi への依頼→翌日納品のフローが即時化。
- **SPC 導入で誤検知激減**：応募数の異常検知を固定閾値±20%から SPC の 3σ管理限界に変更、7クライアント×4媒体=28系列で運用。8月の異常検知件数 47→13、うち真異常 11 件（F1 スコア 0.83）。誤検知アラート疲れがなくなり Marketing の初動が速くなった。
- **Streamlit で広告リアルタイムダッシュボード稼働**：Meta Ads API + Airwork API を 60 秒ポーリング、予算消化 90% 到達で Slack 即通知。稼働2週間で 3 件の「予算焼き切れ直前検知」に成功、従来なら翌朝発覚して1日配信停止していた損失（推定¥120k×3=¥360k）を回避。
- **OKR ダッシュボード v1 公開**：Q3 のOKR（O:サクバズ事業を建設業DXのデファクトに）を KR 5 本で分解、Notion Formula で週次進捗を自動集計。金曜17時に CEO・部長へ Slack DM で「今週の KR コミット率 68%・遅れは KR3」を配信、月次レビュー時の「振り返って気付く」を週次発見に前倒し。
