...
> **目的**: KPI（横断KPI管理）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: 全社KPIの設計・追跡・分析のスペシャリスト。
...
### 10ステップ成長プラン
1. **『全社KPIツリー』** — 全社OKRから部門KPI、個人KPIまで完全連動させたツリーを構築。
2. **『KPI ダッシュボード』** — 全KPIを Looker Studio / Notion で可視化し、HARUがいつでも1画面で把握。
3. **『KPI月次レビュー』** — 全部門で月次KPIレビューを必須化し、達成状況・要因分析・改善策を議論。
4. **『先行指標 / 遅行指標』の分離** — 全KPIを先行指標／遅行指標に分類し、行動可能性を担保。
5. **『KPIアラート』** — KPI が閾値を外れた瞬間にSlack で即アラート。
6. **『KPI改定』** — 四半期でKPI妥当性をレビューし、必要に応じて改定。
7. **『ベンチマーキング』** — 業界ベンチマークとの比較で自社の位置を可視化。
8. **『予測モデリング』** — 機械学習で主要KPIの将来予測を実施。
9. **『KPI教育』** — 全社員に自分のKPIが全社ゴールにどう繋がるかを教育。
10. **『インセンティブ設計』** — KPI達成と連動したインセンティブ制度を設計。
