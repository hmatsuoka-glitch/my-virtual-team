...
### 2026-08-08
- **効率化テクニック：ノーススター指標（NSM）を「入力→活動→出力→成果→影響」の5層で機械的に分解し、各層に1個ずつ先行指標（Leading）を置く**。NSMだけでは動かせる打ち手が見えず「結果が悪い→どこを直す？」で停止する。例えば「月商」(成果層)を分解すると、入力=リード数、活動=商談件数、出力=受注数、成果=月商、影響=LTV/リピート率と5層になり、各層のleadingを毎日追うことで悪化の発生源を層別に特定できる。トップ5KPIの「leading 2/lagging 3」構成（05-27記録）を、この5層のどこから2つleadingを取るかの選択問題に変換すると、KPI選定の議論が構造化される。新規KPI登録フォーム（06-23記録）にも「NSM分解のどの層に属するか」を必須項目に追加する
- **失敗パターン：KPIドリフト（指標定義は変えていないのに、データソース側の仕様変更や運用実態の変化で数値の意味が徐々にズレる）を検知せず、時系列で無意識に別物を比較する** → 回避策：主要KPIに「四半期ごとのメタ監視（分母構成比・データソース別比率・データ収集経路の変化）」を仕込み、構成比が±10%以上変わったら「定義は同じでも中身が変わった疑い」としてSSOT定義書に注記する（理由：例えば「新規リード数」の定義は同じでも、フォーム経由が主だったのがチャット経由が主に変わると質が違うのに数値上は連続に見え、施策評価を歪める。定義書のスナップショット回帰・06-12記録は改修由来の変化を捕捉するが、データ由来のドリフトは別の監視が要る）
- **品質チェックポイント：ダッシュボードの色使いにアクセシビリティ（色覚多様性）検証を組み込み、緑/赤の状態表現を「色＋形＋方向矢印」の3重冗長化で表示する**。日本人男性の約5%が色覚特性を持ち、緑/赤の識別が難しい閲覧者にとって色だけの状態表示は判読不能になる。方向矢印併記（06-07記録）は既に運用しているが、CRITICAL/WARNING/INFOのバッジ形状（●/▲/■）も分けて色に依存しない判別を担保。BIツールの色覚シミュレーション機能で配信前チェックを品質ゲートに追加し、包摂性を数字の伝わりやすさと同格に扱う
- **連携ヒント：コホートベースレポートを月次レポートに標準セクション化し、Datのコホート分析（Datの06-13/06-24記録）と隣接して同一ダッシュボード上に置く**。継続率・LTV・チャーン系の指標は月次断面（今月の解約率）だけで見ると新規流入で希釈されて改善に見えるが、契約月別コホートで見ると悪化していることがある。Kpi側は「契約月×経過月」のヒートマップで生存曲線を可視化し、Datの生存時間分析（Datの08-03記録）のインプットとなる打ち切りデータ（観測途中の継続顧客）を正しくストック指標（06-13記録）として集計する。ガードレール指標（06-17記録）としてコホート別継続率をNSMに対で置く
- **トレンド察知：エージェントAIによる「対話型ダッシュボード（Conversational Analytics）」がBIツールの標準機能として実装フェーズに入り、閲覧者がChatで「なぜ今月の翔星建設の解約率が高い？」と聞くと候補要因＋関連KPI＋Dat深掘り起票までワンストップで返る流れ**。AI異常検知の候補要因提示（08-03記録）の次の進化で、閲覧者側の「気になったら深掘りを依頼する」動線をチャット1回に集約する。ただしAI応答の数値裏取り・目標形骸化フラグ判定（07-16のDat連携）は人手判断を残し、対話ログをSSOT定義書の「よくある問い」ライブラリ（Datの06-23の問い集ライブラリと同期）として蓄積すると、次回同じ問いへの応答品質が上がる
