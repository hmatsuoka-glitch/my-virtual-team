...
### 2026-09-07
- **KPIツリー「North Star Metric×Input/Output KPI」再設計**：全社KPIをNSM（月次アクティブクライアント継続率）を頂点にInput KPI（施策実行数）・Output KPI（成果指標）に分離した3層ツリーで再設計。CEO/HARUの意思決定に「どのKPIを動かすと何が変わるか」の因果構造を可視化、経営会議の議論効率50%向上。
- **Metabase×BigQuery「リアルタイム経営ダッシュボード」構築**：Metabase Cloud+BigQuery Streaming Insertsでリアルタイム更新のダッシュボードを構築。日次バッチ→リアルタイム化により、CRITICALアラート検知タイミングを24時間→数分に短縮、機会損失を月間推定150万円削減。
- **OKR運用「四半期OKR×週次チェックイン」の全社標準化**：全部署でOKR形式（Objective+3 Key Results）を四半期設定、週次チェックインで達成度スコアを自動集計。KPIダッシュボードにOKR進捗を統合、全エージェントの成果可視性が飛躍的向上。
- **建設業7社×サクバズKPI「クライアント別ROIダッシュボード」**：翔星建設・宮村建設・cantera等の広告費・応募数・採用単価・LTVを7社別ダッシュボード化、月次経営レビューで即時比較可能に。クライアント継続率+15%、ryotaの提案書にも直結。
- **異常検知「Prophet+曜日効果補正」で偽陽性70%削減**：Prophet（Meta OSS）のトレンド分解+曜日効果補正を導入、季節性起因の誤アラートを70%削減。「アラート慣れ」を予防し、CRITICALの即応性を担保。
- **AI-native「Claude APIによる月次経営レポート自動生成」**：Metabaseのデータ+月次PL+全エージェント出力をClaude APIで統合分析、経営レポート初稿を自動生成する仕組みを実装。月次レポート作成8時間→30分（94%削減）、CEO判断の質と速度両立。
- **Dat連携：異常検知の原因仮説を自動付与するパイプライン**：Datと共同で、CRITICALアラート発火時に原因仮説×推奨アクション×担当エージェント×期限を自動付与、対応リードタイム2日→2時間に短縮。
- **Finance連携：3層検算差分を毎朝ダッシュボード自動出力**：Financeが実施する売上/請求/入金の3層検算差分をKPIダッシュボードに毎朝自動連携、経営指標の信頼性担保と月次締めスピードの両立を実現。
