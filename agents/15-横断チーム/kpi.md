...
### 1. Balanced Scorecard (BSC) 全社導入
- **導入内容**: 財務・顧客・業務プロセス・学習と成長 の4視点で戦略マップを描き、非財務指標を財務指標と同格運用。
- **導入基準**: 全社KPI設計で必須採用。
- **期待効果KPI**: 非財務指標可視化 100%／戦略実行率 30%向上
- **連携先**: haruto、hr、marketing

### 2. North Star Metric Framework
- **導入内容**: 全社の最重要指標（NSM）を1つに絞り、そこから逆算してサブKPIを設計。
- **導入基準**: 全社戦略策定で必須。
- **期待効果KPI**: 全社アラインメント 40%向上／リソース配分最適化
- **連携先**: haruto、HARU

### 3. OKR Enterprise Rollout
- **導入内容**: 全部門・全社員でOKR運用。四半期ごとに更新。Objective は野心的、Key Results は測定可能。
- **導入基準**: 全社員で必須運用。
- **期待効果KPI**: 目標達成率 55%→80%／エンゲージメント 30%向上
- **連携先**: hr、pm、全部長

### 4. Real-time KPI Dashboard
- **導入内容**: 全社KPIをリアルタイムダッシュボード化（Looker/Grafana）。異常検知即座にSlackアラート。
- **導入基準**: 全社KPIで必須運用。
- **期待効果KPI**: 異常検知遅延 24時間→即時／対応速度 3倍
- **連携先**: dat、deng、kai

### 5. Executive Reporting Automation
- **導入内容**: 日次/週次/月次経営レポートを完全自動化。データ収集→分析→レポート生成→配信をゼロ工数化。
- **導入基準**: 全経営層向けレポートで必須。
- **期待効果KPI**: レポート作成工数 90%削減／配信リードタイム 80%短縮
- **連携先**: dat、yuto、bo

### 6. Anomaly Detection (Isolation Forest / Prophet)
- **導入内容**: 全KPIに機械学習ベースの異常検知を組み込み。統計的変化点を即座に検知。
- **導入基準**: 全KPIで必須運用。
- **期待効果KPI**: 異常検知精度 95%以上／機会損失 60%削減
- **連携先**: dat、deng

### 7. Cascading KPIs（KPIの階層展開）
- **導入内容**: 全社KPI→部門KPI→個人KPI の3階層で展開。全社員が自分のKPIと全社KPIの関係を理解。
- **導入基準**: 全社員で必須。
- **期待効果KPI**: 全社アラインメント 60%→95%／個人モチベーション 40%向上
- **連携先**: hr、全部長

### 8. Leading vs Lagging Indicators
- **導入内容**: 遅行指標（売上・利益）だけでなく、先行指標（活動量・パイプライン・エンゲージメント）も同格運用。
- **導入基準**: 全KPI設計で必須。
- **期待効果KPI**: 早期対応可能率 40%向上／異常兆候検知 平均45日前
- **連携先**: haruto、sales、marketing

### 9. Financial + Operational KPI Integration
- **導入内容**: 財務KPIと運用KPIを同一ダッシュボードで統合表示。因果関係の可視化。
- **導入基準**: 経営層向けダッシュボードで必須。
- **期待効果KPI**: 経営判断精度 40%向上／部門横断意思決定速度 3倍
- **連携先**: finance、dat、haruto

### 10. Post-Mortem Culture for KPI Misses
- **導入内容**: KPI未達時に必ずBlameless Post-Mortemを実施し、根本原因分析＋改善策を組織資産化。
- **導入基準**: KPI未達で必須実施。
- **期待効果KPI**: 同種KPI未達再発率 60%削減／組織学習速度 3倍
- **連携先**: sora、qa、全部長

---

### 🎯 横断KPIダッシュボードマネージャーの5原則
1. **測れないものは改善できない**
2. **NSMは1つに絞り、そこから逆算**
3. **先行指標なくして予防なし**
4. **異常検知は自動化、対応は即時**
