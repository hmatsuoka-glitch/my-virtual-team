...
### 2026-08-30
- **KPI ツリーの SSOT を dbt + BigQuery で完全コード化し、Git 管理下に置く**：指標定義・親子リンク・ガードレール・stock/flow区分を dbt モデルとしてバージョン管理し、変更は PR レビュー必須。指標の意図せぬ変更・二重定義を構造的にゼロ化し、監査ログを Git 履歴でそのまま提供。
- **Claude 5 系による "アノマリー説明自動生成" を日次ダッシュボードに常設**：異常検知したKPIについて Claude が「原因仮説（過去類似事象・関連指標の同時変動・外部要因の可能性）」を自動生成し、Slack のアラートに添付。原因調査の初動が2時間→10分に短縮、閲覧者の解釈精度も上がる。
- **ダッシュボードの "経営／現場／事務" 三層アクセス制御を Looker Studio + IAM で自動化**：層別初期ビュー（08-18記録）を IAM ロールベースで自動出し分けし、権限外の指標には物理的にアクセス不可。情報統制と使いやすさを両立し、経営専用指標の漏洩リスクをゼロ化。
- **KPI アラートの "疲れ検知" を Bo と協働で四半期モニタ**：既読率・アクション実行率をアラート種別ごとに測定し、閾値のチューニングを四半期で回す。アラート疲れによる本命警告のスルーを構造的に防ぎ、CRITICAL アラートへの初動時間を1時間→15分に短縮。
- **経営会議用の "変化点1枚レポート" を Claude で自動生成**：前月比・前年比の変化率上位10指標を Claude が抽出→「なぜ変化したか」の仮説と「次月の見通し」を自動生成し、経営会議前日に HARU へ配信。会議準備時間が半日→30分に短縮、会議の議論深度も上がる。
- **予測 KPI（Forecast KPI）を Prophet で自動更新し、目標線と並列表示**：目標／実績／予測の3線を同一グラフに載せ、月中の予測着地を Kpi 側で提示。経営の先読み判断が2週間→1週間前倒しでできるようになる。
- **KPI 定義変更の "影響分析" を dbt exposure で自動可視化**：ある指標の定義変更がどのダッシュボード・レポート・ステークホルダーに影響するかを dbt の exposure/lineage で可視化。変更影響レビューの網羅性を人手ゼロで担保し、影響漏れによる報告事故をゼロ化。
- **顧客別 KPI パッケージを Notion テンプレで7社共通化**：クライアント別のダッシュボード・月次レポート・アラート閾値を Notion テンプレ + 変数差替で自動生成する運用に統一。新規クライアント受入時の初期構築が5日→半日に短縮、7社間の品質ばらつきも消える。
