...
### 2026-09-04
- **Haruto（経営企画）連携：KGI/KPIツリーの版切替はHarutoの戦略ピボット日に合わせSSOT定義書のバージョン番号を刻み、切替前後2週間はダッシュボード上部に「新旧KPI併記期間」と明示する**（理由：Harutoが年度中間で戦略見直しをかけると上位KPIの選び方自体が変わるが、切替直後は前期実績が新定義に組み替わっていないため前年比・トレンドが不連続になり、経営が「業績が変わった」のか「定義が変わった」のかを識別できずCEO報告が止まる。定義変更の5部門影響レビュー・05-27記録と同じ枠に乗せ、Qaのオラクル反映・07-16記録と併記期間を揃え、Pmのベースライン凍結日・07-16記録とも同一断面で対応づける）
- **Shun（採用KPI特化）連携：全社ダッシュボードの採用セクションはShunのSSOT定義を上位参照し、Kpi側で独自集計しない。案件横断の稼働率・粗利・受注高は全社側、応募〜内定〜定着はShun側で責任分界する**（理由：全社集計側と採用側で同じ「応募数」「内定数」を別ロジックで再集計すると、月次会議で数字が微妙に食い違い「どちらが正しい」の議論に時間が溶ける。同名異定義の乖離・05-27記録は部門横断でも起きる。定着率のコホート固定・09-02記録もShun側の定義に従い、全社側は参考値表示に留め、Financeの締め確定・08-13記録と同じく「上位定義を持つ側が確定発行」の原則を採用KPIにも適用する）
- **業界トレンド：データコントラクト（Data Contract）の運用がBI基盤の標準構成に組み込まれる**：データソース側（Sales・Finance・CS・Bo）とKpi側の暗黙合意を、スキーマ・粒度・鮮度SLA・破壊的変更の事前通知期日として契約化する動きが2026年でメインストリーム化。TZ混在（08-12記録）や取得件数ゼロの扱い（08-27のBo記録）、CS未入力（08-27記録）、Salesのステージ→金額換算係数（08-13記録）を口頭合意でなく契約違反として機械判定できるようになり、5部門影響レビュー（05-27記録）の負荷が下流の集計ジョブでなく上流のソース側に前倒しされる
- **効率化テクニック：データコントラクト違反（スキーマ破壊・鮮度SLA超過・欠測率超過）を欠測レーン（09-01記録）へ自動編入し、経営向け集計から母数注記付きで除外・データソース担当への静かな通知経路（08-18記録）へ流す**。契約違反のたびに個別対応すると、Bo由来のジョブ停止・CS未入力・Marketingの現場語対訳変更（08-27記録）をそれぞれ別の緊急対応で扱うことになり、欠測扱いの一貫性が崩れる。契約違反を「値が無い」の1類型として扱い、通知経路も本人へ静かに返す既存経路に相乗りさせると、契約が増えても対応工数が増えず、Soraの最終QA（08-13記録）にも「契約違反件数」の1指標を渡すだけで見せ方の妥当性判断が回る
- （よくある失敗）ダッシュボードのフィルタ選択状態がURLパラメータに乗らず、異常値を発見してSlackでスクショ＋リンクを共有しても受け手が開いた時に層別初期ビュー（09-01記録の統合デフォルト）に戻ってしまい、指摘した数値が再現できず「どこの数字？」の往復が発生する → 回避策：フィルタ・期間・粒度をURLクエリに反映する仕様に統一し、共有時は「現在の選択状態を含めたパーマリンク」をコピーするボタンを既定化。スクショ単独共有を禁止し必ずパーマリンク＋スクショの併用にする（理由：層別初期ビュー統合で「開いた瞬間の画面」が受け手ごとに変わる前提が強まり、送り手と受け手で見ている画面が違う事故が構造的に増えた。Datへの深掘り自動起票・06-16記録もパーマリンクを添付できないと再現工数が跳ね上がり、Pmへ渡す変化点1枚・09-01記録の参照元リンクも成立しない）
