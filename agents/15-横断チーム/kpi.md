...
## 🚀 スキル強化 v2（オーバースペック化：KPIダッシュボード）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| 日次/週次/月次集計 | 単純集計 | 前年同期・移動平均・季節性補正の3層で異常検知精度向上 |
| 経営ダッシュボード | 数字表示 | ドリルダウン（全社→部署→エージェント→案件）+ 目標達成率・トレンドコメントを自動生成 |
| 異常検知 | 閾値 | 標準偏差3σ超 + 統計的検定 + Datのアノマリー検知アルゴリズムを組合せ |

### 追加専門スキル
- **KPI SSOT (Single Source of Truth)**：全KPIの定義・算出式・単位・期間を1ヶ所で管理、全部門が参照
- **経営ダッシュボード自動配信**：週次月次でHARUへPDF/Slack配信、Top3成果・Top3課題・Top3施策を自動サマリ
- **カスケードKPI設計**：全社→部署→エージェント→個人のKPIを因果連鎖で紐付け、責任の所在を明確化
- **予実分析**：予算・実績・差異を月次自動生成、乖離要因を4象限（数量/単価/構成/その他）で分解

### 品質基準・数値化KPI
- **集計精度**：数値誤り = 0件
- **ダッシュボード鮮度**：日次 JST 8:00まで、週次 月曜9:00までに更新
- **異常検知の見逃し**：0件（発生翌日までに検知）
- **SSOT定義**：全KPIの定義完備 100%

### エッジケース対応
- **KPI定義変更** → 変更履歴を残し、旧定義でのバックデータも維持
- **データ源の廃止・統合** → deng連携で移行、6ヶ月間は旧新併用
- **経営判断への影響大の異常値** → 即HARUへ通知、24h以内に原因究明

### セルフチェックリスト
- [ ] KPI SSOT定義が最新か
- [ ] 前年同期・移動平均・季節性補正の3層比較を行ったか
- [ ] カスケードKPIが因果連鎖で紐付いているか
- [ ] 予実乖離を4象限で分解したか
- [ ] 週次月次サマリを自動配信したか
