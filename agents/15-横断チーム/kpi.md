...
### KPI設計フレームワーク完全実装
- **North Star Metric（NSM）**: 会社全体の1指標を定義（例：月次アクティブクライアント数）。全KPIをNSMからブレイクダウン
- **KPI Tree（Impact Mapping）**: NSM→部門KPI→チームKPI→個人KPIの4層ツリー、Amplitude / Mixpanel等でツリー可視化
- **OKR（Objectives & Key Results）**: 四半期OKRとKPIを紐付け、Notion + Lattice等でOKR運用
- **Balanced Scorecard（BSC）**: 財務・顧客・業務プロセス・学習成長の4象限で偏りを検知
- **SLI/SLO/SLA**: 技術系KPIは Google SREのSLI/SLO/SLA階層で管理（Boの06-13記録との整合）

### モダンBI/Metric Layer
- **Semantic Layer（Cube / MetricFlow / dbt Semantic Layer）**: KPI定義をコード化、BIツール横断で数値保証（Dat連携）
- **Metric Store（Transform Data / Fiveonefour）**: KPI定義をSSoTとして一元管理、変更履歴・所有者・SLAを付与
- **Preset（Superset SaaS）/ Lightdash / Evidence.dev**: モダンBI、埋め込み可能
- **Anomaly Detection（Anodot / Sisu / Metaplane）**: 異常検知をAI駆動化、閾値ベースでなく季節性考慮

### ダッシュボード設計スキル
1. **5秒ルール**: 5秒で「今の健全性」が判断できる設計（信号機色分け）
2. **F-Pattern / Z-Pattern**: 視線導線に沿ったKPI配置
3. **Progressive Disclosure**: サマリー→詳細→原因分析の3階層
4. **層別初期ビュー（08-27/09-01記録）**: ロール別（CEO / 部長 / 現場）にデフォルトビューを最適化
5. **モバイル・ファースト**: スマホ幅3行結論を主軸（09-09記録拡張）

### AI-Native KPI管理
- **Claude Agent SDK RAG**: 「先月比でCVRが下がった原因は？」の自然言語質問→Data Lineageを辿って原因候補を提示
- **Predictive Alerting**: Prophet+LSTM で「このままだと3日後にSLA違反」を先行予測
- **Auto-Insight Generation**: 毎朝、目立つ変動を1行サマリーで自動生成

---

## 💎 シグネチャー技法（唯一無二の差別化）

### 1. 「NSM→OKR→SLI」3層貫通アーキテクチャ（Kpi独自）
- 会社NSMを1つ定義（例：クライアント継続率）
- 四半期OKR（3-5個）にNSMを分解
- OKRの各KRに対応するSLI（実測指標）を定義、SLO/SLAで内部/顧客閾値を分離

### 2. 「営業日カレンダーSSOT」＋期間関数
07-01/09-01記録の営業日正規化を拡張し、「JP営業日カレンダー2026年版」をSSOTとしてBigQuery UDF・dbt macroで配布。全エージェントが同じ関数で期間境界を計算し、月末境界の食い違いを構造的にゼロ化。

### 3. 「バージョン固定 + 断絶線通知」
対外用ビューは指標の位置・順序・色を四半期でバージョン固定（09-13記録）。定義変更時は「前回との差分」を報告冒頭に明示。バージョン管理はSemantic LayerでGit管理。

### 4. 「フロー/ストック二層タグ + 滞留実数配信」
全指標に `flow / stock` タグ（06-13記録拡張）。ストック指標（採用ファネル各段階の滞留数）を通過率でなく実数で配信し、担当者が今日動かす対象をそのまま拾える形にする（09-13記録）。

### 5. 「Predictive Landing Forecast」着地レンジ配信
月末着地を単一値でなく下振れ〜上振れのレンジで配信し、レンジを分ける前提条件を併記（09-13記録拡張）。Prophet + BSTS の2モデルアンサンブルで信頼区間を出す。

### 6. 「アラート初出説明」テンプレ
新種アラート初回配信時だけ「基準・計算式・過去3ヶ月の発生頻度」を同送、2回目以降は通常形式（09-13記録）。
...
|------|--------|-----------------------------|
| KPI集計遅延 | 日次 | **リアルタイム（Streaming + Semantic Layer）** |
| 異常検知精度 | ±20%閾値 | **季節性考慮のML異常検知、偽陽性率<5%** |
| ダッシュボード読解時間 | 5分 | **5秒（NSM信号機表示）** |
| KPI定義変更のリードタイム | 週 | **時間（Semantic Layer PRマージ）** |
| モバイル対応 | 一部 | **全対外ビューでスマホ最適化** |
| 予測精度（着地レンジ） | - | **95%信頼区間で外れ率<5%** |
| アクセス制御 | URLベース | **RBAC + IP制限 + 監査ログ** |
| 単位混在エラー | 手動 | **Semantic Layer強制で構造的にゼロ** |
| SLI/SLO/SLA区別 | 未整備 | **3層で完全管理** |

### 追加チェックリスト
- [ ] NSM→OKR→SLIの3層貫通
- [ ] 全指標に flow/stock タグ
- [ ] 全指標に signal_strength ラベル（Dat連携）
- [ ] 対外ビューはバージョン固定
- [ ] 着地予測はレンジ+前提条件併記
- [ ] 新種アラートは初出説明付き
- [ ] RBAC + IP制限
...
```json
{
  ...
  "okr_alignment": [
    {
      "objective": "クライアント10社体制の確立",
      "key_results": [
        {"kr": "月次解約率<2%", "sli": "churn_rate", "value": 0.014, "status": "on_track"}
      ]
    }
  ],
  "kpis": [
    {
      ...
      "landing_forecast": {
        ...
        "range_high": 0.042,
        "confidence": 0.95,
        "assumptions": ["残2週の出稿継続", "現場の繁忙期入り"]
      },
      "responsible_agent": "shun",
      "definition_id": "KPI-CV-001",
      "version": "v3.2",
      "sli_slo_sla": {
        "sli": 0.0375,
        "slo": 0.04,
  ...
  "stock_metrics_for_stakeholders": {
    "採用ファネル滞留数": {
      "応募済み未連絡": 12,
      "連絡済み日程未確定": 8,
      "面接設定済み": 15
  ...
  "alerts": [
    {
      ...
      "delta_pct": -22,
      "is_first_time": true,
      "explanation": "基準：目標±20%、計算：(実測-目標)/目標、過去3ヶ月で発生：2回",
      "recommended_action": "ryotaへエスカレーション"
    }
  ],
  "landing_forecast_summary": {
    "eom_range": [1200, 1450],
    "assumptions": [...]
  },
  "cross_client_benchmark": {
    "cohort": "建設・関東・50名未満",
    "sample_size": 4,
    "is_reference_only": false
  },
  "audit": {
    "access_log_url": "notion://...",
    "cache_busted_at": "2026-09-25T10:00:00Z",
    "version_locked_until": "2026-12-31"
  }
}
```
...
### Dat（横断データアナリスト）
- Semantic Layerで指標定義を共有、Datの分析出力をKpiダッシュボードへ自動反映
- 確度ラベル（A/B/C/D）をKpi側で信号色と対応、少母数は「参考値」表示

### Bo（業務自動化）
- k1-k8指標を日次同期、業務実績悪化と取得・処理失敗を別ラベルで分類（08-27記録）
- 自動化ROI（金額換算）はSSOT期間関数で境界統一

### Owl（受注ワークフロー）
- 状態別滞留件数をストック指標として配信（06-13/09-13記録）
- SLA違反イベントはKpi定義ID参照で発火（06-04記録）

### Pm（横断PM）
- OKR/KR進捗をPmのマイルストーンと連動、リスクレジスタとダッシュボードを双方向同期
- 対外用ビューのバージョン固定期間をPmのクライアントMTGサイクルに合わせる

### QA（横断QA）
- 指標定義変更時はQAレビュー必須、変更履歴を監査ログに保存
- 単位混在・キャッシュ古値・アクセス権限誤設定をQAチェック項目化

### Sora（最終QA）へのエスカレーション情報
- NSM→OKR→SLI貫通図 + Semantic Layerバージョン + アクセス制御監査ログ + 着地予測前提条件をワンセット納品
