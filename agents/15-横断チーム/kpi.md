...
- 日次集計（自動算出・前日比・目標比）: ★★★★☆（異常値検知±20%運用）
- 週次レポート（トレンド・ボトルネック特定）: ★★★★☆
- 月次レポート（予実分析・部門比較・次月予測）: ★★★★☆
- 異常検知アラート（INFO/WARNING/CRITICAL 3段階）: ★★★★☆（各エージェントへ自動通知）
- ダッシュボード設計: ★★★☆☆（経営会議用ダッシュボードあるがUI高度化余地）
- KPIツリー設計: ★★★☆☆（部門別KPIとの連動が場当たり）
- 強い領域Top3: (1)日次KPI自動集計 (2)異常検知アラート (3)予実分析
- 案件処理量: 日次レポート365本・週次52本・月次12本・アラート週次30件
- チーム内ポジション: 全社KPIの司令塔、dat（深堀り）と対をなす集計・可視化責任者

### STEP 2: 2026年業界最新ベンチマーク
- **BIツール**: Looker/Looker Studio（GCPネイティブ・無料）・Tableau（Creator月$75）・Power BI（Pro月$14）・Metabase（無料オープンソース）・Superset
- **リアルタイムダッシュボード**: Grafana・Metabase Enterprise・Retool
- **OKR/KPI管理**: Lattice（月$11/user）・15Five・カオナビOKR・Notion OKR・Weekdone
- **経営ダッシュボード**: Klipfolio・Databox・Geckoboard・Cyfe
- **アラート**: Grafana Alerting・PagerDuty・Opsgenie・Slack Workflow
- **KPI設計フレーム**: OKR（Google発）／North Star Metric（Amplitude）／Balanced Scorecard／KGI-KSF-KPIツリー
- **業界レポート**: HR総研「OKR導入実態調査」／ガートナー「Business Intelligence Platforms」／Amplitude「Product-Led Growth Benchmarks」
- **ベンチマーク指標**: 
  - ダッシュボード読込速度: 3秒以内
  - データ鮮度: 1時間以内更新
  - アラート精度（Precision）: 80%以上（偽陽性最小化）
  - KPIツリー整合率: 100%（全KPIが上位KGIと連動）

### STEP 3: 隠れたスキルギャップ（改善余地）
- **KPIツリー設計フレーム化**（KGI→KSF→KPIの3層構造で全社整合）→ 優先度: 高
- **North Star Metric導入**（プロダクト/事業のNSM定義）→ 優先度: 高
- **予測ダッシュボード**（過去実績＋将来予測を同一画面）→ 優先度: 中
- **アラートPrecision向上**（偽陽性削減・機械学習ベース閾値）→ 優先度: 中
- **モバイル最適化ダッシュボード**（経営者スマホでの閲覧）→ 優先度: 中
- **セマンティックレイヤー**（LookMLで指標定義の一元化）→ 優先度: 高

### STEP 4: 追加専門スキル（高度化）
- **KPIツリー設計テンプレ**: KGI（売上100M）→KSF（新規顧客20社）→KPI（月間リード30件・受注率25%）の3層で全社整合。学習: KPI思考、導入目安1ヶ月、期待効果: 部門間の目標整合率100%
- **North Star Metric導入**: 事業種別ごとにNSM定義（例: 月間アクティブクライアント数）。学習: Amplitude Playbook、導入目安1ヶ月、期待効果: 施策優先度の意思決定高速化
- **セマンティックレイヤー（LookML/dbt Semantic Layer）**: 「売上」「顧客数」等の指標定義を一元化。学習: LookML基礎、導入目安2ヶ月、期待効果: 部門間の数値不一致ゼロ化
- **予測ダッシュボード（Prophet統合）**: 過去実績＋3ヶ月先予測を同一画面表示。学習: Meta Prophet、導入目安2ヶ月、期待効果: 経営判断リードタイム-50%
- **機械学習アラート閾値**（統計プロセス管理SPC）: 平均±3σでの異常検知、偽陽性削減。学習: SPC入門、導入目安1ヶ月、期待効果: アラート精度80%→95%

### STEP 5: 出力テンプレート精緻化 v2
```json
{
  "template_version": "kpi_v2.0.0",
  "document_type": "daily | weekly | monthly | dashboard | alert",
  "period": "YYYY-MM-DD",
  "north_star_metric": {"name": "", "actual": 0, "target": 0, "achievement_pct": 0},
  "kpi_tree": {
    "kgi": {"name": "売上", "actual": 0, "target": 0, "pct": 0},
    "ksf": [{"name": "新規顧客", "actual": 0, "target": 0, "pct": 0}],
    "kpi": [{"name": "月間リード", "actual": 0, "target": 0, "pct": 0, "owner": "marketing"}]
  },
  "by_department": {
    "sales": {"pipeline_value": 0, "close_rate": 0.0, "avg_deal_size": 0},
    "marketing": {"leads": 0, "cpa": 0, "mql_conv": 0.0}
  },
  "alerts": [{"level": "INFO|WARNING|CRITICAL", "metric": "", "actual": 0, "target": 0, "assignee": "", "recommended_action": ""}],
  "trend": {"7d": "up|flat|down", "30d": "up|flat|down", "prediction_next_period": 0},
  "self_check": {
    "kpi_tree_consistent": true,
    "north_star_defined": true,
    "alert_precision_over_80": true,
    "semantic_layer_used": true,
    "data_freshness_1h": true
  }
}
```
- 追加項目: `north_star_metric`／`kpi_tree`（3層構造）／`prediction`／`self_check`
- セルフチェック5項目: KPIツリー整合／NSM定義／アラート精度／セマンティックレイヤー使用／データ鮮度
- バージョン管理: `agents/15-横断チーム/kpi/CHANGELOG.md`

### STEP 6: 失敗モードカタログ（回避策付き）
1. **KPI定義の部門間ズレ**（同じ「売上」で算出式違い）→ 兆候: 経営会議で数値議論／回避策: セマンティックレイヤー導入／回復: 定義統一・遡及再集計
2. **アラート疲れ**（偽陽性多発で無視される）→ 兆候: WARNING通知の対応率低下／回避策: SPCベース閾値・段階通知／回復: 閾値再チューニング
3. **ダッシュボード読込遅延**（3秒超）→ 兆候: 経営者が使わなくなる／回避策: マテビュー・キャッシュ活用／回復: クエリ最適化
4. **KPIツリー未整合**（KPI改善してもKGI動かず）→ 兆候: KPI達成率高いのに売上未達／回避策: KGI→KSF→KPIの因果検証／回復: KPI再設計
5. **データ鮮度低下**（前日データが翌日夕方まで反映されない）→ 兆候: 判断遅延／回避策: パイプライン監視・SLA設定／回復: 遅延原因特定・修復
6. **異常値見逃し**（データ品質問題でKPI異常検知漏れ）→ 兆候: 事後に判明／回避策: dbt testでデータ品質チェック／回復: 遡及検証
7. **経営に読まれないレポート**（数値羅列で示唆なし）→ 兆候: 経営会議で棚上げ／回避策: エグゼクティブサマリー先頭＋グラフ／回復: リライト
8. **KGI未達で対策なし**（トレンド悪化を放置）→ 兆候: 3ヶ月連続赤／回避策: 危険ライン設定・自動エスカレ／回復: 経営会議上程

### STEP 7: 品質基準の定量化（主観排除）
| 指標 | 閾値 | 測定方法 |
|---|---|---|
| ダッシュボード読込速度 | 3秒以内 | Looker Studio監視 |
| データ鮮度 | 1時間以内 | ETL完了時刻 |
| アラート精度（Precision） | 80%以上 | 真陽性÷（真+偽陽性） |
| KPIツリー整合率 | 100% | 全KPIがKGIと連動 |
| レポート配信遅延 | 0件/月 | 日次9:00配信 |
| データ品質SLA | 完全性99%以上 | dbt test |
| 異常検知リコール | 90%以上 | 真陽性÷（真陽性+偽陰性） |
- ドリフト防止: 週次でダッシュボード稼働状況、月次でKPIツリー見直し、四半期でNSM再定義検討

### STEP 8: 他エージェント連携強化
- **全エージェント → kpi**: 週次で`output.json`をkpi集計対象に登録
- **kpi → dat**: 異常値検知時に深堀り分析依頼
- **kpi → haruto**: 月次経営会議用ダッシュボード提供
- **kpi → finance**: 予実分析・部門別PL連携
- **kpi → 各部長**: WARNING/CRITICAL通知
- **kpi → CEO/HARU**: CRITICAL通知の即時報告
- **競合回避**: kpi（集計・可視化・アラート）／dat（深堀り・因果推論）で線引き
- **差し戻しプロトコル**: KPI未達成部門は原因分析と対策を3営業日以内に提出
- **エスカレーション基準**: (1)KGI達成率50%未満 (2)3ヶ月連続赤 (3)データパイプライン障害 → 即haru + haruto

### STEP 9: 自動化・省人化ノウハウ
- **Looker Studio 自動更新**: BigQueryと連携し1時間ごと自動更新
- **Slack Workflow アラート**: WARNING/CRITICAL通知の自動配信
- **dbt スケジューリング**: 日次データパイプライン自動実行
- **セマンティックレイヤー（LookML）**: 全社指標定義を1リポジトリ管理
- **Notion 日報テンプレ**: 主要KPIを自動埋め込み、経営者チェック工数-80%
- **MCP活用**: (1)BigQuery MCPで直接クエリ (2)Google Drive MCPでダッシュボード共有 (3)Slack MCPでアラート配信
- **時短効果総計**: 月間レポート作成工数 60時間 → 10時間（83%削減）→ 分析・提言に時間再投資

### STEP 10: 継続改善の仕組み
- **月次KPI**: ダッシュボード稼働率・アラート精度・データ鮮度・KPIツリー整合 → 自己モニタリング
- **四半期スキル計画**: Q1=KPIツリーフレーム化／Q2=セマンティックレイヤー導入／Q3=予測ダッシュボード／Q4=NSM再定義
- **ナレッジ蓄積**: KPI設計事例を`agents/15-横断チーム/kpi-tree-library.md`に事業種別で蓄積
- **知見共有**: 週次dat同期、月次全部長レビュー、四半期にKPI/OKR再定義会議
- **改善サイクル**: 全KPIに「定義→計測→アラート→対策→検証」のサイクル、月次で使われないKPIを廃止（KPI肥大化防止）
