...
### 2026-09-04
- **2026年後半トレンド『Linear AI のクロスプロジェクト依存グラフ自動生成』を横断クリティカルパス検知に接続する**：Linear が同一メンバー・同一Issueラベルを跨いだ依存を自動抽出しグラフ表示する機能を実装（AI Triage・05-25記録の延長）。従来メンバー軸ガント1枚（07-01記録）に手で寄せていた横断クリティカルパスをLinearから直接取り込めるため、Kai（システム開発PM）のassignment（08-13記録）とRyotaのクライアント案件WBSを同一グラフに載せ、7社案件をまたぐキーメンバーの律速地点を自動検出する。ただしAIの再スケジュール提案は非律速・単一案件内に限定（08-05記録）し、横断リソースに触る再配置は人手承認ゲートを残す
- **2026年トレンド『Notion 3.0 のプロジェクトDB刷新（関係プロパティ強化＋AIブロック）』を正式記録の正本に据える**：LINE要点3行＋Notion正式記録の2形式運用（08-18/09-01記録）で、宮村建設・翔星建設のような建設クライアントの正本側を Notion 3.0 のプロジェクトDBに集約し、WBSタスクとハンドオフ4点セット（06-12記録）を関係プロパティで相互参照させる。Qa（横断QAレビュアー）が受入基準スニペット（07-16記録）を関係先から自動引用でき、ゲート条件の曖昧さ由来の差し戻しが構造的に減る。AIブロックには変更履歴とベースライン凍結（07-03記録）の差分を要約させ、リプランの意思決定跡をNotion内で追える状態にする
- **2026年トレンド『プロジェクトパイプライン分析』を受注前段階から Dat と共同で回す**：Ryotaが握る受注前案件をパイプラインステージ別（初回接触→提案→内示→受注）に加重確率で加重し、Datに「加重期待稼働（案件工数×受注確率）」を四半期先まで算出してもらう。キャパシティ・プランニング（08-03記録）の分母は現稼働＋確定案件だけでなく加重期待稼働を含め、Ryotaの提案タイミング調整とPmの受注可否判断（09-02記録の外注リードタイム込み）を同一ダッシュボードで握る。DID純効果の考え方（Datの07-01記録）で「追い風で確度が上がっただけの案件」を過大評価しない
- **2026年トレンド『AI 依存関係サジェスト』は暗黙の依存を拾わせつつ、FS/SS/FF/SF 判別は Kai/Nao と人手で確定する**：Linear/Asana系AIが過去案件のタスクパターンから「このタスクの前に◯◯があったはず」と暗黙の依存を提案するようになった。08-12記録の依存種別取り違え（全FS化・不当な並行化）はAI提案でも起き得るため、システム開発案件は Kai/Nao（09-システム開発部）の設計ゲートと突合し、部分完了で着手可能なSS/リードタイム付き並行を人手で確定する。制作案件は 07-LP部・10-資料作成部の各部長と部署間ハンドオフ4点セット（06-12記録）の受領確認者（RACIのA）を1人に絞る作業に AI 提案をそのまま乗せない
- **部署間ハンドオフSLAを Kpi の全社共通指標として定義し、Sora の最終QA前に違反地点を自動列挙させる**：ハンドオフ4点セット（06-12記録）の「受領期限」を案件ごとの約束から Kpi の全社SLA指標へ格上げし、部署ペア別（例：制作→Qa・Qa→Sora・Ryota→Pm）に受領遅延分数・件数を週次でKpiダッシュボードに表示。金曜納品前の滞留（07-16記録）が特定ペアに偏る構造を可視化し、Sora の最終QA着手時にはSLA違反地点の一覧を先出しして「体制起因の遅延」と「案件個別の遅延」を切り分ける。Kpiの週次稼働率（06-11記録）とSLA違反件数をセットで見ると、稼働率は高いのに違反が減らない部署＝プロセス設計側の課題として HARU 判断に上げられる
