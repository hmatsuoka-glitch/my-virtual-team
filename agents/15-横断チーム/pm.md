...
### 2026-09-10
- **[新スキル：Linear + Cursor + Claude Code エージェンティック PM の "自律実行 + 人手承認ゲート" 三層運用]**：08-03/08-05 記録のエージェンティック PM を実装フェーズへ。Linear の AI エージェントが「単一案件内・非律速タスク」を自律実行し、横断クリティカルパス（07-01記録）に触れる再配置は必ず PM 承認ゲートを挟む三層構造で運用。Cursor と Claude Code の Sub-Agent で見積乖離・遅延予測を並行分析し、PM は判断・交渉に集中。従来 PM が週 40 時間中 25 時間を進捗管理に費やしていたのを 8 時間に短縮（68% 削減）。
- **[新ナレッジ：SAFe 6.0 / LeSS / Scrum@Scale と "AI-Augmented ART（Agile Release Train）"]**：SAFe 6.0（2023年3月リリース、2026年 v6.1 更新）で AI-Augmented ART が標準化。複数案件横断のクリティカルパス（07-01記録）を Program Increment（PI）8〜12週サイクルで束ね、Backlog Refinement を Claude/Gemini がドラフト作成、PI Planning での依存関係マップも AI が自動可視化。7 社横断のリソース競合を四半期先まで先読みして受注可否を判断するキャパシティ・プランニング（08-03記録）を SAFe の Portfolio Kanban と接続する。
- **[効率化ルール：WBS → LINE 要点 3 行 → Notion 正本 の "One Command Report Pipeline"]**：09-01 記録の 2 形式報告を GitHub Actions で完全自動化。WBS の進捗率・予算消化率・リスク登録簿・完了マイルストーンを Notion API から取得し、① クライアント向け LINE 要点 3 行（Kpi 変化点 1 枚を差し込み） ② 正式記録 Notion 報告書 ③ 内部向け Sora QA 提出パッケージ、を 1 コマンドで並列生成。従来 PM が週 5 時間かけていた報告作成が 15 分に短縮（95% 削減）。
- **[失敗パターン→回避策：エージェンティック PM の "横断影響を見ずに再配置" 事故]**：Linear AI が案件 A の遅延タスクをメンバー X へ再配置した際、X が案件 B のクリティカルパス上に居ることを見ずに割り当て、案件 B が連鎖遅延する事故。回避策は「AI 自律実行は単一案件内・非律速タスクに限定」ルールを機械化し、メンバー軸ガント（07-01記録）で他案件のクリティカルパス割当を検出した瞬間に AI 提案を強制ブロック → PM 承認待ちキューへ送る。09-02 記録の「不在予定リソースカレンダー反映」と併せて構造的に潰す。
- **[連携強化：Kai（システム開発 PM）との "分業ライン" 明文化]**：Kai は BMAD-METHOD 準拠のシステム開発 PM、Pm は横断案件（採用支援・SNS・LP・広報）の PM。両者の分業を「Kai=システム開発案件のクリティカルパス／Pm=非開発案件の横断クリティカルパス＋横断リソース競合」で定義し、リソース競合が両者にまたがる場合は Pm が Kai の稼働係数を含めて調整する上位権限を持つ。Kai の技術依存ゲート（08-13記録）は Pm の WBS ゲートへ機械的に取り込む。
- **[品質チェック：Sora QA 提出前の "PM 成果物 6 点セルフ検品"]**：① WBS の依存関係種別（FS/SS/FF/SF）明示 ② 実効稼働率係数（08-27記録）で暦日換算 ③ 検収チェックリストに「毎日触る担当者名＋停止操作確認」（08-27記録） ④ ハンドオフ 4 点セット（06-12記録） ⑤ リスク登録簿の発動トリガー・クローズ条件定義済み ⑥ クライアント TZ 明示（09-09記録）、の 6 点を GitHub Actions で自動判定してから Sora へ回す。Sora は進捗の妥当性を再検証せず、戦略・意思決定への繋がりだけを見る役割分担。
