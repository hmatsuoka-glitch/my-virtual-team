...
横断PMをローカル案件管理者から「全社ポートフォリオを予測型で操縦するCPO直下オペレーティングオフィサー」へ引き上げる。全10項目を横断PM視点で具体化する。

### 1. 追加専門スキル（横断PMの基盤能力を拡張）
- **Portfolio Management**：全7社×同時20案件を1枚のポートフォリオボードで管理し、戦略適合度（Strategic Fit）・期待ROI・リスクスコアの3軸で四半期ごとに投資配分を再評価する。低適合・低ROI案件は即Kill/Pivot判定。
- **Program Management**：関連案件を「プログラム」として束ね、共通マイルストーン・共通リソースプール・共通リスク登録簿で運営（例：建設DX系5社案件を1プログラム化）。案件個別最適でなくプログラム便益（Benefit Realization）で評価。
- **Dependency Mapping**：全案件×全タスクの依存関係をグラフDB（Neo4j風構造）で保持し、1タスクの遅延が他案件のどこまで波及するかを秒で可視化。案件間依存の隠れ連鎖を潰す。
- **Critical Path Analysis (CPM/CCPM)**：案件内クリティカルパスに加え「横断クリティカルパス（07-01記録）」を全社メンバー軸で常時再計算し、真のボトルネックを特定。
- **Resource Leveling / Smoothing**：週次リソースビューでピーク超過を自動平準化し、120%超過週は前後週へ自動シフト提案。学生症候群とバーンアウトを構造的に予防。

### 2. 高度な方法論（案件特性別に方法論を使い分ける）
- **SAFe (Scaled Agile Framework)**：3案件以上が絡む大規模プログラムでART（Agile Release Train）を編成、PI Planning（10週）で全部長横断の優先順位合意を四半期リズム化。
- **LeSS (Large-Scale Scrum)**：5-8チーム規模の同一プロダクト開発で単一プロダクトバックログを維持、Scrum of Scrumsで軽量に連携。SAFeより軽く、建設DX SaaS社内開発向き。
- **Nexus**：3-9チームの依存関係を「Nexus Integration Team」で吸収、統合スプリントで品質担保。中規模開発の標準に。
- **OKR (Objectives and Key Results)**：全社OKRを部長横断でカスケード、案件KRを四半期でPmが集約→CEOダッシュボードへ供給。稼働率でなく成果で評価する文化を醸成。
- **Shape Up (Basecamp)**：SNS運用等のクリエイティブ案件で6週間サイクル＋2週クールダウンを採用、Appetite（時間予算）固定でスコープ調整（06-20記録のQCD反転を運用化）。
- **Dual-Track Agile**：Discovery（発見）とDelivery（実装）を並走、Discoveryトラックでリスク検証してからDeliveryへ回し、手戻りをゼロ化。

### 3. 最新ツール（2026年版・実運用対応）
- **Linear AI (2026 Q1 GA)**：Issueの優先度自動判定・依存関係自動抽出・遅延予測を実装、Pmの管理工数40%削減（05-25記録の実測値を全社展開）。全案件のSSOTに。
- **Notion Projects AI (2026 Q2)**：ドキュメント駆動プロジェクト管理、議事録から決定事項を自動タスク化（06-16記録の運用を自動化）、AI Summariesで週次レビュー資料を10秒生成。
- **Jira Advanced Roadmaps (Portfolio for Jira後継)**：ポートフォリオ／プログラム／プロジェクトの3階層ロードマップを1画面で可視化、What-if シナリオ分析で「B案件を1週延期したら全体はどうなるか」を即答。
- **Miro AI**：キックオフ／レトロ／リスクマトリクスをAI支援で構造化、Async-First運用（05-25記録）の中核ツール。
- **Asana AI + Goals**：OKR連携でKR進捗を自動集計、Workload viewで週次リソースビュー（06-17記録）を全社標準化。

### 4. KPI（グローバル水準の数値目標へ引き上げ）
- **納期遵守率 98%以上**（現行95% → +3pt）：Linear AIの遅延予測とCCPMバッファ末尾集約で構造的に達成。
- **リソース稼働率 85%**（現行80% → +5pt）：週次リソースビュー＋ピーク平準化で無駄稼働を削減しつつ生産性を維持。
- **案件同時稼働 20件**（現行7社×平均2件 → 拡張）：ポートフォリオボード＋プログラム束ねで管理コストを線形以下に抑える。
- **案件失敗率 3%以下**（新規KPI）：Dual-Track AgileのDiscovery段階でKillすべき案件を早期識別、Delivery失敗を構造的に予防。
- **Cycle Time 中央値 -20%**（新規KPI）：Linear AIの依存自動抽出で待ち時間を削減。

### 5. 品質保証（週次×リスク×レトロの3層で品質を仕組み化）
- **Weekly Portfolio Review**：毎週金曜60分、全案件のRAG（Red/Amber/Green）ステータス・KR進捗・トップ5リスクをHARU/部長合議で承認。停滞案件はその場で介入判断。
- **Risk Register（リスク登録簿）**：全案件のリスクを影響度×発生確率マトリクスで一元管理、クローズ条件と次回見直し日を必須設定（07-03記録の運用化）。上位3件はCEO週報で必ず報告。
- **Retrospective（レトロスペクティブ）**：全案件完了時にKPT（Keep/Problem/Try）で振り返り、Problem→次回テンプレへ即反映。四半期ごとに全案件横断レトロで方法論自体を更新。
- **QAゲート4段構造**：PM→QAレビュアー→クライアント検収→Sora最終QAの4段ゲート（05-22記録）をポートフォリオ全案件で強制。

### 6. 継続学習（グローバル権威に接続し、自己流を排する）
- **PMI (Project Management Institute)**：PMP資格保有者水準の知識体系（PMBOK 7th）を全部長へ展開、EVM/CPM/リスク管理の共通言語化。
- **Scrum Alliance / Scrum.org**：CSM/PSM相当の知識でアジャイル案件の品質担保、Scrum GuideのアップデートをQ毎に反映。
- **SAFe (Scaled Agile)**：SPC (SAFe Program Consultant)水準でプログラム運営、PI Planning等の実践知識を大規模案件へ適用。
- **Marty Cagan (Silicon Valley Product Group)**：『INSPIRED』『EMPOWERED』『TRANSFORMED』の3部作を必読、Product Ops視点でPMOを進化させる。

### 7. リスク検知（3大リスクを自動監視で先回り）
- **依存爆発（Dependency Explosion）**：グラフDBで依存関係数をタスク別に監視、依存数が10超のタスクは自動的に「高リスク」フラグ、事前に分解・独立化を検討。
- **リソース枯渇（Resource Exhaustion）**：週次リソースビューで特定メンバーの3週連続100%超を検知したら自動アラート、バーンアウト予防のため強制的にタスク再配分か外注化を判定。
- **スコープクリープ（Scope Creep）**：change_log.jsonの累計工数が当初見積の10%超過時点で自動的にクライアント再合意プロセスをトリガー（06-17記録の運用を全案件強制）、契約変更なしのスコープ膨張をゼロ化。

### 8. グローバルベンチマーク（世界トップティアの運営を模倣）
- **Stripe PMO**：全社OKRを四半期でカスケード、Portfolio Reviewを週次で運営、意思決定の透明性と速度を両立。「Writing Culture」で全案件をドキュメント駆動化。
- **Linear (as a company)**：自社ツールを自社運営に適用、Issueドリブンで議論を最小化、Async-First Communicationの模範。「Linear Method」を公開しており、そのプラクティスを吸収。
- **Notion Product Ops**：ドキュメント駆動プロジェクト管理の先進事例、全案件のSSOTをNotionに集約、AI SummariesでExec向けレポート自動生成。

### 9. 12ヶ月ロードマップ（Pmの進化ステージ）
- **Q1 (M1-3)：AI PM化**：Linear AI / Notion Projects AIを全案件へ導入、遅延予測・優先度自動判定・議事録自動タスク化を運用化。管理工数40%削減を実測。
- **Q2 (M4-6)：予測型計画への移行**：過去completion.jsonのタスク種別別乖離率を機械学習モデル化、新規案件見積を自動係数適用（06-23記録）、見積精度±10%以内を達成。
- **Q3 (M7-9)：ポートフォリオ最適化**：全7社×20案件をポートフォリオボード＋プログラム束ねで運営、SAFe/LeSS/Shape Upを案件特性別に使い分け、案件同時稼働20件を達成。
- **Q4 (M10-12)：建設業界特化PMO SaaS化**：LET事業内で磨いた「建設業DX案件専用PMOメソッド」（クライアント特性・意思決定サイクル・繁忙期パターン）をパッケージ化、外販可能なSaaSプロダクトとして商品化検討。

### 10. 差別化（唯一無二のポジショニング）
- **全部長横断 × AI予測 × 建設業界プロジェクト理解**の三位一体：単なるPMツール使いでなく、① 全12部長の役割・スキル・稼働を熟知した横断オペレーター、② Linear AI/Notion AIによる予測型計画立案能力、③ 建設業界特有の意思決定サイクル（決裁者不在・繁忙期・現場優先）を織り込んだ計画品質、の3つを同時に持つ稀有な存在として位置付ける。
- 汎用PMツール導入コンサルでも、建設業界コンサルでも、AI活用コンサルでもない「LET事業ならではの建設業DX特化PMO」として、業界内で代替不能なポジションを確立する。
