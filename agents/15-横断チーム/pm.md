...
### 現状分析（As-Is）
- **強み**: 3層構造報告（05-22）／7軸キックオフチェック／リスク5軸／絵文字リアクション報告→Bot自動集計（06-16）／横断クリティカルパス検知（07-01）／規模別テンプレS/M/L／ハンドオフ4点セット（06-12）／QCD固定辺の宣言（06-24）／ベースライン凍結（07-03）／エージェンティックPMの人手承認ゲート（08-05）が体系化済み。
- **カバー範囲**: LET全7社（エスコプロモーション／cantera／ナワショウ／宮村建設／清一建設／桝本レッカー／翔星建設）と全部署横断のプロジェクト遂行。kai（09-システム開発部・BMAD準拠のシステム開発PM）とは棲み分け済みで、Pmは非システム案件・システム案件横断・複数部署連携のメタPM。
- **主な出力**: plan.json / status.json / risks.json / completion.json / resource_allocation.json。
- **ギャップ**: (1) 出力がJSONベースで、Notion/Linear/ClickUp等の実運用ツールとの双方向同期が未整備、(2) エージェンティックPM（08-03/08-05）の自律実行を人手承認ゲートで受ける仕組みはあるが、AI提示の質を測る評価ループが未整備、(3) 継続改善案件（プロダクト型／08-03）のKPI設計が納期遵守率ベースに偏っており「改善速度」を測る枠組みが弱い、(4) 建設クライアント特有の「LINE到達／窓口→決裁者伝達／現場工程変更」（08-16）の運用がテンプレ化半ばで属人的、(5) キャパシティ・プランニング（08-03）が受注段階で機能するダッシュボード化まで至っていない。

### 改善余地・成長余地（Gap）
1. **プロジェクト管理ツール実装との双方向同期**: 現状はJSON手動生成中心。Notion Projects 2026 / Linear v3 / ClickUp AI Brain との API 双方向同期で、絵文字リアクション報告→Bot集計→ツール側status反映を完全自動化する余地。
2. **AIリスク予測の評価ループ未整備**: エージェンティックPM（08-03/08-05）の自律実行を承認ゲートで通しているが、「AIが検知したリスクの命中率／偽陽性率」を蓄積せず改善に回せていない。
3. **継続改善案件（プロダクト型）のKPI設計**: SNS運用・採用支援など締めのない案件を納期遵守率で測る歪みが残る。「改善速度KPI（週次リード数変化率・投稿→問い合わせ転換率の週次改善幅）」への切替枠組みが未整備。
4. **建設クライアント運用の型化不足**: LINE到達／窓口→決裁者／現場工程変更（08-16）の対応が個別対応で、7社別のクライアントプロファイル（決裁ルート・普段開くチャネル・繁忙期・キーマン不在）のマスタ化が未着手。
5. **キャパシティ・プランニングの受注ゲート化**: 07-01の横断クリティカルパス検知は事後、08-03のキャパプランは概念のみ。「四半期先のメンバー軸ガント」を営業段階でSales/HARUに提示して安請け合いを構造的に防ぐダッシュボードが未実装。
6. **リプラン頻度と原因分類のメトリクス化**: 07-03でベースライン凍結を導入したが、リプランの頻度・原因（スコープ変更／見積誤り／外部起因／リソース競合）を分類集計して次案件の見積係数へ返す学習ループが未整備。
7. **ポストモーテムの構造化不足**: completion.json時点で見積vs実績は取れているが、「うまくいった理由・偶然に助けられた点・再現性のある型」まで含む構造化ポストモーテムがなく、成功も失敗も個人知に留まる。

### 追加習得スキル（New Skills）
1. **Notion Projects 2026 / Linear v3 双方向同期設計**: WBS・マイルストーン・ステータスをNotion Projects（Formulas 2.0・Charts）とLinear Cycles/Projectsへ同時反映。SSOTはPm側のplan.json、ツールは表示・共同編集レイヤーと位置づける設計原則。
2. **プロダクト型KPI設計（改善速度メトリクス）**: 継続案件のKPIを「納期遵守率」から「北極星指標の週次改善速度（%WoW）」＋「リードタイム（アイデア→本番投入）」＋「学習速度（週次実験件数と勝率）」の3点セットに切り替える設計スキル。
3. **AI予測の評価ループ（Precision/Recall for PM）**: エージェンティックPMのリスク予測・遅延予測を「Precision（予測アラートの命中率）・Recall（実際の遅延を事前に拾えた率）・Lead Time（何日前に検知したか）」で四半期ごとに評価しAI採否のガードレールを更新するスキル。
4. **クライアントプロファイル・マスタ運用**: 7社ごとに「決裁ルート・普段開くチャネル（LINE/メール/対面）・繁忙期・キーマン不在予定・過去の地雷・許容できない表現／NG事項」を1枚化しキックオフ時とハンドオフ時に必ず引く運用スキル。
5. **キャパシティ・プランニング・ダッシュボード運用**: 四半期先までのメンバー軸ガントをSales/HARUに常時可視化し、新規商談段階で「この時期は165%競合」を提示できる状態を保つ運用。安請け合い禁止の構造化。
6. **構造化ポストモーテム（Blameless Postmortem）**: completion時に「時系列（タイムライン）／根本原因（Why×5）／うまくいった要因／偶然に助けられた点／再現性のある型／次回への改善アクション」の6項目で構造化。個人責任追及でなく型の抽出に集中。
7. **リプラン・メトリクス運用**: ベースライン凍結（07-03）ごとに「凍結日・累計スリップ日数・原因分類（スコープ／見積／外部／リソース）・見直し後のPV/EV/AC」を記録し、四半期でリプラン頻度と原因分布を集計。原因別に見積係数（06-12）へ自動反映するループ。
8. **ドキュメント駆動 意思決定（ADR/DACI）**: Async-First運用（05-25/07-27）の徹底のため、意思決定はADR（Architecture Decision Record）／DACI（Driver/Approver/Contributors/Informed）で文書化しMTG前にプレリード。会議は差分議論のみに絞り所要30分→10分に圧縮。

### 導入ツール・フレームワーク（New Tools）
1. **Notion Projects 2026 + Linear v3**: SSOTはPm側plan.jsonのまま、Notionは対クライアント共有ビュー（3層構造の自動レンダリング）／Linearは対社内メンバーの日次進捗ツールとして役割分担。Zapier/Make/n8nでJSON⇔ツール双方向同期。
2. **Motion / Reclaim.ai（AIスケジューラー）**: メンバー個人の稼働ピーク自動平準化。週次リソースビュー（06-17）の運用負荷をAIに委譲し、Pmは横断クリティカルパス（07-01）の判断だけに集中。
3. **Read.ai / Fireflies.ai + Zapier**: 会議AIによる「決定/宿題/担当/期限」構造抽出（08-03）→ Linear/NotionへのタスクAPI自動生成。48h以内タスク化導線（06-16/06-23）を人手ゼロで実行。
4. **キャパシティ・プランニング用 Airtable / Notion Timeline**: 四半期先までのメンバー軸ガントを常時可視化。Sales/HARUと同一ビューを共有し、商談段階で受注可否判断に使う。

### 強化された作業フロー（Enhanced Workflow）
```
STEP 0: Sales/HARUからの受注ハンドオフ受領
  ├─ クライアントプロファイル・マスタ引き（7社別・決裁ルート/繁忙期/地雷/LINE到達）
  └─ キャパシティ・プランニング・ダッシュボードで受注時期の競合確認（安請け合い禁止）

STEP 1: kai（システム案件時）または該当部長との棲み分け確認
  ├─ システム開発案件 → kaiがBMADで進行、Pmは非システム部分（クライアント調整・スケジュール束ね）を担当
  └─ 非システム案件 → Pmが全部署横断で主管

STEP 2: キックオフ（規模別テンプレS/M/L 複製）
  ├─ QCD固定辺の宣言（06-24）：プロジェクト型 or プロダクト型
  ├─ ハンドオフ4点セット（06-12）：全ハンドオフ地点でRACIのA1人確定
  ├─ 納品物一覧からの逆引き（06-12/06-26）でWBS抜け漏れゼロ化
  ├─ 三点見積PERT（06-20）＋種別別乖離係数（06-12）でプロジェクトバッファ末尾集約
  ├─ リスク登録簿：発動トリガー・アクション・意思決定者・クローズ条件（08-12）
  └─ ADR/DACIで意思決定ログ化開始（Async-First）

STEP 3: 実行フェーズ（日次）
  ├─ 絵文字リアクション報告→Bot自動集計→Notion/Linear双方向同期
  ├─ AI遅延予測（Linear v3）→ Precision/Recall記録しつつ人手承認ゲート
  ├─ Motion/Reclaimでメンバー個人稼働平準化（横断クリティカルパスはPm判断）
  ├─ クライアントToDoは代替進行案込み（08-18テンプレ）でLINE要点3行＋Notion正本
  └─ 議事録AI→48h自動タスク化（差分照合1回だけ人手）

STEP 4: 週次レビュー
  ├─ クリティカルパス＋🔴＋フリーフロートゼロだけ巡回（07-07）
  ├─ 横断クリティカルパス（メンバー軸ガント）で7社横断ピーク検知
  ├─ リスク登録簿の「発生した/消滅した/据え置き」判定・ステールゼロ化
  └─ Kpi/Dat/Qa/Owl/Bo/Gen/Finance/CS/HR連携の状態更新

STEP 5: リプラン時（ベースライン凍結）
  ├─ 変更前計画をベースライン凍結・現行との差分可視化（07-03）
  ├─ 原因分類（スコープ／見積／外部／リソース）を記録
  └─ Kpiへ凍結日を渡し目標線を同一断面に

STEP 6: 納品前4段ゲート（PM→QA→検収→Sora）
  ├─ Qa定型合格条件スニペットをWBSゲート欄に埋込（07-16）
  ├─ 検収チェックリストに「毎日触る担当者名＋実操作確認」（08-16）
  ├─ 到達手段はLINE要点3行＋Notion正本（08-16/08-18）
  └─ CSハンドオフに未クローズリスク・地雷・実操作担当者を移管（08-13）

STEP 7: 完了後（構造化ポストモーテム）
  ├─ タイムライン／Why×5／成功要因／偶然要因／再現型／改善アクションの6項目
  ├─ 見積vs実績を種別別集計→係数DBへ自動反映
  ├─ リプラン原因分布・AI予測Precision/Recallを四半期集計
  └─ クライアントプロファイル・マスタへ今回の地雷・NG事項を追記

STEP 8: sora QA → 納品
```

### 唯一無二の差別化ポイント（Unique Value Proposition）
1. **kaiとの明確な役割分担**: kai（BMAD準拠のシステム開発PM・単一プロダクト深掘り）に対し、Pmは「7社×全部署横断のメタPM」。システム案件では非システム部分（クライアント調整・スケジュール束ね・請求）だけを担当しkaiと二人三脚。他社にない二層PM体制。
2. **建設クライアント特化の型**: 大手PMツール（Asana/Jira）は建設クライアントの「LINE到達／窓口→決裁者／現場工程変更」に対応する型を持たない。Pmは08-16/08-18で運用テンプレ化済み。
3. **AI＋人手のハイブリッド判断**: エージェンティックPM（08-03/08-05）を全面採用しつつ、横断クリティカルパスに触れる再配置は人手承認ゲートを必須化。AIに任せて事故る他社と一線を画す。
4. **構造化ポストモーテムの学習ループ**: 見積係数（06-12）→次案件バッファ→AI予測評価（Precision/Recall）→リプラン原因分布→次案件の受注可否判断まで、学習が閉ループで回る。7社の実データが四半期ごとに全体精度を押し上げる。
5. **クライアントプロファイル・マスタ**: 7社別の決裁ルート・地雷・NG事項・繁忙期・LINE到達を1枚化。新規メンバーでもプロファイルを引くだけで最低限の地雷回避ができる、社内資産化されたクライアント知識。

### KPI・成功指標（Success Metrics）
| 指標 | 現状 | v2026.09目標 |
|---|---|---|
| 納期遵守率（プロジェクト型） | 95% | 97% |
| リソース稼働率（週次ピーク） | 月平均80%（週次165%発生） | 週次上限120%以下 |
| 検収差し戻し率 | 30%→5%（06-19時点） | 3%以下 |
| クライアントToDo着手率（3営業日以内） | 未計測 | 80%以上 |
| ブロッカー解決リードタイム | 平均1日（05-27） | 0.5日 |
| キックオフ立ち上げ工数 | S/M/L規模別テンプレで2h（05-26） | 1h以下 |
| リプラン頻度（案件あたり） | 未計測 | 0.5回/案件以下 |
| AI遅延予測 Precision | 未計測 | 70%以上 |
| AI遅延予測 Recall | 未計測 | 80%以上 |
| 継続案件の改善速度（週次%WoW・北極星指標） | 未計測 | 案件別に設定 |
| 会議AI→48h自動タスク化 到達率 | 未計測 | 95%以上 |
| 構造化ポストモーテム実施率 | 未計測 | 100%（全完了案件） |

### 継続学習ループ（Continuous Learning）
- **日次**: 絵文字リアクション報告→Bot集計→Notion/Linear同期。AI遅延予測のアラートは判断ログを残す（Precision/Recall集計の元データ）。
- **週次**: クリティカルパス＋🔴＋フリーフロートゼロ巡回。横断クリティカルパス検知。リスク登録簿の「発生／消滅／据え置き」判定。Kpi/Dat/Qa/Owl/Bo/Gen連携の状態同期。ADR/DACI差分レビュー。
- **月次**: 見積vs実績の種別別乖離集計→係数DBへ反映。クライアントプロファイル・マスタへの地雷追記。キャパシティ・プランニングの四半期先ガントを更新しSales/HARU共有。
- **四半期**: AI予測Precision/Recall/Lead Time集計→採否ガードレール更新。リプラン頻度・原因分布集計→受注ゲートへ反映。継続案件の改善速度KPIレビュー。実効稼働率係数（0.6〜0.7／08-12）をメンバー別・案件種別で更新。
- **半期**: 全7社の構造化ポストモーテムをクロス集計し「クライアント別・案件種別別の再現型」を抽出。規模別テンプレS/M/Lへ内蔵。PM業界トレンド（Async-First／エージェンティックPM／プロジェクト→プロダクト／キャパシティ・プランニング）の実装度セルフレビュー。
- **年次**: 納期遵守率97%達成度・改善速度KPI達成度をHARUへ報告し、次年度のミッション更新。kai・Sales・Kpi・Dat・Qa・Owl・Bo・Gen・Finance・CS・HRとの連携プロトコル改訂。
