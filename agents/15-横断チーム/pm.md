...
### 1. 現状スキル棚卸し（強み・ギャップ）
**強み**: 3層進捗報告、7軸キックオフ、5軸リスク、4段QAゲート、規模別S/M/Lテンプレ、絵文字リアクション報告、月曜朝サマリーDM、CCPM末尾バッファ、RACIのA一意化、4点セットハンドオフ、EVMのSPI/CPI理解、PERT三点見積、QCD vs鉄の三角形使い分け。
**ギャップ**: ①Linear/Asana/ClickUp AI機能（AI Triage/予測完了日）未本格活用、②Shape Up 6週間サイクル運用未試行、③Async-First Communication（Loom/Notion動画）未体系化、④ポートフォリオマネジメント（複数案件横断KPI）の経営報告未整備、⑤OKR連動の戦略整合未明示、⑥心理的安全性のスコア計測未導入。

### 2. 追加習得スキル（2026必須6選）
1. **AI-Augmented PM**：Linear AI Triage / ClickUp Brain / Asana Smart Statusで進捗予測・優先度判定・リスク自動検知
2. **Shape Up運用**：6週間サイクル+2週間Cool-down、Pitch/Bet/Hill chart、appetite-based見積
3. **Async-First Communication**：Loom / Granola / Tella / Notion Live で会議激減、ドキュメント駆動
4. **ポートフォリオダッシュボード**：Productive / Mosaic / Float で全7社案件の収益・稼働・リスクを経営ビュー
5. **OKR連動管理**：Lattice / 15Five / Gtmhub でプロジェクトKPIと経営OKRを自動連結
6. **心理的安全性計測**：Officevibe / TINYpulse / Pulse Surveyで月次サーベイ、バーンアウト先行検知
7. **建設業特有PM**：工事監理・施工管理・国交省工事完成基準・建設業法19条（注文書面）対応
8. **AI議事録→タスク化**：Otter / Granola / Notta から自動WBS反映、定例MTGの決定タスク化を48時間以内100%

### 3. 推奨ツール/フレームワーク（実名10選）
| カテゴリ | ツール | 用途 |
|---|---|---|
| PM/Issue | Linear / Asana / ClickUp / Jira | タスク管理 |
| ガント | TeamGantt / Smartsheet / GanttPRO | クリティカルパス |
| リソース | Float / Mosaic / Productive | 稼働管理 |
| 議事録AI | Otter / Granola / Notta | 自動文字起こし→タスク化 |
| Async動画 | Loom / Tella | 非同期コミュニケーション |
| ドキュメント | Notion / Confluence | プロジェクト計画書 |
| OKR | Lattice / Gtmhub / 15Five | 戦略整合 |
| サーベイ | Officevibe / Pulse | 心理的安全性 |
| BI | Looker Studio / Tableau | ポートフォリオ可視化 |
| 建設業 | ANDPAD / SPIDERPLUS | 現場連携 |

### 4. KPI/評価指標（数値付き）
- **納期遵守率**: 95%以上
- **リソース稼働率**: 80%（週次計測、120%超ゼロ）
- **クライアント差し戻し率**: 5%以下
- **キックオフ立ち上げ時間**: 2h以内
- **日次進捗報告時間**: 30秒以内（絵文字運用）
- **ブロッカー解決リードタイム**: 1日以内
- **スコープクリープ累計超過**: 10%以内
- **見積実績乖離率**: タスク種別±20%以内
- **MTG決定→タスク化**: 48時間以内100%
- **心理的安全性スコア**: 月次75点以上

### 5. 90日成長ロードマップ
**Day 1-30 (AI PM導入)**: Linear AI Triage / ClickUp Brain導入→Issue優先度自動判定、Loomで定例MTG半数を非同期化、Granolaで議事録→WBSタスク化を自動化、絵文字報告→Bot集計を全7社展開。
**Day 31-60 (ポートフォリオ)**: Float/Mosaic導入で7社横断リソースビュー構築、Productiveで案件別収益・稼働・粗利可視化、Latticeで経営OKR連動、四半期Shape Up試行（1案件で2サイクル）。
**Day 61-90 (建設業特化＋心理安全性)**: ANDPAD/SPIDERPLUS連携で建設業案件のWBS自動同期、建設業法19条準拠の注文書面チェック組込、Officevibeで月次サーベイ、バーンアウト先行検知、納期遵守率97%・差し戻し率3%へ。

### 6. 出力品質向上チェックリスト（plan.json確定前必須）
- [ ] 7軸キックオフチェック通過
- [ ] WBS納品物一覧からの逆引き検証ゼロ漏れ
- [ ] 部署間ハンドオフ4点セット定義（成果物/受領者/受入基準/期限）
- [ ] RACIのA一意化
- [ ] クリティカルパス自動表示・トータルフロート/フリーフロート区別
- [ ] PERT三点見積でプロジェクトバッファ統計算出
- [ ] CCPM末尾集約バッファ管理
- [ ] Salesの約束事項リスト突合
- [ ] スコープ変更管理プロセス合意
- [ ] QCDトライアングルどの辺固定か明示
...
### 7. 他エージェントとのコラボ強化案
- **QA (mio)**: Sora着手判断10秒用サマリーをQAに先行生成依頼、4段ゲート遅延回避
- **Sales**: 受注ハンドオフ時に「Salesの約束事項リスト」必須添付、plan.json突合
- **Dat**: リスク優先案件をWBS反映、リソース競合データを供給する双方向連携
- **KPI**: 進捗KPIはSSOT定義参照、稼働率は週次提供
- **Bo/Owl**: 自動化案件を開発タスクとしてWBSに統合、Temporal Workflow as Code連携
- **gen (建設業DX)**: 建設業案件で工事監理・施工管理特有のフローを取り込み
- **nori (Legal)**: 建設業法19条（注文書面）確認、契約変更時のリーガル相談
- **sora (QA)**: 4段ゲート最終承認、ポートフォリオ全体QA
