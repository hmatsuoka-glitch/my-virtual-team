...
- **導入内容**: 全社プロジェクトを「戦略適合度×リソース×ROI」の3軸で評価しポートフォリオ管理。優先度自動判定。
- **導入基準**: 月次で全プロジェクト評価。
- **期待効果KPI**: 高価値プロジェクトへのリソース集中率 40%向上／低価値プロジェクト打ち切り 30%
- **連携先**: haruto、HARU、finance

### 2. Critical Path Method (CPM) + Gantt
- **導入内容**: 全大型プロジェクトでCPM分析＋ガントチャート運用。クリティカルパス自動特定。
- **導入基準**: 3ヶ月以上のプロジェクトで必須。
- **期待効果KPI**: 納期遅延 60%削減／リソース最適配分 40%向上
- **連携先**: kai、tsumugi、kaito

### 3. Resource Leveling & Allocation
- **導入内容**: 全社リソース（人・予算・ツール）の稼働率を可視化し、過負荷/遊休を機械検出。
- **導入基準**: 週次で全プロジェクトに適用。
- **期待効果KPI**: リソース稼働率 70%→90%／燃え尽き 60%削減
- **連携先**: hr、finance、全部長

### 4. Risk Register & Mitigation
- **導入内容**: 全プロジェクトの リスク（Impact×Probability）を Register 化し、月次レビュー＋対策実行。
- **導入基準**: 全プロジェクトで必須。
- **期待効果KPI**: リスク顕在化 60%削減／プロジェクト成功率 40%向上
- **連携先**: deva、kai、legal

### 5. Stakeholder Engagement Matrix
- **導入内容**: プロジェクト関係者を Power×Interest マトリクスで整理し、コミュニケーション戦略を最適化。
- **導入基準**: 全プロジェクトで必須。
- **期待効果KPI**: ステークホルダー起因の炎上 70%削減／プロジェクト成功率 40%向上
- **連携先**: ryota、pr、hr

### 6. Agile / Scrum / Kanban Hybrid
- **導入内容**: プロジェクト特性に応じてAgile Scrum / Kanban / Waterfall を使い分け。ハイブリッド運用も対応。
- **導入基準**: 全プロジェクト着手時に方法論選定。
- **期待効果KPI**: 手法適合度 40%向上／プロジェクト成功率 30%向上
- **連携先**: kai、hr

### 7. Earned Value Management (EVM)
- **導入内容**: PV/EV/AC/SPI/CPI でプロジェクト進捗を金額換算で管理。予算・スケジュール逸脱を早期検知。
- **導入基準**: 大型プロジェクト（1000万円以上）で必須。
- **期待効果KPI**: 予算超過 60%削減／早期是正可能率 40%向上
- **連携先**: finance、kai

### 8. Cross-project Dependency Management
- **導入内容**: 部門横断プロジェクトの依存関係を可視化し、ボトルネック自動検出。
- **導入基準**: 複数プロジェクト並行時に必須。
- **期待効果KPI**: 依存関係起因の遅延 60%削減／全体リードタイム 30%短縮
- **連携先**: 全部長、kai

### 9. Post-Project Review & Learning
- **導入内容**: 全プロジェクト完了後にRetrospectiveを実施し、成功要因・失敗要因を組織資産化。
- **導入基準**: 全プロジェクトで必須。
- **期待効果KPI**: 学習速度 3倍／同種失敗再発率 60%削減
- **連携先**: sora、hr、kai

### 10. PMO Center of Excellence
- **導入内容**: PMO（Project Management Office）を組織化し、標準テンプレ・ツール・研修を全社展開。
- **導入基準**: プロジェクト数増加時に発動。
- **期待効果KPI**: PM生産性 3倍／全社プロジェクト成功率 40%向上
- **連携先**: hr、kai、HARU
