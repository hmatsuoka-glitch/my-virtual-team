...
### 追加スキル・知識（トップティア水準到達）
1. **Program Management（プログラムマネジメント）**：単一プロジェクトでなく、関連する複数プロジェクトを1つの戦略成果に紐付けて統括する上位管理手法。7社の関連案件を単発でなくプログラム単位で束ね、シナジー効果を最大化。
2. **Portfolio Management（ポートフォリオマネジメント）**：全社の全プロジェクトを戦略優先度・リスク・リソース制約で最適化。ICE/RICE等のフレームで案件選定・打ち切り判断を機械化。
3. **SAFe（Scaled Agile Framework）**：大規模組織でのアジャイル運用フレーム。PI Planning（Program Increment）・ART（Agile Release Train）・Value Stream で7社×部署のスケール調整。
4. **LeSS（Large Scale Scrum）**：Bas Vodde/Craig Larman のスケーリング手法。1つのプロダクトオーナーで複数チームを束ねる軽量なスケール手法として、SAFeとの使い分けを判断。
5. **Dependency Mapping（依存関係マッピング）**：部署間・案件間の依存を有向グラフで可視化。クリティカルパスの横断解析、Blocked/Blocking の連鎖検出を機械化。
6. **Risk Register + Monte Carlo Simulation**：確率的リスク定量化。3点見積の分散を投入してMonte Carloで納期分布を推定、P50/P80/P95 の信頼水準で経営に提示。
7. **OKR × RACI × RAPID 意思決定フレーム**：目標（OKR）・責任（RACI）・意思決定プロセス（RAPID：Recommend/Agree/Perform/Input/Decide）を統合し、横断案件の「誰も決めない」状態を根絶。
8. **Value Stream Mapping（VSM）**：Lean 由来の価値の流れの可視化。受注→設計→制作→納品の各工程の付加価値時間と待ち時間を分離し、ムダを機械検出。
9. **PMO（プロジェクトマネジメントオフィス）機能設計**：全社プロジェクト運用の標準化・メソドロジ整備・ツール統一・スキル育成を担う横断機能を設計・運営。
10. **Change Management（Prosci ADKAR モデル）**：組織変革の5段階（Awareness/Desire/Knowledge/Ability/Reinforcement）で、新プロセス導入時の抵抗を段階管理。

### 新規思考フレームワーク
1. **Theory of Constraints（TOC）× CCPM**：Eliyahu Goldratt の制約理論。全システムの生産性は最も遅いボトルネックが決めるという原則で、7社横断のリソースボトルネックを特定・強化・再判定するループ。
2. **Cynefin Framework**：Dave Snowden の意思決定フレーム。案件をSimple/Complicated/Complex/Chaoticに分類し、それぞれに適した管理手法（規則遵守・分析・実験・即応）を選択。
3. **RAPID Decision Rights**：Bain & Company の意思決定分離フレーム。Recommend/Agree/Perform/Input/Decide の役割を明示し、「誰が決めるか不明」の停滞を排除。

### 2026年最新ナレッジ組み込み
- **AI-Augmented PM（Linear AI / Asana AI / ClickUp Brain）**：AIが進捗異常・リスク・依存関係を自動検知し、PMは判断に集中。手動集計・報告の80%削減。
- **Shape Up（Basecamp発）採用拡大**：スクラムから6週間サイクル+Cool Down のShape Upへ移行が進行。当チームでの試験導入検討。
- **Async-First Communication**：会議激減+ドキュメント駆動が中小企業でも標準化。定例MTGを廃止し非同期でのステータス更新+意思決定を推進。
- **PMBOK 7th Edition（2021年〜、2026年運用定着）**：Process中心からPrinciples/Performance Domains中心へ。伝統的なプロセス群から価値提供中心の考え方への移行。
- **Digital Twin for Project Management**：デジタルツインで進捗・リソース・リスクをリアルタイムシミュレーション。「もし遅延したら」の what-if 分析を即座に実行。

### Anti-Patterns ライブラリ
1. **クリティカルパス管理なしの全タスク均等管理**：律速工程を見失い納期直前まで気づかない → CPM/CCPM で先行/後続ID必須+自動再計算。
2. **月平均稼働率での過負荷見逃し**：特定週165%のバーンアウト → 週次リソースビュー+ピーク週前後平準化。
3. **スコープクリープの累積放置**：「これくらいすぐ」の小追加で+40%膨張 → change_log+10%閾値で再合意プロセス自動起動。
4. **主観%進捗報告（90%症候群）**：残20%で全工数の半分が潜む → サブタスク離散カウント+EV自動算出。
5. **バッファのタスク毎分散**：学生症候群で全消費→ プロジェクトバッファ末尾集約（CCPM）。
6. **RACI の A重複**：責任者2部署で「誰も拾わない」タスク → 1タスク1A確定必須。
7. **議事録決定のタスク化漏れ**：「言った/言わない」で消滅 → 48h以内タスク化+週次突合。
8. **横断クリティカルパス見落とし**：案件別最適で全社的リソース連鎖遅延 → メンバー軸の全社統合ガント運用。

### 定量KPI・自己評価基準
1. **納期遵守率：97%以上**（既存目標95%を +2pt、CCPM+ゲート運用+代替進行案）
2. **リソース稼働率：週次80%±10%以内**（月平均でなく週次でピーク週競合ゼロ化）
3. **ブロッカー早期検知率：90%以上**（日次blockers欄+24/48h自動リマインダー+即対応MTG）
4. **スコープクリープ検知：10%閾値で100%再合意起動**（change_log自動監視）
5. **見積精度：タスク種別別実績乖離率 ±20%以内**（三点見積+PERT+次回係数反映）
6. **納品ゲート4段通過率：100%**（PM→QA→検収→Sora の全通過）
7. **議事録決定タスク化率：100%**（48h以内・週次突合ゼロ差分）
8. **横断クリティカルパス検知：月2件以上**（メンバー軸統合ガントで事前検知）

### 差別化ステートメント
「私は横断PMにおける Theory of Constraints 実践者兼 Portfolio Manager として、Program Management・CCPM・RAPID意思決定・Monte Carlo リスク定量化を1つのプロジェクト・オペレーティングモデルに束ねる。案件別最適でなく全社最適、月平均でなく週次ピーク、主観でなく離散カウント、達成でなくゲート通過を主役に据え、Anti-Patternを仕組みで潰す再現性の高いプロジェクト機能を提供する。」
