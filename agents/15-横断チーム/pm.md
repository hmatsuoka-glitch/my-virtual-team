...
### 2026-08-08
- **効率化テクニック：依存グラフを「上流→下流の一方向DAG（有向非巡回グラフ）」で描画し、循環依存（AがBを待ち、BがAを待つ）を自動検出してキックオフ前にブロックする**。WBSの依存関係をExcel/ガントで表現するとタスク数が50を超えた辺りから循環依存に気づけず、キックオフ後に「両方が相手待ち」で全停止する事故が起きる。依存を明示的にDAGとして持ちtopological sortが失敗するタスクをplan.json確定前のassertでブロックすると、循環依存が構造的にゼロになる。さらにDAG化しておくとクリティカルパス自動再計算（06-16記録）と横断クリティカルパス検出（07-01記録）が同一データ構造で回せる
- **品質チェックポイント：ハンドオフゲートに「4点セット（06-12記録）」の実装だけでなく、「受領後24h以内の受入判定完了SLA」と「差し戻し理由コード」を追加する**。受領確認者を1人に確定しても、判定が滞留すれば下流着手は同じく止まる。受入判定を24h SLA化し、差し戻し時は「不足資料/受入基準未達/仕様認識齟齬/その他」の4コードで理由を残すと、ハンドオフ地点ごとの差し戻し率とパターンが集計可能になり、恒常的に差し戻しの多いハンドオフ地点を次回キックオフのテンプレ改善に還元できる（Datの06-11記録の分位点データ供給と同思想）
- **失敗パターン：見積の三点見積（06-20記録）で楽観・悲観の幅を「メンバー個人の感覚」に任せ、悲観値がタスク種別ごとに揃わず統計的なプロジェクトバッファ（CCPM／06-13記録）が意味を成さない** → 回避策：楽観/悲観の幅は個人裁量でなく「過去completion.jsonのタスク種別別実績分布のP10/P90」から機械的に設定し、メンバーは最可能値（P50）だけを申告する（理由：三点見積は分散を統計的に扱う前提なのに、悲観値が「安全側に振っておこう」の個人差で決まると分散が過大・過小の両方に振れ、末尾集約バッファが空回りする。P10/P90自動設定はDatの信頼区間思想・Datの06-13記録と同じで、見積不確実性の客観化を担保する）
- **連携ヒント：Owl（受注ワークフロー設計者）との受注→プロジェクト化ハンドオフに「見積時のQCD固定辺（06-20記録）と想定リソースアサイン」を必須引き継ぎ項目化する**。受注時に「スコープ固定・時間コスト調整」で合意したのにPMがアジャイル型で走らせるとキックオフ後に握り直しになる。Owlの状態遷移（Owlの06-20記録）とPmのWBSゲート（06-20記録）の対応（07-02記録）に加え、受注時想定リソースをキャパシティ・プランニング（08-03記録）の入力にすると、受注段階からピーク週競合を可視化でき安請け合いを構造的に防げる
- **トレンド察知：シナリオプランニング（複数の未来仮説を並列で計画）とモンテカルロ・シミュレーションによる納期確率分布提示が、リスク管理の主流ツールとして中小PMにも降りてきた**。単一納期日をコミット値として渡す従来型に対し、モンテカルロで「50%納期・80%納期・95%納期」の分布をクライアントに提示し、クライアント側の必達要件に応じてどの確率で握るかを合意する運用が広がる。三点見積の分散データ（06-20記録）と横断クリティカルパスDAG（本日記録）があれば、既存の @jira/Linear アドオン等で1コマンドで実行でき、単一日程コミットの脆さ（07-01記録のクラッシング前提での隠れ残業）を根本から解消できる
