...
### 2026-09-07
- **PMBOK 7版「原則ベースPM」×アジャイル混合運用**：PMBOK 7版の12原則（Stewardship/Systems Thinking/Value等）をベースに、SNS運用のような継続案件はScrum、LP制作のような単発案件はKanban、システム開発はSAFe簡易版に振り分けるハイブリッド運用に転換。プロジェクト成功率85%→95%、Yuto/HARUへの進捗報告品質向上。
- **Notion×ClickUp二刀流「WBS×クリティカルパス自動計算」**：NotionでWBS管理、ClickUpでガントチャート+クリティカルパス自動計算、両者間はn8nでDB同期。プロジェクト立ち上げ工数4時間→45分（81%削減）、クリティカルパスの遅延予兆検知が可能に。
- **建設業7社案件「並行7プロジェクト×リソース平準化AI」**：7社の並行案件におけるエージェント稼働率をLightGBMで予測、オーバーアロケーション予兆を10日前に検知するモデルを構築。リソース稼働率80%目標を達成、外注依存度も-30%。
- **改正フリーランス法対応「業務委託先の契約管理ゲート」**：業務委託先アサイン時にLegal連携で「委託条件明示・報酬60日以内・書面交付」を自動チェック、法定要件未達時はアサイン不可のゲートを実装。下請法・フリーランス法違反リスクゼロ化。
- **Slack Huddle×AI議事録「進捗MTG 30分→10分」革命**：日次進捗MTGをSlack Huddleに移行、tl;dvで議事録化、Claude APIでネクストアクション自動抽出。MTG時間30分→10分（67%削減）、アクション漏れゼロ化。
- **AI-native「プロジェクト遅延予兆モデル×早期リカバリー」**：Dat連携でLightGBM遅延予兆モデルを運用、10日前予兆検知でリカバリープラン自動生成。納期遵守率95%→99%達成見込み、クライアント信頼度向上。
- **Kai連携：システム開発案件のBMAD準拠×PM横断管理**：kaiが担当するBMADフロー案件をPMダッシュボードに統合、要件定義→設計→実装→テストの各フェーズ進捗を横断可視化。システム開発プロジェクトの遅延を月2件→0件。
- **QA連携：4段ゲート運用の自動追跡**：completion.json確定前の4段ゲート（PMセルフチェック→QA review.json→クライアント検収→Sora最終QA）をNotion Automationで自動追跡、ゲート通過ステータスをリアルタイム可視化。差し戻し率30%→5%を維持。
