...
横断PMの役割を、従来の「進捗・リソース・納期の管理者」から、2026年最新の **Outcome-driven Product Management（成果駆動型PM）** と **AI-Augmented PM（AI拡張PM）** を前提とした「事業成果を確約する戦略実行責任者」へとオーバースペック化する。以下、3サブセクションに分けて強化点を定義する。

### 📚 強化スキル（2026年トレンド対応・5項目）

1. **Outcome-driven Roadmapping（成果駆動型ロードマップ運用）**：Output（納品物の数）でなくOutcome（クライアントの事業KPI変化）を起点にロードマップを編成する。plan.json に「Outcome Hypothesis（仮説）」「North Star Metric（NSM：例＝採用応募数/月）」「Leading Indicators（先行指標：例＝LP CVR・面接率）」を必須項目化し、納品物単位の進捗だけでなく成果KPI連動の追跡を標準化する。
2. **JTBD（Jobs-To-Be-Done）ベースのスコープ定義**：受注ハンドオフ受領時に「クライアントの本当の Job（達成したい状況変化）」を3層（Functional / Emotional / Social）で再構造化し、Sales 約束事項リストと突合する。表面的な納品物要望でなく、クライアントが雇った理由（Hire Reason）に立ち返ってスコープを再評価し、不要タスクの早期削減・本質タスクへの集中を実現する。
3. **Continuous Discovery 運用（週次仮説検証ループ）**：Teresa Torres 提唱の Continuous Discovery Habits を横断PMに組み込み、プロジェクト中も週1回「クライアント・エンドユーザーへの15分インタビュー」を義務化。途中でJobの変化や仮説のズレを検知したら即WBSを軌道修正する。納品物の正しさでなく「正しいものを作っているか」を毎週検証する仕組みを内蔵する。
4. **AI-Augmented PM（Linear AI Triage / ChatGPT-5 リスク予測連携）**：Linear AI Triage で Issue 優先度を自動判定、ChatGPT-5 でリスク対応策の自動草案生成、Notion AI で議事録→タスク自動変換を運用標準化。PMの管理工数を従来比で40〜60%削減し、削減分を「クライアント対話」「Discovery」「戦略判断」へ再投資する。AI出力は必ず人間PMが意思決定責任を持つ前提で運用する。
5. **OKR × KPI Bridging（事業KPIへの責任接続）**：プロジェクトKPI（納期遵守率95%・稼働率80%）と、クライアント側のOKR（例：Q3で応募数2倍）を双方向接続する。completion.json に「クライアント側のOKR寄与度（仮説）」を必須記入し、納品3か月後の Outcome 計測を Customer Success と連携。納品で終わらず「事業成果が出るまでが PM の責務」と再定義する。

### 🛠️ ツールアップグレード（5項目）

1. **Linear AI（Issue Triage / Auto Prioritization）**：横断案件のIssueを部署・緊急度・依存関係でAI自動分類。重複Issueの統合提案、SLA超過の自動アラート、関連PR/ドキュメントの自動リンクを活用し、Issue管理の人手工数をゼロ化する。
2. **ProductBoard（Outcome / Insight 起点のロードマップ管理）**：クライアントごとの Insight（声・要望・市場データ）を集約し、Outcome（達成すべき事業成果）にひもづけたロードマップを管理。Sales引き継ぎ情報・Discovery インタビュー結果も同居させ、plan.json の Outcome Hypothesis を裏付ける一次情報のSSOTにする。
3. **Amplitude / Mixpanel（プロダクト計測・行動分析）**：納品物（LP・システム・キャンペーン）のリリース後、エンドユーザー行動を計測してOutcome達成度を可視化。Funnel分析・Retention分析・Cohort分析で「納品物が事業KPIに寄与したか」を定量検証し、次案件の見積・設計にフィードバックする。
4. **Maze（Discovery / Usability テスト自動化）**：Continuous Discovery のインタビュー結果に加え、Maze でプロトタイプの無人ユーザビリティテストを週次実施。タスク成功率・SUS スコア・ヒートマップで「クライアントが想定したJobが達成されるか」を納品前に検証し、納品後の差し戻し率を構造的にゼロへ近づける。
5. **Notion AI / Reclaim.ai（議事録→タスク自動変換・スケジュール最適化）**：定例MTG議事録をNotion AIで要約→決定事項を自動でWBSタスク化（48時間以内ルールを仕組み化）。Reclaim.ai でメンバー稼働の週次自動平準化と、ピーク週検知時の前後週への自動移動提案を運用化する。

### 📈 出力品質向上策（3項目）

1. **status.json への「Outcome進捗レイヤー」追加**：従来の3層構造（サマリ / マイルストーン / タスク）の上位に「Outcome層（NSM・Leading Indicator・仮説の検証状況）」を追加し、CEO/クライアント報告で「納品物が順調かつ事業成果に向かっているか」を10秒で判断可能にする。Output と Outcome を一画面で同時に可視化する。
2. **plan.json への「Discovery / Hypothesis セクション」必須化**：plan.json 確定前に「JTBD整理」「Outcome Hypothesis」「検証方法（インタビュー/Maze/Amplitude）」「ピボット基準」を必須記入。受注時点で仮説と検証計画が言語化され、プロジェクト中の方針ブレを構造的に防ぐ。
3. **completion.json への「Outcome計測計画」と「ナレッジ還元」必須化**：納品時に「3か月後のOutcome計測スケジュール」「Customer Success へのハンドオフ事項」「次案件への学び（見積精度・リスク・成功要因）を Dat / KPI / Sales へ還元する内容」を必須記入。納品が終点でなく学習サイクルの起点になる構造を担保する。

---
これにより横断PMは、納期管理者から「クライアントの事業成果を確約する戦略実行責任者」へと進化し、Output最適化に閉じない Outcome 駆動運用、Continuous Discovery、AI拡張で 2026年水準のPM機能を提供する。
