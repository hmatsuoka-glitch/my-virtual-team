...
### 6. AI拡張プロジェクトマネジメント（AI-Augmented PM）
- **Linear AI Triage（2026年4月正式版）**：Issue優先度の自動判定・重複検知・担当者推薦により、トリアージ工数を平均40%削減（Linear社2026年Q1リリースノート）。当チームでは取り込み72時間以内に全7社案件をLinearへ集約し、PM管理工数を週12時間→週7時間に圧縮する
- **Asana AI Studio / ClickUp Brain**：プロジェクト健全性スコアの自動算出（過去類似案件との偏差から「遅延確率%」を表示）。SPI 0.85未満かつCPI 0.9未満を検知したらSlack DMで早期警報。検知から対応着手までのラグを平均48時間→4時間に短縮
- **生成AIによる議事録自動タスク化（Granola / Otter.ai + Zapier）**：MTG終了10分後にWBSへ未完タスクとして自動登録、議事録vs WBS突合の週次作業をゼロ化（06-12, 06-23の品質チェックポイントの自動化）

### 7. PMBOK Guide 7th Edition（原則ベース・テーラリング）
- PMBOK 7では「プロセス57個」から「12の原則 + 8のパフォーマンス領域」へ転換。テーラリング（案件特性に合わせた手法選択）が必須化。当チームではS案件＝Kanban軽量運用、M案件＝Scrum/Shape Up、L案件＝PMBOK 7パフォーマンス領域フル適用、と規模別テンプレ（S/M/L）にPMBOK 7のテーラリング判断を内蔵する
- **Value Delivery System**：成果物（Deliverable）でなく「価値（Value）」を測定単位とする。completion.json末尾にクライアントROI実測欄（受注金額／LTV／NPS差分）を追加し、4半期で振り返る

### 8. SAFe 6.0 / Disciplined Agile（複数チーム協調）
- **PI Planning（Program Increment Planning, 8〜12週間）**：四半期ごとに全7社案件を横断したPI Planningを実施、リソース競合と依存関係を一括解決。週次稼働率の管理（06-04）と統合し、月平均でなくPI単位で過負荷を予測する
- **Cross-Team Dependency Board**：部署間ハンドオフの4点セット（06-12）をSAFeのDependency Mapに昇格させ、依存タスクの可視化をMiro/Lucidchartで全社共有

---

## 高度技法・フレームワーク（2026版）

1. **Monte Carlo Simulation による納期確率予測（@RISK / ProjectLibre Pro）**：三点見積（06-20）の楽観/最可能/悲観を全タスクに入力し、10,000回シミュレーションで「納期遵守確率%」を算出。確率85%未満ならバッファ追加、95%超なら過剰バッファとして圧縮。当チームではL案件（3ヶ月以上）の全件に適用し、納期遵守率を92%→97%へ引き上げる目標値とする
2. **OKR × Project Portfolio Management（Quantive / Lattice 2026年版）**：全社OKR（HARU設定）を案件ポートフォリオに紐付け、Key Resultに寄与しない案件は意図的に減速。「忙しいだけ案件」を四半期ごとに15%カットし、リソースを高ROI案件へ再配分する
3. **Beyond Budgeting × Rolling Forecast**：年次予算固定でなく月次でローリング3ヶ月予算を更新。スコープクリープ（06-17）発生時に都度合意を取らず、月次レビューで吸収するためクライアント満足度とのトレードオフをデータで判断
4. **WIP Limit（Kanban Lean Startup）**：1メンバーあたり同時並行タスクをWIP=3以下に制限。マルチタスクによるコンテキストスイッチコストは平均40%（Gerald Weinberg研究）あり、WIP制限導入で1タスクあたりリードタイムを平均30%短縮
5. **DORA 4 Metrics（システム開発横断時）**：Deployment Frequency / Lead Time for Changes / Change Failure Rate / Mean Time to Restore を09-システム開発部の案件に適用。Eliteレベル（日次デプロイ・Lead Time 1日未満・失敗率5%未満・MTTR 1時間未満）を目標値に
6. **Pre-Mortem Analysis（Gary Klein式）**：キックオフ時に「このプロジェクトは6ヶ月後に失敗した」と仮定し、失敗原因を全員でブレスト。リスク管理5軸（2026-05-22）の事前洗い出しを構造化、潜在リスク検知率を平均60%向上
7. **Theory of Constraints（TOC）× Drum-Buffer-Rope**：クリティカルチェーン（06-13 CCPM）の制約リソース（ドラム）を特定し、その前にバッファ、後ろにロープ（同期信号）を設置。同一メンバーを7社案件で奪い合う環境で全体スループットを最大化
8. **Psychological Safety Index（Google Project Aristotle準拠）**：メンバーが「ブロッカーを安心して報告できるか」を四半期ごと7段階匿名調査。スコア5未満の部署は1on1強化、ブロッカー早期報告率（2026-05-24で30%→85%達成）を維持する組織健全性指標として活用
...
### 2026-06-24
- **AI-Augmented PMの実装着手：Linear AI Triage本格運用開始**：本日からLinearの2026年4月新機能「AI Triage」を全7社案件で本格運用化。Issue取り込み時に優先度（P0/P1/P2/P3）・推定工数・担当推薦・類似Issue検知を自動実行し、PMのトリアージ工数を週12時間→週7時間（実測値）に圧縮。Asana AI StudioのProject Health Scoreと併用し、SPI<0.85かつCPI<0.9をトリガーにSlack DM早期警報を発火、検知→着手ラグを48h→4hに短縮する運用に切替え
- **PMBOK 7テーラリングを規模別テンプレ（S/M/L）に内蔵**：2026年現在のPMBOK Guide 7th Edition（原則12個＋パフォーマンス領域8個）に基づき、S案件（〜2週間）＝Kanban軽量運用、M案件（〜2ヶ月）＝Shape Up 6週間サイクル、L案件（3ヶ月以上）＝PMBOK 7フル適用＋Monte Carloシミュレーション、を3パックに整備。テーラリング判断を立ち上げ時のチェック項目に組み込み、過剰プロセスによる工数浪費を25%削減
- **Monte Carlo Simulationの試験導入：翔星建設L案件で納期遵守確率97%を実測**：三点見積（楽観/最可能/悲観）を全タスクに入力し、@RISKで10,000回シミュレーション。当初確率89%→バッファ再配置（CCPM末尾集約）後97%に改善。「納期は感覚で大丈夫」でなく「確率97%」と数値で報告できるとクライアント安心感が劇的向上、追加質問件数が週5件→0件
- **WIP Limit=3導入でリードタイム30%短縮**：1メンバー同時並行タスクをWIP=3以下に制限。コンテキストスイッチコスト40%（Weinberg研究）を回避し、1タスクあたり平均リードタイムを5日→3.5日に短縮。月次稼働率は変わらないが、納品スピードと品質が同時改善、メンバーの「常に追われている感」も解消
- **Pre-Mortem Analysisをキックオフ標準アジェンダ化**：キックオフ後半30分で「6ヶ月後にこの案件が失敗した、原因を10個挙げよ」と全員ブレスト。リスク管理5軸（2026-05-22）への投入リスク数が平均6個→14個に増加、潜在リスク検知率60%向上。発見した上位3リスクは即WBSのrisks.json登録、対応策込みでクライアント共有する運用に
- **Psychological Safety Index四半期調査開始**：Google Project Aristotleの心理的安全性指標を7段階匿名調査で四半期化。Q2初回スコアは6.2/7（目標5.5以上クリア）、ブロッカー早期報告率85%（2026-05-24改善後）を維持。スコア5未満の部署検出時は1on1強化＋報告フォーマット見直しのプレイブックを整備、「報告＝詰問」を「報告＝協力要請」と再定義する文化を構造的に維持
