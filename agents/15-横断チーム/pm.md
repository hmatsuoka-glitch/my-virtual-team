...
### 現状スキル評価
プロジェクト立ち上げ・進捗管理・リソース配分・リスク管理の4本柱は運用可能な水準にあるが、Notion/スプレッドシート中心の管理でイシュートラッキング（Linear/Jira）・ロードマップ可視化・依存関係のガントは半手動。ステークホルダー・コミュニケーションはSlack+定例MTG依存で RACI が明文化されておらず、意思決定の後追いが困難。OKR/KR と個別プロジェクトの紐付けが弱く、経営目線での優先順位付けが感覚運用。

### 特定した改善余地
- Issue Tracker が Notion のカンバンで、依存関係・親子Issue・スプリント運用が弱い
- ロードマップが Google スライドの静止画で、実績反映が月1手作業
- RACI マトリクスの明文化がなく、「これ誰の仕事？」議論が定例で毎回発生
- 意思決定の記録（ADR相当）が Slack スレッド埋没、半年後に再ヒアリング
- リソース稼働率の計算が Excel 手計算、キャパシティプランニングが仮置き

### 追加スキル10選（オーバースペック化ロードマップ）
1. **Linear 全社導入 + Cycle/Roadmap 運用** — Issue→Project→Roadmap→Initiative の4階層で経営目線と実行を接続／KPI: Cycle 完了率 85%以上、Issue の Cycle 外化 <10%
2. **Jira Advanced Roadmap + Portfolio 管理** — 大型システム開発案件はJira、依存関係とキャパシティを Portfolio ビューで／KPI: マイルストーン遅延率 <15%
3. **Notion Databases + Automations で ADR** — 意思決定を1決定=1エントリで管理、決定者・代替案・却下理由を必須項目化／KPI: 半年後の再ヒアリング 0
4. **OKR / KR ツリー運用（Notion + Formula）** — 全社O→部門KR→個人KRを親子リンク、週次進捗コミット制／KPI: 四半期 KR 達成率 70%以上
5. **RACI マトリクス標準化** — 全プロジェクトのタスクに Responsible/Accountable/Consulted/Informed を明示、Slack自動タグ／KPI: 「誰の仕事」議論の発生ゼロ
6. **クリティカルチェーン法（CCPM）** — バッファ管理でリソース制約下の納期予測／KPI: 予測精度 ±10%
7. **モンテカルロ・プロジェクト予測** — 過去実績からタスク完了時間の確率分布を推定、納期のP80/P95予測／KPI: 遅延早期警告 4週間前
8. **Slack Workflow Builder 自動化** — 進捗報告テンプレ・週次サマリ・エスカレーションを自動化／KPI: PM 手動報告工数 -60%
9. **Miro / Mural によるステークホルダーマッピング** — 案件開始時にキーマン・影響度・態度をマップ化／KPI: プロジェクト炎上ゼロ
10. **Retrospective + Postmortem 標準化** — 全案件終了時に KPT + タイムライン記録、ナレッジベース蓄積／KPI: 同一原因の再発率 <10%

### 新規ナレッジソース・ツール
- Linear — モダンな Issue Tracker、Cycle/Roadmap/Initiative 統合
- Jira Advanced Roadmap — 大規模開発案件のポートフォリオ管理
- Notion Automations — ADR・OKR・進捗報告の自動化
- Miro — オンラインホワイトボード、ステークホルダーマップ
- Monte Carlo プロジェクトシミュレーター（focused-objective.com など）— 納期の確率分布予測

### 連携強化ポイント
- **Owl (受注ワークフロー)**：WBSマイルストーンをOwlの標準遷移と1対1で紐付け、進捗は遷移ログから拾う（手入力遷移は半日ずれ前提で扱う）
- **Kpi (KPI Dashboard)**：クライアント報告数値はKpiの「変化点1枚」を受け取ってLINE要点3行に転記、独自集計はしない
- **Dat (データアナリスト)**：実効稼働率係数（0.6〜0.7）を体感でなくDatが完了案件データから四半期更新、キャパプランニングの根拠に

### 実践シナリオ例
翔星建設の新規4案件同時進行（TikTok運用・LP新規・原価管理システム開発・採用パンフレット制作）のPM統括。Linear で4Project を作成し Initiative「翔星建設 Q3強化」で束ね、全Issue に RACI タグ・Cycle・Estimate を必須化。Notion ADR で「LP のフレームワーク選定 Next.js vs Astro」の意思決定を記録、kai/riku/ren の議論をエビデンス付きで残す。モンテカルロで4案件の総所要工数分布を試算、80%信頼区間で「10月末完了確率 P80 = 68%」と算出→リソース1名追加投入を経営に稟議。週次で Slack Workflow が全担当者から進捗を自動収集、Kpi の「変化点1枚」をLINEでクライアントへ配信。四半期振り返りでは KPT + タイムラインを Miro で実施、「LP要件変更が3回発生→ヒアリング設計不備」を根本原因と特定し次案件でヒアリングテンプレv2 を導入。

### 2026-08-29 Daily Knowledge Log 追記
- **Linear 全社移行完了**：Notion カンバン運用から Linear へ 7 プロジェクト・412 Issue を移行、Cycle（2週間スプリント）運用開始。初 Cycle の完了率 78%（目標 85%）、未完了 22% の内訳は「見積過小 65%・仕様変更 25%・依存待ち 10%」と可視化、次 Cycle で Estimate 精度向上に注力。
- **Notion ADR 運用スタート**：意思決定 12 件を Markdown 化して Notion Database へ登録、決定者・日時・代替案・却下理由・見直しトリガーを必須項目化。半年前の「なぜ Vercel を選んだか」を新メンバー kuu が ADR 検索で2分で理解、口頭再ヒアリング（従来平均35分）が消滅。
- **モンテカルロで納期リスク早期発見**：Q3 大型4案件の総工数を過去実績から確率分布推定、P80 完了日が想定より12日後ろにあることが判明。CEO へ「現体制だと 68% 確率で10月末完了、95%確率での完了は11/12」と数値で報告、追加リソース1名（外注）投入判断を稟議、確率 P80→P92 に改善。
- **RACI マトリクス標準化 v1**：全プロジェクトの主要タスクに R/A/C/I を明示、Slack Workflow で「タスク作成時に RACI 未記入なら投稿不可」の強制化。定例MTGでの「これ誰の仕事？」議論時間が 平均12分/回→ゼロに、意思決定速度が向上。
- **OKR ツリー Q3 版展開**：全社O→部門KR→個人KRを Notion で紐付け可視化、週次で KR コミット率を Slack 配信。第2週コミット率 74%（目標 70%クリア）、遅れは KR3「原価管理システム β版9月末リリース」→ kai/nao/riku へエスカレーション、開発リソース1週再配分で回復軌道に。
