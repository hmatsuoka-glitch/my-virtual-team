...
### 2026-07-12
- **2026年潮流：Flow Metrics（WIP・Cycle Time・Throughput・Age of WIP）を横断ガント上で常時可視化し、Little's Lawで納期を確率的に予測**：これまで納期予測はガント積み上げの点推定で、遅延伝播時の再計算に時間がかかっていた。Kanban 4計量を全案件で自動計測し、Little's Law（納期≒WIP÷Throughput）でスループット基準の着地予測に切り替えると、リプラン時の再計算工数が2h→10分、納期予測精度が±5営業日→±1営業日に向上。稼働率単独でなくフローで捉えることで、07-01の横断クリティカルパス検知とも整合し「忙しいのに前進していない」状態を可視化できる。
- **先端手法：モンテカルロシミュレーションで納期の80%/95%信頼区間を出し、点推定の「◯月◯日完了」を確率分布に置き換える**：三点見積（06-20）は個別タスクの分散を扱えるが、案件全体の納期は依存関係とリソース競合で分布が歪む。全タスクの三点見積を入力に1000回モンテカルロを回すと「95%確率で7/25まで完了、80%なら7/20」と確率区間で提示できる。CEO/クライアントへの納期コミット（06-20の目標/予測/コミット3線）が「保証できる線」で伝えられ、コミット割れの早期起票率が20%→60%に向上する。
- **横断連携アップグレード：Kpi/Owl/Qa/Datの4連携を「週次同期パイプライン」に束ね、案件横断リスクを1画面のヒートマップで束ねる**：現行の連携（07-02）は各エージェント別に個別Slack Workflowで、PMが4件を手で統合していた。週次同期パイプラインでKpi稼働率＋Owl受注SLA＋Qaゲート待ち＋Datリスク優先案件を1つのダッシュボードに集約し「案件×リスク種別」のヒートマップで表示すると、リスク統合把握が30分→3分に短縮、上位3件集中（06-22）の判断速度も倍速化する。ハンドオフ4点セット（06-12）の未定義箇所も同画面で自動検出する。
- **意思決定支援：リスク登録簿を「発生確率×影響度×時間軸（残猶予日数の逆数）」の3次元スコア化し、Top 3リスクを自動抽出**：これまで06-22のマトリクス2次元だけで上位3件を選んでいたが、直近1週間で顕在化するリスクと月末まで猶予があるリスクが同列で並んでいた。時間軸を第3軸に追加し優先度を機械算出すると「今動くべきリスク」が自動浮上、リスク見落とし率が15%→2%、対応遅延起因の納期スリップも週2件→月1件以下に減少。07-03のクローズ条件・見直し日と組み合わせ、ステールリスクも自動降格する。
- **AI活用：週次ステータスレポート（3層構造・05-22）の生成をClaude/GPTに任せ、PMは差分と判断のみに集中する**：現在は各案件のstatus.jsonからPMが手動で「サマリ層・マイルストーン層・タスク層」に整形していた。LLMにstatus.json＋前週レポートを渡し「変化のあった項目を強調・順調時は先読みリスク開示を添え・クライアント側ToDoを冒頭定位置に固定（06-23）・専門用語を相手手番に翻訳（06-07）」の指示で生成させると、案件1件あたり15分→2分、生成品質も週次サンプリングで承認率90%を維持できる。
- **2026年標準：Basecamp Shape Up 2.0の「6週間サイクル＋2週間クールダウン」を継続改善型案件で全面採用し、Circuit Breaker運用でスコープクリープを構造的に予防**：SNS運用や継続改善型案件で06-20のQCD固定辺（時間/コスト固定・スコープ調整）を運用する具体形として、Shape Up 2.0の「Pitch→Betting Table→Building→Cooldown」サイクルを導入。プロジェクトバッファを別途積まず「6週間の予算を使い切ったら止める」Circuit Breakerで管理すると、スコープクリープ（06-17）が構造的に予防され、継続型SNS案件の炎上率が30%→8%に低減、クライアント期待値マネジメントも6週間リズムで安定化する。
- **効率化：Linear AI Triage＋Motion / Reclaim.aiでAI自動アサイン＋カレンダー最適化を全社リソース平準化に組み込む**：Linear AI TriageはIssue優先度を自動判定し、Motion/Reclaim.aiはメンバーカレンダーに応じてタスクを自動配置する。両者を連携させPMの「リソース平準化＋週次アサイン調整」の手作業を週5h→1hに削減。横断クリティカルパス（07-01）の検知結果を制約条件として渡すと、同一メンバーの複数案件競合を機械的に回避したアサインが生成され、稼働率ピーク（06-17）も予防的に平準化される。Kpiへのガードレール指標受け渡し（07-02）も自動化できる。
