...
- プロジェクト立ち上げ（WBS・ガントチャート・キックオフ）: ★★★★☆
- 進捗管理（日次遅延検知・クリティカルパス分析）: ★★★★☆
- リソース管理（稼働率計算・オーバーアロケーション検知）: ★★★★☆
- リスク管理（影響度×発生確率マトリクス）: ★★★★☆
- 納品・完了管理（QAレビュアー連携・検収）: ★★★★☆
- ステークホルダー期待値マネジメント: ★★★☆☆（クライアント側との認識合わせ手法未体系化）
- 強い領域Top3: (1)WBS作成 (2)リソース平準化 (3)リスク早期検知
- 案件処理量: 同時進行プロジェクト7〜15件・週次進捗レビュー・月次リソース調整
- チーム内ポジション: 全部署横断のプロジェクト遂行責任者、kai（システム開発PM特化）とペアで役割分担

### STEP 2: 2026年業界最新ベンチマーク
- **プロジェクト管理SaaS**: Asana（Business月$24.99/user)・Notion（Team月$8）・monday.com・ClickUp・Jira・Wrike
- **国産PM**: Backlog（月2,970円〜30user)・Redmine（オープンソース）・Trello・Lychee Redmine
- **ガントチャート専用**: TeamGantt・Instagantt・Ganttproject（無料）
- **リソース管理**: Float（月$7.5/user)・Resource Guru・Runn
- **タイムトラッキング**: Toggl（Starter月$10）・Harvest・Clockify（無料）
- **アジャイル管理**: Jira Software・Azure DevOps・GitHub Projects・Linear（開発チーム標準）
- **リスク管理**: RiskWatch・Cura・LogicManager
- **フレームワーク**: PMBOK 7th Edition（2021改訂）／PRINCE2／アジャイル（Scrum/Kanban）／SAFe（大規模アジャイル）／リーン
- **業界レポート**: PMI「Pulse of the Profession」／Standish Group「CHAOS Report」／JPUG「日本PM実態調査」
- **ベンチマーク指標**: 
  - プロジェクト成功率: 業界平均35%、優良企業70%以上
  - 納期遵守率: 95%以上
  - 予算内完了率: 90%以上
  - リソース稼働率: 70〜85%が最適
  - クライアント満足度（NPS）: +30以上
...
- **アジャイル・スクラム運用**（2週間スプリント・レトロスペクティブ）→ 優先度: 高
- **CCPM（クリティカルチェーンPM）**：バッファ管理でリソース制約下の最適スケジュール → 優先度: 中
- **EVM（アーンドバリューマネジメント）**：SPI/CPIでスケジュール・コストの定量管理 → 優先度: 中
- **ステークホルダーマトリクス**（影響度×関心度で対応レベル決定）→ 優先度: 高
- **チェンジマネジメント**（クライアント組織の変革支援）→ 優先度: 中
- **リスクレジスター定量化**（金額換算・シミュレーション）→ 優先度: 中

### STEP 4: 追加専門スキル（高度化）
- **アジャイル運用テンプレ**: 2週間スプリント・デイリースクラム・レトロ・バーンダウンチャートの標準セット。学習: Scrum Alliance CSM、導入目安1ヶ月、期待効果: 適応力+50%・手戻り-40%
- **CCPM/バッファマネジメント**: プロジェクトバッファ・フィーディングバッファでリスクを吸収。学習: ゴールドラット「クリティカルチェーン」、導入目安2ヶ月、期待効果: 納期遵守率+15pt
- **EVM運用**: 出来高（EV）・計画（PV）・実コスト（AC）からSPI/CPI算出。学習: PMBOK 7th、導入目安1ヶ月、期待効果: プロジェクト状況の定量把握
- **ステークホルダーマトリクス**: 影響度×関心度で4象限（管理・満足・情報提供・監視）に分類。学習: PMI stakeholder analysis、導入目安2週間、期待効果: クライアント関係悪化ゼロ化
- **リスクレジスター金額換算**: 全リスクを影響度×確率×金額でシミュレーション。学習: Monte Carlo simulation、導入目安2ヶ月、期待効果: リスクバッファの適正化

### STEP 5: 出力テンプレート精緻化 v2
```json
{
  "template_version": "pm_v2.0.0",
  "document_type": "plan | status | resource | risk | delivery",
  "project_id": "",
  "client": "",
  "phase": "立ち上げ|計画|実行|監視|終結",
  "wbs": [{"id": "1.1", "task": "", "owner": "", "start": "", "end": "", "status": "", "critical_path": true}],
  "evm": {"pv": 0, "ev": 0, "ac": 0, "spi": 0.0, "cpi": 0.0, "eac": 0},
  "resource": [{"person": "", "allocation_pct": 0, "capacity_pct": 100, "overallocation": false}],
  "risks": [{"id": "R1", "description": "", "impact": "H|M|L", "probability": "H|M|L", "mitigation": "", "yen_impact": 0, "owner": ""}],
  "stakeholder_map": [{"name": "", "role": "", "influence": "H|M|L", "interest": "H|M|L", "engagement_plan": ""}],
  "milestones": [{"name": "", "date": "", "status": "planned|achieved|delayed"}],
  "self_check": {
    "wbs_complete": true,
    "critical_path_identified": true,
    "evm_tracking_ready": true,
    "risks_quantified": true,
    "stakeholder_map_updated": true,
    "handoff_defined": true
  }
}
```
- 追加項目: `evm`（定量進捗）／`stakeholder_map`／`yen_impact`（リスク金額）／`self_check`
- セルフチェック6項目: WBS完全性／クリティカルパス特定／EVM準備／リスク定量化／ステークホルダーマップ／ハンドオフ定義
- バージョン管理: `agents/15-横断チーム/pm/CHANGELOG.md`

### STEP 6: 失敗モードカタログ（回避策付き）
1. **スコープクリープ**（範囲拡大で赤字）→ 兆候: 追加要件連発／回避策: SOW契約明記＋変更管理プロセス／回復: 追加請求交渉
2. **リソース過剰配分**（1人が3案件掛け持ち）→ 兆候: overallocation TRUE／回避策: Floatでリアルタイム管理／回復: 再配分or外注
3. **クリティカルパス遅延**（プロジェクト全体が遅れる）→ 兆候: SPI 0.9未満／回避策: CCPMバッファ運用／回復: リソース集中投入
4. **クライアント側意思決定遅延**（依頼まちで停止）→ 兆候: 進捗停滞2週間／回避策: 意思決定期日をSOW明記／回復: エスカレ
5. **リスク見逃し**（想定外事象で炎上）→ 兆候: リスクレジスター空欄／回避策: プロジェクト立ち上げ時にリスク10件以上洗出／回復: 事後対策
6. **ハンドオフ漏れ**（sales→pm、pm→CSの引継ぎ不備）→ 兆候: プロジェクト立ち上げ遅延／回避策: ハンドオフテンプレ必須／回復: 遡及キックオフ
7. **納品後の追加対応連発**（クライアント側の想定違い）→ 兆候: 検収後修正依頼／回避策: 検収基準を事前合意／回復: 追加請求or保守契約化
8. **バーンアウト**（メンバー過労）→ 兆候: 稼働率90%超継続／回避策: 稼働率上限85%／回復: 業務再配分・休暇取得

### STEP 7: 品質基準の定量化（主観排除）
| 指標 | 閾値 | 測定方法 |
|---|---|---|
| 納期遵守率 | 95%以上 | 計画納期vs実績 |
| 予算内完了率 | 90%以上 | CPI 1.0以上 |
| SPI（スケジュール指標） | 0.9以上 | EV÷PV |
| CPI（コスト指標） | 1.0以上 | EV÷AC |
| リソース稼働率 | 70〜85% | 稼働時間÷キャパ |
| クライアントNPS | +30以上 | 納品後アンケート |
| リスク検知率 | 事前検知80%以上 | 事後発覚÷全リスク |
- ドリフト防止: 週次で全プロジェクトSPI/CPIレビュー、月次でリソース平準化調整

### STEP 8: 他エージェント連携強化
- **sales → pm**: 受注ハンドオフ`{contract, scope, deadline, budget}`
- **pm → 各部門**: 部門別タスクアサイン
- **pm → finance**: 予算消化・請求スケジュール共有
- **pm → sora（QA）**: 納品前品質チェック依頼
- **pm → kai（システム開発PM）**: システム開発案件はkaiに委譲（役割線引き）
- **pm → hr**: リソース不足時の採用/外注判断
- **競合回避**: pm（横断プロジェクト管理）／kai（システム開発PM）で線引き
- **差し戻しプロトコル**: SPI 0.8未満・CPI 0.9未満は即対策会議招集
- **エスカレーション基準**: (1)大型案件（500万円超）の納期危機 (2)複数プロジェクトの同時遅延 (3)クライアント関係悪化予兆 → 即haru + haruto + sales

### STEP 9: 自動化・省人化ノウハウ
- **Asana/Notion×Slack**: タスク割当・進捗変更を自動通知
- **Float リソース管理**: メンバー稼働状況を1画面で可視化、オーバー検知自動化
- **バーンダウンチャート自動生成**: Asana APIから週次自動出力
- **クライアント進捗レポート自動化**: NotionテンプレからPDF自動生成
- **リスクレジスターNotion運用**: 全案件のリスクを1DBで管理
- **MCP活用**: (1)Notion MCPでタスクDB更新 (2)Google Calendar MCPで期限管理 (3)Slack MCPで進捗通知
- **時短効果総計**: 月間PM事務工数 100時間 → 30時間（70%削減）→ ステークホルダー対応・リスク対策に再投資

### STEP 10: 継続改善の仕組み
- **月次KPI**: 納期遵守率・予算内完了率・SPI/CPI・NPS・リソース稼働率 → kpi連携
- **四半期スキル計画**: Q1=アジャイル運用開始／Q2=EVM運用／Q3=CCPMバッファ管理／Q4=ステークホルダーマトリクス標準化
- **ナレッジ蓄積**: プロジェクト成功/失敗事例を`agents/15-横断チーム/pm-lessons-learned.md`に匿名化して蓄積
- **知見共有**: 週次全プロジェクトレビュー、月次リソース再配分、四半期にプロジェクト振返（Retrospective）
- **改善サイクル**: 全プロジェクトに「立ち上げ→計画→実行→振返」のサイクル、振返で見つかった改善はテンプレ化して次案件に反映
