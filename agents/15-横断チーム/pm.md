...
### 2026-08-30
- **Claude 5 系による "WBS 自動生成＋依存関係推定" を受注ハンドオフ時に常設**：Sales の受注情報と過去類似案件から Claude が WBS 初版（タスク分解・所要日数・FS/SS/FF/SF依存）を自動生成し、PM が調整するだけの状態にする。プロジェクト立ち上げ工数が2日→半日に短縮、初期WBSの網羅性も向上。
- **クリティカルパス（CPM）を自動計算＋変更検知する Notion 連携ダッシュボードを構築**：タスク進捗の変更を検知して CPM を毎日再計算し、変更があった場合のみ Slack 通知。手動でのCP更新工数が消え、遅延の早期検知が3日→当日に短縮。
- **リスク登録簿を "発動トリガー＋発動時アクション＋負担者＋判断者" の4項目必須化テンプレで運用**：Notion DB で必須フィールドバリデーションを設定し、4項目が埋まっていないリスクは登録できない仕組みに。リスク発動時の意思決定が0秒で始まる状態を担保。
- **クライアント検収を "Loom動画による説明＋オンライン検収書" のハイブリッド化**：機能確認をLoom動画（5〜10分）で先に見てもらい、その後30分のオンライン検収で実操作を体験してもらう二段構え。検収リードタイムが平均10日→3日に短縮、後日の「聞いていない」トラブルもゼロ化。
- **リソース稼働率を Notion + Google Calendar で自動集計＋可視化**：各エージェント・人間メンバーの案件アサインを Notion で管理し、Calendar と同期して稼働率をリアルタイム表示。オーバーアロケーションを月次→リアルタイム検知に切替、稼働率80%を安定維持。
- **建設業クライアント向け "LINE 進捗自動配信" を Bo と協働で常設**：正式記録は Notion、要点3行＋リンクは LINE 公式アカウントで自動配信する2チャネル運用を Bo で自動化。決裁者への到達率が30%→95%に向上、進捗確認電話も -70%。
- **プロジェクトポストモーテムを "納品後7日以内実施" で強制運用**：完了案件は必ずポストモーテム（うまくいったこと3件・課題3件・次回改善3件）を実施し、Notion ナレッジDBに蓄積。同じ失敗の再発を四半期で -50%、新規PM の立ち上げ期間も -30%短縮。
- **AI予測による "納期遅延リスクスコア" を週次更新**：過去100件の案件データを LightGBM で学習させ、進行中案件の遅延確率を週次スコアリング。リスク高案件への先手介入で納期遵守率95%→98%を実現。
