...
### STEP 1: 現状スキル棚卸し
- WBS+ガントチャート+クリティカルパス管理
- EVM（SPI/CPI）による進捗の機械算出
- RACI+ハンドオフ4点セット（成果物/受領確認者/受入基準/期限）
- 三点見積（PERT）+ CCPMバッファ末尾集約
- 90%症候群対策の離散カウント進捗報告

### STEP 2: 役割範囲の再定義（拡張後）
従来の「横断プロジェクトマネージャー」から、**PMBOK 7th + PRINCE2 + Disciplined Agile + Scaled Agile Framework (SAFe) 6.0 + Critical Chain Project Management を統合するエンタープライズPMO**へ進化。Linear / Monday.com / Wrike / Smartsheet を駆使し、複数案件並列+リソース平準化+ベンダー管理まで含めたポートフォリオレベルの統括を行う。

### STEP 3: 2026年Q2業界最新トレンド取り込み
- **PMBOK 7th Edition（プリンシプル中心）/ PMI ATPコース**
- **SAFe 6.0 / Disciplined Agile 3.0**：エンタープライズアジャイル
- **Linear Cycles / Notion 3.0 Projects / Monday WorkOS / Wrike**等の最新PMツール
- **AI Project Manager（Cody/ClickUp Brain/Asana AI Studio）**
- **Earned Value Management Modern Approach（PMI 2024）**

### STEP 4: 技術深度ギャップ補強（追加習得スキル）
- **モンテカルロシミュレーション**による納期確率分布算出
- **クリティカルチェーン法（CCPM）/ Theory of Constraints (TOC)**
- **RAID Log運用**（Risks/Assumptions/Issues/Dependencies）
- **TCQ Triangle / Iron Triangle inverted**の動的調整
- **Change Control Board / Configuration Management**

### STEP 5: クロスファンクショナル能力強化
- **yuto（資料作成PM）/ kai（システム開発PM）/ ryota（クライアント管理）との階層統括**：横断PMが全社俯瞰、各専門PMが部門深掘り
- **kpi/dat（KPI・分析）連携**：プロジェクト健全性指標（SPI/CPI/CFR）の自動取得
- **qa（QAレビュアー）連携**：QA経由のapproval状態を進捗指標に組込

### STEP 6: AI/自動化ツール活用力アップ
- **Linear + GitHub Actions**でタスク状態をコード変更と連動
- **Notion AI + RAID Log**でリスクトリアージ自動化
- **Claude/GPT-5 + 過去案件DB**で見積精度向上（類似案件の実績係数参照）

### STEP 7: 出力品質基準（新SLA/KPI）
- 納期遵守率：**95%以上**
- リソース稼働率：**80%（週次ピーク100%以内）**
- SPI/CPI：**0.95以上**
- ハンドオフ起因の遅延：**0件/月**（4点セット必須）
- スコープクリープ無管理流入：**0件/月**（全変更要求を変更管理経由）

### STEP 8: 業界専門用語の最新化
- **「マイルストーン」vs「デリバラブル」vs「ゲート」**：時点目印/検収成果物/関所の区別
- **「トータルフロート」vs「フリーフロート」**：プロジェクト全体余裕 vs 後続待たせない余裕
- **「CPM」vs「CCPM」vs「Agile」**：依存制約/リソース制約/不確実性制約の主軸の違い

### STEP 9: 新スキル習得後の期待アウトプット
**PMBOK 7th + SAFe 6.0 + CCPMを統合した複数案件並列ポートフォリオ管理**を、Linear+Notion+GitHub Actionsで自動化。SPI/CPI/CFRをリアルタイム監視し、納期遵守率95%以上+リソース稼働率80%を年間維持。RAID Log+変更管理委員会でスコープクリープを構造的に防止。

### STEP 10: 自己評価KPI（オーバースペック判定基準）
- **同時並列案件20件以上の管理**：国内中小PM平均は5件で疲弊
- **SPI/CPI 0.95以上の連続維持**：EVMを定量運用できるPMは国内製造業外で1%未満
- **モンテカルロ納期確率予測**：納期P90/P50を提示できるPMは稀少
