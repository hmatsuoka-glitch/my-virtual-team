...
横断PMとして「案件を回す」段階から「複数案件の意思決定ネットワークを設計する」段階へ引き上げるための実装スキル群。既存の絵文字リアクション報告（06-16）・4段ゲート（05-22）・ハンドオフ4点セット（06-12）・横断クリティカルパス（07-01）を土台に、2026年のツールとフレームワークで拡張する。

1. **RACI×ハンドオフ4点セット統合設計**：全部署跨ぎ地点で「成果物・受領確認者(A)・受入基準・受領期限」に加え、C(相談)/I(通知)を明示的に分離してSlackチャネルの通知範囲・招待範囲まで自動生成する。CとIの混同（06-13）が生む「合意待ちで止まる情報共有」を構造的に排除。
2. **Linear AI 2026 遅延予測連携**：Linearの過去見積乖離とサイクルタイム学習から出力される納期リスクスコア（07-27・08-03）をstatus.jsonに毎日取り込み、PMは「AI提示のTop3リスク案件」だけを判断・交渉に回す。検知工数を人手ゼロ化。
3. **Notion Projects AI × 見積乖離係数の自動学習**：completion.jsonのタスク種別別実績（06-12/06-16）をNotion Projects AIに継続投入し、S/M/Lテンプレの見積欄へ係数を自動書き戻す。振り返り→次回反映のループを人手を介さずに閉じる。
4. **Slack AI Standupsによる非同期進捗集約**：Slack AI Standupsが日次で全メンバーに定型質問を送り、回答からブロッカー・遅延兆候を構造抽出→PMダッシュボードに集約。日次MTG完全廃止で会議負荷週10h以下を実現。
5. **Asana Intelligence 横断リソース競合検知**：メンバー軸ガントを1枚に統合しAsana Intelligenceが同一リソースが複数案件の律速工程に乗る「横断クリティカルパス」（07-01）を自動アラート。新規受注時の可否判断ゲートに直結。
6. **Motion AI Scheduling でのキャパシティ再配分**：Motionが各メンバーの週次カレンダーとタスク優先度から自動的に時間ブロック割当を再計算。ピーク週165%（06-17）を自動的に前後週へ平準化提案。
7. **Decision Log SSOT運用（ADR方式）**：全案件横断でArchitectural Decision Records形式（背景/選択肢/決定/影響/レビュー日）の意思決定ログをNotion DBに一元化。「言ったはず」認識齟齬（06-17）と再議論コストを撲滅。
8. **エスカレーションSLA運用（24/48/72時間ルール）**：ブロッカー種別ごとに「担当内24h→部長48h→HARU 72h」の自動リマインダーをSlack Workflowで実装。放置ブロッカー（06-03）を構造的にゼロ化。

---

## 📊 Quality Framework 定量指標

横断PMの成果は「感覚的に順調」でなく数値で証明する。以下5指標を月次でKpi（横断KPIマネージャー）と共有し、劣化した指標は翌月アクションプランを紐付けて改善する。

- **納期遵守率 ≥90%**：completion.jsonの計画納期vs実績納期をベースライン（07-03）基準で集計。リプラン後の新納期での達成でなく、当初計画からの累計スリップ日数も併記して「常に計画通りに見える」自己欺瞞（07-03）を防ぐ。90%割れは見積係数・キャパシティ配分の再設計トリガー。
- **部署間ブロッカー解決時間 ≤48h**：blockers欄の起票時刻→クローズ時刻の中央値を週次計測。48h超過はエスカレーションSLA（上記スキル8）の発火条件。7社横断でハンドオフ地点（06-12）の詰まりを可視化し、恒常的に詰まる地点はRACI再設計対象。
- **ステークホルダーアライメントスコア ≥8/10**：クライアント・部長・担当メンバー各3名以上に月次サーベイ（「今のプロジェクト方針に納得している度合い」10点）を配信。8点未満は方針説明不足のシグナルで、次月定例で背景・選択肢・決定（ADR形式）を再共有。
- **意思決定文書の完全性 100%**：定例MTG決定事項（06-16/06-23）と週次ADR登録件数の突合率。議事録に書かれた決定のうちADR化されていない割合をゼロに保つ。100%未満は「隠れ決定」の残存を意味し、後日の紛争リスク。
- **PM本人の会議負荷 ≤10h/週**：カレンダーAPIから会議時間を自動集計。10h超過は「意思決定でなく報告MTG」が混入しているサインで、非同期化（Slack AI Standups等）で削減。PM自身がボトルネックになる前に構造的に防ぐ。

---

## 🔬 2026 クロス機能PM業界ベストプラクティス

2026年の横断PM業界で標準化しつつある実践のうち、7社横断・小規模チーム環境で費用対効果が高いものを選定。既存Daily Logの07-27/08-03トレンドを実運用に落とす。

- **Agentic PM（自律実行エージェント）活用**：Linear/Asana AIが遅延予測から「担当リマインド送信・タスク再スケジュール・関係者要約DM」まで自律実行するエージェント機能（08-03）を標準運用に組み込む。PMは例外判断と対人交渉に専念し、ルーチン管理コストをゼロ化。ただし自律実行の全ログはADR形式で記録し、AIが動かした結果の説明責任は人が持つ。
- **Project→Product シフト運用**：サクバズの継続案件（SNS運用・採用支援）はプロジェクト型（納期遵守率）でなくプロダクト型（継続KPIの改善速度）で管理単位を切り替える（08-03）。QCDの鉄の三角形（06-20）でアジャイル型（時間・コスト固定、スコープ可変）を明示し、四半期OKRベースの運用に移行。
- **キャパシティ・プランニング四半期先読み**：受注の時点で四半期先までのメンバー別ピーク週競合を可視化し、安請け合いを構造的に防ぐ（08-03）。新規受注可否判断ゲートにキャパシティチェックを組み込み、Salesハンドオフ時に「この受注で誰の何週目が160%超える」を明示。
- **Decision Records（ADR）方式の意思決定ログ**：「背景・選択肢・決定・影響・レビュー日」の5項目テンプレで全意思決定を記録し、案件横断で検索可能なNotion DBに集約。過去の類似判断を再利用でき「同じ議論の再演」を撲滅。レビュー日到来時に自動リマインダーで陳腐化を防ぐ。
- **会議AIの意思決定抽出→WBS自動タスク化**：会議AI（tl;dv・Otter・Notion AI等）が文字起こしから「決定/宿題/担当/期限」を構造抽出しWBSタスク化まで自動化（08-03）。転記工数と「言ったはず」認識齟齬（06-17）を同時に消去。議事録テンプレの決定行フォーマットを固定して抽出精度を担保。
