...
#### 1. AI-Augmented Risk Radar（AIリスク検知アシスタント）
2026年のPM業界標準となった「AI-Augmented PM」（05-25記録）を本チームに実装する。日次のstatus.json、Slack投稿、議事録テキストをClaude/GPTに投入し、5軸リスク（スコープ／スケジュール／リソース／意思決定／技術／06-11記録）のシグナルを自動抽出する。人間PMは抽出結果の妥当性判断と対応策選定に集中し、単純な「異常検知」の目視工数をゼロ化する。既存の「リスク5軸テンプレ＋ChatGPT壁打ち」（05-26記録）を発展させ、能動的な検知フェーズまでAIに委ねる。
- **抽出プロンプト定型**: 「以下のstatus.json/議事録から、5軸リスクの兆候を『軸／シグナル文言／推定発生確率／影響度／対応策候補3つ』でJSON出力せよ。根拠となる元テキスト箇所を必ず引用する」
- **確認プロセス**: PMは AI抽出リスクの「採用／却下／保留」を1件30秒で判定し、採用分のみrisks.jsonへ転記
- **サクバズ文脈**: 建設クライアント7社の週次進捗テキストは類似パターンが多く、AI検知の精度が上がりやすい

#### 2. Portfolio Kanban（全社横断ポートフォリオ可視化）
「横断クリティカルパス」（07-01記録）の発想を、リソース軸から案件軸へも拡張する。7社×全案件を「Discovery（提案中）／Ready（受注確定）／In Progress（実行中）／Review（QA中）／Done（納品済）」の5レーンで一枚のカンバンに配置し、CEO/HARUが30秒で全社状態を把握できる状態を作る。案件ごとのplan.jsonをこのポートフォリオビューに集約し、WIP制限（各レーン同時最大数）を設けて過剰並行を構造的に防ぐ。
- **WIP制限例**: In Progress ≤ 8案件（メンバー数×2）／Review ≤ 3案件（QAチームキャパ上限）
- **可視化項目**: 案件名／クライアント／規模（S/M/L・05-26記録）／納期／overall_status（05-22記録）／横断クリティカルパス該当有無

#### 3. WSJF優先度スコアリング（Weighted Shortest Job First）
複数案件のリソース競合（06-03記録）で「どれを優先するか」を勘でなく数値で決める仕組み。WSJF = (顧客価値 + 時間の緊急性 + リスク低減) / ジョブサイズ で各案件のスコアを算出し、上位から着手・下位は明示的に後回し合意する。従来の「先着順」「クライアントの声の大きさ順」をやめ、経営合理性でリソース配分する。
- **スコア入力**: 各要素を1-10で評価（Salesと合議）
- **反映先**: リソース平準化提案（プロセス3）と横断クリティカルパスの優先解消順序

#### 4. ステークホルダー・エンゲージメントマップ
各案件で「誰が意思決定に影響し、今どのレベルの関与状態にあるか」を4象限（Power×Interest）で可視化する。意思決定待ちの放置（06-03/07-01記録）を予防するため、「決裁者・推進者・キーマン・妨害者リスク」を明示し、案件ごとに「今週誰にどうアプローチするか」を計画へ組み込む。建設業クライアントは決裁ラインが複雑（社長・現場責任者・経理）で、キーマン把握の巧拙が意思決定リードタイムを左右する。
- **象限**: High Power/High Interest（キーパーソン）／High Power/Low Interest（要満足化）／Low Power/High Interest（要情報提供）／Low Power/Low Interest（要監視）
- **更新頻度**: キックオフ時＋月次見直し

#### 5. Async-First Communication Protocol
2026年トレンド「Async-First」（05-25記録）を本チームで運用化。同期会議（定例MTG）を月1回に絞り、日次進捗は絵文字リアクション報告（05-26記録）＋週次サマリーはLoom動画（5分以内）＋意思決定は非同期ドキュメント（Notion/Google Docs）で行う。7社×複数案件を抱えるPM工数の最大節約策として、会議時間を月20時間→月4時間に圧縮する。
- **同期MTG発動条件**: 意思決定リードタイム>48h／ブロッカー連続3日／クライアント緊急要請のみ
- **非同期ドキュメント標準**: 「背景／選択肢／推奨案／期限（デフォルト48h）／未回答時のデフォルト決定」を必須項目化

### 追加ツール・手法

#### 1. Linear AI Triage連携（自動優先度判定）
2026年4月にリリースされたLinearの「AI Triage」機能（05-25記録）を活用し、新規Issueの優先度を自動判定させる。PMは判定結果を「承認／変更／却下」の1クリックで確定でき、Issue triageの管理工数を40%削減。7社の並行案件で発生する日次Issue処理を、Botベースの機械処理＋PMのレビューだけに絞る。
- **連携先**: LinearのGitHub連携／Slack連携／Loom連携
- **代替**: Linear未導入クライアントは Notion Database + Claude API で同等機能を自作

#### 2. Claude Projects でプロジェクト・メモリ永続化
案件ごとにClaude Projectを作成し、plan.json / status.json / risks.json / completion.json / 議事録 / Sales約束事項リストをProject Filesとして格納。PMがClaude に質問すれば案件横断のコンテキストで即回答する。人事異動やPM交代時の引き継ぎコストをゼロに近づけ、「あの時なぜこう決めたか」の記憶ロスを構造的に防ぐ。
- **格納構造**: 1案件=1 Project、命名は`{client}_{project}_YYYY`
- **クエリ例**: 「宮村建設案件で先月ベースラインを凍結した経緯を教えて」「翔星建設のクリティカルパスに現在乗っているタスクは？」

#### 3. Miro/FigJam連携によるリモートキックオフ
規模別テンプレ（S/M/L・05-26記録）をMiro/FigJamのボードテンプレとして整備し、キックオフMTGをリモート・非同期で完結させる。WBS雛形・マイルストーン・体制図・リスク5軸を1つのボードで並行編集し、参加者は各自のタイミングで貼り付け→PMが集約。地理的に離れたクライアント（宮村建設・翔星建設等）とのキックオフ調整コストをゼロ化し、キックオフ所要時間を90分→30分に短縮する。
- **テンプレボード**: S案件用／M案件用／L案件用の3種を標準ライブラリ化
- **連携**: Miro→Notion/Linear への自動タスク起票（Zapier or Miro API）

### 追加出力フォーマット

#### portfolio_view.json（全社横断ポートフォリオビュー）
```json
{
  "updated_at": "YYYY-MM-DD",
  "wip_limits": {
  ...
  "projects": [
    {
      "project_id": "client_project",
      "client": "宮村建設|翔星建設|エスコプロモーション|cantera|ナワショウ|清一建設|桝本レッカー",
      "lane": "discovery|ready|in_progress|review|done",
      "size": "S|M|L",
      "due_date": "YYYY-MM-DD",
      "overall_status": "on_track|at_risk|delayed",
      "on_cross_critical_path": false,
      "wsjf_score": 0,
      "wsjf_rank": 0,
      "key_stakeholder": "決裁者名",
      "next_action_owner": "内部|クライアント",
      "next_action_due": "YYYY-MM-DD"
    }
  ],
  "cross_critical_resources": [
    {
      "member": "メンバー名",
      "concurrent_critical_projects": [],
      "peak_week": "YYYY-WW",
      "peak_load_pct": 0
    }
  ],
  "portfolio_health": {
    ...
    "at_risk_pct": 0,
    "delayed_pct": 0,
    "avg_wsjf_of_in_progress": 0
  }
}
```

#### wsjf_score.json（優先度スコアリング結果）
```json
{
  "scored_at": "YYYY-MM-DD",
  "scoring_committee": ["PM名", "Sales名", "HARU"],
  "scale": "1-10",
  "candidates": [
    {
      "project_id": "client_project",
  ...
  "resource_reallocation_actions": [],
  "next_scoring_date": "YYYY-MM-DD"
}
```
...
#### 3ヶ月目標（2026-10-20まで）
- **AI Risk Radar 全案件適用**: 全7社×全案件の日次status.json＋議事録をAI検知パイプラインへ流し、リスク検知の平均リードタイムを現状「発生後3日」→「発生前2日（先行検知）」へ短縮する
- **Portfolio Kanban v1 稼働**: 全案件を5レーン統合ビューで管理開始、WIP制限違反を週次でHARU/Salesへ可視化。CEO報告時間を月4時間→月1時間に圧縮
- **納期遵守率 95% → 97%**: AI検知＋ポートフォリオ横断視点で、事後リカバリーでなく事前予防型の運用へ完全移行

#### 6ヶ月目標（2027-01-20まで）
- **WSJF運用の全社定着**: Salesとの月次スコアリング会議を定例化、案件着手順を「勘・声の大きさ」から「WSJFスコア」ベースへ100%移行。低スコア案件の意識的なdefer/rejectで、稼働率のピーク超過を四半期通じて発生ゼロにする
- **Async-First Protocol 全案件標準**: 同期MTG時間を月20h→月4hに圧縮完了、非同期ドキュメント意思決定リードタイム中央値を48h以内に安定化
- **Claude Projects メモリ永続化 全案件展開**: PM交代・メンバー入れ替え時の引き継ぎコストをゼロ化、「なぜこう決めた」問い合わせを月10件→0件に

#### 12ヶ月目標（2027-07-20まで）
- **納期遵守率 99% 到達＆離脱率0件**: 予防型運用の完成形として、7社×全案件で年間納期遅延を1件以下に。同時に稼働率ピーク管理の徹底でメンバー離脱・体調不良ゼロを維持
- **横断PMのAI Co-Pilot化完了**: 日次進捗管理・リスク検知・議事録タスク化・見積係数反映を完全自動化し、PMは「意思決定・ステークホルダー対話・戦略調整」の3業務に100%リソース集中。1人PMで7社→12社を管理可能な体制へ拡張
- **サクバズ流「予防型横断PM」メソッドの外販化**: 本チームで検証済みの「AI Risk Radar × Portfolio Kanban × WSJF × Async-First」の4点セットを、建設業DX顧客向けのPM運用パッケージとして提供開始、新規収益源として月次MRR 100万円確立
