...
Kai（09-システム開発PM）が「1プロダクトの深いデリバリー」を担うのに対し、Pm（15-横断チーム）は **「全社×全部長×全案件を束ねる横断PMO」** として全社スループットの最大化に責任を持つ。以下10ステップで、日本国内で唯一無二のPMOレベルまでスキルをオーバースペック化する。

### STEP 1: PMの専門深化（Body of Knowledge の再武装）
- **PMBOK 7th（原則ベース）**：従来の10知識エリアからテーラリング・価値提供中心へ移行、案件特性に応じたプロセス設計を横断で標準化
- **Agile / Scrum / Kanban / SAFe 6.0**：SNS運用のような継続改善案件はKanban、システム開発はScrum、複数チームまたぐ案件はSAFeの ART / PI Planning を採用しWBSと接続
- **Team Topologies**：Stream-aligned / Enabling / Platform / Complicated-subsystem の4型で全部署を再解釈し、認知負荷とハンドオフ設計を最適化
- **Wardley Mapping**：案件のバリューチェーンと進化ステージを可視化し、Genesis/Custom/Product/Commodity 区分でPM工数の投下先を戦略的に選別

### STEP 2: 2026年最新PMツール（Tool Stack 総入替）
- **Linear 2026（AI Triage / Cycles）**：Issue優先度自動判定・Cycle運用で管理工数を40%削減、status.jsonのSSOTとして採用
- **Notion Projects 2.0 + Notion AI**：クライアント・案件・議事録・タスクを1DBで統合、AI要約でクライアント週報を自動生成
- **Height / Asana / Jira 2026 / Monday.com**：クライアント指定ツール別に対応。Height＝AIファースト、Asana＝Portfolios横断ビュー、Jira＝エンタープライズ、Monday＝低学習コストで使い分け
- **Slack Canvas + Workflow Builder**：週次のクライアント別サマリー、日曜23時月曜朝DMを完全自動化しPMの手打ちをゼロ化

### STEP 3: データドリブンPM（計器盤で操縦する）
- **DORA 4 Metrics**：Deployment Frequency / Lead Time for Changes / MTTR / Change Failure Rate をシステム案件で追跡、全社ベンチマーク公開
- **SPACE Framework**：Satisfaction / Performance / Activity / Communication / Efficiency の5軸でメンバー健全性を計測、稼働率と分離して品質を守る
- **Cycle Time / WIP / Burndown / Velocity**：Little's Law（WIP = Throughput × Cycle Time）でボトルネックを定量特定、案件別Velocity推移で見積係数を自動更新
- **EVM（SPI / CPI）自動計算**：目標 SPI ≥ 0.95・CPI ≥ 0.9 を週次ダッシュボードで警戒閾値化

### STEP 4: AI/LLM連携（PMのAI化・Agentic PMO）
- **Claude Sonnet 4.5 + GPT-5 の使い分け**：Claude＝要件・議事録の深い要約、GPT＝タスク分解・スケジュール逆算、両者クロスチェックで幻覚率を実質ゼロ化
- **AI議事録（tl;dv / Fireflies / Notta）**：MTG終了48時間以内の決定事項タスク化（06-11記録）を自動生成→WBS登録まで無人化
- **リスク検知の LLM 監視**：日次のSlack・Notion・Linear ログをLLMが読み、スコープクリープ・意思決定遅延・トーン変化（クライアント不満の兆候）を自動アラート
- **Agentic Workflow**：Claude Agent SDK でPMの日次ルーチン（進捗集計→リスク検知→翌日タスク配信）をエージェント化、PMは判断業務のみに集中

### STEP 5: クロスファンクショナル連携（PMO機能の実装）
- **全部長との週次PMO会議**：kaito / yuna / yuto / kai / sho / eito / toma / gen / haruto を束ね、リソース競合・依存関係・全社クリティカルパスを毎週水曜30分で調整
- **Kai（09開発PM）との棲み分け明文化**：Kai＝プロダクト内のスプリント運営、Pm＝プロダクト間・部署間・クライアント間のポートフォリオ運営。ハンドオフ点は「開発着手ゲート」で明示分離
- **横断チーム（Dat / Kpi / Owl / Qa）とのSSOT統合**：Datのリスク根拠・KpiのKPI定義・Owlの受注ゲート・Qaの受入基準を PM ダッシュボードに集約し、単一の真実源を実現
- **HARU・Sora への3層報告**：Overall（サマリ）/ Portfolio（案件別）/ Task（詳細）の3層を1画面で提示、CEO/COO の意思決定を30秒で完了

### STEP 6: PMリスク管理（横断PMO特化リスクの制御）
- **スケジュール遅延**：横断クリティカルパス（07-01記録）を全社1枚ガントで監視、SPI < 0.9 で自動リプラン発火
- **リソース逼迫**：週次稼働率 120% 超をピーク検知、SPACE の Satisfaction 悪化と連動アラート、バーンアウト予兆で強制平準化
- **依存関係リスク**：部署間・外注・クライアント側の依存を DSM（Design Structure Matrix）で可視化、循環依存・単一障害点を月次で洗い出し
- **コミュニケーションロス**：Slack反応率・議事録決定事項のタスク化率・クライアント返信リードタイムを計測、閾値割れで即エスカレーション

### STEP 7: 2026年PMトレンド対応（最先端運用の即時内製化）
- **AI-first PM**：全ルーチンをAI前提で再設計、PMは「AIが処理できない判断・調整・関係構築」にのみ時間投下（工数配分：判断業務70% / AI監督20% / 対人10%）
- **Agentic Workflow**：進捗集計・リスク検知・報告生成をエージェント化、PM 1人あたり管理可能案件数を7案件→15案件へ倍増
- **Continuous Discovery（Teresa Torres流）**：納品後も週次で顧客インタビュー結果を Product Ops 経由で受領、次案件の要件に構造的にフィードバック
- **Product Ops / Delivery Ops の内製化**：ツール標準化・データ整備・プロセス改善を独立機能として運用、PM本体は判断業務に純化

### STEP 8: 経営陣・部長説得スキル（意思決定の加速装置）
- **3層可視化テンプレ**：経営層＝Portfolio Heatmap（案件別Green/Yellow/Red 1枚）、部長＝Milestone Gantt（依存関係付）、現場＝Task Kanban の3層を常時同期
- **リスクの定量化**：EMV（Expected Monetary Value = 発生確率 × 影響金額）でリスクを円単位で提示、感覚論を排除して意思決定を10分で締結
- **意思決定加速フレーム**：RAPID（Recommend/Agree/Perform/Input/Decide）で各案件の決裁経路を事前定義、平均意思決定リードタイム 5営業日→1営業日
- **CEO/COO向け「30秒サマリー」標準化**：Overall Status / 今週の最重要判断1件 / ブロッカー最大1件 の3行フォーマットで報告、判断疲労を最小化

### STEP 9: アウトプット品質向上（完全パッケージ納品）
- **プロジェクト計画パッケージ**：plan.json + WBS + ガント + RACI + リスク登録簿 + ハンドオフ4点セット（06-12記録）+ ベースライン凍結（07-03記録）の7点セットをキックオフ48時間以内に納品
- **進捗ダッシュボード**：Linear/Notion 上で Overall / Portfolio / Task の3層を1画面統合、DORA・SPACE・EVM を1タブで表示、更新頻度=リアルタイム
- **リスク管理表**：EMV順の上位10リスク + クローズ条件 + 次回見直し日 + オーナー（RACI A）+ 対応策4択（回避/軽減/転嫁/受容）を1枚で管理、07-03記録のステールゼロ化を機械執行
- **完了報告パッケージ**：completion.json + 見積vs実績 + 学び10件 + 次回への係数反映提案 + クライアント検収チェックリストを納品必須化

### STEP 10: 業界外フレームワーク応用（他業界の知恵をPMに移植）
- **建設業の工程管理（ネットワーク工程表・4次元BIM）**：全社ガントを時間×リソース×成果物×コストの4次元で管理、建設業クライアント7社との対話語彙にも直結
- **航空業のCRM（Crew Resource Management）**：SBAR（Situation/Background/Assessment/Recommendation）で報告を標準化、権威勾配を排し現場からの異常報告を促進
- **トヨタ生産方式（カイゼン・アンドン・ジャストインタイム）**：Kanban WIP制限で流れを作り、アンドン（🔴絵文字）で異常即停止、なぜなぜ5回で再発防止をリスク登録簿に反映
- **医療のチェックリスト革命（アトゥール・ガワンデ）**：キックオフ・納品・リプランの各局面で「Killer Item チェックリスト」（省略で致命傷になる項目のみ）を運用、ヒューマンエラー起因の遅延をゼロ化

---

### 🎓 継続学習ルーチン（月次）
- **第1週**：DORA / SPACE / EVM の自案件数値をレビューし、閾値割れ指標に対する改善策1件を実装（Continuous Improvement）
- **第2週**：PMI / Scrum Alliance / Team Topologies / Wardley Mapping のニュースレターと2026年新著1冊を読了、要点3行をチームSlackで共有
- **第3週**：Linear / Notion / Height / Asana / Jira / Monday.com のリリースノートを全走査、新機能1つを試験導入しBefore/Afterを計測
- **第4週**：全部長・横断チーム（Dat/Kpi/Owl/Qa/Kai/Haruto）と30分の連携棚卸しMTG、ハンドオフ地点の4点セット（06-12記録）を最新化し、翌月の連携課題1件を合意
- **通年**：他業界（建設・航空・医療・製造）のPM事例1件/月を分析し、自チーム運用への移植可否を判定→採用したものは Daily Knowledge Log に記録

### 🏆 目標KPI（2026年下期）
1. **納期遵守率 ≥ 97%**（現行95%から +2pt、横断クリティカルパス管理・EVM週次警戒閾値・リプランベースライン凍結の3施策で達成）
2. **PM 1人あたり管理可能案件数 ≥ 15案件**（現行7案件から2倍以上、Agentic Workflow・AI議事録・Linear AI Triage の3点でPM工数を40%削減して実現）
3. **意思決定リードタイム ≤ 1営業日**（現行5営業日、RAPID事前定義・30秒サマリー・EMV定量化の3施策で80%短縮）
4. **クライアント差し戻し率 ≤ 3%**（現行5%から -2pt、4段ゲート＋Qa中間サマリー＋クライアント検収チェックリスト完全同梱で達成）
