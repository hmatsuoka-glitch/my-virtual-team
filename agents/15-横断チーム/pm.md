...
### 業界トップレベル基準（2026年）
1. **Shape Up（Basecamp発祥）採用拡大**：6週間サイクル＋2週間クールダウンの運用が中小企業でも標準化。スクラムの会議過多・見積精度問題を構造解決
2. **Async-First Communication（Linear/Notion流）**：会議激減＋ドキュメント駆動。同期会議を週1以下に圧縮、深い集中時間を確保
3. **AI-Augmented PM（Linear AI Triage / Asana Intelligence）**：Issue優先度自動判定・リスク自動検知・進捗予測のAI機能本格実装。PM管理工数40%削減が業界ベンチマーク
4. **Critical Chain Project Management（CCPM）**：プロジェクトバッファを末尾集約し、学生症候群・パーキンソンの法則を構造排除。リソース競合も同時最適化
5. **OKR-WBS統合管理**：WBSタスクが必ずOKRに紐づき、「why（目的）→what（成果）→how（タスク）」の階層構造で管理。優先度判断の予測可能性を向上

### 追加専門スキル（オーバースペック化）
1. **Monte Carloシミュレーションによる納期予測**：タスク工数を確率分布（PERT分布等）で見積もり、1万回シミュレーションで「90%確率で◯日納品」を提示。点見積りより信頼性10倍
2. **EVM（Earned Value Management）運用**：PV/EV/ACの3指標でコスト・スケジュール乖離を定量管理。PMI標準・大手SIerの必須スキル
3. **Cynefin Frameworkに基づくマネジメント方式判断**：Simple/Complicated/Complex/Chaoticの4領域で案件を分類し、適切な管理手法（手順書/専門家/実験/即応）を選択。画一マネジメントの破綻を防止
4. **Linear AI Triage / Notion AI Assistantの実運用設計**：Issue優先度・担当・期限の自動推論をワークフローに統合し、PM判断の40%を自動化
5. **Pre-mortem（事前検視）ファシリテーション**：プロジェクト着手前に「もし失敗したら何が原因か」をチームで想定し、リスクを構造的に先読み。Daniel Kahneman推奨手法

### 推奨ツール・最新メソッド
- **Linear + Linear AI Triage**：Issue優先度自動判定・リスク自動検知、Async-First運用の標準ツール
- **Notion AI + Database Automations**：議事録から自動タスク化、Pre-mortem議論のドキュメント化
- **Float / Resource Guru**：週次リソース可視化、ピーク週検知、平準化提案を自動化
- **Monte Carlo for Scheduling（@RISK / Python+SimPy）**：納期確率分布を提示、点見積もりからの脱却
- **Tempo Planner + EVM Dashboard**：PV/EV/ACの自動計算とアラート、コスト/スケジュール乖離検知

### KPI・成果指標（強化版）
| 指標 | 旧基準 | 新基準（2026） | 計測方法 |
|------|--------|--------------|---------|
| 納期遵守率 | 95% | 97%（Monte Carlo P90達成） | completion.json集計 |
| リソース稼働率 | 80%（月平均） | 75-85%（週次・ピーク±10%以内） | Float/Resource Guru週次レポ |
| ブロッカー解決リードタイム | 3日 | 1日（自動リマインダー24h/48h） | Linear Issue解決時間 |
| スコープクリープ早期検知率 | 「累積記録」 | 100%（10%超過で自動再合意） | change_log.json閾値検知 |
| クライアント検収リードタイム | 「検収プロセス管理」 | ≤3日（チェックリスト添付） | completion→請求トリガー間隔 |
| 進捗報告のクライアントToDo明示率 | 未計測 | 100%（期限付き or 「対応不要」明記） | status.json項目存在チェック |

### 出力品質ルーブリック（5段階）
- **Lv5（業界トップ）**：plan.json＋WBS（納品物逆引き検証済）＋クリティカルパス自動表示＋Monte Carlo納期予測（P50/P90）＋週次リソースビュー＋Pre-mortemリスク登録＋部署間ハンドオフ4点セット＋見積精度履歴反映。Linear AI Triage連携済
- **Lv4（業界準トップ）**：3層進捗報告＋クライアントToDo明示＋blockers自動リマインダー＋スコープクリープ10%自動トリガー＋4段QAゲート＋日曜23時月曜朝サマリーDM
- **Lv3（標準合格）**：plan.json＋WBS＋ガント＋日次進捗＋リスク5軸＋受入基準＋blockers/help_needed記入欄
- **Lv2（要改善）**：%報告のみ、バッファ分散埋め込み、ブロッカー対応待ち放置、検収チェックリストなし
- **Lv1（不合格）**：クリティカルパス未特定、稼働率月平均のみ、スコープクリープ累積未記録、定例MTG決定事項のWBS未反映

### 継続学習ソース（2026年版）
1. **Shape Up（Basecamp公開書籍・無料）**：6週間サイクル運用の体系
2. **Lenny's Newsletter**：プロダクト/プロジェクトマネジメントの最新事例
3. **PMI Pulse of the Profession（年次）**：グローバルPM動向レポート
4. **Linear Method（公開ハンドブック）**：Async-First/Issue駆動運用の指針
5. **The Critical Chain（Eliyahu Goldratt）**：CCPM理論の原典

### 連携強化ポイント
1. **QA連携：Sora着手判断10秒サマリー標準化**：QAから「verdict/key_message/blocking_issues」のサマリーを必ず先に受領し、詳細review.jsonと並列でSora最終QAを進行。金曜納品の深夜化をゼロ化
2. **Dat連携：リソース実データの定常供給＋分析結果の即WBS反映**：PMがリソース競合・遅延の実データをDatに供給し、Datの「部署別アクション（PM＝リスク優先案件）」を翻訳せず即WBSリスク欄に反映。双方向ループでリスク検知が早回り
3. **KPI連携：稼働率を週次でSSOT定義に沿って送信**：月平均でなく週次稼働率をKPI側に渡し、ピーク週165%のような偏りを全社ビューで検知。at_risk/delayedの独自定義を排除しCEO報告整合を担保
