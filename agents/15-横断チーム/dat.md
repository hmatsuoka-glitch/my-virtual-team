...
## 2026 横断データ活用ベストプラクティス（追補）

### Data Mesh / ドメイン責任分散
- 各部（02〜10）を「データドメイン」として扱い、生データの一次品質責任はドメイン部長に帰属させる。横断チームは統合・整合性のみを担う
- ドメイン間受け渡しは output.json スキーマで固定し、直接参照ではなくプロダクト化された成果物経由に限定する
- ドメイン別データオーナー一覧を四半期ごとに更新し、欠落・重複責任をゼロにする
- 部署横断KPIは「連邦型ガバナンス評議会（haruto＋sora＋dat）」で月次レビューし、単独判断を禁止する
- 中央集権的DWHへの依存を減らし、ドメインが自律的に自ドメイン分析を回せる状態を目標にする

### Data Contracts / データ契約
- 各エージェント output.json に schema_version・owner・SLA（更新頻度・鮮度・欠損許容率）を必須メタデータとして付与する
- 破壊的スキーマ変更は最低7日前に「contract_change_notice」で下流全エージェントへ通知し、承認なき変更は禁止する
- 契約違反（スキーマ不一致・SLA割れ）は自動検知し、下流分析を停止して haruto・sora へエスカレーションする
- 新規指標追加時は「定義・算出式・分子分母・除外条件」を契約文書に明記し、口頭定義を禁止する
- 過去契約は agents/data_analyst/contracts/ にバージョン管理で保持し、指標定義の遡及照会に応える

### Semantic Layer / 指標統一
- 「応募数」「CVR」「LTV」「着任見込み」など全社共通指標は semantic_layer.yml に単一定義で集約し、部署ごとの独自算出を禁止する
- 指標名は日本語正式名＋英語スラッグ（例: applications_count）の対で管理し、レポート・ダッシュボード・SQLで完全一致させる
- 分子・分母・時系列カット（week/month/quarter）・タイムゾーン（Asia/Tokyo固定）を全指標で明示する
- 指標変更は semantic_layer PR → sora レビュー → 全部長への影響通知の3段階を必須にする
- 「同じ名前で違う数字」を撲滅するため、Metabase・Looker Studio・BigQueryビューは全て semantic_layer 参照で構築する

### Modern Data Stack 運用（dbt Cloud / BigQuery / Metabase / Looker Studio）
- Raw層（各エージェント output.json 生取込）→ Staging層（クレンジング）→ Mart層（部署別・横断別集計）の3層構造を dbt で固定する
- BigQueryをDWHの単一真実の源とし、Snowflakeは要件発生時のみ検討（現時点でLET規模では過剰）
- Metabaseは社内探索・ad-hoc、Looker Studio はクライアント向けレポート、と用途を分離する
- dbt test（not_null・unique・relationships・accepted_values）を全 Mart モデルに必須付与し、CI で失敗したらデプロイ不可
- コスト管理として BigQuery スキャン量を週次モニタし、月$200を超えたらパーティション・クラスタリング見直しを haruto へ提案する

### Reverse ETL / データ利活用
- DWH に閉じ込めず「BigQuery → Airwork / SNS運用ツール / Notion / Slack」への逆流を Census 相当ツールで設計する
- チャーン予兆スコア・アップセル候補リストは自動でCS担当（該当エージェント）のNotionタスクとして起票する
- 建設クライアント別の着任見込み人数は毎朝8時に Slack #client-daily へ自動配信する
- Reverse ETL のジョブは「配信先・配信頻度・鮮度SLA・失敗時通知先」を semantic_layer と別ファイルで管理する
- 実務で使われない指標配信は四半期棚卸しで停止し、通知疲れを回避する

### Product Analytics / 行動データ分析
- LP・サイト・システム開発物には計測タグ設計を初期リリース前に必須化し、後付け実装を禁止する（07-LP部・09-システム開発部と連携）
- イベント命名規則（object_action、例: cta_click / form_submit）を統一し、GA4・自前計測を semantic_layer で正規化する
- ファネル分析（訪問→フォーム開始→送信完了）は全LPで標準テンプレ化し、CVRの相対比較を可能にする
- コホート・リテンション分析は月次自動生成し、離脱ポイントを LP改善タスクとして 07-LP部 saki へ渡す
- 定性データ（ヒートマップ・録画）との突き合わせを推奨する場合は、必ず個人情報保護法・GDPR観点で nori 事前関所を通す

### Data Governance / GDPR・個人情報保護法対応
- 個人情報を含むデータ（応募者氏名・電話・メール）は Raw 層でハッシュ化またはトークン化し、Mart層以降は識別子で扱う
- 保持期間は「応募者データ：採用終了後1年以内で削除」「クライアント統計：契約終了後3年」など指標別に保持ポリシーを定義する
- アクセス権はロールベース（RBAC）で最小権限とし、個人情報テーブルへのアクセスは監査ログを sora が月次レビューする
- クライアントとのデータ授受は必ず契約書に処理目的・保持期間・第三者提供有無を明記し、nori リーガル関所を通す
- 個人情報保護法改正・GDPR動向は四半期に一度 rui にリサーチ依頼し、社内ガバナンス文書に反映する
