...
### 2026-08-08
- **品質チェックポイント：小標本（n<30）の分析は「点推定を主役にせずブートストラップ信頼区間を必ず併記」する**。建設7社の月次データは各社n=1件、全社合わせても年間84件と小標本領域が常態で、正規近似の信頼区間は幅が過小に出て経営判断を歪める。1,000回リサンプリングのブートストラップで分位点ベースのCI（P2.5/P97.5）を出し、区間幅が点推定の±30%を超える場合は「参考値」ラベル（06-07記録）と検出力不足の明示（07-03記録）を必須にする。母数が小さい変化率の過大振れ（07-01記録）と同じ思想で、小標本での断定を構造的に予防する
- **効率化テクニック：季節性の可視化に「STL分解（Seasonal-Trend decomposition using LOESS）」を定型テンプレ化し、トレンド・季節・残差を1枚で示す**。建設採用は3月末繁忙・8月お盆閑散・年度末駆け込みの季節性が強く、素の月次数値だけで前月比を語ると「季節要因を施策効果と誤読」する事故が常態化する。statsmodels等でSTL分解を1コマンド化し、レポートには「季節調整済み系列＋残差の外れ値」を必ず出す。DID純効果（07-01記録）の対照群補正が組めない単発施策でも、残差の異常が施策時期と一致するかで純効果の当たりが付く
- **失敗パターン：Text-to-SQL/AIが返した集計を鵜呑みにする前段で、「AIに集計させる問い自体をメトリクスストア語彙に限定」しないと、指標定義の外の集計を勝手に組んでSSOTから外れる** → 回避策：AIへのプロンプトに「dbt Semantic Layer/data_dictionary定義の指標のみ使用可・未定義の集計はNULLを返す」制約を組み込み、生SQLは一切書かせない（理由：08-03記録の「メトリクスストア経由のガードレール」を実装レベルに落とすと、AIが「revenue_tax_inclusive」でなく「revenue」で自作集計を組む同名異定義事故が防げる。08-05記録のtoy検証・独立検算を最終ゲートに残しても、入り口の語彙制約がないと検証工数が跳ね上がる）
- **連携ヒント：Gen（建設業DX）連携で導入事例のROI試算を返す際、「事例値の適用条件」を統計的マッチング（傾向スコア）で明示する**。Genのn=1事例値をそのまま試算入力にする誤り（07-16記録）を一歩進め、事例クライアントと提案先クライアントの「規模・工種・原価管理成熟度・従業員数」の傾向スコアを算出し、マッチ度スコア（0.0〜1.0）を試算に添える。0.7未満なら「参考事例扱い（横展開適用条件チェック・05-27記録の4条件と同思想）」で提案書への数値転載を禁止し、Genが脱文脈引用（Genの06-24記録）できない構造にする
- **トレンド察知：回帰不連続デザイン（RDD）とベイズ構造時系列（CausalImpact）が「単発施策の純効果測定」で合成コントロール（08-03記録）の次の候補として実務投入されつつある**。RDDは「閾値で処理群/対照群が自然に分かれる施策（例：応募数◯件以上のクライアントに媒体最適化を実施）」で使え、合成コントロールが要求するドナープールの汚染懸念（08-05記録）を回避できる。CausalImpactはGoogleが公開したベイズ構造時系列で、対照群系列があれば単一クライアント施策の反実仮想を確率区間付きで返す。7社横断で対照群が組みにくい単発施策の効果検証に、DID→合成コントロール→RDD/CausalImpactの手法選択肢を意思決定型（06-23記録）に追加する
