...
### 現状スキル評価
定期分析・施策効果検証・顧客分析の分析設計は堅牢だが、データ基盤の実装層（dbt モデル化・DWH・オーケストレーション）が未整備で、CSV→スプレッドシート→手作業ピボットの循環に依存。統計検定は行うが Bayesian A/B や因果推論（DID/Synthetic Control）の実装ライブラリが個人ツール、機械学習モデルの本番運用（MLOps）は皆無。BI ダッシュボードの単一ソース化ができておらず、Kpi との数値定義の突合が手作業。

### 特定した改善余地
- 集計 SQL がスプレッドシート内 QUERY 関数と手貼りで、モデル化・テスト・バージョン管理がされていない
- DWH がなく、生データが Sheets・GA4・HubSpot・Airwork API に散在
- 因果推論の実装が Excel 手計算で、DID/Synthetic Control が再現性を持たない
- 予測モデル（LTV・チャーン・応募数予測）が Prophet 単発運用で、再学習・監視パイプラインが不在
- ダッシュボード配布が Slack PDF で、インタラクティブ探索ができない

### 追加スキル10選（オーバースペック化ロードマップ）
1. **dbt Core / Cloud モデリング** — 生データ→Staging→Intermediate→Mart の 3 層モデル、テスト付き / KPI: モデルテスト通過率 100%・データ品質SLA 99.5%
2. **Snowflake / BigQuery DWH 運用** — マルチソース統合、パーティション・クラスタリング最適化／KPI: クエリレイテンシ p95 < 8秒
3. **Fivetran / Airbyte でELT自動化** — HubSpot/GA4/Meta Ads/Airwork API を BigQuery へ日次同期／KPI: 同期成功率 99.9%、鮮度SLA 6h以内
4. **Dagster / Prefect オーケストレーション** — dbt run + Python ML パイプラインの依存管理／KPI: 定期ジョブ SLA 遵守率 99.5%
5. **CausalImpact + DoWhy 因果推論** — 施策効果の Synthetic Control 実装、DID を R/Python で再現可能に／KPI: 因果推論レポート月2本・意思決定への引用率 80%
6. **Bayesian A/B Testing (PyMC / Stan)** — 頻度論的検定より早い意思決定・確率ベース報告／KPI: 実験判定時間 -40%
7. **MLflow / Weights & Biases 実験管理** — モデル再学習・バージョン・メトリクス追跡／KPI: モデルドリフト検知〜再学習 24h以内
8. **Looker (LookML) / Metabase セマンティックレイヤ** — 指標定義の SSOT を LookML に集約／KPI: 指標定義の重複ゼロ
9. **Cohort Analysis + Retention Curve** — SaaS 型サービスの解約曲線・LTV推計を業種別に／KPI: 予測 LTV 精度 ±15%
10. **Statistical Power Analysis** — A/B テスト設計時にサンプルサイズと検出力を事前算出／KPI: 検出不能実験ゼロ

### 新規ナレッジソース・ツール
- dbt Cloud — SQL モデリング・テスト・ドキュメント・リネージュ
- Snowflake / BigQuery — スケーラブルなクラウドDWH
- Fivetran / Airbyte — 数百コネクタのELTパイプライン
- CausalImpact（Google 提供 R パッケージ） — Synthetic Control 因果推論
- Metabase — 非エンジニア向けセルフサービス BI

### 連携強化ポイント
- **Kpi (KPI Dashboard)**：dbt Mart 層を Kpi の SSOT データソースに一元化、指標定義変更は dbt テスト＋ADR で管理し Kpi・Marketing の対訳表と同期
- **Bo (業務自動化)**：BO 由来のジョブ実績を「業務悪化」と「取得失敗」で別ラベル受信、欠測フラグを dbt テストで検知
- **Pm (プロジェクトマネージャー)**：暦日換算の実効稼働率を、案件別の見積 vs 実績（分子・分母実数）を Pm から受領し dbt モデルで係数化、四半期更新

### 実践シナリオ例
翔星建設の TikTok 施策の因果効果検証。介入期間（TikTok 開始月）と対照群（同業4社の応募トレンド）で Synthetic Control（CausalImpact）を実装、6か月分の週次データで反実仮想を推定。結果、TikTok なしなら月応募12件→施策により月応募84件、うち純粋な介入効果は+58件（95%信用区間 41-72件、事後確率 99.6%）と算出。同時にコホート分析で「TikTok経由応募者」の6か月定着率が84%（他媒体67%）と有意に高いことを確認、LTV換算で1応募あたり¥180kの価値と算出。dbt モデルで自動更新化し、Looker で経営が任意時点で参照可能に。Kpi のマスタとも定義突合済み、Pr のリリース数値と一致。

### 2026-08-29 Daily Knowledge Log 追記
- **dbt Core 導入完了**：7クライアントの Airwork・GA4・HubSpot データを BigQuery に集約、dbt で Staging 42 モデル・Mart 18 モデルを構築。全モデルに not_null・unique・relationships テストを設定、初週で 3 件のデータ不整合（HubSpot Deal 重複・Airwork 応募日欠損）を自動検知→修正。従来は月次集計時に手作業で気付いていた品質問題を継続監視化。
- **CausalImpact で 翔星建設 TikTok 効果推定**：介入前後30週データで推定、純増応募 +58件/月（信用区間 41-72件）。Marketing 経由で「TikTok は月10件増」と概算されていた効果が実は5.8x大きいことが判明、経営会議で TikTok 投資枠を月¥600k→¥1.8M に増額決定。
- **Bayesian A/B で LP 差替判定を高速化**：Ren の LP コピーテストで、頻度論だと 4 週必要だった判定が Bayesian で 11 日で「勝率 96.2%」に到達、早期 Winner Declaration で新コピーを本番投入。実験サイクル 4週→11日で年間可能実験数が 12→33 に増加見込み。
- **Cohort Retention 分析で「TikTok 経由応募者」の高定着発見**：6か月定着率 TikTok 84% vs Indeed 67% vs Airwork 71%、有意差 p<0.001。LTV 換算で 1応募あたり ¥180k の価値、Marketing の CAC/LTV レポートに業種×媒体粒度を追加し、Sales の提案書にもエビデンスとして掲載。
- **MLflow で応募数予測モデル運用開始**：クライアント7社×週次で Prophet + XGBoost のアンサンブル予測を実装、実績乖離が ±25% を超えたら Slack 通知して再学習トリガー。従来は「勘」で来週応募数を Sales に伝えていたのが数値付きになり、Sales の営業計画精度が向上。
