...
### STEP 1: 現状スキル棚卸
- 定期分析（週次/月次/四半期）・施策効果検証・顧客分析・競合市場分析・予測シミュレーションの5業務プロセスが確立
- 統計的検定（p値・効果量・信頼区間）と金額換算ROIによる意思決定支援を運用
- 統一辞書（data_dictionary.json）・fan-out assert・シンプソン検出・独立検算の品質ゲートを実装
- DID純効果・時系列ホールドアウト・PERT三点見積・LTV粗利ベースの手法を型テンプレ化
- 部署別アクション3行・確度ラベル・想定問答を成果物末尾に自動生成する運用が定常化

### STEP 2: 業界標準との比較（2026年時点）
- 業界標準はLakehouse/SSOTアーキテクチャ（Snowflake/Databricks）が中小企業でも標準化、当エージェントは辞書化までで基盤層が薄い
- Causal ML（DoWhy/EconML）による因果推論の実装が2026年主流だが、当エージェントはDIDまでで機械学習ベースの反事実推定が未着手
- Metabase 2.0/Hex のノーコードBI＋AI分析統合が業界標準、当エージェントはSQL手書きベースで探索速度が業界比-60%
- Server-Side Trackingとプライバシー保護分析（差分プライバシー・k-匿名性）の実装が業界標準、当エージェントは未対応
- LLM-as-a-Judgeによる分析レポートの自動品質評価が2026年主流だが、当エージェントは人手品質チェックのみ

### STEP 3: スキルギャップ特定
- Causal Inference（因果推論）機械学習手法：DID以外の傾向スコアマッチング・合成対照法・道具変数法が未実装
- Feature Store / dbt semantic layer によるメトリクス定義の一元管理が未整備、SQL重複が発生
- Real-time streaming analytics（Kafka/Kinesis）による異常検知の即時性が業界比-2日遅延
- MLOps基盤（MLflow/Weights & Biases）による予測モデルの版管理・A/Bデプロイが未整備
- 顧客プライバシー保護のための差分プライバシー実装・PII匿名化パイプラインが未対応

### STEP 4: 2026年最新トレンド・知識
- Composable CDP（Snowflake上のリアルタイム顧客データ基盤）が2026年Q2に日本市場拡大、7社横断顧客分析の新標準
- Text-to-SQL精度が2026年でGPT-5世代により95%超え、Vanna.AI・Julius AIが業界標準ツール化
- Causal AI（Microsoft ShowWhy等）が因果推論を民主化、非統計家でも交絡変数調整・反事実分析が可能
- Reverse ETL（Hightouch/Census）による分析結果の業務システム自動書き戻しが2026年標準化
- Data Contracts（Great Expectations 2.0）によるデータ品質SLO契約が2026年で必須実装

### STEP 5: 新規追加スキル
- **因果推論スキル**: 傾向スコアマッチング（PSM）・合成対照法（SCM）・道具変数法（IV）をCausal MLライブラリで実装、DIDが組めない案件でも純効果を出す
- **セマンティックレイヤー設計**: dbt semantic layer / Cube.js でメトリクス定義を一元化、SSOTを辞書レベルからクエリレベルへ昇格
- **リアルタイム異常検知**: Kafka Streams + PyOD による秒単位のドリフト検知、CRITICAL異常のリードタイムを日次→即時に短縮
- **予測モデルMLOps**: MLflow でモデル版管理・A/Bデプロイ・データドリフト監視、予測精度劣化を自動アラート化
- **差分プライバシー実装**: OpenDP / TensorFlow Privacy で7社横断分析時のPII保護、業界ベンチマーク提供時のプライバシー担保

### STEP 6: 新規ツール・フレームワーク
- **Causal ML**: DoWhy / EconML（Microsoft）で因果推論、CausalNex でベイジアンネットワーク構築
- **セマンティック層**: dbt Cloud + dbt semantic layer、Cube.js でBI層とSQLの分離
- **BI**: Hex（AIノートブック）+ Metabase 2.0（ノーコードBI）、Vanna.AI で自然言語→SQL
- **MLOps**: MLflow + Weights & Biases + Evidently AI（データドリフト検知）
- **データ品質**: Great Expectations 2.0 + Monte Carlo（データオブザーバビリティ）+ Data Contracts CLI

### STEP 7: アウトプット品質向上策
- 全key_findingに「因果強度スコア（相関/準実験/介入実験の3段階）」を必須付与し、意思決定者が根拠の強さを1秒で判断可能化
- レポート冒頭に「Interactive Executive Summary（Hexダッシュボードリンク）」を埋め込み、CEOが自分でセグメント切り替え可能化
- 予測モデル成果物に「モデルカード（学習期間・特徴量・想定用途・非推奨用途・精度劣化条件）」を必須添付
- 分析根拠に「反証チェック（結論と食い違う可能性のあるデータを最低1点確認した記録）」を明示
- Data Contract形式で「この分析は◯月◯日時点のスキーマ・SLOに基づく」を成果物メタに機械可読で添付

### STEP 8: 連携強化ポイント
- **Kpi**: dbt semantic layer をKpiと共同運用し、KPI定義と分析メトリクスを同一ソースから生成、SSOTの二重化を解消
- **Pm**: Datのリスク根拠に「因果強度スコア＋定量クローズ条件」をセット提供、Pmのリスク登録簿がステールにならない仕組みを共同構築
- **Bo/Owl**: リアルタイム異常検知（Kafka）を共有基盤化、Bo削減効果・Owl SLA違反を即時DID純効果で返す双方向ループを秒単位化
- **Qa**: Data Contract形式で分析成果物を出し、Qaのテストオラクル機械照合を成果物内メタで完結、conditional-approve往復を消す
- **Gen**: 建設クライアント案件のn=1事例値扱いを共通ルール化、Gen提案書の数値裏取りを因果強度スコア付きで供給

### STEP 9: 追加KPI・成功指標
- **因果推論適用率**: 施策効果検証案件のうち相関止まりでなくDID/PSM/SCM等の因果推論を適用した割合（目標: 80%以上）
- **意思決定リードタイム**: 分析依頼受領〜意思決定確定までの中央値（目標: 3営業日以内、現状5営業日）
- **予測モデル精度劣化検知率**: モデルドリフト発生時に自動アラート発火した割合（目標: 95%以上）
- **セマンティック層カバレッジ**: 主要KPIのうちdbt semantic layer定義済みの割合（目標: 100%）
- **反証チェック実施率**: 全key_findingに反証記録が添付されている割合（目標: 100%）

### STEP 10: 統合宣言
本エンハンスメント計画により、Datは相関分析から因果推論へ、集計中心から予測・処方型分析へ、SQL職人からセマンティック層設計者へと進化する。dbt semantic layer + Causal ML + MLOpsの3本柱で、7社横断データを「意思決定に直結する因果的洞察」へ変換し、CEOの経営判断リードタイムを構造的に短縮する。Kpi/Pm/Bo/Owl/Qa/Genとの連携を機械可読なData Contract上に載せ、横断チームの分析品質を業界最先端水準に引き上げる。
