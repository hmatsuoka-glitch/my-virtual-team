...
### 2026-09-21
- **2026年最新トレンド：EWMA（指数加重移動平均）+ CUSUM 併用による異常検知の標準化**：単発の閾値超え（例：CVR前月比-20%）でアラートを出すと、季節性・少母数のばらつきで誤警報が増える問題に対し、EWMA（直近を重く見る移動平均）と CUSUM（累積偏差）を組み合わせて「継続的な変化」を検出する手法が中堅企業のダッシュボードで実装され始めた。Kpiの誤警報抑制（Kpi 07-01記録）と同思想で、DatはEWMA/CUSUMの窓長パラメータ（EWMAのα・CUSUMの警戒閾値）をクライアント業種の変動特性から算出し、7社ぶんを一括設定する運用に切り替える検討価値がある。
- **2026年最新トレンド：Causal Impact（時系列因果推論）+ Prophet の組み合わせで単発施策の純効果測定が実務化**：DID（07-01記録）・合成コントロール（08-03記録）に加えて、Google の Causal Impact ライブラリ（ベイズ構造時系列）で「施策を打った後の観測値」と「打たなかった場合の予測値」の差を可視化する手法が普及。7社の中で1社だけキャンペーンを打ったような場面（対照群が取れない）でも、他6社と過去自社データから反事実を組み立てられる。合成コントロールのドナー汚染リスク（08-05記録）との使い分け基準を明文化し、少母数の建設採用でどちらを既定にするかを判断する必要がある。
- **2026年最新トレンド：DuckDB + Iceberg + dbt Semantic Layer の「ローカル分析＋クラウド永続化」ハイブリッド構成**：DuckDB（インプロセスOLAP）で手元PCから直接クラウドのIcebergテーブルを読み、dbt Semantic Layer（07-27記録）で指標定義を統一する構成が中堅企業でも実現可能に。Snowflake等の従量課金を圧縮しつつ、統一辞書（05-27記録）思想を基盤側で担保。CTE分割＋materialize（06-16記録）の代替として、DuckDB のクエリキャッシュで探索的分析の試行錯誤サイクルを数秒に短縮する。
- **失敗パターン：EWMA/CUSUM の窓長パラメータをクライアント業種の変動特性を考慮せず全社一律で設定し、変動の大きいSNS運用と変動の小さい建設採用で同じアラート閾値になり、片方で誤警報・もう片方で見落としが同時発生する** → 回避策：窓長は業種別・指標別に「過去◯期の変動係数」から算出し、8社×主要10指標のマスタとして四半期バッチ（09-01記録）で更新する。Kpi との連携でパラメータを SSOT に置き、Dat と Kpi が別の窓長で判定して同じアラートが二重発火する事故を防ぐ。
- **失敗パターン：Causal Impact の予測区間を経営層に「施策の効果は95%の確率で+◯円〜+◯円」と説明し、ベイズの事後信用区間を頻度論的な信頼区間として誤って伝える** → 回避策：Causal Impact の出力は「ベイズ事後分布の95%信用区間」であることを明記し、経営向けには「効果ゼロの可能性が◯%程度残る」という頻度論的な言い換えでなく、事後分布そのものの意味（信念の分布）を確度ラベル（06-07記録）に置き換えて伝える。信頼区間と予測区間の混同（06-13/06-24記録）と同じ罠を、ベイズ手法の導入で再発させない。
- **品質チェックポイント：DuckDB でローカル分析した結果を経営報告に載せる前に、必ずクラウド側（Snowflake/BigQuery）で同一クエリを実行して数値一致を確認**：DuckDB の SQL 方言差・浮動小数点丸めの差・Iceberg 読み込み時のスキーマ推論差で、ローカルとクラウドで数値がわずかに乖離することがある。独立検算（06-17記録）と再現性チェック（06-26記録）の思想を、実行エンジン間の互換性検証にも拡張し、「ローカルで速く回してクラウドで正本確認」の二段構えを納品ゲートにする。
- **品質チェックポイント：AI アシスタント（Text-to-SQL / 分析エージェント）が生成した集計結果は、必ず「同じ問いを別の AI モデル（Claude / GPT / Gemini 等）で再生成」して SQL の構造・数値が一致するかクロスチェック**：単一AIの誤りを別AIが同じ誤りをする確率は低く、モデル横断の一致確認は toy データ検証（07-03記録）よりも安価に系統的誤り（08-05記録の JOIN 行膨張・GROUP BY 粒度）を検出できる。メトリクスストア経由（08-03記録）で AI に集計させる前提と組み合わせ、生成 AI の誤値を経営報告に載せない最後の網にする。
- **効率化テクニック：頻出する20問（部署別生産性・顧客セグメント別LTV・チャネル別ROI・解約予兆等）の問い集ライブラリ（06-23記録）を、DuckDB + dbt Semantic Layer で「Slackに問いを打つと集計結果と解釈テンプレが返る」形の Slack Bot 化する**：Slack Bot は問いのパラメータ（期間・対象クライアント・除外条件）をチャットで対話的に確定し、集計 SQL の生成→実行→結果表示→部署別アクション3行（05-26記録）まで自動生成する。Dat の稼働がなくても定型分析が回る構造にし、Dat は非定型2割の思考時間に完全に集中する。
- **効率化テクニック：四半期バッチ（09-01記録）に加え、月次バッチで「今月中に発生した AI 生成 SQL の誤検出ケース・独立検算での不一致ケース」を積み上げ、AI アシスタント（Text-to-SQL）のプロンプトチューニング材料として蓄積**：AI が誤りやすいパターン（税込税抜混在・カレンダー差補正抜け・シンプソン逆転見落とし）を月次で言語化して AI へのシステムプロンプトに追記すると、次月以降の誤検出率が下がる。品質検証パイプライン（07-07記録）で捕捉した不一致は「気付いて終わり」でなく AI のガードレール強化に還元する。
- **Kpi/Qa 連携の小ヒント：EWMA/CUSUM の窓長パラメータの SSOT 化は Kpi 側に置き、Dat は「窓長算出ロジック」と「四半期バッチでの再計算結果」だけを提供する役割分担にする**：Dat が値そのものを配る側に回らない原則（08-27記録）を EWMA パラメータにも適用し、算出ロジックの妥当性検証は Qa の 5系統カバレッジ（Qa 07-16記録）の異常系（境界値：変動係数がゼロや無限大に近い場合）でカバーしてもらう。Dat・Kpi・Qa の3者連携で異常検知の信頼性を担保し、単一エージェントの判断ミスがアラート運用全体を歪めない構造にする。
