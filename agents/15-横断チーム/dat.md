...
### 2026-09-10
- **[新スキル：dbt Semantic Layer + Cube.js の "指標定義 SSOT" 完全実装]**：07-27 記録のセマンティックレイヤー普及を実装フェーズへ。dbt Semantic Layer（v1.7+）で全指標をコードとして定義し、Cube.js の Rest/GraphQL API を Looker Studio・Notion・Slack Bot・Text-to-SQL AI から共通参照させる。従来「同じ CVR が BI とレポートで違う値」だった事故（05-27 記録）を基盤レベルで根絶。指標定義の変更は Pull Request 経由で Kpi・Finance レビュー必須にし、Slack への Diff 自動通知で全部門が変更を即認識する。
- **[新ナレッジ：Causal ML（因果 ML）と Uplift Modeling の 2026 年実装]**：DID・合成コントロールに続き、機械学習で個体別因果効果（ITE：Individual Treatment Effect）を推定する Causal Forest / DR Learner / Uplift Trees が実用化。「どの求職者に広告を追加配信すると応募確率が最も上がるか」を個別に推定でき、Marketing のカスタムオーディエンス（09-10 記録）へ ITE スコア上位層を自動配信する。09-09 記録の予測モデル・特徴量重要度の "相関≠因果" 事故を Causal ML で構造的に解消。
- **[効率化ルール：分析依頼 → 成果物の "10 分応答テンプレ" と Notion Database 集約]**：分析依頼を Notion Database で受け、① 問い（意思決定）・② 主指標・③ 対照条件・④ 期間・⑤ 必要サンプル・⑥ 出力形式・⑦ 期限、の 7 項目テンプレで受付。埋まっていない項目は 10 分以内に Datbot（Claude Sonnet 4.5）が追加ヒアリング。従来「依頼受付→意図確認 3 往復→分析着手」で平均 3 日だったリードタイムが、テンプレ化で 4 時間に短縮（92% 削減）。同一依頼の再現・監査もこの Database から辿れる。
- **[失敗パターン→回避策：AI 分析アシスタントの "指標定義ハイジャック" 事故]**：ChatGPT や Claude に生 SQL を書かせて集計させると、CVR の分母を「訪問数」でなく「セッション数」で計算する等、指標定義を勝手に解釈する事故が頻発。回避策は AI に生 SQL を書かせず、必ずセマンティックレイヤー経由（dbt Semantic Layer + Cube.js の GraphQL API）でのみ集計させる。プロンプトに「指標名は必ず data_dictionary から選択・独自解釈禁止」を Function Calling の Schema Constraint で強制する。
- **[連携強化：Kpi との "指標定義 CI/CD" 相互統合]**：Kpi の週次ダッシュボード指標・Dat の分析用指標を共通の dbt モデルで管理し、変更は GitHub PR で相互レビュー。Kpi の速報/確定区別（Kpi 06-03 記録）と Dat の確定期/未確定期区別（08-12 記録）を同じ状態タグ（`is_finalized` 列）で表現し、両部門が同じ真実を共有する。指標定義の食い違いによる報告齟齬をゼロ化。
- **[品質チェック：Sora QA 提出前の "分析成果物 7 点セルフ検品"]**：① toy データ期待値一致 ② 独立検算パス ③ 加重平均で集計 ④ 端点効果補正済み ⑤ 確度ラベル（◎/○/△）付与 ⑥ 限界・前提（limitations）本文冒頭 ⑦ 意思決定への推奨アクション（誰が・いつ・何分）まで到達、の 7 点を自動判定してから Sora へ回す。Sora は数値検証を再走査せず、意思決定への繋がりだけを見る役割分担。
