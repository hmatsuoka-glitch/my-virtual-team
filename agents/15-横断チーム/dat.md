...
### 2026-07-12
- **「因果推論（Causal Inference）」を標準装備：A/Bが組めない施策のROI検証をDoWhy / EconML / 合成対照法（Synthetic Control）で純効果算出**：07-01のDID（差分の差分）に加え、Propensity Score Matching・Uplift Modelingをツールセット化。「7社中3社にだけ施策を打った」ような非ランダム介入でも純効果を統計的に分離し、追い風・選択バイアスを差し引く。ROI過大計上率25%→3%、経営投資判断の見誤りを構造的に削減、Bo/Owl（07-02）へ返す削減実績の説得力も同時に強化。
- **「Metrics Layer / Semantic Layer（dbt Semantic Layer / Cube.dev）」導入で指標定義SSOTを機械化**：現行のdata_dictionary.json（05-27）を静的辞書からsemantic layerに置換し、指標定義（税込/税抜・月次/累計・週始まり曜日）をコードで一元管理して全ツール（BI・分析ノートブック・レポート・Reverse ETL）が同一定義を参照する状態にする。指標定義ズレによる横断分析破綻をゼロ化、Kpi連携（06-11）の定義突合工数15分/依頼→0分、断絶点補正（06-17）も定義バージョンで自動追跡。
- **Sora（COO/最終QA）連携：分析レポートをSoraへ渡す前に「AIリバースレビュー（Claude Code /code-review・LLM反対尋問）」で自分の分析に反証仮説を機械生成し、想定される『違う解釈』を先に潰してから納品する**：Soraの否定的チェックで差し戻される主因は「別解釈の余地が残る結論」。LLMに「この分析結果に対する反対解釈を3つ挙げよ」と問い、シンプソン逆転（06-12）・交絡・逆因果（06-17）の候補を先に潰した上で納品。Sora差し戻し率20%→2%、QA通過リードタイム2日→0.5日。
- **「Value of Information（EVPI／情報の価値）」ベースの分析工数配分：ROI試算に「この分析でどれだけ意思決定の期待値が改善するか」を含める**：Datが受ける依頼を「その分析なしで判断した場合の期待損失」「分析後の意思決定精度向上分」で金額換算し、EVPI < 分析工数コストなら「概算で十分」と回答する運用にする。低ROI分析（EVPI 5万円のために工数10万円）を構造的に削減、深掘り時間の20%を高ROI案件（因果推論・予測モデル）に再配分。
- **「AIエージェント委譲：自然言語→SQL→インサイト」ワークフロー**：Marketing/PM/CEOからの単純集計質問（「先週の翔星の受注件数は？」「宮村建設のCVRトレンドは？」）をClaude Agent SDK + Text-to-SQLに委譲し、Datは深掘り分析（施策効果検証・因果推論・予測モデル）に集中。LLM SQLはsemantic layer経由で実行し指標定義SSOT違反を構造的に防止。単純集計依頼の工数20h/月→0h、Datの純深掘り時間35h/月→55h/月に拡張。
- **2026年業界標準「Bayesian A/B Testing（ベイズ流A/Bテスト）」導入で覗き見問題（06-03）を根本解決**：頻度論のp値検定は事前サンプルサイズ固定＋終了日固定が必要でリアルタイム経営判断と相性が悪い。ベイズ流は事後確率（施策が優れている確率）を逐次更新でき、いつ止めても正しい意思決定が可能。「施策A>Bの確率が95%超で勝ち判定」で覗き見問題自体を消滅、A/Bテスト所要期間14日→7日、経営層への説明も「95%の確率で施策Aが効く」と直感的（06-20のp値誤解も同時に解消）に伝わる。
- **効率化：「自動インサイト検出（Automated Insight Detection）」で週次分析の異常値5Why深掘り（05-26）を先回り**：日次サマリーテーブル（06-16）上にAI駆動の異常検知（Isolation Forest + LLMによる仮説生成）を常駐させ、KPI乖離値が閾値を超えた瞬間にDatへ「乖離検出＋自動抽出した3つの候補要因＋関連セグメント＋棄却済み仮説スキップ済み（06-23）」がプッシュされる。異常発生を「気づく」時間をゼロ化、月次の見落とし異常値検出率40%→95%、深掘り着手を平均3日早期化。
