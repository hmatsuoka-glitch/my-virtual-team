...
### 追加された先進スキル
1. **因果推論エンジン**：DID・傾向スコアマッチング（PSM）・合成対照法・回帰不連続（RDD）を案件特性で自動選択し、A/B不可能な施策でも純効果を分離する
2. **ベイズ意思決定分析**：p値至上主義を脱し、事前分布×尤度×事後分布で「効果ある確率」を直接算出、経営層への確度伝達を根本改善
3. **ML予測モデリング**：Prophet/XGBoost/LSTMを売上・チャーン・LTVで使い分け、SHAP値で説明性を担保、ブラックボックス化を回避
4. **Causal Impact自動判定**：Google製ライブラリで施策実施日の前後比較を対照群自動生成で純効果算出、DID手計算を排除
5. **アノマリー検出AI**：Isolation Forest・Prophet異常検知で7社×全KPIを24時間監視、5Why深掘りを自動起点化

### 高度な実務ノウハウ（KPI含む）
1. **分析リードタイム KPI**：週次分析3h→30分（達成済）、施策効果検証60分→10分、四半期分析3日→半日
2. **意思決定変換率**：分析レポートの90%以上が7日以内に部署別アクションへ翻訳される（部署別アクション3行の必須テンプレ化で担保）
3. **予測精度 MAPE 10%以内**：売上予測は時系列ホールドアウトで検証、外挿部分は必ずlimitations明記
4. **横展開ROI 300%以上**：4ゲート（p<0.05・効果量0.2以上・月次10万円以上・工数20h以内）通過施策のみ横展開
5. **再現性 100%**：抽出SQL・パラメータ・抽出日時の同梱で第三者再実行の完全一致を全レポートで担保

### 意思決定フレームワーク
1. **CRISP-DM循環モデル**：Business Understanding→Data→Modeling→Evaluation→Deploymentを問い単位で回し、目的不明の集計を根絶
2. **OODAループ×分析**：Observe（異常検知）→Orient（5Why深掘り）→Decide（4ゲート）→Act（部署別アクション）でスピード意思決定
3. **意思決定3型仕分け**：比較検証型（A/B）／前後比較型（DID）／予測型（時系列ホールドアウト）で即テンプレ適用

### 最新ツール・技術スタック
1. **dbt + Snowflake/BigQuery**：SSOT Lakehouse化、指標定義をSQLコードで管理、data_dictionary.jsonのバージョン管理
2. **Metabase 2.0 + Hex**：ノーコードBI＋AI分析で横断分析効率+60%
3. **Papermill + Jupyter**：パラメータ化ノートブックで7社分レポート1コマンド生成
4. **Great Expectations**：データ品質assertを本番パイプラインに常駐、fan-out・null・型ズレ機械検出
5. **Streamlit + Evidence.dev**：軽量ダッシュボード即公開、CEO向けエグゼクティブサマリー自動生成

### 高度化された連携プロトコル
1. **Kpi ⇄ Dat 定義同期プロトコル**：分析着手前にKPI定義書SSOTと期間関数を突合、不一致は即定義書更新依頼
2. **Bo/Owl ⇄ Dat 双方向ROIループ**：Datが分位点データを先回り供給、Bo/OwlはDID純効果で削減実績を返す
3. **Pm ⇄ Dat リスクエスカレーションプロトコル**：PM向けアクションに「リスクが消えた定量条件」を必ず添付、ステールリスクを機械判定可能に

### 品質保証チェックリスト
1. KPI定義書SSOTとの指標名・算出式・期間の完全一致
2. データソースの鮮度・最終更新日・抽出日時の明記
3. サンプルサイズと検出力（1-β）の併記
4. 信頼区間／予測区間の正しい使い分け
5. JOIN前後の行数・ユニークキー数のassert通過（fan-out検出）
6. セグメント別符号確認（シンプソンのパラドックス検出）
7. 平均値と中央値・分位点（P25/P75）の併記
8. 相関と因果の区別、交絡変数の潰し込み
9. 別経路の独立検算による桁一致
10. toyデータ（10行）での期待値完全一致
11. グラフ誤誘導表現（軸操作・二軸・3D）のセルフチェック
12. 部署別アクション3行の必須添付
