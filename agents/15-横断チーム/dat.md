...
### 2026-09-07
- **dbt Cloud×Airbyte×BigQuery「モダンデータスタック」全面刷新**：Airbyte（120+コネクタ）でHubSpot/Meta/Google/TikTok/GA4/freeeを日次同期、dbt CloudでSSOT変換・テスト・ドキュメント化、BigQueryに集約する構成に転換。データ整合性チェック手動30分/日→自動0分、SSOT定義書との一致率100%達成。
- **顧客コホート分析「Kaplan-Meier生存分析」の導入**：クライアントLTV分析にKaplan-Meier生存曲線を適用し、業種別・サービス別・獲得チャネル別の解約リスクを可視化。SNS運用契約の中央値継続期間が18ヶ月と判明、Finance側のLTV算出精度向上、Sales側の高LTVセグメント特化にも直結。
- **建設業クライアント7社「Airwork×採用市場データ」統合分析基盤**：Airworkの求人パフォーマンス（Shun分析）と全国建設業求人市場データ（総務省・国交省API）をBigQueryで統合、市場相対でのクライアント競争力を毎週可視化。翔星建設・宮村建設等の採用戦略提案の説得力が飛躍的向上。
- **統計手法「因果推論×傾向スコアマッチング」を施策評価に導入**：単純A/Bテスト不可能な施策（LP改修・PR掲載効果等）にPropensity Score Matching+Difference-in-Differencesを適用、疑似実験デザインで因果効果を推定。施策ROI評価の精度+40%、CEO意思決定の説得力向上。
- **予測モデル「Prophet×LightGBM×SHAP解釈可能AI」の3層構造**：Prophet（季節性・イベント）でベースライン予測、LightGBMで残差学習、SHAPで特徴量寄与を可視化する3層構造モデルを実装。リード予測MAPE従来12%→6%、Marketing/Sales予算配分の意思決定精度向上。
- **AI-native分析「Claude API + Text-to-SQL」でアドホック分析瞬時化**：CEO/HARUのアドホック質問（「翔星建設の今月応募数を業種別で見せて」等）をText-to-SQL変換で即座回答、従来アドホック分析リクエスト30分/件→30秒に短縮。データアナリスト工数を戦略分析にシフト。
- **KPI連携：異常検知アラートの「原因仮説×推奨アクション」自動生成**：KPIエージェントのCRITICALアラートに、DatがClaude APIで「原因仮説1行＋推奨アクション＋担当エージェント＋期限」を自動付与、対応リードタイム2日→2時間の運用を安定化。
- **PM連携：プロジェクト進捗の「遅延予兆モデル」共同運用**：PM進捗データを学習データにLightGBMで遅延予兆モデルを構築、10日前予兆検知でPMに自動アラート。納期遵守率95%→99%達成見込み、リソース平準化にも直結。
