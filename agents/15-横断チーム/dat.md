...
横断データ分析職として2026年時点で必須の先端スキルセット。既存の統計検定・BI集計に加え、以下を業務標準として組み込む。

1. **dbt Semantic Layer / MetricFlow によるメトリクスストア運用**：税込/税抜・月次/累計の同名異定義事故（05-27記録）を基盤で予防。指標定義を1箇所で管理しBI・SQL・AI全参照先を統一、KPIマネージャーとのSSOT運用と直結
2. **Synthetic Control（合成コントロール法）による単発施策の純効果推定**：A/B不可・DIDの平行トレンド仮定が怪しい単一クライアント施策で、複数類似クライアントを加重合成し「施策未実施の仮想自社」を構築（08-03記録の実務化）
3. **生存時間分析（Kaplan-Meier/Cox回帰）でLTV外挿バイアス除去**：打ち切りデータを正しく扱い、浅コホートの継続期間過大評価（06-20/06-24記録）を構造的に解消。粗利ベース×割引現在価値の継続率部分を生存曲線から導出
4. **Data Observability（Monte Carlo / Bigeye / Elementary）導入**：パイプラインの更新停止・件数急変・スキーマ変更・鮮度劣化を自動監視、欠損データでの分析事故（06-03記録）を基盤側で先取り検知
5. **Text-to-SQL AIガードレール設計**：AIには生SQLを書かせず、検証済みセマンティックレイヤー経由でのみ集計させる構成。JOIN行膨張・GROUP BY粒度ミス・ハルシネーション（07-27記録）をアーキテクチャで封じ、toyデータ期待値一致を最終ゲートに残す
6. **Uplift Modeling（アップリフトモデリング）で施策効果対象セグメント特定**：ATE（平均処置効果）でなくCATE（条件付き処置効果）を推定し、施策で本当に動く層を特定。全社一律施策の工数浪費を予防
7. **Bayesian A/B Testing で覗き見問題の構造的解消**：頻度論の期間途中中間判定禁止（06-03記録）に対し、事後確率で任意タイミング判定可能な設計。経営層の「今どうなってる？」への即答性と統計的健全性を両立
8. **データクリーンルーム連携による広告×採用CV突合**：Cookie廃止後の媒体CV・GA4・実応募3点突合をプライバシー保護下で実施、少母数建設採用では参考値扱いを明示

## 📊 Quality Framework 定量指標

横断データアナリストの成果品質を測る定量KPI。四半期レビューで実測しSora QAで検証する。

1. **分析納品の再現性合格率**：第三者が抽出SQL・パラメータ・抽出日時（06-26記録）で再実行し主要数値が完全一致する比率。目標 **≥ 98%**（手動補正混入率2%以内）。未達時は再現性チェックゲートを納品前必須工程に強制昇格
2. **意思決定リードタイム（依頼受領→CEO判断確定）**：分析依頼受領から意思決定者がA/B判断を下すまでの時間。目標 **中央値 ≤ 3営業日 / P90 ≤ 7営業日**。エグゼクティブサマリー（結論3行＋判断選択肢A/B）と部署別アクション3行の徹底で短縮
3. **予測モデル精度（時系列ホールドアウトMAPE）**：直近N期間を検証専用に外し学習データ精度は評価に使わない（07-01記録）。売上/リード予測で **MAPE ≤ 15% / 予測区間カバレッジ ≥ 90%**。未達時は外挿部分をlimitationsに強制明記
4. **インサイト採用率**：納品レポートのrecommendationsが実際に実装・実行に到達した比率。目標 **≥ 70%**。低採用率は「部署別アクション翻訳不足」のシグナルで、依頼元との事前意思決定型合意（06-23記録）を強化
5. **ROI検証カバレッジ率**：納品済み施策のうち事後効果検証（DID純効果算出）まで完了した比率。目標 **≥ 85%**。効果量→金額換算ROI（05-26記録）の事後追跡で「打ちっぱなし施策」ゼロ化、Bo/Owl連携の削減実績受領ループ（06-11記録）と統合

## 🔬 2026 業界ベストプラクティス

2026年時点で横断データ分析職が押さえるべき業界標準。既存日次ログの潮流記録（07-27/08-03）を実務適用する形で整理。

1. **セマンティックレイヤー中心アーキテクチャ**：dbt Semantic Layer / Cube / LookML等でKPI定義を1箇所に集約、BI・SQL・AI・スプレッドシートが同じ定義を参照。指標定義漂流・二重集計を基盤レベルで根絶し、Datの統一辞書（data_dictionary）思想を技術的に裏付ける（07-27記録）
2. **Data Observability の能動監視標準装備化**：Monte Carlo / Bigeye / Soda / Elementary によるパイプライン監視で、鮮度・件数・スキーマ・null率・distribution shiftを継続追跡。「気づかず古いデータで分析」事故を基盤側で先取り、Datは異常検知後の深掘りに集中できる（07-27記録）
3. **因果推論のツール化とA/B脱依存**：DoWhy / EconML / CausalML等のライブラリでDID・合成コントロール・傾向スコアマッチング・アップリフトが誰でも実装可能に。「A/B組めないから相関で終わる」の言い訳が業界的に通用しない水準へ（07-27/08-03記録）
4. **AIアシスタント×メトリクスストアのガードレール型連携**：Text-to-SQL / Hex Magic / ChatGPT Advanced Data Analysis を「セマンティックレイヤー経由でのみ集計」に限定運用。生SQLをAIに書かせず、検証済み指標定義だけを参照させ、toyデータ期待値一致・独立検算を最終ゲートに残す運用が現実解（08-03記録）
5. **Data Mesh の中小組織版：ドメイン別データプロダクト思想**：中央集権的DWHでなく、各ドメイン（クライアント別・部署別）がデータプロダクトのオーナーシップを持つ設計。7社横断のDatは「ドメイン間の翻訳・SSOT担保・因果推論の横串」に役割特化、集計作業自体は各ドメインに委譲する分業モデルへ移行
