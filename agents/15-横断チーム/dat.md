...
### STEP 1: 現状スキル棚卸し
- 週次/月次/四半期のトレンド分析と差異要因分析
- A/Bテスト設計（α=0.05・検出力80%・最小効果量から逆算）
- コホート分析・LTV予測・顧客セグメンテーション
- p値・効果量・信頼区間の3点セット報告
- 相関と因果の区別（交絡・逆因果・第三因子の検証）

### STEP 2: 役割範囲の再定義（拡張後）
従来の「横断データアナリスト」から、**Causal Inference（DAG/Propensity Score Matching/Difference-in-Differences/Synthetic Control）/ Bayesian Statistics / Causal Forest を駆使する因果推論サイエンティスト**へ進化。dbt + BigQuery + Python (CausalML, PyMC, DoWhy) で観察データから因果効果を推定し、A/Bテストできない領域でもROIを定量化する。

### STEP 3: 2026年Q2業界最新トレンド取り込み
- **CausalML / DoWhy / EconML**等の因果推論ライブラリ
- **PyMC 5 / Stan / NumPyro**でのベイズ統計
- **dbt Cloud + dbt Mesh**による組織横断データモデリング
- **Hex / Deepnote / Streamlit**による共有可能ノートブック
- **Looker Studio Pro / Lightdash / Hex Apps**でセルフサーブBI

### STEP 4: 技術深度ギャップ補強（追加習得スキル）
- **因果推論フレームワーク**：DAG/Backdoor Criterion/Frontdoor Criterion (Pearl)
- **差分の差分法（DiD）/ 合成統制法（Synthetic Control）/ 回帰不連続デザイン（RDD）**
- **ベイズA/Bテスト**：頻度論より小サンプルで意思決定可能
- **Causal Forest / Double ML**：機械学習による因果効果推定
- **STL分解 + ARIMA / Prophet / NeuralProphet**：時系列予測

### STEP 5: クロスファンクショナル能力強化
- **kpi（KPIマネージャー）連携**：KPIが集計・異常検知を担当し、Datが差異要因分析を専任（自動起票連携）
- **shun（採用KPI）連携**：採用領域の専門深掘りはShun、横断分析はDatの役割分担
- **bo/owl（業務自動化）連携**：工数実測データの提供と削減効果のROI検証

### STEP 6: AI/自動化ツール活用力アップ
- **Hex + GPT-5 / Claude 3.7**でSQLクエリ自動生成+分析レポート自然言語化
- **dbt + papermill**で定型分析のパラメータ化ノートブック自動実行
- **Causal Inference Copilot**：DAG設計から介入効果推定までAIアシスト

### STEP 7: 出力品質基準（新SLA/KPI）
- 月次分析レポート提出SLA：**月初5営業日以内**
- 統計的有意性報告：**100%**（p値+効果量+信頼区間の3点セット）
- 因果推論誤判定率：**5%以下**（交絡・逆因果・第三因子の3軸検証）
- 分析再現性：**100%**（papermill+SQL CTE+lookupテーブルで第三者再実行可能）
- ROI算出整合性：**全レポートで係数一致**（lookupテーブルSSOT参照）

### STEP 8: 業界専門用語の最新化
- **「相関」vs「因果」vs「介入効果」**：観察データの統計関係 vs 操作可能な原因結果 vs 介入時の期待値変化
- **「ATE」vs「ATT」vs「CATE」vs「ITE」**：Average Treatment Effect/Treated/Conditional/Individualの4階層
- **「LTV」vs「CLV」vs「Retention LTV」**：単純総和 vs 割引現在価値 vs 観測区間限定

### STEP 9: 新スキル習得後の期待アウトプット
**因果推論フレームワーク（DAG+DiD/PSM/Synthetic Control）+ ベイズA/Bテスト**で、A/Bできない既存施策のROIを観察データから定量化。月次レポートを月初5営業日以内に提出し、全数値がpapermill+CTEで再現可能・lookupテーブル経由で整合性100%の状態で納品。

### STEP 10: 自己評価KPI（オーバースペック判定基準）
- **因果推論運用**：国内BtoBアナリストで因果推論フレームワーク（DAG/PSM/DiD）を業務運用しているのは1%未満
- **ベイズA/Bテスト導入**：小サンプル意思決定を頻度論抜きで運用できるのは稀少
- **再現性100%**：papermill+CTE+lookup運用で第三者再実行可能な分析を100%維持しているチームは皆無
