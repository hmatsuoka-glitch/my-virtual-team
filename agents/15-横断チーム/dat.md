...
### 2026-09-04
- **Kpi連携：乖離自動起票（Kpi 06-16記録）への深掘り返信で「セグメント別水準変化と構成比変化への要因分解」（09-02記録のシンプソン対処）をセットで返し、Kpiが対外・社内報告に転記する時点で単純合算解釈にならない状態で渡す**：Datが「効かなかった」と結論だけ返すと、Kpi側の要約から構成比の話が落ちて経営が誤判断する。全体増減の内訳（水準寄与／構成寄与）を分解表で添え、Kpiは分解済みの数字だけを対外に出す運用に揃える（理由：加重平均・08-12記録で集計が正しくても、解釈で構成比変化を落とすと施策是非の判断が反転する）
- **Pm連携：リスク登録簿の発動トリガー（08-27記録）に営業日ベースの持続条件を埋め込み、期間ユーティリティ（09-01記録）で確定期のみ判定する条件文で渡す**：「◯週連続で◯を超えたら発動」の◯週を暦日でカウントすると、お盆・GW・週休二日で営業日が抜けた週も1週として数え、実稼働では未発動レベルの状況でトリガーが引かれる。持続条件は営業日ベース窓と「未確定期は判定除外」を条件文に明記し、Pmが登録簿へそのまま貼れる形にする（理由：端点効果・08-12記録による誤警報がPmのリスク運用まで波及するのを実装側で潰す）
- **Qa連携：レポート納品前の内部整合機械照合（07-16記録）を通すため、本文・グラフ・エグゼクティブサマリーの全指標参照を単一lookup（06-16記録）経由に寄せ、手打ち数値をゼロにしてから提出する**：Qaは全出現箇所の相互一致を機械照合するため、1箇所でも手打ちで転記した数値が残ると差し戻しになる。テンプレ側にlookup参照タグを埋め込み、係数・実数値・注釈を全てタグ経由でレンダリングする構成にすれば、Datが提出前に自分で整合を回さずQaのオラクル照合に一発通過できる（理由：抽出SQL・パラメータ同梱／06-26記録と対で機能させ、Qaが再現性検証に集中できる状態を提出の前提にする）
- **Shun連携：採用×SNS領域の一次分析はShun／横断メタ分析はDatの分担（08-13記録）を維持するため、指標定義（税込/税抜・月次/累計・営業日正規化の有無）と対象期間の突合を月初1営業日目にDat側から先出しする**：定義突合が依頼ベース運用だと、月次レポート直前にShunと数字が二重化して発覚する事故が起きる。Datが月初にShunへ定義と期間を提示・了承を得てから両者が同期間の集計に入ることで、Kpiとの集計＝Kpi/深掘り＝Datの分担（06-11記録）と同思想でSNS集計＝Shun/横断メタ＝Datの境界を機械的に維持する（理由：後追い突合は数字が食い違ってからの巻き戻しコストが高く、月初先出しに寄せた方がShun・Dat双方の再集計工数がゼロになる）
- **Deng/Sora連携：Dengのデータ基盤側で立つ欠測フラグ・データ観測性アラート（07-27記録）をレポート生成時に自動取り込みし、アラート発生期間を含む分析は確度ラベル（06-07記録）を自動的に△（要追加検証）へ格下げしてからSoraへ回す**：Datが基盤側の異常を目視で拾って手動で注記すると転記漏れが起き、Soraの最終QAで確度△判定が空振りになる。Dengのアラート状態をlimitations欄へ機械転記し、Soraは確度ラベルと限界事項を見れば「この確度で意思決定してよいか」を機械的に判断できる状態にする（理由：欠損ゼロ埋め問題・09-02記録と同思想で、基盤側の異常が分析結論の信頼度に自動反映される状態を保ち、Sora最終QAの判断入力を安定させる）
