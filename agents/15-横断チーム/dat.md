...
### 2026-08-30
- **Claude 5 系による "分析コード自動生成＋レビュー" を BigQuery/DuckDB 上で常設**：分析依頼を自然言語で受け、Claude が SQL/Python を自動生成→ Dat が最終レビュー→実行する運用。定型分析（コホート・LTV・チャネル別ROI）の作成時間が半日→30分に短縮、ヒューマンエラーによる集計ミスも構造的にゼロ化。
- **因果推論（DID・Synthetic Control・Uplift Modeling）を四半期分析の標準ツールキットに常設**：施策効果の測定を単純前後比較から因果推論に格上げし、外部要因（季節性・競合施策・広告バジェット変動）を統計的に除去。「効いた／効かなかった」の意思決定精度を上げ、無駄な施策継続を四半期で3〜5件停止できる。
- **CausalPy/DoWhy による因果 DAG の可視化を全経営分析に添付**：分析結果に「何を因果と仮定したか（前提のDAG）」を明示し、Sora QA・経営会議での議論を根拠ベースに変える。因果と相関の取り違えによる意思決定ミスをゼロ化。
- **建設業7社の "採用データ横断ベンチマーク" を Rui と共同で四半期公開**：7社の応募数・CVR・面接歩留まり・入社率を匿名化してベンチマーク化し、クライアント経営者へ「同業他社比較」として提示。自社ポジションの相対把握が可能になり、施策改善の意思決定スピードが2倍に。
- **Looker Studio + BigQuery で "セルフサーブ分析基盤" を各部長に提供**：定型分析は各部長が自分で回せる形にし、Dat は非定型・因果推論・予測モデリングに集中。分析依頼の総数を月50件→月20件に減らしつつ、1件あたりの分析深度を3倍に。
- **顧客解約予測モデル（Churn Prediction）を月次で更新し、CS へ配信**：LightGBM/XGBoost で予測モデルを構築し、解約確率上位10社を月次で CS へ配信。予防的介入により解約率を年間で -30% 削減。
- **A/B テストの "サンプルサイズ計算＋事前登録" を Marketing 案件で標準化**：施策実施前に効果量・α・β から必要サンプルサイズを計算し、Notion に事前登録する運用にする。P-hacking・後知恵検定によるフォールスポジティブを構造的にゼロ化し、施策の学びを正しく次に活かす。
- **Time Series Forecasting（Prophet/Neural Prophet）で受注・応募の3ヶ月先予測を月次更新**：Finance のキャッシュフロー予測、Sales の受注予測、HR の採用計画に予測値を供給し、経営会議での見通し精度を上げる。予算実績乖離が -50% に改善。
