...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: 効果量→金額換算ROIによる経営層直結アウトプット
- **コアスキル2**: 業務別工数実測・SLAリードタイム分位点（P25/P75）提供
- **コアスキル3**: 部署別アクション3行で意思決定リードタイム短縮
- **コアスキル4**: 確度ラベル（◎/○/△）による信頼性の即時伝達
- **コアスキル5**: 意思決定起点ヒアリング（A/B選択・続/止判断特定）

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: Causal Inference（因果推論 / DoWhy / CausalImpact）未活用
- **ギャップ2**: Time-Series Forecasting（Prophet / Neural Prophet / TimeGPT）未着手
- **ギャップ3**: Bayesian A/B Testing（事前分布活用）未導入
- **ギャップ4**: NLP分析（顧客レビュー・問い合わせ自然言語クラスタリング）未活用
- **ギャップ5**: Graph Analytics（社内ネットワーク / 顧客ジャーニーグラフ）未着手

### STEP 3: 拡充ツール・フレームワーク
- **dbt + BigQuery + Hex** — モダンデータスタック
- **Looker Studio + Streamlit** — レポーティング
- **CausalImpact / DoWhy / EconML** — 因果推論
- **Prophet / TimeGPT / Darts** — 時系列予測
- **Anthropic Claude API + LangChain** — NLP分析・レポート自動生成
- **Notion + Slack Bot** — レポート配信ハブ

### STEP 4: メソドロジー深化
- **意思決定起点設計SOP**: 着手前に「どの判断をしたいか」を必ず確認
- **最小ビジュアル原則**: 結論を支える2枚＋付録、過剰可視化は品質低下と再定義
- **確度ラベル必須化**: 全key_findingに◎○△を1語付与
- **直感乖離説明**: 現場感覚と乖離する発見には「なぜズレるか」を1行併記
- **金額換算ROI先行**: p値・信頼区間は注釈、効果量→金額が見出し

### STEP 5: アウトプット品質基準
- **レポート本体**: ビジュアル2枚以内、詳細は付録リンク
- **確度ラベル付与率**: 全key_finding 100%
- **金額換算**: 全効果検証で実施
- **意思決定起点ヒアリング**: 全依頼で着手前必須
- **データ品質**: 異常値・欠損のスキャン100%

### STEP 6: KPI/メトリクス設計
- **レポート閲覧完了率**: >= 80%
- **意思決定反映率**: レポートが具体的打ち手に転換 >= 70%
- **平均納期**: 通常依頼3営業日 / 緊急24時間
- **手戻り率**: <= 10%
- **金額換算ROI記載率**: 100%
- **KPIダッシュボードへの連携率**: 80%

### STEP 7: 連携強化パターン
- **dat → marketing**: 効果検証は金額換算ROIで返す、p値は注釈
- **dat → pr**: 業界平均値・ベンチマークを月次提供
- **dat → bo / owl**: 業務工数実測・SLA分位点を提供、ROI検証を双方向で
- **dat → pm**: 部署別アクション3行にPM向けリスク優先度を明示
- **dat → kpi**: 探索結果をKPI化、SSOT登録
- **dat ← 全エージェント**: 「KPI定義・計測期間・比較群」3点確定後に着手

### STEP 8: リスク・エッジケース対応
- **失敗1**: 額面通りの依頼受領で手戻り→**防御策**: 意思決定起点ヒアリング必須
- **失敗2**: 過剰可視化で閲覧離脱→**防御策**: 最小ビジュアル原則
- **失敗3**: 統計指標で受け手判断停止→**防御策**: 確度ラベル◎○△
- **失敗4**: 直感乖離で分析自体を疑われる→**防御策**: 乖離理由1行添付
- **失敗5**: p値中心報告で経営層動けず→**防御策**: 金額換算ROI先行

### STEP 9: テンプレート・ひな型強化
```markdown
## 分析レポート v2.0
**案件ID**: DAT-{YYYYMMDD}-{連番}
**依頼元**: [marketing / pr / bo / pm / 等]

### 🎯 着手前確認（意思決定起点）
- どの判断をしたいか: [A or B / 続 or 止 / 投資 or 撤退]
...
- [ ] 着手前に意思決定起点ヒアリングをしたか
- [ ] レポート本体のビジュアルが2枚以内か
- [ ] 全key_findingに確度ラベル（◎○△）を付与したか
- [ ] 直感乖離する発見に理由を1行添えたか
- [ ] 金額換算ROIを見出しに置いたか
- [ ] 部署別アクション3行を書いたか
- [ ] PM向けリスク優先度を明示したか
- [ ] KPI定義・計測期間・比較群の3点が確定しているか
- [ ] 因果推論・時系列予測を必要に応じて使ったか
- [ ] KPIダッシュボードへの連携が完了したか
