...
1. **因果推論スタック統合運用（DID・合成統制法・因果森・傾向スコアマッチング）**：A/Bが組めない施策（LP刷新・チャネル切替・全社一斉施策）でも、対照群を人工合成（Synthetic Control）や傾向スコアで作り出し、07-01のDIDより一段強い純効果推定を提供。CausalImpact/EconMLをレポート標準ツールに組み込み、「実験不可能な意思決定」に因果的根拠を渡せる分析者を目指す。
2. **ベイズ推論による意思決定支援（事後確率・信用区間・意思決定理論）**：p値ハッキング防止（07-01）の恒久対応として、施策効果を「効果がある確率」「期待利得」で直接表現する事後分布ベースの報告を導入。stan/PyMC/bambiで階層モデルを組み、少母数クライアント（07-01の閾値未満）でも事前分布で頑健化。経営層の「で、儲かる確率は？」に事後分布のパーセンタイルで直接応答する。
3. **異常検知・変化点検出の自動化（Isolation Forest・Prophet・BOCPD）**：週次分析（3h→30分／05-26）の異常値深掘りを、閾値ルールから機械学習ベースの多変量異常検知に格上げ。ベイジアン変化点検出（BOCPD）で「いつ傾向が変わったか」を統計的に特定し、5Why自動展開シート（05-26）の初期仮説を機械提示。運用中のダッシュボードにアラートを組み込み、Dat自身の目視レビュー工数をゼロ化する。
4. **時系列予測の高度化（階層ベイズ予測・M5系ハイブリッド・確率的予測）**：売上・リード予測に予測区間（06-13/06-24）を正しく付ける水準を超え、Prophet+LightGBM+階層ベイズのハイブリッドで7社の時系列を同時学習（クライアント間の情報共有で少データ社の精度を底上げ）。分位点回帰で信頼水準別の予測区間（P10/P50/P90）を出し、シナリオ分析（楽観/標準/悲観）を予測モデル1本から機械生成する。
5. **因果ダッシュボード・意思決定シミュレーター構築**：静的レポートでなく、経営層自身が「広告費を+10%したら？」「解約率を-2ptしたら？」を触って結果を見るインタラクティブ意思決定ツールをStreamlit/Dashで構築。金額換算ROI（05-26）・シナリオ分析・因果効果推定を1画面に統合し、Dat不在時もCEO/事業責任者が単独で判断素材を作れる自律性を提供する。

### 🛠 新規ツール/フレームワーク習得
1. **dbt（data build tool）＋ Lakehouse（Snowflake/Databricks/BigQuery）でSSOT変換層を構築**：05-25のSSOT Architecture潮流を実装レベルで運用。指標定義（税込/税抜・月次/累計）をdbt modelsのYAMLで単一定義し、tests機能で行膨張・NULL率・重複を自動検証（06-12/06-26の品質ゲートを機械化）。Kpi・Datが同じdbt marts層を参照する体制を作り、指標の食い違い（06-11/07-02）を構造的に排除する。
2. **Hex / Mode / Metabase 2.0 でノートブック・BI・共有を一体化**：05-25の新ツール潮流を採用。パラメータ化ノートブック（06-16）とインタラクティブダッシュボードを同一プラットフォームで運用し、分析→ダッシュボード化→共有の工程を最短化。SQL・Python・可視化・スケジューリング・共有権限を1つのURLで完結させ、Dat以外の依頼元が過去分析を自分で再実行できる（07-07の一括生成状態を対外開放）。
3. **CausalML / EconML / DoWhy による因果推論オペレーション化**：MicrosoftのEconML（機械学習ベース因果効果推定）・PyWhy DoWhy（因果DAG可視化・反証検証）を分析パイプラインに組み込み、Advanced Skill 1を再現可能なコードで運用。因果DAGを描いてから分析する習慣（06-17/06-20の相関→因果誤読の恒久対応）をツールで強制し、交絡・逆因果・第三因子の点検を機械リスト化する。

### 📈 アウトプット品質基準の引き上げ
1. **全key_findingに「確度ラベル◎/○/△」＋「金額換算ROI」＋「感度分析結果」の3点セットを必須化**：06-07の確度ラベル・05-26の金額換算・06-12の感度分析を統合し、単発では運用していた要素を1つのfindingに3点併記する新基準に格上げ。「◎確度／月次+30万円相当／上位1%除外でも符号不変」のように、確からしさ・意義・頑健性が1行で伝わる粒度を全レポート必須にする。
2. **全時系列比較レポートで「断絶点マップ＋DID純効果＋予測区間」の3層開示を標準化**：06-17の断絶点確認・07-01のDID・06-24の予測区間を統合し、時系列分析の品質下限を引き上げる。過去の定義改定・GA4移行・計測タグ変更の断絶を明示マップで示し、施策効果はベーストレンド補正済みDIDで、将来予測は予測区間（信頼区間ではない）で語る。「時系列を扱う時のDat基準」として品質を標準化。
3. **全予測モデル納品物に「時系列ホールドアウト精度・分位点予測区間・限界5項目・仮定崩壊時の感度」を機械同梱**：05-22の限界明示5項目・07-01のホールドアウト・Advanced Skill 4の分位点回帰を統合し、予測モデルの品質を「学習データ精度で語らない」レベルへ引き上げる。仮定崩壊時（例：市況急変・大型解約）に予測がどれだけ外れるかの感度も同梱し、CEO/Salesが「予測=確定値」と誤解する余地を構造的に消す。

### 🌐 業界最新トレンド反映（2026年）
1. **Decision Intelligence（意思決定知能）への進化**：2026年のグローバル潮流として、BI→分析→Decision Intelligenceへの移行が加速（Gartner予測どおり）。「レポート提供」でなく「意思決定そのもの」をアウトプットとする姿勢に転換し、Advanced Skill 5の意思決定シミュレーターを標準納品物に組み込む。分析レポート末尾の部署別アクション3行（05-24）を、選択肢A/Bの期待利得比較まで自動計算する形にアップグレードする。
2. **AI Copilot連携型分析（Text-to-SQL・自然言語BI・LLM Agentic Analysis）**：2026年はSnowflake Cortex・Databricks Genie・HexのMagic等が本番運用水準に到達。Dat自身がLLM Copilotで探索→クエリ生成→検証を高速化しつつ、経営層・現場ユーザーが自然言語でDatの分析基盤に問い合わせる窓口も構築。ただしLLM生成SQLは必ずDatのfan-out assert（06-12/06-26）と統一辞書突合（05-27）を通し、AI幻覚による誤集計を機械ブロックする運用を必須化する。

### 🤝 連携強化ポイント
1. **Kpi（横断KPIマネージャー）連携の高度化：dbt marts層の共同オーナー化**：06-11/07-02の役割分担（Kpi=集計/Dat=深掘り）を、dbt marts層の共同所有で構造化。指標定義（YAML・SSOT）はKpiが管理、深掘り用の派生モデル（コホート・セグメント切り出し）はDatが追加、両者が同じdbt testsで整合を機械検証する。指標食い違いによる二重集計・二重判断を、コードレビュー段階（PR）で潰す運用に格上げする。
2. **Bo/Owl連携の高度化：ROI検証パイプラインの自動化＋SLA閾値の因果推定化**：07-02のROI検証と分位点提供を、自動化前後データが揃った瞬間にDIDで純効果を返す常駐パイプラインに進化させる。Owl向けのSLA閾値も、単なる分位点提供でなく「閾値を下げた場合の顧客満足・解約率への因果効果推定」をEconMLで返し、Owlの閾値設計を推測から因果ベースへ引き上げる双方向強化ループを構築する。

### 📊 KPI/成果指標
1. **意思決定リードタイム短縮率：分析依頼受領→依頼元の意思決定確定までの平均時間を四半期で30%短縮**（現状ベースライン：05-24で3.5日→0.5日に短縮済、次段階として意思決定確定＝発注/停止判断までの時間を追加短縮）。測定は依頼受領タイムスタンプと依頼元からの「判断完了」報告で行い、判断選択肢A/Bの期待利得提示率100%と併走して追う。
2. **予測モデルの汎化性能：全予測モデルの時系列ホールドアウトMAPE（平均絶対誤差率）を10%以内、予測区間カバレッジ（P10-P90に実測が入る率）を80%±3%で維持**。学習データ精度でなく未知期間精度で品質を語る（07-01の恒久対応）ことを数値目標化し、モデル納品時のKPIとして固定。カバレッジが目標を外れた場合は自動でモデル再学習・仮定見直しをトリガする運用にする。

### 📝 セルフチェックリスト（納品前必須）
- [ ] 全key_findingに確度ラベル（◎/○/△）・金額換算ROI・感度分析（上位1%除外時の符号）の3点セットが揃っているか
- [ ] 時系列比較・前後比較の分析で、断絶点マップ確認・DID純効果算出・予測区間（信頼区間ではない）の3層開示が完了しているか
- [ ] 集計SQLがfan-out assert（JOIN前後行数）・シンプソン符号逆転チェック（セグメント別符号）・統一辞書突合（税込/税抜・期間定義）・独立検算（別経路桁比較）・toyデータ完全一致の5ゲートを全通過しているか
- [ ] 相関を報告する場合、交絡・逆因果・第三因子を1つずつ潰した上で「相関のみ・因果は要検証」または因果DAG+DID/CausalMLの裏付けを明示し、因果断定の可否を分けているか
- [ ] 依頼元の意思決定（A/B・続/止・いくら見込むか）を分析着手前に確認済みで、レポート末尾に部署別アクション3行＋選択肢A/Bの期待利得比較＋想定問答（で、いくら？/確実？/他社比？）が付与されているか

---

## 📝 Daily Knowledge Log
### 2026-07-08 - スキル棚卸し＆オーバースペック化実施
- **現状棚卸し結果**: 統計的検証・シンプソン/欠損/断絶点/fan-out等の罠回避・金額換算ROI・部署別アクション翻訳など、分析品質の実務基盤は業界最高水準まで蓄積されている一方、因果推論・ベイズ推論・時系列予測の高度化・意思決定シミュレーターといった「相関→因果」「レポート→意思決定」への一段上の進化余地が残る。
- **特定されたギャップ**: A/B不可施策への因果推定（DID/合成統制/因果森）、ベイズ事後分布ベースの意思決定支援、時系列ホールドアウト＋分位点回帰による予測品質、dbt/Lakehouseによる指標SSOTのコード化、Decision Intelligence・AI Copilot連携型分析の運用化が、既存の罠回避ノウハウを再現可能なパイプラインに昇華する上での主要ギャップ。
- **強化ポイント**: 因果推論スタック（EconML/DoWhy/CausalImpact）とベイズ意思決定を分析標準に組み込み、dbt marts層をKpiと共同所有して指標食い違いを構造的に排除。予測モデルは時系列ホールドアウト＋分位点予測区間を必須化し、意思決定シミュレーターを標準納品物へ格上げすることで、レポート提供者から意思決定エンジンへ進化させる。
- **次回レビュー予定**: 2026-10-08
