...
### 2026-08-30
- **Claude 5 系による "多モデル合議 LLM-as-a-Judge" を対外納品前ゲートに常設**：GPT-5・Claude 5・Gemini 3 の3モデル合議＋人手キャリブレーション（月次）で判定バイアスを分散させ、単一評価者の偏りを構造的に排除。判定精度が人手一致率85%→95%に向上、レビュー工数も -60%削減。
- **要件からの逆引きレビューを自動化する "要件↔成果物 traceability matrix"**：Notion DB で要件と成果物の対応を全リンク化し、対応記述のない要件を自動で blocker 検出。欠落項目の検出漏れを構造的にゼロ化し、レビュー品質を上げる。
- **合計＝内訳の縦整合を自動照合する "reconciliation script" を標準ツールキット化**：Python + Pandas で内訳の総和・構成比合計・小計積上げを機械照合し、丸め差の明示処理まで含めて自動検証。整合性チェックが手作業10分→3秒に短縮、内訳漏れによる納品事故をゼロ化。
- **承認の"有効期限＋依存断面" を Notion + Airtable で自動管理**：verdict に「承認時点で依拠したオラクル版数・依存出力の断面」を必須フィールド化し、依存側更新時は承認自動失効→再レビュー発火。陳腐化した承認を使い回す事故をゼロ化。
- **建設業向け業務システムの "実機検証プリセット" を7社共通標準化**：直射日光下コントラスト・手袋タップ間隔・通信断復帰後の入力保持の3プリセットを標準ツールキット化し、案件ごとの条件再設定を撤廃。blocker 級欠陥の検出深度を全案件で均質化。
- **AI ハルシネーション裏取り自動化：出典 URL・数値の一次ソース照合を Claude で自動実行**：AI 生成物に含まれる出典 URL の実在性・引用数値の一次ソース照合を Claude が自動チェック。ハルシネーション検出漏れによる対外リスクをゼロ化、裏取り工数も -80%削減。
- **QAカバレッジを "本番データ分布ベース" で運用**：正常系サンプルでなく、本番データから外れ値・欠損・境界値・表記揺れをサンプリングしてカバレッジ分母を作る運用に統一。カバレッジ率の見かけ倒しを撲滅し、本番稼働後の障害率を -70%削減。
- **QA自体の品質指標（メタQA）を四半期モニタ**：escape rate（納品後発見バグ数）・レビュアー間キャリブレーション一致率・AI判定と人手判定の一致率の3指標を四半期モニタし、QA品質そのものを継続改善。標準ツール化して他部署にも展開できる状態にする。
