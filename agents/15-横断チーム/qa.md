...
### 現状分析（As-Is）
- **役割の明確性**: 中間QA（整合性チェック・スキーマ検証）としてSora（COO最終QA）と役割分担が確立。5軸共通基準＋6軸クロスチェック＋5系統カバレッジがフレームとして機能。
- **自動化度**: JSON Schema自動validation・Slack絵文字→review.json自動生成・固有名詞マスタ突合の3点で機械化が進行。人手判定は「機械が判定不能な曖昧さ」に集中する分業モデルが確立。
- **連携範囲**: sora / Kpi / Dat / Pm / Gen / Mio / Bo・Owl / nori / 制作部との連携ヒントを蓄積。ただし7社（エスコ・cantera・ナワショウ・宮村・清一・桝本・翔星）別のリスクプロファイルは未体系化。
- **弱点**: (1) LLM-as-a-Judge運用は認知済みだが自組織パイプラインへの実装が未整備、(2) escape rate計測が月次に留まり本番テレメトリ連動が弱い、(3) AI生成物のセキュリティQA（OWASP LLM Top 10）は概念整理止まりで実行チェックリスト未整備、(4) 承認オラクル版数管理が手動、(5) 建設業クライアント特有の「現場条件プリセット」がテストデータ資産として蓄積されていない。

### 改善余地・成長余地（Gap）
1. **AI生成物QAの実行フレーム未整備**: OWASP LLM Top 10（LLM01 Prompt Injection〜LLM10 Model Theft）を成果物種別テンプレへ組み込み、SNS投稿・LP・提案書ごとの敵対的テスト観点が未整理。
2. **Evalsデータセット管理の欠落**: 合格閾値だけ守っても分母（評価データセット）のドリフト監視が四半期棚卸しの単発運用で、鮮度指標の定量化が未着手。
3. **シフトライトQAループの未完成**: 本番オブザーバビリティ（Vercel Analytics・Sentry・GA4）連動が手動集計で、escape rate自動計測→5軸チェックリスト即反映のクローズドループ未実装。
4. **レビュアー間キャリブレーション頻度不足**: qa×soraの独立レビュー一致率測定が四半期1回止まりで、観点解釈ブレの早期是正サイクルが遅い。
5. **建設業DX特化テストプリセット未資産化**: 現場条件（直射日光・手袋操作・通信断復帰・現行帳票再現）が案件別再定義になっており、7社共通の実機検証プリセットとして固定化されていない。
6. **QA自体の品質指標（メタQA）の可視化不足**: escape rate・偽陽性率・平均レビュー時間・往復回数・conditional-approve比率のダッシュボード化が未実装。
7. **多言語・アクセシビリティQAの空白**: WCAG 2.2 AA準拠・スクリーンリーダー対応・キーボード操作のQA観点がLP/資料の成果物テンプレに未組込。

### 追加習得スキル（New Skills）
1. **OWASP LLM Top 10適用QA**: Prompt Injection（直接／間接）・Sensitive Info Disclosure・Insecure Output Handling・Excessive Agencyの4項目を最低限、成果物種別テンプレへチェック項目化。AI生成LP・SNS投稿・提案書の敵対的テストを標準化。
2. **LLM-as-a-Judge合議運用**: Claude Opus 4.7 + GPT-5 + Gemini 2.5 Proの3モデル合議＋人手キャリブレーションで、5軸評価のうちaccuracy/consistencyの一次判定をAI合議へオフロード。判定不一致案件のみ人手レビューへ回す。
3. **本番テレメトリ連動escape rate自動計測**: Vercel Analytics・Sentry・Airwork運用ログから「QA通過後7日以内の不具合検知」を自動集計し、月次→週次へ計測サイクル短縮。
4. **WCAG 2.2 AA準拠チェック（axe-core自動監査）**: LP部・資料作成部の成果物にコントラスト比4.5:1・タブ順序・alt属性・ARIAロールの機械チェックを組込み、conditional-approve未然防止。
5. **回帰テストのAI自己修復連携**: Playwright + healenium等のself-healing testを導入し、UI変更起因のロケータ破損は自動修正・「壊れるべきリグレッション」だけQAへエスカレーション。
6. **7社別リスクプロファイル運用**: クライアントごとの過去escape履歴・法令リスク（宮村・清一等の建設業許可法系）・帳票再現要件をリスクスコア化し、リスクベース抽出（06-12）の優先度算定へ組込。
7. **ISO/IEC 42001 AIMSトレーサビリティ証跡準拠**: AI生成物承認時のプロンプト・モデル版・オラクル版・承認者・日時をreview.jsonへ必須記録し、監査対応可能な承認正本化を完成。
8. **Contract Testing（Pact等）による連携整合**: エージェント間の入出力契約をスキーマで固定し、consistency軸の判定を「契約違反の有無」で機械化。

### 導入ツール・フレームワーク（New Tools）
1. **Promptfoo / DeepEval**: Evals駆動運用の標準ツール。評価データセット管理・スコア閾値・ドリフト検知を一元化し、AI生成成果物の合格条件を「テストケース×閾値」で回帰検知。
2. **axe-core + Lighthouse CI**: LP・資料のアクセシビリティ／パフォーマンス自動監査。GitHub Actions組込でPR単位に品質ゲート化。
3. **Sentry + Vercel Analytics + Datadog Synthetics**: シフトライトQAの本番テレメトリ基盤。escape rate自動計測とアラート→チェックリスト反映ワークフローを構築。
4. **Great Expectations（データ品質QA）**: Airwork・GA4等の数値レポート成果物に「期待値スキーマ」を定義し、Dat連携の数値整合3軸を自動判定へ完全移行。

### 強化された作業フロー（Enhanced Workflow）
```
STEP 0: 受付ゲート（機械軸で先に潰す）
  - schema自動validation / 固有名詞マスタ完全一致 / 3点サマリー添付 / 
    参照PDF改訂日・税区分・オラクル版数記載 / axe-core通過（LP系）
  - 未達は中身を読む前に即差し戻し
STEP 1: リスクベース抽出＋7社リスクプロファイル評価
  - 初回もの／差し戻し歴／工程圧縮／高リスク社をキュー優先化
STEP 2: LLM-as-a-Judge合議一次判定（accuracy/consistency）
  - 3モデル合議で機械判定可能軸を先行処理
STEP 3: 人手レビュー（feasibility/validation/セキュリティ）
  - OWASP LLM Top 10チェック（AI生成物）
  - ペルソナ検証・現場条件プリセット（建設業案件）
  - 合計＝内訳の縦整合／内部整合の機械照合結果確認
STEP 4: verdict生成（review.json正本化）
  - verdict/key_message/blocking_issues 3点サマリー
  - 承認オラクル版数・依存断面・承認ハッシュ記録（自動失効連動）
  - strengths/quick_wins/critical_fixes/next_iteration 4区分
  - 未検証範囲・前提条件・残存リスク必須明記
STEP 5: Sora最終QAへハンドオフ＋申し送り消込表（定量条件＋期限付き）
STEP 6: 本番テレメトリ7日監視 → escape検知時は5軸へ即項目追加
```

### 唯一無二の差別化ポイント（Unique Value Proposition）
- **「機械軸／人手軸／AI合議軸」の3層QAアーキテクチャ**: 一般的なQAが人手一辺倒か機械一辺倒に振れる中、LET事業では「機械validation→LLM-as-a-Judge合議→人手（feasibility/validation/セキュリティ）」の3層で工数と精度を両立。
- **建設業DXクライアント特化の実機検証プリセット**: 直射日光・手袋操作・通信断復帰・現行帳票再現の4大現場条件を7社共通プリセット化。同業他社QAでは獲得不能な現場ノウハウを資産化。
- **承認正本の完全監査可能性（ISO/IEC 42001準拠水準）**: review.json＋オラクル版数＋モデル版＋承認ハッシュで、承認後変更凍結と承認前提側変更の両方向で自動失効するクローズドループ。
- **クライアント別リスクプロファイル運用**: escape履歴・法令リスク・帳票要件をスコア化した7社別優先度算定は、大手QAコンサルティングでも案件横断で持たない資産。

### KPI・成功指標（Success Metrics）
| 指標 | 現状（推定） | 目標（2026 Q4） | 計測方法 |
|---|---|---|---|
| escape rate（QA通過後7日以内の下流不具合率） | 8-12% | 3%以下 | 本番テレメトリ自動集計・週次 |
| 平均レビュー時間（1件あたり） | 20-30分 | 8分以下 | Slack Bot打刻・受付〜verdict確定 |
| 再レビュー往復回数 | 平均1.8回 | 平均1.1回以下 | 定型合格条件スニペット導入効果測定 |
| conditional-approve比率 | 未計測 | 15%以下（過剰保留の抑制） | review.json集計・月次 |
| qa×sora判定一致率 | 未計測 | 90%以上 | 四半期→月次キャリブレーション測定 |
| 固有名詞誤記による納品事故件数 | 過去12ヶ月 3件 | ゼロ | クライアント台帳完全一致機械化 |
| AI生成物セキュリティ検出率（OWASP LLM Top 10） | 未計測 | 敵対的テスト実施率100% | 成果物種別テンプレ実施ログ |
| WCAG 2.2 AA準拠率（LP・資料） | 未計測 | 95%以上（axe-core自動） | Lighthouse CI月次集計 |

### 継続学習ループ（Continuous Learning）
- **週次**: 本番テレメトリからescape案件を抽出→原因軸特定→5軸チェックリストへ項目追加／不要項目降格（07-03の四半期棚卸しを週次前倒し）。
- **月次**: qa×soraキャリブレーション（同一成果物独立レビュー）で判定一致率測定→乖離大の観点は合格基準記述を具体化。LLM-as-a-Judge3モデル合議のAI判定vs人手判定一致率も同時測定。
- **四半期**: Evalsデータセット鮮度棚卸し（母集合の妥当性・データドリフト評価）／チェックリスト総項目数レビュー（肥大化抑制）／7社リスクプロファイル再算定／OWASP LLM Top 10 / WCAG改訂追随。
- **年次**: ISO/IEC 42001・ISO/IEC TR 24028・DORA Metrics最新版レビュー、Promptfoo / DeepEval / axe-core等のツールバージョンアップ検証と乗換判断。
- **外部インプット**: OWASP GenAI Security Project公開資料／MLCommons AI Safety Benchmarks／Google DORA Report／IEEE Software Quality Standards年次動向を四半期でRui（リサーチ）経由キャッチアップ。
