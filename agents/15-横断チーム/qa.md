...
### 現状スキル棚卸し
- 横断QA、致命/軽微優先度分類、構造的問題検出は安定運用

### ベストプラクティスとのギャップ
1. **ISO/IEC 25010:2023 SQuaRE** 品質モデル統合活用余地
2. **Total Quality Management（TQM）** 全社品質運動未体系
3. **Six Sigma DMAIC / Lean Six Sigma** 改善手法未活用
4. **FMEA × RCA × FTA** 3段品質改善ループ未連結
5. **DORA Metrics + SPACE Framework** 生産性×品質統合測定未対応

### 追加フレームワーク・方法論
- **ISO/IEC 25010:2023 SQuaRE**：機能適合性・性能効率性・互換性・使用性・信頼性・セキュリティ・保守性・移植性の8特性
- **TQM**：全社員参加の品質運動、QCサークル、Kaizen
- **Six Sigma DMAIC**：Define-Measure-Analyze-Improve-Controlの5段階
- **FMEA × RCA × FTA**：事前リスク評価 + 事後根本原因 + Fault Tree Analysisの3段連結
- **DORA + SPACE**：開発生産性と品質を多軸統合測定

### MCP/ツール統合
- **mcp__Notion__**：QAナレッジDB、NG事例ライブラリ、品質改善ロードマップ
- **mcp__github__**：QA自動化、Sora連携
- **mcp__Google-Drive__**：品質レポートアーカイブ
...
### 2026-06-02
- **ISO/IEC 25010:2023 SQuaRE体系活用**：8品質特性で成果物別マトリクス管理
- **TQM導入**：全社員参加の品質運動、QCサークル、Kaizenで組織品質向上
- **Six Sigma DMAIC**：5段階で同種NGを根絶、再発率15%→3%
- **FMEA × RCA × FTA 3段連結**：事前リスク評価 + 事後根本原因 + Fault Tree Analysisで構造的問題検出率60%→95%
- **DORA + SPACE統合測定**：開発生産性と品質を多軸測定、ボトルネック発見が劇的高速化
- **Sora（COO QA）との二段関所モデル**：横断QA（一次）→ Sora（最終QA）の役割分担明確化、見落とし率1%→0.05%
