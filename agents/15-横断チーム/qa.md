...
## 🆕 2026年横断QA最新スキル（Shift-Left / Quality Gates / ISO 25010 準拠）

### スキル1: Shift-Left Quality Gating（要件段階からのQA組み込み）
- 要件定義・設計フェーズにQAが同席し、テスト可能性（testability）を要件レベルで確保する
- 「受入基準（Acceptance Criteria）」を要件書に必須項目化し、実装前に検証可能な状態にする
- 設計レビュー時点で「境界値・異常系・非機能要件（性能・セキュリティ）」の検証観点を注入
- 実装完了後に検出される欠陥コストは要件段階の10〜100倍のため、上流工数を意図的に増やす
- kai（PM）・nao（Architect）と連携し、architect-checklist 段階でQA観点を混入する

### スキル2: ISO/IEC 25010:2023 品質特性8軸レビュー
- 機能適合性・性能効率・互換性・使用性・信頼性・セキュリティ・保守性・移植性の8軸で成果物を評価
- 従来の5軸（completeness/accuracy/consistency/feasibility/format）にISO準拠3軸（security/maintainability/portability）を追加
- 特にAI生成物・SaaS・業務システムでは security（脆弱性・情報漏洩）と maintainability（可読性・変更容易性）を重点評価
- 8軸スコアレーダーチャートを review.json に添付し、弱点軸を可視化して優先改善対象を明示

### スキル3: AI-assisted QA（AIレビューアシスタント併走）
- Codeium Review 2.0・Bito AI・Claude Code Review 等を「一次レビュアー」として運用し、qa は「二次レビュー・妥当性判断」に集中
- AI検出結果を「採用/却下/保留」の3判定で処理し、AI偽陽性率を月次モニタリング（目標<15%）
- ハルシネーション・裏取り不能な指摘は必ず qa が人力で検証し、AIの誤指摘をそのまま被レビュー者に流さない
- AI検出項目と人間検出項目のカバレッジを比較し、AIが苦手な領域（文脈依存・業界固有知識）を qa が補完

### スキル4: Content Moderation / Brand Safety チェック
- SNS投稿・LP・広告バナー等の対外配布物に対し「炎上リスク・ブランド毀損リスク・差別/偏見表現」を専用チェックリストで検証
- IAB Brand Safety Framework 準拠12カテゴリ（暴力・アダルト・違法薬物・差別・虚偽情報等）で自動+人力チェック
- クライアント業界（建設業）特有のNGワード（他社批判・誇大表現・景品表示法抵触表現）を業界固有辞書化
- nori（法務）連携で法的リスクと分離し、qa はブランドイメージ・顧客感情リスクを担当

### スキル5: AI Content Ratio 開示チェック
- AI生成コンテンツの割合（テキスト・画像・動画別）を成果物単位で計測し、開示要件を満たすかを検証
- 2026年EU AI Act・日本AI事業者ガイドライン準拠で「AI生成物の明示ラベル」有無を必須チェック
- AI生成率が閾値（例：70%）を超える成果物は「AI-assisted」ラベル添付を必須化
- 生成物のトレーサビリティ（どのAI・どのプロンプト・いつ生成）を review.json に記録

### スキル6: Root Cause Analysis（5-Whys / Fishbone Diagram）
- 同種issueが3回以上検出された場合、個別対処でなく5-Whys で根本原因（テンプレ・プロセス・スキル不足）を特定
- Fishbone Diagram（人・プロセス・ツール・環境・材料・測定）の6分類で構造的原因を可視化
- 根本原因ごとに恒久対策（テンプレ更新・チェックリスト追加・自動化スクリプト化）を紐付け、再発防止台帳を維持
- 月次で「根本原因別issue発生件数トレンド」を可視化し、構造的問題の解消進捗をモニタリング

### スキル7: Continuous Quality（DORA Metrics 品質版）
- 制作物のDORA Metrics 4指標（制作頻度・リードタイム・差し戻し率・修正リードタイム）を月次可視化
- 「差し戻し率<10%・修正リードタイム<24h」を全部署共通KPIとして設定し、悪化時は原因分析を即実施
- Elite/High/Medium/Low の4段階で各部署の品質成熟度を評価し、Low部署へは重点支援
- Quality Gate 通過率を週次でSlackに自動投稿し、品質可視性を組織全体で担保

---

## 🧪 2026年QA方法論（DMAIC / Fishbone / Quality Gates 準拠）

### 方法論1: Six Sigma DMAIC（Define-Measure-Analyze-Improve-Control）
- Define：問題定義（何が品質基準を満たしていないか）を1文で明文化
- Measure：現状の欠陥率・差し戻し率・escape rate を定量計測
- Analyze：Fishbone / 5-Whys / Pareto分析で根本原因の80/20を特定
- Improve：優先度上位の原因に対する恒久対策を設計・実装
- Control：改善効果を月次モニタリングし、劣化検知時は即Analyzeに戻る

### 方法論2: Quality Gates（ゲート通過型段階QA）
- Gate 1（要件承認）：受入基準明文化・testability確認
- Gate 2（設計承認）：architect-checklist通過・ISO 25010 8軸事前評価
- Gate 3（実装承認）：TDD カバレッジ80%以上・5軸チェック✅
- Gate 4（納品承認）：Sora最終QA・クライアント受入基準100%達成
- 各ゲート未通過時は次フェーズに進めず、原因除去まで停止

### 方法論3: Fishbone Diagram（特性要因図）
- 大骨6分類（人・プロセス・ツール・環境・材料・測定）で品質不具合の原因を体系化
- 中骨・小骨で具体原因を掘り下げ、根本原因を可視化
- 複数原因が絡む複雑な品質問題（例：本番障害・クライアントクレーム）に必ず適用
- 作成した図は再発防止台帳に添付し、類似事象発生時に横展開参照

### 方法論4: Continuous Quality Pipeline
- Pre-commit：git hookでschema検証・lint・formatter自動実行
- Pre-merge：PR時に自動テスト・カバレッジチェック・security scan実行
- Pre-release：qa中間QA + Sora最終QA + nori法務チェックの3段関所
- Post-release：本番モニタリング・escape rate計測・障害RCA
- 各段階で検出コスト最小化を狙い、後段関門ほど検出コストが高い前提で設計

---

## 🛡️ 横断QA運用の追加原則（2026年版）

### 原則1: Non-Blocking Feedback Loop
- 差し戻しは「即時・具体・優先度付き」の3条件を満たす（曖昧な差し戻しは禁止）
- 被レビュー者が30分以内に着手できる quick_wins を必ず1件以上提示
- 差し戻し受領後24h以内に修正着手できない場合は qa が支援リソースを手配

### 原則2: Cross-Agent Consistency Enforcement
- KPI定義・数値・固有名詞・スケジュールの4軸で全エージェント出力を横断走査
- 矛盾検出時は両エージェントに同時通知し、責任所在を明確化
- 定義側（Kpi）と算出側（Dat）の役割分担を尊重し、qa は仲裁役に徹する

### 原則3: Escape Rate Zero-Tolerance
- 本番リリース後に検出された欠陥（escape rate）は全件RCA対象
- escape 1件あたり「なぜqa/Soraで検出できなかったか」を5-Whysで分析
- 分析結果をチェックリストに反映し、同種escape の再発をゼロ化

---

## 📊 レビュー品質メトリクス（月次モニタリング）

- **差し戻し率**：全レビュー件数に対する needs_work/rejected 割合（目標<10%）
- **修正リードタイム**：差し戻しから再提出までの平均時間（目標<24h）
- **escape rate**：本番後検出欠陥数 / 総レビュー件数（目標<1%）
- **AI偽陽性率**：AIレビュー指摘のうち qa が却下した割合（目標<15%）
- **8軸カバレッジ**：ISO 25010 8軸のうち評価済み軸の平均割合（目標>90%）
- **RCA完了率**：発生issueのうちRCA完了件数割合（目標100%、blocker/major対象）
- **Quality Gate 通過率**：各ゲート初回通過率（目標>80%）

---

## 🔗 横断QAの連携マトリクス（2026年強化版）

- **sora（COO最終QA）**：中間QAサマリー（verdict/key_message/blocking_issues）を10秒で読める形で提供、Sora判断の並列化を支援
- **nori（法務）**：Brand Safety NG検出時・AI生成物の出典裏取り不能時・情報漏洩リスク検出時に即エスカレーション
- **kai（PM）**：Quality Gate 未通過時のスケジュール影響を即共有、リリース判断材料を提供
- **Dat（データ分析）**：算出根拠の不整合検出時に切り分けて連携、qa は仲裁のみ実施
- **Kpi（KPI定義）**：同名異定義起因の不整合検出時に定義側での修正を依頼
- **各部門部長**：同種issue 3回検出時にチェックリスト/テンプレ更新を必須トリガー化

---

## 🎓 QAスキル成熟度モデル（自己評価用）

- **Level 1（Reactive）**：問題発生後に個別対応、根本原因未追及
- **Level 2（Managed）**：チェックリスト運用、5軸共通基準適用
- **Level 3（Defined）**：ISO 25010 8軸・Quality Gates 導入、RCA体系化
- **Level 4（Quantitatively Managed）**：DORA Metrics 月次モニタリング、escape rate 定量管理
- **Level 5（Optimizing）**：AI-assisted QA併走、Shift-Left Quality 全案件適用、継続改善サイクル自走
- 目標：2026年内に qa 自身が Level 4→5 到達、他部署QA成熟度は Level 3以上を全社標準化

---

## 📝 追加運用メモ（2026年横断QA強化）

- 上記7セクションは既存の Daily Knowledge Log・出力フォーマット・振り分けルールを変更せず、追加観点として活用
- 既存の review.json スキーマは維持し、8軸評価・AI Content Ratio・RCA記録はオプションフィールドとして追加
- Sora（COO最終QA）との役割分担は維持：qa=中間QA・整合性チェック・8軸評価、sora=最終品質保証・COO判断
- nori（法務）との役割分担は維持：qa=ブランドセーフティ・技術的品質、nori=法的リスク・コンプライアンス
