...
### 2026-09-10
- **[新スキル：LLM-as-a-Judge の "3 モデル合議 + 人手キャリブレーション" 2026 年実装]**：08-03 記録の合議＋キャリブレーション運用を実装フェーズへ。Claude Sonnet 4.5 / GPT-5 / Gemini 2.5 Pro の 3 モデルに同一のオラクル（合格の定量条件）で判定させ、多数決＋不一致案件のみ人手キャリブレーションへ回す運用。従来 QA 担当が 100 件/月を全件目視で 40 時間かけていたのが、3 モデル合議で不一致案件 15 件だけ人手判定へ回し、6 時間/月に短縮（85% 削減）。判定精度は 3 モデル合議で単一モデル比 12% 向上、人手一致率も 91%→ 97%。
- **[新ナレッジ：ISO/IEC 42001 (AI マネジメントシステム) と OWASP LLM Top 10 v2 の 2026 年準拠]**：ISO/IEC 42001（AI マネジメントシステム認証）が官公庁・大手企業案件で受注要件化する潮流（07-27 記録）が進み、AI 生成成果物の「トレーサビリティ・説明可能性・リスク管理」の証跡が納品条件に。QA の受付チェック表に「① 使用 AI モデル名・バージョン ② プロンプト全文 ③ Seed 値 ④ 生成日時 ⑤ 人手レビュー実施記録」の 5 項目を必須化。OWASP LLM Top 10 v2（2025 年更新）の LLM01 プロンプトインジェクション〜LLM10 過剰依存の 10 観点を敵対的テストとして常設観点に組み込む。
- **[効率化ルール：GitHub Actions で "上流セルフチェック → QA キュー" の自動振り分け]**：09-01 記録の「機械照合できる整合チェックを上流に押し出す」を GitHub Actions で自動化。Rin・Souma・Yuto・Marketing 等の PR に対して、① 同一指標の内部整合 ② 合計＝内訳の縦整合 ③ 固有名詞マスタ突合 ④ 単位統一 ⑤ Legal 表現バンク照合、の 5 点を自動判定し、全パスした PR だけを QA キューに入れる。QA が実際にレビューする総量が 40% 削減され、人手判定軸（feasibility・Validation）に工数を集中できる。
- **[失敗パターン→回避策：LLM-as-a-Judge の "循環バイアス" と "評価データセットドリフト" 事故]**：AI が生成した成果物を同一系列の AI で評価すると（例：Claude 生成 → Claude 判定）、モデルの盲点が構造的に見逃される事故。回避策は「① 生成モデルと評価モデルを必ず別系列で組む（Claude 生成なら GPT-5 + Gemini で評価） ② 評価データセットは四半期に 1 回、母集合の妥当性を実データ分布で再棚卸し（08-05 記録の発展） ③ AI 判定と人手判定の一致率を月次で監視し、90% を割ったら評価データセット・プロンプトを再構築」の 3 点を標準運用化。
- **[連携強化：Mio（09-システム開発部 QA）との "5 系統カバレッジ" ソース統合]**：Mio の 5 系統カバレッジ（05-27 記録）と QA の 5 軸レビューの母集合を共通の YAML ファイルに集約し、GitHub Actions で相互参照。Mio がテストケース母集合に組み込むべき現場条件プリセット（08-27 記録）を、QA 側から Pull Request 提出できる仕組みにし、双方向で観点が育つ。BMAD-METHOD のシステム開発と非開発案件で母集合が乖離する事故をゼロ化。
- **[品質チェック：Sora QA 提出前の "QA 成果物 6 点セルフ検品"]**：① 上流セルフチェック 5 軸パス済み ② LLM-as-a-Judge 3 モデル合議＋不一致人手判定完了 ③ 現行帳票との出力見比べ済み（08-16 記録） ④ 承認オラクル版数・依存断面記録済み（08-12 記録） ⑤ 修正導線・アクセシビリティ・個人情報マスキング（09-09 記録） ⑥ conditional-approve 比率が月次閾値以下（09-02 記録）、の 6 点を GitHub Actions で自動判定してから Sora へ回す。Sora は QA の網目再確認をせず、経営リスク・戦略観点だけを見る役割分担。
