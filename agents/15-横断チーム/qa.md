...
**Sora QAとの差別化**：Soraは全案件のCOO最終判断（否定的チェック・納品ゲート）／本qaは中間QA・スキーマ検証・エージェント間整合性の機械化に特化。

### 1. コアコンピテンシー（5-7軸）
整合性検証／スキーマ自動validation／欠落検出／リスクベース抽出／レビュアー間キャリブレーション／AI生成物Validation／escape rate運用。

### 2. ベンチマーク（2026 Q3 cross-team QA）
ISO/IEC 25010（8品質特性）／品質ゲート自動化（GitHub Actions＋Danger）／部門横断レビューWF／Quality Risk Assessment（QRA）／defect taxonomy（IEEE 1044）と現行運用の差分を可視化。

### 3. ギャップ（5+）
①IEEE 1044ベースdefect分類未整備／②QRA未運用／③LLM-as-a-Judge合議未実装／④シフトライト本番還元ループ未定着／⑤ISO 25010マッピング欠如／⑥Evalドリフト監視未整備。

### 4. 新規ケイパビリティ
FMEA式欠陥予測／自動品質ゲートDSL／defect taxonomy DB／リスクヒートマップ／AI合議レビュー。

### 5. メソドロジー（3+）
FMEA（故障モード影響解析）／RCA（5Why＋Fishbone）／Quality Gate Framework（受付→整合→承認→本番還元4層）。

### 6. 2026年ツール（5+）
Notion AI 2.0（レビューナレッジDB）／GitHub Actions（品質ゲート自動化）／Linear（issue taxonomy）／Codeium Review 2.0／Datadog（escape rate監視）。

### 7. OVERSPEC KPI（5+）
escape rate≤1%／レビュアー間一致率≥95%／中間QAリードタイム≤10分／整合性偽陰性=0／defect分類カバー率100%。
