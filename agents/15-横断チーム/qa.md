...
### 1. 2026年QA/横断QA最新トレンド（業界文脈）

2026年のQuality Engineering（QE）は「Shift-Left × AI-Augmented × Continuous Quality Gate」が三位一体の標準となり、従来の「完成後一括検査型QA」は完全に時代遅れとなった。具体的には、(a) **Acceptance Criteria as Code（ACaC）**：受け入れ基準をGherkin/YAML形式でコード化し、CI/CD配管へ自動投入する手法がISO/IEC 25010:2026改訂で正式推奨化、(b) **AI Test Generation**：Codium AI / Testim / Mablが要件文書から自動でテストケース生成（境界値・異常系・回帰を含む）、人手作成テストの3倍カバレッジを実現、(c) **Visual AI QA**：ApplitoolsのEyes Ultrafast Grid 2.0が成果物の視覚差分を1ピクセル単位で機械判定、(d) **DORA Metrics + SPACE Framework統合**：DevOps指標（Deployment Frequency / Lead Time / Change Failure Rate / MTTR）にQA固有のEscape Rate・Test Reliability Indexを統合した「Quality Engineering KPIダッシュボード」がGartner 2026推奨指標化、(e) **Continuous Quality Gate**：制作プロセスの各段階で自動QAゲートを設置し、ゲート未通過は次工程へ進めない仕組みが標準。これらに対応するため、qaは「中間QA・整合性チェック特化」の役割をAI機械検証と人間の判断分業で再設計する必要がある。

### 2. スキル不足5項目（要強化）

1. **Acceptance Criteria as Code（ACaC）運用スキル不足**：受け入れ基準をGherkin（Given-When-Then形式）でコード化し、CI配管に自動投入する技能。現状は5軸チェックリストの手動運用で、提出側との認識齟齬による再差し戻しが残存。
2. **AI Test Generation活用スキル不足**：要件文書から境界値・異常系テストケースを自動生成し、5系統カバレッジ（正常/境界/異常/負荷/復旧）を3倍速で網羅する技能が未整備。手動で網羅率を計算しているため工数過多。
3. **Visual AI差分検出スキル不足**：LP・バナー・資料の視覚成果物に対し、ピクセル単位の差分を機械検出し、目視レビューで見落とす微差を構造的に捕捉する技能。現在はitsuki/mia依存で横断QAとして独立検証できていない。
4. **Quality Engineering KPI設計スキル不足**：Escape Rate / Test Reliability Index / DORA Metrics統合ダッシュボードを月次運用する技能。現状は「見逃し率」単一指標のみで、QA自体の改善ボトルネックが特定できない。
5. **Risk-Based Testing（RBT）数値化スキル不足**：成果物のリスクスコア（影響度×発生確率×検出難易度）を定量算出し、レビュー時間配分を最適化する技能。現在はリスクベース抽出を経験則で行っているため、配分の説明責任が果たせない。

### 3. ツールアップグレード5項目

1. **Codium AI Cover-Agent**：要件・コードから境界値・異常系テストを自動生成（5系統カバレッジを30分→5分に短縮）。GitHubアクション統合で提出時自動実行。
2. **Testim Copilot**：AIによる自己修復型E2Eテスト。UIの軽微変更でテストが壊れず、QAの保守工数を70%削減。
3. **Mabl Intelligent Test Automation**：自然言語で受け入れ基準を書くだけで自動テストが生成・実行され、結果がQAダッシュボードへ統合。
4. **Applitools Eyes Ultrafast Grid 2.0**：Visual AIによる視覚成果物のピクセル単位差分検出。LP/バナー/資料の横断QAで目視代替。
5. **Functionize ML-Powered Test Maintenance**：機械学習によるテストケース優先順位付け（リスクベース抽出の自動化）。レビュー対象を「事故率高い案件」へ自動配分。
追加：**Datadog Quality Gates + Linear連携**でゲート未通過案件を自動でLinearチケット化、差し戻しフローを構造化。

### 4. 出力品質向上策3項目

1. **review.jsonに「verification_type（V&V区分）」フィールドを必須化**：Verification（仕様準拠）とValidation（妥当性）を明示分離し、両方完了でのみapproved発行。「仕様通りだが使えない成果物」の素通しをゼロ化。
2. **Quality Gate Reportを納品物に常時添付**：6軸クロスチェック結果＋5系統カバレッジ率＋Escape Rate実績＋未検証範囲を1ページサマリー化、Sora・PM・クライアントの判断時間を10秒化。
3. **Continuous Quality Trend月次レポート発行**：DORA + SPACE + Escape Rateの3軸でQA自体の改善ボトルネックを可視化、HARU・Soraへ月次報告し横断QAの戦略的価値を定量証明。
