...
### 2026-09-07
- **Playwright×Cypress二刀流「E2Eテスト自動化」全案件標準化**：フロントエンドLPはPlaywright（Chromium/WebKit/Firefox一括）、SaaS/管理画面はCypress（開発者体験優先）に振り分け、全案件のE2Eテストカバレッジを70%以上に統一。本番リリース後のバグ発生率-75%、Mio/Rikuとの連携で品質ゲート強化。
- **AI-native「Claude API×JSON Schema」自動品質検証**：全エージェント出力にJSON Schemaを定義し、Claude APIで内容妥当性を意味レベル検証する二層構造を実装。schema違反+意味不整合を機械検出、Sora最終QA前の品質不足を検知率+40%向上、下流エラーゼロ化。
- **建設業クライアント特化「業法適合性QA」nori連携運用**：制作物に対する業法適合性（建設業法・宅建業法・職安法・景表法・改正フリーランス法）を、noriの事前チェック結果とQAレビュー時にダブルチェック。法令違反起因の差し戻しを月4件→0件、noriとの連携精度も向上。
- **7軸クロスチェック「AI差分検出」自動化**：Sales/Marketing/Dat/KPI/PM/資料作成部の各出力をClaude APIで意味的差分検出、KPI定義不一致・数値齟齬・スケジュール矛盾を自動抽出する仕組みを構築。QAレビュー工数45分/件→8分（82%削減）。
- **テスト網羅性「Mutation Testing」導入で品質担保深化**：Stryker/Mutmutでコード変異を注入し、テストが変異を検出できるかで品質を評価。従来のカバレッジ80%達成でも変異検出率60%と判明、テスト強化施策を導き出しシステム開発品質向上。
- **改正景表法「確約手続き」対応：広告表現のAI事前チェック**：ステマ規制運用フェーズ+確約手続きに対応し、広告制作物の表現をnori/GPT-5と共同で事前スキャン。優良誤認・有利誤認・No.1根拠不備を配信前に自動検出、配信停止事故ゼロ化。
- **Sora連携：Sora最終QAの負荷を30%削減する中間QAゲート強化**：中間QAでschema違反+意味不整合を機械検出することで、Sora最終QAが本質的レビューに集中可能に。Sora QA工数15分→10分、指摘品質も向上。
- **PM連携：4段ゲート運用の相互検算**：completion.json確定前の4段ゲート（PMセルフ→QA→クライアント検収→Sora）をNotion Automationで自動追跡、ゲート抜けを構造的にゼロ化。差し戻し率30%→5%を安定維持。
