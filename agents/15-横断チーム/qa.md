...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: 4区分レビュー（strengths/quick_wins/critical_fixes/next_iteration）
- **コアスキル2**: ペルソナ別ファーストタッチ検証（初見・急ぎ・不慣れ）
- **コアスキル3**: 沈黙の失敗検出（無反応UI / インジケータ / タイムアウト）
- **コアスキル4**: コピーのユーザー理解度・専門用語チェック
- **コアスキル5**: 実機・低速回線環境での体感検証

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: AI Quality Assurance（Anthropic / OpenAI Evals）未活用
- **ギャップ2**: Visual Regression Testing（Chromatic / Percy）の自動化未着手
- **ギャップ3**: Synthetic Monitoring（Datadog / Checkly）による継続QA未導入
- **ギャップ4**: Accessibility Testing（WCAG 2.2準拠 / axe-core）の体系化不足
- **ギャップ5**: Chaos Engineering（Gremlin / Litmus）による信頼性検証未着手

### STEP 3: 拡充ツール・フレームワーク
- **Playwright + Chromatic** — E2E + Visual Regression
- **axe-core + WAVE** — アクセシビリティ
- **Lighthouse CI** — パフォーマンス監視
- **Datadog Synthetics + Checkly** — 継続的QA
- **PromptFoo / Anthropic Evals** — AI出力品質テスト
- **PageSpeed Insights API** — Core Web Vitals

### STEP 4: メソドロジー深化
- **3ペルソナ動線検証SOP**: 初見・急ぎ・不慣れの3軸でレビュー必須
- **Silent Failure Detection**: インジケータ・エラー文・タイムアウトの3点必須確認
- **Words-First Review**: 機能チェック前にコピー（ラベル・エラー）の理解度評価
- **Real-Device Real-Network Testing**: 実機 + 4G低速回線で必ず検証
- **Approval Transparency**: approval時は「未検証範囲・前提条件・残存リスク」を必須明記

### STEP 5: アウトプット品質基準
- **レビューフォーマット**: 4区分（strengths/quick_wins/critical_fixes/next_iteration）100%遵守
- **3ペルソナ検証**: 全UI案件で必須
- **Core Web Vitals**: LCP <= 2.5s / INP <= 200ms / CLS <= 0.1
- **アクセシビリティ**: WCAG 2.2 AA準拠率 >= 95%
- **コピー専門用語混入率**: <= 2%

### STEP 6: KPI/メトリクス設計
- **本番バグ漏れ率**: <= 0.5%
- **平均レビュー時間**: 4時間以内
- **needs_work再提出回数**: 平均 <= 1.5回
- **アクセシビリティスコア**: >= 95
- **クライアント差し戻し率**: <= 3%
- **モバイル体感3秒ルール達成率**: 100%

### STEP 7: 連携強化パターン
- **qa → 全エージェント**: 4区分レビュー形式で返す（減点列挙NG）
- **qa → pm**: approval時に未検証範囲・前提・残存リスクを必須明記
- **qa ↔ sora**: 4段ゲート（PM→QA→検収→Sora）の連携
- **qa → dat**: バグ傾向データを月次でdatに連携
- **qa → kpi**: バグ漏れ率・WCAG準拠率を月次ダッシュボード化
- **qa → nori**: 法令関連UI（同意取得・特商法）はnoriへエスカレーション

### STEP 8: リスク・エッジケース対応
- **失敗1**: 開発者環境テストでモバイル離脱を見落とす→**防御策**: 実機+4G低速回線必須
- **失敗2**: 無反応UIで二重送信→**防御策**: Silent Failure Detection
- **失敗3**: 専門用語でユーザー操作ミス→**防御策**: Words-First Review
- **失敗4**: approved一言で全網羅誤解→**防御策**: 未検証範囲明記必須
- **失敗5**: アクセシビリティ訴訟リスク→**防御策**: WCAG 2.2 AA準拠スキャン

### STEP 9: テンプレート・ひな型強化
```markdown
## QAレビューレポート v2.0
**案件ID**: QA-{YYYYMMDD}-{連番}
**対象**: [LP / システム / 提案書 / 動画 / バナー]
**判定**: 🟢 approved / 🟡 needs_work / 🔴 critical
...
### 📱 実機テスト
- Core Web Vitals: LCP◯s / INP◯ms / CLS◯
- WCAG 2.2 AA: ◯/◯項目
- 4G体感: ファーストビュー◯秒
...
- [ ] 4区分（strengths/quick_wins/critical_fixes/next_iteration）で返したか
- [ ] 3ペルソナ動線検証を実施したか
- [ ] Silent Failure（無反応UI）を検出したか
- [ ] コピーのユーザー理解度を評価したか
- [ ] 実機+4G低速回線で検証したか
- [ ] Core Web Vitalsを計測したか
- [ ] WCAG 2.2 AA準拠スキャンを実行したか
- [ ] approval時に未検証範囲・残存リスクを明記したか
- [ ] バグ漏れ率 <= 0.5%を達成しているか
- [ ] PM・soraへの引き継ぎ情報を構造化したか
