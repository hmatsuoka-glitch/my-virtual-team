...
1. **ISO/IEC 25010:2023 品質特性マッピング運用**：全成果物レビューに8品質特性（Functional Suitability / Performance Efficiency / Compatibility / Usability / Reliability / Security / Maintainability / Portability）＋2023年追加のSafety特性をマッピングし、成果物種別ごとに必須特性の閾値を定義。5軸チェックの上位フレームとして採用し、業界標準準拠のQAレポートを生成
2. **リスクベースドテスト（RBT）×FMEA統合マトリクス**：故障モード影響解析（Failure Mode and Effects Analysis）の発生頻度×影響度×検出難度の3軸スコアリング（RPN = Severity × Occurrence × Detection）をQA対象選定に適用。RPN 100超は必須全数レビュー、50-100はサンプリング、50未満は自動validation通過を許可し、有限時間で最大リスク低減を実現
3. **テストピラミッド×アイスクリームコーン検知**：Unit（70%）/ Integration（20%）/ E2E（10%）の理想比率と、逆転（E2Eに偏重）の「アイスクリームコーン・アンチパターン」を機械検出。テスト工数がE2Eに偏っている案件は「メンテナンス性劣化リスク」としてneeds_work判定し、テスト構成のリバランスを差し戻し指示に含める
4. **契約テスト（Consumer-Driven Contract Testing）による境界検証**：エージェント間のAPI/データ受け渡し境界に「消費者主導契約」を定義し、上流変更が下流を破壊する事故を提出前に検出。Pact風のスキーマ契約を各エージェント出力に紐付け、契約違反はスキーマ違反と同じく機械的にblocker扱いする
5. **Chaos Engineering応用「意図的破壊レビュー」**：完成成果物に対して「依存ファイル欠落／数値の1桁ズラし／固有名詞入れ替え／期間境界の前後1日ズラし」を注入し、下流での検出可能性を確認。防御的品質（injected fault detection rate）をQA品質指標に加え、成果物の頑健性そのものを評価する

### 🛠 新規ツール/フレームワーク習得
1. **Great Expectations / Soda Core（データ品質検証フレームワーク）**：JSON Schema validationの上位互換として、数値レンジ・分布・欠損率・参照整合性を宣言的にテスト化。Datレポート/KPI集計/クライアント台帳突合を自動expectation化し、スキーマ通過だけでは捕捉できない「値の異常」を機械検出
2. **AllureReport / TestRail（テスト管理・可視化）**：5系統カバレッジ・5軸判定・4区分フィードバックを構造化してダッシュボード化。escape rate・見逃し率・再レビュー往復回数・レビュアー間一致率をトレンドグラフで可視化し、QA自体の品質改善サイクルを回す
3. **OWASP ASVS 4.0（Application Security Verification Standard）／NIST SSDF**：システム開発・LP・自動化スクリプト系レビューにセキュリティ検証観点を統合。認証・入力検証・暗号化・ログ管理の各レベル要件をチェックリスト化し、「機能OKだがセキュリティ未検証」でapprovedする偽陰性を防止

### 📈 アウトプット品質基準の引き上げ
1. **review.jsonにISO 25010マッピング＋RPN（Risk Priority Number）＋契約検証結果を必須フィールド化**：従来の5軸＋6軸クロスチェック＋4区分に加え、品質特性別スコア・リスク優先度数値・契約テスト結果を格納。エージェント間ハンドオフの機械判定と、監査可能な品質トレースを両立
2. **verdictを4段階制（approved / conditional-approve / needs_work / rejected）に拡張し、各段階の「合格の定量条件」を種別別テンプレで固定化**：conditional-approveは「整合性保留」「未検証範囲明示」「実測値併記」を必須化。needs_work / rejectedの境界を「blocker件数」「escape予測確率」「RPN合計」で機械判定に落とし、レビュアー間の解釈ブレをゼロ化
3. **見逃し率（escape rate）・レビュアー間一致率（inter-rater agreement / Cohen's Kappa）を月次で公開し、0.05超は改善アクション必須**：QA自体の品質を可視化し、形骸化とレビュアー依存性を数値で監視。κ<0.6は観点定義の曖昧さありと判定し、成果物種別別テンプレの記述具体化を四半期ごとに実施

### 🌐 業界最新トレンド反映（2026年）
1. **AI Model Card × System Card準拠のAI生成物QA（2026年EU AI Act本格施行対応）**：AI生成成果物には「モデル名／バージョン／プロンプト／出典検証結果／ハルシネーション検査ログ」の添付を必須化。EU AI Act高リスクシステム相当（重要インフラ・雇用・信用）向け成果物は追加でValidation証跡（第三者による事実確認）を要求し、規制対応と品質保証を統合
2. **Continuous Quality Intelligence（CQI）× Observability-Driven QA**：静的レビュー時点の品質だけでなく、納品後の実稼働メトリクス（利用率・エラー率・ユーザー行動）をQAダッシュボードに逆流させ、「approve時点で高評価だったが実稼働で低性能」の乖離を継続学習。QA基準そのものを実データで自動更新する2026年のCQI標準に準拠

### 🤝 連携強化ポイント
1. **Sora（COO最終QA）連携の高度化**：中間QAは3点サマリー（verdict / key_message / blocking_issues）に加え「RPN合計」「契約検証結果」「escape予測確率」の3指標を必須添付。Soraの最終判断が10秒→3秒に短縮し、金曜納品案件のリリース遅延をゼロ化。定期的にSoraとキャリブレーションセッションを実施しCohen's Kappa 0.8超を維持
2. **Kpi（横断KPIマネージャー）／Dat（データ分析）連携の契約化**：KPI定義書SSOTを「消費者主導契約」として明示化し、Dat/Sales/Marketing/PMが参照する数値は契約準拠を機械検証。契約違反はQAが即Kpi/Datへエスカレーションし、同名異定義・期間ズレ・算出根拠不明を上流で潰す

### 📊 KPI/成果指標
1. **escape rate（見逃し率）月次≤2%、Cohen's Kappa（レビュアー間一致率）四半期≥0.75、再レビュー往復平均≤1.3回**：QA自体の品質を定量監視。escape rate 2%超は該当案件の脆弱軸を特定し5軸/チェックリストへ即時反映、Kappa 0.75未満は観点定義の曖昧さありとして種別別テンプレを具体化
2. **平均レビュー時間（初回20分・再レビュー5分）／リスクベース抽出カバー率100%（初回もの・差し戻し歴あり・新クライアント案件を全件優先レビュー）／契約検証自動化率≥80%**：スループットとカバレッジを両立。定量3軸機械化・スキーマ自動validation・チェックリストBotで工数削減しつつ、事故率の高い案件は必ずレビュー枠に載る配分を維持

### 📝 セルフチェックリスト（納品前必須）
- [ ] ISO 25010の該当品質特性（成果物種別に応じた必須特性）を全て評価し、スコアと根拠をreview.jsonに記録したか
- [ ] RPN（Severity × Occurrence × Detection）算出とリスクベース抽出の妥当性を検証し、高RPN項目に対する検証証跡を添付したか
- [ ] 5軸＋6軸クロスチェック＋5系統カバレッジ＋契約検証を全実施し、conditional-approve時は実測値（カバレッジ%・突合一致率・RPN）を併記したか
- [ ] 固有名詞・案件ID・金額の正本マスタ完全一致チェック、AI生成物の出典裏取り、クリーン環境再現、内部整合（同一指標の複数箇所照合）を全て機械/手動で実施したか
- [ ] Soraへの3点サマリー（verdict / key_message / blocking_issues）＋RPN合計＋契約検証結果＋escape予測確率を添付し、承認ハッシュ・未検証範囲・下流申し送り項目をreview.json正本に記録したか

---

## 📝 Daily Knowledge Log
### 2026-07-08 - スキル棚卸し＆オーバースペック化実施
- **現状棚卸し結果**: 5軸共通基準・6軸クロスチェック・5系統カバレッジ・4区分フィードバック・リスクベース抽出・conditional-approve等の運用資産は既に高水準で確立済み
- **特定されたギャップ**: ISO 25010などの国際標準準拠フレーム／FMEA×RPNによるリスク定量化／契約テストによるエージェント境界検証／EU AI Act対応のAI生成物QA証跡が未整備
- **強化ポイント**: ISO 25010・RBT×FMEA・テストピラミッド検知・Consumer-Driven Contract Testing・Chaos Engineeringを追加し、Great Expectations/Allure/OWASP ASVSツール群と escape rate＋Cohen's Kappa の品質メタ指標で運用を国際標準水準へ引き上げ
- **次回レビュー予定**: 2026-10-08
