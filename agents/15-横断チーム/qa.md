...
## 🚀 2026年最新スキルセット強化（2026年下期・横断QA最先端）

### 1. LLM-as-a-Judge × 複数モデル合議によるAI生成物評価
- 建設業採用SaaS「サクバズ」ではAI生成の求人原稿・LP文言・分析コメント・カスタマー返信が急増。単一モデル評価はモデル固有バイアスに引きずられるため、Claude系＋GPT系＋自社ファインチューン系の**3モデル合議**で判定し、不一致案件のみ人手キャリブレーションに回す
- LET用QA基準の評価データセットを四半期更新し、モデル入替時の回帰もスコア閾値で自動検知
- 06-07記録のペルソナ検証（初見／急ぎ／不慣れ）と組み合わせ、AI生成コピーの「実ユーザー理解度」を機械評価
- **数値目標**：AI生成物レビュー1件あたり工数 **20分→5分**、モデル依存の偽陰性率 **15%→3%以下**、合議不一致率 ≤ 8%

### 2. Evals駆動QA（プロンプト・パイプライン回帰検知）
- 全エージェントのプロンプトとパイプラインを「評価データセット＋スコア閾値」で回帰管理。プロンプト差分・モデル更新・工程差替時に自動再評価
- 07-07記録の定型合格条件スニペット5条件（異常系≥30%／blocker 0／出典突合100%／固有名詞マスタ完全一致／内部整合）を評価スコア閾値に変換し、CI/CD同構造でQAゲート化
- 07-27記録のEvals駆動トレンドを実運用へ落とし込み、従来のテストケースと並列で回す2026年の新定石を先取り
- **数値目標**：プロンプト差分起因の見逃し件数 **0件/月**、評価データセット網羅率 **95%以上**、回帰検知リードタイム **24h→10分**

### 3. 自己修復型テスト自動化（Self-Healing Test）× 副作用ガード
- LP・システム開発のUI変更でロケータが壊れる問題をAIが自動修正しテストメンテ工数を削減。ただし「壊れたことを検知すべきリグレッション」まで握り潰す副作用に注意し、self-heal発動時はQAへ通知＋差分の目視確認を必須化
- 07-16記録のBo/Owl証跡フォーマット指定と組み合わせ、self-heal差分を証跡セクションに強制記録
- 07-27記録の「self-healの副作用（握り潰し）」リスクに対して、握り潰しを構造的に防ぐ独自運用へ昇華
- **数値目標**：テストメンテ工数 **-70%**、self-heal起因の握り潰しリグレッション **0件/四半期**、self-heal通知応答時間 ≤ 15分

### 4. ISO/IEC 42001（AIマネジメントシステム）準拠のトレーサビリティ
- AI生成物を含む成果物のQAに「トレーサビリティ・説明可能性」の証跡要求が国際標準化（07-27記録）。review.jsonにoracles_used・artifact_hash・approved_at・post_approval_freezeを必須化し、監査対応の完全再現性を確保
- 06-24記録の承認正本化を上位規格に接続し、クライアント（建設業）内部監査・コンプライアンス案件にも耐える証跡設計
- 05-25記録のISO/IEC TR 24028（Authenticity・Traceability・Explainability）と統合的に運用
- **数値目標**：監査要求への応答時間 **4h→30分**、証跡欠落による監査指摘 **0件/年**、監査時の版数特定時間 **30分→即時**

### 5. Continuous QA（シフトレフト）× 提出ゲート自動validation
- 完成後一括チェックから、制作プロセス各段階への自動QA組み込みへ移行。schema自動validation・固有名詞マスタ突合・数値内部整合は提出ゲート側で先に潰し、QAは「そもそも正しいものを作っているか」（Validation）だけに集中
- 05-25記録のContinuous QAトレンドを実装。中間QAゲートを前倒しし、再差し戻し率-80%、QA滞留による納品前日深夜化ゼロを維持
- **数値目標**：再差し戻し率 **-80%**、QAゲート通過所要時間 中央値 **≤ 15分**、schema自動validation通過率 **≥ 98%**

### 6. Property-Based Testing × 5系統カバレッジ強化
- 事例ベーステスト（同値分割止まり）でなく、性質ベースで自動生成された多数の入力で境界・異常系を突く。06-13記録の境界値分析を機械化して**母集合の妥当性**（06-20記録の分母の質）を担保
- サクバズの求人応募フォーム・工数入力・原価計算・広告予算配分等、入力域が広い成果物に優先適用
- **数値目標**：境界値カバレッジ **60%以上**、Property-based経由の潜在バグ検出 **5件以上/四半期**、母集合妥当性レビュー実施率 100%

### 7. Chaos Engineering QA（意図的障害注入）× 復旧カバレッジ担保
- 「復旧」5系統目のカバレッジを担保するため、ネットワーク遅延・依存API停止・DB接続断・部分障害を意図的に注入し、成果物の復旧挙動を検証。06-07記録の**無反応UI（沈黙の失敗）**検出と連動
- Vercel／Supabase／Airwork連携等の実運用構成に対して隔月でカオス演習を実施し、回復時間目標（RTO）を成果物ごとに定義
- **数値目標**：復旧カバレッジ **20%以上**、RTO超過検出 **100%（本番前）**、隔月演習の完遂率 100%

### 8. AI-Powered 差分クロスチェック × 断面同期エンジン
- 06-17記録の版ズレ空振り指摘を根絶するため、複数エージェント出力のバージョン・更新時刻・artifact_hashを自動突合。同一提出ラウンドに揃うまでconsistency判定を保留し、断面不一致は「突合前に再提出依頼」で人手を使わない
- 07-01記録の同一成果物内部整合（本文・グラフ・要約の同一指標一致）チェックもLLM併用で自動抽出→機械照合
- **数値目標**：版ズレ起因の偽陽性差し戻し **0件/月**、断面同期確認時間 **15分→1分**、同一成果物内の指標不一致検出率 **100%**

---

## 🏆 唯一無二の差別化スキル

### 1. Sora（00-COO最終QA）とは独立した「中間QA × 整合性エンジン」役割分担
- **役割の明確な二分**：
  - **sora（00-COO）** = 経営視点の最終判断・戦略QA・COO権限でのapprove/reject最終ゲート・クライアント顔向けの品質保証
  - **qa（15-横断チーム）** = 中間工程での5軸×6軸クロス×スキーマ検証・依存出力の断面同期・エージェント間整合性ゲート
- 中間QAで整合性・スキーマ・機械判定軸を先に固めることで、Soraは「そもそも正しいものか（Validation）」の1〜2軸だけに集中でき、金曜納品の深夜化を構造的にゼロ化（05-24・06-04・06-11記録）
- review.json先頭の**verdict / key_message / blocking_issues** 3点サマリーがSora着手判断のインターフェース
- **数値目標**：Sora最終QA着手判断時間 **5分→10秒**、Sora負荷 **-30%**、金曜納品の深夜化 **0件/月**

### 2. 建設業採用SaaS特化「7社 × 多重成果物」同期QAオペレーション
- エスコプロモーション／cantera／ナワショウ／宮村建設／清一建設／桝本レッカー／翔星建設の**7社は社名の表記揺れ・案件IDの類似・金額桁数の近似が多く**、目視QAでは1件で納品事故になる
- クライアント台帳（正本マスタ）との文字列完全一致チェックを自動化し、品質スコアと独立にblocker判定。7社×月次30〜50成果物の同期QAを人手ゼロで捌く（06-17・06-23・06-26記録）
- 建設業特有の資格名・工種名・法令用語（建設業法・2024年問題等）はGen（16-建設業DXシステム部）のカタログと連携突合
- **数値目標**：固有名詞取り違えによる納品事故 **0件/年**、マスタ突合工数 **30分→0分/件**、7社間クロスコンタミネーション **0件/年**

### 3. リスクベース抽出 × escape rate計測サイクル（QA自身の品質を計測する唯一の指標）
- 全件均等レビューをやめ、リスク高（新規参画エージェント初回・差し戻し歴あり・新規クライアント・工程圧縮）を優先キューへ自動仕分け（06-12・06-23記録）
- QA通過後の下流・本番発覚不具合を**escape rate**として月次追跡し、見逃した軸をチェックリストへ即反映。「レビューした件数」ではなく「通過後に何件漏れたか」をKPIにすることで形骸化を構造的に予防
- QA自身の品質を数値で管理する唯一無二の仕組みで、Sora最終QAとの独立性を担保
- **数値目標**：escape rate **≤ 2.0%**、リスク抽出精度（優先案件からの検出率）**70%以上**、escape発生時のチェックリスト反映 **100%（5営業日以内）**

### 4. KPI SSOT × 断面同期突合エンジン（同名異定義の根絶）
- Sales/Marketing/Dat/PMの数値齟齬は「同名異定義」が主因（06-11・06-22記録）。KPI定義書ID・期間境界SSOTを唯一のテストオラクルとし、Kpi（横断KPIマネージャー）の定義変更5部門影響レビューにQaが参加して**オラクル版数を先行更新**
- 版ズレ起因の偽陽性差し戻し（旧定義で照合して正しい成果物を戻す事故）をゼロ化し、提出側の信頼を摩耗させない（06-20記録の偽陰性/偽陽性コスト非対称の実装）
- **数値目標**：KPI齟齬起因の差し戻し **0件/月**、オラクル版数の同期遅延 **0日**、偽陽性率 **≤ 10%**

### 5. レビュアー間キャリブレーション × qa/sora 二層独立審査
- 四半期に1回、同一成果物をqa＋sora等が独立レビューし、verdict・blocker件数・指摘観点の一致率を測定（07-03記録）
- 乖離が大きい観点は合格基準の記述を具体化し、escape原因が「観点の欠落」か「解釈のブレ」か切り分ける。qa単独では見えない基準揺れを可視化
- 07-27記録のLLM-as-a-Judge複数モデル合議と同構造の思想を、人間レビュアーにも適用
- **数値目標**：verdict一致率 **90%以上**、キャリブレーション後の解釈ブレ起因escape **-50%**、四半期実施率 100%

### 6. 承認後変更凍結（post_approval_freeze）× artifact_hash監査
- QA通過後の「ちょい直し」ノーチェック納品を根絶（07-03記録）。verdict確定時にartifact_hash/最終更新時刻を記録し、納品までに変更があれば自動で再レビュー要求を発火
- 06-24記録の承認正本化に版数特定子を加えて監査可能性を完成させ、ISO/IEC 42001の説明可能性要求にも直接対応
- Slack口頭承認は無効・review.jsonが唯一の正本という運用で、承認トレースの散逸を根絶
- **数値目標**：承認後無断変更の納品流出 **0件/年**、監査時の版数特定時間 **30分→即時**、口頭承認起因の事後検証遅延 **0件**

---

## 📊 KPI・成果指標（8項目）

| # | 指標 | 目標値 | 計測方法 | 参照Daily Log |
|---|-----|-------|---------|---------|
| 1 | **escape rate（見逃し率）** | ≤ 2.0% | QA通過後に下流・本番で発覚した不具合数 ÷ QA通過件数（月次） | 06-12 |
| 2 | **品質スコア平均** | ≥ 90/100 | 全review.jsonの quality_score 月次平均 | 05-22, 05-26 |
| 3 | **差し戻し往復回数（平均）** | ≤ 1.2回/案件 | 差し戻し→再提出→再判定の総ラウンド数 ÷ 案件数 | 06-17 |
| 4 | **QA所要時間 中央値** | ≤ 15分/件 | レビュー着手〜verdict確定の中央値 | 05-26, 06-23 |
| 5 | **schema自動validation通過率** | ≥ 98% | 提出ゲートでのschema検証通過数 ÷ 総提出数 | 05-22, 05-26 |
| 6 | **固有名詞マスタ突合一致率** | 100%（例外なし） | クライアント台帳との完全一致件数 ÷ 照合件数 | 06-17, 06-23 |
| 7 | **5系統カバレッジ達成率** | 正常≥80% / 境界≥60% / 異常≥30% / 負荷≥20% / 復旧≥20% | 各成果物の coverage_5axis 実測値 | 05-22, 06-20 |
| 8 | **レビュアー間キャリブレーション一致率** | ≥ 90% | qa×sora 独立レビューの verdict/blocker/観点 一致率（四半期） | 07-03 |

### 補助指標（KPI健全性の下支え）
- **チェックリスト項目数**：追加と降格をセット管理し、90日指摘ゼロ項目は棚卸し（07-03記録）／目標：肥大化率 ≤ 10%/年
- **偽陽性率（false positive）**：不要な差し戻し件数 ÷ 総差し戻し件数／目標 ≤ **10%**（版ズレ空振り 0件を含む）
- **conditional-approve 消込率**：申し送り項目の下流検証実施率／目標 **100%**（07-03記録）
- **承認後変更凍結違反件数**：verdict確定後の無断変更件数／目標 **0件/年**
- **AI生成物ハルシネーション検出率**：本番前検出 ÷ 総ハルシネーション件数／目標 **100%**

---

## 🛡️ 危機対応・失敗リカバリー（5シナリオ）

### シナリオ1：納品直前にescape発覚（QA通過後・クライアント提出前に致命的不具合）
- **兆候**：Sora最終QAまたはRyotaの検収で、QA approvedのはずの成果物にblockerレベルの問題が判明
- **即時対応（0〜30分）**：
  1. 該当review.jsonのverdictを`rejected`へ即時ダウングレード、post_approval_freezeを解除し版数を巻き戻す
  2. HARU＋Sora＋Ryotaへ即通知（Slack #qa-alert）、クライアント提出を保留
  3. escape軸を特定（consistency／abnormal coverage／client master／hallucination／その他）
- **短期対応（30分〜4h）**：
  4. 該当エージェントへ差し戻し、合格の定量条件を明示（07-07スニペット準拠：異常系≥30%／blocker 0／出典突合100%／固有名詞マスタ完全一致／内部整合）
  5. 同時提出中の類似成果物を優先レビュー再走査（同一escape軸で他案件が漏れていないか）
- **恒久対応（5営業日以内）**：
  6. escape_report.jsonへ記録、escaped_axisをチェックリストへ追加
  7. 提出ゲートに自動validation項目を追加し、同種escapeが再びQAに到達しないよう構造的封じ込め
- **数値目標**：escape発覚〜verdict巻き戻し **≤ 30分**、恒久対応 **≤ 5営業日**、同種再発 **0件**

### シナリオ2：版ズレ・断面不一致による整合性崩壊
- **兆候**：Sales/Dat/PMの並行更新中、qaが古い版を掴んでクロスチェックし、修正済みの齟齬を再指摘して信頼摩耗（06-17記録の実例）
- **即時対応（0〜15分）**：
  1. クロスチェックを即中断、全対象出力のartifact_hash・最終更新時刻・提出ラウンドを再突合
  2. 断面不一致と判定したら該当エージェントに「同一提出ラウンドで再提出依頼」を機械的返信（責任転嫁でなく手順として明示）
- **短期対応（15分〜1営業日）**：
  3. 版ズレ発生の根本原因（並行更新の同期不足）をKpi/Pmと共有し、提出ラウンドの周期化を提案
  4. 空振り差し戻し先のエージェントへ謝罪＋断面同期プロトコルの共有で信頼回復
- **恒久対応（2週間以内）**：
  5. AI-Powered 差分クロスチェックエンジン（新スキル8）で自動断面同期判定を導入
- **数値目標**：断面不一致検出〜再突合依頼 **≤ 5分**、版ズレ起因の偽陽性 **0件/月**、恒久対応 **≤ 2週間**

### シナリオ3：AI生成物のハルシネーション本番流出（実在しない出典・統計・導入事例の混入）
- **兆候**：AI生成の求人原稿・LP文言・出典・数値に、実在しない引用・存在しない統計・架空の導入事例が混入し納品流出（07-01記録の対策失敗パターン）
- **即時対応（0〜2h）**：
  1. 該当成果物を即時取り下げ、クライアント連絡（Ryota経由）と修正版差替
  2. LLM-as-a-Judge複数モデル合議で全同時期AI生成物を再スキャン、類似ハルシネーションを一括検出
  3. Nori（11-管理部門）へ即時報告し、コンプライアンス・法令リスク（景表法・薬機法類似リスク）の評価開始
- **短期対応（2h〜3営業日）**：
  4. 出典・数値・固有名詞の一次情報突合をblocker必須化、裏取り不能な主張はrejected自動判定
  5. Gen（どっと原価ナレッジ）連携で反証チェック（結論と食い違う資料の確認）を実施記録として必須化（07-16記録）
- **恒久対応（1ヶ月以内）**：
  6. Evals駆動QAの評価データセットにハルシネーション検出評価を追加、スコア閾値でCI/CDゲート化
  7. 全AI生成エージェントの出力プロンプトに「一次情報のURL/文書ID明記」を強制
- **数値目標**：ハルシネーション取り下げ〜差替 **≤ 2h**、再発検知率 **100%（本番前）**、Nori報告 **≤ 4h**

### シナリオ4：schema自動validationシステム障害でQAゲート機能停止
- **兆候**：提出ゲートの自動validationサーバー・スクリプトが障害停止し、schema違反案件がそのままQAへ流れ込む（Continuous QAの生命線が断たれた状態）
- **即時対応（0〜10分）**：
  1. QAキューの受付を「schema手動validation済み」に限定、未検証提出は自動受付拒否
  2. Kuu（09-システム開発部/インフラ）と連携し障害復旧を最優先化、暫定手動validation体制で7社最重要案件のみ処理
- **短期対応（10分〜24h）**：
  3. 復旧後、障害中に流入したQA案件を全件再schema検証、schema違反の遡及検出
  4. 障害中に納品されてしまった案件があればescape_report.jsonへ記録し追跡
- **恒久対応（1ヶ月以内）**：
  5. schema validationの冗長化（2系統並列＋フェイルオーバー）、監視アラートを5分粒度化
  6. QAキュー側にも簡易validation機能を持たせ、提出ゲートとQAゲートの二層防御化
- **数値目標**：障害検知〜暫定運用開始 **≤ 10分**、遡及検証完了 **≤ 24h**、冗長化完了 **≤ 1ヶ月**

### シナリオ5：固有名詞取り違えでクライアント情報混入（別クライアント名・別案件ID）
- **兆候**：宮村建設向け成果物に清一建設の社名・案件IDが混入するなど、7社間のクロスコンタミネーション（06-17記録の重大リスク）
- **即時対応（0〜1h）**：
  1. 該当成果物を即rejected・提出停止、Ryota経由でクライアント連絡（誤情報が既に届いていれば謝罪・回収）
  2. 同一エージェントの同時期全成果物をマスタ突合再走査、他案件への混入有無を24h以内に確定
  3. Nori（11-管理部門）へ即時報告、情報漏洩インシデント該当性の評価開始
- **短期対応（1h〜3営業日）**：
  4. Nori・HARU連携で情報漏洩リスク評価と再発防止策の策定、必要ならクライアントへの正式謝罪
  5. 該当エージェントのテンプレ・変数マッピングを監査、混入経路を根本特定
- **恒久対応（3営業日〜1ヶ月）**：
  6. 全提出ゲートに正本マスタ突合を必須化、案件ID・社名・金額の3点同時一致を強制
  7. 7社の類似社名・略称・工種名を「衝突リスク辞書」に登録し、近似検出時は追加confirm必須化
- **数値目標**：混入検知〜取り下げ **≤ 1h**、Nori報告 **≤ 4h**、恒久対応 **≤ 3営業日〜1ヶ月**、再発 **0件/年**
