...
**強化目的**: 国内AIエージェント組織で唯一無二の「横断QAレビュアー」として、中間QAゲート・スキーマ検証・整合性チェックを機械化する。

### STEP 1: 現状スキル棚卸
- 全エージェント出力の品質検証・相互整合性チェック・スキーマ検証

### STEP 2: 業界最先端ベンチマーク
- JSON Schema / Zod / Ajv（スキーマ検証）
- CUE（Configure Unify Execute）
- Great Expectations（データ品質）
- Contract Testing（Pact）

### STEP 3: スキルギャップ抽出
1. output.json スキーマ集中管理未装備
2. 自動スキーマ検証CIパイプ未装備
3. Cross-agent整合性チェックの自動化未装備
4. QA結果のダッシュ化未装備
...
### STEP 5: 追加ツール
- Ajv / Zod / JSON Schema Validator
- Great Expectations
- Notion QA DB
- GitHub Actions（自動検証CI）

### STEP 6: 追加出力アーティファクト
1. **Schema Registry**（全エージェントのoutput.json）
2. **Validation Report**
3. **Consistency Check Matrix**
4. **中間QAダッシュボード**
...
|---|---|
| スキーマ違反検出率 | 100% |
| Cross-agent不整合検出率 | 95% |
| 検証LT | 5分以内 |

### STEP 8: 失敗パターン
- **スキーマ未維持**：Freeform出力の氾濫
- **sora重複作業**：中間QA機能停止
- **手動チェック**：スケールしない
