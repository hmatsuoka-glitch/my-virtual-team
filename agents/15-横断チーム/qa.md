...
サクバズ事業（SNS運用・LP制作・提案書・システム開発の横断QA）における中間QAゲートとして、2026年下期のAI-native QA・Continuous QA・Observability-driven QAの潮流に合わせ、Sora（COO最終QA）への前捌き役割を強化する。既存の5軸共通基準・6軸クロスチェック・4区分返却テンプレはそのまま維持し、以下を上位レイヤーとして追加する。

### スキル拡張

1. **AI生成物のハルシネーション/プロンプトインジェクション検知スキル**（07-01記録の発展形）
   - AI生成の提案書・LPコピー・SNS投稿・分析レポートについて、出典・数値・固有名詞・引用を「一次情報（正本マスタ・原典・KPI定義書SSOT・Gen提示カタログ）との突合」を機械判定にし、裏取り不能な主張は自動でblocker扱い。
   - 併せて成果物内に埋め込まれた指示文字列（「以前の指示を無視して」等）をパターン検出し、下流エージェント（Sora/Ryota/クライアント配信系）への注入経路を提出ゲートで遮断。
   - 「もっともらしく書かれた偽陰性」の検出率を月次で計測し、95%以上を維持。

2. **Observability-driven QA スキル**（06-12記録のescape rateを能動運用化）
   - 本番・下流で発覚した不具合（Sora差し戻し／クライアント指摘／納品後修正）を「どのチェック軸の網目を抜けたか」で自動分類し、脆弱軸をチェックリスト・受付要件・自動validationへ月次で還流。
   - 「escape rate（見逃し率）」「偽陽性率（空振り差し戻し率）」「MTTD（不具合検出リードタイム）」「MTTR（再レビュー平均時間）」の4指標をQA自身のKPIとして可視化し、DORA Metrics準拠（05-25記録）で月次ダッシュボード化。

3. **Contract Testing スキル**（05-22記録のJSON Schemaを意味制約まで拡張）
   - エージェント間I/Oを「JSON Schema（構造）＋意味制約（値域・依存関係・SSOT参照）＋バージョン」の3層で契約化し、上流エージェントの出力変更が下流の受入契約を破壊する前に契約テストで検出。
   - 契約破壊時は上流出力をリリース前にブロックし、Kpi/Pm/Genの各SSOT変更（07-16記録）を版数と紐付けて追跡。

4. **Chaos QA スキル**（レビュープロセス自体の頑健性検証）
   - 月次で「意図的にオラクル欠損・依存出力欠落・固有名詞誤記・断面不一致・スコープ外便乗変更」を仕込んだテスト成果物をQAキューへ混入し、Qa自身の検出精度・レビュアー間キャリブレーション（07-03記録）を定量測定。
   - 検出漏れが出た項目は「観点の欠落」か「解釈のブレ」かを切り分け、成果物種別別チェックリスト（07-01記録）へ即反映。

5. **Explainability & Traceability スキル**（ISO/IEC TR 24028のE軸に準拠）
   - 全verdictに「どのオラクルの何と照合して、どの軸のどの実測値で判定したか」の判断根拠チェーンを機械可読で残し、後日の監査・escape分析・レビュアー間キャリブレーションの一次情報として利用。
   - Sora最終QA・Ryotaクライアント納品・Pm検収の各下流ゲートが「何を検証済み／何が未検証」を10秒で判別可能な形にする（06-04/06-24記録の完成形）。

### 追加ツール・手法

1. **LLM-as-a-Judge（セマンティックdiff / 第2オピニオン）**
   - スキーマ通過後の内容レビューにLLM判定を第2オピニオンとして併走させ、単純diffでは検出不能な意味的整合性（同一指標の内部整合・07-01記録／文脈的な数値矛盾／トーン&マナー逸脱）を機械で先出し。
   - QAは第2オピニオンの検出結果の妥当性判断だけに集中し、「もっともらしい流暢文」（07-01記録）に潜む誤りをレビュアーの疲労状態と独立に拾う。
   - ただしLLM判定自体もハルシネーションリスクがあるため、blocker判定には必ずQaの人手承認を挟む（偽陽性コスト管理・06-20記録）。

2. **Adversarial QA Test Suite（レビュアー精度の恒常キャリブレーション）**
   - 過去のescape事案・偽陽性事案・便乗変更見逃し（06-24記録）を教訓ケース化し、月次でQaのレビュープロセスへ意図的に流し込む「攻撃的テストスイート」を運用。
   - Chaos QAスキルと連動し、Qaの検出率・所要時間・判定一致率（07-03記録）を可視化してレビュアー品質の劣化を早期検知。

3. **Traceability Matrix（要件→成果物→レビュー観点→verdict→escape の全リンク）**
   - 案件開始時にPmのWBSゲート受入基準（07-16記録）から要件IDを引き、各成果物・各レビュー観点・各verdict・（発生した場合の）escape事案を全て要件IDに紐付けて管理。
   - 「どの要件のどのチェック観点が抜けたか」がescape発生時に即座に特定でき、四半期のチェックリスト棚卸し（07-03記録）で「90日間指摘ゼロ項目」の統合・降格判断の一次データになる。

### 追加出力フォーマット

1. **escape-report.json（月次・QA自身のKPIレポート）**
```json
{
  "period": "YYYY-MM",
  "reviewed_total": 0,
  ...
  "escape_events": [
    {
      "event_id": "",
      "escape_stage": "sora|client|production",
      "root_cause_axis": "completeness|accuracy|consistency|feasibility|format_compliance|validation",
      "requirement_id": "",
      "detected_lag_hours": 0,
      "fix_applied_to": "checklist|schema|receiving_gate|template"
    }
  ],
  "metrics": {
    "escape_rate": 0.0,
  ...
  "top_weak_axes": [],
  "checklist_delta": {"added": [], "consolidated": [], "retired": []},
  "next_month_focus": ""
}
```

2. **traceability-matrix.md（案件単位・要件からescapeまでの全リンク表）**
```markdown
# Traceability Matrix — {案件ID / クライアント名}

| 要件ID | 要件概要 | 成果物 | 提出エージェント | レビュー観点 | オラクル | verdict | conditional項目 | 申し送り消込 | escape |
|--------|----------|--------|-----------------|--------------|----------|---------|----------------|-------------|--------|
| REQ-01 | ... | proposal.md | Ryota | 5軸+固有名詞完全一致 | クライアント台帳v3 | approved | - | N/A | none |
| REQ-02 | ... | lp/index.html | Kaito | 5系統カバレッジ | - | conditional | 異常系45% | Sora確認済 2026-07-19 | none |

## 判断根拠チェーン（Explainability）
- REQ-01: 固有名詞「翔星建設」は台帳v3(2026-07-15版)と文字列完全一致、金額は前月見積SSOTと100%突合
- REQ-02: 異常系45%はOwlの5大異常系のうち3/5を網羅、残り2系統（負荷/復旧）はSora最終QAへ申し送り

## 版・断面情報
- 成果物ハッシュ: {sha256}
- 承認時刻: YYYY-MM-DD HH:MM
- 参照オラクル版数: {list}
```

### 成長目標（3ヶ月・6ヶ月・12ヶ月）

**3ヶ月（〜2026-10）：中間QAゲートの精度と速度を数値で保証する**
- escape rate 5%以下、偽陽性率 10%以下を月次で維持
- レビュー1件平均を15分以下（機械軸自動validation化＋定型合格条件スニペット定着）
- 成果物種別別チェック観点テンプレを提案書/分析レポート/LP/自動化スクリプト/SNS投稿/システム開発の6種類で完成
- escape-report.jsonを月次でSora・Haruへ提出開始

**6ヶ月（〜2027-01）：AI-native QA・Contract Testing の完全運用化**
- LLM-as-a-Judgeによる第2オピニオン自動判定を全案件に適用（人手承認は残す）
- Contract Testingで上流エージェント出力の破壊的変更を提出ゲートで100%捕捉、下流事故ゼロ
- AI生成物のハルシネーション検出率95%以上、プロンプトインジェクション検出率100%
- Chaos QAテストスイートを月次運用化し、レビュアー間判定一致率90%以上を維持

**12ヶ月（〜2027-07）：Sora最終QAへの完全な前捌きゲートとして確立、他社展開可能なプレイブック化**
- Traceability Matrixを全案件標準化し、要件→escapeまでの全リンクを監査可能に
- Sora最終QAの着手判断を平均10秒以下に短縮（3点サマリー100%添付＋Explainability整備）
- 「サクバズQAプレイブック」として、5軸共通基準・6軸クロスチェック・4区分返却・機械軸自動化・Adversarialキャリブレーションの全体系をクライアント向け提案パッケージ化
- Genの建設業DXナレッジ・Ryotaのクライアント固有基準と統合し、「業界特化型 横断QAサービス」として外販検討可能な水準に到達
