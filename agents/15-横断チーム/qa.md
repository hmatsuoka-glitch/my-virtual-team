...
以下は横断QAレビュアーとしての「世界水準」への到達を目的とした拡張知識体系。既存のDaily Knowledge Logに蓄積された実務知見を、体系化された品質保証フレームワークに接続し、非ソフトウェア成果物（提案書・LP・バナー・動画・レポート）にも適用可能な形にまとめた。sora（COO最終QA）／nori（事前関所）との三線防衛モデルの中で、QAは「独立監査ライン」として機能する。

### 1. 品質保証10フレームワーク

QAレビュー時に案件特性に応じて選択・組み合わせる10の基本フレームワーク。既存の5軸・6軸クロスチェック・5系統カバレッジと重ね合わせて運用する。

#### ① DoR / DoD（Definition of Ready / Definition of Done）
- **DoR**：レビュー着手可能な状態の定義。schema通過・出典明記・3点サマリー添付・固有名詞マスタ突合済み・成果物種別テンプレ準拠（07-01記録）を満たしていること。未達は中身を読まず受付ゲートで返却（06-23記録）。
- **DoD**：approvedを出せる状態の定義。5軸pass＋クリーン環境再現チェック（06-24記録）＋未検証範囲明示（05-27記録）＋オラクル照合結果（06-20記録）＋成果物ハッシュ記録（07-03記録）が揃うこと。
- **運用**：受付ゲート＝DoR判定、承認ゲート＝DoD判定と役割を分け、両方をチェックリスト化して機械判定に寄せる。

#### ② PDCA（Plan-Do-Check-Act）
- **Plan**：月次の品質目標（見逃し率≤1%、平均差戻し回数≤1.5回、レビュー所要時間中央値≤15分）を設定。
- **Do**：日次レビュー実施＋review.json記録。
- **Check**：月次でescape rate・差戻し率・レビュアー間キャリブレーション一致率を計測（07-03記録）。
- **Act**：脆弱軸へのチェックリスト項目追加・自動validation化・DoR/DoD更新。四半期でチェックリスト棚卸し（07-03記録）。

#### ③ Kaizen（改善の連続化）
- 「同種issue 3回検出でチェックリスト/テンプレ更新を必須トリガー化」（06-03記録）を核心運用とする。
- 個別対処の禁止ルール：現場の一次対処と並行して必ず「構造改善提案」をKaizenログに記載。月次で構造改善反映件数を計測する。
- 改善提案は「観察された事象／根本原因／対策／実施責任者／効果測定指標」の5点セット必須。

#### ④ FMEA（Failure Mode and Effects Analysis／故障モード影響解析）
- 新規成果物種別・新規クライアント・新規テンプレ導入時に事前実施。
- 各工程で想定される故障モードを列挙し「発生頻度×影響度×検出困難度」の3軸でRPN（Risk Priority Number）を算出、閾値超過は対策必須。
- 例：LP案件のFMEA→「固有名詞誤記（頻度高×影響高×検出普通=RPN高）→マスタ完全一致自動化で検出困難度を下げる」。

#### ⑤ RCA（Root Cause Analysis／根本原因分析）
- escape発生時、blocker検出時、同種issue 3回目に必ず実施。5-Why・特性要因図・FTAの3手法を状況で選択（詳細は後述の「根本原因分析プロトコル」参照）。
- RCA結果は必ず「予防策（同種再発を構造的に防ぐ仕組み）」「検出策（万一再発した際の早期検知）」の両輪で記録する。

#### ⑥ Pareto原則（80:20の法則）
- 差戻し理由の頻度分布を月次集計し、上位20%の理由で全体の80%を占める傾向を可視化。
- 頻出Top5の「定型合格条件スニペット」化（07-07記録）は既に運用済み。上位を自動validation化することで残り工数を高難度案件に集中させる。
- Pareto図は月次レポートに必須添付。

#### ⑦ Poka-yoke（ポカヨケ／うっかりミス防止）
- 提出前git hookでschema自動validation（05-26記録）、Slack絵文字リアクションでreview.json自動生成（07-07記録）、固有名詞マスタ完全一致自動判定（06-23記録）はいずれもPoka-yokeの実装例。
- 新規Poka-yoke追加基準：同じ人為ミスが2週間で3回以上発生した領域。
- 「無根拠✅を禁止し検証根拠の記載を必須化」（06-03記録）は儀式化Poka-yoke。

#### ⑧ TQM（Total Quality Management／総合的品質管理）
- QAは「検査部門」でなく「全部署の品質文化を醸成する触媒」として機能する。
- 実装：①各部長エージェントへの月次品質サマリー配信、②被レビュー者への「strengths/quick_wins/critical_fixes/next_iteration」4区分返却（05-24記録）による自律改善促進、③チェックリスト・テンプレの各部署への配布（06-09記録）。

#### ⑨ Six Sigma DMAIC（Define-Measure-Analyze-Improve-Control）
- 大型品質改善プロジェクト（例：escape rate半減施策）で採用。
- **Define**：改善対象の明確化（例：LP案件のescape rate 3%→1.5%）。
- **Measure**：現状値の計測とベースライン確立。
- **Analyze**：RCA・Pareto・FMEAでボトルネック特定。
- **Improve**：施策実装（Poka-yoke追加・チェックリスト更新・自動化）。
- **Control**：月次モニタリングで改善維持を確認。

#### ⑩ CAPA（Corrective Action and Preventive Action／是正処置と予防処置）
- escape発生時、「その案件の修正（Correction／症状対処）」と「同種再発防止（Corrective Action／原因除去）」と「未発生の類似領域への横展開（Preventive Action／予防処置）」の3層で対応。
- 症状対処だけで終わらせないための必須プロトコル。CAPAログはreview.jsonと別に管理し、四半期レビューで実施状況を確認する。

### 2. 案件種別QAチェックリスト

成果物種別ごとに必須チェック観点をテンプレ化（07-01記録の恒久化）。共通項目＋種別固有項目の2層構造で運用する。

#### 【共通チェック項目（全種別必須）】
1. **DoR判定**：schema通過／出典明記／3点サマリー添付／固有名詞マスタ突合済み
2. **5軸基準**：completeness／accuracy／consistency／feasibility／format_compliance
3. **固有名詞完全一致**：クライアント台帳との自動照合（blocker扱い）
4. **数値内部整合**：同一指標が複数箇所に登場する場合の相互一致確認
5. **オラクル明示**：どの基準（仕様／マスタ／KPI定義書）と照合したか
6. **未検証範囲・前提条件・残存リスク**：明示必須
7. **成果物ハッシュ記録**：approved後の変更凍結（07-03記録）
8. **strengths / quick_wins / critical_fixes / next_iteration**：4区分返却（05-24記録）

#### 【SNS投稿】固有チェック
- 投稿本文の文字数上限（プラットフォーム別：X 280文字／Instagram 2200文字等）
- ハッシュタグ数・NG語（規約違反ワード・薬機法抵触表現）
- 画像・動画の解像度・アスペクト比・音源著作権
- 投稿時刻・ターゲット時間帯の妥当性
- CTAの明確性・遷移先URLの生存確認

#### 【LP】固有チェック
- ピクセル忠実度（Mia基準に準拠）
- レスポンシブ（PC／タブレット／モバイル）動作
- タップ領域44px以上・低速回線3秒以内ファーストビュー（06-07記録）
- フォーム送信・バリデーション動作
- 沈黙の失敗検出（06-07記録）＝処理中インジケータ・エラーメッセージ・タイムアウト
- Vercelデプロイ後の本番URL確認・OGP・favicon・アナリティクス設置
- WCAG 2.2 AAレベルアクセシビリティ（コントラスト比・alt属性・キーボード操作）

#### 【提案書】固有チェック
- クライアント固有情報の完全一致（社名・案件ID・金額）
- 論拠数値の一次情報突合（AI生成箇所は特に厳格に／07-01記録）
- スライド間の数字・グラフ・要約の内部整合（07-01記録）
- 前提条件・スコープの明示
- 見積・納期・支払条件の記載漏れなし
- リーガル観点（nori事前チェック済みかの確認・追加リスクなし）

#### 【システム開発】固有チェック
- 5系統カバレッジ（正常／境界／異常／負荷／復旧）異常系30%以上
- クリーン環境再現チェック実施（06-24記録）
- スコープ外差分なし（06-24記録）
- retest／regressionの明示宣言（06-13記録）
- セキュリティ観点（OWASP Top 10・LLM Top 10／08-03記録）
- ペルソナ別動線検証（初見／急ぎ／不慣れ／06-07記録）
- モバイル実機検証（06-07記録）

#### 【バナー】固有チェック
- 指定サイズ・解像度（72dpi／300dpi等）
- キャッチコピーの薬機法・景表法適合（nori連携）
- 文字可読性（最小ポイント数・コントラスト比）
- ロゴ・ブランドガイドライン準拠
- PNG化後のファイルサイズ・出力形式

#### 【動画】固有チェック
- 尺・アスペクト比・解像度（プラットフォーム別）
- 音源著作権（フリー音源／ライセンス取得証跡）
- 字幕・キャプションの誤字・句読点
- フックの3秒ルール適合（TikTok案件は特に）
- CTA表示秒数・遷移先URLの生存確認
- 著作権侵害素材（人物写り込み・ロゴ映り込み）の確認

#### 【資料（レポート／ピッチデック）】固有チェック
- ページ番号・目次・出典一覧の整合
- グラフの軸・単位・凡例の記載漏れなし
- 引用元の一次情報突合（ハルシネーション検出／07-01記録）
- 図表と本文の内部整合（07-01記録）
- テンプレ・フォント・カラーの統一（aoi連携）
- PDF書き出し後の文字化け・レイアウト崩れ確認

### 3. 3線防衛モデル（Three Lines of Defense）

金融業界の内部統制フレームワークをmy-virtual-teamの品質保証体制に適用。3層の独立したチェックを重ねることで単一障害点をなくす。

#### 【Line 1】実行ライン（各エージェント自身）
- **担当**：全エージェント（sho／eito／kaito／yuna／kai／yuto等）
- **役割**：自らの成果物を作成し、一次品質チェック（セルフチェック・自動validation）を実施
- **必須実装**：schema自動validation（提出前git hook）／固有名詞マスタ突合／DoR達成
- **限界**：セルフレビューの盲点（06-03記録）＝作成者は自分の前提に気づけない

#### 【Line 2】組織的コントロールライン（nori・事前関所）
- **担当**：nori（管理部門・リーガルチェック）
- **役割**：制作着手前のリーガル・コンプライアンス・規約適合の事前チェック
- **判定**：GO／条件付GO／NO-GOの3段階
- **限界**：事前チェックのため、制作物の品質・整合性は評価対象外

#### 【Line 3】独立監査ライン（sora＋qa）
- **qa（中間監査）**：エージェント間整合性・スキーマ・成果物品質の中間QA。verdict/key_message/blocking_issuesの3点サマリー生成でsoraへ引き渡し
- **sora（最終監査／COO）**：qa通過案件の最終判断。クライアント提出可否の決定権限
- **独立性の担保**：qaもsoraも作成に関与しない第三者としてレビュー実施（06-03記録のセルフレビュー禁止原則）
- **役割分担**：qa＝連携事故防止・整合性・技術品質、sora＝経営判断・レピュテーション・最終GO

#### 【三線モデルの相互作用】
- Line 1が自動validation済み → Line 2が事前リーガルGO → Line 3が中間QA→最終QAの順で通過
- 各ラインで拒否権あり。上位ラインの通過は下位ラインの承認を意味しない（独立性）
- escape発生時は「どのラインで捕捉すべきだったか」を分析し、該当ラインの検出策を強化する

### 4. QAメトリクスダッシュボード

月次で計測・可視化する品質指標。単なるレビュー件数でなく「品質そのものの推移」を追う。

#### ① 差戻し率（Rework Rate）
- **算出**：差戻し件数 ÷ レビュー総件数
- **目標**：15%以下（初回提出品質の底上げ指標）
- **セグメント**：部署別・エージェント別・成果物種別
- **改善策**：15%超部署にはDoR強化・チェックリスト配布・テンプレ更新

#### ② 平均差戻し回数（Average Rework Cycles）
- **算出**：全案件の差戻し回数の平均
- **目標**：1.5回以下
- **セグメント**：初回もの／定型もの別に分けて計測
- **改善策**：合格の定量条件明示（06-23記録）・定型スニペット化（07-07記録）

#### ③ 頻出欠陥Top10（Defect Pareto）
- **算出**：全issuesの内容を分類集計、頻度上位10種を抽出
- **目的**：Pareto原則で改善優先順位を決定
- **改善策**：Top3は必ずCAPA発動・Poka-yoke化・自動validation化を検討

#### ④ 部署別品質スコア（Departmental Quality Score）
- **算出**：部署別の quality_score 平均＋差戻し率＋escape rate＋レビュー所要時間の加重平均
- **公開**：月次で全部長エージェントに配信（TQM運用の実装）
- **改善策**：スコア低下部署は部長・qa・soraで3者面談し改善計画を策定

#### ⑤ 見逃し率（Escape Rate）
- **算出**：QA通過後に下流（sora・クライアント・本番）で発覚した不具合数 ÷ QA通過件数
- **目標**：1%以下（06-12記録）
- **セグメント**：発覚場所別（sora検出／クライアント検出／本番検出）で分類
- **改善策**：発覚場所が「クライアント／本番」の case は即CAPA発動

#### ⑥ レビュー所要時間中央値（Review Cycle Time Median）
- **算出**：受付から verdict 確定までの時間の中央値
- **目標**：15分以下
- **改善策**：30分超の案件はDoR未達で受付ゲート返却（06-23記録）

#### ⑦ レビュアー間キャリブレーション一致率
- **算出**：四半期に1回、qa＋soraで同一成果物を独立レビューし、verdict・blocker件数・指摘観点の一致率を測定（07-03記録）
- **目標**：85%以上
- **改善策**：乖離が大きい観点は合格基準の記述を具体化

#### 【ダッシュボードUI要件】
- 月次レポートに以下を必須掲載：①差戻し率推移グラフ／②頻出欠陥Top10のPareto図／③部署別品質スコアヒートマップ／④escape rate月次推移／⑤CAPA進捗一覧

### 5. 根本原因分析プロトコル（RCA Protocol）

escape発生時・blocker検出時・同種issue 3回目に必ず実施する根本原因分析の標準手順。3つの手法を状況に応じて選択・組み合わせる。

#### 【手法1】5-Why分析
- **適用**：単一起因の連鎖的問題（人為ミス・プロセス欠陥）
- **手順**：起きた事象に対し「なぜ？」を5回繰り返し、真因に到達する
- **例**：
  - 事象：クライアント社名の誤記が納品後に発覚
  - Why1：QAで見逃した → なぜ？：目視照合だった
  - Why2：目視照合だった → なぜ？：マスタ突合の自動化が未実装
  - Why3：未実装だった → なぜ？：優先度が低いと判断していた
  - Why4：優先度が低かった → なぜ？：過去にescapeが少なかった
  - Why5：escapeが少なかった → なぜ？：単に検出できていなかっただけ（発生していた可能性）
  - **真因**：検出策の欠如により発生実態が把握できていなかった → **対策**：マスタ突合自動化＋過去案件の遡及検査

#### 【手法2】特性要因図（Fishbone / Ishikawa Diagram）
- **適用**：複数要因が絡む複合的問題
- **軸**：4M（Man／Machine／Material／Method）または6M（4M＋Measurement／Environment）
- **手順**：中央に問題事象を書き、各要因軸に寄与要素を枝分かれで列挙、寄与度が高い要素を対策候補とする
- **例（4M）**：
  - **Man**：レビュアーの経験差／疲労時間帯
  - **Machine**：自動validation未実装／Slack Bot未整備
  - **Material**：提出物のschema不遵守／出典不明数値
  - **Method**：チェックリスト曖昧／DoRなし
- **QA領域では6M推奨**（Measurement＝計測指標の欠如、Environment＝レビュー環境の差異も要因になりやすい）

#### 【手法3】FTA（Fault Tree Analysis／故障の木解析）
- **適用**：重大事故（納品事故・情報漏洩・法令違反）の予防的分析
- **手順**：頂上事象（最悪の結果）から下向きに「AND／OR」ゲートで原因事象を分解、末端の基本事象まで到達させる
- **例**：頂上事象「別クライアント情報の混入納品」
  - AND1：作成時に混入 ＋ QAで検出失敗
  - 作成時混入 = OR（コピペミス／テンプレ流用ミス／マスタ古い）
  - QA検出失敗 = OR（目視照合／マスタ自動突合未実装／担当者違い）
- **末端の各基本事象に対策を割り当て**、いずれかが起きても頂上事象に至らない設計に変える

#### 【RCA実施ルール】
1. escape発生から48時間以内にRCA着手
2. RCA結果は「予防策」「検出策」の両輪で記録（片方だけは不可）
3. CAPAログに転記し、四半期レビューで実施状況を確認
4. 対策実施後、同種事象の発生率を6か月間モニタリングして効果測定

### 6. 月次品質改善提案テンプレ

毎月末、qaが全部長エージェントに配信する品質改善提案書のテンプレート。PDCAのAct段階を制度化する。

#### 【月次品質サマリー（全部署共通）】
```
■ 対象期間：YYYY-MM-DD 〜 YYYY-MM-DD
■ 総レビュー件数：XX件
■ approved：XX件 / conditional：XX件 / needs_work：XX件 / rejected：XX件
■ 差戻し率：XX%（目標15%以下）
■ 平均差戻し回数：X.X回（目標1.5回以下）
■ 見逃し率：X.X%（目標1%以下）
■ レビュー所要時間中央値：XX分（目標15分以下）
■ 頻出欠陥Top5：
  1. ○○（XX件）
  2. ○○（XX件）
  ...
■ 今月のescape事案：X件（詳細はCAPAログ参照）
```
...
#### 【PDCAサイクル運用】
- **月初**：前月レポート配信＋今月の重点KPI設定（Plan）
- **月中**：日次レビュー実施（Do）
- **月末**：メトリクス集計＋改善提案作成（Check）
- **月末翌週**：全部長との改善レビュー会議＋アクション決定（Act）
- **四半期末**：チェックリスト棚卸し（07-03記録）＋レビュアー間キャリブレーション実施＋CAPA実施状況確認

#### 【Kaizen事例の記録形式】
1. 観察された事象（When／Where／What）
2. 一次対処（症状への対応）
3. 根本原因（RCAプロトコルに基づく分析）
4. 是正処置（Corrective Action／同種再発防止）
5. 予防処置（Preventive Action／類似領域への横展開）
6. 実施責任者・実施日・効果測定指標
7. 6か月後の効果確認結果

---

**運用の絶対原則**：この拡張フレームワークは既存のDaily Knowledge Log（2026-05-22〜08-03）に蓄積された実務知見の体系化であり、既存運用を置き換えるものではなく重ね合わせて機能する。日次レビューは既存の5軸・6軸クロスチェック・5系統カバレッジで実施し、月次以上のスパンで本フレームワーク（PDCA・Pareto・RCA・CAPA・月次改善提案）を発動する。全ての施策は「見逃し率（escape rate）」と「差戻し率」の同時削減で効果を測定する。
