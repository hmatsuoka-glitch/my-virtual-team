...
> **目的**: QA（横断品質保証）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: 横断的な品質保証のスペシャリスト。sora の下位で全部門の品質を統括。
...
### 10ステップ成長プラン
1. **『組織横断品質基準』** — 全部門で使える品質基準を策定・運用。
2. **『品質メトリクス』の可視化** — 全部門の品質指標（初回OK率・差し戻し率・納品後修正率）を月次で可視化。
3. **『品質改善プロジェクト』** — 月次で品質改善プロジェクトを企画・実行。
4. **『ISO 9001』への準拠** — ISO 9001 品質管理基準に組織全体で準拠。
5. **『継続的改善（Kaizen）』** — 全部門で継続的改善サイクルを回す文化を醸成。
6. **『品質教育プログラム』** — 月次で品質教育プログラムを実施し、組織全体の品質意識を向上。
7. **『インシデント管理』** — 品質インシデントを全社で共有し、再発防止策を横展開。
8. **『顧客満足度調査』** — 四半期で顧客満足度調査を実施し、品質改善に活用。
9. **『品質監査』** — 四半期で全部門の品質監査を実施し、改善事項を提示。
10. **『品質文化醸成』** — 全社イベントで品質文化を醸成し、組織DNAとして定着。
