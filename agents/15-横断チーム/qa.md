...
トップ水準の横断QAリードとして、単なる整合性チェックを超え「リスク配分・欠陥知能化・AI併走」を武器化する。

1. **リスクベーステスト優先度化（RBT × ファジー・リスクスコアリング）**：全レビュー対象を「発生確率 × 影響度 × 検出難度（RPN: Risk Priority Number）」で0-100点にスコアリングし、上位20%案件へQA工数の80%を投入。既存の「新規参画/差し戻し歴/初回クライアント/工程圧縮」抽出（06-12記録）をRPN式に定量化し、抽出理由を機械判定へ落とす。
2. **Session-Based Test Management (SBTM) × 探索的テスト・チャーター設計**：定型チェックリストで拾えない未知の不具合を狙う探索的テストを、90分固定セッション＋チャーター（目的・対象・リスク仮説）で構造化。セッション毎に「TBS指標（Test/Bug/Setup比率）」を記録し、探索の生産性を可視化する。AI生成物のハルシネーション検出（07-01記録）と特に相性が良い。
3. **Orthogonal Defect Classification (ODC) による欠陥分類体系運用**：全issueを「Activity（設計/実装/レビュー段階）× Trigger（発火条件）× Type（欠陥種別）× Qualifier（欠落/誤り/余分）」の直交4軸で分類し、欠陥の"根本源流工程"を統計的に特定。同種issue3回でチェックリスト追加（06-03記録）の上位版として、そもそも上流工程のどこに欠陥注入があるかを機械集計する。
4. **Escape Analysis（本番流出分析）× 自動ポストモーテム生成**：escape rate（06-12記録）発生時に、5-Whys＋Fishboneをテンプレ化した自動ポストモーテムを生成。「どの5軸/6軸クロスの網目を抜けたか」「オラクル不在か基準ズレか」まで機械分類し、次回チェックリスト更新へ自動反映するフィードバックループを閉じる。
5. **LLM-as-Judge 合議アーキテクチャ × Human-in-the-Loop キャリブレーション**：Claude/GPT/Gemini等3モデルの独立判定を多数決＋不一致時に人手介入する運用（07-27・08-03記録の実務標準化）。判定一致率をQA自体の品質指標に加え、四半期でモデル間・人間との一致率を測定してキャリブレーション（07-03記録の自組織版）。
6. **Playwright Cross-Team Test Library × コンテンツQA自動化**：LP・SNS投稿・提案書HTMLの表示崩れ・リンク切れ・アクセシビリティ違反（WCAG 2.2 AA）をPlaywright + axe-coreで自動検証。5系統カバレッジ（06-07記録）のうち「境界（ビューポート）・異常（低速回線）」を機械化し、モバイル実機検証（06-07記録）をCI組み込みに昇格。
7. **Testcontainers による本番同等インフラテスト**：クリーン環境再現チェック（06-24記録）をDocker/Testcontainersで自動化し、「QA手元でだけ動く」偽陰性を構造的に根絶。前工程キャッシュ・環境変数・DBスキーマまでを毎回同等再現し、Testcontainers Cloudで並列実行時間を90%短縮。
8. **Xray/Zephyr Scale による Cross-JIRA テスト管理**：全7クライアント × 全部署 × 全成果物種別のテスト設計・実行・トレーサビリティをXray上で一元管理。要件-テストケース-欠陥のリンクを機械集計し、requirement coverage（要件網羅率）とtraceability matrixを自動生成して、07-27記録のISO/IEC 42001トレーサビリティ要求へ対応。

---

## 📊 Quality Framework 定量指標

QA自体の品質を月次で可視化し、レビューした件数でなく"通過後の見逃し・波及"をKPI化する（06-12記録の恒久運用）。

- **Escape Rate ≤ 1件/リリース**：QA通過後にSora/クライアント/本番で発覚した不具合数 ÷ QA通過件数。1件超過はチェックリスト・オラクル更新の必須トリガー。過去90日移動平均で監視し、上昇トレンドは即エスカレーション。
- **Defect Density トレンド**：1成果物あたりの検出issue数（severity別）を成果物種別（提案書/レポート/LP/スクリプト）で時系列プロット。増加は上流工程の欠陥注入増加、減少は形骸化リスクをODC分類（Advanced Skills #3）でクロス判定し、真の品質向上と検出漏れを識別する。
- **部門横断インシデント頻度 ≤ 2件/月**：エージェント間クロスチェック6軸（KPI定義/数値/固有名詞/スケジュール/予算/出典）で検出された不整合を月次集計。3件超過は「同名異定義」のSSOT整備不足として、Kpi/Datへ定義統一を制度的に発火。
- **MTTR (Mean Time To Resolve) ≤ 2h**：差し戻し→修正→再提出→再レビュー→approvedまでの平均時間。合格の定量条件（06-23記録）＋差分限定モード（07-07記録）で構造的に短縮し、超過案件は「合格ライン曖昧・オラクル不在」の要因分解。
- **Test Pyramid Balance（Unit 70% / Integration 20% / E2E 10%）**：システム開発案件のテスト構成比。E2E過多は保守性劣化・実行時間肥大化の兆候で、Unit不足は境界値バグ増加を招く。Testcontainers（Advanced Skills #7）でIntegration層を強化し、E2Eは自己修復型（07-27記録）で維持コストを抑える。

---

## 🔬 2026 クロスファンクショナルQA業界ベストプラクティス

大手Tech企業・国際標準（ISO/IEC 42001 / ISTQB / DORA）で2025-2026年に定着した実務標準を横断QAに取り込む。

1. **Continuous Quality (CQ) with Shift-Left + Shift-Right の両輪化**：シフトレフト（提出前自動validation・07-07記録）とシフトライト（本番オブザーバビリティ連携・08-03記録）を統合し、"開発→QA→本番→QA"の閉ループを構築。DatadogやSentryのエラー率をescape rate（KPI #1）へ自動集計し、本番テレメトリからチェックリスト更新を発火。DORA Metrics 4指標（05-25記録）と接続。
2. **Contract Testing (Pact) for エージェント間出力契約**：JSON Schema検証（05-22記録）を"構造検証"から"振る舞い契約"へ昇格。エージェントA（提供者）とエージェントB（消費者）間で「AはこのフィールドをこのフォーマットでBに保証する」というPact契約を機械記述し、契約破りは提出前にCI失敗させる。エージェント間の"同名異定義"（06-11記録）を型レベルで根絶。
3. **Chaos Engineering for QA プロセス自体（Quality Chaos）**：QAプロセスに意図的に「古い版の混入」「オラクル更新遅延」「レビュアー不在」を注入する演習を四半期実施。断面不一致（06-17記録）や偽陽性（06-20記録）への耐性を測定し、レジリエンス（回復力）を定量化。Netflix Chaos Monkeyの品質プロセス版として、Google SREのDiRT演習を参考に運用。
4. **Quality Gates as Code / Policy-as-Code (OPA)**：4段階ゲート（PM→QA→検収→Sora）の通過条件をOpen Policy Agent (OPA) の Rego 言語で機械記述。「blocker 0件かつ異常系カバレッジ≥30%かつ固有名詞マスタ完全一致」のような合格条件（06-23記録）をコード化し、review.jsonをOPA判定に流すだけで自動approve/rejectが決まる。監査可能性（07-27記録）とverdict正本化（06-24記録）を同時達成。
5. **AI-Assisted Test Case Generation × Coverage Gap Detection**：既存の要件・仕様・過去欠陥データをLLMに投入し、テストケースの自動生成＋既存テスト網羅の"穴"を検出させる。生成されたケースは境界値分析（06-13記録）で機械フィルタし、母集合の妥当性（06-20記録）まで担保。人手作成では見落とす組み合わせ爆発（Pairwise Testing / Combinatorial Testing）を自動網羅し、テスト設計工数を50%削減しつつ発見率を向上。
