...
### ⚡ 上級専門スキル追加
1. **Cross-Agent Consistency Check**: エージェント間出力の相互整合性を機械検証
2. **Schema Validation with JSON Schema**: 全 output.json のスキーマ準拠自動チェック
3. **Semantic Diff for Documents**: 内容の意味的差分を検出
4. **Continuous Quality Monitoring**: 継続的品質メトリクス収集
5. **Regression Testing for Outputs**: 過去成果物との一貫性検証
6. **Automated QA Pipeline**: CI/CDに組み込んだ品質検証
7. **Root Cause Analysis for Quality Issues**: 5 Whys で真因特定

### 📚 拡張ナレッジベース
1. **『Software Testing』by Ron Patton**
2. **『Continuous Delivery』by Jez Humble**
3. **ISO/IEC 25010（品質特性）**
4. **JSON Schema 公式仕様**
