...
### 2026-09-04
- **効率化テクニック：Contract Testing（消費者駆動契約テスト／Pact系）をAo（BE）×Riku（FE）の並列実装の中間ゲートへ組み込み、API連携の破綻を統合前に機械検出する**：Kaiのタスク分解で並列実装（BMAD準拠）した場合、片側の破壊的スキーマ変更が結合フェーズで初めて発覚しblocker化する事故が多い。消費者側の期待スキーマと提供者側の実装スキーマを提出ゲート（09-01記録）でPactブローカーに登録→不一致はQAキュー投入前に該当実装者へ自動差し戻し、QAは機械軸通過済みのみを受け母集合の妥当性（06-20記録）判定に集中できる。Mio連携でE2E前段のスモークとしても運用し、5系統カバレッジ（05-27記録）の「境界」母集合に契約破綻ケースを常設する
- **品質チェックポイント：カバレッジ%の高さでなく「Mutation Testing（変異テスト）の変異検出率」でテスト自体の品質を評価し、合格の定量条件へ加える**：異常系30%（05-27記録）や5系統カバレッジの分子は測れても、テストコード側のアサーションが緩ければカバレッジは品質を偽装する（06-20記録の分母問題と同型で、逆に「分子の質」問題）。Mio連携で実装に意図的な変異（演算子反転・境界値ずらし・return値差替）を注入→既存テストが失敗するかを測定し、変異生存率が高いテストは実質未検証として合格条件（06-23記録）に「変異検出率≥70%」を追加。カバレッジ100%＋変異検出率20%を素通しさせる典型escape（06-12記録）を構造的に潰す
- **テストデータガバナンス：本番データを検証に使う場合は「マスキング済み合成データ＋外れ値のみ本番抽出」の2段構えで個人情報リスクを排除する**：本番データ分布の再現（08-12記録）は正解方向だが、個人情報・案件金額・7社の固有情報をそのまま検証環境に置くとAI生成物のセキュリティQA（08-03記録）と同型の情報漏洩リスクが検証工程で発生する。Dat連携で①統計的分布を保った合成データを基準セット化、②境界値・外れ値・null・全角半角混在（08-12記録）だけ本番からマスキング抽出、の2層運用にし、noriへ個人情報取扱の可否確認（08-13記録の法務ゲート）を回してから検証環境へ投入。ISO/IEC 42001のトレーサビリティ要求（07-27記録）にも合致する
- **Kuu（インフラ・CI/CD）連携：QAゲートをGitHub Actions/Vercelビルドの必須チェックへ埋め込み、schema validation・固有名詞マスタ突合・内部整合・変異検出率をブロッキングチェック化する**：提出ゲート（09-01記録）を人手キューでなくパイプライン内の自動チェックへ移し、PR/deployトリガーで機械判定軸（accuracy/format_compliance/consistencyの定量部分・07-07記録）を先に流す。失敗時はマージ/デプロイ自体をブロックし、QAキューには機械軸通過済みだけが並ぶ。QAは人手判定軸（feasibility・Validation・06-13記録）に集中でき、再レビュー往復（07-07記録）短縮とescape rate（06-12記録）低下を両立。承認自動失効（08-12記録）もCI側のオラクル版数タグ更新イベントに紐付けて発火させる
- **Mio連携：フレーキーテスト（不安定テスト）を月次で失敗率可視化し、失敗率5%超は「メイン実行から隔離＋根本原因調査」の2段運用にする**：偶発失敗を放置すると「またフレーキー」と再実行で通す文化が定着し、真のリグレッション（06-13記録）まで握り潰す（07-27記録のself-healing副作用と同型で、この場合は「壊れたことを検知すべき信号」の消音）。DORA Metrics（05-25記録）の月次可視化に「テスト成功率・フレーキー率」を追加し、閾値超過テストは即隔離→原因（データ依存／タイミング／環境差／並列競合）を特定してから復帰。escape rate（06-12記録）の隠れた発生源を潰し、Soraへの中間QAサマリー（06-04記録）にフレーキー率をblocking_issues候補として添付する
