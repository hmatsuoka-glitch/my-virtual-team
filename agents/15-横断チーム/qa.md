...
### 2026-08-08
- **失敗パターン：AI生成物の出典・URL・参考文献を「存在チェック（URLが200を返す）」だけで通し、そのURL先が実際に主張の根拠になっているかを読まずに承認する** → 回避策：AI生成物の出典は「① URL/ファイルが実在（HTTP 200・パス到達可）② そのページ/資料に主張の該当箇所が実在（本文中の該当パラグラフを引用抜粋してreview.jsonに記録）③ 該当箇所が主張を支持している（曲解・逆解釈でない）」の3段検証を必須にする（理由：ハルシネーション裏取り・07-01記録の実務では「実在URLに実在しない主張を紐づける」高度な偽陰性・06-20記録がAI生成物で頻出する。存在チェックだけだとチェリーピッキング・07-16記録より一段悪い「実在するが無関係な出典」を通す。3段検証で該当箇所抽出まで機械可能化する）
- **効率化テクニック：チェリーピッキング検出を「反対証拠検索」の定型ワークフローで機械化する**。AI生成物・分析レポート・提案書の結論に対し「同じテーマで結論と食い違いうる資料/データを最低3点」を意図的に検索し、review.jsonの「反証検討」欄に記載する。反証が見当たらなければ「本当に反対証拠がない領域」か「検索が甘い」かを区別し、後者なら追加検索を差し戻す。Genの反証チェック（Genの07-03記録）を横断QAの成果物種別テンプレ（07-01記録）に展開し、AIハルシネーション（07-01記録）とチェリーピッキング（07-16記録）を同じ「反対を探す」動作で同時に検出する
- **品質チェックポイント：カバレッジ数値（05-22記録）と実質網羅の距離を「ミューテーションテスト（Mutation Testing）」で測定する**。異常系カバレッジ80%と表記されていても、テストが本当にバグを検出できるかは別問題（分母の妥当性・06-20記録と同型）。コード/ロジックに意図的な誤り（境界を1ずらす・符号を反転する・分岐を除去する）を注入し、既存テストがそれを検出できるか（mutation score）を測ると、「テストは通るが実は何も検証していない」形骸化テストが定量的に露見する。システム開発案件・自動化スクリプト系の受付要件（06-23記録）に「mutation score ≥ 70%」を追加し、カバレッジ％の見せかけを構造的に潰す
- **連携ヒント：Sora（COO最終QA）連携で「中間QA→最終QA」の申し送り事項を、Soraのペルソナ判定基準（決裁者/実務者視点）にマップして渡す**。verdict/key_message/blocking_issues（06-04記録）に加え、「Soraが決裁者視点で見るべき論点」と「実務者視点で見るべき論点」を分けて明示すると、Soraが同じ成果物を2軸で並列レビューでき、最終QAの見落としが減る。中間QAはValidation（06-13記録の"そもそも正しいものか"）とVerification（"仕様通りか"）のうちどちらまで実施したかも明記し、Soraが担当すべき残り軸を明確に渡す
- **トレンド察知：EU AI Act (2026年適用フェーズ) およびISO/IEC 42001 (07-27記録) 準拠で、AI生成成果物の「学習データ由来リスク・偏見バイアス検証」がQA項目に加わる**。特にクライアント提出物のAI生成部分は「学習データが誰の著作物か・特定属性への差別的表現がないか・生成プロセスのログが監査可能か」を成果物種別テンプレ（07-01記録）へ組み込む必要が出てきた。従来のペルソナ検証（06-07記録）を「多様性検証（人種・性別・年齢・障害の有無）」に拡張し、AIレッドチーミング（08-03記録）とセットで運用する。トレーサビリティ証跡（07-27記録）はreview.jsonの承認正本（06-24記録）にAI生成部分の生成モデル名・プロンプト・生成日時を必須追記する形で対応する
