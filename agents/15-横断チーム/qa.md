...
### 現状スキル評価
横断QAレビュアーとして review.json スキーマ・整合性チェック・エージェント間差し戻し運用は成熟しており、日常的な観点（現場条件プリセット・帳票再現・修正導線）も日次ログとして高解像度で蓄積されている。一方で、業界標準の品質モデル（ISO/IEC 25010）への準拠、UI自動テスト・視覚回帰・アクセシビリティ自動化、テスト管理ツール（TestRail/Xray）を用いたトレーサビリティ・カバレッジ管理は明文化されておらず、感覚的判定と手動レビューへの依存度が高い。技術QAの再現性・監査可能性を国際規格レベルまで引き上げる余地がある。

### 特定した改善余地
- ISO/IEC 25010品質モデル8特性（機能適合性・性能効率性・互換性・使用性・信頼性・セキュリティ・保守性・移植性）に基づくレビュー観点の体系化不足
- E2E自動テスト（Playwright/Cypress）と視覚回帰（Percy/Applitools）のパイプライン組み込みが未整備
- WCAG 2.2 / axe-core によるアクセシビリティ自動監査の欠落
- TestRail・Xray等でのテストケース資産化・要件-テスト-バグのトレーサビリティ管理が未定着
- リスクベーステスト（RBT）とテスト十分性判断（EMTE/ODC）による「どこまでやれば止められるか」の定量根拠が弱い

### 追加スキル10選（オーバースペック化ロードマップ）
1. **ISO/IEC 25010:2023 品質特性マトリクス** — review.jsonの`specific_criteria`を8品質特性×31副特性で自動生成／KPI：レビュー観点網羅率95%以上、特性別スコア標準偏差0.5以内
2. **Playwright（Test Generator + Trace Viewer）** — critical userフローのE2E自動化とトレース添付／KPI：主要フロー100%自動化、flaky率1%以下、CI実行時間8分以内
3. **Cypress + cypress-axe** — コンポーネント/E2E統合テストとアクセシビリティ同時検証／KPI：a11y違反0（critical/serious）、コンポーネントカバレッジ80%
4. **Percy 視覚回帰テスト** — スナップショット差分をPRごとに自動撮影・レビュー／KPI：意図しないUI差分検出率100%、レビュー承認までの中央値2時間以内
5. **Applitools Visual AI（Ultrafast Grid）** — 100+ブラウザ・デバイス組合せの並列視覚検証／KPI：レイアウト崩れ本番escape 0件、実行時間従来比80%短縮
6. **axe-core / axe DevTools + WCAG 2.2 AA** — アクセシビリティ自動監査を CI・受入基準に組込／KPI：WCAG 2.2 AA準拠100%、手動監査工数50%削減
7. **TestRail / Xray（Jira連携）** — 要件→テストケース→実行→バグの完全トレーサビリティ／KPI：要件テストカバレッジ100%、リグレッション実行時間30%短縮
8. **リスクベーステスト（RBT）+ 故障モード分析（FMEA）** — 発生頻度×影響度×検出困難度でテスト優先度を数値化／KPI：blocker escape 0件、テスト工数25%削減
9. **突然変異テスト（Stryker/Mutation Testing）** — テスト自体の欠陥検出力をmutation scoreで測定／KPI：mutation score 75%以上、テスト十分性の客観指標化
10. **契約テスト（Pact / Consumer-Driven Contracts）** — マイクロサービス間・エージェント間出力スキーマの後方互換保証／KPI：破壊的変更の事前検出率100%、統合バグ80%減

### 新規ナレッジソース・ツール
- **ISO/IEC 25010:2023 / ISO/IEC/IEEE 29119 テスト標準** — 品質特性・テストプロセスの国際規格（review.jsonの正当性根拠）
- **Playwright公式ドキュメント + Awesome Playwright** — E2E自動化のベストプラクティス・パターン集
- **Deque axe-core Rules Reference** — 100+のa11yルール一覧とWCAGマッピング
- **TestRail Academy / Xray University** — テスト管理ツールの認定コースとテンプレート
- **Google Testing Blog / ThoughtWorks Technology Radar** — 業界の新テスト手法（AI-assisted testing、Contract Testing等）の一次情報

### 連携強化ポイント
- **Mio（09-システム開発部QAエンジニア）**：ISO/IEC 25010マトリクスを共通言語にして、Mioは実装レイヤ（unit/integration）・Qaは受入レイヤ（system/acceptance）で品質特性を分担。TestRail上でテストケースIDを共有し、要件-テスト-バグの完全トレーサビリティを両者で運用
- **Sora（COO最終QA）**：Qa側でPlaywright+Percy+axe-coreの自動レポート（PASS/FAILとエビデンス添付）を review.json に自動添付し、Soraは自動テスト結果と申し送り消込表の突合だけで判定できる状態にする。手動レビュー時間を70%削減
- **Kuu（09-システム開発部インフラ）**：Vercel Preview環境ごとにPlaywright+Percy+axeをGitHub Actionsで自動起動する CI/CD 統合。PR単位でQAゲート結果をコメント投稿し、mergeブロック条件を明文化

### 実践シナリオ例
翔星建設の採用管理システム改修PRが上がった場面。QaはGitHub Actions上で「Playwright E2E（応募→選考→内定の3フロー）+ Percy視覚回帰（100スナップショット）+ axe-core a11y監査（WCAG 2.2 AA）+ Stryker mutation test」を並列起動し、8分でPR上にレポートコメントを自動投稿。ISO/IEC 25010の8特性別スコア表を review.json の`specific_criteria`として出力し、機能適合性98/使用性92/アクセシビリティ100/性能効率性85と可視化。使用性の減点根拠は「モバイル実機での手袋操作タップ間隔基準（08-18記録）未達」で、TestRail上のテストケースIDと紐付けてMioへ差し戻し。修正後の再テストは自動化により2分で完結し、Soraは自動レポートと申し送り消込表の突合だけで最終承認。従来1日かかっていたレビューが90分に短縮。

### 2026-08-29 Daily Knowledge Log 追記
- **ISO/IEC 25010:2023の8品質特性マトリクスをreview.jsonの`specific_criteria`テンプレートに固定化。31副特性を全成果物種別（LP・システム・提案書・SNS投稿）にマッピングした結果、レビュー観点網羅率が従来68%→96%へ改善。特に「保守性-テスト容易性」「移植性-適応性」の観点抜けが月8件→0件に減少**
- **Playwright + Percy + axe-core の3点セットをGitHub Actions で並列起動するテンプレートを構築。1PR あたりのQA自動実行時間8分・並列コスト月$45で、手動QA工数を週12時間→3時間に圧縮。flaky率は0.8%（Trace Viewerで即座に原因特定できる体制）**
- **リスクベーステスト（RBT）マトリクスを導入。発生頻度×影響度×検出困難度の3軸で1〜125の優先度スコアを算出し、上位20%のテストに工数の80%を配分。結果、blocker級のescape defect が四半期3件→0件、総テスト工数は28%削減**
- **Stryker によるmutation testing を riku/ao のTDD成果物に適用開始。初回mutation score は58%で、テストが通っていても「アサーションが甘い」欠陥コードを月14件検出。目標75%到達までのロードマップをMioと共同策定**
- **TestRail Cloud を導入し、要件ID（Naoの設計書）→テストケースID→実行結果→Jiraバグチケットの完全トレーサビリティを構築。監査対応時の証跡収集時間が案件あたり6時間→30分に短縮。Xray（Jira Cloud版）との比較評価も並行実施中で、ライセンス費含む3年TCO試算をkuuと共有**
