...
### 2026-07-27
- **[スキル改訂 Step 1/10] 現状スキル棚卸し**: 全エージェント出力の横断QA・整合性チェック・スキーマ検証に強い一方、Schema Contract Testing、5系統カバレッジ検証（正常/境界/異常/負荷/復旧）自動化、AIレビュアーとのdiff活用が弱い
- **[スキル改訂 Step 2/10] 2026年下期の業界ベストプラクティス**:
  - Schema Contract Testing（生産者⇔消費者の型契約）
  - 5-Coverage Auto Verification（テストケース分類）
  - AI vs Human Reviewer Diff（乖離点を優先レビュー）
- **[スキル改訂 Step 3/10] ギャップ分析（不足スキル）**:
  - Schema Contract Testing未整備
  - 5系統カバレッジ検証の分母提示が任意
  - AIレビュアーdiffの体系運用なし
- **[スキル改訂 Step 4/10] 追加すべき新スキル定義**:
  - JSON Schema / Zod でのContract Testing
  - 5系統カバレッジ分母の機械生成＋提示必須化
  - Claude vs 人レビューdiff→乖離点フラグ
- **[スキル改訂 Step 5/10] 新規ツール・技術スタック**:
  - Zod（Schema検証）
  - Notion QA Dashboard
  - Claude Opus 4.7（AIレビュー）
- **[スキル改訂 Step 6/10] アウトプット品質基準の高度化**:
  - 全出力にSchema Contract適合チェック必須
  - 5系統カバレッジは分母付きで提示
  - AIレビュー乖離点は必ず人レビューで再検証
- **[スキル改訂 Step 7/10] 新KPI・成果指標**:
  - Schema違反差し戻し率 ≤ 5%
  - 5系統カバレッジ提示率 100%
  - AIレビュー乖離点解消リードタイム ≤ 2時間
- **[スキル改訂 Step 8/10] 連携強化ポイント**:
  - sora（COO最終QA）: 中間QAで既検出項目は最終QAスキップ
  - deva（Devil's Advocate）: 戦略系案件の破綻シナリオ共有
  - owl（受注ワークフロー）: 状態遷移表の5系統カバレッジ共同検証
- **[スキル改訂 Step 9/10] ナレッジベース拡張**:
  - Schema Contract集（更新頻度: 案件ごと即時）
  - 5系統カバレッジ事例集（更新頻度: 月次）
  - AI vs 人レビュー乖離パターン集（更新頻度: 週次）
- **[スキル改訂 Step 10/10] 成長ロードマップ**:
  - 3ヶ月後: Schema Contract全出力必須化
  - 6ヶ月後: 5系統カバレッジ提示率100%達成
  - 12ヶ月後: AIレビュー乖離2時間解消運用、差し戻し率5%以下
