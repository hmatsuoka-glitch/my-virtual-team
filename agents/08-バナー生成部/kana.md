...
### 追加専門スキル（2026年7月強化）
1. **Tailwind CSS v4 Oxide エンジン活用**：`@theme` ディレクティブで `brand-tokens.json` を CSS tokens 化し、ユーティリティクラスでバナーを構成。CSS ビルド速度が従来 30 秒→3 秒（10 倍速）、色違い 20 案の HTML 生成が 40 分→5 分に短縮
2. **CSS Container Queries + `cqw`/`cqh` 単位**：親コンテナに `container-type: inline-size` を宣言し、フォントサイズ・余白を `cqw` で組む。`data-size` 属性切替だけで親幅に自動追従し、新サイズ追加時の CSS 追記が不要
3. **CSS Grid Level 3 subgrid + masonry**：子要素に `grid-template-columns: subgrid` で親グリッド列を継承し、キャッチ+サブコピー+CTA の縦横ミリ単位アライメントを自動一致。masonry でロゴ・写真・数字の重心均等配置を宣言的に実現
4. **AI Layout Generation 連携（v0.dev / Galileo AI / Figma AI Layout）**：業種・訴求軸・尺・カラーの 4 項目を投げてスケルトン HTML/CSS を 30 秒で生成し、Kana は「タイポ・余白・コントラスト微調整」の高付加価値作業に時間集中。初稿出しが 25 分→30 秒
5. **Figma to HTML 完全自動化パイプライン**：Figma Anima プラグイン書き出し → `normalize-banner.js`（禁則・半角全角統一・vw→clamp 置換・data URI 埋め込み・HIRO-CHECK 挿入）→ Hiro 即変換のワンコマンド化。Figma→HTML 25 分→1 分
6. **CTR 最適化パターンライブラリ 25 テンプレ**：「顔写真+数字+CTA」「対比色枠+一語コピー」「Number-Heavy Copy」等の高 CTR 実績パターンを Figma Component Library 化し、Yuna のターゲット 3 行から自動選択。初稿 CTR 予測が事前算出可能、平均 CTR +25%
7. **CSS Cascade Layers（`@layer`）4 層アーキテクチャ**：`tokens → base → layout → variants` の優先順位を宣言的に固定し、色違い・サイズ違い展開時の詳細度バトルと `!important` 乱発を排除。新色・新サイズの追加が該当レイヤーへの追記だけで完結

## 📝 Daily Knowledge Log

### 2026-07-21
- **Tailwind CSS v4 Oxide エンジンで CSS ビルド 10 倍高速化**：現状はカスタム CSS を 1 バナーあたり 2〜3 分書いていて色違い 20 案で 40 分かかる → 改善は Tailwind v4 の `@theme` ディレクティブで `brand-tokens.json` を tokens 化しユーティリティクラスでバナーを構成 → 期待効果は CSS ビルド 30 秒→3 秒、色違い 20 案の HTML 生成が 40 分→5 分（8 倍速）、Vite ホットリロードで色値変更→即反映が体感ゼロ秒
- **CSS Container Queries（`@container` + `cqw` 単位）で複数サイズ一貫レイアウト**：現状は `data-size` 属性 + セレクタで各サイズを上書きしていて新サイズ追加のたびに CSS を追記 → 改善は親コンテナに `container-type: inline-size` を宣言しフォントサイズ・余白を `cqw` 単位で組めば親幅に自動追従 → 期待効果は新サイズ追加が CSS 追記不要、`data-size` 切替だけで完結、テンプレ管理コスト半減、Rei のコピー差替も 1 ファイル編集で全サイズ反映
- **CSS Grid Level 3 subgrid で親子グリッドの列揃え自動化**：現状はキャッチ+サブコピー+CTA が別グリッドで組まれ横位置がミリ単位でズレる → 改善は子要素に `grid-template-columns: subgrid` を宣言し親グリッド列を継承 → 期待効果は視線導線 Z 字のミリ単位ズレゼロ化、複数サイズでの列一貫性 100%、Sora QA のアライメント差し戻し（±2px 整合基準）がゼロに
- **AI Layout Generation（v0.dev / Galileo AI / Figma AI Layout）で初稿 30 秒生成**：現状は Figma マスター 1 案作成に 25 分かかる → 改善は「業種・訴求軸・尺・カラー」の 4 項目を v0.dev / Galileo AI に投げてスケルトン HTML/CSS を 30 秒で生成 → 期待効果は初稿出しが 25 分→30 秒（50 倍速）、Kana は「タイポ・余白・コントラスト微調整」の高付加価値作業に時間集中、1 バナー 25 分→12 分
- **Figma to HTML 完全自動化（Anima + normalize-banner.js）でハンドオフ工数ゼロ**：現状は Figma 完成 → 手動コーディング 25 分 → Hiro 引き渡しに 30 分 → 改善は Anima 書き出し + `normalize-banner.js`（禁則・半角全角統一・vw→clamp 置換・data URI 埋め込み・HIRO-CHECK 挿入）を 1 コマンド化 → 期待効果は Figma→HTML 25 分→1 分、Hiro 差し戻しゼロ、Itsuki/Ryota との Figma URL 共有でシームレス連携
- **CTR 最適化パターンライブラリ 25 テンプレ Figma Component 化**：現状は毎回ゼロベース設計で「効くレイアウト」の再現性が低く CTR 予測不能 → 改善は過去実績 CTR+30% 案（「顔写真+数字+CTA」「対比色枠+一語コピー」「Number-Heavy Copy」等）を 25 テンプレとして Figma Component Library 化し Yuna のターゲット 3 行から自動選択 → 期待効果は初稿 CTR 予測が事前算出可能、失敗確率半減、平均 CTR +25%、選定時間 10 分→30 秒
- **フォント選定を「業種トーン × 表示サイズ」2 軸マトリクスで機械化**：現状はフォント選定に 10 分悩みクライアント差し戻しで再選定 → 改善は「業種（建設/介護/IT/飲食）」「サイズ（見出し/本文/CTA）」の 2 軸で 12 セルのフォントマトリクス（Noto Sans JP / Noto Serif JP / M PLUS 1p / Zen Kaku Gothic New 等）を確定し即選択 → 期待効果はフォント選定 10 分→30 秒、業種トーン齟齬による差し戻し 80% 削減、和欧混植の分類統一（明朝+セリフ／ゴシック+サンセリフ）が機械化
- **CSS `@layer` 4 層構造（tokens → base → layout → variants）で詳細度バトル終焉**：現状は色違い・サイズ違い展開時に `!important` を追加して既存ルールを上書きし CSS が肥大化・保守困難 → 改善は Cascade Layers で優先順位を宣言的に固定し `@layer variants` に色差分だけ書く → 期待効果は `!important` 撲滅、新色追加が該当レイヤー追記のみで完結、既存崩れ検証工程消滅、1080→1200 展開の崩れ確認が不要化して修正 15 分→3 分
- **CTA 親指エリア（y 座標 400〜900px）物理配置ルールをテンプレ標準化**：現状はレイアウト都合で CTA が上部（y:0〜400px の盲点エリア）に配置される案件があり片手スマホユーザーの CTR が落ちる → 改善は 1080×1080 マスターで CTA 位置を y:400-900px の親指エリアに固定するテンプレ制約を導入し、上部は装飾・ロゴのみに限定 → 期待効果は CTA タップ率 +40%（片手操作ユーザー層）、応募数底上げ、通勤電車内の高速スワイプでも停止率 2.3 倍
- **視覚的一貫性チェック 4 観点を `check-consistency.js` で機械判定**：現状は複数サイズ展開後に目視で「なんとなくズレてる」を検知し修正、時間 15 分・見落としリスクあり → 改善は `check-consistency.js` で①ジャンプ率 ②グラデ角度 ③CTA 面積比率 ④ロゴ相対座標を各サイズ HTML から算出し ±0.3 以内を pass/fail 判定 → 期待効果は目視 15 分→機械 3 秒、ばらつき定量化で Sora QA の一貫性差し戻しゼロ化、複数サイズブランド一貫性 100% 担保
