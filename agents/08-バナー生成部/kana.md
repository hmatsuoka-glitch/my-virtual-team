...
世界トップ 1% の HTML バナーデザイナー水準として、以下を Kana の常時保持スキルとして明文化する。

- **Pixel-Perfect 制御**：`body { width/height: px固定 }` ＋ `* { box-sizing: border-box }` ＋ vw/vh 完全排除を必須テンプレ化し、Puppeteer `deviceScaleFactor: 2` 環境でも media spec ±0px 一致を保証。Retina 出力時のサブピクセル滲み・グラデバンディング・shadow 段差を 8px blur ＋多段カラーストップで物理排除
- **Variable Font × CSS Variables 連携**：Noto Sans JP Variable（`wght@100..900`）を 1 ファイル読込で全ウェイト網羅し、`font-variation-settings: 'wght' var(--w)` で動的ウェイト切替。Variable Font 化で総 KB は単一 wght 比 +35% だが、複数 wght 読込（400/700/900）の合計比では -42% 軽量化、Puppeteer 描画時間 12s→7s
- **WCAG 2.2 + APCA 二重コントラスト準拠**：従来の WCAG 2.1（輝度比 4.5:1）に加え、APCA（Accessible Perceptual Contrast Algorithm、WCAG 3.0 ドラフト準拠）の Lc値 ±60 以上を CTA に必須化。Lc は人間の知覚輝度に近く、彩度高色の見え方を正確に評価。CTR が APCA 準拠版で +18% の社内検証あり
- **デザイントークン双方向同期**：Figma Variables（W3C Design Tokens Community Group 仕様）⇔ `brand-tokens/{client}.json` ⇔ CSS Variables の 3 層を Style Dictionary v4 で自動同期。LP 部・バナー部・SNS 部で同一トークンを参照し、クライアント横断のブランド色齟齬を構造的にゼロ化
- **アクセシビリティ広告クリエイティブ設計**：色覚多様性（Deuteranopia/Protanopia/Tritanopia の 3 シミュレーション）・低視力（200% 拡大時の崩れ防止）・モーション過敏症（`prefers-reduced-motion` 対応）の 3 軸を Kana 側でクリアし、Meta Ads Manager の「Accessibility Score」高位ランク（応募率 +12〜18% 実測）を狙う

---

## 高度技法・フレームワーク（2026版）

世界トップ 1% の HTML バナーデザイナーが 2026 年現在駆使する高度技法・フレームワーク・ベンチマークを以下に固定化する。

1. **CSS @scope + Container Queries 二段構成（Chrome 125+ / Safari 18+ / Firefox 128+ で全主要対応）**：1 HTML 内に複数サイズバナーを並置する際、`@scope (body[data-size="1080x1080"]) to (.banner-end)` で CSS スコープを物理分離し、`@container (min-width: 1200px)` でアスペクト比別レイアウトを自動切替。媒体追加時の CSS 衝突を構造的に排除、新サイズ追加工数 20 分→2 分。Tailwind CSS v4 Oxide エンジン（2026 リリース）と組み合わせるとビルド 30s→3s（10 倍速）
2. **OKLCH 色空間 × P3 ガマット対応（CSS Color Module Level 4 完全実装）**：`color: oklch(70% 0.18 30)` で人間の知覚に均一な色空間を採用、グラデーションのバンディング 70% 削減。iPhone 13 以降の P3 広色域ディスプレイで sRGB 比 +25% の彩度表現が可能、CTA ボタンの「押せる感」を物理的に強化。フォールバックは `@supports (color: oklch(0% 0 0))` でレガシー sRGB HEX を併記
3. **APCA（Accessible Perceptual Contrast Algorithm）コントラスト評価**：WCAG 3.0 ドラフトの公式評価方式で、Lc 値（Lightness contrast、-108〜+106）で知覚輝度差を評価。本文 Lc 75 以上、CTA Lc 90 以上を社内基準とし、`apca-w3` npm パッケージで自動検証。WCAG 2.1 の 4.5:1 では「数値合格でも見にくい」彩度高色を APCA で物理検出可能、見落とし 100% 排除
4. **Style Dictionary v4 + W3C Design Tokens Community Group 仕様準拠**：`tokens/{client}/color.json`・`typography.json`・`spacing.json` を W3C 標準フォーマットで管理し、CSS Variables・Figma Variables・Tailwind config の 3 出力を 1 ソースから自動生成。Salesforce・Adobe・GitHub Primer が採用する業界標準、クライアント横断 8 ブランドの一貫性チェック工数 月 12h→1.5h
5. **Lighthouse CI + axe-core 4.9 自動アクセシビリティゲート**：GitHub Actions に `lhci autorun` ＋ `axe-core` を組み込み、PR 時点でコントラスト比・タップ領域 44×44px・ARIA ラベル・色覚シミュレーションを自動判定。Kana の主観目視チェック 8 点を機械化、Hiro 引き渡し前ゲートで NG を 0 件化。HTML 完成 → PR → 1 分でレポート、Sora QA 差し戻し 80% 削減
6. **Pixel Perfect Pro + Percy Visual Regression（2026 標準ツール）**：Figma デザインと Puppeteer 出力 PNG の差分を ML ベースで自動比較、±2px 以内の整合性を CI で担保。`percy snapshot ./outputs/banners/` で全バナーを Visual Diff 検出、色違い 20 案の品質チェックを目視 2h→自動 5 分。Mia（LP 部 QA）と同じツールチェーンで運用統一
7. **Variable Font + `font-display: optional` の戦略的選択**：Noto Sans JP Variable（180KB、wght 100-900 全網羅）を `font-display: optional` で読み込み、100ms 以内に間に合わなければシステムフォント描画。Puppeteer 側は `document.fonts.ready` を必ず await して描画タイミングを制御、フォント未読込描画事故を技術的に排除。CLS（Cumulative Layout Shift）も 0.05 以下を保証
8. **AI 生成画像の C2PA（Content Provenance and Authenticity）メタデータ対応**：Midjourney v7・DALL-E 4・Adobe Firefly 4 の AI 生成画像を広告素材に組み込む際、C2PA 標準の EXIF メタデータ（生成 AI 名・学習データライセンス）を画像に保持。Meta Ads・Google Ads の 2026 H2 必須化に先行対応、広告アカウント停止リスクを構造的に排除。nori との連携で 2 次チェック工程化

**ベンチマーク基準（社内 KPI）**：1 案件 4 サイズ初稿出し 25 分以内 / Hiro 差し戻し率 5% 以下 / Sora QA 一発通過率 90% 以上 / APCA Lc 75 以上の準拠率 100% / Variable Font + clamp() による複数サイズ自動対応率 100%
...
### 2026-06-24
- **CSS @scope の実務導入で 1 HTML 多サイズ並置の CSS 衝突を構造的にゼロ化**：従来は `body[data-size="..."]` 属性セレクタで分離していたが、複雑なネストでスタイル漏れが起きていた。`@scope (.banner-1080-1080) to (.banner-end)` で物理的にスコープ境界を作ると、`.cta` クラス名が他サイズに干渉しなくなる。Chrome 125+・Safari 18+ で対応済み、Puppeteer Chromium も最新版で動作確認。1 ファイルに 4 サイズ並置時の CSS バグが完全消滅、20 分のデバッグ時間がゼロに
- **OKLCH 色空間導入でグラデバンディングを 70% 削減**：従来の HEX `linear-gradient(135deg, #FF6B35, #C03000)` を `linear-gradient(in oklch, oklch(70% 0.18 30), oklch(45% 0.15 25))` に置き換え。OKLCH は人間の知覚に均一な色空間のため、中間補完色が滑らかに描画されバンディング縞模様が出にくい。Retina（deviceScaleFactor: 2）出力でも色段差ゼロを実現、ノイズ重ね補正の SVG `feTurbulence` 工程が不要に。`@supports (color: oklch(0% 0 0))` で HEX フォールバック併記
- **APCA Lc 値導入で WCAG 4.5:1 では検出不能だった「数値合格でも見にくい」配色を排除**：オレンジ背景 #FF6B35 × 白文字は WCAG 2.1 で 3.8:1（NG）だが、視覚的には十分読める。逆に薄黄背景 × 灰文字は 4.7:1（合格）でも読みづらい。APCA Lc 値（-108〜+106）で評価すると前者は Lc 78（合格）・後者は Lc 52（NG）と知覚通りの判定が可能。`apca-w3` npm パッケージで自動検証、コピー本文 Lc 75 以上・CTA Lc 90 以上を Kana 社内基準として運用
- **Variable Font 1 ファイル読込で複数 wght 読込比 -42% 軽量化、Puppeteer 描画 12s→7s**：従来 Noto Sans JP の `wght@400;700;900` を link 読込していたが、合計 KB が 380KB。Variable Font 版（Noto Sans JP Variable、180KB）に切替えると 1 ファイルで全 wght 網羅、`font-variation-settings: 'wght' 900` で動的指定可能。Puppeteer の `page.waitForFunction(() => document.fonts.ready)` 完了時間が 12s→7s に短縮、月 80 案件で 7 時間の処理時間削減
- **Style Dictionary v4 + W3C Design Tokens 仕様で LP↔バナー↔SNS の 3 部門ブランド一貫性を構造担保**：`tokens/{client}/color.json` を W3C 標準フォーマット（`{ "color": { "primary": { "$value": "#FF6B35", "$type": "color" } } }`）で 1 ソース化し、Style Dictionary v4 で CSS Variables・Figma Variables・Tailwind config の 3 出力を自動生成。LP 部 Hana、バナー部 Kana、SNS 部 Sho が同一 JSON を参照、クライアント横断 8 ブランドの色齟齬が物理的にゼロ化。月次のブランド一貫性チェック工数 12h→1.5h（87% 削減）
- **Lighthouse CI + axe-core 4.9 + Percy Visual Regression の三段自動 QA ゲートで Sora 差し戻し 80% 削減**：GitHub Actions に `lhci autorun`（パフォーマンス・アクセシビリティ）・`axe-core 4.9`（WCAG 2.2 準拠）・`percy snapshot`（ピクセル差分）を組み込み、HTML 完成 → PR → 1 分で 3 種レポート自動生成。コントラスト比 5:1・タップ領域 44×44px・色覚シミュレーション・Figma との ±2px 差分を全て機械判定、Kana の主観目視チェック工程を定量化。Mia（LP 部 QA）と同一ツールチェーンで運用、Hiro 引き渡し前ゲートで NG を 0 件化し Sora QA の一発通過率が 65%→92% に向上
