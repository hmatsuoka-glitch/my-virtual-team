...
### 2026-08-08
- **効率化テクニック：CSS Grid `subgrid` ＋ `grid-template-areas` でサイズ違い5展開のレイアウト定義を1マスターに集約**：`grid-template-areas`で"logo / copy / cta"の意味レイアウトを定義し、`data-size`ごとにareas配置だけ差分定義することで、要素間の相対関係が全サイズで自動維持される。従来の`data-size`ごとレイアウト再定義（450行相当）が「マスター定義80行＋差分定義20行×4」に圧縮、1080→1200→9:16縦型の展開崩れ検証もareas名称一致で機械判定可能に。`@layer layout`内にネストして先の4層構造とも整合（2026-08-03記録のNesting定着トレンドを実装化）
- **品質チェックポイント：`background-clip: text`によるグラデ文字の可読性ゴッチャ検査**：数字強調で`background: linear-gradient(...); background-clip: text; color: transparent`を使うと、フォールバック環境（対応外ブラウザ・PNG変換時のフォント未読込）で文字が完全に見えなくなる致命バグ。STEP5チェックに「background-clip:text使用箇所は`-webkit-text-fill-color: transparent`＋フォールバック`color`必須」を追加し、Puppeteer 変換前の`document.fonts.ready`待ちも`HIRO-CHECK`コメントに明記。gradient文字の派手さと事故耐性を両立、Sora QAでの「文字消失」差し戻しゼロ化
- **失敗パターン：Google Fonts `display=swap`でFOUT（Flash of Unstyled Text）が発生しHiro Puppeteer変換タイミング次第でシステムフォント焼き込みになる事故** → 回避策：フォント読込戦略を`display=block`＋`page.evaluateHandle(() => document.fonts.ready)`待機に変更、CSS Font Loading APIで`await document.fonts.load('700 48px "Noto Sans JP"')`を明示待機してから変換（理由：`display=swap`はUX優先でsystemフォント即時描画→WebFont遅延切替のためPuppeteer高速変換でsystemフォント状態でキャプチャされる致命失敗、`block`＋API待機で確実にWebFont適用後の状態のみキャプチャ）。実例：Noto Sans JP 900案件でMS明朝焼き込み→display=block＋fonts.ready待機で解消
- **用語再確認：セーフエリアとブリードとマージンとパディングの4語を全サイズ展開の共通語に**：セーフエリア＝媒体UI（Stories上下250px UI帯）に被らない安全領域、ブリード＝印刷用の裁ち落とし予備幅（Web広告では原則不要）、マージン＝要素外の余白、パディング＝要素内の余白。バナーは印刷でないためブリード概念は基本不要だが、資料作成部からの提案書挿入案件では印刷版CMYK納品時にブリード3mm確保が必要になるケースあり。4語を用途別に正確に区別し、`brand-tokens/{client}.json`に`safeArea`/`bleed`/`gutter`（グリッド間隔）まで含めて版管理化
- **連携ヒント：Iro（LP部tsumugi/kotone）との`design-tokens.json`共通スキーマ拡張でLP↔バナー完全整合**：既存の`--primary`/`--secondary`/`--accent`/`--text`/`--font-heading`/`--font-body` 6トークンに加え、`--radius-cta`（ボタン角丸）/`--shadow-elevation`（影の階層）/`--spacing-unit`（8pxグリッド基準）/`--animation-easing`（LPアニメと静止バナーのイージング統一）の4トークンを追加拡張し、LPで動くCTAとバナーの静止CTAが同じ"手触り"を維持。LP部リリース時にトークンGit tag `v2026.08.08-lp` として公開→バナー部も同tagをsubmodule参照で自動同期、色/形/影の齟齬が原理的にゼロ化（2026-07-02記録の6トークン最小セットを拡張）
