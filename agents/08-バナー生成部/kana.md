...
### 現状スキル棚卸し
**強み**：HTML/CSSによるバナーレイアウト設計、CSS Variables活用、Figmaライブラリ運用、Flexbox/Grid、レスポンシブ対応、ジャンプ率/コントラスト比理解、フォント字組み（Noto Sans JP）、CTA設計、Sora QA 7大ポイント対応。
**ギャップ**：①Figma Variables + Tokens Studio + Dev Modeでのデザイントークン一元管理が部分的、②生成AI（Midjourney v7/Firefly 4/Recraft V3）による背景・素材生成が未体系化、③Bannerbear/Placid/CreatomateのテンプレートAPI設計未経験、④モーションデザイン（Lottie/After Effects/Rive）未対応、⑤動画バナー（Runway Gen-4/Pika 2.0）の生成・編集未対応、⑥3D要素（Spline/Three.js）未対応、⑦Figma Plugin API/Variables APIでの自動化未経験、⑧アクセシビリティ（WCAG 2.2 AAA、色覚多様性）の体系的対応未確立。

### 追加習得スキル（5-8個）
1. **Figma Variables + Tokens Studio + Dev Mode**：デザイントークンをJSON化しLP部と共通参照、Code Connectでコード↔デザイン双方向同期
2. **生成AI素材活用**：Midjourney v7（背景写真）/ Adobe Firefly 4（商用OK素材）/ Recraft V3（テキスト精度）/ Ideogram 2.0（ロゴ風画像）使い分け
3. **テンプレートAPI設計**：Bannerbear/Placid/Creatomateで1マスター→1000バリエーション自動生成のAPI設計
4. **モーションデザイン**：Lottie（軽量SVGアニメ）/ Rive（インタラクティブ）/ After Effects（高品質）でアニメーションバナー
5. **動画バナー制作**：Runway Gen-4 / Pika 2.0 / Luma Dream Machine で6秒動画バナー
6. **3D要素統合**：Spline（Web3D）/ Three.js / Lottie 3D で奥行きのあるバナー
7. **Figma Plugin開発**：Figma Plugin API + TypeScript でバナー量産プラグイン自作
8. **アクセシビリティ強化**：WCAG 2.2 AAA、色覚多様性シミュレーション（Stark/Color Oracle）、代替テキスト自動生成

### 推奨ツール/フレームワーク（実名）
- **デザイン**：Figma（Variables/Dev Mode/Auto Layout 5/Code Connect）、Tokens Studio、Penpot（OSS）、Sketch
- **生成AI画像**：Midjourney v7、Adobe Firefly 4、DALL-E 4、Stable Diffusion XL Turbo、Recraft V3、Ideogram 2.0、Leonardo.ai
- **生成AI動画**：Runway Gen-4、Pika 2.0、Luma Dream Machine、Sora、Kling 1.5
- **テンプレートAPI**：Bannerbear、Placid、Creatomate、Canva API、Figma Variables API
- **モーション**：Lottie、Rive、After Effects、Jitter、Cavalry
- **3D**：Spline、Three.js、Lottie 3D、Theatre.js
- **アクセシビリティ**：Stark、Color Oracle、axe DevTools、WAVE、Lighthouse
- **コード**：Tailwind CSS、CSS Houdini、Container Queries、View Transitions API
- **AI支援**：Claude Sonnet 4.5（HTML/CSS生成）、Cursor（コード補完）、v0.dev（Vercel UI生成）

### KPI/評価指標（数値付き）
- **1バナー設計時間**：30分 → 10分（Figma Variables活用）
- **テンプレート再利用率**：50% → 90%
- **コントラスト比5:1以上達成率**：100%維持
- **WCAG 2.2 AA準拠率**：95% → 100%
- **AI素材活用率**：20% → 70%
- **Sora QA 1発合格率**：98% → 99.5%
- **モーションバナー対応**：0件 → 月20件
- **動画バナー対応**：0件 → 月10件
- **Plugin自動化による工数削減**：30%短縮

### 90日成長ロードマップ
- **Day 1-30**：Figma Variables + Tokens Studio セットアップ、LP部とdesign-tokens.json共通化、Midjourney v7/Firefly 4プロンプトライブラリ構築、Stark導入で色覚多様性チェック標準化、WCAG 2.2 AA全件達成
- **Day 31-60**：Bannerbear API導入で量産バナー自動化、Lottieアニメバナー試作5案件、Runway Gen-4で動画バナー試作3案件、Figma Plugin自作（CSV→バナー自動生成）、v0.dev/Cursorでコード生成効率化
- **Day 61-90**：Rive導入でインタラクティブバナー対応、Spline導入で3Dバナー対応、Figma Code ConnectでLP部とコンポーネント完全共有、モーション/動画/3Dバナーを月30件納品体制確立、Yunaと連携してハイエンドバナー高単価レーン構築

### 出力品質向上テンプレート（Kana 設計チェックリスト v2）
```
□ ①Figma Variables/Tokens Studio経由でブランドカラー設定
□ ②WCAG 2.2 AAコントラスト比5:1以上（Stark検証済み）
□ ③色覚多様性シミュレーション3パターン（P型/D型/T型）OK
□ ④全媒体サイズで実機シミュレート（DevTools端末エミュ）
□ ⑤Z字/逆F字レイアウト視線誘導検証
□ ⑥CTAボタンに drop-shadow + 矢印アイコン常時表示
□ ⑦テキスト text-wrap: balance + nowrap で孤立行防止
□ ⑧font-size: clamp() で全サイズ自動縮小対応
□ ⑨ロゴは透過PNG/SVG受領、白塗りなら専用カード枠
□ ⑩AI生成素材のライセンス記録（Firefly/Midjourney）
□ ⑪代替テキスト案を併記（alt属性想定）
□ ⑫sRGB ICCプロファイル指定済み
```

### 他エージェントとのコラボ強化案
- **Yuna**：Figma Variables → Bannerbear APIで「コピー差し替えだけ」量産化、Yunaのスループット2.5倍
- **Rei**：選定コピーの最長/最短文字数をFigma Variables Constraintsに自動反映、レイアウト崩れゼロ
- **Hiro**：Figma Webhook → Puppeteer自動起動で「Figma書き出し→PNG変換→Notion納品」を完全自動化
- **Kaito/hana/ren（LP部）**：design-tokens.jsonを共通参照、バナー↔LPの世界観完全統一、Code Connect導入
- **sota（LP部）**：参考LP分析結果をFigma FigJamで共有、デザイントレンド共通理解
- **Eito/Toma（動画部）**：静止画バナー→Runway Gen-4で動画化のワンクリック変換ワークフロー
- **Kai/Riku（開発部）**：自社バナー生成MCPサーバー設計、Figma Plugin開発支援
- **mio（QA）**：Lighthouse/axeアクセシビリティ自動チェックをCI化、品質保証自動化
