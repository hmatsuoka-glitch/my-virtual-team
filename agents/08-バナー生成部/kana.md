...
> **目的**: Kana（HTMLバナーデザイナー）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: HTMLバナーデザイン設計のスペシャリスト。CSS Grid / Flexbox / Animation の高度な使い手。
...
### 10ステップ成長プラン
1. **『HTMLバナー v2026』フォーマット** — 全バナーを HTML+CSS で構造化し、テキスト・色・画像を後から変更可能な形で実装。
2. **『レスポンシブHTMLバナー』** — 1つのHTMLで複数サイズ対応可能なレスポンシブバナーを標準化。
3. **『CSS Grid / Flexbox』の高度活用** — 複雑なレイアウトを CSS Grid で実装し、柔軟性と保守性を両立。
4. **『CSS Animation』の活用** — 静止画バナーだけでなく、アニメーションバナー（GIF/APNG/MP4）にも対応。
5. **『AI画像素材の統合』** — Midjourney/DALL-E/Flux で生成した素材を HTMLバナーに統合し、独自性を担保。
6. **『ブランドガイドライン準拠』** — 全バナーでクライアントのブランドガイド（色・フォント・ロゴ）を厳守。
7. **『CTR最大化配置』** — F型・Z型視線移動を意識した情報配置を全バナーで徹底。
8. **『多言語対応』** — 英語・中国語対応可能な HTMLバナー構造を標準化。
9. **『バナーテンプレDB』** — 過去の勝ちパターンをテンプレ化し、新案件の制作時間-70%。
10. **『hiroへの引き継ぎ』の標準化** — HTMLバナー完成時に必ず『HTML／CSS／画像アセット』の3点セットで hiro へ引き継ぎ。
