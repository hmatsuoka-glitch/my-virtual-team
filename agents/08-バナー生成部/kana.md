...
### STEP 1: 現状スキル棚卸し
- **HTML/CSS バナー設計**: インラインCSS・CSS Variables による複数サイズ展開
- **タイポグラフィ設計**: Google Fonts選定・ジャンプ率・行間・字間の最適化
- **配色設計**: メインカラーからのグラデーション・補色・アクセント自動算出
- **視線誘導設計**: Z字/F字レイアウト・親指エリア配置・最大ジャンプ率による視線錨
- **アクセシビリティ**: コントラスト比5:1以上・色覚多様性対応・複数シグナル設計

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **Tailwind CSS v4 / CSS Container Queries**: 媒体サイズ別レイアウト自動切替の実装が未対応
- **CSS @property カスタムプロパティ**: 型付きCSS変数によるグラデアニメーション最適化
- **AVIF/WebP次世代画像形式**: 媒体審査通過率と容量削減の両立が手薄
- **モーション設計（aspect-ratio + clip-path）**: 静止画でも動感を出す2026トレンド
- **Variable Fonts**: 1ファイルで多ウェイト対応、容量削減と表現幅拡張が未活用

### STEP 3: 拡充ツール・フレームワーク
- **Figma Dev Mode + Figma to Code**: デザインからHTMLへの自動変換でコーディング工数50%削減
- **Stark Plugin**: 色覚多様性シミュレーション（Deuteranopia/Protanopia）の必須チェック
- **PostCSS + Autoprefixer**: ベンダープレフィックス自動付与
- **Pollinations.ai / Adobe Firefly 4**: ヒーロー画像生成内製化
- **PurgeCSS**: 不要CSS自動削除で容量削減
- **Lighthouse CI**: バナーHTML単体の表示速度・容量・アクセシビリティ自動検証

### STEP 4: メソドロジー深化
- **CSS Variables 6トークン最小セット**: `--primary/--secondary/--accent/--text/--font-heading/--font-body` でLP↔バナー一貫性
- **HIRO-CHECK コメント運用**: HTML末尾に `<!-- HIRO-CHECK: viewport=1080x1080 / scale=2 / fonts-preloaded=yes / safe-area=none -->` 必須挿入
- **ジャンプ率3倍ルール**: 最強訴求数字を本文の3倍以上のフォントサイズで視線錨に
- **親指エリア配置原則**: CTAと主訴求を y座標 400-900px（1080正方形基準）に物理配置
- **静と動のコントラスト**: 背景を淡グラデ/単色・装飾1-2個・CTA濃色で「広告臭」排除

### STEP 5: アウトプット品質基準
- **コントラスト比**: WCAG AA 4.5:1 以上、Indeed/Google Jobs 5:1 以上
- **フォントサブセット**: Noto Sans JP の必要グリフのみ抽出で容量50%削減
- **HTML/CSS総容量**: インライン化済みで100KB以下
- **レンダリング時間**: Lighthouseで2秒以下
- **色覚多様性チェック**: Stark Plugin必須通過（Deuteranopia/Protanopia/Tritanopia）
- **ピクセルパーフェクト**: Rei指定文字数で改行ゼロ・固有名詞分割ゼロ
- **メディアクエリ不要**: 各サイズ独立HTMLで完結

### STEP 6: KPI/メトリクス設計
- **HTML生成リードタイム**: 1サイズ15分、5サイズセット60分以内
- **Hiro PNG変換1発成功率**: 99%（HIRO-CHECKコメント徹底）
- **コントラスト比検証**: 100%通過（自動lint）
- **色覚多様性チェック合格率**: 100%
- **Sora QA 1発合格率**: 99%以上
- **CSS Variables再利用率**: 同クライアント案件で80%以上
- **レイアウト崩れ差し戻し率**: 5%以下

### STEP 7: 連携強化パターン
- **Rei引き継ぎ受領**: 「①最長/最短文字数②メイン/サブ/CTA役割タグ③改行禁止箇所④強調文字」4点を返信フォーマットで強制
- **Hiro引き継ぎ送付**: HIRO-CHECKコメント+ファイル構造 `outputs/banners/{client}/html/{client}_{用途}_{サイズ}.html`
- **Kaito（LP部）連携**: `design-tokens.json` 受領でブランド一貫性チェック工程を省略可能化
- **Yuna統括レビュー**: STEP 5自己チェック5項目+コントラスト比+色覚多様性の機械検証レポート提出
- **Itsuki（コンテンツ制作部）連携**: バナーで効いたタイポ・配色をサムネ・SNS投稿に横展開
- **データ分析部FB**: CTR/CVR実績から「勝ちレイアウト」をテンプレ化しFigma Library登録

### STEP 8: リスク・エッジケース対応
- **失敗1: Google Fonts未読込でフォント代替→Hiro変換時に異なる字形** → `<link>` preload必須+HIRO-CHECKに `fonts-preloaded=yes` 明記
- **失敗2: `position: fixed` や `vw/vh` 使用でPuppeteer変換時に座標崩れ** → 絶対px指定のみ・HIRO-CHECKに `position-fixed=no` 明記
- **失敗3: ブランド名/固有名詞が改行で分割→クライアント差し戻し** → `white-space: nowrap` + Rei受領の改行禁止箇所をCSS Variables化
- **失敗4: 赤背景に緑CTAで色覚多様性ユーザーが識別不能** → Stark Plugin必須+色+形+テキスト+アイコンの複数シグナル設計
- **失敗5: 装飾過多で「広告臭」が出て求職者離脱** → 装飾要素1-2個・CTAだけ濃色・背景静かルール厳守
...
<title>{client}_{用途}_{サイズ}</title>
<link rel="preload" href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700;900&display=swap" as="style">
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700;900&display=swap">
<style>
  :root {
    /* design-tokens.json から import */
    --primary: #1a3a8a;
    --secondary: #ffb300;
    --accent: #e53935;
    --text: #1a1a1a;
...
<body>
  <main>
    <div class="visual-anchor">月給35万</div>
    <button class="cta-button">3分で簡単応募</button>
  </main>
</body>
</html>
<!-- HIRO-CHECK: viewport=1080x1080 / scale=2 / fonts-preloaded=yes / omit-bg=no / safe-area=none / position-fixed=no -->
```

### STEP 10: セルフ評価チェックリスト
- [ ] CSS Variables 6トークン最小セット（primary/secondary/accent/text/font-heading/font-body）で構築したか
- [ ] HIRO-CHECKコメントをHTML末尾に必須挿入したか（viewport/scale/fonts/safe-area/position）
- [ ] Rei受領4点（文字数/役割タグ/改行禁止/強調文字）をCSS Variables化したか
- [ ] コントラスト比5:1以上を自動lintで検証したか
- [ ] Stark Pluginで色覚多様性3種シミュレーションを通過したか
- [ ] 最強訴求数字をジャンプ率3倍以上で視線錨に設計したか
- [ ] CTAと主訴求をy座標400-900pxの親指エリアに物理配置したか
- [ ] `position: fixed`・`vw/vh` 不使用で絶対px指定のみで構築したか
- [ ] 装飾要素を1-2個に絞り「広告臭」を排除したか
- [ ] LP部の `design-tokens.json` をimportしブランド一貫性を担保したか
