...
### 2026-09-05
- **`data-media` プリセット×`@layer variants` の運用開始**：Meta 1:1／4:5、Google Demand Gen 1200×628・1200×1500、TikTok 9:16、Indeed、LINE VOOM、Amebaちゃん、Yahoo!ディスプレイの中央60%セーフエリア／CTA 定位置／UI 干渉領域（TikTok 上150px・下350px 等）を `@layer variants` に事前登録し、案件では `data-media="tiktok"` を付けるだけで媒体制約が自動適用。案件ごとに媒体規定を調べ直す工数が消え、Yuna の媒体確認シートとの突合も 1 行の属性値で終わる
- **色空間三系統管理（sRGB／Display P3／OKLCH）で iPhone P3 表示と Meta 圧縮後 sRGB の CTA 沈み事故を予防**：`brand-tokens.json` に `srgb`／`p3`／`oklch` の 3 表現を並置し、`@supports (color: color(display-p3 1 0 0))` で分岐。iPhone Safari P3 表示ではブランド色の彩度がフルに立ち、Meta 圧縮 sRGB フォールバック時は OKLCH 明度計算で CTA コントラストが機械的に維持される。「iPhone で見ると鮮やか、広告配信されると眠い」の色ズレクレームを色空間側で構造的に予防
- **景表法・薬機法禁止語辞書 `nori-check.json` を `normalize-banner.js` に統合し nori 2 次ゲートを機械化**：優良誤認（No.1・業界初・完全・絶対）・有利誤認（今だけ・期間限定の打消し表示比率）・医薬品的効能効果・断定表現を辞書化、HTML 内テキストを走査して該当時 `<!-- nori-check: NG:優良誤認:業界初 -->` を自動挿入。Rei 1 次チェック通過後の文脈変化 NG（レイアウト後に「圧倒的」が主表示化する等）を Kana 段階で機械捕捉、nori の負荷を人力チェックから例外承認へ移行
- **HTML5 IAB 対応の二層納品（静止 PNG + Rive アニメ CTA）で DoubleClick Studio 廃止後の HTML5 動画枠にも 1 マスターから対応**：CTA だけを Rive 20KB 以下のステートマシンで実装し、背景・コピーは静止レイヤーに分離。Hiro が初期フレームを PNG 焼き込み、HTML5 版はそのまま Meta / TikTok 動画広告枠に転用可能。**1 マスターから静止画と HTML5 アニメの 2 形態が同時出力**、DoubleClick Studio 2026 完全終了後の代替出稿経路を確保
- **BIZ UDPGothic / Zen Kaku Gothic New / Klee One の業種別書体選定基準をテンプレ化**：自治体・公共案件は BIZ UDPGothic 必須（UD フォント指定要件）、教育系は Klee One の楷書系親近感、建設業求人は Zen Kaku Gothic New のニュートラル力強さ、と業種×トーンで書体マトリクスを固定。Yuna のクライアント業種通知だけで書体自動選定が可能に、書体選びで案件開始が止まる待ち時間ゼロ化
- **Noto Sans JP Variable の `wght` 軸連続指定＋`font-optical-sizing: auto` で 5〜200px 単一書体完走**：従来の `wght@400;700;900` 列挙式は 600 指定時に黙ってフォールバックする事故があったが、可変フォントで `wght` 連続指定＋`opsz` 自動最適化により、給与数字 180px から注釈 11px まで単一 woff2 で違和感なく描画。link href 列挙漏れ事故が構造的に消え、サブセット化と組み合わせて 1 ファイル 80KB 以下で全ウェイト運用可能
- **Container Query `cqi`／`cqb`＋`subgrid` で親アスペクト比変更に子グリッドが完全追従、Magic Resize 微修正コストを実質ゼロ化**：`aspect-ratio: 1 / 1` を親に、子は `subgrid` で列・行を継承、フォントは `cqi`（インライン軸）でスケール。1:1 マスターから 4:5／9:16／16:9／1200×628 を無設定生成でき、Yuna の Magic Resize 後の微修正 2〜3 箇所が「そもそも発生しない」状態に。1 案件 4 サイズ展開が 25 分→5 分
- **`preview-index.html` の 35% 縮小俯瞰ビューを Yuna 社内提示 URL の標準に格上げ**：全サイズ×全色パターンを `iframe` × `transform: scale(0.35)` で 1 画面グリッド表示し、実配信相当の縮尺で 7 社横断の面積配分・ロゴ位置・色一貫性をブラウザ 1 枚で俯瞰。Yuna へは URL 1 本共有、Hiro の書き出しを待たずラフ段階で構成可否確定。ファイル 1 つずつ開いて確認する工数が 20 分→2 分、横並びで初めて見える崩れも取りこぼしゼロ化
