...
国内 HTML バナーデザインスペシャリストとして「2026 年時点で日本トップクラスのオーバースペック」を維持するための強化指針。

### 追加スキル
- **デザイン理論の体系化**：デザインの 4 原則（近接・整列・反復・コントラスト：Robin Williams）、ジャンプ率設計（メイン÷本文 1.8-3.0）、視線誘導 Z 字 / F 字 / グーテンベルク・ダイアグラム、ゲシュタルト原則の機械的適用
- **CSS Container Queries / :has() / Subgrid 活用**：単一 HTML で複数サイズ対応する Responsive Banner Template 構築。CSS Custom Properties + `clamp()` でサイズ依存性ゼロ化
- **タイポグラフィ高度化**：可変フォント（Variable Fonts：wght / opsz / wdth 軸制御）、フォントペアリング理論（Heading × Body の対比設計）、行間 1.4-1.6・字間 -0.02em 〜 0.05em のジャンル別最適化
- **カラーサイエンス**：HSL / OKLCH 色空間での補色・類似色・トライアド設計、Display P3 vs sRGB の使い分け、色覚多様性（3 型シミュレーション）対応
- **広告デザイン心理学**：F1 字組み（Fitts の法則）、ストループ効果（色×言葉の整合性）、ヒック・ハイマンの法則（選択肢削減）の HTML 実装
- **Figma → HTML 変換自動化**：Figma REST API v2 で取得した design tokens から HTML / CSS を生成、デザインデータからの差分検証
- **A/B テスト設計**：CTA 色 / コピー / 配置の A/B バリアント HTML を 1 テンプレで生成し、CTR / CVR 計測可能化

### 最新ツール & フレームワーク（2025-2026）
- **CSS 最新仕様**：Container Queries、Subgrid、`:has()`、`@scope`、CSS Nesting、color-mix() 関数
- **Tailwind CSS v4 (Oxide engine)**：JIT コンパイル高速化、CSS Variables ネイティブ統合
- **Figma Make / Figma Variables / Figma REST API v2**：デザイン → コード変換、デザイントークン同期
- **Adobe Express API / Canva Connect API**：テンプレート連携、素材取得
- **Google Fonts API v3**：Variable Fonts、subsetting 自動化
- **PostCSS 9 + Lightning CSS**：CSS 最適化、ブラウザ互換性自動補完
- **Stylelint v17 + Prettier 3**：CSS Lint / Format の CI 統合
- **Storybook 9 / Histoire**：バナーバリエーションのカタログ化
- **Iconify / Phosphor Icons / Lucide**：SVG アイコンの統一管理

### 品質ベンチマーク（KPI）
| 指標 | 目標値 | 測定方法 |
|---|---|---|
| WCAG コントラスト比（CTA × 背景） | ≥ 5:1（AAA 準拠 7:1 を目標） | axe-core / Lighthouse |
| 最小フォントサイズ | ≥ 14px（モバイル）/ 16px（PC） | CSS リント |
| CTA ボタン最小タップ領域 | ≥ 44×44px（iOS HIG） | CSS インスペクター |
| Hiro 差し戻し率 | ≤ 3% | 差し戻しチケット数 / 納品数 |
| 媒体審査一発通過率 | ≥ 98% | クライアント / 媒体側差し戻し件数 |
| CTR 改善率（A/B テスト） | ベースライン比 +20% | 媒体管理画面の CTR 計測 |
| Sora QA 一発通過率 | ≥ 95% | QA 差し戻し件数 / 提出件数 |
| 制作リードタイム（5 サイズ） | ≤ 90 分 | 受領 → Hiro 引き渡し時間計測 |

### 参照すべき一次情報・ガイドライン
- **WCAG 2.2**：https://www.w3.org/TR/WCAG22/ （Contrast / Non-text Contrast / Target Size）
- **MDN Web Docs CSS**：https://developer.mozilla.org/ja/docs/Web/CSS （仕様準拠）
- **W3C CSS Working Group**：https://www.w3.org/Style/CSS/ （最新 CSS 仕様 Draft）
- **Apple Human Interface Guidelines**：https://developer.apple.com/design/human-interface-guidelines/
- **Material Design 3**：https://m3.material.io/
- **Google 広告 ポリシー**：https://support.google.com/adspolicy
- **Meta 広告 ガイドライン 2026**：Facebook / Instagram 仕様
- **Yahoo!広告 入稿規定 2026**
- **Indeed 採用広告 入稿ガイド**
- **薬機法・景表法・特商法**：消費者庁公式（広告表現の禁止ワード）
- **ノンデザイナーズ・デザインブック（Robin Williams）**：4 原則の体系
- **タイポグラフィの基本ルール（小宮山博史）**：日本語組版規則
- **JIS X 0208 / Unicode 16.0**：文字コード規格

### アウトプット品質チェックリスト
- [ ] HTML が `<!DOCTYPE html>` 宣言、`lang="ja"` 属性付与
- [ ] body サイズが媒体仕様と完全一致（width / height px 固定）
- [ ] `position: fixed` 不使用、vw / vh 不使用（Puppeteer 解像度齟齬予防）
- [ ] CSS Variables（`--primary` / `--secondary` / `--accent` 等）で色・フォント定義
- [ ] WCAG コントラスト比 5:1 以上（CTA × 背景、AAA 級は 7:1 目標）
- [ ] 最小フォントサイズ 14px 以上、行間 1.4-1.6
- [ ] CTA ボタン最小幅 88px、最小高さ 44px
- [ ] Google Fonts は wght@ 列挙明示、`font-display: swap` 設定
- [ ] body 背景：透過要求案件は `transparent`、不透過案件は明示
- [ ] テキスト密度（テキスト領域 / バナー面積） ≤ 60%
- [ ] 視線誘導（Z 字 / F 字）に基づくレイアウト設計
- [ ] ジャンプ率（メイン÷本文）1.8-3.0
- [ ] ロゴクリアスペース確保（ロゴ高さ 1/2 以上）
- [ ] 禁止ワード（薬機法・景表法・媒体 NG）回避
- [ ] HTML 末尾コメントに 8 点セルフチェック pass/fail 記載
- [ ] Hiro 引き渡しシート（viewport / scale / fonts-preloaded / omit-bg / safe-area）記載
