...
### 1. HTML Banner Templating System
- **導入内容**: バナーHTML/CSSをコンポーネント化し、テキスト/画像/CTAを差し替えるだけで量産可能。テンプレライブラリ100種完備。
- **導入基準**: 全バナー案件で必須採用。
- **期待効果KPI**: 制作時間 2時間→20分／バリエーション生成速度 10倍
- **連携先**: hiro、yuna、rei

### 2. Typography Design System
- **導入内容**: Google Fonts / Adobe Fonts の最新トレンド書体を業種別・ムード別にカタログ化。フォールバック指定必須。
- **導入基準**: 全バナーで必須指定。
- **期待効果KPI**: 読みやすさスコア 40%向上／ブランド一貫性 100%
- **連携先**: iro、itsuki

### 3. Grid System (12/16 column)
- **導入内容**: 12/16カラムグリッドシステムをバナーサイズ別に標準化。要素配置の一貫性を機械的に担保。
- **導入基準**: 全バナー設計で必須採用。
- **期待効果KPI**: レイアウト崩れ 0件／制作時間 30%短縮
- **連携先**: hiro、itsuki

### 4. Color Contrast Verification
- **導入内容**: バナー内の色コントラスト比を自動計算し、WCAG AAA準拠（テキスト 7:1）を保証。低コントラスト自動アラート。
- **導入基準**: 全バナーで必須検証。
- **期待効果KPI**: 視認性クレーム 0件／A11y違反 0件
- **連携先**: iro、mia

### 5. Motion Banner (Lottie / Rive)
- **導入内容**: 静止画バナーだけでなく、Lottie/Rive で軽量アニメーションバナーを標準対応。CTR向上効果高い。
- **導入基準**: 動画配信可能プラットフォームで発動。
- **期待効果KPI**: CTR 60%向上／視認時間 40%増
- **連携先**: hiro、itsuki、eito

### 6. Retina & High-DPI Support
- **導入内容**: 全バナーを Retina 2x/3x 解像度で書き出し。SVGフォーマットも活用。
- **導入基準**: 全バナーで必須。
- **期待効果KPI**: モバイル表示品質 100%／ぼやけクレーム 0件
- **連携先**: hiro

### 7. Emoji & Icon Library
- **導入内容**: Twemoji / Material Icons / Heroicons / Lucide の統一アイコンライブラリを業種別ムード別に整備。
- **導入基準**: 全バナーで必須参照。
- **期待効果KPI**: アイコン選定時間 30分→3分／統一感 100%
- **連携先**: iro、itsuki

### 8. Background Design Patterns
- **導入内容**: グラデーション/幾何学模様/イラスト/ノイズテクスチャ 等の背景パターン100種をカタログ化。TailwindCSS対応。
- **導入基準**: 全バナーで背景設計時に参照。
- **期待効果KPI**: デザイン差別化スコア 40%向上／制作速度 3倍
- **連携先**: itsuki、iro

### 9. Responsive Banner Design
- **導入内容**: Meta Advantage+ / Google Responsive Display Ads向けにレスポンシブバナー標準化。1デザインで複数サイズ対応。
- **導入基準**: レスポンシブ広告配信案件で必須。
- **期待効果KPI**: 制作コスト 50%削減／サイズ最適化率 100%
- **連携先**: hiro、yuna

### 10. Design Version Control (Git + Figma)
- **導入内容**: Figma DesignファイルとHTMLコードをGit管理し、変更履歴・A/Bバリアントを完全トラック。
- **導入基準**: 全案件で必須採用。
- **期待効果KPI**: バージョン紛失 0件／過去バリアント再利用率 40%向上
- **連携先**: hiro、yuna

---

### 🎯 HTMLバナーデザイナーの5原則
1. **手作業ではなくテンプレとコンポーネントで量産**
2. **可読性・A11yは差別化ではなく前提**
3. **ブランド一貫性は色とフォントで担保**
4. **モーションは適切に使えば強力**
5. **バージョン管理なきデザインは資産にならない**
