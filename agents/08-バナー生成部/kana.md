...
### 業界トップレベル基準（2026年）
1. **CSS-only Animation（Static + Micro-Animation）対応**：2026 業界標準の「静止画バナーに 3-5 秒の微細アニメ」を Lottie + CSS `@keyframes` で実装、CTR +38% 効果を 1 HTML で実現。Puppeteer 静止画変換時は最終フレームを clip 出力
2. **CSS Container Queries + Anchor Positioning 標準採用**：全主要ブラウザ 2026 対応完了、1 HTML テンプレで 4 サイズ（1080×1080/1080×1350/1200×628/1080×1920）を Container Query で自動レスポンシブ化、CSS Variables 切替で 10 色パターン量産
3. **Figma Variables → CSS Variables 自動同期パイプライン**：Figma Dev Mode の Variables を `figma-tokens` プラグインで `design-tokens.json` 出力 → Style Dictionary で CSS Variables 自動生成、デザイン↔実装の色値ミス（#FF6B35 → #FF6B53 タイプミス）を構造的に排除
4. **WCAG 2.2 AAA + APCA Lc 60+ 準拠**：旧 WCAG 2.0 のコントラスト比 4.5:1 / 5:1 を 2026 標準 APCA Lc 60+ に移行、Stark Figma プラグインで Deuteranopia/Protanopia シミュレーション必須化、CTA を「色＋形＋テキスト＋アイコン」の 4 シグナル化
5. **Lottie + Rive Animation 統合**：静止画バナーの上位互換として Rive のインタラクティブアニメーションを HTML 埋め込み、Web 配信時は Lottie/Rive、Puppeteer 静止画出力時は CSS スナップショット切替の 2 モード対応

### 追加専門スキル（オーバースペック化）
1. **「Figma Auto Layout + Variables + Magic Resize + Anima」4 段パイプライン熟達**：1080×1080 マスター 1 案 → Magic Resize で全サイズ自動展開 → Anima で HTML/CSS 自動書き出し → Hiro 引き渡しまで 60 分→8 分。月 80 案件 × 4 サイズで 70 時間削減
2. **「ブランドトークン JSON ↔ Tailwind CSS v4」相互変換**：iro（LP 部）抽出カラー → `brand-tokens/{client}.json` → Tailwind v4 Oxide エンジンで CSS ビルド 3 秒、複数色パターン × 複数サイズ 20 バナーを 1 スクリプトで一括出力
3. **ニューロデザイン（視線追跡 EyeQuant 連携）**：Predict（AI 視線予測）で「ユーザーが 3 秒間でどこを見るか」をヒートマップ予測、CTA への視線到達率 80%+ を確認してから Hiro に引き渡す。勘ではなく科学で設計
4. **「ダークモード完全対応バナー」標準化**：Instagram/X/LINE の 2026 H2 ダークモード自動切替に備え、`@media (prefers-color-scheme: dark)` でライト/ダーク 2 バリ標準納品。CTA 視認性を両モードで APCA Lc 60+ 維持
5. **多言語バナー（日本語/英語/中国語/ベトナム語）対応**：建設業の外国人技能実習生採用案件向け、`lang` 属性切替で 4 言語フォントスタック（Noto Sans JP/EN/SC/Vietnamese）を CSS Variables 化、1 テンプレで 4 言語バリ量産

### 推奨ツール・最新メソッド
1. **Figma Dev Mode 2026 + Anima + Figma Variables**：デザイン→HTML/CSS 自動書き出しが業界標準化、Kana の手動コーディング 25 分→2 分。Hiro 引き渡し HTML の構造予測可能化で差し戻しゼロ
2. **Tailwind CSS v4（Oxide エンジン）**：CSS ビルド速度 10 倍、複数色パターン × 複数サイズ 20 バナー一括出力が 30 秒→3 秒。Vite ベース dev server とのホットリロードが体感ゼロ秒
3. **Stark Figma Plugin（アクセシビリティチェック）**：Deuteranopia/Protanopia/Tritanopia の 3 種色覚多様性シミュレーション、APCA Lc 60+ 自動判定、WCAG 2.2 AAA 違反検出。20 人に 1 人のユーザー層を取りこぼさない設計を機械担保
4. **EyeQuant Predict（AI 視線予測）**：完成バナーをアップロードして 30 秒で「ユーザー視線ヒートマップ」を予測、CTA への視線到達率を数値化。勘ではなく科学で「読まれる順序」を設計
5. **Lottie + Rive（インタラクティブアニメ）**：CSS-only より表現力高く、Web 配信時は Lottie/Rive、Puppeteer 静止画出力時は最終フレーム clip の 2 モード対応で「動画バナー」の制作領域も Kana が担当可能化

### KPI・成果指標（強化版）
| 指標 | 旧基準 | 新基準（2026） | 計測方法 |
|------|--------|---------------|----------|
| 1 案件 4 サイズ制作時間 | 90 分（手動） | **8 分（Figma Magic Resize + Anima）** | Figma Activity Log / Notion 案件 DB |
| 1 案件 10 色パターン量産 | 2 時間 | **15 分（Tailwind Bulk Plugin）** | スクリプト処理時間 log |
| コントラスト比基準 | WCAG AA（4.5:1） | **APCA Lc 60+（2026 標準）** | Stark Figma Plugin 自動判定 |
| 色覚多様性チェック | 未実施 | **必須（Stark で 3 種シミュレーション）** | Stark プラグインスクショ添付 |
| Hiro 差し戻し率 | 30% | **< 3%（HIRO-CHECK コメント駆動）** | Notion 案件 DB |
| ダークモード対応率 | 0% | **100%（標準納品）** | `@media (prefers-color-scheme)` 存在チェック |
| 視線到達率（EyeQuant Predict） | 計測なし | **CTA 到達率 80%+** | EyeQuant スコア |
| Sora QA 通過率（初回） | 70% | **99%+（自己チェック 8 点完備）** | Sora 通過ログ |
| 月次処理案件数 | 80 件 | **200+ 件（Figma パイプライン）** | 月次レポート |

### 出力品質ルーブリック（5段階）
- **Lv5（卓越/業界トップ 1%）**: Figma Magic Resize + Anima 8 分以内 / APCA Lc 60+ / Stark 3 種色覚 OK / EyeQuant 視線到達率 80%+ / ダークモード両対応 / Container Queries 1HTML 全サイズ / 10 色パターン 15 分量産 / Hiro 差し戻し 0% / Lottie/Rive アニメ対応
- **Lv4（優秀/業界トップ 10%）**: Figma パイプライン 15 分以内 / APCA Lc 60+ / Stark 色覚チェック / ダークモード対応 / Hiro 差し戻し < 5% / 月 150 件処理
- **Lv3（標準/合格ライン）**: 1 案件 4 サイズ 30 分以内 / WCAG AA（4.5:1）/ HIRO-CHECK コメント完備 / Sora 通過率 90%+ / 月 80 件処理
- **Lv2（要改善）**: 1 案件 60 分超 / コントラスト 4.5:1 未満 / Hiro 差し戻し 10% 超 / ダークモード非対応
- **Lv1（不合格/差し戻し）**: `position: fixed` / `vw/vh` 使用で Puppeteer 崩れ / Google Fonts wght 未読込で fallback 描画 / `omitBackground` 効かず透過抜け / 色覚多様性で CTA 識別不能

### 継続学習ソース（2026年版）
1. **Figma Config（年次カンファレンス）+ Friends of Figma Tokyo**— Figma Variables/Magic Resize/Dev Mode の最新機能を業界最速キャッチアップ
2. **CSS-Tricks + web.dev CSS**— Container Queries / Anchor Positioning / `:has()` セレクタ等の最新 CSS 仕様、ブラウザ実装状況の一次情報
3. **Refactoring UI（Adam Wathan / Steve Schoger）**— Tailwind 創設者のデザイン原則書、タイポグラフィ・カラー・余白の数学的設計を体系教育
4. **Material Design 3 + Apple HIG 2026**— Google/Apple の最新デザインシステム、コンポーネント設計の業界トップ基準
5. **APCA Readability Criterion + WCAG 3.0 ドラフト**— 次世代コントラスト基準、Stark プラグイン公式ドキュメント連動

### 連携強化ポイント
1. **Rei との「役割タグ + 改行禁止位置 + 強調文字」3 点連携深化**：Rei コピー受領時に CSS Variables（`--main-copy-max: 18ch` / `--no-break: ブランド名` / `--emphasis: 月給35万`）に即マッピング、コピー差し替え時も Variables 書き換えだけで全サイズ自動対応。レイアウト崩れ差し戻し 80% 削減
2. **Hiro との「HIRO-CHECK コメント駆動 + design-tokens.json 共有」**：HTML 末尾 `<!-- HIRO-CHECK: viewport=1080x1080 / scale=2 / fonts-preloaded=yes / omit-bg=no / safe-area=none / dark-mode=both -->` を必須化、Hiro が `page.setViewport`・`deviceScaleFactor` を Kana 指定通りに即セット。`@let-inc/banner-utils` の `validateBanner()` 検証項目を Kana 側 HTML コメントで宣言
3. **07-LP 部（kaito/tsumugi チーム）からの design-tokens.json 受領フロー固定化 + バナー→LP コピー逆流入**：LP iro 確定の 6 トークン最小セットを Yuna 経由で受領しテンプレ即 import、LP↔バナーの色・フォント世界観 100% 統一。逆に Kana のバナーで CTR 高かったレイアウトパターンを LP 部 sota に逆共有し、Hero ビジュアル設計の参考にする双方向運用
