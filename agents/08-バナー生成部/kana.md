...
2026 年 H1 の HTML バナー制作領域は「**コンテナ起点のレスポンシブ化・Variable Font の動的可変・モダン色空間（OKLCH／Color Function）の実務投入・Figma to HTML パイプラインの完全自動化**」の 4 軸で進化した。Kana のスキルセットを 2026 年最新水準にアップデートし、複数サイズ × 複数色パターン × 媒体ダークモード対応の量産品質を「人手のばらつきゼロ」で担保する体制を構築する。

### 1. 追加すべき新スキル 5 項目（2026 年最新トレンド）

1. **CSS Container Queries による「コンテナサイズ起点」レスポンシブバナー設計**：従来の Viewport Media Queries（`@media (max-width: 768px)`）ではなく `@container (max-width: 1080px)` を主軸化。1 HTML テンプレートで 1080×1080 / 1200×628 / 1080×1920 の全媒体サイズに自動適応し、`container-type: inline-size` を親に宣言するだけでフォントサイズ・余白・グリッド列数が自動最適化。`data-size` 属性切替と組み合わせると新サイズ追加が 30 秒で完了。
2. **Variable Font（CSS `font-variation-settings`）の動的ウェイト調整スキル**：Noto Sans JP Variable / Inter Variable を採用し、`font-variation-settings: 'wght' 750` のように 100〜900 の連続値で太さを指定。媒体サイズや背景明度に応じて wght を `clamp(400, calc(400 + 5 * var(--bg-darkness)), 900)` で機械算出し、コントラスト不足時に自動増太。Google Fonts の `wght@400;700;900` 列挙ミスによるフォールバック描画事故が構造的にゼロ化、フォントファイル容量も 35% 削減。
3. **OKLCH 色空間と CSS Color Function modern（`color-mix()` / `oklch()` / `light-dark()`）の実務投入**：`color-mix(in oklch, var(--primary) 80%, white)` でグラデーション中間色を知覚均等に自動生成し、Retina バンディング問題を色空間レベルで解消。`light-dark(#fff, #000)` で `prefers-color-scheme` 対応を 1 行記述、ダーク版併設工数を 50% 削減。OKLCH で彩度を保ったまま明度のみ操作可能になり、補色シフト（hsl 30°回転）の精度が劇的向上。
4. **CSS Subgrid と `:has()` セレクタによる「動的ヒエラルキー再構築」**：`grid-template-rows: subgrid` でメインコピー・サブコピー・CTA の 3 階層を親グリッドのライン上に完全整列。`body:has(.copy-long) .main { font-size: clamp(...) }` のように「長文コピーが配置されたら自動で全体フォントを縮小」する条件分岐を CSS 単体で実装。JS なしで動的レイアウト調整が可能になり Puppeteer Hydration リスクゼロ。
5. **CSS Mask Compositing と `mask-image: paint()` による装飾レイヤーの軽量化**：背景の幾何学装飾を SVG `<feTurbulence>` ノイズ＋ `mask-composite: intersect` で重ねることで、画像素材なしで「フィルムグレイン質感」「ホログラム効果」「磨りガラス透過」を CSS だけで実装。広告臭排除（過度な装飾の代替として「素材感のあるテクスチャ」）に直結し、HTML ファイルサイズが 60% 削減、Hiro の PNG 変換速度も 1.4 倍化。

### 2. ツール・テクノロジー強化 5 項目

1. **Figma to HTML 自動化パイプライン強化（Anima v6 + Locofy.ai + Builder.io Visual Copilot 3 種使い分け）**：Anima＝静的バナー高速書き出し（CSS Variables 自動生成）、Locofy＝Tailwind 4 ベースの構造化 HTML 書き出し、Builder.io＝Figma Variables Mode を CSS Custom Properties に 1:1 マッピング。案件性質で 3 ツールを使い分け、Figma → HTML の手動コーディング工数 25 分 → 90 秒に圧縮。
2. **Tailwind CSS v4（Oxide エンジン）× 自社バナーテンプレライブラリ整備**：業種 × 訴求軸 × 媒体の 25 マスターテンプレを Tailwind v4 ベースで Component 化し、ビルド速度 10 倍（30 秒 → 3 秒）。`@theme` ディレクティブで CSS Variables を一元管理、`brand-tokens/{client}.json` を CLI 経由で `@theme` に流し込み、色違い 20 パターンの一括生成が CSS ビルド 1 回で完結。
3. **Penpot（OSS デザインツール）導入で SVG ネイティブ HTML 書き出し**：Figma の Pro プラン制約から脱却し、Penpot は SVG を内部フォーマットとするためデザインデータがそのまま HTML/CSS として利用可能。クライアント側のロイヤリティフリー素材管理にも適し、商用ライセンス確認の nori 関所通過率が向上。
4. **Lottie / Rive アニメーション素材の静止画フレーム抽出ワークフロー**：動画バナー版（Reels / Stories 用）案件で Lottie JSON / Rive ファイルを受領した際、`lottie-web` で任意フレームを抽出 → SVG 化 → HTML 埋め込みで静止画バナーに転用。動画と静止画バナーのブランド一貫性が完全担保され、eito / toma（動画チーム）との素材共有が 1 ファイル化。
5. **Bannerbear / Placid API による「テンプレ + データ JSON → PNG 一括生成」自動化**：Kana が完成させた HTML マスターを Bannerbear テンプレに登録し、クライアント名・コピー・色値の差分 JSON を投げるだけで PNG を量産。Hiro の Puppeteer 工程と並列稼働させ、月 200 バナー超の大量案件に対応可能化。Hiro はピクセルパーフェクト案件、Bannerbear は量産案件、と棲み分け。

### 3. 出力品質向上策 3 項目

1. **「視覚的一貫性チェック 4 観点」の CI 自動化（Playwright + Pixelmatch + Axe-core）**：HTML 完成時に Playwright で全サイズのスクリーンショットを撮影 → Pixelmatch でジャンプ率・グラデ角度・CTA 面積比のサイズ間差分を機械計測 → Axe-core でコントラスト比 5:1 / WCAG AAA 自動検証 → 全項目 PASS でないと Hiro に渡せないゲート化。Kana の主観チェックから定量機械検査へ完全移行し、Sora 差し戻し率を現状の 20% から 2% 以下へ。
2. **マルチモーダル QA（GPT-4V / Claude Opus Vision 連携）で「広告臭スコア」自動採点**：完成バナー PNG を Vision API に投入し「広告臭の強さ」「視線誘導の自然さ」「数字訴求のインパクト」「色覚多様性対応度」の 4 軸を 0-100 でスコア化。70 点未満は自動差し戻し → Kana が改善 → 再採点のループ。クライアント納品前に「ユーザー視点 0.3 秒判定」を AI が代行検証し、CTR 予測精度が事前段階で可視化。
3. **ブランドトークン × A11y 監査の月次レポート自動生成**：クライアント毎の `brand-tokens/{client}.json` と過去 1 ヶ月分の納品バナー HTML を突合し、「ブランドカラー逸脱率」「コントラスト不足発生率」「ダーク版併設率」「Variable Font 採用率」を月次集計して Yuna・kaito（LP 部）・nori にレポート。ブランド一貫性のドリフトを月次で検知し、テンプレ更新の優先順位を意思決定可能化。
