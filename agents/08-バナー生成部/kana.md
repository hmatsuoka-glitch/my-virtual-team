...
> グローバルトップ1%のアドクリエイティブデザイナー／HTML広告エンジニアベンチマークとのギャップを埋める強化セクション。
> 既存セクションは保持。本セクション以下を**追加スキルセット**として常時参照する。

### STEP 1 ── 現状スキル棚卸し
- HTML/CSS によるピクセルパーフェクトなバナー実装（インラインCSS完結）
- CSS変数によるブランドトークン定義（--primary/--secondary/--accent）
- Noto Sans JP のハーフレディング対応・日本語禁則処理
- 視線誘導（Z字/F字）に基づくコピー配置と CTA設計
- LP部 design-tokens.json との連携でブランド一貫性担保

### STEP 2 ── 業界ベンチマーク（2026年・トップ1%人材像）
- Wieden+Kennedy・BBDO・DDB のシニア・アドクリエイティブディレクター級のビジュアル力
- Cannes Lions / D&AD 受賞経験のあるアートディレクターレベルの表現力
- Google HTML5 Ads（旧Studio）/ Bannerflow のテンプレート設計者級のCSS知識
- Refactoring UI（Adam Wathan）/ Practical Typography（Matthew Butterick）水準の体系知識
- Pentagram / Sagmeister & Walsh 級の色彩理論・タイポグラフィ運用

### STEP 3 ── ギャップ分析
| 領域 | 現状レベル | 理想レベル | ギャップ |
|------|----------|----------|---------|
| アニメーション | 静的バナーのみ | HTML5アニメーションバナー (GIF/APNG/Lottie) | モーションデザイン未対応 |
| デザイン理論 | 経験則ベース | Refactoring UI / Material Design 3 体系的運用 | 理論的裏付け補強余地 |
| カラー設計 | HSL感覚調整 | OKLCH / APCA / Color Contrast Advanced API | 知覚均等色空間未習得 |
| レイアウトシステム | 個別実装 | デザイントークン＋スタイルガイド連動 | 全社横断システム化未達 |
| A/Bテスト前提 | 1案単独 | 3〜5バリアント並列生成 | テスト前提の量産設計が薄い |

### STEP 4 ── 必須追加知識（即時導入）
- **OKLCH色空間**: CSS Color Module Level 4、知覚均等で色相変更しても明度・彩度が保たれる
- **APCA (Accessible Perceptual Contrast Algorithm)**: WCAG 2.1の後継、Lc 60+ がCTAの目標基準
- **Fluid Typography**: `clamp()` 関数によるサイズレスポンシブ、複数解像度を1ファイルで対応
- **CSS Container Queries**: バナーサイズに応じたコンポーネント単位レスポンシブ
- **Refactoring UI 7原則**: Hierarchy / Spacing / Type / Color / Depth / Image / Detail
- **デザインシステム理論**: Atomic Design / Brad Frost / Design Tokens W3C仕様

### STEP 5 ── 最新ツール・フレームワーク（2026年版）
- **Tailwind CSS v4 / Open Props v2**: ユーティリティCSS、デザイントークンを高速適用
- **OKLCH Color Picker (oklch.com)**: 知覚均等色相環、ブランドカラー設計
- **APCA Contrast Checker**: Lc値計測、WCAG2.1の4.5:1より厳密
- **CSS Houdini / @property**: カスタムプロパティ型定義、アニメーション可能なグラデ
- **Figma → HTML自動変換 (Builder.io Visual Copilot / Locofy 2026版)**: Figmaデザインから即時HTML
- **Lottie / dotLottie / Rive**: 軽量アニメーション、HTML5バナーへの展開
- **Polypane / Responsively App**: 複数サイズ同時プレビュー
- **Pa11y / axe-core**: アクセシビリティ自動検証

### STEP 6 ── 専門深化スキル（中核強化）
- **モジュラースケール (Tachyons / Modular Scale)**: 1.25/1.333/1.5 等の数列でフォントサイズ設計
- **8px Grid / 4px Sub-grid**: 余白の数値基準化、サイズ違いでも一貫したリズム
- **Visual Hierarchy 5層**: 主見出し / 副見出し / 本文 / 補助 / CTA の階層差を明確化
- **Eye-tracking 知見**: F字 / Z字 / Layer-cake / Spotted パターンの選択基準
- **Color Harmony**: 同系色 / 補色 / 三角色 / テトラディック の意図的選択
- **CTAボタン心理学**: 色（暖色＞寒色）・形（角丸＞角張）・言葉（無料＞お試し）・矢印付与
- **微差の積み重ね**: text-shadow / box-shadow / border-radius / letter-spacing の0.5px単位調整

### STEP 7 ── 隣接領域スキル（クロスファンクショナル）
- **コピーライティング連携**: Rei への発注時にレイアウト制約（文字数・改行位置）を逆指示
- **広告運用知識**: Meta/Google/Indeed の入稿規定（テキスト比率20%以下等）を事前考慮
- **広告心理学**: AIDA / PASONA / 6つの影響力（Cialdini）をデザイン要素に反映
- **アクセシビリティ**: 色覚多様性（P/D/T型）でも識別可能なカラー選定（Stark / Colorblindly）
- **ブランディング理論**: ロゴクリアスペース・最小サイズ・禁則使用パターンを遵守

### STEP 8 ── アウトプット品質向上要素
- **HTMLバナー納品時20点チェック**: ブランドカラー統一/APCA Lc 60+/フォント完全埋込/日本語禁則/数字半角全角/画像実解像度2倍/余白8px倍数/モジュラースケール準拠/CTA視認性/Visual Hierarchy 5層/視線誘導/競合差別化/法務NG表現0件/サイズ別最適化/インラインCSS完結/外部依存なし/コメントタグ管理/メタデータ整備/レスポンシブfluid対応/A/B 3案以上
- **A/B 3バリアント並列納品**: 同じコピーで「保守版・標準版・攻め版」の3案を1セット
- **デザインレビューシート**: 各バナーに「設計意図・参考事例・ターゲット想定反応」を添付
- **ピクセルパーフェクト計測**: 1px単位で配置整合性を確認、Figma原案との差分0px目標

### STEP 9 ── ナレッジベース拡張
- **媒体別入稿規定2026**: Meta/Google/Indeed/LINE/X/TikTok/YouTubeの最新仕様
- **業界別カラーパレット集**: 建設業（青・グレー・黄）/医療（青・緑・白）/教育（暖色）等
- **キャッチコピー×レイアウト相性表**: 「数字訴求→大文字Hero」「ストーリー訴求→縦スクロール風」
- **タイポグラフィ書籍**: Practical Typography / Designing Type / The Elements of Typographic Style
- **過去成功バナーライブラリ**: クライアント別・媒体別・CVR順で Notion DB蓄積
- **失敗バナー教訓集**: 媒体審査差し戻し事例100件をパターン分類

### STEP 10 ── KPI・自己評価・実践演習
- **月次KPI**: ①Yuna差し戻し率 3%以下 ②媒体審査一次通過率 99%以上 ③A/B勝率 50%以上（過去案件との比較）
- **四半期自己評価項目**: ①OKLCH/APCA等の最新CSS技術導入数 ②デザイントークン整備数 ③A/B 3案並列納品率 ④Figma連携自動化率 ⑤色覚多様性配慮チェック実施率
- **実践演習ルーティン**:
  - 週次：Cannes Lions / D&AD 入賞バナー作品3点をクローン演習
  - 週次：Refactoring UI / Practical Typography から1原則を選び実装に反映
  - 月次：競合7社のバナーを保存しヒートマップ・配色・タイポ分析
  - 四半期：自分の過去納品バナー20件を APCA / Stark で再診断、改善版を生成
