...
### 2026-08-30
- **CSS Anchor Positioning本格採用でバッジ・ふきだしの吸着配置を宣言的に固定**：`anchor-name`/`position-anchor`で「給与テキストの右上に吸着する未経験OKバッジ」を絶対座標でなく相対関係で定義し、コピー長が変わってもズレない。展開サイズ違いでのバッジ位置調整コストが消滅。効果例：15サイズ展開のバッジ配置調整30分→2分／展開コピー変更時の再調整不要
- **可変フォント（Variable Font）1本化で`wght`軸を`clamp()`連動、サイズ帯別フォントリンク管理を廃止**：Noto Sans JP Variableを1本読み込み`font-variation-settings: 'wght' clamp(500,50cqw,900)`で、小サイズは細く大サイズは太くを自動化。Google Fontsのweight複数指定によるFOUTと不要リクエストが消える。効果例：フォントリクエスト5本→1本／FOUT起因のフォント差し戻し月3件→0件
- **Rei/iroからの入力を「1つのJSONスキーマ」に統一、`banner-payload.schema.json`で妥当性検証**：条件3点・パレット・改行許可位置・lossless セレクタを単一スキーマで受け取り、Ajv検証を通してからレイアウト開始。仕様漏れが着手前に発覚し、着手後の"聞き返し"が消える。効果例：Rei/iroとの往復2回→0回／案件着手から初稿までの所要時間45分→18分
- **`content-visibility: auto`と`contain: layout paint`をバナー要素に付与し、Hiro側変換のペイントコストを半減**：バナー内の独立ブロックにcontainmentを明示すると、ヘッドレスChromiumのレイアウト・ペイント計算が要素単位で完結し、大サイズ書き出し時のCPU負荷が下がる。Hiroのバッチ時間が15%短縮。効果例：105枚バッチのペイント時間3.7分→2.9分／CPU使用率ピーク85%→60%
- **Claude 5 Vision Preview で「配色コントラスト×可読性」を着手前に予測、実HTML生成を回避したNG検出**：ラフ段階のカラー案画像を Claude 5 に「求職者視点で3秒で条件3点を読み取れるか／背景と主訴求の分離が成立しているか」を判定させ、NG時点で iro/Rei へ差し戻し。HTML実装後のやり直しが消える。効果例：実装後NGによる作り直し月5件→0件／初稿到達までの試行回数3回→1.4回
- **07-LP部 iro との「LP↔バナー同期プレビュー」共通ページ運用、色ズレ検出を目視完結**：iroの`design-tokens.json`更新時にKuu管理のVercel Blobへ「LP断片＋バナー1枚を並べた同期プレビュー」を自動デプロイし、色ズレ・トーン違いを1画面で目視確認可能に。tokens変更起因の差し戻しが構造的に消える。効果例：LP↔バナー色ズレクレーム月2件→0件／iro更新の確認往復2日→10分
- **09-システム開発部 Mio との「ビジュアルリグレッションテスト」連携、7社×15サイズをCIで自動監視**：Playwright + pixelmatch のスクショ比較を`banner-utils`のCIに組み込み、共有ライブラリ改修時に7社×15サイズ=105枚の差分を機械検出。Hiroの決定性テストと二重ゲートで納品後の差戻しをゼロに。効果例：ライブラリ改修起因の納品後発覚バグ月3件→0件／改修時のQA工数2時間→自動15分
