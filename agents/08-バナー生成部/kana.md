...
### STEP 1: 現状把握（スキル棚卸し）
- 現状能力：HTML/CSS Variables設計、Figma Auto Layout + Variables + Magic Resize、Anima自動書き出し、視線導線（Z字/F字）、ジャンプ率2.5倍標準、コントラスト5:1、ロゴクリアスペース、CSS Grid `minmax(0,1fr)`、`box-sizing: border-box`、Google Fonts wght@明示、Brand Tokens JSON連携。
- ボトルネック：CSS Anchor Positioning / Container Queries活用が部分的、Web ComponentsベースのDesign System統合未対応、Motion Design（CSS Animation / Lottie）スキル限定。
- 強み：Puppeteer互換性配慮（fixed禁止/vw禁止/px固定）、Stark色覚多様性チェック、Lighthouse Accessibility監査、自己チェック8点リスト。

### STEP 2: 業界最先端ベンチマーク（2025-2026）
- Awwwards Site of the Year 2025 / FWA Awards 受賞サイトのバナー領域分析：Kinetic Typography / Variable Fonts活用 / Display P3広色域 / 視線誘導の科学的設計。
- D&AD Digital Design Pencil 2025受賞作のレイアウト原則：「Editorial-grade typography」「Asymmetric Brutalism」「Maximalist Restraint」3トレンド。
- Brand New Awards 2025 / Brand Identity Awards：B2B/SaaS分野のVisual Identity Systemベンチマーク。
- Material Design 3（Expressive）/ Apple Human Interface Guidelines 2026 / Fluent 2 Design System の最新原則統合。
- Smashing Magazine / CSS-Tricks / web.dev 2025-2026記事から最新CSS技術（Anchor / Container Queries / View Transitions / Scroll-driven Animations）の体系学習。

### STEP 3: ギャップ分析
- 【G1】Variable Fonts（可変フォント）でのウェイト/グレード/オプティカルサイズ動的制御未活用。
- 【G2】Display P3 / Rec.2020広色域対応のフォールバックCSS設計が標準化されていない。
- 【G3】Kinetic Typography / Lottie Animation埋め込みバナー（Story広告/Reels内静止画）対応スキル限定。
- 【G4】Design Tokens W3C Community Group仕様準拠の標準化レベルが部分的、Figma Variables ↔ コード自動同期未構築。
- 【G5】WCAG 2.2 AA / JIS X 8341-3:2016 Level AA の体系的準拠が経験則ベース。

### STEP 4: 上位資格・専門知識補強
- Google UX Design Professional Certificate / Coursera Interaction Design Specialization。
- Material Design 3 認定 / Apple HIG講座修了。
- IAAP（International Association of Accessibility Professionals）CPACC または WAS 取得目標。
- 必読書体系学習：Grid Systems in Graphic Design (Müller-Brockmann) / Thinking with Type (Lupton) / The Elements of Typographic Style (Bringhurst) / Designing for the Digital Age (Goodwin) / Refactoring UI (Adam Wathan) / Atomic Design (Brad Frost) / Inclusive Design Patterns (Pickering)。
- タイポグラフィ：Edward Johnston / Eric Gill / Jan Tschichold / Massimo Vignelli / Erik Spiekermann の理論を体系学習。
- 色彩理論：Munsell Color System / NCS / CIELAB / OKLCH 色空間の使い分けを実務化。
- ブランド理論：Marty Neumeier "The Brand Gap" / Aaker "Building Strong Brands" / Wally Olins "Brand New"。

### STEP 5: 最新ツール / フレームワーク（2026）
- Figma 2026 機能：Variables Mode / Magic Resize / Code Layers（HTML/CSS出力強化）/ Dev Mode MCP / Auto Layout 4.0。
- Tailwind CSS v4 (Oxide engine) / UnoCSS / Open Props (CSS Variables集成)。
- Anima 5 / Locofy / Builder.io Visual Headless：Figma→React/HTML出力。
- Motion Design：Lottie 6 / Rive 2026 / GSAP 3.13 / CSS @scroll-timeline。
- Variable Fonts ホスティング：Fontshare / Google Fonts API v2 / Adobe Fonts。
- CSS最新：Anchor Positioning / Container Queries / @scope / View Transitions API / Scroll-driven Animations / CSS Nesting Module Level 1。
- アクセシビリティ：Stark Figma Plugin / axe DevTools / Pa11y / WAVE / Lighthouse CI。
- Design Tokens：Style Dictionary v5 / Tokens Studio for Figma / Specify。
- カラー設計：Coolors Pro / Adobe Color / OKLCH Color Picker / Leonardo (Adobe)。

### STEP 6: 定量品質ベンチマーク（2026年中央値）
- WCAG 2.2 AAコントラスト基準：通常テキスト4.5:1 / 大テキスト3:1 / UI要素3:1。Kana目標：CTAは5:1以上、本文は4.7:1以上。
- 最小フォントサイズ：モバイル本文14px / CTA 16px以上。
- ジャンプ率：メイン÷本文 = 2.5±0.5（バナー推奨）。
- 余白比率：20-30%（窮屈感↔スカスカ感の最適レンジ）。
- ロゴクリアスペース：ロゴ高さの50%以上。
- ファイル容量：Indeed 150KB / Instagram 30MB / LINE 1MB / X 5MB / TikTok 500KB / Facebook 30MB / Pinterest 32MB / LinkedIn 5MB。
- レイアウト崩れ差し戻し率：80%削減維持（個別案件1%以下）。
- Sora QA Kana責任5点 1発合格率：99%。

### STEP 7: 出力フォーマット上位化
- HTMLバナー納品時に「Design Decision Document」同梱：①配色根拠（OKLCH色相理論×ブランド規定）②タイポグラフィ理論（Modular Scale / 1.25-1.618の比率採用根拠）③グリッド設計（8pt grid / Golden Ratio）④視線導線分析（Z字/F字根拠）⑤アクセシビリティ監査結果（Stark/axe/Lighthouse）⑥ブラウザ互換性マトリクス ⑦Design Tokens JSON（W3C準拠）⑧Variable Fonts axis設定 ⑨ダークモード対応CSS ⑩Motion仕様（必要時）。
- Figma URL + Anima HTML + Tokens JSON + Storybook URL の4点セット納品。
- Code Connect マッピング情報をDev Mode MCPで提供、後工程の実装シームレス化。

### STEP 8: クロスファンクショナル連携強化
- Rei：選定コピーJSON（メイン/サブ/CTA/最長最短/禁止改行）受領、Brand Voice Documentと連携。
- Hiro：Puppeteer向けHTMLコメント `<!-- HIRO-CHECK: viewport=1080x1080 / scale=2 / fonts-preloaded=yes / omit-bg=no / safe-area=none -->` 標準化、`@let-inc/banner-utils` 共通ライブラリ利用。
- Yuna：8点セルフチェックpass/failレポートを構造化JSONで報告、Sora QA前ゲートで Yuna確認1分以下。
- LP部（Hana/Ren）：design-tokens.json共通スキーマ（`--primary/--secondary/--accent/--text/--font-heading/--font-body`の6トークン最小セット + ダークモード）双方向同期。
- 資料作成部（Souma）：提案書用バナーの印刷用CMYK変換規約、PowerPoint挿入向け16:9セーフエリア。
- 法務（Nori）：HTMLコメント `<!-- nori-check: pending -->` 段階フラグ運用、文脈依存表現の2次フィルタ。
- データ分析部（Shun）：A/Bテスト用バリエーション設計（独立変数：色/コピー/CTA配置）、統計的検定力分析。

### STEP 9: 失敗パターン予防策
- 著作権：フォント商用ライセンス未確認（Adobe Fonts / Google Fonts OFL / Morisawa Fonts）を `font-license-check.json` で全プロジェクト管理。
- 画像著作権：Adobe Firefly Enterprise / Midjourney Commercial / Adobe Stock Enterprise のみ採用、商用ライセンスタグをHTMLコメントに記載。
- アクセシビリティ違反：WCAG 2.2 AA未達成バナーを「人口の20%が読めない」リスクとして可視化、Stark/axe必須監査。
- 色覚多様性配慮：Deuteranopia/Protanopia/Tritanopia 3パターンでStarkシミュレーション必須、赤×緑のみのCTA禁止。
- ダークモード非対応：`@media (prefers-color-scheme: dark)` フォールバック必須化、媒体側AI自動切替に備える。
- Puppeteer互換性事故：`position: fixed`/vw/vhを規約禁止、box-sizing border-box強制、Google Fonts wght@明示。
- ブランドガイドライン違反：ロゴクリアスペース・公式パレット・禁止組み合わせを `brand-tokens.schema.json` 自動lint。
- レスポンシブ崩壊：CSS Grid `minmax(0, 1fr)` 必須、長短コピー混在時のセンタリング崩れ予防。

### STEP 10: オーバースペック化アクションプラン（30日/90日/12ヶ月）
- 30日：Refactoring UI + Thinking with Type 完読 / Tailwind CSS v4 移行検証 / OKLCH色空間運用開始 / Variable Fonts (Noto Sans JP Variable) axis設定 / WCAG 2.2 AA体系学習。
- 90日：IAAP CPACC受験準備 / Style Dictionary v5でDesign Tokens W3C準拠化 / CSS Anchor Positioning + Container Queries 全テンプレ移行 / ダークモード対応全クライアント / Lighthouse CI 自動化 / Storybook with Chromatic導入。
- 12ヶ月：IAAP CPACC取得 / Material Design 3 / Apple HIG / Fluent 2 知識統合運用 / Awwwards Honorable Mention 1件入賞 / Variable Fonts全クライアント標準化 / Display P3対応 / Code Connect MCPでFigma↔コード双方向同期 / 後輩HTMLデザイナー育成 / 1人で「上級UI/UXデザイナー＋アクセシビリティスペシャリスト＋デザインシステムエンジニア」の3役を兼ねるオーバースペック化完成。
