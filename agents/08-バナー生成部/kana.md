...
### 2026-09-21
- **2026年最新トレンド：CSS Container Queries + `@property` 型付きカスタムプロパティで多サイズ 1 HTML 化**：`@container (min-width: 1200px)` で Indeed 1200×628 レイアウトを、`(aspect-ratio: 1/1)` で Instagram 1080×1080 レイアウトを 1 HTML 内に共存させる新標準。`@property --cta-scale { syntax: '<number>'; initial-value: 1; inherits: false }` で CTA スケールを型安全に補間、Hiro の viewport 切替だけで異媒体版が生成される。従来 4 サイズ = 4 HTML → 1 HTML でメンテ工数 75% 削減、色パターン差替も CSS Variables 1 箇所で完結。
- **2026年最新トレンド：Tailwind CSS v4 + Lightning CSS で本番用インライン CSS を 20KB 以内に自動収束**：Puppeteer 出力用 HTML で `@tailwindcss/cli --minify` を通し、未使用ユーティリティを PurgeCSS で削除。フォント woff2 の subset を `unicode-range: U+3000-9FFF,U+FF00-FFEF` で日本語常用漢字＋半角に限定し、CTA バッジ 1 個で数十 KB 消費していたのを 2KB に圧縮。読み込み速度が Puppeteer の networkidle 待機を 800ms 短縮、Hiro のバッチ処理時間短縮に直結。
- **失敗パターン：`clamp(min, vw, max)` を CTA 文字サイズに使ってしまう** → 回避策：バナーは Hiro が固定 viewport で描画するため `vw` 単位は「意図せず巨大化・微小化」の原因になる。CTA・条件3点は `px` 固定 or `em`（親フォントサイズ相対）に限定し、`clamp` を使うのはコンテナ本体の padding だけ（理由：Kana ローカルプレビュー時のブラウザ幅で見た目が変わり、Hiro 変換後と齟齬が出て mia 差し戻しになる）。
- **失敗パターン：`background-image: url(data:...)` の Base64 埋込を全部やってしまう** → 回避策：Puppeteer は `waitUntil: 'networkidle2'` でネットワーク読込を待つが、Base64 は「ネットワーク往復ゼロ」で完了扱いになり、フォント読込との race condition が発生。写真素材（10KB 超）は外部 file:// パスで参照、ロゴ SVG や 2KB 未満のアイコンのみ Base64 化に統一（理由：フォント未読込のまま screenshot が撮られ「豆腐化 PNG」の再現性ある事故）。
- **品質チェックポイント：`document.fonts.ready` await + 空セレクタ検出**：HTML 最終盤に `<script>document.fonts.ready.then(() => document.body.dataset.fontsReady = 'true')</script>` を仕込み、Hiro 側で `page.waitForSelector('body[data-fonts-ready="true"]', {timeout: 5000})` を待機。同時に `document.querySelectorAll('[data-required]:empty')` で条件3点の空要素を検出、テンプレ差込漏れを screenshot 前に例外送出。
- **品質チェックポイント：`prefers-color-scheme: dark` 表示検証を Kana 段階で強制**：DevTools エミュレーションで dark モード切替時に「白背景 + 白文字」が発生していないかを事前確認。近年 iOS/Android の自動 dark 変換で背景色 fallback が効かないケース増加、条件3点エリアには `color-scheme: light only` を明示指定、CTA には `background-color: white !important; color: #E60012 !important` の 2 段固定で強制ライト固定。
- **効率化テクニック：Figma → HTML の中間変換に `figma-to-html` MCP + Tailwind v4 の JIT**：Sota や sota が Figma に用意した参考 LP のセクションから Auto Layout ノードだけ抽出し、Tailwind クラスに機械変換。Kana は「テキスト差替 + Variables 調整」だけで実装完了、参考 LP からのバナー派生案件で着手 → 初稿 3時間 → 45分に短縮。
- **Rei 連携の小ヒント：キャッチコピー 15 案受領時に「文字数 × ウェイト」マトリクスを返す**：Rei が「日給12,000円〜／未経験OK／週休2日」の 3 案を持ってきたら、Kana は各案に「10文字 × Bold 72px」「16文字 × Medium 56px」の CSS 計算値を即座に返信。Rei は残りの 12 案を「実装可能な文字数レンジ」に絞って再検討可能、コピー往復回数 3 回 → 1 回に集約。
