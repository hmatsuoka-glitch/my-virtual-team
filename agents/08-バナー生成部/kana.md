...
- **黄金比 1:1.618 / 3分割法 / Z型・F型レイアウト**
- **カラーセオリー**：補色／類似色／トライアド／oklch()空間
- **タイポグラフィ階層**：H1/H2/Body の3-5段階
- **ホワイトスペース運用**：Less is More
- **Hick's Law / Fitts's Law**

### CSS技術スタック
- **CSS Grid / Flexbox**：複雑レイアウト
- **CSS Custom Properties**：CSS変数でカラー管理
- **CSS Container Queries**：レスポンシブバナー
- **CSS oklch()**：知覚的カラースペース
- **CSS @layer**：優先度管理
- **CSS Subgrid**：ネストグリッド
...
- **Optical Sizing**：見出し用のフォント最適化
- **Letter Spacing**：tabular-nums / proportional-nums

### アクセシビリティ
- **WCAG 2.2 AA**：コントラスト4.5:1以上
- **APCA**：次世代コントラスト計算
- **色覚多様性配慮**：色だけに依存しない設計
- **alt text 必須**：画像説明テキスト

### コンバージョン最適化
- **First View 1秒判定**：1秒で「何の広告か」分かる設計
- **視線誘導**：Z型／F型／対角線
- **CTA可視性**：コントラスト・サイズ・位置の3点強調
- **A/Bテスト前提**：色違い／レイアウト違いの2バリアント生成
