...
### 1. HTMLバナーデザインの世界水準
- **HTML5 Banner Best Practices (IAB)** — クリック領域・ファイルサイズ・互換性
- **Responsive HTML Banners** — 1ソース→複数サイズ自動派生
- **CSS Grid / Flexbox / Container Queries** — 柔軟レイアウト
- **CSS Variables / Custom Properties** — テーマ切替
- **Web Fonts (variable fonts)** — 高速ロード

### 2. タイポグラフィ・配色精密設計
- **Typography Scale (Major Third 1.25 / Perfect Fourth 1.333)**
- **Vertical Rhythm / Baseline Grid**
- **WCAG 2.2 AAA (7:1) コントラスト**
- **OKLCH色空間** — 知覚均等の配色
- **Color Harmony (Triadic / Tetradic / Split-Complementary)**

### 3. 視線誘導・行動心理
- **Z-Pattern / F-Pattern Visual Hierarchy**
- **Gestalt 原則** — Proximity/Similarity/Closure
- **Pre-Attentive Processing** — 色・形・動き・空間で250ms以内訴求
- **Eye-Tracking Predictive (Attention Insight)**

### 4. 最新ツールスタック
- **Figma + Figma Dev Mode** — デザイン→HTML直結
- **shadcn/ui / Radix UI** — アクセシブルコンポーネント
- **Tailwind CSS 4.0** — モダンCSS
- **CSS Houdini / Paint Worklet** — カスタム描画

### 5. 重点強化KPI
| 指標 | 現状 | H2目標 |
|---|---|---|
| Pixel-Perfect実装率 | 90% | 99.9% |
| Hiro一発PNG化成功率 | 85% | 100% |
| バナー制作リードタイム | 2時間 | 30分 |
| アクセシビリティAAA達成 | 60% | 100% |

### 6. 成長ロードマップ
- **M1**: HTML5 Banner Standards / IAB認定 / Figma Dev Mode習熟
- **M2**: Tailwind+OKLCH+Container Queries標準化
- **M3**: AI支援HTMLバナー（Figma → v0 → Kana監修）
