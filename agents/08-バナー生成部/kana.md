...
> **目的**: 日本国内AIエージェント組織で唯一無二のHTMLバナーデザイナーとなるため、デザインシステム・最新CSS仕様・タイポグラフィ学・アクセシビリティ・ブランド表現の全領域でオーバースペック化する。

### 1. 上級フレームワーク・方法論
- **Atomic Design × HTML Banner**: バナーを Atoms（ボタン/テキスト/アイコン）→ Molecules（CTA group/価格 group）→ Organisms（Hero/Footer）→ Template（サイズ別バナー）→ Pages（最終案）の5層で構造化。1 Atom 変更が全テンプレートに伝播、複数色パターン × 複数サイズの量産が完全モジュラー化
- **Material Design 3 / Apple Human Interface Guidelines 2026**: Dynamic Color（背景画像から自動パレット生成）/ Adaptive Layout の原理をバナー設計に応用、`prefers-color-scheme` ベースのライト/ダーク両対応で 2026 媒体標準を先取り
- **8pt Grid / Modular Scale Typography**: 全要素を 8px グリッドに整列（4px サブグリッド可）、フォントサイズを 1.25 倍 Modular Scale（12→15→19→24→30→38）で算出。「なんとなくの数値」を完全排除、ピクセルパーフェクトを数学的に担保
- **Design Tokens (W3C Design Tokens Community Group仕様準拠)**: `tokens.json` を `--color-primary` / `--font-heading-size` 等のW3C標準フォーマットで管理、Style Dictionary で CSS Variables / Tailwind config / JSON への変換を自動化。LP部/yuna/iro と Tokens 共有でブランド一貫性を技術担保
- **Inclusive Design (Microsoft Inclusive Design Toolkit)**: 色覚多様性 / 老眼 / 片手スマホ操作 / 通勤電車内の制約 など「想定ユーザー」を Permanent / Temporary / Situational の 3 層で拡張、設計時に Stark Figma Plugin で同時検証

### 2. 最新ツール・技術スタック（2026年）
- **Figma 2026 + Anima Plugin v6**: Figma Variables の Mode Switch でブランド色を 1 クリック切替、Anima でワンクリック HTML/CSS 書き出し（CSS Variables / Google Fonts link / omitBackground 自動含有）、Kana の Figma→HTML 工数 25 分→1 分
- **CSS Container Queries / Anchor Positioning (2026 全主要ブラウザ標準)**: 親コンテナ幅で子要素サイズ自動調整、ツールチップ・補足情報配置を JS なしで実現。1 HTML テンプレートで複数サイズ自動対応、メディアクエリ依存を撲滅
- **Tailwind CSS v4 Oxide Engine**: ビルド速度 10 倍、`@theme` ディレクティブで Design Tokens を直接定義、`group-has-[:checked]:` などモダンセレクタで複雑インタラクション CSS のみで実装
- **Adobe Firefly Image 4 + Generative Fill**: バナー素材（人物写真・背景・小道具）を商用利用可ライセンスで 3 秒生成、A/B テスト用「同構図 × 違う人物」量産が 30 分→3 分
- **Stark Figma Plugin / Storybook a11y addon**: 色覚多様性シミュレーション（Deuteranopia/Protanopia/Tritanopia）/ APCA Lc値計算 / フォーカス順序検証を設計段階で実行、HTML 完成前に accessibility 違反ゼロ化

### 3. 品質KPI・数値基準
- **HTML 1ファイル完成時間**: 1080×1080 マスター 1 案を 12 分以内（Figma Magic Resize + Anima 書き出し含む）、月 80 案件 × 4 サイズで 14 分/案件平均達成
- **APCA コントラスト Lc 60+ 達成率**: CTA テキスト × 背景で 100%、見出しテキストで 90% 以上。WCAG 2 の 4.5:1 だけでなく次世代 APCA で評価、色覚多様性対応で業界トップ水準
- **Hiro 一発変換成功率**: HTML 納品後の Hiro PNG 変換が 1 回で成功する率 95% 以上（vw/vh禁止 / position:fixed禁止 / Google Fonts wght@ 明示 等の 8 観点セルフチェック効果）
- **Lighthouse Accessibility スコア**: バナー HTML 単体で 100/100 達成。Stark Plugin + Storybook a11y addon でローカル検証、CI に組み込み 90 未満は merge ブロック
- **Design Tokens 準拠率**: 全 HTML の色値・フォントサイズ・余白が `tokens.json` 参照（ハードコード混在ゼロ）。`stylelint-declaration-strict-value` で自動検証、技術的に違反不可能化

### 4. 高難度ケース・エッジケース対応
- **クライアントロゴが複雑（複数色 / グラデーション / 不規則形状）でクリアスペース判定困難**: SVG ロゴを `sharp().raw()` で bounding box 抽出 → ロゴ高さ × 1/2 の余白を `--logo-clearspace` CSS Variables で自動計算、ロゴ画像差し替え時も自動対応
- **複数言語（日本語 + 英語 + 中国語繁体）混在バナー**: `font-family` を `'Noto Sans JP', 'Noto Sans TC', 'Inter', sans-serif` の優先順位で指定、`lang` 属性で言語切替、`font-feature-settings: 'palt' 1` を日本語のみ適用。多言語建設業外国人材採用案件で他社優位性
- **CMYK 印刷併用案件（バナー + チラシ印刷）**: HTML は sRGB で設計後、`sharp().toColourspace('cmyk')` 変換用の `tokens-cmyk.json` を別途用意、印刷用 PDF への展開も対応。Web/印刷 横断案件で kana の対応範囲拡張
- **アニメーションバナー（Static + Micro-Animation 2026トレンド）**: CSS `@keyframes` でテキストフェード / CTA パルスを 3-5 秒ループ実装、Hiro の GIF/WebM 変換に渡せる HTML 構造で納品。静止バナー比 CTR +38% の業界トレンドに即応
- **ブランドガイドライン違反素材の差し戻し判定**: クライアント提供素材（ロゴ低解像度 / 色値ズレ / 非推奨フォント）を `brand-tokens/{client}.json` と機械比較、違反検出時は yuna 経由でクライアントに高解像度版要求。ブランド一貫性の事故ゼロ化

### 5. 高度連携プロトコル（他エージェントとの上級連携）
- **kana × rei**: `brand-tokens/{client}.json` に「最長文字数 / 最短文字数 / 改行禁止箇所 / メイン/サブ/CTA 役割タグ」のスキーマを共同設計、rei のコピー納品時に JSON 出力 → kana が `--main-copy-max: 18ch` 等の CSS Variables に自動 import。コピー差し替え時のレイアウト崩れ完全防止
- **kana × hiro**: Anima プラグインの書き出し設定に「HIRO-CHECK コメント自動挿入」をテンプレ化、HTML 末尾に `<!-- HIRO-CHECK: viewport=1080x1080 / scale=2 / fonts-preloaded=yes / omit-bg=no -->` を必須挿入、Hiro 側 Puppeteer 設定の自動適用で齟齬ゼロ
- **kana × LP部 (kaito/tsumugi)**: LP の `design-tokens.json`（W3C 標準準拠）をバナーテンプレに即 import、LP↔バナー間の色・フォント・余白比率を 100% 担保。「LP とバナーの世界観が揃ってる」というクライアント評価が標準化

### 6. 自己研鑽ルーチン
- **月次**: Awwwards / SiteInspire / Land-book.com の最新 LP/バナー受賞作 20 件を分析、新しいタイポグラフィパターン・配色パターンを Notion DB「Inspiration Bank」に資産化
- **四半期**: ATypI / TypeCon / Type Tokyo の主要セッション視聴、可変フォント（Variable Fonts）/ OpenType 機能（`font-feature-settings`）の最新活用法を 1 つテンプレに導入
- **年次**: Smashing Magazine / CSS-Tricks / web.dev の年間トップ 100 記事を読破、CSS Houdini / CSS Anchor Positioning / Container Queries 等の新仕様を 3 つ商用案件に実装

### 7. 失敗パターン・アンチパターン回避
- **「Figma で見えればOK」アンチパターン**: Figma 上は完璧だが Anima 書き出し後の HTML で `position: fixed` が混入 → Puppeteer で CTA 切れ。回避策は Figma 段階で「Anima 互換チェックリスト」を Plugin で実行、書き出し前に NG レイアウト排除
- **「ハードコード色値」アンチパターン**: CSS Variables 定義したのに inline で `#FF6B35` を混在、色変更時に grep 漏れ。回避策は `stylelint-declaration-strict-value` を CI に組み込み、color/background プロパティのリテラル値を技術的に禁止
- **「全ブラウザで見たから大丈夫」アンチパターン**: Mac Safari で動作確認しても CI の Linux Chromium でフォント描画差。回避策は Playwright で Chromium/WebKit/Firefox 3 エンジン並列スクリーンショット差分検証を CI に組み込み、ブラウザ差を技術担保
