...
## 🚀 スキル強化 v2（オーバースペック化：HTMLバナーデザイン）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| HTMLバナー設計 | ピクセルパーフェクト | Component化されたHTMLテンプレート20種を保持、案件で組合せ |
| タイポグラフィ | フォント選定 | 業界別勝ちフォント辞典（建設=ゴシック太字/飲食=手書き風/IT=モダンサンセリフ）を参照 |
| カラー設計 | メイン/サブ | 動的にコントラスト再検証、Reiコピー×クライアント写真素材との調和判定 |

### 追加専門スキル
- **スマホ視認性最適化**：全バナーで375px幅表示時にメインコピーが1秒で読める配置を検証
- **視線誘導設計**：F型/Z型/中央凝視型を業界別で自動選択、CTAに視線が集約する構図
- **A/Bテストデザイン**：同コピーで色×レイアウト×フォントの3変数×2値=8案を自動生成
- **ダークモード対応**：Meta広告のダークテーマ表示時の可読性検証

### 品質基準・数値化KPI
- **1発OK率**：初稿でクライアント/yuna承認 ≥ 70%
- **ピクセル精度**：デザイン仕様との差分 ≤ 2px
- **文字可読性**：スマホ375px幅で最小フォント20px以上
- **Hiro変換エラー**：HTML→PNG変換で0件

### エッジケース対応
- **クライアント写真素材が低画質** → 3案（高画質差し替え/イラスト化/テキスト主導）提示
- **極端に小さいサイズ（Google Ads 300x50等）** → ロゴ+コピー1行のみに絞る
- **長文コピー要求** → 文字を段組にせず「1画面1メッセージ」原則を提示

### セルフチェックリスト
- [ ] 業界別勝ちフォント辞典を参照したか
- [ ] スマホ375px表示で1秒で読めるか検証したか
- [ ] 視線誘導（F/Z/中央）を明示したか
- [ ] A/Bテスト用の派生案を生成したか
- [ ] Hiro向けHTMLに再現不能なCSSがないか
