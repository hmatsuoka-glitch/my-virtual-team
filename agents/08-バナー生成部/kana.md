...
### 2026-07-12
- **「CSS Cascade Layers による多クライアント共通テンプレの優先度制御」**：`@layer base, brand, override;` の 3 層構造で「共通ベース → クライアント別ブランド → 案件固有オーバーライド」を宣言し、!important やセレクタ詳細度によるスタイル打ち消し合戦を排除。20 クライアント × 4 サイズを 1 テンプレで管理する際の CSS 衝突デバッグ時間が 25 分→3 分、案件横断の「他クライアントの色が漏れて出る」事故が月 2 件→0 件。
- **「CSS Houdini Paint API で GPU 描画のノイズ・多段グラデ実装」**：`registerPaint()` でカスタムペイントワークレットを登録し、`background: paint(brand-gradient)` の 1 行で多段グラデ＋フィルムグレインノイズを GPU 合成。従来 SVG `feTurbulence` を重ねる 8 行実装が 1 行化、Retina PNG でのバンディング目視劣化チェックが 4 分→30 秒、Hiro からの「縞模様出てる」差し戻しが月 3 件→0 件。
- **「Sora 7 点 QA の事前パスチェッカーを HTML 末尾コメントに埋込」**：Sora が最終 QA でチェックする 7 項目（サイズ整合・コントラスト 5:1・視線誘導・ヒエラルキー・ブランド遵守・差別化・ファイルサイズ）を `<!-- SORA-PRECHECK: {項目}: pass/fail/reason -->` の形式で自己申告記録。Sora が実測値と申告を突合するだけで済み、QA 所要時間 12 分→4 分、Kana への差し戻し率 30%→3%。
- **「A/B テスト前提のバリアント量産を data-variant 分岐で 1 HTML 化」**：`<body data-variant="A">`〜`"C"` の attribute selector 切替で色・コピー配置・CTA 形状の 3 パターンを 1 HTML に集約、Bulk Rendering で 3 案 × 4 サイズ = 12 PNG を 1 コマンドで書き出し。akari の A/B 効果測定に即投入可能、バリアント量産工数 90 分→10 分、改善サイクルが月次→週次に加速して平均 CTR 改善率 12%→28%。
- **「Claude Vision マルチモーダル入力で参考画像→CSS Variables 自動抽出」**：クライアント指定の参考バナー画像を Vision API に投入し「配色 HEX・タイポ階層・CTA 位置・ジャンプ率」を JSON 抽出、テンプレの CSS Variables に自動流し込み。「参考画像を渡されたが構成分析に時間かかる」の初動 20 分→2 分、初稿の「参考忠実度」が体感 60%→95%、Rei のコピー役割タグ抽出も同一プロンプトで自動化して STEP 1〜2 の判断負荷を機械側にオフロード。
- **「OKLCH 色空間（2026 全主要ブラウザ対応）でブランドカラーの知覚均等化」**：`oklch(70% 0.15 30)` 記法で L（明度）・C（彩度）・H（色相）を分離管理し、`oklch(from var(--primary) l c calc(h + 180))` で補色・トーン違い派生色を数式生成。HSL 記法での「明度は同じなのに色によって暗く見える」問題が解消、A/B 用色バリ 5 案生成が 30 分→3 分、コントラスト比 5:1 の保持も oklch 計算で機械的担保しダーク版 CSS Variables 生成も自動化。
- **「Playwright + Percy でビジュアルリグレッション CI 自動化」**：Kana の共通テンプレ更新時に Playwright が全クライアント × 全サイズ × 全バリアントを自動レンダリング、Percy が pixel diff で「意図しない変更」を検出。テンプレ CSS 更新時の手動確認 45 分が CI 自動実行 90 秒に、Hiro への PNG 変換前に破綻を検知可能で本番事故ゼロ化、月 20 案件のリグレッション事故が 3-4 件→0 件で Sora QA の差し戻しも激減。
