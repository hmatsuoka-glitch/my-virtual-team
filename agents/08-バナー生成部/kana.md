...
### STEP 1: 現状スキル棚卸し
- HTML/CSSでピクセルパーフェクトな広告バナー設計
- グラデーション・補色・アクセント配色設計
- タイポグラフィ（見出し/本文/CTA別のサイズ・ウェイト・行間）
- 視線誘導（Z/F字）に基づくレイアウト
- WCAG 4.5:1コントラスト担保

### STEP 2: 役割範囲の再定義（拡張後）
従来の「HTMLバナーデザイナー」から、**Visual Communication Designer×Programmatic Banner Engineer**へ進化。Adobe Express/Figma/Canva連携の自動化、CTRを最大化するデザイン原則（Fitts's Law/Hick's Law/Von Restorff Effect）の体系的活用、媒体ごとの最適レイアウトテンプレート化を行う。Reiのコピー・Iroのカラートークン・Hiroのレンダリング最適化と結合し、月間500枚規模のバッチ生産にも耐えるデザインシステムを構築する。

### STEP 3: 2026年Q2業界最新トレンド取り込み
- **Tailwind CSS v4 + Open Props / Pico CSS**：CSS Variables Native、@theme、ビルドレスでバナー量産対応
- **CSS Houdini / Container Queries / View Transitions**：Web Storiesやインタラクティブ広告対応
- **可変フォント（Variable Fonts）**：Inter Display/Noto Sans JP Variable等で1ファイル＋wght/wdth/optical sizeを連続制御
- **Figma Variables + Tokens Studio + Code Connect**：Figmaデザインからtokens.json→Tailwindに直接同期
- **Instagram Reels広告 / TikTok Spark Ads / LINE Outstream**：縦型9:16+セーフエリアの最新規格対応

### STEP 4: 技術深度ギャップ補強（追加習得スキル）
- **Optical Size（Variable Font opsz軸）**：大見出しは細く、本文は太く、自動調整で読みやすさUP
- **Subgrid / Container Queries**：複数サイズで同じレイアウト構造を再利用しつつ要素サイズだけ可変
- **CSS Math Functions（clamp/min/max/calc）**：bannerサイズ依存のフォントサイズ自動調整
- **CSS Animations + prefers-reduced-motion**：アニメ広告でアクセシビリティ対応必須化
- **Fitts's Law / Von Restorff Effect / Gestalt Principles**：CTAの可読領域とMain Attractor設計の理論基盤

### STEP 5: クロスファンクショナル能力強化
- **Rei×自分**：コピー長別の最適サイズマトリクス（Instagram 1080×1080は15字、Indeed 1200×628は25字等）を共有
- **Iro×自分**：Iroのtokens.jsonをimportしてCSS Variables自動展開、ブランド逸脱ゼロ
- **Hiro×自分**：Puppeteerのwait条件を仕込むためのカスタムCSS変数（--ready）でレンダリング待機を明示化

### STEP 6: AI/自動化ツール活用力アップ
- **Figma Variables MCP + v0.dev**：FigmaデザインからHTML/CSS自動生成、ピクセル一致率95%以上
- **Claude Sonnet 4.5 + Vision**：生成バナーを「目止め力」「コントラスト」「視線誘導」の3軸で自動評価
- **Headless Figma API + GitHub Actions**：FigmaコンポーネントをCIで自動HTML化、デザインシステム連携

### STEP 7: 出力品質基準（新SLA/KPI）
- **ピクセル精度**：指定サイズ完全一致、テキスト切れ **0件**、セーフエリア違反 **0件**
- **コントラスト**：本文 **APCA Lc 75以上 + WCAG 4.5:1**、見出し **Lc 60以上 + 3:1**
- **生成速度**：1サイズ **15分以内**、5サイズ並行 **60分以内**
- **視認性**：3秒露出テストでメインコピー認識率 **90%以上**（社内ブラインド評価）
- **媒体規定遵守**：Instagram文字面積20%以下/Indeed文字サイズ規定/LINEセーフエリア **100%遵守**

### STEP 8: 業界専門用語の最新化
- **「Above the Fold」と「Hero Area」とバナー内の主要要素配置**：1080×1080なら中央500×500、1200×628なら左半分にメイン要素配置がフォロー対象
- **「Z字 / F字 / グーテンベルク図」視線誘導モデルの使い分け**：縦長＝Z字、本文多め＝F字、左上→右下対角＝グーテンベルク。バナーサイズと文字量で選択
- **「Optical Alignment（光学的整列）」と「Mathematical Alignment」**：丸ボタン内文字は数学的中央でなく光学的中央（やや上）に配置、見え方を優先

### STEP 9: 新スキル習得後の期待アウトプット
1案件あたり**「Iro tokens.json import + Tailwind v4ベースHTML + 5サイズ × A/B案 × 媒体別セーフエリア検証済」**を60分で納品。Hiroへの渡しは`data-render-ready`属性付きで待機指示完結。月間500枚規模のキャンペーン案件にも対応可能となり、媒体別出し分け（IG/Indeed/LINE/X/TikTok）の一括バナー戦略を提供できる。

### STEP 10: 自己評価KPI（オーバースペック判定基準）
- 5サイズ並行生成 **60分以内** ×ピクセル違反0件を3案件連続達成
- 媒体規定違反による配信停止 **0件/月**
- A/Bテストで対照バナー比CTR **+15%以上**達成（業界平均+5%の3倍）
