...
### 2026-09-04
- **CSS Subgrid で条件3点（給与・職種・勤務地）のラベル列・値列を親グリッドから継承し、頭揃えを構造的に固定する**：これまで条件3点は各行 flex で揃えていたため、行ごとの内容幅でラベル頭が数pxずれ、Yuna 提示時の「なんとなく整っていない」印象の主因になっていた。親を `grid-template-columns: auto 1fr` で組み、各条件行を `display: grid; grid-template-columns: subgrid; grid-column: span 2` にすれば、ラベル・値の縦線が全行で一直線に揃う。Baseline 済みで主要ブラウザに広がったのを受け、`@layer layout` の条件3点テンプレ骨格へ標準化する（理由：見た目の揃いは各行の `align-items` でなく、共通の縦グリッド線を親子で共有して初めて構造的に成立する。flex の目視合わせは 35% 縮小時に必ずズレる）
- **Meta のフィード配信推奨が 1080×1350（4:5）と 1080×1920（9:16）併用へ寄ったのを受け、4:5 マスター起点＋9:16 派生の二枚1組を媒体プリセット `data-media="ig-feed-4x5"` `ig-reels-9x16` として追加**：1080×1080 正方形のみでフィードに出すと占有面積が縮み、Reels 面ではブラックバーが両脇に入って「Reels 用に作っていない」バナーとして配信効率が落ちる。9:16 派生は中央 60% セーフエリアに条件3点を収め、上部 20%（プロフィール・キャプション被り）・下部 20%（音源／シェアUI被り）を主訴求から外す。Yuna から Meta 案件を受けた時点で「4:5 と 9:16 の二枚セットで良いか」を先に返し、9:16 だけ後追いで作り直す事故を入口で消す（媒体プリセット二層構成は 2026-09-01 参照）
- **LINE 広告（LINE VOOM／LINE NEWS 面）の 3:2＝1200×800 は Meta の 1.91:1＝1200×628 と別プリセットで持ち、機械縮小での流用を禁止する**：縦 172px 差は 3 行構成の条件3点で 1 行分に相当し、1.91:1 テンプレを 3:2 に流用すると下部が余って「素材を伸ばした」印象に、逆に 3:2 マスターを 1.91:1 に落とすと条件3点の 3 行目が押し込まれる。`@layer variants` に `data-media="line-3x2"` を独立で持ち、面積配分を LINE 面専用に最適化する。Yuna の用途確認シートに「LINE 面あり／なし」を必ず含めてもらう連携をラフ着手前に固定し、Rei からもらう条件3点（2026-08-27 参照）を 3:2 の余白前提でグリッド選定する
- **WCAG 2.2 の 2.5.8（ターゲットサイズ 最小 24×24 CSS px）はバナー静止画内では成立させず、押下領域は kaito チームの LP CTA に責任分界して nori 事前チェックで明示する**：バナーは配信面上「リンク付き画像」として扱われ、CTA ボタンの面自体は媒体側のクリック領域に吸収される。それを 24×24 で満たそうとすると小サイズ展開でボタン装飾が肥大化し、実表示 11px 以上を確保すべき条件3点（2026-08-16 参照）の面積を逆に奪う。バナー側の CTA 表現は「視認・押したくなる訴求」まで、実タップ領域は LP 側で 44×44 相当を担保、と 2.5.8 の適用境界を nori チェックシートに書き分け、制作前関所で「バナーは 2.5.8 対象外・LP は対象」の合意を取ってから着手する（理由：静止画バナーの内部 UI に押下判定はなく、押せるかどうかは配信面と遷移先で決まる）
- **可変フォントを `wght` 軸を残したまま日本語サブセット化し、全ウェイトを 1 ファイルで賄って preload を単一化する**：現行はウェイト別 woff2 を個別 preload しているが（2026-09-01 参照）、`pyftsubset` で `gvar`／`fvar` 等の可変軸テーブルを維持したまま日本語基本＋案件固有名詞（2026-09-02 参照）だけに絞れば、1 ファイルで `font-weight: 100 900` の連続指定が効く。`@font-face { font-weight: 100 900 }` 宣言と `<link rel="preload">` 1 本に集約でき、Hiro の `fonts.ready` 待ちも 1 回で済む。Rei の見出し・本文ジャンプ率変更が同一ファイル内で完結し、ウェイト未列挙による黙ったフォールバック（2026-08-05 参照）とも二重に無縁化する。異体字ローカルフォールバック無効化のルール（2026-09-02 参照）はそのまま維持し、可変ファイル側で不足文字を露見させる
