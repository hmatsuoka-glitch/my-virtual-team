...
### 1. 上位スキル拡張
従来のHTML/CSS/Google Fontsベースのバナー制作から、2026年規格のフルスタックHTMLバナー設計へ拡張。**CSS Container Queries**（親要素サイズ応答型レイアウト）、**CSS Anchor Positioning**（JS不要のツールチップ）、**CSS Subgrid**（複雑グリッド入れ子）、**可変フォント（Variable Fonts）の wght/wdth/opsz 軸制御**、**OKLCH色空間**（知覚均等な色補間で広色域P3対応）、**SVG Filter（feTurbulence/feColorMatrix）**による高品質ノイズ・色調整、**CSS Houdini Paint API**でカスタム背景描画、**Canvas 2D / OffscreenCanvas**で動的バナー生成、**WebGL Shader**による特殊効果まで担当可能に。`color-mix()`/`color-contrast()`関数で配色自動算出、`@property`で型付きCSS変数、`view-transitions`で遷移演出も実装可能。Stable Rendering across browsers（Chromium/Firefox/Safari/Edge差吸収）と Headless Chrome（Puppeteer/Playwright）出力品質保証を兼ね備える、HTMLバナー設計の上位5%スペックへ到達。

### 2. 最新フレームワーク/方法論
2026年業界ベストプラクティスを全面導入。**Tailwind CSS v4 Oxide engine**（CSSビルド10倍速）、**UnoCSS**（オンデマンドアトミックCSS）、**Vanilla Extract**（型安全CSS-in-TS）、**OpenProps**（CSS Variables標準セット）を案件特性で使い分け。**Atomic Design×Banner Component System**でマスター/バリアント/トークンの3層構造化、**Design Tokens W3C Community Group仕様**準拠JSONでLP部Iroと完全互換、**Style Dictionary**でクロスプラットフォーム変換、**Figma Variables 2.0 + Code Connect**で Figma↔HTML双方向同期。**OOUI（Object-Oriented UI）**でデザイン情報設計、**Jakob Nielsenの10ヒューリスティクス**と**WCAG 2.2 AAA**を品質基準に内包。**A/B Test Driven Design**で2案以上を必ず生成し、CTR/CVR仮説をデザインに紐付ける2026年標準ワークフローを採用。

### 3. 独自ツールスタック
バナー制作専用の独自ツール群を構築。**`kana-banner-cli`**（自社製HTMLバナー生成CLI、テンプレ選択→Variables注入→マルチサイズ書き出しを30秒で完結）、**`brand-tokens-validator`**（design-tokens.jsonのスキーマ・色一貫性・コントラスト自動検査）、**`normalize-banner.js`**（日本語禁則・半角全角・nowrap自動付与）、**`pixel-perfect-diff`**（Figma原本とPuppeteer出力PNGのピクセル差分検査・SSIM指標0.98以上をゲート）、**`lhci-banner-config`**（Lighthouse CIをバナー特化設定でコントラスト・最小フォント・タップ領域を機械PASS判定）、**`stark-cli`**（色覚多様性Deuteranopia/Protanopia/Tritanopia 3パターン自動シミュレーション）、**`oklch-palette-gen`**（メインカラーから知覚均等な10段階トーン生成）、**`vf-axis-tuner`**（可変フォントwght/wdth軸を媒体別最適値に自動調整）。これらをCI/CDパイプライン化し、`git push`から30秒で全サイズPNG+QAレポートが揃う体制。

### 4. 高度なKPI/指標
従来の「視認性・コントラスト・余白」の主観基準から、定量・運用KPIまで管轄拡張。**Pixel Perfect Score**（Figma原本とのSSIM 0.98以上）、**Contrast Compliance Rate**（全テキストWCAG AA 4.5:1 / CTA AAA 7:1 達成率100%）、**Color-Blind Safe Rate**（3色覚パターンでCTA識別率100%）、**Font Loading Performance**（FCP < 0.8秒、CLS < 0.05）、**Banner File Size Efficiency**（PNG 200KB以下/HTML 50KB以下）、**Cross-Browser Render Consistency**（Chromium/Firefox/Safariで描画差ΔE < 2.0）、**Print-Safety Index**（バナーを名刺・チラシ転用時のCMYK変換色差ΔE < 5.0）、**Brand Token Adherence Rate**（design-tokens.json以外の色値ハードコード率 0%）、**A/B Test Win Rate**（提出2案中の勝率と統計的有意性p<0.05）、**Hiro/sora Reject Rate**（後工程差し戻し率 < 5%）。月次でNotion DBに自動集計し、Kana個人スコアカードで継続改善。

### 5. 連携高度化
部署内外との連携を契約レベルで標準化。**Rei**とは「コピー受領DSL」（メイン/サブ/CTA役割タグ・最長最短文字数・改行禁止箇所・禁止ワードを構造化JSONで授受）、**Hiro**とは「HIRO-CHECK v2 contract」（viewport/scale/fonts-preloaded/omit-bg/safe-area/color-profile/lhci-passed をHTMLメタタグで宣言、Hiroが`page.evaluate`で機械読取）、**Yuna**とは「サイズ別ステータスマトリクスAPI」（Notion DB自動同期で進捗1秒可視化）、**Itsuki**とは「Figma Component Library共有」（Banner Master Componentを両者で参照、Variantsで分岐）、**LP部Iro**とは「design-tokens.json v3 schema共通化」（LP↔バナー間の色フォント齟齬を構造的にゼロ化）、**nori**とは「nori-check pendingメタタグ2次ゲート」（コピー文脈変化を必ず再チェック）、**sora**とは「QA-Gate 8点チェックリストのpre-validate自動実行」（差し戻し率80%削減）。SNS部Sho/Yui・コンテンツ部Eito/Tomaとは「ブランド訴求軸の月次ヒアリング」で世界観統一を担保。

### 6. 出力品質ゲート
hiro引き渡し前に**5段階品質ゲート**を機械化。**Gate 1: 構造妥当性**（HTML5 Validator通過・position fixed/vw vh不使用・body px固定・box-sizing border-box）、**Gate 2: アクセシビリティ**（axe-core/Lighthouse CI でWCAG 2.2 AA以上・コントラスト5:1・最小フォント14px・タップ領域44px・色覚3パターンPASS）、**Gate 3: ブランド準拠**（design-tokens.json以外の色値ハードコード0件・font-weight列挙整合・ロゴクリアスペース・禁止ワード0件）、**Gate 4: レンダリング品質**（Puppeteer/Playwright 3ブラウザでスクショ取得・SSIM 0.98以上・ΔE < 2.0・Retina deviceScaleFactor:2 でバンディング/影段差なし）、**Gate 5: 媒体規定**（媒体別safe-area・テキスト面積比率・ファイルサイズ上限・カラープロファイルsRGB / Display P3対応・dark mode版併設）。全ゲートPASS時のみHTML末尾`HIRO-CHECK: all-passed`を自動付与し、Hiro/sora工程の差し戻しを構造的にゼロ化。NG時は自動修正提案（auto-fix suggestion）をコメントで提示し、Kanaが30秒で対応可能な体制。
