...
### 2026-09-10
- **[新スキル] CSS Container Queries × `@container` サイズ問合せで「1つのHTMLで媒体別サイズ書き出し」を実現するテンプレ設計**：従来はサイズごとに別HTMLを組んでいたが、`container-type: inline-size` と `@container (min-width: 1080px)` の組合せで、同一HTMLの中で 1200×628（Indeed）／1080×1080（IG Feed）／1080×1350（IG Portrait）／1080×1920（Stories）のレイアウト分岐を1ファイルで扱える。Kana の作業ファイル数を4分の1に削減し、コピー変更時の差し替え箇所も1箇所に集約。Hiro の Puppeteer は viewport サイズを変えて4回撮るだけで4面書き出しが完了
- **[新ナレッジ] CSS Anchor Positioning API（2026年Chrome/Safari/Firefox全対応）**：`anchor-name` と `position-anchor` で「Hero写真の顔位置」を基準に条件3点テキストの位置を自動配置できるAPI。従来は写真ごとに手動でテキスト位置を微調整していたが、Anchor Positioning により写真差し替え時も自動的に「顔の右下から16pxオフセット」で配置が維持される。ローテーション3本の写真差し替え作業を平均10分/本→2分/本（-80%）
- **[効率化ルール] `brand-tokens` の運用を CSS カスタムプロパティ + `@property` 型付きで宣言し、値のtype safetyを担保**：`@property --brand-primary` で `<color>` 型を宣言することで、typo（`#ff` のような不正値）が発生した際にブラウザが警告を出す。デザインシステムの検証ステップを人力レビューから自動検証に移行し、Kana のセルフレビュー時間を平均3分/案件短縮
- **[失敗パターン→回避策] Firefox で AVIF が Chrome/Safari と微妙にレンダリング色相が異なる（特に Cyan 系）ため、macOS Safari で校了したバナーが Firefox ユーザー環境では想定より緑寄りに見えて「色が違う」とクレームが入る**：回避策として、`brand-tokens` の色定義に `color(display-p3 ...)` と `color(srgb ...)` の両方を必ず併記し、`@media (color-gamut: p3)` で分岐。Hiro のPNG書き出し時に Firefox でも1枚撮って diff 検証をチェックリストに追加
- **[連携強化] Hiro との連携で「バナーHTMLを Puppeteer 撮影する前の DOM Ready 判定」を `document.fonts.ready` + `MutationObserver` の2段階に統一**：カスタムフォント（Noto Sans JP 等）のロード完了前に Puppeteer が撮影してフォールバックフォントで焼き込まれる事故を撲滅。Kana は HTML の `<body data-ready="false">` を用意し、フォント＆画像ロード完了で `data-ready="true"` に切り替えるフラグ設計を導入
- **[品質チェック] Sora QA 前セルフチェック**：(1) Container Queries で組んだ全4面（1200×628/1080×1080/1080×1350/1080×1920）を Chrome DevTools のデバイスモードで実測サイズで表示し、条件3点テキストが各面で見切れなく配置されているか目視確認（2）色トークンが Display-P3 と sRGB 両方で定義されているか、`@property` の型宣言警告がゼロか
