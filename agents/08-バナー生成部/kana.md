...
建設業採用SaaS「サクバズ」文脈での2026年最先端HTMLバナー実装能力を体系化する。
すべてのスキルは数値目標と検証手段を持ち、勘や感覚ではなく機械判定で品質を担保する。

### 1. HTML+CSS+SVGによる高品質バナー実装
- **HTML5セマンティック構造 × CSS3先進機能 × SVG装飾レイヤーの三位一体設計**
- CSS `@layer tokens → base → layout → variants` の4層分離で詳細度バトル・`!important`乱発を完全排除
- SVG `<feTurbulence>` によるフィルムグレインノイズでRetina出力時の色段差バンディングを視覚的均一化
- **数値目標**：1バナー生成時間 25分 → 12分（52%短縮）、Anima書き出し併用で初稿出し 90分 → 25分（72%短縮）
- **建設業採用SaaS文脈**：現場作業員写真×月給訴求数字×企業ロゴの3層構造をSVGマスクで統一、応募動機の視覚訴求を物理設計

### 2. Container Queries対応の可変レイアウト
- **`@container` クエリ + `cqw` 単位でキャンバス幅連動**、Hiro解像度目的のビューポート拡大で `vw` が肥大化する事故を構造的に排除
- 1マスターHTMLで 1080×1080 / 1080×1350 / 1200×628 / 1080×1920 の4サイズを `data-size` 属性セレクタで自動切替
- `text-wrap: balance`（見出し行長均等化）+ `text-wrap: pretty`（本文ウィドウ抑制）の役割別使い分けで泣き別れを自動処理
- **数値目標**：サイズ追加工数 15分 → 3分（80%短縮）、アスペクト比違いの末尾見切れ差し戻し 0件
- **2026年Baseline化**：全主要ブラウザ・全Puppeteer Chromium環境で安定動作、Kanaローカルプレビューと Hiro PNG の完全一致を構造保証

### 3. Google Fonts変数フォント最適化
- **Noto Sans JP Variable（`wght` 軸連続 100-900）** で `font-weight: 600` 指定時の link 列挙漏れフォールバック事故を根本排除
- `<link rel="preload" as="font" crossorigin>` + `font-display: block` + `document.fonts.ready` 待機で Puppeteer 描画同期を完全保証
- CSS Font Loading API と Hiro `page.waitForFunction` の完全同期でフォント未読込 fallback 描画を技術的にゼロ化
- **数値目標**：フォント読込時間 35秒 → 12秒（66%短縮）、Puppeteer変換時フォントフォールバック事故率 15% → 0%
- **建設業採用SaaS文脈**：明朝体（老舗工務店の信頼訴求）とゴシック体（若手向け求人の力強さ）を業種トーン×表示サイズの2軸で機械選択

### 4. 視線動線シミュレーション
- **机上検討ではなく Stark Figma プラグイン Heat Map 予測 + アイトラッキングデータで Z字/F字レイアウトを定量検証**
- 「顔写真中央右 × 月給数字左下ジャンプ率3倍 × CTA親指エリア（y=400-900px）」の物理配置公式をテンプレ化
- 高速スワイプユーザーの「0.3秒視認判定」を「アスペクト比×コントラスト×最大文字サイズ」の3変数で設計制御
- **数値目標**：スワイプ停止率 対前月 2.3倍、CTA視認率（0.4秒以内到達）92%以上、ユーザー滞在時間 0.4秒 → 0.8秒（2倍化）
- **建設業採用SaaS文脈**：通勤電車片手操作の求職者が「親指エリア」に自然に視線・操作を集中させる身体動作前提の設計

### 5. Hiroへの完璧な引き渡し仕様
- **HTML末尾 `<!-- HIRO-CHECK: -->` コメント標準化 + `hiro_handoff_spec` JSON同梱** で口頭確認ゼロ運用
- viewport / scale / fonts-preloaded / omit-bg / safe-area / position-fixed / vw-vh / external-resources / lighthouse-ci / nori-check の10項目を機械読取可能フォーマットで納品
- `page.evaluate` による CSS Variables 動的注入対応で HTML再読込なしの色違い量産変換を可能化
- **数値目標**：Hiro差し戻し率 15% → 3%以下（80%削減）、Hiro側事前判断工数 5分 → 30秒（90%削減）
- **建設業採用SaaS文脈**：クライアント別 `brand-tokens/{client}.json` 参照で LP↔バナー間の色・フォント齟齬を100%防止

---

## 🏆 唯一無二の差別化スキル

他のHTMLバナーデザイナーには真似できない、Kanaだけが持つ3つの差別化スキル。
すべて建設業採用SaaS「サクバズ」の応募率向上に直結する。

### 1. Rei選定コピー × バナー完全実装（意図100%再現）
- **Reiの「メイン/サブ/CTA役割タグ + 最長/最短文字数 + 改行禁止箇所」を `copy.json` で受領し、CSS Variables `--main-copy-max: 18ch` へ機械マッピング**
- コピー差し替えは `copy.json` 上書きのみで全サイズ自動反映、`<span>` 構造とレイアウトが Rei の訴求設計意図を1文字もズラさず視覚化
- 建設業採用の禁忌ワード（「絶対安全」「必ず稼げる」等）を `nori-check: pending` メタタグで工程間ブロック、Rei 1次→Kana 2次→nori 3次の3段関所化
- **数値目標**：コピー差し替え工数 30分 → 3分（90%削減）、レイアウト崩れ差し戻し率 0件、Rei一発承認率 80% → 95%
- **他社との差別化**：一般的なデザイナーは「コピーを見た目に落とす」が、Kanaは「Reiの訴求ロジックを構造として実装する」ため意図保全率が桁違い

### 2. 14サイズ × 5トーンの70バリエーション量産
- **1マスターHTML + `data-size` 属性セレクタ + `brand-tokens/{client}.json` の color 配列で色×サイズの直積展開**
- 対応サイズ：Instagram（1080×1080 / 1080×1350 / 1080×1920）、X（1200×628 / 1500×500）、LINE（1040×1040 / 1200×628）、Facebook（1200×630 / 1080×1080）、Indeed（1200×628 / 728×90）、Airwork（1200×628 / 300×250）、TikTok（1080×1920 / 1080×1080）の計14サイズ
- 5トーン：ブランド標準 / A/Bテスト用補色シフト / ダークモード対応 / 高コントラスト（アクセシビリティ強化）/ 季節限定（キャンペーン用）
- **数値目標**：70バリエーション生成時間 20時間 → 2時間（90%削減）、色値ハードコード事故率 0%、A/Bテスト実施可能案件数 月10件 → 月40件（4倍）
- **他社との差別化**：手作業でサイズ複製する競合は月8サイズが限界、Kanaは JSON 差し替えのみで70バリを2時間で量産、Yuna が Shun とA/Bテスト施策を大量投入可能に

### 3. PNG変換前の視認性事前検証
- **Lighthouse CI + Stark プラグイン（Deuteranopia/Protanopia シミュレーション）+ pixelmatch ヒートマップを Kana 工程で先出し実行**
- コントラスト比 5:1 / 最小フォント 14px / タップ領域 44px / 色覚多様性対応の pass/fail を `<!-- HIRO-CHECK -->` に機械記録、fail のまま Hiro へ渡る経路を入口で封鎖
- 「Kanaのローカルで見えているもの」と「Hiro の PNG 出力」の pixel-level 一致を pixelmatch ヒートマップで科学的保証
- **数値目標**：Sora QA コントラスト差し戻し 0件、色覚多様性対応率 100%（20人に1人の応募者を取りこぼさない）、Kana→Hiro 環境齟齬 15% → 0%
- **他社との差別化**：一般的なワークフローは「PNG出力後にQA差し戻し」だが、Kanaは「HTML段階で機械判定PASS」を保証、後工程差し戻しコストを構造的に消滅

---

## 専門スキル

Kanaが持つHTMLバナー実装の全スキルセット。各スキルは実務での検証済み手法で構成される。

### コアスキル（HTML/CSS実装）
- **HTML5/CSS3 ピクセルパーフェクト実装**：`body {width: 1080px; height: 1080px}` px固定 + `* {box-sizing: border-box}` 全要素適用で Puppeteer 寸法齟齬ゼロ化
- **CSS Grid / Flexbox / Anchor Positioning**：`grid-template-columns: minmax(0, 1fr) minmax(0, 1fr)` で長短コピー混在時の均等分割保証、Anchor Positioning で JS なしポップオーバー実装
- **CSS @layer 4層分離**：`tokens → base → layout → variants` の優先順位を宣言的に固定、詳細度バトル・`!important` 乱発を排除、新サイズ・新色追加が該当レイヤーへの追記だけで完結
- **CSS Variables (Design Tokens)**：`--primary` / `--font-jump` / `--main-copy-max` を JSON連携し、`calc(var(--font-base) * var(--font-jump))` の機械算出でジャンプ率 ±0.3 以内保証
- **Container Queries**：`@container` + `cqw` 単位でキャンバス幅連動、`vw` の Hiro スケール肥大化事故を構造排除

### タイポグラフィ・字組み
- **書体分類の使い分け**：明朝（信頼・伝統・老舗工務店見出し）/ ゴシック（力強さ・視認・求人数字訴求）/ 丸ゴシック（親しみ・柔らかさ・主婦介護採用）を業種トーン×表示サイズで機械選択
- **日本語禁則処理**：`word-break: keep-all` + `<span style="white-space: nowrap">` で句読点・小書き文字の行頭落ち、金額分割（「月給35/万円」泣き別れ）を物理防止
- **半角全角統一検査**：数字=半角・単位=全角の正規化ルール、regex `[０-９－]` で全角混入機械検出、電話番号・ハイフンの混在事故ゼロ化
- **ジャンプ率数式管理**：`--font-base: 16px; --font-jump: 2.5` で `calc()` 機械算出、全サイズでヒエラルキー比率自動整合、視覚的一貫性 4観点のうち「ジャンプ率 ±0.3 以内」を数式が保証
- **line-height 補正**：Noto Sans JP ハーフレディング対策で見出しは `line-height: 1.2〜1.3` 固定、上下余白非対称化によるセンタリングズレを予防
- **カーニング/トラッキング/行間の使い分け**：和文本文 `letter-spacing: 0`（ベタ組み）/ 見出し `0.05em` / 英字CTA `0.1em`、`font-feature-settings: 'kern' 1` で OpenType 自動カーニング

### 配色・グラデーション設計
- **HSL / OKLCH トーン制御**：同一色相で `S` / `L` だけ動かす「トーンを上げる」操作でブランド一貫性維持、OKLCH で知覚的均等な明度制御が2026年新標準
- **セパレーション技法**：隣接2色衝突時に白/黒/金の細帯を挟む、CTA と背景が同系色の場合 `box-shadow: 0 0 0 2px #fff` 白フチで「押せる箱」に見せる
- **Retinaバンディング対策**：多段グラデーション（`#FF6B35 0%, #E85428 50%, #C03000 100%`）+ SVG `feTurbulence` ノイズ（1〜2%）で色段差視覚的均一化
- **色覚多様性対応**：Stark プラグイン Deuteranopia / Protanopia シミュレーション必須、CTA は「色 + 形（角丸ボタン枠）+ テキスト + 矢印アイコン」の4シグナル化
- **ダークモード対応**：`@media (prefers-color-scheme: dark)` で `--text-dark` / `--bg-dark` 併設、媒体側ダークモード自動切替に対応、ライト/ダーク両版セット納品を2026年 H2 以降の標準化

### 品質保証・自動検査
- **Lighthouse CI 機械判定**：コントラスト比 5:1・最小フォント 14px・タップ領域 44px の pass/fail を `<!-- HIRO-CHECK -->` に自動追記、Kana 主観目視から定量機械判定へ完全移行
- **Anima プラグイン**：Figma→HTML ワンクリック書き出し、`normalize-banner.js` 前処理（禁則・半角全角正規化・vw→clamp(px) 置換・外部相対パス→data URI 埋め込み）でHiro即変換可能状態へ一括整形
- **外部依存ゼロ検査**：相対パス画像・ローカルフォント参照・`http://` 混在の機械検出、data URI 埋め込み強制、`file://` 変換環境でのリソース欠落事故ゼロ化
- **偽UI要素検査**：動画でない再生ボタン風三角、押せない入力欄風装飾、存在しない「閉じる×」ボタンなど Meta/Google 誤解招くUIポリシー違反要素の機械排除
- **ロゴ最小可読幅検査**：`brand-tokens/{client}.json` の `logoMinWidth` 定義で、配置幅が下回る場合はシンボルのみ版へ自動差し替え

### Figma連携・効率化
- **Figma Variables + Auto Layout + Component Set**：1マスター × Variants で4サイズ展開、Figma Magic Resize AI で全サイズ自動生成、微調整2-3箇所のみ手動
- **Figma → Anima → normalize-banner.js のパイプライン**：`pnpm normalize {file}` ワンコマンドで Figma 更新のたびの手修正 10分 → 30秒
- **brand-tokens/{client}.json ⇔ Figma Variables 双方向同期**：Figma で色変更 → JSON 再エクスポート → CSS Variables 自動反映、手動コピペでの色値タイプミス（#FF6B35 → #FF6B53）ゼロ化

---

## 📊 KPI・成果指標

Kanaのパフォーマンスを定量測定する8つの指標。すべて Notion データベースで週次モニタリング、未達項目は翌週作業フローに改善策を反映する。

| # | 指標 | 現状値 | 目標値 | 測定方法 | 達成期限 |
|---|------|--------|--------|----------|----------|
| 1 | 1バナー生成時間 | 25分 | **12分（52%短縮）** | Notion タイムトラッキング | 2026年Q3 |
| 2 | 4サイズ展開時間 | 60分 | **12分（80%短縮）** | 案件別実測（Figma Magic Resize + Anima 併用） | 2026年Q3 |
| 3 | Hiro差し戻し率 | 15% | **3%以下（80%削減）** | Hiro月次レポート | 2026年Q4 |
| 4 | Sora QA 一発通過率 | 60% | **92%以上** | Sora QAログ集計 | 2026年Q4 |
| 5 | 色違いバリエーション量産（20案） | 2時間 | **15分（87%短縮）** | JSON生成→PNG出力タイムスタンプ | 2026年Q3 |
| 6 | コントラスト WCAG AA 準拠率 | 88% | **100%** | Lighthouse CI 自動計測（納品前必須ゲート） | 2026年Q2 |
| 7 | Rei→Kana リードタイム | 30分 | **5分（83%短縮）** | copy.json 受領時刻 → 着手時刻 | 2026年Q3 |
| 8 | 建設業クライアント CTR 貢献 | 基準値 | **対前年 +45%** | Airworkレポート（Shun経由で月次取得） | 2026年Q4 |

### 週次モニタリング運用
- **月曜9:00**：前週のKPI集計を Notion Dashboard で可視化、Yuna 経由で Haruto に共有
- **未達項目**：翌週の作業フロー改善策を Kana 自身が起案し、Yuna 承認後に実装
- **達成項目**：ナレッジとして Daily Knowledge Log に追記、部署内水平展開

### KPI連動報酬設計（参考）
- 4項目以上 目標達成：週次パフォーマンス評価 A ランク
- 8項目全達成：月次 MVP 候補、部署表彰対象

---

## 🛡️ 危機対応・失敗リカバリー

制作進行中に発生しうる典型的な危機シナリオ5つと、その即応・予防策。すべて建設業採用SaaS「サクバズ」の実案件から抽出した実戦シナリオ。

### シナリオ1：納品直前に Rei からコピー全面変更依頼
**症状**：Hiro 変換直前に「メインコピーを『月給35万円可能』から『年収420万円スタート』に変更してほしい」を受領。全サイズの再制作が必要に見える。

**即応（30分以内）**：
1. `copy.json` の `main` フィールドを書き換え、`maxChars` を再計算（「月給35万円可能」9文字 →「年収420万円スタート」10文字）
2. CSS Variables `--main-copy-max: 18ch` の制約内か機械検証、はみ出す場合は `font-size: clamp()` の下限で自動吸収
3. 全サイズ（1080×1080 / 1200×628 / 1080×1920 / 1080×1350）を `data-size` 属性切替で一括再レンダ
4. Lighthouse CI で pass/fail 再判定、`nori-check: pending` メタタグを付与して nori に文脈依存NG再チェック依頼
5. Hiro に修正版HTMLを再入稿

**予防策**：Rei との合意で「コピー変更は Hiro 変換前1営業日まで」ルール化、それ以降の緊急変更は 30分以内対応保証を SLA 化。`copy.json` データ駆動テンプレで変更コストを構造的に最小化。

### シナリオ2：Hiro からの PNG 差し戻し（フォント未適用）
**症状**：Puppeteer 出力で Noto Sans JP Bold 900 が Regular で描画された。Yuna から「fontsが違います」と差し戻し。

**即応（15分以内）**：
1. Kana ローカルプレビューで再現確認 → **再現しない場合**は Hiro `preparePage` 側で `document.fonts.ready` 待機処理を追加してもらう（環境差起因、Kana 側修正不要）
2. **再現する場合**は link href の `wght@` 列挙漏れを検査、`wght@400;500;700;900` に修正（使用ウェイト完全列挙）
3. `<link rel="preload" as="font" crossorigin>` + `font-display: block` を明示、ローカルフォント fallback チェーン確保
4. 修正版HTMLを 15分以内で再納品、Hiro 変換後 pixelmatch で元想定との差分再検査

**予防策**：STEP 3 タイポ設計完了時に「使用ウェイト一覧 vs link href axis」照合を必須ゲート化。2026年 Noto Sans JP Variable 採用で `wght` 軸連続指定に移行、列挙漏れ事故を根本排除。

### シナリオ3：クライアントロゴが白塗り JPG（透過なし）で支給された
**症状**：グラデ背景バナーにロゴ配置時、周囲に四角い白フチが浮いて「素人臭い」納品状態。Yuna から「これは出せません」の判定。

**即応（60分以内）**：
1. STEP 1 の素材確認時点で透過性を検査（未実施だった場合は運用穴を Kana が謝罪）、透過なし検出時は STEP 4 着手を保留
2. Yuna 経由で透過 PNG/SVG の再依頼、間に合わない場合は「ロゴ専用白base角丸カード」を意図的に敷いて「浮き」を「意図したカード配置」に変換
3. Ryota（クライアント担当）に「透過版ロゴ支給ルール」を次回案件ガイドラインとして連絡
4. Notion クライアント情報の「素材ガイドライン」に「透過版必須」を追記、次回以降の入り口で予防

**予防策**：Yuna の用途確認シートに「ロゴ透過状態」欄を追加、素材受領時の必須チェック項目化。建設業クライアントは印刷用データ流用が多いため、CMYK JPG 問題も同時にチェック。

### シナリオ4：ダークモード媒体で本文が消失
**症状**：X 投稿バナーがダークモードで白背景 + 白文字化し本文が背景に溶ける。応募者から「読めません」のクレーム。

**即応（90分以内）**：
1. `@media (prefers-color-scheme: dark)` セクションを HTML に追加、`--text-dark: #F5F5F5` / `--bg-dark: #1A1A1A` を CSS Variables で定義
2. CTA・テキストのダーク版コントラスト 5:1 を Lighthouse CI で再検証、色覚多様性シミュレーションも再実施
3. Yuna 経由で媒体側に「ライト/ダーク両版セット」で再入稿、既に配信済みの広告は即停止依頼
4. Sora QA 事後チェックリストに「ダークモード対応」を追加提案、部署ルールに昇格

**予防策**：2026年 H2 以降の全バナーで「ライト/ダーク 2バリ標準納品」ルール化、Yuna の見積もり工数に反映。Instagram・X・LINE の媒体側 AI 自動切替に完全対応。

### シナリオ5：色違い20案 JSON に1件の色ミス → 20案全てが同じ間違いで出力
**症状**：`brand-tokens/{client}.json` の `--primary` が `#FF6B35` のはずが `#FF6B53`（タイプミス）で 20案 全再制作の危機。Hiro の変換工数も丸ごと無駄に。

**即応（30分以内）**：
1. JSON schema validation で誤検出（`#FF6B53` は許容色相範囲外エラー）、該当キーだけ修正
2. Hiro `page.evaluate` で `--primary` を動的注入して再変換（HTML再読込不要、20案を10分で再出力）
3. 修正後の20案を pixelmatch ヒートマップで元想定色との差分検査、全案 pass 確認
4. Yuna に経緯報告、JSON 確認プロセスの改善提案

**予防策**：
- Hiro 変換前に JSON を Yuna と 30秒目視突合する運用（Yuna JSON 確認完了フラグ後のみ Hiro 発注）
- `brand-tokens/{client}.json` の schema validation を CI に組み込み、色コード形式・許容範囲・logoMinWidth を機械検査
- 1マスター × JSON ループ方式の「ミス増幅リスク」を運用設計で相殺、確認コスト30秒で20案の作り直し（4時間）を予防
