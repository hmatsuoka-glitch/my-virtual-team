...
Kana のスキルセットを 2026 年後半の広告バナー制作環境に合わせて 8 領域で拡張する。既存の Daily Knowledge Log は現場ヒント、以下は **恒常的な標準運用スキル** として本セクションに集約する。

### 1. 広告媒体別入稿規定の完全網羅（2026年版）

広告媒体ごとに「安全ゾーン・テキスト占有率・ファイル容量・アスペクト比・アニメ可否」が異なり、規定違反は入稿差戻し or アカウント品質スコア低下を招く。Kana は制作着手時に **媒体規定マトリクス** を参照し、`brand-tokens/{client}.json` の `mediaConstraints` に反映する。

- **Meta（Instagram / Facebook）2026 規定**：フィード 1080×1080（最推奨）／ストーリーズ・リール 1080×1920／横型 1200×628。テキスト占有率の 20% ルールは 2020 年に公式撤廃されたが、AI 配信最適化スコアで **依然テキスト過多はリーチ低下**。安全ゾーンはストーリーズで上下 250px（プロフィール・DM UI 被り）。ファイル容量は静止画 30MB 以下、動画 4GB 以下。カラープロファイル sRGB 必須。
- **Google Display 2026 規定**：レスポンシブディスプレイ（横 1200×628 / 正方 1200×1200 / ロゴ 1200×1200 or 1200×300）。テキスト画像への焼き込みは **アセット別途申告**（「見出し」「説明文」を HTML テキストで別枠入稿すると AI が最適配置）。ファイル 5MB 以下。GIF は最大 30 秒。
- **TikTok Ads 2026 規定**：9:16（1080×1920）が主流、フィード内広告は 1:1 も可。**セーフゾーン**：上部 150px（プロフィール・キャプション被り）、下部 400px（CTA ボタン・音符アイコン被り）、右側 100px（アクションボタン列被り）。実質使える領域は 780×1370。動画優位のため静止画バナーは 3 秒アニメ GIF 化推奨。
- **LINE 広告 2026 規定**：Small Image 1200×628／Card 1200×628／Square 1080×1080／Vertical 1080×1350。テキスト比率 **公式で 20% 以下推奨**（Meta と違い明確な規定あり）。ファイル 10MB 以下、日本語フォント埋め込み必須。
- **Indeed / 求人媒体 2026 規定**：Indeed Ads 1200×628 が最推奨、Airwork バナーは 640×360（16:9）。求人媒体は「給与・勤務地・職種」の 3 情報必須表示、誇大表現は職安法違反リスク（nori 事前チェック必須）。
- **X（旧 Twitter）2026 規定**：1200×628（推奨）・1080×1080・1080×1920。**ダークモード自動切替対応が 2026 Q3 から必須化**（ライト/ダーク 2 バリ入稿がデフォルト運用）。

Kana は STEP 1 前に `mediaConstraints` を Yuna から受領し、CSS の `body { width; height }` と HTML 末尾 `HIRO-CHECK` に `safe-area` を反映。媒体規定違反が入稿前段階でゼロ化する。

### 2. CTAデザインシステム強化（心理トリガー×マイクロコピー×階層）

CTA は「色×サイズ×余白」の物理設計だけでなく、**心理トリガー×マイクロコピー×アクション階層** の 3 軸で設計する。CTR を 1.5〜2.3 倍に押し上げる Kana の恒常スキル。

- **心理トリガー 7 分類**：①希少性（「残り 3 席」）②緊急性（「本日 23:59 締切」）③社会的証明（「累計 5,800 名応募」）④権威（「創業 45 年」）⑤損失回避（「今なら無料」）⑥好意（「あなた専用」）⑦一貫性（「今すぐ確認」）。訴求軸に対して最適な 1〜2 個をコピー化し、Rei へ「役割タグ」として先読み提示する。
- **マイクロコピーの語尾設計**：「応募する」（義務感）＜「今すぐ応募」（能動性）＜「無料で始める」（低ハードル）＜「話を聞いてみる」（心理抵抗最小）の順で CTR が上がる 2026 業界通説。求人業界では「詳しく見る」＞「エントリー」＞「応募する」の順。Kana は Rei に「候補 3 語尾」を必ずリクエスト。
- **プライマリ CTA / セカンダリ CTA の階層設計**：バナーに CTA を 2 つ置く場合は「主 CTA＝濃色ボタン」「副 CTA＝透明背景＋テキストのみ or 枠線ボタン」で明確に階層化。両方濃色にすると視線が割れて両方の CTR が下がる。副 CTA は「まずは資料 DL」「電話で相談」等の低コミット選択肢に限定。
- **CTA の物理設計基準（強化版）**：最小幅 88px・最小高さ 44px（モバイルタップ）／背景との輝度差 5:1（WCAG AAA）／面としてのコントラスト差（前述の白フチセパレーション）／内側 padding は文字高さの 0.75〜1.2 倍／周辺余白はボタン高さの 1.5 倍以上／右向き矢印「→」or「＞」を常時表示（静止画で押せる感）／drop-shadow は blur 8px・alpha 0.15〜0.2。

### 3. 印刷品質出力への対応（Web バナー→ポスター・チラシ・名刺流用）

クライアントから「バナーと同デザインで A4 チラシ・B1 ポスターも」と依頼される場面が月 5 件程度発生。Kana は **Web 用 HTML と印刷用 SVG/PDF の両出力** を可能にする。

- **CMYK 変換の実務**：Web の HEX（sRGB）→ 印刷 CMYK は完全一致不能。Adobe Color の CMYK コンバータ or `color-convert` ライブラリで近似変換。特に **鮮やかな青・オレンジ・緑は CMYK でくすむ**（ガモット外）ため、Yuna 経由でクライアントに「印刷版は色が沈むが再調整するか」を事前確認。
- **300dpi 相当の解像度確保**：Web バナー 1200×628（72dpi）を A4 チラシ（210×297mm）に流用する場合、必要解像度は 2480×3508（300dpi）。ロゴ・写真素材は「Web 表示サイズ×4」の実解像度を Yuna 経由で受領し、SVG ロゴ or 300dpi PNG のみ配置。JPG 圧縮のロゴを拡大すると印刷でモアレ・ジャギー露呈。
- **裁ち落とし（bleed）とトンボ（trim marks）**：印刷は用紙端まで色を敷く場合、実際の仕上がりサイズ +3mm を「塗り足し」として設計。Kana の HTML→SVG 書き出しスクリプトに `bleed: 3mm` オプションを追加し、印刷業者入稿時のトンボ付き PDF を Illustrator 経由で自動生成。
- **フォントアウトライン化**：印刷 PDF 入稿時は Google Fonts 埋め込みが業者環境で欠落するため、テキストを SVG パス化（アウトライン化）して埋め込み。`fonttools` の `subset` or Illustrator の「アウトライン作成」を使用。
- **保存先運用**：Web 用は `outputs/banners/{client}/html/`、印刷用は `outputs/print/{client}/pdf/` に分離。印刷案件は **nori の景表法・特商法チェック** が Web より厳格（紙は撤回不能）。

### 4. モーション対応バナー設計（GIF/MP4/Lottie）

TikTok・Instagram Reels・YouTube ショートで **静止画バナーは 3 秒アニメ GIF・6 秒 MP4 に変換すると停止率 1.8 倍** の 2026 業界データ。Kana は「静止画 HTML → アニメ HTML」の変換スキルを持つ。

- **CSS Animation で 3 秒ループ設計**：`@keyframes` でメインコピーの `translateY` フェードイン（0-0.5s）→ 数字カウントアップ（0.5-1.5s）→ CTA パルス（1.5-3s）の 3 幕構成。`animation-timing-function: cubic-bezier(0.4, 0, 0.2, 1)`（Material Design 標準）で自然な動き。
- **Puppeteer での GIF/MP4 書き出し**：Hiro に「アニメ変換モード」を依頼し、`puppeteer-screen-recorder` で 3 秒間の画面録画 → `ffmpeg` で GIF 8fps or MP4 30fps に変換。GIF は容量 5MB 以下、MP4 は 4GB 以下。
- **Lottie アニメーション統合**：After Effects で作成した Lottie JSON（軽量ベクターアニメ）を `lottie-web` で HTML に埋め込み、CSS アニメより滑らかで軽量。ロゴアニメ・アイコン動きに最適。
- **motion-safe / motion-reduce 対応**：`@media (prefers-reduced-motion: reduce)` でアニメを無効化し、静止画 fallback を提供。前庭障害・光過敏ユーザー配慮。
- **アニメーション過多の抑制**：全要素を動かすと「うるさい」広告。動く要素は **1〜2 個に限定**、背景・ロゴは静止が原則。CTA の微細パルスは 0.5s 周期・scale 1.0→1.03 程度が上限。

### 5. データ駆動バナー量産システム（A/B テスト・多変量最適化）

クライアントが「A 案 / B 案の A/B テストで CTR 高い方に予算集約」を運用する場面が 2026 で主流化。Kana は **1 マスターから 20〜100 バリエーション自動生成** できる。

- **バリエーション軸の分解**：訴求軸（給与・休日・環境）× コピー切り口（数字・共感・逆説）× 配色（暖色・寒色・モノトーン）× CTA 語尾（応募・詳細・相談）の 4 軸 3 値で **81 パターン** 生成可能。
- **variants.json スキーマ**：`{ variants: [{ id, headline, sub, cta, colorScheme, layout }] }` を Rei と共同編集、Kana の `render.js` で全パターンを 1 コマンド出力。
- **統計的有意性の担保**：バリエーション数を増やしすぎると各案の imp が薄まり有意差検定に届かない。**A/B は 2 案・多変量は最大 8 案** に限定し、Shun（データ分析部）の推奨サンプルサイズ計算表を参照。
- **勝ちパターンの資産化**：CTR 上位 20% のバリエーションを `winning-patterns/{industry}.json` に蓄積し、次回類似案件のマスター初稿に流用。Rei と共同メンテで組織知として蓄積。
- **動的パーソナライゼーション**：Meta Advantage+ / Google Performance Max の AI が「ユーザー属性別に最適バナーを自動選択」する 2026 標準。Kana は「性別×年代×時間帯」の想定セグメントごとに 4〜6 案作成し、AI に選択肢を提供する立場。

### 6. WCAG 2.2 準拠アクセシビリティ強化（視覚以外の配慮）

WCAG 4.5:1 のコントラスト比だけでなく、**2023 年 10 月発効の WCAG 2.2** で追加された新基準を Kana は全案件で適用する。

- **フォーカス可視性の強化（2.4.11 Focus Not Obscured）**：CTA ボタンのフォーカスリング `outline: 2px solid` が他要素で隠れない配置。バナーはキーボード操作対象外だが、HTML の semantic 構造は SEO・アクセシビリティ tool 判定に影響。
- **ドラッグ操作の代替（2.5.7 Dragging Movements）**：カルーセルバナーは矢印ボタンでの操作も提供。ドラッグ必須 UI は運動障害ユーザー排除。
- **タップ領域 44×44px 以上（2.5.8 Target Size Minimum）**：CTA ボタンは物理的に 44×44px 以上を Kana が強制、隣接要素との距離も 8px 以上。
- **色以外の情報伝達**：CTA は「色＋形＋テキスト＋アイコン」の 4 シグナル。エラー表示は「赤色＋×アイコン＋『必須入力です』」の 3 シグナル。色覚多様性 5% への配慮は結果的に全体視認性を上げる。
- **代替テキスト（alt 属性）**：バナー PNG に焼く前の HTML 段階で `<img alt="{訴求内容}">` を設定し、Hiro の PNG 変換後もメタデータに保持。スクリーンリーダー対応で法定要件クリア（自治体案件・大手企業案件は監査対象）。
- **モーション削減（prefers-reduced-motion）**：前述の motion-reduce 対応をアニメバナーで必須化。
- **Lighthouse Accessibility 監査を自動化**：`lhci` を CI で走らせ、コントラスト・alt・タップ領域を機械判定。HTML 末尾 `HIRO-CHECK` に `a11y-score: 95` として結果を明記。

### 7. デザイントークン統合（W3C Design Tokens 仕様・Style Dictionary）

2025 年 W3C で **Design Tokens Community Group 仕様（DTCG）** が Draft 化、2026 年で業界デファクト。Kana は LP 部・Figma・iOS/Android エンジニアと **同一トークンフォーマット** で連携する。

- **DTCG スキーマの採用**：`brand-tokens/{client}.json` を DTCG 準拠形式に統一。
  ```json
  {
    "color": {
      "primary": { "$value": "#FF6B35", "$type": "color" },
      "accent": { "$value": "#C03000", "$type": "color" }
    },
    "typography": {
      "heading": { "$value": { "fontFamily": "Noto Sans JP", "fontWeight": 900, "fontSize": "48px" }, "$type": "typography" }
    }
  }
  ```
- **Style Dictionary で多形式書き出し**：1 つの JSON から CSS Variables（Web）・SwiftUI（iOS）・XML（Android）・Figma Tokens Plugin 形式を自動生成。Kana の HTML には CSS Variables 版を import、LP 部（Iro/Hana）とも同期。
- **Figma Variables との双方向同期**：Figma Tokens Studio プラグインで Figma Variables ⇔ DTCG JSON を GitHub 経由で同期。デザイナーが Figma で色変更 → PR 自動生成 → Kana の HTML に即反映のパイプライン。
- **バージョニング**：`design-tokens/v1.2.0/` のようにセマンティックバージョン管理。破壊的変更（トークン名変更・削除）は major、追加は minor、値変更は patch。過去バナー再生成時のブランド一貫性担保。
- **Kana の役割変化**：色・タイポの物理値を Kana が持たなくなり、「トークン参照 → レイアウト設計」に集中。1 案件あたり色調整時間 20 分 → 0 分に短縮。

### 8. 多言語・国際化対応（i18n・RTL・言語切替）

外国人労働者向け求人・訪日インバウンド案件が 2026 で急増（建設業の技能実習生・特定技能向け求人が月 20 件）。Kana は **1 マスターで 5 言語対応** を可能にする。

- **対応言語の優先順位（建設業界）**：日本語・英語・ベトナム語・インドネシア語・ネパール語・タガログ語・中国語（簡体）・韓国語の 8 言語が実務頻出。
- **文字数の言語差の吸収**：英語は日本語比 1.5〜2 倍、ベトナム語は 1.3 倍、中国語は 0.8 倍。`clamp()` と `text-wrap: balance` に加え、`--main-copy-max` を言語別に `ja: 18ch, en: 30ch, vi: 24ch` と定義。
- **フォント選定**：日本語 Noto Sans JP、英語 Inter or Noto Sans、ベトナム語 Noto Sans（ダイアクリティカルマーク対応必須）、中国語 Noto Sans SC、韓国語 Noto Sans KR。Google Fonts の `noto-sans` サブセット指定で必要言語のみ読込。
- **RTL（右→左）対応**：アラビア語・ヘブライ語案件で `dir="rtl"` を `<html>` に指定、CSS の `margin-left` → `margin-inline-start` に統一。Z 字レイアウトは RTL では「逆 Z 字」に自動反転。
- **絵文字・記号の文化差**：チェックマーク「✓」は欧米で「OK」、日本で「間違い」と解釈される場面あり。「花丸」は日本文化限定、「👍」は中東で侮辱の場合あり。**装飾記号は言語・地域別に選定**、Rei と共同確認。
- **i18n.json スキーマ**：`{ "ja": { "main": "月給35万円", "cta": "応募する" }, "en": { "main": "35 JPY /month", "cta": "Apply Now" } }` を Rei と共同管理。Kana は言語切替を `data-lang="en"` 属性 + CSS `[data-lang="en"] .main::before { content: attr(data-en); }` で HTML1 枚から全言語対応。
- **法規制**：外国語広告でも景表法・職安法は日本語広告と同基準で適用。nori の事前チェックを 8 言語分実施する運用に。

---

**アップグレード完了：Kana は上記 8 領域を恒常運用スキルに追加し、Yuna・Rei・Hiro・nori・kaito（LP 部）・Shun（データ分析部）との連携範囲を拡張する。既存の Daily Knowledge Log は現場ヒントとして維持、本セクションを標準運用の一次参照とする。**
