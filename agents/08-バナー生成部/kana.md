...
#### 【新スキル1】CSS Grid × subgrid による「条件3点セーフエリア」の絶対配置化
- 建設採用バナーの「給与・職種・勤務地」ブロックは`display:grid; grid-template-areas`で領域名を切り、コピー変動でも面積比を固定する。文字量が増えても隣接ブロックが押されず、35%縮小時の視認面積が守られる
- 親グリッドで区切ったカードやバッジ列は`grid-template-columns: subgrid`で親の列トラックを継承させ、社名ロゴ・給与額・CTAの縦ラインを1px単位で揃える。ロゴ差替え時の目視調整が消える
- 給与「35万円」の数字と単位は`grid-template-columns: max-content max-content`で並べ`align-items: baseline`でベースライン一致。単位が小さくても「35万」の数字の下端がズレて素人臭くならない
- 主訴求ブロックは`place-items: center`ではなく`align-self:end; justify-self:start`のように明示配置。中央寄せは縮小時に「なんとなく真ん中」になり、視線導線の起点が定まらない
- グリッドギャップは`gap: clamp(8px, 2cqi, 24px)`で最小可読サイズ（2026-08-16）と連動させ、余白が縮小で潰れて情報密度が上がりすぎるのを防ぐ

#### 【新スキル2】Container Queries（`@container`）で「1マスターHTML→全サイズ展開」の脱・メディアクエリ
- バナー親要素に`container-type: inline-size; container-name: banner`を付与し、`@container banner (max-width: 400px) {...}`でサイズ別スタイルを親要素基準に切り替える。ビューポート基準の`@media`では1080×1080と1200×628が同じ枝に落ち崩れる
- 「7社×6サイズ=42案」を1HTMLで生成する運用に切り替える。CSS変数（クライアント色）とcontainer幅（サイズ）の二軸で全ケースが出るため、HTMLの重複コピーとそれに伴う修正漏れが消える
- コンテナ幅で`font-size: clamp(14px, 4cqi, 48px)`と`cqi`（container inline %）を使い、実キャンバス幅の関数として文字サイズが決まる構造にする。サイズ別に手動でpxを書き換える工数が消える
- Feed（Meta 1080×1080）とStories（1080×1920）は同一マスターで`@container (aspect-ratio > 1)`と`(aspect-ratio < 1)`で縦横を分岐。Hiro側の変換ループがそのまま42案に効く
- container query length（`cqw`/`cqh`）は古いChromiumで未対応の場面があるため、Hiroのヘッドレスバージョン（Chrome 130+）を前提に合意した上で採用。フォールバックは`@supports (container-type: inline-size)`で切る

#### 【新スキル3】WCAG 2.2 コントラスト比の「APCA/Lc値」併用チェック
- 4.5:1（WCAG 2.1）は白背景+黒文字などの大コントラストで通ってしまい、青地に濃紺文字などの実務ケースを見逃す。APCAの`Lc 75`以上を主訴求（給与・職種）に、`Lc 60`以上をバッジ・CTAに設定する
- 建設系ブランドカラー（濃紺・カーキ・ダークグレー）に白文字を載せる際は、単純な明度差でなくAPCAで測る。特に「紺×濃いオレンジCTA」はWCAG 2.1で通ってもAPCAで沈むため、CTAだけ明度を1段上げる
- ダーク背景の白文字は「見た目より明るく感じる（極性反転）」ため、細字ウェイト（<600）を避けて`font-weight: 700`以上を下限にする。細字は同じLc値でも実機で潰れる
- 半透明オーバーレイ（写真上に文字）は`background: rgba(0,0,0,0.55)`を最小として、写真の最明部でもLc 75を維持できるかピクセル単位でチェック。写真差替え運用ではこのオーバーレイ濃度をトークン化する
- コントラスト比は「文字色×直下背景」だけでなく「文字色×フィード背景」も見る。透明背景PNGを想定するなら、白フィード/黒フィード両方でLc 60を満たす色に閉じる（2026-08-16「白背景は輪郭が消える」と対）

#### 【新スキル4】Web Fonts の`size-adjust`＋可変フォントで「FOUT無しの日本語ウェイト連続制御」
- Google Fonts `Noto Sans JP`の可変版を`<link>`で読み込み、`font-weight: 400 900`のレンジ指定＋`font-variation-settings: 'wght' 850`で「880」など中間ウェイトを使う。給与数字だけ極太化しても総ダウンロード量が増えない
- `@font-face`側で`size-adjust: 105%`を指定し、フォールバック（`sans-serif`）とNoto Sans JPのx-heightを合わせる。FOUT発生時のレイアウトシフトがゼロになり、Hiroの`fonts.ready`待ちが失敗しても崩れない
- 数字表記は`font-feature-settings: 'tnum' 1, 'palt' 0`で等幅数字＋プロポーショナル無効。「35万円」と「28万円」の桁位置がズレず、色違い20案の量産時に給与欄が揃う
- 日本語見出しは`font-feature-settings: 'palt' 1`（プロポーショナル）を条件付きで使い、句読点前後の間延びを詰める。ただし数字を含む行では`tnum`と競合するため`span`単位で切り替える
- 使用ウェイトはGoogle Fontsのhref内`wght@400;700;900`にすべて列挙するが、可変フォント採用時は`wght@400..900`のレンジ記法で1リクエストに集約。読込RTTが減り、Hiroキャプチャの安定性が上がる

#### 【新スキル5】Fitts's Law × 視線導線に基づく「CTA面積・位置」の定量設計
- Fitts's Law（`T = a + b·log2(D/W + 1)`）は物理タップだけでなく視線移動にも比例する。CTAは主訴求からの視距離（D）が短く、幅（W）が広いほど「押される感」が上がる。CTA最小面積は`min(width, height) ≥ 88px`（Appleの44pt×2倍解像度換算）
- SNSフィードでは親指到達エリア（画面下1/3）が有利。1080×1920のStoriesはCTAを下1/3内かつ左右エッジから88px以上内側に配置し、UIバーとの重なりを避ける
- 「F字視線」は情報密度が高い場合、「Z字視線」は疎な場合に成立。建設採用バナーは条件3点+CTAで情報密度が高いため基本F字を採用し、左上→右上→左下→右下（給与→職種→勤務地→CTA）で配置する
- CTAボタンは「テキスト+アイコン+余白」の三層構造で`padding: 16px 32px`を下限とする。テキストのみ・角丸なし・境界なしのCTAは、写真背景の上でボタン認識されず視線が素通りする
- 主訴求→CTAの視距離（D）はキャンバス対角線の40%以内。1080×1080なら約610px。これを超える場合は主訴求とCTAの間に矢印や色ブロックで「視線橋」を掛ける

#### 【新スキル6】Meta / Indeed / Google 各媒体規格の同時充足マスター設計
- **Meta広告バナー規格2026**: Feed=1080×1080/1200×628、Stories/Reels=1080×1920、テキスト比率20%ルールは廃止済みだが「文字面積が広いと配信が絞られる」傾向は継続。文字面積は画像全体の35%以下に収める
- **Indeed求人広告仕様**: サムネイル320〜400px幅で表示されるため実表示11px以上（2026-08-16）を厳守。会社ロゴは最大高さ64px（実表示24px相当）内、CTAは求人詳細ページ側にあるためバナー内CTA重複禁止
- **Google広告レスポンシブ**: ランドスケープ1200×628、スクエア1200×1200、ロゴ1200×1200と1200×300の縦横2種を必須納品。マスターHTMLを`aspect-ratio`分岐で3種+ロゴ2種の5案が同時に出る構造にする
- **TikTok広告**: 1080×1920のみ、上部10%と下部20%はUI領域でセーフエリア外。主訴求はキャンバス中央70%内に収める（2026-08-13「セーフエリア確認」の媒体別数値化）
- 5媒体×7クライアント=35マスター案件を、`brand-tokens/{client}.json` × `media-preset/{platform}.json` の二次元テーブルにする。新媒体追加時はプリセットJSONを1つ足すだけで7社分展開が回る

#### 【新スキル7】`text-box-trim`＋`line-height`絶対値制御による「バッジ・数字の天地中央」精密化
- 「未経験OK」バッジや給与数字の枠内センタリングは`padding`で目視調整せず、`text-box: trim-both cap alphabetic`で欧文キャップハイトと日本語仮想ボディの天地を強制トリム。ブラウザ差異が消える
- 数字は`line-height: 1`かつ`text-box-trim: trim-both`で「35」の上下余白をゼロにし、単位「万円」との視覚バランスを`vertical-align: baseline`で取る。目視の1px調整ループが不要になる
- バッジは`display: inline-flex; align-items: center; justify-content: center`＋`line-height:1`＋`padding: 8px 16px`で幅可変・高さ固定。文字数が変わっても円形/角丸バッジの形が崩れない
- `text-box-trim`未対応ブラウザ（Firefox一部）向けフォールバックは`@supports not (text-box: trim-both cap alphabetic) { padding-top: calc(...); }`で分岐。Hiroキャプチャ環境（Chrome 130+）優先で運用
- 給与額の桁区切りは`Intl.NumberFormat`をJS実行させず、静的に「350,000円」を書く。ヘッドレスキャプチャ時のJS実行タイミングに依存させないためのバナー内ルール
