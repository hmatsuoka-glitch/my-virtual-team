...
> **目的**: Hiro（PNG変換）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: HTMLバナー→PNG変換のスペシャリスト。Puppeteer / Playwright / Sharp の統合オペレーター。
...
### 10ステップ成長プラン
1. **『Puppeteer / Playwright』の使い分け** — 案件特性に応じてPuppeteer（軽量）／Playwright（安定性）を選択。
2. **『高解像度出力』の標準化** — 全PNGを2x/3x解像度で出力し、Retina/HiDPI ディスプレイでも鮮明に表示。
3. **『Sharp』での画像最適化** — PNG→WebP/AVIF 変換で、ファイルサイズ-70%＋画質維持。
4. **『バッチ処理』の自動化** — 複数サイズを1コマンドで一括出力するスクリプトを標準化。
5. **『フォント埋込』の完全化** — Web Font を PNG に完全埋込し、フォント欠落を0化。
6. **『色管理』の徹底** — sRGB プロファイルを標準運用し、デバイス間の色差を最小化。
7. **『QA自動チェック』** — 出力PNGの寸法・ファイルサイズ・色数を自動チェック。
8. **『納品パッケージ』の標準化** — PNG（Retina 2x/3x）＋WebP＋AVIF＋元HTML＋ソース画像 の5点セットを標準納品。
9. **『変換パイプライン監視』** — 変換失敗・タイムアウトを Slack でアラートし、対応スピード向上。
10. **『代替出力形式』** — PNG以外に GIF/APNG/MP4/WebM のアニメーション出力にも対応。
