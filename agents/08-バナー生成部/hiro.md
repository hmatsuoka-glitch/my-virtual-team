...
### 2026-09-21
- **2026年最新トレンド：Puppeteer v24 + Chrome for Testing 128 の CDP セッション共有**：`browser.createBrowserContext({ downloadBehavior: 'allow' })` で軽量コンテキストを分離し、`page.emulateVisionDeficiency('none')` と `page.emulateCPUThrottling(1)` を明示指定することで、CI ランナー（GitHub Actions ubuntu-24.04）と開発 Mac の間で ±1px ずれ問題が解消。従来の `puppeteer.launch()` を全て `launchAndConnect()` パターンに置換、20枚バッチ処理で 18秒→11秒（38%短縮）。Kana からの HTML を並列処理する新標準として config 化する。
- **2026年最新トレンド：AVIF 出力併走で媒体入稿 2 系統対応**：Google Ads Performance Max と Meta Advantage+ が 2026-Q3 から AVIF を受け付け開始。`sharp(pngBuffer).avif({ quality: 65, effort: 6 })` で同じバナーから AVIF を副生成し、PNG（従来納品）+ AVIF（次世代納品）の 2ファイル同梱納品に切替。AVIF は PNG 比 40% サイズ削減、Indeed 150KB 上限を余裕でクリアしつつ画質向上、Yuna 経由でクライアント運用担当に「AVIF 対応媒体はこちら」で差別化。
- **失敗パターン：Chrome for Testing 自動更新でフォントレンダリング微差** → 回避策：`package.json` の `puppeteer` バージョンを `~24.5.0` にピン留め、`.puppeteerrc.cjs` で `skipDownload: false` と `cacheDirectory: './node_modules/.cache/puppeteer'` を明示。CI では毎回同一 Chromium リビジョンをキャッシュヒットさせ、macOS/Linux 間のヒラギノ vs Noto の代替フォント問題を撲滅（理由：クロスOSでの Kana HTML 検証時、CTAボタン内の「応募する」1文字の字幅が違うと右端切れが発生する）。
- **失敗パターン：sharp のバージョン差で ICC プロファイル埋込が挙動不一致** → 回避策：`sharp@0.34.x` を lockfile で固定し、`.withMetadata({ icc: 'sRGB IEC61966-2.1' })` を必ずチェーン。libvips バイナリの ABI 差でカラープロファイルが Adobe RGB として書き出されるバグを踏むと、印刷併用時に彩度が飛ぶ（理由：クライアント側で名刺・チラシに流用され「Web と紙で色が違う」クレームの温床になる）。
- **品質チェックポイント：`sharp().stats()` による白飛び・黒潰れ検出**：出力 PNG の各チャネル histogram を取得し、`stats.channels[0].max === 255` かつ最頻値が 250 以上なら「白飛び」、`min === 0` かつ最頻値が 5 以下なら「黒潰れ」と判定してログ出力。Kana の CSS で `filter: brightness(1.2)` などが混入したまま出力されるケースを自動検出、目視 30 秒 → 0 秒。
- **品質チェックポイント：tesseract.js v5 の OCR で入稿文字と原稿の完全一致検証**：`Tesseract.recognize(pngPath, 'jpn+eng')` で PNG 内の全テキストを抽出し、Kana から受領した原稿 JSON と Levenshtein 距離で照合、距離 3 以上なら差し戻し。フォント読込失敗による「豆腐化（□□□）」や、CSS `text-transform: uppercase` で意図せず大文字化するミスを撲滅、nori 法務チェック連携（薬機法 NG ワード検出）も同一パイプラインで併走。
- **効率化テクニック：Puppeteer worker_threads プールパターン**：`piscina` ライブラリで worker_threads を 4 並列プール化し、各 worker が Puppeteer browser インスタンスを 1 つ保持。20 枚バッチで従来 18秒 → 6秒（3倍高速）、CPU コア 4/8 の Mac mini M2 でメモリ 1.2GB 上限に安定収束。ブラウザプールパターン（2026-05-12）の worker 版として標準テンプレート化。
- **Kana 連携の小ヒント：HTML 内に `<meta name="banner-config" content='{"scale":2,"clip":{...}}'>` を埋め込む契約**：Kana に「Puppeteer 設定を HTML の meta タグに JSON で埋込」を仕様依頼し、Hiro は `page.$eval('meta[name=banner-config]', el => JSON.parse(el.content))` で設定取得。HTML ファイルが自己記述的になり、Yuna の指示シート（2026-05-14）との二重管理を撤廃、config 欠落による再変換ロスをゼロ化。
