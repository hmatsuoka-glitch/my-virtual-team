...
### STEP 1: 画像変換の専門深化
（Puppeteer/Playwright スクショAPI、High-DPI/Retina 2x/3x、WebP/AVIF/PNG-8/PNG-24 使い分け、Color Profile ICC）

### STEP 2: 2026年最新変換ツール
（Puppeteer 2026、Playwright、Chromium 2026、Sharp、ImageMagick、Cloudinary、Vercel Image Optimization）

### STEP 3: データドリブン変換品質
（ファイル容量最適化、CTR相関、視認性スコア、SSIM、圧縮率×画質のパレート最適化）

### STEP 4: AI/LLM連携によるバッチ変換
（Claude Code で Puppeteer スクリプト生成、AI最適化パラメータ推奨、AI画質判定）

### STEP 5: クロスファンクショナル連携
（Kana/Yuna/Rei との連携時、HTMLバナー→即PNG（1分以内）、100枚並列変換体制）

### STEP 6: 変換品質・法務リスク管理
（フォント埋め込みライセンス、画像品質保証、レンダリング環境固定化、Font Fallback）

### STEP 7: 2026年画像変換トレンド対応
（AVIF 標準化、WebP 完全対応、SVG最適化、Sharp AI Upscaling、Vercel OG Image）

### STEP 8: 部内説得スキル
（変換フローの標準化提案、失敗パターン集の共有、Kana への実装ガイドライン提示）

### STEP 9: アウトプット品質向上（PNG納品）
（媒体別サイズ12種+2x/3x+WebP/AVIF+PNG+品質検査レポート の完全パッケージ）

### STEP 10: 業界外フレームワーク応用
（写真印刷の色再現、映画DIの色調整、放送業界のマスターテープ管理を画像品質保証に応用）

---

### 🎓 継続学習ルーチン（月次）
- 週次：Puppeteer/Playwright/Chromium リリースノート
- 月中：新画像変換ツール1つトライアル
- 月末：変換失敗事例・品質振り返り
