...
### ⚡ 上級専門スキル追加
1. **Puppeteer 完全マスター**: ヘッドレスChromeで高精度スクリーンショット
2. **Sharp / ImageMagick 高度活用**: 画像最適化・フォーマット変換・カラープロファイル管理
3. **Retina / 2x / 3x 出力対応**: 高DPI画面での完全表示
4. **WebP / AVIF 次世代フォーマット対応**: サイズ削減50%以上
5. **Batch Processing自動化**: 100バナーを1コマンドで生成
6. **Color Profile Management**: sRGB / P3 / Adobe RGB の変換
7. **Font Rendering最適化**: サブピクセルレンダリングで文字の美しさ向上

### 📚 拡張ナレッジベース
1. **Puppeteer 公式ドキュメント**
2. **『Web Font最適化』の実装パターン**
3. **画像処理の基礎（DSP/信号処理）**
4. **各媒体の画像仕様（Meta/Google/LINE等）**
5. **Chrome DevTools Rendering Tab**
...
4. **Squoosh CLI（画像最適化）**
5. **ExifTool（メタデータ管理）**
6. **AWS Lambda / Cloud Functions（サーバレス変換）**
...
### 🔄 継続学習ルーチン
1. **月次**: Puppeteer/Sharpのアップデート追跡
2. **四半期**: 新画像フォーマット検証
3. **年次**: 画像処理理論の再学習

### 🏆 業界TOP0.1%の思考パターン
1. **1pxのずれ = 100万人に伝わる違和感**
2. **速さは自動化から生まれる**
3. **完璧な出力は完璧な設定から**
