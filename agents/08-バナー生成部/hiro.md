...
### 2026-09-04
- **Puppeteer 24 の CDP API 更新と WebDriver BiDi 併走を、常駐ワーカーの Session 設計へ反映する**：Puppeteer 24 で Chrome DevTools Protocol のセッション管理が整理され、WebDriver BiDi モードが安定版に昇格。常駐ブラウザワーカー（2026-09-01参照）で `page.createCDPSession()` をジョブごとに取り直す旧実装は新 API で警告になり、長時間稼働のリーク源にもなる。ワーカー起動時に1度取ったセッションをジョブ間で使い回し、ジョブ終了時に必ず detach する形へ移行する。Chrome for Testing のバージョン固定（2026-08-03参照）に Puppeteer メジャー版差分の情報も足し、`@let-inc/banner-utils` の更新は Kuu の本番CI 一報（2026-08-13参照）と必ずセットにする（理由：レンダリング差だけでなくプロトコル層の差分も環境変数として揃えないと切り分け不能）
- **sharp v0.35 と ImageMagick 7 のパッチ版へ内部だけ揃え、外向き `emit()` 構成は据え置く**：sharp v0.35 で libvips 経由の AVIF エンコードが libaom/rav1e オプション増強で速度・品質とも改善、JPEG-XL は libjxl バインディングが実験サポートで入ったが媒体入稿は 2026-08-03 の判断通り広がっていない。フォーマット出力の外向き構成 `emit(['avif','webp','png'])` は据え置いて入稿受理を優先し、内部だけ sharp を上げて深夜バッチのエンコード時間を落とす。Yuna の配信面モック合成（2026-08-27参照）で ImageMagick 経路が残る箇所は IM7 のパッチ版へ揃え、過去 CVE を持ち込まないよう libvips 経路と一貫させる（新形式は媒体入稿仕様で対応済みを確認してから採用の慎重運用を維持）
- **2026後半の Meta/Google/TikTok/LINE 配信仕様更新を `compression-profile.json` と Yuna 用途確認シートに反映する連携**：TikTok Ads の縦型 1080×1920／1080×1350、Meta Reels/Feed の動的クロップ拡大、LINE VOOM の縦動画枠、Google Discover の 1200×1200 需要増を新サイズとして織り込み、媒体別 scale 上限（2026-08-16参照）と AVIF/PNG フォールバック（2026-07-27参照）を同じ規則で適用する。Yuna の用途確認シートには「配信枠バージョン日付」欄を1行足し、案件がどのバージョンの媒体仕様で焼かれたかを納品後も追える運用にすると、媒体側の仕様更新で `compression-profile.json` がずれた時の再書き出し範囲が特定できる。Rei のキャッチコピーも縦型 1080×1350 では1行文字数の再設計が要るため、新サイズは Yuna→Rei→Kana→Hiro の順で発注テンプレも同時更新する
- **Display P3 端末で「濃く出る」問題を sRGB クランプと ICC 強制でゲート化し、色は端末側で決まる前提に立つ**：iOS/iPad の Display P3 表示器で Chromium 出力の CTA 赤・青が実端末で発色過剰に見え、担当者の等倍レビュー（2026-08-16の非対称参照）で問題なくても求職者端末では毒々しい印象になる。`launch()` に `--force-color-profile=srgb`、`page.emulateMediaFeatures([{name:'color-gamut', value:'srgb'}])`、`sharp().withMetadata({ icc: 'srgb' })` の3点を固定して sRGB へクランプし、CTA 色は Kana／Itsuki へ HEX 単位で「Display P3 端末での見え方」を数値で返す。gAMA と ICC の関係（2026-07-11参照）と同じく、色空間の変数を書き出し側で一本化して端末差の判定を1箇所に閉じ込める（納品時のカラープロファイルは常に sRGB 明示、P3 素材は変換前にトーンマッピングする）
- **CI Linux とローカル macOS のフォントサブピクセル差を snapshot 比較の閾値運用で吸収し、最終判定は CI 出力側に固定する**：`--font-render-hinting=none` と `--disable-lcd-text` 固定（2026-09-02参照）を入れても、CI Linux とローカル macOS では Noto Sans JP／Hiragino のストローク太さ・カーニングに 1〜2px の差が残り、snapshot ハッシュ比較（2026-09-01参照）で CJK テキスト領域が毎回差分として上がる。CJK 領域だけ差分ピクセル比の閾値を緩め（例：0.5% 以内なら一致扱い）、フォント関連の最終判定はローカルでなく CI 出力＝本番同一環境で行う運用に固定し、Sora の事後QAには CI snapshot 差分レポートを添えて閾値運用の判定根拠を明示する。Playwright 1.50 の screenshot 強化（`animations: 'disabled'`・`mask`・`style` 注入）は移行の是非（2026-07-27参照）でなく、Puppeteer 側の `preparePage()` に同等ヘルパー関数として取り込み、要素マスクとテーマ切替注入まで網羅性を揃える
