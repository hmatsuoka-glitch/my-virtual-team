...
### 2026-07-12
- **「AVIF + WebP + PNG の 3 形式並列出力＋`<picture>` 自動生成」を 2026 業界標準として全案件必須化**：Meta が 2026 Q1 に AVIF 正式対応、LINE 広告も 2026 Q2 から AVIF 受入開始したため、`sharp(buf).avif({ quality: 78 })` `.webp({ quality: 82 })` `.png()` を 1 スクリプトで並列生成し、納品時に `<picture><source type="image/avif">...</picture>` の HTML スニペットも同梱。Indeed 150KB 上限案件で AVIF 88KB / WebP 118KB / PNG 148KB を同時納品でき、媒体選択の自由度と CDN 配信速度が同時向上。1 案件あたりのファイルサイズ平均 -47%、Yuna の媒体別再変換依頼が月 20 件 → 2 件に激減
- **「Playwright 1.52 のマルチブラウザ並列 screenshot」を Puppeteer から段階移行し WebKit 差異事故ゼロ化**：Chromium 単一エンジンの Puppeteer では検知できなかった iPhone Safari 特有の「Noto Sans JP `font-feature-settings` 差異」「AVIF レンダリング差異」を Playwright の WebKit エンジンで事前検出。同じ HTML を Chromium/WebKit の 2 エンジンで screenshot し `sharp` 差分抽出で 3px 以上のズレを警告。iPhone Safari での「バナー文字微妙にズレる」納品後クレームが月 3 件 → 0 件、テスト実行時間は Playwright 並列で +8 秒のみ
- **「AI セマンティック圧縮ツール Squoosh CLI + `sharp` パイプライン統合」で品質維持したまま追加 -35% 圧縮**：Google の Squoosh CLI が 2026 に CLI 版正式リリース、テキスト領域は無損失・写真領域は強圧縮の AI セマンティック圧縮を Node.js から実行可能に。`sharp` の後段に `@squoosh/cli --avif '{"cqLevel":33}' --wp2` を挟むことで、Indeed 150KB 案件の実測ファイルサイズが 148KB → 96KB に。テキスト判読性は tesseract.js OCR で 100% 維持を自動検証、超 Retina（`deviceScaleFactor: 3`）出力も現実解に
- **Kana / Rei / Yuna との「brand-tokens.json 中央リポジトリ」を GitHub Actions で自動シンクし色ズレ物理排除**：`brand-tokens/{client}.json`（HEX / フォント / ロゴクリアスペース / NG 表現）を GitHub リポジトリで一元管理し、Rei のコピー / Kana の HTML / Hiro の PNG 検証がすべて同一 JSON を参照。JSON 更新時に GitHub Actions が Slack `#banner-{client}` チャンネルへ差分通知＋自動 PR 作成、色違反バナーが月 12 件 → 0 件。LP 部（tsumugi）とも同 JSON を共有し、LP↔バナー↔SNS 投稿の世界観 100% 統一化
- **Vercel Image Optimization API + `next/image` `<Image>` タグ納品オプション追加で配信速度 +52%、Hiro 単価 +30%**：静的 PNG 納品に加え、Vercel の Edge Image Optimization を経由した「URL 納品（`https://cdn.example.com/banner.png?w=1080&q=82&fm=avif`）」を選択肢化。訪問者デバイスに応じて iPhone Retina=2160px AVIF / Android 中位=1080px WebP / PC=1080px PNG を自動配信、LCP が平均 2.1s → 1.0s に短縮。「動的配信」オプションを Yuna 経由でクライアントへ提案し、単価 +30% のプレミアム納品ラインを新設
- **予測モデル「Banner CTR Predictor v1（画像特徴量 CNN + テキスト BERT）」でクライアント CTR を提案時に定量提示**：過去 300 案件の PNG を CNN で特徴抽出（コントラスト・視線誘導・CTA サイズ比率）＋テキスト OCR＋BERT でコピー感情分析、Vertex AI で学習し「このバナー案の予測 CTR は 3.2%（媒体平均 1.8%）」と数値提示。Rei/Kana の 15 案から上位 3 案を CTR 順で Hiro が PNG 化する運用にし、Yuna 経由のクライアント承認が「なんとなく」→「データ根拠」で承認リードタイム 3 日 → 半日、実配信 CTR も業界平均 +85%
- **Claude Code + Puppeteer MCP による「HTML 受領→PNG 変換→Sora QA レポート」の 1 コマンド完全自動化**：Kana から Slack で HTML パス受領 → `claude "hiro variant client=清一建設"` 1 発で Puppeteer 起動、全サイズ AVIF/WebP/PNG 並列変換、sharp メタデータ検証、tesseract.js OCR 法務スキャン（nori 連携）、sharp コントラスト計算（WCAG 5:1 検証）、Notion に成果物 URL＋検証レポート自動投稿、まで完結。Hiro の手動作業時間 45 分/案件 → 4 分/案件、月 80 案件処理時の総工数 60 時間 → 5.3 時間に圧縮
