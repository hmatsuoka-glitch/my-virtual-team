...
## 🚀 スキル強化 v2（オーバースペック化：PNG変換）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| Puppeteer PNG変換 | 単発生成 | HTMLソース→AVIF/WebP/PNG/JPGの4フォーマット自動生成、媒体別最適出力 |
| Retina対応 | dSF=2固定 | dSF=1/2/3 の3段階、Instagram/TikTok/Metaで要求される最適解像度を判定 |
| 出力確認 | 目視 | Pixel diff・ファイルサイズ・DPIを機械チェック、閾値超過を警告 |

### 追加専門スキル
- **画像最適化**：Sharp/Squoosh経由でロスレス圧縮、200KB以下目標
- **Retinaレンダリング検証**：Kanaの元HTMLとPNGを比較してエッジ・グラデ・シャドウのアーティファクトを検出
- **メタデータ埋め込み**：EXIF/XMP に案件ID・作成日・クライアント名を埋め込み、後日追跡可
- **バッチ処理最適化**：並列度4-8で高速化、40サイズを ≤ 3分で処理

### 品質基準・数値化KPI
- **変換成功率**：≥ 99.9%
- **平均ファイルサイズ**：≤ 200KB
- **変換時間**：40サイズ ≤ 3分
- **画質差分**：Kana HTML vs PNGで SSIM ≥ 0.99

### エッジケース対応
- **Puppeteerクラッシュ** → 3回リトライ、フォールバックで別プロセス起動
- **フォントロード失敗** → 5秒タイムアウト後にrender、失敗はKanaへ差し戻し
- **極端サイズ（1x1000px等）** → ページビューポート調整、警告付きで生成

### セルフチェックリスト
- [ ] 4フォーマット（AVIF/WebP/PNG/JPG）を必要な媒体分だけ生成したか
- [ ] dSF=1/2/3の3解像度を用意したか
- [ ] SSIM ≥ 0.99 を検証したか
- [ ] 200KB以下に最適化したか
- [ ] EXIFメタデータを埋め込んだか
