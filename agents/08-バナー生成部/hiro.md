...
### 2026-08-30
- **Chrome for Testing 137系のGPU合成刷新に追従、backdrop-filter・mix-blend-modeの描画差分を回帰テストに追加**：Chromiumの新描画パイプラインでガラス質・ブレンド系のバナーがヘッドレスと実機で微差を出す事例が増加。`ref-images/`に前バージョンの基準PNGを固定し、pixelmatch差分を`@let-inc/banner-utils`のCIに組み込むと「更新翌日に7社分の再納品」が消える。効果例：Chrome更新起因の差し戻しゼロ／CI追加工数15分で月8時間の予防効果
- **Puppeteer BrowserContext並列を「サイズ数=並列度」で自動チューニング、150枚バッチが3分台に**：`browser.createBrowserContext()`で媒体別サイズをコンテキスト並列に流し、Node worker_threadsではなくブラウザ内並列で走らせるとRAM使用量を抑えつつ並列度を上げられる。7社×15サイズ=105枚が従来18分→3分40秒。効果例：バッチ処理80%短縮／深夜バッチのZombieプロセス消滅
- **Claude 5 Vision API連携で「書き出し後の自動視認QA」を導入、豆腐文字・輪郭消失・被り検出を機械化**：出力PNGをClaude 5に「求職者が3秒で読めるか／CTAが背景に埋もれていないか／文字が潰れていないか」を判定させ、NGだけKanaへ差し戻す。tesseract.js OCRでは拾えない「読めるが視認性が低い」を検出可能。効果例：目視QA工数70%削減／視認性クレーム月3件→0件
- **sharp v0.34系のAVIF Speed=6デフォルトでエンコード時間が半減、3形式同時出力の実運用コスト消滅**：libvips 8.16の最適化でAVIF書き出しが従来の1.8倍速に。PNG単体運用と3形式併産のバッチ時間差が15%以内に収まり、媒体別フォーマット選択が"コスト理由で妥協"されなくなった。効果例：AVIF併産の追加コスト40秒→8秒／月7社分で月90分短縮
- **Kana との「フォント埋め込み検証ゲート」共通化、`document.fonts.check()`をpreparePageに常設**：`document.fonts.ready`だけでは代替フォント描画のまま完了扱いになる罠を回避し、指定font-familyが実ロード完了かを`fonts.check('16px "Noto Sans JP"')`で明示検証。未ロード検出時は3秒リトライしてから書き出しへ。効果例：フォント代替による差し戻し月5件→0件／Kana側の追加負担なし
- **09-システム開発部 Mio との「決定性テスト」連携、同一HTMLの複数回変換でピクセル完全一致を保証**：Mio のTDDフローに乗せ、`banner-utils`のCIで同一HTMLを2回変換してSHA256一致を検証。フォント読込・非同期リソース・タイムスタンプ混入を全て排除する条件が固まり、共有ライブラリの信頼性がテストで担保される。効果例：ライブラリ改修時のリグレッション検出率95%／改修後Yuna通達の一報が"変更点のみ"で済む
- **納品直前の「実端末プレビューURL」自動発行で、クライアント確認を実機QRで完結**：出力PNG群をVercel Blob（Kuu管理）にpushし、7社別の一時プレビューページを生成してQRコード付きPDFで納品。クライアントがスマホで実機表示を確認できるため「PC画面では読めるがスマホで潰れる」を発見時点で解決できる。効果例：納品後の"スマホで見づらい"差し戻し月4件→0件／確認往復2日→当日
