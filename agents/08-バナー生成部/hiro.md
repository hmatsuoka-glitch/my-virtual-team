...
### 2026-08-08
- **効率化テクニック：ブラウザプール5並列＋案件シャーディングで20枚バッチを従来90秒→18秒（5倍速）**：常駐Chromiumインスタンス5本を `puppeteer.connect()` で保持し、20サイズを4枚ずつシャーディングして並列実行、`Promise.all` でなく `Promise.allSettled` で1枚失敗が他ジョブを巻き込まない設計を維持。CPU使用率は 60% で頭打ち、メモリは1インスタンスあたり300MB×5＝1.5GB上限。深夜200枚バッチが 15分→3分に圧縮され Yuna 翌朝チェック時に全案件揃う運用へ（2026-07-07記録の常駐ブラウザ運用を並列化で強化）
- **品質チェックポイント：`document.fonts.load()`＋`document.fonts.check()`の二重フォント確認で FOUT/FOIT 事故ゼロ化**：`preparePage()`内で使用ウェイト・ファミリを列挙して `await Promise.all(fontSpecs.map(spec => document.fonts.load(spec)))` を明示実行し、次に `fontSpecs.every(spec => document.fonts.check(spec))` で `true` を assert。`fonts.ready` だけだと `@font-face` declared だが未使用ウェイトは load 対象外の落とし穴があり、実描画で使うウェイトを名指し確認しないと FOIT で豆腐化する。Kana の `HIRO-CHECK` コメント `fonts-preloaded=` から使用ウェイト配列を機械抽出し二重検査へ渡す運用（2026-06-26記録の3連awaitを厳密化）
- **失敗パターン：バッチ変換中に1枚だけメモリリークで Chromium がクラッシュし、`Promise.allSettled` の rejected として拾えず「無応答のまま20分待機→タイムアウト」で全体バッチ遅延** → 回避策：各ジョブに `AbortController` ＋ `Promise.race([convert(), timeout(30000)])` の30秒タイムアウトを必ず付与し、超過時は該当プロセスを強制kill、`retry-failed.json` に該当ジョブを書き出して次バッチで再実行（理由：Chromiumクラッシュは無応答で返ってこないケースがあり、Promise.allSettled は正常/異常終了は拾えても無応答は拾えない）。実例：透過PNGで内部エラー発生→20分無応答→タイムアウトゲート導入後、遅延最大30秒に限定
- **用語再確認：DPR / deviceScaleFactor / PPI / DPI の4語をクライアント説明の共通語に**：DPR（devicePixelRatio）＝ブラウザが持つCSS論理px と物理px の比、deviceScaleFactor＝Puppeteer で DPR を疑似指定する引数、PPI（Pixels Per Inch）＝画面の物理密度（iPhone 15 Pro＝460PPI）、DPI（Dots Per Inch）＝印刷解像度（オフセット印刷350dpi標準）。Web配信は PPI/DPI概念は不要で「論理px＋deviceScaleFactor」だけで完結、印刷併用案件（提案書挿入）のみ CMYK＋350dpi 変換を追加。混同したまま「印刷向けに scale 5 で書き出して」等の依頼を受けると容量爆発、用語で線引きを明示する
- **連携ヒント：資料作成部（Yuto/Rin/Souma）との「提案書挿入用CMYK PNG＋16:9セーフエリア」共同ワークフロー確立**：Yuna から「提案書挿入用（印刷併用）」タグが来たら、Hiro が `sharp` → ImageMagick 経由で `-colorspace CMYK -profile USWebCoatedSWOP.icc` 変換を実行し、350dpi メタデータ付きPNG（`sharp.withMetadata({density: 350})`）とsRGB Web版の2セット納品。Souma（デザイナー）向けにはPowerPoint貼付想定の16:9セーフエリア座標（中央 90%）を validateBanner レポートに併記し、Rin（コンテンツ）へは「印刷で色が沈む可能性の高い彩度」を事前警告。資料作成部の後追い修正リクエストを構造的にゼロ化（2026-06-24記録のCMYK変換ワークフローを他部門連携化）
