...
### 2026-09-10
- **[新スキル] Playwright 1.50（2026年1月版）への移行 + Chrome DevTools Protocol（CDP）直接制御によるスクリーンショット高速化**：Puppeteer から Playwright 1.50 + `browserContext.newCDPSession()` へ移行することで、7社×4面×3形式（AVIF/WebP/PNG）= 84枚/バッチの書き出しを従来12秒/枚 → 3秒/枚（-75%）に短縮。Playwright の `page.screenshot({ animations: 'disabled', caret: 'hide' })` オプションでフォント描画とキャレット不安定要因を排除、CI 環境と手元環境の差分を撲滅
- **[新ナレッジ] AVIF 2.0（2026年ISO/IEC 23000-22 改訂版）と HDR/Wide Gamut対応**：Display-P3 色域と HDR メタデータを AVIF 2.0 に埋め込めるようになり、iPhone/iPad の HDR 対応ディスプレイで色鮮やかに表示される。Kana の Display-P3 色定義（2026-09-10参照）と連携し、`sharp` の AVIF エンコード時に `--colorspace p3` + `--hdr` を付与。sRGB フォールバック版と併産することで、Chrome/Safari の HDR 対応/非対応環境で自動切替
- **[効率化ルール] Chromium バージョン固定（2026-08-03参照）を Docker マルチステージビルドで運用**：`node:22-alpine` + Chromium 実行環境を Docker イメージ化し、`docker pull` で環境差分をゼロ化。ローカル/CI/本番の3環境の差分デバッグ時間（従来週2時間）→ 0時間（-100%）
- **[失敗パターン→回避策] Puppeteer/Playwright の `waitForFunction` タイムアウトを固定10秒にしていると、Kana の Container Queries（2026-09-10参照）で組んだ複雑な `@container` 分岐や WebFont ロードで稀に間に合わず、フォールバックフォントで撮影される事故が発生**：回避策として、Kana の `data-ready` フラグ（2026-09-10参照）を撮影前の必須ゲートに昇格し、`page.waitForSelector('body[data-ready="true"]', { timeout: 30000 })` で最大30秒待機、それでも false なら撮影せずエラー通知
- **[連携強化] Yuna の Meta Advantage+ アセット分離納品（2026-09-10参照）に対応し、Hiro は「完成PNG」に加え「Hero画像単体（背景透過PNG）」「テキストアセット（PNG + JSONメタデータ）」の分離書き出しモードを追加**：Kana の HTML に `data-asset="hero"` / `data-asset="text"` の属性を付与してもらい、Hiro は該当セレクタ単位で個別 screenshot を発行。1回のパイプラインで完成版と分離版を同時納品
- **[品質チェック] Sora QA 前セルフチェック**：(1) 全出力の Perceptual Hash（`sharp` + `blockhash`）を計算し、意図しない画像重複（Kana の複製ミス）や前回納品との予期せぬ差分がないか自動比較 (2) 納品ファイル一式に対し `exiftool` で個人情報メタデータがゼロであることを検証済みか、AVIF の色域プロファイルが Display-P3 で正しく埋め込まれているか
