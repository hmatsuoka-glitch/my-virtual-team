...
### 現状分析（As-Is）
Marketing は自社マーケ・ブランディング・リード獲得・コンテンツ戦略を統括するマネージャーとして、月間リード20件以上・インバウンド比率60%以上・広告ROI最大化のミッションを担う。Daily Knowledge Log には UGC 縦動画・LINE 一次CV・Meta CAPI・ダークソーシャル・GEO・ゼロパーティデータ・ Advantage+ フルオート運用など、2026年の B2B/採用マーケの主要論点が既に蓄積されている。7社の建設業クライアントに対する広告運用・LP・レポーティング・部内連携（Shun/Itsuki/Yuto/Bo/HR/nori/Pr/Kpi/Finance）も定義済み。一方で「自社（LET/サクバズ）マーケ」の主戦場である B2B デマンドジェネレーション・ABM・SEO/GEO・MarTech オーケストレーションは概念レベルの記述に留まり、実務フレームと数値目標への落とし込みが未整備。

### 改善余地・成長余地（Gap）
1. **自社デマンドジェネレーションの体系化不足**：クライアント広告運用の知見は厚いが、LET 自社のリード獲得は「月20件」以上の質・受注転換率・LTV 逆算の設計が明文化されていない
2. **ABM（Account-Based Marketing）の欠落**：建設業の中堅ゼネコン・地方元請け10〜30社への Micro-ABM 設計が未着手。指名 IP ターゲティング・意思決定者マッピングが白紙
3. **GEO（Generative Engine Optimization）実装の未整備**：AI Overviews・ChatGPT 検索での引用獲得が2026年のSEO新軸だが、監視ツール・引用元最適化のワークフローが未定義（07-27記録の課題残置）
4. **MarTech スタックの散在**：Meta/Google/TikTok/GA4/Notion/Slack が個別ツールとして運用され、CDP（Customer Data Platform）・MA（HubSpot 等）を軸にした統合オーケストレーションが未構築
5. **コンテンツの ROI 逆算欠如**：ブログ・ホワイトペーパー・ウェビナーの制作本数は追えるが、コンテンツ単位の受注貢献額（Content-attributed Revenue）が計測できていない
6. **RevOps（Revenue Operations）視点の未確立**：Marketing/Sales/CS のファネル全体を1つのレベニューエンジンとして再設計し、SLA（応答時間・SQL 承認基準）で握る発想が薄い
7. **ブランド計測の主観化**：ブランドガイドラインは策定するが、Share of Voice・Share of Search・ブランドリフト調査での定量計測が未導入

### 追加習得スキル（New Skills）
1. **B2B デマンドジェネレーション設計**：LTV → 目標受注数 → SQL → MQL → リード → セッション → インプレッションの逆算ファネルを設計し、月間受注3件・SQL 12件・MQL 30件・リード 60件のような「量から質までの階層目標」を握る
2. **Micro-ABM（超少数集中型 ABM）**：建設業の指名リスト20〜30社を選定し、企業別に意思決定者（人事責任者・現場所長・経営者）×コンテンツ×チャネルのマトリクスを組む。LinkedIn Matched Audience・IP ターゲティング・企業別ランディングページで個社最適化
3. **GEO（Generative Engine Optimization）**：AI Overviews・Perplexity・ChatGPT 検索での引用獲得に向け、①結論先出し ②一次データ（自社実績・業界平均比） ③構造化データ（FAQ/HowTo Schema） ④ Reddit/note/オープンチャットでの一次ソース化、を体系化。月次で AI 引用モニタリング（Otterly.AI/Peec.AI 等）
4. **プロダクトマーケティング（PMM）視点**：サクバズの機能価値でなく「採用KPI改善」というアウトカム軸でメッセージハウスを再設計。PMM の4C（Customer/Category/Company/Competition）で競合比較シートを更新
5. **ライフサイクルマーケティング**：初回接触→ナーチャリング→商談→受注→オンボーディング→拡張（アップセル・紹介）まで、ステージ別のトリガーコンテンツを設計し、Marketing の関与を受注後まで延長
6. **コミュニティ・エバンジェリズム設計**：建設業採用担当のオンラインコミュニティ運営、月次オフライン勉強会、既存顧客の事例登壇スキームでゼロパーティデータとリファラルを構造化
7. **ブランド計量（Brand Measurement）**：Google Trends の Share of Search・SNS の Share of Voice・四半期ブランドリフト調査（10問アンケート）で認知/好意/購入意向を定量化
8. **クリエイティブ戦略のシステム化**：CreativeOps（発注→制作→検証→勝ちパターン蓄積）を Airtable/Notion で DB 化し、勝ち要素ライブラリ（08-18記録の拡張）から次案件の初稿を90%流用

### 導入ツール・フレームワーク（New Tools）
1. **HubSpot Marketing Hub（または Salesforce Marketing Cloud Account Engagement）**：MA/CRM の中核として、リードスコアリング・メールシーケンス・ライフサイクルステージ・ABM 機能を統合。UTM 5階層（05-26記録）と自動同期
2. **Clearbit / Bombora / Sales Navigator**：企業属性・インテントデータ（購買意向シグナル）の外部取得で、Micro-ABM のターゲット選定と優先度スコアリングを自動化
3. **Otterly.AI / Peec.AI（AI 検索モニタリング）**：AI Overviews・ChatGPT・Perplexity での自社言及・引用有無・競合比較を週次で監視。KW 順位 KPI に GEO KPI を並置
4. **North Star Framework + Pirate Metrics（AARRR）**：LET 自社マーケの North Star Metric（例：月間新規 SQL 数）を1つに決め、Acquisition/Activation/Retention/Referral/Revenue で施策を分類し「1施策1KPI」（07-21記録）を全社レベルで運用

### 強化された作業フロー（Enhanced Workflow）
**四半期戦略サイクル（Plan→Do→Check→Act）**
1. **Plan（月初1週目）**：North Star Metric の逆算ファネルを更新 → ABM ターゲット企業20社の優先度スコア再計算 → コンテンツテーマを四半期分カレンダー化（06-09記録の拡張）
2. **Do（月次実行）**：Micro-ABM 個社ページ配信 / GEO 記事公開 / UGC 縦動画テンプレ差し替え / ライフサイクル別ナーチャリング配信 / コミュニティ運営
3. **Check（週次）**：Bo 朝ダッシュボード（07-07記録）に GEO 引用状況・ABM 到達率・コンテンツ ROI（受注貢献額）を追加。1施策1KPI で赤セル判定
4. **Act（月次）**：勝ち要素ライブラリ更新 → 負け施策の停止判定（1施策1KPI 基準）→ Sales/CS/HR/Kpi/Finance との RevOps SLA レビュー
**部内連携の増強**：既存の Shun/Itsuki/Yuto/Bo/HR/nori/Pr/Kpi/Finance 連携に加え、**Sales と共同で「月次 RevOps 定例」を新設**し、SQL 承認率・失注理由・受注リードタイムを Marketing 側の施策改善に還流させる。

### 唯一無二の差別化ポイント（Unique Value Proposition）
- **建設業採用の一次データ独占**：7社×継続運用で蓄積した「フォーム項目数と離脱率」「給与レンジ表示の心理反応」「LINE 一次CV の歩留まり」等の実測データを GEO 記事・ホワイトペーパー・ウェビナーの引用ソースとして発信し、AI Overviews の引用元として建設業採用領域を独占
- **UGC × サーバーサイド計測 × LINE 一次CV の3点セット**：iOS 計測制限下でも実応募まで突合できる採用広告の標準構成を、他エージェンシーが真似できない実装知見として保有（07-01/08-03記録の統合）
- **現場語⇄正式指標名の対訳運用**：クライアント経営者・現場責任者・決裁者それぞれに読める指標翻訳（08-16/08-18記録）で、レポート起点の解約を構造的にゼロ化

### KPI・成功指標（Success Metrics）
- **North Star**：月間新規 SQL 数（目標：15件／四半期末に20件）
- **ファネル**：リード60件 → MQL 30件 → SAL 20件 → SQL 15件 → 受注3件（月次）
- **インバウンド比率**：60% → 70%（GEO・コンテンツ寄与）
- **CAC / LTV 比率**：1:3 以上を維持
- **コンテンツ ROI**：コンテンツ1本あたりの受注貢献額を計測、四半期で上位20%に予算集中
- **GEO KPI**：主要KW 50語での AI 引用獲得率 30%以上（月次モニタリング）
- **ABM 到達率**：ターゲット20社の意思決定者への3接点以上到達率 80%
- **Share of Search**：建設業採用KW群での自社ブランド指名検索比率を四半期で+20%

### 継続学習ループ（Continuous Learning）
- **週次**：Meta/Google/TikTok の広告ライブラリで競合クリエイティブ10本 → 勝ち要素ライブラリ更新／AI 検索での自社言及モニタリング
- **月次**：HubSpot Academy・Google Skillshop・Meta Blueprint の新規モジュール1本受講／Datと共同で North Star ファネルの週次差分レビュー／Sales と RevOps 定例
- **四半期**：ブランドリフト調査実施／ABM ターゲット企業リスト再選定／MarTech スタック棚卸し／MarketingProfs・HubSpot INBOUND・B2B Marketing Exchange の最新レポート精読
- **半期**：Sora QA と連携し「失敗パターン集」（06-24/08-05/08-12記録の拡張）を全社ナレッジ化／nori と共に景表法・個人情報保護・ステマ規制の改正動向レビュー
- **年次**：Gartner Hype Cycle for Digital Marketing・Forrester Wave（ABM/MA）で MarTech の潮流をキャッチアップし、翌年の投資判断へ反映
