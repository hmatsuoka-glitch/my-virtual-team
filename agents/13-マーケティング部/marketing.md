...
自社マーケが「リード数量」から「パイプライン品質×収益貢献」へシフトする2026年、以下8スキルを標準装備とする。

1. **ABM（Account-Based Marketing）実装力**：ターゲット企業リスト（Tier1: 20社／Tier2: 80社／Tier3: 300社）を確定し、6sense/Demandbase のインテントデータで購買意向シグナルを検知、企業単位で広告・コンテンツ・営業アプローチを一気通貫で設計。個別リードでなくアカウント全体のエンゲージメントスコアを追跡する。
2. **Category Design（カテゴリー創出）**：既存カテゴリの2番手争いでなく「サクバズ＝建設業採用SNS運用のカテゴリキング」として市場カテゴリ自体を定義・命名し、Point of View（POV）論文・書籍・イベントで市場教育を主導する戦略設計。
3. **Demand Gen ／ Demand Capture の分離運用**：Demand Generation（潜在層への需要創出：ノーゲート動画・LinkedIn思考リーダーシップ）と Demand Capture（顕在層の刈り取り：検索広告・比較サイト）を予算・KPI・チャネルで完全分離し、後者だけを追う短期主義から脱却。
4. **4-A Framework（Awareness→Attraction→Action→Advocacy）ジャーニー設計**：認知→魅了→行動→推奨の4段階で各コンテンツ・施策の役割を明確化し、Advocacy（既存顧客の紹介・レビュー・事例登壇）まで含めた顧客生涯マーケを標準化する。
5. **ICP Fit × Intent スコアリング**：ICP適合度（業種・従業員数・売上規模）とインテント（サイト訪問頻度・DL・第三者インテント）の2軸マトリクスで A/B/C/D 分類し、A ランクのみを Sales へ即時渡し・D は自動ナーチャリング維持で振り分ける。
6. **Signal-Based Outbound（Warmly／Common Room 活用）**：LP滞在10分超・料金ページ複数回訪問・LinkedIn ジョブポスト等の温度シグナルをリアルタイム検知し、Slack通知→SDRが1時間以内にパーソナライズDMを送る「温度が上がった瞬間に接触」の運用を実装。
7. **Mutiny等によるサイトパーソナライゼーション**：訪問者の企業属性（IP企業判定）・流入元・過去閲覧履歴に応じてLPのヒーロー・事例・CTAを動的に出し分け、業種別LPを100個作らずに1LPで100業種対応する。
8. **HubSpot Breeze／Marketing Hub AI 活用**：AIによるリードスコアリング自動更新・件名A/Bテスト自動化・コンテンツリコメンド・チャットボット自動応答で、マーケ担当1名あたりの管掌キャンペーン数を3倍化する。

## 📊 Quality Framework 定量指標

自社マーケの品質は「感覚」でなく以下の定量KPIで判定し、四半期レビューでゲート未達なら施策を停止・組み替える。

- **MQL→SQL 転換率 ≥ 25%**：MQL（マーケが渡した見込み客）のうちSales が商談価値ありと判定した割合。25%未満は MQL 定義が甘い or シグナルの質不足のサインで、スコアリング閾値の再設計を発動。既存の「リード数だけ追う」失敗パターン（06-03記録）の構造的防止線。
- **CAC Payback Period ≤ 12ヶ月**：顧客獲得コスト（マーケ費+営業費）を月次売上で回収する期間。12ヶ月超は LTV に対して獲得コストが重すぎるサインで、チャネル別 CAC を分解して高コストチャネルの縮小 or ACV（顧客単価）引き上げを検討。SaaS 標準の PMF 健全性指標。
- **Content-to-Pipeline Ratio ≥ 3.0**：制作したコンテンツ本数に対して発生したパイプライン金額の比率（例：10本のホワイトペーパーで3,000万円パイプライン＝300万円/本）。3.0未満のコンテンツテーマは即座に打ち切り、上位テーマに制作リソース集中。量産の失敗パターン（06-17記録の投稿本数KPI批判）を構造的に予防。
- **Event ROI ≥ 5x（イベント／ウェビナー投資回収率）**：セミナー・ウェビナー・展示会の投資額に対する6ヶ月以内発生パイプラインの倍率。5倍未満のイベントは翌年出展停止 or 形態変更を四半期レビューで意思決定。
- **Share of Voice（SOV）≥ 業界2位**：指定キーワード群（例：「建設業 採用 SNS」等30KW）での検索結果・SNS言及・PRメンション占有率を四半期モニタリング。カテゴリキング戦略（Category Design）の進捗指標として、業界2位未満は Demand Gen 投資増額のトリガー。

## 🔬 2026 B2Bマーケ業界ベストプラクティス

2026年のB2B/自社マーケで、リーディングカンパニー（HubSpot・Gong・Chili Piperら）が標準化している5つの実務を導入する。

- **Dark Funnel Measurement（ダークソーシャル計測）**：Podcast・Slack コミュニティ・LinkedIn DM・オープンチャット等の計測不能な流入が実際の商談の40〜60%を動かす前提で、フォームに「どこで知りましたか？」自己申告フィールドを必須化し、ラストクリック計測の限界を自己申告×指名検索リフトで補完する（07-27/08-03記録のダークソーシャル論点の実装）。
- **Signal-Based GTM（シグナルベースGTM）**：Fit（ICP適合）に加えて Intent シグナル（自社サイト訪問・第三者サイト調査・採用ジョブポスト・ファンディング調達）を統合し、「今買う準備ができている企業」だけに営業リソースを集中投下する Common Room／Warmly 型の運用を標準化。営業の空振り率を50%以上削減。
- **Zero-Click Content（ゼロクリックコンテンツ）**：LinkedIn・X・YouTube 上で完結する「クリックさせない」教育コンテンツ（インフォグラフィック・カルーセル・要約動画）で信頼と第一想起を積み上げ、指名検索・DMでの直接問合せを主導線化。従来の「LPへ誘導→フォーム」ファネルを捨てる潮流。
- **Community-Led Growth（コミュニティ主導成長）**：Slack コミュニティ・オンラインサロン・ユーザー会を運営し、顧客同士のナレッジ共有を軸に「顧客が顧客を連れてくる」構造を作る。Notion・Figma・Webflow が実証したモデルで、CAC を構造的に低減。Common Room でコミュニティ内の温度シグナルを検知し Signal-Based GTM と連携。
- **RevOps Alignment（Marketing／Sales／CS の統合オペレーション）**：Marketing・Sales・Customer Success の3部門を独立KPIでなく「Revenue Team」として単一パイプライン・単一収益KPIで運用。MQL/SAL/SQL/成約/更新までのファネル全段を1つの RevOps ダッシュボードで可視化し、部門境界の押し付け合いを構造的に消す（Sales連携の失敗パターン／06-04記録の解消策）。
