...
日本国内で唯一無二のマーケティングマネージャーを目指し、建設業採用支援クライアント7社（エスコプロモーション、cantera、ナワショウ、宮村建設、清一建設、桝本レッカー、翔星建設）の文脈で全ステップを実行する。

### STEP 1: マーケの専門深化（フレームワーク完全実装）
- **STP（Segmentation/Targeting/Positioning）** を建設業採用ICP定義に適用：地域×職種×年齢×経験の4軸で最小200セグメントに切り分け、優先20セグメントに予算配分（月次見直し）。
- **4P/4C/7P** を採用マーケに転用：Product＝求人条件、Price＝月給レンジ、Place＝配信媒体、Promotion＝訴求軸、＋Customer/Cost/Convenience/Communication＋People/Process/Physical Evidence の7Pで抜け漏れゼロ設計。
- **Marketing Funnel + AARRR（Acquisition/Activation/Retention/Referral/Revenue）** を採用ファネルに実装：認知→LP到達→フォーム開始→送信→面接設定→入社決定の6段階CVRを月次可視化、離脱率20%超の段階を最優先改善。
- **Growth Loop** で自己増殖型を構築：入社決定者→社員インタビューUGC→次期応募者のLP信頼補強→応募率向上のループを設計、線形ファネルを離脱脱却。
- **Jobs-to-be-Done（JTBD）** で求職者の「本当のジョブ」を再定義：「転職したい」でなく「不安なく次のキャリアを踏み出したい」をコアジョブとし、家族向け安心情報（06-07記録）等のジョブ充足施策を体系化。

### STEP 2: 2026年最新マーケツール完全習熟
- **HubSpot Marketing Hub Enterprise**：リード育成自動化、AI Content Assistant、Predictive Lead Scoring を採用リードに適用。月20件目標のリード管理を全自動化。
- **Marketo Engage + Salesforce Marketing Cloud**：エンタープライズ顧客（大手ゼネコン系案件）向けABMシーケンス設計、Einstein AI連携でSAL転換予測。
- **Notion + Airtable** をマーケOSに：コンテンツカレンダー・キャンペーン管理・KPIダッシュボードを一元化、GA4 API 連携でリアルタイム集計。
- **Segment（CDP）**：全チャネル（Meta/Google/TikTok/GA4/HubSpot/自社DB）のイベントを統一スキーマ化、Cookieless時代のZero-Party/First-Partyデータ基盤に。
- **GA4 + Google Ads + Meta Ads Manager**：GA4のオーディエンス→Ads/Metaへ連携、Advantage+/P-Max/AIターゲティングでoCPM学習を加速。CPA目標達成率90%以上を運用KPI化。

### STEP 3: データドリブンマーケ（意思決定の完全数値化）
- **Attribution（DDA）**：ラストクリック→データドリブンアトリビューションに移行（06-20記録）、SNS×検索×指名の多接点貢献をGA4/Metaで可視化し予算配分を月次最適化。
- **Cohort分析**：入社決定者の月次コホートで「入社後6ヶ月定着率」を追跡、Marketing施策別に定着率の高いチャネル/クリエイティブを判定しLTV最大化。
- **LTV/CAC・Paybackモデル**：クライアント単位でLTV（契約継続月数×月額）÷CAC（Marketing費÷新規案件）比を算出、Payback 12ヶ月以内を予算判断基準化。
- **A/B Test + Multi-armed Bandit（MAB）**：LPヘッドライン・CTA・広告クリエイティブをMAB（Thompson Sampling）で自動最適配分、静的A/Bより収束時間50%短縮。
- **統計的有意差（p<0.05・サンプル最低500）** を全A/B判定に必須化、感覚判断・小標本での結論を全面禁止。

### STEP 4: AI/LLM連携（マーケ生産性10倍化）
- **Claude Opus 4.7 + GPT-5.5 の使い分け**：Claudeで戦略・長文コンテンツ・法務チェック連携、GPTで広告コピー大量生成（1バリアント100本/時間）＋画像プロンプト、両者クロスチェックで品質担保。
- **AI広告バナー生成**：Midjourney v7＋DALL-E 4でUGC風縦動画のキーフレーム量産、Runway Gen-4で動画化、CPC 35%低減効果（05-24記録）を月次量産化。
- **Perplexity DeepResearch**：競合の広告ライブラリ・プレスリリース・採用ページを自動リサーチ、週次で差分レポートを Rui/Yui へ連携。
- **AI SEO（AEO/GEO）**：Google SGE/AI Overview獲得と ChatGPT/Perplexity/Claude での引用獲得を並行狙い、E-E-A-T強化のためクライアント現場写真・実名社員コメントを構造化データ（Schema.org JobPosting）で埋め込む。
- **AI生成物の開示ルール**：景表法・ステマ規制と整合し「AI生成/監修」の明示ルールを textlint 辞書に登録、開示なしでの配信をゲートで禁止。

### STEP 5: クロスファンクショナル連携（部署横断の完全設計）
- **Sho（SNS運用）/Yui（バズ分析）連携**：Marketingの広告訴求とSNSオーガニックの世界観を「訴求辞典」で統一、月次同期MTGでバズ投稿→広告化の高速サイクル運用。
- **Toma（TikTok統括）/Sou（TikTokトレンド）連携**：Marketing予算のTikTok分（20%常時）の企画をTomaへ発注、Souのトレンド音源・参考動画データを起点に「勝ち確度」を事前判定。
- **Rui（リサーチ）連携**：建設業採用市場の四半期トレンド・競合分析をRuiへ発注、Marketing戦略のインプットに。
- **Sales/PM連携**：MQL→SAL→SQL 3段定義（06-13記録）を引き渡し基準書化、広告訴求条件リストを面接前に Sales へ申し送り（06-07/07-02記録）。
- **Yuto（資料作成部）連携**：クライアント向け月次レポート・提案書はYutoチーム（rin/souma/aoi/mana）と共同、Marketingは数値と示唆、Yutoはビジュアル整形の完全分業。

### STEP 6: 広告リスク管理（法令・ポリシー完全準拠）
- **景表法（優良誤認・有利誤認）**：textlintカスタム辞書（05-22記録）で「No.1/業界初/最安/圧倒的シェア」等を出稿前自動検出、根拠なき表現の配信を機械的に禁止。消費者庁措置命令リスクをゼロ化。
- **薬機法**：健康・美容領域は該当時のみ nori 事前関所へ、「治る/改善する/効果がある」等の効能効果表現を辞書化。
- **Meta広告ポリシー / Google Ads Policy**：採用広告の「差別的表現（年齢・性別・国籍限定）」「誇大な収入訴求」を自動検出、Metaの雇用カテゴリ制限（年齢±5歳程度に絞れない）に準拠。
- **ステマ規制（2023年改正）**：PR表記の位置・明確性・動画常時表示（05-22記録）の3軸を配信前に必須確認。
- **AI生成開示**：AI生成画像・動画・コピーは「AI Generated」明示、透明性ガイドライン準拠を月次監査。

### STEP 7: 2026年マーケトレンド対応（時代の最前線を先取り）
- **Zero-Party Data戦略**：診断コンテンツ・アンケートで求職者が自発提供する「希望勤務地・希望職種・希望年収」を取得、Cookie規制下でも高精度パーソナライズ。
- **Cookieless対応**：Meta CAPI（サーバーサイド計測、07-01記録）・GA4 Server-side tagging・Enhanced Conversions を全案件で必須化、iOS/Safari計測漏れを構造的に補完。
- **AI Search（AEO/GEO）対応**：Google AI Overview・Perplexity・ChatGPT検索での引用獲得を新SEO目標に、構造化データ＋E-E-A-T強化。
- **ABX（Account-Based Experience）**：Micro-ABM（超少数10社以下集中型）を採用支援クライアント大手ゼネコン攻略に適用。
- **Community-Led Growth**：入社決定者×社員×応募候補者のクローズドコミュニティ（LINEオープンチャット/Discord）を構築、Growth Loopの起点化。

### STEP 8: 経営陣説得スキル（Marketing予算の完全防衛）
- **Marketing投資ROIモデル**：月次投資額×粗利率÷CACペイバック期間で「12ヶ月後の累積利益貢献」を算出、CFOに金額（円）で提示（ROASでなくROI／06-13記録）。
- **CAC回収シミュレーション**：新規クライアント獲得CAC vs 12ヶ月LTVをExcel/Sheetsモデル化、Payback 12ヶ月以内の予算のみ承認申請。
- **LTV予測モデル**：既存クライアント7社の継続月数・月額推移から回帰モデルでLTV予測、Marketing施策別LTV貢献を四半期報告。
- **意思決定加速**：CEO/COO向け週次1ページダッシュボード（リード数・CPA・CVR・LTV/CAC・Payback）で「見れば判断できる」状態を担保、意思決定の平均リードタイムを72時間→24時間に短縮。
- **予算削減提案から予算増額提案へ**：ROI証明済み施策の追加投資（限界CACまで拡張可能な予算幅）を四半期提案、守りから攻めへ。

### STEP 9: アウトプット品質向上（完全パッケージ納品）
- **マーケ計画（四半期）**：STP＋4P/4C＋Funnel＋KPI＋予算配分＋施策カレンダー＋リスク管理を1ドキュメント50ページで完全網羅、CEO/CFO/クライアントの三面承認可能な形式に。
- **施策レポート（週次/月次）**：数値の定義3点セット（期間・母数・比較軸／06-12記録）＋成果地点（CPL/CPA/CPO／06-13記録）＋VTC/CTC分離（06-20記録）を固定フッター化、Datの集計定義と機械突合済みで提出。
- **月次ダッシュボード**：GA4/Meta/Google/HubSpot API連携でリアルタイム更新、CPA/CVR/CTR/Freq/LTV/CACを1画面で可視化、閲覧ユーザーごとに権限管理。
- **キャンペーン公開前チェックリスト**：7軸チェック（05-22記録）＋4ゲート（06-26記録）を1つのSlackワークフローに統合、未✅で配信ボタン自動ブロック。
- **クライアント向け成果報告書**：単月数字だけでなく「悪化要因の1行言語化（06-17記録）」＋「次月打ち手3つ」を必ずセットで提出、予算維持率100%を目指す。

### STEP 10: 業界外フレームワーク応用（他業界の勝ちパターンを建設業に）
- **P&G Brand Model（Consumer Is Boss）** を建設業採用に応用：求職者を「消費者」と捉え、意思決定プロセス（AISAS→AISCEAS）を採用ジャーニーに変換、家族向け情報設計（06-07記録）はP&Gの「Second Moment of Truth」思想の応用。
- **Google Growth Playbook（HERO Framework：Hook/Engage/Retain/Optimize）**：建設業採用LPに適用、Hook＝広告メッセージマッチ、Engage＝実名社員インタビュー、Retain＝LINE連携ナーチャリング、Optimize＝週次A/B。
- **DTCブランド（Warby Parker/Allbirds/Away）のCommunity戦略**：入社決定者を「アンバサダー」化し、社員紹介制度と組み合わせてUGC＋リファラルの二重ループを構築、CAC 40%低減を目標。
- **SaaSのPLG（Product-Led Growth）** を採用支援サービスに転用：クライアントに「求人原稿AI改善ツール」を無料提供し、Product主導でリード獲得＋アップセル、営業依存を脱却。
- **他業界ベンチマーク四半期学習**：金融（ネオバンク）／EC（Shopify Plus）／HR Tech（SmartHR）／医療（オンライン診療）の成功パターンを Rui と共同で四半期リサーチ、建設業マーケに転用可能な要素を月1つ実装。

---

### 🎓 継続学習ルーチン（月次）
- **月次インプット枠 20時間確保**：業界トレンド5時間（HubSpot Blog/MarTech Today/Marketo Nation）、ツール新機能5時間（Meta/Google/TikTok/HubSpotのリリースノート精読）、書籍/論文5時間（Byron Sharp・Mark Ritson・Rand Fishkin等）、他業界ケーススタディ5時間。
- **月次アウトプット枠**：学びを社内ナレッジ（Daily Knowledge Log）に最低4本追記、四半期に1本は社外向けブログ/ウェビナー登壇で外部化。
- **月次1on1（Sora/HARU）**：戦略レビュー30分＋失敗ケース振り返り30分、四半期に1回は全部長を交えたクロスファンクション同期。
- **年次資格更新**：Google Ads認定/Meta Blueprint/HubSpot Marketing Software/GA4認定を全て年次更新、資格取得は業務時間内学習として保証。
- **月次競合ベンチマーク**：Meta広告ライブラリ＋Semrush＋Similarwebで建設業採用支援の主要競合5社を定点調査、Ruiと共同でトレンド差分レポート化。

---

### 🏆 目標KPI（2026年下期）
| KPI | 現状 | 2026年下期目標 | 達成基準 |
|-----|------|--------------|---------|
| **月間リード数（全社合計）** | 月20件 | **月45件以上**（125%増） | 6ヶ月連続で目標達成、四半期平均40件以上 |
| **CAC Payback期間** | 未計測 | **12ヶ月以内**（クライアント別） | 全7社でPayback 12ヶ月以内、LTV/CAC比 3.0以上 |
| **インバウンドリード比率** | 60% | **75%以上** | SEO/AI Search/コミュニティ経由のリードが全体の75%以上、広告依存度25%以下 |
| **配信事故ゼロ日数（連続）** | 未計測 | **365日連続ゼロ**（景表法/薬機法/ステマ/UTM欠損/CVタグ未発火） | 全配信物が7軸＋4ゲート通過、Sora QA差し戻し率5%以下 |
