...
### 2026-08-30
- **Cookie廃止（Chrome 2026年後半段階廃止）後の"ゼロパーティ＋ファーストパーティ"データ戦略を四半期でロードマップ化**：診断コンテンツ・アンケート・LINE友だち・メルマガの4層で自社データを積み上げ、Meta/Google の Advantage+/P-MAX へ Conversions API（CAPI）経由で送信する運用を標準化。3rd Party Cookie 依存の広告 CPA 悪化を先回りで吸収し、CPA を現状比 -25% に維持。
- **Claude 5 系による広告クリエイティブ自動生成→ A/B テストの高速サイクル化**：訴求軸×フォーマットのテスト設計表（08-18記録）を Claude に渡すと、テロップ案・キャッチコピー案を1案件あたり20本自動生成し、itsuki が実写素材と組み合わせるだけで配信開始。クリエイティブ制作リードタイムが5営業日→1営業日に短縮、テスト本数を月10本→月40本に増やせる。
- **UGC 型縦動画の"クライアント現場社員起用スキーム"を標準化**：現場責任者→若手職人→総務担当の3層で30秒縦動画を撮り、環境音・作業着の汚れ・言い淀みを残す原則（08-16記録）で編集する。CPA が整った作り込み動画比で -40%、応募後辞退率も -30% で歩留まりを両立。
- **AI検索（SGE/Perplexity/ChatGPT検索）対応の"AEO"（Answer Engine Optimization）を SEO 施策に組込**：クライアント成功事例・業界データ・FAQ を構造化データ（JSON-LD の HowTo/FAQ Schema）で徹底マークアップし、AI検索の引用ソースになるよう最適化。指名検索・AI 経由の流入を四半期で +50% を目指す。
- **アトリビューションを "ラストクリック→ Data-Driven（DDA）→ MMM（マーケティングミックスモデリング）" の3層で運用**：ラストクリックの限界（08-11記録）を超え、Google Ads/Meta の DDA と Robyn（Meta OSS の MMM）で初期認知施策の貢献を数値化する。予算配分の意思決定精度を上げ、SNS×検索併用案件の ROI を年間 +30% 改善。
- **リード育成メールを Claude 5 で"1to1個別化"し、開封率・CVRを引き上げる**：業種・規模・過去ダウンロード資料を条件に Claude が個別の件名・導入文・CTAを毎回生成し、HubSpot 経由で自動配信。開封率30%→50%、SQL 転換率5%→12% を実現。
- **セミナー・ウェビナーを"月2回・オンデマンド常時視聴可"の二段構えで恒常化**：ライブ回でリアクション獲得、録画をゲート付き資料として常時配布して MQL を積む運用にし、月次リード数を20件→60件に増やす。CPA も広告依存より低く抑える構造を作る。
- **オフライン施策（イベント出展・現場訪問・紹介会）と Meta CAPI を連携**：オフラインで獲得したリードを CRM 経由で Meta CAPI に還元し、類似オーディエンス学習の材料にする。オンライン単独の伸びしろが尽きた領域で新規開拓 CPA を -20% で継続的に下げる。
