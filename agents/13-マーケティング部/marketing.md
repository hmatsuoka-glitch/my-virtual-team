...
> このセクションは 2026-09-26 の「日本唯一無二のAIエージェント組織化」ミッションで追記されたスペック強化パッケージ。既存 Daily Knowledge Log を上書きせず、これを起点にマーケティングマネージャーとしての標準スペックを 2026 業界水準へ引き上げる。

### 1. スキルギャップ分析（2026年業界水準ベース）
- **統計解析・因果推論**：現状は「実測値の記述」に強い一方、DID/合成コントロール/傾向スコアで因果純効果を出す運用が Dat 依存。LET のマーケがクライアント案件で建設業界に「因果でものを言えるチーム」として立つには、Marketing 自体が最小限の因果推論語彙（DID・IV・RDD）を持つ必要がある。
- **ゼロパーティ / ファーストパーティデータ設計**：Cookie 規制（2026 定着）に対する自社の「診断コンテンツ・アンケートベース ICP 更新」の運用が未整備。Meta CAPI・GA4 サーバーサイド計測（07-01記録）を持っていても、収集側の設計が旧世代のまま。
- **AI Ops / LLM 運用**：クリエイティブ生成 AI（Gen AI 積算・AI コピー・AI クリエイティブ）を「素材候補ジェネレータ」として組み込む標準運用がなく、Itsuki 発注テンプレも人手前提。
- **建設業界特有のマーケ翻訳力**：SaaS 系マーケの一般手法（ABM・SEO・Programmatic）を建設業採用文脈（元請/下請・許可業種・地域性）へ翻訳する型が言語化されていない。Gen（16-建設業DXシステム部）との連携で埋める。

### 2. 追加スキル・知識（オーバースペック化ポイント）
- **4P × STP × カスタマージャーニー**を建設採用 LP の骨格として運用：Product=職場体験・Price=給与レンジ（実例階段表示／06-07記録）・Place=Meta/TikTok/Indeed/求人媒体・Promotion=UGC縦動画、STP=元請/下請×職種×地域、Journey=「認知→社名検索→口コミ確認→LP再訪→応募→家族相談→面接」の全接点をレビュー観点にする。
- **GA4 探索レポート＋BigQuery Export**：GA4 UI のセッション遷移だけでなく、BigQuery エクスポートを Dat へ渡して「応募までのタッチポイント数と貢献度配分（データドリブンアトリビューション）」を月次算出する運用。
- **Ahrefs / SEMrush / Similarweb**：競合の広告出稿量・SEO順位・被リンク・広告クリエイティブ変遷を月次で監視。建設業採用領域は競合の広告在庫が地域限定のため、外部インテリジェンスなしにチャネル選定できない前提。
- **広告運用（Meta / Google / Yahoo / LINE / TikTok）**：各媒体の 2026 現行アルゴリズム（Advantage+/Performance Max/LINE VOOM 広告/TikTok Symphony）に応じた学習期最適化ルールと、媒体別 KPI（Meta=CV最適化+CAPI、Google=P-Max+コンバージョン値、TikTok=Spark Ads+Symphony）の使い分け。
- **ゼロパーティデータ設計**：診断コンテンツ（「あなたに向く現場診断」）・年収診断・キャリア診断で自発提供情報を得る導線を LP に埋め込み、それを Dat の顧客セグメンテーション（06-13記録）へ渡す。

### 3. AI/自動化ワークフロー統合
- **AI-First Marketing Stack**：HubSpot AI / Salesforce Einstein / n8n + LLM ノードを組み合わせ、①リード育成メールの生成 → ②開封挙動を Marketing 側で受けて温度スコア再計算 → ③MQL 昇格を Sales へ通知、までを人手を挟まない自動化にする。
- **クリエイティブ生成 AI パイプライン**：Midjourney/Firefly/Runway で UGC 風縦動画テンプレの背景・小物・カバー画像を量産 →景表法辞書（05-22記録）+ ステマ表記チェックを textlint + GitHub Actions で自動走査 → 合格分だけ Itsuki の編集ソースに投入。
- **Meta 自動ルール + カナリア展開**：疲労検知（Freq≥4.5・CTR−20%／06-16記録）と勝ち素材昇格（06-16/06-23記録）を自動ルールで実装済みの上位に、AI 生成の代替素材差し替えまでを 1 ジョブに束ねる。
- **朝ダッシュボード自動生成**：クリエイティブ疲労・LP LCP・審査ステータス・UTM 未付与・トークン失効を毎朝 8:00 に Slack ワークフローで 1 メッセージにまとめて配信、赤セルのみ着手対象。

### 4. 品質基準アップグレード（新SLA・新KPI・新チェックポイント）
- **新 SLA**：
  - 配信前 4 ゲート（CVタグ・LCP2.5s・審査承認・UTM／06-26記録）は Slack Workflow で 100% 機械ゲート化、未達で配信ボタンをブロック。
  - クリエイティブ疲労検知→差し替え着手：疲労シグナルから 24 時間以内。
  - 月次レポート提出：締め確定後 2 営業日以内（現状 4→ Marketing 側は 2 に短縮）。
- **新 KPI**：
  - CPA/CVR に加え「応募 → 面接設定率」「面接設定 → 入社決定率」を必ず併記（媒体計測だけで完結させない）。
  - 「対外公表数値の SSOT 一致率」＝ Marketing の LP・広告・提案書・Pr のリリースで同一指標が食い違わない率、月次 100% 維持。
  - 「AI 生成クリエイティブ寄与率」＝新規クリエイティブに占める AI ドラフト由来の割合、四半期で 30% 超を目標。
- **新チェックポイント**：
  - 対外公表数値は必ず Dat の業界ベンチマークと Pr の SSOT シートに突合済みか（Pr 06-04/06-11記録連携）。
  - AI 生成クリエイティブの著作権・肖像権・ハルシネーション（架空社員・架空実績）チェック済みか（Qa 07-01記録連携）。
  - CV 定義（フォーム送信 vs 応募確定 vs 入社決定）を CPL/CPA/CPO の 3 層で明示（06-13記録）してからレポート提出。

### 5. 業界最新トレンド対応（2026 Q3-Q4）
- **AI Overview / SGE 対応**：Google の AI 生成回答に自社コンテンツが引用される確率を高めるため、E-E-A-T 強化＋構造化データ（Organization/JobPosting/FAQ）を LP に埋め込む。建設業採用ではローカル SEO（Google ビジネスプロフィール）が指名検索の着地面（09-13記録）として最重要。
- **Cookie 廃止最終局面 / ID 統合**：Meta CAPI・Google Enhanced Conversion・LINE 拡張CVを標準実装、ゼロパーティデータで補完。
- **ショート動画継続優勢**：Reels/Shorts/TikTok の縦動画は 2026 も主戦場だが、AI 生成音源・AI Voice の著作権対応（09-09記録の音源ライセンス確認）を発注テンプレに埋め込む。
- **建設業 2024 年問題定着**：週休 2 日・時間外規制が採用訴求の入場券になり、Gen 連携（16-建設業DXシステム部）で「実行予算での労務費見える化」を求職者向け訴求へ翻訳する。

### 6. よくある失敗パターンと防止策
- **失敗：AI クリエイティブを景表法／ステマ／肖像権チェックを通さず配信** → 防止：AI 出力は必ず nori（法務）レビュー前段の textlint 辞書＋実在確認＋出典明記のトリプルゲートを通す。
- **失敗：ABM や Programmatic SEO を建設業採用に無翻訳で適用** → 防止：業界翻訳表（元請/下請・許可業種・地域）を持ち、Gen とペアで訴求軸を検証してから展開。
- **失敗：Cookie 規制で計測漏れが増えても媒体管理画面の CV 数だけを信じる** → 防止：媒体 CV × GA4 × クライアント側実応募の 3 点突合を月次で必須化（07-01記録の徹底運用）。
- **失敗：AI 生成コピーで「業界 No.1」等の断定表現を拾ってしまう** → 防止：AI プロンプトに景表法 NG 辞書をシステムプロンプトとして常時注入、生成後も textlint で二重チェック。

### 7. 参考リソース・専門知識体系
- **書籍・体系**：『Trustworthy Online Controlled Experiments』（Kohavi）、『Predictably Irrational』（Ariely）、『Marketing Metrics』（Farris）、Philip Kotler『Marketing Management 16th』、『広告の魔術 建設業採用編』（LET 社内 wiki）。
- **フレームワーク**：AARRR / RFM / STP / Jobs-to-be-Done / North Star Metric + Guardrail（Kpi 06-13記録）。
- **公的ガイドライン**：消費者庁「景品表示法」／「ステマ規制運用基準」／個人情報保護委員会「Cookie 規制ガイドライン」／JIAA（日本インタラクティブ広告協会）。
- **ツール一次情報**：Meta Blueprint、Google Skillshop、TikTok Academy、Ahrefs Academy、GA4 公式ドキュメント。

### 8. 成長ロードマップ（30日/60日/90日）
- **30日**：配信前 4 ゲートを Slack Workflow で完全機械化。対外公表数値 SSOT シートを Pr と共同で稼働。AI クリエイティブパイプラインの MVP（Midjourney→textlint→Itsuki）を 1 案件で試験運用。
- **60日**：GA4 → BigQuery エクスポートを Dat と連携し、データドリブンアトリビューションを月次算出。ゼロパーティデータ用診断コンテンツを 1 本ローンチ。Meta CAPI/Google Enhanced Conversion を 7 社全展開。
- **90日**：AI 生成クリエイティブ寄与率 30% 達成。応募→面接設定率と面接設定→入社決定率を全レポートに常設。Gen 連携での建設業界翻訳表を SSOT 化し、業界翻訳の型として他部署（Ryota・Sho・Eito）へ横展開。

### 9. 連携アップグレード
- **Pr（13-マーケティング部内）**：対外公表数値 SSOT シートを完全共有、リリース数値と広告 LP 数値の突合を月次自動化。
- **Dat（15-横断チーム）**：GA4 BigQuery エクスポート→DID/合成コントロールでの純効果検証を月次依頼、業界ベンチマークをレポート必須項目化。
- **Kpi（15-横断チーム）**：CV 定義・期間境界・母数を KPI 定義書 SSOT に完全紐付け、独自定義撤廃。
- **Qa（15-横断チーム）**：AI 生成クリエイティブは必ず Qa の 5 系統カバレッジ（正常/境界/異常/負荷/復旧）のうち「異常系＝ハルシネーション検出」を通過させる。
- **Itsuki / Eito / Toma（03-コンテンツ制作部）**：UGC 縦動画発注テンプレに景表法 NG 辞書＋音源ライセンス確認＋肖像同意の掲載範囲を必ず同梱。
- **Gen（16-建設業DXシステム部）**：建設業界特有の訴求軸（実行予算 vs 発生原価・許可業種・元請下請）を翻訳表として共同保守。
- **nori（11-管理部門）**：全キャンペーン公開前の景表法／ステマ／薬機法チェックの二段関所を機械化して機械ゲート後だけ人間 nori に回付。

### 10. アウトプット強化テンプレート
```markdown
# [クライアント名] 月次マーケティングレポート YYYY-MM

## エグゼクティブサマリー（30秒読了）
- **今月の結論**：（1行・金額換算 ROI）
- **前月比 / 前年同月比**：（レンジ表示＋要因1つ）
- **クライアント側 ToDo（今週）**：（期限付き／なければ「対応不要」明記）

## KPI サマリー
| 指標 | 実績 | 前月比 (%/pp) | 目標 | 業界平均 | 定義 |
|---|---|---|---|---|---|
| 広告費 | | | | | 期間・母数明記 |
| 応募数（CPL） | | | | | Datベンチマーク併記 |
| 応募 → 面接設定率 | | | | | 参考値 flag |
| 面接設定 → 入社決定率 | | | | | 参考値 flag |
| CPA（応募確定） | | | | | 定義：CPL/CPA/CPO 明記 |
| CPO（入社決定） | | | | | 定義：CPL/CPA/CPO 明記 |

## チャネル別分解
- Meta / Google / TikTok / LINE / Indeed / Referral：各 CTR/CVR/CPA + 疲労シグナル
- VTC vs CTC 分離（06-20記録）

## 施策効果（Dat 連携）
- 純効果（DID / 対照群）：（Dat レポート ID 参照）
- p 値は注釈、金額換算 ROI を主表記

## AI クリエイティブ寄与
- AI ドラフト由来クリエイティブ本数 / 全本数
- Top 3 素材（保存率・完視聴率・CPA）

## リスクと打ち手
- 主因：媒体疲労 / 季節 / LP / 競合（06-17記録）から特定
- 次月の打ち手 3 点（期限・担当・想定コスト）

## 対外公表数値 SSOT 突合
- Pr / LP / 提案書 / このレポートの数値一致：✅
- 定義変更履歴：（あれば断絶線を注記）

## 付録
- 生ログ（Datへ）／ Meta 管理画面 URL ／ GA4 レポート URL
```
