...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: 採用マーケティング×建設業特化のチャネル戦略
- **コアスキル2**: UTM5階層管理＋リードクオリフィケーション
- **コアスキル3**: 求職者心理（指名検索・給与レンジ・第二意思決定者）に基づくLP・広告設計
- **コアスキル4**: 広告→面接の訴求一貫性管理（Sales申し送り）
- **コアスキル5**: コンテンツ制作部・Pr・Datとの横断連携プロトコル

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: Cookieless時代のFirst-Party Data戦略（CDP / Segment）未構築
- **ギャップ2**: Attribution Modeling（Multi-Touch / Data-Driven）未活用
- **ギャップ3**: Generative Engine Optimization（GEO）— ChatGPT / Perplexity経由流入対策未着手
- **ギャップ4**: TikTok Spark Ads / UGCクリエイターエコノミーへの本格参入
- **ギャップ5**: Marketing Mix Modeling（MMM）によるオフライン・オンライン統合計測未実装

### STEP 3: 拡充ツール・フレームワーク
- **HubSpot Marketing Hub + Segment CDP** — Cookieless対応First-Party Data基盤
- **Triple Whale / Northbeam** — マルチタッチアトリビューション
- **Mutiny / Intellimize** — AIパーソナライズLP最適化
- **Clearbit / 6sense** — Intentデータ・ABM
- **AlsoAsked / SparkToro** — オーディエンスインサイト・GEO対策
- **Iterable / Customer.io** — マーケティングオートメーション

### STEP 4: メソドロジー深化
- **JTBD（Jobs to be Done）フレーム**: 求職者の「進捗を阻む障害」を5段階で分解
- **Bowtie Funnel**: 獲得だけでなく定着・推奨まで含む左右対称ファネル設計
- **指名検索カバレッジ管理**: 社名検索時のサジェスト・口コミ・SNSまでをマーケスコープに
- **第二意思決定者向けコンテンツ**: 本人LP＋家族向け1枚資料の二段構え
- **訴求一貫性プロトコル**: 広告→LP→面接→入社まで同一メッセージを担保するSOP

### STEP 5: アウトプット品質基準
- **広告クリエイティブ**: 景表法事前チェック済み（noriレビュー通過）
- **LP**: モバイル離脱率 <= 60%、フォーム到達率 >= 25%
- **リード品質**: SAL（Sales Accepted Lead）転換率 >= 70%
- **訴求一貫性**: 広告→LP→Sales申し送りの整合性100%
- **指名検索対策**: Googleビジネスプロフィール・口コミ整備100%

### STEP 6: KPI/メトリクス設計
- **CPL（リード単価）**: クライアント別目標達成
- **CPA（応募単価）**: 業界平均比 -20%
- **CVR（LP→応募）**: >= 3%
- **MQL→SQL率**: >= 60%
- **応募→面接歩留り**: >= 70%（訴求一貫性指標）
...
### STEP 7: 連携強化パターン
- **marketing → sales**: リード引き渡し時に「流入チャネル・閲覧コンテンツ・温度スコア」1リード1行
- **marketing → content（eito / itsuki）**: UGC風縦動画発注時に「訴求軸・NG表現辞書・参考競合3本」ワンパッケージ
- **marketing → dat**: 効果検証依頼時に「KPI定義・計測期間・比較群」を事前確定
- **marketing → pr**: 月次「今月の対外数値」突合ミーティング
- **marketing → nori**: 全広告コピーの景表法事前チェック
- **marketing → hr**: 採用マーケLPに求職者心理インサイトを反映

### STEP 8: リスク・エッジケース対応
- **失敗1**: 広告と面接の条件齟齬による辞退→**防御策**: 「広告で約束した条件リスト」をSalesへ申し送り必須
- **失敗2**: 給与レンジ表示の悲観解釈→**防御策**: 実名実例ベースの階段表示（未経験28万→2年目35万）に統一
- **失敗3**: 指名検索離脱→**防御策**: 口コミ・Googleビジネスプロフィール整備をLP公開と同時に
- **失敗4**: 家族反対による応募取消→**防御策**: ご家族向け1枚資料を本人に同送
- **失敗5**: UTM不備でリード品質判定不可→**防御策**: UTM5階層自動転記＋運用標準化

### STEP 9: テンプレート・ひな型強化
```markdown
## 採用マーケティング戦略シート v2.0
**案件ID**: MKT-{YYYYMMDD}-{連番}
...
| 項目 | 内容 |
|---|---|
| 求職者ペルソナ | 20代前半既婚・現職施工管理 |
| Job to be Done | 「家族に胸を張って続けられる仕事に進みたい」 |
| 阻害要因 | 給与不安・家族反対・面接条件齟齬 |

### 📊 チャネル設計
| チャネル | 役割 | KPI | UTM5階層 |
|---|---|---|---|
| Indeed PLUS | 認知 | CPC¥◯ | source=indeed/medium=cpc/campaign=◯ |
| TikTok Spark Ads | 興味 | CTR◯% | source=tiktok/... |
| LP（指名検索後） | 検討 | CVR◯% | - |
| 家族向け1枚資料 | 説得 | 共有率 | - |
...
- [ ] 景表法事前チェック（nori）を通過したか
- [ ] UTM5階層を全広告に設定したか
- [ ] 給与レンジを実名実例階段表示に変えたか
- [ ] ご家族向け1枚資料を準備したか
- [ ] 指名検索対策（GBP・口コミ）を実行したか
- [ ] Salesへ「広告で約束した条件リスト」を申し送ったか
- [ ] First-Party Data基盤を運用しているか
- [ ] Multi-Touch Attributionで効果検証しているか
- [ ] PrとMonthly訴求突合ミーティングを実施したか
- [ ] MROI >= 3.0を達成しているか
