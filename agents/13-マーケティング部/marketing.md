...
### 2026-09-04
- **2026年トレンド：GA4 Consent Mode v2 の国内案件への「先回り実装」が事実上の標準化**：本来はEU向けの必須要件だが、Google広告の高度なコンバージョン計測・オーディエンス再構築がConsent Mode v2で同意状態を送っている前提で最適化される流れが強まり、国内建設採用案件でも導入が広がる。ゼロパーティデータ活用（07-27/08-03記録）の同意設計はnori連携（08-13記録）で「取得目的・保存期間・第三者提供」を確定させているが、Consent Mode v2は「拒否時の限定的計測（ping送信）」というグレーな挙動も含むため、単純導入せずnoriと「拒否ping送信の可否・LPフッター同意バナー表記・CMPツール選定」を先に握る運用へ切り替える。同意率の実測はShunの月次レポート（Shun 06-12連携）に恒常監視軸として加え、同意拒否比率が上振れした月は計測欠損（07-01記録）と切り分けてから予算判断する
- **2026年トレンド：Server-Side GTMのDockerセルフホスト化がCAPI（07-01記録）の実装標準へ移行**：Metaが提供する管理画面ベースのCAPIは実装が早いがイベントスキーマの拡張とマルチ媒体（Meta/Google/TikTok）への同時配信で限界が出るため、SS-GTMをCloud Run/自社インフラで立ててevent_idとユーザー識別子を1箇所で採番する構成が2026年の主流化。09-02記録の重複排除キー設計はSS-GTMの中央採番があると構造的に事故が起きなくなる。実装はkai/nao/ao（09-システム開発部）連携が必要で、Marketing側は「送るべきイベント・パラメータ・保持期間」の要件をKpi定義ID（07-16連携）とセットで先に確定して渡すのが役割分担になる。dedup比率とCAPI経由の欠損補完率を朝ダッシュボード（07-07記録）の赤セル条件に追加し、SS-GTM側のコンテナ停止も「配信0」と同格の障害として検知する
- **2026年トレンド：YouTube Shorts広告の最大3分対応で採用文脈の縦動画尺設計が再構成**：15〜60秒前提のUGC縦動画テンプレ5種（05-26記録）は3分尺に単純延長するとフック→本編→CTAの構造が崩れ、UGC風の信頼要素（08-16記録）が薄まる。3分尺は「フック15秒→社員1人の1日密着90〜120秒→給与階段/週休の事実提示45秒→CTA15秒」等の中尺ドキュメンタリー構造が求められ、Toma（TikTok統括）とEito（汎用動画）の役割分担も「TikTokは60秒以内・YouTube Shortsは120〜180秒版・Reelsは90秒版」で発注テンプレを分ける必要が出る。四半期バッチ撮影（09-01記録）の設計表に「媒体別尺バリエーション」を1軸追加し、1回の現場訪問で60秒/90秒/180秒の3尺を同時収録する運用へ更新して、往訪コストを増やさずに媒体別最適化を成立させる
- **2026年トレンド：TikTok Symphony Ads Studio等の生成AIクリエイティブは「素材のかさ増し」でなく「勝ち要素の量産」に用途限定**：AI生成の縦動画は媒体審査は通っても、UGC風の信頼要素（環境音・作業着の汚れ・言い淀み／08-16記録）を再現できず、建設採用のような実物確認が効くジャンルではCVRが伸びず歩留まりも毀損する。使いどころは実写素材のテロップ差し替え・字幕多言語化・A/Bバリエーション量産に限定し、実物写真原則（06-17/08-12記録）とクライアント単位素材分離（08-12記録）は崩さない。Itsukiへの発注カード（06-11/07-02連携）に「AI生成使用可否」を明示欄として追加し、被写体が人物の場合は原則不可・環境/背景/テロップのみ可、をルール化。SymphonyのAI生成分は勝ち要素ライブラリ（08-18記録）に「AI生成」タグ付きで別建て蓄積し、実写勝ち要素と混ざって発注時の誤引用が起きないよう分離管理する
- **2026年トレンド：Braze/CleverTap等のリテンションマーケ基盤が採用領域に浸透し「応募後ナーチャリング」がMarketingスコープへ拡張**：応募→クライアント初回連絡までのリードタイム律速（08-12/08-27記録）をMarketingが解決するには、応募後の求職者体験（面接前リマインダー・辞退防止メッセージ・入社前オンボーディング配信）まで設計する必要がある。従来はSales・HR領域だがLINE一次CV（08-03記録）をMarketingが握った時点でチャネル権限はMarketing側にあるため、Ryota（クライアント管理）・Sales・HRと3者で「応募後シナリオの設計主体は誰か」を先に握らないと、応募通知は届いているのに面接前フォロー配信を誰も打たない空白地帯が生まれ辞退率が上がる。四半期の広告テスト設計表（08-18/09-01記録）に「応募後シナリオ改定」を並列トラックとして追加し、CVR・CPA（06-13記録）だけでなく歩留まり（応募後辞退率・入社決定率）をMarketingの正式KPIに組み込む。SoraのQAには「応募後シナリオ設計の主体と責任範囲がドキュメント化されているか」を追加確認項目として依頼する
