...
### 現状スキル評価
コンテンツカレンダー・広告配信・リード獲得の一連運用は動いているが、MA（マーケティングオートメーション）ツールでの Nurture、SEO/競合分析、アトリビューション、BI ダッシュボードの層が薄く、施策成果を「感覚」で語る場面が残る。特にオフサイト（SNS→自社サイト）→オンサイト（LP→フォーム）→SFA の流れが分断されており、CAC/LTV を業界別に切って出せない。

### 特定した改善余地
- Nurture メールが単発配信で、スコアリング×行動トリガーによる自動化が未構築
- SEO の意図調査・競合ギャップ分析がスプレッドシート運用で属人化
- 広告のクリエイティブテストが「見た目」評価に留まり、統計的有意性まで踏み込めていない
- チャネル横断アトリビューションが Last-Click 一択、Assisted Conversion が経営レポートに載らない
- リードスコアリングが行動ベース（資料DL・ページ閲覧・イベント参加）に紐付いていない

### 追加スキル10選（オーバースペック化ロードマップ）
1. **HubSpot Marketing Hub Enterprise 運用** — Contact/Company/Deal 三層の Property を建設業向けに再設計、Workflow で MQL 自動判定／KPI: MQL→SQL 転換率 25%以上
2. **Marketo Engage / Pardot 併用ライフサイクル設計** — 業種別 5 段階ライフサイクル（Suspect→Prospect→MQL→SAL→SQL）を Trigger Campaign で自動化／KPI: リードライフサイクル滞留 30日以下
3. **Ahrefs Site Explorer + Content Gap 分析** — 競合 3 社（建設DXベンダー）の Top ページを毎月抽出、Content Gap キーワードで自社ブログ企画／KPI: Organic Traffic MoM +15%
4. **SEMrush Position Tracking + Topic Research** — 主要 500KW を日次順位追跡、Topic Cluster で内部リンク設計／KPI: Top10 KW 数 +50%
5. **SimilarWeb Digital Intelligence** — 競合サイトのトラフィック構成・流入チャネル比率を月次ベンチマーク／KPI: 自社サイト訪問数の対競合シェア +5pt
6. **Meta Ads Manager Advantage+ Campaigns** — AI 自動最適化のクリエイティブ 5 本 × ターゲット拡張運用に切替／KPI: CPA -20%、ROAS 4.0以上
7. **Google Ads P-Max + Conversion Modeling** — Performance Max キャンペーンで検索/GDN/YouTube 統合配信、Enhanced Conversions で計測強化／KPI: CV 数 +30% at CPA 同水準
8. **Looker Studio + BigQuery でチャネル横断ダッシュボード** — Ads/GA4/HubSpot/CRM を BigQuery に集約、Data-Driven Attribution で貢献配分／KPI: 経営会議の意思決定リードタイム 1週→即時
9. **Segment / RudderStack CDP 導入** — サイト行動・広告接触・メール開封を1顧客IDに統合、下流ツールへイベント配信／KPI: 顧客セグメント作成時間 3日→30分
10. **Optimizely / VWO で LP A/B テスト** — Ren の LP に対しヒーロー・CTA・フォーム項目数を統計有意（信頼度95%）まで検証／KPI: LP CVR +40%

### 新規ナレッジソース・ツール
- Ahrefs — 競合サイトの被リンク・オーガニックキーワード網羅調査
- SEMrush — キーワード順位追跡・広告出稿状況の可視化
- SimilarWeb — 競合トラフィック・流入チャネル比率の定量測定
- Looker Studio + BigQuery — マーケティング全チャネル統合ダッシュボード
- Segment CDP — ファーストパーティデータの中央集約基盤

### 連携強化ポイント
- **Sales (sales)**：HubSpot 上で MQL→SQL の SLA（24h 以内一次接触）を Workflow 強制化、Bombora インテントスコア上位は即 Salesloft シーケンス起動
- **Ren (LP部)**：LP デプロイと同時に GA4 イベント・Meta Pixel・HubSpot トラッキングを実装する仕様を発注テンプレへ組込
- **Data Analyst (dat)**：チャネル別 CAC/LTV を dbt モデルで正規化し、業種別コホート分析を四半期でレビュー

### 実践シナリオ例
「建設業向け原価管理SaaS」のリード獲得キャンペーンで、Ahrefs で「建設 原価管理 クラウド」等のキーワード群を抽出→競合3社が抑えていない「一人親方 経費精算」領域を発見、Rin にブログ企画発注（月8本）。Meta Ads は Advantage+ で動画3本×静止画2本を配信、Optimizely で LP のヒーローコピーA/Bを実施し「導入60日で工数40%削減」バージョンが有意勝ち（CVR 3.2%→5.7%）。CDP で「資料DL→事例ページ2回訪問」の行動をトリガーに Marketo が Nurture メール5通配信、リードスコア 60 到達で HubSpot が Sales へ自動アサイン。四半期で MQL 156件・SQL 41件・受注7件・ROAS 5.2 を達成。

### 2026-08-29 Daily Knowledge Log 追記
- **HubSpot Lifecycle Stage 自動化完了**：資料DL2回＋料金ページ滞在1分以上をトリガーに MQL 自動昇格、逆に90日無反応で Recycled に降格。過去90日で MQL カウントの水増しが 31% あったことが判明、実質 MQL は 220→152 に是正。SQL 転換率は 18%→26% に改善（無反応リードを Sales に渡していないため）。
- **Ahrefs で競合ギャップ発掘**：某建設DXベンダー2社の Top 100 KW と自社の比較で、月間検索数計 8,200 の「未攻略クラスター」を7つ特定。Rin へ発注する記事本数を月4→月10へ増枠、3か月後に Organic Session +42% 見込み。
- **Optimizely でヒーローコピーテスト決着**：LP-esco-2 で「業界No.1」vs「導入60日で工数40%削減」を4週間検証、後者が CVR 4.1%→6.8%（p<0.01）で圧勝。旧コピーの他LP4本も順次差替、月間問い合わせ +23 件見込み。
- **Meta Advantage+ 切替1週目実績**：手動ターゲティング配信の CPA ¥8,400 → Advantage+ で ¥6,100、ROAS 3.2→4.7。ただしクリエイティブ疲弊が早く、動画3本/週の追加供給が必要。Itsuki へ制作枚数を月8→月20に発注上方修正。
- **Looker Studio ダッシュボード v2 公開**：Ads/GA4/HubSpot を BigQuery 統合、経営会議用「今週のマーケ 1枚」を自動更新化。CEO と Sales マネージャーが同じ数字（MQL・SQL・CAC・パイプ額）を見て議論する時間が生まれ、会議の「数字合わせ」に費やされていた 25 分が施策議論に転換。
