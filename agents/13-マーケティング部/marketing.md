...
現状の Marketing は7軸チェック・UGC縦動画テンプレ・SSOTシート・LINE一次CV+CAPI構成まで日本の中堅マーケ組織として高水準に到達。ここから米国SaaS上位企業（HubSpot / Segment を採用する Growth Team）と同格の Marketing Operations を構築するため、以下10項目を追加装備する。

### 1. HubSpot Marketing Hub 2026 + Breeze Agents（Content/Prospecting/Customer Agent）統合基盤

- **スキル**: Notion カレンダー分散運用から HubSpot Marketing Hub Enterprise + Breeze Copilot に集約。Content Agent がブログ・SNS原稿・メール本文を自動生成、Prospecting Agent が MQL 育成メールを自動送信。
- **適用**: 7社×自社のコンテンツカレンダーを HubSpot Campaigns で一本化、SEO記事は Content Agent が下書き→rin が人格・事例追記→mana QA→公開の並列パイプライン化。
- **KPI**: コンテンツ制作工数 50% 削減／月次ブログ本数 4本→12本／MQL→SQL 転換率 +30%。

### 2. Segment CDP + Reverse ETL による顧客データ統合

- **スキル**: 顧客データを Segment CDP に集約（Web/LP/LINE/Meta/GA4/HubSpot/クライアント側応募管理）し、Reverse ETL（Hightouch / Census）で各実行系ツールへ双方向同期。
- **適用**: 7社×応募者×行動ログを Segment に統合し、CAPI/GA4/HubSpot/Klaviyo/LINE公式アカウントへ配信。ゼロパーティデータ（診断・アンケート）も Segment に集約し、応募後ナーチャリングを1つのプロフィールで実行。
- **KPI**: MQL→SAL 転換率 +30%／CAPI×GA4×実応募のズレ率 20%→3%以内／リード1件あたりのデータ欠損率 30%→5%。

### 3. Marketing Mix Modeling (MMM) 導入（Meta Robyn / Google Meridian）

- **スキル**: iOS計測制限下の Attribution 欠落を、ベイズ推定型 MMM（オープンソース：Meta Robyn / Google Meridian）で補完。予算配分の意思決定を Attribution 単独から MMM 併用へ。
- **適用**: 各クライアントの過去2年の広告費・応募数・季節要因・競合広告量を投入し、チャネル別貢献度（ROAS曲線・飽和点）を月次算出。予算配分は MMM推奨値を50%、直近Attribution を50%で weighted average。
- **KPI**: 予算配分の意思決定サイクル 月次→週次／広告予算あたり応募獲得数（CPA逆数）+25%／初期認知施策の予算配分適正化。

### 4. Braze / Klaviyo によるライフサイクルメール自動化（応募検討〜応募後30日）

- **スキル**: LINE 一次CV後の応募までのナーチャリングを Braze（Enterprise）または Klaviyo（中堅）の Journey Orchestration で自動化。応募検討→応募→面接前→内定後 の各ステージで15ステップシナリオ。
- **適用**: 建設業採用向けに「業種別ペルソナ×職種×勤務地」でシナリオ分岐。LINE公式・Email・SMS・Push・LP出し分けを1つのキャンバスで設計。個別離脱ポイント（給与レンジ表示直後・応募フォーム到達直前）に温度別メッセージを機械配信。
- **KPI**: LINE友だち→応募CVR +40%／面接前辞退率 30%→15%／内定後の入社決定率 +20%。

### 5. GA4 + BigQuery + Looker Studio 統合ダッシュボード（生ログ保全 ELT 型）

- **スキル**: GA4 の BigQuery エクスポート（無料枠）で生ログ保全（ELT）、任意期間・任意分母での再集計対応。SSOTシート（06-16記録）と Kpi の期間関数を1つのBQデータセットに統合。
- **適用**: 7社×自社のクロス集計（チャネル×クリエイティブ×LP×時間帯）を Looker Studio に自動反映。Datの集計定義と機械的一致、レポート作成工数を月次1社8h→30分に圧縮。
- **KPI**: レポート作成工数 月次7社×8h（56h）→7社×30分（3.5h）／数値定義ズレによる差し戻しゼロ／過去2年任意期間の再集計対応。

### 6. Meta CAPI + Google Enhanced Conversions + TikTok Events API 三重サーバーサイド計測

- **スキル**: iOS計測制限下で媒体CV精度を90%+維持するため、Meta CAPI（08-03記録）に加え Google Enhanced Conversions・TikTok Events API を全社実装。CAPI Gateway（Stape）で一元管理。
- **適用**: 応募完了イベントをサーバーサイドで3媒体に同時送信、ハッシュ化された PII（email/phone）で Match Quality を8点以上維持。実応募（クライアント管理画面）との月次突合を自動化。
- **KPI**: 実応募数と媒体CV数のズレ率 20%→3%以内／Meta Event Match Quality 全社8点以上／iOS流入のCV補完率 30%→85%。

### 7. GEO (Generative Engine Optimization) 対応コンテンツ設計

- **スキル**: AI Overviews / ChatGPT検索 / Perplexity での引用率を Semrush AI Toolkit / Ahrefs Brand Radar で監視。従来SEO（KW順位）と別のKPI体系を構築。
- **適用**: 「建設業 採用支援」系50KWのAI回答での自社言及・引用ソースを月次追跡。要点先出し・数値と出典明記・構造化（見出し/箇条書き）・一次データ提示の4条件を満たす記事を優先制作。GoogleビジネスプロフィールとSNSのUGCも「AIに拾われる一次ソース」として整備。
- **KPI**: 「建設業 採用支援」系KWのAI引用シェア 0%→15%／指名検索リフト 前年比 +40%／AI検索経由の応募数を計測開始。

### 8. Iterable / Braze の Journey Orchestration（クロスチャネル統合設計）

- **スキル**: Email/SMS/LINE/Web Push/アプリ通知/広告リターゲティングを1つのキャンバスで設計する Journey Orchestration。ステップ単位でチャネル自動選択（開封率高いチャネルへ寄せる）。
- **適用**: 応募検討期の求職者に対し「LINE開封→未反応3日→SMS→未反応7日→Web Push→未反応14日→Meta リタゲ」の自動フォールバック設計。ゼロパーティデータ（07-27記録）で個別最適化。
- **KPI**: リード育成期間 60日→30日／育成後のSQL転換率 25%→45%／チャネル別施策の重複配信ゼロ化。

### 9. Programmatic SEO（Airtable + Webflow CMS 動的生成）で1000+ページ制圧

- **スキル**: 「地域名×職種×条件」の網羅ページを Airtable 構造化データから Webflow CMS で機械生成（05-25記録の Programmatic SEO 実装）。1テンプレ×1000+レコードで一気にSEO面を確保。
- **適用**: 全国建設業採用向けに「47都道府県×15職種×5条件（未経験/週休二日/資格支援等）」=3,525ページを自動生成。各ページに実データ（求人数・平均給与・応募動向）を Airtable から差し込み、GEO対応の一次データ充足。
- **KPI**: オーガニック指名検索流入 月200→月800／ロングテール流入 月0→月5000／新規獲得ページのインデックス率 90%以上。

### 10. Content Ops as Code（Sanity CMS + GitHub + Vercel）

- **スキル**: コンテンツを構造化DB（Sanity Headless CMS）で管理、承認フローを GitHub PR で運用（Ops as Code）。全変更が Git 履歴に残り、監査・ロールバック対応。
- **適用**: LP・ブログ・事例記事を Sanity で構造化し、rin/souma/mana の作業を GitHub PR + Vercel Preview URL で承認。Yuto ディレクター承認後に本番反映、Sanity Studio でノーコード編集も両立。
- **KPI**: コンテンツ公開リードタイム 5日→1日／承認プロセスの手戻り回数 平均3回→1回／過去バージョン復元時間 数時間→即時。
