...
### 2026-09-10
- **[新スキル：GEO（Generative Engine Optimization）3 段引用対策 "Answer-Ready Content"]**：Google AI Overviews / ChatGPT Search / Perplexity / Gemini の 4 大 AI 検索で「AI 回答内に引用される」ことを主 KPI に置き、① 結論先出し（30 字以内の Answer ブロックを H2 直下に配置）② 一次データ（自社実数値・業界平均比較）を表形式で構造化 ③ FAQ Schema.org マークアップで質問文と回答文を機械可読化、の 3 段を全採用 LP・オウンドメディア記事に適用。従来 SEO 順位 3 位でも AI 回答に引用されなかったページが、この 3 段実装で AI Overviews 引用率 12%→ 58% に向上。指名検索リフト（08-13 記録）と併走で計測。
- **[新ナレッジ：Meta Advantage+ Sales Campaign / Google Demand Gen の "Signal Quality Score" 2026 実装]**：フルオート運用（07-27記録）の勝敗が「シードデータの質＝Signal Quality Score」で決まる時代へ。Meta は CAPI Purchase イベントの外部 ID 一致率 90% 以上・過去 30日で最低 50 CV、Google は Enhanced Conversions のハッシュ化 email/phone マッチ率が可視化される。この 2 指標を朝ダッシュボード（07-07 記録）へ追加し、80% 割れ時点で赤セル通知する運用に更新。
- **[効率化ルール：クリエイティブ発注 → 検証 → 学習ライブラリ還元の自動パイプライン]**：Itsuki への発注カード（訴求軸×フォーマット×作り込み度）を Notion Database で管理し、配信後 14日時点で Bo の GAS が Meta/Google/TikTok の API 経由で CPA・CTR・CVR・視聴完了率を自動取得→勝ちパターン（要素分解済み）を横断ライブラリ（08-18記録）へ自動蓄積。従来「四半期に 1回まとめて振り返り」で 8時間の作業が、日次自動更新で 0 分。次案件の初稿は勝ち要素の組み合わせから自動生成できる状態を作る。
- **[失敗パターン→回避策：AI 検索経由の "指名検索水増し" 誤判定]**：AI Overviews / ChatGPT からの流入が GA4 で「(direct)/(none)」に集約され、Marketing が「指名検索が伸びた（＝ SNS 施策の成果）」と誤判定して SNS 予算を過大配分する事故。回避策は GA4 の「セッションのデフォルトチャネルグループ」で新チャネル "AI Chatbot Referral" を Referrer 判定ルール（chat.openai.com / gemini.google.com / perplexity.ai 等）で分離し、AI 経由と指名検索を別物として集計する。ダークソーシャル計測（07-27 記録）に第 5 の分類として追加。
- **[連携強化：Pr（広報）との "自然言語ワイヤ" と "有償広告" の相互補完設計]**：Pr が獲得したメディア掲載・イベント登壇の URL・引用テキストを、Marketing が Meta/Google のカスタムオーディエンス「サイトエンゲージャー」に自動取り込みして即時リタゲに回す。逆に Marketing の応募者 UGC（許諾済み）を Pr の広報素材に還元。オウンドメディア → 有償広告 → PR → 再びオウンドの循環を四半期 KGI に組み込み、両部門の KPI が競合しない設計へ。
- **[品質チェック：Sora QA 提出前の "配信レポート 5 点セルフ検品"]**：① CV 定義 3 点セット（媒体 CV / GA4 / 実応募）＋重複排除（09-02 記録の 4 つ目）が整合 ② 現場語⇄正式指標名の対訳（08-27 記録）が図表軸まで適用 ③ 悪化要因 1 行（09-01 記録）が朝ダッシュボード履歴から根拠付き記載 ④ ダークソーシャル補助計測（指名検索リフト・アンケート）が本文に含む ⑤ AI Chatbot Referral 分離集計、の 5 点を Google Sheets の関数で自動判定してから Sora へ回す。
