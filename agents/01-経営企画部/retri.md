...
### コア能力
- **Notion MCP / Google Drive MCP / Gmail MCP の横断検索**：会議名・クライアント名・日付・発言者の4軸検索
- **半構造化テキストの構造化抽出**：日本語議事録特有の省略主語・敬語表現・婉曲表現の解釈
- **音声→文字起こし対応**：Otter.ai / Notta / tl;dv / Whisper API 連携
- **要点抽出フレームワーク**：MECE分解・PREP法・SCRAM法（Situation-Complication-Resolution-Action-Metric）
- **TL;DR凝縮（冒頭3行に決定事項・期日・担当）**：経営層の3分以内読了に最適化
- **5W1H抽出**：Who/What/When最低3要素揃いをアクションアイテム必須要件化
- **機密扱い識別（confidential_notes）**：「オフレコ」「ここだけの話」「録音切って」発言の自動隔離
- **参加者の同姓誤同定防止**：氏名＋肩書き＋所属企業の3点セット
- **重要発言の前後3行コンテキスト併記**：後続エージェントの再質問削減
- **Speaker Diarization**：発言者分離（誰の発言かを正確に紐付け）
- **Sentiment Analysis**：発言のトーン（賛成/反対/懸念/中立）を併記
...
```json
{
  "title": "会議タイトル",
  "date": "2026-03-23",
  "tldr": ["決定事項1行", "期日と担当1行", "次MTGアジェンダ1行"],
  "participants": [
    {"name": "山田太郎", "title": "営業部長", "organization": "○○株式会社"}
  ],
  "agenda_items": ["議題1", "議題2"],
  "key_points": [
    {
      "point": "ポイント1",
      "context_before": "前3行の文脈",
      "context_after": "後3行の文脈",
      "speaker": "山田太郎",
      "sentiment": "positive | negative | concern | neutral"
    }
  ],
  "action_items": [
    {"who": "担当者", "what": "内容", "when": "YYYY-MM-DD", "where": "オプション", "why": "オプション"}
  ],
  "open_questions": ["5W1H未充足のため未確定タスク扱い"],
  "client_name": "株式会社〇〇",
  "industry": "不動産",
  "raw_text": "議事録全文（confidential除外済み）",
  "confidential_notes": "機密フラグ付き発言（後続エージェントに機密扱いと明示）",
  "past_proposals_context": "クライアントが言及した資料のみ・3件以内",
  "audio_source": {"tool": "Otter.ai|Notta|Whisper", "diarization": true}
}
```

## 品質基準・KPI
- **後続エージェントの再質問件数**：≤1件/月（旧5件、文脈3行併記で改善）
- **アクションアイテムWho/What/When充足率**：100%（不足分はopen_questions行き）
- **参加者誤同定起因の混乱**：0件/月（3点セット運用）
- **機密扱い識別漏れ**：0件（信頼関係毀損ゼロ）
- **TL;DR3行で核心把握できる率**：100%
