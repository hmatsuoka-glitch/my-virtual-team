...
### STEP 1: 現状スキル棚卸し
- TL;DR3行・Who/What/When 3要素・機密自動タグ・参加者3点セット・議題ラベル再グルーピング・3件以内資料絞り
- 議事録構造化のフローは堅実。**国際標準の議事録規格・知識マネジメント（KM）標準への準拠が弱い**

### STEP 2: 世界トップ水準とのギャップ
| 領域 | 現状 | 世界水準 | ギャップ |
|---|---|---|---|
| 議事録規格 | 自社テンプレ | Robert's Rules of Order / ISO 15489（記録管理）| 国際標準未準拠 |
| 抽出手法 | 手動+一部AI | NER (Named Entity Recognition), Topic Modeling, Sentiment Analysis | NLP 高度技術未導入 |
| 機密管理 | キーワード辞書 | ISO/IEC 27001 (情報セキュリティ), DLP (Data Loss Prevention) | 体系的セキュリティ未整備 |
| 知識グラフ化 | フラット記録 | Knowledge Graph (Neo4j) / Ontology | グラフ構造化未実施 |

### STEP 3: 追加吸収すべき専門知識
1. **Robert's Rules of Order**：国際標準の議事進行・議事録規約
2. **ISO 15489 / JIS X 0902 記録管理**
3. **NER (Named Entity Recognition)**：人名・組織名・金額・日付の自動抽出
4. **Topic Modeling (LDA, BERTopic)**：議題自動分類
5. **Sentiment Analysis**：発言のトーン分析（不満・賛成・懸念）
6. **Knowledge Graph + RAG**：過去議事録の関係性グラフ化
7. **GDPR / 個人情報保護法 2026年改正**：個人情報の取扱い

### STEP 4: 2026年最新ツール
- **Notion AI 2.0 + Otter.ai**：議事録自動構造化
- **Tactiq / Fireflies.ai**：会議録音→AI議事録（多言語対応）
- **Neo4j / Memgraph**：議事録の知識グラフ化
- **Microsoft Purview / DLP**：機密情報自動検知

### STEP 5: 高度メソドロジー
1. **Decision Log**：決定事項を時系列DB化、後で「なぜそう決めたか」逆引き
2. **RACI Matrix適用**：アクションアイテムに Responsible/Accountable/Consulted/Informed を機械的付与
3. **Meeting Effectiveness Score**：MTGの成果度を5指標で点数化（決定件数/参加者発言バランス/時間効率/AI3要素充足率/Open Question減少率）
...
- **機密漏洩件数**（目標 0件）
- **後続エージェント再質問率**（目標 ≤ 10%）
- **議事録取得→構造化リードタイム**（目標 ≤ 12分）

### STEP 7: 2026年トレンド吸収
- **会議のAI記録義務化**：欧州一部国では自動化・透明性が法的要件化進行
- **個人情報保護法改正（2026春）**：議事録の保管期間・削除権の管理
- **Multilingual MTG**：特定技能2号採用増で多言語議事録対応が必要

### STEP 8: アウトプット品質基準
```json
{
  "tldr_3lines": ["決定", "期日", "担当"],
  "meeting_effectiveness_score": 85,
  "raci_matrix": [{"action": "...", "R": "...", "A": "...", "C": "...", "I": "..."}],
  "decision_log": [{"decision": "...", "rationale": "...", "stakeholder_concerns_addressed": "..."}],
  "confidential_notes": ["..."],
  "sentiment_analysis": {"positive": 60, "neutral": 30, "negative_concerns": 10},
  "topics_extracted": ["..."],
  "entities": {"persons": [], "orgs": [], "amounts": [], "dates": []}
}
```

### STEP 9: DoD（Definition of Done）
- [ ] TL;DR 3行を冒頭配置
- [ ] 全 action_item に Who/What/When の3要素
- [ ] 参加者は氏名+肩書き+所属の3点セット
- [ ] 機密キーワード辞書スキャン済
- [ ] 添付資料は3件以内+言及根拠あり
- [ ] 議題ラベルで重要ポイント再グルーピング
...
### STEP 10: 継続学習ループ
- **週次**：機密キーワード辞書の更新
- **月次**：Meeting Effectiveness Score の傾向分析
- **四半期**：Knowledge Graph 構築進捗レビュー
