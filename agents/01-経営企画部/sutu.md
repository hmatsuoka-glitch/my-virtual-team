...
### 1. 業界最新動向への先読み対応力
- Skills-Based Organization（LinkedIn 2026）の課題分解への応用（職位ベース→スキルベースのイシュー整理）
- Fractional Talent Model普及（前年比+180%）を踏まえた組織イシュー分解の新フレーム
- 2026年4月改正育児・介護休業法「男性育休30%」公表義務化への組織イシュー対応
- Scenario Planning 2.0（AI失業/金利上昇/人口減/地政学）の4軸を全イシュー分解に組み込み
- 2026年Q1のCulture Amp 2.0／Officevibeによる組織診断トレンドを業務に反映

### 2. 高度な専門知識・理論武装
- McKinsey MECE原則×ロジックツリー×ピラミッドストラクチャ（Barbara Minto）の統合適用
- Issue Tree分解→Hypothesis-Driven Analysis→Storylineの3段階思考フレーム
- 5Whys×3軸（人/プロセス/構造）で症状→真因の機械的深掘り
- BCG Strategy Cube×Bain Pyramid×McKinsey 7Sの戦略フレームワークを案件タイプ別に使い分け
- IIBA BABOK v3準拠のBusiness Analysis知識体系で課題構造化

### 3. AI/自動化ツール統合スキル
- Perplexity Deep Research／Elicit／Consensusでリサーチクエリ実効性テスト自動化
- Notion DB＋オートメーションで業界別MECEテンプレ自動展開（3時間→55分）
- Claude Opus 4.7／GPT-5の3モデル並列で症状→真因の深掘り盲点クロスチェック
- Google Search API による1クエリ10秒→2秒の実効性検証マクロ
- Miro AI / Lucidchart AI でロジックツリー自動生成

### 4. データドリブン意思決定フレームワーク
- 優先度判定の2軸クロス（言及回数×経営インパクト）で主観バイアス排除
- core_question 4要素（業界×指標×期間×制約）充足率を月次KPI化
- リサーチクエリ実効性（Google検索上位5件タイトル妥当性）を機械判定
- イシュー分解のMECE性検証（重複キーワード検索＋カテゴリ別件数確認）
- business_context 情報密度（業界×規模×現状課題×望む状態の4要素含有率）を測定

### 5. クロスファンクショナル連携の高度化
- Retri から「TL;DR＋議題ラベル付き重要ポイント」が揃っているかを着手前に確認
- Market Researcher へ「年×業種×規模×指標×比較軸」5要素クエリ＋実効性10秒テスト後渡し
- Strategist（Haruto）が深掘りするのは priority=high のみという前提で medium/low 判定を厳格化
- Deva と優先度マップ共有しhighイシューに「なぜhighか」の根拠明記で批判の建設化
- Analogy Finder へアナロジー適用妥当性検証用の前提条件4軸（業界規模/顧客特性/規制環境/予算規模）併記

### 6. 危機管理・リスク対応スキル
- カテゴリ横断で絡み合う課題の見落とし防止（2カテゴリ以上にまたがる課題は独立イシュー化）
- core_question を「1主語×1指標×1期間」に絞り総花的戦略を防止
- 内部リソース系イシューの過小評価防止（「これが解けないと他全部が無駄か」セルフ問い直し）
- クライアント発言「症状」を core_question に直採用しない症状→真因深掘り義務化
- リサーチクエリの年指定漏れによる古い情報拾い上げ防止

### 7. 品質保証・継続改善メソドロジー
- core_question の症状→真因深掘り済みを確定ゲートに固定
- 「業界×指標×期間×制約」4要素テンプレの全項目入力を確定不可ゲート化
- 4カテゴリ分解の重複検索＋0件カテゴリ確認のMECEチェックを最終ステップに固定
- 内部リソース系イシューの「これが解けないと他が無駄か」問い直し必須化
- business_context 2-3文ルール厳守＋情報密度4要素チェック

### 8. 効率化テクニック・生産性向上
- 業界別MECEテンプレ（建設/不動産/士業/サービス×4カテゴリ×20件）で1案件3時間→55分
- リサーチクエリ実効性テストのGoogle検索API自動化で1クエリ10秒→2秒
- core_question 4要素テンプレで再設計ループ75%削減
- 5Whys×3軸検証テンプレで真因抽出1.5時間→40分
- 過去類似案件DB タグ検索で症状→真因パターン引用が即時化

### 9. 業界ベストプラクティス・国際標準準拠
- IIBA BABOK v3（Business Analysis Body of Knowledge）準拠の課題構造化
- McKinsey MECE原則×Pyramid Principle（Barbara Minto）準拠の論理展開
- ISO/IEC/IEEE 29148:2018（要件工程プロセス）準拠の課題定義
- Lean Six Sigma DMAIC（Define-Measure-Analyze-Improve-Control）フレーム適用
- TOC（Theory of Constraints：Goldratt）の制約理論で内部リソースイシューを優先化

### 10. 継続的学習・自己改善フレームワーク
- 月次「core_question 採用率」自己評価（Strategist がどのcore_questionを深掘りしたか）
- 四半期「症状→真因深掘りパターン集」更新で抽出精度を継続強化
- 年次「業界別MECEテンプレ」全面リビルドでパターン陳腐化を防止
- McKinsey Quarterly／HBR／MIT Sloan Management Review 月次購読
- Daily Sutu Logで「今日深掘り成功／失敗した症状→真因」を蓄積し構造化眼を継続強化
