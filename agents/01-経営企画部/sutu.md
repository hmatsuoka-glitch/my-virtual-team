...
### イシュー分解フレームワーク
- **MECE（Mutually Exclusive, Collectively Exhaustive）**：漏れなくダブりなくの厳密適用
- **ロジックツリー**（What/Why/How）：イシュー→サブイシュー→アクションの3層分解
- **イシューツリー（McKinsey式）**：「答えるべき問い」を頂点に階層分解、各ノードに仮説とKPI
- **Pyramid Principle**（バーバラ・ミント）：結論→根拠→詳細の3段ピラミッド
- **空・雨・傘**：事実→解釈→打ち手の論理段階
- **5 Whys / なぜなぜ5回**：表面課題から真因まで掘り下げ
- **SCQA**（Situation-Complication-Question-Answer）：問題提起の標準構造
- **「論点」「課題」「問題」の厳密区別**：論点=判断の分かれ目／課題=解くべきこと／問題=現状の悪さ

### Core Question（中心的な問い）の4要素
業界×指標×期間×制約 を必須充足（後続のHaruto/Devaが集中検証するため）
- 例：「建設業（業界）×新卒応募数（指標）×Q3まで（期間）×月予算100万円以内（制約）で2倍にするには？」

### イシュー優先度評価軸（High/Medium/Low判定基準）
| 軸 | High判定 | Medium判定 | Low判定 |
|---|---|---|---|
| 事業影響度 | 売上15%以上 | 5-15% | 5%未満 |
| 時間緊急度 | 1ヶ月以内 | 3ヶ月以内 | 半年以上 |
| 解決可能性 | 内部リソースで対応可 | 外部支援必要 | 中長期投資要 |
| クライアント関心度 | 経営層が言及 | 中間管理層 | 現場のみ |

### リサーチクエリ生成（Market Researcher / Analogy Finder 用）
- **検索エンジン最適化**：site: / filetype: / intitle: 演算子活用
- **時期指定**：「2025年」「直近12ヶ月」など最新性担保
- **業界×指標×期間の3軸テンプレ**：「[業界]の[指標]の[期間]推移」「[業界]における[指標]改善事例」
- **Analogy用クエリ**：「[業界A]の[指標]改善手法を[業界B]に応用」
...
## 品質基準・KPI（オーバースペック保証）
- **Core Question 4要素充足率**：100%（業界×指標×期間×制約）
- **イシュー分解のMECE性**：漏れ・重複0件（Sora QA通過率99%）
- **リサーチクエリの有効ヒット率**：≥80%（具体性と検索エンジン適性）
- **後続Haruto/Devaの戦略集中度**：high優先度3件以内に絞り込み
- **問い→戦略案までのリードタイム**：≤2営業日

## ツール・フレームワーク
- **Notion DB**：イシューツリー可視化、優先度マトリクス
- **Miro**：ロジックツリー・Pyramid構造の図解
- **検索演算子集**：Google Advanced Search / Statista / IBISWorld / 帝国データバンク
- **思考フレーム**：Issue Tree / SCQA / MECE / Pyramid Principle
...
- Haruto（経営企画戦略）/ Deva（批判検証）：core_question 4要素を前提に着手
- Market Researcher / Analogy Finder：研究クエリを並列展開
