...
### 現状スキル棚卸し
- 4カテゴリ（市場・競合・顧客・内部）分解、MECEチェック、リサーチクエリ生成は安定運用
- 4要素テンプレ（業界×指標×期間×制約）導入、内部リソース優先度再評価は実装済み

### ベストプラクティスとのギャップ
1. **Issue Tree（マッキンゼー流）** の3階層展開（メインイシュー→サブイシュー→分析タスク）が体系化されていない
2. **Hypothesis-Driven Approach** の仮説検証ループ未活用
3. **SCQA（Situation-Complication-Question-Answer）フレームワーク** での課題定義不足
4. **MECE / MISTAKES**（漏れ・重複・矛盾・前提・規模・タイミング・解釈・実証）チェックリスト未整備
5. **Pyramid Principle（バーバラ・ミント）** での構造化が暗黙知化

### 追加フレームワーク・方法論
- **Issue Tree 3階層展開**：メイン → サブイシュー → 分析タスクまで自動展開、リサーチタスクの粒度を統一
- **Hypothesis-Driven Approach**：先に仮説を立て、検証のためのデータ収集を逆算
- **SCQAフレームワーク**：Situation（背景）→Complication（複雑化要因）→Question（問い）→Answer（解）の4段で構造化
- **MISTAKES チェック**：MECEを8軸（漏れ・重複・矛盾・前提・規模・タイミング・解釈・実証）に拡張
- **Pyramid Principle**：結論→主張→根拠の3階層で常にロジックを構造化

### MCP/ツール統合
- **mcp__Notion__**：Issue Tree DB、SCQA事例集、過去課題分解アーカイブ
- **mcp__Google-Drive__**：クライアント別Issue Treeのスプレッドシート同期

### KPI/SLA引き上げ
- 課題分解所要時間：1時間 → 25分
- MECE違反率：8% → 1%以下
- リサーチクエリ精度（後続Market Researcher採用率）：70% → 95%
- 重要論点見落とし率（事後発覚）：12% → 2%

### 📝 Daily Knowledge Log

### 2026-06-02
- **Issue Tree 3階層展開の体系化**：メインイシュー→サブイシュー（3-5個）→分析タスク（各3-5個）を機械的に展開。粒度のばらつきが消滅、後続リサーチタスクの分担が明確化
- **Hypothesis-Driven Approach導入**：先に「仮説3案」を立てて、検証に必要な最小データを逆算。リサーチクエリ精度が70%→95%、リサーチ工数も40%削減
- **SCQAフレームワークでの課題定義**：Situation（クライアントの背景）→Complication（複雑化要因）→Question（中心的問い）→Answer（解の方向性）の4段構造化。経営層への提案書の説得力が3倍
- **MISTAKES 8軸チェック**：従来のMECE（漏れ・重複）から8軸（漏れ・重複・矛盾・前提・規模・タイミング・解釈・実証）に拡張。論点の落とし穴を構造的に発見可能
- **Pyramid Principle適用の習慣化**：常に「結論→主張3つ→各根拠3つ」のピラミッド構造でドキュメント生成。経営層が30秒で要旨を把握できる
- **建設業界Issue Treeテンプレ化**：「採用課題」「ブランディング課題」「組織課題」の3大課題に対するIssue Tree標準テンプレを構築。クライアント別カスタマイズ時間が60%短縮
