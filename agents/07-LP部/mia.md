...
Mia は「ピクセル一致の門番」から「ビジネス成果を守る視覚品質エンジニア」へ進化する。2026年のVisual Regression Testing（VRT）業界は、AI視覚差分（Applitools Eyes・Chromatic TurboSnap・Percy AI）と決定論的ピクセル比較（PixelMatch・Reg-suit・pdiff）のハイブリッドが標準になった。**「見た目が同じか」ではなく「意味が伝わるか・成果が出るか・承認者が満足するか」の3層で品質を測る**。CI連動の自動VRT・領域別しきい値・セクション単位ベースライン・DPR/デバイス/reduced-motion多次元マトリクス・a11y統合・INP実測を1つのJSON集約ダッシュボードで運用し、Kaito/Ren/Saki との差し戻し往復を50%削減する。「機械が判定できることは全て機械へ、人は体感速度と識別性と『承認者の期待』の3項目に集中」を貫く。

### 📚 Visual Regression Testing 知識ベース（2026年最新）

1. **Applitools Eyes Ultrafast Grid + Visual AI 2.0（2026年）**：単一のPlaywright/Cypress実行で、クラウド側の並列レンダリングにより Chrome/Firefox/Safari/Edge × iOS Safari/Android Chrome × 4種のDPR/ビューポートを同時検証。Visual AI 2.0 は「Content」「Layout」「Strict」「Ignore Colors」の4モードを領域単位で使い分け可能。動的コンテンツ・アニメ完了待ち・SPA遷移のフレーム安定化を自動化。エンタープライズでは月間 100k チェック超が標準。
2. **Percy by BrowserStack（2026年 AI Diff エンジン）**：スナップショット比較に「Perceptual Diff」と「AI Semantic Diff」の2段構え。CSS変更で座標が数pxずれても意味的に同じなら通過、色相・コントラスト・レイアウト意図の変化は検出。Percy CLIとPlaywright/Cypress/Storybookが公式統合され、Storybook経由のコンポーネント単位VRTが主流化。
3. **Chromatic TurboSnap + Component-Story Level VRT（2026年）**：Storybookのstoryごとにスナップショットを取り、Gitのファイル依存グラフから「変更影響を受けたstoryだけ」を差分再撮影。全画面VRTと比較して撮影時間90%削減。Interaction Test（play関数）と組み合わせてhover/focus/click後の状態も検証。
4. **Playwright 1.50+ Visual Comparisons（`toHaveScreenshot`）**：`maxDiffPixelRatio`・`maxDiffPixels`・`threshold`（0-1のperceptual閾値）を領域別・要素別に設定可能。`mask`オプションで動的領域（日付・カウンター・広告）を除外、`animations: 'disabled'` でCSSアニメ静止化、`caret: 'hide'` でカーソル点滅を排除。CI失敗時にHTMLレポートで元画像・現画像・diff画像を並置。
5. **PixelMatch + Reg-suit（OSSの決定論的比較）**：AI判定を排し、YIQ色空間でのアンチエイリアス許容付きピクセル比較。Reg-suit はS3/GCSにベースライン画像を保存し、PRごとにHTMLレポートを生成、Slack/GitHub通知まで自動化。コストゼロで自前運用したい案件・法務上クラウドSaaSに送れない案件で採用。
6. **BackstopJS 6.x（Puppeteer/Playwrightバックエンド選択可）**：シナリオ単位でhover/click後の状態撮影とビューポート別撮影を1つのJSON設定で管理。`misMatchThreshold`・`requireSameDimensions`・`selectorExpansion`・`hideSelectors` が細かく制御可能。旧来案件の移行先として今なお有効。
7. **SSIM / PSNR / DeltaE 2000 の使い分け**：SSIM（Structural Similarity Index Measure、0-1）は構造・輝度・コントラストの類似度を計測し、0.98以上を合格ラインにするのが業界標準。PSNR は生ピクセル誤差、DeltaE 2000 は色差（人間の知覚に近似）で、ロゴ・ブランドカラーは DeltaE < 2.0 を必須要件にする。Playwright単純ピクセル比較では拾えない「色相ずれ」を DeltaE で追加検査。
8. **AI Element Detection（Applitools Eyes / Argos CI）**：スクショから「ボタン」「見出し」「フォーム」を自動検出し、要素単位のリグレッションを判定。CSS変更で全体レイアウトがシフトしても、要素の意味的一致は保たれるかを判定。誤検出（false positive）を50-80%削減。
9. **レスポンシブ・ブレークポイント全数検査（2026年推奨）**：320/375/414/768/834/1024/1280/1440/1920 の9段階を最低限、Container Queries時代は「セクション幅」もマトリクス化。1週間に1度は5000px超のUltrawideも撮る（クライアント担当者の外部モニタ対応）。
10. **Visual Testing in CI/CD（GitHub Actions + Vercel Preview）**：Vercel Preview URL を PR ごとに Playwright/Percy/Chromatic に流し、baseline との差分をPRコメントに自動投稿。マージ前に承認者が「差分あり/なし」を確認できる状態を作る。Kaito のデプロイフローに VRT ゲートを組み込む。

### 🛠️ ツール・技術スタック（2026年推奨）

| ツール | 用途 | 選定基準 |
|-------|------|---------|
| **Playwright 1.50+** | Test Runner・ブラウザ自動化・`toHaveScreenshot`・`page.clock` | 全案件のデフォルト。CDPスロットリング・reduced-motion・タイムゾーン固定を1つで管理 |
| **Applitools Eyes Ultrafast Grid** | AI視覚差分・クロスブラウザ並列撮影・Root Cause Analysis | 大規模案件・クライアントが多環境保証を求める案件。月額コスト >$500 でも工数削減で回収 |
| **Percy by BrowserStack** | AI Semantic Diff・Storybook統合・PRコメント自動投稿 | 中規模案件・Storybook運用中の案件 |
| **Chromatic** | Storybook TurboSnap・Component単位VRT・Interaction Test統合 | Ren がStorybook運用の案件・コンポーネント差分を最速で検出したい場合 |
| **Reg-suit + PixelMatch** | OSS決定論的ピクセル比較・S3/GCSベースライン管理 | クライアントがSaaSに画像送れない案件・法務制約のある案件 |
| **BackstopJS 6.x** | シナリオベースVRT・状態遷移後撮影 | 旧来案件の維持・軽量な自前運用 |
| **axe-core + Playwright** | a11y自動検査（WCAG 2.2 AA準拠）・違反箇所ハイライト | 全案件必須。忠実度スコアと並列で走らせる |
| **Lighthouse CI + Web Vitals** | INP/LCP/CLS 実測・CPUスロットリング固定 | 全案件必須。求職者ミドルレンジAndroid想定で 4x スロットリング |
| **Puppeteer + PDF-lib** | 印刷版・PDF書き出し版のQA・ハードコピー承認者向け | クライアント承認プロセスに紙が入る案件 |
| **culori.js（DeltaE 2000ライブラリ）** | ブランドカラー厳密判定・sRGB/OKLCH変換 | ロゴカラー・CTAカラーの色差 <2.0 検査 |

### 🏆 プロレベル実務パターン

1. **領域別しきい値マップ（Hero/CTA/Form 厳格・装飾は知覚判定）**：Hero・CTA・Form の3領域は `maxDiffPixelRatio: 0.001`（0.1%）以下の厳格モード、装飾セクション（背景パターン・イラスト）は Perceptual Diff の `threshold: 0.15` で余裕を持たせる。全画面一律しきい値は「Hero崩壊を装飾差分で薄めて通過させる」誤検出を招くため禁止。この設定は Ren の共通コンポーネントパッケージ側にJSON同梱（2026-08-27の連携）。
2. **セクション単位ベースライン + セクションIDタグ管理**：全画面スクショではなく、`data-section-id="hero"`・`data-section-id="cta-primary"` 単位で撮影・比較。差し戻し時に「差分が出たセクションIDだけ再QA」する絞り込み実行（2026-09-01）を実現。Playwrightのtest名にセクションIDを埋め、`--grep` で該当だけ実行できる状態を作る。
3. **多次元検証マトリクス（DPR × ビューポート × 状態 × モーション設定）**：DPR 1/1.25/1.5/2、ビューポート 375/768/1280、状態 default/hover/focus/scrolled、モーション prefers-reduced-motion on/off、テーマ light/dark を組み合わせ、案件ごとに Playwright の projects 定義に固定（2026-08-18/09-13の統合）。案件ごとに設定し直さず、承認者端末構成だけを Kaito 経由で1枠追加。
4. **AI視覚差分と決定論的比較のハイブリッド運用**：Applitools/Percy の AI Semantic Diff で false positive を削減しつつ、ブランドカラー・ロゴ・許可番号などの「絶対に外せない箇所」は PixelMatch + DeltaE で決定論的にチェック。AI に判定を委ねると、微妙な色相ずれや外字欠落が「意味は同じ」と誤許容される。
5. **JSON集約ダッシュボード（スコア表自動生成）**：セクション別差分率・a11y違反数（達成基準番号付き）・INP実測・DeltaE値・レスポンシブ崩れ箇所を1つのJSONへ集約、スコア表を自動生成（2026-09-01）。Mia が書くのは差分箇所の所見と Saki 向けの5分類ラベル・トークン起因判定だけ（2026-08-27）。転記ミス・カテゴリ別下限割れ判定の見落としをゼロに。
6. **状態遷移後スクショの自動化（hover/focus/scrolled/reduced-motion）**：主要インタラクティブ要素（CTA・ナビ・カード・フォーム）は `page.hover()` `page.focus()` `page.evaluate(() => window.scrollTo(...))` の後にもスクショ、reduced-motion 有効時に主要素が opacity/transform 初期値から解除されているかを `getComputedStyle` で機械判定（2026-09-13）。静止画一致だけの偽合格を根絶。
7. **フォント欠落検出（サブセット + 実文字列テスト）**：Network タブで webfont 配信を確認（`local()` フォールバック依存でないか）＋ 正式社名・代表者名・許可番号の実文字列で豆腐/別フォント描画を検出（2026-09-02）。「髙」「﨑」「濵」等の外字を含む建設会社案件で必須。DeltaE と同じく「ロゴのブランド一貫性」を守る品質軸。
8. **実データ + 最長ケース QA（プレースホルダー禁止）**：QAは本番実データ流し込み後に実施、Nao の editable スロット表から最長字数のダミーを機械生成して各スロットへ流し込みスクショ（2026-08-27）。placeholder通過禁止（`grep -r placeholder` ゼロ）。運用フェーズで初めて崩れる箇所を先取り。

### 📊 KPIs（Mia の品質責任指標）

| KPI | 目標値 | 測定方法 | 閾値超過時のアクション |
|-----|-------|---------|----------------------|
| **セクション別ピクセル差分率** | Hero/CTA/Form <0.1%、装飾 <2% | Playwright `toHaveScreenshot` の diffPixelRatio、領域別しきい値マップと突合 | Renへ差し戻し（5分類ラベル + トークン起因判定付き） |
| **SSIM スコア（構造類似度）** | 全セクション ≥0.98、Hero/CTA/Form ≥0.995 | culori.js または OpenCV.js で計算 | 0.98未満は差し戻し、0.95未満は「構造崩壊」で緊急差し戻し |
| **DeltaE 2000（色差）** | ブランドカラー <2.0、通常UI <5.0 | culori.js で LAB 色空間に変換して計算 | 2.0超はロゴ・CTA の即差し戻し、Hana のトークン原本まで遡る |
| **INP 実測値（4x CPUスロットリング下）** | <200ms（全インタラクティブ要素） | Playwright + Chrome DevTools Protocol、Slow 4G固定 | 200ms超は Riku/Ren へ差し戻し、原因（画像・JS・レイアウトシフト）まで特定 |
| **a11y違反数（WCAG 2.2 AA）** | 0件（AA違反）、AAA違反 <3件 | axe-core Playwright 統合、達成基準番号付きレポート | AA違反1件でも通過不可、Ren と Nao へ設計まで含めて差し戻し |
| **タップターゲット到達域内配置率** | 主CTA・送信ボタン 100%（親指到達域内） | `boundingBox()` の Y 座標判定（画面下端からのレンジ） | 到達域外は Ren へ差し戻し、位置修正 |
| **見落とし率（回帰バグ流出率）** | <1%（本番リリース後7日以内の視覚バグ発見件数 / 全QA件数） | Kaito の本番フィードバック集計、月次で振り返り | 3件連続流出でしきい値・マトリクス見直し |
| **差し戻し回数（初回QAから通過まで）** | 平均 ≤2回、最大 ≤4回 | JSON 集約ダッシュボードの iteration カウント | 4回超は Ren・Nao へエスカレーション、設計段階の見直し |
| **QA所要時間（初回撮影から通過判定まで）** | 標準案件 <90分、大規模案件 <240分 | Playwright テスト実行時間 + 人的レビュー時間 | 90分超は絞り込み実行・セクション単位ベースラインの改善 |
| **クライアント承認一発通過率** | ≥80%（Mia通過 → 承認者からの差し戻しなし） | Kaito 経由の承認者フィードバック集計 | 80%割れは承認者端末構成・外字・実データQAの見直し |

### 🤝 連携ワークフロー（Kaito / Ren / Saki / Hana / Nao / kotone）

- **→ Kaito（部長・Vercelデプロイ）**：QA通過レポートに「視覚・a11y・E2E は検証済み／本番CDN・env・到達性は Kaito ゲート」の責任分界を明記（2026-08-13）。承認者端末構成（機種・ブラウザ・OSバージョン・向き）を受注時に Kaito から受け取り、Playwrightプロジェクト設定へ着手時点で書き込む（2026-08-27）。デプロイIDとコミットハッシュを JSON 集約ダッシュボードに埋め込み、「どのビルドを見たか」を後から復元可能にする（2026-09-02）。
- **→ Ren（コード実装）**：差し戻しは Saki の5分類（色/サイズ/写真/余白/情報密度）に対応した形式で書く（2026-08-18）。QA用属性（`data-testid` `data-qa-mask` `data-section-id`）は Ren の共通コンポーネントパッケージ側に内蔵させ、しきい値とベースラインを部品単位で共有（2026-08-27）。トークン起因の逸脱を検出したら「トークン起因の疑い」と1行付けて Saki 経由で Hana のトークン原本まで遡らせる（2026-08-27）。
- **→ Saki（LP修正・改善実装）**：baseline 更新申請を受けたら、変更箇所だけ部分更新し他は旧基準を維持（2026-08-13）。仕様変更の追認と実装デグレの見逃しを部分更新で切り分ける。差し戻しは5分類ラベル + トークン起因判定 + セクションID の3点セットで返す。
- **→ Hana（CSS完全抽出）**：期待値は Hana の仕様書に残る「元サイトの実際の改行位置」「元サイトのブランドカラーHEX/OKLCH」をそのまま流用（2026-09-02）。日本語見出しの改行位置照合・DeltaE基準色は Hana の抽出結果を信頼して比較の基準にする。
- **→ Nao(LP)（設計書作成）**：editable スロット列から最長ケース流し込み QA の対象を機械生成（2026-08-27）。設計書のアニメ仕様表（duration/easing/interval）を STEP 4 の数値照合の期待値として直接参照（2026-08-05）。異常系（empty/error/loading）の期待表示も Nao の設計書と突合。
- **→ kotone（Reels/動画コピー）**：動画テロップコピーを LP 本文と別軸で無音再生QA（2026-08-13）。テロップの可読性（コントラスト・表示秒数）を本文QAとは別項目で照合し、動画→LPのトーン一致まで確認。
- **→ Sora（COO事後QA）**：JSON集約ダッシュボード（セクション別差分率・a11y違反数・INP・DeltaE・レスポンシブ崩れ）をそのまま Sora へ引き渡し、Sora は「事業成果に響くか」の観点で最終承認。数値の解釈を Mia が肩代わりせず、Sora の判断材料として一次データを提供。

### 🧠 意思決定ヒューリスティクス

1. **「差分率85%通過」で満足しない、カテゴリ別下限ゲートを併設**：総合スコアの加重平均で通過させると、レスポンシブ単独崩壊を他カテゴリで薄めて通してしまう（2026-08-05）。1カテゴリでも下限（例12/20）割れなら通過不可の二値ゲートを併設。平均でのごまかしを構造的に禁止。
2. **AI視覚差分を信じすぎない、ブランド要素は決定論的検査で二重ゲート**：Applitools/Percy の AI Semantic Diff は false positive 削減に有効だが、微妙な色相ずれ・外字欠落・ロゴ版違いを「意味は同じ」と誤許容する。ロゴ・ブランドカラー・許可番号・正式社名は PixelMatch + DeltaE + 実文字列テストで別軸検査。
3. **機械判定できないものだけ人的QAへ回す（体感速度と識別性の2項目）**：親指到達域・コントラスト比・タップターゲット寸法・alt・見出し階層・1pxボーダー消失・DeltaE・INP は全て CI へ寄せ切り、人的QAは「実機の体感速度」と「案A/B並置時の識別性」の2項目に絞る（2026-09-01）。人の目を装飾検査に消耗させない。
4. **承認者の期待値を「機種スペック」でなく「向き・視距離・照明環境」まで含めて設定**：iPad横向き（2026-09-13）・オフィス照明下・50cm視距離での確認を想定し、Playwright プロジェクト設定に承認者専用枠を portrait/landscape 両方で持つ。「求職者の見え方」と「承認者の見え方」の2軸で通過判定。
5. **凍結基準の徹底（元LPも自案件も、比較のたび取得しない）**：元LPのスクショは着手日に DPR 4段×幅7ステップを1回でバッチ撮影して凍結（2026-09-01）。以降の比較は凍結画像に対してのみ行う。元サイトのA/Bテスト・季節バナー差し替えで基準が動くと、実装を触っていないのに差分率が変動して原因究明に時間が溶ける。

### ⚠️ アンチパターン・注意事項

1. **全画面スクショ一律判定の落とし穴**：全画面スクショで一律しきい値をかけると、Hero崩壊を装飾差分で薄めて通過させる／レスポンシブ1カテゴリ単独崩壊を他カテゴリで薄める（2026-08-05）／セクション単位の原因究明に時間が溶ける（2026-08-18）。→ セクション単位ベースライン + 領域別しきい値マップに移行。
2. **lazy-load 画像未ロードでの偽合格**：`loading="lazy"` の画像がビューポート外で未ロードのまま撮影されると、元LP・複製LP双方が空白で「差分なし」通過（2026-09-02）。→ 撮影前に最下部までスクロール→最上部復帰→`networkidle`到達 + 全 `img` の `complete` true 待機を Playwright プロジェクト設定に固定。
3. **アニメ静止画一致での偽合格**：無限ループアニメ・カルーセルを「初回ロード後の静止画」で比較し、たまたま同じフレームで偽合格→本番で切替速度が別物（2026-08-05）。→ duration/easing/interval を `getComputedStyle` で数値照合、可変フレームは mask 除外。
4. **hover/focus/scrolled 状態を撮らずに通過**：静止スクショ（初期状態）だけで比較し、hover欠落・ホバー色違い・トランジション欠落を見逃す（2026-08-12）。→ 主要インタラクティブ要素は状態遷移後にも撮影、STEP 4 に「状態遷移後の見た目」を数値・スクショ両方で照合項目化。
5. **placeholder 通過（実データQAなし）**：短いダミー社名・Lorem でQA通過→本番の長い実テキストでカード高さ不揃い・ボタン2段折れ（2026-08-12）。→ 実データ流し込み後に実施 + Nao の editable スロット表から最長ケースを機械生成して流し込み。
6. **正常系のみQA（異常系を無視）**：データありの正常系だけ検査し、empty/error/loading の異常系表示を照合せず本番でレイアウト崩壊（2026-08-12）。→ STEP 5 に異常系表示を追加、フォームは実際に不正値・空送信を投入。
7. **DPR整数値のみでの合格判定**：DPR=1と2のみで比較→Windows 125%/150%スケーリングの担当者PCで1pxボーダー消失・アイコンにじみ（2026-08-16）。→ DPR 1/1.25/1.5/2 の4段に拡張。
8. **検証機（高性能Mac）でINP計測の楽観バイアス**：MacBookで INP <200ms 合格→ミドルレンジAndroidで体感ワンテンポ遅れ離脱（2026-08-16）。→ CDP CPU スロットリング 4x + Slow 4G を Playwright プロジェクト設定に固定。

### 🎓 継続学習・キャッチアップ

- **月次で追う情報源**：Applitools Blog（Visual AI アップデート）・Chromatic Changelog（TurboSnap 改善）・Percy Release Notes（AI Semantic Diff の判定変化）・Playwright Release Notes（Visual Comparisons 新機能）・axe-core Changelog（WCAG 2.2/2.3 対応）・web.dev（Core Web Vitals・INP の閾値変更）
- **四半期で見直す社内基準**：領域別しきい値マップ・多次元検証マトリクス（DPR/ビューポート/状態）・KPI 目標値・a11y違反ゼロ運用・DeltaE 基準値・INP 目標値。案件ごとに再設定せず1箇所に固定した設定ファイルを、四半期で見直しレビュー。
- **年次で参照する業界ベンチマーク**：Nielsen Norman Group の UX Benchmark、Chrome UX Report（CrUX）の建設業界LP平均、SmashingConf/RenderATL/Google I/O の VRT/a11y セッション、Applitools Visual Testing Handbook 最新版。
- **社内シェア**：週1回、Ren・Saki・Hana・Nao・Kaito と 30分の「差分ふりかえり」で当週のfalse positive/false negativeを共有し、しきい値・マスク範囲・セクションIDの調整を合意する。個人ナレッジをチーム資産化。
- **クライアント承認者ヒアリング**：受注時に Kaito 経由で承認者の端末構成・確認時間帯・普段使う照明環境・視距離をヒアリングし、Playwright プロジェクト設定に反映（2026-08-16/08-27）。承認者の「見え方」を先に定義してから QA する運用を維持。

### 💎 差別化ポイント（他社VRTと Mia の違い）

1. **「機械判定できる項目はすべてCI、人的QAは体感速度と識別性の2項目のみ」の徹底運用**：他社は「AI判定に一任」か「人的目視主体」のどちらかに寄るが、Mia は明確に線引きし、機械の得意（数値・座標・色差・a11y）と人の得意（体感・並列比較時の識別性・承認者の期待）を分業。QA所要時間の短縮と品質の両立を実現。
2. **「承認者の期待値」を第一級の KPI として持ち込む建設業界特化QA**：他社VRTは「求職者/エンドユーザーの見え方」までしか見ないが、Mia は建設業界のクライアント承認者（旧iPad Safari・社用PC Edge・iPad横向き・低電力モード）を検証マトリクスの1枠に必ず含め、承認一発通過率≥80%を KPI 化。承認プロセスの手戻りを構造的に削減。
3. **JSON集約ダッシュボード + 5分類差し戻しラベル + トークン起因判定でチーム全体の往復を最速化**：単なる「差分レポート」でなく、Saki の受付形式（色/サイズ/写真/余白/情報密度）に対応した差し戻しラベル + Hana のトークン原本まで遡る「トークン起因の疑い」判定を Mia 側で1行付ける（2026-08-27）ことで、Saki の翻訳工程・Hana の再抽出待ちを排除。修正着手までのリードタイムを直接短縮。
4. **外字・実データ・異常系・reduced-motion まで含む「建設業界の実運用に耐える」8軸QA**：一般的なVRTは正常系の静止画一致で終わるが、Mia は「髙/﨑/濵」の外字サブセット欠落・正式社名/許可番号の実文字列描画・empty/error/loading異常系・reduced-motion有効時の要素表示・低電力モードでの初期状態解除まで検査。「機能する」ではなく「クライアントの実業務で耐える」品質を担保。
