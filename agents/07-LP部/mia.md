...
### 2026-09-10
- **[新スキル] 「Playwright Visual Regression + SSIM + Perceptual Hash」の三段検査による偽合格ゼロ化**：ピクセル差分（面積比較）だけでは日本語見出しの改行1文字移動（2026-09-02参照）や lazy-load 欠損を見逃す問題を、SSIM（構造類似性）＋ pHash（知覚ハッシュ）で補完。SSIM < 0.95 または pHash Hamming 距離 > 5 で自動差し戻し。従来 Mia の目視で拾っていた「差分率は低いが人には違う」ケースが機械化され、忠実度スコアの信頼性が JIS X 25010 品質特性でいう「信頼性」水準に到達
- **[新ナレッジ] 「Core Web Vitals 2026」＝LCP/INP/CLS/TTFB 4指標 + Long Animation Frames (LoAF) API 対応**：2026年の Chrome 更新で LoAF（50ms超のロングタスクを詳細分解可能）が主要ブラウザ全対応。INP < 200ms 通過だけでは分からない「どの JS のどの関数がジャンクを起こしているか」を Playwright + CDP で自動収集し、Ren への差し戻しレポートに関数名レベルで指摘可能に。CWV assertion（Kaito 9/10参照）と組み合わせ、忠実度スコアに「元LP以上のパフォーマンス」まで含める
- **[効率化ルール] Percy / Chromatic / Playwright の3ツール併用から Playwright 単騎への集約**：Percy・Chromatic は SaaS 費用が月$100〜、Playwright は OSS で無料。DPR 4段×幅7ステップ×セクション別の28ショット/LPが Playwright Test Runner 単体で並列実行可能に。ツール往復のコンテキストスイッチが消え、1LPあたり QA 実行時間が45分→15分（67%削減）
- **[失敗パターン→回避策] 「LLM 生成レポートの誤指摘が Saki 差し戻し工数を増やす」**：Claude/GPT の Vision API に「元LPと複製LPの差分を指摘して」と投げると平気で幻覚差分を返す（実際は同じ位置なのに「20pxずれている」等）。回避策は LLM 出力を差し戻しレポートの本文としては使わず、あくまで「Mia の目視前の候補提示」に留め、Mia が Playwright の自動判定結果と目視で確定してから Saki へ渡す。AI は候補提示、判定は機械＋人の役割分担を明確化
- **[連携強化] Saki への差し戻しレポートの「機械判定 + 目視所見 + LLM 候補」3層構造化**：差し戻しレポートに①機械判定（Playwright/SSIM/pHash/LoAF の数値）、②目視所見（Mia のコメント）、③LLM 候補（Vision API の指摘・参考情報）の3層を明記。Saki は①を優先的に修正し、②を補足で見る運用にすることで、LLM 幻覚指摘に振り回されず修正着手を高速化
- **[品質チェック] Sora QA 提出前セルフチェック2項目**：①「Playwright 実行ログに DPR 4段×幅7ステップ×セクション別の全ショットが含まれ、SSIM/pHash/LCP/INP/CLS/LoAF の6指標がすべて自動判定済みか」、②「差し戻し先レポートに『機械判定＋目視＋LLM候補』の3層が明示されているか」。両方 GO で Sora へ
