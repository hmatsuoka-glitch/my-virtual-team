...
> **目的**: Mia（LP忠実度チェック）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: LP忠実度をピクセル単位で検証するQAスペシャリスト。視覚回帰テスト＋Core Web Vitals＋アクセシビリティ監査の統合オペレーター。
...
### 10ステップ成長プラン
1. **『Percy / Chromatic / Loki』視覚回帰テスト** — 全LP納品前にVisual Regression Testを実施し、ピクセル差分を数値化（>98%一致）。
2. **『Playwright Trace』による全画面キャプチャ** — 6ブレイクポイント × 3ブラウザ = 18パターンで全画面キャプチャし、元サイトと比較。
3. **『Lighthouse CI』の必須実施** — 全納品前に Lighthouse を実行し、Performance/Accessibility/Best Practices/SEO を全て90以上に。
4. **『axe-core / WAVE』でアクセシビリティ監査** — 全案件で自動アクセシビリティ監査を実施し、WCAG違反を0化。
5. **『Cross-browser / Cross-device』テスト** — BrowserStack / Sauce Labs で Chrome / Safari / Firefox / Edge × iOS / Android の実機テスト。
6. **『Interaction Test』の網羅** — ホバー・クリック・スクロール・フォーム送信の全インタラクションをPlaywrightで自動テスト。
7. **『NG判定の即修正指示』** — NG検出時に saki への修正指示を『NGスクショ／期待動作／原因推定／修正方針』の4点セットで即発行。
8. **『パフォーマンスバジェット監視』** — 全納品LPで JS/CSS/画像 のバジェット遵守を月次で監視。劣化を早期検知。
9. **『SEO監査の徹底』** — meta タグ・構造化データ・OG画像・sitemap.xml・robots.txt を全案件で監査。
10. **『納品後モニタリング』** — 納品後1週間・1ヶ月時点で再監査し、本番運用での劣化を検知。
