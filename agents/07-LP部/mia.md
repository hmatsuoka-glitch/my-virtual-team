...
### 2026-08-08
- **効率化テクニック：Playwright の component test（2026-08-03参照）＋Storybook で Hero/CTA/Form/Footer の4部品を並列VRT実行し、変更差分ファイルから影響部品だけを動的選択するCIジョブ化**：ページ全画面比較の再QA時間が15分→2分に短縮（部品単位のみ再撮）。Renの差分PRから `git diff --name-only` で `.tsx` を検出→影響部品マッピングで撮影対象を絞る。領域別しきい値（Hero/CTA/Form厳格・装飾は知覚判定、2026-07-27参照）と組み合わせ、変更ゼロ部品はスキップ。
- **品質チェックポイント：WCAG 2.2 の 2.5.8「最小ターゲットサイズ 24px」（2026-07-27参照）を axe-core の自動検出で拾いつつ、SP実機タップ領域で「隣接ボタンとの間隔8px以上」も併せて検証**：ボタン単体が24pxを満たしても隣接密接だと誤タップが発生。自動検査の合格でも手動キーボード操作＋実機タップの2層QA（2026-08-03参照）を必須化し、達成基準番号（2.5.8）付きで根拠を提示。フォームCTAの誤タップ→送信失敗を防ぐ実装後ゲートを追加。
- **失敗パターン：`prefers-reduced-motion: reduce` 環境でのアニメーション無効化実装を確認せず、モーション過敏なユーザーに元と同じアニメが再生されて健康被害・WCAG 2.3.3違反**：健常者環境だけで合格判定するとアクセシビリティ違反を見逃す。回避策：Playwright の `emulateMedia({ reducedMotion: 'reduce' })` でreduce時のアニメ挙動（transition:none等）を必須検証し、durationが0または最小化されているかを数値照合。STEP 4 のアニメ忠実度チェックにreduced-motion断面を追加し、健常者/過敏者2断面での合否を出す。
- **用語再確認：「ピクセル差分（pixelmatch）」「知覚差分（perceptual diff、SSIM/CIEDE2000ベース）」「AI視覚差分」の3方式の判定原理と使い分け**：pixelmatchは各画素RGBの絶対差、知覚差分は人間の視覚系（明度優位・色差鈍感）を近似、AI視覚差分（2026-07-27参照）は学習モデルで「別物に見えるか」を判定。Hero/CTA/Form=pixelmatch厳格、装飾帯/写真=知覚差分、レイアウト全体の妥当性=AI視覚差分、と3方式を用途別に自動振り分けする mia.config.json スキーマに拡張。判定方式を混同したまま「差分ある/ない」だけで報告しない。
- **連携ヒント：Ren への差し戻しレポートに「差分箇所のセレクタ＋期待computedStyle値＋実測computedStyle値＋修正パッチ案（Tailwindクラス変更またはCSS差分）」の4点セットで納品**：スクショと「ズレてる」だけ渡すと Ren が差分箇所を目視で探す時間が発生。Playwright `page.evaluate` で差分検出セレクタの `getComputedStyle` を実期待両方取得し、修正パッチ案まで含めた1JSONを saki（LP修正）にも同時共有。差し戻し1周のリードタイムを従来の30分→10分に短縮する。
