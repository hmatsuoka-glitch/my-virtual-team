...
### 2026-09-04
- **Playwright 1.50 系の `toMatchAriaSnapshot`（YAML スナップショット）を a11y ツリー回帰の主軸に据える**：axe-core は違反の有無しか返さず、`page.accessibility.snapshot()` の JSON 比較（2026-06-20参照）は構造差分を人が読解できない。`toMatchAriaSnapshot` は見出し階層・landmark 構造・アクセシブルネームを YAML で表現するため diff がそのまま読める。Ren の再構築で `<div>` 化してロールが消えた `<button>`、見出しレベル飛び（h2 → h4）、`main`/`nav` の landmark 欠落を規格番号（WCAG 1.3.1）付きで差し戻せる。Nao のブレークポイント別 表示/非表示マトリクス（2026-07-16参照）と同様に、a11y ツリーも設計側の期待 YAML を Nao(LP) から受領する運用に切り替え、axe-core の自動＋手動ハイブリッド（2026-08-03参照）の3層目として YAML スナップショット差分を追加する
- **odiff（Rust 実装）を「フル画面比較」担当に据え、pixelmatch は Hero/CTA/Form の厳格判定専用に絞る二層エンジン化**：pixelmatch は JS 単一スレッドで 4K × 7 幅 × 4DPR（2026-08-16参照）を回すと数十分かかり、`qa:full`（2026-07-07参照）並列化のボトルネックになる。odiff は同条件で 10〜40倍速く、しきい値・アンチエイリアス許容パラメータも揃っている。装飾帯・全画面比較は odiff、Hero/CTA/Form の領域別しきい値は pixelmatch、と用途分割して `mia.config.json` に engine 指定列を足す。フル regression が数分→秒台に落ち、再 QA 待ちで Ren/Saki が止まる時間が消える。セクション単位ベースライン（2026-08-18参照）の共有部品側にも engine 指定を埋めておき、部品ごとに最適な差分器が回るようにする
- **`content-visibility: auto` セクションと Speculation Rules（`<script type="speculationrules">`）の prerender で偽合格が量産される新パターンを、撮影前提として封じる**：`content-visibility: auto` はビューポート外セクションを未レンダリングにするため、全画面スクショで下部が空白のまま撮れて元 LP・複製 LP ともに「差分なし」で通過する（lazy-load の偽合格 2026-09-02 と同型だが別要因）。prerender 済みバージョンを撮ると初期表示前の状態と乖離した完成形を基準にしてしまい、本番の初回訪問者が見る表示と一致しない。回避策：撮影前に最下部まで自動スクロールに加え、`content-visibility: visible` へ強制上書きするスタイル注入と、Speculation Rules 用 `<script>` の DOM 削除を Playwright のプロジェクト設定（2026-08-18参照）の共通前処理に固定する。Hana の CSS 抽出時にも `content-visibility` 指定を仕様書へ明記してもらい、複製側で意図せず外れて撮影条件が変わる事故を防ぐ
- **View Transitions API のクロスドキュメント遷移が入る案件は「初期・50% 途中・完了」の3フレーム比較を必須化する**：MPA 間の `::view-transition-*` は完了状態のスクショだけを比べると FOUC 的な破綻・要素の一時消失・z-index 逆転が中間フレームだけに現れて見逃す。Nao(LP) のアニメーション仕様表（2026-07-16参照）に `view-transition-name` の対応列と遷移 duration を追加してもらい、`page.evaluate` で `document.startViewTransition` 発火直後・duration の 50%・完了時の3点で撮影して数値照合する。`prefers-reduced-motion: reduce`（2026-06-15系参照）の代替遷移も同表から機械的に決まり、Ren への差し戻しは中間フレームのセレクタ＋期待挙動＋実挙動の4点セット（2026-07-02参照）で返す
- **`prefers-reduced-transparency`（macOS「透明度を下げる」）と `prefers-contrast: more` を STEP 5 のエミュレート対象に追加する**：ガラスモーフィズム風の半透明 CTA・sticky ヘッダーは、透明度低減設定で背景ぼかしが不透明化して背景色に埋もれ、コントラスト強化設定でカスタム `accent-color` が上書きされてブランド色が失われる。`emulateMedia({ reducedTransparency: 'reduce', contrast: 'more' })` を `forced-colors: active`（2026-06-26参照）・`colorScheme: dark`（2026-06-15系参照）と同じ枠で回し、CTA・フォーム送信ボタン・sticky ナビが各設定下で読めるかを機械判定に組み込む。視覚一致だけを合格にせず、OS 個別設定での可読性を通過ゲートに含める。Sora への引き継ぎレポートには「どの `prefers-*` 条件下で検証済みか」を明記し、Kaito の責任分界（2026-08-13参照）と同様に「検査済み OS 設定／未検査 OS 設定」を線引きする
