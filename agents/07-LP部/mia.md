...
### 2026-08-30
- **Playwright 1.50 + Percy 3.0 でセクション単位ビジュアルリグレッションを全ブレークポイント×全DPRで並列実行**：DPR 1/1.25/1.5/2 × 幅 320/375/768/1024/1440 の20マトリクスで並列スクショ、差分をPercyダッシュボードで承認。効果例：QA所要時間 4時間→25分、差分検出率 +38%
- **Claude 5 の視覚推論でスクショ差分の「意図的変更／実装デグレ／元設計欠陥」を3分類自動判定**：Saki申請なしで差分が出た時、Claude 5 が原因分類を初期提示、Mia は判定を確認するだけ。効果例：差分分類の一次判断が15分→30秒、Saki への差し戻し精度 +52%
- **Axe-core 5 + APCA Lc 検証をCI に組み込み PR時点で機械判定**：コントラスト・alt・見出し階層・ARIA・tap target・landmark を PR ごとに自動チェック、閾値未達でマージブロック。効果例：a11y起因の差し戻し 月4件→0件、WCAG 2.2 AA 遵守率 100%継続
- **BrowserStack Automate + 実端末（旧 iPad Safari / 社用 PC Edge / ミドルレンジ Android）の常設検証マトリクス**：Kaito から取得した承認端末構成を毎回自動流し込み、実機QAを CI 化。効果例：「承認者の画面で崩れている」戻り月2件→0件、実機再検証工数 -90%
- **CPU 4x スロットリング下での INP 200ms 判定を GitHub Actions で毎PR実行**：ミドルレンジAndroid相当の性能条件で対話系性能を計測、実ユーザー体感と乖離しない基準を維持。効果例：本番実測 INP がGood率 68%→95%、フォーム離脱率 -18%
- **Nao の editable スロット列＋kotone 記入ガイドの最大字数から「最長ケース流し込みQA」を機械生成**：スロットDBから字数上限のダミーテキストを自動流し込み、崩れ箇所を全パターンチェック。効果例：運用フェーズで初めて崩れる事故 月3件→0件、公開後の緊急修正 -100%
- **Ren の共通コンポーネントパッケージに `data-testid`/`data-qa-mask` を予埋め済みで受領する運用**：Storybook 化された共通部品はベースラインと属性が同梱、Mia は差分観察に集中。効果例：属性欠落による差し戻し ゼロ継続、共通部品QAの初期セットアップ -85%
- **Saki への差し戻しレポートを「トークン起因／個別箇所」の Notion フォームで構造化**：同一逸脱を2箇所以上検出時に自動で「トークン起因」タグを付与、iro/Hana へ即エスカレーション可能に。効果例：トークン修正で吸収できる差し戻しの発見率 +72%、Saki の個別修正積み上げ -55%
