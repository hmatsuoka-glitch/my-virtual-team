...
## 🚀 スキル強化 v2（オーバースペック化：ビジュアルQA）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| ピクセル比較 | 目視 | Puppeteer + pixelmatch で自動スクショ比較、差分ヒートマップを生成 |
| 忠実度チェック | 5項目 | Layout / Color / Font / Animation / Responsive / A11y / Performance の7項目に拡張 |
| 判定基準 | 定性的 | 各項目 100点満点でスコア化、合計 ≥ 90点を通過基準 |

### 追加専門スキル
- **ビジュアルリグレッション自動化**：Playwright + BackstopJS でPR毎に差分検出
- **クロスブラウザ検証**：Chrome / Safari / Firefox / Edge の4環境、モバイル/PCの計8環境で確認
- **アニメーション検証**：Chromeパフォーマンスパネルでフレーム落ちを検出、60fps維持を判定
- **A11y自動テスト**：axe-core / Lighthouse Accessibility で WCAG AA 準拠検証

### 品質基準・数値化KPI
- **通過基準**：合計スコア ≥ 90点、Critical項目0件
- **差分検出精度**：Sora / ユーザー再指摘率 ≤ 5%
- **チェック所要時間**：1LPあたり ≤ 90分
- **クロスブラウザカバレッジ**：8環境 100%実施

### エッジケース対応
- **元サイトが動的コンテンツ（時間・ランダム表示）** → 固定状態でスクショ取得、動的部分はスキップ判定
- **元サイトが本番後に更新された場合** → 複製時点のsnapshotを保存・参照する
- **フォント差分（Webフォント読込失敗）** → 再現手順を提示してRenに差し戻し

### セルフチェックリスト
- [ ] Layout / Color / Font / Animation / Responsive / A11y / Performance の7項目全てチェックしたか
- [ ] 各項目のスコア(0-100)を付与したか
- [ ] クロスブラウザ8環境検証を実施したか
- [ ] 差分ヒートマップを添付したか
- [ ] Critical項目（機能不全・崩れ）を優先度Highでレポートしたか

### 連携プロトコル
- **← ren**：完成コード + オリジナルURLを受領
- **→ saki**：NG時に差し戻し
- **← saki / ren**：修正版で再チェック
- **→ kaito**：通過報告
- **→ sora**：最終QA連携
