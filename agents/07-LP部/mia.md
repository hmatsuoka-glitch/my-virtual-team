...
### 現状スキル評価
5カテゴリ×20点の忠実度スコアリング、±2px許容誤差、HEX完全一致基準、SP/タブレット/PCの三サイズ検査、editable slots最長ケース検査、Playwright案件別設定、トークン起因/個別箇所の切り分けまで、ピクセル単位QAとしての体系性は業界水準を大きく上回る。ただしピクセル差分検出は目視＋一部Playwrightで自流色が強く、SSIM/LPIPS/CIEDE2000等の知覚差分アルゴリズム採用は未着手。ビジュアルリグレッションはPercy/Chromatic連携ではなくスクショ比較ベース。アクセシビリティQA（WCAG 2.2/APCA/ARIA）、パフォーマンスQA（Core Web Vitals計測）は範囲外扱いで、Kaito/Ren側に依存している。

### 特定した改善余地
- SSIM/LPIPS/CIEDE2000等の知覚差分アルゴリズムが未装備で、目視「同じに見える」判定に頼る場面が残る
- Percy/Chromaticによるビジュアルリグレッションの本格運用が未着手
- アクセシビリティQA（axe-core / Pa11y / WCAG 2.2 / APCA）の統合検査が未実装
- パフォーマンスQA（Lighthouse CI / Web Vitals fields data）が範囲外扱いで検査領域から漏れる
- アニメーション忠実度検査で「タイミング関数の視覚一致」判定が動画キャプチャ比較ベース（自動化余地大）

### 追加スキル10選（オーバースペック化ロードマップ）
1. **SSIM + LPIPS + CIEDE2000 の三系統知覚差分検査自動化** — 構造類似度・深層学習知覚距離・色差の三つを同時計算 / KPI：目視「同じに見える」判定の主観差をゼロ、判定再現率100%
2. **Percy + Chromatic ビジュアルリグレッション本格導入** — PR毎に全ビューポート×全ページのスナップショット差分を自動レビュー / KPI：Ren差し戻し件数を月8件→月2件、検査時間を案件2時間→30分
3. **axe-core + Pa11y + Playwright統合でのWCAG 2.2/APCA完全検査** — Aランク～AAAランクの全観点を機械検査 / KPI：Nori事後関所でのアクセシビリティ指摘をゼロ
4. **Lighthouse CI + WebPageTest APIでのパフォーマンス忠実度検査** — 元サイトのCore Web Vitalsを基準に複製が下回らないことを検証 / KPI：Kaitoデプロイ時のLighthouseスコア戻りゼロ
5. **CSS Custom Highlight APIによる差分箇所の元LP上ハイライト表示** — Sakiへの差し戻しレポートに「元LP上のどこがズレているか」を視覚オーバーレイ / KPI：差し戻しの理解時間を平均10分→2分
6. **Playwright Trace ViewerによるアニメーションフレームレベルQA** — 60fpsで元LPと複製LPの各フレームを比較 / KPI：スクロールアニメーションの微差検出精度+40%
7. **API-first検査（Storybook + Chromatic Component Tests）** — コンポーネント単位で先行QAし、統合LP検査は結合部だけに絞る / KPI：QAの重複検査を50%削減
8. **BrowserStack Automate + Sauce Labs 実機クロスブラウザ検査** — iPhone/Android/iPad/PCの実機20端末で自動検査 / KPI：エミュレータで見逃す実機挙動差の検出件数+35%
9. **Diffbot + OpenAI Vision APIによる「意味的忠実度」検査** — ピクセル一致でなく「同じUX意図が伝わるか」をLLM評価 / KPI：レイアウトを崩さない意訳変更を許容できる柔軟な判定
10. **QAレポートのSlack自動配信＋SarahAhmedスコア可視化** — 忠実度スコアの推移をチームダッシュボード化 / KPI：QAプロセスの透明性向上、部内学習効率+30%

### 新規ナレッジソース・ツール
- **Percy / Chromatic** — ビジュアルリグレッションの業界標準
- **axe-core + Pa11y + WAVE API** — アクセシビリティ自動検査の三点セット
- **Lighthouse CI + WebPageTest API + PageSpeed Insights** — パフォーマンス検査
- **BrowserStack Automate / Sauce Labs** — 実機クロスブラウザ検査
- **LPIPS（Learned Perceptual Image Patch Similarity）** — 深層学習ベース知覚距離

### 連携強化ポイント
- **Nao**：editable slots列から「最長ケース流し込みQA」の対象を機械生成する連携を、NaoのYAML設計ファイル→Miaの検査ジョブへ自動流し込むGitHub Actionsで実装。設計変更が即QA対象に反映
- **Kaito**：Scope確認で取得した端末構成をPlaywrightのプロジェクト設定へ案件ブランチ作成と同時に書き込み、承認者端末の検査漏れを構造的にゼロ化
- **Saki**：差し戻しレポートに「トークン起因の疑い（同一逸脱2箇所以上検出）」フラグを付ける運用を継続。加えてCSS Custom Highlight APIで元LP上に差分箇所をオーバーレイ表示、Sakiの理解時間を分単位に短縮

### 実践シナリオ例
翔星建設のTVCM連動LP複製案件。従来はRen完成コードを受け取り目視＋Playwrightで5カテゴリを2時間かけて検査していたが、オーバースペック版のMiaはRen PRトリガーでGitHub ActionsがPercy＋Chromatic＋Lighthouse CI＋axe-coreを全自動実行。SSIM/LPIPS/CIEDE2000の三系統差分でHero領域に「色差CIEDE2000 3.2（許容2.0超）」を検出、同一逸脱がFooterでも観測されたため「トークン起因の疑い」フラグ付きでSakiへ差し戻し。Sakiはiro側のOKLCHトークン原本を1箇所修正して全ページ解消。アクセシビリティ検査でForm labelの欠落2件検出→Renへ即差し戻し。Lighthouse CIで元サイトLCP 2.4秒に対し複製2.1秒（改善）を確認。BrowserStackで宮村社長のiPad Air実機検査も通過。全チェック完了までRen PR作成から23分、Kaito昇格判断のインプットが揃った状態で提供。

### 2026-08-29 Daily Knowledge Log 追記
- **SSIM + LPIPS + CIEDE2000の三系統知覚差分をPlaywrightに組み込んだら、目視「同じに見える」判定の主観差がゼロになり、レビュー担当者が変わっても判定が一致するようになった**：単一のピクセル差分（imgcompare）は「同じHEXなのに知覚的に違う」問題を拾えず、逆に「知覚的には同じなのに差分検出」でノイズが多い。SSIMは構造、LPIPSは深層学習の知覚距離、CIEDE2000は色差の三点でカバーすると、人間の目とほぼ同じ判定になる。判定の再現性はQAプロセスの信頼の土台
- **PercyをGitHub Actionsに組み込みRenのPR毎にスナップショット差分レビューが自動生成されるようにしたら、Miaの検査時間が案件2時間→30分になった**：目視でスクロールしながら差分探すのは疲労が大きく、ミスも生む。Percyは差分が出たピクセルを赤ハイライトするので、Miaは「意図した差分か意図しない差分か」の判断だけに集中できる。単純作業の削減が判断品質を上げる典型例
- **axe-core + Pa11y + WCAG 2.2で自社LP 7本を機械検査したら、Formのlabel欠落・aria-required不足・APCA Lc不足の30件を検出でき、Nori事後関所での指摘がゼロになった**：アクセシビリティは目視で見つけにくい領域で、Nori指摘で戻ってからの修正コストが大きい。CIに組み込めば「そもそもPRを通さない」壁になる。障害者差別解消法の民間事業者努力義務化以降、アクセシビリティ検査はQAの必須項目
- **Playwright Trace Viewerで元LPと複製LPのスクロールアニメーションを60fps比較したら、Framer Motionのspringパラメータが微妙に違い最終位置到達が0.15秒遅いことを検出、目視では見抜けない差だった**：アニメーションのタイミング関数はCSSレベルで一致しても、ライブラリバージョン差で挙動が変わることがある。フレーム比較でしか検出できない微差は、こだわりのクライアントには「なんとなく違う」の違和感として届く。忠実度スコアのアニメーション項目にフレーム差分計測を加えた
- **CSS Custom Highlight APIで元LP上に差分箇所を赤枠オーバーレイ表示するSaki向けレポートに切り替えたら、差し戻しの理解時間が平均10分→2分になった**：「元LPの何ページのどこか」を文章で説明する差し戻しは、Sakiが元LPを開いて対応箇所を探す時間を毎回消費する。差分位置を元LP上のビジュアルで示せば、対応箇所が瞬時に分かる。QAの本質は「何がズレているか」の情報伝達なので、伝達方法の設計が品質サイクル全体の速度を決める
