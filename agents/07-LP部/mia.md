...
| 領域 | 現状の到達点 | 限界 |
|---|---|---|
| ピクセル差分 | `pixelmatch` 領域別しきい値（Hero/CTA/Form=0.05／テキスト帯=0.2〜0.3／装飾=`looks-same` 知覚判定）を `mia.config.json` で固定 | 指標が「差分ピクセル率」1本。構造・色・幾何を分離採点できず、NG の原因軸が不明のまま Ren/Saki へ渡る |
| スコアリング | 5カテゴリ×20点＝100点＋カテゴリ別下限ゲート（12/20）＋9段階品質ゲート `npm run qa:full` | 各カテゴリ内の点数配分が Mia の裁量。85点／92点の根拠が数式に接続されていない |
| 撮影条件 | 7幅ステップ／DPR 1・1.25・1.5・2／`document.fonts.ready` 待ち／mask 除外／baseline 凍結／Playwright project 設定に固定 | 撮影は決定論化済みだが、**しきい値そのものが実測ノイズフロアに基づいていない** |
| 色 | HEX ±5 ＋ WCAG 4.5:1 ＋ APCA Lc 補助、ΔE00 は「用語として」把握（2026-06-13） | ΔE00 が**実装されていない**。全画素の知覚色差分布（p95/最大/面積率）を出せない |
| フォント | family/weight/size/line-height/letter-spacing／`font-display`／`size-adjust`／実配信確認 | CSS 値の一致は見るが、**レンダリング結果の行数・折返し位置**という最終アウトカムを計測していない |
| モーション | duration/easing の `getComputedStyle` 照合＋Nao アニメ仕様表突合＋`reducedMotion` 検証 | 実時間再生依存でフレーム比較が非決定論。中間フレームの軌道ズレは検出不能 |
| a11y | `@axe-core/playwright` violations 0＋Tab／SR 手動＋WCAG 2.2（2.4.11／2.5.8） | 自動＋手動の2層は確立済み。ここは追加余地が小さい |
| CI／連携 | タグ別5〜10並列、GitHub Issue 自動起票、責務元振り分け、Kaito ゲート責任分界 | 運用は成熟。**指標の質**が上限を作っている |

> 結論：**運用・体制は完成域。ボトルネックは「単一指標・裁量配点・未校正しきい値」という計測工学側にある。** v2 はここだけを潰す。

### 到達すべきベンチマーク

1. **Playwright `toHaveScreenshot`**：`maxDiffPixelRatio`／`maxDiffPixels`／`threshold`／`mask`／`stylePath` をプロジェクト設定に固定した決定論的 VRT
2. **pixelmatch / odiff**：odiff は Rust 実装で pixelmatch 比 10〜20 倍高速、差分検出時 exit code 21。全幅×全DPR の総当たり比較を現実的な時間に収める
3. **SSIM / DSSIM（構造的類似度）**：`ssim.js` / `dssim` で局所窓ごとの輝度・コントラスト・構造の一致度を 0〜1 で算出。ピクセル一致とは独立に「構造が壊れているか」を測る
4. **ΔE2000（CIEDE2000）**：CIELAB 上の知覚均等色差。ΔE00<1＝識別不能、1〜2＝並置で判別、>3＝明確に別色
5. **Percy / Chromatic**：ベースライン承認ワークフロー・スナップショット履歴・変更影響範囲限定（`--only-changed`）の運用モデル
6. **axe-core**：WCAG 達成基準番号にマップされた機械判定（既存運用を維持しつつ FFI に統合）

> 目標水準：**「NG／OK」ではなく「どの指標が、どれだけ、どの部位で外れたか」を数値ベクトルで返す QA**。国内 LP 制作で ΔE00 と SSIM と IoU を同時に回している体制はほぼ存在しない。

### 特定されたスキルギャップと優先度

| # | ギャップ | 事業インパクト | 即戦力度 | 優先度 |
|---|---|---|---|---|
| 1 | 多指標フュージョン忠実度指数（FFI）が無く、単一指標＋裁量配点で 100 点を作っている | 大（スコアの説明責任・クライアント係争の防衛線） | 高 | **S** |
| 2 | DOM ジオメトリ IoU 差分が無く、レイアウトズレを「px 目視」で拾っている | 大（レイアウト NG の自動特定・偽差分ゼロ） | 高 | **S** |
| 3 | ΔE2000 の実装が無い（用語把握のみ）。色は HEX ±5 の RGB 距離判定のまま | 大（ブランドカラー係争・写真上テキストの誤判定） | 高 | **S** |
| 4 | 決定論的モーション差分（`getAnimations()` の `currentTime` 固定サンプリング）が無い | 中〜大（中間フレーム軌道ズレの検出） | 中 | **A** |
| 5 | タイポグラフィの**レンダリング実測**（行数・折返し点・実測字幅）差分が無い | 中〜大（FOUT／実データ流し込み時の折返し破綻） | 高 | **A** |
| 6 | しきい値の自己校正（ノイズフロア実測）と FP/FN 台帳が無く、しきい値が経験値 | 中（誤 NG と見逃しの同時削減、判定の再現性） | 高 | **A** |
| 7 | 納品後の常設 VRT 監視（視覚デグレの継続検出）が無い（CrUX 性能監視のみ） | 中（運用フェーズの CMS 編集起因デグレ検出） | 中 | **B** |
| 8 | 差分アーティファクト（onion-skin／blink／ヒートマップ）の成果物化が無く、Issue にスクショ2枚を貼るのみ | 中（Saki/Ren/クライアントの合意形成速度） | 高 | **B** |

→ 本 v2 では **#1〜#7 を新規習得スキルとして実装**（#8 は #3・#1 のテンプレートに統合）。
...
**なぜ必要か**：
現行の 100 点は「pixelmatch 差分率」という 1 指標＋Mia の裁量配点で作られている。差分率は「どこが」「なぜ」ずれたかを説明せず、アンチエイリアスで膨らみ、レイアウトが 10px ずれても面積が小さければ埋もれる。クライアントに「なぜ 84 点なのか」を説明できない時点で、QA は交渉材料として弱い。指標を **幾何・色・構造・文字・動き** の 5 軸に分離し、加重合成する。

**具体的手法**：
1. 5 軸の生指標を独立に計測する
   - 幾何 `G` = DOM ジオメトリ IoU（スキル2）
   - 色 `C` = ΔE00 分布（スキル3）
   - 構造 `S` = SSIM（セクション単位）
   - 文字 `T` = タイポメトリクス一致率（スキル5）
   - 動き `M` = モーション数値照合＋決定論フレーム SSIM（スキル4）
2. 各軸を 0〜100 に正規化 → 重み付き合成して **FFI** を算出
3. **FFI ≥ 92 かつ 全軸が下限を割らない** で通過。片方でも欠ければ通過不可（加重平均でのごまかし禁止＝2026-08-05 のカテゴリ下限ゲートを指標層まで降ろす）
4. 従来の 100 点満点レポートは FFI から機械変換して併記（Kaito/Sora との互換維持）

**判断基準・数値ライン**：

| 軸 | 生指標 | 合格ライン | 軸下限（1つでも割れば通過不可） | 重み |
|---|---|---|---|---|
| 幾何 G | 主要ブロック IoU / 重心オフセット | IoU ≥ 0.995、重心 ≤ 2px、幅差 ≤ 0.15% | IoU ≥ 0.985 | 0.30 |
| 色 C | ΔE00 p95 / ブランド色 ΔE00 | p95 < 2.0、最大 < 5.0、ブランド色 < 1.0 | p95 < 3.0 | 0.25 |
| 構造 S | SSIM（セクション単位） | SSIM ≥ 0.980（DSSIM ≤ 0.010） | SSIM ≥ 0.960 | 0.20 |
| 文字 T | 行数一致率 / 折返し点一致率 | 行数 100% 一致、折返し点 ≥ 98% | 行数 ≥ 98% | 0.15 |
| 動き M | duration/easing 差 / フレーム SSIM | duration 差 ≤ max(5%, 16ms)、フレーム SSIM ≥ 0.970 | フレーム SSIM ≥ 0.940 | 0.10 |

- 差分ピクセル率は**補助指標に降格**：Hero/CTA/Form のみ `diffRatio ≤ 0.1%`（threshold 0.05）、全体 `≤ 0.5%`
- FFI 92 未満 → 差し戻し／FFI 92〜95 → 条件付通過（軽微差異を明記）／FFI ≥ 96 → 無条件通過
- 事実整合チェック（数値・単位・固有名詞）は従来どおり **FFI と独立の 0/100 二値**。FFI 100 でも事実 NG なら納品不可

**実務テンプレート**：
```js
// scripts/mia-ffi.mjs — 5軸フュージョン忠実度指数
import fs from 'node:fs';
const W = { G: 0.30, C: 0.25, S: 0.20, T: 0.15, M: 0.10 };
const FLOOR = { G: 0.985, C: 3.0, S: 0.960, T: 0.98, M: 0.940 };

const norm = {
  G: v => Math.max(0, 100 - (1 - v.iou) * 20000),        // IoU 0.995 → 0点減
  C: v => Math.max(0, 100 - Math.max(0, v.p95 - 1.0) * 40), // ΔE00 p95 2.0 → 60点
  S: v => Math.max(0, 100 - (1 - v.ssim) * 2500),        // SSIM 0.98 → 50点
  T: v => v.lineMatchRate * 100,
  M: v => Math.max(0, 100 - (1 - v.frameSsim) * 2500),
};

const raw = JSON.parse(fs.readFileSync('qa-out/metrics.json', 'utf8'));
const scores = Object.fromEntries(Object.entries(norm).map(([k, f]) => [k, f(raw[k])]));
const ffi = Object.entries(W).reduce((a, [k, w]) => a + scores[k] * w, 0);

const floorBreach = [
  raw.G.iou   <  FLOOR.G && 'geometry',
  raw.C.p95   >  FLOOR.C && 'color',
  raw.S.ssim  <  FLOOR.S && 'structure',
  raw.T.lineMatchRate < FLOOR.T && 'typography',
  raw.M.frameSsim     < FLOOR.M && 'motion',
].filter(Boolean);

const verdict = floorBreach.length ? 'REJECT(floor)' : ffi >= 96 ? 'PASS'
              : ffi >= 92 ? 'PASS(conditional)' : 'REJECT';
console.log(JSON.stringify({ ffi: +ffi.toFixed(2), scores, floorBreach, verdict }, null, 2));
process.exit(verdict.startsWith('REJECT') ? 1 : 0);
```

**期待効果**：
差し戻し理由が「差分率 1.2% で NG」から「幾何 98.1／色 71.4／構造 99.0／文字 100／動き 96 → 色軸が下限割れ」に変わる。Ren・Saki・Hana への責務振り分け（既存運用）が**指標から自動決定**され、Mia の裁量判断がスコアから消える。
...
**なぜ必要か**：
「±2px 許容」も「相対比率で見る」も、結局は Mia が DevTools で個別に計測している。ピクセル差分はレイアウトズレを「面積」でしか語れず、10px 下にずれた小さなボタンは差分率 0.02% で埋もれる（＝偽陰性）。**要素の矩形（bounding box）そのものを両サイトから全数抽出して突合すれば、ズレは px 単位で機械的に列挙できる。**

**具体的手法**：
1. 元 LP・複製 LP の双方で、対応付け可能な全要素の矩形を抽出
   - 対応付けキー：`data-testid`（Ren 実装済み／2026-08-13）→ 無ければ「role + アクセシブルネーム + DOM 深さ」の複合キー
2. 各ペアで **IoU（Intersection over Union）**・重心オフセット・幅高さ差を算出
3. IoU 降順でなく**昇順**にソートし、ワースト 20 件を差し戻しレポートへ自動転記
4. スクショ比較の前段に置く（幾何 NG が残ったまま pixelmatch を回すと全面赤になり本質が埋もれる）
5. `data-testid` 欠落の主要要素は「QA フック実装漏れ」として検査前に差し戻し（既存運用と接続）

**判断基準・数値ライン**：
- 主要ブロック（Hero／CTA／Form／各セクション `<section>`）：**IoU ≥ 0.995**、重心オフセット **≤ 2px**
- 一般要素：**IoU ≥ 0.985**、幅差 **≤ コンテナ幅の 0.15%**（1280px なら 1.9px）
- IoU < 0.95 が 1 件でもあれば **幾何軸を即下限割れ**とし FFI 計算前に差し戻し
- 対応要素が見つからない（片側にのみ存在）＝**欠落 NG**、IoU=0 として最優先度で起票
- 全ビューポート（375／768／1280）× DPR 1 で実行（幾何は DPR 非依存のため DPR 展開は不要＝計測コスト削減）

**実務テンプレート**：
```js
// tests/geometry-iou.spec.ts — DOMジオメトリ突合
...
for (const vw of [375, 768, 1280]) {
  ...
  const boxes = await page.evaluate(collect);   // 元LP／複製LP 双方で取得し key で突合
  // → iou(orig, clone) 昇順ソートしてワースト20をレポート出力
}
```

**期待効果**：
「余白が窮屈に見える」という言語化不能な違和感が「`[data-testid=cta-primary]` の y が +14px、IoU 0.71」という一行になる。Saki の受付5分類（余白）にそのまま流し込め、翻訳工程が消える。
...
**なぜ必要か**：
HEX ±5 は RGB 空間のユークリッド距離で、人間の知覚と一致しない。同じ ±5 でも青系（人間の色識別が粗い領域）は誰も気付かず、緑・肌色系（識別が細かい領域）は一発で「色が違う」と言われる。既存の運用は「DevTools 採取値＋3台デバイス肉眼確認」という**人力の補正**で埋めていた。ΔE00 を実装すれば人力補正が不要になり、しかも**全画素**に対して評価できる。

**具体的手法**：
1. 元 LP・複製 LP のスクショを `sharp` で raw RGB に展開し、sRGB → CIELAB 変換（`culori` の `converter('lab')`）
2. 全画素で ΔE00（CIEDE2000）を計算 → **ヒートマップ PNG** と **分布統計（p50／p95／最大／ΔE>2 面積率）** を出力
3. 写真・グラデーション領域は `mask` で除外（既存の mask 運用を色軸にも適用）。除外領域は「比較対象外リスト」に明記
4. ブランドカラー（ロゴ・主 CTA・見出しアクセント）は computed style から直接 HEX を抜き、**ピンポイントで ΔE00 を算出**（スクショ経由の色管理プロファイル影響を避ける）
5. 差分ヒートマップは onion-skin（半透明重ね）・blink（交互切替 GIF）と併せて **QA 成果物 3 点セット**として Issue 添付

**判断基準・数値ライン**：
- **ブランドカラー：ΔE00 < 1.0**（識別不能域。ここだけは絶対基準、1.0 超で即差し戻し）
- 本文テキスト色・背景色：**ΔE00 < 2.0**
- 全画素分布：**p95 < 2.0／最大 < 5.0／ΔE00 > 2.0 の面積率 < 1.0%**
- 装飾・写真上：**ΔE00 < 3.0**（マスク除外しない場合の緩和枠）
- 平均値は使わない（大面積の白背景が平均を押し下げ、CTA の色ズレを隠すため）。**必ず p95 と面積率で判定**

**実務テンプレート**：
```js
// scripts/delta-e-map.mjs — ΔE00 分布 + ヒートマップ
import sharp from 'sharp';
import { differenceCiede2000, converter } from 'culori';
const toLab = converter('lab'), dE = differenceCiede2000();

const read = f => sharp(f).raw().toBuffer({ resolveWithObject: true });
const [A, B] = await Promise.all([read('baseline/hero.png'), read('clone/hero.png')]);
const n = A.info.width * A.info.height, ch = A.info.channels;
const heat = Buffer.alloc(n * 3), vals = new Float32Array(n);

for (let i = 0; i < n; i++) {
  const p = i * ch;
  const c1 = toLab({ mode: 'rgb', r: A.data[p]/255, g: A.data[p+1]/255, b: A.data[p+2]/255 });
  const c2 = toLab({ mode: 'rgb', r: B.data[p]/255, g: B.data[p+1]/255, b: B.data[p+2]/255 });
  const d = dE(c1, c2); vals[i] = d;
  const t = Math.min(1, d / 5);                     // ΔE00=5 で赤飽和
  heat[i*3] = 255 * t; heat[i*3+1] = 255 * (1 - t); heat[i*3+2] = 0;
}
const sorted = [...vals].sort((a, b) => a - b);
const p = q => sorted[Math.floor(n * q)];
const areaOver2 = vals.filter(v => v > 2).length / n;

await sharp(heat, { raw: { width: A.info.width, height: A.info.height, channels: 3 } })
  .png().toFile('qa-out/deltaE-heatmap.png');
console.log(JSON.stringify({ p50: +p(0.5).toFixed(3), p95: +p(0.95).toFixed(3),
  max: +sorted[n-1].toFixed(3), areaOver2: +(areaOver2*100).toFixed(3) }));
```
```bash
# 高速一次スクリーニング（odiff：pixelmatch 比 10〜20 倍高速・差分検出で exit 21）
npx odiff baseline/hero.png clone/hero.png qa-out/diff.png \
  --threshold=0.05 --antialiasing --diff-color='#ff00ff' --fail-on-layout-diff
# exit 0=一致 / 21=差分あり → 21 のときだけ ΔE00 マップの重い計算を回す
```

**期待効果**：
「#FF0000 のはずが #FF0001」という無意味な指摘が消え、「ブランドレッドが ΔE00 2.8＝並べれば明確に別色」という知覚根拠つきの指摘になる。クライアントの「色が違う気がする」に対して、ヒートマップ 1 枚で合否を即答できる。
...
**なぜ必要か**：
既存運用は「duration/easing の数値照合」＋「可変フレームは mask 除外」で、**中間フレームの軌道そのものは検査していない**。duration 300ms・easing `ease-out` が一致していても、`transform: translateY(40px)` と `translateY(24px)` では動きの印象が別物になる。実時間で撮ると毎回違うフレームが写る（flaky）ため、**時間軸を固定して撮る**必要がある。

**具体的手法**：
1. `document.getAnimations()` で全アニメーション（CSS Animation／Transition／WAAPI）を取得
2. `anim.pause()` → `anim.currentTime = anim.effect.getComputedTiming().duration * t` で **正規化進捗 t = 0／0.25／0.5／0.75／1.0** の 5 点を強制設定
3. 各 t でスクショ → 元 LP と同じ t のフレーム同士を SSIM 比較（**フレーム SSIM**）
4. 併せて `getComputedStyle` から `animation-duration／timing-function／delay／iteration-count`、`transition-property／duration／timing-function` を全数抽出し Nao のアニメ仕様表と数値照合
5. `emulateMedia({ reducedMotion: 'reduce' })` でも同手順を実行し、**アニメーションが 0 件になるか／fade のみに縮退するか**を機械判定
6. スクロール連動（IntersectionObserver 発火）要素は、`t=0` の未発火状態と `t=1` の完了状態を必ず両方撮る（永久非表示バグの検出＝既存知見の機械化）

**判断基準・数値ライン**：
- duration 差：**≤ max(5%, 16ms)**（16ms＝60fps の 1 フレーム。これ未満は人間が知覚不能）
- easing：`cubic-bezier` の制御点 4 値それぞれ **差 ≤ 0.05**。キーワード（`ease-out` 等）は数値展開して比較
- delay 差：**≤ 50ms**、iteration-count・direction・fill-mode は**完全一致**
- フレーム SSIM：全 5 点で **≥ 0.970**（下限 0.940）
- `reducedMotion: 'reduce'` 時：`getAnimations().length` が **元 LP と同数以下**、かつ移動量 40px 超のアニメが **0 件**
- flaky 判定：同条件 3 回実行でフレーム SSIM の分散が **0.005 超なら決定論不足**とし、しきい値を緩めず**原因固定**で対応

**実務テンプレート**：
```ts
// tests/motion-deterministic.spec.ts — 時間軸固定サンプリング
...
for (const t of SAMPLES) {
  await page.evaluate((t) => {
    document.getAnimations().forEach(a => {
      ...
      if (typeof d === 'number') a.currentTime = d * t;
    });
  }, t);
...
// アニメーション定義値の全数抽出（Nao 仕様表と JSON diff）
const specs = await page.evaluate(() =>
  [...document.querySelectorAll('[data-testid]')].map(el => {
    const s = getComputedStyle(el);
    return { key: el.dataset.testid,
      animation: [s.animationName, s.animationDuration, s.animationTimingFunction, s.animationDelay].join('|'),
      transition: [s.transitionProperty, s.transitionDuration, s.transitionTimingFunction].join('|') };
  }).filter(x => !x.animation.startsWith('none|0s') || !x.transition.startsWith('all|0s')));
```

**期待効果**：
「なんとなく速い／ふわっと感が違う」という形容詞ベースの差し戻しが消滅する。無限ループ・自動再生カルーセルの偽合格（たまたま同じフレーム／2026-08-05）が構造的に発生不能になる。
...
**なぜ必要か**：
`font-size`／`line-height`／`letter-spacing` が全一致していても、フォントのフォールバックメトリクス差・字幅差・コンテナ幅差で **折返し位置が変わる**。折返しが 1 箇所変われば行数が変わり、カード高さが変わり、CTA が押し出される。既存知見（FOUT・`size-adjust`・実データ流し込み）は「起こる」ことを把握しているが、**行数と折返し点を数値で突合する手段を持っていない**。

**具体的手法**：
1. テキストノードを `Range` で包み、`range.getClientRects()` の**矩形数＝実際の行box数**を取得
2. 各行の先頭文字インデックスを二分探索で求め、**折返し点の文字位置配列**を生成
3. 元 LP・複製 LP で「行数」「折返し点配列」「テキストブロック実測幅」「実効 line-height（行box の y 差）」を突合
4. **フォント未読込状態**（`page.route` で woff2 を abort）でも同手順を実行し、フォールバック時の行数差を計測（FOUT 起因のズレを数値化）
5. **最長ケースストレス**：kotone／Nao から受領した各テキストスロットの想定字数上限を流し込み、行数増加が設計許容内かを検証（既存の 1.5 倍ダミーを「実データ上限」に置換）

**判断基準・数値ライン**：
- 行数：**完全一致**（1 行でも差があれば文字軸を下限割れ扱い）
- 折返し点一致率：**≥ 98%**（禁則処理のブラウザ差で末尾 1 文字がずれるケースのみ許容）
- テキストブロック実測幅：**差 ≤ 1.0%**
- 実効 line-height（行box y 差の実測）：**差 ≤ 0.5px**
- letter-spacing：**差 ≤ 0.01em**
- フォント未読込時の行数差：**≤ 1 行**（超える場合は `size-adjust`／`ascent-override` 未調整として差し戻し）
- 最長ケース流し込み時：カード高さの不揃い **≤ 8px**、ボタンラベルの 2 段折れ **0 件**

**実務テンプレート**：
```js
// tests/typo-metrics.spec.ts — 行box実測
const measure = (sel) => {
  ...
  const s = getComputedStyle(el);
  // 折返し点：各行の先頭文字インデックスを走査で確定
  const breaks = []; let prevTop = null;
  for (let i = 0; i < node.length; i++) {
    const cr = document.createRange(); cr.setStart(node, i); cr.setEnd(node, i + 1);
    const top = cr.getBoundingClientRect().top;
    if (prevTop !== null && Math.abs(top - prevTop) > 1) breaks.push(i);
    prevTop = top;
  }
...
// フォント未読込（フォールバック）状態の計測
await page.route('**/*.{woff,woff2,ttf,otf}', r => r.abort());
const fallback = await page.evaluate(measure, '[data-testid="hero-title"]');
```

**期待効果**：
「テキストが詰まっている感」「なんか窮屈」が「Hero 見出しが元 3 行／複製 4 行、折返し点が 12 文字目→9 文字目」に変わる。本番実データ投入後の崩れを、納品前に数値で予告できる。
...
**なぜ必要か**：
現行の `threshold: 0.05／0.2〜0.3` は「経験的にこのくらい」で決まっている。適正値は**案件ごとのノイズフロア（同一ページを2回撮ったときに出る差分＝環境由来の下限ノイズ）**に依存する。ノイズフロアを測らずにしきい値を決めるのは、ゼロ点調整をしない計測器で合否を出すのと同じ。さらに偽陽性・偽陰性の**実測値**を持たないため、しきい値を緩めた判断が正しかったか誰も検証できない。

**具体的手法**：
1. **ノイズフロア測定**：複製 LP の同一ページを、同一条件で 5 回撮影し総当たり比較（10 ペア）。得られた最大差分率を `N` とする
2. `N > 0.05%` なら**しきい値を緩めず決定論を修正**（`document.fonts.ready` 待ち・アニメ停止・時刻固定・可変要素 mask）。ノイズフロアが下がるまで QA 本体に入らない
3. しきい値 = **`N × 1.5`（下限 0.02%）** として `mia.config.json` に案件別に書き込む
4. **FP/FN 台帳**：差し戻した NG が Ren/Saki 側で「実装は正しかった（＝偽陽性）」と判明した件数、および Sora／クライアント指摘で後から発覚した見逃し（＝偽陰性）を案件ごとに記録
5. 月次でレビューし、FP 率が目標超過ならしきい値を緩め、FN が 1 件でもあればその軸のしきい値を締める（**片方向でなく双方向の校正**）

**判断基準・数値ライン**：
- ノイズフロア `N`：**< 0.05%**（超過は決定論不足＝QA 開始不可）
- 校正後しきい値：`max(N × 1.5, 0.02%)`
- **偽陽性率（FP率）目標：≤ 5%**（差し戻し NG 20 件中 1 件まで）
- **偽陰性（FN）目標：Hero／CTA／Form は 0 件**（1 件でも発生したら該当軸の下限を 1 段階厳格化）
- 校正の再実行トリガー：ブラウザメジャー更新／CI ランナー変更／フォント差し替え／案件開始時（**最低でも案件ごとに 1 回**）
- 台帳は `qa-out/calibration/{案件}/{日付}.json` に保存し、通過レポートに「本案件のノイズフロア＝0.018%、適用しきい値＝0.027%」を明記

**実務テンプレート**：
```bash
#!/usr/bin/env bash
# scripts/calibrate-threshold.sh — ノイズフロア実測 → しきい値決定
set -euo pipefail
URL="$1"; OUT="qa-out/calibration"; mkdir -p "$OUT"
for i in 1 2 3 4 5; do
  npx playwright screenshot --full-page --wait-for-timeout=2000 "$URL" "$OUT/run-$i.png"
done
MAX=0
...
echo "noise floor = ${MAX}%"
python3 - "$MAX" <<'PY'
import sys, json, pathlib
n = float(sys.argv[1])
assert n < 0.05, f"NG: ノイズフロア {n}% > 0.05% → 決定論を修正してから QA 開始"
cfg = {"noiseFloor": n, "threshold": round(max(n * 1.5, 0.02), 4)}
pathlib.Path("qa-out/calibration/threshold.json").write_text(json.dumps(cfg, indent=2))
print(cfg)
PY
```

**期待効果**：
しきい値が「Mia の経験」から「本案件で実測された物理量」になる。誤 NG を減らす判断と見逃しを減らす判断が、感覚論でなく FP/FN の実数で議論できるようになる。
...
**なぜ必要か**：
既存の納品後監視は CrUX（性能）だけで、**見た目のデグレは監視していない**。実際には運用フェーズでクライアントが CMS から画像を差し替える、Kaito が別 LP のデプロイでスタイルを巻き込む、CDN の画像最適化設定が変わる、といった理由で納品後に見た目が崩れる。崩れは「クライアントが気付いて怒る」タイミングでしか発覚しない。

**具体的手法**：
1. 納品時点の全ビューポート・全 DPR スクショと FFI スコアを **`baseline/delivered/{案件}/{日付}/` に確定保存**（ゴールデンイメージ化）
2. GitHub Actions の `schedule`（日次 03:00 JST）で本番 URL に対し odiff 一次スクリーニング → 差分検出時のみ 5 軸フル計測
3. **FFI が納品時点から 3 pt 以上低下**、または幾何 IoU < 0.985／ΔE00 p95 > 3.0 で Kaito へ Slack 通知＋Issue 自動起票
4. 監視期間：**納品後 90 日**（7／30／90 日目は差分の有無に関わらずフルレポートを Kaito へ送付）
5. 意図的変更（クライアント自身の更新）は Kaito 経由で確認後、**該当セクションのみ baseline 部分更新**（既存の baseline 部分更新運用を運用フェーズへ延長）
6. baseline の更新は**承認台帳**（誰が・いつ・どのセレクタを・何故）を必須とし、`--auto-accept` による無審査更新を禁止（劣化の累積防止）

**判断基準・数値ライン**：
- 監視頻度：日次（本番 URL、DPR 1、375／1280 の 2 幅で軽量実行）
- アラート閾値：**FFI 低下 ≥ 3.0pt**／幾何 IoU < 0.985／ΔE00 p95 > 3.0／Console error ≥ 1 件／`requestfailed` ≥ 1 件
- フルレポート送付：納品後 **7／30／90 日目**（CrUX の Field Data 取得と同タイミングに束ねる）
- baseline 部分更新の承認：**Kaito の明示承認＋台帳記載が揃って初めて反映**（Mia 単独判断で更新しない）
- 監視終了：90 日で自動アーカイブ。継続希望は Kaito 経由で保守契約として切り出す
...
jobs:
  watch:
    ...
    steps:
      ...
      - name: capture
        run: npx playwright test tests/watch.spec.ts --project=chromium
        env: { TARGET_URL: '${{ vars.PROD_URL }}', VIEWPORT: '${{ matrix.vw }}' }
      - name: screen (odiff)
        id: diff
        run: |
          npx odiff "baseline/delivered/${{ matrix.vw }}.png" "qa-out/${{ matrix.vw }}.png" \
            "qa-out/diff-${{ matrix.vw }}.png" --threshold=0.05 --antialiasing || echo "changed=1" >> $GITHUB_OUTPUT
      - name: full metrics + issue
        if: steps.diff.outputs.changed == '1'
        run: node scripts/mia-ffi.mjs && gh issue create \
          --title "[VRT-WATCH] ${{ matrix.vw }}px 視覚デグレ検出" \
          --body-file qa-out/report.md --assignee kaito --label "vrt/degrade"
...
**期待効果**：
「納品したら終わり」の QA が「納品後 90 日の品質保証」に変わる。クライアントが崩れに気付く前に Mia が検出して Kaito へ回すため、LP 部の信頼が納品時点でなく運用期間全体で積み上がる。

---

### 🚫 新たに定義したNGパターン

1. **単一指標での合否判定 NG**：pixelmatch の差分率だけで通過/差し戻しを決める。幾何・色・構造・文字・動きの 5 軸を分離計測せず、原因軸を特定しないまま Ren へ投げるのは差し戻しとして不成立。FFI と軸別スコアの併記がないレポートは発行しない
2. **ノイズフロア未測定でのしきい値設定 NG**：案件開始時に同一ページ 5 回撮影の総当たり比較を行わず、前案件のしきい値を流用する。ゼロ点調整なしの計測であり、その QA 結果は数値として無効。`calibration/threshold.json` が存在しない状態で STEP 1 に入らない
3. **平均値での色判定 NG**：ΔE00 や差分率を平均値で評価する。大面積の白背景が平均を押し下げ CTA の色ズレを隠すため、**p95 と ΔE>2 面積率**での判定を必須とする。「平均 ΔE00 0.8 なので合格」は根拠にならない
4. **実時間再生でのモーション比較 NG**：アニメーションを実時間で再生してスクショ比較する。`getAnimations()` の `currentTime` を固定せずに撮ったフレームは非決定論であり、一致も不一致も証拠能力がない。t=0/0.25/0.5/0.75/1.0 の 5 点固定サンプリングを経ないモーション判定は無効
5. **CSS 値一致をもってタイポ合格とする NG**：`font-size`／`line-height`／`letter-spacing` の一致だけで文字軸を合格にする。行box 実測（行数・折返し点）を突合していない合格は、実データ投入で必ず崩れる。行数不一致は軽微差異ではなく下限割れとして扱う
6. **FP/FN を記録せずにしきい値を緩める NG**：誤 NG が多いという体感だけでしきい値を緩和する。台帳に FP 率の実数が記録されておらず、かつ FN が 0 であることを確認していない緩和は、見逃しの温床として禁止。緩和は必ず双方向（FN 発生時は締める）で運用する
7. **無審査 baseline 更新 NG**：`--auto-accept-changes` 相当の一括承認、および承認台帳（誰が・いつ・どのセレクタを・何故）の記載なき baseline 更新。劣化が累積して基準そのものが壊れるため、Saki/Kaito の明示承認と対象セレクタ限定の部分更新以外は行わない
8. **納品時 FFI 未記録 NG**：納品時点の全ビューポート・全 DPR のゴールデンイメージと FFI スコアを `baseline/delivered/` に確定保存しないまま案件をクローズする。後日のデグレを「元からそうだったのか、後で壊れたのか」判定できなくなり、保守フェーズの責任分界が消失する
9. **幾何 NG を残したままのピクセル比較 NG**：IoU < 0.95 の要素がある状態で pixelmatch／ΔE00 を回す。全面が差分で赤く染まり本質的な色・構造の NG が埋もれるため、**幾何軸をクリアしてから色・構造軸へ進む**順序を守る

### 📈 強化後の到達水準

| 観点 | v1（強化前） | v2（強化後） |
|---|---|---|
| 判定指標 | pixelmatch 差分率＋裁量配点の 100 点 | **FFI（幾何 IoU／ΔE00／SSIM／タイポ実測／フレーム SSIM の 5 軸加重合成）＋軸別下限ゲート** |
| 合格ライン | 総合 85 点＋カテゴリ下限 12/20 | **FFI ≥ 92（無条件通過は 96）＋5 軸すべてが下限クリア＋事実整合 0/100 二値** |
| 色判定 | HEX ±5＋肉眼 3 デバイス確認 | **ΔE00 全画素マップ（p95 < 2.0／面積率 < 1.0%／ブランド色 < 1.0）＋ヒートマップ成果物** |
| レイアウト判定 | ±2px 目視／相対比率の裁量 | **DOM ジオメトリ IoU 全数突合（≥ 0.995、重心 ≤ 2px）／ワースト 20 自動起票** |
| 文字判定 | CSS 値の一致確認 | **行box 実測（行数完全一致／折返し点 ≥ 98%／実効 line-height ≤ 0.5px 差）＋フォールバック状態計測** |
| モーション判定 | duration/easing の数値照合 | **t=0/0.25/0.5/0.75/1.0 の決定論フレーム SSIM ≥ 0.970＋cubic-bezier 制御点 ≤ 0.05 差** |
| しきい値 | 経験値（0.05／0.2〜0.3） | **案件ごとのノイズフロア実測 × 1.5 で自動決定＋FP 率 ≤ 5%／FN 0 の台帳運用** |
| 納品後 | CrUX 性能監視 7 日 | **日次 VRT 監視 90 日（FFI 低下 3pt でアラート）＋承認台帳付き baseline ガバナンス** |
| 説明責任 | 「差分率 1.2% で NG」 | **「色軸 p95 = 2.8 で下限割れ、対象は `[data-testid=cta-primary]`、ヒートマップ添付」** |
