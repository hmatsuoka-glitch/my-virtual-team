...
### 2026-09-21
- **2026年最新トレンド：Playwright v1.50+ の`toHaveScreenshot()` + Percy 2026版でピクセル差分閾値をAI判定**：従来の`maxDiffPixelRatio: 0.01`固定閾値だと「フォントヒンティング差」だけで数百ピクセル差分が出て検出ノイズ化。Percy 2026のAI Visual Diffが「意味のある差分」と「レンダリング揺れ」を自動分離し、真の欠陥だけを通知。7社LPのQA工数が体感で40%削減（実務影響：iPad承認端末でのフォントヒンティング差の誤検出がゼロ化）
- **2026年最新トレンド：Chrome DevTools MCP + `puppeteer-cluster`によるヘッドレスマルチデバイス並列QA**：iPhone SE / iPhone 15 Pro / Galaxy S24 / iPad Pro / Nexus 5X等12機種を並列起動し、全解像度スクショと差分検出を10分で完了。手動デバイスラボの巡回テストと同等品質が自動化可能に（実務影響：物理端末が無い場合でも12機種のcross-browserテストが完結）
- **2026年最新トレンド：Core Web Vitals 2026スペック更新（INP 150ms・CLS 0.08）への対応検証必須化**：Google Search Consoleが2026-Q4以降、旧基準合格LPも「要改善」に格下げ予定。`web-vitals@4.x`を`onINP()`・`onCLS()`実測モードで組み込み、Lighthouse Labテストではなくfield data基準の合否判定に切替（実務影響：SEO順位維持のためのQA基準を先取り）
- **失敗パターン：AI Visual DiffがOKと判定した差分の中に、ブランドロゴの余白が2px狭くなっている微差が紛れ込み、クライアントCEOが目視で発見** → 回避策：ブランドロゴ・企業名・許可番号の3要素は「AI判定に依存せず必ずピクセル単位マスク比較」に固定し、`test.only()`でロゴ領域を切り出す専用テストケースを常設（理由：AI判定は「見た目自然」を優先するためロゴのわずかなズレを許容してしまう、承認者が最も気にする箇所なので機械判定に任せない）
- **失敗パターン：Playwright実行時のシステムフォント読み込み待機が不十分で、Noto Sans JPが未適応の状態でスクショが撮られ、実際の本番と別レイアウトが記録される** → 回避策：全テスト冒頭で`await page.evaluateHandle(() => document.fonts.ready)`をawaitし、`page.waitForFunction(() => document.fonts.status === 'loaded')`で二重確認する（理由：CI環境のフォントキャッシュ状態によりレンダリングが揺れ、差分検出が信頼できなくなる事故が2026-Q3で3件）
- **品質チェックポイント：`view-transition-name`を使ったページ遷移アニメーション中のスクショタイミング制御**：View Transitions APIによるフェード/スライド遷移中の中間フレームでスクショを撮ると本番と異なる画像に。`page.waitForFunction(() => !document.startViewTransition)`で遷移完了を明示待機し、遷移前後の2状態を別テストとして分離（実務影響：AnimationフレームのFalse Positive誤検出ゼロ化）
- **品質チェックポイント：`prefers-reduced-motion`・`prefers-contrast: more`・`forced-colors: active`の3アクセシビリティモードでの目視確認をQA必須項目化**：官公庁・自治体LPだけでなく、建設業クライアントでもシニア応募者比率が高いため必須化。Playwrightの`emulateMedia({ reducedMotion: 'reduce' })`で3モード分のスナップショットを追加取得（実務影響：40代以上求職者のクレーム予防、AA準拠のためのエビデンス確保）
- **効率化テクニック：GitHub Actions matrix strategyで「7社×4デバイス×3アクセシビリティモード」= 84テストを並列実行**：`matrix: { client: [...], device: [...], a11y_mode: [...] }`で並列度84に設定、`fail-fast: false`で全結果を1回で回収。手動なら4時間分のQAを15分で完了（実務影響：週次リリース前の最終QAが劇的短縮）
- **Ren・Saki連携の小ヒント：NG検出時のフィードバックにDOM要素の`data-testid`＋`getComputedStyle`差分JSONを機械添付**：「ここが崩れています」の言葉ではなく、`{ selector: '[data-testid="hero-cta"]', expected: { padding: '16px' }, actual: { padding: '14px' } }`のJSONを直接投げる。Saki・Renの修正が数分単位で完了し、往復回数が体感で半減
