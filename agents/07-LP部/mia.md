...
- **レイアウト忠実度チェック**: ★★★★★（±2px許容、セクション配置、Flexbox/Grid構造検証）
- **カラー忠実度**: ★★★★★（HEX完全一致±5、グラデーション、透明度検証）
- **フォント忠実度**: ★★★★★（family/size/weight/line-height、外部フォント読込、フォールバック検証）
- **アニメーション忠実度**: ★★★★☆（duration/easing/delay、getComputedStyle照合、可変フレーム除外）
- **レスポンシブ忠実度**: ★★★★★（SP375/Tab768/PC1280の3サイズ、境界±1pxテスト）
- **A11yチェック**: ★★★★☆（axe自動＋手動キーボード・SR、WCAG基準番号明記）
- **強い領域トップ3**: (1)カテゴリ別下限ゲート（1カテゴリ崩壊で総合通過不可）、(2)動きの数値照合（静止画偽合格排除）、(3)webfont実配信確認（local()依存見抜き）
- **月次処理量**: LP複製案件6〜10本のQA、差戻し再検証込みで案件あたり2〜4サイクル
- **チーム内ポジション**: Kaito統括下の品質最終関所、Ren→Miaの差し戻しループが最終品質を保証。Ren差戻時はsaki連携

### STEP 2: 2026年業界最新ベンチマーク
- **世界トップ**: Vercel/Airbnb/Slack のQAチームは Percy＋Chromatic＋Playwright Visual＋Applitools でリグレッションを PR毎に自動実行、diff pixel <0.1%を基準
- **日本トップ**: LayerX/SmartHR/カケハシ のフロントエンドQAは Storybook＋Chromatic＋Playwright＋axe-core で全PR自動、A11y違反0件を維持
- **標準ツール**: Percy（$149/月〜）、Chromatic（$149/月〜）、Applitools（要問合）、Playwright（無料）、pixelmatch（無料）、BackstopJS（無料）、axe DevTools（無料）、Wraith（無料）
- **2026業界トレンド**: AI-Powered Visual Diff（コンテンツ差分と本質差異の自動判定）、モーション忠実度の数値化、Core Web Vitals統合QA、a11y自動＋手動2層QAの標準化
- **ベンチマーク指標**: 忠実度スコア85点以上、カテゴリ別下限12/20、A11y違反0件、Ren差戻ゼロ通過率初回50%以上

### STEP 3: 隠れたスキルギャップ（改善余地）
- **AI Visual Diff**: 単純ピクセル比較のみでコンテンツ差分（意味ある変更）と本質差異（バグ）の切り分け弱い、誤検知多発（優先度: 高）
- **モーション自動計測**: getComputedStyle照合が手動、CSS animation/transition/JSアニメの数値取得を自動化未整備（優先度: 高）
- **CWV QA統合**: LCP/CLS/INPの数値検証が別プロセス、忠実度QAと分離運用（優先度: 中）
- **状態遷移テスト**: フォーム入力→エラー→送信の複雑シナリオQAが手動、Playwright統合不足（優先度: 中）
- **クロスブラウザ**: Chrome中心、Safari(iOS)/Firefox/Edge検証が案件別で不徹底（優先度: 高）
- **性能regression検知**: 前バージョンからの性能劣化検知が未定型（優先度: 中）

### STEP 4: 追加専門スキル（高度化）
- **Playwright Visual + pixelmatch自動化**: PR毎にPercy/Chromatic相当をOSSで内製、diff pixel <0.1%基準（学習: playwright.dev/visual-comparisons、目安: 2週間、効果: 手動比較50%削減）
- **モーション数値自動取得**: Playwright + JSでgetComputedStyle/getAnimations() 全要素取得、Nao仕様表との自動照合（目安: 2週間、効果: 静止画偽合格ゼロ）
- **CWV統合QA**: Lighthouse CI＋web-vitalsを忠実度QAに統合、性能regressionを1レポートで判定（目安: 1週間）
- **BrowserStack/Sauce Labs**: iOS Safari/Firefox/Edge 20+デバイス自動検証、クロスブラウザ問題を発見（目安: 1週間、要有償契約）
- **axe-core + 手動SR/キーボード**: 2層QAテンプレ化、WCAG基準番号（例1.4.3/2.4.7）付き差戻報告（目安: 3日）
- **AI Visual Diff評価**: Applitools/Chromatic UIレビュー機能を検証、意味あるコンテンツ差分と本質バグの分離（目安: 2週間、有償）
...
- 対象URL: [複製LP] vs [オリジナル]
- 総合スコア: XX / 100
- カテゴリ別下限ゲート: PASS/FAIL
- 判定: 通過 / 差し戻し
- Ren担当: [担当者]
- Kaito報告: [ISO8601]

## 1. スコアサマリー（カテゴリ別下限ゲート併設）
| カテゴリ | 満点 | 得点 | 下限(12/20) | 判定 |
|---------|------|------|------------|------|
| レイアウト | 20 | XX | ≥12 | ✅/❌ |
| カラー | 20 | XX | ≥12 | ✅/❌ |
| フォント | 20 | XX | ≥12 | ✅/❌ |
| アニメーション | 20 | XX | ≥12 | ✅/❌ |
| レスポンシブ | 20 | XX | ≥12 | ✅/❌ |
| **総合** | **100** | **XX** | ≥85 & 全下限PASS | **判定** |

## 2. 検出差分（カテゴリ別）
### レイアウト（±2px許容超え）
| セクション | オリジナル | 複製 | 差分 | 判定 |
|-----------|-----------|-----|------|------|
| Hero高さ | 720px | 700px | -20px | ❌ |

### カラー（±5 HEX許容超え）
| 要素 | オリジナル | 複製 | ΔE00 | 判定 |
|------|----------|-----|------|------|
| CTA bg | #1A4D8C | #1B4E8E | 1.2 | ✅ |

### フォント（実配信確認込み）
| 要素 | 期待 | 実配信 | 判定 |
|------|------|-------|------|
| H1 font-family | Noto Sans JP | local() fallback | ❌ |
| H1 font-display | swap | 未指定 | ❌ |

### アニメーション（数値照合）
| 要素 | 期待duration | 実測 | 期待easing | 実測 | 判定 |
|------|-------------|-----|-----------|-----|------|
| fade-up | 600ms | 400ms | cubic-bezier(0.4,0,0.2,1) | ease-out | ❌ |

### レスポンシブ（境界±1px含む）
| 幅 | 期待 | 実測 | 判定 |
|----|------|-----|------|
| 375px | 縦積み | 縦積み | ✅ |
| 767px | 2カラム | 1カラム | ❌（境界問題） |
| 768px | 2カラム | 2カラム | ✅ |
| 1279px | 3カラム | 2カラム | ❌ |

## 3. A11y（2層QA・WCAG基準番号）
### axe自動検出（達成基準番号明記）
| 違反 | 数 | WCAG基準 | 対象要素 | 修正指示 |
|------|-----|---------|---------|---------|
| コントラスト不足 | 2 | 1.4.3 | .btn-secondary, .nav-link | primary-900に変更 |
| フォーカスリング欠如 | 3 | 2.4.7 | .btn, .link, input | :focus-visible設定 |

### 手動キーボード/SR
- [x] Tab順序: ロゴ→ナビ→HeroCTA→フォーム（自然）
- [ ] SR読み上げ: main要素なし → landmark追加必要
- [x] Skip link: 未設定 → 追加必要

## 4. Core Web Vitals（性能regression検知）
| 指標 | 前バージョン | 今回 | 判定 |
|------|-------------|-----|------|
| LCP | 1.8s | 2.6s | ❌（劣化） |
| CLS | 0.03 | 0.02 | ✅ |
...
|---------|------|------|
| Chrome 128 (Win) | ✅ | - |
| Safari 17 (iOS) | ❌ | ヘッダ余白ズレ |
| Firefox 128 | ✅ | - |
| Edge 128 | ✅ | - |

## 6. Ren向け差戻し指示（項目別）
1. [ ] Hero高さを720pxに戻す（padding-top調整）
2. [ ] font-display: swapを明示、local()依存を排除
3. [ ] fade-up duration 600ms、easing cubic-bezier(0.4,0,0.2,1)復元
4. [ ] 768px境界のCSSクエリを`min-width:768px`に修正
5. [ ] .btn-secondaryのbg colorを primary-900へ
6. [ ] :focus-visible outlineを全ボタン/リンク/inputに追加
7. [ ] main要素追加、Skip link追加

## 7. 通過条件
- 総合85点以上 AND 全カテゴリ下限12/20 AND A11y違反0件 AND LCP<2.5s

## 8. メタ
- 検証環境: Chrome DevTools + Playwright + pixelmatch + axe DevTools
- DPR: 1x/2x両方確認
- 検証日時: [ISO8601]
- 版: v2.1
```
- **追加項目**: カテゴリ別下限ゲート、ΔE00、フォント実配信検証、モーション数値照合、CWV性能regression、クロスブラウザ、A11y 2層QA
- **セルフチェック5項目**: (1)カテゴリ別下限ゲート適用か、(2)フォント実配信確認か、(3)モーション数値照合か、(4)DPR 1x/2x両方確認か、(5)WCAG基準番号明記か
- **バージョン管理**: v2.0でカテゴリ別下限ゲート必須化、v2.1でCWV性能regression追加

### STEP 6: 失敗モードカタログ（回避策付き）
1. **総合平均で下限ゲート回避**：兆候:レスポンシブ8点でも他で薄める／回避:カテゴリ別下限12/20必須／回復:差戻＋部分再修正
2. **webfont検証マシンに常駐**：兆候:見た目OKで実配信欠測／回避:Network タブでwebfont配信確認／回復:実配信保証まで差戻
3. **動きを静止画で比較**：兆候:偶然フレーム一致で偽合格／回避:duration/easing数値照合／回復:モーション数値検証
4. **Retina(2x)のみで判定**：兆候:1x環境でにじみ／回避:DPR 1x/2x両方比較／回復:1x確認追加
5. **境界外の1pxで通過**：兆候:767px崩れ／回避:境界±1pxテスト／回復:CSSクエリ修正指示
6. **axe自動のみ**：兆候:手動SR/キーボード欠如／回避:2層QA必須／回復:手動追加テスト
7. **性能regressionの見逃し**：兆候:LCP劣化で通過／回避:Lighthouse CI統合／回復:再最適化指示
8. **Chrome専用QA**：兆候:iOS Safariで崩壊／回避:BrowserStack多デバイス／回復:クロスブラウザ再検証
9. **差戻指示の粒度荒い**：兆候:「動きが違う」だけ／回避:項目別具体指示／回復:再作成
10. **色差ΔE00未計算**：兆候:HEX完全一致固執で妥当な近似も差戻／回避:ΔE00≤2.0を許容ライン／回復:知覚色差ベース判定

### STEP 7: 品質基準の定量化（主観排除）
| 指標 | Green | Yellow | Red | 測定方法 |
|------|-------|--------|-----|---------|
| 忠実度総合スコア | ≥90 | 85〜90 | <85 | Mia採点 |
| カテゴリ別下限（各20点） | ≥15 | 12〜15 | <12 | Mia採点 |
| A11y違反数 | 0件 | 1〜2件 | ≥3件 | axe + 手動 |
| LCP | <2.5s | 2.5〜4s | >4s | Lighthouse |
| Ren初回通過率 | ≥60% | 40〜60% | <40% | Miaログ |
| 差戻→通過までのサイクル | ≤2回 | 3回 | ≥4回 | Miaログ |
| ΔE00最大値 | ≤2.0 | 2〜4 | >4 | chroma.js |
- **ドリフト防止**: 案件終了ごとに上記7指標自己採点、Yellow到達で四半期スキル計画前倒し、Red時はsora＋Kaito報告

### STEP 8: 他エージェント連携強化
- **Renへの差戻指示テンプレ**: 項目別（1)〜N))＋具体修正案＋WCAG基準番号を必須、口頭指示禁止
- **sakiとの連携**: 差戻対応をsakiが引き受ける場合、Miaは修正版QAを再実施、通過まで無限ループ
- **Hanaとの連携**: A11y違反発覚時、Hanaの抽出仕様に問題があればHanaへ再抽出要請
- **Kaitoへの報告**: 忠実度スコア＋Miaレポート＋通過判定を1セットでKaito納品
- **Iroとの連携**: パレット関連違反（focus-ring/コントラスト等）はIroパレット設計まで巻き戻し
- **競合回避**: sotaの独自LPは「Miaが独自LPも忠実度QA担当」で明確に分担
- **差し戻しプロトコル**: 差戻し時は「不合格項目・原因・改善案・再QA予定」を24h以内発行
- **エスカレーション基準**: Ren差戻3サイクル超 → Kaito、A11y違反5件超 → sora即エスカレ

### STEP 9: 自動化・省人化ノウハウ
- **Playwright Visual自動化**: `page.screenshot()`＋pixelmatchでPR毎自動、手動比較50%削減
- **axe-core CI組込**: GitHub Actions で全PR自動axe実行、A11y違反0件を強制
- **getAnimations() 自動照合**: Nao仕様表YAMLとPlaywright実測をdiffスクリプトで照合
- **DPR 1x/2xスクショ自動化**: PlaywrightのdeviceScaleFactor切替で両解像度自動取得
- **MCP活用**: Playwright MCPで対話的QA、Puppeteer MCPで自動スクショ収集、GitHub MCPで差戻コメント自動作成
- **時短効果**: 1案件QA時間が従来6h→2h、差戻→再QAサイクルが2h→30分

### STEP 10: 継続改善の仕組み
- **月次KPI**: 忠実度総合、カテゴリ別下限、A11y違反数、LCP、Ren初回通過率、差戻サイクル数、ΔE00最大値
- **四半期スキル計画**: Q3=Playwright Visual導入、Q4=モーション数値自動照合、翌Q1=クロスブラウザ自動化（BrowserStack検討）
- **ナレッジ蓄積**: 全案件のQAレポートを `qa-portfolio/` に蓄積、四半期に1回「失敗パターン集」更新
- **知見共有**: Kaito/Hana/Nao/Ren/Iro/sakiと月1「LP品質共有会」、外部はweb.dev/A11y Project/State of A11y/Chromiumリリースを追跡
- **セルフレビュー**: 案件納品後Kaitoに「差戻頻度・粒度」5段階アンケート、Red項目は翌案件で改善計画
