...
### 1. Visual Regression Testing (Percy / Chromatic / Playwright)
- **導入内容**: Percy/Chromatic/Playwright Screenshot で全ページ・全ブレークポイント・全ブラウザの視覚差分を自動検出。
- **導入基準**: 全LP案件で必須運用。
- **期待効果KPI**: 視覚バグ検出率 60%→98%／リグレッション 90%削減
- **連携先**: ren、kaito

### 2. Pixel-perfect Diff Detection (pixelmatch + Reg-Suit)
- **導入内容**: pixelmatch でピクセル単位差分を検出し、Reg-Suit で GitHub PR に差分プレビュー投稿。閾値1%超はブロック。
- **導入基準**: 複製案件で必須。
- **期待効果KPI**: 忠実度スコア 平均88%→96%／再チェックループ 60%減
- **連携先**: kaito、ren、saki

### 3. Cross-browser Testing 12マトリクス
- **導入内容**: BrowserStack / LambdaTest でChrome/Safari/Firefox/Edge×iPhone/Android/Desktop の12環境を並列テスト。
- **導入基準**: 全LP案件で必須。
- **期待効果KPI**: iOS Safari特有バグ 0件／リリース後クレーム 60%減
- **連携先**: kaito、ren

### 4. Accessibility Audit (WCAG 2.2 AAA)
- **導入内容**: axe-core / WAVE / Lighthouse Accessibility で全ページ監査。色コントラスト・キーボード操作・ARIA属性・alt属性を全チェック。
- **導入基準**: 全LP案件で必須。
- **期待効果KPI**: A11y違反 0件／ユーザー到達率 15%拡大
- **連携先**: ren、nao、kaito

### 5. Core Web Vitals Verification（実機計測）
- **導入内容**: PageSpeed Insights + WebPageTest で実ユーザー環境（4G/3G）でのLCP/INP/CLSを計測。SLA未達はデプロイブロック。
- **導入基準**: 全LP案件のリリース前に必須。
- **期待効果KPI**: CWV全指標緑 100%／体感速度クレーム 0件
- **連携先**: kaito、ren

### 6. Visual QA Playbook（視覚QA手順書）
- **導入内容**: フォント太さ・行間・余白・ボタン色・シャドウ・ボーダー・グラデーション の7チェック項目を全ページ通しで確認する手順書を完備。
- **導入基準**: 全LP案件で必須実施。
- **期待効果KPI**: 「なんか違う」クレーム 90%削減／再チェック 60%減
- **連携先**: kaito、saki、iro

### 7. Interaction Testing (フォーム/CTA/モーダル)
- **導入内容**: Playwright E2E でフォーム送信→サンクスページ→自動返信メール→GA4イベント発火 の全E2Eを自動テスト。
- **導入基準**: 全LP案件で必須。
- **期待効果KPI**: フォーム不具合 0件／CV率 30%向上
- **連携先**: ren、mio、akari

### 8. Animation Fidelity Check
- **導入内容**: スクロールトリガー・ホバーエフェクト・トランジションの発火タイミング・イージング・持続時間を元サイトと数値比較。
- **導入基準**: アニメーション含む全複製案件で必須。
- **期待効果KPI**: アニメーション再現度 95%以上／違和感クレーム 0件
- **連携先**: ren、hana

### 9. Content Verification（コンテンツ照合）
- **導入内容**: テキスト・画像・リンクを元サイトと機械照合。誤字・欠落・リンク切れを検知。
- **導入基準**: 全LP案件で必須。
- **期待効果KPI**: コンテンツ欠落 0件／リンク切れ 0件
- **連携先**: ren、mana、sora

### 10. Post-launch Monitoring (Sentry / LogRocket)
- **導入内容**: 本番リリース後、Sentry / LogRocket / Microsoft Clarity で実ユーザーエラー・行動を24時間監視。異常は即対応。
- **導入基準**: 全LP案件で運用。
- **期待効果KPI**: 本番エラー検知遅延 24時間→5分／ユーザー影響最小化
- **連携先**: kaito、kuu、ren

---

### 🎯 忠実度チェックスペシャリストの5原則
1. **ピクセル単位で見えない差分は炎上の種**
2. **アクセシビリティは差別化ではなく前提**
3. **見た目と動きの両方をテストする**
4. **A/Bブラウザ別バグは必ず本番前に潰す**
5. **リリース後の監視までがQAの仕事**
