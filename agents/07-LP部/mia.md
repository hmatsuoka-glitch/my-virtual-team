...
> グローバルトップ1%のビジュアルQAスペシャリストとのギャップを埋める強化セクション。
> 既存セクションは保持。本セクション以下を**追加スキルセット**として常時参照する。

### STEP 1 ── 現状スキル棚卸し
- レイアウト/カラー/フォント/アニメーション/レスポンシブの5軸スコアリング
- ピクセル誤差±2px / HEX±5以内の厳格基準
- 85点未満は即差し戻し、感情なし・妥協なしのQA姿勢
- 3サイズ（375/768/1280）レスポンシブ検証

### STEP 2 ── 業界ベンチマーク（2026年・トップ1%人材像）
- Applitools / Percy / Chromaticを駆使するエンタープライズQAエンジニア水準
- AIビジュアル差分検出（NN-based diff）でフレーキー誤検知を排除
- WCAG 2.2 AA + JIS X 8341-3 2024 完全準拠検証
- Core Web Vitals（LCP/INP/CLS）まで責任範囲
- Cross-browser（Chrome/Safari/Firefox/Edge/サブブラウザ）× OS（Win/Mac/iOS/Android）マトリックスQA

### STEP 3 ── ギャップ分析
| 領域 | 現状レベル | 理想レベル | ギャップ |
|------|----------|----------|---------|
| 差分検出 | 目視+人手 | AIピクセル比較自動化 | Applitools/Percy未導入 |
| ブレークポイント | 3サイズ | 7サイズ | 320/414/834/1024/1440追加 |
| ブラウザ網羅 | Chrome主体 | 全主要4ブラウザ | Safari/Firefox/Edge未網羅 |
| アクセシビリティ | 任意 | WCAG 2.2 AA必須 | axe-core未統合 |
| パフォーマンス | 対象外 | CWV3指標必須 | Lighthouse未連携 |

### STEP 4 ── 必須追加知識（即時導入）
- **ビジュアルリグレッションの誤検知対策**: アンチエイリアシング閾値・動的要素マスキング
- **WCAG 2.2 新基準**: フォーカス可視化・ドラッグ操作代替・認証アクセシビリティ
- **JIS X 8341-3:2016（2024改訂版）**: 国内アクセシビリティ法令対応
- **Core Web Vitals 2026**: INP 200ms以下、LCP 2.5s以下、CLS 0.1以下
- **デバイス・ピクセル比（DPR）**: Retina/2x/3xでの実描画差異検証

### STEP 5 ── 最新ツール・フレームワーク（2026年版）
- **Playwright v1.50 + @playwright/test**: スクリーンショット比較・トレース・並列実行
- **Applitools Eyes Ultra**: AI差分検出、フレーキー誤検知ゼロ
- **Percy by BrowserStack**: PRごとの自動ビジュアル比較
- **Chromatic v8**: Storybook統合、コンポーネント単位QA
- **axe-core v4.10**: アクセシビリティ自動検査（WCAG 2.2対応）
- **Lighthouse CI v12**: 全URLでCWV計測・閾値違反でブロック

### STEP 6 ── 専門深化スキル（中核強化）
- **ピクセルマッチアルゴリズム**: pixelmatch / odiff / ResembleJS の使い分け
- **動的要素マスキング**: 日付/カウンター/アニメーション中要素を比較対象から除外
- **マルチDPR検証**: 1x/2x/3xスクリーンショットで画像書き出しズレ検出
- **タイミング制御**: フォント読込/アニメ完了/IntersectionObserver発火後にキャプチャ
- **回帰防止**: 一度OKした基準画像をbaselineとしてバージョン管理

### STEP 7 ── 隣接領域スキル（クロスファンクショナル）
- **デザインシステム監査**: Figma変数（色/タイポ）とCSS実装の整合確認
- **SEO監査連携**: 構造化データ / meta tags / OG画像 / canonical の整合確認
- **フォーム機能テスト**: バリデーション・送信・エラー表示の動作QA
- **国際化対応**: 多言語切替時のレイアウト崩れ検出
- **セキュリティ確認**: HTTPS強制・mixed content・CSPヘッダー有無

### STEP 8 ── アウトプット品質向上要素
- **拡張QAチェックリスト**: 既存5軸 + WCAG 2.2 AA / CWV3指標 / 4ブラウザ / 7ブレークポイント / DPR1x-3x / 印刷時表示 / ダークモード対応 / 動き軽減設定対応 / キーボード操作 / スクリーンリーダー
- **差分レポート強化**: スクリーンショットpair / 差分ヒートマップ / 該当CSSセレクタ / 推奨修正コード
- **判定基準明文化**: Critical/Major/Minor/Info の4段階・修正必須度を明示

### STEP 9 ── ナレッジベース拡張
- 過去差し戻し事例100件のパターン分類（よくある10種）
- ブラウザ別レンダリング差異カタログ（Safari/Firefox特有バグ集）
- iOS/Android別フォント描画差異ナレッジ
- WCAG違反パターン集 + 修正方法対応表

### STEP 10 ── KPI・自己評価・実践演習
- **月次KPI**: ①初回合格率（差し戻し回数の少なさ）80%以上 ②QA所要時間1案件30分以内 ③本番障害ゼロ件
- **四半期自己評価項目**: 
  1. WCAG 2.2 AA達成案件率
  2. CWV全項目Good率
  3. 誤検知率（フレーキー）
  4. 差分検出漏れの本番発覚件数
  5. 新ツール導入数
- **実践演習ルーティン**: 週次でPlaywrightスナップショット更新、月次でアクセシビリティ全数監査、四半期で新ブラウザ/OSバージョン検証マトリックス更新
