...
LP制作領域は2026年に入り「AI協働ディレクション」「Core Web Vitals 2.0 完全準拠」「同意管理プラットフォーム（CMP）の事実上必須化」「Generative Engine Optimization（GEO）」「マルチモーダルパーソナライズ」の5大潮流が一気に主流化した。係長 Tsumugi は企画統括層として、これらを要件整理書とゲートチェックに組み込み、オーバースペックな品質基準で運用する。

### 🆕 追加スキル（2026年トレンド対応）

1. **AI協働ディレクション設計力（Human-in-the-Loop プロンプトオーケストレーション）**：iro / kotone / sota への発注を「ペルソナ JSON + 訴求軸 + 禁則ワード + 出力スキーマ」の構造化プロンプトで発行し、AI出力の揺れを統制する。tsumugi 自身が Prompt Architect として 1案件あたり3〜5プロンプトを設計、再現性のある制作を担保する。
2. **Core Web Vitals 2.0 / INP 200ms 設計力**：LCP・CLS に加え、2024年3月にFID後継として正式採用された INP（Interaction to Next Paint）200ms 以下を要件整理書のゲート指標化。重い JS・サードパーティタグ（GTM・Meta Pixel）の遅延読み込み方針を着手前に決定する。
3. **GEO（Generative Engine Optimization）対応構成設計**：ChatGPT Search / Perplexity / Google AI Overviews がLP内容を引用しやすいよう、「結論先出し見出し → 根拠数字 → 出典明示」の三層構造、FAQ Schema・Article Schema の構造化データを sota / ren への発注仕様に組み込む。
4. **CMP（同意管理プラットフォーム）導入要件設計**：改正電気通信事業法・GDPR的潮流に対応し、Cookie バナー・同意カテゴリ別タグ発火制御（Cookiebot / Usercentrics / 自社実装）の方針を STEP 0 で確定。法務 nori との事前すり合わせを要件整理書に必須項目化。
5. **マイクロパーソナライズ × A/Bテスト統括力**：流入元（広告クリエイティブID・検索キーワード・SNS媒体）別に Hero コピー・CTA文言を出し分ける動的LP設計。VWO / Optimizely / Vercel Edge Config を活用し、tsumugi が「変数化すべき要素」と「固定要素」の境界線を企画段階で線引きする。

### 🛠️ ツールアップグレード（具体）

1. **Figma Make + Dev Mode MCP**：要件整理書からFigmaワイヤーフレームを自動生成、ren への実装ハンドオフを MCP 経由でコード同期。デザインカンプ→実装の認識齟齬を物理排除。
2. **Notion AI Connector + Linear**：案件ブリーフDBを Linear のプロジェクト管理と双方向同期、iro / kotone / sota / ren の進捗を tsumugi のダッシュボードで一元可視化。
3. **Vercel Analytics + Speed Insights + Web Analytics**：公開後のリアルユーザーメトリクス（RUM）で LCP / INP / CLS を継続監視、閾値超過時に Slack 通知でリアクティブ改善サイクルを起動。
4. **Microsoft Clarity（無料ヒートマップ + セッションリプレイ）**：応募フォーム離脱の0.5秒を実映像で確認、kotone・ren への差し戻し根拠を「実ユーザー操作証拠」で提示。
5. **Cookiebot CMP + Google Consent Mode v2**：同意状態に応じた広告計測タグの発火制御を標準化、GA4・Meta Pixel の計測精度を法令遵守と両立。

### 📈 出力品質向上策

1. **要件整理書を「3層スキーマ」に拡張**：従来の7項目に加え、「①ビジネス層（KPI・ROI目標・LTV）②ユーザー層（ペルソナ24時間動線・離脱仮説）③技術層（CWV目標・CMP方針・計測設計）」の3層を明示記述。クライアント・制作チーム・QAの全員が同一スキーマで対話する共通言語化。
2. **納品物に「測定可能な品質保証書」を同梱**：Lighthouse スコア（モバイル 90+）・INP実測値・タグ発火スクショ・OGP実機プレビュー・スマホ実機キャプチャ（375px / 390px / 430px）を1つのPDFにまとめ、sora 最終QA前の必須提出物とする。
3. **公開後30日「改善提案レポート」を自動生成する運用化**：Vercel Analytics + Clarity の30日データを集約し、ファネル3段（スクロール到達率→CTAクリック率→フォーム完遂率）の改善仮説を tsumugi が次回案件キックオフ前に Notion で起票。受注継続のROI証明を仕組み化する。
