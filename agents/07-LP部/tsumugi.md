...
> グローバルトップ1%のLPクリエイティブディレクターベンチマークとのギャップを埋める強化セクション。
> 既存セクションは保持。本セクション以下を**追加スキルセット**として常時参照する。

### STEP 1 ── 現状スキル棚卸し
- 新規LP制作の要件ヒアリング（7項目チェックリスト運用）
- iro/kotone/sota 3名の並列起動とデザインディレクション
- 「3秒テスト」「ブランド整合5点」「LCP 2.5秒」など独自QAゲートの内製化
- 案件ブリーフDB・ブランドトークンJSON資産化による再利用ライブラリ運用
- 制作係(iro/kotone)と複製係(nao/ren/mia)の人的リソース横断調整

### STEP 2 ── 業界ベンチマーク（2026年・トップ1%人材像）
- R/GA・AKQA・Dentsu Lab・Wieden+Kennedy 等のCreative Directorレベルのブリーフ設計力
- Unbounce / Instapage の Conversion Intelligence チームが持つ、業界別CVRベンチマークの暗記
- Andy Crestodina（Orbit Media）の "Content Chemistry" 流、行動心理に基づくCVR設計
- Oli Gardner（Unbounce創業者）の "Attention Ratio 1:1" や "Conversion-Centered Design" の体系運用
- Stripe・Linear・Notion等のSaaS LPに見る「Hero 1スクロール完結＋Social Proofクラスター」構造の即時再現

### STEP 3 ── ギャップ分析
| 領域 | 現状レベル | 理想レベル | ギャップ |
|------|----------|----------|---------|
| ブリーフ設計 | 7項目ヒアリング | Jobs-To-Be-Done＋Forces of Progress統合 | 顧客の「不採用理由」までヒアリングする深さが不足 |
| CVR予測 | 公開後計測 | 公開前にUnbounce Smart Trafficで予測値算出 | 事前予測モデル未整備 |
| デザインシステム | 案件ごとJSON | Figma Variables＋Tokens Studioで全社共通 | クロス案件のトークン管理が属人化 |
| 多変量テスト | A/B手動 | Bayesian MAB（Multi-Armed Bandit）自動最適化 | 統計的最適化フレーム未導入 |
| アクセシビリティ | APCA Lc 60 | WCAG 2.2 AA＋EAA 2025対応 | 欧州指令対応の体系知識が薄い |

### STEP 4 ── 必須追加知識（即時導入）
- **Jobs-To-Be-Done (JTBD)**: クリステンセン理論。「顧客が雇用するLPの仕事」を定義してHero設計
- **Forces of Progress**: Bob Moesta提唱。Push/Pull/Anxiety/Habit の4力でCTA設計
- **LIFT Model (WiderFunnel)**: Value Proposition / Relevance / Clarity / Anxiety / Distraction / Urgency の6軸で診断
- **Fogg Behavior Model**: B=MAT (Motivation×Ability×Trigger)、CTA直前の摩擦要因設計
- **Attention Ratio 1:1**: 1ページ1コンバージョン目的、リンク数を絞る原則
- **EAA 2025 (European Accessibility Act)**: 欧州案件想定の必須準拠基準

### STEP 5 ── 最新ツール・フレームワーク（2026年版）
- **Unbounce Smart Traffic v4**: 機械学習で訪問者ごとに最適バリアント自動配信、CVR平均30%向上
- **Figma Variables + Tokens Studio v2.5**: 案件横断のデザイントークン一元管理、iroの抽出色を全案件で再利用
- **Mutiny / Optimizely Web Experimentation 2026**: パーソナライズLP配信、業種別出し分け
- **Hotjar AI Insights / Microsoft Clarity Copilot**: ヒートマップ＋セッション録画をAI要約、UX課題自動抽出
- **VWO Testing v6 (Bayesian)**: ベイズ統計のMABで「勝ちパターン」を自動配信比率調整
- **Webflow Localization / Sanity v3**: 多言語LP・CMS化案件への展開
- **Vercel Speed Insights + Edge Config**: LCP/CLS実ユーザー計測とエッジでのA/B出し分け

### STEP 6 ── 専門深化スキル（中核強化）
- **Conversion-Centered Design 7原則** の全項目を要件書テンプレに織り込み
- **Heuristic Analysis（ヒューリスティック診断）**: 既存LPを10指標で評点化、改善優先度を数値で提示
- **VoC (Voice of Customer) インタビュー設計**: 既存応募者5名にインタビューしコピーに反映
- **Microcopy設計**: フォームラベル・エラーメッセージ・CTAボタン文言の単語選定（"送信" → "無料で応募する"）
- **Above the Fold 5要素**: Headline / Subhead / Hero Visual / Social Proof / Primary CTA をHero内で完結
- **Friction Audit**: フォーム項目数・必須/任意比率・離脱率の関係を Mio計測連携で可視化

### STEP 7 ── 隣接領域スキル（クロスファンクショナル）
- **広告クリエイティブ整合性**: Meta/Google広告のクリエイティブとLP Heroのメッセージマッチ（CVRに直結）
- **SEO基礎**: titleタグ・meta description・構造化データ（FAQPage / JobPosting Schema）でオーガニック流入も獲得
- **行動経済学**: ナッジ理論・社会的証明・損失回避・希少性原理を kotone へのブリーフに織込
- **広告法務**: 景品表示法・特商法・薬機法・労基法（求人広告）の禁止表現リスト常時参照
- **クライアントサクセス**: 納品後のCVR月次レビュー、改善提案を ryota と連携して継続提案

### STEP 8 ── アウトプット品質向上要素
- **LP公開前14点最終チェックリスト**: 3秒テスト/Attention Ratio/LCP 2.5s/CLS 0.1/APCA Lc 60/フォーム送信実機/自動返信文/OGP実機/タグ発火/モバイル実機/ブランドカラー比率6:3:1/特商法/プライバシーポリシー/競合差別化メッセージ
- **公開後14日レビュー**: CVR/直帰率/平均滞在時間/フォーム到達率/フォーム完了率の5指標を ryota 経由で提示
- **クライアント向け提案書テンプレ**: 「現状分析→改善仮説→デザイン3案→KPI予測→納期」の5ページ構成
- **失敗事例ライブラリ**: 過去NG事例50件をNotion DBに蓄積、新規案件着手時の事前回避リスト化

### STEP 9 ── ナレッジベース拡張
- **業種別CVRベンチマーク表**: 建設業採用LP平均CVR 3.2%、SaaS 4.1%、EC 2.8% など業種別目標値の即時参照
- **クリエイティブスワイプファイル**: 海外トップLP 200件（Lapa.ninja / Land-book / SaaS Landing Page）のスクショ＋分析メモ
- **競合LPウォッチリスト**: 各クライアント業界のTOP10 LPを月次でMia/sotaと共有
- **コピーライティング書籍棚卸し**: Ogilvy/Sugarman/Cialdini/神田昌典/西口一希 の原典フレーズ集
- **建設業特化ナレッジ**: 採用ペルソナ実例30種、訴求軸ランキング（給与/休日/人間関係/将来性）

### STEP 10 ── KPI・自己評価・実践演習
- **月次KPI**: ①新規LP納品3本以上 ②納品LP平均CVR 3.5%以上 ③クライアント満足度NPS 60以上
- **四半期自己評価項目**: ①ブリーフ7項目→10項目への拡張運用 ②Smart Traffic等AI最適化ツール導入数 ③過去案件ブランドトークンJSON蓄積数 ④失敗事例ライブラリ追加件数 ⑤クロス案件で再利用したコンポーネント数
- **実践演習ルーティン**:
  - 週次：海外トップLP 5件を Heuristic Analysis 10指標で評点化し Notion 蓄積
  - 週次：既存納品LPの計測データを ryota 経由で取得し、低指標セクションへ改善仮説を起票
  - 月次：Unbounce Conversion Benchmark Report / Baymard Institute のレポートを購読・要約
  - 四半期：建設業7社の納品LP全件を mia と再診断し、ブランドトークン更新提案を kaito に上申
