...
### 現状スキル棚卸し
- 新規LP制作統括、要件ヒアリング、進行管理は安定運用
- 実機表示・フォーム動作確認、FV内CTA確認は実装済み

### ベストプラクティスとのギャップ
1. **Design Sprint（GV式5日間）** の体系活用不足
2. **Jobs-to-be-Done（JTBD）** クライアント要件深掘り未活用
3. **RACI Matrix**（Responsible/Accountable/Consulted/Informed）でのチーム責任分担明確化が手薄
4. **Risk Register**（リスク登録簿）でのプロジェクトリスク管理未体系化
5. **Stakeholder Mapping** でのキーパーソン管理が暗黙知

### 追加フレームワーク・方法論
- **Design Sprint**：Day1 Understand / Day2 Diverge / Day3 Decide / Day4 Prototype / Day5 Test の5日間で意思決定加速
- **Jobs-to-be-Done**：「クライアントは何のジョブを片付けたくてLPを発注したか」の本質的ニーズ抽出
- **RACI Matrix**：iro/kotone/sota/ren/mia/saki/kaitoの7名の役割を明確化
- **Risk Register**：プロジェクトリスクを発生確率×影響度で優先度評価、対策事前準備
- **Stakeholder Mapping**：クライアント側の決裁者・実務者・推進者・反対者を整理

### MCP/ツール統合
- **mcp__Notion__**：プロジェクト管理ボード、RACI/Risk Register/Stakeholder Map
- **mcp__Google-Calendar__**：Design Sprintの5日間スケジュール自動化
- **mcp__Gmail__**：クライアントヒアリング招待・議事録共有
...
### 2026-06-02
- **Design Sprint導入**：5日間で要件定義→プロトタイプ→クライアント検証まで完了、要件定義5日→2日
- **Jobs-to-be-Done**：「LPの本質的ジョブ（求職者を獲得する/採用ブランディング/応募導線最適化）」を深掘り、提案精度向上
- **RACI Matrix明示**：7名チームの役割明確化、責任の曖昧さによる手戻りが消滅
- **Risk Register運用**：プロジェクトリスクを発生確率×影響度で優先度評価、事前対策で大事故ゼロ化
- **Stakeholder Mapping体系化**：決裁者・実務者・推進者・反対者の4種を整理、合意形成スピード3倍
- **建設業界クライアント別Stakeholder Map7社版**：各社のキーパーソンを構造化、初回MTGからの提案承認速度が劇的向上
