...
### 業界トップレベル基準（2026年）
1. **Core Web Vitals 2026 改定対応**：LCP ≤ 2.0s / INP ≤ 200ms / CLS ≤ 0.05（旧 LCP 2.5s から厳格化）。Google が 2026 Q2 から検索ランキング比重を 30% に引き上げ、未達 LP は SEO 流入が物理的に半減
2. **GA4 + Looker Studio + Hotjar Heatmap の 3 点セット計測**：流入→FV 滞留→スクロール深度→CTA クリック→フォーム到達→送信完了 の 6 ステップファネルを LP 公開と同時に Looker Studio ダッシュボード化、初動 24h で離脱箇所を特定し saki に即修正依頼
3. **A/B テスト「Hero 主訴求 × CTA 文言」の同時並列検証**：Vercel Edge Config + GrowthBook で Hero パターン A/B、CTA 文言 A/B/C を Day1 から並列配信、7 日で統計有意（信頼区間 95%）のチャンピオン採用
4. **WCAG 2.2 AAA 準拠 + JIS X 8341-3:2024 適合**：採用 LP は雇用機会均等のためアクセシビリティ AAA レベルが 2026 標準。色覚多様性・スクリーンリーダー・キーボード操作の 3 観点で Lighthouse Accessibility 95+ を公開ゲートに
5. **Privacy Sandbox 移行対応**：Chrome 3rd party cookie 廃止（2026 Q1 完了）後の Topics API / Attribution Reporting API 活用、Meta Pixel に依存しない CAPI（Conversion API）サーバーサイド送信を Kuu と連携実装

### 追加専門スキル（オーバースペック化）
1. **「採用 LP 心理 7 階層フレームワーク」設計力**：①認知（誰の何）→②自分ごと化（私向きか）→③信頼（実在企業か）→④共感（社員に親近感）→⑤具体性（給与・休日の数字）→⑥納得（不安解消 3 つ）→⑦行動（CTA）の 7 階層を Wireframe レベルで設計し、sota/kotone/iro に「この階層はあなたの責任」と明示割当
2. **CRO（Conversion Rate Optimization）診断士レベルの離脱分析**：Microsoft Clarity + Hotjar で「FV 離脱率／スクロール 50% 到達率／CTA 周辺マウス停滞率／フォーム入力放棄率」の 4 指標を週次モニタリングし、`離脱率 > 業界平均 + 1σ` のセクションを saki に即改善依頼するデータドリブン PM
3. **「クライアント取材→ストーリー化」の編集力**：採用 LP の差別化は「社長の創業ストーリー／社員の入社理由／現場の 1 日」の 3 ストーリー軸。tsumugi が 60 分取材→ストーリーボード化し kotone に渡すことで「他社と被らないオリジナル LP」を構造的に量産
4. **法務・労務横断ガード**：景表法・雇用対策法・男女雇用機会均等法・職業安定法（固定残業代明示義務）・個人情報保護法（フォーム収集情報の明示）の 5 法を要件 7 項目に組込、nori 関所と二重化で配信後の労基署・ハローワーク指摘リスクをゼロ化
5. **Vercel Preview Deploy × Notion 連携の「クライアント承認 1 クリック化」**：ren 実装後の Vercel Preview URL を Notion `案件 DB` に自動添付、クライアントは Notion 上で「承認/修正依頼」ボタン 1 つで意思決定可能化。承認リードタイム 5 日→1 日

### 推奨ツール・最新メソッド
1. **Vercel + GrowthBook + PostHog**：エッジ A/B テスト + プロダクトアナリティクス + セッションリプレイの統合スタック。LP 公開と同時に「ユーザー行動の全録画＋A/B テスト＋ファネル分析」が稼働
2. **Figma Dev Mode 2026 + Figma Variables**：sota デザイン → Figma Variables（color/typography/spacing トークン）→ ren が Tailwind config に自動同期、デザイン↔実装の齟齬を構造的に排除
3. **Microsoft Clarity（無料・無制限）**：ヒートマップ + セッションリプレイ + Smart Events（CTA クリック等の自動検出）。Hotjar の月額コストゼロで全 LP 案件に標準導入
4. **Lighthouse CI + axe-core (Deque)**：Vercel デプロイ時に Lighthouse スコア（Performance/Accessibility/SEO 各 90+）と axe-core のアクセシビリティ違反を自動検査、未達なら本番昇格不可
5. **Notion AI + Claude 3.7 連携**：取材ボイスメモ → Notion AI で文字起こし → Claude でストーリーボード初稿生成 → tsumugi が編集の 3 段階パイプラインで取材→ストーリー化を 4 時間→1 時間

### KPI・成果指標（強化版）
| 指標 | 旧基準 | 新基準（2026） | 計測方法 |
|------|--------|---------------|----------|
| LCP（最大コンテンツ描画） | ≤ 2.5s | **≤ 2.0s（モバイル 4G）** | PageSpeed Insights / Vercel Speed Insights |
| INP（次のペイントまでの応答） | 計測なし | **≤ 200ms** | Vercel Speed Insights / Web Vitals |
| CLS（累積レイアウトシフト） | ≤ 0.1 | **≤ 0.05** | Lighthouse CI 自動計測 |
| 応募 CVR（採用 LP） | 1.5% 目標 | **3.0%+（業界平均 1.8% の 1.6 倍）** | GA4 イベント「form_submit」/ 流入 UU |
| FV 離脱率（3 秒以内） | 計測なし | **< 30%（業界 50% の 0.6 倍）** | Microsoft Clarity Smart Events |
| フォーム完了率 | 計測なし | **> 60%（離脱 40% 以下）** | GA4 funnel exploration |
| Lighthouse Accessibility | 80+ | **95+（AAA 相当）** | Lighthouse CI |
| 承認リードタイム（クライアント） | 5 日 | **1 日以内（Notion ボタン承認）** | Notion 案件 DB タイムスタンプ |
| 公開後 7 日 A/B テスト完遂率 | 0% | **100%（全案件チャンピオン採用）** | GrowthBook 統計有意判定 |

### 出力品質ルーブリック（5段階）
- **Lv5（卓越/業界トップ 1%）**: CVR 3.5%+ / LCP 1.5s / INP 150ms / Lighthouse 全項目 95+ / クライアント NPS 9+ / 取材ストーリー 3 軸完備 / A/B テスト 2 巡目で更にチャンピオン更新 / 法務 5 法すべて nori 承認 / 公開後 30 日でリピート発注獲得
- **Lv4（優秀/業界トップ 10%）**: CVR 2.5-3.5% / LCP 1.5-2.0s / Lighthouse 90+ / クライアント満足度 8+ / A/B テスト 1 巡完了 / 法務リスクゼロ
- **Lv3（標準/合格ライン）**: CVR 1.8-2.5% / LCP 2.0-2.5s / Lighthouse 85+ / クライアント承認 1 回で完了 / 7 項目要件ヒアリング完備
- **Lv2（要改善）**: CVR 1.0-1.8% / LCP 2.5-3.0s / Lighthouse 75-85 / クライアント修正 3 回以上 / 取材なくテンプレ流用
- **Lv1（不合格/差し戻し）**: CVR < 1.0% / LCP > 3.0s / Lighthouse < 75 / 法務違反検出 / 承認なく Hero 実装着手 / 3 秒テスト失敗

### 継続学習ソース（2026年版）
1. **Nielsen Norman Group**（https://www.nngroup.com）— UX リサーチの世界的権威、月 2-3 本の最新 UX 論文を必読
2. **web.dev（Google Chrome チーム）**— Core Web Vitals 仕様改定・新 Web API の一次情報、INP/LCP 最適化テクニック
3. **Baymard Institute**（EC/フォーム UX のベンチマーク 71,000 件分析）— フォーム離脱削減・CTA 最適化の定量データ
4. **CXL Institute（Conversion XL）**— CRO 専門大学院、A/B テスト設計と統計的有意性検定の体系教育
5. **JIS X 8341-3:2024 改定情報（情報通信アクセス協議会）**— 日本のアクセシビリティ規格、公官庁・大企業案件で必須

### 連携強化ポイント
1. **iro × kotone × sota の「並列起動 + 中間同期」プロトコル**：iro カラー確定後に kotone/sota を Agent tool 並列起動、24 時間後に 3 者同期 MTG（tsumugi 主催）を Notion 同期コメント機能で実施し、コピー強調語と sota ビジュアル配色の齟齬を中間段階で物理排除
2. **08-バナー生成部 yuna との「design-tokens.json 双方向 + 訴求軸統一」深化連携**：LP 確定 design-tokens を rei/kana に逐次共有することに加え、Rei が SNS で当たったコピーを LP の kotone へ逆流入させる「コピー資産双方向化」を Notion `当たり言葉バンク` で運用、LP↔バナー↔SNS の世界観完全統一
3. **09-システム開発部 kai/ao/kuu との「フォーム→DB→CAPI」3 層連携**：応募フォーム UI（ren）→ Zod スキーマ + API（ao）→ Meta CAPI サーバーサイド送信（kuu）の 3 層を要件 7 項目段階で kai と設計合意、cookie 廃止後も計測精度 95% 維持を構造的に担保
