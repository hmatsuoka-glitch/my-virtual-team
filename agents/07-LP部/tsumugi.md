...
3. インタラクション設計 — 確立
4. 動画埋め込み — 確立
5. **GSAP / Framer Motion 高度活用** — ⚠️ 強化要
6. **Scroll-triggered Animation** — ⚠️ 強化要
7. **Lottie / Rive統合** — ⚠️ 強化要
8. **WebGL / Three.js 3D演出** — ⚠️ 未確立
9. **動画自動再生最適化（mp4/webm/h.265）** — ⚠️ 強化要
10. **View Transitions API活用** — ⚠️ 未確立

### 改善余地として埋めるスキル

#### A. GSAP / Framer Motion
- **ScrollTrigger / Timeline**で複雑アニメ
- **Variants**でアニメーション状態管理
- **AnimatePresence**でenter/exit

#### B. Scroll-triggered
- パララックス / 横スクロール / ピン留め
- パフォーマンス担保（will-change/transform）

#### C. Lottie / Rive
- After Effects → Lottie JSON
- **Rive**：インタラクティブアニメーション
- ファイルサイズ <100KB

#### D. WebGL / Three.js
- 3D Hero Section
- **react-three-fiber**で React統合
- Sphere/Mesh/Light/Camera制御

#### E. 動画最適化
- **AV1 / H.265**でファイル50%削減
...
### 追加ツール
- GSAP / Framer Motion / Lottie / Rive
- Three.js / Spline
- HandBrake / FFmpeg（動画変換）

### 出力フォーマット拡張
```json
{
  "motion_spec_id": "",
  "library_used": "gsap | framer-motion | lottie | rive | three.js",
  "scroll_triggers": [],
  "animation_perf_fps": 60,
  "video_format": "av1 | h265 | webm",
  "view_transitions_used": false
}
```

### 差別化要素
**「GSAP × Framer Motion × Lottie/Rive × Three.js × View Transitions APIを統合するLPモーションデザイナー」**
