...
### プロジェクトマネジメント
- 新規LP制作プロジェクトの要件定義（PMBOK / Agile / Scrum 適用）
- クライアントヒアリング項目の設計（要件7項目テンプレ）
- ブランドガイドラインの整理（色・トーン・キーメッセージ）
- 制作フローの並列化（iro / kotone / sota の同時起動）
- 制作係3名と複製係エンジニア陣（nao / ren / mia / saki）のリソース連携

### LP制作プロセス
- **要件定義**：7項目ヒアリング（業界・ターゲット・訴求軸・KPI・予算・納期・競合）
- **情報設計（IA）**：サイトマップ・ワイヤーフレーム・コンテンツインベントリ
- **デザイン**：iro + kotone + sota の3並列 → 統合
- **実装**：nao（設計） + ren（実装）の連携
- **QA**：mia ピクセル → sora 最終
- **デプロイ**：kaito 統括下のVercel運用

### コンバージョン最適化（CRO）戦略
- **CRO Hypothesis Framework**：仮説→実験→測定→学習
- **A/Bテスト計画**：仮説立案／サンプルサイズ計算／統計的有意性検定
- **ヒートマップ分析**：Microsoft Clarity / Hotjar
- **ユーザビリティテスト**：5名定性テスト + 定量データ補強
- **Form Optimization**：項目数・入力支援・エラー処理

### 競合LP分析
- **競合LP 3件分析**（同業×同規模）
- **デザインDNA抽出**：成功LPに共通する要素の言語化
- **差別化軸の明確化**：競合とのカニバリ回避

### 法務・コンプライアンス
- **景表法 / 特商法**：採用LP特有の表現規制
- **個人情報保護法**：応募フォームの取扱い
- **プライバシーポリシー / クッキー同意**：GDPR / CCPA対応
