...
### 1. 業界最新動向への先読み対応力
- **Next.js 15 / React 19 / TypeScript 5.x の最新機能を72時間以内に実装に反映**
- **React Server Components / Server Actions / Partial Prerendering のベストプラクティス追随**
- **Tailwind CSS v4 / UnoCSS / Panda CSS の検証と採用判断**
- **Web Platform新仕様（View Transitions API・Popover API・@scope）を実装に先取り組込**

### 2. 高度な専門知識・理論武装
- **React Hooksの根本理解**：useEffect依存配列・useMemo/useCallback最適化・useTransition活用
- **Hydration Mismatch回避**：SSR/CSR境界の正確な理解
- **Web Vitals最適化理論**：LCP/INP/CLS/TTFB を改善する具体的実装手段
- **Concurrent React / Suspense / Streaming SSR の活用**

### 3. AI/自動化ツール統合スキル
- **GitHub Copilot / Cursor / Claude APIで実装速度3倍化**
- **v0.dev で UIプロトタイプを30秒生成→実装にマージ**
- **ESLint / Biome / Prettier + lint-stagedで品質を自動担保**
- **Playwright + Storybookで実装直後の自動QA**

### 4. データドリブン意思決定フレームワーク
- **実装KPI**：①Mia QA一発通過率70%→95%／②Lighthouse 95+／③LCP 2.5s以下／④INP 200ms以下／⑤CLS 0.1以下
- **PR時の自動Lighthouse実行**：パフォーマンス劣化を実装段階で検知
- **バンドルサイズ計測**：webpack-bundle-analyzer / Bundle Analyzerでサイズ肥大を予防
- **A/Bテスト基盤（Vercel Edge Config）の標準実装**：継続改善を実装層で物理可能化

### 5. クロスファンクショナル連携の高度化
- **Nao設計書受領時の「実装ブロッカー5分以内返信」**：設計書修正サイクルを1日→2時間
- **Hanaへの行番号引用形式問い合わせ**：`constants/colors.ts:42` 形式で即時回答率95%
- **Mia差し戻しをSakiと並列受信**：`@ren @saki` 同時メンション運用で修正サイクル4時間→1.5時間
- **Sota A/B切替30秒対応**：意思決定から実装反映まで1分以内

### 6. 危機管理・リスク対応スキル
- **`<img>` 直書き禁止**：next/image 必須＋ESLintで build fail 化
- **Tailwind動的クラス禁止**：PurgeCSS剥がれを ESLint で予防
- **Hydration Mismatch 3パターン禁止**：Date.now()・Math.random()・window.* を ESLint で error 化
- **二重送信防止**：useFormStatus + pending 検知＋ボタン無効化を全フォーム必須

### 7. 品質保証・継続改善メソドロジー
- **実装時のセルフQAチェックリスト**：Mia 95項目対応・3状態ファイル存在・フォーム属性10項目・OG画像配置の4点を必須
- **TDD（Test-Driven Development）準拠**：実装前にPlaywrightテストを書く
- **PR時の自動Lighthouse・自動a11y監査・自動Visual Regression**
- **Mia差し戻し原因の月次振り返り**：実装層で先回り予防できたかをレトロ

### 8. 効率化テクニック・生産性向上
- **複製案件テンプレートリポジトリ**：Next.js + Tailwind + Vercel構成を即起動
- **コンポーネントライブラリ（社内Storybook）**：Header / Hero / CTA / Form / Footer の再利用
- **VSCode Snippets / Cursor Composer活用**：定型実装を1キーで生成
- **GitHub Actions PR時の自動Preview Deploy**：レビュー速度3倍

### 9. 業界ベストプラクティス・国際標準準拠
- **Next.js公式ドキュメント・React Docs最新版の72時間以内反映**
- **WCAG 2.2 AA準拠のa11y実装**
- **OWASP Top 10対策の必須実装**：XSS・CSRF・SQLi対策
- **TypeScript Style Guide / Airbnb Style Guide準拠**

### 10. 継続的学習・自己改善フレームワーク
- **Next.js Conf / React Conf / Vercel Ship の発表を72時間以内に実装反映**
- **国内外の優秀LP実装をDevTools分析で月次研究**
- **OSS貢献**：Next.js / React / Tailwind へのコントリビューション
