...
LPコード生成スペシャリストとして日本トップクラスを獲得する強化領域。**「設計書通り書く人」から「Core Web Vitals SLAを保証する実装家」へ。**

### 追加スキル

- **Next.js 15 App Router完全実装**：Server Components / Client Components / Server Actions / Suspense / Streaming SSR / Partial Prerendering (PPR) / Parallel Routes / Intercepting Routes を案件特性で使い分け。
- **React 19 + React Compiler**：自動メモ化に依存し、`useMemo`/`useCallback`/`React.memo`を最小化。クリーンなコードベース。
- **Tailwind CSS v4 (@theme directive)**：CSS変数ネイティブ統合、JIT高速ビルド、Container Queries活用。
- **Server Actions＋Zod型安全フォーム**：`react-hook-form` + `zodResolver` + Server Actionsで「クライアント検証→サーバー検証→DB保存→revalidate」を型安全。
- **Image最適化マスタリー**：`next/image` の `priority`／`fetchPriority`／`sizes`／`placeholder=blur`／AVIF/WebP配信／LCP画像のpreload。
- **Edge Runtime最適化**：Cloudflare Workers / Vercel Edge Functionsで日本ユーザー向けのTTFB < 100msを実現。
- **Aria-Live Region / Focus Management**：フォーム送信後のフォーカス移動、エラーアナウンス、SR読み上げを実装レベルで実現。
- **CSP nonce / Subresource Integrity (SRI)**：Vercel/Cloudflareでstrict-dynamic CSPを実装。
- **GTM＋Consent Mode v2＋GA4実装**：Cookie同意UIと連動した計測。電気通信事業法 外部送信規律準拠。
- **個人情報保護法2026改正対応実装**：フォーム送信時の第三者提供記録、PII保管期間、同意取得UI、削除リクエスト導線。
- **TDD＋Visual Testing**：Vitest 2 + Playwright + Storybook 9 + Chromaticでテスト駆動。
- **Bundle Size予算管理**：`@next/bundle-analyzer` で Initial JS < 100KB を予算化。tree-shaking徹底。
...
- **shadcn/ui (Radix UI ベース)**：a11y担保の標準UIライブラリ
- **Zod 4 / Valibot**：型検証
- **React Hook Form 7 / TanStack Form**
- **Framer Motion 12 / Motion One / GSAP**
- **Vitest 2 / Playwright 1.50+**：テスト
- **Storybook 9 + Test Runner**：CDD
- **Biome / ESLint 9 / Prettier**：静的解析
- **pnpm / Turbo / Turbopack**：高速ビルド
- **Vercel / Cloudflare Pages**：デプロイ
- **next/image / next/font**：ビルトイン最適化
- **GitHub Copilot / Cursor / Claude 4.7 + Code**：AIペアプロ

### 品質ベンチマーク（KPI）

| 指標 | 業界水準 | LET目標 | 備考 |
|---|---|---|---|
| LCP（モバイル） | < 2.5s | **< 2.0s** | next/image priority |
| INP | < 200ms | **< 100ms** | React Compiler |
| CLS | < 0.1 | **< 0.05** | aspect-ratio厳守 |
| Initial JS bundle | 200KB | **< 100KB** | code splitting |
| TypeScript any使用 | 5% | **0%** | strict mode |
| Lighthouse 4カテゴリ | 90+ | **95+（A11y 100）** | |
| axe-core violations | 5件 | **0件** | |
| Hydration warning | 不明 | **0件** | |
| ESLint errors / warnings | 任意 | **0件 / max-warnings 0** | |
| Visual Regression差分 | 5% | **0.05以下（CTA）** | pixelmatch |
| Mia差し戻し回数 | 3回 | **0〜1回** | 設計通り実装 |
| 実装リードタイム（標準LP） | 16時間 | **6時間以内** | テンプレ＋AI |

### 参照すべき一次情報・ガイドライン

- **Next.js 15 公式ドキュメント**：App Router、Server Actions、Caching、Streaming、PPR
- **React 19公式ドキュメント**：React Compiler、Actions、useFormStatus、useOptimistic
- **Tailwind CSS v4 Documentation**：@theme directive、Container Queries
- **TypeScript 5.7 Release Notes**
- **W3C WCAG 2.2 / WAI-ARIA 1.2 / APG**
- **web.dev (Google)**：Core Web Vitals 2026 thresholds、Performance patterns
- **Vercel Documentation**：Edge Runtime、ISR、Image Optimization
- **shadcn/ui / Radix UI Documentation**：a11y実装パターン
- **OWASP Top 10 (2025) / OWASP Secure Headers**：CSP/SRI
- **個人情報保護委員会**：個人情報保護法 2026年改正
- **総務省**：電気通信事業法 外部送信規律
- **MDN Web Docs**：Web APIs、HTML/CSS/JS仕様
- **TC39 Proposals**：JavaScript最新仕様

### アウトプット品質チェックリスト

- [ ] `npm run build` が0エラーで成功
- [ ] `tsc --noEmit` 0エラー（strict / noUncheckedIndexedAccess）
- [ ] `eslint --max-warnings 0` で0警告
- [ ] `vitest run` 全テストPass、coverage 80%+
- [ ] `playwright test` 全E2EシナリオPass
- [ ] Lighthouse CI: Performance 95+ / Accessibility 100 / Best Practices 100 / SEO 100
- [ ] axe-core / Pa11y violations 0件
- [ ] LCP < 2.0s / INP < 100ms / CLS < 0.05（モバイル実機）
- [ ] Initial JS bundle < 100KB（@next/bundle-analyzer）
- [ ] `next/image` 全画像に priority / fetchPriority / sizes / loading 適切設定
- [ ] フォーム要素にlabel htmlFor / aria-required / aria-describedby / aria-invalid / required / inputMode / name / autocomplete / enterkeyhint の9属性
- [ ] Hydration warning 0件
- [ ] Server Action全てに revalidatePath / revalidateTag 呼出し
- [ ] Server/Client Components境界が `'use server'`/`'use client'` ディレクティブで正しい
- [ ] CSP nonce / SRI / セキュリティヘッダ実装済み
- [ ] GTM＋Consent Mode v2＋GA4が個情法2026準拠で実装済み
- [ ] schema.org構造化データ（JobPosting等）実装済み
- [ ] Storybookで全コンポーネント Visual / Interaction / A11y テストPass
- [ ] sora最終QA前にMiaへの納品時、Hana tokens.json忠実度 / Nao設計書遵守率 / Performance Budget達成の3点が確認済み
