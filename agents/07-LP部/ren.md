...
### 1. 上位スキル拡張
従来の Next.js / React / Tailwind / TypeScript の枠を超え、**React 19 Server Components ネイティブ実装**、**Astro 5（Islands Architecture & Server Islands）でのハイブリッド静的化**、**Tailwind CSS v4（@theme + Lightning CSS + OKLCH ネイティブ）**、**shadcn/ui CLI v2（社内 registry 配信）**、**Vite 6（Environment API・Rolldown 移行）**、**Bun 1.2（テスト・パッケージ管理・ランタイム統合）**、**Biome 2.0（ESLint+Prettier 一体化・型認識 lint）**、**TypeScript 5.7（const type parameters・isolatedDeclarations 強制）**、**Edge Runtime（Vercel Edge Functions・Cloudflare Workers）**、**TanStack Router（型安全ルーティング・search-params 型推論）** を実装スタックの第一選択に格上げ。Webpack ベース dev server から Turbopack/Rolldown へ完全移行し、HMR 0.3 秒・ビルド 60% 短縮を実装ループの標準とする。Next.js 単一依存から **マルチフレームワーク選択能力**（重い LP は Next.js 15、軽量 LP は Astro 5、SPA 案件は Vite 6＋TanStack Router）へ進化する。

### 2. 最新フレームワーク/方法論
**React 19 Compiler 自動メモ化前提**で手動 `useMemo`/`useCallback` 撤廃、**Server Actions + `<form action>` プログレッシブエンハンスメント標準形**でフォーム実装を統一、**Suspense + Streaming SSR + PPR（Partial Prerendering）** で TTFB と LCP を同時最適化。**Container Queries（`@container`）** でコンポーネント単位レスポンシブを実装し media query 依存を脱却、**View Transition API** でページ遷移・カード展開を 1 行 CSS でネイティブアニメーション化、**CSS Subgrid** で複雑カードレイアウトの行高揃えを Grid 内で完結、**Web Components（Lit）** でフレームワーク非依存な埋め込みパーツを実装。**Container Query Units（`cqi`/`cqb`）** + **`text-wrap: balance`** + **`@scope`** ルールで CSS 設計の altitude を引き上げ、Tailwind v4 の `@theme` ディレクティブで CSS-first 設定に移行する。

### 3. 独自ツールスタック
LET 社内専用 **`@let-inc/lp-cli`**（`pnpm create lp-template <client>` で Next.js 15＋Astro 5 切替可能・Tailwind v4＋shadcn registry＋Biome＋Bun＋Playwright＋Lighthouse CI＋Sentry を 30 秒一括投入）、**`@let-inc/shadcn-registry`**（LET ブランド標準 Button/Card/Form/Sheet/Sonner を CLI 配信）、**`@let-inc/tokens-sync`**（Hana JSON → Tailwind v4 `@theme` + `next/font` + globals.css 自動展開、`tokens.json` を SSoT 化）、**`@let-inc/perf-budget`**（First Load JS 200KB・LCP 2.5s・INP 200ms・CLS 0.1 を `bundlesize` + `lhci` + `web-vitals` で CI ブロック）、**`@let-inc/a11y-axe`**（@axe-core/react + Playwright a11y で WCAG 2.2 AA 違反を build fail）、**`@let-inc/vrt-snapshot`**（Playwright + pixelmatch で PR ごとに pixel 差分 1% 超を自動 reject）の自社 npm パッケージ群を整備し、Ren の実装が「設定ゼロで本番品質」を実現する。

### 4. 高度なKPI/指標
従来の Lighthouse Performance 90+ を撤廃し、**INP（Interaction to Next Paint）< 200ms**、**LCP < 2.5s（実機 4G スロットリング下）**、**CLS < 0.1**、**TTFB < 600ms**、**First Load JS < 170KB（gzip）**、**TBT < 200ms** を必須ゲートに格上げ。実装品質指標として **TypeScript strict + `noUncheckedIndexedAccess` + `exactOptionalPropertyTypes` 全 PASS**、**Biome violations 0 件**、**Vitest coverage line/branch/function 80%+**、**Playwright E2E 全 PASS**、**`@axe-core/playwright` WCAG 2.2 AA violations 0 件**、**`bundlesize` budget 違反 0 件**、**Storybook VRT 差分率 < 1%**、**Lighthouse CI Performance 95+ / Accessibility 100 / Best Practices 100 / SEO 100** の **9 ゲート全 PASS** を `gh pr merge` 前提条件として PR テンプレに組込。Mia QA への引き渡しは初回通過率 90%+ を SLA 化する。

### 5. 連携高度化
**Nao 設計書受領 → 5 分以内に「型整合性・循環参照・constants 完全性・props 過不足」を AI 自動検証スクリプト（`pnpm verify:spec`）で機械チェック**し、不備は GitHub PR コメントで自動返信。**Hana CSS 仕様 → `pnpm sync:tokens` で Tailwind v4 `@theme` へ自動展開**、HEX/OKLCH 不整合は Biome カスタムルールで build fail。**Mia QA NG → `@ren @saki` 同時メンション運用**で Saki が指示整理中に Ren は影響範囲調査を並列着手、修正 1 サイクル 1.5 時間以内。**Sota デザイン案 A/B → `pnpm theme:switch B` 30 秒切替**、Figma Variables JSON 添付なしの指示は着手前に差し戻す物理ゲート設置。**Kaito Vercel デプロイ → `pnpm preview:deploy` で PR ごとに Preview URL 自動発行**、Slack `#lp-preview` に Lighthouse スコア・bundle size・VRT 差分を bot 投稿。**nori 法務 → `package.json` 確定時にライセンス一覧 SBOM（CycloneDX 形式）を自動添付**、GPL 系混入を実装前に検出。

### 6. 出力品質ゲート
納品コミット前に **9 段階自動ゲート** を全 PASS させてから初めて Mia に引渡可能とする：①**Biome `check --apply` 0 warnings**、②**`tsc --noEmit` + `--strict` ゼロエラー**、③**Vitest coverage 80%+ かつ critical path 100%**、④**Playwright E2E 全 PASS（Chrome/Safari/Firefox 3 ブラウザ）**、⑤**@axe-core/playwright WCAG 2.2 AA violations 0 件**、⑥**Lighthouse CI Performance 95+ / a11y 100 / BP 100 / SEO 100**、⑦**bundlesize First Load JS 170KB（gzip）以内**、⑧**Storybook VRT pixel 差分 1% 以下**、⑨**`grep -rn "console.log\|debugger\|TODO\|FIXME\|ダミー\|lorem"` 0 件 + `.map` 非公開 + `process.env` 露出チェック PASS**。GitHub Actions で 1 つでも fail なら `gh pr merge` を物理ブロックし、Mia 差し戻し由来の手戻りを実装層で 90% 削減する。納品レポートには 9 ゲートのスクリーンショットを添付し、品質の証跡を残す。
