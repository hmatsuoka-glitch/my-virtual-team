...
### 2026-09-04
- **スクロール連動アニメは `animation-timeline: view()` を第一選択にし、IntersectionObserver 実装を末端に追いやる**：Chromium 系で `animation-timeline`/`view-timeline` が実装され、要素が視界に入る進捗に応じたフェード・パララックスを JS ゼロ・observer ゼロで書けるようになった。共通アニメユーティリティ（2026-07-21参照）に `view()` ベースの fade-in/rise-up を追加し、Framer Motion `whileInView` は「進捗ではなく状態遷移が必要な箇所」に限定する。observer 多重登録による INP 劣化（2026-07-03参照）と JS 失敗時の永久非表示事故（2026-06-24参照）が構造的に起きなくなる。非対応ブラウザ向けには `@supports (animation-timeline: view())` フォールバックで従来実装を並置し、`prefers-reduced-motion: reduce` は timeline アニメ側でも必ず分岐する
- **カード・カラムの折返しは画面幅ではなくコンテナ幅で決める（`@container` を section レイアウトの既定にする）**：同じカードコンポーネントを Hero 下 3 カラム／サイドバー付き 2 カラム／モーダル内 1 カラムで使い回すと、`md:grid-cols-3` 等の viewport ブレークポイントでは親幅と合わず崩れる。Section ラッパーに `container-type: inline-size` を付け、カード側は `@container (min-width: 32rem)` で内部レイアウトを決める設計に統一する。Tailwind v4 の `@container` ユーティリティ＋Hana のブレークポイント JSON にコンテナ側閾値列を追加し、`grid-template-columns: subgrid` と組み合わせて内側の見出し・本文・CTA を親グリッドの列に揃える。Sota のセクション横展開案で「同じカードを別枠で使い回したい」要望が来ても、実装差分がゼロで済む
- **フォーム・カード・ナビの状態スタイルは `:has()` で親側から書き、JS の state 同期を廃する**：エラー表示・入力済み表示・アコーディオン開閉の見た目差分を「子の状態を親の `useState` にコピー→親に class を付ける」構成で書くと、状態が二重管理になり Mia のスナップショットで状態不整合の差分が出る。`form:has(input:user-invalid) .submit { opacity:.5 }`／`fieldset:has(input:checked) { ... }`／`details:has(> summary:focus-visible) { ... }` のように親セレクタ側で表現し、JS 側は「値を送る」責務だけに絞る。`user-invalid` は「一度 blur した後だけ赤くする」挙動をブラウザが担保するため、blur 前赤化（2026-07-01参照）の実装を書かずに済む。Nao(LP) の状態表（idle/hover/focus/disabled/loading/error）は「JS で切替える状態」と「`:has()`/`:user-invalid` で CSS が自動で当たる状態」を列で分離してもらい、実装の入口で責務境界を確定させる
- **ツールチップ・ドロップダウン・小メニューはネイティブ Popover API（`popover` 属性）で組み、`<dialog>` は真のモーダルだけに残す**：`<dialog>` を第一選択にする方針（2026-09-02参照）は残しつつ、非モーダルな開閉 UI（求人カードの補足説明・ヘッダーのお知らせバッジ・住所コピー完了トースト）まで `dialog.show()` で組むと `::backdrop` や body スクロールロックが不要な場面まで持ち込むことになる。`<button popovertarget="tip1">`＋`<div id="tip1" popover>` の宣言型 API で組めば、top-layer 配置・Esc/外側クリックでの自動閉じ・アクセシブルな露出/非表示の切替をブラウザが担保し、ARIA の手当ては不要になる。位置決めは CSS Anchor Positioning（`anchor-name`/`position-anchor`）で JS フリーにし、非対応ブラウザには `@supports not (anchor-name: --x)` で従来の Popper.js フォールバックを分岐する。判定表：モーダル＝`<dialog>`／背景に触らせない開閉＝`popover=manual`／外側クリックで閉じてよい開閉＝`popover=auto`／トグル群＝`popover=hint`＋Anchor Positioning
- **WCAG 2.2 の新3基準（Target Size 24×24 / Focus Not Obscured / Dragging Movements）を Mia の QA ゲート項目としてテンプレ実装側に埋め込む**：LP でよく落ちるのは (1) 固定 CTA の隣接ボタン間隔が 24px 未満で誤タップ、(2) 固定ヘッダー配下でフォーカスされた入力欄がスクロール位置的にヘッダーの裏に隠れる、(3) スライダー/カルーセルがドラッグ操作しかできない、の 3 点。共通コンポーネント（2026-09-01参照）に「タップターゲット 24×24 以上＋周囲 24px クリアランス」「`:focus-visible` 時に `scroll-margin-top: <ヘッダー高さ>` を効かせる」「カルーセルは前後ボタン＋ドット必須（スワイプは補助）」を既定で組み込み、Mia の QA チェックリストにこの 3 項目のセレクタと期待値を渡す。Sota のヒーロー 3 型骨格（2026-08-27参照）と Kotone の CTA 文言スロットにも Target Size の最小値を含めてもらい、Sora の最終ゲートで WCAG 2.2 AA 相当を機械的に判定できる状態にする
