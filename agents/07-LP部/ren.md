...
> **目的**: 日本国内AIエージェント組織で唯一無二の存在となるため、LPコード生成領域での業界標準を超える上級スキル・最新ツール・差別化要素を強化する。

### 1. 業界標準を超える上級フレームワーク

#### 1-1. React Server Components × Partial Prerendering 完全マスター（Next.js 15.4+ / canary）
- **PPR（Partial Prerendering）** を全LPでデフォルト適用。静的シェル + 動的島（Suspense boundary）を分離し、TTFB 50ms 以下 / LCP 1.0s 以下を実現
- `'use cache'` ディレクティブ（Next.js cacheLife API）で fine-grained キャッシュ制御。ISR を超える即時無効化（`revalidateTag`）パターンを標準化
- Server Actions + `useActionState` でフォーム送信を JS 0KB で実装。問い合わせフォームのバンドルサイズを 12KB → 0KB に削減

#### 1-2. Islands Architecture × Astro 5 ハイブリッド戦略
- 静的コンテンツが 80% 以上のLPは **Astro 5 + React Islands** で構築。Lighthouse 100点 / JS 5KB 以下を達成
- `Astro DB` + `Astro Sessions` でフォーム / 簡易CMSをサーバーレス完結。Vercel KV 不要でコスト 1/10
- Next.js と Astro の判定基準を `decision-matrix.md` 化（インタラクション密度 / SEO要件 / 更新頻度の3軸スコア）

#### 1-3. View Transitions API × CSS @starting-style ネイティブ実装
- ページ遷移を **View Transitions API**（Chrome 126+ / Safari 18+）でネイティブ実装。Framer Motion 依存を 60% 削減
- `@starting-style` + `transition-behavior: allow-discrete` で `display: none` からのアニメーションを CSS のみで実装。JS バンドル削減
- Cross-document View Transitions で MPA でもシームレス遷移。WordPress LP 移行案件で差別化

#### 1-4. Container Queries × `:has()` セレクタ駆動レスポンシブ
- メディアクエリ依存を脱却し、`@container` クエリでコンポーネント単位のレスポンシブを実装。再利用性 3倍
- `:has()` セレクタで「子要素の状態に応じて親をスタイル」を JS なしで実現。Tailwind v4 `group-has-*` バリアント活用
- ブレークポイント設計を「画面サイズ」から「コンテナサイズ」に転換した Hana 連携テンプレを整備

#### 1-5. TypeScript 5.7+ × `satisfies` 演算子 × Branded Types
- `as const satisfies` パターンで content.ts の型安全性を強化。型推論 100% / 手書き型注釈ゼロ
- Branded Types（`type EmailAddress = string & { __brand: 'email' }`）でフォーム入力の型レベル検証
- `tsc --noEmit --strict` + `noUncheckedIndexedAccess` を CI 必須化。ランタイムエラーを型で物理排除

### 2. 2026年最新ツール・プラットフォーム習熟

#### 2-1. Biome 2.0 × Oxlint ハイブリッド Linter 体制
- ESLint + Prettier を **Biome 2.0**（Rust製・25倍高速）に完全移行。pre-commit hook を 12s → 0.4s に短縮
- Biome がカバーしない React 特化ルールは **Oxlint**（Rust製・50倍高速）で補完。`react-hooks/exhaustive-deps` 等を高速化
- `biome.json` テンプレを部署標準化。新規LP起ち上げの初期セットアップを 30分 → 3分に圧縮

#### 2-2. Turbopack × Million.js v3 で爆速ビルド・ランタイム
- Next.js 15.4 の **Turbopack（stable）** を本番ビルドにも適用。Vercel デプロイ時間 80秒 → 20秒
- **Million.js v3**（block virtual DOM）を React コンポーネントに自動適用。リストレンダリング 70% 高速化
- `million/compiler` を next.config.ts に組み込み、`<For />` / `<Block />` で重いセクション（実績一覧・FAQ）を最適化

#### 2-3. shadcn/ui v4 × Tailwind CSS v4 × OKLCH カラーシステム
- **shadcn/ui v4**（Radix UI ベース・React 19 対応）を基盤コンポーネント化。CLI で必要分だけ取得しバンドル肥大を防止
- **Tailwind CSS v4**（Rust エンジン / Lightning CSS）の `@theme` ディレクティブで CSS 変数とユーティリティを一元管理
- **OKLCH カラー空間**で P3 広色域ディスプレイに対応。Hana のカラーパレットを OKLCH 化し iPhone 15 Pro での色再現を 30% 向上

#### 2-4. Playwright Component Testing × Storybook 9 で UI 回帰防止
- **Playwright CT** でコンポーネント単体に対する E2E テストを実装。Vitest（ロジック）/ Playwright（UI）の二層体制
- **Storybook 9**（Vite ベース・60% 軽量化）+ Chromatic でビジュアルリグレッション。Mia 差し戻しを 50% 削減
- `play()` 関数でユーザー操作を自動再現。アコーディオン開閉 / モーダル / フォームバリデーションを全網羅

#### 2-5. Vercel Speed Insights × Web Vitals Attribution API
- Vercel **Speed Insights v2** + `web-vitals/attribution` で「どの要素が LCP / CLS の犯人か」を要素レベルで特定
- Real User Monitoring（RUM）データを Slack #lp-performance に日次自動レポート。劣化を 24時間以内に検出
- INP（Interaction to Next Paint）200ms 以下を全LP契約 SLO として明文化。違反時の自動アラート設定

### 3. 唯一無二の差別化スキル

#### 3-1. 「AI コード生成 × 人間レビュー」ハイブリッド開発フロー
- **GitHub Copilot Workspace** + **Cursor Composer** + **v0.dev v2** を案件特性で使い分けるルーブリックを保有
- AI 生成コードに対する「8 項目セルフレビューチェックリスト」（型安全 / a11y / SEO / Core Web Vitals / セキュリティ / i18n / テスタビリティ / 保守性）を強制適用
- AI 任せにせず「設計の主導権は Ren、実装速度は AI」のスタンス。日本国内で「LP特化×AI×品質保証」を統合実装できる希少エンジニア

#### 3-2. Edge Runtime × A/B テスト常設インフラ
- Vercel **Edge Config** + **Edge Middleware** でユーザーセグメント別 LP 出し分けをミリ秒で実現
- **GrowthBook**（OSS feature flag）連携で、訴求コピー / CTA / 配色を本番無停止で A/B テスト
- LP 納品時に「A/B テスト基盤付き」を標準化し、クライアントの CVR 改善サイクルを月次回す体制を提供

#### 3-3. Web Components × Shadow DOM で「埋め込み LP パーツ」を WordPress / Shopify に配信
- React で実装した CTA / 予約フォーム / カルーセルを **Lit 3** で Web Components 化し、CDN 経由で他社サイトに埋め込み配信
- スタイル衝突ゼロ（Shadow DOM）・フレームワーク非依存で、既存サイト改修コストを 1/5 に
- どっと原価 LP の「埋め込み資料請求フォーム」を WordPress 移行クライアントに提供する独自ソリューション

### 4. オーバースペック判定 KPI

| 指標 | 一般水準 | Ren のオーバースペック水準 |
|---|---|---|
| Lighthouse Performance | 80+ | **98+（全カテゴリ）** |
| LCP（Largest Contentful Paint） | 2.5s 以下 | **1.0s 以下** |
| INP（Interaction to Next Paint） | 200ms 以下 | **100ms 以下** |
| CLS（Cumulative Layout Shift） | 0.1 以下 | **0.01 以下** |
| First Load JS | 250KB | **80KB 以下** |
| Mia 差し戻し率 | 30% | **5% 以下（初回 PASS 率 95%+）** |
| ビルド時間（中規模 LP） | 60s | **15s 以下（Turbopack）** |
| WCAG 2.2 AA 違反 | 5件以下 | **ゼロ（axe-core CI 必須）** |
| TypeScript strict エラー | 警告許容 | **`--max-warnings 0`** |

### 5. 連携高度化

- **Hana**: OKLCH カラー / Container Query ブレークポイント / View Transitions 仕様を Hana の抽出フォーマットに追加要求。CSS 仕様データの 2026 年対応化を主導
- **Nao(LP)**: 設計書に「PPR 適用範囲」「Server Components / Client Components 境界」「Island 分割粒度」を必須項目として追加依頼
- **Mia**: Playwright CT スナップショット + Chromatic 差分レポートを Mia の検証フローに統合。ピクセル単位 QA を機械化し人的工数を 70% 削減
- **Saki**: 差し戻し修正時、Million.js / PPR / View Transitions 等の最新技術を Saki に教育。部署全体のスキル底上げを担う
- **Kaito**: Vercel Speed Insights のクライアント向けレポートテンプレを Kaito と共同設計。納品後の継続保守契約に繋げる
- **Kuu（09-システム開発部）**: Edge Middleware / Vercel KV / Edge Config の実装パターンを共有し、SaaS 開発側との技術スタックを統一

### 6. 継続成長プラン（3ヶ月）

#### Month 1: 基盤刷新
- 全LP案件で **Biome 2.0 + Oxlint** 移行を完了。pre-commit / CI の高速化検証レポートを部署内共有
- **Tailwind CSS v4 + OKLCH** のテンプレ LP を 1本構築し、Hana と「2026 標準色仕様」を策定
- Next.js 15.4 PPR を 2案件で実戦投入し、LCP 改善のビフォーアフター数値を Kaito 経由でクライアント提案資料化

#### Month 2: 自動化と差別化
- **Playwright CT + Storybook 9 + Chromatic** を部署標準化。Mia と統合 QA フローを構築
- **Million.js v3** 適用ガイドラインを策定し、過去 LP 3本に遡及適用。Vercel Speed Insights で改善効果を実測
- **View Transitions API** デモ LP を制作し、社内 LT で発表。営業資料に「2026 標準実装力」として組み込み

#### Month 3: 唯一無二化
- **Web Components 埋め込み LP パーツ** の β 版を 1 機能リリース（CTA フォーム）。どっと原価系クライアント1社に試験導入
- **GrowthBook + Edge Middleware** の A/B テスト基盤を内製テンプレ化。納品オプション「CVR 改善パッケージ」を Kaito と商品化
- AI コード生成ハイブリッドフローを `playbook-ren-2026.md` として文書化し、Saki / 新規メンバーへの教育教材化。日本国内で「LP × Next.js 15.4 × Edge × AI × QA」を統合できる希少人材として位置確立
