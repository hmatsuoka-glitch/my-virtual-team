...
世界トップ1%のフロントエンド実装スペシャリストとして、2026年時点で「知っているだけでなく実装テンプレ化して即時投入できる」レベルまで習熟しておくべき技法・フレームワーク群。Lighthouse 95+ / Core Web Vitals 全項目「Good」/ INP < 200ms / a11y violations 0 件 を実装の不変条件として満たすための武器庫。

### 1. PPR（Partial Pre-Rendering, Next.js 15.x stable）の本番投入
Next.js 15.x で stable 化した PPR を `next.config.ts` の `experimental.ppr: 'incremental'` で有効化し、ルートごとに `export const experimental_ppr = true` を明示。**静的シェル（Hero／ナビ／LP テンプレ）は CDN edge 配信、動的部分（在庫・お問い合わせ件数）は `<Suspense>` 内で stream**するハイブリッド構成で TTFB 100ms 切り＋LCP 1.5s を達成。従来 SSG/SSR の二択だった LP 配信を「1ページ内で両立」させる 2026 年のデファクト技法。**動的部分の Suspense fallback には必ず `<Skeleton>` を配置**し、ストリーム遅延中の白画面ゼロを実装ルール化。

### 2. Tailwind CSS v4 + OKLCH カラー空間 + `@theme` ディレクティブ
Tailwind v4 の Lightning CSS エンジン（Rust 製、ビルド 60% 高速化）＋ `globals.css` 内 `@theme { --color-primary: oklch(0.62 0.18 250); }` 形式で**P3 広色域ディスプレイ（iPhone 13 以降・MacBook M1 以降）でも色潰れしない OKLCH 表記をネイティブ採用**。Hana 抽出の HEX を `culori` パッケージで `oklch()` 変換し `tokens.json` に格納、`pnpm sync:tokens` で `globals.css` へ自動展開するパイプラインを標準化。**HEX 比 12% の色域拡張**を実装層で確保し、Mia ピクセル比較で「微妙に色が違う」差し戻しを根絶。

### 3. React 19.1 Compiler によるメモ化自動化
`babel-plugin-react-compiler` を `next.config.ts` の `experimental.reactCompiler: true` で有効化し、`eslint-plugin-react-compiler` を error 化。**`useMemo` / `useCallback` / `React.memo` の手動記述を 90% 削減**しつつ、コンパイラが安全に最適化できないパターン（mutation・条件分岐内 hooks）を CI で fail 化。Ren 1 ファイル平均行数 180→130 行に圧縮、INP 計測値 280ms→140ms を実装層で恒常確保。コードレビュー時の「ここ memo 化すべき？」議論を消失させ Saki セルフ QA 時間を 50% カット。

### 4. Server Actions + `after()` API + 冪等性キー の 3 点セット
フォーム送信は `<form action={serverActionFn}>` でプログレッシブエンハンスメント（JS 無効環境でも動作）を確保し、`'use server'` 内で重い処理（GA4／Slack／Sentry／DB 書込）は `import { after } from 'next/server'; after(() => ...)` でレスポンス外に逃がす。**さらに送信ペイロードに `idempotencyKey: crypto.randomUUID()` を必須化**し、サーバー側で重複検知して二重応募事故を物理ゼロ化。INP 計測値 350ms→120ms、CV 直前の最重要処理を多層防御で守る 2026 年標準テンプレ。

### 5. View Transitions API + `unstable_ViewTransition`（React 19）
Chrome 111+ / Safari 18+ ネイティブ対応の `document.startViewTransition()` を React 19 の `<unstable_ViewTransition>` でラップし、**ページ遷移・モーダル開閉・タブ切替時に CSS だけで滑らかなクロスフェード／スライドアニメーション**を実装。Framer Motion を一切使わずに INP 0ms（メインスレッドブロックなし）で実現でき、バンドルサイズ 35KB 削減。`prefers-reduced-motion: reduce` メディアクエリで自動無効化される a11y 配慮も標準装備。

### 6. `content-visibility: auto` + `contain-intrinsic-size` による長尺 LP 最適化
10 セクション超の縦長 LP で、ファーストビュー外の `<section>` 全てに `content-visibility: auto; contain-intrinsic-size: auto 800px;` を Tailwind の `[content-visibility:auto] [contain-intrinsic-size:auto_800px]` で付与。**画面外セクションの style／layout／paint を丸ごとスキップ**し初期描画コストを 40% 削減。`contain-intrinsic-size` の予約高さ併記でスクロールバー暴れと CLS 悪化を予防する 2 点セットを実装ルール化。JS の lazy load（dynamic import）と役割を明確に区別。

### 7. Astro 5 + Islands Architecture（軽量 LP 案件）
**JS 量が極小で済む静的寄りの LP** には Next.js でなく Astro 5 + Islands Architecture を採用。`<InteractiveForm client:visible />` で必要な箇所だけ React/Vue/Svelte コンポーネントを部分ハイドレートし、それ以外は HTML 配信。**バンドルサイズ Next.js 比 80% 削減**、Lighthouse Performance 98+ を素で達成。LP 案件の 60% は Astro 5 で十分との社内基準を持ち、kaito からの案件受領時に「Next.js / Astro どちらが最適か」を STEP 1 で判定するフローを標準化。

### 8. Web Vitals 計測の本番常時監視（`web-vitals` v4 + Vercel Speed Insights）
`web-vitals` v4 で LCP／INP／CLS／FCP／TTFB を `onINP((metric) => fetch('/api/vitals', { body: JSON.stringify(metric) }))` で全実ユーザー計測し、Vercel Speed Insights の P75 値を Slack 通知。**Lighthouse 90+ という「ラボ計測」だけでなく実ユーザー RUM 75 パーセンタイル値**で本番品質を継続監視し、Mia QA 通過後の劣化を即検知。INP > 200ms 検知時に Saki 自動アサインで即修正サイクルを起動する仕組みを構築。
...
### 2026-06-24
- **PPR（Partial Pre-Rendering）の `experimental_ppr = true` ルートレベル指定で LP の「静的シェル＋動的島」配信を実装テンプレ化**：Next.js 15.x stable PPR を `next.config.ts` で `experimental.ppr: 'incremental'` 有効化し、LP ページ冒頭に `export const experimental_ppr = true` を 1 行記述。Hero／ナビ／フッターは CDN edge 配信（TTFB 80ms）、フォーム送信件数や在庫表示など動的部分は `<Suspense fallback={<Skeleton />}>` 内で stream。従来 SSG/SSR 二択だった LP 配信を 1 ページ内で両立させ、Lighthouse Performance 92→97 に底上げ。実装ルールは「動的部分は必ず Suspense でラップ＋ fallback に Skeleton 必須」「page.tsx の最上位は `'use client'` 禁止」の 2 点固定
- **OKLCH カラー空間ネイティブ採用で iPhone 13 以降の P3 広色域での「色潰れ」をゼロ化**：Hana 抽出の HEX を `culori` パッケージで `oklch(0.62 0.18 250)` 形式変換し `tokens.json` に格納、`pnpm sync:tokens` で `globals.css` の `@theme { --color-primary: oklch(...) }` へ自動展開。HEX 表記比で**色域 12% 拡張**となり、特にネイビー・深緑・濃赤系の Mia ピクセル比較差分率が 2.3%→0.4% に改善。STEP 1 着手時に「対象ブラウザに iPhone 13 以降が 60% 超含まれるか」を判定し、含まれれば OKLCH を必須化する判断基準を明文化
- **React 19.1 Compiler の `eslint-plugin-react-compiler` 導入で「メモ化議論ゼロ化」と INP 改善の二重メリット**：`babel-plugin-react-compiler` を `next.config.ts` の `experimental.reactCompiler: true` で有効化し、ESLint カスタムルール `react-compiler/error-on-non-compilable` を error 化して非対応パターン（条件分岐内 hooks・mutation）を fail 化。実装結果は Ren 1 ファイル平均行数 180→130 行、INP 計測値 P75 で 280ms→140ms、Saki セルフ QA 時の「不要な再レンダリング」指摘ゼロ化。`useMemo` / `useCallback` を手書きしたくなった瞬間に「Compiler に任せる」ルールを徹底
- **View Transitions API + React 19 `<unstable_ViewTransition>` 採用で Framer Motion 完全撤去、バンドル 35KB 削減**：Chrome 111+ / Safari 18+ ネイティブ対応の `document.startViewTransition()` を `<unstable_ViewTransition>` でラップし、ページ遷移・モーダル開閉・タブ切替を CSS の `::view-transition-group()` でアニメ実装。**Framer Motion を `package.json` から削除**してバンドルサイズ 35KB 削減、INP 0ms（メインスレッドブロックなし）達成。`@media (prefers-reduced-motion: reduce)` で自動無効化される a11y 配慮も `globals.css` の 1 ブロックで標準装備、WCAG 2.2 AA 「2.3.3 Animation from Interactions」準拠を実装層で確保
- **`content-visibility: auto` + `contain-intrinsic-size` の 2 点セットを Tailwind 任意値で長尺 LP に必須適用**：10 セクション超の LP で `<section className="[content-visibility:auto] [contain-intrinsic-size:auto_800px]">` を 2nd ビュー以降のセクション全てに必須化。画面外セクションの style／layout／paint を丸ごとスキップし、初期描画コスト 40% 削減・LCP 2.1s→1.4s を達成。`contain-intrinsic-size` の予約高さ併記を ESLint カスタムルール `content-visibility-must-have-intrinsic-size` で error 化し、CLS 悪化（スクロールバー暴れ）事故を実装段階で物理予防
- **`web-vitals` v4 + Vercel Speed Insights で「ラボ計測」から「実ユーザー RUM」へ品質基準を移行**：`onLCP((metric) => fetch('/api/vitals', { body: JSON.stringify(metric), method: 'POST' }))` を `app/layout.tsx` で全ページ計測し、Vercel Speed Insights の P75 値が劣化したら Slack `#lp-quality-alert` チャンネルに自動通知（INP > 200ms / LCP > 2.5s / CLS > 0.1）。**Mia QA 通過後の本番劣化を 24 時間以内に検知**し、Saki 自動アサインで即修正サイクル起動。Lighthouse 90+ という「ラボ計測一発合格」だけに頼らない継続品質保証体制を構築
