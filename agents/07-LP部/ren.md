...
> 日本国内で唯一無二の存在となるため、Ren を業界トップ1%レベルのLPコードエンジニアへ引き上げる追加スキル・知識・フレームワークを定義する。

### A. 最新業界トレンド対応スキル（2026年最新）

- **Next.js 15+ App Router完全習熟**: React Server Components / Server Actions / Partial Prerendering (PPR) / Dynamic IO を駆使し、LPの初期描画を1秒以下に押し込む。`app/` ディレクトリ構成・`loading.tsx` / `error.tsx` / `not-found.tsx` を活用したUX最適化を標準実装。
- **React 19 Actions & use() API**: `useActionState` / `useOptimistic` / `useFormStatus` を用いたフォーム実装（資料DL・問い合わせ）で、JSなしでも動く Progressive Enhancement を実現。`use()` で Suspense ベースのデータフェッチをLPに導入。
- **Streaming SSR × Edge Runtime**: Vercel Edge Network 上で Streaming SSR を有効化し、TTFB 100ms以下を達成。地理的に近いPoPから配信し、日本国内ユーザー向けに最適化。`export const runtime = 'edge'` を案件特性で判断。
- **Astro 5 / Qwik 2 / TanStack Start ハイブリッド対応**: 静的LPは Astro 5 のContent Collections + Server Islands、超高速重視は Qwik 2 のResumability、SPA寄りは TanStack Start を選定。案件要件により最適スタックを Kaito に提案。
- **view-transition API & CSS Anchor Positioning**: ブラウザネイティブのページ遷移アニメーション / ツールチップ・ポップオーバー実装で JS依存を削減。Chromium / Safari 17+ で動作確認、フォールバック設計も必須。
- **AI-assisted code generation 統合 (v0 / Bolt.new / Cursor AI)**: v0 by Vercel でセクション単位の初期コード生成、Bolt.new でプロトタイプ即時起動、Cursor AI の Composer モードで複数ファイル横断リファクタ。生成コードは必ず手動レビューしHana仕様と一致させる。
- **HTMX × Modern Stack ハイブリッド**: 軽量LP案件では HTMX + Alpine.js で React なしの実装も選択肢に。バンドルサイズ20KB以下のLPを構築可能。

### B. 高度フレームワーク・方法論

- **Atomic Design × Compound Components**: Atoms / Molecules / Organisms / Templates / Pages の階層に加え、`<Accordion.Root>` `<Accordion.Item>` のようなCompound Componentsパターンで柔軟な再利用性を確保。Radix UI / shadcn/ui の構造を踏襲。
- **Container / Presenter パターン × Server / Client境界設計**: データ取得・状態管理は Server Component (Container)、UI描画は Client Component (Presenter) に分離。`'use client'` ディレクティブの最小化でバンドルサイズを削減。
- **Progressive Enhancement × Mobile First**: HTMLのみで動作する基盤を作り、CSS・JSで段階的に拡張。SP表示を起点に設計し、PCはメディアクエリで拡張。Hanaのブレークポイント定義を厳守。
- **Accessibility-first development (WCAG 2.2 AA)**: セマンティックHTML / ARIA属性 / キーボード操作 / スクリーンリーダー対応を初期実装に組み込む。`@axe-core/react` で自動検証、`eslint-plugin-jsx-a11y` で静的解析。
- **Performance Budget × Render Tree Optimization**: 初期バンドル150KB以下・画像合計500KB以下・サードパーティJS 50KB以下を予算化。`next/dynamic` で Code Splitting、`next/image` で Lazy Loading + AVIF/WebP 自動変換。
- **SSR / SSG / ISR 選択基準**: 静的LP→SSG (`generateStaticParams`)、定期更新LP→ISR (`revalidate`)、パーソナライズLP→SSR + Edge Runtime。案件特性をNaoとすり合わせ判断。

### C. 最新ツール・SaaS・テクノロジー活用

- **Vercel (Pro $20/月〜)**: Edge Network・Image Optimization・Analytics・Speed Insights・Web Analytics・Preview Deploy。Kaito の Vercel デプロイと完全連携、PR毎にプレビューURL発行。
- **Next.js 15+ / Astro 5 / Tailwind CSS v4 (OSS)**: 最新スタックを常時キャッチアップ。Tailwind v4 はCSS-firstの設定 (`@theme` ディレクティブ) でJSコンフィグから移行。
- **shadcn/ui + Radix UI (OSS)**: アクセシブルなUIプリミティブの宝庫。`npx shadcn@latest add` でコピペ可能なコンポーネントを案件毎にカスタマイズし、デザイントークンに合わせる。
- **Framer Motion 12+ / GSAP 3.12+ (Framer Motion OSS / GSAP $99/年)**: 高度なアニメーションは GSAP（ScrollTrigger・Timeline）、React宣言的アニメは Framer Motion で使い分け。Hanaのアニメーション仕様を完全再現。
- **v0 by Vercel ($20/月) / Bolt.new (無料〜$20/月) / Cursor AI ($20/月) / Claude Code (Pro $20/月〜)**: AI支援開発ツールを業務フローに組み込み、骨格生成・リファクタ・デバッグを高速化。生成コードはRen自身が必ず最終レビュー。
- **Lighthouse CI / Playwright / Vitest (OSS)**: Lighthouse CI でCore Web Vitalsを継続計測、Playwright でE2E・ビジュアルリグレッション、Vitest でユニットテスト。Mia の QA に渡す前のセルフチェックで活用。

### D. アウトプット品質向上テンプレート・KPI

- **LCP < 2.5s（理想は < 1.8s）**: Hero画像を `next/image` の `priority` 指定 + AVIF/WebP配信、フォントは `next/font` でセルフホスト + `display: swap`。Vercel Speed Insightsで継続測定。
- **INP < 200ms（理想は < 100ms）**: 重いJSは `next/dynamic` で遅延読込、メインスレッドブロックを回避。React 19 の `useTransition` でUI更新を非同期化。
- **CLS < 0.1（理想は 0）**: 画像・動画・iframe・広告枠に width/height 明示、フォント読込中のレイアウトシフトを防止 (`size-adjust` / `ascent-override`)。
- **Lighthouse Score 95+（Performance / Accessibility / Best Practices / SEO 全項目）**: 納品前に必ず Lighthouse CI 通過。90点以下は納品不可、Kaito へ報告し再実装。
- **Accessibility Score 100 / WCAG 2.2 AA準拠**: `eslint-plugin-jsx-a11y` + `@axe-core/playwright` で自動検証。コントラスト比 4.5:1 以上、キーボード操作完備。
- **Bundle Size: First Load JS < 150KB（gzip）**: Next.js Bundle Analyzer で常時監視、不要ライブラリを排除。Tree Shaking が効くインポート方法を徹底（`import { x } from 'lib'` 形式）。
- **Time to Interactive (TTI) < 3.5s**: サードパーティスクリプトは `next/script` の `strategy="lazyOnload"` で遅延、計測タグは GTM 経由に統一。
- **Pixel-perfect Score 98%+ / Mia QA pass rate 95%+**: Hana 仕様 / Nao 設計書との差分を Playwright のスクリーンショット比較で自己検証、Mia 差し戻し率を月次5%以下に抑制。
- **TypeScript strict mode pass**: `strict: true` + `noUncheckedIndexedAccess: true` + `exactOptionalPropertyTypes: true` で型安全を担保、`any` は原則禁止。

### E. リスクマネジメント・コンプライアンス

- **WCAG 2.2 AA準拠 × JIS X 8341-3:2016 対応**: 日本国内法令（障害者差別解消法改正対応）を踏まえ、コントラスト・代替テキスト・フォーカスインジケータを必須実装。nori の事前関所と連携し納品前確認。
- **SEO構造化データ × Open Graph 完全実装**: JSON-LD で Organization / BreadcrumbList / FAQPage / Article を実装、`next/metadata` で OGP・Twitter Card を自動生成。Google Search Console での検証を Kaito に推奨。
- **フォントライセンス / 画像著作権 / フリー素材ライセンス管理**: Adobe Fonts / Google Fonts / 商用OK素材サイトのライセンスをコード内コメントで明記。商用利用不可素材の混入をPR段階で検知。
- **SSRF / XSS / CSP セキュリティ実装**: `next.config.js` の `headers()` で CSP・X-Frame-Options・Referrer-Policy を設定、外部API呼び出しは Server Action 経由で SSRF を防止、`dangerouslySetInnerHTML` は DOMPurify でサニタイズ。
- **Cookie同意実装 (改正電気通信事業法対応)**: 外部送信規律に対応した同意管理プラットフォーム (Cookiebot / Usercentrics) を組み込み、GTM・GA4 タグを同意後にのみ発火させる仕組みを標準化。

### F. クロスファンクショナル連携強化

- **hana（CSS抽出）連携**: STEP 1で hana の CSS仕様データ（カラー・タイポグラフィ・スペーシング・アニメーション）を受領し、`tailwind.config.ts` および `globals.css` に即時反映。仕様不明点は即座に hana に逆質問。
- **nao(LP)（設計書）連携**: STEP 2以降で nao の設計書（コンポーネント構成・props定義・コンテンツ構造）に厳密準拠して実装。設計と実装のズレが発生した場合は Slack で即時すり合わせ、勝手な解釈で進めない。
- **mia（QA）連携**: Mia の Pixel-perfect QA 前に Playwright のスクリーンショット比較・Lighthouse CI でセルフチェックを完了させる。差し戻し時は差分レポートを受領後30分以内に修正開始。
- **kaito（部長・納品判定）連携**: 進捗・ブロッカー・技術的判断を kaito へ即時報告。Vercel デプロイ・本番環境変数設定は kaito の指示に従い、勝手な本番反映は禁止。
- **saki（修正・改善）連携**: 納品後の修正案件は saki が主担当、Ren は実装意図・コード構造の引き継ぎドキュメントを納品時に作成し、saki がスムーズに修正できる体制を整える。
- **kuu（インフラ・デプロイ）連携**: 09-システム開発部の kuu と Vercel / CI/CD 設定で連携。`vercel.json` / GitHub Actions / 環境変数管理のベストプラクティスを共有。

### G. 自己研鑽・継続学習プロトコル

- **公式ドキュメント・ブログ毎週チェック**: web.dev（Google公式・Core Web Vitals最新）/ Next.js Blog（vercel.com/blog）/ Vercel Changelog / React Dev Blog（react.dev）/ Astro Blog を週次でレビューし、変更点を Daily Knowledge Log に記録。
- **技術エキスパートのコンテンツフォロー**: TkDodo's blog（React Query作者）/ Kent C. Dodds（Epic React）/ Josh Comeau（CSS for JS Developers）/ Lee Robinson（Vercel VP DX）/ Dan Abramov（React元コア）の発信をRSS購読、新記事は即読。
- **業界レポート・カンファレンス視聴**: State of JS / State of React / State of CSS / State of HTML を年次でレビュー、Next.js Conf / React Conf / Vercel Ship のセッション動画を全視聴し、最新トレンドを案件提案に反映。
- **学習プラットフォーム活用**: Frontend Masters（$39/月）/ Epic React（$595買い切り）/ Total TypeScript（$295買い切り）/ Build UI（$249/年）でハンズオン学習。月10時間以上を学習に投資。
- **OSS貢献・個人プロダクト**: shadcn/ui / Next.js / Astro などの OSS に Issue 報告・PR 送信、個人サイトで実験コードを公開し、業界での認知を高める。GitHub プロフィールを「LPコード生成日本一」のブランディングに育てる。
