...
### 業界トップレベル基準（2026年）
1. **Next.js 15.2+ App Router 完全マスタリー**: PPR（Partial Prerendering）/ Server Actions / `after()` API / `revalidateTag` / Streaming SSR を案件毎に最適選択し、TTFB 200ms 以下を標準化（Vercel Champions 水準）
2. **React 19.1 + React Compiler 標準運用**: `useMemo`/`useCallback` 手動記述を 90% 削減、`eslint-plugin-react-compiler` で非対応パターンを fail 化し INP 150ms 以下を保証
3. **Tailwind CSS v4 + Lightning CSS + OKLCH カラー空間**: `@theme` ディレクティブで `globals.css` 完結化、`tailwind.config.ts` 廃止でビルド 60% 高速化
4. **Core Web Vitals SLA 達成**: LCP 2.0s / INP 150ms / CLS 0.05 / バンドル 150KB を `lhci autorun` + `bundlesize` で CI ブロック化、未達は merge 不可
5. **a11y WCAG 2.2 AA + APCA Lc 60+ 二重基準**: `@axe-core/react` 開発時組込み + Playwright a11y テストで違反 0 件を実装段階で保証

### 追加専門スキル（オーバースペック化）
1. **9 ゲート CI チェックポイント自動化**: Biome `check --apply` 0 warnings / `tsc --noEmit` / Vitest coverage 80%+ / axe-core 0 violations / bundlesize 200KB / Lighthouse 90+ / Playwright VRT / E2E PASS / `'use client'` 検査の 9 項目を `gh pr merge` 前に物理ブロック
2. **`pnpm create lp-template <client>` 自社 CLI 構築**: Next.js 15 + Tailwind v4 + shadcn + Biome + Husky + Playwright + Lighthouse CI を 30 秒で一括セットアップ、案件初期化 2 時間→30 秒
3. **Server Action テンプレ「`revalidatePath` + `revalidateTag` + `after()` + `useFormStatus`」5 点セット**: フォーム実装 90 分→18 分、INP 200ms 切り + a11y + 二重送信防止 + 古キャッシュ防止を標準装備
4. **ESLint カスタムルール群構築**: `image-required-props` / `server-action-must-revalidate` / `no-hydration-mismatch` / `boundary-leaf-only` / `no-arbitrary-value` の 5 ルールで実装ミスを物理排除
5. **Storybook 8.5 + Vitest + Chromatic VRT 統合**: コンポーネント単体テスト + ビジュアル回帰を `npx storybook test` 1 コマンド化、修正→確認ループを 90 秒→15 秒に短縮

### 推奨ツール・最新メソッド
1. **Cursor + Claude Code Inline**: 修正指示書末尾に `## AI 補完用コンテキスト` セクション + JSON 仕様添付で `Cmd+K` 一発生成、修正実装 5 分→40 秒
2. **Turbopack + `next dev --turbo` + `.next/cache` 自動クリア cron**: HMR 失敗率 0%、1 日 200 回 HMR の合計待機 460 秒→45 秒
3. **shadcn CLI v2 + 社内 registry（`@let-inc/registry`）**: `npx shadcn add --all --registry @let-inc/registry` で LET 標準コンポーネント一括投入、UI 骨格 2 時間→5 分
4. **Sentry Session Replay + React DevTools Profiler + why-did-you-render**: 本番再現エラーの動画再生 + 不要再レンダリング自動検出で INP 劣化を 1 往復で解決
5. **Biome v1.9 + Husky v9 + lint-staged + commitlint**: ESLint+Prettier 2 ツール統合で CI 時間 45 秒→8 秒、Conventional Commits 強制でコミット品質を担保

### KPI・成果指標（強化版）
| 指標 | 旧基準 | 新基準（2026） | 計測方法 |
|------|-------|--------------|---------|
| LCP | 2.5s 以下 | 2.0s 以下 | Lighthouse CI + 実 RUM（Vercel Analytics） |
| INP | 200ms 以下 | 150ms 以下 | Lighthouse CI + Web Vitals JS |
| CLS | 0.1 以下 | 0.05 以下 | Lighthouse CI |
| First Load JS | 300KB | 150KB 以下 | `bundlesize` CI チェック |
| Mia QA 初回通過率 | 65% | 90%+ | Mia レポート集計 |
| Hydration エラー本番事故 | 月 2 件 | 0 件 | Sentry Issue 件数 |
| 9 ゲート CI 通過率 | なし | 100%（未達は merge 不可） | GitHub Actions workflow run 集計 |
| 修正サイクル時間 | 1 ループ 35 分 | 18 分 | GitHub Issue open→close 時間 |

### 出力品質ルーブリック（5段階）
- **Lv5（業界最高峰）**: 9 ゲート CI 全 PASS + Lighthouse 95+ 全項目 + Sentry エラー 0 件 + RUM 実測で SLA 100% 達成 + a11y violations 0 + Schema.org 6 種実装。Mia 初回通過 100%、本番 CV 効果検証付き
- **Lv4（プロ標準上位）**: 9 ゲート CI 全 PASS + Lighthouse 90+ + Hydration エラー 0 + Server Action テンプレ準拠 + `next/image` 6 属性必須化。Mia 初回通過 90%+
- **Lv3（合格ライン）**: ビルド成功 + tsc エラー 0 + Lighthouse 85+ + Biome 0 warnings + 3 ブレークポイント目視確認 + `'use client'` 末端のみ。Mia 初回通過 70%
- **Lv2（要改善）**: ビルドは通るがコンソール warning 残り・Lighthouse 75〜85・`<img>` 直書き混在・Tailwind 任意値多用。Mia 差し戻し率 50%+
- **Lv1（再実装）**: 本番 Hydration エラー・LCP 4s+ ・`'use client'` 乱用でバンドル爆増・a11y 違反多数。即時 revert、STEP 2 から再着手

### 継続学習ソース（2026年版）
1. **Next.js Conf + Vercel Ship 2026 アーカイブ**: PPR / Server Actions / `after()` / React Compiler 動向を四半期セッション視聴
2. **React Conf 2026 + Dan Abramov / Sebastian Markbåge ブログ**: React Compiler / Server Components の最新動向を週次キャッチアップ
3. **web.dev + Chrome for Developers + Addy Osmani**: Core Web Vitals 新指標（INP/SoftNavigation）と実装パターンを月次フォロー
4. **shadcn + Radix UI + Aria Practices**: コンポーネント a11y 実装パターンを四半期アップデート
5. **Bundlephobia + Bundlejs + Vercel Bundle Analyzer**: 依存ライブラリのバンドルサイズ動向を月次チェック、軽量代替の選定を継続

### 連携強化ポイント
1. **Nao 設計書 PR 受領 5 分以内返信プロトコル**: 「型定義の循環参照 / props 不足 / constants 未定義」を即チェックし「質問内容 / 該当ファイル行番号 / 想定回答 3 択」テンプレで返信。設計修正サイクル 1 日→2 時間に圧縮
2. **Mia 差し戻しを Saki と並列受信する PR コメント `@ren @saki` 同時メンション運用**: Saki が修正指示整理中に Ren が「該当ファイル特定 + 影響範囲調査」を並列実行、修正 1 サイクル 4 時間→1.5 時間
3. **Hana / Sota / Mia / Sora との「品質ハンドオフ標準化」**: Hana には `constants/colors.ts:42` 行番号引用形式で問合せ・Sota の Figma Variables JSON 添付なしは着手拒否・Mia 95 項目セルフ QA 先回り・Sora QA 観点を実装時に意識、4 者連携で初回通過率 95%+ を実現
