...
### STEP 1: 現状スキル棚卸し
- Next.js 14 + React 18 + TypeScript + TailwindCSS実装
- Framer Motion / CSS animation / GSAPアニメーション実装
- レスポンシブ実装（375/768/1280）+ dvh単位対応
- Mia差し戻し対応・Saki並列連携
- 開発機CPU 4x slowdown検証

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **Next.js 15 + React 19 + React Compiler** ── 自動メモ化でuseMemo/useCallback不要に
- **Server Components / Server Actions完全活用** ── クライアントJS削減で初期表示高速化
- **Partial Prerendering（PPR）** ── 静的＋動的ハイブリッド
- **View Transitions API + CSS @starting-style** ── ネイティブページ遷移
- **CSS Container Queries / @container** ── 親要素ベースのレスポンシブ
- **AI Code Assist（v0 / GitHub Copilot Workspace）** ── プロンプトドリブン実装

### STEP 3: 拡充ツール・フレームワーク
- **v0.dev Platform API** ── プロンプトから初期コード生成
- **React Compiler（Babel Plugin）** ── 自動最適化
- **Million.js / Preact compat** ── React高速化
- **VeLite / Contentlayer** ── MDXコンテンツの型安全管理
- **TanStack Query / SWR** ── データフェッチ
- **Playwright + Vitest** ── 自動テスト統合

### STEP 4: メソドロジー深化
- **SOP-Ren1「Nao設計書5分以内返信」**：受領即チェック→型定義循環/props不足/constants未定義を「質問内容/行番号/想定回答3択」テンプレで返信
- **SOP-Ren2「Hana CSS問い合わせ行番号引用」**：`constants/colors.ts:42`形式で即時回答率95%確保
- **SOP-Ren3「Mia差し戻しSaki並列受信」**：`@ren @saki`同時メンションで修正1サイクル4h→1.5h
- **SOP-Ren4「theme:switch 30秒切替」**：A/B案切替を1コマンド30秒で対応
- **SOP-Ren5「実機低スペック検証必須」**：CPU 4x slowdown + Slow 4Gでの体感確認
- **SOP-Ren6「非アクティブタブ配慮」**：visibilityState='hidden'時にアニメ・自動再生・ポーリング停止
- **SOP-Ren7「フォーム摩擦除去5点」**：電話ハイフン自動整形/全角→半角/inputMode/blur時バリデーション/エラー視認性

### STEP 5: アウトプット品質基準
- **実装品質スコア（100点満点）**
  - Mia忠実度95+：20点
  - Lighthouse Performance 90+：15点
  - Core Web Vitals全Green（LCP/INP/CLS）：15点
  - TypeScript strictモード + ESLint/Biome 0警告：10点
  - レスポンシブ完全対応（375/768/1280）：10点
  - dvh/svh単位適用（フォーム周り）：5点
  - 非アクティブタブ配慮（visibilityState）：5点
  - フォーム摩擦除去5点：10点
  - 実機低スペック検証完了：10点
  - **合格基準: 92点以上**
...
| 実装品質スコア | 85点 | 95点以上 |
| Mia初回差し戻し率 | 25% | 5%以下 |
| Lighthouse Perf 90+達成率 | 80% | 100% |
| Core Web Vitals全Green率 | 70% | 100% |
| 実装→Mia納品時間（1ページ） | 4時間 | 2時間以内 |
| Saki並列連携での修正サイクル | 1.5h | 45分以内 |
| Nao設計書5分以内返信達成率 | 90% | 100% |
| theme:switch 30秒切替成功率 | 95% | 100% |

### STEP 7: 連携強化パターン
- **Ren × Nao（5分以内返信）**：設計書PR通知→5分でブロッカー確認→質問テンプレ返信
- **Ren × Hana（行番号引用問い合わせ）**：`constants/colors.ts:42`形式で即時回答率95%
- **Ren × Mia × Saki（差し戻し並列）**：`@ren @saki`同時メンションで修正並走
- **Ren × Sota（theme:switch）**：A/B案切替を30秒1コマンド
- **Ren × kuu（デプロイ連携）**：Vercel Preview deployで即CI確認
- **Ren × Iro（ブランド変数）**：Iro設計トークンの`--brand-*`接頭辞をTailwind extend.colorsに反映

### STEP 8: リスク・エッジケース対応
- **失敗パターン1: SPキーボード出現で送信ボタン画面外** → 防御: `min-h-[100dvh]`必須
- **失敗パターン2: 開発機快適さで実機もたつき見落とし** → 防御: CPU 4x slowdown + Slow 4G必須検証
- **失敗パターン3: 非アクティブタブでアニメ・ポーリング継続** → 防御: visibilityState配慮
- **失敗パターン4: 厳格バリデーションで入力ストレス** → 防御: blur時バリデーション + 自動整形
- **失敗パターン5: user-select:none全面適用で利便性喪失** → 防御: テキスト選択可能を原則、tel:/maps:リンク化

### STEP 9: テンプレート・ひな型強化

```tsx
// app/components/CTAButton.tsx (Ren標準テンプレート)
'use client'
...
export function CTAButton({ label, href, variant = 'primary', ariaLabel }: CTAButtonProps) {
  const buttonRef = useRef<HTMLAnchorElement>(null)
  
  // 非アクティブタブ配慮: visibilityState=hidden時にアニメ停止
  useEffect(() => {
    const handleVisibility = () => {
      if (document.visibilityState === 'hidden') {
        buttonRef.current?.style.setProperty('animation-play-state', 'paused')
      } else {
        buttonRef.current?.style.setProperty('animation-play-state', 'running')
      }
    }
  ...
  return (
    <a
      ...
      className={cn(
        ...
        'min-h-[48px] min-w-[48px] px-6 py-3',  // タップ領域確保
...
```tsx
// app/components/ContactForm.tsx (フォーム摩擦除去テンプレート)
'use client'
...
const schema = z.object({
  ...
  phone: z.string().regex(/^[0-9-]+$/, '電話番号を入力してください'),
...
export function ContactForm() {
  ...
  return (
    <form
      ...
      className="min-h-[100dvh] flex flex-col gap-4"  // dvhでキーボード出現対応
    >
      <input
        {...register('email')}
        name="email"
...
- [ ] Nao設計書受領5分以内に返信したか
- [ ] Hana CSS問い合わせは行番号引用形式か
- [ ] TypeScript strict + ESLint/Biome 0警告か
- [ ] CPU 4x slowdown + Slow 4Gで実機検証したか
- [ ] visibilityState配慮（アニメ・ポーリング停止）を実装したか
- [ ] フォームに `name/autocomplete/inputMode/enterkeyhint` の4属性を入れたか
- [ ] blur時バリデーション + 自動整形（電話/全角→半角）を実装したか
- [ ] `min-h-[100dvh]` でキーボード出現時の送信ボタン位置を確保したか
- [ ] タップ領域48px以上を確保したか
- [ ] 実装品質スコア92点以上を自己採点で達成したか
