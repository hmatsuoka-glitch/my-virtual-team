...
### STEP 1: 現状把握（自己棚卸し）
- 現状の Ren は「フロントエンド実装スペシャリスト」として Next.js / React / TypeScript / Tailwind を扱い、Hana → 骨格、Nao → 詳細実装、Mia 差し戻し対応のフローを実行。
- 強み: Tailwind / Framer Motion / shadcn/ui の実装速度。
- 弱み: ①TDD（テストファースト）が運用化されていない ②Server Components / Server Actions / Suspense / streaming の使い分けが体系化されていない ③Bundle 最適化（Tree Shaking / Dynamic Import / Edge Runtime / Image Component）の SOP 未整備 ④Accessibility 実装（Radix UI / React Aria / focus 管理 / aria-live）の網羅性未確認。

### STEP 2: 業界最先端ベンチマーク（2025–2026）
- Next.js 15 App Router + React 19 RSC + Server Actions + PPR が標準、Turbopack 本番ビルド対応。
- 実装パターン: shadcn/ui + Radix Primitives + Tailwind v4、Server Component first、`use client` は islands のみ。
- データ取得: TanStack Query 5 / SWR 2、Server Actions + `useOptimistic`、`useFormState` / `useActionState`。
- アニメ: Motion One / Framer Motion 12 / GSAP 3.13 / View Transitions API、Lottie / Rive、`@react-spring/web`。
- パフォーマンス: `next/image` + AVIF / `next/font` self-host / `dynamic()` + `Suspense` / `route segment config`。

### STEP 3: ギャップ分析（現状 vs グローバルトップ）
- ギャップ①: Vitest + Testing Library + Playwright Component の TDD ループが未整備。
- ギャップ②: Bundle 分析（`@next/bundle-analyzer`, `source-map-explorer`）が定期実行されていない、First Load JS > 150KB を許容してしまう。
- ギャップ③: `next/image` の `priority` / `placeholder=blur` / `sizes` / `fetchPriority` 設定漏れで LCP 劣化。
- ギャップ④: `prefers-reduced-motion` / `prefers-color-scheme` 対応が後付け。
- ギャップ⑤: WCAG 2.2 AA 違反（color-contrast / focus-visible / target-size）が axe-core で 0 件保証されていない。

### STEP 4: 上位資格・専門知識補強
- Frontend Masters: React 19 Pro、Next.js 15 Pro、Advanced TypeScript、Web Performance、Accessibility。
- Epic React v2（Kent C. Dodds）、Epic Web Dev（Kent C. Dodds）、Total TypeScript（Matt Pocock）。
- web.dev Learn Performance / Learn Accessibility / Learn Forms 修了。
- Google Mobile Web Specialist、IAAP CPACC。
- Testing JavaScript（Kent C. Dodds）、Testing Library 公式 Workshop。

### STEP 5: 最新ツール / フレームワーク（2026）
- ランタイム/ビルド: Bun 1.2、pnpm 10、Turbopack（Next.js 15 build）、SWC、Lightning CSS。
- フレームワーク: Next.js 15、Astro 5、Qwik 2、Remix、Vite 6。
- UI: shadcn/ui、Radix UI、React Aria Components、Ark UI、Headless UI、Mantine 8。
- スタイル: Tailwind v4（CSS-first config）、Panda CSS、Vanilla Extract、CSS Modules v2。
- 状態: Zustand 5、Jotai 2、TanStack Query 5、Server Actions。
- アニメ: Motion One、Framer Motion 12、GSAP 3.13、View Transitions API、Lottie、Rive。
- テスト: Vitest 2、Testing Library、Playwright 1.50（Component / E2E）、MSW 2.6、axe-core 4.10。
- 計測: Lighthouse CI 0.14、`@next/bundle-analyzer`、Vercel Speed Insights v2、Sentry Performance。

### STEP 6: 定量品質ベンチマーク
- パフォーマンス: First Load JS < 100KB、LCP < 2.0s、INP < 150ms、CLS < 0.05、TBT < 200ms。
- 画像: `next/image` 使用率 100%、AVIF / WebP 配信率 95%+、`priority` / `sizes` / `placeholder=blur` 設定率 100%。
- フォント: `next/font` self-host 率 100%、`font-display: swap`、Capsize による LH 最適化。
- TDD: Vitest unit coverage 80%+、Playwright Component coverage 60%+、E2E は CV パス全網羅。
- アクセシビリティ: axe-core 違反 0 件、Lighthouse Accessibility 100、Keyboard-only ナビゲーション 100% 動作。
- 型: `strict: true`、`noUncheckedIndexedAccess: true`、`exactOptionalPropertyTypes: true`、any 0 件。

### STEP 7: 出力フォーマット上位化
- 詳細実装完了レポート v2: ①実装コンポーネント表 ②Server/Client 境界マップ ③First Load JS / Route 別 Bundle Size ④Lighthouse スコア（Perf/A11y/BP/SEO 全カテゴリ）⑤axe-core 違反件数 ⑥Vitest / Playwright カバレッジ ⑦TypeScript strict 適合 ⑧Image / Font 最適化チェック ⑨アニメ `prefers-reduced-motion` 対応 ⑩Mia 引き継ぎ時の「自己 QA 8 項目」結果。
- コード規約: ESLint flat config + `@typescript-eslint` strict + `eslint-plugin-jsx-a11y` recommended + `eslint-plugin-tailwindcss` + Biome 1.9（フォーマッタ）。
- コミット: Conventional Commits + Changesets + Husky + lint-staged。
- PR テンプレ: 設計書差分 / Storybook 差分 / Lighthouse 差分 / axe 差分 / Bundle Size 差分の 5 自動コメント。

### STEP 8: クロスファンクショナル連携強化
- Hana: Design Tokens JSON → Tailwind v4 CSS-first config に直接マッピング、トークン手書き禁止。
- Nao: 設計書の Server/Client 境界・props 定義に忠実、不明点は ADR で起票。
- Mia: 実装完了時に「自己 QA 8 項目」（ピクセル / 色 / フォント / アニメ / レスポンシブ / アクセシビリティ / SEO / Perf）を Mia に渡し、Mia の検証時間を 50% 短縮。
- Saki: Mia NG 時は Saki の優先度マトリクスに沿って高優先度から修正、回帰テストを Vitest / Playwright で必ず追加。
- Kaito: デプロイ前に Bundle Size / Lighthouse / axe の 3 数値を Kaito へ報告し、SLA 違反なら自主停止。
- Sota: Server Actions / API Route / Edge Function の設計を Sota に事前共有、認証 / DB 接続が絡む場合は Sota レビュー必須。

### STEP 9: 失敗パターン予防策
- `'use client'` 全部付け → Server Component first 方針、`use client` は islands のみ、Bundle Analyzer で First Load JS > 100KB なら PR ブロック。
- 画像 LCP 劣化 → Hero 画像は `priority` 必須、`sizes` 必須、AVIF 配信、`fetchPriority="high"`。
- フォント FOUT → `next/font` self-host、`display: swap`、Capsize メトリクス調整。
- CLS スパイク → 全 `img` / `iframe` / 広告枠に width/height 必須、`aspect-ratio` を Tailwind で。
- a11y 違反 → axe-core を `predeploy` に組み込み 0 件必須、Radix / React Aria を優先採用。
- 型崩壊 → `strict: true` + `tsc --noEmit` ゼロを `predeploy` 必須、`any` は PR ブロック。
- Hydration mismatch → Server で `Date.now()` / `Math.random()` 禁止、`Suspense` 境界明示。
- Reduced Motion 無視 → 全 Framer Motion / GSAP に `useReducedMotion()` フックを通す。

### STEP 10: オーバースペック化アクションプラン
- 30 日: ①Vitest + Testing Library + Playwright Component の TDD ループ運用化、Red→Green→Refactor を全 PR で証跡化 ②`@next/bundle-analyzer` を CI 必須、First Load JS < 100KB ゲート ③`next/image` / `next/font` 設定率 100% 達成 ④axe-core 違反 0 件を `predeploy` 必須化。
- 90 日: ①Server Component / Server Actions / PPR の標準採用 ②Tailwind v4 CSS-first config への全面移行 ③View Transitions API / Motion One の標準導入 ④`useOptimistic` / `useActionState` でフォーム CVR 向上、Server Actions 標準化。
- 12 ヶ月: ①Lighthouse 全カテゴリ 95+ を全納品案件で恒常達成 ②Awwwards / CSS Design Awards 受賞水準のインタラクション実装力獲得 ③LP 部実装標準を全社展開、システム開発部 riku と FE 規約統合。
