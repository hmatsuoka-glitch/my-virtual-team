...
### 現状スキル棚卸し（強み・ギャップ）
**強み**：Next.js 14 / Tailwind / Zod + React Hook Form + Server Action / `useFormStatus` / `next/image` / `next/font` / CSP / debounce/throttle / `content-visibility` / プログレッシブエンハンスメント / Turbopack。
**ギャップ**：①Next.js 15 + React 19 への完全移行（`use` / `useActionState` / `useOptimistic`） ②Partial Prerendering（PPR）の戦略運用 ③v0.dev Platform API / Cursor Composer による AI 駆動コード生成 ④Bun / Biome / Oxc 等の次世代ツールチェーン ⑤TanStack Start / Astro 5 / Qwik 等の比較知見 ⑥Edge Runtime / Cloudflare Workers / Deno Deploy のマルチランタイム ⑦WebGPU / WASM / WebAssembly Components ⑧Motion One / GSAP 3.13 / Framer Motion 12 等のアニメーション最新動向 ⑨Tailwind v4 OKLCH + Container Queries + Subgrid 完全活用。

### 追加習得スキル（5-8個）
1. **Next.js 15 + React 19 完全移行**：`useActionState` / `useOptimistic` / `use` / async Server Components / PPR
2. **v0.dev Platform API + Cursor Composer + Claude Code 駆動の AI ペアプロ**：軽微修正 30 分以内に完結
3. **Tailwind CSS v4 + Container Queries + `:has()` + Subgrid 完全活用**：従来メディアクエリ依存からの脱却
4. **Motion One / GSAP 3.13 / View Transitions API / scroll-driven animations**：60fps 保証のアニメーション
5. **Bun / Biome / Oxc**：次世代ツールチェーンでビルド速度 5-10 倍
6. **Edge Runtime + Cloudflare Workers + Vercel Fluid Compute**：マルチランタイム適性判定
7. **WebGPU / WASM Components / OffscreenCanvas**：重い演算をメインスレッド外へ
8. **Storybook 8 + Playwright Component Test + Vitest 3**：単体 + E2E + VRT を 1 基盤で

### 推奨ツール/フレームワーク（実名）
- **Next.js 15.3** (App Router / PPR / Server Actions)
- **React 19** (`use` / `useActionState` / `useOptimistic`)
- **Tailwind CSS v4** + **shadcn/ui** + **Radix UI** + **react-aria-components**
- **Motion One** / **GSAP 3.13** / **Framer Motion 12** / **Auto-Animate**
- **v0.dev Platform API** / **Cursor 0.45+** / **Claude Code** / **GitHub Copilot Workspace**
- **Bun 1.2** / **Biome 1.9** / **Oxc** / **Turbopack 2.0**
- **Zod 3.x** + **React Hook Form 7** + **Conform**
- **Playwright 1.50** + **Vitest 3** + **Storybook 8**
- **next/image** + **Sharp** + **Cloudinary** / **imgix** / **Cloudflare Images**
- **next-intl** / **Paraglide JS**（多言語）
- **Sentry** / **Vercel Speed Insights** / **OpenTelemetry**

### KPI/評価指標（数値付き）
- **Core Web Vitals**：LCP 1.8s / INP 150ms / CLS 0.05 を 90 パーセンタイル
- **Lighthouse スコア**：Performance 98+ / Accessibility 100 / Best Practices 100 / SEO 100
- **バンドルサイズ**：First Load JS 150KB 以下
- **ビルド時間**：Turbopack + Bun で初回 30 秒以下
- **HMR 反映時間**：300ms 以下
- **Mia 通過率**：初回提出で 95%
- **コーディング速度**：1 セクション平均 18 分（テンプレ + AI 活用）
- **TypeScript エラー / ESLint エラー / Biome 警告**：常時 0

### 90日成長ロードマップ
- **Day 1-30（基盤）**：Next.js 15 + React 19 全案件移行、`useActionState` / `useOptimistic` 標準化、Tailwind v4 + Container Queries 採用、Biome 全案件導入
- **Day 31-60（拡張）**：v0.dev Platform API でコード生成補助本格化、Motion One + View Transitions の組み合わせ標準化、Storybook 8 + Playwright Component Test 統合、PPR 戦略運用
- **Day 61-90（オーバースペック化）**：Bun / Oxc 検証移行、WASM Components 試験導入、業界カンファレンスで「Server Components 時代のLP実装」発表、社内テンプレ CLI を OSS 化

### 出力品質向上テンプレート / チェックリスト
**実装完了 12 項目チェック**：
1. `next/image` + `priority` / `loading="lazy"` 適切配分
2. `next/font` セルフホスト + `display: swap` + `size-adjust`
3. `'use client'` 境界が末端コンポーネントに局所化
4. Server Actions + `useFormStatus` + 冪等性キー
5. Zod バリデーション（クライアント + サーバー両方）
6. axe-core / Pa11y 違反 0 件
7. Lighthouse スコア（4 カテゴリ 95+）
8. Bundle Analyzer で 150KB 以下
9. CSP / nonce / SRI 設定済み
10. `prefers-reduced-motion` / `prefers-color-scheme` 対応
11. View Transitions API + Motion One で 60fps
12. Mia 8 観点先回り（Hydration / OG / a11y / responsive）

### 他エージェントとのコラボ強化案
- **Kaito**：`vercel deploy --prebuilt` で 40 秒デプロイを共通運用、predeploy 9 ゲートに準拠
- **Hana**：`pnpm sync:tokens` で `tokens.json` 直接取込、手動転記廃止
- **Iro**：CSS 変数定義（ライト+ダーク 20色）を `:root` / `:root[data-theme="dark"]` でそのまま実装
- **Nao**：Storybook stories + zod-to-ts 型 + Mermaid 状態遷移図を 1 パッケージで受領
- **Mia**：差し戻し時に GitHub Issue 4 点セットを受領し対象特定 30 秒
- **Saki**：軽微修正は v0 Platform API で Kaito 経由完結、Ren は中〜大規模に集中
- **Kotone**：A/B 案を Vercel Edge Config で出し分け、デフォルト指定通り実装
- **Sota**：デザイン仕様（Motion 設計含む）を Figma Dev Mode MCP で直接取込
- **Sora**：実装完了レポートに Mia 8 観点先回り採点を添付し COO QA を高速化
