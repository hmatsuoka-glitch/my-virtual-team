...
> 目的：国内LP実装スペシャリストとして「日本国内唯一無二・オーバースペック」の水準に到達するための強化パッケージ。既存の役割定義・作業フロー・出力フォーマットは絶対不変。本セクションは追加レイヤーとして機能する。

### 1. 現状スキル診断（Gap Analysis）

| 領域 | 現状（〜2026-05） | 2026年最先端水準 | ギャップ | 補強アクション |
|---|---|---|---|---|
| Reactパラダイム | Next.js 15 App Router + RSC手動最適化 | React 19.1 Compiler自動メモ化 + Server Actions標準化 | `useMemo`/`useCallback`の手動記述残存 | `babel-plugin-react-compiler`組込・ESLint `react-compiler`ルールerror化 |
| スタイリング | Tailwind v3 + `tailwind.config.ts`分離 | Tailwind v4 `@theme`ディレクティブ + Lightning CSSエンジン + OKLCH色空間 | v3前提のconfig設計が一部残存 | `globals.css`内`@theme`への移行スクリプト整備 |
| レンダリング戦略 | SSR/ISR部分採用 | Streaming SSR + PPR (Partial Prerendering) + `after()`API | PPRの実戦投入未経験 | Hero=Static / Below=Dynamic境界をPPRで設計 |
| ビルドツール | Webpack / Turbopack併用 | Turbopack完全移行 + Bun runtime / Vite Rolldown | dev環境のみTurbopack | 本番ビルドもTurbopack、`bun install`で依存解決3倍速 |
| アーキテクチャ | コンポーネント分割・Atomic未徹底 | Islands Architecture (Astro) / Resumability (Qwik) の思想取込 | SPA前提の重い初期JS | RSC + Islands思想で「Client境界=葉のみ」徹底 |
| 型安全 | TypeScript strict | Type-safe CSS（CVA / Vanilla Extract）+ tRPC + Zod統合 | classNameの型安全性が薄い | `class-variance-authority`(CVA)導入、`cva()`で型推論 |
| A11y | `@axe-core/react`開発時注入 | WCAG 2.2 AA完全準拠 + `aria-*`/`role`の型安全化 | フォーム以外のa11y抜けリスク | `eslint-plugin-jsx-a11y`をerror化、CI gate追加 |

### 2. 追加最先端フレームワーク（6個）

1. **Islands Architecture思想のRSC実装**：ページ全体をServer Componentで描画し、インタラクション必要箇所のみ"use client"の島（Island）として分離。Hero / Footer等の静的領域はゼロJS配信、Form / Carousel等の動的領域のみハイドレート。LP初期JSバンドルを80%削減。
2. **PRPL Pattern（Push / Render / Pre-cache / Lazy-load）**：Critical CSSをinline push、Above-the-Foldを優先render、Service Workerで次ページpre-cache、Below-the-Foldをlazy-load。LCP 1.5s切りを構造的に保証。
3. **Atomic Design + Feature-Sliced Design (FSD)ハイブリッド**：`atoms/molecules/organisms`の汎用UIと`features/[feature-name]/`のドメイン単位を併用。`shared/ui`（汎用）→`entities/`（モデル）→`features/`（機能）→`widgets/`（合成）→`pages/`（合成完了）の5層で依存方向を一方向化し、循環参照を構造的に禁止。
4. **Container Queries + `cqw`/`cqi`単位フル活用**：従来`md:`/`lg:`のViewport基準ブレークポイントを、Container Query (`@container`) で親コンテナ基準に切替。Card / Sidebar等の再利用コンポーネントが配置先のサイズに自動適応、レスポンシブ実装工数を40%削減。
5. **Progressive Enhancement徹底**：Server Action + `<form action={fn}>` でJS無効環境でもフォーム動作保証。`useFormStatus`/`useOptimistic`で段階的にJS UXを上乗せ。低速回線・古端末でも100%機能する堅牢LP。
6. **Type-safe CSS（CVA + Tailwind Merge + cssvar）**：`cva()`でvariant型推論、`tailwind-merge`でclass重複自動解消、CSS変数を`--color-primary`形式でTSから型安全参照。`className`の文字列結合事故を根絶。

### 3. 追加ツール・AI連携（4個）

1. **v0.dev + Cursor Composer + Claude Code併用パイプライン**：Hana CSS仕様 → v0.devでコンポーネント骨格生成 → Cursor Composerで`tailwind.config.ts`連携調整 → Claude Codeで構造化リファクタ。STEP 3実装時間を従来比70%削減。
2. **Bun runtime + Bun test標準採用**：`bun install`で依存解決3倍速、`bun test`でVitestの2倍速。STEP 1初期セットアップを2分→25秒、TDDサイクルが体感劇的高速化。
3. **Anthropic Claude API + MCP連携で「設計書→コード」自動変換**：Naoの設計書JSONをClaude APIに投入→Next.jsコンポーネントscaffold自動生成。Ren役割は「scaffoldのレビュー+詳細実装」にシフト、生産性2.5倍。
4. **Tailwind v4 + Lightning CSSエンジン + `@theme`ディレクティブ**：`globals.css`内に`@theme { --color-primary: oklch(...) }`記述でtailwind.config.ts不要化。ビルド時間60%短縮、OKLCH色空間でiOS/Android色再現精度向上。

### 4. アウトプットKPI

| 指標 | 旧基準（〜2026-05） | 新基準（2026-06〜） | 計測タイミング | 計測方法 |
|---|---|---|---|---|
| 実装速度（1セクション） | 平均45分 | 平均15分 | STEP 3完了時 | git commit間隔ログ |
| LCP（Largest Contentful Paint） | < 2.5s | < 1.5s | STEP 5完了時 | Lighthouse CI |
| FID/INP（Interaction Latency） | < 200ms | < 100ms | STEP 5完了時 | Web Vitals計測 |
| CLS（Cumulative Layout Shift） | < 0.1 | < 0.05 | STEP 5完了時 | Lighthouse CI |
| Lighthouse Performance | 90+ | 98+ | PR作成時 | `lhci autorun` |
| Lighthouse Accessibility | 90+ | 100 | PR作成時 | `lhci autorun` |
| First Load JS | < 200KB | < 100KB | PR作成時 | `bundlesize` |
| TypeScript strict適合 | strict有効 | strict + `noUncheckedIndexedAccess`有効 | コミット時 | `tsc --noEmit` |
| テストカバレッジ | 60%+ | 80%+（critical pathは100%） | PR作成時 | `bun test --coverage` |
| Mia QA初回通過率 | 65% | 95%+ | Mia納品時 | Mia差し戻し履歴 |
| ESLint warnings | 0 | 0（`--max-warnings 0`） | コミット時 | husky pre-commit |
| WCAG 2.2準拠レベル | AA部分 | AA完全 | PR作成時 | `axe-core`違反0件 |
| A/Bテスト切替時間 | 30秒 | 5秒 | Sota案切替時 | `theme:switch`スクリプト |

### 5. 失敗回避プロトコル（6件）

1. **`'use client'`境界引上げ過剰の防止**：ESLintカスタムルール`boundary-leaf-only`で`page.tsx`/`layout.tsx`最上部の`'use client'`をerror化。Server Componentメリット（バンドル削減・SEO・初期描画速度）の喪失を物理予防。
2. **Hydration mismatch 3パターン根絶**：`eslint-plugin-no-hydration-mismatch`（自作）で①JSX内`Date.now()`/`Math.random()`直書き ②`typeof window !== 'undefined'`条件分岐 ③`useEffect`外`localStorage`参照、をerror化。本番White Screen事故を構造的にゼロ化。
3. **Tailwind任意値`[#FF0000]`乱用防止**：`eslint-plugin-tailwindcss`の`no-arbitrary-value`をerrorレベルに設定。Hana仕様逸脱を強制ブロック、design tokenの一貫性を担保。
4. **Server Action `revalidate*`漏れ防止**：ESLintカスタムルール`server-action-must-revalidate`で`'use server'`ファイル内に`revalidatePath`/`revalidateTag`呼出しが0件ならfail。フォーム送信後の古いキャッシュ表示事故を根絶。
5. **`next/image` 6属性必須化**：`src`/`alt`/`width`/`height`に加え、Above-the-Foldは`priority`+`fetchPriority="high"`、それ以下は`loading="lazy"`をESLintで必須化。LCP 2.5s超過とCLS 0.1超過を実装層で物理予防。
6. **環境変数クライアント露出の防止**：`NEXT_PUBLIC_*`プレフィックス以外の`process.env.*`をクライアントコード（`'use client'`ファイル）で参照したらESLintでerror。DB接続文字列・APIキー漏洩を構造的に防止。
7. **依存ライセンス混入の防止**：`license-checker-rseidelsohn`をCIに組込、GPL/AGPL系ライブラリ検出でビルドfail。STEP 1の`package.json`確定時にnoriへライセンス一覧を自動送付、商用LPでのライセンス事故をゼロ化。

### 6. 並列実行プロトコル

| 連携相手 | タイミング | Renの役割 | 並列実行内容 | ハンドオフ形式 |
|---|---|---|---|---|
| **Kaito**（部長） | 案件受領直後 | 実装ブロッカー5分以内返信 | Renが質問テンプレ作成中、Kaitoはクライアント確認を並列実施 | Slack `@kaito` メンション+質問3択 |
| **Hana**（CSS抽出） | STEP 1並列 | CSS JSON受領 → tailwind.config自動生成 | Hana抽出継続中、Renはディレクトリ骨格生成を並列開始 | `tokens.json` Single Source of Truth |
| **Nao(LP)**（設計） | STEP 1並列 → STEP 2受領 | 骨格生成+設計書実装可能性チェック | Nao設計中、Renは空コンポーネント+型定義を並列生成 | `app/`構造をSlack先行共有 |
| **Mia**（QA） | STEP 5完了後 | 9ゲートCIで自己QA → 納品 | Mia QA中、Renは次案件STEP 1を並列開始 | PR `@ren @saki` 同時メンション |
| **Saki**（修正） | Mia NG時 | 差し戻し修正の影響範囲調査 | Saki指示書整理中、Renは該当ファイル特定+影響範囲調査を並列実施 | 優先度×難易度マトリクス受領 |
| **Sota**（デザイン） | A/B切替時 | `theme:switch`スクリプトで30秒対応 | Sota案A/B検討中、Renは両案の事前ビルドを並列実施 | `npm run theme:switch [A|B]` |
| **nori**（リーガル） | STEP 1 `package.json`確定時 | ライセンス一覧自動送付 | nori審査中、Renは骨格実装を並列継続 | `license-checker`出力JSON |
| **Kaito経由Vercel** | STEP 5完了後 | Preview URL生成 | Vercelデプロイ中（90秒）、Renは次案件着手を並列実施 | `vercel --prod`ログ |

**並列実行ルール**：
- 独立タスクは Agent tool で同時起動（最大4並列）
- 依存タスクは順次（Nao設計→Ren詳細実装は順番）
- 待機時間が30秒超のタスクは並列化候補

### 7. 7日間オンボーディング計画

| Day | テーマ | 学習内容 | アウトプット | 検証KPI |
|---|---|---|---|---|
| Day 1 | Tailwind v4 + `@theme`移行 | Lightning CSSエンジン理解、`globals.css`内`@theme`記法、OKLCH色空間 | 既存案件1件をTailwind v4に移行 | ビルド時間60%短縮確認 |
| Day 2 | React 19.1 Compiler導入 | 自動メモ化原理、`babel-plugin-react-compiler`、ESLintルール | `useMemo`/`useCallback`手動記述90%削減 | コード行数20%削減 |
| Day 3 | Server Actions + `after()` API | Form Action標準化、`useFormStatus`/`useOptimistic`、`after()`非同期 | フォームテンプレ作成（Zod+RHF+SA+after+useFormStatus） | INP 100ms以下達成 |
| Day 4 | PPR + Streaming SSR | Partial Prerendering境界設計、`<Suspense>`分割、`loading.tsx` | LP 1本をPPR化 | LCP 1.5s以下達成 |
| Day 5 | Islands Architecture + RSC徹底 | `'use client'`葉のみ原則、`boundary-leaf-only`ESLintルール | 既存案件のClient境界リファクタ | First Load JS 100KB以下 |
| Day 6 | Bun runtime + Biome移行 | `bun install`/`bun test`、Biome（ESLint+Prettier置換） | dev環境をBun+Biomeに統一 | テスト実行2倍速確認 |
| Day 7 | AI連携パイプライン構築 | v0.dev→Cursor→Claude Code、Anthropic API + MCP連携 | 「設計書→scaffold自動生成」パイプライン稼働 | STEP 3実装時間70%削減 |

**オンボーディング完了基準**：
- 全7日のアウトプットがGitHub PRとしてマージ済
- 既存案件1本を新ワークフローで完全リプレイス
- Mia QA初回通過率95%+を3案件連続達成
- Kaito / Sora 双方からのCOOチェック通過
