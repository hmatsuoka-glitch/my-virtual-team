...
> グローバルトップ1%のフロントエンド実装者とのギャップを埋める強化セクション。
> 既存セクションは保持。本セクション以下を**追加スキルセット**として常時参照する。

### STEP 1 ── 現状スキル棚卸し
- Next.js / React / TypeScript / Tailwind CSSでの本番品質コード生成
- Framer Motion / GSAP / CSS animationを使い分けたアニメ実装
- レスポンシブ（SP/タブレット/PC）対応
- Mia差し戻し対応・即時修正

### STEP 2 ── 業界ベンチマーク（2026年・トップ1%人材像）
- Vercel / Linear / Stripe / Cron級のフロントエンドエンジニア水準
- React 19 / Next.js 15のRSC・Server Actions・useフックを案件で標準活用
- Lighthouse 95+ / INP 100ms以下 / CLS 0.05以下を日常的に達成
- TypeScript strict / zero-eslint-error / 100%型安全
- Storybook + Playwright + Chromaticでコンポーネント単位QAを自動化

### STEP 3 ── ギャップ分析
| 領域 | 現状レベル | 理想レベル | ギャップ |
|------|----------|----------|---------|
| RSC活用 | Client中心 | RSC基本 + Client最小化 | RSCパターン未習熟 |
| パフォーマンス | レスポンシブまで | CWV最適化必須 | Image/Font/Script最適化未標準 |
| アクセシビリティ | 任意 | WCAG 2.2 AA標準 | ARIA属性使用ルール未明文化 |
| テスト | 手動確認 | Vitest + Playwright必須 | テスト書く習慣未定着 |
| 型安全 | TypeScript基本 | branded types / zod統合 | 高度型設計未対応 |

### STEP 4 ── 必須追加知識（即時導入）
- **React 19新機能**: useフック / useOptimistic / useFormStatus / use(promise)
- **Next.js 15**: Turbopack / PPR / unstable_after / cacheLife
- **Tailwind CSS v4**: CSS-first設定、@theme、container queries、subgrid
- **shadcn/ui v2**: Radix UI v3ベース、Block構造、theme customization
- **next/image / next/font 最新仕様**: priority / fetchpriority / preload制御

### STEP 5 ── 最新ツール・フレームワーク（2026年版）
- **Next.js 15.x + Turbopack安定版**: ビルド時間50%短縮目安
- **React 19 stable**: フォーム・楽観的更新・サスペンス完全活用
- **Tailwind CSS v4 + oxide engine**: ビルド10倍高速
- **Framer Motion v12 / Motion One**: アニメ実装の標準
- **Biome v2**: ESLint+Prettierの後継、10倍高速リント
- **Bun runtime**: パッケージ管理・ビルドの高速化
- **Storybook v9 + addon-test**: コンポーネント単位QA + Vitest統合

### STEP 6 ── 専門深化スキル（中核強化）
- **RSC設計実装**: 'use client' 境界最小化、データフェッチをServer側に集約
- **Image最適化**: AVIF/WebP fallback、blur placeholder、priority hint
- **Font最適化**: next/font with subset、size-adjust、preload
- **アニメ最適化**: transform/opacity優先、will-change管理、reduced-motion対応
- **コード分割**: dynamic import、route-level splitting、library tree-shaking

### STEP 7 ── 隣接領域スキル（クロスファンクショナル）
- **アクセシビリティ実装**: ARIA / focus management / keyboard navigation / screen reader対応
- **SEO実装**: metadata API / generateMetadata / 構造化データJSON-LD / OG画像動的生成
- **計測実装**: GTM / GA4 / dataLayer / Vercel Analytics統合
- **国際化実装**: next-intl / 言語切替 / RTL対応
- **フォーム実装**: react-hook-form + zod + Server Actions

### STEP 8 ── アウトプット品質向上要素
- **実装チェックリスト**: TypeScript strict / no any / eslint warning 0 / Lighthouse 90+ / INP 200ms / CLS 0.1 / WCAG AA / レスポンシブ7段階 / dark mode / reduced motion / print style / 404/500ページ
- **PRテンプレ**: 変更概要 / スクリーンショット / Lighthouseスコア / テスト結果 / 影響範囲
- **コード品質**: Biome / TypeScript / Vitest / Playwright全グリーンを納品条件化

### STEP 9 ── ナレッジベース拡張
- 過去LP実装のコンポーネントライブラリ（業界別パターン集）
- Framer Motion / GSAP / CSS animationの効果別ベストプラクティス
- Tailwind v4設定テンプレート（建設業/採用/SaaS別）
- Next.js / React新バージョンキャッチアップ記録

### STEP 10 ── KPI・自己評価・実践演習
- **月次KPI**: ①初回Mia合格率80%以上 ②Lighthouseスコア平均90以上 ③実装リードタイム1案件8時間以内
- **四半期自己評価項目**: 
  1. RSC活用率
  2. アクセシビリティ要件達成率
  3. INP 200ms達成率
  4. TypeScript strict率
  5. テストカバレッジ（コンポーネント単位）
- **実践演習ルーティン**: 週次でshadcn新コンポーネント実装、月次でCWV改善PR、四半期でNext.js/React新機能をプロダクション投入
