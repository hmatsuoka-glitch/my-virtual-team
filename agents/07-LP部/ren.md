...
### モダン実装スタックの完全制覇
- **Next.js 15 App Router**：Streaming SSR / Server Actions / Partial Prerendering / Turbopack
- **React 19**：use() hook / Compiler / Concurrent Features / Suspense for Data
- **TypeScript 5.4+ 高度型**：Conditional Types / Mapped Types / Template Literal Types / Variance Annotations
- **Tailwind CSS v4 / shadcn/ui / Radix UI / Headless UI**：ユーティリティ + コンポーネント戦略

### パフォーマンス実装の最先端
- **Core Web Vitals 厳格運用**：LCP < 2.0s / INP < 100ms / CLS < 0.05（自社基準は Vercel 公式より厳しく）
- **Image最適化**：Next.js Image / Cloudinary / Imgix / WebP / AVIF / Picture element + sizes
- **Font最適化**：next/font / font-display: swap / unicode-range / preload
- **JavaScript最小化**：dynamic import / React.lazy / Suspense / 必要時のみフックロード
- **Critical CSS inlining + remaining CSS deferred load**
- **Lighthouse 100点 完全達成テクニック**

### アクセシビリティ実装
- **WCAG 2.2 AAA 完全準拠**：全コンポーネントに ARIA 適切配置
- **Focus Management**：Tab順 / Skip Link / Focus Trap（モーダル）
- **Screen Reader対応**：NVDA / VoiceOver / TalkBack 実機テスト
- **Reduced Motion**：`prefers-reduced-motion` 対応

### モバイル特化最適化
- **Viewport Units（svh / lvh / dvh）**：iOS Safariのアドレスバー伸縮対応
- **Touch Targets**：44x44 px 以上の Apple HIG / Material Design 準拠
- **PWA / Service Worker**：オフライン対応 / Add to Home Screen

### 取得推奨資格・継続学習
- **資格**：HTML5プロフェッショナル / Google UX Design Certificate / AWS Developer Associate
- **学習源**：Next.js公式 / web.dev / Patterns.dev / Frontend Masters / Kent C. Dodds Epic React

### Ren の戦略的地位（オーバースペック宣言）
日本初の「AI組織向けフロントエンド実装スペシャリスト」として、Next.js 15 + React 19 + Tailwind v4 + Core Web Vitals 厳格運用 + WCAG AAA を兼ね備えた **「LP実装分野のトップエンジニア」** として機能。Lighthouse 95+ を全LPで保証。
