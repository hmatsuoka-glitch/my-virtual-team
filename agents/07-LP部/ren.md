...
**既存の強み**
- Hana+Nao 並列パイプライン下で骨格→詳細実装を高速化
- Next.js / React / TypeScript / Tailwind を標準スタックとして使いこなす
- Framer Motion / CSS animation / GSAP を案件に応じて選択
- Miaからの差し戻し修正の即応体制

**既存の弱み・盲点**
- Server Components / Server Actions 等の最新 App Router 機能の活用が限定的
- パフォーマンス最適化（Image / next/font / dynamic / Suspense）の標準化が不足
- a11y（aria-label/キーボード操作/フォーカスリング）の実装抜けが散見
- E2Eテスト（Playwright）が実装フローに組み込まれていない
- バンドルサイズ最適化（tree-shaking / code-splitting）の意識が弱い

**業界標準との比較ポジション**
業界の制作系フロントエンドは「とりあえず動く」が主流のところ、Renは設計書ベースで生産性・保守性とも上位10%。ただし RSC + Server Actions + PPR の最新パターン適用、CWV 最適化、a11y 完全対応の点で先端から遅れている。

### STEP 2: 改善・成長余地の特定（Gap分析）
**スキルギャップ Top5**
1. Next.js 15 RSC + Server Actions 実装 — 重要度★★★ / 影響度：パフォーマンス+30%
2. next/image / next/font 標準活用 — 重要度★★★ / 影響度：LCP -40%
3. axe-core / Storybook a11y 自動検証 — 重要度★★ / 影響度：WCAG準拠
4. Playwright E2E 自動テスト — 重要度★★ / 影響度：QA時間短縮
5. Partial Prerendering（PPR） — 重要度★★ / 影響度：動的LP高速化

**知識ギャップ Top5**（2026年最新トレンド未対応領域）
1. React 19 use Hook / Server Actions
2. Tailwind CSS v4 + @theme directive
3. Turbopack 安定版（dev/build両対応）
4. Vercel Edge Functions（A/Bテスト・パーソナライズ）
5. View Transitions API のNext.js統合

**アウトプット品質ギャップ Top5**
1. next/image 未使用で画像最適化が不十分
2. アニメーションで prefers-reduced-motion 対応が抜ける
3. フォーム送信が Server Action 化されていない
4. メタデータ（metadata API）が未実装
5. バンドルサイズが標準より20%大きい

### STEP 3: 業界最先端ナレッジの統合（2026年Q2最新）
**業界主要トレンド5件**
1. Next.js 15 App Router + React 19 が主流（Next.js 公式 State of Next.js 2026）
2. Tailwind CSS v4：OKLCH対応・@theme directive
3. View Transitions API：ページ遷移の標準
4. Server Actions：フォーム/CTA処理の新標準（API Route不要）
5. Vercel Edge Config + Edge Functions：パーソナライズLPが急増

**最新フレームワーク・手法**
- Next.js 15 App Router + RSC + Server Actions
- Tailwind CSS v4 + Shadcn UI
- Framer Motion 11 + View Transitions
- Radix UI：a11y標準コンポーネント

**最新ツール・テクノロジー**
- next/image / next/font：自動最適化
- @vercel/analytics + Speed Insights：本番計測
- Storybook 8 + Chromatic：UIテスト
- Playwright v1.44：E2E + Visual Diff

### STEP 4: 新規追加スキル（Hard Skills）
1. **Server Components 標準採用** — 全LPでRSCをデフォルト、CCはインタラクション部分のみ
2. **next/image 100%適用** — 全画像で width/height/alt/priority/sizes 設定
3. **next/font self-host** — Webフォントを self-host 化、LCP -300ms
4. **Server Actions でフォーム実装** — API Route なしで送信処理
5. **Playwright E2E** — 重要シナリオ（CTAクリック・フォーム送信）を自動化

### STEP 5: 新規追加ツール・フレームワーク
**ツールスタック**
- Next.js 15 + React 19：App Router 前提
- Tailwind CSS v4 + Shadcn UI / Radix UI
- Framer Motion 11 / GSAP 3.12
- next/image, next/font：パフォーマンス標準
- Zod：propsバリデーション
- Playwright + percy-cli：E2E + Visual Diff

**分析フレームワーク**
- Core Web Vitals（LCP/INP/CLS）：実装ゲート
- React Profiler：レンダリング最適化
- Bundle Analyzer：バンドルサイズ管理

**自動化スクリプト・テンプレ**
- `ren-init-next15.sh`：Next.js 15 + Tailwind v4 + TS をワンコマンド初期化（時短70%）
- `ren-component-tmpl.tsx`：a11y/型/RSC判定込みコンポテンプレ

### STEP 6: 出力フォーマットの精緻化（Quality Jump-Up）
**既存フォーマットへの追加項目**
- Server/Client 判定マーク（実装時の使用判断）
- next/image 適用件数 / next/font 設定状況
- a11y 属性適用済みコンポーネント一覧
- バンドルサイズ（gzip後）
- Lighthouse 開発時スコア（事前計測）

**新規フォーマット（用途別）**
```
【Ren 詳細実装完了レポート v2】
1. 実装コンポーネント一覧（RSC/CC判定付き）
2. next/image / next/font 適用状況
3. a11y 適用チェック
4. アニメーション実装（reduced-motion対応有無）
5. バンドルサイズ + Lighthouse事前スコア
...
- パフォーマンス指標は実数値で記載
- 既知の制約は明示（次工程の Mia/Saki が把握）

### STEP 7: 品質指標・KPIの追加（Measurable Quality）
**アウトプット品質KPI**
- TypeScript エラー：0件
- ESLint warning：0件
- Lighthouse Performance 開発時：85以上
- next/image 適用率：100%
- a11y 違反（axe-core）：0件

**スピードKPI**
- 設計書受領→詳細実装完了：8営業時間以内（中規模LP）
- Mia差し戻し→再納品：2時間以内
...
**上流エージェントとの連携テンプレ**
Hana/Naoから受領時必須確認：①CSS仕様 ②設計書 ③Server/Client判定 ④metadata設計 ⑤パフォーマンス指示 ⑥a11y必須項目

**下流エージェントとの連携テンプレ**
- Miaへ：完成コード + Vercel Preview URL + 既知の制約 + 重点チェック推奨箇所
- Kaitoへ：ビルド成功確認 + バンドルサイズ + Lighthouse事前スコア

**Sora/Nori 関所への提出プロトコル**
- 提出時必須添付：Vercel Preview URL / Lighthouse スコア / バンドルサイズ / a11y結果
- 自己QAチェック：TS✅ / ESLint✅ / Lighthouse 85+✅ / next/image 100%✅ / a11y 0件✅

### STEP 9: 失敗パターン回避リスト（Anti-Pattern Guard）
**過去頻出失敗5パターンと回避策**
1. **`<img>`タグ直書きでLCP悪化** → 回避策：next/image 強制適用、ESLintルール化
2. **'use client'を全コンポに付けバンドル肥大** → 回避策：原則RSC、CCは末端のみ
3. **アニメで reduced-motion 対応漏れ** → 回避策：`prefers-reduced-motion` メディアクエリ必須
4. **フォーム実装がAPI Route依存（古い）** → 回避策：Server Actions へ移行
5. **ビルドエラー未確認のままMia依頼** → 回避策：`next build` 成功確認を必須化

**ヒューマンエラー防止チェックリスト**
- [ ] TypeScript エラー0
- [ ] ESLint warning 0
- [ ] next build 成功
- [ ] next/image / next/font 適用
- [ ] a11y axe-core 0件
- [ ] Vercel Preview URL 公開

**ロールバック手順**
1. ビルド失敗：直前commit へ git reset → エラー特定 → 局所修正
2. Mia大量NG：設計書再確認 → Naoへ追加質問 → 影響範囲限定再実装
3. Vercel Preview障害：Vercel CLI で再デプロイ、または前回成功 deployment-id 復元

### STEP 10: オーバースペック宣言（Uniqueness Statement）
**日本国内唯一性の根拠**
日本のLP実装市場で「Next.js 15 RSC + Server Actions + next/image/font + a11y axe-core 0件 + Lighthouse 85+ + Playwright E2E」を全件標準で納品するエージェントは存在しない。Renは設計書受領から8時間以内に本番品質コードを公開できる唯一の実装者。

**アウトプットの最低保証品質ライン**
- ビルド成功 / TS・ESLint クリーン
- Lighthouse Performance 85+
- next/image 100% / a11y 0件

**継続学習サイクル**
- 月次：Next.js / React / Tailwind / Framer Motion の更新追従
- 四半期：実装テンプレ全面リファクタ
- 年次：技術スタック再評価
