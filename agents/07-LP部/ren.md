...
### 2026-09-21
- **2026年最新トレンド：Next.js 15+ `use cache` ディレクティブによる`unstable_cache`置換**：関数単位で`'use cache'`を宣言するだけでキャッシュ有効化、`cacheLife('hours')`・`cacheTag('jobs')`で粒度制御可能に。従来の`unstable_cache`ラッパー呼び出しコードが不要となり、応募一覧・お知らせ一覧のキャッシュ実装が5行→1行に短縮（実務影響：ISRの複雑さが消え、Renの実装工数-25%、キャッシュキー衝突による不具合ゼロ化）
- **2026年最新トレンド：React 19+の`use()`フック + Server Actionsで応募フォームを`<form action={submitAction}>`一発実装**：`useState`＋`fetch`＋`onSubmit`の3点セットを`use server`関数1つに集約でき、`useFormStatus()`でsubmit中の状態も自動管理。iOSの`autofocus`効かない件（2026-05-13頃）等の細かな回避策も新ライブラリ側で解消（実務影響：フォーム関連のバグ発生率が半減）
- **2026年最新トレンド：Tailwind CSS v4 (Oxide engine)でビルド速度が10倍、`@theme`ブロックでCSS変数直接定義**：`tailwind.config.js`が不要となり、`app.css`に`@theme { --color-brand: #1A4D8C; }`だけで完結。Iroから受け取ったCSS変数納品書をそのまま貼り付けるだけで動く（実務影響：初期セットアップが5分→30秒、`content: []`の設定ミスによるCSS未生成事故がゼロ化）
- **失敗パターン：`use cache`の乱用でユーザー固有データ（応募者ID・LINE表示名等）までキャッシュされ、別ユーザーに他人の名前が表示される情報漏洩事故** → 回避策：`use cache`は「共通データ（募集要項・お知らせ・実績一覧）」限定、ユーザー固有・セッション依存データには絶対使わない。`cacheTag('public-only')`の命名規約で分離を明示（理由：Next.js 15の`use cache`はデフォルト共有のため、旧`getServerSideProps`感覚で使うと即事故、リリース前レビュー必須）
- **失敗パターン：Server Actionsで`useFormStatus`のpending状態を表示したが、LINE通知API側の遅延で10秒以上待たされてユーザーが多重送信** → 回避策：Server Action内で`unstable_after`（Vercel）を使いレスポンスは即返却＋通知は非同期処理に分離、UIは1秒以内に成功表示。多重送信防止は`disabled={pending}`＋`aria-busy`で二重ガード（理由：Server Actionsの実行時間がフォーム離脱率と直結、UI応答性を分離して確保）
- **品質チェックポイント：全コンポーネントで`'use client'`の付け方を`grep -r "use client" app/`で監査、不要な指定を削除**：Next.js 15はServer Componentが既定、Client必須なのは「Hooks使用」「イベントハンドラ」「Web API使用」の3ケースのみ。誤って`'use client'`を親レイアウトに付けると全ツリーがクライアント化しバンドル肥大化（実務影響：ビルドサイズ平均35%削減）
- **品質チェックポイント：`next build`後の`.next/analyze/`でバンドル可視化、100KB超のchunkを検知**：`@next/bundle-analyzer`を全案件のCI必須項目化。特に`framer-motion`・`gsap`・大きな`lucide-react`一括importが原因の肥大化を検出。単一アイコンは`import { X } from 'lucide-react/icons/x'`のtree-shake対応を強制（実務影響：JSバンドル-40%）
- **効率化テクニック：Cursor / GitHub Copilot Workspaceで「設計書 → コード自動生成」のスカフォールディング**：naoの設計書（Mermaid階層図付き）をCursor Composerに投入すると、Layoutからセクションコンポーネントまでの骨格が3分で生成される。手作業でボイラープレート書く時間が半分以下に、実装本体（ロジック・状態管理）に集中できる
- **Mia連携の小ヒント：`data-testid`属性を全主要コンポーネントに埋め込み、Playwright差分検出の対象を明示**：`<section data-testid="hero">`のように全セクションのルートに`data-testid`を必須で付与し、MiaのVisual Regressionテストが特定エリアだけを検証できる形にする。誤検出が減り、フォントヒンティング差等のノイズも局所化可能に
