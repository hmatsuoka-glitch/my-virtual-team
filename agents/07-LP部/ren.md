...
### 2026-08-08
- **効率化テクニック：Server Component / Client Component 判定を Nao の理由コード表（nao 2026-08-08参照）に沿って機械化し、`use client` 冒頭宣言の要否を PR チェックで自動検証**：CC 化はバンドルサイズ肥大化の主因なので、`grep -l "use client" app/**/*.tsx` の結果と設計書の CC 一覧を diff し、差異があればCIで失敗させる。誤ってCC化されたコンポーネント（interaction/formState/browserAPIどれにも該当しない）は SC に戻す修正パッチを自動提案。First Load JS の削減に直結。
- **品質チェックポイント：アニメーションは transform/opacity のみを使い、`width/height/top/left/margin` を transition したらCIで検出して差し戻し**：レイアウトプロパティのtransitionは60fps維持不能でINP悪化の主因。回避策：`grep -rE "transition.*\b(width|height|top|left|margin|padding)\b" app/` を PR 前フックで実行し、ヒット箇所を「transform/opacity で書き直せないか」の指摘コメントとして自動投函。GPU accelerated properties以外のトランジションを実装レベルで根絶する。INP同格化（Kaito 2026-08-03参照）に対応。
- **失敗パターン：`next/image` の `sizes` 属性を書かず全端末で最大解像度画像を配信し、SP端末で不要な高解像度データを転送してLCPが悪化**：Retina対応と `srcset` は `next/image` が自動生成するが `sizes` の宣言がなければ全ビューポート同一サイズ扱いで無駄な帯域消費。回避策：Hero/カード等の画像は `sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"` のようにビューポート別サイズを明示。DevTools Network で SP 3G スロットリング時の画像転送量を計測し、100KB超なら sizes 未指定と判定して差し戻す運用に。
- **用語再確認：「PPR (Partial Prerendering)」「ISR (Incremental Static Regeneration)」「SSG」「SSR」「CSR」の5レンダリング戦略の使い分け**：SSG=ビルド時静的生成、ISR=定期再生成、SSR=リクエスト毎、CSR=クライアント側、PPR=静的シェル＋動的ホールを同ページで混在（Nao 2026-08-08参照）。採用LPは「Hero/実績=SSG／お知らせ=ISR60s／フォーム送信=Server Action」の混在が最適。設計書のキャッシュ境界（Nao 2026-08-03参照）に従って各ファイルに `export const revalidate` `export const dynamic` を明記し、判断の再検証コストをゼロ化。
- **連携ヒント：Mia の差し戻しレポート（mia 2026-08-08参照）を受け取ったら、`修正パッチ案（Tailwindクラス変更またはCSS差分）` を検証せずマージせず、必ず「差分箇所のBefore/After computedStyle」を再取得して原因層（token誤設定/Tailwind arbitrary誤用/CSS カスケード優先度）を特定してから修正**：症状対症でクラスを追加するとカスケード衝突で別箇所が壊れる。saki（LP修正）と連携し、修正差分レポートに「原因層コード」を付けてMia再QAへ回すことで、差し戻し2周目以降のリバウンドを防ぐ。
