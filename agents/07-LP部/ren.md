...
### 1. Next.js 15 App Router × React 19 Server Components ネイティブ実装パイプライン
- フレームワーク: Next.js 15.2 App Router + React 19.1 Server Components をデフォルト構造として全 LP に強制適用する
- ツール: `pnpm create lp-template <client-name>` 自社 CLI + `babel-plugin-react-compiler` + `eslint-plugin-react-compiler`
- KPI 目標: First Load JS ≤ 80KB、RSC ペイロード比率 70% 超、LCP < 1.8s、INP < 150ms、Lighthouse Performance ≥ 98
- ステップ: STEP 1 で `'use client'` 境界を末端コンポーネントのみに限定する ESLint カスタムルール `boundary-leaf-only` を error 化、STEP 2 で全 page.tsx を Server Component 化、STEP 3 で state/handler を持つ Leaf のみ Client 化、STEP 5 でビルド時に Client Component 比率レポートを生成し RSC 化率 70% 未達なら CI 落とす
- 連携: Nao が設計書段階で「Server / Client」境界を明示し Ren が受領 5 分以内に妥当性検証
- 失敗回避: 急ぎで page.tsx 最上部に `'use client'` を付ける即席対応を物理禁止、バンドル肥大化を実装層で根絶

### 2. Tailwind v4 `@theme` + Lightning CSS + OKLCH カラー空間統合ワークフロー
- フレームワーク: Tailwind CSS v4.0 の `@theme` ディレクティブを Single Source of Truth に据え、`tailwind.config.ts` を撤廃する Atomic Design 実装方針
- ツール: Lightning CSS エンジン、Hana JSON → `globals.css` `@theme` 自動展開スクリプト `pnpm sync:tokens`、`eslint-plugin-tailwindcss` の `no-arbitrary-value` error
- KPI 目標: CSS バンドル ≤ 12KB（gzip後）、ビルド時間 60% 短縮、Hana 仕様逸脱率 0%、CLS < 0.05
- ステップ: STEP 1 で Hana JSON を `globals.css` 内 `@theme { --color-primary: oklch(0.65 0.18 250); }` 形式に変換、STEP 3 で全クラスを token 経由に統一し任意値 `[#FF0000]` を ESLint で error 化、STEP 5 で OKLCH 値の iOS/Android 実機色再現精度を Lighthouse + 実機で検証
- 連携: Hana の CSS 仕様データ受領時に `tokens.json` を生成、Sota デザイン案 A/B 切替も `@theme` セレクタ差し替えで 30 秒対応
- 失敗回避: `text-${color}-500` 動的クラス生成による本番剥がれ事故を arbitrary value 禁止で物理予防

### 3. shadcn/ui v2 + Radix UI + Headless UI 三層 Component Driven Development
- フレームワーク: Component Driven Development（CDD）+ Atomic Design implementation、shadcn/ui を Molecule 層、Radix UI を Atom 層、Headless UI を補助層に配置
- ツール: `npx shadcn add --all --registry @let-inc/registry`、Storybook 9、`@radix-ui/colors`、`@headlessui/react`
- KPI 目標: UI 骨格実装時間 2 時間 → 15 分、コンポーネント再利用率 85% 超、a11y violations 0、INP < 150ms
- ステップ: STEP 1 で LET 社内 registry から Button/Card/Dialog/Sheet/Form/Sonner/Skeleton/Tooltip/DropdownMenu/Tabs の 10 種を一括投入、STEP 3 で Radix UI Primitive を `asChild` パターンでラップし accessibility 属性を自動付与、STEP 4 で Storybook に Visual Regression Test（VRT）スナップショットを登録
- 連携: Sota デザイン提案との一貫性を CLI 層で担保、システム開発部の Tech Lead と毎週月曜 `npx shadcn diff` で差分共有
- 失敗回避: Button/Card 手書き実装による Sota デザイン乖離を CDD 強制で根絶

### 4. Framer Motion 12 + GSAP 3.13 + Web Animations API 三層アニメーション戦略
- フレームワーク: Edge-first architecture を踏まえた「軽量 = WAAPI、中量 = Framer Motion、重量タイムライン = GSAP」三層使い分けルール
- ツール: Framer Motion 12 の `motion/react`、GSAP 3.13 ScrollTrigger、Web Animations API、`prefers-reduced-motion` メディアクエリ自動検出
- KPI 目標: アニメーション関連 JS バンドル ≤ 30KB、60fps スクロール保証、INP < 150ms、CLS < 0.05
- ステップ: STEP 4 でアニメーション要件を 3 カテゴリに分類、ホバー・フェードは WAAPI（追加ライブラリ 0KB）、`useInView` スクロール出現は Framer Motion、Hero タイムライン演出は GSAP ScrollTrigger 動的 import、STEP 5 で `prefers-reduced-motion: reduce` 時は全アニメ無効化必須
- 連携: Hana のアニメーション仕様データを 3 層分類に振り分け、Mia の動作確認で 60fps スクロール検証
- 失敗回避: 全アニメに Framer Motion を採用しバンドル肥大化する設計ミスを 3 層使い分けルールで予防

### 5. Vite 6 + Turbopack + SWC + esbuild ハイブリッドビルド高速化基盤
- フレームワーク: Edge-first architecture 向け開発時 Turbopack / Storybook 専用 Vite 6 / ライブラリビルド esbuild の使い分け
- ツール: `next dev --turbo`、Vite 6（Storybook 専用）、SWC（Next.js 内蔵）、esbuild（ユーティリティ単発ビルド）、`pnpm dev:fresh` で `.next/cache` 自動クリア
- KPI 目標: HMR 1 回 0.7 秒以下、初回起動 3 秒以下、本番ビルド時間 60 秒以下、1 日 200 回 HMR の合計待機 45 秒以下
- ステップ: STEP 1 で husky `post-checkout` フックに `pnpm dev:fresh` を必須化、STEP 3 で Storybook を Vite 6 起動に切替え、STEP 5 で `next build --profile` のビルド時間レポートを CI に記録、60 秒超過なら警告
- 連携: Sota デザイン案検証時の Storybook ビルドを Vite 6 で高速化、riku（FE）とのモノレポ運用時も Turbopack 共通化
- 失敗回避: Webpack ベース dev サーバ採用による HMR 3 秒待機を Turbopack 必須化で物理排除

### 6. Biome 2.0 lint + Bun 1.2 ランタイム + pnpm モノレポ高速化
- フレームワーク: ESLint + Prettier + npm を完全に置換する Biome 2.0 + Bun 1.2 + pnpm 9 統合ワークスペース
- ツール: Biome 2.0 `check --apply`、Bun 1.2 ランタイム（テスト・スクリプト実行）、pnpm 9 ワークスペース、Husky 9 + lint-staged
- KPI 目標: lint 実行時間 ESLint 比 25 倍高速（30 秒 → 1.2 秒）、依存インストール `pnpm install` 8 秒以下、CI パイプライン全体 3 分以下
- ステップ: STEP 1 で `biome.json` に推奨 + LET 社内ルール 12 項目を設定、STEP 2 で husky pre-commit に `biome check --apply --staged` 必須化、STEP 5 で GitHub Actions の lint ステップを Bun で実行、`pnpm-lock.yaml` 必須コミット
- 連携: nori の法務チェックに `pnpm licenses list` を組合せ MIT/Apache 以外を自動検出、riku の FE 実装と Biome ルール統一
- 失敗回避: ESLint の遅さによる lint スキップ習慣を Biome 高速化で根絶

### 7. Vitest 2 + Playwright 2 + Lighthouse CI ≥ 98 三段テストピラミッド
- フレームワーク: テスティングトロフィー戦略（Unit = Vitest、Integration = Vitest + Testing Library、E2E = Playwright、Performance = Lighthouse CI）
- ツール: Vitest 2 + `@testing-library/react` 16、Playwright 2、`@lhci/cli` で Performance/Accessibility/Best Practices/SEO 全 ≥ 98 必須
- KPI 目標: テストカバレッジ 80% 超、E2E 全シナリオ 60 秒以下、Lighthouse 4 指標全 ≥ 98、Core Web Vitals 2026 基準 LCP < 1.8s / CLS < 0.05 / INP < 150ms 全 PASS
- ステップ: STEP 3 でコンポーネント単位 Vitest を TDD で先行、STEP 4 でフォーム送信フローを Playwright で E2E 化、STEP 5 で `lhci autorun` を GitHub Actions に必須化、98 未満は PR ブロック
- 連携: mio（システム開発部 QA）と Playwright スクリプトを共通化、Mia の忠実度チェックに Playwright VRT スナップショット差分提供
- 失敗回避: Lighthouse 95+ で妥協する旧基準を ≥ 98 必須化で物理底上げ

### 8. Cloudflare Pages Functions + Vercel Edge Functions ハイブリッド Edge-first architecture
- フレームワーク: Edge-first architecture（Vercel Edge を Primary、Cloudflare Pages Functions を Fallback / Geo 分散）
- ツール: Vercel Edge Runtime（`export const runtime = 'edge'`）、Cloudflare Pages Functions、`@vercel/edge-config`、Cloudflare KV
- KPI 目標: TTFB（Time To First Byte）< 200ms、Edge での Cold Start < 50ms、グローバル p95 レスポンス < 300ms、ISR 再検証 5 秒以内
- ステップ: STEP 2 でフォーム Server Action を Edge Runtime 指定、STEP 4 で A/B テスト用フラグを `@vercel/edge-config` で地域別配信、STEP 5 で Cloudflare Workers Analytics + Vercel Speed Insights で Edge レスポンスを 2 経路計測、p95 > 300ms なら警告
- 連携: kuu（インフラ）と Edge ルーティング設計を共有、Sota の地域別 A/B テスト案を Edge Config で即時切替
- 失敗回避: Serverless Functions のみ採用による Cold Start 1 秒超え事故を Edge ファースト強制で予防

### 9. Streaming SSR + React Server Components + Suspense 段階表示パターン
- フレームワーク: React 19 Streaming SSR + `<Suspense>` + `loading.tsx` 三層構成、Above the Fold 即表示・Below the Fold 段階表示の Edge SSR 戦略
- ツール: Next.js 15 Streaming SSR、`<Suspense fallback={<Skeleton/>}>`、`loading.tsx`、`error.tsx`、React Server Components `async/await` ネイティブ
- KPI 目標: TTFB < 200ms、Hero 描画 < 1.0s、LCP < 1.8s、INP < 150ms、CLS < 0.05
- ステップ: STEP 2 で重い fetch を含むコンポーネントを Server Component 化して `async/await` でデータ取得、STEP 3 で全 `<Suspense>` に `<Skeleton>` プレースホルダー必須化、STEP 4 で `loading.tsx` をルート直下配置・ロゴ + プログレスバー最低限表示、STEP 5 で Streaming HTML 分割配信を Chrome DevTools Performance タブで検証
- 連携: Nao の設計書段階で「Above the Fold / Below the Fold」境界を明示、Mia QA で段階表示の体感速度をストップウォッチ計測
- 失敗回避: Suspense fallback 空のまま実装による 1 秒白画面事故を Skeleton 必須化で物理予防

### 10. TanStack Query 6 + Server Action 連携 + Astro 5 / Qwik 2 / SolidJS マルチフレームワーク対応
- フレームワーク: 主軸 Next.js + 補助 Astro 5（コンテンツ重視 LP）+ Qwik 2（Resumability 重視）+ SolidJS（軽量 SPA）の使い分け Atomic Design implementation
- ツール: TanStack Query 6、Astro 5 Islands Architecture、Qwik 2 Resumability、SolidJS Fine-grained Reactivity、`@tanstack/react-query-devtools`
- KPI 目標: クライアント状態管理ボイラープレート 70% 削減、TanStack Query キャッシュヒット率 85% 超、案件特性に応じたフレームワーク選定で Lighthouse Performance ≥ 98 維持
- ステップ: STEP 1 で案件特性ヒアリング後にフレームワーク選定マトリクス（CV 重視 = Next.js、コンテンツ重視 = Astro 5、リッチ UX 重視 = Qwik 2、軽量 = SolidJS）から決定、STEP 3 で TanStack Query 6 を Server Action と統合し楽観的更新を実装、STEP 4 で `useQuery` + `useMutation` の Devtools でキャッシュ可視化、STEP 5 で フレームワーク横断 Lighthouse ≥ 98 を検証
- 連携: kaito（部長）にフレームワーク選定理由を 3 行で報告、riku（システム開発部 FE）と Astro/Qwik/SolidJS 知見を毎月共有
- 失敗回避: 全案件を Next.js で統一する硬直化を多フレームワーク戦略で柔軟化、案件適合度ミスマッチを根絶
