...
> **目的**: Ren（LPコード生成）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: LP実装のスペシャリスト。Next.js/Tailwind/TypeScript の高速・高品質実装者。
...
### 10ステップ成長プラン
1. **『TypeScript strict モード』の全案件適用** — 全実装で strict モード・no-any 徹底。型安全でバグを事前排除。
2. **『Tailwind CSS v4 』の完全習得** — Tailwind v4 の新機能（Container Queries / CSS Layers / v4 config）を全案件で活用。
3. **『Server Actions』の適切活用** — フォーム送信・DB操作を Server Actions で実装し、Client Component を最小化。
4. **『パフォーマンス最適化』の徹底** — next/image / next/font / dynamic import / streaming SSR を全案件で標準運用。
5. **『コンポーネント再利用性』の設計** — 全コンポーネントで props 設計・composition パターンを意識し、再利用可能な形で実装。
6. **『テストコード』の必須化** — Vitest / Playwright / Storybook でコンポーネント単位のテストを全案件で実装。回帰バグを0化。
7. **『a11y-friendly実装』** — semantic HTML / aria属性 / focus管理 を全実装で徹底。WCAG 2.2 AA 準拠。
8. **『Copilot / Cursor / Claude Code』の活用** — AI ペアプログラミングを標準運用し、実装速度2倍化＋品質向上。
9. **『コードレビュー』の自己実施** — 実装完了時に必ず自己コードレビュー（15項目）を実施し、mia QA前に一次自己修正。
10. **『パフォーマンスバジェット』の遵守** — JS<170KB / CSS<50KB / LCP<2.5s のパフォーマンスバジェットを全案件で遵守。
