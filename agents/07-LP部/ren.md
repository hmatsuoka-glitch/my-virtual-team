...
### 2026-09-10
- **[新スキル] 「Next.js 15 PPR (Partial Prerendering) + React 19 use() + Server Components」の3セット標準採用**：Static Shell（ヘッダー・Hero・フッター）を CDN からエッジで即返し、動的部分（問い合わせフォーム・お知らせ）だけをストリーミング。従来1LPあたり LCP 1.8秒→0.6秒（67%短縮）、Kaito 9/10 の CWV assertion（LCP < 1.2秒）を Ren 実装側で担保。`export const experimental_ppr = true` を LP テンプレートの既定に
- **[新ナレッジ] 「React Server Components + Server Actions での Cookie 同意ゲート」ネイティブ実装**：GTM/CMP プラグインを使わず、Server Action で同意状態を Cookie に書き込み、GA4/Meta Pixel の script 読込を Server Component 側で条件分岐。3rd party CMP（OneTrust 等）の重い JS を排除し、初回 Load JS を平均 60KB 削減。Mia の Cookie 拒否 QA（Mia 2026-09-09参照）で確実に「拒否 → 発火0件」を保証
- **[効率化ルール] 「shadcn/ui + Tailwind 4 + tokens.json」の3層で新規実装を Copy-Paste から Generate へ**：Iro/Hana の tokens.json（W3C Design Tokens 準拠・9/10参照）を Tailwind 4 の `@theme` に流し込み、shadcn/ui コンポーネントをその token で自動スタイル。従来 Kotone/Iro の指示を毎回手書きしていた実装が「token 更新→再ビルド」で自動反映。新規セクション実装が平均45分→10分（78%削減）
- **[失敗パターン→回避策] 「LLM 生成コードの `useEffect` 過多で INP 劣化」**：Copilot/Cursor が生成する React コードは平気で `useEffect` を3個以上重ねて、依存配列の変化で余分な再レンダリングを引き起こす。回避策は生成コードを必ず「`useEffect` 数 ≤ 1／コンポーネント」の ESLint ルール（`react-hooks/exhaustive-deps` + custom rule）に通し、超過時は Server Component や `useMemo` への書き換えを CI で強制。Mia の INP 200ms 通過を実装側で担保
- **[連携強化] Mia との「Playwright Test Runner での Storybook スナップショット共有」**：Nao の設計書（Nao 9/10参照）で定義された Component API を Storybook で実装確認し、その Storybook を Mia の Playwright テストの baseline として共有。Ren は実装後 Storybook を更新するだけ、Mia は Storybook を screenshot して比較。従来 Mia が本番 URL から都度撮影していた工程が Storybook 参照で完了、QA サイクル時間が短縮
- **[品質チェック] Sora QA 提出前セルフチェック2項目**：①「Next.js Bundle Analyzer で First Load JS が 130KB 以下、Lighthouse Performance 95以上、INP 実測 200ms 以下の3閾値を通過しているか」（機械化された Performance Budget）、②「Server Action / API Route のランタイム指定（`runtime = 'nodejs' | 'edge'`）が明示され、ローカルで対象ランタイムでの実行確認が済んでいるか」。両方 GO で Sora へ
