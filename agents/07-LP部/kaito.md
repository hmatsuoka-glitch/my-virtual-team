...
### 2026-08-08
- **効率化テクニック：`vercel build`＋`--prebuilt` 運用（2026-08-03参照）に加え、CI側で `--skip-build` プレビュー作成→Slackに Preview URL 自動投函する Github Actions を Kaito 起点で追加**：Ren の実装完了PR時点で Preview URL がクライアントMTG前に自動生成されており、STEP 4 Mia のQAより前に「クライアント目視確認→修正指示」の1周を回せる。緊急修正リードタイムがローカル vercel build 依存から CI 依存へ移り、Kaito が手動で叩く工程を消せる。
- **品質チェックポイント：STEP 5 デプロイ後の Lighthouse assertion に「LCP 2.5s / INP 200ms / CLS 0.1」の3指標をゲートで並列必須化**：INP同格化（2026-08-03参照）に対応し、`lighthouserc.json` の assert セクションに `categories:performance` だけでなく `audits: largest-contentful-paint, interaction-to-next-paint, cumulative-layout-shift` を明示閾値付きで宣言。1指標でも fail なら本番昇格を阻止し、STEP 5 の「PC/SP動作確認」を数値ゲート付き自動判定へ格上げする。Rolling Releases（2026-07-27参照）の10%配信中に実測INPが閾値超なら100%配信をブロックする運用と接続。
- **失敗パターン：複製LPを Vercel Project 単位で乱立させ、環境変数（フォーム送信先・reCAPTCHA・アナリティクスID）を Project ごとに手入力してタイポ事故が月1件発生**：クライアント7社×複数LPで Project 数が20超になった時点で環境変数の一元管理が破綻する。回避策：Vercel Team の Environment Variable Groups（または Turborepo monorepo 集約、2026-08-03参照）へ移行し、Project 側は Group 参照のみにする。STEP 5 のデプロイ前チェックに「環境変数の Group 継承」を必須項目として追加し、Project 直書きを禁止する。
- **トレンド察知：Next.js 15.3+ Turbopack stable（2026-07-27参照）に加え、Vercel の `Fluid Compute` によるサーバレス関数のコールドスタート削減が LP のフォーム送信500系事故を構造的に減らす**：フォーム API Route の初回応答レイテンシが従来の Serverless から大幅短縮され、SNS流入バーストの序盤リード取りこぼしが減る。STEP 5 のデプロイ設定チェックに「Fluid Compute 有効化」の項目を追加し、フォーム/検索APIを持つLPでは既定オンとする。既存Project の一括切替スクリプトも Kuu と連携して準備する。
- **連携ヒント：Sora への引き継ぎレポート（成果物）に「Deployment ID＋直前デプロイ ID＋ロールバック手順1行」を必須欄として追加**：Instant Rollback（2026-08-03参照）の10秒切戻しが SNS 流入バースト時に生きるが、当番エージェント（HARU/Sora）が Deployment ID を知らないと切戻しコマンドが即時に打てない。納品レポートに「vercel alias set &lt;prev-deployment-id&gt; &lt;domain&gt;」の1行を含めておくと、深夜/休日の緊急時に代表本人がGUI操作不要でロールバック実行可能になる。
