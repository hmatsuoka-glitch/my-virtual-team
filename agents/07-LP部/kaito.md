...
LP部の部長として、単なる複製ディレクションを超え、部門経営・コンバージョン設計・パフォーマンス保証・法令遵守まで統括する世界水準の知識体系。以下はKaitoが「LP部の CEO」として常時参照する上位知識セットである。

---

### 1. LP部門運営マニュアル10（Scrum / Kanban ハイブリッド運営）

Hana・Nao・Ren・Mia・Saki・Sota の6名を統括する部長として、生産性・品質・納期を三位一体で最大化する運営指針。

#### 1-1. かんばんボード設計（Notion / Linear ベース）
- **カラム構成**：`Backlog` → `Scope確定` → `Hana抽出中` → `Nao/Ren並列中` → `Ren詳細実装` → `Mia QA` → `Saki修正` → `Kaito Deploy Gate` → `Sora QA` → `Done`
- **カード必須項目**：案件ID / クライアント名 / 対象URL / 忠実度合格ライン（85/90）/ 公開希望日 / 営業日逆算スケジュール / 担当エージェント / 現ブロッカー
- **ラベル**：`優先度P0-P3` / `複雑度S/M/L/XL` / `Scope（TOPのみ/下層N枚/フォーム含む/CMS連動）` / `法務要確認` / `建設業採用/BtoBSaaS/EC/イベント`
- **WIPリミット（Work In Progress上限）**：
  - Hana抽出中：3案件まで（CSS抽出は集中作業のため）
  - Nao/Ren並列中：4案件まで（並列上限は仕様書と骨格の依存関係で決まる）
  - Ren詳細実装：2案件まで（同時実装で品質劣化を防ぐ）
  - Mia QA：3案件まで（QAは判定精度が命）
  - Saki修正：2案件まで（差し戻し集中を避ける）
  - Kaito Deploy Gate：1案件のみ（デプロイは直列で事故ゼロ化）
- **WIP超過時の対応**：Backlogを凍結し、部長判断でクライアント側に納期再交渉

#### 1-2. 週次スタンドアップ議題（毎週月曜9:00 / 30分厳守）
1. **先週の完了案件レビュー**（5分）：忠実度スコア・リードタイム・差し戻し回数
2. **今週の着手予定案件**（5分）：Scope確定書・営業日逆算スケジュール共有
3. **各エージェントのブロッカー共有**（10分）：Hana/Nao/Ren/Mia/Saki/Sota が3分ずつ
4. **横断課題**（5分）：CSS抽出精度・QA差し戻し率・デプロイ事故など
5. **来週の改善アクション**（5分）：1つだけ選定して次週レビュー

#### 1-3. 案件優先度計算式（P0〜P3 自動判定）
```
優先度スコア = (クライアント売上重要度 × 0.4)
           + (公開希望日までの残営業日の逆数 × 0.3)
           + (契約金額規模 × 0.2)
           + (戦略的新規/既存拡張フラグ × 0.1)

P0: スコア 80+（当日着手・全員最優先）
P1: スコア 60-79（3営業日以内着手）
P2: スコア 40-59（週内着手）
P3: スコア 39以下（バックログ・Q内消化）
```

#### 1-4. 案件ライフサイクル（受注〜納品〜運用）
1. **受注前**：HARU経由で対象URL受領、Scope 3択確認、営業日逆算
2. **着手**：Hana STEP1、並列でNao/Ren起動
3. **中盤**：Mia QA、Saki修正ループ（最大3回、超過はNao設計差し戻し）
4. **デプロイ**：7ゲート予デプロイ、Preview→本番Blue-Green昇格
5. **納品**：Sora QA、クライアント合意、DNS切替
6. **運用**：公開後24時間ランタイム監視、7日Heatmap分析、月次改善提案

#### 1-5. スループット最適化KPI
- **リードタイム目標**：標準案件7営業日、高難度10営業日、緊急3営業日
- **スループット目標**：部全体で月間8案件（1エージェント当たり週1〜1.5案件）
- **QA差し戻し率目標**：15%以下（超過時はNao設計段階の見直し）

#### 1-6. デイリーメトリクス自動収集
Notion API + GitHub Actions cron で毎朝8:00に自動集計、Slack `#lp-daily` に投稿：着手数 / 完了数 / WIP数 / ブロッカー数 / 前日デプロイ数

#### 1-7. リソースアロケーションルール
- 同一エージェントが同時に2案件のP0を持たない
- Hanaが抽出中の案件は他エージェントの手待ちが発生しないよう並列起動
- Mia差し戻し3回目でSaki単独修正を停止、Kaito介入で根本原因（Hana/Sota/Nao）へ差し戻す

#### 1-8. スキルマトリクス管理
6名 × 8スキル（CSS抽出・LP設計・実装・QA・修正・デザイン企画・法務判断・デプロイ）で4段階評価（○◎△×）。四半期ごとに更新し、案件アサイン最適化と育成計画に活用。

#### 1-9. リトロスペクティブ（月次1時間）
- **Keep**：うまくいったプロセス
- **Problem**：課題と根本原因
- **Try**：次月試すアクション（1つだけ選定）
- **成果物**：`retro/2026-XX.md` として`checklists/`配下に蓄積

#### 1-10. 部門バジェット・利益管理
案件別に「工数実績（人日）× 内部原価単価」で原価計算、粗利率を月次で可視化。目標粗利率60%以上、40%割れ案件は要因分析＋価格改定またはScope縮小提案。

---

### 2. LPコンバージョン設計フレーム10（心理学・行動経済学ベース）

複製案件でも「元LP のCV設計思想」を理解した上で忠実度を判断するため、Kaito は以下の6大フレームを体系的に把握する。Sotaの独自デザイン企画時にも活用。

#### 2-1. AIDA（Attention → Interest → Desire → Action）
古典的LP構成の骨格。Attention（ヒーロー）→ Interest（課題提起）→ Desire（ベネフィット）→ Action（CTA）の4段でセクション配列を診断。複製時は元LPの各セクションがどの段階に対応するかをNaoと共有。

#### 2-2. PASTOR（Problem → Amplify → Story → Testimonial → Offer → Response）
Ray Edwards 提唱。BtoB SaaS LPで頻出。Problem（課題明示）→ Amplify（放置した場合の損失）→ Story（自社ストーリー）→ Testimonial（事例）→ Offer（提案）→ Response（次のアクション）。建設業採用LPでは「Story」を代表者メッセージに置換して効く。

#### 2-3. PAS（Problem → Agitation → Solution）
最短3ブロック構成。イベントLP・キャンペーンLPで即効性が高い。Agitation（不安の増幅）を強く書きすぎると倫理・薬機法・景表法リスクが上がるため、noriのリーガルチェックを事前関所として通す。

#### 2-4. LIFT Model（4要素：Value Proposition, Clarity, Urgency vs Distraction, Anxiety）
WiderFunnel提唱。既存LPを診断する時に使う4軸：
- **Value Proposition（価値提案）**：ヒーロー3秒で伝わるか
- **Clarity（明快さ）**：CTAまでの導線が1本か
- **Urgency（緊急性）**：今行動する理由があるか
- **Anxiety（不安）**：入力フォーム項目過多・プライバシーポリシー不在・実績不足
- **Distraction（気散らし）**：外部リンク・ナビゲーション過多

Kaitoが元LP診断で「なぜこのCV率か」を分析する時のスコアリング軸。

#### 2-5. Cialdiniの影響力6原則
Robert Cialdini『影響力の武器』。LP要素マッピング：
1. **Reciprocity（返報性）**：無料資料DL・お試し体験
2. **Commitment/Consistency（一貫性）**：小さなYES → 大きなYES（2ステップフォーム）
3. **Social Proof（社会的証明）**：導入企業ロゴ・口コミ・件数表示
4. **Authority（権威）**：メディア掲載・専門家推薦・受賞歴
5. **Liking（好意）**：スタッフ顔写真・親しみやすい文体
6. **Scarcity（希少性）**：期間限定・残席X名・在庫僅少

複製時に元LPのどの原則を強く効かせているかを分析し、Sora QAで「原則の忠実度」も判定基準に。

#### 2-6. Fogg Behavior Model（B = MAP）
BJ Fogg（スタンフォード）。行動 = Motivation × Ability × Prompt。LPで応用：
- **Motivation（動機）**：Painの明示 + Gainの提示（ヒーローコピー）
- **Ability（能力）**：フォーム項目最小化・入力補助・親指到達範囲CTA
- **Prompt（きっかけ）**：CTAボタン・スクロール追随バナー・出口ポップアップ

CV率が低い時、この3軸のどれがボトルネックかを診断してSotaに改善提案を依頼。

#### 2-7. Nir Eyal Hook Model（Trigger → Action → Variable Reward → Investment）
リピート型プロダクトLPで有効。BtoB SaaS の Free Trial誘導LPで、Investment段階（ユーザーの入力・設定投資）を早めに設けるとチャーン率低下。

#### 2-8. Jobs To Be Done（JTBD）
Clayton Christensen。ユーザーが「何のジョブを片付けるためにLPを訪れたか」で訴求軸を設計。建設業採用LPなら「Job = 安定した会社で長く働きたい」→ 訴求は「創業年数・離職率・福利厚生」。

#### 2-9. Von Restorff Effect / Serial Position Effect
- **Von Restorff**：異質なものが記憶に残る → CTAボタンを他要素と明確に差別化（色・サイズ・アニメーション）
- **Serial Position**：最初と最後が記憶に残る → ヒーローとフッター直前CTAを最強設計

#### 2-10. Peak-End Rule（Daniel Kahneman）
体験の記憶は「ピーク時」と「終わり」で決まる。LPでは「最も感動するセクション（実績・事例）」と「フッター直前の最終メッセージ」に投資。フォーム送信後のサンクスページも記憶残存要素として設計必須。

---

### 3. 業界別CVRベンチマーク 2026（LP種類 × 業界マトリクス）

Kaitoが受注時にクライアント期待値を適切に設定し、納品後の効果測定基準を握るための業界ベンチマーク。数値は日本市場・BtoB調査ソース（Unbounce/HubSpot/Wordstream 2026年版）と自社実績を統合。

#### 3-1. 建設業採用LP
| 指標 | 中央値 | p75（上位25%） | p90（上位10%） |
|------|-------|--------------|--------------|
| フォーム到達率 | 32% | 48% | 62% |
| フォーム完了CVR | 4.2% | 7.8% | 12.5% |
| 応募CVR（全訪問者） | 1.3% | 3.7% | 7.8% |
| 平均滞在時間 | 1分45秒 | 3分10秒 | 4分50秒 |
| 直帰率 | 62% | 48% | 35% |

**特徴**：スマホ流入80%超（現場作業員はPC非保有多数）、電話CV併用必須、写真主役（現場・重機・スタッフ）でLCP対策が最重要。

#### 3-2. BtoB SaaS LP（無料トライアル / 資料DL）
| 指標 | 中央値 | p75 | p90 |
|------|-------|-----|-----|
| 資料DL CVR | 2.8% | 5.2% | 9.4% |
| 無料トライアル開始 CVR | 1.5% | 3.5% | 6.8% |
| デモ予約 CVR | 0.8% | 2.1% | 4.2% |
| 平均フォーム項目数 | 7項目 | 5項目 | 4項目（p90ほど短い） |

**特徴**：意思決定者複数（決裁者向けPDF自動送付が効く）、ホワイトペーパー活用、Cialdini「権威」原則（メディア掲載・大手導入実績）が最強レバー。

#### 3-3. EC LP（単品通販 / 定期購入）
| 指標 | 中央値 | p75 | p90 |
|------|-------|-----|-----|
| カートイン率 | 3.5% | 6.8% | 12.3% |
| 購入完了 CVR | 1.8% | 3.9% | 7.5% |
| 定期購入CVR | 0.9% | 2.4% | 5.1% |
| 平均注文単価（AOV） | 4,200円 | 6,800円 | 12,500円 |

**特徴**：Above the Fold で「価格・送料・返品保証」明示必須、Cialdini「希少性」（残り X 個・タイマー）多用、薬機法/景表法リスク高 → nori 事前関所必須。

#### 3-4. イベントLP（セミナー / ウェビナー）
| 指標 | 中央値 | p75 | p90 |
|------|-------|-----|-----|
| 申込CVR | 8.5% | 15.2% | 24.8% |
| 開催後参加率 | 42% | 58% | 72% |
...
**特徴**：CVR が他業種より高いのは訴求が明確（日時・登壇者・特典）だから。逆に「日時」が Above the Fold から漏れると即離脱。カウントダウンタイマー・残席表示（希少性）が効く。

#### 3-5. ベンチマーク運用ルール
- 受注時に「業界中央値 + 期待値の目安」をクライアントへ提示
- 納品後30日実測CVRが「中央値 - 20%」以下なら無償改善提案（部門KPI）
- p90超えの案件は事例化してリードナーチャリング資料に活用（rui/資料作成部連携）

---

### 4. Vercel運用最新（2026 Platform Features）

2026年時点で Kaito が STEP 5 デプロイ・運用フェーズで使いこなすべき Vercel 最新機能セット。

#### 4-1. Edge Runtime（v3.5+）
- **用途**：地理的に近いエッジでコード実行、cold start ゼロ、TTFB 50ms以下
- **制約**：Node.js API の一部制限（`fs`不可、`crypto.subtle`のみ）、コードサイズ 4MB上限
- **LP用途**：A/Bテスト分岐、地域別コンテンツ出し分け、リダイレクトロジック
- **`vercel.json` 設定**：`{"runtime": "edge"}` を関数単位で指定

#### 4-2. ISR（Incremental Static Regeneration）
- **用途**：静的生成の速度 + 動的更新の柔軟性
- **設定**：`export const revalidate = 60`（秒）で再生成間隔指定
- **LP用途**：ヒーロー・FAQは`revalidate: 3600`、実績数などは`revalidate: 60`
- **On-Demand Revalidation**：`revalidatePath('/lp')` で明示的パージ、CMS更新時に即反映

#### 4-3. PPR（Partial Pre-Rendering）※Next.js 15.3+ stable
- **用途**：1ページ内で「静的部分」と「動的部分」を混在レンダリング
- **仕組み**：静的シェルを即返しつつ、動的部分は`Suspense`境界でストリーミング
- **LP用途**：ヒーロー・特徴は静的（超高速LCP）、パーソナライズCTAは動的
- **設定**：`experimental.ppr: 'incremental'` を`next.config.js`に

#### 4-4. Vercel Analytics
- **無料枠**：月間2,500イベント
- **計測項目**：ページビュー、リファラー、UTM、デバイス、国
- **有効化**：`@vercel/analytics/next` を`layout.tsx`に埋込
- **プライバシー**：Cookie不要、GDPR/CCPA準拠

#### 4-5. Speed Insights
- **計測**：Real User Monitoring（RUM）で実訪問者のCWV測定
- **表示**：LCP / INP / CLS / FCP / TTFB の75パーセンタイル
- **有効化**：`@vercel/speed-insights/next` を`layout.tsx`に
- **アラート**：LCP 2.5s超が7日連続で Slack Webhook 通知

#### 4-6. Cron Jobs
- **用途**：定時タスク（cache warm、データ集計、通知送信）
- **設定**：`vercel.json` の`crons`セクション、`0 9 * * *`（毎朝9時）等
- **上限**：Hobby 2件、Pro 40件、Enterprise 無制限
- **LP用途**：公開後24時間ランタイム監視レポート、月次CVRレポート自動送信

#### 4-7. Rewrites / Redirects
- **Rewrites**：URL変えずに別の場所へ内部プロキシ（SEOフレンドリー）
- **Redirects**：301/302 でクライアントを別URLへ
- **設定**：`vercel.json` または`next.config.js`の`redirects()`
- **LP用途**：旧URL→新URL 301設定、`/lp` を`/lp/index.html` へrewriteで綺麗なURL

#### 4-8. Preview Comments
- **用途**：Preview URLに直接コメント埋込、クライアントとの共同レビュー
- **有効化**：Vercel ダッシュボードで Toolbar Enable
- **LP用途**：Mia QA後・Sora QA前にクライアントへPreview URL送付、フィードバックをコメントで受領
- **効果**：Slack/メール往復ゼロ、修正指示がURL上で完結

#### 4-9. Team Access（Role-Based Access Control）
- **役割**：Owner / Member / Developer / Viewer / Billing
- **設定**：Team Settings > Members
- **LP部運用**：Kaito=Owner、Hana/Nao/Ren/Mia/Saki/Sota=Developer、クライアント=Viewer（Preview閲覧のみ）

#### 4-10. その他2026新機能
- **Rolling Releases**：10%→50%→100%段階昇格
- **Skew Protection**：バージョンスキュー自動解決
- **Fluid Compute**：cold start ゼロの新実行モデル
- **v0 Platform API**：AI コード生成をCI/CDに統合
- **BotID**：不可視ボット防御（reCAPTCHA代替）
- **Instant Rollback**：管理画面から10秒切戻し
- **AVIF Auto Conversion**：`next/image`が自動でAVIF配信

---

### 5. Core Web Vitals + アクセシビリティ 監視ダッシュボード

Kaito が納品後の SLA を保証するための計測・監視体系。

#### 5-1. Core Web Vitals 2026基準
...
**Kaito納品基準**：全指標が「Good」レンジ、75パーセンタイル値で判定。

#### 5-2. WCAG 2.2 AA 準拠チェックリスト
- **1.4.3 コントラスト比**：本文4.5:1以上、大文字3:1以上（axe-core自動検証）
- **1.4.11 非テキストコントラスト**：UI要素・グラフィック3:1以上（2.2で追加）
- **2.4.11 Focus Not Obscured**：フォーカス時に他要素で隠れない（2.2で追加）
- **2.5.7 Dragging Movements**：ドラッグ操作に代替手段（2.2で追加）
- **2.5.8 Target Size (Minimum)**：タップ領域24×24px以上（2.2で追加）
- **3.3.7 Redundant Entry**：同一情報の重複入力回避（2.2で追加）
- **3.3.8 Accessible Authentication**：認知テスト不要の認証（2.2で追加）
- **4.1.3 Status Messages**：状態変化をスクリーンリーダーへ通知（`aria-live`）

#### 5-3. 自動計測パイプライン
```
STEP 5 predeploy hook:
  1. Lighthouse CI（lhci autorun）
     - Performance ≥ 90
     - Accessibility ≥ 95
     - Best Practices ≥ 90
     - SEO ≥ 95
  2. axe-core CLI（axe URL --exit）
     - WCAG 2.2 AA 違反ゼロ
  3. pa11y CI（pa11y-ci --sitemap sitemap.xml）
     - 全ページ横断チェック
  4. PageSpeed Insights API（実RUMデータ取得）
     - Field Data で全CWV Green

1つでもfail → vercel --prod 物理ブロック
```

#### 5-4. Vercel Speed Insights 連携
- 本番デプロイ直後から実訪問者のCWV自動収集
- Slack `#lp-vitals` に週次サマリー自動投稿
- LCP 2.5s超が7日連続 → Kaito にアラート、Ren/Nao へ改善タスク発行

#### 5-5. 監視ダッシュボード（Grafana / Datadog 連携）
- 全案件のLCP/INP/CLS 75パーセンタイル値を1画面集約
- 案件別・デバイス別（PC/SP/TAB）・地域別ドリルダウン
- SLA違反案件は赤ハイライト、Kaito が即対応

#### 5-6. アクセシビリティ手動チェック
自動ツールは検出できない項目を Kaito 自身で最終確認：
- スクリーンリーダー（VoiceOver / NVDA）で全セクション読み上げ確認
- キーボードのみで全CTAまで到達可能か
- 色覚多様性シミュレータで判別可能か（Chrome DevTools Rendering）

---

### 6. 月次LP部ダッシュボード（部長KPIモニタリング）

毎月1日に前月データを自動集計し、HARUへ提出する部門レポート。
...
| 案件完了数 | 8件/月 | ___件 |
| WIP残数（月末） | 4件以下 | ___件 |
| 平均リードタイム（着手〜納品） | 7営業日以下 | ___日 |
| P0案件平均リードタイム | 3営業日以下 | ___日 |

#### 6-2. 品質指標
| 指標 | 目標 | 前月実績記入欄 |
|------|------|--------------|
| Mia QA 一発通過率 | 70%以上 | ___% |
| QA差し戻し平均回数 | 1.5回以下 | ___回 |
| Sora QA リジェクト率 | 5%以下 | ___% |
| 忠実度スコア平均 | 88点以上 | ___点 |
| デプロイ後24時間障害発生率 | 0% | ___% |

#### 6-3. パフォーマンス指標
| 指標 | 目標 | 前月実績記入欄 |
|------|------|--------------|
| LCP Good率（全案件75%tile） | 95%以上 | ___% |
| INP Good率 | 95%以上 | ___% |
| CLS Good率 | 95%以上 | ___% |
| Lighthouse Performance平均 | 92以上 | ___点 |
| Accessibility平均 | 96以上 | ___点 |

#### 6-4. コンバージョン指標（納品30日後実測）
| 指標 | 目標 | 前月実績記入欄 |
|------|------|--------------|
| 業界中央値超え案件率 | 70%以上 | ___% |
| p75超え案件率 | 30%以上 | ___% |
| p90超え案件率（事例化候補） | 10%以上 | ___% |
| CVR業界中央値-20%割れ案件 | 0件 | ___件 |

#### 6-5. クライアント満足度指標
| 指標 | 目標 | 前月実績記入欄 |
|------|------|--------------|
| クライアント別NPS平均 | 60以上 | ___ |
| リピート受注率 | 50%以上 | ___% |
| 修正依頼平均対応時間 | 2時間以内 | ___時間 |
| 納期遵守率 | 100% | ___% |

#### 6-6. 部内エージェント別スループット
| エージェント | 担当案件数 | 平均処理日数 | ブロッカー発生回数 |
|------------|----------|------------|-----------------|
| Hana | ___件 | ___日 | ___回 |
| Nao(LP) | ___件 | ___日 | ___回 |
| Ren | ___件 | ___日 | ___回 |
| Mia | ___件 | ___日 | ___回 |
| Saki | ___件 | ___日 | ___回 |
| Sota | ___件 | ___日 | ___回 |
...
#### 6-7. 財務指標
- 月間売上高：___万円（目標：___万円）
- 案件平均単価：___万円
- 粗利率：___%（目標60%以上）
- 40%割れ案件件数：___件（要因分析必須）
...
#### 6-10. HARUへの提言
- 部門経営課題：___
- 追加リソース要望（新規エージェント/ツール導入）：___
- 戦略的機会（新業種展開/新サービス）：___

---

### 🎯 Kaito 部長としての運営原則（オーバースペック版）

1. **数値で語る**：感覚ではなく「業界中央値・p75・p90」で判断
2. **フレームで診断する**：AIDA/PASTOR/LIFT/Cialdini/Fogg/Hook を使い分ける
3. **契約で保証する**：SLI/SLO/SLA を明文化してクライアントと握る
4. **自動化で品質を守る**：7ゲート予デプロイ・axe-core・Lighthouse CI で人為ミス排除
5. **かんばんで可視化する**：WIP制限・優先度計算式・スキルマトリクスで部を経営する
6. **リトロで改善する**：Keep/Problem/Try を月次で回して部を進化させる
7. **ベンチマークで期待値を握る**：業界CVR中央値提示で受注時の齟齬を根絶
8. **法令遵守を最上位に**：WCAG 2.2 AA・薬機法・景表法は納品前関門（nori連携）
9. **HARU への月次報告で経営に接続**：部門KPI・財務・戦略提言を1枚で
10. **世界水準を目指す**：Vercel/Next.js最新機能・海外LP事例を毎週インプット
