...
### 2026-08-30
- **Vercel v0 + Claude 5 でLP複製フロー全体の状態管理をダッシュボード化**：Hana/Nao/Ren/Miaの各STEP進捗、Preview URL、CWV実測、ダミー送信結果を1画面で監視、Slackへ自動通知。効果例：STEP間の引き継ぎ抜け -100%、統括工数 3時間/案件→30分
- **Vercel Speed Insights + Web Vitals API で公開後48時間の実ユーザーCWV自動レポート**：本番実測データを Claude 5 が要約し、Ryota経由でクライアントへ「速さの実績」を可視化。効果例：HARU の次回営業提案での実績データ活用率 +100%、Saki への劣化検知アラート -平均リードタイム 3日→当日
- **BrowserStack Live + Playwright で LINE/Instagram/Gmail アプリ内ブラウザ実機確認を自動化**：主要WebView 5種で追従CTAの表示・タップ可否・OGP レンダリングを毎回自動チェック。効果例：アプリ内ブラウザ崩れの本番事故 月2件→0件、Kaitoの本番実機確認時間 -85%
- **Vercel Deployment Protection + Password Protection で Preview URL のクライアント誤共有を構造的に防止**：Preview は全て認証必須、本番昇格時のみ認証解除、alias 付替の履歴を Notion に自動記録。効果例：Preview URL の誤公開・誤引用事故ゼロ、担当者の共有事故起因の緊急対応 -100%
- **Slow 4G + Mobile プリセット + WebPageTest CI で LCP/INP/CLS を PR ごとに自動計測**：閾値未達だとマージブロック、Ren/Sakiに即差し戻し。効果例：本番実測CWVで「良好（Good）」判定の維持率 78%→98%、実ユーザー体感の底上げ
- **クライアント実環境到達性チェックを「情シス許可依頼テンプレ」とセットで運用**：*.vercel.app / 独自ドメイン / CDN オリジンの許可依頼をクライアント情シス向け1枚に定型化、初回納品時に自動送付。効果例：「うちだけ開けない」問い合わせ 月3件→0件、独自ドメイン切替の判断が受注時点で完結
- **Sora への引き継ぎ表を Notion データベース化し、Mia/Kaito/クライアント実環境の3区分責任分界を自動生成**：Mia レポート・Kaito ゲート結果・実環境到達性確認結果を CSV から自動生成、Sora は未記入セルだけを見れば良い状態に。効果例：Sora の最終判定時間 45分→8分、責任分界の曖昧化ゼロ
- **BMAD-METHOD 準拠でシステム開発部 Ao との「フォーム送信API責任分界」を Terraform IaC 化**：env 登録・API キー管理・エンドポイント設定を Terraform 経由で宣言的に管理、Kaito と Ao の役割が構造的に明示。効果例：env 設定漏れによる本番500エラー 月2件→0件、Ao との「相手がやると思った」ゼロ継続
