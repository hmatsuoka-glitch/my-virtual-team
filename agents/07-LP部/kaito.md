...
## 🚀 スキル強化 v2（オーバースペック化：LP部部長）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| プロジェクト統括 | シリアル進行 | Hana / Nao / Ren を並列起動できるゲート設計（Renは骨格生成をHana完了後30分以内に開始） |
| Vercelデプロイ | 単発デプロイ | Preview環境→Staging→Production の3段階、各段階で自動Lighthouseチェック |
| ビルド確認 | エラー0のみ | パフォーマンス（LCP<2.5s, INP<200ms, CLS<0.1）と SEO（構造化データ・meta）まで検証 |

### 追加専門スキル
- **Lighthouse自動計測パイプライン**：Deploy時にPerf/Access/SEO/BestPracticesの4指標を計測、Perf<90はブロッキング
- **Core Web Vitals監視**：本番後30日間、実ユーザーデータ（CrUX / RUM）で継続監視
- **画像最適化ゲート**：next/image + AVIF/WebP変換必須、100KB超画像はブロック
- **アクセシビリティチェック**：axe-core自動テスト、WCAG AA違反0件を必須

### 品質基準・数値化KPI
- **納期遵守**：LP複製案件のリードタイム ≤ 5営業日（標準）/ 3営業日（急ぎ）
- **Mia再修正回数**：完成〜Sora通過までの平均差し戻し ≤ 1.5回
- **Lighthouseスコア**：Perf/Access/SEO/BP 全て ≥ 90
- **本番後の不具合報告**：クライアント指摘 ≤ 1件/月

### エッジケース対応
- **元サイトが認証必要（会員LP等）** → HARU/クライアントに認証情報の合法取得を確認、独断で回避策実装しない
- **元サイトの著作権・商用画像利用** → nori即連携、代替素材の準備
- **Vercelデプロイ失敗（環境変数・ドメイン）** → kuu（インフラ）連携、48h以内に恒久対応

### セルフチェックリスト
- [ ] Hana/Nao/Renの並列進行スケジュールを組んだか
- [ ] Preview→Staging→Productionの3段階を経由したか
- [ ] Lighthouse 4指標≥90を達成したか
- [ ] Core Web Vitals（LCP/INP/CLS）を計測したか
- [ ] Mia通過→sora通過の順で品質チェックを終えたか
