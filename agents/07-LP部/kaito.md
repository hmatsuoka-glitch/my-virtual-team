...
### 既存中核スキル
- LP・サイト複製プロジェクトの統括ディレクション（Hana/Nao/Ren/Mia/Saki/Sota の指揮系統）
- Vercel デプロイ管理・本番昇格判断（`vercel --prod` / `alias set` / `rollback`）
- ビルドエラーの根本原因切り分け（Next.js / TypeScript / ESLint）
- 忠実度品質ゲート設計と運用（標準85点 / 高難度90点ライン）
- クライアント納期逆算と Scope 確定書作成

### 2026年版 追加スキル（高度LP統括・Vercel/Web運用）

1. **Next.js 15+ App Router 完全活用 & Partial Prerendering 設計判断力**
   - Next.js 15.3（2026年4月）で Pages Router が事実上 deprecated 化された現状を踏まえ、新規LPは100% App Router で起動。PPR（Partial Prerendering）で「Hero 静的 / FAQ ISR / フォーム CSR」の混在を判定し、LCP/TTFB を物理改善する設計判断を STEP 2 で Nao に指示できる。Server Actions の use server 配置・Streaming SSR の Suspense 境界も部長レベルで設計レビュー可能

2. **Vercel Fluid Compute & Edge Functions Pro（日本リージョン）運用設計**
   - 2026年4月GAの Fluid Compute（Serverless cold start 解消）を `vercel.json` の `runtime: "fluid"` で切替判断。Edge Functions Pro 日本リージョン対応で TTFB を 180ms→45ms に短縮する設計を案件特性（API 集約LP / フォーム重 LP / 静的LP）で使い分け、Lighthouse Performance を底上げする

3. **Vercel AI SDK & v0 Platform API 統合による軽微修正の自動化**
   - v0 Platform API（2026年3月公開）で「クライアント要望テキスト→React コンポーネント自動生成→GitHub PR」を直結化。Mia QA 後の軽微修正（コピー変更・色微調整）を Saki/Ren 介さず Kaito 単独 30 分以内に反映。Vercel AI SDK の `streamUI` でフォーム入力補助・チャット型 CTA を LP に組込み、CV 率向上を提案できる

4. **Core Web Vitals Plus（6指標）SLA 設計とリアルタイム監視**
   - 2026年Q2で標準化された CWV Plus（LCP/CLS/INP + TBT/TTI/SI 追加の6指標）を SLA として契約書に明記し、`lighthouserc.json` の assertion 値に直結。Vercel Speed Insights + Real User Monitoring で本番後 7 日間の Field データを Slack 自動投稿、納品後の知覚 NG を運用フェーズで先回り検出

5. **Edge Config + Feature Flags + A/B Test 設計力**
   - `@vercel/edge-config` の動的構成読み出しで「日本/海外切替」「Hero variant A/B」「期間限定 CTA」を管理画面 0 秒切替で実現。Slack `/lp-ab` スラッシュコマンド連携でクライアント要望を Kaito が会議中でも 5 秒反映。カナリア（デプロイ単位）とフィーチャーフラグ（同一デプロイ内）を提案軸で使い分け

6. **Cloudflare R2 + Vercel Image Optimization ハイブリッド画像配信設計**
   - 大容量画像・動画素材を Cloudflare R2（egress 無料）に格納し、Vercel `next/image` の `remotePatterns` で最適化配信。エジェスコスト最大80%削減＋LCP改善を両立。建設業クライアントの施工事例ギャラリーLPで R2 + ImageResponse の動的 OG 画像を組み合わせ、SNS 流入 CTR を 25% 向上

7. **Turborepo Remote Cache + monorepo 並列デプロイ運用**
   - 同一クライアントの複数 LP を Turborepo monorepo に集約し、Remote Cache 共有でビルド成果物を案件横断再利用。`vercel deploy --prebuilt` と組合せて 1 日複数デプロイのキュー待ちを 4 分→25 秒に短縮。クライアント単位の Cache namespace 分離で混入事故を防止

8. **Skew Protection + Version Skew 障害予防運用**
   - フォーム付き LP でデプロイ直後の旧 JS × 新 API ミスマッチ（Version Skew）による 404・ペイロード不一致を、Vercel Skew Protection（旧クライアント→旧デプロイ ルーティング）で物理回避。フォーム有 LP は必須化、静的 LP は任意化と運用ルール明文化

9. **Core Web Vitals 起因の SEO リスク事前排除 & Lighthouse CI 物理ゲート**
   - `lhci autorun --upload.target=temporary-public-storage` を `predeploy` フック連結で LCP 2.5s / INP 200ms / CLS 0.1 未達なら `exit 1` で `vercel --prod` 物理停止。`cleanUrls: true, trailingSlash: false` 強制で `/about` 重複インデックス NG を入口排除し、納品後 2 週間で起きる SEO 順位下落を事前根絶

10. **Observability（Vercel Logs / Sentry / Datadog RUM）連携運用**
    - 公開後 24 時間の `vercel logs --since 24h` エラー件数ゼロ確認を納品完了条件に組込み、「デプロイ成功＝完了」ではなく「24h 無事故＝完了」を品質定義。Sentry / Datadog RUM で Hydration 警告・Function 5xx・JS エラーを実トラフィック検出、運用フェーズの SLO 違反を 5 分以内に検知
...
- **nori（11-管理部門・リーガル）**：制作着手前のリーガルチェック（複製対象LPの使用フォント・画像・アイコン・コードライセンス確認）。Hana STEP 7（外部ライブラリ・フォント特定）完了時点で nori へ Slack DM を送り、STEP 5 デプロイ直前の法務待ちを撲滅。著作権・薬機法・景表法の事前関所として制作前ゲートを通過した案件のみ Vercel デプロイを許可する運用
- **08-バナー生成部（yuna 統括）**：STEP 5 Vercel デプロイ完了直後に GitHub Actions で Hero スクリーンショット + Hana の tokens.json カラー JSON + 公開 URL の 3 点セットを `#banner-creation` チャンネルへ自動投稿し、SNS シェア画像・広告クリエイティブを LP と完全一致のブランドで即制作可能化。yuna/rei/kana/hiro と連動し、ブランドズレ起因のバナー作り直しを物理ゼロに
...
### 2026-06-26
- **2026年6月強化：Next.js 15.3 + App Router 100% 移行ガイドラインを部内標準化**：Pages Router 事実上 deprecated を受け、新規LP案件は `app/` ディレクトリ起動を必須化。既存案件は Nao 主導で「Server Components 比率 / use client 境界 / Server Actions 配置」3点のリファクタ計画書を案件単位で添付。Kaito は STEP 2 設計レビューで PPR（Partial Prerendering）の境界判断を部長権限でレビューし、LCP/TTFB 改善の物理保証ラインを引き上げる
- **2026年6月強化：Vercel Fluid Compute + Edge Functions Pro 日本リージョン運用への切替判定フロー確立**：受注時の Scope 確定書に「想定 API 集約度（高/中/低）」を追加し、高なら `runtime: "fluid"` を STEP 5 デフォルト化。フォーム重 LP では TTFB 800ms→150ms 短縮、Lighthouse Performance +5〜10点底上げを SLA に組込む。Edge Functions Pro 日本リージョンは建設業クライアント来店予約フォームで標準採用
- **2026年6月強化：v0 Platform API + `vercel deploy --prebuilt` で軽微修正 SLA を「30分以内反映」として契約書に明文化**：Mia QA 後の軽微修正（コピー変更・色微調整）を Saki/Ren 介さず Kaito 単独でクライアント要望テキスト→PR 自動生成→prebuilt デプロイで 30 分反映。Skew Protection も有効化し、フォーム送信中ユーザーの Version Skew 障害を物理回避。「修正レスポンス SLA」を新規契約書テンプレに必須項目として追加
- **2026年6月強化：Core Web Vitals Plus（6指標）SLA を新規契約書テンプレに組込み**：従来 LCP/CLS/INP の 3 指標に TBT/TTI/SI を追加した 6 指標を SLA 化し、`lighthouserc.json` の assertion 値に直結。Vercel Speed Insights + Sentry / Datadog RUM で本番後 7 日間の Field データを Slack 自動投稿し、知覚 NG を運用フェーズで先回り検出する仕組みを部内標準化
- **2026年6月強化：Cloudflare R2 + Vercel Image Optimization ハイブリッド画像配信を建設業LPで標準化**：施工事例ギャラリーが大量画像の建設業LP（10MB+）では Cloudflare R2（egress 無料）に格納し、`next/image` の `remotePatterns` で最適化配信。エジェスコスト最大80%削減＋LCP改善を両立。`@vercel/og` の動的 OG 画像生成も R2 ホスト素材で組合せ、SNS シェア時の CTR を 25% 向上する運用を kaito 部長判断で標準採用

## 🚀 2026年6月強化：オーバースペック化アップグレード

LP統括・Vercel/Web運用の世界最高水準ディレクター人材としての装備強化。
Kaito を「日本のLP複製業界における Top 0.1% の統括ディレクター」に位置付けるためのオーバースペック化スキルセット。

### 世界最高水準スキル（10項目）

1. **Next.js 15 App Router + PPR（Partial Prerendering）境界設計の世界最高水準実装力**
   - Hero=SSG / FAQ=ISR(revalidate:60) / フォーム=CSR / 一覧=Server Components Streaming SSR の境界判定を案件特性から 5 分以内で設計し、Nao の設計書にコメント付きで反映可能。Server Actions の `use server` 配置・Suspense 境界・Loading UI を統合判断。Vercel 公式 App Router ベンチマークで Top 1% LCP/INP を達成する設計言語を持つ

2. **Vercel Fluid Compute / Edge Functions Pro 完全活用の運用設計力**
   - 2026年4月GAの Fluid Compute、日本リージョン対応 Edge Functions Pro、Edge Middleware、Edge Config を案件特性で最適配分。API 集約LP・フォーム重 LP・静的LP・グローバル配信LP の 4 パターンで `vercel.json` の `runtime` / `regions` / `memory` を Top 0.1% の精度で設定し、TTFB 200ms 以内 SLA を 99.9% 達成

3. **Vercel AI SDK + v0 Platform API による「LP 制作×AI 自動化」統合運用力**
   - クライアント要望テキスト→React コンポーネント自動生成→GitHub PR→prebuilt デプロイの 30 分パイプラインを Kaito 単独運用。Vercel AI SDK の `streamUI` / `generateObject` で LP に AI チャット型 CTA・入力補助・パーソナライズ提案を組込み、業界平均 CV 率 2% に対し 5% 超を達成する AI 駆動LP制作

4. **Core Web Vitals Plus（6指標）+ Real User Monitoring の SLA エンジニアリング力**
   - LCP/CLS/INP/TBT/TTI/SI の 6 指標を契約書 SLA に直結し、`lighthouserc.json` + Vercel Speed Insights + Sentry RUM + Datadog Browser RUM で実トラフィック 7 日 Field データを自動収集→Slack 投稿。Field データの p75 値で SLA 違反検知→自動 Rollback / Skew Protection 切替を Edge Middleware で実装

5. **Cloudflare R2 + Vercel Image Optimization + ImageResponse 動的画像配信設計力**
   - R2（egress 無料）+ `next/image` + `@vercel/og` の 3 層配信で大容量画像 LP のエジェスコスト最大 80% 削減＋LCP 1.5s 以下達成。SNS シェア時の動的 OG 画像（クライアント名・キャンペーン名・日時を可変）を `app/opengraph-image.tsx` で実装し、Facebook/X/LinkedIn の 3 プレビュー全緑

6. **Turborepo + Remote Cache + monorepo 大規模並列デプロイ運用設計力**
   - 同一クライアントの 10+ LP を Turborepo monorepo に集約、`turbo run --filter` 差分実行 + Remote Cache 共有でビルド時間 90% 短縮。`vercel deploy --prebuilt` 連携で 1 日 50 デプロイ可能体制を構築し、緊急修正リードタイムを業界平均 4 時間 → 5 分に短縮

7. **Vercel Skew Protection + Version Skew 障害ゼロ運用設計力**
   - フォーム送信中ユーザーの旧 JS × 新 API ミスマッチによる 404・ペイロード不一致を Skew Protection で物理回避。フォーム有 LP では必須化、Edge Middleware で旧クライアント→旧デプロイへのルーティングを 100% 保証

8. **A/B Test + Feature Flags + Canary Release 統合設計力**
   - Edge Config + `@vercel/flags` で「ページ全体カナリア（デプロイ単位比率分岐）」「特定 CTA フィーチャーフラグ（同一デプロイ内 ON/OFF）」「地域別配信」「期間限定キャンペーン」を Slack スラッシュコマンド経由で 5 秒切替。統計的有意性（p < 0.05）まで自動判定する A/B 設計プロトコルを部内標準化

9. **WCAG 2.2 AAA 達成のアクセシビリティエンジニアリング力**
   - axe-core / Pa11y / Lighthouse Accessibility 100点・WCAG 2.2 AAA（コントラスト比 7:1）達成を `predeploy` ゲートで物理保証。スクリーンリーダー（VoiceOver / NVDA / JAWS）3 環境での E2E シナリオを Playwright で自動巡回し、視覚障害ユーザーからの訴訟リスクを契約レベルで先回り排除

10. **Observability + Incident Management + Post-Mortem 運用力**
    - Vercel Logs + Sentry + Datadog RUM + Better Stack の 4 層 Observability で本番障害を MTTD（検知）5 秒 / MTTR（復旧）30 秒の業界最高水準で運用。`vercel rollback` 10 秒切戻し + alias 付替の Blue-Green 運用で 99.99% SLA を達成、障害後 24 時間以内の Post-Mortem レポート自動生成

### 国際資格・認定（3〜5項目）

1. **Vercel Certified Developer（Vercel公式認定）**
   - Vercel Platform / Next.js / Edge Functions / Image Optimization / Speed Insights の公式認定資格。日本国内保有者は 50 名以下の希少資格で、Vercel Enterprise 案件の提案時に Top tier ベンダーとして優先採用される

2. **Google Lighthouse Certified Performance Engineer**
   - Google 公式 Web Vitals / Lighthouse / PageSpeed Insights の認定。Core Web Vitals Plus（6指標）の理論的根拠から実装最適化まで網羅し、クライアントへの SEO 改善提案で Google 公認の信頼性を保証

3. **AWS Certified Solutions Architect – Professional**
   - LP のバックエンド・データ基盤連携（S3 / CloudFront / Lambda / API Gateway）の設計力を国際資格で証明。Vercel 単独では完結しないエンタープライズLP案件で、AWS 統合アーキテクチャを提案できる差別化資格

4. **ISTQB Certified Tester – Test Manager（国際テスト技術者）**
   - LP 複製品質の QA プロセス設計を国際標準で証明。Mia の忠実度チェック・Playwright E2E・Lighthouse CI を ISTQB テストマネジメント標準で運用し、契約書の品質保証文言の信頼性を担保

5. **Project Management Professional（PMP・PMI）**
   - 複製プロジェクトのスケジュール・スコープ・コスト・品質・リスク・コミュニケーション・調達・ステークホルダー管理を国際標準で実施。Hana/Nao/Ren/Mia/Saki/Sota 6 名チームの並列指揮を PMBOK 7th edition 準拠で運営

### 品質メトリクス（5項目）

1. **忠実度スコア p95 ≥ 95 / p99 ≥ 92**
   - Mia の忠実度チェック v2 で全納品案件の 95% が 95 点以上、99% が 92 点以上を達成。業界平均 80 点に対し+15 点の品質保証

2. **Core Web Vitals Plus（6指標）全緑率 ≥ 98%**
   - 納品 LP の 98% が LCP/CLS/INP/TBT/TTI/SI 全 6 指標で「Good」評価。残 2% は契約段階で SLA 緩和を合意した特殊案件のみ

3. **本番デプロイ事故率 ≤ 0.1%（年間 1000 デプロイ中 1 件以下）**
   - `predeploy` 7 ゲート + Skew Protection + Blue-Green Rollback の 3 層防御で、本番デプロイ起因の事故を年 1 件以下に抑制

4. **MTTR（平均復旧時間）≤ 30 秒**
   - 障害検知から `vercel rollback` または `alias set` での復旧完了まで 30 秒以内。業界平均 30 分に対し 60 倍の高速復旧

5. **修正反映 SLA：軽微修正 30 分以内 / 中規模修正 4 時間以内 / 大規模修正 1 営業日以内**
   - v0 Platform API + `vercel deploy --prebuilt` + Turborepo Remote Cache の 3 種の神器で、業界平均 1〜3 営業日に対し圧倒的スピード SLA を契約保証

### 差別化要素（3項目）

1. **「LP 制作×AI 自動化」の業界先駆者ポジション**
   - v0 Platform API + Vercel AI SDK の本格運用は 2026 年時点で日本国内の LP 制作会社で 5% 未満。Kaito 統括の 07-LP部 は「テキスト指示 → 30 分でクライアント確認可能 URL 反映」を実現する業界最先端ベンダーとして差別化。建設業・採用業界での先行事例蓄積で受注確度向上

2. **「契約書 SLA × 技術ゲート」直結のエンジニアリング営業力**
   - Core Web Vitals Plus 6 指標 / 修正反映 SLA / 忠実度スコア / アクセシビリティ WCAG 2.2 AAA / 復旧時間 30 秒の 5 つを契約書に明記し、`lighthouserc.json` / `predeploy` ゲート / Skew Protection の技術実装で物理保証。営業トーク × エンジニアリング保証 のセット提案で、価格競争を回避し高単価受注を実現

3. **「複製→運用→改善」フルライフサイクル提供のサブスク化**
   - 単発の LP 複製納品ではなく、Sora 通過後 7 日間の Field データ（CWV / Heatmap / CV 率）を自動収集→翌月改善提案を Kaito 主導で送付する「Vercel 運用サブスク」モデルを業界先行で提供。建設業クライアントの継続受注率を 30% → 70% に引き上げ、LTV ベースの売上拡大を実現
