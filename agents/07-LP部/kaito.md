...
### 2026-09-10
- **[新スキル] 「Vercel Fluid Compute + Skew Protection」を組み合わせた次世代デプロイフロー**：2026年GAとなった Fluid Compute（従量課金型サーバーレス）と Skew Protection（新旧デプロイの同時共存で古いクライアントの JS bundle 参照エラーを防止）を組み合わせ、本番昇格直後の CDN キャッシュ差でユーザーが 404 モジュールに当たる事故を構造的にゼロ化。従来の週次定時デプロイ枠（2026-08-27参照）で発生していたビジーユーザーの一時的な JS エラーが月2〜3件→0件
- **[新ナレッジ] 「Next.js 15 の PPR (Partial Prerendering)」＋「React 19 の use API」による LP 高速化の標準化**：Static Shell を CDN からエッジで即返し、動的部分だけをストリーミングする PPR を LP 複製の標準にすることで、LCP（Largest Contentful Paint）を平均1.8秒→0.6秒（67%短縮）。Mia の CWV assertion（2026-09-01の CI ゲート）に PPR 適用有無を必須項目化し、複製後のパフォーマンスを元サイト以上に押し上げる
- **[効率化ルール] Vercel v0＋Chrome DevTools MCP による「Scope 確認〜設計初稿」の自動化**：受注5分の Scope 確認フォーム（2026-09-01参照）の回答を Vercel v0 に渡すと、複製元URLから設計初稿・骨格コードを自動生成。従来 Hana→Nao→Ren の直列で1.5日かかっていた STEP 1〜3 初稿を、v0 生成→Hana/Nao/Ren の3並列レビュー＋修正で0.5日（67%削減）に短縮
- **[失敗パターン→回避策] 「v0 自動生成コードを検証なしで本番投入し、複製元由来のライブラリ依存や外部フォント参照が残って計測タグ漏洩と同じ経路の事故になる」**：v0 は生成過程で複製元の `<script src>`・`<link href>` を保持することがあり、他社CDNへのリファラ流出が起こる。回避策は v0 生成コードを Hana の CSS 完全抽出フェーズと同等の「外部リソース棚卸し（`grep -rE "https?://" src/`）」に必ず通し、承認外ドメインは exit code で落とす CI ゲートに追加（2026-09-02 の GA4 ID チェックと並列運用）
- **[連携強化] Sora QA との事前接続点として「デプロイ完了レポート自動生成」**：デプロイ完了時に「本番URL・Preview URL・CWV スコア・GA4/GTM 検証・SSL 4パターン確認・画像出所台帳・Deployment Protection状態」の7項目を Notion に自動記録し、Sora QA 開始時にこの1ページを見るだけで判定可能に。従来 Sora が個別に確認していた項目のヒアリング往復が月10件→0件
- **[品質チェック] Sora QA 提出前セルフチェック2項目**：①「PPR 適用済み＋CWV assertion で LCP < 1.2秒・INP < 200ms・CLS < 0.1 の3閾値をすべて通過しているか」、②「本番昇格後のロールバック手順（旧デプロイ ID＋`vercel alias set`）が Notion レポートに明記されているか」。両方 GO で Sora へ
