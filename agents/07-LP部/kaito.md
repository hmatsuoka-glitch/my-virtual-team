...
### 2026-09-05
- **Next.js 15 App Router + React 19 Server Components を新規案件のデフォルトに固定し、Ren 着手前に Kaito が「Server / Client / Server Action」の三層マップを承認する運用へ切替**：`use client` の乱用で Hydration ペイロードが肥大し INP p75 が 300ms を超える案件が増えていたため、境界を Hero・フォーム・計測系だけに絞る三層マップを Nao 設計書に必須項目として追加。Ren はマップ承認後にしか実装着手できないゲートを敷き、INP p75 を案件平均 90ms 短縮する構造改革を係長判断で強制化
- **Turbopack 本番ビルド `next build --turbopack` を全案件 stable 運用に切替し、Vercel Remote Cache と連結**：緊急修正の反映を 4 分 → 40 秒 → 8 秒台の3段短縮まで詰め、週次デプロイ枠1回で最大6案件を束ねて昇格可能に。同一クライアントの職種別・エリア別 LP を1プロジェクトへ束ねる運用（2026-09-01参照）と組合せ、昇格作業とビルド待ちを同時に N 分の1化
- **Vercel Fluid Compute + Edge Config を「動的 API／A/B・地域別切替」の判定表として STEP 3 で確定運用開始**：問い合わせ受付 API を従来 Serverless Function から Fluid に移し、cold start 起因の初回送信タイムアウトを撲滅（TTFB 800ms → 150ms）。地域別・職種別出し分けは Edge Config で管理画面制御化し、クライアント自身が Slack `/lp-ab` から切替可能に。Ren に載せ先判定を丸投げしない
- **v0 Platform API 生成コードの4点関門レビュー（不要 `use client`／`any` 型残存／aria 属性欠落／`next/image` 未使用）を Kaito 承認事項として関門化**：無編集で流すと Lighthouse Accessibility が 78 点まで落ちる事象を検知したため、v0 → PR の間に `eslint-plugin-jsx-a11y` ＋ 自作 rule を挟み、Kaito 承認後にしか merge できない CI ゲートを敷設。生成の速度（30 分修正反映）と Accessibility 95 点維持の両立を確立
- **`web-vitals` v4 で INP／LCP／CLS を GA4 + Speed Insights へ二重送信し、案件横断ダッシュボードで p75 を常時監視。閾値超過で Saki へ自動 issue 起票する連携を実装**：INP 200ms 超過を検知したら pre-fix タグ運用（saki 側）の粒度で受け取り、Kaito のバックログへ「即対応／週次まとめ／次リリース」の3分類で流し込む。「本番でだけ遅い」の検知リードタイムを7日→30分に短縮し、Speed Insights 単体依存を脱却
- **Vercel WAF + BotID を採用 LP フォーム経路に標準組込みし、reCAPTCHA を全案件で撤去**：reCAPTCHA secret 未設定による送信 500 事故（2026-07-01参照）が構造的に消え、スパム率も同時に低下。Ao 提供 API キーの管理項目が1本減り env 台帳もシンプル化。BotID は不可視で UX を損なわず、WAF ルールでレート制限・地域制限・悪性 IP ブロックを同時付与
- **JP LP コンプライアンス関門（特商法／景表法／改正個情法／Cookie 同意 UI 同格ボタン／JIS Q 15001 必須項目）を `predeploy` で静的解析し、nori の事前承認と機械照合する二重関門を構築**：「業界No.1」「効果保証」等の禁忌表現、特商法表記へのフッター遷移リンク、Cookie 同意 UI の「同意する／拒否する」同格配置、プライバシーポリシーの必須7項目の4系統を CI で判定。nori の目視レビューを補完し、人依存の見落としをゼロ化。景表法の措置命令リスクを構造的に排除
- **sitespeed.io を CI に組込み「Mobile Slow4G × iPhone SE」条件の CWV を毎コミット記録、回帰を PR レベルで検知するゲートを敷設**：Speed Insights の本番実測（納品後）だけでなく、開発段階で回帰を止められるようになり、Mia QA 通過後に「本番で INP 悪化」となる後戻りを撲滅。CDN 意図的ウォームアップ（主要 30 パス × 3 地域を Playwright で先行アクセス）と組合せ、クライアント初回体験の TTFB p50 を 150ms 未満に固定して「開いた瞬間の重さ」の第一印象固定化を予防
