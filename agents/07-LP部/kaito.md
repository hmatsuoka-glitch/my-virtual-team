...
### 2026-09-04
- **Vercel Fluid Compute の Active CPU 課金を月次コスト報告の内訳に組み込み、HARU 経由でクライアントへ「アイドル時間は課金されない」根拠を数値で返す**：従来のリクエスト単位課金と違い、DB 書き込み待ち・外部 API レスポンス待ちのアイドル時間は課金対象外で、フォーム送信主体の建設業採用 LP は請求額が実行時間ベースで大きく圧縮される。Speed Insights の CWV 週次レポート（2026-08-13参照）に「Active CPU 時間・関数実行回数・アイドル比率」の3列を追加して HARU へ渡し、値下げ交渉ではなく「同じ機能で運用コストがこれだけ下がった」実績として次回提案（LP 追加・機能拡張）の材料にする
- **Cloudflare Pages / Workers を「月10万PV以下・フォーム動的処理なし」の複製 LP 向け代替として選定表に載せ、Kuu と共同メンテする**：Vercel は INP 最適化・Speed Insights・Rolling Releases（2026-07-27参照）が強い一方、静的表示のみのシンプル建設 LP は Cloudflare Pages の無料枠で足りコストが月0円に落ちる。Kuu（09-システム開発部インフラ）と「フォーム有無／想定 PV／ISR 要否／A/B テスト要否」の4軸で判定表を作り、受注5分の Scope 確認（2026-09-01参照）にコスト上限条件を追加。Vercel を売るのでなく案件特性で選ぶ運用にし、価格で他社へ流れる小規模案件を取りこぼさない
- **Playwright の visual regression を `predeploy` に組み込み、Saki 修正案件で Mia の 1 ピクセル QA を差分ありの時だけに絞る連携**：複製直後のデプロイでスクリーンショットを baseline として保存し、Saki の修正後は CI で差分検出だけを回す。差分ゼロなら Mia を通さず Kaito ゲートへ直行、差分ありの領域だけを Mia へ渡して人の目を差分部分に集中させる。全画面の再検査を毎回回す運用（saki 2026-08-18参照の同日束ねリリース）と噛み合い、修正案件のリードタイムが半減する。baseline は複製元 URL の変更で無効化されるため、Hana の抽出更新時に必ず再撮影するルールも同時に決める
- **Vercel Edge Config でコピー・CTA 文言を再デプロイなしで差し替え、kotone の A/B テストと Saki の tag ベース修正を粒度で分界する**：Edge Config は KV 値の変更が数秒でエッジ全域に反映されるため、Hero キャッチ・CTA 文言・電話番号などの「コード変更を伴わない差し替え」は Saki の pre-fix タグ運用（2026-07-16参照）を経由させない。着手時に「コピー・数値のみ＝Edge Config 差し替え（Kaito が即時実行）／構造・ロジック変更＝Saki の tag ベース修正／デザイン変更＝Hana→Ren 再抽出」の3層分界を案件チャンネルに固定し、コピー最適化のたびに週次デプロイ枠（2026-08-27参照）を消費する事態をなくす
- **Next.js 15 の Partial Prerendering (PPR) を「Hero は静的シェル・フォームはストリーミング」で運用し、Nao の設計時点で `<Suspense>` 境界を握る連携**：従来は「全 SSG で LCP を稼ぐ／全 SSR でフォームを動かす」の二択で、複製 LP はどちらかを犠牲にしていた。PPR で静的シェルを即返してフォーム部分だけ後追い挿入すれば LCP と INP を同時に最適化できるが、境界設計は Ren の実装段階で決めると Nao の設計書と矛盾する。STEP 2 の Nao 設計書に「PPR 静的領域／動的ストリーミング領域」の色分け図を必須項目として追加し、Ren はその境界に沿って `<Suspense>` を配置する。Mia の忠実度チェック（mia 2026-08-16参照の検証マトリクス）にも「静的部分の即時表示・動的部分の遅延挿入時のガタつき」を項目として渡す
