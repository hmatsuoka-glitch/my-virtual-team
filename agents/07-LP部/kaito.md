...
### 2026-09-21
- **2026年最新トレンド：Vercel Fluid Compute + Edge Middleware v3による「LP毎の動的CDN最適化」**：2026年ローンチのFluid Computeで、同一プロジェクト内でも「LPページはEdge Static / フォームはServerless / 管理画面はLambda」と粒度別にインフラを自動振り分け。`vercel.json`の`framework: "next"`＋`functions.runtime`指定で従来のRewritesベース設定より30%コスト削減。7社LP全案件で標準採用へ（実務影響：月額$20→$14に、応答速度は逆に平均120ms→80ms）
- **2026年最新トレンド：Vercel Deployment Protection v2 + Comments Beta連携**：プレビューURLへのGitHub PR連動コメント機能が正式GAとなり、クライアント承認プロセスを`vercel.com`の内蔵コメントUIで完結可能に。従来の「LINEでURL送信→スクショで指摘」フローをやめ、クライアントに`kaito-preview-only`のRead-onlyロールを発行し、プレビュー上に直接ピン留めコメントさせる（実務影響：修正指示の往復が5往復→2往復に、履歴も自動保存）
- **2026年最新トレンド：Core Web Vitals 2026スペック更新（INPが200ms→150ms、CLS 0.1→0.08）**：Google Search Console閾値が2026-Q3から段階的に厳格化。旧LPで合格していたJSアニメが「Poor」判定に落ちるケースが多発。STEP 5の実機確認に`web-vitals@4.x`ライブラリを組み込み、`onINP()`・`onCLS()`の実測値がラボテスト基準を満たすかを本番昇格前に自動チェックする（実務影響：SEO順位低下リスクの事前検知）
- **失敗パターン：Vercel Fluid Computeへの移行時、既存の`api/*`エンドポイントが旧Serverlessランタイム前提でメモリ制限に達し、フォーム送信が突然503を返す** → 回避策：移行前に`vercel dev --debug`で全APIルートの`memory`・`maxDuration`使用実績をログから抽出し、Fluid側の`config.maxDuration: 60`を明示指定。特に日本語メール本文の生成処理はSJIS変換で瞬間的にメモリ増加する（理由：Fluidはコールドスタート改善の代わりにインスタンス共有でメモリ制限が厳しく、7社中1社で発生済み）
- **失敗パターン：クライアントが独自ドメインを`Cloudflare`経由で運用しており、Vercelの`Automatic HTTPS`とCloudflare側の`Full (strict)`が衝突して証明書エラーで一時ダウン** → 回避策：ドメイン紐付けSTEP前に`dig +short NS <domain>`と`whois <domain>`でNS委任先を確認し、Cloudflare配下ならCFのSSL設定を`Full`（strictなし）に事前変更してもらう。設定変更依頼メールの雛形を常設ドキュメントに置く（理由：7社中3社がCF利用、CFの`Full (strict)`はVercel側のSNI証明書と一致せずダウンタイム30分〜数時間の実害）
- **品質チェックポイント：本番昇格前に`vercel inspect <deployment-url> --logs`でビルドログ内の警告レベル全チェック**：`warning`レベルでも`legacy peerDeps`・`deprecated package`・`webpack fallback`は次回build失敗の予兆。特にNext.js 15→16マイグレーション期にありがちな`experimental.appDir`削除警告は要即対応（実務影響：突然のbuild失敗による深夜対応の回避）
- **品質チェックポイント：Vercel Analytics 2026年更新の`Real Experience Score`が90以上か本番昇格後30分以内に確認**：Web Vitalsの実ユーザー計測がRESとして統合表示され、90未満は求職者体験に影響。低ければ即Sakiに改善タスク発注し、Hana側でHero画像フォーマット（AVIF/JPEG XL）の見直しに戻す
- **効率化テクニック：`vercel promote`コマンド + `--yes`フラグの一括昇格スクリプトをGitHub Actionsで管理**：週次デプロイ枠（2026-08-27参照）で7社分を手動実行していたのを、`workflow_dispatch`トリガーで一斉実行に切替。`matrix: [syosei, miyamura, ...]`で並列7ジョブ、失敗した案件だけSlack通知（実務影響：手作業60分→自動15分、ヒューマンエラーゼロ）
- **Sora連携の小ヒント：QA完了報告に「Fluid Compute設定・CWV実測値・RESスコア」を必須3項目として同梱**：Soraが要求する「動作確認済み証跡」に、上記3つのスクショまたはJSONダンプを標準添付。QA差し戻し回数が減り、案件クローズまでの時間が体感で30%短縮
