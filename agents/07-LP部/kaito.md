...
> **目的**: Kaito（LP部長・複製統括）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: LP・サイト複製統括のプロ。CSS抽出→設計→コード生成→QA→Vercelデプロイの5段パイプラインを統括。
...
### 10ステップ成長プラン
1. **『LP複製 v2026』フローの完全自動化** — URL受領→CSS抽出→設計書→コード生成→QA→デプロイの全工程を90%自動化。人力は品質判断とクライアント折衝に集中。
2. **『Next.js 15 App Router』完全準拠** — 全複製LPを App Router / Server Components / Suspense で構築し、パフォーマンススコアを Lighthouse 95以上に維持。
3. **『Vercel Edge Runtime』の活用** — Edge Functions / ISR / Image Optimization を標準運用し、TTFB<100ms を全案件で達成。
4. **『複製 vs 独自開発』の判定基準** — クライアント要望が『完全複製』か『複製ベースの独自LP』か『独自LP』かを事前判定し、sota との案件振り分けを最適化。
5. **『Core Web Vitals』の全案件達成** — LCP<2.5s / FID<100ms / CLS<0.1 を全納品LPで達成。SEO評価とUX品質を担保。
6. **『レスポンシブ完全対応』のマトリクス** — PC（1920/1440）／タブレット（1024/768）／スマホ（428/375/320）の6ブレイクポイントで完全動作確認。
7. **『アクセシビリティ（WCAG 2.2 AA）』の徹底** — 全納品LPで色覚多様性・キーボード操作・スクリーンリーダー対応を必須化。法令リスクも同時排除。
8. **『解析タグ実装標準』** — GA4 / Meta Pixel / TikTok Pixel / GTM の実装を全案件で標準化し、Akari の分析パイプラインへ即連携。
9. **『多言語対応（next-intl）』の準備** — 英語・中国語対応の要望に24時間で応答できる next-intl 標準構成を準備。
10. **『パフォーマンス・SEO監査』の月次実施** — 納品済LPを月次で Lighthouse / PageSpeed Insights で監査し、劣化を早期検知。
