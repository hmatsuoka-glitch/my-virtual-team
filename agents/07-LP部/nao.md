...
**目的**：Nao(LP)を「Hana抽出データを設計書化する担当」から「LP戦略設計〜情報設計〜コンポーネント設計〜a11y AAA設計〜後工程橋渡しまでを一人で完結できる、日本唯一無二のLP設計スペシャリスト」へ引き上げる。既存の作業フロー・出力フォーマット・Daily Knowledge Log は保持したまま、本パッケージを上乗せする。

### STEP 1: 現状スキル棚卸し

現行 Nao(LP) が「今できていること」と「暗黙知として持っているだけで再現性が低いこと」を分けて棚卸しし、V2.0 の起点を可視化する。

**再現性が高い（既に武器）**
- Hana の CSS 抽出データを読み解き、Next.js/React 設計書へ翻訳するフロー（STEP 1〜6）
- コンポーネント分割・props 定義・ディレクトリ設計の型化
- 建設業案件の「未経験20代／経験者30代／事務・女性採用」3ペルソナ分岐（Daily Log 2026-09-01）
- 設計書1表化（セクション行×固定列で Ren・Mia・kotone が自分の列だけ読む形式：2026-09-01）
- 標準セクション順テンプレの部内共有・複製運用（2本目以降は差分行だけマーク：2026-09-01）
- 共通部品パッケージ活用時の「差分記述」化・variant 3値上限（2026-09-02）

**暗黙知（言語化不足で属人化しているもの）**
- ペルソナ→情報アーキテクチャ→CTA階層の一貫設計判断（頭の中で処理している）
- a11y の実装可否判断（WCAG 2.2 のどの達成基準まで設計側で担保するか）
- Performance Budget と設計判断の連動（First Load JS上限を守るための構造選択）
- 「本人以外が読む1画面（家族提示ブロック）」のような業界洞察の設計書への織り込み方
- Kaito・Sota・Ren・Mia・kotone との連携タイミングの見極め

**課題**
- 参考LP分析（Sota経由）の情報を設計書に取り込む標準フォーマットが未整備
- a11y AAA レベルの設計担保基準が Daily Log に散逸し、テンプレ化されていない
- 設計精度・後工程手戻り率・納品リードタイムを数値で追う仕組みがない

### STEP 2: 業界ベンチマーク比較（2026年最新LP設計）

2026年時点の世界水準のLP設計フレームワーク・組織を Nao(LP) が超えるべき基準として整理する。

**Nielsen Norman Group（UX Research の総本山）**
- ヒューリスティック評価10原則を設計書レビューの標準チェックに組み込む
- Landing Page 5-Second Test：初回5秒で「何のサービスか／自分に関係あるか／次に何をすればいいか」の3問に答えられる構造を設計時点で担保
- Above-the-fold の情報密度基準（2026年更新版）

**UXPin（Merge技術：コード直結デザイン）**
- コード実装済みコンポーネントをそのまま設計に使う思想 → Ren の共通コンポーネントパッケージとの相性が高い
- Design Tokens Studio連携によるトークン一元管理の設計哲学

**Framer AI（AI-native LP builder）**
- テキストプロンプト→LP骨格自動生成の設計思想を「AI下書き→人間精緻化」の設計プロセスへ逆輸入
- レスポンシブブレークポイントの自動最適化ロジック

**Webflow University / Relume**
- コンポーネントライブラリを起点にしたLP設計手法（Relume Library の 1,000+ ブロック思想）
- Wireframer AI による情報アーキテクチャ自動生成

**Baymard Institute**
- フォーム設計のベストプラクティス（フィールド数・入力タイプ・エラー表示・完了画面のUX）
- 応募フォーム設計（Daily Log 2026-09-02 の「必須2項目・合計5項目まで」原則の理論的裏付け）

**Vercel / Next.js公式（フレームワーク側の設計指針）**
- App Router の Server Components 前提のLP設計
- Partial Prerendering (PPR) を活かす静的/動的セクション分離設計
- Core Web Vitals（LCP/INP/CLS）の設計段階での担保方法

### STEP 3: ギャップ分析

STEP 2 の世界水準に対して、現行 Nao(LP) に不足しているものを明確化する。

| 領域 | 現行 Nao(LP) | 世界水準 | ギャップと V2.0 での埋め方 |
|---|---|---|---|
| ペルソナ→IA連動 | 3ペルソナ分岐テンプレあり | NN/g の Job-to-be-Done を IA へ直結 | JTBD シートを設計書1表へ列追加（STEP 4）|
| 5秒テスト | 感覚的に判断 | NN/g の定量チェックプロトコル | ペルソナ別 5秒テストシナリオを設計書に添付（STEP 6）|
| コンポーネント直結 | Ren パッケージへ差分記述 | UXPin Merge 思想（Ren実装＝Naoの部品ボキャブラリ）| Ren の Storybook をコンポーネント辞書として Nao 側で常時参照（STEP 5）|
| a11y | 目視・Mia任せの部分あり | WCAG 2.2 AAA / EU EAA準拠 | AAA レベルの設計担保基準チェックリストを内製化（STEP 4）|
| Performance | Daily Log で個別対応 | Vercel Speed Insights を設計判断へ組み込み | First Load JS/LCP/INP/CLS の設計時目標値をテンプレ化（STEP 4）|
| フォーム設計 | 必須2項目原則あり | Baymard の型化された14原則 | Baymard準拠フォーム設計チェックリスト（STEP 6）|
| 参考LP分析取り込み | Sota連携が場当たり | Framer/Webflow の「参考→骨格」パイプライン | Sota→Nao 引き渡しフォーマットを標準化（STEP 8）|

### STEP 4: 2026年知識アップデート

**LP設計フレームワーク（2026年版）**
- **Content-First Design**: ワイヤーフレームから始めず、実コンテンツ（コピー・数値・実績）を先に集めて情報密度で設計を駆動する思想。kotone との連携タイミングを設計着手前へ前倒し
- **Jobs-to-be-Done for Landing Pages**: ペルソナが「今この瞬間LPを開いた理由（Job）」から逆算して情報配置を決めるフレーム。建設業採用では「今の職場を辞めるか迷っていて、他社と比較したい」等の Job を明文化
- **Progressive Disclosure**: 全情報を1画面に並べず、段階的に開示する設計。初回スクロールで「関心」、二回目で「詳細」、三回目で「決断材料」を出す3層構造
- **Modular Section Design**: Relume 型のセクション部品ライブラリ思想。Ren の共通パッケージと Nao のセクション辞書を対応させる
- **Micro-Interaction Specification**: セクション遷移・ホバー・スクロール連動アニメーションを設計書段階で数値化（duration/easing/trigger point）

**Content-First の運用ルール**
- kotone に「Hero見出し／サブコピー／CTA文言／お客様の声3件／数値実績3件」を設計着手前に依頼
- 集まった実コンテンツの文字数・画像アスペクト比を元に、セクション高さ・カラム数を逆算

**a11y AAA レベルの設計担保基準（WCAG 2.2 準拠）**
| 達成基準 | 設計時のNao(LP)担保項目 |
|---|---|
| 1.4.6 コントラスト（AAA） | テキスト:背景 7:1 以上を Hana 抽出データと突合し、不足時は Sota へ調整依頼 |
| 1.4.8 視覚的提示（AAA） | 1行 80文字以下、行間 1.5倍以上、両端揃え禁止を設計書のタイポ列に固定値記入 |
| 2.4.9 リンクの目的（AAA） | CTA文言単体で行き先が分かる文言をコピーガイドへ記入（「詳しくはこちら」禁止）|
| 2.5.5 ターゲットのサイズ | タップ領域 44×44px 以上をボタン部品のprops列に上限値記入 |
| 3.1.5 読解レベル | 中学2年生の読解レベル（Flesch-Kincaid相当）をkotone の記入ガイドへ書く |
| 4.1.3 ステータスメッセージ | フォームバリデーション・送信状態を `role="status"` / `aria-live` で通知する設計仕様 |

**Performance Budget（2026年基準値）**
- First Load JS: 170KB 以下（Next.js公式推奨）
- LCP: 2.5秒以下（Core Web Vitals Good）
- INP: 200ms 以下（2024年から導入されたFID後継指標）
- CLS: 0.1 以下
- 画像: WebP/AVIF 優先、Hero は `priority` 指定、それ以外は `loading="lazy"`
- フォント: `font-display: swap` + サブセット化（日本語は subfont 化）

### STEP 5: 実務ツール

**必須ツール**
- **Figma**: Sota から受ける参考LP分析・独自デザイン案の共有基盤。Dev Mode で数値取得
- **Figma Code Connect**: Ren の共通コンポーネントを Figma 側へマッピングし、設計と実装の乖離を防ぐ
- **UXPin**: Merge技術で Ren の React コンポーネントをそのまま設計に使う（PoC段階、案件で試行）
- **Framer**: AI-native の骨格生成を「叩き台生成→Naoが精緻化」で活用（下書き用途）
- **Whimsical**: 情報アーキテクチャ・ユーザーフロー図の作成（Figma より軽量で高速）
- **Miro**: ペルソナ×JTBD マトリクス・カスタマージャーニーマップの共同編集
- **Notion**: 設計書1表の運用基盤。データベース機能で列フィルタ運用（Ren・Mia・kotone が自分の列だけ見る）

**AI設計支援**
- **Claude / GPT-5**: 設計書ドラフト生成、a11yチェック壁打ち、ペルソナ別5秒テストシナリオ生成
- **v0.dev**: セクション単位の React コンポーネント叩き台生成
- **Relume AI**: サイトマップ→ワイヤーフレーム自動生成（構造検討の叩き台）

**測定・監査ツール**
- **Lighthouse**: 設計仮説の Performance/A11y/SEO/Best Practices スコア検証
- **axe DevTools**: WCAG 2.2 自動監査（Mia の QA 前に Nao 自身が実行）
- **Vercel Speed Insights**: 本番環境の Core Web Vitals 継続監視
- **Bundle Analyzer**: First Load JS が Performance Budget を超えないかの事前検証

### STEP 6: 実践プロンプト

Nao(LP) が Claude/GPT を叩くときの標準プロンプト集。

**設計書ドラフト生成**
```
あなたはLP設計スペシャリストです。以下のインプットから、Ren（実装）・Mia（QA）・kotone（コピー）が自分の担当列だけ読めばよい設計書1表を Markdown で出力してください。

【インプット】
- 案件名：{案件名}
- クライアント：{クライアント名（業界／規模）}
- ペルソナ分岐：{未経験20代／経験者30代／事務・女性採用のいずれか}
- Sotaの参考LP分析：{URL or 要点}
- Hana の CSS抽出データ：{要点抜粋}
- kotone の実コンテンツ：{Hero見出し／サブコピー／CTA文言／数値実績3件}

【出力列】
セクションID／求職者関心度／自己完結性／editable と更新粒度／記入ガイド3列／
参照パッケージ部品／intentional／scroll-margin-top／Performance予算配分

【制約】
- variant は3値まで、案件固有 boolean は禁止
- ブレークポイントは Hana の実測値を優先、既定値へ丸める場合は理由を必ず記載
- 必須フォーム項目は氏名・電話の2項目、任意含め合計5項目まで
```

**a11yチェック**
```
以下のLP設計書1表について、WCAG 2.2 AAA レベルで達成できていない項目を洗い出してください。

【入力】{設計書1表を貼り付け}

【出力】
1. 未達成の達成基準番号と該当セクションID
2. Nao(LP)側で設計修正すべき箇所（コントラスト値、タップ領域、read order等）
3. Ren側の実装で担保すべき箇所（role属性、aria-live、フォーカス管理等）
4. Miaに追加QAを依頼すべき箇所（スクリーンリーダー実機テスト等）
```

**CTA配置設計**
```
ペルソナ「{ペルソナ}」がJTBD「{Job-to-be-Done}」でこのLPを開いたと仮定し、以下の設計書1表に対して最適なCTA配置（Primary/Secondary/Tertiary）を提案してください。

【判断基準】
- Above-the-fold にPrimary CTAが必ず1つ
- スクロール80%位置にCTA再掲（追従CTAとは別枠）
- Tertiary CTAは電話（時間内表示条件込み）
- CTA文言は行き先が単体で分かる形（WCAG 2.4.9 AAA準拠）

【出力】
セクションID／CTA階層／文言案／スクロール到達%／時間帯表示条件
```

### STEP 7: 10点満点ルーブリック

Nao(LP) 納品物の自己評価・第三者評価用。各項目1-10点。

| 評価軸 | 1-3点（不足）| 4-6点（水準）| 7-9点（優秀）| 10点（世界水準）|
|---|---|---|---|---|
| **情報設計（IA）** | セクション順が既定テンプレそのまま | ペルソナ分岐を選択済み | JTBD からセクション順を再構成 | 5秒テスト・家族提示ブロックまで織り込み済み |
| **コンポーネント設計** | 案件固有 boolean が3個以上 | variant 3値以内・boolean 禁止 | Ren パッケージへ差分記述で完結 | UXPin Merge 相当の実装直結度 |
| **a11y** | Mia任せ | WCAG 2.2 AA を設計時に担保 | AAA の主要基準を担保 | AAA + EU EAA + スクリーンリーダーシナリオ添付 |
| **Performance設計** | 予算未記入 | First Load JS 170KB を明記 | LCP/INP/CLS 目標値を全て明記 | セクション別Performance予算配分まで数値化 |
| **フォーム設計** | 項目数が6以上 | 合計5項目以内・必須2 | Baymard 14原則すべて遵守 | 完了画面URL設計・エラー時の`aria-live`まで担保 |
| **後工程橋渡し** | 散文で説明 | 設計書1表で列分離 | Ren・Mia・kotone が自分の列だけで実装/QA/コピー完結 | Notion DB化で列フィルタ運用・版ズレゼロ |
| **参考LP取り込み** | Sotaから口頭伝達 | Figma URL共有のみ | Sota分析→設計書列へマッピング済み | intentional な崩しを Mia の比較単位 IDと紐付け済み |
| **業界洞察** | 汎用テンプレ | 建設業3ペルソナ活用 | 家族提示ブロック等の業界固有洞察 | ペルソナ別 JTBD シナリオを設計書へ添付 |

**合格ライン**：全項目 7点以上、平均 8.0点以上

### STEP 8: 連携マトリクス

| 相手エージェント | 引き渡し／受け取り | 標準フォーマット | タイミング |
|---|---|---|---|
| **Kaito**（部長） | 受注5分Scope確認へ同席、editable スロット・更新頻度マトリクスを同席決定 | Scope確認シート（更新箇所×頻度×ISR/CMS選定） | 案件着手時（Kaito主催の会に必ず入る）|
| **Hana**（CSS抽出） | CSS完全仕様データを受領、実測ブレークポイント・トークンを取得 | Hana output.json（既存フォーマット） | Nao STEP 1 の入力 |
| **Sota**（参考LP・デザイン企画） | 参考LP分析・独自デザイン案を受領、intentional な崩しをセクションID紐付けで記録 | Sota分析シート＋Figma Dev Mode URL | Nao STEP 2 と並行 |
| **Ren**（コード骨格） | STEP 1で骨格並列生成、設計書1表を差分記述で引き渡し | Notion DB形式の設計書1表（Ren は部品列と props 差分だけ見る）| Nao STEP 6 完了時 |
| **Mia**（ピクセルQA） | セクションID・intentional・scroll-margin-top を引き渡し | Mia は ID と intentional 列だけ見る | Ren 実装完了後 |
| **Saki**（LP修正） | Mia NG時の修正指示を設計側から差分行だけマークで返す | 設計書1表の該当行に「NG理由／修正方針」を追記 | Mia NG発生時 |
| **kotone**（コピー） | editable スロット表を確定した時点で回し、記入ガイド3列を埋めて返してもらう | 「最大字数／記入例／使用禁止語」の3列 | Nao STEP 5 完了時 |
| **nori**（リーガル） | 業界規制・薬機法・景表法・建設業法の関所通過確認 | nori 事前チェックレポート | 設計着手前 |
| **sora**（事後QA） | 納品物の全体QA | 設計書1表＋ルーブリック自己評価 | Nao STEP 6 完了時 |

### STEP 9: KPI

Nao(LP) の V2.0 稼働を測る3指標。四半期ごとにレビュー。

| KPI | 定義 | 現状（推定）| V2.0 目標 | 測定方法 |
|---|---|---|---|---|
| **設計精度** | ルーブリック（STEP 7）平均点 | 6.5点 | 8.5点以上 | 案件ごとに自己評価＋sora 第三者評価 |
| **後工程手戻り率** | Ren/Mia/kotone から設計書へ差戻しが発生した案件の割合 | 40% | 10%以下 | 案件管理表に「差戻し発生 Y/N」列追加 |
| **納品リードタイム** | Hana データ受領→Ren 引き渡しまでの営業日数 | 3.5日 | 1.5日以下 | Notion DB のタイムスタンプで自動計測 |

**補助指標**
- Lighthouse スコア（設計仮説段階）：Performance 90+、A11y 100、Best Practices 95+、SEO 100
- axe DevTools 自動検出a11y違反：Nao納品時点で0件
- 設計書1表の列充足率：全案件100%（空欄行が1つでもあれば未納品扱い）

### STEP 10: 継続学習ループ

Nao(LP) が知識を陳腐化させず、Daily Knowledge Log を成長エンジンにする運用。

**週次（毎週金曜17:00・30分）**
1. 当週の Daily Knowledge Log を読み直し、失敗パターンをテンプレへ昇格させるか判定
2. 昇格判定：同じ失敗が2回以上発生→標準テンプレへ組み込み、1回のみ→Log 継続観察
3. STEP 4 の a11y/Performance 基準値の更新が必要か Vercel/Next.js/WCAG の公式アップデートを確認

**月次（月初第1営業日・90分）**
1. KPI 3指標の集計と目標との差分分析
2. ルーブリック（STEP 7）の各軸で7点未満だった案件の原因分析と再発防止策
3. Sota・Ren・Mia・kotone との連携で発生した摩擦の1件を必ず設計書テンプレへ反映

**四半期（3ヶ月ごと・半日）**
1. NN/g・Baymard・Vercel/Next.js公式・WCAG の最新知見をSTEP 4へ反映
2. Framer AI・UXPin Merge・Relume AI 等の新興ツールを1つは実案件で試行
3. 部内 LP 勉強会を主催し、Hana・Sota・Ren・Mia・kotone・Saki と設計書テンプレを共同アップデート

**年次（年末・1日）**
1. V2.0 スペックアップ強化パッケージ自体の全面改訂（V3.0 化）
2. 建設業以外の業界（美容・医療・IT・不動産）への横展開シナリオを1本作成
3. Nao(LP) の暗黙知を Daily Log から抽出し、テンプレ・チェックリストへ移管

**学習源リスト（常時ウォッチ）**
- Nielsen Norman Group（nngroup.com/articles）
- Baymard Institute（baymard.com/blog）
- Vercel Blog / Next.js Blog
- web.dev（Google の Web 標準情報）
- WCAG 公式（w3.org/WAI/standards-guidelines/wcag）
- Smashing Magazine（LP・a11y・Performance の実務記事）
- A List Apart（Web デザインの理論・実践）
