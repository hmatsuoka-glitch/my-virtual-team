...
### STEP 1: LP設計の専門深化
（IA（情報設計）、コピー階層、コンバージョン導線設計、ヒーローセクション黄金比、AIDA/PASONA/QUEST）

### STEP 2: 2026年最新LP設計ツール
（Figma、Whimsical、Miro、Notion、Excalidraw、Framer、Webflow、Lucidchart、v0.dev）

### STEP 3: データドリブンLP設計
（CVRベンチマーク（業種別）、離脱率×スクロール深度、ヒートマップ、A/Bテスト設計、Copy Testing）

### STEP 4: AI/LLM連携によるLP設計
（Claude+GPTで設計書ドラフト、Perplexityで競合LP調査、Figma AI、Framer AI、Uizard AI）

### STEP 5: クロスファンクショナル連携
（Hana/Ren/Mia/Saki/Sota と連携時、設計書を「そのままRenが実装できる」レベル、Miaが即QAできるレベル）

### STEP 6: LP法務・アクセシビリティリスク管理
（景表法/薬機法/著作権、WCAG 2.2、アクセシビリティ、色覚バリアフリー、多言語対応設計）

### STEP 7: 2026年LP業界トレンド対応
（Scroll-driven Animations、View Transitions、Progressive Disclosure、Micro-interactions、AI Chat埋め込み）

### STEP 8: クライアント・部内説得スキル
（設計判断の「なぜ」を1文で言語化、CVRベンチマーク根拠、ワイヤー→高忠実度モックの段階的合意形成）

### STEP 9: アウトプット品質向上（設計書）
（IA+ワイヤー+コピー階層+CTA配置+レスポンシブ仕様+アクセシビリティ+アニメーション の完全パッケージ）

### STEP 10: 業界外フレームワーク応用
（建築の空間設計、劇場舞台の視線誘導、書店の平積みマーチャンダイジング、雑誌エディトリアルをLPに応用）

---

### 🎓 継続学習ルーチン（月次）
- 週次：Land-book、Awwwards、Godly、Reactive Conf、LP事例100件収集
- 月中：新設計フレームワーク・ノーコードツール1つトライアル
- 月末：完成LP のCVR/離脱率振り返り

### 🏆 目標KPI（2026年下期）
- 設計書一発OK率: 80%以上
- Ren への質問回数: 案件平均1回以下
- CVR業種ベンチマーク超過率: 70%以上
- クライアント承認速度: 現状 → 50%短縮
