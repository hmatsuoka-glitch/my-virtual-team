...
### 2026-08-08
- **効率化テクニック：Above-the-Fold 優先度スタックを「Hero要素×5行」の固定テンプレで設計初期に確定**：ヒーロー領域の要素配置を「1.訴求ワード（数値/25字以内、Kotone 2026-08-03参照）／2.サブコピー／3.主CTA／4.信頼指標（応募数・実績）／5.補足リンク（会社概要等）」の5行優先度スタックとして設計書冒頭に必須明記。Ren実装時に「Hero何を入れるか」で迷う時間がゼロ化し、SP幅で1画面に収まる要素選定を数値ゲート（縦680px以内）付きで判定できる。sotaのデザイン企画（sota担当）にもこのテンプレを共通言語として提供。
- **品質チェックポイント：Server Component / Client Component 区分表（2026-07-27参照）に「境界の理由コード（fetch/interaction/browserAPI/formState/thirdParty）」列を追加し、区分根拠を機械照合可能にする**：SC/CC 区分だけを書くと「なぜCCか」がRen実装後にレビュー不能になる。5コードのラベル化で、レビュー時に `grep 'use client'` 検出→設計書の理由コード照合を自動化。誤ってSCで書けたはずのCC化（バンドル肥大化）を設計段階で潰す。
- **失敗パターン：フォームのフィールド設計で「必須/任意」ラベルだけ書き、入力エラーメッセージ文言・エラー発生タイミング（送信時/blur時/入力中）・エラー時のフォーカス復帰動作を書かず、Renが各自の判断で実装してUX不整合**：フォーム離脱率の最大要因はエラー体験の分断。回避策：フィールド仕様表に「エラー文言（Kotoneに依頼）／発火タイミング／aria-live領域指定／エラー解消時のライブ検証」の4列を必須化。React 19 `useActionState`（2026-08-03参照）のpending/error状態と接続し、Renが実装で迷わない完成度で納品。
- **トレンド察知：Next.js の `Partial Prerendering (PPR)` が採用LPの標準構成へ**：Hero・特徴・実績等の静的部分をSSGでプリレンダし、応募フォーム・在職者数リアルタイム表示等の動的部分だけSSRで穴埋めする構成。設計書の「キャッシュ境界」（2026-08-03参照）欄に「PPR静的シェル / 動的ホール」の区分を追加し、LCPを維持しつつ動的コンテンツ配信を両立する。基本静的＋フォームのみ動的の建設業採用LPで即恩恵。
- **連携ヒント：Ren着手前の5分同期MTG（設計書引き渡し時）に「Registry 部品採用マトリクス」を持参し、shadcn/tailwindui等のどの部品をコピー起点にするかを機械的に決定**：shadcn Registry主流化（2026-07-27参照）に対応し、Button/Input/Dialog/Accordion/Tabs 等の設計行に「Registry 起点候補（shadcn/aria-kit/none）＋差分要件」を先出しで書いておく。Renが実装時にRegistry検索する時間がゼロ化し、Registry採用/フルスクラッチの判断のブレを設計段階で解消する。
