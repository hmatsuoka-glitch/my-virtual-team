...
### 8. 設計書テンプレ統合フォーマット（CSD × LIFT × CWV）

STEP 6 納品時の最終設計書は以下12ブロックで構成し、`templates/lp-design-spec.md` に固定化する。

1. **メタ情報**：プロジェクト名 / クライアント / 業界 / 担当 / 期限 / 想定CVR / 業界平均CVR
2. **JTBD カード**：Functional / Emotional / Social 各1枚以上、計3枚以上
3. **LIFT スコアシート**：6軸 × 10点 ＋ 改善コメント
4. **PIE 優先度表**：A/Bテスト候補×PIE3軸スコア＋順位
5. **Attention Ratio計算**：CTA数 / リンク数 / 比率 / Encapsulation判定
6. **F/Z/G視線パターン**：セクション別ラベル＋CTA配置論拠
7. **Cognitive Load Score表**：セクション別 1-10点、9点超は強制分割記載
8. **CSD 8観点表**：全コンポーネント × 8観点（Props数/再利用/責務/排他/SA・IM・HO/a11y/testid/3状態）
9. **Performance Budget**：LCP/INP/CLS/JS bundle / 画像サイズ / フォント数
10. **Mermaid 図3点**：ページ遷移 / データフロー / コンポーネント状態遷移
11. **Mia 95項目セルフチェック**：観点別 ○/△/× ＋対応策
12. **A/Bテスト仮説書**：仮説 / 期待効果 / MDE / 必要N / 判定期間 / 中止条件

各ブロックは1ページ以内に収め、設計書全体で12ページ以内を厳守。13ページ超は情報過多と判定し圧縮ルール適用。

### 9. CTA配置最適化プロトコル（Fitts' Law × Hick's Law 数値化）

- **Fitts' Law**: CTA 到達時間 T = a + b × log₂(D/W + 1)。STEP 3 で各CTAに対し「平均視線距離D（px換算）/ CTAサイズW（px）」を測定し、T が 2.0秒以下になるサイズ・配置に調整。
- **Hick's Law**: 選択時間 RT = a + b × log₂(n+1)。1セクションあたり選択肢n=5以下を厳守、6以上は二段階選択（カテゴリ→詳細）に再設計。
- **CTA 数量則**：ファーストビュー1個（Primary）、スクロール中2-3個（Secondary・同一文言）、ページ末1個（Closer）の最大5個まで。10個超のページは即時再設計。
- **CTA テキスト構造**：「動詞 + 対象 + ベネフィット + 時間/コスト」4要素（例：「無料で資料をダウンロードする・1分完了」）。STEP 5 で必須テンプレ化。
- **配置位置3則**：①Above the Fold必須 ②各セクション末必須 ③スマホ画面では sticky-bottom CTA を必須実装 を Ren への設計指示に固定化。

### 10. レスポンシブ・モバイルファースト強化プロトコル

- **ブレークポイント4段**：375 / 768 / 1024 / 1440px の4段を必須化、Tailwind の `sm/md/lg/xl` と完全一致させる
- **タッチターゲット48px厳守**：全インタラクティブ要素に `min-w-12 min-h-12` 必須、隣接要素間 spacing 8px 以上
- **モバイル CV 動線優先**：日本のLP流入70%超がモバイルである事実を踏まえ、STEP 2 のコンポーネント設計時に「モバイル版から先に設計、デスクトップは拡張」のモバイルファースト順序を物理ルール化
- **スマホ Sticky CTA**：スクロール中常時表示の bottom CTA を全LP必須化、安全領域（safe-area-inset-bottom）対応も設計書に明記
- **読込速度モバイル基準**：4G環境 LCP 2.5秒・3G環境 4秒以下を必達、`next/image` の `sizes` 属性をモバイル優先で記述

### 11. 行動経済学・心理トリガー設計マップ

LP設計に組み込むべき主要トリガーを STEP 5 のコンテンツ定義で必ず1要素以上配置。

| トリガー | 配置推奨セクション | 実装例 | 効果計測指標 |
|---|---|---|---|
| Social Proof（社会的証明） | Hero直下 / CTA直前 | 「導入企業1,200社」「★4.8/5」 | CTAクリック率 +15-30% |
| Scarcity（希少性） | CTA直前 | 「残り3枠」「先着50名」 | フォーム到達率 +20% |
| Authority（権威性） | About / 事例 | メディア掲載ロゴ / 受賞歴 / 専門家推薦 | 滞在時間 +25% |
| Reciprocity（返報性） | Lead Magnet | 無料診断 / 無料テンプレ配布 | リード獲得 +40% |
| Loss Aversion（損失回避） | CTA / FAQ | 「やらない場合の損失額」明示 | 検討完了率 +18% |
| Anchoring（アンカリング） | 料金表 | 高額プラン最上位配置 | 中位プラン選択率 +35% |
| Commitment（コミットメント） | Multi-step Form | ステップ式入力（3ステップ） | フォーム完遂率 +22% |
| Cognitive Ease（認知容易性） | 全セクション | 平易な言葉 / 短文 / 1行30字以内 | 直帰率 -15% |

各トリガーを濫用すると逆効果なため、1ページあたり最大4種類までを上限ルール化。

### 12. 運用ハンドオフ・継続改善プロトコル

- **納品後30日レビュー**：Mia QA通過後30日時点で GA4 / Clarity / Hotjar から CVR / Scroll depth / Form 離脱率を取得、設計書の想定値と乖離10%以上は再設計トリガー
- **A/Bテスト発火条件**：PIE スコア上位3軸を自動で saki への改修指示にエスカレーション、Optimizely でテスト起動
- **クライアント月次レポート連携**：akari の月次レポート様式に「LP設計KPI 12項目」セクションを追加、CVR / LCP / Mia QA / A/Bテスト勝敗を1ページで可視化
- **設計ナレッジ蓄積**：勝ち施策・負け施策を `agents/07-LP部/knowledge/` 配下にナレッジカード化、新案件 STEP 1 で過去類似案件3件を必ず参照
- **クライアント教育プロトコル**：技術リテラシー低クライアント向けに「設計書サマリー1ページ版（A4縦）」を別途生成、Markdown → PDF 自動エクスポートで提案資料化
