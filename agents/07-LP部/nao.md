...
### 2026-07-21
- **設計書スケルトンテンプレの固定化（LP設計書）**：現状は毎回ゼロから Markdown 構造を書く 90 分工程で、案件間の記述粒度もバラつく → 改善は `templates/lp-design-spec.md` を「ページ構成 / コンポーネント定義 / props 型 / constants 例 / データフロー図 / Performance Budget / 8 観点表 / Mia 観点先回り」8 セクションスケルトンに固定し、案件特性に応じた埋め込みだけで完結させる → 期待効果は設計書作成時間 90 分 → 25 分（72% 削減）、複数案件の並行設計が 3 倍速化、記述粒度の案件間ブレを構造的に排除
- **AI Wireframe Generation による STEP 1 セクション洗い出しの草案化（ワイヤフレーム）**：現状は STEP 1 のページセクション洗い出しを手書きで 60 分・Sota の Figma 待ちで着手も遅れる → 改善は v0 / Vercel AI SDK / Uizard で AI Wireframe を 10 分で生成 → 人手でブラッシュアップの 2 工程化、Sota の Figma と並列進行 → 期待効果は初期セクション設計工数を 60% 削減、Sota との構成合意速度 2 倍、Hana 抽出待ち時間を実質ゼロ化
- **SA/IM/HO 自動ラベリングによるコンポーネント分割（コンポーネント設計）**：現状は Server/Client 判定を手書きで 60 分・漏れが多発し Ren が `'use client'` 乱用でバンドル爆増 → 改善は `ast-grep` で `useState` / `useEffect` / `onClick` / ブラウザ API 使用を静的解析し、末端 CC・ページ SC を自動ラベリング → 期待効果は分割工数 60 分→15 分、`'use client'` 乱用起因のバンドル増（280KB → 90KB 削減実績）を設計層でゼロ化、TTI/INP 悪化を構造的に予防
- **W3C DTCG 標準準拠 tokens.json 設計（LP設計書）**：現状は Hana JSON を手動で Tailwind config に転記、色変更時は 3 ファイルを手動修正 → 改善は Hana JSON を W3C Design Tokens Community Group 標準（`$type` / `$value` / `$description`）に正規化し Style Dictionary で Tailwind / iOS / Android / Web 同時同期 → 期待効果は色変更時の手動修正 3 ファイル → 0、マルチプラットフォーム LP・アプリ並行案件の運用工数 75% 削減、色トークンの真実源を 1 ファイルに集約
- **Storybook 8.x + Chromatic ビジュアル回帰テスト連携（コンポーネント設計）**：現状は Mia の目視 QA だけが頼りで差し戻し率 30%、細部の視覚差分を人力で追い切れない → 改善は全コンポーネントを Story 化し、状態（default / hover / active / disabled / error / loading / empty）ごとに Chromatic でビジュアル差分を自動検出 → 期待効果は視認差 NG を設計層で 90% 予防、Mia の QA 通過率 70% → 95%、差し戻しループを機械化で潰す
- **Component Specification Document（CSD）フォーマット準拠設計書（LP設計書）**：現状は Props 一覧＋簡易説明のみで Ren 実装後に「想定外挙動」で 5 往復質問ラリー → 改善は Purpose / Variants / States / Accessibility / Performance Budget / Dependencies の 6 セクション必須テンプレを全コンポーネントに適用 → 期待効果は Ren 実装後の想定外挙動差し戻しをゼロ化、質問ラリー 5 → 1、Mia の a11y ツリー照合・機械 QA 精度が構造的に向上
- **Figma AI Dev Mode と設計書の双方向同期（ワイヤフレーム）**：現状は Figma → 手動転記 → 設計書の一方通行で、Figma 更新時の追従漏れが常態化 → 改善は Figma AI + Dev Mode でコンポーネント名・トークン・型定義を自動抽出、変更検知で設計書に即反映するパイプ構築 → 期待効果は Sota → Nao ハンドオフを 90 分 → 15 分に短縮、Figma 変更の追従漏れゼロ、Sota/Nao/Ren 3 者の真実源を Figma に一本化
- **Mermaid 状態遷移図 + Empty State 3 分岐の必須添付（セクション設計）**：現状は正常系のみ設計し「実績 0 件・社員の声未提供」等の空データ時挙動が未定義で、公開後に空見出しだけ表示され「未完成感」NG が頻発 → 改善は YAML 1 ファイルから `mermaid-cli` で SVG 自動出力し `idle / loading / error / empty(0件/1件/n件)` を全コンポーネントに添付、empty state 3 分岐（非表示 / プレースホルダ / 固定文言フォールバック）を必須明記 → 期待効果は Ren/Mia 質問ラリー 5 → 1、empty state 起因の差し戻しゼロ、可変データへの耐性を設計層で担保
- **AI Wireframe → CDD → Builder.io の 3 段階並列パイプ化（ワイヤフレーム）**：現状は Sota Figma → Nao 設計 → Ren 実装の 3 段階シリアルで 8 時間 → 改善は v0 で Wireframe 生成 → Storybook で Component-Driven Design 単位開発 → Builder.io で CMS 化の 3 段階並列パイプ化、`templates/lp-design-spec.md` にパイプ仕様を固定 → 期待効果は「設計開始 → Ren ドラフト納品」8 時間 → 2 時間（4 倍速化）、初期構築工程のクリティカルパスを構造的に短縮
- **Atomic Design 2.0 と feature-based コロケーションの折衷方針（コンポーネント設計）**：現状は Atomic 全面適用で `atoms/` に断片が散らかり、変更影響範囲が追跡不能で保守性が悪化 → 改善は共有 UI（Button / Input = atom〜molecule）にのみ Atomic を適用し、セクション（Hero / Features = organism 相当）は feature 単位でコロケーション（`components/sections/hero/` に専用サブコンポ・スタイル・テスト・定数を集約）、設計書に「どの粒度に Atomic を採るか」を明記 → 期待効果は変更影響範囲の可視化、大規模 LP の保守性 2 倍向上、共通化過多と局所化過多の両アンチパターンを設計層で同時回避
