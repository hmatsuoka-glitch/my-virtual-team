...
### 2026-09-04
- **Next.js 15 PPR (Partial Prerendering) 前提でセクション単位に「静的シェル/動的スロット」区分を設計表へ必須追加する**：PPR は 1 ページ内で静的シェルと動的スロットを Suspense 境界で切り分ける仕組みだが、設計時に区分ラベルが無いと Ren はページ全体を dynamic か static のどちらかに寄せ、PPR の初回描画メリットを丸ごと失う。ページ構造表の列に「PPR 区分（static shell / dynamic slot）」と「動的スロットの fallback 予約寸法」を追加し、CLS 予約寸法（2026-08-12参照）と同じ数値で fallback を設計する。Kaito の受注時 Scope 確認（2026-08-27参照）で ISR/CMS 選定と同席する場に PPR 適用可否も並べ、更新頻度マトリクスと PPR 区分を1つの決定として1枚に残す
- **Suspense 境界は「fetch 単位」でなく「求職者の読み進み単位」で切り、境界ごとの fallback を実コンテンツ同寸で設計する連携**：Server Components + streaming SSR で境界を細かく切りすぎると各セクションが個別にポップインしてページ全体がガタつき、逆に大きく取ると先頭の遅い fetch がページ全体をブロックする。求職者関心度順の並び（2026-08-16参照）と対応させ、first-view の Hero と条件サマリを 1 つ目の境界に閉じ、実績・社員紹介など下部セクションを 2 つ目以降の境界に置く。境界ごとに fallback の高さを実コンテンツと同寸で設計し、Mia のセクション単位ベースライン比較（2026-08-27参照）でも streaming 中の中間状態を除外できる ID を境界と一致させる
- **フォーム設計を STEP 3 の一部から「STEP 0.5：入口設計」へ前倒し、フォーム項目確定を全設計の起点にする運用**：応募フォームがページ最下部の付属物になっているうちは項目数の上限（2026-09-02参照）や Ao の Zod スキーマ整合（2026-07-16参照）が後追いになり、CV 直前で削減交渉が発生する。案件開始時に「必須2＋任意3の合計5項目まで」の枠と Ao のスキーマを先に確定し、Hero・CTA・条件サマリの設計はフォーム項目から逆算する。kotone の CV 直前3点再掲（kotone 2026-08-16参照）もフォーム項目と1対1で対応させ、下流の再設計を起点固定で消す
- **Figma dev mode の Variables/Modes を Hana の tokens.json より先に Sota から受領し、設計書 semantic 層に mode 列を持たせる連携**：Sota の A/B 案やクライアント別ブランドは semantic 1層の付け替えで済む設計（2026-08-03／2026-08-13参照）にしていても、Figma 側で mode 定義が無いとデザイナーの手元では色差替が反映されず、レビュー会で「ここだけ変えて」と個別依頼が戻る。Sota から Figma Variables の mode 定義（light/dark、ブランド A/B）を受け取り、semantic 割当表に mode 列を追加して DTCG（2026-07-27参照）準拠で書き出す。iro の semantic 割当（2026-08-13参照）とも mode 単位で対応させ、デザイン側と実装側の真実源を1つに統合する
- **Registry 採用部品（2026-07-27参照）は「案件で上書きしたスロット」を差分表で管理し、Registry 元版更新時の追従判定を設計層で可能にする**：shadcn 等の Registry を採用しても案件ごとに Button/Input へ上書きを入れると、Registry の元版が更新された時にどの案件へマージ可能かが不明になり、結局全案件で個別確認するコストが戻る。設計書に「Registry 部品名／採用バージョン／上書きスロット一覧／上書き理由」の差分表を持ち、Ren の共通コンポーネントパッケージ（ren 2026-08-18参照）へ上書き差分をパッチとして格納する。Registry 更新時は差分表を突き合わせるだけで追従可否が決まり、再利用の恩恵を運用フェーズまで持続させる
