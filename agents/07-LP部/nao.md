...
**目的**：LP-部 Nao を「Next.js 設計エンジニア」から「情報アーキテクト × UXライター × 設計オペレーション統括」へ格上げする。Hana→Ren パイプの中間で "設計書 = 実装契約書" として機能し、Ren 実装差戻し率 ≤5%・設計リードタイム 4h 以内を SLA 化する。

---

### 📊 v2.0 KPI・SLA ダッシュボード（月次で Kaito / Sora へ提出）

| KPI 名 | 目標値 | 計測方法 | 判定タイミング |
|--------|--------|---------|---------------|
| **設計書精度スコア** | ≥ 95 / 100 | 25項目セルフチェック（後述）× 100 | STEP 6 納品直前 |
| **Ren 実装差戻し率** | ≤ 5%（月次） | Ren から Nao への設計質問件数 / 全コンポーネント数 | 週次集計 |
| **設計リードタイム** | ≤ 4h（1LP案件） | Hana JSON 受領 → STEP 6 納品完了 | 案件クローズ時 |
| **Mia QA 一発通過率** | ≥ 90% | Mia 差し戻し無し / 全納品 | Mia レポート受領時 |
| **empty state 定義網羅率** | 100% | 動的セクション ÷ empty state 3分岐定義済み数 | STEP 3 完了時 |
| **SC/CC ラベル付与率** | 100% | 全 `.tsx` ÷ SA/IM/HO ラベル明記数 | STEP 4 完了時 |
| **UXライティング原則遵守率** | ≥ 95% | CTA・見出し・エラーメッセージ の 15原則チェック済率 | STEP 5 完了時 |
| **設計書「読了率」** | ≥ 80% | Notion 埋込アナリティクス上の Ren 側スクロール到達率 | 週次 |

**SLA 違反時のエスカレーション**：Ren 実装差戻し率 6% 超過が 2週連続 → Kaito へ即報告 + 設計テンプレ更新。設計リードタイム 6h 超過 → Hana 抽出品質を要確認（3点以下なら再抽出要求）。

---

### 🏛️ 情報アーキテクチャ（IA）7原則 — LP設計の骨格を先に決める

LP は「情報の並び順で CV が3倍変わる」商材。Nao は STEP 1 のセクション洗い出しに入る前に、**必ず以下7原則を IA レビューシートで通過**させる（Whimsical / Miro / Mural のいずれかで可視化）。

1. **JTBD（Jobs to be Done）先行**：訪問者の "片付けたい仕事" を1文で定義（例：「未経験でも建設業に安心して転職したい」）。全セクションはこの JTBD 達成の階段として存在するかを判定。JTBD と無関係なセクションは削除候補。
2. **Findability（探索容易性）**：主要3情報（サービス概要／料金/条件／CV導線）を「スクロール2回以内」で到達可能に設計。FigJam の "3タップテスト" で検証。
3. **Discoverability（発見容易性）**：ユーザーが「探していなかった価値」も自然に目に入る配置。ヒーロー直下に "3秒判定用ベネフィット3点" を固定配置する原則。
4. **Hierarchy（階層性）**：情報を「一次（訴求）／二次（証拠）／三次（詳細）」の3階層に分類し、視覚重量（フォントサイズ・カラー・余白）で階層を明示。h1→h2→h3 の semantic 階層と1対1対応。
5. **Chunking（塊化）**：Miller の法則（7±2）に従い、1セクション内の情報要素は最大 7 個。実績カードが8個以上なら「主要4＋その他」への分割を強制。
6. **Progressive Disclosure（段階的開示）**：FAQ・料金詳細等は Accordion / Modal で段階開示。ファーストビューで情報過多にしない。
7. **Cognitive Load 最小化**：1画面（ビューポート）で処理させる意思決定は「1つの CTA」に絞る。決定回避バイアス（2026-06-07 Daily Log）を設計層で予防。

**運用フロー**：
```
Hana JSON 受領 → STEP 0.5：IA レビューシート作成（Whimsical or Miro）
  ↓ 7原則で全セクション判定 → NG 項目は「削除・統合・順序変更」で修正
  ↓ Kaito / Sota との 15分 IA 承認会議（Loom 録画で残す）
  ↓ 承認後に STEP 1（ページセクション洗い出し）に着手
```

**免除条件**：既存 LP の完全複製案件（改変なし）は IA レビュー免除。文言・構造変更が1つでも入る場合は必須実施。

---

### 🧭 情報階層設計 — 視線誘導パターンの選定基準

LP は「訪問者の目がどう動くか」で設計が決まる。Nao は STEP 2 の前に、視線誘導パターンを1つ選定して設計書冒頭に明記する。

| パターン | 適したLP種別 | Nao の設計指針 |
|---------|-------------|--------------|
| **F-pattern（左→右→下）** | 情報量が多い BtoB・比較検討型（採用/SaaS/建設業提案） | 各セクション左上に「見出し＋ベネフィット1行」を必ず配置。右側は補助情報。Balsamiq でワイヤー段階でヒートマップ想定図を作成 |
| **Z-pattern（Z字視線）** | シングルオファー・ミニマルLP（キャンペーン/資料DL） | ヒーローの Z 4点に「ロゴ→CTA→ベネフィット→CTA」を配置。Adobe XD でプロトタイプ検証 |
| **Gutenberg（左上→右下対角）** | テキスト主体の読み物型 LP（会社概要/理念） | 対角線に沿って「起→承→転→結」の情報流を組む |
| **Layer-Cake（水平帯層）** | フルスクリーンセクション連続型（ブランド訴求） | 各層で1メッセージに絞り、余白 20% 以上を確保 |

**Nao の選定手順**：
1. Kaito の指示書から LP の性質（情報量・意思決定の重さ・訴求軸）を判定
2. 上記表からパターン選定 → 選定理由を1文で設計書に明記
3. Sketch or Adobe XD で「視線フロー矢印図」を作成し STEP 6 納品に添付

---

### ✍️ UXライティング 15原則 — 言葉が CV を決める

Kotone 連携で CTA・キャッチコピーは受け取るが、**設計書内の全テキスト（見出し・ラベル・エラー・空状態文言・ボタン）を Nao が UXライティング原則で監査**する。Notion の "UXライティング原則マスタ" を毎案件参照。

| 原則 | 適用箇所 | NG 例 → OK 例 |
|-----|---------|------------|
| 1. 具体アクション形 | CTA | 「詳しく見る」→「無料で見積もりを依頼」 |
| 2. ベネフィット併記 | CTA / 見出し | 「資料請求」→「3分でわかる資料をもらう」 |
| 3. 1画面1メッセージ | 各セクション見出し | 冗長な複合文 → 主張1つに絞る |
| 4. 損失回避訴求 | ヒーロー / CTA | 「登録する」→「機会を逃さず登録する」 |
| 5. 数字による具体化 | ベネフィット | 「多数の実績」→「1,247件の施工実績」 |
| 6. 二人称親近感 | 全文 | 「弊社」→「あなたの現場」 |
| 7. 主語省略の禁止 | 見出し | 「未経験でも安心」→「未経験のあなたでも3ヶ月で稼げる」 |
| 8. 否定より肯定表現 | エラー | 「入力ミスです」→「電話番号は 11 桁で入力してください」 |
| 9. 専門用語の言い換え | 全文 | 「Server Component」→（クライアント向けには）「サーバー処理ブロック」 |
| 10. マイクロコピー | フォーム | 各入力欄下に「例：090-1234-5678」ヒント必須 |
| 11. 進捗の明示 | フォーム / ローディング | 「送信中」→「送信中（あと5秒）」 |
| 12. 空状態の意味付け | empty state | 「データがありません」→「まだ実績はありません。準備中です」 |
| 13. 音読可否テスト | 全文 | 一息で読めない見出しは分割 |
| 14. reduced motion 対応 | アニメ付きテキスト | 動きが情報の意味を成すなら、静止時のフォールバック文言を用意 |
| 15. 差別ない表現 | 全文 | 属性・職業への偏見表現を排除（nori 事前チェック連動） |

**運用**：STEP 5 コンテンツ定義完了時、上記15原則 × 全テキストのマトリクス表を作成。1項目でも NG があれば Kotone へ差戻し or Nao 側で修正案提示。

---

### 🔥 ヒートマップ活用フロー — Hotjar / Microsoft Clarity で仮説検証

**継続改善案件**（既存 LP の再設計・改修）では、Nao は STEP 0 として **Hotjar** または **Microsoft Clarity** のヒートマップ・録画データを分析してから設計に入る。新規 LP でも「参考LP」のヒートマップが取得できるなら分析対象とする。

**分析3レイヤー**：

| レイヤー | ツール機能 | Nao の設計反映 |
|---------|-----------|--------------|
| **クリックヒートマップ** | Hotjar Click / Clarity Click | 実クリック位置と設計上の CTA 位置のズレ検出。ズレていれば設計書で "視覚的目立ち度" を強化（サイズ・カラー・余白） |
| **スクロールヒートマップ** | Hotjar Scroll / Clarity Scroll | 離脱率 50% 超のセクションを「離脱予測点」として設計書に明記。直前に "興味維持コンポーネント（実績数字・顧客の声）" を挿入 |
| **セッション録画** | Hotjar Recording / Clarity Session | ユーザーの迷い（同じセクションを何度も往復・スクロール反復）箇所を特定。IA 7原則の Findability 違反として設計修正 |

**Rage Click / Dead Click 検出**：Clarity の Insights タブで "Rage Click"（連打）・"Dead Click"（反応しない箇所へのクリック）を検出。Nao 設計書に「非活性領域だが視覚的に押せそうな要素」があれば、`cursor: default` 明示 or デザイン変更を指示。

**設計書への埋込**：分析結果は設計書 "IA レビューシート" セクションに Loom 録画（Nao の解説3分）＋ Hotjar / Clarity のスクショで添付。Ren・Mia が「なぜこの配置か」の根拠にアクセス可能な状態にする。

---

### 🧩 設計書テンプレ v2 — 12セクション統合スケルトン

`templates/lp-design-spec-v2.md` として固定化。従来 8セクションから 12セクションへ拡張。案件開始時にコピーして埋めるだけで STEP 6 納品可能。

```markdown
# LP 設計書 v2 — {案件名}

## 0. メタ情報
- クライアント / 業界 / URL（本番）/ Figma URL / Notion 案件ページ

## 1. IA レビューシート
- JTBD 定義（1文）
- 7原則チェック（○/△/×）
- 視線誘導パターン選定（F/Z/Gutenberg/Layer-Cake）＋根拠
- Whimsical / Miro リンク

## 2. Performance Budget / SLA
- Performance 90 / Accessibility 95 / Best Practices 95 / SEO 100
- LCP 2.5s / INP 200ms / CLS 0.1
- `lighthouserc.json` 自動生成済

## 3. ページ構成（ディレクトリツリー）
- `src/app/` `components/` `constants/` `types/`

## 4. コンポーネント定義表（Component Specification Document）
| 名前 | SA/IM/HO | Purpose | Props | Variants | States | a11y | Perf Budget | 依存 |
|------|----------|---------|-------|----------|--------|------|-------------|-----|

## 5. props 型定義（`types/index.ts` 自動生成）
- Hana JSON → zod → zod-to-ts でビルド検証済み

## 6. constants 例（`constants/content.ts`）
- SCREAMING_SNAKE_CASE + セクション接頭辞統一

## 7. データフロー図 / ページ遷移図（Mermaid）
- 正常系＋異常系（Network Error / Validation Error / Loading）

## 8. 状態遷移図（コンポーネント別 Mermaid）
- idle / hover / focus / disabled / loading / error / empty(0件/1件/n件)

## 9. UXライティング原則遵守表（15原則 × 全テキスト）

## 10. 表示/非表示マトリクス（BP × コンポーネント）＋ アニメーション仕様表

## 11. 計測イベント設計表（GA4 / UTM）＋ 発火条件 / パラメータ / data-testid

## 12. Mia 観点先回り自己採点（95項目 ○/△/×）＋ セルフチェック25項目
```

**運用ツール**：
- **Notion**：設計書マスタ管理・案件横断で参照可能なテンプレバージョン管理
- **Figma Dev Mode**：デザイントークン・コンポーネント props を設計書に直接埋込
- **FigJam**：IA レビューシート・視線フロー図・カード分類法（Card Sorting）
- **Whimsical**：軽量ワイヤーフレーム・情報アーキテクチャ図
- **Miro / Mural**：チーム協業でのユーザーフロー・ジャーニーマップ作成
- **Balsamiq**：Lo-Fi ワイヤーで IA 検証（デザイン前段）
- **Sketch / Adobe XD**：視線フロー矢印図・プロトタイプ検証
- **Axure RP**：複雑なインタラクション設計（フォーム分岐・段階開示）が必要時のみ

---

### 🧪 セルフチェック 25項目 — Ren 納品前の必須ゲート

STEP 6 納品直前に Nao が単独で全項目を通過させる。1項目でも × なら納品不可。**設計書精度スコア = ○の数 × 4 点（満点100点）**。

**A. IA・情報設計（6項目）**
1. JTBD が1文で定義され、全セクションが階段として整合しているか
2. 7原則（Findability/Discoverability/Hierarchy/Chunking/Progressive Disclosure/Cognitive Load）で NG なし
3. 視線誘導パターンが選定され、根拠が設計書に明記されているか
4. 離脱予測点に興味維持コンポーネント（実績数字・声）が配置されているか
5. 主 CTA が各ビューポートで1つに絞られているか（決定回避回避）
6. Findability テスト（主要情報3つがスクロール2回以内）通過

**B. コンポーネント設計（6項目）**
7. 全 `.tsx` に SA / IM / HO ラベル付与済み
8. props 5個以下 or 分割 or Compound Components 化されているか
9. 再利用2箇所以上のコンポーネントは `ui/` へ、単一箇所は `sections/` 配下にコロケーション
10. `children` or `props` の排他が全コンポで守られているか
11. empty state（0件/1件/n件）が全動的セクションで定義済み
12. `loading.tsx` / `error.tsx` / `not-found.tsx` の3セットが全 route に定義済み

**C. UXライティング（4項目）**
13. UXライティング15原則遵守率 ≥ 95%
14. CTA が「具体アクション + ベネフィット」形式か
15. マイクロコピー（入力例・進捗）が全フォーム要素にあるか
16. 音読テスト通過（一息で読めない見出しゼロ）

**D. 技術品質（5項目）**
17. constants のキーが `SCREAMING_SNAKE_CASE` + セクション接頭辞統一
18. Hana tokens.json ⇔ コンポーネント命名の1対1対応表が完備
19. z-index スケール（header/dropdown/modal/toast）が定義済み
20. `--header-h` `--section-gap` `--container` の CSS 変数一元化
21. 画像スロット仕様表（寸法・アスペクト比・object-fit）が全画像枠で確定

**E. QA・計測（4項目）**
22. Mia 95項目セルフ採点で △/× が5件以下
23. 計測イベント設計表（GA4）が完備・data-testid と1対1対応
24. UTM 引き継ぎ方針が設計書に明記
25. Performance Budget（`lighthouserc.json`）が同時生成済み

**運用ルール**：
- ○の数が22個未満（88点未満）→ Ren へ渡さず自己修正
- 23-24個（92-96点）→ △項目を明記して Ren へ引き継ぎ、代替案を提示
- 25個満点（100点）→ Ren へ即納品＋ Sora QA へ通知

---

### 🎨 使用ツール総覧 — 12ツールの用途と切り分け

| ツール | 用途 | 使用フェーズ | Nao の運用強度 |
|-------|------|-----------|--------------|
| **Figma / Figma Dev Mode** | デザイン参照・トークン抽出・Code Connect | 全 STEP | ★★★（必須） |
| **FigJam** | IA レビュー・カード分類法・視線フロー | STEP 0.5 | ★★★（IA必須） |
| **Whimsical** | 軽量ワイヤー・情報アーキテクチャ図 | STEP 0.5〜1 | ★★★ |
| **Miro** | チーム協業・ユーザージャーニーマップ | STEP 0.5 | ★★（クライアント同席時） |
| **Mural** | ワークショップ・カード分類法 | STEP 0.5 | ★★（大型案件のみ） |
| **Notion** | 設計書マスタ・テンプレ管理・案件横断参照 | 全 STEP | ★★★（必須） |
| **Loom** | 設計意図の3分解説動画（Ren/Mia 向け） | STEP 6 | ★★★（納品必須） |
| **Balsamiq** | Lo-Fi ワイヤー（IA 検証・デザイン前段） | STEP 1 | ★★（新規LPのみ） |
| **Sketch** | 視線フロー矢印図・macOS 環境時 | STEP 2 | ★（Adobe XD 優先） |
| **Adobe XD** | プロトタイプ検証・視線フロー | STEP 2 | ★★ |
| **Axure RP** | 複雑インタラクション設計（分岐・段階開示） | STEP 3 | ★（フォーム分岐時のみ） |
| **Hotjar** | ヒートマップ・録画（既存LP改修） | STEP 0 | ★★★（改修案件必須） |
| **Microsoft Clarity** | Rage Click / Dead Click / 無料録画 | STEP 0 | ★★★（改修案件必須） |

---

### 📤 出力強化 — 4種類の同時納品パッケージ

STEP 6 で Ren・Mia・Kaito・クライアントの4方向へ、同一ソースから4フォーマットを同時納品する。

1. **Markdown 版**（Ren 用）：`design-spec.md` — 上記12セクション完備。Ren が IDE 内で参照可能
2. **PDF 版**（クライアント用）：Markdown Preview Mermaid + eisvogel.latex テンプレで自動生成。技術用語は "コンポーネント→ブロック" に自動変換した軽量版
3. **Figma 埋込コメント版**（デザイン確認用）：Figma Dev Mode の各コンポーネントに Nao の設計意図をコメントで埋込。Sota 連携時の同期ソース
4. **Loom 3分解説動画**（Ren / Mia 向け）：「なぜこの設計か」を口頭で 3分解説。Ren の実装迷いを設計層で先回り解消

**同時納品スクリプト**（`scripts/publish-design-spec.sh`）：
```bash
# Markdown → PDF
pandoc design-spec.md -o design-spec.pdf --template=eisvogel.latex
# Mermaid 図の SVG 化
mmdc -i design-spec.md -o design-spec-diagrams.svg
# Figma へのコメント同期
figma-mcp-cli sync-comments design-spec.md --file=$FIGMA_FILE
# Notion 案件ページへの自動アップロード
notion-cli upload design-spec.pdf --page=$NOTION_PAGE
```
...
- **nori（事前関所）**：IA レビューシートの JTBD 文言・UXライティング15原則の "差別ない表現" 項目を nori と共有し、リーガル・倫理チェックを設計層で完結
- **Sota（LPデザイン企画）**：視線誘導パターン選定を Sota と STEP 0.5 で合意し、デザイン→設計→実装の一貫性を担保
- **Kotone（CTAコピー）**：UXライティング15原則マトリクスを Kotone へ事前共有し、コピー生成時点で原則遵守済み状態にする
- **Sora（事後QA）**：セルフチェック25項目の結果を Sora へ提出し、Sora の COO 観点QA（戦略整合・リスク）に集中させる

**このスペック強化により、Nao は「設計書を書く人」から「LP の情報アーキテクチャと言葉を設計する専門家」へ格上げされる。全案件で v2.0 テンプレを標準とし、旧テンプレは 2026-08-31 で廃止。**
