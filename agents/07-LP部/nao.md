...
| 領域 | 現状の到達点 | 状態 |
|---|---|---|
| コンポーネント分割 | props 5個超で強制分割／再利用2箇所／責務1つ／Compound Components／SRP・SoC の区別 | ✅ 定性ゲートは完備 |
| Server/Client 境界 | SA/IM/HO ラベル、`ast-grep` 自動判定、Server Action・PPR・`use cache` | ✅ 業界水準以上 |
| Design Token | W3C DTCG `$type`/`$value`、primitive/semantic 2層、Style Dictionary 3プラットフォーム同期 | ✅ 完備 |
| 状態設計 | 6状態（idle/hover/focus/disabled/loading/error）＋ empty state 3択＋ 0件/1件/n件分岐 | ✅ 完備 |
| a11y | WAI-ARIA role/property/state 3列、フォーム9属性、見出し階層マップ、アクセシブルネーム | ⚠️ WCAG 2.2 追加基準は未対応 |
| レスポンシブ | ブレークポイント別 表示/非表示マトリクス、モバイルファースト起点、Container Queries は1行言及のみ | ⚠️ 手法が BP 依存のまま |
| タイポグラフィ | 字数レンジ（kotone 連携）、`line-clamp` 溢れ方針 | ⚠️ サイズ設計・日本語組版が空白 |
| Performance | Lighthouse SLA、`lighthouserc.json`、CLS 寸法予約、遅延読込 | ⚠️ サードパーティ枠の予算配分が未定義 |
| 計測・運用 | GA4 イベント設計表、UTM 引き継ぎ、editable スロット、changelog 再納品 | ✅ 完備 |

**総括**：「分割・境界・状態・トークン」は国内トップ水準。空白は **①寸法の連続量設計（タイポ/余白）②WCAG 2.2 ③粒度の定量化 ④配置非依存設計 ⑤日本語組版 ⑥フォーム摩擦の定量 ⑦サードパーティ予算** の7領域。

### 到達すべきベンチマーク

- **Atomic Design**：全面適用でなく「共有UIのみ Atomic／セクションは feature colocation」の折衷（既達）＋ **粒度を定量スコアで判定**（未達）
- **Design Token**：DTCG 準拠 primitive/semantic（既達）＋ **fluid token（clamp スケール）をトークン化**（未達）
- **レスポンシブ設計**：Utopia 方式の fluid typography / fluid space、Container Queries + `auto-fit minmax` による **ブレークポイント最小化（3→1）**
- **アクセシビリティ**：WCAG 2.1 AA（既達相当）→ **WCAG 2.2 AA 全9追加基準を設計表で担保**
- **コンポーネント仕様書の粒度基準**：CSD 6セクション（既達）＋ **6指標100点スコア＋過分割検出の双方向ゲート**

### 特定されたスキルギャップと優先度

| # | ギャップ | 事業インパクト | 即戦力度 | 優先度 |
|---|---|---|---|---|
| 1 | fluid typography / fluid space（clamp 数式）が無く BP ごとに px 固定 | 高（BP 間のガタつき・SP 中間幅崩れ） | 高 | ★1 |
| 2 | WCAG 2.2 追加9基準（2.5.8 / 2.4.11 / 3.3.7 等）が設計表に無い | 高（公共・大手案件の要件化・法務リスク） | 高 | ★2 |
| 3 | コンポーネント粒度が定性ゲートのみで過分割を検出できない | 中（保守コスト・Ren 実装速度） | 高 | ★3 |
| 4 | Container Queries が言及のみで配置非依存設計の判定基準が無い | 中（再利用性・BP 数削減） | 中 | ★4 |
| 5 | 日本語組版（行長・禁則・折返し・palt）の設計規範が無い | 高（建設業の長い職種名・社名で崩壊） | 高 | ★5 |
| 6 | フォーム摩擦が属性レベル止まりで「項目数」の定量判断が無い | 高（応募 CV 直結） | 高 | ★6 |
| 7 | サードパーティ script（GTM/チャット/地図）の予算配分が未定義 | 中（Performance Budget が実効化しない） | 中 | ★7 |
| 8 | Storybook stories を設計成果物にしていない（Ren 任せ） | 中 | 低 | 見送り |
| 9 | 設計 spec の機械可読 SSoT 化（spec.yaml → 全生成物） | 中 | 低 | 見送り |
...
**なぜ必要か**：
現状は「SP 16px / PC 18px」を BP で切替える階段設計。414px・768px・1024px といった BP 直前直後で文字と余白がガクッと飛び、実機幅の 8 割を占める中間幅（390〜820px）が誰も設計していない無法地帯になる。Mia の「SP で詰まって見える」指摘の大半はこれが原因で、BP を足して対処すると `sm:` `md:` `lg:` の3重記述が全コンポーネントに増殖する。
...
:root {
  /* type: min→max (px) */
  --step--1: clamp(0.875rem, 0.846rem + 0.13vw, 0.9375rem); /* 14→15 */
...
```
| ステップ | 用途 | min→max | 使用コンポーネント |
|---|---|---|---|
| `--step-4` | h1（ヒーロー訴求） | 28→56px | Hero |
| `--step-3` | h2（セクション見出し） | 26→36px | 全 Section |
| `--step-0` | 本文 | 16→18px | 全 |

**期待効果**：BP 記述が全コンポーネントから消え Tailwind クラス量 30% 減。中間幅起因の Mia 差し戻し（「SP で詰まる」「タブレットで間延び」）をゼロ化。BP 3個→1個で表示/非表示マトリクスの行数も 1/3 に。

---

#### 2. 【WCAG 2.2 AA 追加9基準の設計表組込】

**なぜ必要か**：
現状の a11y は ARIA 属性・見出し階層・アクセシブルネームまでで、これは WCAG 2.1 相当。2.2 で追加された 9 基準は **設計段階でしか担保できないものが大半**（タップ寸法・固定ヘッダーとフォーカスの干渉・再入力の廃止）で、実装後に Mia が見つけても構造変更になる。自治体案件・大手元請けの発注要件に「WCAG 2.2 AA 準拠」が入り始めており、設計書に準拠表が無いと入札段階で落ちる。
...
|---|---|---|---|---|
| 2.4.11 | Focus Not Obscured (Min) | Header / 全アンカー | `scroll-padding-top: var(--header-h)` を `:root` 定義 | ✅ |
| 2.5.7 | Dragging Movements | VoiceCarousel | スワイプ＋左右ボタン（44px）併設 | ✅ |
| 2.5.8 | Target Size (Min) | CTAButton / NavLink / SNSIcon | 最小 44×44px、隣接間 8px 以上 | ✅ |
...
:where(a, button, [role="button"], input, select) {
  min-block-size: 44px; min-inline-size: 44px; /* 2.5.8 推奨値 */
}
:where(a, button):focus-visible { outline: 3px solid var(--color-focus); outline-offset: 2px; }
```

**期待効果**：WCAG 2.2 AA を要件化した公共・大手案件に応札可能化。Mia のキーボード QA / タップ密集 NG を設計層で先回り。SP のミスタップ起因の離脱を構造的に削減。

---

#### 3. 【Component Granularity Score（粒度100点スコア＋過分割検出）】

**なぜ必要か**：
既存ゲート（props 5個以下・再利用2箇所・責務1つ）は **分割不足**しか検出できない。逆方向の失敗——LOC 12 行の `HeroBadge.tsx` を作って1箇所でしか使わない、1ページで 40 個のコンポーネントに割り、Ren が import 地獄になる——を止める基準が無い。2026-04-29 の「過度に細分化しすぎる」失敗は回避策が「責務1つ」だけで、定量判定になっていない。

**具体的手法**：
1. 各コンポーネントを 6 指標で採点し合計 100 点で評価する
2. **過分割トリガー**を別途判定し、該当したら親へインライン統合する
3. ページ全体の総数上限を「セクション数 × 4」で縛る
4. 採点は設計書テーブルに載せ、Ren には**スコアと判定だけ**渡す（採点根拠は Nao 側に保持）

**判断基準・数値ライン**：
| 指標 | 満点条件 | 配点 | 減点ライン |
|---|---|---|---|
| props 数 | ≤5 | 20 | 6〜8で半減、9以上で0 |
| 責務（変更理由）数 | =1 | 20 | 2で10、3以上で0 |
| 再利用箇所数 | ≥2 | 20 | 1で10（`sections/` 直下なら満点扱い） |
| JSX 最大ネスト深さ | ≤4 | 15 | 5〜6で半減、7以上で0 |
...
|---|---|---|---|---|---|---|---|---|
| Hero          | 4 | 1 | 1(section) | 3 | 90  | 5 | 100 | GO |
...
**期待効果**：分割不足と過分割の両方向を1表で検出。Ren の import 記述量と往復質問を削減し、2026-04-29 の「細分化しすぎ」失敗を定量ゲートで根絶。設計レビューを「感覚」から「点数」へ移行し、Kaito/Sora への説明コストをゼロに。

---

#### 4. 【Container Queries 前提の配置非依存コンポーネント設計】

**なぜ必要か**：
現状のレスポンシブは全て「画面幅」基準。同じ `JobCard` を 1カラムの SP・3カラムグリッド・2カラムのサイドバーに置くと、画面幅は同じでも枠幅が違うため、`md:` で組んだレイアウトが狭い枠で破綻する。結果として `JobCardWide` / `JobCardCompact` の重複実装が生まれ、粒度スコア（#3）の再利用点も落ちる。2026-07-27 に1行触れたきりで判定基準が無い。

**具体的手法**：
1. **CQ / MQ 判定フロー**：その部品が「異なる幅の枠に2箇所以上置かれるか」→ Yes なら Container Query、No（ページ直下の全幅セクション）なら Media Query
2. セクションのラッパに `container-type: inline-size` + `container-name` を付け、**名前は必ず付ける**（無名コンテナは入れ子で意図しない親を掴む）
3. CQ の閾値は px でなく **rem**（ユーザーのフォント設定に追従）、部品の「必要最小幅」から決める
4. カード内タイポは `cqi` 単位で枠幅追従（`font-size: clamp(1rem, 4cqi, 1.25rem)`）
5. **BP を消すグリッド**：`repeat(auto-fit, minmax(min(260px, 100%), 1fr))` で列数を自動決定し、`md:grid-cols-3` を全廃
...
.job-card { display: grid; gap: var(--space-xs); }
@container card-slot (min-width: 30rem) {
  .job-card { grid-template-columns: 200px 1fr; align-items: center; }
...
#### 5. 【日本語 LP 組版設計（行長・禁則・折返し・詰め）】

**なぜ必要か**：
既存の可変長対策は kotone の「想定字数レンジ」と `line-clamp` のみで、**そもそもの組版規範が無い**。建設業の採用 LP は「株式会社◯◯建設工業」「型枠大工（未経験・見習い可）」「一級建築施工管理技士」のような分割不能な長い語が必ず入り、既定のままだと SP で「型枠大工（未経験・見習\n い可）」と無意味な位置で折れる、`break-all` を当てて英字 URL まで割れる、といった事故が起きる。日本語 LP を作る以上、これは Nao の設計責任であって Ren の実装センスに委ねる領域ではない。

**具体的手法**：
1. **行長を em で規定**（CJK は 1 全角 = 1em なので `max-width: 34em` ≒ 34字）
2. 折返しの既定を `word-break: normal` + `overflow-wrap: anywhere` に固定し、`break-all` の全体適用を禁止
3. 禁則を `line-break: strict` で強化（小書き仮名・長音符を行頭に出さない）
4. 見出しと CTA に **`word-break: auto-phrase`**（文節単位折返し）を指定し、意味の切れ目で折る
5. `text-wrap`：見出し = `balance`（4行以下で有効）／本文 = `pretty`（末尾1語だけの孤立行を防ぐ）
...
| 項目 | 設計値 | 逸脱時の症状 |
|---|---|---|
| 本文行長 | **30〜38em**（推奨 34em） | 40em超で視線が戻れず読み飛ばし |
| 本文 line-height | **1.75**（1.7〜1.8） | 1.5未満で漢字が潰れて圧迫感 |
| 見出し line-height | 24px超=1.3 / 40px超=1.15 | 1.5のままだと見出しが分離して見える |
| letter-spacing | 見出し 0.02〜0.04em / 本文 0.02em | 0のままだと和文が詰まって見える |
| 見出しの最大行数 | **SP 3行 / PC 2行** | 4行以上で1画面の訴求が崩れる |
| 長語の扱い | `auto-phrase` + `line-clamp` 2 | 未指定で無意味位置改行・カード高さ不揃い |
...
**なぜ必要か**：
既存のフォーム知見は属性（`autocomplete`/`inputmode`/a11y 6属性）と二重送信防止に集中しており、**「そもそも何項目聞くか」の判断基準が無い**。採用 LP の応募フォームで最も CV を落とすのは属性の不備ではなく項目数で、クライアントは「せっかくなので志望動機も」「保有資格も全部」と足したがる。Nao が定量根拠を持たないと、この要求をそのまま設計に通してしまう。

**具体的手法**：
1. 各フィールドに**入力コスト係数**を割り当て、合計 = Friction Score を算出
2. スコアに応じて 1画面／複数ステップ／要削減 を機械的に判定
3. 質問順を **低コスト→高コスト**に並べる（一貫性の原理で完了率が上がる）
4. 高抵抗項目（電話番号・生年月日）には「なぜ必要か」のマイクロコピーを props で必須化
5. 郵便番号 → 住所自動補完で住所4欄（4.0）を 1.5 に圧縮
6. スコア超過時は Kaito 経由でクライアントへ **「この項目を足すと Friction +2.0、想定 CV -◯%」** と定量提示して削減交渉
...
**なぜ必要か**：
Performance Budget（LCP 2.5s / INP 200ms）は設計書に明記済みだが、**予算のうちサードパーティに何 KB 割くかが未定義**。GTM・チャットボット・Google Maps・SNS 埋込・広告タグは公開直前にクライアントから追加されることが多く、自社コードをどれだけ最適化しても、後から入る 300KB のチャットウィジェットで Budget が一撃で吹き飛ぶ。設計時に枠を切っていないため、Mia が NG を出しても「タグは外せない」で押し切られる。

**具体的手法**：
1. Performance Budget を **自社 / サードパーティに分割**して設計書冒頭に明記
2. 各タグを `next/script` の strategy に**分類表で割り当てる**
3. 重量級埋込（YouTube・Google Maps）は **facade パターン**（静止画＋クリックで初めて実体を読込）を設計で強制
4. Consent Mode v2 前提で、同意前は計測タグを発火させない設計にする（nori 連携）
5. タグを1つ追加するたびに Budget を再計算し、超過なら **Kaito 経由で「どれかを外す」交渉**をルール化
...
|---|---|---|---|---|---|
| GTM（GA4含む） | 計測 | 48KB | afterInteractive | 要（Consent Mode v2） | ✅ |
...
1. **font-size / margin を BP ごとに px 直書きして階段状に切り替える設計**（`text-base md:text-lg lg:text-xl`）→ 中間幅が未設計になり BP 直前直後でガタつく。fluid token（`--step-*`）での連続量指定を必須とし、部品内のタイポ BP 分岐は 0 個で納品する
2. **タップターゲットを 24px 未満・隣接間 8px 未満で並べる設計**（フッターの SNS アイコン列・ナビのテキストリンク詰め）→ WCAG 2.2 SC 2.5.8 違反かつ SP のミスタップ離脱要因。全インタラクティブ要素に最小 44×44px を設計値として与え、密集配置は間隔を設計表に明記する
3. **`container-type` を宣言していない親に `@container` を書く／コンテナを無名で使う設計**→ クエリが常に偽になり無反応、または入れ子で意図しない祖先を掴んで別の枠幅に反応する。CQ を使う部品は必ず `container-name` とセットで設計表に記載し、どのコンテナに属するかを1対1で明示する
4. **日本語本文に `word-break: break-all` を全体適用する設計**→ 長語の折返しは解決するが英単語・URL・型番まで途中で割れて可読性が壊れる。既定は `word-break: normal` + `overflow-wrap: anywhere`、意味単位の折返しが要る見出し・職種名だけ `auto-phrase` を当てる
5. **LOC 20 行未満・props 1個以下・1箇所でしか使わない部品を切り出す過分割設計**（`HeroBadge` `SectionLabel` 等）→ import 地獄と変更影響の追跡困難を招く。粒度スコアの過分割トリガーに該当したら親へインライン統合し、1ページの部品総数は「セクション数×4」を上限とする
...
| 領域 | 強化前 | 強化後 |
|---|---|---|
| タイポ・余白 | BP 3個で px 階段指定 | fluid token 8段（clamp 数式化）／部品内 BP **0 個** |
| ブレークポイント数 | 3（sm/md/lg） | **1 以下**（構造変化のみ）＋ Container Query |
| アクセシビリティ | WCAG 2.1 AA 相当（ARIA 属性中心） | **WCAG 2.2 AA 追加9基準を準拠表で担保**（公共・大手案件応札可） |
| 粒度判定 | 定性ゲート3つ（分割不足のみ検出） | **6指標100点スコア＋過分割トリガー＋総数上限**（双方向検出） |
| 部品の再利用性 | 画面幅前提で枠幅違いに弱い | **配置非依存**（`cqi`＋`auto-fit minmax`）で Wide/Compact 重複部品ゼロ |
| 日本語品質 | 字数レンジと `line-clamp` のみ | **行長・禁則・`auto-phrase`・palt・`text-wrap` の組版規範を全案件標準化** |
| フォーム | 属性9個の網羅 | 属性＋**Friction Score による項目数の定量交渉**（必須3項目・Score 3.5 が理想形） |
| Performance Budget | Lighthouse SLA のみ | **自社120KB / サードパーティ100KB の分割予算＋facade 強制** |
| 差し戻し | Mia 通過率 95%（自己採点先回り） | 中間幅・タップ密集・組版・外部タグ起因の NG を追加で封殺し、**設計起因の差し戻し実質ゼロ**を狙う |

**Nao(LP) の新定義**：CSS 仕様を React 構造へ翻訳する設計者から、**「連続量（fluid）・配置非依存（container）・日本語組版・法適合（WCAG 2.2）・入力コスト・外部依存予算」の6つを数値で握り、実装前に品質を確定させる LP 設計の最終責任者**へ。Ren は迷わず書き、Mia は照合するだけ、クライアントの追加要求には数値で応える状態を常態化する。
