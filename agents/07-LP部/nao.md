...
| Performance | Lighthouse SLA、`lighthouserc.json`、CLS 寸法予約、遅延読込 | ⚠️ サードパーティ枠の予算が未定義 |

**総括**：「分割・境界・状態・トークン」は国内トップ水準。空白は **①寸法の連続量設計 ②WCAG 2.2 ③粒度の定量化 ④配置非依存設計 ⑤日本語組版 ⑥フォーム摩擦の定量 ⑦サードパーティ予算** の7領域。

### 到達すべきベンチマーク

- **Atomic Design**：共有UIのみ Atomic／セクションは feature colocation の折衷（既達）＋ **粒度を定量スコアで判定**（未達）
- **Design Token**：DTCG 準拠 primitive/semantic（既達）＋ **fluid token（clamp スケール）をトークン化**（未達）
- **レスポンシブ設計**：Utopia 方式の fluid typography / fluid space、Container Queries + `auto-fit minmax` で **BP 最小化（3→1）**
- **アクセシビリティ**：WCAG 2.1 AA（既達相当）→ **WCAG 2.2 AA 追加基準を設計表で担保**
...
|---|---|---|---|---|
| 1 | fluid typography / fluid space（clamp 数式）が無く BP ごとに px 固定 | 高（中間幅が未設計・BP前後でガタつき） | 高 | ★1 |
| 2 | WCAG 2.2 追加9基準（2.5.8 / 2.4.11 / 3.3.7 等）が設計表に無い | 高（公共・大手案件の要件化） | 高 | ★2 |
| 3 | 粒度が定性ゲートのみで過分割を検出できない | 中（保守コスト・Ren 実装速度） | 高 | ★3 |
| 4 | Container Queries が言及のみで配置非依存設計の判定基準が無い | 中（再利用性・BP 数削減） | 中 | ★4 |
| 5 | 日本語組版（行長・禁則・折返し・palt）の規範が無い | 高（建設業の長い職種名・社名で崩壊） | 高 | ★5 |
| 6 | フォーム摩擦が属性止まりで「項目数」の定量判断が無い | 高（応募 CV 直結） | 高 | ★6 |
...
**期待効果**：BP 記述が全部品から消え Tailwind クラス量 30% 減。中間幅起因の Mia 差し戻し（「SP で詰まる」「タブレットで間延び」）をゼロ化し、表示/非表示マトリクスの行数も 1/3 に。
...
**実務テンプレート**（WCAG 2.2 AA 準拠表（設計書 必須セクション））：
...
**期待効果**：WCAG 2.2 AA を要件化した公共・大手案件に応札可能化。Mia のキーボード QA / タップ密集 NG を設計層で先回りし、SP のミスタップ離脱を構造的に削減。

#### 3. 【Component Granularity Score（粒度100点スコア＋過分割検出）】

**なぜ必要か**：
既存ゲート（props 5個以下・再利用2箇所・責務1つ）は **分割不足**しか検出できない。逆方向の失敗——LOC 12 行の `HeroBadge.tsx` を1箇所でしか使わない、1ページで 40 個に割って Ren が import 地獄になる——を止める基準が無い。2026-04-29 の「過度に細分化しすぎる」失敗は回避策が「責務1つ」だけで定量判定になっていない。

**具体的手法**：
1. 各コンポーネントを 6 指標で採点し合計 100 点で評価、**過分割トリガー**を別途判定して該当なら親へインライン統合する
2. ページ全体の総数上限を「セクション数 × 4」で縛り、採点表は設計書に添付して Ren にはスコアと判定を渡す
...
**実務テンプレート**（コンポーネント粒度スコア表（STEP 2 完了時に必ず埋める））：
...
**なぜ必要か**：
現状のレスポンシブは全て「画面幅」基準。同じ `JobCard` を SP1カラム・3カラムグリッド・2カラムサイドバーに置くと、画面幅が同じでも枠幅が違うため `md:` で組んだレイアウトが狭い枠で破綻する。結果 `JobCardWide` / `JobCardCompact` の重複実装が生まれ、粒度スコア（#3）の再利用点も落ちる。2026-07-27 に1行触れたきりで判定基準が無い。

**具体的手法**：
1. **CQ / MQ 判定**：その部品が「異なる幅の枠に2箇所以上置かれるか」→ Yes なら Container Query、No（ページ直下の全幅セクション）なら Media Query
2. ラッパに `container-type: inline-size` + `container-name` を付ける（**無名禁止**：入れ子で意図しない祖先を掴む）
3. CQ 閾値は px でなく **rem**（ユーザーのフォント設定に追従）、部品の「必要最小幅」から決める。カード内タイポは `cqi` 単位で枠幅追従
4. **BP を消すグリッド**：`repeat(auto-fit, minmax(min(260px, 100%), 1fr))` で列数を自動決定し `md:grid-cols-3` を全廃
...
**なぜ必要か**：
既存の可変長対策は kotone の「想定字数レンジ」と `line-clamp` のみで、**そもそもの組版規範が無い**。建設業の採用 LP は「型枠大工（未経験・見習い可）」「一級建築施工管理技士」のような分割不能な長語が必ず入り、既定のままだと SP で無意味な位置で折れる／`break-all` を当てて英字 URL まで割れる事故が起きる。日本語 LP を作る以上これは Nao の設計責任であり、Ren の実装センスに委ねる領域ではない。
...
**なぜ必要か**：
既存のフォーム知見は属性（`autocomplete`/`inputmode`/a11y 6属性）と二重送信防止に集中しており、**「そもそも何項目聞くか」の判断基準が無い**。応募フォームで最も CV を落とすのは属性の不備ではなく項目数で、クライアントは「せっかくなので志望動機も」「保有資格も全部」と足したがる。Nao が定量根拠を持たないとこの要求をそのまま設計に通してしまう。

**具体的手法**：
1. 各フィールドに**入力コスト係数**を割り当て、合計 = Friction Score を算出。スコアで 1画面／複数ステップ／要削減 を機械的に判定
2. 質問順を **低コスト→高コスト**に並べ、高抵抗項目（電話番号・生年月日）には「なぜ必要か」のマイクロコピーを props で必須化
3. 郵便番号 → 住所自動補完で住所4欄（4.0）を 1.5 に圧縮
...
**実務テンプレート**（Form Friction 設計表（STEP 3 必須））：
...
**なぜ必要か**：
Performance Budget（LCP 2.5s / INP 200ms）は明記済みだが、**予算のうちサードパーティに何 KB 割くかが未定義**。GTM・チャットボット・Google Maps・SNS 埋込は公開直前に追加されることが多く、自社コードをどれだけ最適化しても後から入る 300KB のチャットウィジェットで Budget が一撃で吹き飛ぶ。枠を切っていないため Mia が NG を出しても「タグは外せない」で押し切られる。

**具体的手法**：
1. Performance Budget を **自社 / サードパーティに分割**して設計書冒頭に明記し、各タグを `next/script` の strategy に分類表で割り当てる
2. 重量級埋込（YouTube・Google Maps）は **facade パターン**（静止画＋クリックで初めて実体を読込）を設計で強制
3. Consent Mode v2 前提で、同意前は計測タグを発火させない設計にする（nori 連携）
...
**実務テンプレート**（サードパーティ予算表（設計書冒頭・Performance Budget と対で記載））：
...
7. **確認用メール再入力欄を設計に含める／Friction Score を計算せずクライアント要望のまま項目を並べる**→ 再入力は WCAG 2.2 SC 3.3.7 違反かつ Friction +1.0 の純粋な離脱要因（1回入力＋可視確認で代替）。志望動機(2.5)＋住所(4.0)＋履歴書添付(3.0) で Score 14 超の応募不能フォームが完成するため、必ず採点し 8.0 超は設計を通さず交渉へ差し戻す
...
| フォーム | 属性9個の網羅 | 属性＋**Friction Score による項目数の定量交渉**（必須3項目・Score 3.5 が理想） |
