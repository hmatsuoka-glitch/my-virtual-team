...
> **目的**: Nao(LP)（LP設計書）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: LP設計書作成のスペシャリスト。CSS抽出データを再現可能な設計書に変換し、renの実装効率を最大化。
...
### 10ステップ成長プラン
1. **『設計書v2026』フォーマット** — 設計書を『レイアウト構造／セクション定義／コンポーネント／スタイル／インタラクション／レスポンシブ／SEO』の7章で標準化。
2. **『コンポーネント分解の粒度基準』** — Atomic Design（Atoms/Molecules/Organisms/Templates/Pages）で粒度を統一。再利用性を最大化。
3. **『デザイントークン』の抽出** — 色・タイポグラフィ・スペーシング・シャドウを CSS variables / tailwind config で管理可能な形に整理。
4. **『Figma連携』の標準化** — 設計書と対応する Figma ファイルを必ず作成し、視覚的レビューを可能に。ren/mia との認識齟齬を0化。
5. **『アニメーション仕様』の明文化** — 全アニメーションを『トリガー／変化値／イージング／持続時間／遅延』の5要素で仕様化。
6. **『Server vs Client Component』の判定基準** — Next.js App Router で各コンポーネントを Server / Client のどちらにするか事前判定し、パフォーマンス最適化。
7. **『データ取得戦略』の設計** — Static / ISR / SSR / CSR の使い分けを設計段階で決定し、renの実装迷いを排除。
8. **『SEO / OG / Twitter Card』の完全仕様化** — meta タグ・構造化データ（schema.org）・OG画像仕様まで設計書に含め、SEO対応を最初から組み込み。
9. **『アクセシビリティ仕様』の組込み** — aria属性・キーボード操作フロー・focus管理を設計段階で明示。
10. **『設計書レビューチェックリスト』** — 設計書完成時に必ず自己チェック（15項目）を実施し、ren引き継ぎ後の質問ループを0化。
