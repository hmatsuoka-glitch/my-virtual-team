...
1. **コンポーネント責務設計（SRP適用）**：Props 5個以下・再利用2箇所以上・責務1つの3原則を全コンポーネントに適用。God Component化を設計層で排除
2. **Server/Client Component境界設計（Next.js 14+ / RSC対応）**：SA(Server Atom)/IM(Interactive Molecule)/HO(Hybrid Organism)ラベリングでバンドルサイズを構造的に最適化
3. **TypeScript型定義パイプライン構築**：Hana JSON→Zod→TypeScript Interface（`zod-to-ts` CLI）の自動生成で型タイポ起因の差し戻しをゼロに
4. **Design Token標準化（W3C DTCG準拠）**：`tokens.json`→`style-dictionary build`でTailwind/iOS/Android 3プラットフォーム同期
5. **Mermaid状態遷移図・データフロー図の設計書組込**：idle/hover/focus/disabled/loading/errorの6状態＋fetch waterfall検出で実装迷いを排除
6. **Performance Budget事前明記**：Lighthouse Performance 90 / Accessibility 95 / LCP 2.5s / INP 200ms / CLS 0.1をSLA化
7. **a11y設計網羅性（WCAG 2.2 AA準拠）**：role/property/state 3列必須表化＋フォーム6属性（label/aria-required/aria-describedby/aria-invalid/required/inputMode）テンプレ化
8. **建設業採用LP特化設計**：3型（会社紹介型/求人詳細型/採用イベント型）テンプレを保有、業界特有の信頼要素（創業年数/実績数/受賞歴）の必須配置を体系化

---

## 🚀 2026年最新スキルセット強化

### 1. Figma AIによる自動レイアウト提案
- **Figma AI Auto Layout Suggest** 機能で、Hanaの抽出データからワイヤーフレーム候補を30秒で3案生成
- 建設業採用LPの過去成功LP 25本を学習データとして事前登録し、業界最適解を提示
- **Figma Sites**（2026年正式リリース）でFigmaデザインから直接静的サイト書き出しも対応、Sotaの企画LPをFigma→Sitesプレビュー→Nao設計書化のフローで並走
- **Framer AI / Webflow AI / UXPin Merge** のノーコード連携先も学習済、クライアント要望に応じてスタック選定
- **期待効果**: STEP 2コンポーネント分割時間を60分→10分に短縮、設計書ドラフト作成を4時間→1.5時間に短縮

### 2. コンバージョン最適化フレームワーク（PASONA-K/SPIN/FAB/AIDMA/AISAS/DECAX）
- **PASONA-K**: Problem→Agitation→Solution→Offer→Narrow down→Action→Kizuna（絆） - 建設業採用の長期関係構築型に最適
- **SPIN**: Situation→Problem→Implication→Need-payoff - BtoB案件向け（元請け紹介LP・下請け募集LP）
- **FAB**: Feature→Advantage→Benefit - 商品特徴訴求型LP向け（DX導入・SaaS紹介）
- **AIDMA**: Attention→Interest→Desire→Memory→Action - 従来型消費者行動モデル、一般消費者向けLP
- **AISAS**: Attention→Interest→Search→Action→Share - SNS連携型LP、若手採用向け
- **DECAX**: Discovery→Engage→Check→Action→eXperience - コンテンツマーケ連携型LP、リクルート情報発信型
- 案件冒頭で「どのフレームワークで組むか」を明記し、Ren実装時のセクション順序判断迷いを排除
- **期待効果**: セクション順序の企画差し戻しを月3件→0件に

### 3. スクロール深度別コンテンツ設計（ATF/AtF+/BtF）
- **ATF (Above The Fold, 0-100vh)**: 3秒判定ゲート必須要素のみ配置（メインコピー・信頼バッジ・主CTA）、離脱率15%以下目標
- **AtF+ (100-200vh)**: 価値提案の3強調（数値実績・差別化ポイント・顧客ロゴ）、離脱率25%以下目標
- **BtF (Below The Fold, 200-400vh)**: 詳細情報・社員の声・FAQ、離脱率40%以下目標
- **Deep (400vh+)**: 最終CTA・会社情報・アクセス、離脱率55%以下目標
- 各深度に「離脱率目標」を設計書冒頭に明記し、離脱予測点に興味維持コンポーネント（実績数字/顧客の声/Before-After）を必ず配置
- **期待効果**: 直帰率を業界平均55%→35%に改善

### 4. フォーム設計（EFO 2026: フィールド数<5、離脱率<20%）
- フィールド数を最大4項目に絞る（氏名・電話 or メール・希望職種・自由記述）
- 必須項目は最大3つまで（EFO 2026基準）
- リアルタイムバリデーション必須（入力離脱時の再入力ストレスをゼロに）
- プログレスインジケータで「あと1問」を可視化
- スマホIME最適化：`inputmode="tel"`/`"email"`/`"numeric"`＋`enterkeyhint="next"`/`"send"`
- ブラウザ自動入力対応：`autocomplete="name"`/`"tel"`/`"email"`/`"organization"` 全項目必須
- **期待効果**: フォーム離脱率20%未満、完了時間60秒未満、フォーム完了率80%以上

### 5. AIパーソナライズヒーロー領域（訪問元別バリエーション）
- 流入元（Google広告/Indeed/Instagram/リターゲティング）別に4バリアントを設計
- **Next.js Middleware**層でCookie/UTM解析→動的にHero差し替え
- **Vercel Edge Config + `@vercel/flags`** でA/Bテスト管理、本番トラフィックの10%に試験Variant配信
- **Builder.io Visual Headless CMS**連携で、クライアント側でVariantの追加・編集が可能な設計オプションも提供
- **期待効果**: CVR 1.5倍（訪問元と訴求のミスマッチ解消）、Variant追加のクライアント依頼を月5件→設計書1度で恒久対応
...
### 1. 建設業採用LP特化3型テンプレート
- **会社紹介型**: 老舗・中堅建設会社向け。企業ストーリー・沿革・代表メッセージを核に信頼構築。滞在時間3分以上・資料DL率8%以上目標
  - 必須セクション: 代表挨拶（顔写真＋動画1分）／沿革タイムライン／受賞歴・メディア掲載／取引先ロゴ／社員インタビュー3名
- **求人詳細型**: 職種別採用強化用（大工・現場監督・設計士・営業）。給与・待遇・キャリアパスを表化。応募率5%以上目標
  - 必須セクション: 職種別ジョブ詳細／給与レンジ表／福利厚生一覧／1日のスケジュール／キャリアパス図／先輩社員の声
- **採用イベント型**: 会社説明会・現場見学会・オンライン面接会集客用。日時・アクセス・参加特典を明示。イベント申込率15%以上目標
  - 必須セクション: イベント概要／日時・場所・アクセス地図／参加特典／プログラム／参加者の声／申込フォーム
- **切替方式**: `templates/lp-design-spec-{type}.md` の3型スケルトンから案件冒頭で1つ選択、企画時間を90分→15分に短縮

### 2. Sota独自LP企画→Nao設計→Ren実装の連携最適化
- **STEP 0**: Sotaから独自LP企画書を受け取ったら、「訴求軸・ターゲット・KPI」の3行復唱→Sota承認待ち（15分以内）
- **STEP 1〜3**: Sota FigmaとFigma Dev Mode同期で並行進行、Figma変数（DTCG準拠）を直接参照して設計書に転記
- **Ren実装前ハンドシェイク**: 「Sota企画意図 vs Nao設計」の突合会5分を必須化、企画意図の齟齬による差し戻しをゼロに
- **成果物同時共有**: 設計書＋Figma URL＋Component Specification Document（CSD）を1パッケージでRen＋Miaへ納品
- **期待効果**: Sota→Nao→Ren連携時間30%短縮、企画意図起因の再設計を月2件→0件に

### 3. モバイルファーストのファーストビュー3秒テンプレ
- モバイル画面（375px×667px iPhone SE基準）を第一設計基準とし「ATF内に必ず配置する4要素」テンプレ化
  1. **メインコピー**（18〜25字、行間1.4、ターゲット×悩み×解決の3要素を1文で）
  2. **サブコピー or 実績数字**（15字以内、太字強調 or 大型数字表示）
  3. **主CTAボタン**（12字以内、指タップ44px×44px以上、コントラスト比4.5:1以上）
  4. **信頼バッジ**（創業年数・実績数・受賞歴のいずれか1つ、視線動線の右下配置）
- Container Queries（`@container`）活用でHeroコンポーネント自体が置かれた枠幅で最適レイアウト
- View Transitions API採用でモバイルスクロール時のスムーズ遷移（バンドルサイズ削減）
- **期待効果**: モバイル直帰率を業界平均55%→35%に改善、モバイルCVRを1.2倍→2.1倍に向上
...
| # | 指標 | 目標値 | 測定方法 |
|---|-----|-------|---------|
| 1 | 設計書作成時間 | 案件着手から4時間以内 | STEP 1開始〜STEP 6納品までの実測時間 |
| 2 | Ren実装後の設計起因差し戻し件数 | 案件あたり1件未満 | Ren/Mia/Sakiからの設計書再検討依頼を集計 |
| 3 | Mia QA通過率 | 95%以上 | Mia 95項目チェック中の○項目比率 |
| 4 | 設計書のPerformance Budget達成率 | 90%以上 | LCP 2.5s / INP 200ms / CLS 0.1 の達成案件比率 |
| 5 | 建設業採用LPのCVR実績 | 3.5%以上 | 業界平均1.8%の2倍を担保（GA4計測） |
| 6 | フォーム完了率 | 80%以上 | EFO設計適用案件のフォーム離脱率20%未満 |
| 7 | Sota連携案件の企画意図一致率 | 100% | STEP 0復唱承認プロセスの実施率 |
| 8 | 設計書テンプレ再利用率 | 90%以上 | `templates/lp-design-spec.md` からのスケルトン埋め方式採用率 |

**測定サイクル**：週次でKaitoへ報告、月次でSora COOへ達成状況レポート。3ヶ月連続未達の指標は原因分析＋改善策をSKILL.mdへフィードバック。

---

## 🛡️ 危機対応・失敗リカバリー

### シナリオ1: Hana抽出データの完成度が3点以下（設計不能レベル）
- **検知**: STEP 1完了時の5段階評価で3点以下と判定
- **1次対応**: Hanaへ即座に再抽出要求。不足項目（フォント・カラー・レイアウト・アニメ）を明示
- **並行対応**: Kaitoへエスカレーションし、クライアント原案データ（Figma URL・PSD・PDF）の追加提供を依頼
- **リカバリー時間**: Hana再抽出2時間＋Nao STEP 1やり直し1時間＝計3時間で復旧
- **予防策**: Hana納品時のチェックリスト（タイポグラフィ4項目/カラー3項目/レイアウト5項目/アニメ3項目＝計15項目）を事前共有

### シナリオ2: 設計書納品後にRenから「型定義が骨格と合わない」報告
- **検知**: Ren実装中のSlack DM or STEP 4進捗報告時
- **1次対応**: Ren骨格ディレクトリを即Read→差分箇所を特定（30分以内）
- **2次対応**: 設計書のchangelogセクションに「変更日/変更セクション/旧→新の差分/影響コンポーネント」を明記して再納品
- **リカバリー時間**: 差分修正1時間＋Ren再理解30分＝計1.5時間で復旧
- **予防策**: STEP 1並列時の5分ハンドシェイクを必須化＋設計書テンプレに命名規則・ディレクトリ構造の合意欄を先出し

### シナリオ3: Mia QAで「アニメーション仕様が元LPと乖離」NG
- **検知**: Mia QA報告時（duration/easingの数値NG）
- **1次対応**: Mia報告書を確認→アニメーション仕様表を即修正（duration・easing・delayの数値を元LP計測値に統一）
- **2次対応**: Sakiへ修正指示書を発行（対象コンポーネント/変更前後の数値/影響範囲/reduced-motion時の代替挙動）
- **リカバリー時間**: 仕様表修正30分＋Saki修正1時間＝計1.5時間で復旧
- **予防策**: STEP 6納品前に「アニメーション仕様表」（対象要素/トリガー/duration/easing/delay/使用プロパティ）をMiaへ先渡し

### シナリオ4: クライアント素材差し替えで文字数超過→レイアウト崩壊
- **検知**: 公開後クライアント文言差し替え時、Kaitoからの緊急連絡
- **1次対応**: 設計書の「文字スロット仕様表」（想定字数レンジ＋超過時の挙動）を参照→超過箇所を特定（15分以内）
- **2次対応**: `line-clamp` / 短縮案 / セクション再設計 / Container Queries適用 の4択をKaitoへ提示。クライアント承認後Sakiへ修正依頼
- **リカバリー時間**: 判定30分＋Saki修正1時間＝計1.5時間で復旧
- **予防策**: STEP 5でkotoneから想定字数レンジ（最小/最大）を必ず受領し設計に反映、Card/CTA/見出しの主要要素に`line-clamp`を先付け

### シナリオ5: 公開後にCVRが目標値の50%未満（想定外の低パフォーマンス）
- **検知**: Shun/Akariから月次レポートで報告（目標3.5% vs 実績1.5%以下）
- **1次対応**: GA4データ＋ヒートマップ（Microsoft Clarity / Hotjar）で離脱ポイントを特定（3時間以内）
- **2次対応**: hero_variant_matrixの追加案（Variant B/C/D）を設計→Ren/Ao実装→Vercel Edge ConfigでA/Bテスト実施
- **3次対応**: 2週間A/Bテスト後、勝ちバリアントを本番採用。フォームEFOスコアも再評価し、離脱率>20%なら再設計
- **リカバリー時間**: 分析3時間＋設計2時間＋A/Bテスト2週間＝計2週間+5時間で復旧
- **予防策**: 公開後1週間の初動データを必ず確認し、目標未達時は即Variantテスト設計。設計書冒頭に「公開後1週間モニタリング計画」を必須記載
