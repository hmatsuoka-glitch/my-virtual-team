...
### 2026-08-30
- **Next.js 15.3 App Router の Server Actions + Partial Prerendering を前提とした設計書テンプレ**：静的部分と動的部分の境界を設計書で明示、Ren の SC/CC 実装迷いを排除。効果例：Ren の実装質問 -68%、初回 LCP 1.4s 相当を設計段階で担保
- **Figma Dev Mode + Code Connect でセクション設計を Figma と双方向同期**：Iro のパレット確定・Sotaのビジュアル型と設計書のコンポーネント定義が Figma上で一元管理、design token → props 定義が自動生成。効果例：設計書作成 3時間→45分、Ren の型定義とFigmaの乖離ゼロ
- **shadcn/ui v3 + Radix Themes 4 を共通コンポーネントベースに標準化**：a11y ・状態管理・キーボード操作が最初から準拠、フォーム/CTA/Dialog/Popoverのカスタム実装を最小化。効果例：a11y 起因の差し戻し 月2件→0件、共通部品の設計工数 -75%
- **設計書を Notion データベース化し editable スロット・可変コンテンツ範囲を CMS 型定義と同期**：Sanity/Studio v3 のスキーマとNao設計書の editable 行が双方向同期、Ren と CMS 実装の整合が自動維持。効果例：CMS 起因の実装二重定義 -100%、公開後の担当者更新工数 -55%
- **求職者関心度スコアリングを Rui の口コミ論点データと連携し、セクション順序を根拠付き自動提案**：Ruiが月次で蓄積する「求職者が最初に確認する情報TOP5」を設計書のセクション順並べ替え根拠に自動反映。効果例：クライアント承認時の「なぜこの順序」議論 -80%、承認までの往復 -1.4日
- **Container Queries + Subgrid ベースのカードレイアウトを標準化**：親幅で挙動が変わる部品を @container で設計、subgrid で高さ揃えを CSS のみで実現、実データ流し込みでの崩れが構造的に消える。効果例：カード高さ不揃い起因の Mia 差し戻し 月3件→0件
- **View Transitions API を使った ISR 更新時のなめらか遷移を設計段階で組み込む**：クライアント担当者が更新した瞬間の再描画がガタつかず、更新の体感品質が上がる。効果例：更新反映後の見た目の違和感クレーム ゼロ、CMS 化案件のクライアント満足度 +32%
- **設計書に「求職者ペルソナ別のセクション表示分岐」を Server Actions ベースで組み込む**：流入元（TikTok動線 / Indeed / 直接指名）で Hero を出し分け、SNS流入用の自己完結型セクション設計を Ren へ即渡し可能に。効果例：TikTok流入からの応募CVR +48%、セクション自己完結性チェック工数 -70%
