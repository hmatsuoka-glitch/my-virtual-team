...
### 2026-08-30
- **Chrome DevTools MCP + Playwright 1.50 で computed style ダンプを全ブレークポイント一括取得**：320/375/768/1024/1440/1920px の6サイズで同時抽出、レスポンシブ差分を1回のクロールで完全網羅。効果例：STEP 2〜5の抽出時間 90分→18分、ブレークポイント別の差分見落とし率 -95%
- **Claude 5 の視覚解析でスクリーンショットから未使用CSSと Above the fold の主要素を自動判別**：DevTools Coverage と併用し、複製に不要なコードを事前排除、Ren の実装コスト削減に直結。効果例：Ren に渡すCSS量 -42%、初期LCP スコア 2.8s→1.4s 相当
- **@property CSS 変数と @container Query の抽出対応**：モダンLPは@container（コンテナクエリ）で親幅ベースのレスポンシブを組む例が増加、@media 単独ではレイアウトが壊れる。効果例：モダンLP複製の初回忠実度 78%→96%、Mia差し戻し件数 -68%
- **View Transitions API・Popover API・Anchor Positioning の検出フラグ追加**：Chrome 130+ で普及した新API群を STEP 5/7 で検出、Ren に「polyfill 要／不要」を明示。効果例：新API絡みの複製漏れによる差し戻し 月3件→0件
- **Iro との `tokens.json` を Style Dictionary 4 経由で Figma Variables と双方向同期**：Iroが Figma で色を変更すると tokens.json が自動更新、逆も可。効果例：色定義の乖離事故ゼロ、Iro/Hana/Ren の3者間キー整合維持 100%
- **Playwright の A11y スナップショットで hover-only/focus-only コンテンツを機械検出**：`hover_only_content`・`focus_visible_missing`・`tap_target_warning` フラグを目視でなく自動生成。効果例：フラグ立て漏れ 8件/案件→0件、Shun への段階別引き渡しが即時可能に
- **JIS Z 8779 + APCA Lc 準拠のコントラスト検査を Node.js で自動化**：屋外閲覧を想定した Lc 60以上判定を全テキスト要素で自動チェック、`outdoor_readability_risk` を機械立て。効果例：Kaito 向け改善提案リストの精度 +60%、Ryota が転記できる状態で納品
- **抽出成果物を Notion データベース化して LP案件横断で再利用可能に**：全過去案件の tokens・フラグ・改善提案候補を検索可能に、同業界クライアントの2本目以降で共通トークン化がさらに加速。効果例：同クライアント2本目LPのSTEP 2〜3工数 -78%、業界横展開時の色相選定 -55%
