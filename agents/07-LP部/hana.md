...
### 2026-09-04
- **Tailwind v4のCSS-first config（`@theme`ディレクティブ）検出をSTEP 7で必須化し、Renへ`extend`ベースの旧設定と混同させない**：Tailwind v4は`tailwind.config.js`でなくCSS内`@theme { --color-brand: ...; --spacing-lg: ...; }`でトークンを定義する方式が主流化。STEP 7の技術スタック特定でTailwindを検出したらv3（config.js）かv4（`@theme`）かをバージョンまで判別し、v4案件は`tokens.json`のキー体系（2026-08-18参照）をそのまま`@theme`宣言へ流し込める形で納品する。v3前提でRenが`extend.colors`実装するとv4案件で二重定義になり、後で全書き換えになる。`json-to-theme.js`変換（2026-06-23参照）もv3/v4の2モード出力に分岐させ、Kaitoのデプロイ時点でRen側の設定判断を消す
- **Chrome DevToolsの「Copy styles as fresh CSS」で採取値の宣言化を機械実行し、生CSSテキスト走査（2026-07-07参照）の重複作業を消す**：2026 DevToolsに追加された「Copy styles as fresh CSS」は、選択要素のcomputed値から非デフォルト・非継承分だけ抽出して整形済みCSSを吐く機能。Puppeteerでこのコマンド相当を各要素へ発火し、`getComputedStyle`一括ダンプ（2026-08-18参照）と並列で走らせると、Renが読める形の宣言済みCSSと解決値ペア（2026-06-26参照）が同時に出る。目視でDevToolsをコピーする作業がゼロになり、Nao向け構造ダンプ／Ren向け要素ダンプの2系統納品（2026-08-27参照）の整形コストも消える。抽出スクリプトのSTEP 8ワンコマンド化（2026-09-01参照）にこの機能相当を組み込む
- **`light-dark()`関数の使用有無をSTEP 2で検出し、Iroとのダーク版設計の合意軸を「メディアクエリ2系統」から「単一トークン内`light-dark()`」へ寄せるかを5分会で決める**：色を`color: light-dark(#111, #eee)`のように1行で書ける`light-dark()`関数が実装現場で普及。元サイトが`light-dark()`採用ならライト/ダーク2系統採取（2026-07-03参照）を1行1トークンで表現でき、`tokens.json`の`--color-*`キー（2026-08-18参照）も2倍にならない。STEP 1で`prefers-color-scheme`ブロックを検出した後（2026-07-16参照の即共有と同じタイミング）、`light-dark()`使用も併せて共有し、Iro設計版のダークをどちらの表現で受け渡すかを役割分担5分会（2026-08-27参照）の議題に足す。`color-scheme: light dark`宣言の有無もセットで採り、Ren実装時のトークン参照回数が半分に減る
- **`@starting-style`による初回マウント時アニメの検出をSTEP 5に足し、JSの`useEffect`エントランス演出との切り分けをRenへ明示する**：モーダル・トースト・ポップオーバー出現時のフェードイン等が、JS制御なしにCSSの`@starting-style { opacity: 0 }`だけで実現できる仕様が普及。STEP 5でこれを見落として初期値だけ採取するとRenがJSでエントランスを再実装し、View Transitions（2026-07-27参照）・scroll-driven animations（2026-06-22参照）と同じ「脱JS化可否の判定軸」を1つ落とす。`popover`属性・`dialog[open]`要素・CSS Anchor Positioning（2026-07-27参照）と組で検出し、`@starting-style`ブロックの初期値と定常値の差分を仕様書に記録。フォールバックは`@supports (transition-behavior: allow-discrete)`で判定してRenへ添える
- **コンテナクエリのrange syntax（`@container (400px < width < 800px)`）とstyleクエリのアニメ連動をSTEP 6で個別採取し、`@media`のブレークポイント表と混ぜない**：コンテナクエリ（2026-07-01参照）とstyleクエリ（2026-08-03参照）は普及に伴い、レンジ構文（範囲指定）と`transition`との連動で「特定範囲でのみ挙動を切替える」設計が増えた。STEP 6のブレークポイント抽出で`@media`とコンテナ/styleクエリを同一テーブルに書くと、Ren実装時にビューポート幅ベースで実装されて親コンテナ幅ベースの意図が壊れる。ブレークポイント表を「①ビューポート `@media` ②コンテナ `@container (size)` ③スタイル `@container style()`」の3系統に分けて納品し、レンジ構文は上限・下限・排他/包含（`<` か `<=` か）まで記録する。Nao向け構造ダンプ（2026-08-27参照）の「セクション単位」列に紐付けてトリガー元コンテナ（`container-type`宣言祖先）も書く
