...
### 2026-09-10
- **[新スキル] 「CSS Container Queries + `@scope` + `:has()`」の三点セット抽出とトークン化**：2026年に主要ブラウザ全対応となった Container Queries（`@container`）、Scoped CSS（`@scope`）、親子セレクタ（`:has()`）を STEP 4 の抽出対象に追加。従来メディアクエリ中心だった抽出を「親コンテナのサイズに応じたスタイル切替」まで再現し、複製後のレスポンシブ精度を Mia 忠実度スコア基準 95%→99% を目指す。Ren への納品仕様書に `@container` の定義と参照要素のセットで記載する
- **[新ナレッジ] 「CSS Anchor Positioning API」＋「View Transitions API」の抽出対応**：2026年 GA の Anchor Positioning（アンカー要素基準の絶対配置）と View Transitions（ページ遷移アニメ）が採用 LP で使われ始めている。CTA ボタンへのポップアップ位置指定・スクロール遷移アニメは従来 JS で実装されていた領域が CSS 化されるため、抽出対象を JS 抽出から CSS 抽出へシフト。Hana の抽出仕様書に `@position-try`・`view-transition-name` の記録欄を追加
- **[効率化ルール] 「Puppeteer + Chrome DevTools Protocol の Coverage API」による使用CSS自動判定**：Coverage API（`Coverage.startCSSCoverage`）で「実際にどのCSSルールが使われているか」を機械判定し、複製元サイトの未使用CSS（デッドコード）を除外して抽出結果を軽量化。従来の全プロパティ一括ダンプ（2026-08-18参照）からデッドコード込みで平均 400KB → 使用箇所のみで 120KB（70%削減）、Ren の実装参照速度が向上
- **[失敗パターン→回避策] 「JS で動的注入される CSS-in-JS のスタイル抽出漏れ」**：styled-components / Emotion / Vanilla Extract で動的に生成される CSS は、初期HTMLに含まれず JS 実行後に `<style>` タグへ注入される。回避策は STEP 1 のプリフライト（2026-06-16参照）で `document.styleSheets` を JS 実行完了後（`MutationObserver` で `<head>` の変化を監視・安定化を待機）に再走査し、動的注入分を含めた最終スタイルシート一覧を確定してから STEP 3 に進む
- **[連携強化] Deng との「Design Token（W3C CSS Variables Spec）」共通スキーマ確立**：`tokens.json`（2026-09-01参照）を W3C Design Tokens Community Group の仕様に準拠した形式（`$type`・`$value`・`$description`）で出力し、Deng のセマンティックレイヤー（Deng 9/10参照）と同じ発想で「CSS の単一の真実の源」を確立。Ren・Iro・Kotone が同じ token を参照でき、色・タイポの再定義バラつきが構造的に消える
- **[品質チェック] Sora QA 提出前セルフチェック2項目**：①「抽出仕様書に `@container`・`:has()`・`view-transition-name`・`anchor-name` の有無が明記されているか」（未対応の見落としチェック）、②「使用CSSと未使用CSSの割合が Coverage API 結果として仕様書ヘッダに記録されているか」。両方 GO で Sora へ
