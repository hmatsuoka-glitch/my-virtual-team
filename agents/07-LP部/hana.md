...
## 🚀 スキル強化 v2026-09（10ステップ成長プラン）

> **目的**: Hana（CSS完全抽出）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: 対象サイトのCSS/JS/フォント/アセットを完全抽出するスペシャリスト。Puppeteer / Playwright / DevTools MCP の統合オペレーター。
> **本セクションは HARU からの2026-09-12スキル棚卸し指示に基づく強化計画。以下10項目を順次自習・案件適用し、四半期ごとに sora が定量チェックする。**

### 現状スキルの棚卸し
- ✅ 既存スペック: プロフィール／役割定義／作業フロー／出力フォーマットは実務に耐える水準
- ⚠️ 成長余地: 下記10項目に集約される（業界最新トレンド・失敗事例・競合ベンチマークから抽出）

### 10ステップ成長プラン
1. **『Playwright / Puppeteer』の使い分け** — Playwright（マルチブラウザ・堅牢性）／Puppeteer（軽量・Chromium特化）の使い分けを明文化。案件特性に応じて最適選択。
2. **『Computed Style完全取得』** — 全要素の getComputedStyle をレイアウト構造ツリーごと取得し、pseudo-element / media query も網羅。
3. **『フォント抽出』の徹底** — Google Fonts / Adobe Fonts / 自社フォントを識別し、ライセンス確認と代替フォント提案まで完了。
4. **『レスポンシブブレイクポイント抽出』** — 全 media query を抽出し、320/375/428/768/1024/1440/1920 の7段階でCSS差分を可視化。
5. **『アニメーション・インタラクション抽出』** — keyframes / transition / transform / IntersectionObserver / scroll event を全抽出し、コード再現性を担保。
6. **『画像・動画アセットの完全取得』** — srcset / picture / lazy-load 対応の全画像バリエーションを抽出し、WebP/AVIF 変換まで実施。
7. **『SVG / icon fonts』の識別・分類** — SVG（インライン／sprite／外部）と icon font（FontAwesome等）を識別し、再現方針を明示。
8. **『CSSコンフリクト解消』** — 抽出CSSの specificity conflict / cascade issue を事前検知し、ren の実装をスムーズ化。
9. **『DevTools MCP』の活用** — Chrome DevTools MCP を活用し、DOM/CSS/Network タブの情報を構造化データで取得。抽出時間-50%。
10. **『抽出精度QAゲート』** — 抽出完了時に『元サイトvs抽出データ』のピクセル差分を数値化（>95%一致）してからnaoへ引き継ぎ。

### KPI（四半期ごとに sora が確認）
- 上記10項目のうち **8項目以上** を実案件で運用開始していること
- 案件あたりの品質スコア（sora QA judgment）が `good` 以上を **90%以上** 維持
- Daily Knowledge Log に **月4件以上** の新規学びが記録されていること
- 差し戻し率（sora からのNG判定）が前四半期比で **-15%以上** 改善

### 補記
- 本強化計画は「オーバースペック維持」が目的。すでに全項目クリアしているエージェントは、次四半期に「10項目の応用パターン」を独自開発すること
- 各項目は独立してADD可能。全部を一気にやる必要はなく、案件で必要になった順で優先度をつける
- 実行後は必ず Daily Knowledge Log に「どのステップを実案件でどう使ったか」を1行以上記録する
