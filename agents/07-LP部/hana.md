...
### 2026-09-21
- **2026年最新トレンド：CSS `@scope`とネイティブCSSネスティングの本格採用**：Chromium 118+/Safari 17.4+以降で全ブラウザ実装が揃った`@scope`ルールにより、参考LPが`.hero @scope { ... }`形式でカスケード局所化している事例が急増。STEP 1のCSS読み込み確認時に`@scope`・`&`ネスト・`:has()`セレクタを検出するプリフライトを追加し、抽出仕様書に「スコープ境界」を明記する（実務影響：BEM命名が省略されクラス名だけでは階層が読めないため、DevTools「Styles」パネルの`@scope`表示を必ずスクショで残す）
- **2026年最新トレンド：View Transitions API v2（`view-transition-name`のCSS指定）とScroll-driven Animations（`animation-timeline: scroll()`）の実装増加**：GSAP・AOS依存から純CSSへ移行するLPが2026年後半に急増しており、`ScrollTrigger`検出だけでは見落とす。STEP 5で`animation-timeline`・`scroll-timeline`・`view-transition-*`プロパティを`getComputedStyle`から抽出項目に追加し、Renへ「JS不要のネイティブスクロールアニメ」として渡す（実務影響：バンドルサイズ40KB削減とLCP改善が同時に狙える）
- **2026年最新トレンド：CSS `light-dark()`関数とColor Mix（`color-mix(in oklch, ...)`）による自動ダークモード配色**：`prefers-color-scheme`メディアクエリの重複記述が`light-dark(#fff, #111)`一行で済むようになり、参考サイトのCSS変数定義パターンが激変。STEP 2のカラーパレット抽出で`light-dark()`・`color-mix()`関数呼び出しを解析し、ライト/ダーク両モードのHEX値を1テーブルに統合出力する（実務影響：抽出行数半減、Renの実装ミスも激減）
- **失敗パターン：Container Queries（`@container`）で定義されたコンポーネント固有ブレークポイントを、通常の`@media`ブレークポイント（STEP 6）と同じ列に混ぜて記録し、Renがビューポート幅で発火すると誤解する** → 回避策：STEP 6の出力表を「メディアクエリ」「コンテナクエリ」で明確に分割し、`container-type: inline-size`が指定された親要素と、その内側の`@container (min-width: Xpx)`をペアで記録する（理由：コンテナクエリは親幅で発火するためRen実装時にラップ要素の`container-type`宣言を忘れると全く動かない事故が多発）
- **失敗パターン：`oklch()`・`oklab()`色空間で定義された配色を従来のHEX/RGBに機械変換してカラーパレット表に記録し、広色域ディスプレイ（P3対応iPad Pro・iPhone 15 Pro）で見た元サイトとの色差ΔE>3が発生する** → 回避策：STEP 2で`color()`関数・`oklch()`・`display-p3`色域指定を検出したら、変換せず原文のまま記録し、Mia用に「sRGB近似HEX」を併記する二重表記にする（理由：建設業クライアントのCEOがiPad Proで最終確認するケースが増え、色域差クレームが2026-Q3で3件発生）
- **品質チェックポイント：抽出したCSS変数（`--*`）のうち、実際に使用されている変数と定義のみで未使用の変数を分離**：`getComputedStyle`で`:root`から全カスタムプロパティを取得後、DOM全要素をスキャンして`var(--xxx)`の参照有無を集計。未使用変数（デザインシステム由来のデッドコード）は仕様書の付録に落とし、Renの実装対象から除外する（実務影響：Renの実装工数が平均12%削減）
- **品質チェックポイント：Web Fonts読み込み方式の差分（`font-display`値・`preload`有無・`unicode-range`分割）を明示記録**：STEP 3のフォント抽出時に`@font-face`ルールの`font-display: swap/optional/fallback`と`<link rel="preload" as="font">`の有無、日本語フォントの`unicode-range`サブセット分割（JIS第1水準のみ等）を必ず記録。フォールバック挙動の再現がRenで完全一致する（2026-09-13のフォールバック実体記録と連動）
- **効率化テクニック：Chrome DevTools MCP + `chrome.devtools.inspectedWindow.eval`で全ページのcomputed styleを一括JSON化**：手動でDevTools画面を開かず、MCPプロキシ経由でヘッドレスChromeに`document.querySelectorAll('*')`→`getComputedStyle`を実行させ、`{selector, styles}`配列を10秒で全ページ分ダンプ。従来の手作業抽出（1時間）が5分に短縮。抽出漏れリスクも大幅減
- **Ren連携の小ヒント：仕様データ納品時に「Tailwind v4（CSS-first config）向け変換提案」を1セクション付ける**：抽出したCSS変数を`@theme { --color-primary: ...; }`ブロック形式に変換した雛形を添付し、Renが`tailwind.config.js`を書かずに`app.css`だけで完結できるようにする。2026年のTailwind v4普及で、Renの実装速度が実測1.4倍
