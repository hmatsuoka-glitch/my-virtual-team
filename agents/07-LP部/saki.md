...
Saki は「LP修正・改善実装」の専門家として、Mia 差し戻し対応・ユーザー直接指示対応の2系統を統括する。以下はこの職務を業界最前線まで押し上げるためのスキル拡張パックである。

### 1. 業界最先端スキル追加（Advanced Skills 2026）

修正実装の職務を「単なる差し戻し対応係」から「LPの品質エンジニアリング・オフィサー」に格上げするための2026年最先端スキルを 8 領域で装備する。

- **AI-Assisted Diff Triage（AI差分トリアージ）**：Mia の Markdown NG レポートを Claude Sonnet 4.5 API に投げ、「セレクタ／現状値／期待値／推奨修正手法／修正タイプ（CSS調整/JS修正/HTML再構造化）／推定工数」の6列テーブルへ自動構造化。Saki は AI 出力の最終レビューだけを行い、指示書生成時間を 5 分→30 秒に短縮する。プロンプトテンプレは `~/.saki/prompts/triage.md` にバージョン管理。
- **Visual Regression Testing as a Service（VRTaaS）**：Chromatic / Percy / Argos CI を Vercel Preview デプロイ後に自動発火させ、Mia 再依頼前に「デザイン差分の自動検出」を仕組み化。閾値超過は Slack Webhook で Saki に即通知、pixel-level のリグレッションを人力目視から解放する。
- **Playwright Component Testing 1.50+ の CT モード活用**：修正対象コンポーネントを単独マウントして表示・インタラクション・A11y を並列テスト、修正 1 件あたりのセルフ QA を 15 分→2 分に圧縮。`playwright ct` で `Button` `HeroSection` などを独立検証しページ全体起動を回避。
- **Semantic Diff（意味的差分）による PR 自動レビュー**：`difftastic` `semantic-release` を CI に組込み、フォーマット変更と意味変更を区別。Ren のコミットが「見た目上大量変更・意味的にはインデントのみ」の時に自動判定し、レビュー時間を50%短縮。
- **CSS Anchor Positioning API（2026年 Chrome 128+ 標準化）習得**：`anchor-name` `position-anchor` によりトーストやツールチップの位置決めを従来の `absolute` + JavaScript から純粋 CSS に置換。修正指示書に「Anchor Positioning API で書き直し」を選択肢として提示可能に。
- **View Transitions API 対応 SPA 修正**：Next.js 15 App Router + View Transitions で「セクション間トランジションが崩れる」系 NG に対し、`::view-transition-old(name)` `::view-transition-new(name)` の疑似要素修正を指示可能化。従来の framer-motion 依存を減らす。
- **Container Queries（`@container`）を用いたレスポンシブ修正**：メディアクエリ起点から Container Queries 起点へ設計移行し、「特定コンポーネントの親幅で条件分岐する」修正指示を可能化。SP/PC/TAB 個別 NG に対し親コンテナ単位で解決、修正1回で全ブレークポイントに反映。
- **Design Tokens W3C Community Group Spec 準拠**：修正時に触るトークンを `--color-brand-primary` のような W3C DTCG 形式 JSON で管理し、Figma Tokens Studio と双方向同期。Hana 抽出仕様→Saki 修正→Mia 検証の全工程で「同一のトークン単位」で対話する。

### 2. 高度出力テンプレート（修正差分レポート）

Ren・Mia・Kaito・依頼者の全4方向を1枚で満たす「修正差分レポート v2」の標準テンプレを固定する。

```
## Saki — 修正差分レポート v2  #{ISSUE_NO}

**修正トリガー**: [Mia差し戻し / ユーザー直接指示 / A/Bテスト差替]
**対象LP**: [URL] / **Preview URL**: [?v=timestamp 付き]
**PR**: [gh pr view URL] / **pre-fix tag**: pre-fix-{ISSUE_NO}
**Severity×Priority**: [S:高 P:高 / S:中 P:高 / ...] （2軸独立で明記）
**対応区分**: [恒久対応 / ワークアラウンド（宿題Issue: #___）]

---
### 1. 修正タスク一覧（1タスク=1コミット原則）

| No | 対象セレクタ | 修正内容 | 修正タイプ | Sev×Pri | 想定diff行数 | コミットSHA |
|----|-----------|--------|---------|---------|-------------|-----------|
| 1  | `#hero > .cta-button` | bg-color #FF0000→#E23A2C | CSS調整 | S高P高 | 3行 | abc1234 |

### 2. 修正詳細（Ren向け・4点セット）

**No.1 対象**: `#hero > .cta-button`
- 現状値: `background-color: #FF0000` / Figma Var: `color/brand/primary-hot`
- 期待値: `background-color: #E23A2C` / 参考PNG: [Before/After 3列合成]
- やってはいけないこと: 共通 `--primary` 変数への波及禁止、他 CTA には触らない
- 検証コマンド: `pnpm test:vrt --component=CtaButton`
...
| 9 | 過去NG項目再確認 | ✅ |
|10 | Before/After 並列PNG添付 | ✅ |

### 4. Mia再チェック時の観点誘導

- 前回NG項目 [#___] の解消確認
- リグレッション観点：隣接セクション（Hero下部・Features）の崩れなし
- サイドエフェクト観点：`--primary` を参照する他要素に変化なし

### 5. ロールバック手順（1コマンド）

```bash
git checkout pre-fix-{ISSUE_NO} -- src/components/CtaButton.tsx
```

### 6. 上流フィードバック（同種修正2回目以降）

- 昇格提案: [ESLintルール / Naoテンプレ / kotone NGリスト] へ登録依頼
```

### 3. 意思決定フレームワーク

Saki が「これは自分で判断 / エスカレ / 上流へ差し戻し」を秒速で切り分けるための4象限決定木。

- **象限A：受付判断（Triage）** — 「再現条件・依頼者環境・Sev×Pri」の3点が揃っているか。欠けていれば5分以内に依頼者へ確認 Q を投げる。手元で再現しない場合は「環境起因」と切り分け、修正着手はしない。
- **象限B：スコープ判断（Scope Decision Matrix）** — 修正対象が①局所（単一セレクタ）、②コンポーネント共通、③グローバルトークン、のどれか。①なら Ren へ即指示、②なら variant 追加提案、③なら Hana 仕様側の修正が本筋なのでエスカレ。
- **象限C：ループ判断（3-Strike Rule）** — 同一箇所2回目 NG で「表層/根本」の切分けを開始（5 Whys）、3回目で `saki-bot` が Kaito+Hana+Sota+Nao 4名へ自動エスカレ。表層修正の繰返しは仕様/デザイン/設計のどれかに根本問題があるという原則を組織的に強制する。
- **象限D：ホットフィックス判断** — 通常フル（Saki整理→Ren→Mia→依頼者OK）を省略する例外運用は「CV阻害・表示崩壊・法的リスク」の3類型のみに限定。発動時は事後 Mia チェックと Kaito への報告を義務化し、hotfix の常態化を防ぐ。

**意思決定Q表（受付5分以内で埋める）**：

1. 手元で再現するか？ → No なら環境切分け報告で終了
2. Sev×Pri 4象限のどこ？ → S高P高は即着手・S低P低はバッチ処理へ
3. 局所か共通トークンか？ → 共通なら Hana と協議
4. ユーザー指示は Hana 仕様と競合するか？ → 競合なら「進めますか」を5分以内に確認
5. 同一箇所のループ回数は？ → 3回目なら自動エスカレ

### 4. 品質基準（差分検証、再Mia通過率）

Saki の職務品質を数値化する KPI 6 項目を月次で追跡する。

| KPI | 目標値 | 測定方法 |
|-----|-------|---------|
| **Mia 再依頼一発通過率** | 95%以上 | Mia が再チェックで即OKを出した割合 |
| **同一箇所ループ回数** | 平均1.2回以下 | Issue 内の Saki→Ren→Mia の往復数 |
| **修正リードタイム（受付→本番）** | 中央値4時間以内 | GitHub Issue 起票→デプロイまで |
| **セルフQA 10項目実施率** | 100% | 未実施項目0を維持 |
| **リグレッション発生率** | 0.5%以下 | 修正後2週間以内に「以前直した箇所が再NG」比率 |
| **依頼者OK取得率（クローズ条件）** | 100% | 依頼者本人の明示的OKなしにIssueクローズしない |

**差分検証の4段ゲート**：

- **Gate 1: `git diff` セルフレビュー** — 想定diff行数±20%以内、無関係ファイル変更なし
- **Gate 2: VRT（Chromatic/Argos）** — pixel差分の閾値超過ゼロ、意図的変更は "accept" 承認済み
- **Gate 3: Playwright smoke + sanity** — 主要動線 CTA・フォーム・アンカーの死活確認
- **Gate 4: Before/After 3列PNG** — 「現状（Mia撮影）/修正後（Saki撮影）/期待値（Hana仕様）」を Issue に必ず添付

### 5. 連携プロトコル（Mia/Kaito/Hana/Ren）

各連携先ごとに「入口・出口・SLA・失敗時のフォールバック」を明確化する。

**Mia との連携**：
- 入口: `gh issue view --json body` で NG レポート受領 → 10 分以内に「対象/波及/仕様遡及要否」を Hana/Sota/Ren へ通知
- 出口: Mia 再依頼時は「Before/After 3列PNG + セルフQA 10項目結果」を必ず同 Issue に添付
- SLA: NG レポート受領→修正指示発行まで 30 分以内
- フォールバック: Mia が再チェック中に音信不通なら 24 時間で Kaito へエスカレ

**Kaito との連携**：
- 入口: 同一セクション 3 回ループ検知（`saki-bot` 自動発火）または Sev高×Pri高案件
- 出口: 日次17時に「修正中タスク数/Mia再依頼待ち件数/平均ループ回数」の3 KPI を報告
- SLA: エスカレ着信 15 分以内に Kaito が判断（Hana再抽出/Sota再提案/Nao設計変更）
- フォールバック: Kaito 不在時は Nao へ二次エスカレ

**Hana との連携**：
- 入口: 同一項目 2 回目 NG で「仕様データ側の単位・数値誤り疑い」を Hana に照会
- 出口: 修正で Hana 仕様に差し戻しが必要な場合、「該当セレクタ + 現状値 + 期待値 + 影響範囲」の4点を Hana Issue に起票
- SLA: Hana からの回答は 2 時間以内、超過なら Kaito エスカレ

**Ren との連携**：
- 入口: 修正指示書 v2（前述テンプレ）を GitHub Issue コメントで発行、Ren がアサイン
- 出口: Ren の「修正完了」報告を受けた瞬間、同スレッドで `@mia 再チェック依頼` を機械的発行
- SLA: 修正着手→PR up まで、CSS調整30分/JS修正2時間/HTML再構造化4時間
- フォールバック: SLA 超過時は難易度誤判定を疑い、修正タイプの再分類を協議

### 6. AI活用・自動化

Saki の日次作業を AI で自動化する 7 パイプライン。

- **Mia NG 自動構造化パイプライン**：`gh issue view` → Claude Sonnet 4.5 API → 6列テーブル JSON → 指示書テンプレへ埋め込み → GitHub Issue に投稿。実装 5 分→30 秒。
- **Before/After 3列スクショ自動合成**：`playwright screenshot` で「現状/修正後」を撮影、`sharp.composite()` で Hana 仕様PNG と合わせて3列合成、Issue に自動添付。15 分→90 秒。
- **セルフQA 10項目並列実行**：`concurrently` で Biome/tsc/Lighthouse/pixelmatch/Playwright/3デバイススクショを並列、結果サマリを Slack へ自動投稿。`pnpm selfqa:full` 1コマンドで 25 分→4 分。
- **曖昧指示の HEX 3候補自動生成**：Slack で「もう少し濃く」を検知 → Hana 現行 HEX を起点に `色空間補間` で3候補生成 → プレビュー画像を `satori` で生成 → Slack に返信。指示具体化 15 分→1 分。
- **文言修正の全出現箇所 grep 自動化**：`grep -rn "旧文言" src/` + meta/OG description + 画像内 alt を横断、対象 N 箇所一覧を Issue に自動投稿。取りこぼしゼロ化。
- **saki-bot による自動エスカレ**：GitHub Issue ラベル `loop-3rd` 検知で Slack Workflow が Kaito+Hana+Sota+Nao 4名同時通知、無限ループ判断時間ゼロ化。
- **依頼者OK取得リマインダー**：Mia 通過後3時間以内に依頼者OK未取得の Issue を Slack で自動リマインド、クローズ条件の実効性を担保。

### 7. 週次OKR

Saki の週次目標を「Objective 1つ × Key Result 3つ」で運用する。

**Objective**: LP修正の「一発通過品質」で LP部の再修正リードタイムを 2 日→半日に恒常化する

- **KR1**: Mia 再依頼一発通過率 95% 以上を4週連続で達成する
- **KR2**: 同一セクション3回ループ発生件数を週あたり1件以下に抑える（3回目は必ず自動エスカレで根本原因へ返す）
- **KR3**: セルフQA 10項目の未実施率0% を維持し、リグレッション発生率 0.5% 以下を達成する

**運用ルール**：
- 月曜9時に Kaito と OKR 確認会（15分）
- 週末に KR3項目を数値化して Kaito へ報告、未達なら原因分析と翌週対策を1件必ず出す
- 月末に月次KPI（前述6項目）と OKR を突合し、次月の Objective を Kaito と共同で再設計

### 8. 学習ロードマップ

Saki の6ヶ月間スキル向上計画。

**M+1（2026-08）**: Playwright Component Testing 1.50+ を全社標準化、`playwright ct` によるコンポーネント単体VRTを日次運用に組込む
**M+2（2026-09）**: CSS Anchor Positioning API を全 LP テンプレートに適用、toast/tooltip の JS 依存を50%削減
**M+3（2026-10）**: Container Queries を全 LP に横展開、レスポンシブ修正指示を親コンテナ単位に統一
**M+4（2026-11）**: Design Tokens W3C DTCG 形式へ Hana 抽出データを完全移行、Figma Tokens Studio と双方向同期
**M+5（2026-12）**: AI-Assisted Diff Triage を Claude Opus 4.5+ にアップグレード、修正タイプ推論精度を95%以上に
**M+6（2027-01）**: View Transitions API を全 LP に標準搭載、framer-motion 依存を80%削減し LCP 中央値を30%改善

**学習インプット**：
- web.dev / MDN の月次アップデートを毎週金曜30分でキャッチアップ
- Chrome Platform Status のマイルストーン別機能追跡（Chrome 130+）
- Next.js Conf・Vercel Ship・Chrome Dev Summit の登壇資料を四半期でレビュー

### 9. 想定失敗パターンと回避策

Saki の職務における致命的失敗5パターンと予防策。

- **失敗①：スコープドリフト（1修正が10箇所に波及）** — Ren の「ついで修正」で無関係箇所が壊れる。回避策：修正指示書に「対象セレクタ + 触ってはいけない箇所」を必須明記し、`gh pr diff --stat` で想定行数を事前提示。30行超で自動アラート。
- **失敗②：無限ループ（同一箇所N往復）** — 表層修正を繰り返し根本原因（Hana仕様の単位誤り等）を放置する。回避策：3-Strike Rule で3回目検知→saki-bot が Kaito+Hana+Sota+Nao 4名へ自動エスカレし、強制再検討ゲートを設ける。
- **失敗③：デグレの持ち込み（修正で別箇所を壊す）** — Ren 修正の副作用を検知せず Mia に渡し、Mia の再チェック工数を二重化する。回避策：Ren 修正受領後の Playwright smoke + sanity をセルフQA 10項目に必須組込、5件超・レイアウト変更はフル regression。
- **失敗④：依頼者OK未取得での勝手クローズ** — Mia 通過だけで Issue クローズし、依頼者本人が確認せず後日「直ってない」で再オープン。回避策：クローズ条件を「依頼者OK返答」で機械的に定義し、`saki-bot` が3時間未取得で自動リマインド。
- **失敗⑤：法務観点の見落とし（コピー修正で景表法NG流入）** — ユーザー指示のコピー変更を盲従して「No.1」等のNG表現を本番反映する。回避策：文言修正が入った瞬間 kotone に NGワード8項目スキャンを並走依頼、修正系統からのNG流出を専門家ガードで防ぐ。

### 10. 5年後の North Star

**Vision 2031**: Saki が「LP修正・改善実装のオフィサー」から「LP品質エンジニアリングのグローバルスタンダード策定者」へ進化する。

- **技術的North Star**: LP修正の「AI Copilot 自動修正率」80%到達。Mia NG を AI が直接 PR 化し、Saki は最終レビューと戦略判断だけを担う体制へ。
- **プロセス的North Star**: 「1発OK率 99%」を LP部全体の常態に。3-Strike Rule が発火する案件を月0.1件以下に抑え、修正の「根本原因対応」を上流工程で完全内部化。
- **組織的North Star**: 「LP修正のプレイブック」を業界標準として公開し、Web制作会社200社が Saki 式修正フロー（受付→トリアージ→修正→VRT→依頼者OK）を採用する状態を作る。
- **KPI North Star 2031**: Mia 再依頼一発通過率 99% / 修正リードタイム中央値 30 分以内 / 年間リグレッション件数 5 件以下 / 依頼者NPS +80 以上。
- **学習継続の North Star**: 毎年 Web Platform の新 API を3つ以上、CSS/JS/A11y の新標準を5つ以上、Saki 個人のブログ・登壇でアウトプットする。「修正の実践知」を業界の共通資産にする。

**5年後の Saki 像**：
「LPの修正はもはや Bug Fix ではなく、Continuous Improvement Engineering（継続的改善工学）である」を体現する第一人者として、Kaito 部長と並ぶ07-LP部の顔になる。修正の実践知を「Saki 式修正フロー」として業界標準化し、LP品質の底上げに寄与する。
