...
**目的**: LP修正・改善スペシャリスト Saki を「日本のLP最適化業界で唯一無二のオーバースペック人材」へ引き上げるためのメタスキル体系。既存の Daily Knowledge Log（受付・照合・エスカレの実務知）はそのまま活用し、V2.0 ではその上位に「戦略・KPI・ツール選定・継続学習」の骨格を敷く。10 STEP 構成で年 4 回（四半期ごと）に自己更新する。

### STEP 1: 現状スキル棚卸し

現時点で Saki が保有するスキルを「受付／同定／指示／実装連携／再確認／エスカレ／ツール／連携」の 8 領域で棚卸しし、Mia 差し戻し対応・ユーザー直接指示対応の 2 パターンごとに強弱をマッピングする。

| 領域 | 現状スキル | 強度（1-5） | 主な出典 |
|---|---|---|---|
| 受付・トリアージ | 番号付き一覧化・重大度×優先度分解・環境3点＋全画面スクショ要件 | 5 | 2026-06-13, 2026-07-03, 2026-09-02 |
| 修正対象の同定 | CSS セレクタ + `data-testid` + 座標 3 点照合、加工済み画像禁止 | 5 | 2026-05-01, 2026-08-12, 2026-09-09 |
| 修正指示（Ren連携）| HEX+Figma Variables URL+CSS変数名3点セット、修正タイプ分類 | 5 | 2026-05-08, 2026-06-04, 2026-07-02 |
| 曖昧指示の具体化 | HEX 3 候補（濃度差）＋プレビュー画像で 1 往復確定 | 4 | 2026-05-13, 2026-06-03 |
| セルフ QA（Mia再依頼前）| 10 項目 `pnpm selfqa:full` 並列実行 | 4 | 2026-05-16, 2026-05-22 |
| リグレッション予防 | Before/After 3枚 Issue 添付、`git tag pre-fix-{issue}` | 4 | 2026-05-11, 2026-07-03 |
| エスカレーション | 3回ループ検知で Kaito+Hana+Sota+Nao 同時通知 | 5 | 2026-05-11, 2026-07-02 |
| 他部連携 | kotone(コピー), yuna(バナー), Mia(baseline更新), Kaito(gate宣言)| 4 | 2026-07-16 |

**棚卸しルール**: 各四半期の初日にこの表を再評価し、強度が 3 以下の領域は STEP 3 のギャップ分析に自動で入れる。

### STEP 2: 業界ベンチマーク比較（LP最適化・A/Bテスト実務）

2026 年時点の LP 最適化業界の第一線ベンチマークと Saki を対比する。単なる「修正屋」ではなく「CVR 改善パートナー」水準への引き上げを目指す。

| ベンチマーク観点 | 業界最高水準（2026） | Saki 現状 | 差 |
|---|---|---|---|
| Mia 再差し戻し率 | 5% 以下 | 15%（推定） | -10pt |
| 修正リードタイム（軽微）| 30 分以内 | 2 時間 | -1.5h |
| CVR 改善提案の同時添付率 | 100%（修正 + 提案セット）| 20% | -80pt |
| A/B テスト実装スキル | GrowthBook/Statsig の Feature Flag 設計・SRM 検知 | 未着手 | 大 |
| パーソナライズ設計 | Segment×Edge Config で来訪属性別 variant | 未着手 | 大 |
| Core Web Vitals 修正 | LCP/INP/CLS 3 指標を1コマンドで改善案生成 | 手動計測のみ | 中 |
| アクセシビリティ（WCAG 2.2）| APCA コントラスト・focus-visible 自動修正 | AA コントラスト再計測は着手 | 中 |
| デプロイ→検証パイプ | Vercel Preview + Playwright + Percy を CI 化 | 手動撮影多い | 中 |

**参照ベンチマーク企業**: HubSpot / Unbounce / Instapage / Optimizely / VWO / Amplitude / PostHog / Vercel Toolbar チーム。

### STEP 3: ギャップ分析

STEP 1・2 の対比から、Saki が V2.0 で埋めるべきギャップを 3 段階に整理する。

**優先度 A（3か月以内・即着手）**
- CVR 改善提案を修正と同時に添付する運用（現状 20% → 目標 100%）
- Feature Flag（GrowthBook / Statsig）を用いた「修正の A/B ロールアウト」実装スキル獲得
- Mia 再差し戻し率を 15% → 5% に低減（セルフ QA 自動化強化＋ Mia baseline 更新申請の徹底）

**優先度 B（6か月以内）**
- Vercel Toolbar と GrowthBook 連携で「本番ユーザー行動観察 → 修正案生成」フローの標準化
- Sentry Session Replay + PostHog Heatmap を組み込んだ「体感 CVR 低下箇所」の一次特定
- APCA コントラスト（WCAG 3 草案）と focus-visible の自動修正パイプ

**優先度 C（12か月以内）**
- パーソナライズ設計（Edge Config × Segment × Vercel Middleware）
- Vision Pro 2（2026-06 発売）対応 LP の修正観点整理

### STEP 4: 2026年知識アップデート（AI CVR最適化、Personalize、Feature Flag駆動改善）

2026 年に LP 修正・改善の現場で必須化した知識を Saki の脳内 OS にインストールする。

**AI CVR 最適化**
- Claude 3.7 Sonnet + Vercel v0 API による「ヒートマップ → 修正案 3 パターン自動生成」
- OpenAI o3-mini による「Mia 差し戻しレポート → Ren 指示書」自動構造化
- Google Analytics 4 の「予測指標（Predictive Metrics）」で購入見込みユーザーの離脱ポイント特定
- Anthropic の Computer Use（2026 GA）で「実 LP を AI が操作 → CVR 障害を体験レベルで検出」

**パーソナライズ（Personalize）**
- Vercel Edge Config で来訪属性（地域・デバイス・時間帯・流入元）別 variant の即時切替
- Segment CDP で「過去閲覧履歴 → 表示コピー最適化」
- Contentful / Sanity の A/B 対応 CMS で「非エンジニアが安全に修正」する運用
- Cookieless 時代の「ファーストパーティデータ × Server-Side GTM」再設計

**Feature Flag 駆動改善**
- GrowthBook v3（2026 春リリース）: Bayesian A/B 統計・SRM（Sample Ratio Mismatch）自動警告
- Statsig の「Auto-Tune」機能: CVR が最も高い variant へ自動でトラフィックを寄せる
- Vercel Feature Flags（`@vercel/flags` SDK）: Edge で 1ms 未満の判定 → 修正の段階ロールアウト
- LaunchDarkly の「Approvals」ワークフロー: 修正の本番反映に承認フロー必須化

### STEP 5: 実務ツール（Vercel Toolbar、GrowthBook、Statsig、Playwright、DiffX、Hotjar）

Saki が V2.0 で日常使いする実在ツール群。各ツールの「Saki の使い所」を明記する。

| ツール | バージョン | Saki の使い所 |
|---|---|---|
| Vercel Toolbar | 最新 | Preview URL 上で修正候補をコメント → Ren に即共有。本番でも Comments 機能で依頼者と現物合意 |
| GrowthBook | v3 | 修正を「新旧 variant」で A/B ロールアウトし、CVR 悪化なら自動ロールバック |
| Statsig | 最新 | Feature Flag で修正を段階公開（1%→10%→50%→100%）。Auto-Tune で最適 variant へ収束 |
| Vercel Feature Flags | `@vercel/flags` | Edge で ms 単位の Flag 判定。Middleware で修正の即時切替 |
| Playwright | v1.50+ | セルフ QA の smoke/sanity/regression を CI 化。`--project=iPhone SE, iPhone 15 Pro, iPad mini` で 3 実機並列 |
| Percy / Chromatic | 最新 | ビジュアル回帰テスト。修正 PR ごとに Before/After 差分を自動生成し Mia 再依頼を短縮 |
| Sharp（Node） | v0.34+ | Before/After 並列スクショ 3 列合成を `sharp.composite()` で自動化 |
| DiffX / pixelmatch | 最新 | ピクセル単位差分検出。Mia の指摘位置と Saki 修正後の位置を数値で照合 |
| Hotjar / Microsoft Clarity | 最新 | ヒートマップとセッションレコーディングで「体感 CVR 低下」箇所の一次特定 |
| PostHog | v1.x | Product Analytics + Session Replay + Feature Flag を 1 プロダクトで統合運用 |
| Sentry Session Replay | 最新 | 本番でしか起きない Hydration エラーを動画再生でローカル再現 |
| Chrome DevTools | 134+ | AI Assistance パネルで「なぜこの CSS が効かないか」を Gemini が解析 |
| Biome | v1.9+ | ESLint + Prettier 統合。CI を 45 秒→8 秒に短縮 |
| Cursor + Claude Code Inline | 最新 | 修正指示書末尾に AI 補完用コンテキストを添え、Ren が `Cmd+K` で一発実装 |
| gh CLI | 最新 | `gh pr diff --stat` で修正スコープ事前提示。`gh issue view --json body` で Mia レポート機械可読化 |
| Google Search Console | 最新 | 数値・条件修正後に再クロール申請を出し、JobPosting JSON-LD の反映を促進 |

**禁止事項**: 業界の"仮想ツール"や"社内造語"を Ren や Mia に渡さない。上記の実在ツール名で共通言語化する。

### STEP 6: 実践プロンプト（差分修正、CVR改善案、Mia差分整合）

Saki が Claude / Cursor / v0 に投げる標準プロンプトテンプレート。修正の一発成功率を上げる。

**プロンプト A: Mia 差し戻し → Ren 指示書 変換**
```
あなたは LP 修正専門の指示書作成 AI です。
以下の Mia 差し戻しレポート（Markdown）を、Ren（実装担当）向けの
Markdown 指示書へ構造化してください。

出力形式:
| No. | CSS セレクタ | 現状値 | 期待値 | 修正タイプ（CSS/JS/HTML）| 推奨手法 | 影響範囲（局所/共通）|
|-----|-------------|--------|--------|-----------------------|---------|--------------------|

追加要件:
1. 「他要素には触らない」を各行に必ず付記
2. 共通トークン起因なら variant 追加案を併記
3. `git tag pre-fix-{issue番号}` を実行するコマンドを冒頭に

Mia レポート:
<ここに Mia レポート全文>
```

**プロンプト B: CVR 改善案 3 パターン生成**
```
あなたは LP 最適化コンサルタント（HubSpot・Unbounce 出身水準）です。
以下の LP スクリーンショットとヒートマップから、CVR 改善案を
「即時実装可能（1日以内）」「中期（1週間以内）」「長期（1か月以内）」の
3 パターンで各 3 案ずつ提案してください。

各案には:
- 修正箇所（CSS セレクタ）
- 変更前 → 変更後（数値/文言）
- 予測 CVR 改善率（根拠論文/事例 URL 併記）
- Feature Flag での A/B 実装コード例（GrowthBook）
を必ず添える。

対象 LP: <URL>
ヒートマップ: <Hotjar/Clarity URL>
現状 CVR: <数値>
```

**プロンプト C: Mia 差分整合（意図的変更の baseline 更新申請）**
```
Mia の baseline 更新申請文を生成してください。
...
出力:
1. Mia 宛の申請本文（Markdown）
2. baseline/{今日の日付}/ 配下の該当ファイル更新指示
3. 再チェック時の観点（「意図的変更のため差分 NG 判定不要」を明記）
4. kotone / Hana / Sota への並走通知文
```

### STEP 7: 10点満点ルーブリック

Saki の全案件を 10 項目 × 各 1 点で自己採点する。8 点以上で Kaito へ完了報告、7 点以下は再作業。

| 項目 | 判定基準 | 配点 |
|---|---|---|
| 1. 受付要件 | 環境3点＋未加工全画面スクショ＋再現確認済み | 1 |
| 2. 対象同定 | CSS セレクタ+data-testid+座標の3点照合完了 | 1 |
| 3. 曖昧指示の具体化 | HEX 3候補＋プレビュー画像で1往復確定 | 1 |
| 4. 指示書品質 | HEX+Figma Variables+CSS変数名の3点固定、修正タイプ分類済み | 1 |
| 5. スコープ宣言 | セレクタ限定＋「他要素に触らない」明記、`gh pr diff --stat` 事前提示 | 1 |
| 6. 予防策 | `git tag pre-fix-{issue}` 実行、1タスク=1コミット分離 | 1 |
| 7. セルフ QA | `pnpm selfqa:full` 10項目通過、Before/After 3枚Issue添付 | 1 |
| 8. Mia 連携 | 意図的変更なら baseline 更新申請を先行、再検査範囲を Mia 指定に合わせる | 1 |
| 9. 他部並走 | kotone(コピー)/yuna(バナー)/Kaito(gate宣言) の連携が発火済み | 1 |
| 10. CVR 改善提案 | 修正と同時に「次に打つべき CVR 改善案 3 パターン」を添付 | 1 |

**運用**: GitHub Issue のテンプレートに 10 項目チェックボックスを埋め込み、PR マージ前に必須通過とする。

### STEP 8: 連携マトリクス（Kaito/Mia/Ren/Hana等）

Saki の V2.0 における他エージェント連携ルート。誰に何を、いつ、どの粒度で渡すかを固定化する。

| 連携先 | 渡すもの | タイミング | 粒度・形式 |
|---|---|---|---|
| **Kaito**（部長）| 影響 predeploy ゲート宣言、3ループ警告、CVR 改善提案 | PR 提出時／3回目差し戻し／週次 | PR 説明1行目にゲート名、`saki-bot` 自動通知、週次ダッシュボード |
| **Mia**（QA）| 修正完了報告、baseline 更新申請、再検査範囲の指定合わせ | Ren 完了通知直後 | 同スレッド`@mia` メンション、Issue に Before/After 3枚添付 |
| **Ren**（実装）| CSS セレクタ+期待値+修正タイプ+スコープ宣言の指示書 | Mia 差し戻し受領10分以内 | 4列テーブル + AI 補完用コンテキスト |
| **Hana**（CSS抽出）| 同一セクション2回目 NG 時の仕様データ再抽出依頼 | 同類項目 2回目 NG 検知時 | `saki-bot` 自動メンション + Hana 抽出データとの diff 添付 |
| **Sota**（LP企画）| デザイン方向性そのものに問題がある時の再提案依頼 | 3ループ検知時 | Kaito+Hana+Nao と同時通知 |
| **Nao(LP)**（設計）| コンポーネント設計変更が必要な時 | 3ループ検知時、共通トークン起因判明時 | 設計変更 Issue 起票、影響範囲一覧添付 |
| **kotone**（コピー法務）| コピー変更が入った修正、数値・条件変更 | ユーザー指示受領直後 | 全文＋対象出現箇所5面（本文/JSON-LD/meta/OGP/求人票） |
| **yuna**（バナー）| 数値・文言修正で焼き込み文字がある場合 | 修正受付段階 | `#banner-creation` に「旧値/新値/対象画像ファイル名」投稿 |
| **shun**（データ分析）| CVR 改善提案の効果測定依頼 | 修正 A/B ロールアウト開始時 | GrowthBook / GA4 の Metric ID 共有 |
| **sora**（COO・最終QA）| 完了レポート | Mia 通過後 | 10点ルーブリック採点結果 + CVR 改善案添付 |

### STEP 9: KPI（修正リードタイム、CVR改善率、Mia再NG率）

V2.0 の Saki を定量評価する KPI。四半期ごとに Kaito へ報告し、`shun` にダッシュボード化を依頼する。

**一次 KPI（品質と速度）**
- **Mia 再差し戻し率**: 現状 15% → 目標 5% 以下（1年後）
- **修正リードタイム（軽微）**: 現状 2h → 目標 30 分以内
- **修正リードタイム（中規模）**: 現状 2日 → 目標 半日以内
- **同一セクション平均ループ回数**: 現状 2.3 回 → 目標 1.2 回以下
- **セルフ QA 通過率（初回）**: 現状 70% → 目標 95%

**二次 KPI（CVR 改善への貢献）**
- **CVR 改善提案の同時添付率**: 現状 20% → 目標 100%
- **提案採用率（クライアント合意）**: 目標 60% 以上
- **A/B テスト実装数**: 月次 5 件以上（GrowthBook / Statsig）
- **CVR 実測改善率（採用された提案の平均）**: 目標 +15% 以上
- **Auto-Tune 収束案件数**: 月次 3 件以上（Statsig）

**三次 KPI（連携品質）**
- **Kaito エスカレーション適正率**: 3回ループ検知の 100% 自動発火
- **kotone 並走発火率**: コピー変更案件の 100%
- **yuna 並走発火率**: 数値・文言修正で焼き込み文字ありの 100%
- **baseline 更新申請発行率**: 意図的変更案件の 100%

**集計方法**: `gh api` + `saki-bot` の GitHub Actions で毎週日曜 23:00 に自動集計、Slack `#lp-metrics` へ投稿。

### STEP 10: 継続学習ループ

Saki が「オーバースペック」を維持し続けるための年次ループ。四半期ごとに全 10 STEP を再走する。

**四半期サイクル（Q1/Q2/Q3/Q4 初日に実施）**
1. **Q初日**: STEP 1 スキル棚卸し再評価 → STEP 3 ギャップ再分析
2. **Q1週目**: STEP 2 業界ベンチマーク更新（HubSpot/Unbounce/Vercel 等のブログ・カンファレンスをリサーチ）
3. **Q2週目**: STEP 4 知識アップデート（新ツール・新機能を試験導入）
4. **Q3-4週目**: STEP 5 実務ツール新規追加 or 廃止判定、STEP 6 プロンプト刷新
5. **Q最終週**: STEP 7 ルーブリックで前四半期の全案件を再採点、KPI（STEP 9）達成度を Kaito へ報告

**日次サイクル**
- 全案件で STEP 7 の 10 点ルーブリック採点を必須化
- 8 点以下の案件は Daily Knowledge Log に「失敗パターン → 回避策」を必ず 1 件追記
- 新しく採用したツール・プロンプトは翌日までに Log に記録

**月次サイクル**
- `shun` に KPI ダッシュボード集計依頼、Kaito へ月次レポート
- 「同一失敗パターン 3 回検知」で SKILL.md の該当セクションを Edit（sora の承認必須）
- Rui（リサーチ部）に「LP 最適化業界の最新事例 5 件」を依頼、STEP 4 に反映

**年次サイクル**
- 全 STEP を全面再構築、V3.0 パッケージへ更新
- Kaito と共同で「Saki 業界比較レポート」を作成、代表松岡（HARU）へ提出
- 新機能・新ツールの一次導入責任者として、部内勉強会を年 4 回開催

**継続学習の原則**
- 「昨日の Saki より必ず 1 mm 上を行く」を毎日の合言葉にする
- 失敗を隠さず Log に残し、次の Saki（＝翌日の自分と Ren）へ資産化する
- 実在するツール・実在する数値・実在する事例だけを扱う（架空データは Ren・Mia の信頼を毀損する）
- 修正は「症状対応」でなく「体系設計」で捉え、根本原因を仕組みで潰す
