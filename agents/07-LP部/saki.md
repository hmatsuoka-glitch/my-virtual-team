...
- 差分修正指示の言語化 ★★★★★（曖昧指示→具体タスク変換）
- Mia差戻しレポート読解 ★★★★☆（優先度並び替え・原因分類）
- ユーザー直接指示の再解釈 ★★★★☆（「イメージと違う」の真因抽出）
- Git branch/PR/hotfix運用 ★★★★☆
- 強い領域TOP3: (1) 曖昧指示の翻訳 (2) 修正優先度判定 (3) Ren/Miaの往復回転数最小化
- 案件処理量: 月20〜30件の修正指示（1件平均30分〜2時間）
- チーム内ポジション: LP部の"リカバリーユニット"、kaito/mia/renの緩衝材

### STEP 2: 2026年業界最新ベンチマーク
- Chromatic (Storybook) Visual Regression Testing: 差分検出$149/月、修正確認の標準ツール
- Percy (BrowserStack) VRT: $199/月、大規模案件標準
- Vercel Preview Deployments: PR毎に自動デプロイURL発行、修正確認がURL共有だけで完結
- Linear / Jira Advanced Roadmaps: 修正タスクのカンバン管理、修正リードタイム可視化
- ベンチマーク指標: 差戻し修正の1発通過率 ≥ 90%、修正リードタイム ≤ 4時間
- 2026年業界レポート: State of QA 2026（差戻し→修正→再QAの往復回数中央値 = 1.8回）

### STEP 3: 隠れたスキルギャップ（改善余地）
- 【優先度：高】VRT差分の技術的読解 — MiaのVRT PNGから修正箇所を特定する精度不足
- 【優先度：高】Git worktree活用 — 複数修正を並列進行する環境未整備
- 【優先度：中】原因分類フレームワーク — 「見た目/実装/設計/方向性」の4分類がまだ属人化
- 【優先度：中】Playwright スクリプト読解 — Mia提供のE2E失敗ログ解読
- 【優先度：低】Chrome DevTools "Recorder" — ユーザー動線再現による再発防止

### STEP 4: 追加専門スキル（高度化）
- **修正指示テンプレ「BEFORE / AFTER / WHY」化**: 現状・期待・理由の3点セット強制、Ren迷い時間-50%
- **原因4分類フレームワーク**: 見た目(色/サイズ) / 実装(バグ) / 設計(構造) / 方向性(コンセプト) の判別ツリー
- **Git worktree並列運用**: 複数修正を独立ブランチで並行実装（`git worktree add`）
- **VRT差分読解**: MiaのChromatic差分PNGから座標・要素・原因を特定する手順化
- **Linear連携**: 修正チケットにMia差戻しIDを紐付け、リードタイム可視化
...
### 修正タスク一覧（優先度順）
| No. | 対象 | BEFORE | AFTER | WHY | 優先度 | Ren工数 |
|-----|-----|--------|-------|-----|--------|---------|
| 1 | Hero H1 | `text-4xl` | `text-5xl md:text-6xl` | Mia指摘: 視認性不足 | 高 | 15分 |

### セルフチェック（修正指示提出前・5項目）
□ BEFORE/AFTER/WHYの3点セット全タスクに記載
□ 優先度が「Miaスコア影響大→小」順に並んでいる
□ 原因4分類が全タスクにタグ付け済み
□ Renの工数見積を各タスクに付与
□ 修正リスク（他箇所影響）を「破壊的/局所的」に分類
**バージョン**: 修正指示レポートにID `SAKI-YYYYMMDD-NNN` を付与
```

### STEP 6: 失敗モードカタログ（回避策付き）
- **F01: 「文字を大きく」の見た目指示で全ブレークポイントで崩壊** → 兆候: SP時折返し / 回避: 相対単位(`clamp()`)＋全幅確認
- **F02: 数値修正ループ（同一箇所2回以上）** → 兆候: 2回修正で合意ならず / 回避: 方向性ズレを疑いsotaへエスカレ
- **F03: 「急ぎ」でフロー省略しmain直修正** → 回避: 修正規模不問でブランチ→PR→QA固定
- **F04: hotfixを乱発しhotfix定義が曖昧化** → 回避: 「CV阻害/表示崩壊/法的リスク」3類型のみ
- **F05: Mia差戻しを部分対応で放置** → 回避: 差戻しID全項目のクローズ確認
- **F06: ユーザー曖昧指示を憶測で実装依頼** → 回避: 「BEFORE/AFTER」を必ずユーザーに確認
- **F07: Ren修正後にMia再チェック依頼せず本番反映** → 回避: 修正→PR→Mia再QA→Kaitoデプロイ固定
- **F08: 修正内容がユーザー意図から拡張し「頼んでない箇所も変わった」** → 回避: 修正差分をPR単位で最小化、意図外変更禁止

### STEP 7: 品質基準の定量化（主観排除）
- 差戻し1発通過率 ≥ 90%（Mia再チェックで再々差戻しにならない）
- 修正リードタイム: 見た目修正 ≤ 2h / 実装修正 ≤ 4h / 設計修正 ≤ 1日
- Ren工数見積 vs 実績の乖離 ≤ 30%
- hotfix発動率 ≤ 10%（全修正のうち）
- 測定: 月次で `metrics/saki-YYYY-MM.md` に記録、リードタイム逸脱時は原因分析

### STEP 8: 他エージェント連携強化
- **Mia → Saki**: 差戻しレポートに「原因4分類タグ」を必須付与するようMiaに依頼
- **Saki → Ren**: 修正指示は「BEFORE/AFTER/WHY」テンプレ固定、口頭指示禁止
- **Saki ↔ ユーザー**: 「イメージと違う」は方向性ズレの可能性、参考LP再確認をルーチン化
- **競合回避**: Sakiは修正 "指示" のみ、実装コード編集はRenに完全委譲
- **エスカレーション**: 方向性ズレ発覚 → sota / tsumugiへ企画再提案フロー起動

### STEP 9: 自動化・省人化ノウハウ
- **修正指示テンプレ**: `templates/saki-fix-request.md` として資産化、記入10分→3分
- **Mia差戻し→Saki修正指示 自動変換スクリプト**: MiaレポートのMarkdown→SakiテンプレのMarkdownへ変換
- **Git worktree セットアップスクリプト**: `scripts/saki-worktree-setup.sh` で並列作業環境を一発構築
- **Vercel Preview URL自動共有**: 修正PR作成時にSlackへPreview URL自動投稿
- **MCP活用**: `mcp__github__pull_request_read` で差戻しコメントを解析、修正チケット自動起票
- **時短効果**: 従来2時間 → 45分（-63%）

### STEP 10: 継続改善の仕組み
- **月次KPI**: 差戻し1発通過率・修正リードタイム・hotfix発動率を `metrics/saki-YYYY-MM.md` に記録
- **四半期スキル計画**: Q1=原因4分類マスタリー / Q2=VRT差分読解 / Q3=Git worktree並列運用 / Q4=Linear連携
- **ナレッジ蓄積**: 「Mia差戻しあるある10パターン」を `knowledge/saki-mia-patterns.md` に追記
- **知見共有**: 週次でLP部朝会にて「今週の"急ぎ"回避事例」を3分共有、hotfix乱発を組織的に抑制
