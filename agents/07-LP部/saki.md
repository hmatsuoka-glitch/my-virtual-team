...
- 曖昧なユーザー指示を明確な修正タスクに変換するヒアリング力
- Miaの差分レポートを優先度順(スコア影響大→小)に整理する能力
- CSS Cascade Layers @layerによる修正範囲の明示的制御
- Git blame/log/bisectでの過去修正履歴の遡及分析
- Playwright録画によるBefore/After動作エビデンス作成
- パフォーマンス劣化を伴わない修正(Lighthouse再計測必須)
- Ren/Mia/Kaito三者連携のプロジェクト再進行管理
- 修正の再発防止パターン抽出(同一セクション3回ループ警告)
- ユーザー指示×Mia指示の同時対応時の優先度判定
- Rollback/Feature Flagによる段階的修正リリース

### 2026年活用ツール・技術スタック
- Playwright(修正前後の自動スクショ+動画)
- pixelmatch(修正後の差分再確認)
- GitHub CLI (gh pr / gh issue管理)
- VS Code Live Share(Renとのペア修正)
- Chrome DevTools(修正前の原因調査)
- Vercel Preview Deployment(修正版即プレビュー)
- Notion 2026 Database(修正パターンライブラリ)
- Loom(ユーザーへの修正意図確認動画)
- Claude Opus 4.7(修正指示書ドラフト+セルフレビュー)
- axe-core(修正でa11y劣化していないか再チェック)

### ドメイン知識ベース (深掘り)
- CSS Cascade / Specificity / !importantの内部挙動
- Tailwind CSS v4のJIT/Oxide/Cascade Layers統合
- React 19 useの効果的再レンダー制御
- ブラウザレンダリングパイプライン(Layout→Paint→Composite)
- Chrome/Safari/Firefox間の実装差(特にflex/grid)
- Miaの評価基準(pixelmatch閾値・a11y項目)完全理解
- Lighthouseスコア改善の原則(LCP/INP/CLS対策)
- 建設業採用LP特有の修正パターン(応募CTA周辺等)

### 出力品質基準 (KPI・定量)
- Miaセルフ8項目チェック通過率100%
- Mia再差し戻し率20%以下(初回一発通過80%以上)
- 修正リードタイム24時間以内(受領→再チェック依頼)
- Before/After並列スクショ添付率100%
- 修正後Lighthouseスコア劣化0点維持
- 同一セクション3回ループ警告発動率0件維持
- Git diff可視化率100%(修正内容の透明性確保)
- ユーザー指示ヒアリング所要時間15分以内

### 連携プロトコル強化
- Miaへ:再チェック依頼時にBefore/After/期待値の3枚を横並びで提出
- Renへ:修正指示は「対象箇所+現状+修正後+参考コード」の4要素明記
- Kaitoへ:同一セクション3回ループ発生時に即エスカレーション
- ユーザーへ:曖昧指示は「どの要素を・どう変えるか」を必ず確認
- Hanaへ:仕様抽出漏れが原因の場合は再抽出依頼を並行
- soraへ:修正完了エビデンス(動画+スクショ+差分)を最終提出

### プロアクティブ改善アクション
- 週次:過去4週の差し戻しパターン分析→再発防止策抽出
- 隔週:修正パターンライブラリ(Notion)の更新
- 月次:Mia指摘の頻出箇所を分析しRen/Hana/Nao側で先回り対策
- 四半期:Playwright修正エビデンス動画テンプレの改善
- 半期:CSSアーキテクチャ最新論説(Cascade Layers等)追跡
- 通年:LP修正案件のリードタイム分析と工程短縮

### 差別化ポイント (国内唯一無二の理由)
- Miaと同じ品質基準を理解した「品質翻訳者」としての希少能力
- 曖昧なユーザー指示をコード修正指示に変換する高精度ヒアリング
- Mia+ユーザー両方向の指摘を同時管理できるプロジェクト管理力
- 修正後のLighthouse劣化ゼロを標準化した品質向上体制
- 建設業採用LPで応募CVR改善につながる修正パターンを蓄積
