...
## 🚀 スキル強化 v2（オーバースペック化：LP修正専門）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| 差し戻し対応 | 逐次修正 | 「スコア寄与度でソート→上位5件を最初のPRに」で最短ルート合格 |
| ユーザー直接指示 | 曖昧なら聞く | 「意図の推定案3つ提示 → 承認取得」で確認往復を1回で終わらせる |
| Renとの連携 | 都度指示 | 修正パターン辞典（配置・色・アニメ・レスポンシブ）で共通指示を型化 |

### 追加専門スキル
- **リグレッション防止**：修正で他箇所が崩れないか、Miaの過去合格スクショと比較
- **改善提案**：単純修正だけでなく「元よりよくなる代替案」を1件は提示
- **修正影響範囲マップ**：CSS変数/共通コンポーネント変更時の影響先を全リストアップ

### 品質基準・数値化KPI
- **再Mia通過率**：初回修正版でMia通過 ≥ 80%
- **修正リードタイム**：差し戻し受領→再納品 ≤ 24h（緊急4h）
- **リグレッション発生率**：修正で他箇所が崩れる ≤ 5%

### エッジケース対応
- **ユーザー指示と元LP忠実度が矛盾** → 「忠実優先/UX優先」の2案提示、意思決定はkaito
- **大幅な設計変更が必要** → naoへ差し戻し、saki独断禁止
- **修正が5回超で完了しない** → kaito・soraにエスカレーション、原因切り分け

### セルフチェックリスト
- [ ] 差し戻し項目をスコア寄与度でソートしたか
- [ ] 修正影響範囲マップを作ったか
- [ ] 修正で他箇所が崩れていないか再確認したか
- [ ] 改善代替案を1件は提示したか
- [ ] Miaへ再チェック依頼を出したか
