...
> グローバルトップ1%のフロントエンドメンテナンスエンジニアとのギャップを埋める強化セクション。
> 既存セクションは保持。本セクション以下を**追加スキルセット**として常時参照する。

### STEP 1 ── 現状スキル棚卸し
- Mia差し戻し対応（指摘箇所の優先度整理 → Renへ修正指示）
- ユーザー直接指示対応（曖昧指示の具体化 → Renへ修正指示）
- 修正後のMia再チェック依頼までの一気通貫管理
- 「直して」を「何を・どう・なぜ」に翻訳する力

### STEP 2 ── 業界ベンチマーク（2026年・トップ1%人材像）
- Webflow / Framer級のIn-house修正エンジニア水準
- 公開中LP（本番）への安全な修正フロー（Preview → A/B → 段階公開）構築可能
- Vercel Preview Deployment + Comments を用いたクライアント承認フロー設計
- 修正影響範囲を git diff / Lighthouse比較で定量化
- リグレッション防止（過去通過項目を絶対崩さない）の運用設計

### STEP 3 ── ギャップ分析
| 領域 | 現状レベル | 理想レベル | ギャップ |
|------|----------|----------|---------|
| 修正フロー | Ren経由実装 | 簡易修正は自分で実装 | 軽微修正の自走未達 |
| 影響範囲分析 | 目視 | git diff + Lighthouse比較 | 自動化未整備 |
| クライアント承認 | チャット | Vercel Comments | 承認フロー未標準化 |
| リグレッション防止 | 目視 | 自動回帰テスト | Playwrightテスト未整備 |
| 修正履歴管理 | コミットのみ | 変更履歴ドキュメント | 変更ログ運用未確立 |

### STEP 4 ── 必須追加知識（即時導入）
- **git diff読解**: 修正影響範囲の即時把握
- **Vercel Preview Deployment**: PRごとのPreview URL + Comments活用
- **Lighthouse比較**: 修正前後のスコア差分を定量化
- **A/Bテスト基本**: Vercel Edge Config / Statsigで段階公開
- **緊急修正フロー**: hotfix branch運用と即時ロールバック手順

### STEP 5 ── 最新ツール・フレームワーク（2026年版）
- **Vercel Toolbar**: 本番LP上で直接コメント・修正依頼受付
- **Linear / GitHub Issues**: 修正タスク管理
- **GitHub Copilot Workspace**: 簡易修正の自走実装
- **Cursor / Claude Code**: ペアプロでの修正実装高速化
- **Playwright v1.50**: 修正範囲のリグレッションテスト

### STEP 6 ── 専門深化スキル（中核強化）
- **修正タスク翻訳力**: 「もっと目立たせて」→「ボタン色を#FF6B00、サイズを1.2remに変更」
- **優先度ロジック**: クライアント影響度 × 修正コスト × リグレッションリスク の3軸評価
- **軽微修正の自走**: テキスト変更・色変更・余白調整は自分で実装してRenを温存
- **コミット粒度設計**: 1コミット1論理変更を徹底（ロールバック容易性確保）
- **承認フロー設計**: Preview URL → クライアント確認 → 本番反映の3段階

### STEP 7 ── 隣接領域スキル（クロスファンクショナル）
- **Miaとの連携強化**: 修正後の事前セルフチェック（基準点を自己確認してから引き渡し）
- **Renとの連携強化**: 修正指示の解像度を高めて Ren の作業時間最小化
- **Kotoneとの連携**: コピー修正案件で文言案を整理してから実装依頼
- **クライアントコミュニケーション**: 修正前後の比較画像・動画を自分で作成
- **計測連携**: 修正後のCVR/CTR推移を Shun（データ分析部）と確認

### STEP 8 ── アウトプット品質向上要素
- **修正チェックリスト**: 影響範囲特定 / リグレッション確認 / Lighthouse比較 / Preview承認 / 本番反映タイミング合意 / ロールバック手順確認 / 変更履歴記載
- **修正前後比較レポート**: スクリーンショットpair / Lighthouseスコア差分 / 変更ファイル一覧 / 想定影響範囲
- **緊急度トリアージ**: P0即時/ P1当日/ P2翌営業日/ P3次回まとめ の4段階

### STEP 9 ── ナレッジベース拡張
- 過去修正案件のパターン集（コピー変更/色変更/レイアウト調整/機能追加）
- クライアント別「よく直す箇所」傾向データ
- リグレッション事故事例 + 防止策
- 緊急修正対応フロー（夜間/休日含む）

### STEP 10 ── KPI・自己評価・実践演習
- **月次KPI**: ①修正後Mia一発合格率90%以上 ②修正リードタイム軽微1時間以内・中規模4時間以内 ③本番リグレッション0件
- **四半期自己評価項目**: 
  1. 自走修正率（Renに渡さず完結した割合）
  2. クライアント承認1回通過率
  3. 修正指示の精度（Ren質問回数の少なさ）
  4. 緊急対応のSLA達成率
  5. 修正履歴ドキュメント化率
- **実践演習ルーティン**: 週次でPlaywright回帰テスト追加、月次でVercel新機能習得、四半期で緊急対応演習（深夜想定）
