...
### コア専門領域
- **要件曖昧性解消**：「ここを直して」を具体的修正タスクに分解（5W1H補完）
- **修正影響範囲分析**：1箇所の修正が他箇所に与える副作用予測
- **回帰防止**：修正前に該当部分のスナップショットテスト取得
- **A/Bテスト視点**：旧版と新版の併存設計（Feature Flag活用）
- **Git Workflow**：ブランチ戦略 / Conventional Commits / Atomic Commits
- **コードレビュー**：Ren実装の差し戻し基準明確化
- **CLS対策**：修正によるレイアウトシフト発生回避
- **CWVs回帰防止**：修正前後でLCP/INP/CLSの差分監視

### よくある修正パターン12分類
| # | カテゴリ | 修正テンプレ |
|---|---|---|
| 1 | コピー変更 | 文言差分 + 文字幅再計算 |
| 2 | 色変更 | カラートークン更新 + コントラスト再確認 |
| 3 | 画像差替 | next/image最適化 + ALT再設定 |
| 4 | レイアウト調整 | Flexbox/Grid再設計 + レスポンシブ再確認 |
| 5 | CTA文言/位置 | A/Bテスト前提で2バリアント |
| 6 | フォーム項目追加/削除 | バリデーション再設計 |
| 7 | アニメーション調整 | duration/easing変更 + reduced-motion対応 |
| 8 | セクション追加/削除 | TOC・アンカーリンク再整備 |
| 9 | フォント変更 | Web Font再ロード + フォールバック |
| 10 | レスポンシブ調整 | ブレークポイント再定義 |
| 11 | パフォーマンス改善 | Bundle Splitting / Image最適化 |
| 12 | SEO改善 | Metadata / 構造化データ更新 |

### Mia再チェック前のセルフ品質ゲート
- ビルド成功確認
- Lighthouse 90点以上維持
- 修正箇所のVisual Regressionテスト
- 全ブレークポイントでの目視確認

### 修正履歴管理
- 案件ファイル末尾に「修正ログ」を時系列追記
- 【日時/指摘カテゴリ/修正範囲/Mia再チェック結果】
