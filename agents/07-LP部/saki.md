...
### 2026-08-30
- **Claude 5 の視覚推論で修正依頼スクショから対象要素・分類・環境依存を自動抽出**：依頼スクショを投げると Claude 5 が「対象セレクタ／5分類ラベル／推定閲覧環境／同種要素の候補一覧」を初期提示、Sakiは判定確認だけ。効果例：受付ヒアリング往復 平均3回→0.5回、着手までのリードタイム -78%
- **Notion データベースで修正台帳を「対象セレクタ×分類×出所（Mia/クライアント/内部）」の3軸管理**：同一箇所への複数依頼を受付段階で自動集約、自己上書きを構造的に防止。効果例：反映漏れ・上書き事故 月4件→0件、修正の反映精度 +100%
- **Vercel Preview URL + `?v=timestamp` を返信テンプレに自動生成**：Sakiの返信作成時、Vercel APIから最新PreviewURLを取得して?v=タイムスタンプ付きで自動貼付、依頼者は同じ幅で確認可能。効果例：「直ってない」再往復 月8件→1件、修正クローズまでの時間 -62%
- **kotone・バナー部・Kaito の同日リリース束ねを Linear + Slack で自動オーケストレーション**：数値・条件系の修正が入ると kotone に全出現箇所リストを自動発注、バナー部に再生成依頼、Kaito の定時デプロイ枠に自動予約。効果例：部分反映起因の虚偽表示リスク ゼロ継続、束ね忘れ -100%
- **トークン起因の修正を検知したら iro/Hana へ Notion ワークフロー経由で承認依頼を自動起票**：Miaが「トークン起因の疑い」ラベルを付けた差し戻し時、iro/Hana の承認起票までワンクリック、原本と実装の乖離を構造的に防止。効果例：トークン起因の同種修正が別箇所で再発 -95%、承認取り回し -1.2日
- **依頼スクショと閲覧環境から実機再現を BrowserStack Live で即時起動**：受付5分類の「サイズ」「余白」で環境依存疑いの依頼は、依頼者の機種・ブラウザ・in-appブラウザで即時再現、PC Chromeで再現しないケースを構造的に切り分け。効果例：環境依存指摘の初期切り分け 30分→3分、全体フォント調整による二次NG ゼロ
- **修正の意図的変更を Mia へ「Baseline 部分更新申請 API」で自動申請**：Sakiが確定した意図変更をNotion経由で申請すると、Mia のPercyベースラインの対象セレクタだけが差し替わる。効果例：意図変更に紛れた実装デグレの見逃し ゼロ継続、Baseline更新工数 -85%
- **Ao との「デプロイ順序」を Github Actions Environments で自動制御**：フォーム送信先変更時、API 先行→LP 後追いを Environments の依存関係でワークフロー的に強制、順序ミスによる本番失敗の余地なし。効果例：デプロイ順序ミス起因の本番失敗 ゼロ継続、切替の谷間でのリード落ち -100%
