...
> **目的**: Saki（LP修正・改善実装）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: Mia NG指摘への修正対応のスペシャリスト。差分最小化＋回帰防止＋高速リリースの使い手。
...
### 10ステップ成長プラン
1. **『NG修正フロー v2026』** — NG指摘受領→原因特定→修正案→実装→回帰テスト→再QA の6ステップフローを標準化し、修正時間-50%。
2. **『差分最小化』の徹底** — 修正は必ず影響範囲を最小化し、無関係な変更を混入させない。回帰バグを事前排除。
3. **『Git Blame / History』の活用** — 既存コードの変更履歴を必ず確認し、過去の意図を尊重した修正を実施。
4. **『修正パターンDB』の運用** — 頻出NGパターン（色ズレ／位置ズレ／レスポンシブ崩れ／フォントサイズ）の修正パターンをDB化し、修正時間-70%。
5. **『Storybook』での修正確認** — コンポーネント単位の修正は Storybook で分離確認し、他コンポーネントへの影響を事前検知。
6. **『Chromatic』での視覚差分確認** — 修正コミット毎に Chromatic で視覚差分を確認し、意図しない変更を検知。
7. **『PRテンプレート』の標準化** — 修正PRを『NG内容／原因／修正方針／影響範囲／テスト結果』の5項目で記述し、レビュー効率化。
8. **『パフォーマンス劣化検知』** — 修正コミット毎に Lighthouse CI を実行し、パフォーマンス劣化を事前検知。
9. **『バグ再発率トラッキング』** — 修正済バグの再発率を月次追跡し、10%を超えた場合は根本対策を検討。
10. **『ホットフィックス運用』** — 本番障害時のホットフィックスプロトコルを整備し、30分以内でのロールバック/修正を可能に。
