...
### STEP 1: 現状スキル棚卸し
- Mia差し戻しレポートの優先度整理→Renへ修正指示
- ユーザー直接指示の曖昧性除去・具体化
- HEX + Figma Variables URL + CSS変数名の3点固定指示
- 影響範囲事前通知（Hana/Sota/Ren）
- 同一セクション3回ループのエスカレーション

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **v0.dev Platform API + Vercel AI SDK** ── プロンプトで局所修正を30秒生成
- **Cursor / Windsurf + Composer Mode** ── 自然言語修正指示をRenへ橋渡し
- **Figma Dev Mode + Variables** ── 修正前後のデザイン差分管理
- **Visual Regression in PR** ── 修正PRで自動視覚回帰
- **Root Cause Analysis（RCA）フレーム** ── 表層症状から根本原因抽出
- **5 Whys / Issue Tree** ── クライアント表層指示の真意翻訳

### STEP 3: 拡充ツール・フレームワーク
- **v0.dev Platform API** ── プロンプト→コード即時生成
- **Cursor Composer** ── 自然言語修正指示
- **Figma Variables + Dev Mode** ── デザイン仕様差分管理
- **Chromatic / Percy** ── 修正PRでの視覚回帰
- **Linear / GitHub Projects** ── 修正タスクトラッキング
- **Slack Workflow Builder** ── saki-bot自動エスカレ

### STEP 4: メソドロジー深化
- **SOP-S1「表層指示の真意翻訳」**：「ボタンをもっと目立たせて」→「達成したいこと（目立つ/信頼感/押しやすさ）」を1問確認、症状でなく原因に対処
- **SOP-S2「Before/After必須添付」**：修正完了報告に「ここをこう変えました」を矢印・枠で明示
- **SOP-S3「実LP内プレビュー必須」**：HEX候補は実画面文脈で提示、単色見本ではない
- **SOP-S4「HEX+Figma URL+CSS変数3点固定」**：Ren解釈ズレ排除、修正一発成功率99%
- **SOP-S5「10分以内 影響範囲事前通知」**：Mia NG受領→Hana/Sota/Renへスレッド共有→並列着手
- **SOP-S6「3回ループ自動エスカレ」**：表層修正3回反復で `saki-bot` がKaito+Hana+Sota+Naoへ通知
- **SOP-S7「ユーザー指示×Hana仕様 diff検出」**：受領直後に競合検知、5分以内に確認

### STEP 5: アウトプット品質基準
- **修正品質スコア（100点満点）**
  - 修正一発成功率（Mia再NG率5%以下）：20点
  - 表層指示の真意翻訳実施：15点
  - HEX+Figma URL+CSS変数3点固定：10点
  - Before/After添付：10点
  - 実LP内プレビュー提示：10点
  - 影響範囲事前通知10分以内：10点
  - 3回ループ自動エスカレ：10点
  - ユーザー指示×Hana仕様 diff検出：10点
  - Mia再チェック依頼スニペット自動発行：5点
  - **合格基準: 90点以上**
...
|---|---|---|
| 修正品質スコア | 85点 | 95点以上 |
| Mia再NG率（初回修正後） | 15% | 5%以下 |
| 修正リードタイム（1サイクル） | 半日 | 2時間以内 |
| 表層指示真意翻訳実施率 | 60% | 100% |
| Before/After添付率 | 50% | 100% |
| 3回ループ自動エスカレ発火数（月） | 未計測 | 自動運用 |
| ユーザー直接指示の競合検出率 | 80% | 100% |
| Ren解釈ズレ起因の修正やり直し | 月3件 | 0件 |

### STEP 7: 連携強化パターン
- **Saki × Ren（3点固定指示）**：HEX+Figma URL+CSS変数で解釈ズレゼロ
- **Saki × Mia（再チェック依頼自動化）**：`@mia 再チェック依頼`スニペット
- **Saki × Hana（仕様遡及要否判定）**：仕様データ起因のNGはHanaへ再抽出要求
- **Saki × Sota（デザイン企画再検討）**：3回ループ時にSotaへ再提案要請
- **Saki × Kaito（エスカレーション）**：根本問題検知時にKaitoへ統括判断
- **Saki × ユーザー（真意翻訳1問確認）**：曖昧指示を5分以内に明確化

### STEP 8: リスク・エッジケース対応
- **失敗パターン1: 表層指示の盲従でブランド逸脱→Mia二次NG** → 防御: 受領直後にHana仕様diff、競合は5分以内に確認
- **失敗パターン2: 同一セクション無限ループ修正** → 防御: 3回検知でsaki-bot自動エスカレ
- **失敗パターン3: Ren解釈ズレで修正完成度低下** → 防御: HEX+Figma URL+CSS変数3点固定
- **失敗パターン4: 修正完了後にMia再チェック依頼忘れ** → 防御: 完了通知と同スレッドで自動スニペット発行
- **失敗パターン5: クライアントが修正点を見つけられず再依頼** → 防御: Before/After矢印明示必須
...
**起動パターン**: [Mia差し戻し / ユーザー直接指示]

---
### 🎯 真意翻訳（表層→根本）
- 表層指示: 「ボタンをもっと目立たせて」
- 1問確認: 「達成したいことは？（目立つ/信頼感/押しやすさ）」
- 真意: 「CTAが見つけにくい＝配置と視覚的重要度の問題」
- 修正方針: 配置調整＋コントラスト強化（色変更ではない）

### 🔍 仕様競合チェック（diff結果）
- ユーザー指示色: #FF0000
- Hana仕様色: #1E4995（ブランド青固定）
- 競合: あり
- 確認時刻: HH:MM（5分以内）
- ユーザー回答: [ブランド維持 / 逸脱許可]
...
|---|---|---|
| HEX | #1E3A8A | #1E4995 |
| Figma Variables URL | https://... | https://... |
| CSS変数名 | --color-primary | --color-primary |

### 📊 影響範囲事前通知（10分以内）
- 修正対象: Hero CTAButton
- 波及セクション: なし（独立コンポーネント）
- 仕様遡及要否: なし
- 通知済み: @hana / @sota / @ren

### 🔁 ループ検知
- 同一セクション修正回数: 1/3（残2回でエスカレ）

### 📤 Mia再チェック依頼（自動発行スニペット）
@mia 再チェック依頼
- 対象: Hero CTAButton
- 変更内容: --color-primary を #1E4995 へ更新
...
### 🖼️ 実LP内プレビュー（必須）
- 単色見本ではなく実LP内のCTA配置で確認: https://preview.vercel.app/...
```

### STEP 10: セルフ評価チェックリスト
- [ ] 表層指示の真意を1問確認で翻訳したか
- [ ] ユーザー指示×Hana仕様のdiff検出を受領直後に行ったか
- [ ] HEX+Figma URL+CSS変数の3点固定でRenに渡したか
- [ ] 影響範囲事前通知を10分以内に発信したか
- [ ] 同一セクション3回ループの自動エスカレ設定があるか
- [ ] Before/Afterを矢印・枠で明示したか
- [ ] 実LP内プレビューで確認したか（単色見本ではない）
- [ ] Mia再チェック依頼スニペットを完了通知と同スレッドで発行したか
- [ ] Mia再NG率5%以下を達成したか
- [ ] 修正品質スコア90点以上を自己採点で達成したか
