...
### 現状スキル評価
Mia 差し戻し対応、ユーザー直接指示対応、情報密度分類、トークン修正判定、数値・条件修正の全出現箇所リスト化まで対応済み。ただし「修正依頼の分類辞書化」「A/B テストによる修正効果検証」「デザインシステム変更ログ管理」「クライアント直接修正フローの言語化」が弱い。

### 特定した改善余地
- 修正依頼の分類（意匠 / 情報密度 / 訴求変更 / 法務 / 数値）が Saki の頭の中にしかなく、退勤時に引き継げない
- 修正前後の Core Web Vitals 差分を計測せず、意匠修正が LCP を悪化させても気付かない
- クライアントが Slack/メールで送ってくる曖昧な指示を Notion チケットに正規化する導線が未整備
- Vercel の Preview URL を修正案ごとに用意して比較レビューする運用がない
- 修正内容の差分を GitHub PR / Chromatic snapshot で可視化して Mia・Kaito へ渡す仕組みがない

### 追加スキル10選（オーバースペック化ロードマップ）
1. **修正依頼分類辞書（意匠/情報密度/訴求変更/法務/数値/技術）の Notion DB 化** — 分類→一次担当（Ren/kotone/iro/nori）ルーティングを Zapier で自動化。KPI：一次判定所要 30分→5分、誤ルーティング 0
2. **修正案ごとの Vercel Preview URL + Chromatic snapshot 自動発行** — GitHub PR 作成時に Preview URL とスナップショットURLを Slack #let-lp-review へ自動投稿。KPI：ユーザー確認→修正着手までのリードタイム 24h→4h
3. **before/after Web Vitals 差分計測 (Lighthouse CI)** — 修正 PR ごとに LCP/INP/CLS の差分を PR コメントへ。KPI：意匠修正で LCP +200ms 超は自動ブロック、Vitals 悪化 0
4. **Notion フォーム経由の「LP修正リクエスト」正規化窓口** — クライアントの Slack/メール曖昧指示を Notion フォーム記入へ誘導する運用テンプレ。KPI：曖昧指示 90% → 20% 削減、確認往復 3回→1回
5. **修正差分の GitHub PR テンプレ標準化** — 「修正箇所・修正内容・期待結果・Mia 再チェック要否・法務再チェック要否」の6項目を PR 説明必須化。KPI：PR レビュー 15分→5分、Mia 再チェック抜け 0
6. **A/B テストによる修正効果検証（Vercel Edge Middleware + PostHog）** — 意匠修正で CVR 影響が懸念される案件は 50/50 A/B で 2週間検証。KPI：CVR 悪化修正の本番投入 0件
7. **Storybook 上での修正プレビュー環境常設** — コンポーネント単位の修正は Storybook で先に確認し、ページ全体反映前にレビュー。KPI：ページ全体ビルド待ち 3分×N回 → 0
8. **修正ログの週次サマリを Slack へ自動投稿** — GitHub Actions で週次 PR まとめを Slack #let-lp-weekly へ。KPI：クライアント別修正頻度・分類割合を月次レポート化、Kaito の改善方針決めに活用
9. **修正着手前チェックリスト（法務・数値・技術影響）の Notion テンプレ** — 全修正 PR 起票時に必須記入。KPI：数値修正時のバナー再生成発注抜け 0、法務再チェック抜け 0
10. **クライアント直接修正 (CMS/Contentlayer) の権限設計と教育資料整備** — 頻繁な文言変更はクライアント自己解決、Saki は構造変更に集中。KPI：文言修正の Saki 対応 -60%、クライアント満足度 +15pt

### 新規ナレッジソース・ツール
- Vercel Preview Comments — Preview URL 上での直接コメント→Notion 起票
- PostHog — 修正効果 A/B の CVR 測定
- Chromatic — 修正差分の視覚回帰
- Lighthouse CI — 修正 PR ごとの Vitals 差分
- Linear（or Notion Task）— 分類→担当ルーティング

### 連携強化ポイント
- **Ren**：PR テンプレ・Preview URL・Chromatic 自動発行の CI を Ren と共同整備、Saki は分類とルーティングに集中
- **kotone**：文言・情報密度分類は Saki が受けて即 kotone へ回すルーティング表を Slack ワークフローへ組み込み
- **nori / gen**：法務起因の修正は必ず nori、建設業慣行起因は gen へ二次確認する分岐を分類辞書に明記

### 実践シナリオ例
桝本レッカーのクライアントから Slack で「なんか給料の見せ方を変えたい」と曖昧依頼が来た。Saki が Notion フォームへ誘導→「日給1.5万→月給30万換算表示」に正規化。分類：訴求変更＋数値→nori へ景表法チェック依頼、同時に kotone へ換算根拠のコピー依頼、hiro へバナー再生成発注。Ren が Vercel Preview を PR で発行、Chromatic snapshot が Slack へ自動投稿され Mia が事前 30% を検出。Lighthouse CI で LCP 差分 +8ms のみ、A/B テスト無しで即本番。クライアント確認→反映まで 6時間で完了、従来 3日から短縮。

### 2026-08-29 Daily Knowledge Log 追記
- **修正依頼分類辞書 v1 を Notion `LP修正DB` に定義**：意匠/情報密度/訴求変更/法務/数値/技術 の6分類、それぞれの一次担当（Ren/kotone/iro/nori/gen）を Zapier ワークフローで自動ルーティング。誤ルーティングを月次レビューで学習させ、四半期で 90%→98% へ引き上げる
- **Vercel Preview URL + Chromatic snapshot の Slack 自動投稿を CI 化**：GitHub Actions で PR 作成時に #let-lp-review へ両 URL 投稿、クライアント確認→着手までのリードタイム 24h→4h を7社共通で狙う
- **修正 PR に Lighthouse CI 差分コメント必須化**：意匠修正で LCP +200ms 超は自動ブロック、Vitals 悪化した意匠変更が本番に載る事故を物理予防
- **クライアント曖昧指示を Notion フォーム経由に正規化**：Slack/メール直依頼は「フォーム記入をお願いしています」テンプレで折り返す運用を Kaito と合意。確認往復 3回→1回、依頼受領〜着手 60% 短縮見込み
- **文言修正のクライアント自己解決化を Contentlayer/Sanity 権限で推進**：頻度上位クライアント（翔星建設・cantera）から順に CMS 権限＋操作マニュアル動画（3分×5本）を配布。Saki の文言修正対応工数を月 12h → 5h まで削減目標
