...
### 2026-09-04
- **Vercel Edge Config で「数値・文言だけの微修正」を再デプロイなしで即時反映するレーンを分離する運用**：給与・電話番号など純粋テキストの変更は本来コード修正＋バナー再生成の同日束ね反映（2026-08-18参照）で回すが、Edge Config へ切り出せる項目は数分で更新でき、失敗時のロールバックもキーの差戻しだけで済む。kotone から受け取る全出現箇所リスト（2026-08-27参照）を「バナー・OGP・JSON-LD に焼かれていない純粋テキスト」と「焼き込み済み」に二分し、前者は Edge Config レーン、後者は 5面チェックリスト（2026-09-02参照）で回す従来レーンへ振り分ける。hotfix 3類型（2026-09-01参照）に該当しない小修正の反映時間を桁で縮める代わりに、Edge Config 側は焼き込み系の修正には絶対に使わないという振り分けを Saki の受付段階で固定する
- **v0 Platform API 経由の「Issue → ブランチ作成＋修正パッチ下書き自動生成」を受付台帳の分類ラベル起点で回す連携**：AI が PR 差分要約を下書きする運用（2026-08-03参照）は 2026 年後半に「Issue 本文からブランチ作成＋パッチ下書きまで一気通貫」に進化した。Saki は受付台帳の5分類ラベル（色／サイズ／写真／余白／情報密度、2026-08-27参照）と対象セレクタ・`data-testid` を Issue のテンプレへ流し込むだけにし、v0 が生成した下書きは Ren が実装として引き取る前に Hana の tokens.json 差分と iro の配色設計との照合を人の関門として残す。ブランド値以外の文言・レイアウト系だけ自動下書きを通し、原本変更が必要な系統は Hana/iro 承認経路（2026-08-27参照）へ強制的に迂回させて、AI 下書きによる原本と実装の乖離を Saki の受付段階で潰す
- **Playwright Visual Diff の CI 標準化を「Mia 再チェック依頼の前倒し関門」として噛ませる連携**：PR プレビューへの pixel 差分自動コメント（2026-08-03参照）は 2026 年に「SP 375px／PC 1440px／in-app ブラウザ幅」の3幅並列撮影が既定になった。Saki は Mia へ再チェックを投げる前に CI の差分レポートで意図外の崩れ（特に prefers-reduced-motion／ダーク分岐の退行 2026-08-03参照）を潰し、Mia には「意図した差分」だけを1文で添えて回す。受付段階で自動取得している画面幅・in-app 判定（2026-09-01参照）と CI の撮影幅を一致させておくことで、依頼者の再現環境・CI の検査環境・Mia の確認環境が同一幅で揃い、「PC では再現しない指摘」（2026-08-16参照）が Mia 段階で蒸し返されない
- **CodeRabbit・Claude Code Reviews による PR 自動レビューで、機械判定可能な指摘を Saki の目から外して方向性判定に集中する連携**：alt 漏れ・APCA コントラスト退行（2026-08-03参照）・命名規約逸脱・トークン外の生 HEX 混入といった規約系の指摘は 2026 年に bot レビューへ寄せるのが標準になった。Saki が人力で拾っていた表層チェックを bot に任せ、Saki 自身は「印象語→5分類マッピング」（2026-08-16参照）と「数値ループを方向性ズレに切り替える判定」（2026-08-05参照）へ集中する。bot が指摘したトークン逸脱系は自動修正コミットを許可せず、Hana/iro 承認経路（2026-08-27参照）を必ず経由させて、ブランド値の勝手な書換えを bot 導入の副作用にしない
- **`main` から cherry-pick で staging を組む「レーン単位ステージング」を今週便／次週便の可逆性担保に使う運用**：週次定時デプロイ枠の3レーン運用（2026-09-01参照）は「今週便に含める修正だけを本番相当環境で確認したい」需要が強く、1タスク=1コミット（2026-06-12参照）の粒度と相性が良い。staging ブランチを長期保持せず、リリース便ごとに `main` から対象コミットを cherry-pick して都度作り直すことで、レーンから外れた作業や次週便の未検証コミットを巻き込まず、便単位で pre-fix タグ（2026-07-03参照）付きの切戻し可能状態を作れる。Kaito のデプロイ枠と1対1で対応するため、束ね反映（2026-08-18参照）の単位がそのままステージング検証の単位となり、レーンをまたぐ自己上書き（2026-08-05参照）を staging 段階で先に検出できる
