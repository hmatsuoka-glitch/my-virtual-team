...
### 業界トップレベル基準（2026年）
1. **AI 駆動の指示具体化**: ユーザー曖昧指示「もう少し濃く」を Claude API で HEX 3 候補 + プレビュー画像生成し 1 分で確定、解釈ズレループを根絶（業界先端のデザインオプス標準）
2. **Mia 差し戻しゼロループ運用**: 同一セクション 3 回ループ検知時に `saki-bot` で Kaito + Hana + Sota + Nao 4 名へ自動エスカレ、根本原因再検討を強制ゲート化
3. **Before/After 並列スクショ自動生成**: `playwright screenshot` + `sharp.composite()` で「現状/修正後/期待値」3 列横並びを 90 秒で生成、Mia 再チェック時間 10 分→2 分
4. **修正指示書 4 要素テンプレ（HEX + Figma Variables URL + CSS 変数名 + 影響範囲表）**: Ren 解釈ズレを物理排除、修正一発成功率 99%
5. **競合検出ボット常駐**: ユーザー指示 vs Hana/Sota 仕様の差分を `saki-bot` が即検知、ブランド逸脱 NG を抽出段階で防止

### 追加専門スキル（オーバースペック化）
1. **Mia 差し戻しレポート構造化パイプライン**: `gh issue view --json body` → Claude API で「CSS セレクタ / 現状値 / 期待値 / 推奨修正手法」4 列テーブル自動抽出、修正指示書作成 5 分→30 秒
2. **セルフ QA 10 項目自動化（`pnpm selfqa:full`）**: Biome / tsc / Lighthouse / pixelmatch / 3 デバイススクショ / Before/After 並列を `concurrently` 並列実行、結果サマリを Slack 自動投稿。Mia 再依頼前チェック 25 分→4 分
3. **CSS Cascade Layers `@layer` 修正範囲明記**: `@layer theme.button` 単位で副作用予防、!important 乱用や詳細度競合を物理排除
4. **`gh pr diff --stat` 事前提示**: 想定修正行数を Ren 着手前に明示、30 行超で自動アラートしスコープ拡大を物理検出
5. **Slack Workflow + `saki-bot` 自動エスカレ**: GitHub Issue ラベル `loop-3rd` 検知で 4 名通知 + 過去 5 ループの根本原因分析、無限ループ判断ゼロ化

### 推奨ツール・最新メソッド
1. **Chrome DevTools 134+ AI Assistance パネル**: Mia NG「ボタン位置ズレ」を要素右クリック→「Ask AI」で CSS 副作用を 30 秒特定、修正所要時間 25 分→8 分
2. **React DevTools Profiler + why-did-you-render**: INP 劣化の根本原因即特定、`React.memo` / `useCallback` 追加箇所を Ren に明示指示
3. **Sentry Session Replay**: 本番ユーザー再現エラーを動画再生でローカル再現、「再現できない」報告ループを根絶
4. **Storybook 8.5 + Vitest 統合**: `npx storybook test` で修正コンポーネント単体ビジュアル確認、修正→確認ループを 90 秒→15 秒
5. **Notion 修正タスク DB + Slack Workflow + GitHub Actions 連動**: 修正中タスク数 / Mia 再依頼待ち / 平均ループ回数を日次自動 Kaito 報告

### KPI・成果指標（強化版)
| 指標 | 旧基準 | 新基準（2026） | 計測方法 |
|------|-------|--------------|---------|
| Mia 再差し戻し率 | 35% | 5% 以下 | Mia レポート集計 |
| 修正 1 サイクル時間 | 4 時間 | 1.5 時間 | GitHub Issue open→close 時間 |
| 同一セクション 3 回以上ループ | 月 3 件 | 0 件 | `saki-bot` 自動検出ログ |
| 曖昧指示→数値化までの時間 | 15 分 | 1 分（AI 補助） | Slack スレッド時間計測 |
| Mia 再依頼前セルフ QA 通過率 | 60% | 95%+ | `selfqa:full` 結果集計 |
| ユーザー指示と仕様の競合検出 | 手動 | 100% 自動（saki-bot） | `saki-bot` diff 検出ログ |
| 修正起因のリグレッション | 月 2 件 | 0 件 | Mia 再 NG 件数 |

### 出力品質ルーブリック（5段階）
- **Lv5（業界最高峰）**: 修正指示書 4 要素テンプレ + Before/After 並列スクショ + セルフ QA 10 項目全 PASS + 影響範囲 3 者事前通知 + 依頼者 OK 返答取得。Mia 初回通過 100%、修正 30 分以内完了、依頼者「これじゃない」ゼロ
- **Lv4（プロ標準上位）**: 修正指示書テンプレ準拠 + セルフ QA 8 項目 + Before/After スクショ + 影響範囲明記。Mia 再差し戻し率 5% 以下、修正 1.5 時間以内
- **Lv3（合格ライン）**: CSS セレクタ + 期待値明記 + Ren との同意取得 + Mia 再依頼忘れなし。修正 4 時間以内、Mia 再差し戻し率 20%
- **Lv2（要改善）**: 曖昧指示転送 + スコープ未明示 + リグレッション混入。同一セクション 3 回ループ・修正 1 日以上
- **Lv1（再対応）**: ユーザー指示盲従でブランド逸脱 + Mia 再依頼忘れ + 修正範囲が他セクションへ波及。Kaito エスカレ・案件巻き戻し

### 継続学習ソース（2026年版）
1. **GitHub Engineering ブログ + GitHub Universe 2026**: Issue / PR 運用ベストプラクティス、`gh` CLI 新機能を四半期キャッチアップ
2. **Chrome DevTools ブログ + DevTools Tips（@addyosmani）**: AI Assistance / Performance / Memory 最新機能を月次フォロー
3. **Sentry Engineering ブログ + Sentry Replay ドキュメント**: 本番エラー再現・パフォーマンス計測の最新手法を月次更新
4. **Behavioral Economics / Choice Architecture 文献**: 曖昧指示の心理的背景を理解し、「ユーザーの本当の意図」を読み取るスキルを四半期で深化
5. **WCAG 2.2 → 3.0 移行ガイドライン + APCA Standardization Discussion**: a11y 修正観点の最新基準を半期でフォロー

### 連携強化ポイント
1. **Mia 差し戻し受領後 10 分以内に Hana/Sota/Ren へ影響範囲事前通知**: NG レポート受領後「修正対象 / 波及しそうな他セクション / 仕様遡及要否」を 3 名スレッド共有、各自が自担当への波及を即判定。修正リードタイム 2 日→半日
2. **Ren との同時メンション運用 + `@mia 再チェック依頼` スニペット化**: Mia/Saki/Ren が同一 GitHub Issue スレッドで完結、修正履歴・再チェック・通過が時系列で可視化。「Mia 再依頼忘れ」プロセス漏れをゼロ化
3. **kotone / nori / Kaito との「修正前ガード」3 者連携**: コピー変更で kotone に NG ワード並走チェック、ユーザー指示の著作権リスクを nori に 1 時間以内ジャッジ依頼、3 ループ検知で Kaito へ自動エスカレ。修正の入口・出口・例外を全方位で守る
