...
### 2026-09-21
- **2026年最新トレンド：Cursor Composer / GitHub Copilot Workspace で「Miaのdiff JSON→修正コミット自動生成」ワークフロー**：Miaから受け取る`{ selector, expected, actual }`形式のJSON差分を、Cursor Composerに直接投入するとCSS/JSXの修正コードが自動生成される。従来「差分を読む→ファイル探す→修正」の3ステップが1コマンドに集約（実務影響：単純CSSズレの修正時間が5分→30秒、複数箇所差し戻しも並列処理可能）
- **2026年最新トレンド：Vercel `vc rollback --interactive`によるインクリメンタルロールバック**：修正後のデプロイで別の箇所が壊れた場合、旧本番デプロイに一発戻し可能に。従来はGitのrevert commit→再ビルドで15分要していたのが30秒で完了（実務影響：即時レーン案件（CV阻害）の復旧速度が劇的向上、クライアントへの謝罪連絡が減少）
- **2026年最新トレンド：`prefers-reduced-motion` + View Transitions API併用実装で「アクセシビリティ配慮の修正」パターン標準化**：AOS等の`opacity: 0`初期状態残留（2026-09-13参照）が全体案件で頻発、`@media (prefers-reduced-motion: reduce)`ブロックの初期状態を`opacity: 1`に強制上書きする修正パッチをスニペット化しSaki専用リポジトリに常備（実務影響：シニア求職者・視覚過敏者の離脱防止、修正コード5行の使い回しで対応）
- **失敗パターン：Miaからの差分JSONを機械的に適用したが、Iroの設計意図（コントラスト比・APCA Lc値）を破壊し、承認後に色覚多様性チェックで再NG** → 回避策：色関連の修正は必ずIroのCSS変数納品書と照合し、`--primary`等のトークンから外れた直接HEXを書かない。修正コミット前に`stylelint-a11y`の`color-contrast`ルールで自動検証（理由：Miaはピクセル一致を優先しIroの意図までは追わない、色修正で3案件でアクセシビリティ後退が発生）
- **失敗パターン：緊急修正でホットフィックスをmainに直push、次回リリース時に他ブランチとコンフリクトして事故** → 回避策：即時レーン案件でも必ず`hotfix/YYYY-MM-DD-<issue>`ブランチを切り、PR経由でmergeする。ホットフィックス完了後は`git cherry-pick`で開発ブランチにも即座に反映（理由：緊急時のショートカットが技術的負債になる、Kaitoの週次デプロイ枠と競合すると復旧が困難）
- **品質チェックポイント：修正後の`?v=<timestamp>`URLをLINE内蔵ブラウザ・Instagram内蔵ブラウザ・X内蔵ブラウザで3種確認**：クライアント担当者の確認経路は主にSNS内蔵WebViewで、各アプリの独自レンダリング（Cookie保持・キャッシュ挙動・広告除去処理）が異なる。3種WebViewのスクショを完了報告に添付し「見える経路すべてで確認済み」を証跡化（実務影響：「直ってない」再連絡がほぼゼロ化）
- **品質チェックポイント：修正コミット時のcommit messageに「原因（1文）」「対応（1文）」「再発防止（1文）」の3行を必須化**：後日別担当者が触った時のコンテキスト共有、Sora QA時の履歴確認、クライアント質問時の即答に活用。GitHub CopilotのCommit Message生成をカスタマイズして3行フォーマットを標準出力（実務影響：属人化解消、Sora QAでの証跡質問が減少）
- **効率化テクニック：Chrome DevTools `Recorder`パネル + Puppeteer変換で「毎回の受付時テスト送信」を自動化**：フォーム故障を疑う初手のテスト送信（2026-09-13参照）を、Recorderで記録した操作をPuppeteerスクリプトに変換し1コマンド化。7社分を並列実行し3分で全社の送信可否判定完了（実務影響：故障判定に要した15分作業が3分に）
- **Kaito連携の小ヒント：修正PR時にKaitoへ「Vercelプレビュー生成完了通知」を`gh pr create --body "cc @kaito"`で自動メンション**：手動でSlack/LINE連絡する手間を廃止、GitHub Actionsのworkflow_dispatchで統合。Kaitoの承認確認→本番昇格までのリードタイムが体感で30%短縮
