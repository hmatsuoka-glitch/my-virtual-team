...
### 2026-09-10
- **[新スキル] 「Chrome DevTools MCP + Playwright Test で修正影響範囲を機械判定」する差戻し前アセスメント**：依頼受付時に MCP 経由で対象URL を Chrome DevTools で開き、Element inspect → CSS Selector 特定 → 同一セレクタが他のセクション/LP でも使われているかを Playwright で全 LP 横断 grep。従来「共通起因か個別か」の判定に平均15分かかっていたのが2分（87%削減）、他 LP への横展開漏れ（2026-09-02参照）を機械化
- **[新ナレッジ] 「Feature Flag（LaunchDarkly / Vercel Flags）」導入で「本番に反映せず依頼者に見せる」フローの実現**：Vercel Flags SDK（2026年GA）で本番URLに Feature Flag を仕込み、Cookie を持つ依頼者だけが新バリアントを見られる状態を作れる。依頼者との Before/After 合意（2026-09-01参照）を本番デプロイ前に完了でき、正式反映後に「やっぱり前が良かった」で切り戻す往復（2026-09-02の復元依頼）を根本的に減らす
- **[効率化ルール] 「PR コメント自動生成ボット」で Before/After 2点セット・修正差分ハイライト・Preview URL を1コマンド化**：Ren の PR に対し、GitHub Actions で①SP幅375px+PC幅1440pxの Before/After スクショ、②`git diff` の視覚化、③Preview URL、④影響範囲（Chrome DevTools MCP 判定結果）を自動生成→ PR コメントに投稿。従来 Saki が手作業で束ねていた10分/件が1分/件（90%削減）
- **[失敗パターン→回避策] 「LLM 生成の修正提案を検証なしで Ren へ丸投げして無関係箇所まで書き換わる」**：Copilot Chat で「この余白を広げて」と依頼すると、指定箇所以外の同種セレクタまで書き換わることがある。回避策は LLM 生成の diff を必ず「変更行数 ≤ 想定行数の1.5倍」の閾値でチェックし、超過時は Saki が手動レビュー→分割 PR に切り分ける。単一責任 PR ルールを機械強制
- **[連携強化] Mia の差し戻しレポート「3層構造（機械判定＋目視＋LLM候補）」（Mia 9/10参照）に対する着手優先順位ルール**：Mia のレポート受領時、①機械判定（Playwright/SSIM/LCP/INP）＝即着手／②目視所見＝当週便／③LLM 候補＝参考のみ着手不要、の3レーンに機械振り分け。LLM 幻覚指摘に振り回される時間が構造的にゼロになり、Mia→Saki→Ren のパイプラインが高速化
- **[品質チェック] Sora QA 提出前セルフチェック2項目**：①「共通起因判定を Chrome DevTools MCP + Playwright grep で機械実施し、他 LP への横展開漏れが無いことを確認済みか」（2026-09-02参照）、②「完了報告時に Vercel デプロイ完了＋`?v=`付きURLの新表示確認済み＋依頼者向け Before/After 2点セットが揃っているか」。両方 GO で Sora へ
