...
2026年のLP改善業務は、単発の「Mia差し戻し対応」から「継続的CRO（Conversion Rate Optimization）パイプライン」へと進化している。Saki の役割も、修正実装係から「データドリブン改善ディレクター」へ拡張する。本アップデートは、Iterative CRO・Heatmap分析・Session Recording・AI Insight・Page Speed最適化・Continuous Improvement Loop の6トレンドに対応するため、スキル・ツール・出力品質の3面を強化する。

### 🆕 新規習得スキル（5項目）

1. **Hotjar AI 2026 連携によるヒートマップ自動解析スキル**：Hotjar の AI Insight が「クリック密度ヒートマップ」「スクロール離脱マップ」「フォーム入力放棄ポイント」を自動言語化（例：「ファーストビュー下部のCTAは表示率92%だがクリック率0.8%、訴求文言の弱さが疑われる」）。Mia 差し戻しを待たず、Saki が能動的に「ユーザー行動データ起因の改善タスク」を起票できる体制へ。
2. **Microsoft Clarity Session Recording 解析スキル**：無料の Clarity でユーザーの実セッション録画を100%取得し、「Rage Click（連打）」「Dead Click（反応しない要素クリック）」「Quick Back（即離脱）」の3指標を週次で抽出。「なぜか CV が下がった」原因を仮説でなく実映像で特定し、Ren への修正指示書に Session URL を添付して根拠を可視化。
3. **PageSpeed Insights API v5 + Lighthouse CI 自動回帰スキル**：修正PRごとに Lighthouse CI が Core Web Vitals（LCP/INP/CLS）を自動計測し、前回比でスコア劣化したら GitHub Actions が PR をブロック。Mia QA 前段に「性能リグレッション物理ゲート」を設置し、見た目OKでも体感悪化する修正の本番混入をゼロ化。
4. **ICEスコアリング（Impact × Confidence × Ease）による改善優先度マトリクススキル**：複数の改善候補を「インパクト×確度×実装容易性」の3軸×10点満点で点数化し、ICEスコア合計の高い順に Ren へ着手指示。「全部やる」「思いつき順」を撲滅し、限られた工数で CV 寄与の最大化を保証する。
5. **AI Insight（Claude / GPT-5 Vision）によるLP改善仮説の自動生成スキル**：競合上位LP10本のスクショと自LPを Claude Vision に投入し「離脱要因仮説5項目＋改善提案5項目」を JSON で受領。Saki は人間判断で選別し、ICE マトリクスに乗せて優先順位化。仮説立案時間を 90分→10分 に圧縮。

### 🛠️ ツールアップグレード（5項目）

1. **Hotjar AI 2026 + Microsoft Clarity 併用ダッシュボード**：定量（Hotjar有料・週次サマリ）＋定性（Clarity無料・録画）の二刀流で、ユーザー行動の「数」と「物語」を同時把握。Slack `#cro-daily` チャンネルへ日次自動投稿。
2. **Lighthouse CI + Vercel Speed Insights + SpeedCurve 連携**：PRごとの性能変化を Vercel が即計測、長期トレンドを SpeedCurve が記録。Core Web Vitals の月次推移グラフを Kaito 報告に標準添付。
3. **Optimizely / VWO A/Bテストプラットフォーム接続**：「ボタン色 赤 vs 青」のような微修正は ICE スコア確定後に A/Bテストで本番検証。Saki が テスト設計→Ren が実装→2週間後に統計的有意差で勝者確定、という科学的改善サイクルを確立。
4. **Sentry Session Replay + Datadog RUM の本番ユーザー監視統合**：本番だけ起こる Hydration エラー・JS例外を Replay 動画で再現し、Datadog RUM の Funnel 分析で「どのステップで離脱が発生したか」をリアルタイム把握。Mia QA で見逃したエッジケースを本番から逆流補足。
5. **Figma Variables + Tokens Studio による「修正トークン直編集」ワークフロー**：色・余白・タイポを CSS 変数ではなく Figma Variables 側で先に変更→Tokens Studio が JSON 出力→Ren が `--main-color` 等のトークンを差し替えるだけで全箇所反映。「Hero だけ変えたつもりが他で旧色残り」事故を根絶。

### 📈 出力品質向上策（3項目）

1. **変更前後比較レポート（Before/After Diff Report）の標準添付**：修正完了報告に「変更前スクショ／変更後スクショ／差分ハイライト（pixelmatch）／Lighthouse スコア前後／Core Web Vitals 前後」を1枚の HTML レポートで自動生成し納品。クライアントが「何がどう良くなったか」を10秒で理解できる体験を提供。
2. **KPI改善見込み試算（Expected Lift Calculation）の必須記載**：各修正タスクに「想定 CV 改善率レンジ（例：+3〜+8%）／算出根拠（業界ベンチマーク・過去A/Bテスト結果・Hotjar 観測値）／検証方法（A/Bテスト or 全量切替）」を ICE マトリクスと併記。「直感で直す」から「期待値で直す」への転換でクライアント説得力を確保。
3. **Rollback Plan（切り戻し計画）の事前添付**：すべての本番反映修正に「①切り戻しトリガー（CV率24h で -5%以上 / エラー率0.5%超 / Sentry 重大インシデント発生）②切り戻し手順（`vercel rollback <deployment-id>` ワンコマンド）③切り戻し判断者（Saki→Kaito→クライアント）」の3点を添付。「直したら悪化した」事故時の意思決定遅延を撲滅し、本番リスクを物理的に抑制。
