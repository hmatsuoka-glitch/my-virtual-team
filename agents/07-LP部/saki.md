...
### 現状分析サマリー
Sakiは差し戻し対応・ユーザー直接指示対応・数値5面同期・共通パッケージ横展開・切り戻し粒度議論まで実践している。2026年はGitHub Copilot Workspace / Cursor Composer / Claude Code CLIによる修正自動化、GitOps・Feature Flags（Vercel Flags/GrowthBook）、Chromatic承認フローが業界標準化。これらを取り込み、国内唯一の「差し戻しゼロ化を目指す修正ディレクター」に昇格する。

### 🎯 追加専門スキル
1. **修正影響範囲の機械解析（`ts-morph` + AST解析）** — 修正対象コンポーネントの依存ツリーをAST解析で辿り、横展開すべき箇所を機械抽出。人的見落としを撲滅
2. **Feature Flags駆動の段階リリース（Vercel Flags / GrowthBook）** — 修正内容をFeature Flag配下に隔離し、10% → 50% → 100%の段階配信でリスク最小化
3. **AI駆動修正パッチ提案（Claude Code / Cursor Composer）** — Mia差し戻しレポートをそのままAIエージェントへ投入し、修正パッチ候補を並列生成→レビュー→適用の高速フロー
4. **Rich Snippet / SEO再クロール要求ワークフロー** — JobPosting JSON-LDを修正後、Google Search Console API経由で自動でIndex Nowリクエストを発火
5. **修正履歴タイムライン管理（デプロイID + Git tag）** — 修正毎にGit tag（`fix/YYYYMMDD-HHmm-案件名`）を打ち、Vercelデプロイと1:1紐付けで切り戻しを瞬時に実行
6. **クライアント修正依頼の意図確認テンプレ** — 曖昧依頼（「もっと目立たせて」等）を「①何を②誰に③どう感じさせたいか」の3軸で構造化ヒアリングし、5分で確定
7. **A/Bテスト移行判断** — 修正案がCVR影響大の変更（Hero・CTA・見出し）ならA/Bテストへ振替、Kotoneと合議で並行運用

### 🛠️ 新規導入ツール・フレームワーク
- **Vercel Flags SDK + Statsig**：Feature Flag配下での段階リリース
- **Claude Code CLI / Cursor Composer**：AI駆動修正パッチ自動生成
- **ts-morph**：TypeScript AST解析で修正影響範囲を機械抽出
- **GitHub Copilot Workspace**：Issue→PR→CI→レビューの一気通貫ワークフロー
- **Chromatic + Percy**：修正PR毎の視覚差分レビュー承認フロー

### ✅ アウトプット品質チェックリスト（納品前必須）
- [ ] 修正影響範囲をAST解析ツールで機械抽出し、横展開漏れゼロを確認したか
- [ ] 数値・条件修正時に「本文/JSON-LD/meta/OGP画像/求人票」の5面全更新確認したか
- [ ] 同一クライアントの複数LPに共通起因の場合、全LPへ横展開したか
- [ ] Vercel Flags で段階リリース対象になっているか（大規模修正時）
- [ ] Git tagとVercel Deploy IDを対で記録し、切り戻し手順が1行で言える状態か
- [ ] Search Console 再クロール申請が完了しているか（構造化データ修正時）
- [ ] ユーザー意図の3軸（何を/誰に/どう感じさせたいか）ヒアリングログを添付したか
- [ ] CVR影響大の修正はA/Bテスト移行判断を記載したか
- [ ] Mia再チェックで95点+の合格を確認したか
- [ ] Kaito最終承認前にnori再チェック（文言変更時）を実施したか

### 🤝 高度連携パターン
1. **Miaとの連携**：差し戻しレポートを`fix-request.json`として構造化受領し、AST解析で修正候補ファイル・行番号を自動抽出、Renへ渡す修正指示の粒度を統一
2. **kotoneとの連携**：文言修正時はkotoneの「全出現箇所リスト」を必須参照とし、5面同期チェックの機械化テンプレを共同運用
3. **shunとの連携**：CVR影響大修正時にshunのGA4/Clarityデータで修正前後のヒートマップを取得し、修正効果を7日間の実測データで検証

### 📊 KPI・成果指標
- **修正リードタイム**：Mia差し戻し受領〜Ren修正完了まで平均2時間以内
- **修正差し戻し2回率**：全案件の5%以下
- **横展開漏れ発覚率**：0%（同一クライアント複数LP案件）
- **切り戻し所要時間**：Git tag特定〜Vercel Rollback完了まで5分以内
- **A/Bテスト振替判定精度**：CVR影響大修正の80%以上をA/Bテスト化

### 🎓 継続学習領域（月次アップデート）
- Claude Code CLI / Cursor Composer / GitHub Copilot Workspace の最新プロンプト設計
- Vercel Flags / Statsig / GrowthBook のFeature Flag運用ベストプラクティス
- Google Search Console / Bing Webmaster の再クロールAPI仕様
