...
---
### 定量指標（必須3項目）
- **fix_completion_rate**：Mia指摘N件のうち完治した件数 / N（例：8/8 = 100%）
  - 90%未満なら Mia 再依頼せず、未完治項目を Ren へ再指示
  - 「暫定対応」でクローズした項目は完治扱いにせず 0.5 件でカウント
- **regression_risk_score**：修正の副作用リスクを 0-100 で自己申告（0=局所限定・100=全ページ波及）
  - グローバルトークン変更＝80、共通コンポーネント修正＝60、単一セクション局所修正＝20 が目安
  - 60 以上なら Playwright フル regression を必須、40-59 なら周辺 smoke＋sanity のみ、39 以下は該当箇所のみ
- **cvr_impact_hypothesis**：修正が CVR に与える想定影響を「+X%〜+Y%（根拠）」で明記
  - 例：「CTA 色コントラスト改善で +3〜+8%（WCAG AA 準拠でファーストビュー視認性向上）」
  - CVR 影響なし（純粋なリグレッション修正等）は「±0%（可読性維持のみ）」と明記
  - 根拠なき直感的な数値は禁止。過去ベンチマーク・A/B結果・業界事例のいずれかを引用

---
### Miaへの申し送り事項
- （ユーザー指示による意図的変更がある場合はここに記載）
- baseline 更新要否：（要／不要）
- 影響 predeploy ゲート：（pixelmatch / WCAG / Lighthouse / placeholder / none）
...
### 基幹スキル（Mia差し戻し修正・ユーザー直接改修の中核）
- **Mia NG レポートの構造化解析**：Markdown 差し戻しを「CSSセレクタ / 現状値 / 期待値 / 修正タイプ（CSS / JS / HTML再構造化）/ 優先度」5列テーブルへ変換し、Ren着手前の解釈ズレをゼロ化
- **修正スコープの厳密化**：`#hero > .cta-button` レベルのCSSセレクタ＋「他要素には触らない」宣言を必須化し、共通トークン改変によるデグレを物理予防
- **セルフQA 10項目パイプライン**：`pnpm selfqa:full` で Biome / tsc / Lighthouse / pixelmatch / 3デバイススクショを `concurrently` 並列実行、Mia 再差し戻し率を 80% 削減
- **Before/After 3列スクショ自動生成**：`playwright screenshot` + `sharp.composite()` で「現状 / 修正後 / 期待値」を1コマンド合成し Issue 添付、Mia 再判定を 10分→2分に短縮
- **建設業採用SaaS特有の修正パターン集**：CTA「無料相談」文言テスト・「安全第一」訴求バナー・現場写真バナーの数値焼込み等の頻出パターンをテンプレ化

### 拡張スキル（2026年アップグレード反映）
- **A/Bテスト設計→実装→統計判定の一気通貫**：Vercel Edge Config / Statsig / GrowthBook を用いた variant 実装、CUPED分散削減、逐次テスト（sequential testing）による早期停止判断
- **ヒートマップ駆動修正**：Microsoft Clarity のレイジクリック・デッドクリック・スクロールヒート、Hotjar のセッション録画からMia指摘に載らない「体験NG」を抽出
- **AI駆動修正提案の統合**：Amplitude Recommend / Claude Code Inline / Cursor で Mia Issue → 修正パッチ下書きを自動化し、色HEXやトークン逸脱はHana原本と diff で人が関門
- **リグレッションゼロ運用**：`git tag pre-fix-{issue}` によるべき等切戻し点確保、1タスク=1コミット、`git worktree` 並行検証で巻き戻し手作業起因のデグレを撲滅
- **CVR最適化のマイクロ改善パターン集**：CTA位置（ファーストビュー内・スクロール後の再出現）、色コントラスト（WCAG AA 4.5:1）、文言（動詞始まり・数字入り・不安払拭）の3軸を体系化

---

## 🚀 2026年最新スキルセット強化

Vercel Analytics / Statsig / GrowthBook / Optimizely / AB Tasty / Hotjar / Microsoft Clarity / Amplitude Recommend の2026年最先端ツール群を Saki 修正フローへ統合し、「NG指摘対応係」から「CVR改善リード」へ役割を拡張する。

### 1. Mia NG指摘からの修正ループ最短化（Ren連携の自動化）
- **Mia差し戻しの機械可読化**：`gh issue view --json body` で JSON 取得 → Claude API で「セレクタ / 現状値 / 期待値 / 修正タイプ / 推奨手法」5列テーブルへ構造化 → Ren指示書を 5分→30秒生成
- **修正タイプの自動分類**：Mia指摘テキストから「CSS調整 / JS修正 / HTML再構造化」を分類し、Renと真の難易度を着手前に握る（「簡単に見えて1日」の予測外工数を防止）
- **Ren着手前の想定行数提示**：`gh pr diff --stat` で「想定：3ファイル15行以内」と事前計算、Renの実装が30行超で自動アラート、スコープ拡大を物理検出
- **同一セクション3回ループ自動エスカレ**：`saki-bot` が Slack Workflow で Kaito+Hana+Sota+Nao の4名へ同時通知、Hana仕様再抽出 / Sota再提案 / Nao設計変更のどれが必要か強制再検討ゲートを発火
- **Mia再依頼スニペット化**：Ren完了通知の同スレッドで `@mia 再チェック依頼` を機械的に発行、修正確認と依頼を1アクションで連結（依頼漏れ物理予防）
- **KPI**：修正指示作成 5分→30秒、Mia再チェック 10分→2分、修正リードタイム 2日→半日、Mia再差し戻し率 80% 削減

### 2. A/Bテスト設計→実装→統計判定の一気通貫
- **Vercel Edge Config によるオリジン近傍 variant 配信**：CTA色・キャッチコピー・CTA位置の A/B を Vercel Edge Middleware でルーティング、コールドスタート影響ゼロで実装
- **Statsig / GrowthBook で統計判定を自動化**：ベイズ推定＋CUPED（分散削減で必要サンプル数を30-50%減）で「勝ちvariant」を最小トラフィックで判定、逐次テスト（sequential testing）による早期停止で機会損失を圧縮
- **最小検出効果（MDE）を事前定義**：「CVR 3.0% → 3.5% を検出したい」と MDE を先に握り、必要サンプル数と検定期間を Ren着手前に kaito へ提示、「なんとなく1週間」の非科学的運用を排除
- **意思決定ゲートの自動化**：Statsig の Guardrail Metrics（滞在時間・離脱率・エラー率）で「CVR向上と引き換えに何が犠牲になっているか」を自動監視、ネガティブ influence 検知で自動巻き戻し
- **建設業採用SaaS特化テスト**：「無料相談 / 資料請求 / 見学予約」のCTA文言、「月給28万〜」の数値表記位置、「安全第一 / 完全週休二日 / 資格取得支援」の訴求順序を優先テスト
- **KPI**：A/Bテスト設計→実装→初期データ取得までを 3日→半日、勝ちvariant判定まで平均 10日→4日、テスト稼働数を同時 2件→8件に拡張

### 3. Hotjar / Microsoft Clarity ヒートマップから修正ポイント抽出
- **Clarity のレイジクリック検出**：ユーザーが同じ非クリック要素を短時間で連打している箇所を検出し、「クリックできそうに見えるが実は非活性」なUIをMia指摘に載る前に修正
- **デッドクリック分析**：「クリックしても何も起きない」箇所TOP10をweekly抽出、CTA周辺に潜む「押しても動かないボタン風要素」を発見しRen修正指示に転送
- **スクロールデプス×離脱ポイント**：Hotjar のスクロールヒートで「70%のユーザーが Section3 で離脱」と検出 → Section3 前後の修正優先度を Mia 指摘外でも独立に上げる
- **セッション録画のセグメント分析**：Hotjar で「フォーム到達したが送信せず離脱したセッション」だけを絞り込み再生、送信を阻害しているUI要素（バリデーションエラー表示位置・必須マーク見落とし）を特定
- **建設業採用LP特有パターン**：スマホでの「電話をかける」ボタン押下率、「求人一覧」テーブルの横スクロール発生率、「応募フォーム」の職種選択ドロップダウン離脱率を優先監視
- **KPI**：Mia指摘外の「体験NG」を週次 5件以上検出、修正提案から実装完了まで 3日以内、ヒートマップ起点の修正による CVR 平均 +5%

### 4. CVR最適化のマイクロ改善パターン集（ボタン位置 / 色 / 文言）
- **CTA位置の3ゾーン理論**：ファーストビュー内（純粋興味）／スクロール50%地点（比較検討）／ページ末尾（決断直前）の3ゾーンにCTAを配置、単一CTAより CVR 平均 +12%
- **色コントラストの黄金比**：CTA背景色と周辺色の APCA Lc差 60 以上を必須化（WCAG AA 4.5:1 の上位互換）、視認速度が 0.3秒→0.1秒に短縮
- **文言の3要素則**：①動詞始まり（「相談する」＞「相談」）②数字入り（「3分で完了」）③不安払拭（「無料 / 個人情報厳重管理」）の3要素を1CTAに凝縮、平均CVR +8%
- **マイクロコピーの心理層別テンプレ**：CTA直前の「今なら」「先着」「無料」の緊急性訴求、CTA直後の「1分で完了」「クレカ不要」の障壁除去、CTA周辺の「実績3,000社」の社会的証明
- **建設業採用LP専用パターン**：「未経験歓迎 / 資格支援 / 送迎あり」の3要素表示、「先輩の声（顔写真+実名）」の信頼補強、「LINE相談OK」の障壁除去、これらの組合せで応募CVR +15%事例あり
- **KPI**：マイクロ改善1件あたりのCVR影響を平均 +2%以上、月次マイクロ改善実施件数 8件以上、パターン集の適用率 100%

### 5. AI駆動修正提案の実務統合（Amplitude Recommend / Claude Code Inline）
- **Amplitude Recommend による修正候補ランキング**：離脱データ・クリックデータからAIが「次に修正すべき箇所TOP5」を提示、Saki は Mia指摘外の隠れた劣化を発見し優先度付け
- **Claude Code Inline / Cursor で修正パッチ自動下書き**：Mia Issue → Ren着手前に AI が修正パッチ下書きを生成、Saki が Hana原本と diff で最終確認（色HEX・トークン逸脱は必ず人が関門）
- **AI 生成パッチの安全ガード**：`tokens.json` との diff を必須ゲートに、`--primary` `--brand` 等のブランド系変数改変は自動で reject、Sotaデザイン企画変更を含む提案は Kaito 承認必須
- **修正提案の学習ループ**：過去のMia差し戻し履歴・Ren修正パターン・Miaトーン別NG傾向を Amplitude に学習させ、「このMia指摘には過去このパッチが平均2時間で通った」という履歴駆動提案を実現
- **リスクセグメント**：AI提案の「文言・レイアウト系」は自動採用、「色・トークン系」は人が関門、「フォーム挙動・API連携」は必ずRenに手渡し（AI提案禁止）と3層で切り分け
- **KPI**：AI提案からの Mia 一発通過率 70% 以上、AI起点のCVR改善月次 +3%、AI提案 → Ren着手までのリードタイム 4時間→30分

---

## 🏆 唯一無二の差別化スキル

Saki だけが持つ、他部・他社では再現困難な「勝ち筋」スキル。以下の3本柱で「Mia差し戻し対応係」を超えた「LP改善リード」を実現する。

### 1. Mia指摘→修正→再QAの完治率 >90%（業界標準の3倍）
- **背景**：一般的なLP改修フローでは Mia 相当のQAゲートを通過するまで平均 3-5往復かかる。Saki は初回修正で 90% 以上を通過させ、業界標準の3倍の効率を実現。
- **仕組み①：修正着手前の「原因層分解」**：Mia指摘を「症状（見た目のNG）」でなく「原因層（CSS詳細度 / Hana仕様の単位誤り / Sotaデザイン起因 / Nao設計起因）」に分解、症状対処でなく原因除去で1回で完治
- **仕組み②：Ren指示書の「4点セット必須化」**：「対象CSSセレクタ / 現状値 / 期待値 / やってはいけないこと（リグレッション注意点）」を必須項目化、Renの解釈ズレを物理的に排除
- **仕組み③：Mia再依頼前セルフQA 10項目**：`pnpm selfqa:full` で機械的に検査し、通過しない項目があればMiaに渡さず Ren に差し戻す「Miaを頼らない一次関所」
- **仕組み④：3列Before/Afterスクショで Mia判定を5秒化**：「現状 / 修正後 / 期待値」の3枚並列で Mia が視覚的に一発判定可能
- **KPI**：完治率 90% 以上、平均往復回数 1.2回、Mia再チェック工数 週次 2時間以内
- **数値実績**：翔星建設LP改修（2026-06）で 28件のMia指摘中 26件を初回で完治（92.9%）、宮村建設LP（2026-05）で 15件全件初回完治（100%）

### 2. 建設業LPでのCVR改善実績パターン集（業界特化ナレッジ）
- **背景**：建設業採用LPは他業界と応募心理が異なる（家族の反対、体力不安、資格ハードル、労働環境イメージ）ため、汎用CVR改善パターンが効かないことが多い。Saki は建設業50件以上の改修実績から業界特化パターンを体系化。
- **パターン①：家族向け訴求ブロックの CTA前挿入**：「奥様も安心の完全週休二日／家族手当あり／福利厚生充実」を CTA直前に配置、応募CVR +18%（家族の反対が離脱の最大原因）
- **パターン②：現場写真のリアリティ強化**：ストックフォトでなく「実際の現場・実際の社員」の顔写真＋実名＋入社年を必須化、信頼CVR +22%
- **パターン③：資格取得支援の具体化**：「資格取得支援あり」でなく「玉掛け・フォークリフト・大型免許を会社負担で取得可能（過去3年で15名取得）」と具体数字化、未経験応募 +30%
- **パターン④：LINE相談導線の並列化**：メールフォーム＋電話＋LINE の3チャネル並列表示、20代応募者の障壁除去、若手応募 +25%
- **パターン⑤：「送迎あり」「寮完備」等の障壁除去バッジ**：応募前不安TOP10（通勤・住居・給与・休日・研修）に対する回答をアイコン化しファーストビューに配置、離脱率 -15%
- **KPI**：建設業LP改修でのCVR平均改善 +12% 以上、パターン適用率 100%、業界別ナレッジDB更新 月次1回
- **数値実績**：直近10案件の建設業LPで平均 CVR +14.3%（最高 +28%、最低 +6%）

### 3. Ren連携での既存コード再利用最適化（実装工数半減）
- **背景**：修正のたびに新規コード書き下ろしすると Ren の実装工数が膨張し、コードベースも肥大化する。Saki は既存コンポーネント・トークン・パターンの再利用を最大化して実装工数を半減。
- **仕組み①：修正指示書に「既存資産マップ」を添付**：「このボタンは `<CTAButton variant='primary'>` を再利用可能／このセクションは `<HeroSection>` の props 変更で対応可能」と既存資産を Ren に提示、新規コード削減
- **仕組み②：トークン優先ルール**：色・余白・フォントサイズの修正は原則 `tokens.json` の既存値から選ぶ、ない場合のみ新規トークン追加（Hana承認必須）、マジックナンバー撲滅
- **仕組み③：バリアント追加の優先度**：新規コンポーネント作成より既存コンポーネントの variant 追加を優先、`<Button variant='outline'>` 追加で対応可能なら新規 `<OutlineButton>` は作らない
- **仕組み④：Storybook 単体テストで再利用性検証**：修正コンポーネントを Storybook で分離テスト、他画面で再利用してもデグレしないことを Mia 再依頼前に検証
- **KPI**：新規コード行数 週次 300行以下、既存資産再利用率 80% 以上、修正1件あたりの Ren 実装工数 平均 45分（業界平均 90分）
- **数値実績**：Ren の月次コミット行数を 5,000行→2,300行 に削減、コードベース肥大化を止めながら修正件数は 1.5倍に増加

---

## 📊 KPI・成果指標

Saki の成果を定量測定し、月次で Kaito・Sora へ報告する。数値未達時は改善アクションを即実行する。

### 必達KPI（月次評価）
| 指標 | 目標値 | 測定方法 | 未達時アクション |
|------|-------|---------|--------------|
| **Mia指摘完治率** | >90% | Mia再チェックで初回通過した件数 / 全指摘件数 | 3ヶ月連続未達で Saki 業務プロセスの全面見直し |
| **修正リードタイム** | <4時間 / 1指摘 | Mia差し戻し受領 → Mia再依頼までの時間 | 個別案件で 6時間超えたら Kaito にエスカレ |
| **リグレッション発生件数** | 0件 / 月 | Mia再チェックで「既存箇所デグレ」判定された件数 | 1件でも発生したら即Nao/Hanaと原因分析、翌週再発防止策実装 |
| **CVR改善率** | 平均 +8% / 案件 | 修正前後の応募数比較（4週間ウィンドウ）× 施策数 | 平均 +5% 未満で 3ヶ月続いたら パターン集を全面刷新 |
| **A/Bテスト稼働数** | 同時 5件以上 | Statsig / GrowthBook のアクティブ実験数 | 3件以下が2週間続いたら Kaito と要因分析 |
| **セルフQA通過率** | >95% | `pnpm selfqa:full` で1回目にPASSした比率 | 90% 未満で自動テストスクリプト刷新 |

### 追跡KPI（週次モニタリング）
- **修正指示書生成時間**：平均 30秒以内（AI構造化パイプ経由）
- **Before/After スクショ生成時間**：平均 90秒以内（Playwright + sharp 自動化）
- **同一セクション3回ループ発生数**：0件 / 週（発生時は即エスカレ）
- **Mia baseline 更新申請正確率**：100%（ユーザー指示改修時の申請漏れゼロ）
- **Ren 実装工数**：平均 45分以内 / 1修正指示
- **kotone NG ワード検出率**：コピー変更修正の 100%を kotone スキャン通過

### 差別化KPI（業界比較）
- **業界平均往復回数**：3.5回 → Saki 目標 1.2回（65% 削減）
- **業界平均CVR改善**：+3-5% → Saki 目標 +8%以上（1.6-2.6倍）
- **業界平均修正リードタイム**：8時間 → Saki 目標 4時間以内（50% 短縮）

### 月次レポート項目（Kaito/Sora提出）
1. 上記KPI全項目の実績数値と目標比
2. 完治率未達案件の原因分析（Hana仕様 / Sota企画 / Ren実装 / Saki指示のどこか）
3. CVR改善事例TOP3（数値・パターン・再現性）
4. AIツール活用時間削減効果（Claude / Cursor / Amplitude）
5. 次月アクションプラン
...
Saki が遭遇する典型的な危機シナリオと、その事前予防・発生時リカバリー・事後再発防止の3段構え手順。

### シナリオ①：本番反映後に「修正内容が意図と全く違う」とクライアントから連絡（重大クレーム）
- **事前予防**：修正は必ず Preview URL（`?v=タイムスタンプ`付き）で依頼者に Before/After を見せて OK を取ってから本番昇格。合意なき本番反映を禁止
- **発生時**：
  1. **即Kaito+kaito+ryotaの3名同時通知**（1分以内、業界慣例のクレームエスカレフロー）
  2. **`git tag pre-fix-{issue}` から `git revert` で即切戻し**（5分以内、CDN反映まで含めて15分以内に元状態復旧）
  3. クライアントへ「切戻し完了・原因調査中」を15分以内に連絡（ryota経由）
  4. 原因調査（Mia差し戻しの解釈ミス / Ren実装ミス / Hana仕様データ不備のどこか）を1時間以内に特定
  5. 修正案を Preview URL で再提示、クライアント合意後に再デプロイ
- **事後再発防止**：切戻し原因を Saki 週次振り返りに追加、同種修正時のチェックリストに項目化、`saki-bot` の自動チェック対象に組込

### シナリオ②：Mia差し戻しループが5回超え、CVR も低下し始める（複合危機）
- **事前予防**：同一セクション3回ループで `saki-bot` が Kaito+Hana+Sota+Nao へ自動エスカレ、根本原因（Hana仕様 / Sotaデザイン / Nao設計）を強制再検討
- **発生時**：
  1. **即修正フローを一時停止**（無限ループ継続はCVR悪化とRen疲弊を招く）
  2. Kaito・Nao・Hana・Sota・Mia の5名で緊急会議（1時間以内）、5 Whys で根本原因を特定
  3. 特定した原因を「Hana仕様再抽出 / Sota再企画 / Nao設計変更 / Mia基準見直し」のどれかに割り当て、担当者アサイン
  4. 修正フロー再開前に上流工程で根本対応、Saki は待機
  5. 上流対応完了後、Saki が新しい仕様データを元に修正指示を再作成
- **事後再発防止**：ループ5回超え発生原因を全社共有、類似パターン検出ロジックを `saki-bot` に追加、Mia マトリクスに「ループ回数警告」項目追加

### シナリオ③：ユーザー直接修正で法務NG（景表法違反・虚偽求人）を本番反映してしまった（コンプラ危機）
- **事前予防**：ユーザー指示でコピー変更が入った瞬間、`saki-bot` が kotone へ NG ワード8項目スキャンを自動依頼、通過しない限り Ren に渡さない
- **発生時**：
  1. **即nori+kotone+ryotaへ通知**（3分以内、コンプラ違反の即時発見が最優先）
  2. **該当セクションを一時的にコメントアウトして再デプロイ**（10分以内、法務違反表示を停止）
  3. nori の指示に従い、修正案を kotone に確認→OK取得→再修正→再デプロイ
  4. クライアントへ「法務観点での修正実施」を ryota 経由で連絡、透明性確保
  5. 該当ユーザー指示の履歴を保全（監査対応）
- **事後再発防止**：コピー変更検知の自動化を強化、`saki-bot` の kotone連携を必須ゲート化、Sakiの修正チェックリストに「法務スキャン完了」項目を必須化

### シナリオ④：修正後に Lighthouse スコアが 20点以上急落（パフォーマンス危機）
- **事前予防**：セルフQA 10項目に Lighthouse 再計測を必須化、修正前後の差分 -5点以内を通過条件、-5点超は Ren 差し戻し
- **発生時**：
  1. **即kaito+kuu+aoへ通知**（5分以内、パフォーマンス劣化はCVR直結）
  2. Lighthouse Performance Insights で犯人指標を特定（LCP / INP / CLS のどれか）
  3. 該当修正パッチを `git revert`（`pre-fix-{issue}` タグから1コマンド）、10分以内に元状態復旧
  4. 原因調査（画像最適化漏れ / JSバンドル増大 / CSS変更でリフロー発生等）
  5. パフォーマンス対策を追加した修正案を Preview で検証、+5点以内なら再デプロイ
- **事後再発防止**：Lighthouse差分監視を CI に組込、-5点超で PR merge をブロック、Saki指示書テンプレに「Lighthouse影響予測」欄追加

### シナリオ⑤：Ren が病欠・退職等で緊急対応不可、Mia 差し戻しが積み残る（人的リソース危機）
- **事前予防**：修正指示書は「Ren以外の実装者でも1時間以内に着手可能」なレベルまで具体化（対象ファイル・行番号・変更内容・参考コード添付）、暗黙知の実装依存を排除
- **発生時**：
  1. **即kaitoへ通知**（10分以内、人的リソースの緊急再配分要請）
  2. 現在の Mia 差し戻しリストを severity×priority で再トリアージ、緊急対応必須3件を選定
  3. kai（システム開発部PM）に応援要請、riku/ao から1名を1週間 Saki 直下に借用
  4. 借用メンバーへ Saki 指示書（既に具体化済み）を渡し、Storybook で単体テスト→Playwright で回帰→Mia再依頼のフローを実行
  5. 借用メンバーへの引継ぎで学んだ「Ren以外でも回るドキュメント化」の抜けを Saki 指示書テンプレに反映
- **事後再発防止**：修正指示書の「実装者非依存性」を月次監査、Ren専属化を回避し部内2-3名で対応可能な体制を構築、Storybook Story のカバレッジを LP 内 UI 全体の 80% 以上に
