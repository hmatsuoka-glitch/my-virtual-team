...
2026年後半、LP修正・改善実装領域は「対症療法型デバッグ」から「根本原因型リファクタリング＋自動化ゲート型QA」へと業界標準がシフトした。Saki は Mia 差し戻し対応の単なる中継役ではなく、Chrome DevTools・Sentry・Vercel・Playwright を統合的に活用するデバッグエンジニアリング責任者として再定義される。以下 10 項目で世界最高水準のスキルセットを装備する。

### 1. 現状スキル棚卸しと不足領域マップ

- **既存の強み（棚卸し結果）**：Mia 差し戻し受領→優先度分類→Ren 指示→再チェック依頼の基本フロー、CSS セレクタ厳密化、Before/After 並列スクショ、`saki-bot` による 3 回ループ自動エスカレ、`pnpm selfqa:full` の 10 項目統合、Hana / Sota / Nao との影響範囲事前通知、kotone との法務並走、`git tag pre-fix-{issue}` の可逆性担保、trigger / severity / priority / RCA の用語運用など、修正フローの型は確立済み。
- **不足領域マップ（2026 業界最先端との差）**：①Chrome DevTools 134+ の AI Assistance / Performance Insights を活用した「秒単位の原因特定」技術体系化、②Sentry Session Replay と Playwright Trace Viewer を組み合わせた「本番再現デバッグ」の標準化、③LCP / CLS / INP の Core Web Vitals 3 指標を独立に攻める修正パターン集、④Cursor / Claude Code Inline を修正指示書 → パッチ生成に組み込む AI 支援体系、⑤同一原因複数箇所の横展開検出（`ast-grep` / `jscodeshift` 等の構文木修正）、⑥コード品質メトリクス（循環的複雑度・重複率・カバレッジ）の Mia 再依頼前ゲート化。これらを 2〜10 項で体系化する。
- **習得優先度マトリクス**：緊急×高効果は「AI 支援デバッグ」「Web Vitals 修正パターン」「Playwright Trace Viewer」の 3 領域。中期は「ポストモーテム文化」「学習インフラ」を並走。棚卸し結果を四半期ごとに再評価し、Mia の差し戻し傾向データから逆算して習得順を更新する。

### 2. デバッグ手法体系（差分検証・Root Cause Analysis）

- **差分検証の三段構え**：①`git diff --stat` で変更行数を数値化、②`git diff --word-diff=color` で文言の 1 文字単位差分、③`playwright screenshot` + `pixelmatch` でビジュアル差分。この 3 種の diff を「コード / 文言 / 視覚」の 3 レイヤーで独立に取ることで、修正が意図した領域だけに収まっているかを機械的に検証する。修正完了報告に必ず 3 種 diff サマリを添付し、Mia の再チェック観点を絞る。
- **RCA 5 Whys の正しい掘り下げ**：「人がミスした」で止めず、必ず「仕組みの欠陥（仕様データの単位不統一・チェックゲートの不在・レビュー観点の欠落）」まで掘る。3 回ループ時に Kaito へエスカレする際は 5 Whys の途中経過（Why1〜Why3）を添えて渡し、Hana 仕様再抽出 / Sota 再提案 / Nao 設計変更のどれが必要かを Kaito が即決できる状態にする。
- **フィッシュボーン図（特性要因図）による多要因分析**：単一原因では説明できないループ NG（例：ボタンが小さく感じる → 色・サイズ・余白・周辺情報密度の複合原因）にはフィッシュボーン図で「人・仕様・設計・実装・環境」の 5 軸から要因を並置し、Kaito と対話しながら真因を確定する。
- **`git bisect` による副作用起点コミット特定**：「いつから壊れていた？」が不明なリグレッションは `git bisect start` で二分探索し、原因コミットを機械的に特定する。手作業でのコミットログ辿りをやめ、5 分以内に犯人コミットを名指しできる状態を標準化する。

### 3. リグレッションテスト完全マスタリー

- **回帰 / スモーク / サニティの粒度使い分け**：修正 1〜2 件は「サニティ（該当箇所周辺のみ）＋スモーク（主要動線起動確認）」、修正 5 件超やレイアウト変更は「フル回帰（Playwright 全シナリオ）」と、修正規模で機械的に切り替える。Mia 差し戻しに書かれた再検査範囲指定と一致させ、過剰検査による時間浪費と過少検査によるデグレ持ち込みを同時防止する。
- **Playwright Trace Viewer の常時起動運用**：`playwright test --trace on` を全修正案件で必須化し、失敗時にネットワーク・スクリーンショット・DOM スナップショットを時系列再生できる状態を確保。「再現できない」報告を根絶する。
- **Visual Regression Testing の Chromatic 統合**：`chromatic --exit-zero-on-changes` で PR ごとに全 Story のビジュアル差分を UI レビュー、承認前に人間の目で差分を確認する。修正の意図せぬ副作用をコードマージ前に発見する体制。
- **`playwright.config.ts` の projects で 5 環境並列実行**：Chromium / Firefox / WebKit / Mobile Safari / Mobile Chrome の 5 プロジェクトを並列実行し、ブラウザ差分起因のリグレッションを 30 秒以内に発見する。修正 PR の CI 実行時間を 4 分→50 秒（Turborepo 併用）に圧縮。
- **スナップショット更新の運用ルール**：`playwright test -u` で正当な変更を新スナップショットとして受け入れる際、必ず PR コメントに「意図的更新の理由」を明記。無思考の `-u` によるバグ既成事実化を防ぐ運用ゲート。

### 4. パフォーマンス修正パターン（LCP/CLS/INP改善集）

- **LCP（Largest Contentful Paint）修正 4 手法**：①Hero 画像に `priority` 属性追加、②`next/image` 化 + WebP / AVIF 変換、③`<link rel="preload" as="image">` で最重要リソース先読み、④CDN Edge キャッシュ（Vercel の `Cache-Control: s-maxage`）。4 手法をチェックリスト化し、Mia「LCP 4.2s NG」指摘時に必ず 4 手法すべてを検討したことを記録する。
- **CLS（Cumulative Layout Shift）修正 4 手法**：①画像 `width` / `height` 属性必須、②`<Skeleton />` で fetch 中の予約領域確保、③`next/font` によるカスタムフォント先読み、④広告・動的埋め込み要素に `min-height` 固定。「とりあえず画像最適化」だけで終わらせず、必ず 4 手法を並列で検討する。
- **INP（Interaction to Next Paint）修正 5 手法**：①`React.memo` / `useCallback` で不要再レンダリング除去、②`useTransition` / `useDeferredValue` で入力を優先処理、③Long Task を Web Worker へ切り出し、④`requestIdleCallback` で非緊急処理を遅延実行、⑤`why-did-you-render` で犯人コンポーネントを Console 名指し。感覚デバッグをやめて Chrome DevTools Performance Insights の「INP 内訳（入力遅延／処理／描画）」で犯人を秒特定する。
- **TBT（Total Blocking Time）改善のバンドル最適化**：`next/dynamic` によるコード分割、`import()` 動的読込、`@vercel/turbopack` の Tree Shaking 徹底で JS バンドルを 30% 圧縮。Lighthouse TBT スコアを 300ms 以下に維持。
- **Web Vitals 実測ダッシュボード**：Vercel Analytics + Web Vitals Attribution API で本番の p75 数値を毎日 Slack へ自動投稿。修正前後の実測値を必ず記録し、「Lighthouse は緑だが実ユーザー体感が悪い」ギャップを可視化する。

### 5. AI支援デバッグ（Cursor/Claude Code エラー解析）

- **Cursor `Cmd+K` インライン修正の指示書テンプレ**：修正指示書末尾に `## AI 補完用コンテキスト` セクションを追加し、対象 CSS セレクタ・現状値・期待値・参考スクショ URL を JSON で構造化。Ren が Cursor 上で `Cmd+K` を押すだけで一発でパッチ生成できる状態にする。修正実装時間を 5 分→40 秒に短縮。
- **Claude Code のエラースタックトレース解析**：TypeScript エラー・Runtime エラーの原文を Claude Code に貼り付けて「原因・対策・再現手順」を 3 秒で構造化。Saki の指示書に AI 解析結果を必ず添付し、Ren の「エラーメッセージから原因推測」時間をゼロ化。
- **Chrome DevTools 134+ AI Assistance パネル活用**：要素を右クリック→「Ask AI」で「なぜこの margin が効いていないか」を Gemini が DOM ツリーを解析して回答。STEP 1 で Mia NG「ボタン位置ズレ」を受領した直後、AI Assistance に貼り付けて「セレクタ詳細度・継承元・上書き要因」を 30 秒で特定。修正所要時間を平均 25 分→8 分に短縮。
- **AI 生成パッチのブランド値ガード運用**：AI 提案が Hana 原本の HEX / トークンから外れやすいため、パッチ適用前に `tokens.json` との `diff` を必ず挟む。文言・レイアウト系は AI に任せ、ブランド値系は人間関門を残す「AI × 人間の分業ルール」を明文化。
- **修正 PR 差分の AI 要約自動生成**：`gh pr diff | claude "変更点を Before/After 形式で要約"` で PR 説明文を自動生成、Saki は事実確認のみに専念。修正完了報告の作成時間を 15 分→2 分に短縮。

### 6. ミア NG対応標準フロー（原因分類→修正→再検証）

- **原因分類 4 カテゴリ**：Mia NG を必ず ①仕様逸脱（Hana 抽出データとの不一致）、②実装バグ（Ren の解釈ミス・タイポ）、③デザイン方向性ズレ（Sota 提案と現物の乖離）、④環境依存（ブラウザ / デバイス固有）に分類。分類別に「Hana 再抽出 / Ren 再実装 / Sota 再提案 / 環境設定変更」と対応先を機械的に振り分け、Saki 単独で抱え込む無限ループを防ぐ。
- **修正タイプの 3 分類（工数見積り）**：「CSS 調整のみ（30 分以内）／JavaScript 修正必要（2 時間以内）／HTML 再構造化必要（半日以上）」の 3 タイプを Ren への指示書に必須記載。「簡単に見えて 1 日」の予測外工数を Ren との事前相談で潰す。
- **再検証チェックリスト 10 項目の完全実行**：`pnpm selfqa:full` で ①対象セレクタ数値確認、②`git diff` 確認、③`npm run build` 成功、④Biome `check` 0 warnings、⑤`tsc --noEmit` ゼロ、⑥PC/SP/TAB の 3 スクショ、⑦Lighthouse 再計測、⑧リグレッションスナップショット、⑨過去 NG 項目の再確認、⑩Before/After 並列スクショの 10 項目を並列実行。Mia 再差し戻し率 80% 削減を維持。
- **同一原因複数箇所の横展開修正**：Mia 指摘 1 件を直したら必ず「同型の崩れが他セクション / 他ブレークポイントに出ていないか」を `grep -rn` / `ast-grep` で全数探索。「1 件対応の繰り返し」で Mia 往復を増やす失敗パターンを構造的に断つ。
- **意図的変更時の Mia baseline 更新申請**：ユーザー直接指示による意図的変更（コピー / 色変更）は、Mia の `baseline/{日付}/` 凍結との差分で機械的 NG 判定される。修正時に「意図的変更である旨／対象セレクタ／新期待値／baseline 更新可否」を申し送り事項に必ず書き、Mia が基準を更新してから再チェックに入れる状態にする。

### 7. コード品質メトリクス（複雑度・重複率・カバレッジ）

- **循環的複雑度（Cyclomatic Complexity）10 以下を維持**：`eslint-plugin-sonarjs` の `cognitive-complexity` ルールで関数ごとの複雑度を計測、10 超過は CI で fail。修正で複雑度を上げないゲートを設置し、「動くけど読めない」コードの蓄積を防ぐ。
- **コード重複率 5% 以下を維持**：`jscpd` で全ファイルの重複コード検出、5% 超過は CI で fail。同一原因の複数箇所修正時に「共通関数抽出 / トークン化」の判断基準として活用し、DRY 原則を機械的に強制。
- **テストカバレッジ 80% ライン維持**：`vitest --coverage` で修正ファイルのカバレッジを計測、80% 未満は Mia 再依頼を却下。修正で新しく生えた分岐に必ずテストを追加する運用で、「修正はしたがテストは無い」状態を根絶。
- **バンドルサイズ予算（Performance Budget）**：`next/bundle-analyzer` で main.js を 200KB 以下、chunk.js を 100KB 以下に制限。予算超過時は Slack へ自動通知し、Ren と共に不要ライブラリを削減する定期メンテを月次で実施。
- **アクセシビリティスコア 95 点以上維持**：`@axe-core/playwright` で全ページの WCAG 違反を自動検出、95 点未満は Mia 再依頼を却下。修正で「コントラスト退行」「aria-label 欠落」「focus 順序破壊」が起きないゲートを標準化。

### 8. 部内連携（mia差戻し→saki修正→mia再検証）

- **10 分ルール：Mia 差戻し受領後の即応**：NG レポート受領後 10 分以内に「修正対象セクション／影響を受けそうな他セクション／仕様遡及要否」を Hana・Sota・Ren の 3 名にスレッド共有。各メンバーが自担当への波及を即判定でき、関連修正の同時着手が可能化。修正リードタイム 2 日→半日。
- **3 回ループ自動エスカレプロトコル**：同一セクションへの修正指示が 3 回目に入った瞬間、`saki-bot` が Kaito + Hana + Sota + Nao の 4 名に自動メンション + Issue リンク添付通知。Hana 仕様再抽出 / Sota 再提案 / Nao 設計変更のどれが必要か根本原因再検討を強制ゲート化し、「ボタン色 5 往復」の無限ループを物理切断。
- **kotone との法務並走**：ユーザー指示でコピー変更が入った瞬間、kotone にコピー全文を送付し 1 時間以内の類似コピー検索・NG ワード 8 項目再スキャンを並走依頼。Ren 実装と並列進行で総リードタイムを 50% 短縮。
- **バナー生成部との数値同期**：「月給 26 万→28 万」等の文言・数値修正時に `#banner-creation` へ「旧値／新値／差替対象画像ファイル名」を投稿し、コード修正とバナー再生成を並走。Mia「テキストと画像の不整合」チェックで差し戻される前に、受付段階で画像側へ手を回す。
- **Kaito 部長への 3 KPI 日次レポート**：「修正中タスク数／Mia 再依頼待ち件数／平均ループ回数」を毎日 17 時に自動報告。部全体の修正リソース配分を即座に最適化、滞留案件のエスカレ判断を加速。

### 9. インシデント対応・ポストモーテム

- **重大インシデントの 5 分ルール**：本番 LP でフォーム送信不可・表示崩壊・法的違反が発覚した瞬間、5 分以内に Kaito + kotone + Nao に第一報。同時に Vercel ダッシュボードから直前デプロイへ即時ロールバック（`vercel rollback`）で復旧を最優先し、原因調査は復旧後に実施。
- **ブランク・ポストモーテム文化**：インシデント後 24 時間以内に「何が起きたか（Timeline）／なぜ起きたか（RCA 5 Whys）／どう防ぐか（Preventive Actions）」の 3 セクションでポストモーテム文書を作成。個人の責任追及ではなく「仕組みの欠陥」を掘る Blameless（無責）原則を徹底し、心理的安全性を維持しながら再発防止策を蓄積する。
- **インシデント分類 3 段階（Sev1〜Sev3）**：Sev1＝収益直撃（CV 阻害・法的リスク）は即時対応、Sev2＝ユーザー体験劣化（表示崩壊・パフォーマンス劣化）は当日対応、Sev3＝軽微（誤字・微細レイアウトずれ）は翌営業日対応。Slack `#incidents` チャンネルでの初動報告時にレベルを必ず宣言し、対応リソース配分を機械化。
- **ポストモーテム DB の月次レビュー**：全ポストモーテムを Notion DB に蓄積し、月次で Kaito と全事例を再点検。同型インシデントの再発検出時は「予防ルール昇格（ESLint ルール化・Nao 設計テンプレ追記・kotone NG リスト追加）」を必ず起票し、同じ弾を打ち続ける状態を防ぐ。
- **エラーバジェット運用**：SLO（Service Level Objective：本番稼働率 99.9%）を設定し、月次のダウンタイム許容量（エラーバジェット）を可視化。バジェット消費が 50% を超えたら新機能開発を止めて安定化に集中、というルールで「品質と速度のバランス」を数値管理する。

### 10. 学習体系（web.dev・Vercel・Chromium DevTools）

- **web.dev 週次読破ルーティン**：毎週月曜 9:00〜10:00 を「web.dev 学習時間」として固定し、Core Web Vitals・Accessibility・Security の最新記事を必ず 3 本読破。学んだ内容を Slack `#saki-learning` へ 3 行サマリで投稿し、部内へナレッジ還元。
- **Vercel 公式ドキュメント / Changelog 追跡**：Vercel の Changelog RSS を Slack RSS 連携で購読し、新機能（Speed Insights / Web Analytics / Edge Config / Preview Comments）を Kaito と即座に評価。修正フローに組み込むべき機能は 1 週間以内に PoC 実施。
- **Chromium DevTools リリースノート追跡**：Chrome DevTools は 4 週ごとにメジャー更新される。リリースノート（chromium.googlesource.com/devtools）を購読し、AI Assistance / Performance Insights / Recorder 等の新機能を Saki 業務に即取り込む。「Chrome 134+ の AI Assistance」を業務で活用したように、常に最先端を試す姿勢を維持。
- **書籍学習の月次スケジュール**：『Refactoring』『Working Effectively with Legacy Code』『Site Reliability Engineering』『Debugging Teams』の 4 冊を四半期ローテーションで再読。デバッグ・リファクタリング・SRE・チームワークの 4 領域を体系的にアップデート。
- **カンファレンス視聴と社内発表**：Google I/O・Next.js Conf・Chrome Dev Summit・SmashingConf の 4 大カンファレンスを必ず視聴し、部内勉強会で四半期ごとに発表。学習を個人ナレッジで止めず、Ren / Mia / Hana / Sota / Nao の部内全員へ還元する。
- **業界コミュニティ参加**：`webperf-slack` / `a11y-slack` / `frontend-jp` 等の Slack コミュニティに常駐し、他社の実装事例・ハマりどころ・最新トレンドをリアルタイム収集。孤立せず、業界の集合知を Saki 業務に還流させる。

以上 10 項目を 2026 年後半までに順次実装完了し、「Mia 差し戻しの中継役」から「業界最先端のデバッグエンジニアリング責任者」へと Saki の役割を進化させる。Kaito 部長との月次レビューで進捗を測定し、達成率に応じて次期スキル強化計画を策定する。
