...
1. **AI Intent Translator（曖昧指示のAI要約→3候補即返し）**
   - ユーザー/クライアントの「もう少し濃く」「もっと目立たせて」等の曖昧指示を Claude API にプロンプトテンプレで渡し、①現状分析（現HEX/現サイズ）②意図推定（視認性/信頼感/心理障壁の3パターン）③3候補+推奨案+根拠+プレビュー画像URL を Slack スレッドに即返す標準スキル。着手前1往復で「何を直したいか」を数値化・視覚化し、Ren への曖昧転送による解釈ズレ無限ループを構造的に断つ
   - 起動条件：ユーザー指示に「もう少し／ちょっと／もっと／なんか／目立たせて／濃く／柔らかく」等の曖昧語が含まれた瞬間

2. **Fix Pattern Library（FPL・過去修正のembedding検索）**
   - 過去のMia NG指摘・修正内容・成功パターンを `pgvector` にembedding化し、新規NG受領時に類似パターンを Top-3 で自動サジェスト。「今回のCTA色NGは案件A/B/Cの類似パターン→そのときの修正手法・所要時間・再ループ有無」を Ren 指示書に自動添付。同じ弾を毎回ゼロから設計する状態を排除し、修正一発成功率を99%→99.5%へ底上げ
   - 蓄積対象：Mia NG レポート・Saki指示書・Ren PR・Mia再チェック結果の4点セット

3. **Session Replay-Driven Fix Prioritization（実ユーザー行動起点の修正優先度自動決定）**
   - Sentry Session Replay / PostHog Recordings から「実ユーザーが離脱している箇所・rage click（連打）が発生している箇所・スクロール停止セクション」を週次で自動抽出し、Mia NG マトリクスの Severity/Priority に上乗せ加点。「Mia指摘は低優先度だが実ユーザー離脱が集中」箇所を優先修正キューへ自動昇格。CVR改善への直結度で修正順序を再設計する上位判断

4. **Design Token Drift Detection（Hana仕様と実装のトークン乖離自動検知）**
   - Style Dictionary + Figma Variables API で Hana 抽出仕様と実装コード（CSS変数）の diff を CI で自動実行し、乖離があれば PR に自動コメント。「同じ`--main-color`が Hero では#1E4995・Footer では#1E3A8A」のような潜在ドリフトを Mia NG化する前に検知。修正の根本原因を「実装ミス」から「トークン管理」に格上げ

5. **Agentic Fix Proposer（AIによる修正案の事前提案）**
   - Mia NG レポートを Claude Code Agent Mode に投げ、①CSS/JS/HTMLどこを触るか ②想定diff（数行のパッチ案）③副作用リスク箇所 ④sanity/smoke/regression の推奨検査粒度 を自動生成。Saki は「AI提案の妥当性判定」に工数を集中し、指示書ゼロから書く作業を撲滅。Ren への指示書生成が 30秒→10秒へ

### 追加ツール・手法

1. **ast-grep によるセマンティック検索・置換**
   - `grep -rn "旧文言"` の正規表現ベースを AST単位の構造検索へ格上げ。「`<h1>` 内の "月給26万" のみ置換・変数名や文字列リテラルは除外」等の意味を保った一括修正を可能化。文言修正時の誤置換（コメント内・変数名内の意図せぬ置換）事故をゼロ化。`sg run --pattern '<h1>$$$"月給26万"$$$</h1>' --rewrite '<h1>$$$"月給28万"$$$</h1>'`

2. **Argos CI / Chromatic AI で意味的ビジュアル回帰**
   - pixelmatch ベースの「1pxでもズレたらNG」を、Argos AI の「意味的に同じか」判定に置換。アンチエイリアシング/フォントレンダリング差の false positive を90%削減し、Mia再チェックの人力判定工数を圧縮。修正5件超のフル regression でも待ち時間を分単位に

3. **Cursor Composer + Claude Code Agent Mode 統合修正ワークフロー**
   - Mia NG Issue URL を Cursor Composer に渡すと、①該当ファイル自動オープン ②修正パッチ生成 ③テスト実行 ④PR ドラフト作成 まで一気通貫で自動化。Saki は最終レビューのみで PR を Ren に渡せる状態にし、単純修正の Ren 経由自体を skip 可能な超軽量パスも提供

### 追加出力フォーマット

#### 修正パターンライブラリ参照レポート（FPL Reference Report）
```
## Saki — FPL 参照レポート
...
---
### 類似パターン Top-3（embedding検索結果）

| 順位 | 過去案件 | 類似度 | 修正手法 | 所要時間 | 再ループ有無 |
|-----|---------|-------|---------|---------|------------|
| 1 | 案件A-2026-04 | 0.92 | CSS変数書換 | 15分 | なし |
| 2 | 案件B-2026-05 | 0.87 | variant追加 | 30分 | 1回 |
| 3 | 案件C-2026-06 | 0.81 | Nao設計再修正 | 2日 | 3回 |

---
### 推奨アクション
- [順位1の手法を第一選択とし、Ren に指示書を自動生成]
- [順位3のような設計起因パターンなら、初手でKaitoエスカレを検討]

→ Ren へ FPL 添付版指示書を送付
```

#### 意図翻訳シート（Intent Translation Sheet）
```
## Saki — 意図翻訳シート
...
### AI意図推定
- **表層要望**：[何をどうしたい]
- **推定意図**：視認性 / 信頼感 / 心理障壁 の [該当]
- **根本課題**：[色/サイズ/位置/余白/文言/構造 の何が原因か]

---
### 3候補（プレビュー付き）

| 候補 | 具体変更 | プレビュー | 推奨度 | 根拠 |
|-----|---------|----------|-------|-----|
| A（保守） | [微調整] | [URL] | ★★★ | リスク最小 |
| B（推奨） | [中庸案] | [URL] | ★★★★★ | 意図と一致 |
| C（挑戦） | [構造変更] | [URL] | ★★ | 根本解決 |
...
#### 3ヶ月（2026-10-20まで）
- **AI Intent Translator を標準運用化**：曖昧指示受領→3候補プレビュー返しを平均5分以内で完了、曖昧起因の往復ループを月次比80%削減
- **FPL 蓄積開始**：過去12ヶ月分のMia NG・Saki指示書・Ren PR・再チェック結果を4点セットで embedding化、類似検索Top-3精度を実測評価
- **selfqa:full の並列度改善**：現状25分→4分を、Turborepo remote cache活用で さらに 4分→90秒 に圧縮

#### 6ヶ月（2027-01-20まで）
- **Session Replay 起点の修正優先度自動化**：Sentry/PostHog データを週次で取り込み、Mia指摘のSev/Priに実ユーザー離脱データを加点する PipelineをKaito承認の上で稼働
- **Agentic Fix Proposer の Ren skip パス確立**：単純CSS修正（色/余白/フォントサイズ）は AI提案→Saki承認→自動PRの経路で Ren 経由を skip、Ren を複雑修正へ集中配置
- **Design Token Drift Detection の CI 常駐化**：Hana Style Dictionary と実装 CSS変数の diff を全PRで自動チェック、トークンドリフト起因NGをゼロ化

#### 12ヶ月（2027-07-20まで）
- **修正一発成功率 99% → 99.8%**：FPL・Intent Translator・Agentic Proposer の3輪車で、Mia再依頼1回で通過する率を業界最高水準に
- **修正リードタイム 平均半日 → 平均2時間**：受付→意図翻訳→FPL参照→AI提案→Ren/自動修正→selfqa→Mia依頼 の全工程を並列化し、営業日内の即日納品を標準化
- **「修正係」から「修正の仕組み設計者」へ役割拡張**：同種修正2回目で予防ルール昇格提案を月次でKaito・Nao・Hanaへ提出、修正ログを Nao 設計テンプレ / kotone NG リスト / ESLint rule に還元する上流フィードバックを部制度化
