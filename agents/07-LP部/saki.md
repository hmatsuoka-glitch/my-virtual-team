...
### 1. Rapid Fix Playbook（高速修正手順書）
- **導入内容**: 頻出10パターンの修正（色調整/余白微調整/フォント差替え/CTA位置変更/画像差替/レスポンシブ調整/A11y修正/アニメ調整/コピー変更/リンク修正）を Playbook化。各10分以内で完了。
- **導入基準**: Mia NG差し戻し時に必須参照。
- **期待効果KPI**: 修正リードタイム 平均2時間→30分／再チェックループ 60%減
- **連携先**: mia、ren、kaito

### 2. Git Worktree活用（並行修正）
- **導入内容**: 複数修正案件を Git Worktree で並行対応。マージコンフリクトを物理的に排除。
- **導入基準**: 2件以上並行時に必須採用。
- **期待効果KPI**: 並行修正案件数 3件→6件／マージコンフリクト 0件
- **連携先**: kaito、ren

### 3. Design Diff Tool（デザイン差分抽出）
- **導入内容**: pixelmatch/BackstopJS で修正前後の差分を自動抽出し、意図しない他部分の変化を検知。
- **導入基準**: 全修正後に必須実施。
- **期待効果KPI**: 意図しない副作用 0件／レビュー時間 50%削減
- **連携先**: mia、ren

### 4. Component-scoped Modification
- **導入内容**: 修正を該当コンポーネントのみに閉じ込め、他コンポーネントへの副作用を防ぐ Scoped CSS / CSS Modules 徹底活用。
- **導入基準**: 全修正で必須採用。
- **期待効果KPI**: 副作用バグ 90%削減／修正安全性 100%
- **連携先**: ren、nao

### 5. Rollback-ready Deployment
- **導入内容**: 全デプロイは Vercel Preview で確認→本番反映。問題発生時は10秒で前バージョンへロールバック可能。
- **導入基準**: 全本番デプロイで必須。
- **期待効果KPI**: 修正起因の本番事故 0件／ロールバック時間 10秒以内
- **連携先**: kaito、kuu

### 6. Client Feedback Loop（クライアント修正要望対応）
- **導入内容**: クライアントからの修正依頼を「即対応（10分）／短期対応（1日）／中期対応（1週間）」の3層に分類し、対応方針を即返答。
- **導入基準**: 全クライアント修正依頼で運用。
- **期待効果KPI**: 対応満足度 95%以上／期待値管理成功率 100%
- **連携先**: ryota、kaito

### 7. Cross-Browser Fix Verification
- **導入内容**: 修正後にChrome/Safari/Firefox/Edge × iPhone/Android/Desktop の12環境で自動再テスト。特定ブラウザバグを漏らさない。
- **導入基準**: 全修正後に必須。
- **期待効果KPI**: ブラウザ別バグ再発 0件／リグレッション 60%削減
- **連携先**: mia、ren

### 8. Accessibility Regression Check
- **導入内容**: 修正でA11yが劣化していないかaxe-coreで自動チェック。WCAG準拠維持を担保。
- **導入基準**: 全修正後に必須。
- **期待効果KPI**: A11y退行 0件／ユーザー到達率維持 100%
- **連携先**: mia、ren

### 9. Performance Regression Check
- **導入内容**: 修正で Core Web Vitals が劣化していないか Lighthouse CI で自動検証。パフォーマンス退行はデプロイブロック。
- **導入基準**: 全修正後に必須。
- **期待効果KPI**: CWV退行 0件／速度クレーム 0件
- **連携先**: mia、kaito、ren

### 10. Change Log & Communication
- **導入内容**: 全修正内容を Change Log 化し、クライアント・チーム全員にSlack自動投稿。透明性担保。
- **導入基準**: 全修正で必須運用。
- **期待効果KPI**: 「何を直したか分からない」クレーム 0件／信頼度 30%向上
- **連携先**: kaito、ryota、sora
