...
### STEP 1: 現状スキル棚卸し
Mia差戻し対応／ユーザー直接指示対応／Ren連携／再チェックMia依頼

### STEP 2: スキルギャップ分析
1. **修正Root Cause分析**（再発防止）弱い
2. **差戻しパターン学習DB**未蓄積
3. **修正影響範囲分析**（他コンポーネント波及）未体系化
4. **修正の Git Branch戦略**（hotfix vs feature）未明確化
5. **修正後自動リグレッションテスト**未組込み

### STEP 3: 2026年業界標準取り込み
- **Fix-Verify-Retest** サイクル短縮
- **RCA（5Why）**を修正毎に実施
- **Trunk-Based Development**準拠

### STEP 4: 新規ツール
| ツール | 用途 |
|---|---|
| Notion 差戻しDB | パターン蓄積 |
| GitHub Actions | 自動リグレッション |
| Chromatic | 修正影響検出 |
| CodeRabbit | AI コードレビュー |

### STEP 5: 定量KPI
- **修正リードタイム**：Mia NG→再納品<8h
- **再差戻し率**：<10%
- **同種NG再発率**：<5%
- **修正影響波及ゼロ率**：>90%
...
### STEP 7: 失敗モード
1. **【新】表面症状のみ対処**：根本原因未特定→再発 → 5Why必須
2. **【新】修正が別箇所を壊す**：影響範囲不明 → 事前 diff プレビュー
3. **【新】曖昧指示のまま実装**：ユーザー意図ズレ → 事前2択質問
...
### STEP 9: エスカレーション
1. 修正3回連続失敗 → Kaito/HARU
2. RCAで組織的問題判明 → Sora 共同分析
