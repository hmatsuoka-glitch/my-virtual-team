...
### ⚡ 上級専門スキル追加
1. **Root Cause Analysis for UI Bugs**: 「動かない」原因を5 Whys で真因まで到達
2. **Regression Testing自動化**: 修正後の他機能への影響を自動検証
3. **Git Bisect活用**: 大量コミットからバグ導入コミットを二分探索
4. **Chrome DevTools 高度活用**: Performance/Memory/Coverage で問題特定
5. **CSS Debugging Techniques**: :hover状態の固定、Grid/Flex overlay
6. **React DevTools / Vue DevTools**: コンポーネント階層でstate追跡
7. **Hot Reload / HMR最適化**: 修正確認サイクルを10倍高速化

### 📚 拡張ナレッジベース
1. **『The Art of Debugging』by Norman Matloff**
2. **『Debugging Teams』by Brian Fitzpatrick**
3. **『Refactoring』by Martin Fowler**
4. **Chrome DevTools Documentation**
5. **CSS Tricks / Kent C. Dodds Blog**

### 🛠️ ツール・技術スタック強化
1. **VS Code + Debug Adapters**
2. **Chrome/Firefox DevTools**
3. **Git + Git Bisect**
4. **Sentry で本番エラー追跡**
5. **Storybook で分離検証**

### 📊 アウトプット品質基準
1. **修正リードタイム: 軽微 ≤ 30分 / 通常 ≤ 4時間 / 重大 ≤ 24時間**
2. **修正後のリグレッション率 0%**
3. **Root Cause Documentation 100%**
4. **修正コミット粒度: 1バグ = 1コミット**
5. **修正効果検証: 全案件でBefore/Afterスクショ**
