...
### 2026-08-08
- **効率化テクニック：Mia差し戻しの4点セット納品物（差分セレクタ＋期待値＋実測値＋修正パッチ案、mia 2026-08-08参照）を受け取ったら、パッチ案は「そのまま適用せず必ず原因層分類（token誤設定/Tailwind arbitrary誤用/CSSカスケード優先度/コンポーネント境界誤り）を先に判定してからRenへ渡す」**：症状対処のクラス追加はカスケード衝突で別箇所を壊す（ren 2026-08-08参照）。分類コードを付けてRenへ回し、tokenレベル修正なら他ページの類似崩れも同時解消する原因起点修正（2026-07-27参照）に切り替え、差し戻し2周目のリバウンドをゼロ化する。
- **品質チェックポイント：修正PRのセルフQAに `prefers-reduced-motion: reduce` と `prefers-color-scheme: dark` の両断面を Playwright `emulateMedia` で常設（2026-08-03参照）**：見た目修正がアニメ/配色の別文脈を壊すデグレを Mia 再依頼前に潰す運用を強化し、加えて `forced-colors: active`（Windowsハイコントラスト、iro 2026-07-03参照）とWCAG 2.5.8 最小ターゲットサイズ24px（mia 2026-08-08参照）も同PRで検証。3軸のアクセシビリティ退行を1PRで検出。
- **失敗パターン：修正指示に「デザインイメージのFigmaリンク」だけ添えられて対応した結果、Figma側が更新中で最新版でなかったため、実装した状態が既にsota/依頼者内では旧案になっており全やり直し**：Figmaの版管理を過信すると起きる。回避策：着手前にFigmaファイルの最終更新日時とスクショ or PDF エクスポートを依頼者から取得し、「このバージョンで着手する」を明示合意してから作業開始。Figma URLだけでは版が動く可能性を前提に、時点固定エクスポートを必須の一次入力にする。
- **トレンド察知：AI コード支援（Cursor/Copilot）による修正パッチ下書き（2026-07-27参照）に加え、Claude Code等のエージェント型AIが「Mia差し戻しJSON→修正パッチPR自動作成」まで一気通貫で回せる域に到達**：Saki の役割が「AI提案の関門」に重心移動する。文言・レイアウトの単純修正はAI起点PRをレビュー承認、ブランド値（HEX/token/font）系はHana原本diffを人が関門、と切り分けを明文化。tokens.json diff通過とMia再QA通過の2ゲートをAI起点PRの必須通過条件に固定する。
- **連携ヒント：ユーザー直接指示対応（パターン2）のSTEP 1で、「修正の目的（CVR改善/クレーム対応/クライアント意向/ブランド刷新）」を必ず1文でヒアリングして修正チケットに記載**：目的が不明のまま「文字大きく」等を実装すると、真因が別（実は方向性違和感、2026-08-05参照）で数値修正が空振りする。目的分類は「①CVR系→A/B前提でsotaのデザイン提案と並列／②クレーム→即時対応・hotfix類型判定／③クライアント意向→依頼者確認プロセス／④ブランド刷新→sota/iroへ再企画依頼」の4分岐で、Rensakiどちらが着手するかも自動決定できる。
