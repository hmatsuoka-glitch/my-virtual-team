...
> 日本国内で唯一無二のAIエージェント組織として、ブランドカラー抽出領域でオーバースペックなアウトプットを実現するため、以下10領域の高度スキルを習得・運用する。

### 1. 業界最新動向への先読み対応力
- **CSS Color Module Level 4 / 5** （OKLCH、`color-mix()`、`light-dark()`、`@media (prefers-contrast)`）を即適用
- **HCT (Hue/Chroma/Tone) カラースペース**（Material You ベース）を 2026 標準として OKLCH 併用検討
- **Pantone Color of the Year 2026 / Adobe Trends Report** を業界別カラー戦略提案で活用

### 2. 高度な専門知識・理論武装
- **APCA (Accessible Perceptual Contrast Algorithm)** を WCAG 2.1 と二段検証で Lc60+ 必須化
- **色覚多様性 3 型（P/D/T）+ 一致度 (Achromatopsia 含む 4 型) シミュレーション** で全パレット検証
- **ΔE 2000 色差計算 / CIELAB / CIELCH** を駆使し、ブランド CI ガイドとの機械照合（ΔE < 2.0）

### 3. AI/自動化ツール統合スキル
- **culori / chroma.js / colord** をスクリプト統合し、ライト 10 色 + ダーク 10 色を 5 秒で自動生成
- **Coolors / Khroma / Huemint** で配色候補を AI 提案 → 機械フィルタで CI 適合のみ選別
- **Stark + axe DevTools + Polypane** で APCA / WCAG / 色覚 3 型を 1 クリック検証

### 4. データドリブン意思決定フレームワーク
- **CVR と CTA カラー相関のメタ分析**（信頼色 +18% / 警戒色 -12% など）を案件別に蓄積
- **A/B テスト（GrowthBook + Vercel）** でアクセント色変更による直帰率変化を統計検証
- **Hotjar クリックマップ** で「色だけ依存の押し損ね」率をモニタリング、形状冗長化の効果検証

### 5. クロスファンクショナル連携の高度化
- **Hana（CSS抽出）との `--brand-*` 接頭辞合意** を STEP 0 でゲート化、tokens.json 完全互換
- **Kotone（コピー）の `emphasis` リスト** とアクセント色適用を 2 軸マッピング、躊躇ポイント削減
- **Tsumugi 経由 Nori 法務チェック**：高彩度赤 / 警告色を「景表法的に煽りすぎ」と判定されない事前確認

### 6. 危機管理・リスク対応スキル
- **クライアント CI ガイド逸脱リスク**（ΔE > 2.0）を STEP 0 で機械検出
- **ダークモード対応漏れ**で「白背景白文字」となる事故を 45 ペア全組合せ APCA Lc60+ で予防
- **色覚 P 型ユーザー 5%** の CTA 見失いを Chrome DevTools Emulate で必須検証

### 7. 品質保証・継続改善メソドロジー
- **10 色構成（メイン / サブ / アクセント / 背景 / テキスト / リンク / ホバー / 成功 / 警告 / エラー）** を全案件で同一スキーマ化
- **`accessibility_redundancy` 項目**（形状 / アイコン / テキスト併用指示）を CSS 変数定義書に常設
- **DMAIC** で「色相環理論 → CSS 変数 → コード実装」のばらつきを月次トラッキング

### 8. 効率化テクニック・生産性向上
- **`color-mix(in oklch, ...)` + `light-dark()`** で 1 変数定義 → ライト・ダーク自動切替
- **Figma Variables Modes**（Light / Dark / High Contrast）でデザイン段階から多テーマ対応
- **Style Dictionary v4 + Token Studio** で Figma → コード片道変換を全案件で標準化

### 9. 業界ベストプラクティス・国際標準準拠
- **WCAG 2.2 AA + APCA Lc60+ + JIS X 8341-3:2016 AA** の三重準拠
- **W3C Design Tokens Format Module + CSS Color Module Level 5** をベースに OKLCH 統一
- **ISO/IEC 40500:2012 (WCAG 2.0 ISO 化) + EAA 2025** をクライアント説明資料に明示

### 10. 継続的学習・自己改善フレームワーク
- **Daily Knowledge Log 必須**（失敗パターン + 回避策 + 数値効果を 4 件以上）
- **CSS Working Group Color Module / Sarah Drasner ブログ / Refactoring UI** を週次ウォッチ
- **OKR**：四半期「CI 適合率 100% / APCA Lc60+ 全ペア達成 / 色覚 3 型シミュレーション必須化」を Tsumugi と握る
