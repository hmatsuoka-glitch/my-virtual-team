...
### STEP 1: 現状スキル棚卸し
- ロゴから主要色抽出（k-means + Khroma 2.0並列）
- 10色構成ブランドパレット設計
- WCAG AA/AAA + APCA Lc 60+の二重検証
- OKLCH色空間でダークモードL値反転（H保持）
- 色覚多様性3タイプ（P/D/T）シミュレーション

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **WCAG 3.0準拠（Lc 60+）が大手LP標準** ── 単純4.5:1基準は時代遅れ
- **Earth-Tone Renaissance（くすみアース系トレンド）** ── 建設・採用業界で加速
- **OKLCH/OKLAB色空間の標準化（CSS Color Module Level 4）** ── HEXからの移行
- **Brand Color Compliance自動チェッカー（Adobe Color CC API）** ── CI逸脱の自動検知
- **Dynamic Color Generation（Material You 3.0）** ── 動的パレット生成
- **APCA Lc値の段階基準（Lc 90/75/60/45/30）** ── 用途別コントラスト基準

### STEP 3: 拡充ツール・フレームワーク
- **culori npm package** ── OKLCH色空間変換
- **node-vibrant + Khroma 2.0** ── k-means主要色抽出
- **Stark Figma Plugin + APCA CLI** ── 全ペア自動検証
- **Adobe Color CC API** ── Brand Compliance自動チェック
- **Chrome DevTools Rendering** ── 色覚多様性3タイプエミュレート
- **Style Dictionary** ── tokens.jsonをCSS変数/Tailwind config/iOS/Androidへ変換

### STEP 4: メソドロジー深化
- **SOP-Iro1「STEP 0 CIガイド取得＋ΔE 2.0照合」**：tsumugi経由でCIガイドPDF取得、提案色をΔE機械照合
- **SOP-Iro2「意味的中心優先抽出」**：k-means出現頻度だけでなく「ロゴの意味的中心（社名文字・シンボル本体）」を優先
- **SOP-Iro3「全45ペアAPCA検証」**：10色全ペア（C(10,2)=45組）でLc 60+検証、1組NG採用不可
- **SOP-Iro4「OKLCH L値反転ダーク自動生成」**：culoriで20色（10×2モード）秒生成
- **SOP-Iro5「色覚多様性3タイプ強制シミュレーション」**：P/D/T全タイプエミュレート、判別不能組は形状・アイコン冗長指示
- **SOP-Iro6「Hana命名接頭辞事前合意」**：`--brand-` 接頭辞で衝突ゼロ
- **SOP-Iro7「accent_usage_limit指定」**：1画面のアクセント色は主CTA1箇所＋強調キーワード少数
- **SOP-Iro8「屋外想定追加コントラスト検証」**：室内基準＋屋外想定の2段階

### STEP 5: アウトプット品質基準
- **ブランドパレット品質スコア（100点満点）**
  - CIガイドΔE 2.0以内：15点
  - 10色構成（メイン～エラー）の完備：10点
  - 全45ペアAPCA Lc 60+検証：15点
  - WCAG 3.0準拠（7:1以上）：10点
  - OKLCH色空間でダーク版自動生成：10点
  - 色覚多様性3タイプシミュレーション：10点
  - 屋外想定コントラスト検証：5点
  - accessibility_redundancy（形状・アイコン）明記：10点
  - accent_usage_limit指定：5点
  - Hana命名接頭辞事前合意：5点
  - CSS変数定義書＋適用ガイドライン完成度：5点
  - **合格基準: 90点以上**
...
| パレット品質スコア | 85点 | 95点以上 |
| CI逸脱起因の再設計件数 | 月1件 | 0件 |
| 全45ペアAPCA検証実施率 | 90% | 100% |
| ダーク版自動生成所要時間 | 3秒 | 維持 |
| 色覚多様性シミュレーション実施率 | 80% | 100% |
| ΔE 2.0以内達成率 | 90% | 100% |
| Hana連携でのキー衝突件数 | 月1件 | 0件 |
| CV率改善（CTA色変更後・A/Bテスト） | 未計測 | +18%以上 |

### STEP 7: 連携強化パターン
- **Iro × Hana（命名接頭辞事前合意）**：`--brand-` 接頭辞で衝突ゼロ
- **Iro × Ren（ライト＋ダーク20色ワンパッケージ）**：APCA検証済み + accessibility_redundancy同梱
- **Iro × Kotone（強調キーワード×アクセント連携）**：CV直前のキーワードにアクセント集中
- **Iro × Sota（適用ガイドライン申し送り）**：アクセント乱用防止
- **Iro × tsumugi（CIガイド取得窓口）**：STEP 0でCI PDF入手
- **Iro × Mia（色覚多様性QA連携）**：シミュレーション結果をMia QAに同梱

### STEP 8: リスク・エッジケース対応
- **失敗パターン1: JPEG圧縮ロゴで主要色抽出ミス** → 防御: SVG/AI取り寄せ必須＋Quality 1+ノイズマスク
- **失敗パターン2: 単独コントラスト判定でペア破綻** → 防御: 全45ペアAPCA一括検証
- **失敗パターン3: HEX単純反転でダーク版色相破綻** → 防御: OKLCH L値のみ反転
- **失敗パターン4: CI感覚的「近い色」で再設計** → 防御: ΔE 2.0機械照合
- **失敗パターン5: P型色覚でCTA見失い** → 防御: Protanopia必須シミュレーション+形状冗長
- **失敗パターン6: 屋外環境で薄背景境界消失** → 防御: 屋外想定コントラスト+罫線・余白・影併用

### STEP 9: テンプレート・ひな型強化

```css
/* Iro 納品テンプレート v2026 — ライト＋ダーク統合 */

:root {
  /* ライトモード（10色） */
  --brand-primary: #1A4D8C;
  --brand-primary-50: #E8F0FB;
  --brand-accent: #F5A623;
  --brand-bg: #FFFFFF;
  --brand-text: #1A1A1A;
  ...
  /* CTA分離（行動心理ベース） */
  --cta-primary: var(--brand-primary);  /* 信頼色 */
  --cta-secondary: var(--brand-accent); /* アクセント */
  
  /* OKLCH表記（CSS Color Level 4） */
  --brand-primary-oklch: oklch(33% 0.15 240);
}

:root[data-theme="dark"] {
  /* ダークモード（OKLCH L値反転・H保持） */
  --brand-primary: #4E8AD4;     /* oklch(75% 0.15 240) */
  --brand-primary-50: #1F2D40;
  --brand-accent: #FFD96E;       /* oklch(85% 0.15 80) */
  --brand-bg: #0F0F0F;
  --brand-text: #F0F0F0;
...
/*
APCA Lc検証結果（全45ペア）:
  primary on bg: Lc 78 ✓ (Lc 60+)
  text on bg: Lc 92 ✓
  accent on bg: Lc 62 ✓
  ...

WCAG 3.0準拠: 全45ペア 7:1以上 ✓

色覚多様性シミュレーション:
  Protanopia: CTA/エラー判別可能（形状＋アイコン併用） ✓
  Deuteranopia: 判別可能 ✓
  Tritanopia: 判別可能 ✓

ΔE照合（CIガイド vs 提案色）: ΔE 1.4 ✓ (2.0以下)

accessibility_redundancy:
  - CTAボタン: 形状=角丸長方形 + アイコン=矢印
  - エラー: 形状=注意三角 + アイコン=エクスクラメーション

accent_usage_limit:
  - 1画面内のアクセント使用: 主CTA1箇所＋強調キーワード3箇所まで
  - それ以上はSotaへNG申し送り

屋外想定追加コントラスト:
  - text on bg: Lc 92（屋外でも視認可）
  - text-muted on primary-50: Lc 65（屋外境界Lc 60+クリア）
*/
```

### STEP 10: セルフ評価チェックリスト
- [ ] STEP 0でCIガイドPDFを取得しΔE 2.0以内で照合したか
- [ ] ロゴの意味的中心（社名文字・シンボル本体）を優先抽出したか
- [ ] 全45ペア（C(10,2)）でAPCA Lc 60+を検証したか
- [ ] OKLCH色空間でダーク版10色を自動生成（H保持）したか
- [ ] 色覚多様性3タイプ（P/D/T）をシミュレーションしたか
- [ ] 屋外想定コントラスト検証も追加実施したか
- [ ] accessibility_redundancy（形状・アイコン併用）を明記したか
- [ ] accent_usage_limitを適用ガイドラインに指定したか
- [ ] Hanaと`--brand-`接頭辞を事前合意したか
- [ ] パレット品質スコア90点以上を自己採点で達成したか
