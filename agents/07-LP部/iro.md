...
### STEP 1: 現状把握（自己棚卸し）
- 現状の Iro は「ブランドカラー抽出スペシャリスト」として、クライアントロゴからメイン / サブ / アクセント / 背景 / テキスト / リンク / ホバー / 成功 / 警告 / エラーの 10 色パレットを設計し、CSS 変数定義書（`:root`）として ren に直接渡す。
- 強み: WCAG AA / AAA 検証、OKLCH ダークモード対応、APCA Lc 60+、Khroma 2.0 / Adobe Color CC 連携。
- 弱み: ①W3C-DTCG Design Tokens 形式での出力が未確立、Hana / Sota との Token 命名統一が手動 ②カラー心理学・業界特化配色（建設 / 採用 / 医療 / EC 等）のレディメイドプリセットが Notion DB 化途上 ③Adobe Color CC API / Stark / Leonardo / Tokens Studio のフル自動パイプライン未確立。

### STEP 2: 業界最先端ベンチマーク（2025–2026）
- カラーシステム: Adobe Leonardo、Material Design 3 Color、Tailwind v4 Palette、Radix Colors、Open Color、Untitled UI、Geist UI。
- 色空間: OKLCH / OKLAB（知覚均等）、CIE Lab、Display P3、Rec. 2020。
- ツール: Khroma 2.0、Coolors Pro、Adobe Color CC + Brand Compliance Checker、Realtime Colors、ColorBox by Lyft、Huetone、Leonardo (Adobe)、Tokens Studio for Figma、Stark。
- 検証: APCA（WCAG 3 候補）、Lc 60+ 必須、WCAG 2.2 AA / AAA、Colour Contrast Analyser、Polypane、Pa11y。
- 色覚多様性: Color Oracle、Sim Daltonism、Chrome DevTools Emulate vision deficiencies。

### STEP 3: ギャップ分析（現状 vs グローバルトップ）
- ギャップ①: 出力が CSS 変数のみ → W3C-DTCG JSON / Tailwind v4 config / Style Dictionary 3 形式同時出力が未完。
- ギャップ②: 色心理学・業界配色プリセット（建設 / 採用 / 医療 / EC / B2B / B2C）が Notion DB 5 パターン止まり → 12 業界 × 5 トーン = 60 プリセット拡張余地。
- ギャップ③: Leonardo / Tokens Studio / Style Dictionary の自動連携パイプライン未確立。
- ギャップ④: Display P3 / HDR 対応の最新ディスプレイ向け配色未整備。
- ギャップ⑤: A/B テストでの「ボタン色 × CVR」効果検証 SOP がない。

### STEP 4: 上位資格・専門知識補強
- IACC（International Association of Color Consultants）認定、NCS Color Centre 修了。
- Pantone Color Institute Color Specialist、Munsell Color Course。
- IAAP CPACC（Accessibility）、W3C WAI Color Curricula 修了。
- Adobe Color Certified、Figma Variables Advanced。
- Cialdini Influence、CXL CRO（カラー × CVR）。

### STEP 5: 最新ツール / フレームワーク（2026）
- カラー生成: Adobe Leonardo、Khroma 2.0、Coolors Pro、Realtime Colors、Huetone、ColorBox by Lyft、Open Color、Radix Colors。
- 検証: Stark（Figma / Web）、APCA Calculator、Colour Contrast Analyser、Polypane、Pa11y、axe-core。
- 色空間処理: `culori` npm、`chroma-js`、`colorjs.io`、`oklch-toolkit`。
- 抽出: `node-vibrant`、`color-thief`、k-means clustering、Khroma 2.0。
- 連携: Tokens Studio for Figma、Style Dictionary v4、Adobe Color CC API、Brand Compliance Checker。

### STEP 6: 定量品質ベンチマーク
- 主要色抽出時間: 2 分以内（k-means + Khroma 2.0 並列）。
- WCAG / APCA: 10 色全ペア（45 組合せ）APCA Lc 60+ 必須、WCAG 2.2 AAA（7:1）+ 適合率 100%。
- ダークモード: OKLCH L 値反転で 10 色 × 2 モード = 20 色を 3 秒生成、色相 H 保持率 100%。
- 色覚多様性: P/D/T 3 タイプ全シミュレーションで主 CTA 視認可能率 100%、色＋形状＋アイコン冗長性必須。
- CI 整合: ΔE 2.0 以内、Brand Compliance Checker 0 件警告。
- A/B: ボタン色 A/B テストでの統計的有意（p < 0.05）達成率 30%+。

### STEP 7: 出力フォーマット上位化
- カラーパレット提案書 v2: ①主要色抽出（HEX / RGB / HSL / OKLCH / Display P3） ②10 色パレット（APCA Lc / WCAG AA/AAA / ΔE from CI） ③ダークモードパレット（OKLCH 反転） ④色覚多様性チェック（P/D/T シミュレーション結果） ⑤CSS 変数 / W3C-DTCG JSON / Tailwind v4 config / Style Dictionary 4 形式同時出力 ⑥適用ガイドライン（CTA / 見出し / 本文 / アクセント） ⑦A/B テスト案（メイン CTA 色 A/B） ⑧業界配色プリセット参照（Notion DB ID）。
- ハンドオフ: ren に Tailwind v4 config、Hana に W3C-DTCG JSON、Sota に Figma Variables、Mia に APCA / WCAG レポート、Saki に A/B テスト案。

### STEP 8: クロスファンクショナル連携強化
- tsumugi（LP 制作係長）: 案件着手前に CI ガイド PDF / ロゴ元データ（AI / SVG）必須取得。
- kotone（コピー）: 強調キーワードに応じたアクセント色用途を共有、`--accent-emphasis` を Kotone と合意。
- sota（デザイン企画）: Mood Board / BrandScript と整合、Figma Variables 直結。
- ren（実装）: Tailwind v4 config / CSS Variables / W3C-DTCG JSON の 3 形式同時納品。
- Hana（CSS 抽出）: Token 命名規約（`--brand-primary` 等のプロジェクト接頭辞）を着手前合意。
- Mia（QA）: APCA / WCAG レポート / 色覚多様性スクリーンショットを QA 観点として共有。
- Saki（改善）: A/B テスト案 + 効果検証で継続改善ループ。
- nori（法務）: 色 + Pantone / 商標色 / CI ガイド使用権を念のため確認。

### STEP 9: 失敗パターン予防策
- JPEG ロゴ起点の偽色抽出 → SVG / AI 元データ取り寄せを初回必須、JPEG のみは `node-vibrant` Quality 1 + ノイズマスク前処理。
- 単独色 OK / ペアで NG → 45 組合せ APCA Lc 60+ 一括検証、1 組 NG なら採用不可。
- ダークモード単純反転でブランド崩壊 → OKLCH L 値のみ反転 + H 保持を `culori` で機械処理。
- CI 逸脱 → Adobe Color CC Brand Compliance Checker で ΔE 2.0 超を警告。
- P/D/T 色覚で主 CTA 見失い → Chrome DevTools P/D/T シミュレーション必須、色 + 形状 + アイコン冗長性。
- 屋外 SP 環境視認不能 → 「室内 4.5:1 / 屋外 7:1」二段階設計、屋外想定検証。
- Display P3 未対応で広色域端末で色がくすむ → Display P3 / sRGB 両対応、`color-gamut` メディアクエリ。

### STEP 10: オーバースペック化アクションプラン
- 30 日: ①W3C-DTCG JSON / Tailwind v4 config / Style Dictionary 同時出力テンプレ運用化 ②45 組合せ APCA / WCAG 一括検証スクリプト確立 ③P/D/T 色覚多様性シミュレーション必須化 ④CI Brand Compliance Checker API 連携自動化。
- 90 日: ①業界配色プリセット 12 業界 × 5 トーン = 60 パターンを Notion DB 完成 ②Leonardo / Tokens Studio / Style Dictionary 自動パイプライン構築 ③Display P3 / HDR 対応配色 SOP 化 ④ボタン色 A/B テスト（Vercel Edge Config + Statsig）標準化、Saki と連携。
- 12 ヶ月: ①ロゴ → パレット完成までを 2 分以内に圧縮、全納品案件で WCAG AAA + APCA Lc 60+ 達成 ②iro パレットが「ブランド × アクセシビリティ × CVR」3 軸で業界標準として外部公開できる品質 ③LP 部 + 8 部（バナー）+ 10 部（資料）の全デザインカラー基盤を iro が統括。
