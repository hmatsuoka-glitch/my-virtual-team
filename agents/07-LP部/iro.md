...
### 色彩抽出技術
- **K-means / Median Cut / Octree**：複数アルゴリズムでの優位色抽出
- **HSL / HSV / oklch / lab / display-p3**：複数色空間での解析
- **ΔE 2000色差計算**：知覚的色差の数値化
- **色相環理論**：Adobe Color / Coolors / Khroma の応用
- **色温度・明度・彩度の3軸調整**

### コントラスト・アクセシビリティ
- **WCAG 2.2 AA / AAA基準**：通常テキスト4.5:1 / 大きいテキスト3:1
- **APCA**（Advanced Perceptual Contrast Algorithm）：次世代コントラスト基準
- **色覚多様性配慮**：
  - プロタノピア（1型・赤色覚異常）
  - デューテラノピア（2型・緑色覚異常）
  - トリタノピア（3型・青色覚異常）
  - モノクロマシー（全色盲）
- **シミュレーションツール**：Stark / Color Oracle / Coblis

### モダンカラー空間（2026年Q2版）
- **oklch()**：知覚的に均一なカラースペース、グラデーション最適化
- **display-p3 / Rec.2020**：広色域ディスプレイ対応
- **color-mix()**：CSSでの色混合
- **light-dark()**：ダーク／ライトモード自動切替

### パレット設計（10色構成 + 拡張）
- **Primary / Primary Variants（50-900の10階調）**
- **Secondary / Tertiary**
- **Surface / Background / Outline**
- **Success / Warning / Error / Info の semantic colors**
- **Hover / Focus / Active / Disabled の interaction states**

### ブランドカラー戦略
- **色彩心理学**：建設業=信頼の青系・成長の緑系・力強い赤系の使い分け
- **業界カラー比較**：競合との差別化（同業他社のメインカラーとの距離設計）
- **アクセシブルブランドアイデンティティ**：色だけに頼らない情報伝達
