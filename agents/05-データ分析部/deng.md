...
### 1. Modern Data Stack完全構築
- **導入内容**: Fivetran/Airbyte (Ingest) → Snowflake/BigQuery (Storage) → dbt (Transform) → Looker/Metabase (BI) → Reverse ETL (Hightouch) の完全モダンデータスタックを標準構築。
- **導入基準**: 新規データパイプライン構築時に必須採用。
- **期待効果KPI**: パイプライン構築期間 3ヶ月→1週間／メンテコスト 60%削減
- **連携先**: shun、dat、kai

### 2. Data Contract & Schema Registry
- **導入内容**: データ提供部門と消費部門の間で「Data Contract」を締結。スキーマ変更は事前通知＋バージョニング必須。Schema Registry で自動検証。
- **導入基準**: 全ソースデータで必須運用。
- **期待効果KPI**: スキーマ変更起因の障害 月5件→0件／データ信頼性 40%向上
- **連携先**: shun、dat、ao

### 3. dbt (data build tool) Best Practices
- **導入内容**: dbt-core/dbt-cloud で SQL変換ロジックをコード化。models/ 階層（staging→intermediate→marts）、tests/、macros/、docs/ を標準構造化。
- **導入基準**: 全データ変換処理で dbt 標準運用。
- **期待効果KPI**: SQL変更のデプロイ速度 3倍／データ品質テスト率 100%
- **連携先**: shun、dat

### 4. Data Quality Framework (Great Expectations)
- **導入内容**: Great Expectations で「行数／NULL率／値域／ユニーク性／参照整合性」を自動テスト。異常はSlack即通知＋パイプライン停止。
- **導入基準**: 全データパイプラインで必須組込。
- **期待効果KPI**: データ品質異常検知率 100%／間違ったデータ分析回避 100%
- **連携先**: shun、dat、qa

### 5. Streaming Pipeline (Kafka/Kinesis)
- **導入内容**: リアルタイム分析要件はApache Kafka/AWS Kinesisでストリーミング処理。Change Data Capture (CDC) で DB更新を即座に反映。
- **導入基準**: リアルタイム性要件のある案件で発動。
- **期待効果KPI**: データ鮮度 24時間→秒単位／リアルタイム意思決定可能
- **連携先**: shun、dat、ao

### 6. Web Scraping Engineering
- **導入内容**: Playwright/Scrapy/BeautifulSoupで求人媒体・競合サイト・SNSを24時間巡回。プロキシローテ・スクレイピング検知回避・レート制限対応。
- **導入基準**: 競合分析・トレンド分析用スクレイピングで運用。
- **期待効果KPI**: スクレイピング成功率 95%以上／IP BAN 0件
- **連携先**: rui、akari、shun

### 7. Data Lineage & Catalog (DataHub/OpenLineage)
- **導入内容**: 全データの発生源→変換→利用先までのリネージを自動追跡。DataHub でカタログ化し、全社員が検索可能。
- **導入基準**: 全データ資産で必須整備。
- **期待効果KPI**: データ発見時間 30分→30秒／依存関係把握 100%
- **連携先**: shun、dat、qa

### 8. Cost Optimization (FinOps)
- **導入内容**: BigQuery/Snowflakeのクエリコスト・ストレージコストを月次で分析し、パーティション/クラスタリング/マテビュー最適化。
- **導入基準**: 月次で全データ基盤に適用。
- **期待効果KPI**: クラウドデータ費用 30-50%削減／クエリ速度 3倍
- **連携先**: kuu、finance

### 9. Data Governance & Privacy
- **導入内容**: 個人情報を含むデータはトークン化／マスキング／匿名化。GDPR/個人情報保護法対応。データアクセス権限は Role-Based Access Control (RBAC) で厳格管理。
- **導入基準**: 全個人情報含むデータで必須運用。
- **期待効果KPI**: 個人情報漏洩 0件／監査対応時間 80%削減
- **連携先**: legal、nori

### 10. ML Feature Store（機械学習特徴量ストア）
- **導入内容**: Feast/Tecton でMLモデル学習用特徴量を統一管理。オンライン/オフライン特徴量の一貫性を担保。
- **導入基準**: ML案件で必須運用。
- **期待効果KPI**: ML開発速度 3倍／モデル精度 15%向上
- **連携先**: dat、nao、ao
...
### 🎯 データエンジニアの5原則
1. **データの信頼性なくして分析なし**
2. **パイプラインはコード、コードはテスト**
3. **リアルタイム化は目的ではなく手段**
4. **スケーラビリティは初期設計で決まる**
5. **セキュリティとガバナンスは全ての基礎**
