...
### 2026-09-05
- **Data Contract導入の段階計画：まず「PII含む上流3ソース（Airwork応募・GA4・LPフォーム）」から契約YAML化する**：全ソース一斉導入は現場のレビュー負荷で頓挫する。PII露出リスクが最も高い3ソースに限定し、`contracts/*.yml`でschema/PII分類/保持期限/owner/SLAを宣言、CI（GitHub Actions）で契約違反commitを即reject。既存のスキーマハッシュ監視（2026-06-03参照）は事後検知として並行運用しつつ、契約テスト（2026-07-03参照）を「入口の関所」へ格上げし、noriのリーガル関所と機械的に接続する。段階導入で4週目までに3ソース完了→四半期で全ソース展開のロードマップをHarutoへ提示。
- **dbt Fusion Engine PoC：既存dbt Coreプロジェクトをブランチ切って並列実行し、コンパイル時間・CI回転を実測してから移行判断**：Fusion移行はSQL方言（Jinja→Rust parser）の細かな非互換が発生しうるため、いきなり本番切替せず`fusion/`ブランチで並列運用しdbt-audit-helperで新旧出力差分0を確認。CI回転が現行の1/3以下になれば移行、5%以上のmodel互換問題が出れば延期の判断基準を先に決める。Semantic Layerの`metrics.yml`化はFusion移行と切り離し、`kpi_def_version`との1対1マッピングだけ先行で着手し、Shunの月初KPI突合MTG（2026-06-04参照）で運用開始。
- **Elementary Cloud導入で祝日誤発火・季節性ノイズを構造排除する**：手動閾値の変化率アラート±30%（2026-06-03参照）は、GW・お盆・年末の稼働日減で誤発火し狼少年化（2026-05-24参照）の温床。Elementaryのanomaly detectionは過去90日の季節性・曜日性・祝日パターンを自動学習し動的閾値を出すため、`elementary_test_configuration`で主要KPI（応募数・CVR・媒体別件数）に適用し、既存のINFO/WARNING/CRITICAL 3階層ルーティング（2026-05-26参照）へ流し込む。ゲート発火実績棚卸し（2026-07-03参照）もElementaryのメタデータテーブルから自動集計化。
- **Datastream CDC導入でRui向け「掲載終了検知」を翌朝→当日中に短縮する**：バッチ差分の削除検出（2026-06-13参照）は前日スナップショット比較のため、掲載終了シグナルは最速でも翌朝発火。Datastream＋PostgreSQL WALで秒単位のCDCストリームをBigQuery/Icebergへ書き込み、`delisted_at`を発生から数分で確定させる。Ruiの競合10社Job Posting Analytics（2026-06-11参照）で「昼に消えた求人」を当日夕方の調査チャンネルへ変化率アラートと同時通知、採用充足シグナルの鮮度をさらに1営業日短縮。
- **Reverse ETL（Hightouch）で「離脱リスク応募者リスト」をRyotaのSlackへ日次配信する接続を試作**：dbt martsで確定した「応募後3日連絡なし・面接キャンセル1回以上」の離脱リスクセグメントを、HightouchでRyotaの案件管理チャンネルへSyncするパイプラインを1クライアント（翔星建設）で試作。分析→業務アクション（追加媒体提案・面談リマインド）の間の人手コピペをゼロにし、成果が出れば7社横展開。PIIハッシュ（2026-06-12参照）は保ちつつRyota画面ではハッシュ復号せず「応募ID＋離脱リスクスコア」のみ露出する設計で守秘義務を担保。
- **Terraform for BigQuery移行で権限棚卸しをGit差分レビュー化する**：四半期権限棚卸し（2026-09-02参照）を手動エクスポートで運用すると、棚卸し漏れ・タイミングずれで委託終了アカウントが残る。データセット/テーブル/RLS/Row Access Policy/スケジュールクエリをTerraform化し、権限変更は必ずPR経由でHaruto/noriレビュー、Git履歴が権限棚卸しそのものになる状態へ移行。手動GUI操作は原則禁止、緊急対応で操作した場合は48時間以内にTerraformへ逆流させる運用ルールを明文化。
- **改正個情法（2026施行）対応の運用設計をnoriへ提示する**：PII保持期限30日の自動削除（partition expiration、2026-08-05参照）、越境移転（US region）の同意記録BQ格納、応募者からの削除要求SLA（30日以内・SHA-256ハッシュキーで特定削除）、Consent Mode v2のモデリング推計と実測の分離保存（2026-08-03参照）を、1枚のガバナンス設計書としてnoriへ提示。noriのリーガルチェックで「基盤側は機械保証済み」を前提化し、制作案件の関所工数を削減。Governance Dashboardの「保持期限迫るテーブル」タイルからnoriへCRITICAL通知するフローも同時設計。
- **DuckDB + MotherDuckを開発時探索の既定環境化して、BQフルスキャン起因のスキャン量超過を発生源で断つ**：スキャン量週次監視（2026-06-12参照）は事後検知で、開発時の探索クエリ（`SELECT * FROM ... LIMIT 100`の全期間スキャン）が積み上がると月末に無料枠1TBを食う。DuckDBでBQサンプルをローカル取込むか、Iceberg外部テーブルをDuckDBから直接クエリすることで、開発時のBQスキャン発生をゼロに寄せる。開発環境のREADMEに「本番BQへのSELECTは確定クエリのみ」を明記し、探索はDuckDBを既定化。
