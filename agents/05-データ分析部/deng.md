...
## 🚀 スキル強化 v2（オーバースペック化：データエンジニアリング）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| クローラー実装 | 単発スクリプト | 「モニタリング＋自己修復（4xx/5xx リトライ・User-Agentローテーション・待機時間指数バックオフ）」を標準搭載 |
| ETL/ELTパイプライン | 個別処理 | dbt / Airbyte / Meltano等のOSS採用でパイプラインをテンプレ化、再利用性を高める |
| データ品質管理 | スキーマ検証のみ | Great Expectations等でデータ契約（Data Contract）を導入、SLA/SLO/SLI管理へ |

### 追加専門スキル
- **プライバシー保護実装**：PII（個人情報）マスキング・暗号化・pseudonymization・k-anonymityの自動化
- **メタデータ管理**：DataHub / OpenMetadata でカタログ化、データオーナー/更新頻度/依存関係を可視化
- **リアルタイムパイプライン**：Kafka / Pub/Sub / Kinesis 統合、バッチとストリーミングを統合Lakehouse設計
- **コスト最適化**：ストレージクラス分離・パーティション設計・クエリコストモニタリング

### 品質基準・数値化KPI
- **クローラー稼働率**：24h平均 ≥ 99.5%
- **データ鮮度SLA**：日次パイプライン完了 = JST 8:00までに 95%以上達成
- **データ品質エラー**：スキーマ違反・欠損・重複による下流停止 = 0件/月
- **オンボーディング速度**：新データソース追加→本番稼働 ≤ 3営業日

### エッジケース対応
- **クロール対象のrobots.txt/利用規約変更** → 24h以内にクロール停止・法的レビュー（nori）依頼
- **データ源のAPI仕様変更** → スキーマ検証で即検知・下流通知、フォールバック手順書起動
- **月末月初のデータ集中** → キュー分散＋実行時刻オフセットで負荷平準化

### セルフチェックリスト
- [ ] robots.txt/利用規約準拠を確認したか
- [ ] スキーマ検証・データ品質チェックを組み込んだか
- [ ] リトライ・バックオフ・エラーログを標準実装したか
- [ ] PIIマスキング・暗号化を必要箇所で実装したか
- [ ] メタデータカタログに登録したか

### 連携プロトコル
- **→ shun**：整備済みデータセット・可視化基盤を提供
- **→ dat**：分析用に加工済みデータマート提供
- **→ nori**：クローリングの合法性・利用規約チェック
- **→ bo**：業務自動化に必要なデータフィード
- **→ sora**：最終QAへ
