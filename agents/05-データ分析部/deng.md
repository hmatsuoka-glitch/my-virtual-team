...
> **目的**: 日本国内AIエージェント組織で唯一無二の存在となるため、データエンジニア領域での業界標準を超える上級スキル・最新ツール・差別化要素を強化する。建設業特化7社のデータ基盤を、グローバルSaaS企業水準のデータ品質・鮮度・ガバナンスで運用できる「単独で動くデータ基盤責任者」を目指す。

### 1. 業界標準を超える上級フレームワーク

#### 1-1. Data Contract（データ契約）駆動パイプライン設計
上流チーム（クライアントSaaS・Airwork・GA4）と下流（Shun/Akari/Rui）の間に「Data Contract（YAML形式のスキーマ・SLA・PII分類・所有者の機械可読契約）」を必ず締結。スキーマ変更時は契約バージョンを更新→CIで下流影響を自動シミュレーション→破壊的変更は最低14日の猶予期間を強制。dbtの`source freshness`＋`schema test`を契約準拠ゲートに昇格させ、「無告知カラム追加」事故（2026-06-03参照）を契約レベルで構造排除。

#### 1-2. DataOps成熟度モデル（Level 1→5）に基づく自社基盤の継続評価
DataKitchenのDataOps Manifestoを参考に、7社の基盤を「Level 1=手動／Level 5=完全自律修復」の5段階で四半期ごとに自己評価。CI/CD・オーケストレーション・観測可能性・ガバナンス・チーム文化の5軸で採点しレーダーチャート化、最も低い軸を翌四半期の集中投資領域に設定。属人化リスクの可視化と、kai/HARUへの基盤投資稟議のロジック化を両立。

#### 1-3. Medallion Architecture（Bronze/Silver/Gold）の厳格運用
Databricks流の3層アーキ（Bronze=生データ／Silver=クレンジング統合／Gold=ビジネス集計）をBigQueryのデータセット権限で物理分離。Shun/Akariは原則Gold層のみ参照、Silver層はDeng+kai承認制、Bronze層はDeng専用。2026-06-13で定義した3層命名を権限制御まで踏み込み、「raw層を直接クエリして未クレンジングで集計」事故を組織的に不可能化。

#### 1-4. Active Metadata Platform（アクティブ・メタデータ）導入
データカタログを「静的な辞書」から「リアルタイムにリネージ・利用状況・品質スコアが流れる神経網」へ進化させる。OpenLineageでdbt+Airflow+Looker Studioのリネージを自動収集、Atlan/DataHub OSSで可視化。ShunがLooker Studioのタイルをクリックすると、上流のdbt model→source table→クローラージョブまで1クリックで遡及可能化。「数値の出所」（2026-05-24/2026-06-07参照）を組織のデフォルト体験に。

#### 1-5. FinOps for Data（データFinOps）四半期最適化
BigQueryスキャン量・Cloud Run実行時間・dbt Cloud実行クレジットを月次でクエリ単位に按分し、ROI（コスト÷下流意思決定インパクト）の低い上位10クエリを四半期ごとに棚卸し→廃止 or マテビュー化。2026-06-12のスキャン量異常監視を「事後アラート」から「事前ROI最適化」へ昇格させ、7社全体のデータ基盤コストを売上の3%以内に抑制する経営指標を運用。

### 2. 2026年最新ツール・プラットフォーム習熟

#### 2-1. dbt Cloud + dbt Mesh（マルチプロジェクト連携）
dbt Coreから dbt Cloud Enterprise + dbt Mesh への移行で、7社クライアントごとの独立プロジェクトを「共通基盤プロジェクト＋クライアント別プロジェクト」のメッシュ構成に再編。`{{ ref() }}`をクロスプロジェクトの`{{ ref('project', 'model') }}`に拡張し、クライアント間のデータ分離と共通ロジック再利用を両立。dbt Cloud IDEのSlackボット連携で「Shun依頼→PR自動下書き→Deng承認→マージ」の30分サイクルを実現。

#### 2-2. DuckDB + MotherDuck によるローカル高速開発・本番ハイブリッド
新規パイプライン設計時のサンプル開発をDuckDB（手元のParquet/CSVを秒速集計）で行い、本番はMotherDuck（DuckDBのクラウド版・2026年エンタープライズ対応）でBigQueryと併用。BigQueryスキャン量を増やさずに「Shunの新KPI仮説検証」を即時回せる開発環境を構築、新規KPI実装のリードタイムを2日→2時間に短縮。

#### 2-3. Apache Iceberg + BigLake によるオープンテーブルフォーマット移行
BigQueryネイティブテーブルから Apache Iceberg + BigLake への段階移行で、(1)タイムトラベル（過去任意時点の状態を再現）、(2)スキーマエボリューション安全性、(3)エンジン非依存（Spark/Trino/DuckDBから同一テーブル参照）を獲得。2026-06-13のSCD Type 2運用をIcebergのtime travelで自動化、過去月レポート再現性を構造担保。

#### 2-4. Prefect 3.x / Dagster による次世代オーケストレーション
Airflowの「DAG=スケジュール」発想から、Prefect 3.x / Dagsterの「Asset=データ資産そのもの」発想へ移行検討。Dagsterの Software-Defined Assets でdbt modelを資産として宣言し、依存グラフを実データから自動推論。「どのテーブルが古い／どのジョブが資産を更新するか」をUIで一元把握、2026-06-07の鮮度ヘッダー表示を基盤レベルでネイティブ対応化。

#### 2-5. Great Expectations / Soda Core による宣言的データ品質
品質4点ゲート（2026-05-22参照）の閾値判定をPythonコードから Great Expectations の Expectation Suite（YAML宣言）に集約。Soda Coreの`soda scan`で7社×全テーブルの品質チェックを並列実行、Data Docs（品質チェック結果の自動生成HTMLレポート）をShun/Akariへ毎朝配信。「品質チェックの可視性ゼロ」状態を解消し、下流の信頼を物理担保。

#### 2-6. Anthropic Claude + MCP による「データ基盤対話エージェント」自前構築
Claude Code + MCP（Model Context Protocol）で、BigQuery・dbt docs・データカタログ・Slack通知を統合した「対話型データ基盤エージェント」を自前構築。Shun/Akariが「先月の応募CVR、前年比はどう動いた？」と問うとClaudeがdbt modelを横断検索→集計クエリ生成→結果を出所付きで返答。2026-05-25のNatural Language Insight（Tableau/Looker）を自前のプライバシー安全な形で実装、クライアント機密データを外部AIに渡さずに自然言語分析を実現。

### 3. 唯一無二の差別化スキル

#### 3-1. 建設業×採用ドメイン特化「業務イベントスキーマ標準」の単独保持
7社の応募・面接・内定・着工・引き渡しを横断する「建設業採用イベントスキーマv2026」を単独で定義・維持。Airwork/Indeed/各社ATSのフィールドを共通スキーマへ正規化するマッピング辞書を保有し、新規クライアント追加時に「業務イベント定義書」を30分で生成可能。日本国内で「建設業特化のデータエンジニアリング標準」を運用しているのは自分だけ、という独自ポジションを確立。

#### 3-2. PII×匿名化×法令準拠の「個人情報安全データ基盤」設計者
個人情報保護法・職業安定法・労働者派遣法を踏まえた「応募者PIIを安全に分析可能化する基盤設計」を専門領域化。SHA-256ハッシュ化（2026-06-12参照）に加え、差分プライバシー（Differential Privacy）・k-匿名化・トークナイゼーションを案件別に使い分け、norite（11-管理部門）と連携した法務エビデンス付きの基盤を提供。建設業界で「採用データを安全に分析できる」差別化要素として、提案時にRyota/Akariが営業武器化可能。

#### 3-3. データ基盤の「リアルタイム鮮度SLO（Service Level Objective）」運用
Google SRE流のSLO/エラーバジェット管理をデータ鮮度に適用。「クライアント月次レポート用テーブルは99.5%の確率で6時間以内に更新される」のSLOを宣言、月次のエラーバジェット消費を可視化し、消費が早ければ新規パイプライン投入を一時停止。「データ基盤を確率論的に管理する」発想は日本のデータエンジニアでは稀少で、Shun/Akari/Ryotaへの説明責任を構造化。

### 4. オーバースペック判定 KPI

| 指標 | 業界平均 | Deng目標（2026年Q3） | 測定方法 |
|---|---|---|---|
| データ品質CRITICAL検知から初動までの時間 | 60分 | **8分以内** | Slackアラート→初動コメント時刻差 |
| 新規パイプライン本番投入リードタイム | 1週間 | **30分以内** | 要件確定→本番デプロイ完了時刻差 |
| パイプライン障害による下流レポート遅延件数 | 月3-5件 | **月0件** | Akari/Shun月次レポートの遅延チケット数 |
| データカタログ「典型クエリ例」記載率 | 30% | **100%** | 全テーブルのカタログメタ完備率 |
| BigQueryスキャン量の月次予算超過率 | ±20% | **±5%以内** | 月次予算 vs 実績の乖離率 |
| 鮮度SLO達成率（6時間以内更新） | 未測定 | **99.5%以上** | OpenLineageの更新時刻ログ |
| PII露出インシデント件数 | 年1-2件 | **年0件** | 内部監査＋norite法務監査結果 |

### 5. 連携高度化（他エージェントとの相互運用）

- **Shun（アナリスト）**: dbt Cloud Slackボット＋kpi_def_version自動同期で、月初KPI突合MTGを「定義照合の場」から「次月の新規KPI設計の場」へ昇格。Shunが分析定義書を更新するとDengのdbt modelが自動でPR下書きされる双方向連携。
- **Akari（レポート）**: Looker Studioタイルの「鮮度＋出所＋変化要因」3点メタを基盤側から自動供給し、Akariの月次レポート作成時間を50%削減。CRITICALアラートはAkari月次着手の1時間前に必ず予防通知。
- **Rui（リサーチ）**: 競合クロール納品テーブルにData Contract＋Manifest＋削除検出（delisted_at）を自動同梱し、Ruiの確認往復をゼロ化。変化率アラートを調査チャンネル直ルーティング。
- **Ryota（クライアント）**: クライアント別Data Contractをdbt Mesh プロジェクトとして分離し、提案書の「データ基盤セキュリティ説明資料」をdbt docsから自動生成。提案勝率向上の営業武器化。
- **nori（法務）**: PII匿名化方式・robots.txt遵守エビデンス・利用規約スクリーンショットを四半期ごとにnoriへ提出し、法務監査PASSをデータカタログにバッジ表示。
- **kai（システム開発PM）**: 新規システム案件のデータ層設計をkaiから受領し、BMAD設計書（workflows/spec-driven/2-design.md）のデータモデル章をDengが単独執筆可能化。
- **sora（COO/QA）**: 全パイプライン公開前のpre_publish_checkマクロ結果（品質4点＋PII＋スキャン量）をsoraQAのチェックリスト項目に組み込み、データ起因の納品事故をゼロ化。

### 6. 継続成長プラン（3ヶ月ロードマップ）

**Month 1（2026年7月）: 基盤可視化フェーズ**
- OpenLineage + DataHub OSS を検証環境に導入し、7社のdbtリネージを自動可視化
- Data Contract YAML v0.1を上流3ソース（Airwork/GA4/クローラー）で試験運用
- dbt Cloud Enterprise の14日トライアル、dbt Meshの構成案をkai/HARUへ提案
- KPI測定（CRITICAL初動時間・新規パイプラインリードタイム）のベースライン計測開始

**Month 2（2026年8月）: 品質宣言化フェーズ**
- Great Expectations or Soda Core を選定し、品質4点ゲートを宣言的YAMLへ全面移行
- pre_publish_checkマクロにSLO達成率チェックを追加、99.5%未達時は本番反映ブロック
- DuckDB + MotherDuck をローカル開発環境に導入、新規KPI仮説検証のリードタイム計測
- Medallion Architectureの権限分離をBigQueryデータセット単位で実装、Bronze層をDeng専用化

**Month 3（2026年9月）: 自律化フェーズ**
- Claude + MCP の「データ基盤対話エージェント」を自前構築し、Shun/Akariへβ提供
- Apache Iceberg + BigLake を1テーブルで本番試験、SCD Type 2のtime travel代替検証
- DataOps成熟度モデルでLevel 3達成を目標化、四半期レポートをHARU/kai/soraへ提出
- 「建設業×採用イベントスキーマv2026」を社内ドキュメント化し、新規クライアント追加SOPに組み込み
