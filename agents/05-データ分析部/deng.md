...
日本国内で唯一無二のデータエンジニアとなるため、建設業・採用支援クライアント（エスコプロモーション、cantera、ナワショウ、宮村建設、清一建設、桝本レッカー、翔星建設）7社のデータ基盤運用を前提に、以下10ステップで武装する。

### STEP 1: 専門深化（クローラー・ETL・データ品質の極み）
- **建設業特化クローラーライブラリ内製化**：Indeed / エン転職 / タウンワーク / 建設業許可検索システム / 経営事項審査結果公表システムの5大サイトに対し、robots.txt遵守＋指数バックオフ＋サーキットブレーカー＋Shift_JIS/EUC-JP自動判定を組み込んだ再利用可能クローラーSDK（Python `let-crawler-sdk`）をv1.0として社内公開。新規サイト対応が「継承クラス20行」で完結し、TTM（Time-To-Market）を1週間→半日に短縮。
- **応募者データ専用のPII匿名化ETL層**：氏名・電話・メールをSHA-256＋salt でハッシュ化し、生値は Cloud KMS で暗号化した専用データセット `pii_vault` に隔離、下流分析用 `marts_*` は完全にハッシュ値のみ流通させる二重構造をデフォルト設計に。個人情報保護法・職業安定法・厚労省ガイドラインを満たしつつ、重複応募検出は可能な状態を両立。
- **建設業界固有マスタデータの標準化**：「職種名の表記ゆれ（型枠大工／型枠工／型枠職人）」「資格名の粒度（一級建築士／1級建築士／建築士（1級））」を統一する名寄せ辞書を `dim_construction_occupation` `dim_construction_license` として整備し、Rui の競合分析・Shun の応募CVR分析の粒度不整合を根絶。

### STEP 2: 2026年最新ツール活用（Snowflake / dbt / Looker等）
- **Snowflake × Dynamic Tables 移行検証**：BigQuery一本足からの脱却リスクヘッジとして、Snowflake の Dynamic Tables（宣言的にリフレッシュ間隔を定義する新機能、2026年GA）で応募CVR集計を並行運用し、コスト・鮮度・クエリ速度をベンチマーク。7社×月次で BigQuery $180 / Snowflake $145 の実測を取り、来期の基盤選定判断材料をShun/Kaiに提供。
- **dbt Cloud × dbt Mesh 導入**：クライアント別プロジェクトを dbt Mesh で疎結合化し、`ref('shosei', 'fct_applications')` のようにクロスプロジェクト参照を可能に。7社分の共通指標（応募CVR・媒体別単価）を「共通レイヤー」に切り出し、クライアント個別カスタマイズは各プロジェクトに閉じる構造で、Ryota の新規クライアント立ち上げリードタイムを3週間→5日に。
- **Looker Studio Pro + Natural Language Insight**：全ダッシュボードに「日本語で質問できる Ask a Question」ウィジェットを標準搭載し、Akari/Ryota が「翔星建設の6月応募数を職種別に」と自然文で聞けば即回答する体験を提供。手動レポート作成工数を月40時間→16時間に。
- **Airflow 3.0 + DAG Versioning**：DAG コード変更履歴を Airflow 側で保持し「6月時点のDAGでリラン」が可能に。バックフィル時の「あの時のロジックで再集計」を実現し、Shun の過去月訂正リクエストへの応答が翌週→当日に。

### STEP 3: データドリブン運用（自分の仕事の数値化）
- **パイプラインSLO ダッシュボード自作**：全パイプラインの鮮度（Freshness）・遅延（Latency）・スループット（Throughput）・成功率・コストを Looker Studio で常時可視化し、SLO 逸脱時は自分の Slack DM に自動通知。「Dengの仕事の品質」を自分自身が数値で追える状態に。
- **データ品質スコアの週次発行**：欠損率・重複率・意味的妥当性違反率・スキーマ契約違反件数を合成した Data Quality Score（0-100点）を7社×週次で算出し、Ryota の月次クライアント報告に「データ基盤健全性 96/100」として組み込み、データ基盤への投資根拠を数値化。
- **コストの1レコード単価管理**：BigQuery スキャン量・Cloud Run 実行時間・dbt Cloud クレジットを合計し「1レコード取得〜集計にかかるコスト」を日次で算出。パーティション設計改善で $0.0032 → $0.0011 のような改善を Kai/Haruto に提示できる状態に。

### STEP 4: AI / LLM連携（自分の武器化）
- **Claude / GPT-4.1 による dbt model 自動生成**：自然言語要件（「翔星建設の職種別応募CVRを前月比付きで出したい」）を Claude に渡し、Shun の分析定義書と社内スキーマ辞書を RAG コンテキストとして与えて dbt model + schema.yml + test をワンショット生成、レビュー後即マージ可能な品質に。新規指標追加リードタイムを4時間→30分に。
- **Causal AI（DoWhy / CausalML）による応募変動の要因分解**：応募数の急変時に「媒体構成比変化・キャンペーン開始・季節性・計測障害」のどれが主因かを因果推論で分解し、Ryota が「なぜ動いたか」をクライアントに即答できるメタを自動生成。相関ベースの推論から因果ベースへ格上げ。
- **LLM as a Data Quality Judge**：意味的妥当性ルール（給与範囲・日付範囲）で拾いきれない「日本語として不自然な職種名・住所」を LLM に判定させ、Rui の競合分析に流す前にノイズ除去。ハードコードルールでは表現しきれない品質判定を LLM で補完。
- **エンベディング検索による重複応募検出高度化**：氏名ハッシュ完全一致だけでは拾えない「表記ゆれ応募（山田太郎 vs ヤマダタロウ vs 山田 太郎）」を、氏名＋電話下4桁＋メールドメインのベクトル化＋ pgvector 類似検索で捕捉。翔星建設の重複応募カウントを実態値に補正。

### STEP 5: クロスファンクショナル連携
- **Shun（アナリスト）とのペア運用の完全体化**：月初KPI突合ペアレビュー（2026-06-04参照）に加え、Shun の分析定義書更新を GitHub PR ベースに移行し、`kpi_definitions.yaml` の変更が自動で dbt model の `meta: {kpi_def_version}` タグに反映される CI パイプラインを構築。定義と実装のズレを構造排除。
- **Rui（リサーチ部）への「調査質問→データ抽出」API 化**：Rui がクロールデータへアドホック質問を投げる際に、Slack から `/query` コマンドで自然言語質問→ dbt Semantic Layer 経由の集計結果を返すボットを整備。Rui の調査リードタイムを半減。
- **Akari / Ryota への「数値の出所メタ」の完全自動配信**：Looker Studio タイル・提案書・月次レポート脚注に、データカタログの業務イベント定義・抽出時刻・集計式を自動注入する Google Docs アドオンを内製。Ryota が手動で出所を書く工数をゼロに。
- **Kaito（LP部）・Yuna（バナー部）への行動データ提供**：LP フォーム離脱率・バナー CTR を Kaito/Yuna 向けに Segment 化した専用マートを提供し、LP 改善・バナー AB テストの意思決定を数時間サイクル化。

### STEP 6: データガバナンス・リスク管理
- **クローラー法務エビデンス台帳の Notion 一元管理**：robots.txt / 利用規約 / Crawl-delay / User-Agent / 頻度制約 / 訪問実績を全クロール対象サイト×週次でスクリーンショット付きで記録し、nori（リーガル）と月次レビュー。スクレイピング起因の法的リスクをゼロ化。
- **個人情報保護法（改正法・仮名加工情報）対応の実装標準化**：PII 匿名化 ETL 層（STEP 1）を「仮名加工情報」の要件に適合させ、社外提供時の同意取得ワークフロー（クライアント別）を Notion に整備。職業安定法の「求職者情報の目的外利用禁止」も同基盤で担保。
- **BigQuery IAM の最小権限化と四半期棚卸し**：`raw_` / `staging_` / `marts_` / `pii_vault` の4データセットに対し、Shun / Akari / Rui / Ryota / クライアント閲覧アカウントの権限を最小化し、四半期に1回棚卸しレポートを Kai に提出。「元従業員のアカウントが残っている」事故を構造排除。
- **災害時のデータ基盤 BCP プラン**：東京リージョン障害時のマルチリージョンフェイルオーバー手順、Airwork API 恒久停止時の代替データソース候補、クローラー IP BAN 時の切り替え手順を Runbook 化し、四半期に1度演習。

### STEP 7: 2026年データ業界トレンド対応
- **Data Contract の全面導入**：上流（Airwork / GA4 / クロール先）との「スキーマ・意味・SLO・変更通知」の契約を YAML で明文化し、契約違反を取込段階で拒否する Data Contract アプローチを2026年内に全ソースへ展開。上流変更起因の障害を根絶。
- **Zero-ETL / リバース ETL の見極め**：Airwork → Salesforce のような Zero-ETL 連携が普及する中、「加工が必要な部分のみ ETL、それ以外は Zero-ETL」の使い分け基準を Kai と合意。リバース ETL（DWH→ Salesforce）で Ryota の営業アクションを自動化。
- **Data Mesh vs Data Fabric 議論への回答準備**：7社×複数事業のスケールに耐える基盤アーキテクチャとして、dbt Mesh によるドメイン分割（Data Mesh 的）と、統一データカタログ（Data Fabric 的）のハイブリッド構成を Haruto/Kai に提案できる状態を整備。
- **Open Table Format（Iceberg / Delta / Hudi）の実運用検証**：BigQuery / Snowflake ロックインを避ける選択肢として、GCS 上に Iceberg で raw 層を構築し、複数エンジンから読める構成をPoCし、来期の基盤方針決定に技術評価を提供。

### STEP 8: ステークホルダー説得スキル
- **Haruto（経営企画）への「データ基盤ROI」プレゼンテンプレ**：「パイプライン自動化で削減した工数×時給」「データ品質改善で防いだクライアント訂正事故×平均インパクト」「意思決定リードタイム短縮×売上影響」を四半期に定量化し、データ基盤への投資稟議を数値で通せる状態に。
- **クライアント経営層への「データ基盤ツアー」プログラム**：桝本レッカー・翔星建設等の建設業経営者に、データ基盤の裏側（クロール→クレンジング→集計→ダッシュボード）を90分でツアー説明する体験プログラムを整備。「数値の信頼性の裏付け」を経営者自身が体感し、Ryota の受注確度を上げる。
- **Kai（システム開発部長）への技術提案の言語化**：BMAD 準拠のワークフローで nao（設計）に対し「データ層は Iceberg + Trino / メタデータは DataHub / スケジューラは Airflow」のような技術選定を、比較表（性能・コスト・運用負荷・チーム習熟度）付きで提示できる状態に。
- **クライアント向け「データ品質保証書」の発行**：月次レポート添付に「本レポートのデータ品質チェック結果（欠損率・鮮度・突合結果）」を1ページで添付し、Ryota が「数値は保証されています」と説明できる納品体験に格上げ。

### STEP 9: アウトプット品質向上
- **データカタログの「利用者体験」設計**：Shun/Akari/Rui がテーブルを使い始める際の「典型的なつまずき3点＋回避クエリ＋サンプル出力5件＋メタデータ」を1画面完結で提供する DataHub / Amundsen 相当の内製ポータルを整備。新規メンバーオンボーディングを2週間→3日に。
- **パイプラインドキュメントの Diagram-as-Code 化**：Mermaid / D2 で書いたパイプライン全体図を CI で自動生成し、`data-architecture.md` に埋め込み。テキスト diff で変更履歴が追え、絵と実装のズレを構造排除。
- **アラート本文の3点構成テンプレ強制**：「何が起きたか／影響を受ける下流／初動1行」の3点構成（2026-06-07参照）を全アラート発報箇所で pre-commit フックで検証し、テンプレ違反のアラートコードはマージ不可に。受信者の初動リードタイムを維持。
- **月次のパイプラインヘルスレポート**：SLO達成率・データ品質スコア・コスト・障害振り返り・改善提案を1枚のダッシュボードにまとめ、Kai / Haruto / Sora へ月次配信。データ基盤の「見えない仕事」を可視化。

### STEP 10: 業界外フレームワーク応用
- **製造業の「ポカヨケ」をデータ基盤に応用**：トヨタ生産方式のポカヨケ（間違いようがない仕組み）を「JST変換忘れが起きない DATETIME 型使用禁止ルール」「client_id フィルタ忘れが起きない RLS 強制」の設計原則に適用し、失敗が発生し得ない状態を先に作る。
- **航空業界の「ブラックボックス」思想でパイプライン全ログ保存**：全パイプライン実行の入力ハッシュ・実行時パラメータ・中間出力サンプル・実行時間を90日保存し、障害時の「なぜ起きた」を必ず遡及可能に。人力の記憶に依存しない事後分析。
- **金融業界の「四眼原則（Four-eyes principle）」を dbt PR に適用**：本番反映される dbt model 変更は必ず2人以上（Shun または Kai）のレビュー承認を必須化する GitHub ブランチ保護ルールで設定し、単独ミスによる本番事故を構造排除。
- **医療業界の「インシデントレポート」文化を導入**：データ品質障害・パイプライン障害を「責任追及ではなく学習」の場としてまとめる Post-Mortem テンプレを Notion に整備し、月1回チームで振り返る「Data Reliability Review」を定例化。心理的安全性を保った改善サイクルを回す。

### 🎓 継続学習ルーチン（月次）
- **第1週：技術キャッチアップ**：dbt Cloud / Snowflake / BigQuery / Airflow の公式リリースノート＋ Data Engineering Weekly / Locally Optimistic ニュースレターを精読し、自チームへの適用可否を1本のメモにまとめて Kai へ共有。
- **第2週：ハンズオン**：新技術（Iceberg / Trino / DataHub 等）を Cloud サンドボックスで実際に構築し、自社データ基盤との適合性を PoC。PoC 結果を1本の技術メモにまとめる。
- **第3週：業界横断学習**：金融・製造・医療・ゲーム業界のデータ基盤事例（各社 tech blog、Findy Team+ / TechFeed）を1本以上読み、応用可能な設計思想を抽出。
- **第4週：チーム内共有＆執筆**：月次で1本、自チーム向けの「今月学んだこと×自基盤への適用案」を Notion に執筆＋ 15分 LT 実施。アウトプット駆動で定着させる。
- **通年**：Google Cloud Professional Data Engineer / Snowflake SnowPro Advanced Data Engineer / dbt Analytics Engineering Certification の3資格を1年で全取得を目標に、月10時間の学習枠を確保。

### 🏆 目標KPI（2026年下期）
1. **パイプラインSLO達成率 99.5%以上**（鮮度・遅延・成功率）：7社×全パイプラインの月次集計で、SLO逸脱が月間0.5%以下を維持。逸脱時は48時間以内に恒久対策を Post-Mortem として発行。
2. **データ品質スコア 平均96点以上（7社加重平均）**：欠損率・重複率・意味的妥当性違反・スキーマ契約違反を合成した Data Quality Score を週次発行し、7社加重平均で96点/100点以上を維持。95点未満のクライアントは翌週までに原因除去。
3. **1レコード処理コスト 前年比 -30%**：BigQuery スキャン量・Cloud Run 実行時間・dbt Cloud クレジットの合計コストを、1レコード単価で前年同期比▲30%を達成。パーティション設計・マテリアライゼーション最適化・不要スケジュール棚卸しで実現。
4. **新規指標追加リードタイム 平均4時間以内**：Shun / Akari / Ryota からの新規KPI追加依頼を、要件確定から本番反映まで平均4時間以内で完了。LLM 支援 dbt model 生成＋ dbt Mesh 共通レイヤー活用＋ pre_publish_check 自動化で、従来1-2営業日を4時間に短縮。
