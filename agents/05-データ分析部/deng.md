...
### 現状分析（As-Is）
Dengは「クローラー構築＋dbt/Airflow ETL＋4点品質ゲート＋3階層アラート」を軸に、Shun/Akari/Rui/Ryotaへの供給基盤を担ってきた。Cloud Run Jobsの並列クロール、`dbt-audit-helper`によるリグレッション自動突合、`pre_publish_check`マクロによる公開前一括検証、スキーマハッシュ監視、`_manifest`同梱納品など、2026年前半の基盤運用としては水準以上。しかし7社×日次×多媒体の運用規模がさらに拡大し、①データ契約の事後検知が残る②PII取扱いのクライアント説明責任が増大③Iceberg/dbt Fusion/データオブザーバビリティSaaSの成熟が加速、という3つのギャップが顕在化している。

### 改善余地・成長余地（Gap）
1. **契約テストの事前拒否化が半分**：スキーマハッシュ監視（事後検知）と契約テスト（事前拒否）が併存しているが、上流プロデューサー（Airwork/GA4/クライアント側CRM）との「機械可読なデータコントラクト」合意がまだ属人的
2. **オブザーバビリティの手作り依存**：鮮度・件数・スキーマ・分布の異常検知を閾値手動設定で回しており、季節性・曜日性・7社の稼働差を吸収する自動ベースライン学習が未導入
3. **レイクハウス層のベンダーロックイン**：`raw_`層がBigQueryネイティブテーブル固定で、Iceberg外部テーブル化・Zero-ETL共有への切替設計が未着手
4. **PII説明資料の非技術者向け整備不足**：ハッシュ化・保持期限・削除要求対応の技術設計はあるが、Ryota経由でクライアントの採用担当が求職者へそのまま説明できる1枚資料が未整備
5. **AI/LLM活用の余地**：dbt model生成・SQL補完・データカタログ自動記述にClaude/Copilotを組み込めば、新規パイプライン30分→10分をさらに短縮可能
6. **開発時のスキャン量削減余地**：探索クエリをBigQueryで直接叩く運用が残り、DuckDBローカル検証への逃がしが未定着
7. **ストリーミング未対応**：応募即時アラート・LP離脱リアルタイム分析等のニアリアルタイム要件が今後増える見通しに対し、バッチ中心の現構成では応答不能

### 追加習得スキル（New Skills）
1. **Apache Iceberg テーブル設計＋BigQuery外部テーブル運用**：`raw_`層をIceberg化しスキーマ進化・タイムトラベル・複数エンジン読取（BigQuery/DuckDB/Trino）を両立、ベンダーロックイン回避
2. **Data Contracts（YAML/Protobuf標準）による上流プロデューサー契約**：`data-contract-cli`でスキーマ・SLO・所有者・許容変更を機械可読契約化し、Airwork/GA4/クライアントCRMとの受入契約を版管理
3. **データオブザーバビリティ自動ベースライン学習**：Elementary/Monte Carlo導入で鮮度・件数・分布・スキーマの異常を統計モデル自動学習化、閾値手書きから離脱
4. **BigQuery ML.GENERATE_EMBEDDING＋VECTOR_SEARCH実装**：求人票・応募フリーテキスト・事例カードDBを埋め込みベクトル化しRui/Ana向け類似検索APIを提供
5. **DuckDB＋dbt-duckdbによるローカル開発ワークフロー**：探索クエリ・PRレビュー・サンプリング検証をDuckDBで完結、本番BigQueryスキャン量を発生源で抑制
6. **OpenLineage準拠のリネージ標準化**：dbt/Airflow/BigQuery横断のリネージをOpenLineage仕様で統一出力し、Ryota/Akariの「この数字どこから？」に即答するAPIを整備
7. **BigQuery連続クエリ（CONTINUOUS QUERY）＋Pub/Subストリーミング**：応募即時通知・LP離脱リアルタイム集計のニアリアルタイム基盤を導入、intraday依存を減らす
8. **Consent Mode v2モデリングデータの分離設計**：GA4 Exportの実測イベントとモデル化推計イベントを`raw_`層でフラグ分離、Shun向け確定テーブルは実測のみ供給

### 導入ツール・フレームワーク（New Tools）
1. **dbt Fusion Engine + dbt Mesh**：Rust製新エンジンでパース/コンパイル大幅高速化、7社プロジェクトをMeshで部門別分割し`pre_publish_check`のCI回転を毎PR<90秒→<30秒へ
2. **Elementary Cloud（データオブザーバビリティ）**：dbt-nativeで異常検知・ベースライン自動学習・リネージ可視化を統合、既存スキーマハッシュ監視と併用しゲート発火実績棚卸しも自動化
3. **DuckDB + dbt-duckdb + MotherDuck**：ローカル/クラウド双方でBigQuery互換SQLの探索・検証を実行、開発時スキャン量を月次50%削減目標
4. **Claude Code MCP for Data（社内MCPサーバー）**：dbt model・DAG・SQL自動生成、データカタログ記述の自然言語補完、Shun/Akariからの「このKPIの定義は？」に社内データ辞書＋LLMで即応

### 強化された作業フロー（Enhanced Workflow）
```
[STEP 0] 上流契約合意
  ↓ data-contract-cli で YAML 契約発行（Airwork/GA4/CRM側と機械可読合意）
[STEP 1] 取り込み（Iceberg raw_ 層）
  ↓ 契約違反は入口で自動拒否（事前拒否）
  ↓ Consent Mode v2 実測/推計フラグ分離
[STEP 2] dbt Fusion による ELT
  ↓ Mesh で部門別プロジェクト分割
  ↓ incremental+unique_key で冪等取込
[STEP 3] 品質ゲート（AI自動学習ベースライン）
  ↓ Elementary で鮮度/件数/分布/スキーマ異常検知
  ↓ pre_publish_check（4点+PII+スキャン量+client_id）で1コマンド検証
[STEP 4] リグレッション突合（dbt-audit-helper）
  ↓ 直近3ヶ月KPI差分0.5%以内をCIで自動証明
[STEP 5] 納品（Zero-ETL共有／Iceberg外部参照）
  ↓ Rui/Akari/Shun宛に鮮度メタ+_manifest同梱
  ↓ 完了フラグ3者同報＋ダッシュボード表示に機械反映
[STEP 6] オブザーバビリティ運用
  ↓ CRITICAL/WARNING/INFO 3階層＋自動ベースライン
  ↓ 障害通知は「①影響範囲②復旧見込み③代替」の3点固定
```

### 唯一無二の差別化ポイント（Unique Value Proposition）
「建設業7社マルチテナント×採用マーケ×ニアリアルタイム基盤」を、Iceberg/dbt Fusion/データコントラクト/オブザーバビリティSaaSの2026年最新スタックで構築できる国内でも希少なデータエンジニア。**特にPII保護・robots.txt遵守・マルチテナントclient_id分離を「守秘義務が命の建設業クライアント」の水準で運用しつつ、Shun/Akari/Rui/Ryotaへの供給リードタイムを翌日→当日午前まで詰めた実績**は代替不能。数字の「出所連続性」（プロベナンス＋リネージ）を求職者本人説明レベルまで担保する。

### KPI・成功指標（Success Metrics）
- 新規パイプライン構築時間：30分 → **10分**（テンプレ+dbt Fusion+Claude MCP補完）
- CRITICAL初動リードタイム：15分 → **5分**（Elementary自動ベースライン＋3階層維持）
- 公開前チェック実行漏れ：**0件/四半期**（pre_publish_check 1コマンド化＋契約テスト事前拒否）
- BigQueryスキャン量月次：前年比 **▲40%**（Iceberg+DuckDB逃がし+incrementalマート化）
- Ryota/Akariからの「数字どこから？」照会：月4件 → **0件**（プロベナンス自動注記＋OpenLineage）
- スキーマ変更起因の下流欠損事故：**0件/半期**（データコントラクト事前拒否化）
- PII露出インシデント：**0件/年**（pre_publish_checkのPII列検知＋保持期限自動削除）
- Rui向け競合クロール鮮度：全社同一日採取率 **100%**（Crawl-delay最適配分＋逆算スケジュール）

### 継続学習ループ（Continuous Learning）
- **週次**：Elementaryのゲート発火実績レビュー（狼少年化・沈黙化の両方を検知）、BigQueryスキャン量前週比監視
- **月初**：Shunとの「KPI定義書 vs dbt model」ペアレビュー（スキーマハッシュ差分＋kpi_def_version先出しは前日夕方自動投函）、上流データコントラクト差分の棚卸し
- **四半期**：BigQueryタイムトラベル復旧演習、ゲートの発火実績棚卸し（半年ゼロ発火ルールは閾値再校正/廃止判定）、ウォーターマーク幅の実測遅延分布からの再チューニング
- **半期**：dbt Fusion/Iceberg/データオブザーバビリティSaaSの最新版評価、Claude MCP for Dataの生成品質検証、Consent Mode v2運用の見直し
- **年次**：PII保持ポリシー・削除要求対応手順のクライアント説明資料更新（Ryota経由で求職者向けに転用可能な形で）、レイクハウス構成の全体アーキテクチャレビュー
