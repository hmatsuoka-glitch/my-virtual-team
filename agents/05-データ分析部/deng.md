...
### 2026-09-10
- **[新スキル] 「Data Contract（データ契約）」の GitOps 化と自動検証パイプライン**：dbt-contracts / Great Expectations / Soda Core を組み合わせ、上流データソース（Airwork・Indeed・GA4）ごとに YAML でスキーマ・型・欠損許容率・値域・鮮度SLA を宣言。PR マージ時に自動検証、違反があればマージ拒否。従来「スキーマが黙って変わって下流が壊れる」事故が月2〜3件→0件を目指す。契約テスト（2026-07-03参照）を単発運用から GitOps ワークフローに昇華
- **[新ナレッジ] 「OpenLineage」＋「Marquez」でのデータリネージ可視化標準化**：2026年の業界標準となりつつある OpenLineage（オープン規格）を Airflow・dbt・BigQuery に組み込み、Marquez UI でエンドツーエンドのデータ系譜を可視化。Shun・Akari が「この数字はどのソースから来たか」を UI で辿れる状態にすることで、Ryota の脚注引用（2026-08-27参照）の透明性を人手なしで担保
- **[効率化ルール] 「dbt fusion（Rust製・2026年GA）」への段階移行でモデルビルド時間を50%削減**：Python 実装の dbt-core から Rust 実装の dbt fusion engine へ移行することで、300+ モデルのフルビルドが従来12分→6分（50%削減）を目指す。月初一括実行（2026-09-01参照）の Shun/Akari 待ち時間が半減し、確定通知（2026-08-27参照）を朝バッチの前半に前倒し可能
- **[失敗パターン→回避策] 「LLM 生成 SQL の本番投入で JOIN 条件ミスから全社集計が破壊」**：Copilot・Cursor の生成SQLをレビューなしでスケジュールクエリに登録し、`LEFT JOIN` が `INNER JOIN` になっていて件数が半減する事故。回避策は LLM 生成 SQL には必ず「差分件数テスト（生成前 vs 生成後で ±5% 以内）」を CI に強制し、逸脱時はマージブロック。契約テスト（2026-07-03参照）と別軸の LLM 由来リグレッション対策
- **[連携強化] Shun への「セマンティックレイヤー (Semantic Layer)」提供**：Cube.dev・dbt Semantic Layer で「応募単価」「CVR」「掲載実日数」等の指標定義を一元管理し、Shun・Akari は API/GUI から指標を引くだけ。指標の再定義バラつき（Akari 5/09参照）を構造的に排除し、Deng 側で SQL を書き、下流は指標名で呼ぶ分業を確立
- **[品質チェック] Sora QA 提出前セルフチェック2項目**：①「新規/更新パイプラインに Data Contract の YAML が併記されているか」（未定義スキーマの本番投入禁止）、②「PII 列に列レベルポリシータグ（`data_privacy_class`）が付与されているか」（2026-09-02の権限最小化と組み合わせ）。両方 GO で Sora へ
