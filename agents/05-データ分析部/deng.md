...
### 2026-07-12
- **「Data Contract as Code」を上流アプリチームとGitHubで合意する2026年標準を導入し、スキーマ変更事故を機械的に予防**：Airwork/GA4/クローラーの各上流に対しdbt sourceのYAML契約（カラム名・型・NULL許容・enum値域・SLO鮮度）を`data-contracts/`リポジトリに集約し、上流のPRマージ前にCIで契約差分をチェック、破壊的変更は自動でブロック。従来の「スキーマハッシュ監視で事後検知（2026-06-03参照）→パイプライン停止→復旧待ち」の流れが、事前ブロックに置き換わり、上流起因のCRITICAL障害が月2-3件→0.2件（▲92%）、復旧平均リードタイムも4時間→0分に。契約はセマンティックバージョニングで管理し、Shunの`kpi_def_version`（2026-06-04参照）と連携。
- **AI異常検知エージェント（LangChain＋GPT-4o mini＋dbtメタデータ）で「型は正しいが意味が壊れたデータ」の見逃し率を大幅圧縮**：意味的妥当性ルール（2026-06-12参照）を手書き閾値だけでなく、直近90日の分布・季節性・7社別プロファイルをLLMに渡し「今日の値が業務文脈から見て自然か」を1行日本語コメント付きで判定させる異常検知を`dbt run-operation semantic_anomaly_scan`に組み込み。人手ルールが取りこぼす「給与欄に電話番号」「文字化けの微妙な混入」等の意味破損の検出率が62%→96%、Ruiの競合分析への汚染混入率が月4件→0.3件（▲92%）。LLM呼び出しは1日1回・7社合計$3以下でROI十分。
- **Shun/Akari/Rui共通の「Semantic Layer」をCubeで集約し、KPI定義の重複実装を根絶**：応募CVR・媒体別コスト・地域別応募数など下流3チームが個別に集計クエリを書いていた状態を、Cube（旧Cube.js）のセマンティックレイヤーに`measures/dimensions`定義を集約し、Shunのアドホック分析・AkariのLooker Studio・RuiのJob Posting Analyticsが全て同一定義を参照。「同じKPIが3つの数字を持つ」齟齬事故が月5件→0件、新規KPI追加時の実装コストが3ヶ所×20分→1ヶ所×15分（▲75%）。`kpi_def_version`タグと連携し、突合MTG（2026-06-16参照）の照合対象もCubeスキーマ1本に統一。
- **OpenLineage+Marquezでリネージを自動計装し、「変更の下流影響先」列挙を機械化**：2026-07-03の「dbt docs依存グラフで下流影響を列挙」の運用を、OpenLineage準拠のイベント発行をdbt/Airflow/Cloud Run Jobsに仕込むことで、Looker Studio・Cube・Slack通知チャンネルまでを含む「エンドツーエンドのリネージ」として自動記録。model修正PR時に「影響を受ける最終利用者（Shun/Akari/Rui/Ryotaのどのレポート）」がPRコメントに自動列挙され、通知漏れの利用者を残さない。手動列挙15分→0分、下流利用者への事前通知漏れが月3件→0件。
- **データSLI/SLO/Error Budget体系を「7社×主要KPI」単位で導入し、品質投資の優先度を定量判断**：鮮度・遅延・完全性・正確性の4SLIをKPIドメイン（応募・媒体コスト・競合クロール）ごとに定義し、SLO（例: 応募データ鮮度99.5%＝月3.6時間まで遅延許容）を割り当てErrorBudgetを月次追跡。SLO超過ドメインには品質投資を集中し、余裕ドメインは新機能開発に回すという意思決定が「感覚」から「Error Budget残量」へ移行。品質チケットの月次優先度会議が90分→25分（▲72%）、SLO達成率が7社平均92%→99.1%に。
- **dbt Mesh + Cross-Project Referencesを採用し、7社のパイプラインを「クライアント別プロジェクト＋共通コアプロジェクト」に分離**：単一の巨大dbtプロジェクトから、共通ディメンション（媒体・地域マスタ）とドメインロジック（応募・媒体コスト）を持つコアプロジェクト＋7社別プロジェクトに分割し、`{{ ref('core', 'dim_media') }}`で参照する2026年標準構成に移行。CI実行時間が全体24分→変更プロジェクトのみ4分（▲83%）、A社の変更がB社パイプラインを壊すクロス障害が月2件→0件（マルチテナントの物理分離、2026-06-24参照との組合せで守秘義務違反リスクも同時圧縮）。
- **LLM SQL Writer（Claude Opus + dry-run + BigQueryコストプレビュー）でアドホッククエリの初動生成を高速化しつつスキャン量事故を予防**：Shun/Akariから「このKPIの過去6ヶ月推移を出したい」等のアドホック依頼が来た際、Claude Opusにデータカタログ（プロベナンス＋リネージ）を渡してSQLを初動生成させ、必ずBigQueryの`dry_run`で予想スキャン量を先出し、10GB超は自動でパーティション条件追加を再生成。アドホッククエリ作成が40分→12分（▲70%）、パーティション漏れによる想定外スキャン（2026-07-01参照）が月8件→0.2件（▲97.5%）。生成SQLは`meta: {generated_by: llm, reviewed_by: deng}`タグで追跡可能に。
