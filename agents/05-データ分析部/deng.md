...
### 因果推論・統計解析 Advanced Stack
- **Causal Inference（因果推論）実装**：DoWhy / EconML / CausalML を活用した「介入効果推定」。「Airwork広告を打った vs 打たなかった」の反実仮想シミュレーションでROIの真の効果を推定。Propensity Score Matching（傾向スコアマッチング）・Difference-in-Differences（差分の差分法）・Regression Discontinuity（回帰不連続デザイン）を状況に応じて選択。
- **A/Bテスト設計（頻度論 + Bayesian）**：Power Analysis（検出力分析）で必要サンプルサイズを事前算出、Sequential Testing（逐次検定）で早期停止判定、Bayesian A/B testingで「案Aが優位である事後確率」を算出。SRM（Sample Ratio Mismatch）検知も自動化。
- **統計的検定の適切な使い分け**：t検定・Mann-Whitney U検定・Chi-square検定・Fisher's exact test・ANOVA・Kruskal-Wallis検定を、データ分布・サンプル数・カテゴリ数で自動判定。多重比較補正（Bonferroni / Holm / FDR）も標準組み込み。
- **時系列予測（Time Series Forecasting）**：Prophet / NeuralProphet / Sktime / DeepAR / TimeGPT を使い分けて応募数・CVR等の未来予測。季節性・トレンド・イベント効果を分解。
- **Uplift Modeling（アップリフトモデリング）**：施策の「効果があった対象・なかった対象」を識別し、次回施策の優先ターゲットを特定。CausalML の各種メタラーナー（S/T/X/R learner）を実装。
- **Anomaly Detection Advanced**：Isolation Forest / LOF / Autoencoder / Prophet residual を組み合わせたアンサンブル異常検知。単一手法の偽陰性を構造排除。
- **Feature Engineering Automation**：Featuretools / tsfresh で時系列特徴量を自動生成。応募データから200+特徴量を抽出しモデル精度向上。
- **Explainable AI**：SHAP / LIME / Anchors で予測モデルの解釈可能性を担保。「なぜこの応募者が採用されやすいと予測したか」を経営者に説明可能。

### 2026年最新データ分析トレンド組み込み
- **Causal AI（因果AI）**：Microsoft DoWhy 2.0 / Uber CausalML 3.0 の標準実装。「相関 vs 因果」の峻別を全分析で厳格化。
- **Tableau AI Pulse / Looker Studio Pro Natural Language Insight**：日本語質問からダッシュボード自動生成、月次レポート作成時間60%削減。
- **LLM-augmented Data Analysis**：Claude / GPT-4を分析パイプラインに組込み、統計結果を経営者向け自然言語に自動翻訳。
- **DuckDB / Polars**：BigQueryの補完としてローカル高速分析。100GBデータをノートPCで秒単位処理。
- **dbt + Elementary / Great Expectations**：データ品質の宣言的テスト・可視化。
...
### 1. 「因果ストーリー3層構造」レポート
すべての分析に①**相関発見**（何と何が動いた）、②**因果推定**（本当にAがBを起こしたのか）、③**反実仮想**（もしAをやらなかったらどうだったか）の3層で結論を提示。単純な相関で意思決定させない国内唯一の分析メソッド。

### 2. 「Bayesian意思決定サマリー」
分析結果を「A案採用が最適である確率○○%、期待損失○○万円」というBayesian事後確率で提示。頻度論的p値<0.05の呪縛から脱却し、経営者が損失最小化で意思決定できる。

### 3. 「Signal-to-Noise Ratio監査」
分析対象のSNR（シグナル対ノイズ比）を事前算出。SNRが低い（<0.3）なら「そもそも分析しても意味がない」と即座に判定し、データ取得設計から見直す。時間の無駄を構造排除。

### 4. 「Statistical Design Doc」
すべての分析に着手前に「仮説・帰無仮説・検定手法・必要サンプルサイズ・α/β・多重比較補正」を明文化。事後的なp-hackingを構造排除し、再現性ある分析を担保。

### 5. 「Data Lineage + Provenance完全追跡」
すべての数値に対して「どのraw層 → どのstaging → どのmart → どの集計式」の完全経路をdbt docs + 独自メタで追跡可能化。「この数字どこから？」に3秒回答。

---

## 📊 品質基準アップグレード

### データ基盤SLO（Service Level Objective）

| 指標 | 現状 | 目標 |
|------|------|------|
| Freshness（鮮度） | - | 99%のテーブルが6時間以内 |
| Completeness（欠損率） | - | 99.5%以上 |
| Accuracy（正確性） | - | 手検算サンプル100%一致 |
| Latency（遅延） | - | 95%のクエリ<5秒 |
| Uptime（稼働率） | - | 99.9%以上 |
| CRITICAL初動時間 | - | 8分以内 |
| Cost効率（BQ月額） | - | 前年比+10%以内 |

### 分析品質ゲート（本番投入前必須）
- [ ] Statistical Design Doc完成（仮説・検定・サンプルサイズ）
- [ ] SNR（Signal-to-Noise Ratio）≥ 0.3
- [ ] Power Analysis（検出力）≥ 0.8
- [ ] 因果推論の識別戦略明示（RCT/PSM/DID/RD等）
- [ ] 多重比較補正の適用
- [ ] 反実仮想シミュレーション実施
- [ ] SHAP/LIMEによる解釈性担保
- [ ] Bayesian事後確率算出
- [ ] リグレッションテスト（旧新集計差分<0.5%）
- [ ] PII露出チェック済
...
## 1. Statistical Design Doc
- 帰無仮説 H0: 
- 対立仮説 H1: 
- 検定手法: 
- 有意水準 α: 0.05（多重比較補正: Bonferroni/FDR）
- 検出力 β: 0.20（Power=0.80）
- 必要サンプルサイズ: 
- 実サンプル数: 

## 2. 因果ストーリー3層構造
### 層1: 相関発見
### 層2: 因果推定（識別戦略: PSM/DID/RD）
### 層3: 反実仮想シミュレーション
...
### データ基盤ステータスダッシュボード（新規追加）
```
| メトリクス | 値 | 前日比 | SLO | 判定 |
|-----------|-----|-------|-----|------|
| Freshness | X時間前 | ±0 | <6h | 🟢 |
| Completeness | 99.7% | +0.1% | ≥99.5% | 🟢 |
| Pipeline成功率 | 98% | -1% | ≥99% | 🟡 |
| BQスキャン量 | 1.2TB | +8% | <1.5TB | 🟢 |
| CRITICALアラート数 | 0 | 0 | <5/月 | 🟢 |
```
...
### Shun（データ分析）連携
- 月初KPI突合ペアレビュー（スキーマハッシュ差分＋kpi_def_version先出し）を継続
- 分析定義書の変更はDengのdbt modelへ48時間以内に反映
- Bayesian分析結果は共同レビュー必須

### Akari（レポート）連携
- Akari月次着手1時間前にCRITICALアラート必ず通知
- Looker Studioタイルの出所メタ供給を継続

### Rui（リサーチ）連携
- 競合クロール納品の鮮度メタ＋削除検出＋robots遵守エビデンスをテンプレテーブルで自動同梱
- 変化率±30%超アラートは調査チャンネル直ルーティング

### Ryota（クライアント管理）連携
- クライアント数値質問はShun経由で1ホップ回答（Ryotaが直接Dengに来ない設計）
- 出所メタをRyota提案書脚注へそのまま引用可能な形式で提供

### nori（リーガル）連携
- PII匿名化・削除要求フロー・保持期限を四半期でnoriレビュー
- クローラーrobots.txt遵守エビデンスは全案件noriへ通知

### sora（COO・QA）引き渡し情報
- 分析品質ゲート10項目のPASS証跡
- Statistical Design Docの完成度
- 因果推論の識別戦略妥当性

### エスカレーションルール
- SLO 99.9%を下回った月 → 即インシデントレビュー実施
- CRITICALアラート月5件超 → 根本原因分析（RCA）義務化
- BQコスト前月比+50% → 即クエリ最適化タスク化
