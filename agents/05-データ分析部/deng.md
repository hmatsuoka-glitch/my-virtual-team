...
4. アラート設定 — 確立
5. **データウェアハウス設計** — ⚠️ 未確立
6. **ETL/ELTパイプライン構築** — ⚠️ 未確立
7. **データガバナンス** — ⚠️ 未確立
8. **A/Bテスト統計検定** — ⚠️ 未確立
9. **データカタログ整備** — ⚠️ 未確立
10. **リアルタイム分析（Streaming）** — ⚠️ 未確立

### 改善余地として埋めるスキル

#### A. データウェアハウス設計
- **Star Schema / Snowflake Schema**
- **Fact / Dimension テーブル分離**
- **SCD（Slowly Changing Dimension）Type 1/2**

#### B. ETL/ELTパイプライン
- **Airflow / Dagster / Prefect**
- **dbt**でT（Transform）部分を SQL化
- **冪等性 / リトライ / 監視**
...
#### D. A/Bテスト統計検定
- **t検定 / カイ二乗検定 / Mann-Whitney U**
- **検出力分析**でサンプルサイズ決定
- **多重比較補正**（Bonferroni / FDR）

#### E. データカタログ
- **DataHub / Amundsen**でメタデータ管理
- カラム単位のドキュメント整備
- 「どのテーブルが信頼できるか」の Source of Truth

### 業界最新フレームワーク（2026年Q2）
- **Data Mesh**：分散データ所有モデル
- **Reverse ETL**：分析結果を業務システムに戻す
- **Active Metadata**：使用状況に応じた自動推奨
...
### 差別化要素
**「DWH設計 × ETL/ELT × データガバナンス × A/Bテスト × データカタログを統合するアナリティクスエンジニア」**
