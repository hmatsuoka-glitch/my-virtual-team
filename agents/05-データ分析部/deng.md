...
### 2026-09-07
- **dbt Cloud＋BigQueryでモデル層3層アーキテクチャ標準化**：staging/intermediate/martsの3層構造をdbt Cloudで実装、Airwork/Indeed/GA4/Meta広告の全ソースデータをmarts層で「クライアント×職種×媒体×日」の統一グレイン集約。データマート追加リードタイム5日→半日、Shun/Akariの分析速度が3倍に。
- **Playwright＋Claude Agent SDKでAirworkスクレイパー刷新**：従来Puppeteerで書いていた7クライアント分のAirworkデータ取得スクリプトをPlaywright＋Claude Agent SDKに移行、DOM変更検知時にClaude自身がセレクタ修正案を生成しPRを自動作成。メンテナンス工数月8時間→月30分、DOM改修起因のクロール停止インシデントが月3件→0件に。
- **Great Expectations 1.0でデータ品質テスト100項目自動化**：Great Expectations 1.0のフレームワークで、7社×主要20テーブル×5〜7項目＝計700テスト超を毎朝5:00に自動実行、Slack通知＋Notion障害チケット自動起票。データ異常検出リードタイム平均48時間→2時間、下流レポート事故が四半期6件→0件。
- **個人情報保護法2026年改正対応「PII自動マスキングパイプライン」**：応募者データに含まれる氏名・電話番号・メールアドレスをdbt seedsのPII辞書＋正規表現でETL時に自動マスキング（本番DWHは仮名化ID保持のみ、原本は暗号化Storage）。個人情報保護法改正の同意取得・削除権対応にDSAR APIを実装、削除リクエスト対応が72時間→15分に。
- **BigQuery DTS＋Cloud Composerでコスト最適化**：Airwork/Indeedの日次バッチをBigQuery DTS化、Cloud Composerのオンデマンド実行に切り替え、パーティション/クラスタリング再設計でスキャン量削減。BigQuery月額コスト¥180,000→¥52,000（71%削減）、Denglobal予算削減目標達成。
- **建設業DX連携「どっと原価データ」パイプライン新設**：Gen（16-建設業DXシステム部）と連携し、翔星建設・宮村建設の「どっと原価」データを日次でBigQueryに同期、採用データと工事売上データを結合可能に。「採用1名あたり工事売上インパクト」を初めて可視化、Akari月次レポートに新KPI追加。
- **AI-nativeログ解析：Vertex AIでクローラー障害の根本原因自動特定**：クローラーエラーログをVertex AI Gemini 2.5に投入、過去1年分の障害履歴と照合し「HTTP 429 / セレクタ変更 / Cloudflare Bot Managerブロック」等の根本原因を自動分類。障害調査時間90分→8分、Shunへのステータス報告即時化。
- **Shunとの「KPI定義書 → dbtモデル」双方向同期ワークフロー**：Notion上のKPI定義書と、dbtモデルのschema.yml（description）を双方向に同期するNotion API + dbt GitHub Actionsを実装。定義書更新→自動でPR発行→レビュー→マージでモデル説明が最新化。KPI定義乖離バグが月2件→0件、Shunの分析着手前確認時間30分→3分。
- **Ana（06-リサーチ部）向け「業界公開データ自動収集棚」**：建設業関連の公開データ（e-Stat・厚労省一般職業紹介状況・国交省建設労働需給調査）を月次で自動取得しBigQueryに格納、Anaがリサーチ時にSQLクエリで即引ける「業界データ棚」を提供。Anaの一次データ収集時間120分→10分、市場分析レポートの一次データ根拠密度が2倍に。
