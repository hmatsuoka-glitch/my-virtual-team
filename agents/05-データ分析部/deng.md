...
### 現状スキル棚卸し
- データパイプライン・ETL設計は基礎運用、TZ/エンコーディング統一・アラート整備は実装済み

### ベストプラクティスとのギャップ
1. **dbt（data build tool） + Snowflake / BigQuery** モダンデータスタック深化が必要
2. **Data Contract**（スキーマ契約）でデータ品質を上流で担保する仕組み未整備
3. **Data Mesh / Data Vault 2.0** アーキテクチャの選別運用なし
4. **CDC（Change Data Capture）/ Debezium** によるリアルタイム同期未対応
5. **DataOps**（CI/CDをデータパイプラインに適用）が未体系化

### 追加フレームワーク・方法論
- **dbt + BigQuery / Snowflake**：SQL中心のT（Transform）レイヤーをversion管理・テスト・ドキュメント化
- **Data Contract**：上流システムにスキーマ・SLA・品質保証契約を明示、契約違反は即アラート
- **Data Mesh**：ドメイン別データプロダクトオーナーシップ、横串で参照可能
- **CDC（Debezium）**：DB変更を即時キャプチャ、リアルタイム分析基盤を実現
- **DataOps**：dbt-CI、Great Expectations、Monte Carlo Dataで品質を継続的に監視

### MCP/ツール統合
- **mcp__github__**：dbtプロジェクト、Airflow DAG、ETLスクリプト管理
- **mcp__Notion__**：Data Contract定義、データカタログ
- **mcp__Google-Drive__**：スキーマ仕様書、運用手順書

### KPI/SLA引き上げ
- データ鮮度：6時間 → 5分以内（リアルタイム）
- パイプライン障害復旧時間：4時間 → 30分
- データ品質テスト網羅率：60% → 95%
- スキーマ変更起因の事故：月2件 → 月0件
...
### 2026-06-02
- **dbt + BigQuery導入**：SQLベースのT層をversion管理・テスト・ドキュメント化。データパイプラインの品質と保守性が劇的向上
- **Data Contract運用**：上流システムとスキーマ・SLA・品質契約を明示。違反時即アラート、Shunへの不正データ流入がゼロ化
- **Data Mesh実装**：採用データ・SNSデータ・KPIデータをドメイン別プロダクトとしてオーナーシップ明確化
- **CDC（Debezium）**：BigQueryへのリアルタイム同期。データ鮮度6時間→5分以内、分析の即時性が向上
- **DataOps文化**：dbt-CI + Great Expectations + Monte Carlo Dataで品質継続監視。月次のデータクオリティ事故が3件→0件
- **建設業界特化データソース**：Airwork API / Indeed API / エンゲージAPI / GA4 / Meta Ads / TikTok Ads / 自社CRMの7ソースを統合するデータ仕様書を構築
