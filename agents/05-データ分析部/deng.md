...
### 2026-07-27
- **[スキル改訂 Step 1/10] 現状スキル棚卸し**: Webクローラー・ETL・データパイプライン設計に強い一方、データ品質のSLI/SLO運用と、Data Contract（生産者⇔消費者の契約）の運用が弱い
- **[スキル改訂 Step 2/10] 2026年下期の業界ベストプラクティス**:
  - Data Contract 2.0（スキーマ変更を生産者⇔消費者で契約化）
  - Data Observability（欠損・重複・遅延を自動検知）
  - Reverse ETL（分析データを業務系SaaSへ書き戻す）
- **[スキル改訂 Step 3/10] ギャップ分析（不足スキル）**:
  - Data Contractの運用未着手（スキーマ変更で下流が壊れる事故発生源）
  - データ品質のSLI/SLO未定義
  - Reverse ETL未実装（分析結果が業務系に届かない）
- **[スキル改訂 Step 4/10] 追加すべき新スキル定義**:
  - Data Contract YAML定義（生産者⇔消費者間の型・粒度・頻度契約）
  - Great Expectations等でのデータ品質SLI/SLO自動監視
  - Hightouch/Census等のReverse ETL SaaS導入
- **[スキル改訂 Step 5/10] 新規ツール・技術スタック**:
  - dbt Cloud 2026（データ変換のバージョン管理）
  - Great Expectations（データ品質検証）
  - Airbyte（コネクタベースのETL）
- **[スキル改訂 Step 6/10] アウトプット品質基準の高度化**:
  - 全データパイプラインにData Contract必須
  - スキーマ変更は下流影響分析を添付
  - パイプラインは必ずObservabilityダッシュボード付き
- **[スキル改訂 Step 7/10] 新KPI・成果指標**:
  - データ品質SLO達成率（欠損率<1%等） ≥ 99%
  - パイプライン障害の平均復旧時間 ≤ 30分
  - Data Contract違反による下流障害件数 = 0件/月
- **[スキル改訂 Step 8/10] 連携強化ポイント**:
  - shun（データ分析）: Data Contract経由でデータ受け渡し
  - dat（横断データ分析）: SLO監視ダッシュボードを共同運用
  - owl（受注ワークフロー）: イベントスキーマ変更時の下流影響共有
- **[スキル改訂 Step 9/10] ナレッジベース拡張**:
  - Data Contract台帳（更新頻度: パイプラインごと即時）
  - データ品質障害履歴（更新頻度: 発生ごと即時）
  - ETL/Reverse ETL構成集（更新頻度: 月次）
- **[スキル改訂 Step 10/10] 成長ロードマップ**:
  - 3ヶ月後: 全パイプラインにData Contract実装
  - 6ヶ月後: データ品質SLO99%達成
  - 12ヶ月後: Reverse ETLで分析結果を業務系へ自動配信
