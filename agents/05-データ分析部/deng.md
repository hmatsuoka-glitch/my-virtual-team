...
### 2026-09-21
- **2026年最新トレンド：dbt Cloud v1.9のContract-based Model適用**：2026年リリースのdbt Cloud v1.9で「モデル契約（column名・data_type・constraint）」がGA化され、下流のスキーマ破壊を事前検知できるようになった。Airwork・Indeed・GA4の主要ETLパイプラインに`contract: {enforced: true}`を全モデル適用し、カラム型変更・カラム削除を本番マージ前に自動ブロックする（理由：従来はShun/Akariが月初集計でエラーに気づいてから発覚するリードタイム3日を、CI時点で0日に短縮できる）
- **2026年最新スキル：BigQuery ML×Vertex AI連携による応募者離脱予測モデル運用**：BigQuery MLで採用LP到達→応募完了までの離脱段階を予測するモデル（XGBoost）を訓練し、Vertex AI Pipelinesで週次再訓練を自動化する。特徴量は「デバイス・時間帯・流入媒体・LP滞在秒・スクロール深度」の5次元。Shunが分析側で予測結果を受け取り、離脱リスクの高いセグメントに対してAkariの月次レポートで「事前警告アラート」を出す設計（Shunと合同運用）
- **2026年最新トレンド：Iceberg on BigQueryへの段階移行検討**：2026年下期からBigQuery（Analytics Hub含む）がApache Iceberg形式のテーブルをネイティブサポートし、複数データ基盤（BigQuery・Snowflake・Databricks）間で同一テーブルを共有できるようになった。7社案件のうちクライアント側がSnowflake導入している桝本レッカー系のBIチームとのデータ共有をIceberg経由で試験（理由：CSVエクスポート／再インポートの往復工数を削減し、ガバナンス境界を保持したまま外部共有できる）
- **失敗パターン：Contract-based Modelを一気に全モデル適用し、既存プロジェクトのCIが5時間ブロックされる** → 回避策：契約適用は「新規モデル→中核集計モデル→周辺モデル」の3段階リリースにし、各段階でCI実行時間を計測してから次段階に進む。段階ごとにShun側の依存クエリを事前ヒアリングして、契約違反が予想されるモデルは事前に修正PRを合わせて出す（理由：CI長時間ブロックは開発速度を止めるだけでなく、緊急パイプライン修正が滞留してSLA違反に直結する）
- **失敗パターン：BigQuery MLのモデル再訓練で「特徴量スキーマの静かな変更」（例：デバイスカテゴリの区分値追加）を検知せず、予測精度が2週間かけてゆっくり劣化する** → 回避策：Vertex AI Model Monitoringの「特徴量ドリフト検知」を有効化し、区分値の分布差がKL divergence 0.15を超えたらSlackに自動通知する。9/13の障害通知テンプレと同じチャネルに流し、Shun/Akariが「なぜ最近の予測が外れているか」を推測せずに済む状態を保つ
- **品質チェックポイント：Icebergテーブルへの移行時「タイムトラベル互換性」の事前検証**：BigQueryのTIME TRAVEL（過去7日データ参照）がIcebergテーブルでは仕様が異なる（Iceberg側はスナップショットベース）。移行前にShun/Akariが過去データを参照するクエリを洗い出し、TIME TRAVEL依存クエリはIceberg移行対象外にする。データ復旧経路（9/13のPII削除経路一覧と同じ思想）を事前に確認しないと復旧不能な事故が起きる
- **品質チェックポイント：dbt Contract定義の「semantic layer」整合性**：モデル契約でカラム定義を強制するだけでは不十分で、`meta.description`（意味論的定義）が実装と乖離すると下流のBIツールで誤解が起きる（例：`applications`カラムの説明が「応募数」だが実装は「連絡がついた応募数」）。契約適用時に必ずShun/Akariに命名レビュー依頼を出し、意味の定義を1行で合意してからマージする
- **効率化テクニック：Cursor+dbt-power-user拡張によるdbt開発の高速化**：Cursor IDEで`dbt-power-user`拡張を有効化すると、SQLファイル上でモデルの参照グラフ・カラム系譜・実行結果プレビューが右ペインに表示され、モデル修正時の影響範囲確認が10分→30秒に短縮される。全開発ワークフロー標準ツールとして統一（Shun側にもdbtモデル修正時は本拡張導入を推奨）
- **Shun連携の小ヒント：BigQuery MLモデルの「特徴量エンジニアリング設計」共同レビュー**：Deng側でモデル訓練の技術実装（BigQuery ML構文・パラメータチューニング）を担当し、Shun側で「どの特徴量が採用KPIとして意味を持つか」のドメイン知識レビューを担当する分業を明文化。月1回30分の共同レビュー会（毎月第2水曜）を固定し、モデル改良の意思決定を「技術的正解」ではなく「業務的正解」で下す
