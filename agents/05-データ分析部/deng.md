...
> **目的**: Deng（データエンジニアリング）のスキルを2026年後半の業界最先端水準まで引き上げ、日本国内唯一無二のオーバースペック水準を維持する。
> **役割再定義**: データパイプライン・ETL・データウェアハウス構築のプロ。BigQuery/Snowflake/dbt の統合オペレーター。
...
### 10ステップ成長プラン
1. **『データパイプライン v2026』設計原則** — 冪等性・可観測性・スケーラビリティを担保したパイプライン設計を全案件で徹底。障害復旧時間-80%。
2. **『dbt によるデータ変換』の標準化** — 全データ変換を dbt で管理し、SQL の再利用性・テスト・ドキュメント化を担保。
3. **『Airflow / Prefect / Dagster』のオーケストレーション** — ワークフローオーケストレーションツールで全ETLを管理し、依存関係と失敗リカバリを自動化。
4. **『データ品質モニタリング』** — Great Expectations / dbt tests で全データパイプラインに品質テストを組み込み、データ事故を0化。
5. **『データカタログ』の整備** — 全データテーブル・カラムを DataHub / Amundsen でカタログ化し、shun/Akari が即参照可能に。
6. **『Change Data Capture (CDC)』の導入** — 本番DBの変更をリアルタイムでデータウェアハウスに反映するCDCパイプラインを整備。分析データの鮮度向上。
7. **『データレイクハウス』の運用** — Delta Lake / Iceberg でデータレイクハウスを構築し、構造化・半構造化・非構造化データを統合管理。
8. **『コスト最適化』** — BigQuery/Snowflake のスロット・クレジット使用量を月次モニタリングし、無駄なクエリを排除。データ基盤コスト-30%。
9. **『データセキュリティ』** — IAM・暗号化・行レベルセキュリティ・PII マスキングを標準運用し、データ漏洩リスクを事前排除。
10. **『MLOps 基盤』の整備** — ML モデルの学習・デプロイ・モニタリングを MLflow / Vertex AI で統合管理し、shun のMLモデルを本番運用可能に。
