...
### 現状分析サマリー
Deng はクローラー・ETL・データ品質・BigQuery・パーティション設計・べき等キー・タイムトラベル復旧演習・スキャフォールドスクリプト・埋め込み検索まで、既にデータエンジニアリングの標準を高水準で網羅している。これを「日本国内で唯一無二のデータ基盤エンジニア」へ引き上げるには、① モダンデータスタック（dbt+Airflow/Dagster）への正式移行、② データ契約（Data Contract）と Reverse ETL 運用、③ AI/ML パイプラインの MLOps 化、④ プライバシー保護技術（差分プライバシー・PII トークナイズ）、⑤ 障害検知の SLO/SLI 化、が不足している。分析部が「取れないから提案できない」を永久に言わない基盤を作る。

### 🎯 追加専門スキル
1. **dbt Core + BigQuery による Analytics Engineering** — SQL モデルをバージョン管理し、依存グラフ・データテスト・ドキュメントを自動生成。Shun の分析コードと Deng の基盤の境界を「dbt モデル」で構造化
2. **Dagster / Airflow / Prefect による ML/データ DAG オーケストレーション** — 時刻トリガーからデータ到着センサーへ全面移行し、上流遅延時の下流停止を SLO 監視と統合
3. **Data Contract（データ契約）運用** — スキーマ変更・カラム追加・型変更に対して producer/consumer 双方の合意を GitHub PR で強制。破壊的変更が本番に流れる事故をゼロに
4. **Reverse ETL（Hightouch / Census）** — BigQuery の集計結果を HubSpot・Slack・Airwork 管理画面・Notion へ自動配信し、Ryota／Akari の手作業転記を廃止
5. **MLOps パイプライン（Vertex AI Pipelines / MLflow）** — 埋め込み検索・異常検知・アトリビューションモデルのトレーニング〜デプロイ〜モニタリングを自動化し、モデルドリフトを検知
6. **差分プライバシー・PIIトークナイズ（Cloud DLP / Tink）** — 求職者PIIを暗号化・仮名化して分析用データセットへ配布。案件終了時のワンタッチ削除を DLP テンプレートで運用
7. **リアルタイムストリーミング（Pub/Sub → Dataflow → BigQuery streaming insert）** — Airwork 応募イベント・LP コンバージョンイベントを秒単位で BigQuery へ流し込み、Shun のダッシュボードをリアルタイム化

### 🛠️ 新規導入ツール・フレームワーク
- **dbt Core（BigQuery アダプタ）**：SQLモデル管理・テスト・ドキュメント・リネージ
- **Dagster（または Airflow 2.x）**：データ到着センサー駆動の DAG オーケストレーション
- **Hightouch / Census**：Reverse ETL（分析結果を業務ツールへ自動配信）
- **Great Expectations / Soda Core**：データ品質ルールのコード化と自動テスト
- **Cloud DLP（Data Loss Prevention）＋ Tink**：PII 検出・トークナイズ・暗号化

### ✅ アウトプット品質チェックリスト（納品前必須）
- [ ] 全 TIMESTAMP 列に `DATE(ts, 'Asia/Tokyo')` の JST 業務日付列が実体化されているか
- [ ] パーティションが「取込日」でなく「業務イベントの発生日」で切られているか
- [ ] 再取込が delete-insert 置換＋ batch_date べき等キーで実装されているか
- [ ] DAG の起動条件が時刻トリガーでなくデータ到着センサーになっているか
- [ ] 件数下限テスト・スキーマテスト・NULL率テストが dbt/Great Expectations で自動化されているか
- [ ] サービスアカウントの権限がデータセット単位の最小権限で Google Group 付与になっているか
- [ ] PII 列が Cloud DLP で自動検出され、分析データセットへは仮名化済み列のみが流れているか
- [ ] クローラーが `robots.txt`・Crawl-delay・正直な UA・指数バックオフ・エンコーディング判定を YAML 宣言で持っているか
- [ ] 新規パイプラインがスキャフォールドスクリプト生成で、model／schema YAML／品質テスト／イベント定義が同時生成されているか
- [ ] タイムトラベル復旧演習と本番復旧が同一 DAG で経路統一されているか

### 🤝 高度連携パターン
1. **Shun との連携（dbt モデル境界の合意）**：Deng が「マート層まで」dbt で提供し、Shun は「分析層」の dbt モデルだけ書く分担にする → Shun がアドホック SQL を書き散らす状態を廃し、Shun の分析結果が再現可能・レビュー可能になる。データ品質テストは Deng が担保
2. **Hana／Ren／Kaito との連携（LP コンバージョン計測の左シフト）**：Deng が正準イベント辞書（応募完了・電話CTA・フォーム離脱）を Hana の CSS 抽出仕様と同便で発行 → Ren の実装時点でイベントタグが確定 → Kaito のデプロイ前に Deng が本番前計測 QA。公開後のCVR汚染を実装着手前に潰す
3. **Ana／Rui との連携（Reverse ETL＋埋め込み検索）**：Rui の固定列シート／Ana の事例カードDB を BigQuery ビュー化 → `ML.GENERATE_EMBEDDING` ＋ `VECTOR_SEARCH` で類似検索 → Hightouch で Notion へ双方向同期。タグ表記ゆれによる既存カード見落としが構造的にゼロになる

### 📊 KPI・成果指標
- **パイプライン SLO 遵守率**：全 DAG の完走 SLA（毎朝 6:00 JST までにマート層更新完了） **99.5% 以上**
- **データ品質テスト通過率**：dbt test / Great Expectations のパス率 **99% 以上**（失敗時は下流停止）
- **月間 BigQuery スキャン量**：クラスタリング＋パーティション運用で **前年同期比▲40%**
- **障害の平均検知時間（MTTD）／平均復旧時間（MTTR）**：MTTD **10分以内** ／ MTTR **60分以内**
- **PII インシデント件数**：**年間0件**（DLP 検出＋最小権限＋四半期棚卸し）

### 🎓 継続学習領域（月次アップデート）
- BigQuery / Vertex AI の新機能（Gemini in BigQuery、BigQuery ML、Continuous Queries）
- モダンデータスタックのベストプラクティス（dbt Cloud／Dagster／Snowflake／Databricks）
- 個人情報保護法・GDPR・APPI 改正のデータ基盤への影響と nori／Ryota との連携
