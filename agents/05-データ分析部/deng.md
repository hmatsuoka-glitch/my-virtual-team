...
### 2026-09-04
- **dbt Semantic Layer（メトリクス層）へ「応募数・CVR・単価」の3大KPIを一元集約し、Shun/Akariが各自のBI・ノートブックで再定義するズレを構造排除**：conformed dimension（2026-07-11参照）と正準イベント辞書（2026-08-13参照）が揃った今、次に潰すべきはKPI式そのものの多重定義。dbt semantic layerに`applications_uniq`・`cvr_by_media`・`cost_per_application`を`kpi_def_version`タグ付きで定義し、Shunのノートブック・AkariのLooker Studio・RyotaのCSVエクスポートは全て`METRIC()`参照経由に寄せる。Shun/AkariのSlack質問「これは延べかユニークか」（2026-08-16参照）はカラムdescriptionだけでは表記ゆれが残るが、semantic layerからの機械参照ならそもそも質問が発生しない。月初KPI突合MTG（2026-06-16参照）の当日完結もさらに前倒しできる
- **Looker Studio Pro（組織管理・チームワークスペース）で「個人所有ダッシュボード」を棚卸し対象に組み込む**：BigQueryのサービスアカウント属人化（2026-06-17参照）と最小権限四半期棚卸し（2026-09-02参照）はデータ側で固めたが、Looker Studio側の個人所有レポートは退職・委託終了で閲覧不能化するリスクが残ったまま。7社分の月次・週次レポートをLooker Studio Proのチームワークスペースへ物理移管し、閲覧権限はGoogle Group経由に統一、四半期棚卸しのチェック項目に「個人所有のまま残っているレポートが無いか」を追加する。Ryotaがクライアント送付経路として使っているダッシュボードが送付直前に開けなくなる事故を予防できる
- **GA4 BigQuery Export のStreaming Export（分単位）を"速報タイル専用"の別データセットで受け、日次確定Exportとの層分離を物理化する**：GA4のStreaming Exportが2026年に一般提供化し、`events_intraday_`より粒度の細かい分単位反映が可能になる。ただし`events_intraday_`と確定`events_*`の72時間ラグ問題（2026-06-17参照）は消えないため、Shunのリアルタイム施策検知（Shun 2026-08-27参照）向けに`raw_streaming_ga4`データセットを新設し、月次・確定KPIは従来通り確定Exportのみ参照、Shun向け速報ビューはstreaming側のみを見る二系統に物理分離する。「速報・確定前」明記ルール（2026-06-26参照）はタイル命名・データセット名の両方で二重に担保する
- **Elementary（dbt統合オブザーバビリティ）のベースライン自動学習検知を、自作の閾値ゲートと"二段構成"で運用する**：データオブザーバビリティSaaS導入（2026-08-03参照）を「学習型に全面移行」ではなく、ビジネス上絶対に外せないルール（NULL率10%超・スキャン量週次+50%超・件数変化率±30%・PII列露出）は自作の固定閾値でCRITICAL発報を維持し、Elementaryは「未知の分布ドリフト・未定義の列追加・鮮度パターン変化」の広域検知に配置する。学習型は初期の狼少年化（2026-05-24参照）と発火実績棚卸し（2026-07-03参照）で潰した課題を再燃させるリスクがあるため、初回3ヶ月は監査モード（通知せず`elementary_anomalies`テーブルに記録のみ）で閾値と誤検知率を校正し、Shun/Akariに通知が届く前に運用側で妥当性を判定する
- **BigQueryのColumn-level Security＋Data Catalog Policy TagでPII列を"実行時マスキング"に寄せ、Ryota/Akariの目視判断依存を構造排除**：PII保持期限のpartition expiration（2026-08-05参照）とマルチテナントRLS（2026-06-24参照）と最小権限（2026-09-02参照）に加えて、応募者氏名・電話・メール・住所の列にPolicy Tag（`pii_high`）を付け、閲覧権限のないアカウントには自動でハッシュ値/NULLを返す設定にする。Ryota/Akariが送付前に「この列は本当にクライアント宛に出していい列か」を人間の目で拾う運用（2026-08-16参照）は必ずどこかで漏れるため、閲覧側の権限が無い限り技術的に見えない状態を既定にする。マスキング前後の突合を`pre_publish_check`（2026-06-16参照）へ追加し、送付用ビューが誤ってPII列を含んだ場合はexit code 1で公開停止する
