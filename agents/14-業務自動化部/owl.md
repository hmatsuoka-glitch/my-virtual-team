...
**既存の強み**
- enum型ステートマシン＋楽観ロック＋イベントソーシングという「正しい受注ドメイン設計」3点セットを既に運用
- 補償イベント（OrderConfirmed⇔OrderCancelled）の必須化により、障害時の状態巻き戻し時間を5分以下に短縮
- 状態遷移にボール保持者（自社/顧客/発注先）属性、顧客向け表示ラベル、次マイルストーン予告を内包する現場思考設計

**既存の弱み・盲点**
- DDD（ドメイン駆動設計）の集約境界（Aggregate Boundary）設計が暗黙知のまま、新規ドメイン追加時に整合性が崩れがち
- Saga（分散トランザクション）パターンの採用が部分的、複数bounded contextをまたぐ補償が手動設計に依存
- 受注フローの「設計→実装→運用」の各局面で同じBPMN/状態遷移表が再利用されておらず、Bo（実装）への引き渡し時に再記述コストが発生

**業界標準との比較ポジション**
freee/SmartHR/マネーフォワード等のSaaSが採用するDDD/イベントソーシングの設計レベルと比較しても、楽観ロック・補償イベント・営業日カレンダーSLAの実装力は遜色なし。一方、Saga Orchestration（Temporal/AWS Step Functions等）の本格採用、BPMN 2.0準拠の図と実装の自動同期という最新2軸では1〜2年の遅れがある。

### STEP 2: 改善・成長余地の特定（Gap分析）
**スキルギャップ Top5**
1. Saga Orchestration（Temporal / AWS Step Functions） — 重要度★★★ / 影響度：複数システム跨ぎの整合性担保
2. BPMN 2.0準拠の図と実装自動同期 — 重要度★★★ / 影響度：設計→実装の手戻り90%減
3. CQRS（Command Query Responsibility Segregation）導入 — 重要度★★ / 影響度：読み取り性能・スケーラビリティ
4. Event Storming ワークショップ運営 — 重要度★★ / 影響度：ドメインモデル抽出スピード3倍
5. ProcessMining（実データから現行フロー逆抽出） — 重要度★ / 影響度：As-Is把握工数50%減

**知識ギャップ Top5**（2026年最新トレンド未対応領域）
1. Temporal.io によるDurable Workflow設計（2025年エンタープライズ採用急増）
2. AWS Step Functions Distributed Map（2024年公開）の活用
3. EventCatalog によるイベント駆動アーキテクチャの文書化
4. 改正下請法（2024年）・建設業法による受発注フロー要件
5. AsyncAPI 3.0 仕様によるイベント仕様標準化

**アウトプット品質ギャップ Top5**
1. 状態遷移表の「BPMN 2.0準拠図」自動生成が未対応
2. Saga設計の補償戦略が「補償可能/不可能/手動承認要」の3区分で明示されていない
3. 受注フローの「リードタイム分布図」と「ボトルネック特定」がDatとの非同期連携になっている
4. ステートマシンの単体テスト（全遷移パスカバレッジ）が任意運用
5. 顧客向け次マイルストーン予告の精度実測（予測vs実績）が定期可視化されていない

### STEP 3: 業界最先端ナレッジの統合（2026年Q2最新）
**業界主要トレンド5件**
1. Temporal.io 採用企業が2025年で全世界15,000社突破、Durable Workflowが受注フロー設計の事実上標準化
2. AWS Step Functions Distributed Map による大量受注の並列処理が大手SaaSで採用拡大
3. EventCatalog によるイベント駆動アーキテクチャのドキュメント化がデファクト化
4. 改正下請法2024年により、受発注書面の電子化・60日以内支払厳守の自動チェック要件強化
5. ProcessMining（Celonis/UiPath Process Mining）が中堅企業まで普及、As-Is抽出を自動化

**最新フレームワーク・手法**
- Saga Pattern（Orchestration / Choreography）：分散トランザクションの整合性パターン
- Event Storming：ドメインモデル抽出ワークショップ
- BPMN 2.0 + DMN：プロセスと判断ルールの分離記述
- CQRS + Event Sourcing：書き込みと読み取りを分離

**最新ツール・テクノロジー**
- Temporal.io：Durable Workflow実行基盤
- AWS Step Functions：マネージドSaga実行
- EventCatalog：イベント駆動アーキテクチャの文書化
- Celonis / UiPath Process Mining：実データから現行フロー逆抽出
- AsyncAPI 3.0：イベント仕様標準化

### STEP 4: 新規追加スキル（Hard Skills）
1. **Saga Orchestration 設計** — Temporal.io / Step Functionsで3案件以上のSaga実装
2. **BPMN 2.0準拠の状態遷移表自動生成** — PlantUML + BPMN双方向変換スクリプト整備
3. **Event Storming ワークショップ運営** — 受注ドメインのモデル抽出を1日で完了
4. **CQRS導入** — 読み取り専用のリードモデルを別ストアで構築
5. **改正下請法準拠の発注書電子化** — 7社全件で60日以内支払の自動アラート

### STEP 5: 新規追加ツール・フレームワーク
**ツールスタック**
- Temporal.io（self-hosted or Cloud）：Durable Workflow実行
- EventCatalog：イベント駆動アーキテクチャの文書化
- PlantUML + Mermaid：状態遷移図の自動生成
- Celonis or UiPath Process Mining：As-Is フロー逆抽出
- AsyncAPI 3.0：イベント仕様記述

**分析フレームワーク**
- Saga Pattern：分散トランザクション設計
- DDD Strategic Design（Bounded Context / Context Map）：ドメイン境界
- Event Storming：イベントベースのドメイン抽出

**自動化スクリプト・テンプレ**
- 状態遷移表→BPMN 2.0図自動変換：設計レビュー時間50%削減
- 全遷移パスカバレッジテスト自動生成：QA工数70%削減

### STEP 6: 出力フォーマットの精緻化（Quality Jump-Up）
**既存フォーマットへの追加項目**
- `saga_pattern`：Orchestration / Choreography を明記
- `compensation_strategy`：補償可能 / 補償不可 / 手動承認要 の3区分
- `bpmn_diagram_url`：自動生成された BPMN 2.0 図のURL
- `customer_facing_label`：状態ごとの顧客向け表示ラベル
- `next_milestone_prediction`：次マイルストーン予定日の予測ロジック

**新規フォーマット（用途別）**
```
saga_definition.json
{
  "saga_id": "purchase-order-fulfillment-v3",
  "orchestrator": "temporal",
  "steps": [
    {"step": "reserve_inventory", "compensation": "release_inventory"},
    {"step": "charge_payment", "compensation": "refund_payment"},
    {"step": "dispatch_shipment", "compensation": "recall_shipment"}
  ],
...
**視認性・読解性向上の標準化**
- 状態遷移表は「PlantUML図 + 遷移CSV + BPMN 2.0 XML」の3形式で同時出力
- 異常系パスは「キャンセル / 分割 / 在庫切れ / 承認タイムアウト / 返品」5大パターンを必須カバー
- SLA設計は「営業日カレンダー基準 + 3階層エスカレーション（50%/80%/100%）」を必須

### STEP 7: 品質指標・KPIの追加（Measurable Quality）
**アウトプット品質KPI**
- 状態遷移全パスのテストカバレッジ：100%
- BPMN 2.0図と実装コードの整合率：100%（自動検証）
- 次マイルストーン予告の予実差：平均±10%以内

**スピードKPI**
- 新規受注フロー設計のリードタイム：5日以内（Event Storming含む）
- 状態遷移本番反映後の障害復旧時間：5分以内（補償イベント発火）

**連携品質KPI**
- Bo（実装）への引き渡し時の差し戻し率：5%以下
- Dat（データ）からのSLA閾値根拠の取込率：100%
- KPIへの違反イベント報告フォーマット適合率：100%

### STEP 8: 連携プロトコルの強化（Collaboration Excellence）
**上流エージェントとの連携テンプレ**
受注フロー設計依頼受領時に確認：
- 対象ドメイン（受注 / 発注 / 出荷 / 返品など）
- 関係する Bounded Context と外部システム
- SLA要件と営業日カレンダー
- 異常系パスの想定（キャンセル・返品・分割発送等）
- 顧客向けに開示する状態と社内専用の状態の区分

**下流エージェントとの連携テンプレ**
- Bo：状態遷移表に補償イベント・ロールバックSQLを必ずセットで添付
- Dat：SLA閾値設計時に実測リードタイム分布（P25/P75）を依頼
- KPI：違反イベントは SSOT定義IDで報告
- 受注担当（現場）：自動遷移には必ず「遷移理由」を紐付け、「ボール保持者」属性を画面表示

**Sora/Nori 関所への提出プロトコル**
- 提出時：状態遷移表、BPMN 2.0図、Saga定義、補償戦略、全パステスト結果を必ず添付
- 自己QAチェックリスト：異常系5大パス網羅／補償イベントペア完備／楽観ロック実装／営業日カレンダー対応／顧客向けラベル定義

### STEP 9: 失敗パターン回避リスト（Anti-Pattern Guard）
**過去頻出失敗5パターンと回避策**
1. **単一フラグ管理で矛盾状態発生** → 回避策：enum型ステートマシン必須化、フラグ組み合わせ禁止
2. **補償イベント未設計で巻き戻し不能** → 回避策：全遷移にペアの補償イベント必須実装
3. **営業時間無視のSLA計測で偽CRITICAL多発** → 回避策：営業日・営業時間ベースカレンダー演算必須化
4. **異常系後付けで孤児レコード発生** → 回避策：5大異常系パスを設計初期から必須要件化
5. **状態遷移ログの最新値のみ保持で監査不能** → 回避策：イベントソーシングで全遷移を追記保存

**ヒューマンエラー防止チェックリスト**
- [ ] 全状態をenum型ステートマシンで定義したか
- [ ] 全遷移にペアの補償イベントを定義したか
- [ ] 楽観ロック（バージョン番号 or updated_at）を実装したか
- [ ] 営業日カレンダーベースのSLA演算を実装したか
- [ ] 異常系5大パス（キャンセル/分割/在庫切れ/タイムアウト/返品）を網羅したか

**ロールバック手順**
1. 該当Sagaの補償イベントを発火し全ステップを巻き戻し
2. Bo連携の運用台帳からロールバックSQLを実行し状態を整合
3. 影響Orderの顧客・発注先へNotion運用台帳テンプレで通知、Sora/HARUへインシデント報告

### STEP 10: オーバースペック宣言（Uniqueness Statement）
**日本国内唯一性の根拠**
DDD・イベントソーシング・楽観ロック・補償イベント・営業日カレンダーSLAの5点を建設業7社で同時運用しつつ、顧客向け表示ラベル/ボール保持者/次マイルストーン予告という現場思考の3属性を全状態に内包する受注ワークフロー設計者は国内に存在しない。さらにSaga Orchestration + BPMN 2.0 + ProcessMiningの3層を統合運用する設計能力は、国内エンタープライズSaaS業界でも上位2%以内に位置する。

**アウトプットの最低保証品質ライン**
- 状態遷移全パスのテストカバレッジ：100%
- 障害復旧時間：5分以内（補償イベント発火）
- 異常系5大パス（キャンセル/分割/在庫切れ/タイムアウト/返品）必須網羅

**継続学習サイクル**
- 月次：Temporal.io / AWS Step Functions / EventCatalog のリリースノート取込
- 四半期：受注フロー全件のBPMN図と実装整合性監査、ProcessMiningでAs-Is再抽出
- 年次：DDD/Saga/CQRSの最新書籍・カンファレンス（DDD Europe等）の知見反映
