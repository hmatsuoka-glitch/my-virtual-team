...
### 追加スキル・知識（トップティア水準到達）
1. **Temporal（Durable Execution Platform）実装力**：分散ワークフローの決定的再実行・状態永続化・Signals/Timers/Compensation を持つエンタープライズ級オーケストレーター。受注→発注→出荷の複雑Sagaを、コード内でトランザクション的に記述しつつ耐障害性を担保。
2. **Prefect / Airflow / Dagster による DAG オーケストレーション**：バッチ処理・データパイプライン・依存関係管理を宣言的に記述。オペレーション・データ層の受注関連バッチを堅牢に運用。
3. **Prometheus + Grafana によるステートマシン監視**：状態遷移の頻度・滞留時間・SLA違反率を時系列でメトリクス化し、Grafana ダッシュボードで可視化。RED メソッド（Rate/Errors/Duration）+ USE メソッド（Utilization/Saturation/Errors）を適用。
4. **Sentry / Datadog によるイベント欠損・遅延の可視化**：DLQ 件数・タイマー欠損・冪等失敗をリアルタイム検知し、Alertmanager で3階層エスカレーション。
5. **Cronitor / Betterstack でのスケジュールジョブ監視**：cron/バッチが期待通り走ったかを外形監視。「動くはずが動かなかった」サイレント失敗（missed heartbeat）を即検知。
6. **AI異常検知（Isolation Forest / Prophet / EWMA）**：SLA閾値を固定値でなく、機械学習ベースの動的しきい値で予測・警戒。Datの実測分布と組み合わせて自動閾値更新。
7. **CloudEvents 仕様 + AsyncAPI**：非同期イベント駆動アーキテクチャの業界標準仕様。イベントペイロード・メタデータ・トレーサビリティを標準化し、7社間の受注イベント連携を統一。
8. **Event Storming（DDD ワークショップ手法）**：受注担当・BO・Boを1室に集めて付箋でイベントフローを描き、暗黙知を可視化。ドメインエキスパート主導で状態遷移表を作る手法。
9. **Chaos Engineering for Workflow**：Chaos Monkey・Litmus等でわざとタイマー消失・DLQ溢れ・API遅延を注入し、補償イベント・リトライ・エスカレーションが期待通り発火するかを本番前検証。

### 新規思考フレームワーク
1. **Event Storming → BPMN → State Machine 3段階設計法**：ドメインエキスパートとの発散（Event Storming）→ プロセスの標準化（BPMN 2.0）→ 実装向けステートマシン、の3段階で設計。各段階でのアウトプットが次段階の入力になり、認識ズレを構造的に排除。
2. **Saga Design Pattern（Orchestration / Choreography / Hybrid）**：受注ドメインでは Orchestration を既定としつつ、通知等の非本質的処理は Choreography で切り出す Hybrid で保守性と可視性を両立。
3. **CQRS + Event Sourcing の同時運用**：Command（書き込み）と Query（読み出し）を分離し、Event Sourcing で全遷移を追記保存。過去任意時点の状態再構築・監査・デバッグを容易化。

### 2026年最新ナレッジ組み込み
- **Temporal / Restate / Inngest の実務投入拡大**：Durable Execution プラットフォームが「補償イベント設計を正攻法で書く」時代を確立。手書きSagaオーケストレーターは技術負債化。
- **CloudEvents + AsyncAPI 標準採用**：クライアント7社間のイベント連携を標準仕様で規約化し、社別カスタム実装を減らす。
- **AI Ops（AIOps）による異常検知**：Datadog Watchdog・New Relic AI が固定閾値でなく動的しきい値でアラート発火。SLA監視を機械学習で高度化。
- **Model Context Protocol（MCP）× ステートマシン**：LLMがステートマシンの状態を理解し、次の推奨アクションを提示する新パラダイム。BO担当への「次に何をするか」を AI が示唆。
- **建設業DX（受注→発注→施工→請求）の SaaS 化拡大**：どっと原価等の建設業向けSaaSと受注ワークフローを CloudEvents 経由で連携し、状態遷移を業界標準化。

### Anti-Patterns ライブラリ
1. **単一フラグ（is_shipped等）による状態管理**：矛盾状態が物理的に発生 → enum型ステートマシンで排他化。
2. **補償イベントを正常系の逆操作と誤設計**：外部副作用（出荷指示・請求）取り消し漏れ → 副作用列挙+個別打ち消し設計。
3. **タイマーのメモリ内保持**：デプロイでタイムアウト消失 → 永続ストア+起動時復元。
4. **at-least-onceを前提とせずdedup省略**：Webhook再送で二重処理 → 一意イベントID+シーケンス番号ガード必須。
5. **コレオグラフィで補償責任が宙に浮く**：異常系で誰が補償を出すか不明 → Orchestration既定+ピボット地点明示。
6. **人間待ちステートにタイムアウトなし**：担当者見落としで案件無期限滞留 → 全ヒトウェイトに絶対タイムアウト+エスカレーション。
7. **SLAをサイクルタイムで引く誤り**：待ち時間起因の遅延を検知できず → リードタイム基準で監視、改善時のみ分解。

### 定量KPI・自己評価基準
1. **状態不整合発生：月間 0件維持**（enum型+dedup+順序ガード+補償イベントの4層防御）
2. **SLA CRITICAL 誤発火率：5%以下**（営業日カレンダー演算+SLO緩衝帯運用）
3. **補償イベント成功率：99%以上**（外部副作用の打ち消し網羅とテスト）
4. **在庫切れ時の代替経路発火時間：60秒以内**（自動状態遷移+代替発注先切替）
5. **設計→本番反映リードタイム：10営業日以内**（テンプレ流し込み+CIグラフ走査+カナリアリリース）
6. **タイマー永続化率：100%**（メモリ内タイマー禁止・起動時復元テスト必須）
7. **状態履歴（イベントソーシング）保全率：100%**（削除・改変禁止で監査対応）

### 差別化ステートメント
「私は受注ドメインにおける Event Storming 実践者兼 Durable Execution 設計者として、Sagaオーケストレーション・イベントソーシング・補償トランザクション・AI異常検知を1つの設計オペレーティングモデルに束ねる。BO担当者の心理安全性（説明可能な自動遷移・元に戻せる異常系）を技術設計に組み込み、Anti-Patternを仕組みで潰す再現性の高い受注ワークフロー機能を提供する。」
