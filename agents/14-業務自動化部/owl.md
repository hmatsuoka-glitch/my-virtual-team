...
### 2026-08-30
- **状態機械の設計を "Statechart" (階層状態機械) 化して直交属性の組合せ爆発を構造的に回避**：主ステートに直交する関心事（要確認・優先度・督促回数）を並行リージョン（AND状態）で分離する設計テンプレを標準化し、XState/SCXML でコード生成。テストケース数が組合せ乗算→加算に減り、状態網羅テストの時間が2日→2時間に短縮。
- **Claude 5 系による "状態遷移設計レビュー" を実装前に自動実施**：状態遷移表とドメイン記述を Claude に渡し、①デッドエンド検出②権限マトリクス欠落③補償イベント未定義④SLA未設定の4観点で自動検出させる。設計レビュー時間が半日→15分に短縮、本番移行後の設計欠陥起因障害をゼロ化。
- **建設業クライアント向け "変更請書ワークフロー" を標準受注フローに常設**：追加工事・数量変更・工期延長を「変更遷移」として正常系に組込み、口頭合意→システム記録の3タップ入力を現場アプリで実現。変更請書の書面化率が30%→95%に向上し、後日の請求トラブルを構造的にゼロ化。
- **プロセスマイニング SaaS（Celonis/UiPath Process Mining）で "設計外経路" を四半期分析**：実運用ログを解析し、頻度上位の設計外経路を自動抽出→ Dat と協働で頻度検証→正常系昇格の意思決定サイクルを四半期で回す。可視化外に潜む地下運用の発見が四半期4件→10件に増え、状態機械の実運用適合度が上がる。
- **AdminOverride イベントの "レビューダッシュボード" を常設**：管理者の強制遷移を全件 Notion に自動記録し、月次で「なぜ AdminOverride が必要になったか」を分析。設計欠陥の早期発見が3ヶ月→1ヶ月に短縮、AdminOverride 件数を月30件→月5件に削減。
- **異常系遷移（キャンセル・返品・分割発送）の"顧客通知テンプレ"を CS と共同で個別化**：定型謝罪文でなく、CS のヘルススコア・過去クレーム履歴に応じて Claude が個別化した通知文を自動生成→ CS 最終承認で送信。異常系起因の解約率を -50% に抑える。
- **状態機械の可視化を Mermaid.js で自動生成し、Notion に埋込**：状態遷移表 JSON から Mermaid の stateDiagram を自動生成し、現場担当・経営者向けの説明資料に貼り付けられる形にする。設計説明の資料作成時間がゼロになり、レビュー会議の理解速度も倍増。
- **SLA違反の"経営インパクト金額換算"を Kpi と協働で四半期モニタ**：k4_sla_violation_count を件数のまま報告せず、Kpi 側の lookup で「◯円の資金コスト」に換算して経営ダッシュボードに載せる。SLA 改善の投資判断が数値ベースで下せるようになり、SLA 違反削減の意思決定が四半期→月次に加速。
