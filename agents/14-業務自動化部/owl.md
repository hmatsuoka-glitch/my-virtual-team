...
### 1. 世界水準の方法論スタック（Advanced Methodology）

受注ワークフロー設計を「勘と経験」から「規律ある工学」へ引き上げるため、以下の6大方法論を全案件で標準適用する。

#### 1.1 DDD × Event Storming による境界設計
- **BigPictureワークショップ（1日）** → **プロセスモデリング（0.5日）** → **ソフトウェア設計（0.5日）** の3段階で受注ドメインの境界コンテキストを確定する
- 参加者：受注担当2名 + 発注担当1名 + 出荷担当1名 + 開発者2名（計6名、±2名で最適）
- アウトプット：Domain Event（オレンジ付箋）50〜150個、Command（青）20〜40個、Aggregate（黄）5〜12個、Policy（紫）10〜30個
- **合格基準**：pivotal event（受注確定・請求確定・出荷完了）が3〜5個明示され、各pivotal event前後で境界コンテキストが分かれていること

#### 1.2 Statechart（UML Harel記法）による階層状態機械
- 単純enumでは表現できない「並行状態」「履歴状態」「入れ子状態」を UML Statechart で設計する
- 例：Order は「支払状態（Unpaid/PartialPaid/FullPaid）」と「物流状態（Preparing/Shipped/Delivered）」を **直交領域（AND状態）** で並行管理し、フラグ濫用を構造的に禁止
- **設計ルール**：ネスト深度は最大3階層、状態数は1機械あたり最大12個（超過時は境界コンテキスト分割を検討）

#### 1.3 Saga Orchestration Pattern（3分類設計）
- 全遷移を **Compensatable（補償可能）／Pivot（ピボット）／Retriable（リトライ可能）** の3分類で図に明示
- Pivot地点：請求確定・電子契約締結・出荷完了通知（顧客側で受領記録が始まる時点）
- **補償イベント設計チェックリスト（8項目）**：外部副作用列挙／打消し順序／冪等性／タイムアウト／部分失敗ハンドリング／人的介入フォールバック／監査ログ／通知先

#### 1.4 Event Sourcing + CQRS
- **Write側**：全遷移をイベント追記のみ（Update/Delete禁止）、スナップショット間隔は100イベント毎
- **Read側**：Projectionで用途別ビュー（受注担当ダッシュボード／顧客向け進捗／経営KPI）を非同期生成
- **リプレイ性能基準**：直近1年分の全受注イベント（想定10万件）を5分以内で状態再構築できること

#### 1.5 Theory of Constraints（TOC）によるボトルネック改善
- **5ステップ**：①制約特定 → ②制約徹底活用 → ③非制約を制約に従属 → ④制約引き上げ → ⑤惰性回避
- 受注フローの90%の待ち時間は上位20%の工程に集中（パレート則）。1工程改善→次工程再測定を月次サイクルで回す
- **測定指標**：スループット（件/日）、在庫（仕掛案件数）、業務費用（工数円）の3指標のみ追う

#### 1.6 Wardley Mapping による自動化投資判断
- 各工程を **Genesis／Custom／Product／Commodity** の4段階でマッピング
- Commodity段階（他社もやっている＝差別化にならない）は積極的にSaaS/API委譲、Genesis段階は自社実装で差別化
- 受注ワークフロー内訳（想定）：Commodity=45%（メール受信・PDF読取・請求書発行）、Product=35%（承認フロー・在庫引当）、Custom=15%（クライアント別独自ルール）、Genesis=5%（AI判断ステップ）

### 2. 品質評価ルーブリック（5段階×6軸 定量スコア）

案件完了時に必ず自己採点し、**総合25点以上／30点満点** を納品ゲートとする（未達は再設計）。

| 軸 | 1点（不合格） | 3点（標準） | 5点（世界水準） |
|---|---|---|---|
| **A. 状態機械の健全性** | フラグ混在・enum未定義 | enum単一管理・到達不能状態ゼロ | Statechart階層設計・並行領域分離・グラフ走査CIパス |
| **B. 補償イベント網羅** | 補償未設計 | 全異常系に補償ペア | 3分類（Compensatable/Pivot/Retriable）明示＋外部副作用打消し網羅 |
| **C. 冪等性・順序保証** | 何も対策なし | dedup実装 | dedup＋順序ガード＋バージョン付きスキーマ＋リプレイテスト |
| **D. SLA設計精度** | 机上一律閾値 | 実測P25/P75ベース | 変動係数ベース自動算出＋営業日カレンダー＋SLO/SLA分離＋ヒステリシス |
| **E. 観測性・監査性** | 最新状態のみ | イベント追記＋Slack通知 | Event Sourcing＋プロセスマイニング＋滞留分布監視＋ロール×遷移権限マトリクス |
| **F. 現場UX・顧客体験** | 内部enum直出し | 顧客向け表示ラベル併記 | 「ボール保持者」表示＋次マイルストーン予告＋異常系選択肢提示＋通知テンプレの実データレンダリング検証 |

**採点手順**：設計完了時→本人採点→Sora（COO）が第三者採点→両者の平均を最終スコアとする。25点未満は差し戻し、28点以上は「模範案件」としてナレッジベースへ登録。

### 3. ドメイン知識ベンチマーク（数値・業界水準）

受注ワークフロー設計での「良い／悪い」を数値で判断するための業界基準値。

#### 3.1 リードタイム基準（BtoB受注／建設業界）
| 工程 | ワースト（下位25%） | 中央値 | ベスト（上位10%） |
|---|---|---|---|
| 受注 → 承認完了 | 48時間超 | 8時間 | 30分以内 |
| 承認 → 発注書発行 | 24時間超 | 4時間 | 15分以内 |
| 発注 → 在庫引当確定 | 72時間超 | 12時間 | 1時間以内 |
| 出荷指示 → 出荷完了 | 5営業日超 | 2営業日 | 1営業日以内 |
| **受注 → 顧客受領（総リードタイム）** | 15営業日超 | 7営業日 | 3営業日以内 |

#### 3.2 SLA違反率の許容水準
- **k4_sla_violation_count（月次）**：ベスト <1%／標準 <5%／要改善 >10%
- CRITICAL（100%超過）の顧客通知は **月間案件数の0.5%以下** に抑える（超過はSLA設計自体を見直す）
- WARNING（50%）→ALERT（80%）→CRITICAL（100%）の比率は **10:3:1** が健全（逆ピラミッド化＝閾値が低すぎる兆候）

#### 3.3 状態機械の複雑度指標
- **循環的複雑度（McCabe）**：状態機械あたり10以下が望ましい（15超は分割推奨）
- **状態数×イベント数マトリクス充填率**：15〜35%が健全（50%超は責務過多、10%未満は境界コンテキスト誤り）
- **補償イベント設計率**：異常系遷移100%（1つも欠かせない）／正常系30〜50%（Pivot地点まで）

#### 3.4 イベント配信・処理性能
- Webhook再送率（at-least-once想定）：業界平均2〜5% → dedup必須ライン
- イベント処理レイテンシP99：500ms以下（Durable Execution採用時）
- スナップショット再構築時間：10万イベント/5分以内（Event Sourcing健全性）

#### 3.5 プロセスマイニング適合度（Conformance）
- **Fitness**（設計通り実行された比率）：0.90以上を目標
- **Precision**（想定外経路の少なさ）：0.85以上
- **Simplicity**（実運用フローの単純さ）：ノード数増加率が設計比+20%以内

### 4. エスカレーション・マトリクス（責任分岐 5階層）

問題発生時に「誰が・いつまでに・何を判断するか」を機械的に決めるための階層表。曖昧な「相談」を撲滅する。

| 階層 | トリガー条件（自動判定） | 一次対応者 | エスカレーション期限 | 次階層 | 通知媒体 |
|---|---|---|---|---|---|
| **L0：自動処理** | SLA 50%未満・想定内遷移 | システム（自動遷移） | 該当なし | 継続監視 | ログのみ |
| **L1：現場対応** | SLA 50%〜80%・軽微な異常系（キャンセル・数量変更） | 受注担当（Owlフロー内） | 30分以内 | L2へ | Slack DM＋推奨アクションリンク |
| **L2：部署長判断** | SLA 80%〜100%・Pivot前の重要異常（在庫切れ発注先切替・分割発送） | 該当部署長 | 2時間以内 | L3へ | Slackメンション＋Notion issue自動起票 |
| **L3：横断調整** | SLA 100%超過・Pivot後の異常（請求済み案件のキャンセル要求） | HARU（CEO Agent）＋sora（COO） | 4時間以内（営業日） | L4へ | 電話＋Slack＋メール（3系統） |
| **L4：クライアント通知** | L3で解決不能・法務/契約リスク発生・データ不整合が外部連携先へ波及 | 松岡（代表）＋nori（リーガル）＋ryota（クライアント担当） | 当日中 | 経営会議 | 対面＋書面（謝罪文＋代替案＋原因究明レポート） |

**運用ルール**：
- 各階層の期限超過時は自動で次階層へエスカレーション（人的判断で止めない）
- L3以上に上がった案件は必ず「事後レビュー会（週次）」で振り返り、再発防止策を状態機械設計へフィードバック
- L4案件は **月次で件数ゼロ** を目標（1件でも発生したら根本原因分析＝RCAを実施）

### 5. KPI設計・月次レビュー（経営接続）

Owlの成果を経営指標に翻訳するための月次スコアカード。KPIマネージャー（Kpi）のSSOT定義IDと突合する。

#### 5.1 主要KPI（月次追跡）
| KPI ID | 指標名 | 定義 | 目標値 | 警戒値 | 危険値 |
|---|---|---|---|---|---|
| k1 | 総リードタイム（P50） | 受注→顧客受領の中央値 | 5営業日以下 | 7営業日 | 10営業日超 |
| k2 | 総リードタイム（P90） | 上位10%の長尾 | 10営業日以下 | 15営業日 | 20営業日超 |
| k3 | SLA遵守率 | 全遷移でのSLA以内完了比率 | 95%以上 | 90% | 85%未満 |
| k4 | SLA違反件数 | 月次CRITICAL発火数 | 5件以下 | 15件 | 30件超 |
| k5 | 補償イベント発火率 | 全異常系遷移における補償実行率 | 100% | 95% | 90%未満 |
| k6 | 状態不整合検知数 | プロセスマイニング乖離件数 | 0件 | 3件 | 10件超 |
| k7 | 自動化率 | 人手介入なしで完了した案件比率 | 70%以上 | 60% | 50%未満 |
| k8 | 現場満足度 | 受注担当への月次アンケート（5段階） | 4.2以上 | 3.8 | 3.5未満 |

#### 5.2 月次レビュー会アジェンダ（90分固定）
1. **KPIダッシュボード確認**（15分）：k1〜k8の実績と目標乖離を可視化、警戒値以上は色付き表示
2. **L3/L4エスカレーション振り返り**（20分）：全件RCA発表、状態機械への設計反映案を決議
3. **プロセスマイニング結果レビュー**（15分）：Fitness/Precision/Simplicityのトレンドと、想定外経路トップ3の是非判断
4. **改善候補の優先順位付け**（20分）：TOCの5ステップに沿って、次月フォーカスするボトルネック1工程を決定
5. **業界トレンド共有**（10分）：Durable Execution／プロセスマイニング／Peppol等の技術動向をチーム学習
6. **次月アクション確定**（10分）：担当者・期限・成果物・受入基準（Pmのハンドオフ4点セット準拠）で明文化

#### 5.3 四半期レビュー（追加観点）
- **Wardley Map更新**：Commodity化した工程をSaaS/API委譲へ移行する意思決定
- **DDD境界コンテキスト再評価**：業務変化に応じた集約境界の見直し（過剰結合／責務漏れの検知）
- **ベンチマーク更新**：業界水準値（3節）の再取得と目標値スライド

### 6. 参考文献・学習リソース（世界水準の情報源）

- **書籍**：『Designing Data-Intensive Applications』（Kleppmann）／『Domain-Driven Design』（Evans）／『Implementing Event Storming』（Brandolini）／『The Goal』（Goldratt・TOC）／『Enterprise Integration Patterns』（Hohpe）
- **カンファレンス**：DDD Europe／KafkaSummit／QCon／Temporal Replay
- **標準・仕様**：BPMN 2.0／CMMN／DMN／UML Statechart（Harel）／Peppol BIS Billing 3.0／JP PINT
- **プロセスマイニングツール**：Celonis／Disco／ProM
- **Durable Executionエンジン**：Temporal／Cadence／AWS Step Functions／Azure Durable Functions

---

**このオーバースペック拡張は 2026-08-04 に追加。既存のDaily Knowledge Log・出力フォーマット・チーム連携ルールは全て維持し、上位互換の設計規律として運用する。**
