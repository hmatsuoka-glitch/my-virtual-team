...
### 2026-08-19
- **新スキル：Claude Agent SDK による状態機械オーケストレーション導入**。従来のBPMN/ワークフローエンジンに加え、Claude Agent SDK の subagent 機能で「Order判断エージェント／PurchaseOrder発注エージェント／Shipment追跡エージェント」を独立subagentとして構成、状態遷移の判断ロジックをLLMに委譲可能な領域（例外系の優先度判定・顧客通知トーン選択）と決定的ロジック領域（正常系enum遷移・SLA計測）に分離設計する
- **新スキル：MCP（Model Context Protocol）サーバー活用による外部システム統合**。atomdenki/packages/domain の現行状態遷移コードをMCPサーバー化し、Bo/Dat/Sales/Financeの各エージェントから統一プロトコルで状態遷移操作を可能化。従来のREST API個別実装から MCP tools 統一に切替、エージェント間の状態機械アクセスを標準化
- **新スキル：n8n AI Workflow Builder による受注フロー試作の高速化**。自然言語で「クライアント別の受注→発注→出荷→請求フローを作って」と指示、n8n が下書きワークフローを生成。Owlは生成された下書きを状態遷移表として検証・補償イベントを追加し本番化。試作時間3日→2時間に短縮（2026-05-25 Dat連携ナレッジのn8n新機能を受注設計にも展開）
- **新スキル：LangGraph による状態遷移グラフのコード化**。PlantUML/CSV二重管理（2026-05-26記録）から LangGraph の StateGraph API に移行、状態遷移の実装コードと可視化図を同一ソースから生成。図と実装の乖離を構造的にゼロ化、レビュー時のPlantUML再生成工数も削減
- **新スキル：CrewAI ロールベース多エージェント設計への理解**。「発注担当ロール／出荷担当ロール／請求担当ロール」をCrewAI の Agent+Task 構造で表現、各ロールの権限マトリクス（2026-08-11記録の遷移権限外部化）と1対1対応させる設計を導入。Owl は状態遷移設計とロール設計の両輪で受注ドメインを表現可能化
- **新スキル：AutoGen conversational multi-agent での異常系判断分岐**。キャンセル・分割発送・在庫切れの5大異常系（2026-05-22記録）で、AutoGen の GroupChat 機能を用い「営業判断エージェント×調達判断エージェント×CS判断エージェント」の対話で最適解を出す設計を試験導入。従来の固定ルール分岐で対応困難な複合条件（顧客ヘルススコア×在庫状況×代替品可否）を対話で解決

### 2026-08-20
- **方法論①：Retrieval-Augmented Generation（RAG）による類似ケース参照の標準化**。SLA違反ALERT通知（2026-05-24記録の4セット通知）の「類似ケースリンク」を、過去のOrder履歴をベクトルDB化しRAG検索で自動抽出する方式に刷新。受注担当が過去対応履歴を手で探す工数をゼロ化、判断時間30秒→10秒に更に短縮
- **方法論②：Agent Memory（長期記憶）による顧客個別文脈保持**。CS連携（2026-08-13記録）で顧客別温度・過去クレーム履歴を毎回CSに問い合わせる代わりに、Agent Memory 層に顧客プロファイル（好むトーン・NG表現・過去の不満点）を保持し、異常系通知テンプレの差込を自動化。CS→Owl のリアルタイム連携依存を減らす
- **方法論③：Tool Use / Function Calling 標準化による外部API呼び出しの冪等化**。補償イベント/外部API呼び出しの冪等キー付与（2026-08-11記録）を、全ツール定義に `idempotency_key: {required}` を強制する Function Calling スキーマで統制。ツール実装側で冪等キー欠落を弾き、二重発注・二重請求を型レベルで防止
- **方法論④：Multi-Agent Orchestration の階層設計**。HARU（CEO）→ Owl（受注設計統括）→ subagent（Order/PO/Shipment 個別判断）の3層構造で、上位層は方針決定・下位層は状態遷移実行に責務分離。BMAD-METHOD（SKILL.md記載）の思想を受注ドメインにも適用
- **方法論⑤：LlamaIndex による設計書・遷移表・過去障害レポートの統合検索**。状態遷移表CSV・PlantUMLソース・障害ポストモーテム・SLA運用ドキュメントを LlamaIndex で統合インデックス化、新規受注フロー設計時に「類似ドメインの過去設計と障害事例」を横断検索可能化。5大異常系テンプレ（2026-05-26記録）の適用判断を根拠付きで実施

### 2026-08-21
- **業務AI Agent化①：受注入口判定Agent（Sales連携ヨミステージ→OrderConfirmed発火判定の自動化）**。Sales商談ステージと状態機械の1対1対応（2026-08-13記録）を、判定Agent が Sales MCP からステージ取得→ルールベース判定→OrderConfirmed発火の自動フロー化。受注担当の「これは確定？内示？」判断介入を80%削減
- **業務AI Agent化②：例外系通知起草Agent（顧客個別文脈×異常系テンプレの自動合成）**。RAG（2026-08-20方法論①）＋Agent Memory（同②）を組み合わせ、在庫切れ発生時に「顧客A向け謝罪＋代替提案文」を自動起草。受注担当は最終確認と送信ボタンのみ、起草工数15分→30秒
- **業務AI Agent化③：SLA違反リスク予測Agent（発火前の予兆検知と先回りアクション提示）**。Dat（データ分析）から工程別リードタイム分布（2026-05-27 Dat連携）を取得、現在進行中のOrderに対して「このままだとSLA80%到達がX時間後」を予測、CRITICAL到達前にWARNING段階で先回りアクションリンクを提示
- **業務AI Agent化④：状態遷移設計レビューAgent（新規フロー草案の5大異常系網羅チェック）**。Owl が新規受注フロー草案を作成→レビューAgent が5大異常系（キャンセル・分割・在庫切れ・承認タイムアウト・返品）の網羅性・補償イベントペア存在・SLA階層設計を自動チェック→未網羅箇所を指摘。設計品質のセルフゲート化
- **業務AI Agent化⑤：契約・電子署名イベント連携Agent（Gen連携で建設業法要件充足を自動検証）**。建設クライアントの契約締結ピボット（2026-08-13 Gen連携）で、電子契約完了イベントを受け取ったAgent が Gen の建設業法ナレッジを参照し「必要書面が全て揃っているか」を検証、欠落があれば締結ピボット発火を保留

### 2026-08-22
- **既存資産の非破壊活用①：状態遷移表CSV（2026-05-26記録）は LangGraph StateGraph の入力ソースとして再利用、CSV自体は正のマスタとして残す**。CSV→LangGraph変換スクリプトを新設し、既存設計資産を捨てずにコード化。過去案件の状態遷移表はそのまま参照可能で、新規案件のみ LangGraph 記法で追加
- **既存資産の非破壊活用②：5大異常系パステンプレ（2026-05-26記録）は RAG インデックスの seed データとして投入、テンプレ自体は編集可能な Notion ページとして維持**。RAG は Notion を data source として同期、テンプレ更新は Notion で行い RAG は自動追随。手動テンプレ運用と AI 検索の両立
- **既存資産の非破壊活用③：補償イベントペア設計（2026-05-22記録）はそのまま残し、Function Calling 冪等キー標準化（2026-08-20方法論③）を上位レイヤーで追加**。既存の補償ロジックには手を入れず、外部API呼び出しラッパーだけを冪等化。段階的移行で本番影響ゼロ
- **既存資産の非破壊活用④：SLA 3階層エスカレーション（2026-05-22記録）は SLA予測Agent（2026-08-21③）と共存、予測は WARNING前段の追加階層として運用**。既存の50%/80%/100% 閾値は変更せず、予測を「予測WARNING」として先行通知。運用側の慣れた通知パターンを維持しつつ先回り性を強化

### 2026-08-25
- **3行サマリー①（新スキル総括）**：Claude Agent SDK / MCP / n8n AI Workflow Builder / LangGraph / CrewAI / AutoGen の6スキルを受注ワークフロー設計に追加導入、状態遷移の判断領域と決定的領域を分離しLLM委譲可能領域を明確化。従来の BPMN 一本足打法から多エージェント協調型設計へ拡張し、異常系分岐と顧客個別対応の質を構造的に向上
- **3行サマリー②（方法論総括）**：RAG／Agent Memory／Function Calling 冪等化／Multi-Agent 階層設計／LlamaIndex 統合検索の5方法論を採用、過去ナレッジ（2026-05〜08の失敗パターン・連携ルール）を検索可能資産化。設計時点のセルフレビューから障害時の類似事例参照まで、AIによる根拠付き意思決定を全工程に組込
- **3行サマリー③（業務Agent化と非破壊統合）**：受注入口判定／例外通知起草／SLA予測／設計レビュー／契約要件検証の5Agent を新設し、既存の状態遷移表CSV・5大異常系テンプレ・補償イベントペア・3階層エスカレーションは全て正のマスタとして温存。段階移行・共存運用で本番影響ゼロを担保しつつ、受注担当の判断介入を80%削減する2026年型ワークフロー基盤へ進化
