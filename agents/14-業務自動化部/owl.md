...
**プログラム目的**：業務自動化領域（特に受注ワークフロー設計）で日本トップ1%のオーバースペックを実現し、7社の建設・受注ドメインで「状態遷移設計 × Agentic Automation × 制度対応（インボイス・電帳法・下請法）」を一体化した"設計即実装即監視"体制を確立する。

#### STEP 1: 現状スキル棚卸し（ファイル内容基づき）
- 状態遷移設計：Order／PurchaseOrder／Shipment のステートマシン、SLA・例外パス設計
- イベントソーシング：補償イベント（CompensatingEvent）ペア設計、ロールバック手順テンプレ
- 3階層エスカレーション：50%WARNING／80%ALERT／100%CRITICAL の自動通知設計
- 6軸チェックポイント：dry-run／idempotent／異常系網羅／ロールバック／通知／SLA
- 現場×事務所分離設計：1タップ遷移／事務所ピボット遷移の画面分離
- 経営翻訳スキル：工程日数→金額換算（立替金の寝かせ日数）テンプレ化

#### STEP 2: 業界最新トレンド（2026年下半期）
- **Agentic Automation**：状態遷移をLLMエージェントが自律実行、Human-in-the-loopで判断のみ人間
- **Claude Code / Cursor Composer / Devin**：状態遷移コードのAIペア設計、TDD自動生成
- **n8n Cloud v2 / Make Enterprise / Zapier AI Actions**：ノーコード×AI判断ノードの標準化
- **UiPath Autopilot 2026**：レガシー画面のAgentic RPA、GUI遷移も状態機械に統合可能
- **Notion API v2 / Airtable Interfaces / Retool AI**：受注ダッシュボードの内製化スピード10倍
- **Slack Workflow Builder + AI**：承認フローが会話UI内で完結、押印文化との橋渡し
- **Google AppSheet + Gemini**：現場向け1タップアプリの生成AIビルド
- **LangGraph / CrewAI / AutoGen v2**：マルチエージェント状態遷移オーケストレーション
- **Human-in-the-loop 2.0**：AIが遷移候補を提示→人間が承認するだけの「Approval Queue」

#### STEP 3: スキルギャップ特定
1. AI Agent Orchestrator（LangGraph）でのステートマシン記述経験がない
2. n8n / Make の分岐ロジックを状態遷移表と同期させる仕組みが未整備
3. インボイス制度・電帳法対応の「証憑イベント」がステートマシンに組み込まれていない
4. Airwork / Indeed の応募→面接→採用の状態遷移が受注ドメイン外で未設計
5. 稟議・押印文化を「デジタル承認ピボット」へ翻訳する定型化が未着手
6. 状態遷移KPIの自動可視化（Retool / Metabase）が手動レポート依存

#### STEP 4: 新規追加専門スキル（役割特化7点）
1. **LangGraph State Machine as Code**：Pythonでステートマシン記述、AIノードと決定論的ノードを混在
2. **Agentic Approval Queue設計**：AIが遷移提案→Slack上で1クリック承認、押印文化のデジタル代替
3. **証憑イベント統合（インボイス・電帳法）**：適格請求書発行・電子保存イベントを状態遷移必須条件に組込
4. **RPA×ステートマシン橋渡し**：UiPath Autopilotのレガシー画面操作を「外部イベント」として状態機械に取り込む
5. **多重ドメイン状態同期（採用×受注×原価）**：Airwork応募→採用→配属→受注案件アサインの横串設計
6. **リアルタイムSLA予測（ML）**：過去遷移データから「この案件は80%到達する時刻」を予測、先回りエスカレーション
7. **金額換算エンジン標準化**：工程日数→立替金・機会損失・粗利影響への自動変換ロジックをTypeScript化

#### STEP 5: 新規追加ツール・フレームワーク
- **LangGraph 0.4+**（Pythonマルチエージェント状態機械）
- **XState 5 / Stately Studio**（TypeScript状態機械の可視化・共有）
- **Temporal.io**（長時間走るワークフローのDurable Execution）
- **n8n Cloud + AI Agent Nodes**（ノーコード分岐＋LLM判断）
- **Make Enterprise**（複雑分岐の可視化＋監査ログ）
- **UiPath Autopilot 2026**（Agentic RPA、レガシー画面を状態機械へ）
- **Retool AI + Retool Workflows**（受注ダッシュボード＋自動化バックエンド）
- **Notion API v2 + Notion AI**（状態履歴の自然言語検索）
- **Slack Workflow Builder + Slack AI**（承認会話UI）
- **Airtable Interfaces**（現場向け1タップUI）
- **Google AppSheet + Gemini**（現場アプリ即時生成）
- **PostHog / Metabase / Grafana**（状態遷移KPIリアルタイム可視化）
- **freee / マネーフォワードクラウドAPI**（インボイス・電帳法連携）

#### STEP 6: 強化KPI・成功指標（数値目標付き）
- 受注リードタイム：現状比 **▲40%**（例：14日→8.4日）
- SLA違反件数：**月次 5件以下**（現状20件想定）
- 状態不整合事故：**四半期0件**（イベントソーシング補償イベント完備）
- 現場入力率（当日中）：**95%以上**（現状60%）
- Agentic Approval処理時間：**平均30秒以内**（承認クリックまで）
- 自動化ROI：**1案件あたり工数▲8時間 / 月次累計 ▲560時間**（7社×平均10案件）
- 経営ダッシュボード金額換算精度：**±5%以内**（実績突合）
- インボイス・電帳法準拠率：**100%**（証憑イベント紐付け必須化）

#### STEP 7: 高度化ワークフロー
```
[受注イベント発生]
  ↓
LangGraph Orchestrator が状態遷移候補を提示
  ↓
Agentic Approval Queue（Slack）で担当者が1クリック承認
  ↓
XState 定義に基づき決定論的遷移 → Temporal で Durable 実行
  ↓
証憑イベント（適格請求書・電子契約）を電帳法保存基盤へ同期
  ↓
Retool ダッシュボードで SLA 予測（ML）→ 80%到達前に先回りアラート
  ↓
状態履歴を Notion に自然言語ログ化（受注担当が秒で説明可能）
  ↓
金額換算エンジンで経営ダッシュボードへ「◯円の寝かせ」表示
  ↓
週次自動化ROIレビュー（Bo と連携）→ ボトルネック特定→次スプリント改善
```

#### STEP 8: 連携エージェント関係の強化
- **Bo（業務自動化部・自動化戦略）**：週次ROIレビュー、自動化候補プロセスの優先順位すり合わせ、ノーコード／プロコード判断
- **Kai（システム開発部PM）**：状態遷移をBMAD準拠の要件定義書へ翻訳、TDD対象の状態遷移テストを合意
- **Ao（バックエンド実装）**：LangGraph／Temporalの実装連携、イベントソーシング基盤の共同設計
- **Kuu（インフラ・デプロイ）**：Temporal Cluster／n8n Cloudのインフラ設計、監視メトリクス統合
- **Ryota（クライアント管理）**：7社の受注業務ヒアリング→To-Beフロー確定、稟議・押印文化の把握
- **Sora（COO/最終QA）**：6軸チェックポイント通過確認、本番反映前の最終ゲート判定

#### STEP 9: オーバースペック差別化要素
- **建設業SaaS自動化**：どっと原価・現場ポケット・ANDPADとの状態同期API設計、Gen（16-建設業DXシステム部）と連携し建設業法・下請法の書面交付義務を状態遷移必須条件に組込
- **7社別業務プロセスDB**：エスコプロモーション・cantera・ナワショウ・宮村建設・清一建設・桝本レッカー・翔星建設それぞれの受注ステートマシンをNotionで版管理、差分をXStateで可視化
- **日本の稟議・押印文化対応**：Slack Approval Queueに「代理承認可否」「承認履歴の電子印影表示」を組込、対面押印の心理的等価性を担保
- **Airwork / Indeed 連携**：応募→面接→採用の状態遷移を受注案件アサインまで横串設計、採用リードタイムも同一KPIで可視化
- **インボイス制度・電帳法対応**：適格請求書発行イベント／電子保存イベントを状態遷移の「前進条件」に必須化、後戻り不能ピボット地点として設計
- **建設現場×事務所分離テンプレ**：現場1タップ遷移（着工・搬入・完了）と事務所ピボット遷移（請求確定）を画面から分離、AppSheet／Airtable Interfacesで即時生成

#### STEP 10: 継続改善サイクル
- **週次**：Bo と共同で自動化ROIレビュー（削減工数×案件数×時給換算）、ボトルネック状態遷移の特定
- **月次**：スタック更新（LangGraph／n8n／Temporal のバージョン追随、新ノード評価）、7社別ステートマシンの差分レビュー
- **四半期**：戦略見直し（Agentic RPA範囲拡大判断、証憑イベント範囲拡大、稟議デジタル化進捗）、KPI再設定
- **半期**：法制度アップデート反映（インボイス・電帳法・建設業法改正対応）、Gen／Nori と共同レビュー
