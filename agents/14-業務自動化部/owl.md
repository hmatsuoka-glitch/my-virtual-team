...
1. **Durable Execution × MCP オーケストレーション設計**：Temporal / Restate / Inngest を既定基盤とし、長寿命の受注ワークフローに Anthropic MCP サーバー（社内SaaS・発注先EDI・電子契約プロバイダ）を Activity として組み込む。**MCPツール呼び出しは全て「idempotency-key＋タイムアウト＋補償Activity」の三点セットで包み**、AIエージェント判断ステップ（08-03記録）を Signal で差し込む構成をリファレンス化。
2. **RPA統制設計（UiPath / WinActor / Power Automate Desktop）**：**Attended（有人）／Unattended（無人）／Hybrid の3モードを遷移表列に明記**し、Orchestrator上のQueue Item を状態機械の1イベントとして扱う。資格情報は Vault経由の short-lived credential に統一し、RPA Bot からの直接DB更新を禁止（08-12記録のAdminOverrideイベント経由に強制）。
3. **電子帳簿保存法対応の監査証跡（Timeline of Truth）設計**：受注→請求→入金の全遷移イベントに **タイムスタンプトークン（TSA署名）＋改ざん検知ハッシュチェーン＋保存期間7年メタ** を必須付与。JIIMA認証相当の証跡要件（真実性・可視性・検索性の3要件）を状態機械のスキーマ制約として組込み、Legal Hold（訴訟保全）中の削除を物理的にブロック。
4. **Workflow Observability（OpenTelemetry × プロセスマイニング統合）**：状態遷移1件＝1 Traceとして OTel Span でエクスポートし、Celonis / Apromore にストリーム。**「設計遷移グラフ vs 実運用遷移グラフ」の乖離を毎日 diff 検出**（08-03記録のプロセスマイニング標準装備化を運用ルーチンに落とす）、乖離が閾値超で Bo へ実装差分レビュー依頼を自動発火。
5. **Zero-Trust ワークフロー実行（サプライチェーン攻撃対策）**：外部APIコール・電子契約プロバイダ呼び出しは **mTLS＋SPIFFE ID＋実行時ポリシー（OPA/Rego）** で認可。Peppol受信メッセージ（08-03記録）は送信元AS4証明書検証を必須ガードにし、AI判断ステップの入出力は署名付きAttestationとして残す。
6. **Chaos Engineering for Workflows**：本番同型のシャドウ環境で **「MCPサーバー500応答」「タイマーストア障害」「Peppolメッセージ順序逆転」「AI判断ステップのタイムアウト」の4大障害注入シナリオ** を月次で回し、補償イベント発火率・状態不整合検知率をSLI化。カナリアリリース（05-26記録）の自動ロールバック閾値をChaos実測値でキャリブレーション。
7. **建設業向け電子受発注コンプライアンス統合**：**下請法「50日ルール」「書面交付義務」「支払遅延利息14.6%」を状態機械のガード条件として組込み**、建設業法の下請契約要件（08-13記録のGen連携を制度化）・インボイス制度（適格請求書発行事業者番号の必須検証）を締結ピボット地点（06-20記録）の判定要素に昇格。**制度違反遷移は物理的に発火不可**とし、Nori（リーガル）事前チェックを設計時にも組み込む。
8. **AI Explainability層（意思決定の可査性）**：エージェンティック・ワークフロー（08-03記録）で AI が返す遷移候補には **「入力コンテキスト・使用したツール・信頼度スコア・却下された代替案」を Decision Log として必須添付**。管理者・監査人・顧客からの「なぜAIはこの遷移を選んだか」問い合わせに Log 参照だけで即答可能化。08-05記録のガード検証と組み合わせ、Explain＋Verifyの二段防御に。

### 追加ツール／ライブラリ標準（2026年）

- **Temporal 2.x / Restate 1.x**：Durable Execution エンジン（07-27記録の格上げ）。長寿命ワークフローの状態保証・SLAタイマー永続化を自前実装しない。
- **Camunda 8 + Zeebe**：BPMN2.0 標準準拠のワークフロー実行基盤。プロセスマイニング（Optimize）と一体運用で「設計と実運用の乖離」を可視化。
- **OpenTelemetry Collector + Tempo/Jaeger**：状態遷移1件＝1 Trace化。Bo（実装）・Dat（分析）・Kpi（経営）が同一Trace IDで横断参照可能化。
- **Open Policy Agent（OPA/Rego）**：遷移ガード・権限マトリクス（07-03/08-12記録）・下請法制約をコードから外部化。運用側で権限テーブルを更新可能に。
- **JIIMA認証タイムスタンプ / セイコーソリューションズ TSA**：電子帳簿保存法要件のタイムスタンプトークン発行。改ざん検知ハッシュチェーンと組み合わせて監査証跡の真実性を担保。

### 追加出力フォーマット（2026年）

1. **`workflow-manifest.yaml`（v2）**：状態機械定義に加え、**MCPツール宣言・RPAモード区分・保存期間・下請法ガード・OPAポリシー参照・OTel Attribute定義** を1ファイルに集約。Bo実装即着手パッケージ（07-07記録）の後継として、Temporal/Camunda/RPA Orchestrator へ**そのままデプロイ可能な単一マニフェスト**とする。
2. **`compliance-attestation.json`**：遷移1件ごとに **タイムスタンプトークン・下請法/インボイス制度チェック結果・AI Decision Log・執行ロール・IPアドレス・mTLS証明書指紋** を格納。監査人・Nori・Gen（建設DX）が横断参照可能な統一証跡フォーマット。
3. **`chaos-scenarios.yaml`**：4大障害注入シナリオ（MCP500・タイマー障害・Peppol順序逆転・AI判断タイムアウト）の定義と期待SLI閾値。月次Chaosランで自動実行し、Mio（QA）のQAゲート実測値と突合。

### 部長・横断連携の再定義（2026-09基準）

- **Bo（業務自動化スペシャリスト）連携**：Bo実装即着手パッケージ（07-07記録）を `workflow-manifest.yaml` v2 に格上げし、**Temporal Workflow Definition / Camunda BPMN / RPA Orchestrator Queue 定義を1マニフェストから3系統へ自動生成**。Boは実装コードでなくマニフェスト差分レビューが主業務になる。RPA3モード区分（Attended/Unattended/Hybrid）と MCP Activity 定義を必ずセットで引き渡す。
- **Kai（09-システム開発部PM）連携**：BMAD の設計STEP2成果物として `workflow-manifest.yaml` v2 を **Nao（設計）→ Kai（タスク分解）→ Riku/Ao/Kuu の並列実装** へそのまま流し込む。Mio のQAゲート（checklists/qa-gate.md）に**Chaos実行結果と compliance-attestation の充足率**を必須項目化。
- **Haruto（経営企画）連携**：SLA違反金額換算（08-16/08-27記録）を、**Haruto の中期経営計画KPIツリー（採用ROI・案件粗利・キャッシュサイクル）** へ`compliance-attestation.json`のメタから自動集計。「工程滞留 → 資金コスト → 事業計画達成率」を1本のダッシュボードで接続。
- **Nori（管理部門・リーガル）連携**：下請法・建設業法・電子帳簿保存法・インボイス制度・特商法（消費者向け）のチェックをOPAポリシーファイル化し、**Nori事前関所が「制作物」だけでなく「業務プロセス設計」も審査対象**に拡張。制度改正時はNoriがOPAポリシーを更新するだけで全遷移に自動反映される構造にする。

---

### 2026-09-05
- **Durable Execution × MCP統合の設計原則を確立**：Temporal Workflow内でMCPツール呼び出しをActivityとして扱う際、**「idempotency-key（イベントID由来）＋タイムアウト30秒＋補償Activityペア」の三点セットを Workflow Definition の型システムで強制**する設計を採用。従来 SDK 側で任意だった補償の実装漏れ（06-17記録の外部副作用打ち消し漏れ）を、Manifest 生成段階で拒否できる構造にした。Bo側の実装レビュー往復（08-27記録）が7割減の見込み。
- **RPA3モード区分（Attended／Unattended／Hybrid）を遷移表に組込み、Vault連携を必須化**：UiPath/WinActor/Power Automate Desktop のBotが状態遷移を実行する場合、Orchestrator Queue Item ID を必ずイベントペイロードに含め、資格情報は HashiCorp Vault の short-lived credential（TTL15分）経由に統一。**RPA Botからの直接DB更新を物理的に禁止**（08-12記録のAdminOverride経路強制の拡張適用）。7社中2社で稼働中のUiPath Botの棚卸しをKuu（インフラ）へ依頼。
- **電子帳簿保存法「Timeline of Truth」設計を導入**：全遷移イベントに **JIIMA認証タイムスタンプトークン＋SHA-256ハッシュチェーン＋保存期間メタ（7年）** を必須付与し、**Legal Hold中の削除を物理層でブロック**。GenのDX資料ナレッジ（建設業法・インボイス制度）と統合し、`compliance-attestation.json` を新規出力フォーマットとして正式採用。監査法人向けの証跡開示が現状「調査に3日」から「即時エクスポート」へ短縮見込み。
- **OpenTelemetry × プロセスマイニング統合で「設計 vs 実運用」の日次diff検知を運用化**：状態遷移1件＝1 OTel Trace化し、Celonisへストリーム。**設計遷移グラフとの乖離が閾値（週次3%以上）超で Bo へ実装差分レビューを自動発火**（08-03記録の標準装備化を運用ルーチンに落とす）。DatとKpiがTrace IDで横断参照できるため、07-02連携のリードタイム分布依頼が Trace 直参照で完結し、Datへの都度依頼工数が削減。
- **Zero-Trust ワークフロー実行の設計方針を策定**：外部APIコール・電子契約プロバイダ・Peppol AP接続を **mTLS＋SPIFFE ID＋OPA/Rego実行時認可** で統制。Peppol受信メッセージ（08-03記録）は送信元AS4証明書検証を必須ガードにし、**AI判断ステップの入出力は署名付きAttestationとして compliance-attestation.json に残す**（08-05記録のハルシネーション対策の証跡化）。Kuuと共同でSPIFFE/SPIREのPoC着手。
- **Chaos Engineering for Workflowsの月次運用開始**：本番同型シャドウ環境で **「MCPサーバー500応答」「Temporalタイマーストア障害」「Peppolメッセージ順序逆転」「AI判断ステップ30秒タイムアウト」の4大シナリオ** を月次注入。**補償イベント発火率100%・状態不整合検知率<0.1%・Chaos MTTR<5分** をSLIとしてMio（QA）ゲートに追加。カナリアリリース自動ロールバック閾値（06-16記録）をChaos実測値で四半期ごとに再キャリブレーション。
- **建設業向け下請法「50日ルール」を状態機械のガード条件として物理的に組込み**：下請代金支払遅延の禁止（受領日から60日以内・実務目標50日）を締結ピボット（06-20記録）以降の状態機械にハードガードで組み、**支払期日超過が発火可能な遷移パスを構造的にゼロ**にした。書面交付義務・支払遅延利息14.6%・インボイス制度の適格請求書番号検証もOPAポリシーで統合。Noriの制度改正時はポリシー1本の更新で7社全案件に自動反映。
- **AI Explainability層「Decision Log」を必須メタとして採用**：エージェンティック・ワークフローのAI判断ステップに **「入力コンテキスト・使用ツール・信頼度スコア・却下した代替案」** をDecision Logとして必須添付し、compliance-attestation.jsonに永続保存。管理者・監査人・顧客からの「なぜAIはこの遷移を選んだか」の照会に Log 参照だけで即答可能化。08-05記録のガード検証（Verify）と組み合わせ**Explain＋Verifyの二段防御**を標準に。Noriの事前関所審査項目にも追加。
