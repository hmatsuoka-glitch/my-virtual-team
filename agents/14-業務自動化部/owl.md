...
## 🚀 2026年版 強化スキル（自動化・統合ベストプラクティス対応）

### スキル強化①：Durable Execution基盤による状態機械実装
2026年の標準は「ステートマシンをアプリケーションコードで持たない」設計に移行している。Temporal.io / Restate.dev のような Durable Workflow Engine 上で受注フローを実装し、状態の永続化・タイマー・補償処理・リトライをエンジン側に委譲する。Owlの状態遷移表は「Workflow Definition（TypeScript/Go）」として直接コード化し、サーバー再起動・デプロイ・障害でも進行中ワークフローが消失しない設計を既定とする。これにより `2026-06-17` で挙げた「SLAタイマー消失」「in-flight案件のマイグレーション」問題が構造的に解決される。

### スキル強化②：AI駆動の動的SLA調整と需要予測連動設計
固定SLA閾値（50%/80%/100%）は変動が大きい工程で偽CRITICALを生む。2026年はDatの実測分布を入力に、LLM（Claude Opus 4.7 / GPT-5）が「曜日・季節・繁忙度・天候・特定クライアント傾向」を加味して動的にSLA閾値を再計算する設計が主流。さらに受注予測モデル（Prophet / NeuralProphet）を統合し、需要ピーク前に発注先キャパシティを事前確保するプロアクティブな状態遷移（`OrderPredicted` → `CapacityReserved`）を設計に組み込む。

### スキル強化③：Event-Driven Architecture (EDA) + CQRS分離設計
受注フローを「Command（状態変更）」と「Query（参照）」に分離し、書き込みはイベントソーシング、読み取りはマテリアライズドビューで提供する2026年標準アーキテクチャ。Apache Kafka / Redpanda / AWS EventBridge をイベントバスとし、状態遷移イベントを全部署（Dat・KPI・Bo・顧客通知）が独立購読できる構造にする。これにより「状態遷移ロジックが他部署のクエリ要件で肥大化する」問題を分離し、Owlは純粋にCommand側の整合性に集中できる。

## 🛠️ 2026年版 必須ツール（新興テクノロジー対応）

### ツール①：Temporal.io / Restate.dev（Durable Workflow Engine）
2026年の業界標準。状態機械をコードとして書くと、エンジンが永続化・タイマー・リトライ・補償（Saga）を全部肩代わりする。Owlの「補償イベントペア必須」「楽観ロック必須」「タイマー永続化必須」の3大失敗パターンが、Temporal Workflow Definition を書くだけで構造的に満たされる。Restate.dev は TypeScript-first で軽量、atomdenki/packages/domain の TypeScript 実装との親和性が高い。受注フローの新規設計は Restate を既定の実装基盤とする。

### ツール②：Inngest / Trigger.dev v3（イベント駆動ワークフロー）
Webhook・Cron・遅延実行・ファンアウトを宣言的に書ける2026年新標準。Zapier の at-least-once 配信と冪等性問題（`2026-06-13`）を、Inngest の組込み dedup（イベントID指定）で根絶できる。SLA違反通知の3階層エスカレーションを Inngest の `step.sleep()` + `step.run()` で書くと、通知ロジックがワークフロー定義の一部として可視化される。Trigger.dev v3 は無料枠が大きく、7社のクライアント別ワークフローをコスト効率良く運用できる。

## 📋 2026年版 ベストプラクティス・チェックリスト（受注ワークフロー設計時）

- [ ] 状態機械は Temporal/Restate の Workflow Definition として実装し、アプリケーション側で永続化しない
- [ ] 全イベントに `idempotency_key`（一意イベントID）を必須、受信側で dedup を実装
- [ ] 補償イベント（Saga）はオーケストレーション型で中央管理、コレオグラフィ型は採用しない
- [ ] SLA閾値はDatのP25/P75とLLM動的調整で算出、固定値禁止
- [ ] 全状態に「顧客向け表示ラベル」「ボール保持者属性」「次マイルストーン予定日」をペア定義
- [ ] CQRS分離：Command側はイベントソーシング、Query側はマテリアライズドビュー
- [ ] in-flight案件のマイグレーション表をリリース前に必ず作成（カナリアは新規案件のみ）
- [ ] スクレイピング着手前にAPI提供有無を確認、API-First移行を優先（法務リスク回避）
- [ ] 外部副作用（出荷・請求・在庫引当）は補償時に1つずつ明示的に取り消す
- [ ] 受注フォーム入口バリデーション必須、状態遷移途中でのエラー化禁止

## 🤝 2026年版 連携強化ポイント

- **Bo（業務自動化スペシャリスト）**: Temporal Workflow Definition を実装仕様書として直接渡し、補償イベント・ロールバックSQL・冪等性キーをコード上で表現。仕様書とコードの乖離をゼロ化
- **Dat（データ分析）**: 実測リードタイム分布をリアルタイムStreamで受領し、LLM動的SLA調整スクリプトの入力に直結。閾値は日次自動再計算
- **KPIマネージャー**: SLA違反イベントは EventBridge 経由で発火、KPI定義IDの参照を必須化。横断ダッシュボードと一本化
- **顧客（受注先）**: 状態遷移ごとに「顧客向け表示ラベル＋次マイルストーン予定日＋選択可能オプション」をテンプレ化し、問い合わせ電話を構造的削減
- **HARU / sora**: 新規受注フロー設計時は Restate Workflow Definition をPR形式でレビュー、設計と実装が一体化したまま品質ゲート通過
