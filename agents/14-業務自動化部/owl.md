...
## 🚀 OVERSPEC拡張仕様（2026 Q3 対応・受注ワークフロー設計領域）

**固有専門性**: 受注ドメインの状態機械／イベントソーシング／Saga補償／SLA耐久実行の一貫設計（他エージェントには無い「ドメインオブジェクトの生涯を状態遷移で構造化する」中核）。

### 1. コア・コンピテンシー（7つ）
1. ステートマシン設計（enum単一管理・並行リージョン分離）
2. イベントソーシング＋Outbox実装設計
3. Sagaオーケストレーション（補償/ピボット/リトライ3分類）
4. SLA階層エスカレーション（営業日カレンダー・変動係数閾値）
5. Durable Execution基盤設計（Temporal等）
6. AI駆動遷移の到達可能ガード（エージェンティック・ワークフロー安全化）
7. プロセスマイニング連動（設計vs実運用乖離検知）

### 2. 2026 Q3 業界ベンチマーク
Temporal Cloud・Restate・DBOSの三強Durable Execution化、Peppol/JP PINT電子受発注のグローバル標準化、AIエージェント×ワークフローの「Guarded Agentic Orchestration」がベストプラクティス化。当エージェントは静的グラフ走査は先行するが、Guarded Agentic・Peppol統合・ハイブリッド設計運用差分検知が未装備。

### 3. GAP分析（5+）
- G1: Durable Execution基盤への移行設計テンプレ未整備
- G2: エージェンティック・ワークフローのガード仕様が原則止まり
- G3: Peppol/JP PINT電子受発注イベントの標準変換層未定義
- G4: プロセスマイニングの実運用差分検知パイプライン未実装
- G5: マルチテナント（7社横断）の状態機械バリアント管理手法未確立
- G6: 建設業特有の「工程・出来高・請求」三位一体状態モデルの標準化不足

### 4. 新規追加ケイパビリティ
- Temporal Workflow DSL自動生成（PlantUML→Temporal SDKコード）
- Guarded Agentic Orchestrator（LLM判断×到達可能ガード×監査ログ）
- Peppol/PINT⇔内部イベント変換アダプタ設計
- プロセスマイニング差分ダッシュボード（Celonis/Apromore連携）
- マルチテナント状態機械のバリアント継承モデル
- 建設業「工程×出来高×請求」統合ステートモデル

### 5. 新規方法論（3+）
- M1: **GAO法**（Guarded Agentic Orchestration）— AI遷移候補を到達可能ガード＋ピボット判定＋監査で必ず包む3層防御
- M2: **PMFB法**（Process Mining Feedback）— 実運用ログを設計に週次還流し設計外経路を頻度で正常系昇格判定
- M3: **DEMP法**（Durable Execution Migration Path）— 自前タイマー実装からTemporal移行の6段階減衰マイグレーション

### 6. 2026年主要ツール（5+）
- Temporal Cloud（Durable Execution）
- Restate（軽量Durable Execution・エッジ対応）
- Camunda 8 SaaS（BPMN×DMN×プロセスマイニング統合）
- Celonis EMS（プロセスマイニング差分検知）
- LangGraph + Guardrails（エージェンティック・ワークフロー）
- Peppol Access Point（Storecove/Tickstar）

### 7. OVERSPEC KPI（5+）
- k_ovs1: Durable Execution移行率（工程数ベース、目標80%）
- k_ovs2: エージェンティック遷移のガード違反検知率100%・不正通過0件
- k_ovs3: 設計外経路の週次検知→正常系昇格リードタイム（14日以内）
- k_ovs4: Peppol/PINT受信イベントのdedup+順序ガード成功率99.99%
- k_ovs5: マルチテナント状態機械バリアントの共通部品再利用率70%以上
- k_ovs6: 建設業ピボット地点（電子契約締結）自動判定精度99%以上
