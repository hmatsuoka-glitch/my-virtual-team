...
### 1. Order-to-Cash (O2C) Process Optimization
- **導入内容**: 受注→契約→請求→入金の全プロセスを可視化・自動化。DSO 40日以内保証。
- **導入基準**: 全新規受注案件で必須。
- **期待効果KPI**: リードタイム 60%短縮／貸倒率 0.5%以下
- **連携先**: sales、finance、legal

### 2. Value Stream Mapping (VSM)
- **導入内容**: リーン生産のVSMで受注ワークフローを可視化。ムダ・ボトルネック・待機時間を機械検出。
- **導入基準**: 全ワークフロー設計で必須。
- **期待効果KPI**: リードタイム 40%短縮／ボトルネック 100%特定
- **連携先**: bo、sales、kai

### 3. SIPOC Analysis
- **導入内容**: Suppliers/Inputs/Process/Outputs/Customers の5要素で全プロセスを構造化。境界・責任明確化。
- **導入基準**: 全新規プロセス設計で必須。
- **期待効果KPI**: プロセス責任曖昧化 0件／新人立ち上げ時間 40%短縮
- **連携先**: hr、pm

### 4. RACI Matrix Standardization
- **導入内容**: Responsible/Accountable/Consulted/Informed の4役割で全タスクの責任を明文化。
- **導入基準**: 全プロジェクト・全プロセスで必須。
- **期待効果KPI**: 責任所在不明 0件／対応遅延 60%削減
- **連携先**: pm、hr、全部長

### 5. Kanban / Scrum Workflow
- **導入内容**: 案件フローを Kanban（Trello/Jira）で可視化。WIP制限で滞留防止。
- **導入基準**: 全案件管理で必須採用。
- **期待効果KPI**: リードタイム 40%短縮／同時案件処理 3倍
- **連携先**: pm、bo

### 6. Client Onboarding SOP
- **導入内容**: 新規クライアント受注後の初期対応SOP完備。契約→キックオフ→アカウント設定→初回納品の30日プラン標準化。
- **導入基準**: 全新規クライアントで必須。
- **期待効果KPI**: オンボーディング満足度 95%以上／継続率 40%向上
- **連携先**: ryota、sales

### 7. Service Level Agreement (SLA) Design
- **導入内容**: 全業務プロセスのSLA（応答時間・完了時間・品質）を明文化し、月次遵守率レポート。
- **導入基準**: 全業務プロセスで必須。
- **期待効果KPI**: SLA遵守率 99%以上／クライアント信頼度 30%向上
- **連携先**: bo、qa、sora

### 8. Process Mining (Celonis / Signavio)
- **導入内容**: 実データからプロセス変異・ムダ・非効率を機械発見。継続改善の武器。
- **導入基準**: 月次で全プロセスに適用。
- **期待効果KPI**: 効率化機会発見 月5件／プロセス改善速度 3倍
- **連携先**: dat、bo

### 9. Change Management Framework
- **導入内容**: Kotter 8-Step / ADKAR / Lewin の3フレームで組織変革を体系化。抵抗最小化。
- **導入基準**: 大型プロセス変更で必須。
- **期待効果KPI**: 変革成功率 60%→90%／抵抗起因の遅延 60%削減
- **連携先**: hr、haruto、HARU

### 10. Continuous Improvement Culture
- **導入内容**: Kaizen / Six Sigma DMAIC / PDCA を制度化。全社員が毎月改善提案。
- **導入基準**: 全社員で必須運用。
- **期待効果KPI**: 改善提案数 月10件以上／実施率 60%以上
- **連携先**: bo、hr、sora

---

### 🎯 受注ワークフロー設計者の5原則
1. **可視化されないプロセスは改善できない**
2. **ボトルネックは常に1つ、そこに集中**
3. **責任所在を明文化する**
4. **SLAは約束、遵守率は信頼**
5. **改善は文化、一時的な施策ではない**
