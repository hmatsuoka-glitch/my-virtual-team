...
### 2026-09-04
- **Kai（09-システム開発部PM）連携：受注ドメインの新規実装案件はBMAD要件定義の入口で標準遷移モデルCSV（09-01記録）を必須成果物として渡し、Nao（09-システム開発部の要件定義担当）の設計書には「案件固有の逸脱列」だけを差し込む形にする**：モデル既定値からの逸脱をレビュー対象に絞ると、Naoの設計時間が案件間で揃い、Boのhandoff（08-27記録）に載る遷移3区分（自動打刻／人手／人手＋承認）もCSV上で先に確定できる。KaiへはBMADのarchitect-checklistへ「逸脱列がゼロ埋めか／既定を上書きした理由が併記されているか」の項目追加を依頼し、Owl側の標準モデルが暗黙前提として設計フェーズに流れ込む状態を作る
- **Ao（09-システム開発部バックエンド）連携：遷移ガードと実行権限マトリクスのドメイン層一本化（09-02記録）は、Aoが実装するAPI・バッチ・管理画面すべてを同じ状態機械ファサード経由に強制するよう、Owl側から入口別テスト観点を先に渡す**：同一の不正遷移リクエストをAPI直叩き・CSV一括取込・管理画面登録の3経路で流し、いずれもドメイン層で同じエラーコードで弾かれることをTDDのRedフェーズで確認してもらう。Aoが入口ごとに検証をコピーし始める前に観点を渡すのがOwlの仕事で、Mio（09-システム開発部QA）のqa-gate通過条件にも同観点を組み込んでもらう
- **現場入力の遅延対策：遷移イベントは「発生時刻（現場でボタンを押した瞬間の自己申告値）」と「登録時刻（サーバー到達時刻）」を別フィールドで必ず持たせ、オフライン→復帰時の一括同期でも滞留監視（07-03記録）の数字が壊れないようにする**：現場は圏外や電池切れで数時間遅れて同期することが常態で、登録時刻だけで滞留を測ると全案件が事務所帰着で一気に「進んだ」ように見え、真のボトルネックが隠れる（08-16記録の入力遅れによる数字の嘘の根治策）。標準遷移モデルCSV（09-01記録）の「現場1タップ遷移」列にはオフライン許容フラグを既定trueにし、発生時刻はイベント本文で保持、Datへ渡す分位点分析（07-02記録）も発生時刻ベースで統一する
- **Qa（横断QAレビュアー）連携：プロセスマイニングで検出した設計外経路（08-03/08-05記録）は、Qaの5系統カバレッジ（07-16記録）「異常系30%」の母集合へ「未定義経路」として毎月末に定期供給する**：Datの頻度実測（08-27記録）で正常系昇格を判断する前段として、まずQaのテストケース母集合を実運用側から更新し、次リリース時に「どの経路が塞がれ／どれが昇格されたか」をQa側から差分検知できるようにする。母集合の妥当性を先に問うQaの流儀（07-16記録）へ、Owl→Qaで増分を流す常設ルートを敷けば、カバレッジ％の分母が実運用と乖離する事故を防げる
- **AdminOverride（08-12記録）の運用ルール整備：緊急対応での強制遷移イベントは、Sora（00-COO最終QA）承認を必須ゲートにし、発火から24時間以内に「なぜ通常遷移が使えなかったか」を標準遷移モデル（09-01記録）へのフィードバックとして書き戻す**：AdminOverrideを禁じ手にすると現場が直接UPDATEへ逃げて監査が壊れる（08-12記録）が、承認なしで自由に使えると常態化してモデル改善の機会が失われる。「必ずイベントとして残す」原則は守りつつ、発火にSora承認を挟み、事後原因を「モデル側へ還元」（09-01記録）のループへ回すことで、緊急対応が次案件の設計改善に自動接続する構造にする
