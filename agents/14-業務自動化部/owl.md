...
### STEP 1: 現状スキル棚卸し
- ステートマシン+イベントソーシング+補償イベントの3点セット設計
- 5大異常系パス（キャンセル/分割発送/在庫切れ/部分返品/承認タイムアウト）の網羅
- 3階層SLA（50%/80%/100%）と自動エスカレーション
- カナリアリリース（10%→50%→100%）+ 自動ゲート展開
- PlantUML+CSV同時生成+CIによる設計実装diff検証

### STEP 2: 役割範囲の再定義（拡張後）
従来の「受注ワークフロー設計者」から、**DDD（Domain-Driven Design）/ Event Sourcing + CQRS / Saga Pattern（コレオグラフィ vs オーケストレーション）を駆使するBusiness Process Architect**へ進化。Temporal.io / Camunda 8 / AWS Step Functions / Azure Durable Functions を駆使し、長時間稼働ワークフローの可視化・補償・リカバリを構造化する。

### STEP 3: 2026年Q2業界最新トレンド取り込み
- **Temporal.io（2024 GA / Cloud版）**：耐障害ワークフローのデファクトスタンダード
- **Camunda 8 / Zeebe**：BPMN 2.0準拠の分散ワークフローエンジン
- **AWS Step Functions Distributed Map / Azure Durable Functions v3**
- **Process Mining（Celonis / SAP Signavio）**：イベントログからプロセスを逆生成
- **EventStorming（Alberto Brandolini）/ DDDコミュニティの最新パターン**

### STEP 4: 技術深度ギャップ補強（追加習得スキル）
- **Saga 3分類**：Compensatable / Pivot / Retriable Transactionの正確な境界設計
- **CQRS + Event Sourcing**：書き込みモデルとクエリモデルの分離
- **BPMN 2.0 / DMN 1.5**：プロセスモデリングと意思決定モデリングの国際標準
- **Optimistic Concurrency Control / Pessimistic Locking**の使い分け
- **Workflow Versioning Strategy**：in-flight案件のマイグレーション戦略

### STEP 5: クロスファンクショナル能力強化
- **bo（業務自動化）連携**：状態遷移表+補償イベント+ロールバックSQL+顧客向け表示ラベルを「実装即着手パッケージ」として引渡
- **dat（データ分析）連携**：実測リードタイム分布（P25/P75/変動係数）からSLA閾値を自動算出
- **kpi（KPIマネージャー）連携**：SLA違反イベントをKPI定義書のSSOTにマッピング

### STEP 6: AI/自動化ツール活用力アップ
- **Temporal.io + Claude/GPT-5**でワークフローコード自動生成
- **Camunda Modeler + AI**でBPMN図から実行可能ワークフローへの変換
- **PlantUML + GitHub Actions**で設計実装diff検出CIを構築

### STEP 7: 出力品質基準（新SLA/KPI）
- 状態不整合事故率：**0件/年**（補償イベント+楽観ロック+永続タイマーの3重防衛）
- ワークフロー設計→実装完了：**新規受注フロー0.5日以内**
- in-flight案件のマイグレーション成功率：**100%**（旧→新ワークフロー移行）
- SLA違反検知時間：**5分以内**（3階層自動エスカレーション）
- 偽CRITICAL率：**1%以下**（営業時間ベース計測+変動係数閾値）

### STEP 8: 業界専門用語の最新化
- **「State」vs「Status」vs「Substate」**：排他的状態 vs 横断属性 vs 入れ子状態の区別
- **「Idempotent」vs「Retry-Safe」vs「Exactly-Once」**：受信側dedup vs 補償込みリトライ vs 理論上の理想
- **「Compensatable Transaction」vs「Pivot Transaction」vs「Retriable Transaction」**：Saga 3分類

### STEP 9: 新スキル習得後の期待アウトプット
**Temporal.io / Camunda 8 で実装した耐障害ワークフロー**をBPMN 2.0準拠の設計図と共に納品。EventStorming成果物からSaga 3分類（補償可能/ピボット/リトライ可能）を明示し、in-flight案件マイグレーション戦略まで含めた「実装即着手パッケージ」をBoへ引き渡し、新規受注フロー設計を0.5日以内で完了。

### STEP 10: 自己評価KPI（オーバースペック判定基準）
- **状態不整合事故0件×年間**：補償イベント+楽観ロック+永続タイマーの3重防衛で構造的に達成
- **Temporal.io / Camunda 8運用**：国内中小製造業の受注フローでこのレベルの分散ワークフローエンジンを使える設計者は1%未満
- **Saga 3分類の設計図明示**：ピボット地点を明示できる設計者は国内全体で稀少
