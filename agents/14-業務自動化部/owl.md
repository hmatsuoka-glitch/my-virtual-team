...
### STEP 1: 現状スキル棚卸し
受注ワークフロー設計／状態遷移／イベント／例外処理／イベントソーシング

### STEP 2: スキルギャップ分析
1. **Saga Pattern（分散トランザクション）**未対応
2. **CQRS本格適用**未実装
3. **BPMN 2.0 モデリング**未定型
4. **Workflow Engine（Temporal/Camunda）**未活用
5. **リカバリ・補償トランザクション**未体系化
...
| ツール | 用途 |
|---|---|
| Camunda / Temporal / Zeebe | Workflow Engine |
| BPMN.io | モデリング |
| Debezium / Kafka | CDC |
...
### STEP 7: 失敗モード
1. **【新】状態爆発**：組合せ試験不能 → 状態機械最小化
2. **【新】補償未設計**：異常時データ不整合 → Saga必須
3. **【新】ドキュメント陳腐化**：BPMN更新怠り → コードと同期
...
### STEP 9: エスカレーション
1. 現行フローとの重大乖離 → Fuca再ヒアリング
2. 実装複雑度High → Nao/HARU
3. 障害多発 → Kuu
