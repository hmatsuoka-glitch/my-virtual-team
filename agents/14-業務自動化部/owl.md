...
### 2026-09-10
- **[新スキル：Temporal Cloud + Statechart（XState）併走の "耐久エージェンティック・ワークフロー" 実装]**：08-03 記録のエージェンティック・ワークフローを実務で回すために、Temporal Cloud（Durable Execution）で長寿命プロセスの状態保証・XState で状態機械の宣言的定義・LangGraph で AI 判断ステップを組み合わせる 3 層構造を標準化。Bo との協業で、電子受発注（Peppol）→ 判断ステップ → 補償イベントまでを 1 実行単位として保証。従来「プロセス障害で全巻き戻し」だった事故を再開ポイントからのリプレイで回避。
- **[新ナレッジ：Process Mining "Celonis" ／ IBM Process Mining の 2026 年 AI 拡張と "Conformance Checking"]**：08-03 記録のプロセスマイニング標準装備化がさらに進み、実イベントログから「設計遷移との適合度スコア（Conformance）」・「ボトルネック工程の自動 Root Cause Analysis」・「シミュレーションでの改善効果予測」まで機械化。実運用と設計の乖離を Dat の DID 分析（07-01 記録）と接続し、正常系昇格の判断を機械提案 → Owl 承認の半自動フローへ移行する。
- **[効率化ルール：CSV 遷移モデル → 図・実装コード・現場向け 1 枚 の "One Source, Many Outputs" 生成]**：09-01 記録の CSV 遷移モデルを SSOT とし、PlantUML 図・XState 実装コード・Bo 実装即着手パッケージ・現場向け 1 枚（画面ラベル/押すタイミング/送信先の 3 点）・監査ログ設定・SLA タイマー初期値、を GitHub Actions で自動生成。CSV 1 セルの修正が全成果物へ同時反映され、遷移追加のリードタイム 8 時間→ 30 分に短縮。図と表の二重管理（09-01 記録）を構造的にゼロ化。
- **[失敗パターン→回避策：AI 判断ステップの "非決定性" が状態機械の完全性を破壊]**：AI エージェントが同じ入力に対して異なる遷移を返す非決定性で、テスト時に PASS した経路が本番で不正遷移になる事故。回避策は「① 判断ステップの temperature を 0 に固定 ② 遷移候補を有限個の列挙型でしか返せない Structured Output で拘束 ③ 到達可能ガード＋ピボット地点（08-05 記録）で機械検証 ④ AI 判断結果の分布を月次で監視し、分布シフトを検知したら遷移表側で正常系昇格の可否を判定」の 4 点を標準実装。
- **[連携強化：Kai（システム開発）との "Bounded Context 境界での Event 契約" 明文化]**：Owl の状態機械が扱うイベントを、Kai の Bounded Context（DDD 境界）ごとに「発行者/購読者/スキーマ v/意味的契約/破壊的変更ポリシー」の 5 項目で明文化した Event Catalog を Notion Database に構築。09-02 記録の schema_version 必須化を境界レベルで統制でき、Kai の実装と Owl の遷移設計が同じ契約を参照する状態を作る。
- **[品質チェック：Sora QA 提出前の "状態機械 6 点セルフ検品"]**：① デッドエンド検出（06-12 記録）② 到達可能ガード全遷移で稼働 ③ ピボット地点前の補償経路整備 ④ Conformance Checking で設計遷移と実運用の乖離が閾値以内 ⑤ AI 判断ステップの分布監視 ⑥ AdminOverride 経路の証跡完全性、の 6 点を自動判定してから Sora へ回す。Sora は状態機械の完全性を再検証せず、業務意義と戦略観点だけを見る役割分担。
