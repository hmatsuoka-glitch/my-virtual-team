...
### 2026-08-08
- **効率化テクニック：Temporal（Durable Execution／07-27記録）へのSLAタイマー・人間待ちステート・in-flight案件マイグレーションの移行テンプレを1本用意し、自前タイマー永続化コード（06-17記録）の運用負荷から解放**：Temporalの`Workflow`単位で状態機械を実装するとタイマー永続・再起動時復元・履歴保存が基盤で担保され、Owlは補償イベント・ピボット地点・dedup/順序ガード（06-24/07-01記録）といったドメイン設計に集中できる。Bo実装即着手パッケージ（07-07記録）の出力にTemporal Workflow雛形（Activity分解・Signal/Query設計・SearchAttributes定義）を含め、Boへの引き渡し完成度をさらに一段引き上げる。
- **効率化テクニック：エージェンティック・ワークフロー（08-03記録）のAI判断ステップは「候補遷移＋信頼度＋根拠テキスト」の構造化出力を必須にし、信頼度<0.8は自動でHuman-in-the-loop（Bo 08-03記録の承認共通UI）へ回す設計テンプレへ**：AIの返す遷移先を無条件採用してハルシネーションで到達不能状態へ飛ぶ事故（08-05記録）を、信頼度閾値と到達可能ガード（07-01記録）・ピボット地点（06-20記録）の二段防御で構造化。根拠テキストは追記専用ログ（Bo 08-03記録のオブザーバビリティ）に構造化保持し、監査で「なぜAIがこの遷移を選んだか」を遡及可能にする。決定論的処理はコードで固める（Bo 08-05記録）と厳密に分離し、金額計算・重複判定はAIに投げない。
- **品質チェックポイント：プロセスマイニング（08-03記録）で検出した「設計外の実運用経路」を四半期の設計レビューにインプットとして必須化し、静的グラフ走査（06-12記録）と動的な実運用ログの両輪で状態機械を進化させる**：設計時点で完璧な状態機械は存在しない前提で、実運用データから浮かぶ想定外滞留・回避経路・頻度上昇した異常系を四半期ごとに設計へ還流。Datへの頻度依頼（08-05記録）と組み合わせ、月◯件以上の実経路は正常系遷移へ昇格し、逆に3ヶ月間発火ゼロの遷移は「本当に必要か」を再検討して状態機械を筋肉質に保つ。設計と実運用の乖離をKPI化してレビュー継続の動機を仕組み側に持たせる。
- **失敗パターン：Peppol/JP PINT等の電子受発注（08-03記録）で受信側の順序ガード（07-01記録）を、送信元1系統のみを想定して実装し、複数取引先・複数プロトコル（Peppol/EDI/自社Webhook）が混在した時にシーケンス番号の名前空間衝突で正しい順序が判定できなくなる** → 回避策：シーケンス番号の一意性は「送信元識別子＋プロトコル＋シーケンス番号」の複合キーで管理し、送信元ごとに独立した順序判定を持つ。dedupキーの複合化（08-05記録）と同じ考え方を順序ガードにも適用し、Bo（08-08記録のWebhookレシーバ三段構成）へ引き渡す際は送信元別の順序判定ロジックを仕様化する。8/2026年後半以降の電子受発注拡大で送信元数が線形に増える前提で、名前空間分離を初期設計から組み込む。
- **連携ヒント：Sales/Bo/Pm/Kpi横断の「受注状態SSOT」を1つの状態機械イベントストリームに集約し、各エージェントが自エリアのビューだけを購読する運用へ**：Sales の受注handoff発火タイミング（Sales 07-16記録）・Bo の請求スケジュール取り消し（Bo 07-16記録）・Pm のマイルストーン更新（Pm連携）・Kpi の SLA発火/解消（07-16記録）が別々の状態管理を持つと、失注に戻った案件の請求スケジュールが Finance 側に残る（Bo 07-16記録）ような整合崩れが必発。Owl の状態機械を全社の「受注に関する真実の源泉」に固定し、各エージェントはイベントを購読して自エリアの派生状態を導出する（イベントソーシング／06-13記録の全社適用）。補償イベントの発火責任は Owl（オーケストレーション型／06-24記録）に集約し、下流エージェントは補償イベントを受信して自エリアを巻き戻すだけの受動側に徹する。
