...
- **Durable Execution エンジン設計（Temporal / Restate / Inngest）**：SLAタイマー永続化・再起動時復元・補償イベント発火責任を自前実装せずエンジンに委譲。ワークフロー定義をコードで宣言し、プロセス障害・デプロイをまたいで長寿命ワークフローの状態を保証する設計を標準化。受注→発注→出荷のような数日〜数週間かかる長寿命プロセスをTemporal Workflow SDKで実装するリファレンスをテンプレ化する。
- **エージェンティック・ワークフロー統合設計**：LLM判断ステップを状態機械の遷移ガードに組み込み、AIが返す非決定的遷移を「到達可能ガード＋ピボット地点マーキング（06-20記録）」で弾く二重防御を必須要件化。LangGraph / LlamaIndex Agent Workflows を Durable Execution 基盤の上に載せ、AI判断の再現性と監査可能性を確保する設計パターンを確立。
- **プロセスマイニング常時運用（Celonis / Apromore）**：静的なグラフ走査（06-12記録）と動的なイベントログ再構成の両輪運用を標準化。設計上使われないはずの経路・想定外滞留・実運用の遷移頻度分布をダッシュボード化し、設計と実運用の乖離を週次レビュー化。設計不備の先行シグナル検知を運用に落とす。
- **Peppol / JP PINT 電子受発注イベント統合設計**：デジタルインボイス受信を状態機械の第一級入口イベントとして扱い、メール/FAX起点からの完全移行を設計。at-least-once配信前提のdedup・順序ガード（06-24/07-01記録）を電子受発注メッセージに拡張し、外部ネットワーク由来の重複・順序逆転に耐える受信側防御を組む。
- **Outbox / Inbox パターン厳密実装**：分散環境でのメッセージロスト・重複を構造的に防ぐため、DBトランザクションと外部イベント発行の atomicity を Outbox テーブル経由で保証。受信側は Inbox で dedup を強制し、Boへの引き渡しパッケージ（07-07記録）にテンプレとして常時同梱。
- **XState v5 / Stately Studio 活用の状態機械設計**：PlantUML＋CSV（05-26記録）に加えて XState 定義を単一ソースとし、ビジュアル編集・自動テスト生成・実装コード自動出力を一気通貫化。設計→レビュー→実装引き渡しの往復を最小化。
- **MCP経由の外部システム連携設計**：スクレイピング依存を減らし公式API/MCPへ寄せる流れ（07-27記録）の実装標準化。データ源の単一障害点化を回避するMCP接続台帳・フェイルオーバー設計をルール化。
- **電子契約締結の「ピボット地点」自動判定設計**：電子署名イベント（08-03記録）を Saga 3分類（補償可能/ピボット/リトライ可能・06-20記録）の境界として自動マーキングし、締結前後の遷移可逆性を機械的に判定するテンプレを整備。

---

## 📊 Quality Framework 定量指標

- **k1_lead_time_p95_reduction_rate**：受注リードタイム P95 の月次短縮率。目標 -10%/月（改善サイクル中）、-2%/月（定常運用時）。営業日カレンダー演算（06-03記録）ベースで計測、待ち時間分解（07-01記録）と併記。
- **k4_sla_violation_count**：3階層エスカレーション（50/80/100%・05-22記録）ごとの月次違反件数。目標 CRITICAL 0件/月、ALERT ≤5件/月、WARNING は工程改善の先行指標として増減トレンド管理。KPI SSOT 定義ID参照（06-04/07-16連携）で通知一本化。
- **k7_compensation_event_success_rate**：補償イベント発火成功率と外部副作用打ち消し完了率（06-17/06-23記録）。目標 99.9%以上、失敗時は自動リトライ＋人手エスカレーション記録を義務化。
- **k12_dedup_hit_rate / order_violation_detection_rate**：dedup による重複排除ヒット率と順序逆転検知率（06-24/07-01記録）。at-least-once環境の異常度合いを可視化し、送信元別に閾値超過を検知。
- **k18_design_impl_diff_zero_rate**：設計表と packages/domain 実装enumのdiff ゼロ達成率（06-12記録）。目標 100%（CI必須ゲート）。差分検出時はレビュー着手をブロックし、リリース前の片側残存を構造排除。

---

## 🔬 2026 業界ベストプラクティス

- **Durable Execution の標準採用（Temporal / Restate 系）**：SLAタイマー永続化・再起動時復元・補償発火責任を自前実装から基盤側へ移譲する流れが受注ドメインで実装フェーズに到達（07-27記録）。長寿命プロセスの状態保証を自作するアンチパターンを卒業し、Workflow SDK でコードとして宣言する設計が業界標準化。
- **AI判断ステップとピボット地点の明示的分離**：エージェンティック・ワークフロー（08-03記録）の実装で「AI が返す遷移は非決定的」を前提に、到達可能ガードとピボット地点マーキング（06-20記録）で不正遷移を弾く二重防御が必須要件化。AIをワークフローに載せる案件は必ずこの二重防御をレビュー項目にする。
- **Outbox Pattern + at-least-once 前提の受信側防御**：exactly-once は存在しない前提（06-13記録）でのdedup・順序ガード（06-24/07-01記録）が業界の共通言語化（07-27記録）。Outbox でトランザクション整合を保ちつつ、Inbox で dedup を強制する設計が受注系の標準実装として定着。
- **プロセスマイニング常時稼働による設計-実運用ギャップ検知**：実イベントログから実際の状態遷移パスを再構成し、設計上は使われないはずの経路・想定外滞留を自動検出するツールが標準装備化（08-03記録）。静的グラフ走査と動的プロセスマイニングの両輪運用が品質検証のベースラインに。
- **デジタルインボイス（Peppol / JP PINT）対応の状態遷移組込**：受注→請求の入口イベントが電子受発注メッセージ受信に置き換わる流れ（08-03記録）。PDF/FAX授受を前提とした状態機械は「構造化データ受信を第一級イベント化」する再設計が必要になり、Peppol対応が受注系の必須設計要件へ格上げ。
- **電子契約締結を機械的ピボット地点として設計**：紙・押印前提の契約ステップが電子署名イベント（08-03記録）に置き換わることで、締結前後の補償可逆性をシステムが自動判定できる時代に。契約締結を Saga 3分類の境界として自動マーキングする設計が受注フローの信頼性を上げる標準実装として広がる。
