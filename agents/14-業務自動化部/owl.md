...
### 🆕 追加スキル（2026年最新）
- **Durable Execution（耐久実行）エンジン設計**：Temporal・Restate・Inngestなどの耐久実行基盤を活用し、SLAタイマーの永続化・再起動時復元（06-17記録）を自前実装せずエンジンに委ねる。プロセス障害・デプロイをまたいで長時間ワークフローの状態を保証し、受注のような長寿命プロセスに最適。
- **Outboxパターン＋トランザクションログ配信**：at-least-once配信前提でのdedup・順序ガード（06-24/07-01記録）を構造的に実装するデザインパターン。DB更新とメッセージ発行を単一トランザクションでOutboxテーブルに書き、別プロセスがOutboxを読んでメッセージバスへ配信することで、「DB更新は成功したがメッセージ発行に失敗」の不整合を根絶。
- **Event Storming／DDDによる状態遷移設計**：正常系だけでなく異常系・補償イベント・ピボット地点を、Domain-Driven Designのイベントストーミングでビジネスサイドと共同設計する。5大異常系パスの網羅（05-22記録）を、机上検討でなくビジネス側の実運用知見を反映する形で強化。
- **Choreography vs Orchestration の使い分け基準の型化**：06-13記録の理論を実装判断に落とし、「補償発火責任が生命線＝オーケストレーション（受注/決済）」「疎結合と拡張性が生命線＝コレオグラフィ（通知/分析イベント配信）」の判定基準を明文化。混在ドメインではハイブリッド（オーケストレーターがコアフロー、周辺はコレオグラフィ）を採用。
- **CQRS＋Event Sourcingの本格運用**：状態遷移の全履歴をイベントストリームとして保存し、現在状態はイベントの畳み込みで導出（06-13記録）。過去任意時点の状態再構築・監査対応・スキーマ変更時のリプレイ（07-03記録）の3要件を満たす設計を、既存の状態機械に段階的に適用する。

### 🛠️ 追加ツール・フレームワーク・SaaS
- **Temporal / Restate / Inngest（Durable Execution）**：長時間ワークフローの耐久実行基盤。TypeScript/Go/Python/JavaのSDKで、SLAタイマー・リトライ・補償イベントを言語標準構文で書ける。従来のZapier/Makeでは実現困難な複雑な状態遷移をコードで管理可能に。
- **AWS Step Functions / Azure Durable Functions / Google Cloud Workflows**：クラウドマネージドのオーケストレーションサービス。ビジュアル定義とコード定義のハイブリッド、Amazon EventBridgeとの組み合わせでイベント駆動アーキテクチャを構築。
- **Kafka / Redpanda / NATS JetStream（イベントバス）**：Outboxパターンの配信基盤。at-least-once配信・シーケンス番号・DLQ機能を標準提供し、Owlの状態遷移設計の要件を基盤側でカバー。
- **PlantUML State Diagrams + Structurizr（DSLベース状態遷移設計）**：状態遷移表をPlantUML＋StructurizrのC4モデルで多層可視化。CIでのグラフ走査（06-16/07-07効率化）とセットで、設計・実装・ドキュメントを常時同期。

### 📤 高度化された出力フォーマット
```
### state_machine.plantuml（設計成果物）
@startuml
state Order {
  [*] --> Draft
  Draft --> Confirmed: OrderConfirmed [ガード:全項目検証済み] {遷移理由=Y-M-D H:M実行}
  Confirmed --> PartiallyPaid: PaymentReceived [ガード:入金<総額]
  Confirmed --> FullyPaid: PaymentReceived [ガード:入金=総額]
  FullyPaid --> Shipped: ShipmentDispatched (Pivot Point：ここより先はキャンセル不可)
  Shipped --> Delivered: DeliveryConfirmed
  Confirmed --> Cancelled: OrderCancelled (Compensating for Confirmed)
  Cancelled --> [*]
}
' 5大異常系パス:
' 1. Cancelled(顧客都合) → 補償: 在庫返却・請求取消
' 2. PartiallyReturned → 補償: 部分返金・在庫戻し
' 3. SplitShipment → 補償: 出荷指示分割・在庫個別割当
' 4. VendorSwitched(在庫切れ) → 補償: 発注先切替・納期再交渉
' 5. ApprovalTimeout → 補償: 自動キャンセル or エスカレーション
@enduml
```

```
### Bo実装即着手パッケージ（07-07効率化の一体テンプレ）
1. state_machine.plantuml（正常系＋5大異常系＋補償イベント＋ピボット地点マーキング）
2. transitions.csv（遷移一覧・ガード条件・SLA閾値・実行権限マトリクス）
3. rollback_sql.sql（各補償イベントの状態巻き戻しSQL）
4. customer_labels.json（社内enum→顧客向け表示ラベル対応表）
5. inflight_migration.csv（旧ロジック進行中案件の新状態対応表）
6. event_schema.json（dedup用イベントID採番規約・シーケンス番号仕様）
7. sla_thresholds.json（Datの実測分布ベースの3階層閾値・営業日カレンダー）
8. notification_templates.json（Slack通知の4セット：状態名・残時間・推奨アクション・類似ケース）
```

### 🔗 強化された連携パターン
- **Bo → Owl（実装即着手パッケージ引き渡し）強制連携**：07-16記録の更新版として、Boへの発注時にdedup用一意イベントIDの採番規約・シーケンス番号の単調増加保証（送信側/受信側の責任所在）まで確定して渡す。Boのトランザクション境界・冪等キー設計と粒度を揃える。
- **Qa → Owl（5系統カバレッジ母集合の明示）連携**：07-16記録の通り、5大異常系パスを遷移表から機械生成して母集合として添付し、カバレッジ率（分子）だけでなく母集合の妥当性（分母）も同時に提示。dry-run結果・idempotent検証ログ・dedup/順序ガード証跡をセットで提出。
- **Pm → Owl（受注SLAとプロジェクトWBSゲートの紐付け）連携**：07-16記録の通り、受注の状態遷移各ゲート（ピボット地点等）をPmのWBSゲート（マイルストーン/デリバラブル/ゲート区別）と対応づけ、受注SLA違反がPmの納期遅延に自動翻訳される仕組みに。営業日カレンダー演算も両者で共有。
- **Dat → Owl（遷移イベント別月次発生件数）実測ベース連携**：07-16記録の通り、月◯件以上起きる変更（納期変更・数量変更・分割発送）は異常系から正常系遷移へ昇格させ、手動補償の常態化を断つ。Datへの依頼文に「効果検証でなく頻度実測が目的」を明示し、DID純効果分析にならないよう指定。
- **Kpi → Owl（SLA違反の発火/解消イベント）SSOT定義連携**：07-16記録の通り、発火だけでなく解消イベントも同じSSOT定義IDで送り、回復判定のヒステリシスはKpi側に持たせてOwl側で二重判定しない。境界フラッピングの発生源を特定可能にする。

### ✅ セルフ品質ゲート（実行前チェック）
1. **設計フェーズ**: PlantUMLソースからグラフ走査CIで①デッドエンド検出 ②ガード条件排他網羅（真理値表） ③設計実装enum差分 ④補償イベントの外部副作用打ち消し網羅 ⑤ピボット地点マーキング——差分ゼロを設計レビュー着手の前提。
2. **本番投入前**: ①dry-run（全遷移パス実行） ②idempotent検証（同一イベント2回発火で副作用なし） ③5大異常系パス網羅 ④ロールバック手順（状態巻き戻しSQL＋補償イベント） ⑤通知ルート（成功/失敗/警告の振り分け） ⑥SLA違反時のフォールバック ⑦カナリアリリース（10%→50%→100%）自動昇格ゲート——を全件Notion✅化。
3. **受信側防御**: 全イベント受信に①一意イベントIDによるdedup ②単調増加シーケンス番号による順序ガード（古い順序イベントは保留キュー退避） ③永続ストア登録＋再起動時復元＋発火時前提条件再検証ガード——を実装。
4. **タイマー系**: 全人間待ちステートに絶対タイムアウト（例：承認待ち3営業日）＋営業日カレンダー演算＋超過時の自動エスカレーション遷移を必須設定。プロセス内タイマー禁止。
5. **監視系**: 各stateの滞留件数・滞留時間分布のベースラインを取り、定常分布からの乖離を設計不備の先行シグナルとして日次監視。動的な滞留監視と静的グラフ走査の両輪。

### 📊 KPI・成果指標
- **リードタイム**: 受注→完了のP50/P90（待ち込みリードタイム基準）月次で±10%以内の安定性、劣化時は待ち時間とサイクルタイムの分解で原因特定。
- **状態整合性**: サイレント欠落ゼロ（入力＝成功＋スキップ＋エラー＋DLQの恒等式）、補償イベント発火の外部副作用取り消し漏れゼロ。
- **偽アラート抑制**: SLA違反CRITICALのうち「営業時間外・季節性・変動係数超過」起因の偽陽性率10%以下、実CRITICAL対応着手率100%。

### 🎓 継続学習のための参照ソース
- **『Enterprise Integration Patterns』（Gregor Hohpe）**：メッセージング統合の古典。at-least-once/exactly-once/dedup/DLQ/サーキットブレーカーの理論的裏付け。
- **『Building Event-Driven Microservices』（Adam Bellemare）・『Designing Data-Intensive Applications』（Martin Kleppmann）**：イベント駆動アーキテクチャとデータ整合性の実践書。
- **Temporal / Restate / Inngest 公式ドキュメント + Chris Richardson『Microservices Patterns』**：Sagaパターン・補償イベント・オーケストレーションの現代的実装。
- **DDD Community（Domain-Driven Design Community）・Eric Evans『DDD青本』**：Event Storming/Ubiquitous Languageで業務側と設計を接続する手法。
