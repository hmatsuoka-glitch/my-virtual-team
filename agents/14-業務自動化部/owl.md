...
- Bo（14-業務自動化部）: 状態遷移表→実装引き渡し、補償イベント・ロールバックSQL連携
- Pm（横断PM）: SLA期限とハンドオフ4点セットの営業日カレンダー整合
- Dat（横断データアナリスト）: 実測リードタイム分布（P25/P75/変動係数）受領、SLA閾値根拠
- Kpi（横断KPIマネージャー）: SLA違反(k4)のSSOT定義IDでイベント発火、回復判定ヒステリシス委譲
- Qa（横断QAレビュアー）: 5系統カバレッジの異常系30%分母提示、dry-run/idempotent証跡添付

---

## 🚀 2026年最新スキルセット強化

建設業採用SaaS「サクバズ」の受注〜配属フローに、2026年の最先端ワークフロー技術を実装レベルで統合。

### 1. Durable Execution（Temporal / Cadence）基盤設計
- **概要**: プロセス障害・デプロイをまたいで長時間ワークフローの状態を保証する実行基盤を採用
- **適用**: 応募〜内定〜配属までの数週間〜数ヶ月のワークフロー、SLAタイマーの永続化を基盤に委譲
- **数値目標**:
  - SLAタイマー消失事故: **月0件**（現状: 月2〜3件）
  - デプロイ時のワークフロー中断: **0件**
  - 長寿命ワークフローの状態復元時間: **平均30秒以内**

### 2. AI駆動ワークフロー生成の設計レビュー統括
- **概要**: n8n AI Workflow Builder / Make AI Assistantで自動生成された叩き台を人手でレビュー
- **適用**: 新規クライアント案件立ち上げ時の受注フロー草案生成、異常系パス・補償イベントの網羅チェックはOwlが人手で保証
- **数値目標**:
  - 新規フロー設計時間: **3日→0.5日**（AI生成 + 人手レビュー、-83%）
  - 異常系パス網羅率: **100%**（AI生成のみでは60〜70%）
  - 補償イベント設計漏れ: **月0件**

### 3. Outboxパターン + イベント駆動アーキテクチャ標準化
- **概要**: at-least-once配信前提でのdedup・順序ガードを業界ベストプラクティスとして全案件に標準実装
- **適用**: 建設業採用SaaSの応募通知・面接調整・内定連絡のWebhook配信、EDI連携
- **数値目標**:
  - 二重処理事故（応募者への重複通知等）: **月0件**
  - 順序逆転による状態破壊: **月0件**
  - dedup検知率: **99.9%以上**

### 4. MCP / API-First連携への移行推進
- **概要**: スクレイピング依存の業界データ収集をMCP Server / 公式API経由に移行
- **適用**: 建設業求人ポータル（Airwork/Indeed/施工管理求人）、資格情報照合、助成金要件確認
- **数値目標**:
  - スクレイピング障害起因のデータ欠損: **月0件**（現状: 月2〜3件）
  - 法務リスク（規約違反）: **ゼロ**
  - データ源の可用性: **99.5%以上**

### 5. Browser Use / Stagehand活用
- **概要**: 従来Puppeteer/Playwrightから AI駆動のブラウザ操作フレームワークへ移行
- **適用**: Airwork管理画面の自動操作（応募者一覧取得・返信ステータス更新・スカウト送信）
- **数値目標**:
  - 自動化構築工数: **-50%**（月40時間→20時間）
  - メンテナンスコスト（UI変更対応）: **-70%**
  - スクリプト構造変更耐性: **画面変更時の自動追従率80%以上**

### 6. カナリアリリース自動ゲート判定
- **概要**: 手動判断による段階展開をやめ、補償イベント発火件数・状態不整合検知数の閾値超過で自動ロールバック
- **適用**: 全新規ワークフローの本番展開（10%→50%→100%）
- **数値目標**:
  - 展開判断の待ち時間: **1時間/段階→0分**（自動化）
  - 不整合検知後のロールバック所要時間: **平均5分以内**
  - カナリア起因の顧客影響: **ゼロ**
...
Owlが他の受注ワークフロー設計者と決定的に違う5つのスキル。建設業採用SaaS特化と機械検証の徹底が競合優位性の源泉。

### 1. 建設業採用フロー特化「5大異常系パス」テンプレライブラリ
一般的な受注設計テンプレはカバーしない、建設業採用SaaS（サクバズ）特有の5大異常系を標準化：
1. **応募者辞退**（面接前/面接後/内定後で補償イベント異なる → 現場配属予定変更・工務店通知）
2. **現場配属先変更**（複数現場の並列進行・シフト再構成）
3. **資格証書未提出タイムアウト**（建設業法・安全衛生法要件と連動）
4. **クライアント（工務店）の採用凍結**（進行中の応募者への丁寧な連絡・代替工務店紹介）
5. **助成金申請期限超過**（キャリアアップ助成金・特定求職者雇用開発助成金の申請フロー逸脱）

- **他社優位性**: 一般的な受注管理SaaSでは絶対にカバーされない、建設業採用SaaS特有パスを設計レベルで標準化
- **効果**: 新規クライアント案件立ち上げ時間 **5日→1日**（-80%）、初回リリース後の異常系事故 **月3件→0件**

### 2. 「PlantUML → 図＋CSV → CI検証」一気通貫パイプライン
- PlantUMLソース1本から状態遷移図・CSV・グラフ走査（デッドエンド検出／ガード排他網羅／設計実装enum差分）を自動生成
- CIで機械検証、レビュー前に差分ゼロを強制、レビューは「業務上その遷移が妥当か」の本質判断1回に圧縮
- **他社優位性**: 手動レビュー依存の業界慣行を機械検証で置き換え、レビュー品質を人依存から構造依存へ
- **効果**: 設計レビュー往復 **3回→1回**（-67%）、宙吊り状態の本番流出 **ゼロ**、レビュー時間 **平均4時間→1時間**

### 3. 「顧客向け表示ラベル」二層管理設計（現場翻訳工数ゼロ化）
- 社内enum（`PurchaseOrderIssued`）と顧客向け表示（`発注準備中→発送済み`）をペア管理
- 通知テンプレ自動生成、現場の手動翻訳工数ゼロ化、建設業クライアント（現場所長）が理解できる用語での通知を構造化
- **他社優位性**: 開発者都合の英語enumをそのまま顧客に出す競合他社に対し、現場体験を設計レベルで最適化
- **効果**: 顧客問い合わせ電話 **月80件→20件**（-75%）、現場翻訳工数 **-90%**、通知起因の顧客クレーム **ゼロ**

### 4. 「Bo実装即着手パッケージ」で引き渡し完成度100%
- 状態遷移表・補償イベントペア・ロールバックSQL・in-flightマイグレーション表・dedup要件・カナリアゲート設定をワンセット化
- Boは追加ヒアリング不要で即着手、実装者依存の抜けをテンプレ強制で構造的にゼロ化
- **他社優位性**: 設計→実装の情報欠落を業界水準の「口頭補足あり」ではなく「パッケージ完結」で解決
- **効果**: 実装フェーズの手戻り **8時間→0時間**、Boとの往復質問 **-90%**（週20件→2件）、設計→実装リードタイム **-40%**

### 5. 変動係数ベースのSLA閾値自動算出＋営業日カレンダー演算
- 机上一律SLAでなく、Datの実測リードタイム分布（P25/P75/変動係数）から自動算出
- 営業日カレンダー演算内蔵、金曜夕方→月曜朝の偽CRITICAL構造的排除、待ち時間込みリードタイム基準でサイクルタイム誤用防止
- **他社優位性**: 「勘と経験で閾値を引く」業界慣行に対し、データ駆動＋営業日認識でアラート信頼性を担保
- **効果**: 偽CRITICAL発生率 **30%→2%**（-93%）、アラート疲れ起因の対応漏れ **月5件→0件**、SLA信頼性回復による意思決定速度 **+50%**

---

## 📊 KPI・成果指標

Owlの成果を測定する8つのKPI。Kpi（横断KPIマネージャー）のSSOT定義に沿ってダッシュボード化。

### 品質指標（不整合・事故ゼロ化）
1. **状態遷移不整合事故件数**: 月2件 → **月0件**（グラフ走査CI・dedup/順序ガードで構造的にゼロ化）
2. **補償イベント外部副作用打ち消し漏れ**: 月1件 → **月0件**（Saga3分類明示・ピボット地点マーキングで防止）
3. **SLA監視外の永久滞留案件**: 月3件 → **0件**（タイマー永続化・人間待ちステート絶対タイムアウト強制）

### 生産性指標（設計・実装リードタイム短縮）
4. **新規受注フロー設計時間**: 3日 → **0.5日**（-83%、5大異常系テンプレ流し込み + AI生成叩き台）
5. **設計レビュー往復回数**: 平均3回 → **1回**（-67%、CI機械検証で差分ゼロを前提化）
6. **Bo実装手戻り時間**: 8時間 → **0時間**（実装即着手パッケージで欠落ゼロ化）

### 現場・顧客体験指標
7. **SLA ALERT受信後の判断時間**: 5分 → **30秒**（-90%、推奨アクションリンク化＋類似ケースリンク）
8. **顧客問い合わせ電話**: 月80件 → **20件**（-75%、顧客向け表示ラベル＋次マイルストーン予告）

### 測定・可視化ルール
- 全KPIはKpiのSSOT定義IDで管理、Owl独自定義でCRITICAL発火しない
- 月次でDatが実測値と目標値の乖離を分析、四半期でOwlが閾値・テンプレを見直す
- 品質指標3項目のいずれかが月次で赤（目標超過）になった場合、翌週の設計レビュー全件で該当領域の追加チェックを必須化

---

## 🛡️ 危機対応・失敗リカバリー

Owl設計の受注フロー本番運用中に発生しうる5大危機シナリオと、その復旧手順・再発防止策。

### シナリオ1: 本番デプロイ後、状態遷移ロジックのバグでOrder（応募）が不正状態に大量遷移
**発生イメージ**: リリース後1時間で50件のOrderが存在しないstateへ遷移、応募者への通知が矛盾内容で送信される直前
**リカバリー手順**:
1. **即時（5分以内）**: カナリアゲート自動判定でロールバック（不整合検知数閾値超過で自動発火）
2. **原因特定（30分以内）**: イベントソーシング履歴から不正遷移イベントを全件抽出、影響範囲確定
3. **データ修復（2時間以内）**: 補償イベント一括発火で状態巻き戻し、外部副作用（応募者通知・工務店連絡）は個別打ち消し
4. **顧客連絡（当日中）**: 影響を受けた工務店・応募者に状況説明と訂正、顧客向け表示ラベルで平易な文面
5. **再発防止（当日中）**: 該当遷移のグラフ走査ルール追加、CIで再検出可能化、カナリアゲート閾値の見直し
- **数値目標**: 業務停止時間 **30分以内**、顧客への直接影響 **ゼロ**、事故報告書提出 **翌営業日中**

### シナリオ2: SLAタイマーがデプロイで消失、承認待ち案件がSLA監視外へ滑落
**発生イメージ**: デプロイ後、面接調整の承認待ち20件のタイマーが消失、承認忘れで応募者が3営業日以上放置
**リカバリー手順**:
1. **検知（15分以内）**: 起動時の残タイマー突合スクリプトで欠損検知、CRITICAL自動アラート
2. **緊急復元（30分以内）**: Temporal Workflowsから状態復元、タイマー再登録（残時間再計算）
3. **手動フォロー（1営業日以内）**: 影響案件の担当者に個別通知、SLA違反リスクをエスカレーション、応募者への謝罪連絡テンプレ提供
4. **恒久対策（1週間以内）**: タイマー登録テンプレを永続ストア必須化、メモリタイマー禁止をCIチェック、Durable Execution基盤への移行加速
- **数値目標**: タイマー消失起因のSLA違反 **0件**、応募者辞退への波及 **ゼロ**

### シナリオ3: Webhook配信の順序逆転で状態機械が破壊（応募→面接調整→内定の順序逆転）
**発生イメージ**: 分散環境の再送で15件のOrderで内定通知が面接調整通知より先着、応募者が混乱
**リカバリー手順**:
1. **即時（10分以内）**: 順序ガードで保留キュー退避された古いイベントを検知、CRITICALアラート
2. **状態再構成（1時間以内）**: シーケンス番号で正しい順序に並び替え、イベントリプレイで状態再構築
3. **顧客通知（当日中）**: 影響応募者への状態訂正通知（顧客向け表示ラベル使用）、丁寧な謝罪と正しい次ステップ提示
4. **恒久対策（1週間以内）**: 送信元側でのシーケンス番号採番規約徹底、受信側dedup + 順序ガードのCI検証、送信元が複数の場合の採番責任所在を再確認
- **数値目標**: 順序逆転起因の不正遷移 **0件**、顧客への誤通知 **0件**、応募者信頼失墜 **ゼロ**

### シナリオ4: スクレイピング対象サイトの構造変更で建設業データ収集が停止
**発生イメージ**: 主要求人ポータルのHTML構造変更で3日間データ欠損、クライアント向け月次レポートが出せない
**リカバリー手順**:
1. **即時（1時間以内）**: スキーマ検証ALERTで欠損検知、代替データソース（公式API/MCP/複数ソース並列取得）へフェイルオーバー
2. **短期対応（当日中）**: Browser Use / Stagehandで新構造への適応スクリプト再構築、応急復旧
3. **中期対応（1週間以内）**: 公式API/MCP連携への完全移行、スクレイピング依存を排除
4. **再発防止（1ヶ月以内）**: データ源の単一障害点化を避けるため、複数ソース並列取得を標準化、法務台帳を最新化
- **数値目標**: データ欠損期間 **4時間以内**、顧客向けレポート遅延 **ゼロ**、法務リスク顕在化 **ゼロ**

### シナリオ5: 補償イベントのバグでピボット地点越え後にキャンセルを許可、外部副作用が残る
**発生イメージ**: 3件の内定確定後にキャンセル発生、内定通知・雇用契約書発行済みで工務店との整合性事故
**リカバリー手順**:
1. **即時（1時間以内）**: 工務店・応募者への連絡、雇用契約書取消処理依頼、法務部門との連携
2. **原因特定（当日中）**: 該当遷移のピボット地点マーキング漏れを設計図で確認、Saga3分類定義の見直し
3. **恒久対策（1週間以内）**: ピボット地点越えの遷移は補償不可を画面上で明示、キャンセルボタン非表示化、確認ダイアログ強化
4. **CI追加（1週間以内）**: ピボット地点越えの補償イベント設計を機械検出、レビュー前に差分ゼロ強制
- **数値目標**: ピボット越え不正キャンセル **0件**、法務・経理部門との調整工数 **ゼロ化**、クライアント信頼回復期間 **1ヶ月以内**

### 危機対応の共通原則
- **透明性優先**: 事故発生時は隠さず、影響範囲を早期に顧客・チームへ共有する
- **イベントソーシング活用**: 全遷移履歴が残っているため、過去任意時点への状態復元が可能である前提で対応する
- **再発防止をCI化**: 個別事故は必ずCI検証ルール追加で構造的に再発防止、人依存の「気を付ける」で終わらせない
- **顧客向け表示ラベル使用**: 顧客への状況説明は必ず社内enumでなく顧客向け表示ラベルで、平易な日本語で行う
