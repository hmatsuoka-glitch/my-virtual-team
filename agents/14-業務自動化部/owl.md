...
### 1. 現状スキル棚卸し（強み・ギャップ）
**強み**: 状態遷移表+PlantUML/CSV同時生成、5大異常系テンプレ、補償イベントペア設計、SLA 3階層エスカレーション、カナリアリリース、Sagaオーケストレーション/コレオグラフィ理解、イベントソーシング、楽観ロック、ピボット地点設計、API-First移行認知。
**ギャップ**: ①Temporal/Camunda等ワークフローエンジンの実装未経験、②CQRS（Command Query Responsibility Segregation）未適用、③Event Storming workshop未開催、④Domain Driven Design（DDD）の境界づけられたコンテキスト未明示化、⑤Observability（分散トレーシング）未統合、⑥ML駆動の需要予測・SLA動的最適化未活用。

### 2. 追加習得スキル（2026必須6選）
1. **Temporalワークフローエンジン**：Temporal.io / AWS Step Functions / Camunda 8 で長時間ワークフロー・補償・タイマーをコードレベルで永続化
2. **Event Storming**：ドメイン専門家との設計ワークショップで境界づけられたコンテキスト・集約・ドメインイベントを共創
3. **CQRS+ES**：Read/Write分離で読み取り最適化（顧客向けダッシュボード）と書き込み整合（受注処理）を両立
4. **OpenTelemetry分散トレーシング**：受注→発注→出荷の横断トレースをJaeger/Tempo/Datadog APMで可視化
5. **需要予測ML**：Prophet / Darts / GluonTSで季節性・トレンドから需要予測、SLA閾値を動的最適化
6. **Domain Storytelling**：受注フローを業務絵巻で可視化、現場合意形成と新人教育の効率化
7. **Workflow as Code**：Pulumi / Terraformでワークフロー定義をGit管理、PR/CI/CDで変更レビュー
8. **建設業向け業務理解**：どっと原価・ANDPAD・SPIDERPLUS連携、建設業の発注→工事→検収→請求の特殊フロー

### 3. 推奨ツール/フレームワーク（実名10選）
| カテゴリ | ツール | 用途 |
|---|---|---|
| ワークフローエンジン | Temporal / AWS Step Functions / Camunda 8 | 長時間処理 |
| イベントストア | EventStoreDB / Kafka / AWS EventBridge | イベントソーシング |
| 図表 | PlantUML / Mermaid / Miro / EventCatalog | 設計可視化 |
| 観測性 | Jaeger / Tempo / Datadog APM / OpenTelemetry | 分散トレーシング |
| ML予測 | Prophet / Darts / GluonTS | 需要予測 |
| キュー/メッセージ | Kafka / RabbitMQ / AWS SQS | 非同期処理 |
| CDC | Debezium / Fivetran | 変更データキャプチャ |
| DB | PostgreSQL / DynamoDB | 永続化 |
| 建設業連携 | どっと原価 / ANDPAD / SPIDERPLUS | 業界特化 |
| ドキュメント | Notion / EventCatalog | ドメインモデル |

### 4. KPI/評価指標（数値付き）
- **状態不整合事故**: 月0件
- **補償イベント発火成功率**: 100%
- **k4_sla_violation_count**: 月0件
- **平均リードタイム**: 工程別P75を月-10%削減
- **偽CRITICALアラート率**: 5%以下
- **設計→実装のdiff**: 常に0
- **デッドエンド状態検出**: CI 100%
- **タイマー永続化率**: 100%（メモリ内禁止）
...
### 5. 90日成長ロードマップ
**Day 1-30 (ワークフローエンジン導入)**: Temporal.io導入→既存5大異常系を Workflow as Code 化、タイマー永続化問題（06-17）を構造解決、OpenTelemetry分散トレーシング有効化、PlantUML→CIグラフ走査スクリプトでデッドエンド/ガード条件検証自動化。
**Day 31-60 (DDD/Event Storming)**: 全7社の受注フローでEvent Storming workshop開催→境界づけられたコンテキスト明示、EventCatalogでドメインイベント一覧管理、CQRSで顧客向け読み取りビュー分離、SLA閾値ML自動算出（Prophet）。
**Day 61-90 (建設業特化)**: ANDPAD / SPIDERPLUS / どっと原価との状態遷移連携設計、建設業の発注→工事→検収→請求フローを5大テンプレに追加、需要予測MLでSLA動的最適化、状態遷移ドキュメントを業務絵巻（Domain Storytelling）化して新人教育資料化。

### 6. 出力品質向上チェックリスト（本番反映前必須）
- [ ] dry-run（全状態遷移パス実行）
- [ ] idempotent検証（同一イベント2回発火で副作用なし）
- [ ] 5大異常系パス網羅（キャンセル/部分返品/分割発送/在庫切れ/承認待ちタイムアウト）
- [ ] 補償イベントペア設計（外部副作用打ち消しまで）
- [ ] ロールバックSQL添付
- [ ] SLA 3階層エスカレーション設定
- [ ] デッドエンド状態・到達不能状態CI検出ゼロ
- [ ] ガード条件の排他性・網羅性真理値表検証
- [ ] 設計実装diffゼロ
- [ ] タイマー永続化（メモリ内禁止）
- [ ] 顧客向け表示ラベル・ボール保持者属性付与
- [ ] カナリアリリース段階展開（10→50→100%）

### 7. 他エージェントとのコラボ強化案
- **Bo**: 状態遷移表+補償イベント+ロールバックSQL+顧客ラベルを「実装即着手パッケージ」で引き渡し
- **Dat**: 工程別P25/P75/変動係数を受領→SLA閾値自動算出
- **KPI**: SLA違反イベントをKPI定義ID参照で発火、横断アラート一本化
- **現場受注担当**: 遷移理由+顧客向け表示ラベル+ボール保持者属性で通知
- **gen (建設業DX)**: 建設業フロー（発注→工事→検収→請求）の特殊性を取り込み
- **nori (Legal)**: 利用規約・スクレイピング規約の事前確認
- **PM (kai)**: ワークフロー変更をBMADフローで管理、in-flight案件マイグレーション計画
- **sora (QA)**: 6軸チェック✅証跡+CI検証ログを添付してQA回付
