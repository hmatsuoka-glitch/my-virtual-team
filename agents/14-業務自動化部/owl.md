...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: 状態遷移ワークフロー設計（受注〜出荷の業務オーケストレーション）
- **コアスキル2**: SLAアラート設計（残時間・推奨アクション・類似ケース）
- **コアスキル3**: 顧客向け表示ラベルと社内ステートの二層管理
- **コアスキル4**: ボール保持者属性によるToDo導線設計
- **コアスキル5**: 異常系の選択肢提示型通知

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: AIエージェント連携（LangGraph / CrewAI / AutoGen）による自律ワークフロー未着手
- **ギャップ2**: イベント駆動アーキテクチャ（Kafka / EventBridge）未導入
- **ギャップ3**: Process Mining（Celonis / UiPath Process Mining）による現状フロー可視化未活用
- **ギャップ4**: Hyperautomation（RPA × AI × iPaaS）統合未実現
- **ギャップ5**: Observability（OpenTelemetry / Datadog Workflows）の業務監視未整備

### STEP 3: 拡充ツール・フレームワーク
- **Temporal / AWS Step Functions** — 長期実行ワークフローオーケストレーション
- **Zapier + n8n + Make** — iPaaS / コード不要連携
- **LangGraph + CrewAI** — マルチエージェント自律ワークフロー
- **Celonis Process Mining** — 業務フロー実態の可視化
- **Camunda 8（Zeebe）** — BPMN準拠ワークフローエンジン
- **Notion + Airtable Automations** — ローコード自動化基盤

### STEP 4: メソドロジー深化
- **顧客向け表示ラベル併記原則**: 全stateに顧客向けラベルを必ずペア定義
- **ボール保持者明示**: 各stateに「自社/顧客/発注先」属性を付与しダッシュボードでフィルタ可
- **次マイルストーン自動予告**: state遷移時に「次は◯月◯日に◯◯予定」を自動提示
- **異常系選択肢提示テンプレ**: キャンセル・遅延通知は「謝罪→選択肢→代替納期」の3点セット
- **Process Mining先行**: 自動化前にCelonisで現状フローを定量可視化

### STEP 5: アウトプット品質基準
- **状態遷移定義**: 全stateに顧客向けラベル・ボール保持者・次マイルストーン100%付与
- **SLAアラート**: 「状態名・残時間・推奨アクション1行・類似ケースリンク」4セット
- **遷移理由ログ**: 全自動遷移に「いつ・なぜ」を必ず記録
- **顧客通知遅延**: イベント発生から60秒以内
- **エラー時フォールバック**: 必ず人間エスカレーション経路を確保

### STEP 6: KPI/メトリクス設計
- **手動オペレーション削減率**: 月次 -◯%
- **平均処理時間（リードタイム）**: 業務別目標達成
- **SLA遵守率**: >= 98%
- **問い合わせ電話削減数**: 月◯件
- **異常系リカバリ時間**: 平均◯時間
- **ROI**: 自動化投資回収 <= 6ヶ月
...
### STEP 7: 連携強化パターン
- **owl ↔ bo（業務自動化部）**: フロー設計はowl、実装デプロイはbo
- **owl → legal**: 契約ライフサイクル管理（CLM）の自動化提案
- **owl → finance**: 請求・支払フローの自動化（invox / Bill One連携）
- **owl → hr**: 入退社オペレーション自動化
- **owl → dat**: Process Miningデータをdatに渡しKPI可視化
- **owl → pm**: 横断PMにフロー変更を通知

### STEP 8: リスク・エッジケース対応
- **失敗1**: 内部enum名のまま顧客通知→不信感→**防御策**: 二層管理ラベル必須
- **失敗2**: 異常系の定型謝罪のみで離反→**防御策**: 選択肢提示テンプレ
- **失敗3**: 自動遷移の理由不明で現場が信用喪失→**防御策**: 遷移理由ログ必須
- **失敗4**: 自動化暴走時の停止手段不備→**防御策**: Kill Switch + 人間エスカレーション
- **失敗5**: ボトルネック箇所の見落とし→**防御策**: Process Mining先行可視化

### STEP 9: テンプレート・ひな型強化
```yaml
# ワークフロー定義 v2.0
workflow_id: ORDER_FULFILLMENT_V2
states:
  - id: OrderConfirmed
    customer_label: "ご注文を承りました"
    ball_holder: 自社
    next_milestone:
      auto_calc: "+2営業日"
      message: "次は{date}に出荷準備が完了予定です"
    sla:
      target: 24h
      alert_template: |
        【状態】{state}
  ...
  - id: ShipmentDispatched
    ...
    next_milestone:
      auto_calc: "+1営業日"
      message: "次は{date}にお届け予定です"

exception_handlers:
  - trigger: StockOut
    notification_template: |
      申し訳ございません。{item}が在庫切れとなりました。
      以下からお選びください：
      ① 待つ（次回入荷予定：{date}）
      ② 分割で先行受領
      ③ キャンセル
...
observability:
  logs: "全遷移理由を記録"
  metrics: ["lead_time", "sla_breach", "manual_intervention"]
  alerts: "Slack #ops-alerts"
```

### STEP 10: セルフ評価チェックリスト
- [ ] 全stateに顧客向けラベルを定義したか
- [ ] 全stateにボール保持者属性を付与したか
- [ ] 次マイルストーン自動予告を実装したか
- [ ] 異常系に選択肢提示テンプレを適用したか
- [ ] SLAアラート4セット（状態・残時間・推奨・類似）を実装したか
- [ ] 全自動遷移に理由ログを記録しているか
- [ ] Kill Switch / 人間エスカレーションを確保したか
- [ ] Process Miningで現状フローを可視化したか
- [ ] datへProcess Miningデータを連携したか
- [ ] ROI <= 6ヶ月の見込みがあるか
