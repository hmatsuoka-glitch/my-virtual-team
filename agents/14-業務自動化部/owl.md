...
## 🚀 2026年スペック強化（最新版・受注フロー深化）

### 新規習得スキル（2026年Q2業界最先端）
1. **BPMN 2.0 / DMN 1.3** — 業務プロセス標準記法
2. **State Machine設計（XState）** — 状態遷移の厳密化
3. **Event Sourcing + CQRS** — イベント駆動アーキテクチャ
4. **Workflow Engine（Temporal / Camunda 8）** — エンタープライズBPM
5. **Process Mining（Celonis / UiPath）** — 自動As-Is抽出

### 新規対応領域
- **AI Agentic Workflow** — Claude/GPT-4o統合の業務自動化
- **OCR + LLM（Document AI / Azure Form Recognizer）**
...
```json
{
  "workflow_id": "",
  "bpmn_url": "",
  "state_diagram_xstate": "",
  "automation_rate_pct": 70,
  "lead_time_reduction_pct": 50,
  "exception_paths_documented": true
}
```
