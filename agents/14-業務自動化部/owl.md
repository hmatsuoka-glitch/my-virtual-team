...
### 2026-07-12
- **「Temporal Workflows-as-Code」導入で受注状態機械の永続実装、Owl設計とBo実装の乖離を原理的にゼロ化**：状態遷移表を実装コードから自動生成する現行方式（設計変更→Bo実装差分→CI diff検出／06-16）を、Temporal.ioの「ワークフロー定義そのものが実装」パラダイムに置換。設計＝実装が同一Go/TypeScript関数となり、補償イベント（06-17）はTemporalのcompensationプリミティブでネイティブ表現、SLAタイマー永続化（06-17/07-07）もWorkflow標準機能で自動化。設計→実装リードタイム3日→0.5日、設計実装diff検出は原理的に不要化。
- **「Process Mining（Celonis / Microsoft Process Insights）」で本番イベントログから実測状態遷移グラフを自動再構成、静的設計との乖離を発見**：静的なグラフ走査（06-12）とベースライン滞留監視（07-03）に加え、本番の全遷移イベントログから「実際に使われた遷移パス・想定外の中間状態滞留・現場ワークアラウンドの迂回経路」を自動抽出。設計上は存在しないショートカット遷移や運用逸脱を発見し、遷移表を実運用に合わせて更新。設計と実運用の乖離検知率60%→98%、四半期の遷移表棚卸し工数16h→2h。
- **Kai（システム開発PM）連携：状態遷移表をKaiのBMAD設計書ドメインモデル章に「機械可読な状態機械記述（XState config / SCXML）」で埋め込み、システム開発時の実装者が同一データから直接ステートマシンを生成できる形式で引き渡す**：従来はKai配下のNaoが要件定義→Rikuが実装で解釈差分が発生していたが、XState/SCXMLは実装ライブラリに直接ロード可能なため設計→実装の解釈経路をゼロ化。BMAD 4-implementation工数25%削減、状態機械起因の実装バグ率8%→0.5%。
- **「イベントログのUnit Economics（1遷移あたり処理コスト・SLA違反1件あたり損失）」で状態機械の投資判断を金額化**：設計改善提案を「1遷移あたり平均何円のコスト削減・SLA違反1件あたり顧客賠償◯円回避」まで金額換算し、Kpi SSOTと連携して経営投資判断に使える形式で出す。設計改善のROI試算を「時間短縮h」でなく「年間◯万円の回避コスト」で提示（Boの06-07金額換算思想を状態機械層に拡張）、投資判断合意所要時間5日→0.5日、無駄な改修着手率30%→5%。
- **「AIエージェント委譲（Agentic Automation）」：異常系遷移の分類・補償イベント選択をLLMに委譲、受注担当の判断待ちをゼロ化**：在庫切れ・キャンセル理由の分類、補償イベント候補（部分返金/代替品送付/納期延長）の生成をClaude Agent SDK / Zapier Agentsに委譲、受注担当は最終承認1点（06-22と同思想）に集約。LLM出力は既存の恒等式・レンジ検証（06-26）と権限マトリクス（07-03）でガード。異常系対応リードタイム4時間→30分、受注担当の心理負荷（06-07の顧客都合キャンセル恐怖）も同時軽減。
- **2026年業界標準「BPMN 2.0 + DMN（Decision Model and Notation）」でビジネスルールと状態遷移を分離、Camunda 8 / Zeebe基盤で実装**：ガード条件・分岐ルール（06-12排他網羅）を状態機械コードに埋め込む現行方式では、ルール変更のたびに実装デプロイが必要。DMNデシジョンテーブルに分離しビジネスユーザーが直接編集可能な形にする。ルール変更の実装工数1日→30分、非エンジニアが承認閾値・SLA階層をノーコードで調整可能に。BPMNモデルはPlantUML（05-26）との相互変換で既存資産を継承。
- **効率化：カナリアリリース段階昇格ゲート（06-16）に「AIによる統計的異常検知（過去正常時分布とのZ-score超過・KLダイバージェンス）」を組み込み、閾値超過を待たず分布シフトを先行検知**：現行の「補償イベント発火件数閾値」判定は明確な数値ゲートだが、微妙な分布シフト（若干の遅延増・特定顧客だけエラー率上昇）は閾値内で見逃す。統計的異常検知を追加し、10%→50%→100%展開時の障害発見を平均4時間→15分に短縮、ロールバック対象を更に1/3圧縮（05-26の1/10と合わせ実質1/30）。
