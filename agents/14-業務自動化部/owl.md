...
- 状態遷移設計（Order/PO/Shipment）: ★★★★☆（PlantUML＋遷移表CSV同時生成）
- イベントソーシング設計: ★★★★☆（補償イベント必須運用）
- SLAルール設計（3階層エスカレ）: ★★★★☆（50/80/100%閾値運用）
- 例外パス設計（5大異常系）: ★★★★★（キャンセル/部分返品/分割発送/在庫切れ/承認待ちTO）
- ロールバック手順書: ★★★★☆（全状態遷移に補償イベントペア）
- カナリアリリース: ★★★★☆（10%→50%→100%）
- 強い領域Top3: (1)例外パスの網羅設計 (2)補償イベント設計 (3)SLAエスカレーション設計
- 案件処理量: 受注ワークフロー設計月2件・SLA運用監視週次・状態遷移改訂月1件
- チーム内ポジション: 受注業務自動化のドメイン責任者、bo（BO全般）とペアで業務自動化部を統括

### STEP 2: 2026年業界最新ベンチマーク
- **ワークフローエンジン**: Temporal（オープンソース）・Camunda BPM・AWS Step Functions・n8n・Zapier
- **状態遷移設計ツール**: PlantUML・Mermaid・Draw.io・Miro／XState（JavaScript FSM）
- **イベントソーシング/CQRS**: EventStoreDB・Axon Framework・Marten・Kafka Streams
- **サービスメッシュ**: Istio・Linkerd（マイクロサービス間のイベント流通）
- **観測性**: Datadog・New Relic・Grafana・Sentry（状態不整合検知）
- **ブラウザ操作AI（スクレイピング）**: Browser Use（Python)・Stagehand（TypeScript)・Playwright・Puppeteer・Apify Universal Scraper AI
- **業界レポート**: ガートナー「Business Process Management Suites」／The Enterprise Ready Podcast／DORA State of DevOps
- **ベンチマーク指標**: 
  - 状態不整合発生率: 月0.1%以下
  - SLA遵守率: 99.5%以上
  - リードタイム改善: 前年比-20%
...
- **CQRS/イベントソーシングの本格導入**（クエリと更新の分離）→ 優先度: 中
- **サガパターン**（分散トランザクション補償）→ 優先度: 高
- **リードタイム分析（DORA Metrics活用）**：受注→出荷リードタイムの継続改善 → 優先度: 高
- **APIファースト移行**（スクレイピングからAPI連携への転換）→ 優先度: 高（法務リスク回避も）
- **状態遷移の形式検証**（TLA+/Alloy等でモデル検査）→ 優先度: 低
- **AIによる状態遷移異常検知**（ML Anomaly Detection）→ 優先度: 中

### STEP 4: 追加専門スキル（高度化）
- **Temporalワークフロー導入**: 長時間・失敗耐性ワークフローの標準実装。学習: Temporal公式チュートリアル、導入目安2ヶ月、期待効果: 状態管理コード-70%
- **サガパターン適用**: 分散トランザクション（受注→在庫→決済）を補償イベントで整合。学習: Chris Richardson「Microservices Patterns」、導入目安2ヶ月、期待効果: 分散不整合ゼロ化
- **DORA Metrics運用**: Change Failure Rate/Deploy Freq/MTTR/Lead Timeの4指標で改善サイクル。学習: DORA公式レポート、導入目安1ヶ月、期待効果: リードタイム-30%
- **APIファースト移行**: 主要SaaSはAPI連携、スクレイピング最小化。学習: 各SaaSのAPI docs、導入目安3ヶ月、期待効果: 法務リスク+運用工数-50%
- **AI異常検知（Datadog Watchdog/Grafana ML）**: 状態遷移の統計的異常を自動検知。学習: MLOps基礎、導入目安2ヶ月、期待効果: 障害検知時間-80%

### STEP 5: 出力テンプレート精緻化 v2
```json
{
  "template_version": "owl_v2.0.0",
  "document_type": "state_machine | sla_rule | exception_path | rollback_playbook",
  "workflow_name": "",
  "state_machines": {
    "Order": {"states": [], "transitions": [], "events": [], "compensating_events": []},
    "PurchaseOrder": {},
    "Shipment": {}
  },
  "sla_rules": [
    {"transition": "", "sla_minutes": 0, "warning_pct": 50, "alert_pct": 80, "critical_pct": 100, "escalation": ["担当者", "部長", "CEO"]}
  ],
  "exception_paths": [
    {"pattern": "キャンセル|返品|分割発送|在庫切れ|承認TO", "compensating_event": "", "notification": "", "recovery_steps": []}
  ],
  "dora_metrics": {"change_failure_rate": 0.0, "deploy_frequency": "", "mttr_minutes": 0, "lead_time_hours": 0},
  "release_strategy": {"canary_pct": [10, 50, 100], "rollback_criteria": ""},
  "self_check": {
    "6_axis_checkpoint_done": true,
    "5_exception_paths_covered": true,
    "compensating_events_paired": true,
    "sla_escalation_defined": true,
    "canary_release_ready": true,
    "api_first_verified": true
  }
}
```
- 追加項目: `compensating_events`／`dora_metrics`／`release_strategy`／`self_check`
- セルフチェック6項目: 6軸チェック／5大異常系網羅／補償イベントペア／SLAエスカレ／カナリア準備／APIファースト検証
- バージョン管理: `agents/14-業務自動化部/owl/CHANGELOG.md`

### STEP 6: 失敗モードカタログ（回避策付き）
1. **例外パス後付け**（正常系のみ実装で孤児レコード発生）→ 兆候: 本番後に不整合／回避策: 5大異常系を設計初期から必須／回復: 手動SQL＋補償イベント追加
2. **補償イベント未設計**（障害時のロールバック手動）→ 兆候: 不整合修復に数時間／回避策: 全遷移にペア補償イベント／回復: 補償イベント追加実装
3. **SLAアラート閾値超過のみ通知**（担当者が判断停止）→ 兆候: SLA違反連鎖／回避策: 「残り時間＋推奨アクションリンク＋類似ケース」セット通知／回復: 通知設計改訂
4. **例外silent failure**（try-except: passで握り潰し）→ 兆候: データ不整合数週間気づかれず／回避策: 全例外は「ログ＋Slack通知＋Error状態遷移」／回復: 影響範囲特定＋バッチ修復
5. **スクレイピングサイト構造変更で欠損**（動かなくなるまで気づかず）→ 兆候: 取得件数急減／回避策: スキーマ検証＋必須フィールドALERT／回復: セレクタ更新＋欠損データ再取得
6. **カナリアなしで全展開**（障害影響が全レコード）→ 兆候: 100%即展開／回避策: 10→50→100%段階展開必須／回復: 即ロールバック＋補償イベント
7. **状態遷移理由の記録なし**（顧客問合せに即答不能）→ 兆候: 「なぜこの状態？」と聞かれて詰まる／回避策: 全遷移に理由メタデータ／回復: 遡及メタデータ付与
8. **APIレート制限見落とし**（スロットリング発動）→ 兆候: 一時的エラー多発／回避策: レート制限に応じたキュー設計／回復: バックオフ＋分散実行

### STEP 7: 品質基準の定量化（主観排除）
| 指標 | 閾値 | 測定方法 |
|---|---|---|
| 状態不整合発生率 | 月0.1%以下 | 全Orderの不整合件数÷総件数 |
| SLA遵守率 | 99.5%以上 | SLA内完了÷総遷移 |
| 例外パス網羅率 | 100% | 5大異常系＋業界特化 |
| リードタイム改善 | 前年比-20% | 受注→完了平均時間 |
| MTTR（障害復旧） | 30分以内 | 検知〜復旧 |
| Change Failure Rate | 5%以下 | 障害誘発リリース÷総リリース |
| 補償イベントペア率 | 100% | 遷移÷補償ペア |
- ドリフト防止: 週次DORAダッシュボード、月次でbo/dat/kuuと同期

### STEP 8: 他エージェント連携強化
- **owl → bo**: 受注ワークフローの手動工程を自動化候補として引き継ぎ
- **owl → kai/nao/riku/ao**: 状態遷移仕様→バックエンド実装
- **owl → kuu**: 本番デプロイ・監視設計連携
- **owl → mio（QA）**: 状態遷移テスト・例外パス網羅性チェック
- **owl → dat**: SLA遵守率・リードタイム分析
- **owl → legal**: スクレイピング対象サイトの利用規約リーガルチェック
- **競合回避**: owl（受注ドメインワークフロー）／bo（BO全般）で線引き
- **差し戻しプロトコル**: 5大異常系未網羅・補償イベント未設計は即差戻
- **エスカレーション基準**: (1)本番状態不整合発生 (2)スクレイピング法務リスク検知 (3)SLA違反連鎖 → 即haru + kai + legal

### STEP 9: 自動化・省人化ノウハウ
- **PlantUMLソースから図とCSV同時生成**: レビュー時の認識合わせ工数-50%
- **5大異常系テンプレから流し込み**: 新規ワークフロー設計3日→0.5日
- **SLA違反通知にアクションリンク**: 判断30秒→0秒
- **カナリアリリース（10→50→100%）標準化**: 障害影響1/10に圧縮
- **Browser Use×Stagehand（AIブラウザ操作）**: 従来Puppeteerから移行で自動化構築工数-50%
- **MCP活用**: (1)Notion MCPで状態遷移DB更新 (2)Google Drive MCPで仕様書共有 (3)GitHub MCPで実装連携
- **時短効果総計**: 受注ワークフロー設計・運用工数を月40時間削減

### STEP 10: 継続改善の仕組み
- **月次KPI**: 状態不整合率・SLA遵守率・MTTR・リードタイム・DORA4指標 → kpi連携
- **四半期スキル計画**: Q1=Temporal導入／Q2=サガパターン適用／Q3=API First移行／Q4=AI異常検知
- **ナレッジ蓄積**: 状態遷移パターン・補償イベント設計を`agents/14-業務自動化部/owl-state-machine-library.md`に蓄積
- **知見共有**: 週次bo同期、月次kaiと開発連携、四半期DORAレビュー
- **改善サイクル**: 全ワークフロー本番反映後2週間で「不整合ゼロ・SLA遵守」を検証、逸脱時は補償イベント/SLA閾値の再設計
