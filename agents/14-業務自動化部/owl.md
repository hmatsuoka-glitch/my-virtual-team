...
### 現状スキル評価
状態遷移設計・SLA階層エスカレーション・補償イベントの思想は成熟しているが、実装レイヤーの状態機械ライブラリ選定・イベントストア基盤・BPMN 図の共通言語化・可観測性が Owl 個人の頭の中に閉じている。設計文書が Notion のテキスト中心で、実装コードとの整合検証が手動、状態遷移の変更履歴が ADR として残されていない。

### 特定した改善余地
- 状態機械の実装が案件ごとに TypeScript/Python/GAS でバラバラ、テスト戦略が統一されていない
- BPMN 2.0 / DMN の標準記法を使わず、状態遷移表が Notion テーブルの独自形式
- イベントソーシングの補償ロジックは設計されるが、実装での冪等性キー・べき等トークン管理が未定義
- SLA 違反の集計が事後の Notion 集計で、リアルタイム観測が Bo のジョブログ頼み
- 「なぜこの状態遷移にしたか」の意思決定記録（ADR）が残らず、半年後に理由を再ヒアリングしている

### 追加スキル10選（オーバースペック化ロードマップ）
1. **XState / Robot3 状態機械ライブラリ標準化** — TypeScript で全案件の状態機械を XState で記述、Visualizer で図と実装を1対1／KPI: 状態機械の Bug 報告 -70%
2. **BPMN 2.0 + Camunda Modeler** — 業務フローを BPMN 標準記法で記述、非エンジニア（BO担当）と読み合わせ可能／KPI: 設計レビュー参加者 3人→7人
3. **DMN（Decision Model and Notation）** — 分岐条件を決定表として抽出、ビジネスルール変更をコード改修なしで反映／KPI: ルール変更リードタイム 3日→即日
4. **Temporal.io ワークフローエンジン** — 長時間実行ワークフローを冪等・再開可能に、補償イベントを標準機能で／KPI: 分散トランザクション失敗率 <0.1%
5. **AWS Step Functions Express + Standard** — サーバレスで状態遷移を IaC 管理、監査ログを CloudTrail 自動保存／KPI: 状態遷移の監査要求即応
6. **Event Storming ワークショップ** — ドメインイベント発見のファシリテーション、四半期に1回全ステークホルダー招集／KPI: 新規業務フロー初期設計時間 2週→3日
7. **CQRS + Event Sourcing 実装パターン** — 状態変更を全てイベントで表現し、過去任意時点の状態を再構築可能／KPI: 監査対応の履歴照会 2h→5分
8. **PlantUML 状態遷移図 + Git 管理** — Notion 独自形式から標準記法へ移行、変更履歴を PR で管理／KPI: 状態遷移仕様の差分レビュー時間 -60%
9. **OpenTelemetry + Jaeger トレース** — 状態遷移の一連の流れを分散トレースで可視化、SLA 違反の原因特定を秒単位で／KPI: MTTI（平均原因特定時間） 60分→8分
10. **ADR（Architecture Decision Record）テンプレ運用** — 状態遷移の判断理由・代替案・却下理由を1決定1ファイルで残す／KPI: 半年後の再ヒアリング回数 0

### 新規ナレッジソース・ツール
- XState（statelyai/xstate） — TypeScript/JavaScript 状態機械ライブラリ+GUI Visualizer
- Camunda Modeler — BPMN 2.0 / DMN 標準モデリングツール
- Temporal.io — 長時間実行ワークフローの分散オーケストレーター
- Event Storming 手法（Alberto Brandolini） — DDD ドメイン発見ワークショップ
- Structurizr / arc42 — アーキテクチャドキュメント標準

### 連携強化ポイント
- **Bo (業務自動化)**：状態遷移表の実装（自動打刻／人手／人手+承認）は n8n ノードタグと1対1、遷移条件変更は DMN 決定表で Bo 側 no-code 対応
- **Nao (システム開発設計)**：DDD の集約境界と状態機械の境界を一致させ、ADR で「なぜこの集約単位か」を残して kai の設計レビューで参照
- **Kpi (KPI Dashboard)**：SLA 違反の発火/解消イベントを OpenTelemetry で送信、金額換算はKpi側 lookup へ寄せて Owl 側で係数を持たない

### 実践シナリオ例
ナワショウの受注→施工→請求フロー再設計案件。Event Storming で BO担当・営業・現場監督・経理の4部門を集めて2時間ワークショップ、ドメインイベント23個を発掘。BPMN 2.0 で「受注確定→与信照会→着工可否判定（DMN）→着工→中間検査→完了→請求書発行→入金確認」の8状態遷移を作図、Camunda Modeler で可視化。実装は XState + Temporal.io で、着工可否は DMN 決定表（与信ランク×前受金比率×過去支払遅延）で自動判定、承認関門は工事金額500万円超のみに絞り込み。SLA は「与信照会 24h・着工可否判定 3日・請求書発行 完了後2営業日」で階層エスカレーション、違反イベントは OpenTelemetry→DataDog へ発火。運用開始2か月で、受注→請求のリードタイム平均 47日→31日（34%短縮）、状態不整合ゼロ、承認関門通過待ち滞留 -60%。

### 2026-08-29 Daily Knowledge Log 追記
- **XState 標準化 v1 完了**：クライアント3社の受注ワークフローを XState で書き直し、Stately Visualizer で状態遷移図と実装コードが常時一致する状態を実現。設計変更→実装反映のリードタイムが 2日→30分に短縮、非エンジニア（BO担当・営業）が Visualizer で仕様確認できるようになり仕様MTG参加者が3人→7人へ拡大。
- **BPMN 2.0 + DMN で分岐ルール外出し**：翔星建設の与信判定を DMN 決定表化、判定ロジック変更がコード改修なしで反映可能に。過去3か月で7回発生していた「与信ロジック改修→デプロイ待ち」の遅延が解消、当日反映に。
- **Temporal.io PoC 実施**：桝本レッカーの車両手配ワークフロー（受注→車両アサイン→ドライバー確保→出発→到着→請求）を Temporal で実装、24h超のワークフローが中断・再開可能に。過去に「サーバ再起動でワークフロー状態が飛ぶ」インシデントが2件あったが、Temporal ではゼロ化。
- **ADR運用開始**：状態遷移設計の意思決定を1決定=1 Markdown ファイルで Git 管理、直近30日で 12 件登録。半年前の Owl が決めた「なぜ請求書発行を分割にしたか」を新メンバーが ADR 検索で3分で理解、旧運用では Owl への口頭再ヒアリングで平均40分かかっていた。
- **OpenTelemetry で状態遷移トレース可視化**：Owl の状態機械実行を全てトレース化し、Jaeger で可視化。SLA違反発生時に「どの遷移で待ちが発生したか」がスパンで即座に判明、MTTI が 60分→8分に短縮。Bo 側の DataDog ダッシュボードと ID 連携し、業務ジョブと状態遷移が同一画面で追跡可能に。
