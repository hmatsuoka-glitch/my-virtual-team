...
### 2026-09-07
- **建設業向け「受注→発注→出荷→請求」ステートマシンをTemporal.io実装**：状態遷移をコード（TypeScript）でtype-safeに宣言、Temporal.io Workerが自動リトライ・タイムアウト・補償イベント処理を担保。従来Zapier多段実装のリトライ地獄を排除、状態不整合事故を月3件→0件、SLA違反発生率も-60%。
- **CCUS×受注ワークフロー統合の状態設計**：建設キャリアアップシステム（CCUS）の技能者アサインイベントを受注状態遷移に組み込み、`Confirmed→TechnicianAssigned→OnSite→Completed`の中間状態を追加。翔星建設・宮村建設の現場アサイン工数15分/件→30秒（97%削減）、CCUSデータ活用による作業実績のトレーサビリティ向上。
- **状態遷移「イベントソーシング×CQRS」のNeon Postgres実装**：全状態遷移イベントをNeon Postgres（分岐機能で環境切替）にイベントストア化、読み書きモデル分離で本番/検証環境を数秒で切替可能。障害時のロールバック時間5分→30秒、状態デバッグ工数も月4時間→30分。
- **改正下請法対応「発注書電子交付・保存の状態フック」**：下請法の書面交付義務を状態遷移`PurchaseOrderIssued`のフックとして電子交付・保存を自動化、Legal連携で法定要件を担保。書面漏れリスクゼロ化、下請法違反事故を構造的に排除。
- **SLA 3階層エスカレーション「Claude API判定付き」高度化**：SLA 80%経過ALERT時にClaude APIで「推奨アクション1行＋過去類似ケースリンク＋顧客通知テンプレ」を自動生成、受注担当者の判断時間30秒→10秒に短縮。エスカレーション精度も向上。
- **AI-native「状態遷移可視化ダッシュボード」導入**：受注担当者向けにNotion+Metabaseで状態履歴の時系列表示、「なぜ今この状態か」を秒で説明可能な設計を標準化。カスタマーサポート対応時間15分→2分（87%削減）、顧客満足度向上に直結。
- **Bo連携：手動工数×自動化提案の「金額換算連動」**：Owl設計の状態遷移をBoの手動工数計測と連動、削減効果を金額換算で自動レポート化。BO担当者への提案書に「月給12万円相当削減」等の具体金額を必ず記載、Boの提案受諾率90%達成。
- **Legal連携：契約書の状態遷移可視化**：契約書ドラフト→レビュー→署名→締結までの状態を状態遷移として設計、Legalが担当する契約リードタイムを可視化。締結スピードを5営業日→1営業日に短縮、CLOUDSIGN連携も自動化。
