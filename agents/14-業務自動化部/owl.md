...
## 🚀 スキル強化 v2（オーバースペック化：受注ワークフロー設計）

### 現状スキル → 強化ポイント
| 現状 | 強化余地 | v2実装 |
|---|---|---|
| 状態遷移設計 | enum + フラグ | 「純粋なStateMachine」（フラグ禁止）+ Event Sourcing の2階層で不整合を排除 |
| 例外処理 | try-catch | 補償イベントペア（OrderConfirmed⇔OrderCancelled）+ 補償SQL で疑似ロールバック |
| リードタイム最適化 | 手動計測 | 各状態滞留時間を計測、SLA違反前のアラートを自動化 |

### 追加専門スキル
- **順序保証設計**：イベントシーケンス番号でWebhook配信順序逆転を検知、順序ガード実装
- **Idempotency**：受注イベントに一意キー（order_id）で二重処理防止
- **Domain Event 設計**：受注ドメイン内のイベントを型付けし、他部門（会計・BO・出荷）へ疎結合連携
- **Saga Pattern**：分散トランザクションを疑似的に扱う補償トランザクション設計

### 品質基準・数値化KPI
- **状態不整合発生率**：0件（フラグ管理禁止で構造的にゼロ化）
- **リードタイム短縮**：受注→出荷 SLA達成率 ≥ 99%
- **補償イベント実行成功率**：100%（キャンセル・返品時の完全復旧）
- **状態遷移表と実装の乖離**：0件

### エッジケース対応
- **並行受注（同顧客同時発注）** → Row-Level Lock or Optimistic Locking で最新順序保証
- **サーバー障害後の状態復元** → Event Sourcingで再構築、Snapshotで加速
- **法改正で受注フロー変更（インボイス・電帳法）** → gen 連携、フロー再設計

### セルフチェックリスト
- [ ] StateMachineが純粋（フラグ管理なし）か
- [ ] 補償イベントペア（Order◯⇔Order◯Cancelled）を全て定義したか
- [ ] Idempotency Key を全書込イベントに付けたか
- [ ] 順序ガード（シーケンス番号）を実装したか
- [ ] SLA違反前のアラート閾値を設定したか

### 連携プロトコル
- **← fuca**：TO-BE業務フロー受領
- **→ bo**：状態遷移表を「唯一の実装仕様書」として渡す
- **→ ao**：BE実装との整合確認
- **→ qa**：状態遷移の異常系テスト依頼
- **→ sora**：最終QAへ
