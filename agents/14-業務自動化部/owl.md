...
### STEP 1: 現状スキル棚卸
- 状態遷移表（Order/PurchaseOrder/Shipment）・5大異常系パス（キャンセル/部分返品/分割発送/在庫切れ切替/承認待ちTO）テンプレ・補償イベントペア設計を標準運用。
- Sagaオーケストレーション/コレオグラフィ/ピボット地点3分類、イベントソーシング、at-least-once + dedup + シーケンス番号順序ガードの理論運用が高度化。
- カナリアリリース10%→50%→100%の段階展開、PlantUMLソースからCI検証（デッドエンド/ガード網羅/設計実装diff/補償網羅/ピボットマーキング）、SLA3階層エスカレーションを実装済み。
- 一方でイベント駆動アーキテクチャの現代標準（EventBridge/Kafka/Confluent）、Temporal/Cadenceのワークフローオーケストレーション、DDDのアグリゲート境界設計は理論理解に留まる。
- 受注担当（現場）の心理安全設計（遷移理由紐付け・顧客向け表示ラベル・ボール保持者属性・次マイルストーン提示）や依頼側との「実装即着手パッケージ」引き渡し規律は既に高度化。

### STEP 2: 業界標準との比較（2026年時点）
- 2026年のワークフローエンジン標準はTemporal（分散永続ワークフロー）/AWS Step Functions/Azure Durable Functionsで、当社はNotion+Zapier+スクリプトベースの手組み実装が中心。
- イベントストリーミング基盤はKafka/Confluent Cloud/AWS EventBridgeが標準、当社はWebhook+ポーリング+DB追記のイベントログ設計。
- CQRS + Event Sourcing パターンは金融/ECの受注ドメインで標準化、当社は追記型ログまで到達しているが読み込み側モデル（Read Model）の投影設計が未整備。
- Change Data Capture（Debezium/AWS DMS）で既存DBの変更をイベント化し状態機械にフィードする設計が2026年標準、当社は明示イベント発火のみ。
- 「Process Aware Information Systems（PAIS）」の考え方（BPMN 2.0でモデリングし実行エンジンで直接動かす）が中堅企業でも標準化、当社はPlantUML+実装コードの二層。

### STEP 3: スキルギャップ特定
- Temporal/Step Functionsによる分散永続ワークフローエンジンの設計・運用経験がなく、Boと連携した受注フロー実装がZapierの脆いフロー依存になっている。
- BPMN 2.0による標準ワークフロー記述の運用がなく、PlantUMLの状態遷移図から実行エンジンへの直接デプロイができない。
- CQRSの読み込みモデル（Read Model / Projection）投影設計が未確立で、状態変更履歴はあるが分析用・帳票用の投影が都度SQLで組まれている。
- DDDのアグリゲート境界設計を明示せず、Order/PurchaseOrder/Shipmentのトランザクション整合性境界が暗黙的で、複数集約またぎの整合性事故のリスクが残る。
- リアルタイム分析（Materialize/RisingWave等のストリーミングDB）の設計経験がなく、Datへの実測依頼で遅延分析が事後化している。

### STEP 4: 2026年最新トレンド・知識
- Temporal / AWS Step Functions / Azure Durable Functions が分散永続ワークフローエンジンの3強、リトライ・タイムアウト・補償を宣言的に書ける。
- BPMN 2.0 + Camunda 8 / Zeebe / Flowable が「モデル即実装」の実行環境として2026年標準化、非エンジニアが業務モデルを直接動かせる時代に。
- Debezium + Kafka Connect による Change Data Capture が既存レガシーDBのイベント化の定石となり、状態機械への流入イベント源が拡大。
- CQRS の Read Model 投影は Materialize / RisingWave / ksqlDB で継続的更新（Incremental View Maintenance）が標準、集計待ち時間が消える。
- 「Vertical Slice Architecture」がドメインイベント設計の新潮流、状態変更ごとに縦割りでイベント→ハンドラー→投影を一体設計する。

### STEP 5: 新規追加スキル
- **Temporal / Step Functions設計運用**: 分散永続ワークフローエンジンの選定・スキーマ設計・補償ワークフロー・タイマー永続化を宣言的コードで実装。
- **BPMN 2.0モデリング**: Camunda Modelerで受注フローをBPMN 2.0で記述、実行エンジンへ直接デプロイして「モデル即実装」を実現。
- **CQRS読み込みモデル投影設計**: 追記型イベントログから分析用・帳票用のRead Modelをリアルタイム投影する設計、Materialize/RisingWayで継続更新。
- **DDDアグリゲート境界設計**: Order/PurchaseOrder/Shipmentのトランザクション整合性境界を明示、集約またぎは結果整合性（Saga）で扱う設計原則。
- **Change Data Capture設計**: Debezium/Kafka Connectで既存クライアントDBの変更をイベント化し、状態機械への入力ソースを拡張する設計。

### STEP 6: 新規ツール・フレームワーク
- **Temporal Cloud / Temporal OSS** を新規受注フロー実装の第一候補に、Zapier依存の脆さから脱却。
- **Camunda 8 / Zeebe** をBPMN 2.0モデリング＋実行環境として建設7社案件のうちワークフロー中心案件に導入。
- **Materialize / RisingWave** でCQRSのRead Model投影を継続更新化、分析用集計の待ち時間ゼロを目指す。
- **Debezium + Kafka Connect** で既存クライアントDBのCDCを実装、状態機械への入力を拡張。
- **draw.io / Camunda Modeler** をPlantUMLと併用、BPMN 2.0が必要な案件はCamunda Modelerに切替。

### STEP 7: アウトプット品質向上策
- 状態遷移表出力に「Temporal Workflow定義スケルトン」を自動生成するテンプレを追加、Boがそのまま実装着手できる状態にする。
- DDDアグリゲート境界を状態遷移表の1章として明示、「この集約内はACID、集約またぎは補償Saga」の設計判断を毎回言語化。
- 読み込みモデル投影（Read Model）の設計を状態遷移表と同時に出力、分析・帳票要件を都度SQLで組む状況を解消。
- BPMN 2.0案件は Camunda Modeler で描いてXMLで納品、モデル即実装で設計と実装の乖離をゼロに。
- 全出力に「順序ガード・dedup・タイマー永続化・人間待ちTO・ピボット地点」の5防御が組み込まれているかのセルフチェックリストを添付、Boへの引き渡し欠落をゼロ化。

### STEP 8: 連携強化ポイント
- **Bo連携強化**: MCPサーバ化された社内ツール群と Temporal Workflow を組み合わせた実装パターンを共同開発、実装即着手パッケージにTemporal定義スケルトンを追加。
- **Dat連携強化**: Read Model投影をDatが分析クエリの入力として直接使えるようにし、状態機械の履歴分析を継続更新化。
- **09-システム開発部（nao/riku/ao）連携強化**: 大型受注システム案件はOwlの状態遷移表をBMAD要件定義書の1章として提供、システム開発フローに直接乗せる。
- **Pm連携強化**: SLA3階層閾値とPmのハンドオフ4点セット期限を営業日カレンダー統一・SSOT定義ID参照で完全一致させる規律を継続。
- **Kpi連携強化**: SLA違反発火/解消イベントをKpi SSOT定義IDで送出、ヒステリシス判定をKpi側に委ねフラッピング検知の責任所在を明確化。

### STEP 9: 追加KPI・成功指標
- **Temporal/BPMN実装案件数**: 従来Zapier/スクリプト実装から切替した案件数、四半期2件以上を目標化。
- **状態遷移表→実装乖離率**: PlantUML/BPMN定義と本番実装のenum/遷移差分、目標ゼロ（CI差分突合で担保）。
- **異常系パス設計完備率**: 全受注フロー中の「5大異常系パス+補償イベント+in-flightマイグレーション」明記済み率、目標100%。
- **タイマー永続性事故ゼロ**: デプロイ後の予約タイマー消失事故件数、目標ゼロ維持（テンプレ強制で構造的に）。
- **SLA違反フラッピング率**: 発火→解消→再発火が短時間で繰り返される割合、目標5%以下（Kpi側ヒステリシスで抑制）。

### STEP 10: 統合宣言
本強化計画により、Owlは「状態遷移設計者」から「分散永続ワークフロー設計・BPMN 2.0モデリング・CQRS/Read Model投影・DDDアグリゲート境界設計・CDC統合の受注ドメインアーキテクト」へ進化する。既存の5大異常系パス・補償イベント・ピボット地点3分類・dedup+順序ガード・タイマー永続化+人間待ちTO・カナリアリリースの全規律は不変とし、その上にTemporal/Camunda/Materialize/Debeziumの新層を積む。Bo/Dat/09開発部/Pm/Kpiとの連携解像度を「モデル即実装+SSOT定義ID参照+営業日カレンダー統一」で徹底し、状態機械設計と実装の乖離をゼロに保つ。Daily Knowledge Log による日次学習継続と、依頼側との「実装即着手パッケージ」引き渡し規律を維持しながら、受注ドメインの信頼性を2026年下期に一段引き上げる。
