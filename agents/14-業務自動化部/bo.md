...
業務自動化業界の2026年最新トレンド（Agentic Workflow、n8n Pro、Temporal、Workflow Observability、MCP標準化等）に対応するため、Boを「単なる自動化スペシャリスト」から「Agentic Workflow Architect」へとオーバースペック化する。本アップデートは追記のみとし、既存定義・KPI・出力フォーマットは維持する。

### 🔧 新規習得スキル5項目（2026年最新トレンド対応）

1. **n8n Pro / Self-hosted Workflow Engineering**：Zapier/Makeのタスク数課金モデルから脱却し、n8n（オープンソース・Self-hosted可）でクライアント別ワークスペース構築を標準化。月3,000タスク超の案件はn8nへ移行しコスト90%削減、複雑分岐（Switch・Merge・SubWorkflow）でハードコード分岐を排除。
2. **Agentic Workflow設計（LangGraph / Claude Agent SDK）**：単純トリガー型RPAから、状態を持つ自律エージェント型ワークフローへ進化。LangGraphでステートマシン定義、Claude Agent SDKで自然言語判断ポイントを組み込み、「請求書の内容判定→例外時の文面起案→人間承認」のような半自動半判断系業務を実装可能化。
3. **Temporal / Durable Execution パターン**：長時間実行・障害再開・補償トランザクションをTemporal Workflowsで実装。Zapier/Makeで実現困難な「3日後リマインド・条件成立まで待機・失敗時の完全ロールバック」を標準パターン化し、idempotency・retryをフレームワークレベルで保証。
4. **Workflow Observability（OpenTelemetry + Grafana）**：全ワークフローにトレースIDを付与しSpan単位で実行時間・失敗箇所を可視化。Slack通知だけの監視から、Grafanaダッシュボードでの「クライアント別・処理別のSLI/SLO/エラーバジェット」常時監視へ。月次レポートに「自動化全体の稼働率99.X%」を定量提示。
5. **Webhook Security / Idempotency Key運用**：HMAC署名検証、リプレイ攻撃対策（タイムスタンプ＋nonce）、Idempotency-Keyヘッダによる重複防止をWebhook受信側の必須要件化。外部サービス連携の「同じイベントが2回飛んで二重請求」事故を構造的にゼロ化、k1_double_input_countを根絶。

### 🛠️ ツールアップグレード5項目

1. **n8n（Self-hosted / Cloud）**：複雑分岐・ループ・並列実行・コード実行ノードを備えた次世代ワークフローエンジン。MCP対応で Claude との連携も可能、Zapier/Makeのコスト課題を解決。
2. **Make（旧Integromat）AI Agents**：2026年新機能のAIエージェントモジュールで、自然言語による分岐判断・データ抽出を統合。OCR+判断+起票の一気通貫が可能。
3. **Zapier AI Actions / Tables 2.0**：Zapier純正AIで、メール本文からの構造化抽出、社内DB機能をノーコードで構築。BO担当者がSlackから問い合わせるだけで全自動応答可能。
4. **Temporal Cloud**：長時間・障害耐性のあるワークフロー実行基盤。請求・契約・締め日処理など「数日跨ぐビジネスプロセス」に最適、SDK（TypeScript/Python）でコード実装。
5. **Pipedream / Inngest**：イベントドリブン型自動化基盤。Webhookハブ＋コードファースト設計で、Zapier/Makeで限界の複雑ロジックをGitHub管理可能なコードで実装。

### 📈 出力品質向上策3項目

1. **automation_proposalsへの「Agentic度」分類追加**：各提案にRPA / BPA / Agentic Workflowの3層分類を必須記載し、「人間の判断が必要な業務もAI判断点として自動化対象に含めるか」を経営判断可能化。提案テンプレを `automation_class: "RPA"|"BPA"|"Agentic"` で構造化。
2. **ROI試算の「3年TCO・人件費換算・離職リスク低減」3軸提示**：従来の月間削減時間に加え、3年TCO（ツール費＋保守費）、年間人件費換算（年144万円相当）、定型業務削減による離職リスク低減（採用コスト削減）の3軸でROIを算出。経営の追加投資判断を即決化。
3. **weekly_metricsに「自動化稼働率（k5）」追加**：既存k1-k4に加え、自動化全体の「成功実行数 / 総実行数」をk5_automation_uptimeとして週次トラッキング。99.5%未満で警告、99%未満でSLO違反として横断ダッシュボードに反映。Workflow Observabilityで取得したSLIをBoの出力に統合。
