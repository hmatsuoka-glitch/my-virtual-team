...
### STEP 1: 現状スキル棚卸
- 6軸公開前チェック（dry-run/idempotent/ロールバック/通知/工数測定/SLA）・DLQ/サーキットブレーカー/指数バックオフの障害対策3点セットを標準運用。
- 3点セット（請求書発行・売上計上・入金消込）テンプレによる新規クライアント立ち上げの16h→2h化、Slackスラッシュコマンド`/automation status`による朝ダッシュボードを実装済み。
- 恒等式検証（入力＝成功＋スキップ＋エラー＋DLQ）・金額レンジアサーション・APIキー最小権限台帳・ゴールデンテストCSVによる境界値検証は自社標準化。
- 一方でAIエージェント型自動化（自律判断込み）、プロセスマイニング、可観測性基盤（OpenTelemetry等）はDaily Logで用語理解に留まり、実装導入が未着手。
- BO担当者の心理安全設計（金額換算・失敗手順書・突合レポート）や依頼側との「フェイルクローズ」要件握りなど、人間側の運用継続性設計は既に高度化している。

### STEP 2: 業界標準との比較（2026年時点）
- 2026年の業務自動化標準は「AI Agent Workforce（LLM+ツール実行）」で、Zapier Agents/Make AI/n8n AI Agent Nodeが業界標準化、当社はRPA/BPA中心。
- MCP（Model Context Protocol）による標準ツール接続がAnthropic/OpenAI/Google共通化、内製ツールもMCPサーバ化して各AIエージェントから利用させるのが2026年標準。
- 可観測性は Datadog / New Relic / Grafana Cloud の3層計測（Metrics/Logs/Traces）が標準、当社はSlack通知＋Notion台帳の手動運用で代替中。
- プロセスマイニング（Celonis / UiPath Process Mining）で既存業務ログから自動化候補を機械抽出する運用が中堅企業でも標準化、当社はDatの実測とヒアリング依存。
- 「LLMによる非構造データ処理（PDFから請求情報抽出・メールから発注確定判定）」が2026年の自動化領域として爆発的拡大、当社は該当領域が未着手。

### STEP 3: スキルギャップ特定
- LLMエージェント設計（プロンプト/ツール定義/フィードバックループ/安全ガードレール）のスキル・実装ノウハウが未確立で、AI Agent型案件の受注ができない。
- MCPサーバ実装・MCPクライアント経由でのツール呼び出しの実務経験がなく、他エージェント（riku/ao/kuu）との連携で最新プロトコル活用が遅れている。
- OpenTelemetry / Grafana Cloud等の統合可観測性基盤の設計・運用経験がなく、Slack通知だけで障害の相関分析ができない。
- プロセスマイニング初級運用（既存ログから頻度×工数の候補自動抽出）のツール導入判断がなく、Datへの実測依頼が繰り返し工数となっている。
- 電帳法対応（真実性・可視性・検索要件）の自動化実装ノウハウが2026年義務化後の業界標準に追いつききれておらず、建設7社案件の重要な自動化領域を逸失中。

### STEP 4: 2026年最新トレンド・知識
- Zapier Agents / Make AI Agent / n8n AI Agent Node の3強がノーコードAIエージェント構築ツールの2026年主流、Anthropic Claude / OpenAI GPT / Google Gemini を切替可能。
- MCP（Model Context Protocol）が Anthropic Claude Desktop / Cursor / Claude Code に加え OpenAI・Google も対応表明済み、社内ツールをMCPサーバ化する動きが加速。
- LLMベースのPDF/画像非構造データ抽出精度が Claude 4 / GPT-5 世代で実用ラインを突破、請求書・発注書・領収書の自動データ化が現実解に。
- 電帳法対応の JIIMA認証取得済みシステム（マネーフォワード/freee/弥生）とのAPI連携が2026年に本格化、建設業の請求電子化と原価管理連携で標準化。
- 「Agent Observability」（LLMエージェントの推論プロセス可視化）が Langfuse / LangSmith / Helicone で標準化、AI Agent型自動化の障害調査に必須。

### STEP 5: 新規追加スキル
- **LLMエージェント設計**: Claude/GPT/Geminiの3モデルで動くエージェント定義（プロンプト/ツール/ガードレール/評価）を書き分け、ノーコード（Zapier Agents）とコード（LangGraph/Claude Agent SDK）両対応。
- **MCPサーバ/クライアント実装**: 社内ツール（Notion/GA4/PR TIMES/クライアント別会計API）をMCPサーバ化し、他エージェント・AI Agentから統一プロトコルで呼び出せる状態を作る。
- **非構造データ抽出パイプライン**: PDF/画像/メール本文からLLMで構造化データを抽出し、既存BPAワークフローに投入する処理設計、抽出精度の恒等式検証も含む。
- **統合可観測性設計**: OpenTelemetryで Metrics/Logs/Traces を統一収集、Grafana Cloudで相関分析、Slack通知を「異常の要約通知」レイヤーに格上げ。
- **電帳法対応自動化**: JIIMA認証システム連携、タイムスタンプ付与、検索要件対応、7年保存の証跡保全を建設クライアント3点セットに追加。

### STEP 6: 新規ツール・フレームワーク
- **Claude Agent SDK / LangGraph / Zapier Agents** の3層をエージェント案件の規模別に使い分け（Zapier=軽量/LangGraph=中規模/Claude Agent SDK=カスタム）。
- **MCP Server SDK（TypeScript/Python）** で社内APIを順次MCPサーバ化、まずは「7社共通の請求書ステータス取得MCPサーバ」から着手。
- **Langfuse（OSS Agent Observability）** をAI Agent型自動化に必須組み込み、LLM推論の全ログ・トークン消費・レイテンシを可視化。
- **Grafana Cloud + OpenTelemetry Collector** を業務自動化基盤の統合監視レイヤーとして導入、Slack通知はGrafana Alertingから発火。
- **Amazon Textract / Google Document AI / Claude Vision** の3択でPDF抽出タスクを分岐、コスト×精度で使い分けるパイプライン設計。

### STEP 7: アウトプット品質向上策
- 全AI Agent実装にLangfuse組込を必須化、推論プロセスの追跡可能性を通常の自動化と同レベルで担保。
- automation_proposalsのJSONに「LLM使用有無・想定トークン数・月次コスト試算・ハルシネーション対策」の4フィールドを追加、AI起因の予測不能コストを事前提示。
- MCPサーバ化した内部ツールは全て「認証・レートリミット・監査ログ・恒等式カウンター」を標準実装し、他エージェントから安全に呼び出せる状態を維持。
- 電帳法対応自動化は「真実性（改変不能ログ）・可視性（検索3項目対応）・7年保存」の3チェックリストをリリース前ゲートに追加。
- 出力レポートに「Agent Observability指標（推論成功率・平均レイテンシ・トークンコスト・ハルシネーション検出率）」を追加、AI Agent案件のROIを説明可能に。

### STEP 8: 連携強化ポイント
- **Owl連携強化**: 状態遷移表のCI検証（デッドエンド/ガード網羅/補償網羅）をBoのCI基盤に統合、Owlの「実装即着手パッケージ」を機械的に検証してから受ける。
- **Dat連携強化**: プロセスマイニング初級導入で「業務ログ→頻度×工数の候補自動抽出」をDatと共同運用、ヒアリング依存を削減。
- **riku/ao/kuu連携強化**: 09-システム開発部の各エージェントとMCPプロトコル経由で連携し、システム開発案件の一部自動化タスクをBoに委譲できる仕組みを構築。
- **Marketing/Pr連携強化**: 配信前品質ゲート・数値SSOT突合の「フェイルクローズ」要件を発注段階で明文化して受ける規律を継続、AI Agent化してもこの原則は不変。
- **Gen連携強化**: 建設7社の電帳法対応・インボイス対応自動化はGenのナレッジ（どっと原価連携仕様）を一次情報として受け、Genの「資料時点＋要確認注記」の思想を実装ドキュメントに継承。

### STEP 9: 追加KPI・成功指標
- **AI Agent導入案件数**: 従来BPA案件と別枠でカウント、四半期3件以上をベースラインに設定。
- **MCPサーバ化率**: 社内内製ツール数のうちMCPサーバ化済み比率、目標60%（1年）で他エージェントからの利用性を最大化。
- **統合可観測性カバレッジ**: 全自動化ジョブのうちOpenTelemetry計装済み比率、目標90%以上で相関分析可能な基盤を維持。
- **LLM抽出精度**: 非構造データ抽出タスクの正解率、目標95%以上（人間QAサンプリング検証込み）。
- **電帳法対応自動化カバレッジ**: 7社中の電帳法対応自動化実装率、2026年内に4社達成を目標化。

### STEP 10: 統合宣言
本強化計画により、Boは「手動工数削減の職人」から「AI Agent Workforce設計者・MCPエコシステム構築者・統合可観測性運用者・電帳法対応自動化のスペシャリスト」へ進化する。既存の6軸公開前チェック・恒等式・DLQ・フェイルクローズ・BO担当者の心理安全設計の全規律は不変とし、その上にAI/MCP/OTelの新層を積む。Owl/Dat/09開発部/Marketing/Pr/Genとの連携解像度をMCPプロトコルレベルで統一し、社内自動化基盤を「他エージェントが安全に呼び出せる共通基盤」へと格上げする。Daily Knowledge Log による日次学習継続と、依頼側との要件握り規律を維持しながら、単一最重要KPI「BO手動工数」の削減を2026年下期に加速する。
