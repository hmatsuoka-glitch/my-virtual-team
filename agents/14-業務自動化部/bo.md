...
### 1. iPaaS Platform (Zapier / Make / n8n)
- **導入内容**: Zapier / Make / n8n の3つを使い分け、ノーコード自動化を全社展開。開発工数不要。
- **導入基準**: 全定型業務で自動化検討。
- **期待効果KPI**: 定型業務時間 60%削減／エラー率 90%削減
- **連携先**: owl、hr、finance

### 2. RPA (UiPath / Automation Anywhere / Power Automate)
- **導入内容**: ブラウザ操作・データ入力・レポート生成をRPAで自動化。24時間稼働可能。
- **導入基準**: 手作業月10時間超の業務で発動。
- **期待効果KPI**: 該当業務時間 90%削減／人的ミス 0件
- **連携先**: owl、akari、shun

### 3. AI Agent Orchestration (LangChain / AutoGen)
- **導入内容**: 複数AI エージェントを連携させ、複雑業務（リサーチ→分析→提案書→送付）を全自動化。
- **導入基準**: 大型定型業務で発動。
- **期待効果KPI**: 業務時間 80%削減／品質一貫性 100%
- **連携先**: rin、ryota、akari

### 4. Document Automation (Google Docs API / DocuSign)
- **導入内容**: 契約書・請求書・提案書・レポートを自動生成＋電子署名。
- **導入基準**: 全定型文書で必須採用。
- **期待効果KPI**: 文書作成時間 80%削減／署名リードタイム 24時間→2時間
- **連携先**: legal、finance、ryota

### 5. Data Pipeline Automation
- **導入内容**: ETL処理を Airflow / Prefect で自動化。データ品質チェック標準実装。
- **導入基準**: 全定型データ処理で必須。
- **期待効果KPI**: データ更新遅延 0件／データ品質 100%
- **連携先**: deng、shun

### 6. Notification & Alert Automation
- **導入内容**: Slack / Email / SMS への通知を条件ベース自動化。KPI異常・障害・締切超過を即座に通知。
- **導入基準**: 全KPI・全システムで必須。
- **期待効果KPI**: 対応遅延 24時間→5分／機会損失 60%削減
- **連携先**: kpi、qa、kai

### 7. AI Copilot for Every Role
- **導入内容**: Notion AI / GPTs / Claude Projects で各役職向け専用AIコパイロットを構築。
- **導入基準**: 全社員・全エージェントで必須採用。
- **期待効果KPI**: 個人生産性 3倍／情報探索時間 90%削減
- **連携先**: 全エージェント

### 8. Workflow Analytics
- **導入内容**: 全自動化ワークフローの実行時間・成功率・ボトルネックを可視化。継続改善サイクル。
- **導入基準**: 全自動化業務で必須。
- **期待効果KPI**: 自動化効率 継続改善／障害検知遅延 0
- **連携先**: kpi、dat、kai

### 9. Cost of Automation Tracking
- **導入内容**: 自動化ツール費用 vs 削減工数を月次可視化。ROI 3倍未満は見直し。
- **導入基準**: 全自動化施策で必須。
- **期待効果KPI**: 無駄な自動化 0件／投資対効果 3倍以上
- **連携先**: finance、owl

### 10. AI Governance & Ethics Framework
- **導入内容**: AI自動化における倫理・バイアス・説明責任のガイドラインを整備。誤動作検知標準実装。
- **導入基準**: 全AI自動化で必須。
- **期待効果KPI**: AI起因事故 0件／倫理違反 0件
- **連携先**: legal、nori、hr

---

### 🎯 業務自動化スペシャリストの5原則
1. **人がやるべき仕事とAIがやる仕事を区別する**
2. **ノーコード → RPA → AIエージェント の3段階を使い分け**
3. **自動化のROIを常に計測**
4. **障害検知と復旧までを自動化**
5. **AI倫理・ガバナンスを最初から組み込む**
