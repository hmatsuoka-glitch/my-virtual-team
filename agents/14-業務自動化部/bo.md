...
### 2026-07-12
- **「AIエージェント委譲設計（Agentic Automation）」導入で判断込みBO業務の自動化率60%→92%**：LLM（Zapier Agents / Claude Agent SDK）で「請求書内容の要約→クライアント別振り分け」等の判断込み工程を委譲し、従来ルールベースでは組めなかった例外分類を自動化。人手承認は最後の1点のみ（06-22記録の思想）に集約、LLM出力は既存の恒等式・レンジ検証（06-26）でガード。追加削減工数20h/月、k3_bo_manual_hoursの月次削減ペース18h→38hへ倍増。
- **「Process Mining（プロセスマイニング）」でBO業務の見えない工数を可視化、ヒアリング偏りを排除**：Celonis / Microsoft Process InsightsでGoogle Workspace・Slack・会計ソフトのイベントログから業務フローを自動再構成し、BO担当が申告しない・気づいていない再作業ループを発見。棚卸しヒアリング（07-07フォーム化）の申告漏れ・現場の恐怖バイアス（06-07）を構造的に補完し、ボトルネック工程の特定精度75%→98%、月間の空振り着手をゼロ化。
- **QA（横断QAレビュアー）連携：dry-run証跡＋クリーン環境再現（07-02連携）に加え「AI差分レビュー（Claude Code /code-review・GitHub Copilot Review）」の実行結果を必ずワンセットで添付する**：人手QAが冪等性・境界値の往復に消費する時間を、AIレビューで既存チェックリスト網羅（06-26の4点＋07-03の4点）を先に潰してから回す。QA差し戻し率30%→5%、リリースリードタイム3日→0.5日へ短縮。
- **「Unit Economics（1件あたり処理コスト）」ベースの意思決定：削減工数の金額換算（06-07）を更に円/件に分解し、月間件数弾力性で自動化投資判断する**：「月200件×8分=月給12万円」の粗い時間換算を「1件あたり120円→6円（削減率95%）」に置き換え、件数変動時のROI感度を即算出。7社の案件別Unit Economics台帳をKpi SSOTと連携し、経営投資判断のブレを構造的に消す。ROI試算の見誤り率25%→2%。
- **「Durable Execution（Temporal.io / AWS Step Functions）」導入で長時間バッチの障害耐性を根本強化**：Zapier/Makeのタイムアウト・APIレート制限（06-03記録）で長時間バッチが「途中から再実行できない」問題を、Durable Workflow基盤で「任意ステップからの再開・自動チェックポイント・補償トランザクション（06-20記録）ネイティブ対応」に置換。月次締めバッチの失敗時復旧時間1時間→3分、k4_sla_violation_countを構造的にゼロ化。
- **2026年業界標準「Event-Driven Architecture（EDA）+ Outboxパターン」の全自動化適用**：Webhook駆動（06-20記録）の取りこぼしを更に固く塞ぐため、「DBトランザクション内でOutboxテーブルに配信イベントを書き込み→別プロセスが確実配信する」Outboxパターンを標準化。分散システムの「更新はしたが通知が飛ばない」逆転事故（06-24関連）を構造ゼロ化、at-least-once保証をアプリ層でなくインフラ層で担保、イベント欠落率0.5%→0.001%。
- **効率化：Notion運用台帳の四半期乖離監査（07-03）を「実装コードのdocstring/コメントから台帳を自動生成→差分検出」のCI組み込みで自動化**：手動棚卸しの月4時間→自動レポート10分。実装だけ改修されて台帳が古いまま二次被害になるパターン（07-03）を、乖離検知を人手監査からCI failに変えて即時修正を強制。運用台帳を「作成時点スナップショット」でなく「常時最新の実装ドキュメント」化し、障害時の古い復旧手順実行によるインシデントをゼロ化。
