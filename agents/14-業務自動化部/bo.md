...
### 2026-08-30
- **Claude 5 系による "業務プロセスの自動発見" を四半期で実施**：クライアント各社の Slack/Email/Notion ログを Claude が読み、繰り返し実行される手作業パターンを自動抽出→自動化候補として提案する運用。BO担当への職務記録調査が四半期4日→半日に短縮、見つかる自動化ネタも従来比3倍。
- **RPA + BPA のハイブリッド運用パターンを標準テンプレ化**：Windows デスクトップ操作は Power Automate Desktop、Web/API は Make/Zapier、AI 処理は Claude API という3層のスタック標準を確立。新規案件の設計時間が3日→半日に短縮、保守性も向上。
- **建設業クライアント向け「どっと原価↔ freee」自動連携パッケージを商品化**：Gen と協働で日次で原価データ→ freee 会計へ自動転記する GAS/API 連携を7社共通パッケージ化。BPO案件の初期構築期間を2ヶ月→2週間に短縮、追加受注の武器にする。
- **AI エージェントによる "自然言語→自動化フロー" 自動生成**：BO 担当が「毎月末に◯社の請求書を集めて freee に入力してほしい」と話しかけると、Claude が Make のシナリオ JSON を自動生成し、Bo が最終レビューして本番投入。プロトタイプ作成時間が4時間→15分に短縮。
- **フェイルクローズ設計を "3層防御" に強化**：①冪等キー（idempotency key）②実行中フラグ（mutex）③スキーマ検証の3層を共通スケルトンに強制組込。二重計上・多重起動・仕様変更起因の障害を構造的にゼロ化。
- **BO担当の "自動化 NPS" を四半期測定し、定着率を KPI 化**：削減工数だけでなく「この自動化を他部署に薦めたいか」を四半期で聴き、NPS 70+ を目標にする。使われなくなる自動化を早期検知し、放置ゾンビ化を防ぐ。
- **オブザーバビリティ SaaS（Datadog/New Relic）に処理系別実行証跡を集約**：ジョブと同一システムに証跡を置かず、外部 SaaS に追記専用で集約する運用に統一。処理系障害でも証跡が消えない設計を全ジョブに適用し、原因調査の平均時間を4時間→30分に短縮。
- **BO担当の "AI活用スキル底上げ" 月次研修を継続開催**：Claude・Make・GAS の実務ハンズオンを月1回1時間で継続し、BO側でも小規模自動化を自作できる状態を作る。Bo への依頼数を月100件→月30件（本当に難しいものだけ）に減らし、Bo は難易度高い案件に集中できる体制を確立。
