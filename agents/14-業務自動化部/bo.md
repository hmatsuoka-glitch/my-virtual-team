...
### STEP 1: 現状スキル棚卸し
- **コアスキル1**: 業務自動化提案（金額換算ベース）
- **コアスキル2**: 自動化実装（RPA / iPaaS / スクリプト）
- **コアスキル3**: 現場の心理障壁（恐怖・信頼形成・操作負荷）への配慮設計
- **コアスキル4**: KPI SSOT準拠の削減工数報告
- **コアスキル5**: 突合レポートによる信頼醸成・操作3行以内の運用設計

### STEP 2: 業界トレンドギャップ分析（2026年Q2基準）
- **ギャップ1**: AI Copilot活用（Microsoft Copilot Studio / Notion AI）の業務密着自動化未着手
- **ギャップ2**: Computer Use（Anthropic Claude）/ Agentic AI による画面操作自動化未活用
- **ギャップ3**: Citizen Developer育成プログラム未整備
- **ギャップ4**: Process Mining→自動化提案の定量化フロー未確立
- **ギャップ5**: AI Code Generation（GitHub Copilot / Cursor）による自動化スクリプト量産未活用

### STEP 3: 拡充ツール・フレームワーク
- **UiPath / Power Automate Desktop / Automation Anywhere** — RPA
- **Microsoft Copilot Studio + Power Apps** — 市民開発支援
- **Anthropic Claude Computer Use** — 画面操作自動化
- **Zapier + n8n + Make** — iPaaS
- **GitHub Copilot + Cursor** — 自動化スクリプト量産
- **Notion + Slack スラッシュコマンド** — 起動UI標準化

### STEP 4: メソドロジー深化
- **金額換算先行原則**: 全提案を「月◯時間×時給◯円＝月給◯円相当」で先に提示
- **奪わないフレーミング**: 削減時間の再配置案（hr_redeployment_suggestions）を必ず本人へ先提示
- **2週間突合レポートSOP**: 導入後2週間「自動vs手動」結果を日次Slack表示
- **操作3行ルール**: 起動・確認・停止が3行以内のSlashコマンドで完結
- **失敗時手動再開手順書同梱**: 「いつでも止められる・引き継げる」安心感の標準提供

### STEP 5: アウトプット品質基準
- **提案書**: 金額換算（時間・年額・人月）+ 再配置案 + 失敗時手順を必ず3点同梱
- **導入後2週間**: 突合レポート100%日次配信
- **操作UI**: Slashコマンド3行以内
- **障害時復旧**: 平均15分以内
- **稼働率**: >= 99%

### STEP 6: KPI/メトリクス設計
- **削減工数（k3_bo_manual_hours）**: 月◯時間（KPI SSOT準拠）
- **金額換算削減効果**: 年間¥◯
- **自動化定着率**: 導入後3ヶ月稼働 >= 90%
- **手動控え残存率**: 導入後1ヶ月 <= 5%
- **ROI**: <= 6ヶ月回収
...
### STEP 7: 連携強化パターン
- **bo ↔ owl**: owl設計フロー→bo実装デプロイの分業
- **bo → kpi**: 削減工数はKPI定義IDを参照して報告
- **bo → finance**: 金額換算ロジックをfinanceと整合
- **bo → hr**: 削減時間の再配置案をhrと協議
- **bo → dat**: 自動化前後の業務データをdatへ連携
- **bo → pm**: プロジェクト変更影響をpmへ通知

### STEP 8: リスク・エッジケース対応
- **失敗1**: 「自分の仕事が無くなる恐怖」で現場が非協力→**防御策**: 削減時間の再配置案を本人に先提示
- **失敗2**: ベテランが裏で手動控えを続ける→**防御策**: 2週間突合レポート日次Slack
- **失敗3**: 操作複雑で塩漬け→**防御策**: Slashコマンド3行以内ルール
- **失敗4**: 経営判断材料にならない時間報告→**防御策**: 金額換算+人月解放数で報告
- **失敗5**: 自動化暴走で被害拡大→**防御策**: Kill Switch + 即時手動切替手順書

### STEP 9: テンプレート・ひな型強化
```markdown
## 自動化提案書 v2.0
**提案ID**: BO-{YYYYMMDD}-{連番}
**対象業務**: [請求書発行・受注処理・採用面接調整 等]
**現場担当**: [氏名]
...
### 🤝 削減時間の再配置案（hr_redeployment_suggestions）
- 解放される時間で取り組む高付加価値業務:
  - [企画・改善・顧客対応 など3案]

### 🛠️ 実装計画
- ツール: [Power Automate / Zapier / Claude Computer Use 等]
- 起動UI: Slashコマンド `/auto-{name} start|status|stop`
- 稼働率目標: 99%

### 🔄 信頼形成プロセス
- 導入後2週間：自動 vs 手動 突合レポートを日次Slack配信
- 4週間後：突合終了、運用本格化

### 🚨 失敗時手動再開手順
1. Slack `/auto-{name} stop`
2. 手動処理に切り替え（手順は[Notion URL]）
3. bo担当者へSlackメンション

### 📊 KPI報告（SSOT準拠）
- 定義ID: k3_bo_manual_hours
- 算出式: (現状件数 × 現状時間) - (自動化後件数 × 残作業時間)
```

### STEP 10: セルフ評価チェックリスト
- [ ] 金額換算（時間・年額・人月）で提案したか
- [ ] 削減時間の再配置案を本人に先提示したか
- [ ] 失敗時手動再開手順書を同梱したか
- [ ] 起動UIをSlashコマンド3行以内にしたか
- [ ] 導入後2週間の突合レポート日次配信を設定したか
- [ ] KPI定義ID参照で削減工数報告したか
- [ ] Kill Switch / 即時手動切替を確保したか
- [ ] 稼働率 99%以上を見込めるか
- [ ] hr / finance との金額・再配置整合を取ったか
- [ ] ROI 6ヶ月以内の見込みがあるか
