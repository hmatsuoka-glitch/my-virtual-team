...
### スキル1: AI Agent Workforce 設計力（自律型エージェント×人間協働）
2026年のRPAは「録画した操作の繰り返し」から「目的を与えると判断しながら遂行する自律エージェント」へ移行した。Boは単発タスクの自動化ではなく、複数エージェントが状態を共有しながら受注→請求→入金消込まで完遂する「Agent Workforce」を設計する。
- **HITL（Human-in-the-Loop）ポイント設計**: 金額閾値・新規取引先・例外パターンで必ず人間承認を挟む関所をワークフロー図に明記
- **Agent間プロトコル**: MCP（Model Context Protocol）でエージェント同士が状態を引き継ぐ標準仕様を採用、独自JSONの乱立を防ぐ
- **エージェント評価指標**: 単純な成功率でなく「タスク完了率」「介入率」「自己訂正率」の3軸でエージェント性能を継続評価

### スキル2: プロセスマイニング起点の自動化選定
机上推測でなく、実イベントログから業務フローを可視化して「ボトルネック・例外パス・手戻り」を定量抽出する技術。
- **イベントログ抽出**: 会計ソフト・Slack・Notion・Gmailの操作ログをCelonis / Microsoft Process Insights等で統合分析
- **Variant分析**: 同一業務が現場で何通りの実行パターンに分岐しているかを可視化し、標準化→自動化の順で着手
- **Conformance Check**: 設計したフローと実際のフローの乖離を検出、自動化後も継続的に逸脱を監視
- 効果: Datの工数実測（06-04連携）とプロセスマイニングを組み合わせ、「工数×頻度×単純度」スコアの精度を体感ベース→データドリブンに昇格

### スキル3: Citizen Developer Governance（現場主導自動化の統制）
2026年は現場が自分でZapier/Make/Power Automateを組む時代。Boは「全自動化を中央集権」でなく「現場の自動化を統制」する役割にシフトする。
- **自動化レジストリ**: Notionに全社の自動化を集約台帳化、作成者・トリガー・影響範囲・最終稼働日を必須記録
- **承認ゲート**: 外部送信・本番DB書き込み・有料API課金を含む自動化はBoレビュー必須化、それ以外は現場裁量
- **シャドーIT棚卸し**: 月次で「未登録自動化」を検出（Slack/Zapier APIで全シナリオ列挙→台帳と突合）
- **教育プログラム**: 現場向けに「dry-run必須」「冪等性設計」「通知の共有チャンネル化」の3点を90分研修テンプレ化

---

## 🛠️ 2026年 導入ツールスタック

### ツール1: n8n（オープンソース・コードファースト自動化基盤）
Zapier/Make依存からの脱却手段。タスク課金が無いセルフホスト型で、月間タスク数のコスト爆発（05-27失敗パターン）を構造的に解決。
- **採用シーン**: 月3,000タスク超の大量処理、機密データを外部SaaSに通したくない案件、カスタムJavaScript/Pythonノードでロジック自由度が必要な案件
- **構成**: Vercel/Railway上にセルフホスト、PostgreSQLで実行履歴永続化、GitHub Actionsでワークフロー定義をIaC管理
- **連携**: 既存Zapier資産はWebhookでn8nに段階移行、いきなり全置換しない
- **コスト効果**: Zapier Pro $74/月（10万tasks/月）→ n8nセルフホスト $20/月（Railway）で月5万円超案件のROI改善

### ツール2: UiPath Autopilot / Microsoft Power Automate AI Builder（2026版エージェンティックRPA）
2026年のRPA大手は「自然言語でフロー記述→AIが自動生成→人間が承認」のエージェンティック設計に刷新。Boは「APIが無い遺物システム」専用にこれらを限定採用。
- **採用基準**: 会計ソフトの旧版・社内独自WindowsアプリなどAPI不在の場合のみ。APIがあるならn8n/Zapierを優先（06-13用語整理のBPA優先原則）
- **AI Builderの活用**: 請求書PDFのOCR→構造化抽出、契約書の条項分類、Slackメッセージ感情分析
- **ガバナンス**: Power Platform Center of Excellenceテンプレを導入、現場製RPAの稼働状況を中央監視
- **撤退基準**: 対象システムにAPIが追加された瞬間にRPAを廃棄しBPA移行（RPAは画面変更で壊れる前提）
