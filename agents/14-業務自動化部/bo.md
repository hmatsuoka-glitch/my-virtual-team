...
### 2026-09-10
- **[新スキル：MCP (Model Context Protocol) + n8n / Make の "エージェント統括レイヤー" 実装]**：Claude/GPT-4o エージェントを Bo の統括レイヤーとして置き、下位に n8n（自ホスト・オープンソース・無料）＋ Make（複雑分岐用）を組み合わせた 3 層構造で判断込み自動化を実現。従来 Zapier のみで組んでいた「1 タスクあたり 200〜500 円/月」のコストが、n8n 自ホスト化で 1/10 に圧縮。同時に MCP 経由で freee/Notion/Slack/GitHub/Airwork を横断操作でき、07-27 記録の判断込み自動化を実務で使える形へ落とし込む。
- **[新ナレッジ：AI Agent Observability 標準 "OpenTelemetry for LLMs" と Anthropic 監査ログ実装]**：判断込み自動化（08-03 記録）の思考トレース・入力/出力・使用ツールを構造化ログで残す仕組みとして、OpenTelemetry の LLM 拡張（W3C 提案中の Semantic Conventions for GenAI）と Anthropic の Prompt Caching + Session Logs を組み合わせる。誤処理時に「どの入力で何を判断したか」を秒単位で遡及可能。BigQuery へストリーム保存し、Dat の DID 分析へも接続する。
- **[効率化ルール：`bo new` 生成器の "ワンコマンド 12 要素" 完全自動化]**：09-01 記録のスケルトン生成を発展させ、`bo new <社名> <ジョブ名>` で「① フェイルクローズ ② 冪等処理 ③ 件数突合ラッパー ④ 台帳登録 ⑤ 通知2系統振り分け ⑥ 現場向け1枚（停止権限・停止手順） ⑦ Owl 確定/下書きフラグ ⑧ Kpi 欠測フラグ ⑨ サービスアカウント認証（09-02 記録） ⑩ 営業日/休業日判定（09-09 記録） ⑪ AI 呼び出しハードリミット（09-09 記録） ⑫ OpenTelemetry ロガー」の 12 要素を全て生成物に含める。従来 30分の初期設定が 3分に短縮（90% 削減）。
- **[失敗パターン→回避策：AI エージェントの "無限ループ" と "コンテキスト汚染"]**：ツール呼び出しの再帰・自己ループで API 課金爆発（09-09 記録）と、前回セッションの状態が次のセッションに混入する Context Pollution が発生。回避策は「① 最大再帰深さを 5 に固定 ② セッション終了時にコンテキスト明示クリア ③ 判断が 3 回連続で同じツールを呼んだら強制終了 ④ ハードリミット到達時は Slack へ Diagnostic Report を自動送信」の 4 点を全 AI エージェントジョブへ標準実装。08-05 記録の決定論/判断分離を維持しつつ、判断側の暴走を構造的に防ぐ。
- **[連携強化：Owl との "変更遷移ライフサイクル" の Event Sourcing 化]**：Owl の状態遷移（下書き→確定→補償イベント）を、Bo が Google Cloud Pub/Sub or Kafka の Event Store に全て記録し、下流ジョブ（Finance 会計仕訳・PM ガントチャート更新・Pr リリース差止め）へ Fan-out。従来 Owl → Bo が 1:1 の同期呼び出しで結合が強かったのを、Event Sourcing で疎結合化。障害時のリプレイ・再構成が容易になり、07-16 記録の受注ボタン取り消し導線も自動で全下流に伝播する。
- **[品質チェック：Sora QA 提出前の "自動化リリース 8 点ゲート"]**：① ゴールデンテスト CSV パス ② スキーマ検証・恒等式チェック稼働 ③ フェイルクローズ設定 ④ ハートビート受信済み ⑤ 台帳登録・現場向け1枚生成 ⑥ 停止権限文書化 ⑦ AI エージェントのハードリミット・観測ログ ⑧ dry-run で本番影響ゼロ、の 8 点を CI で自動判定してから Sora へ回す。Sora の技術品質チェックを構造的に代行し、判断・戦略観点に労力を集中させる。
