...
現状の Bo は Zapier/Make/GitHub Actions・冪等性/DLQ/サーキットブレーカー・オブザーバビリティ・現場向け1枚まで日本の中堅BPO自動化として高水準に到達。ここからグローバル AI Agent Workforce（UiPath/Automation Anywhere を活用する Fortune 500 の Automation CoE）と同格の Hyperautomation Platform を構築するため、以下10項目を追加装備する。

### 1. n8n Self-hosted 導入（Zapier/Make 依存からの脱却）

- **スキル**: Zapier/Make のタスク数課金・SaaS依存から、n8n Self-hosted（オープンソース、$0〜VPS費のみ）へ主軸移行。オンプレ/VPS運用でタスク数制限撤廃、Airtable/Notion/建設業SaaS/どっと原価まで無制限接続。
- **適用**: 高頻度・大量処理ジョブ（月10,000タスク超）を n8n へ、低頻度・簡易ジョブは Zapier 継続の階層戦略。Docker Compose で 7社分のワークフローを namespace 分離、GitHub 管理で全変更をコード履歴化。
- **KPI**: 自動化課金 月2万円→月3千円（VPS費のみ）／タスク数上限撤廃／ワークフローのバージョン管理率 0%→100%。

### 2. UiPath / Automation Anywhere / Power Automate によるレガシー画面RPA

- **スキル**: APIなし建設業SaaS（電子受発注・原価管理・給与）の画面操作を Attended/Unattended RPA で自動化。UiPath Community Edition（無料）→ Enterprise、または Power Automate Desktop（Microsoft 365 付帯）を業務量で選択。
- **適用**: 06-13記録の「APIあるならBPA、なしならRPA」原則で、どっと原価・レガシー会計ソフトの画面転記を UiPath Attended（担当PC上で監視付き）で自動化。画面変更検知（06-03記録のスキーマ検証相当）を Computer Vision で実装し、UI改修時は即警告。
- **KPI**: 月次締めのBO手動工数 月30h→月5h／RPA破損時の検知時間 数日→当日／画面転記業務のカバー率 20%→80%。

### 3. MCP (Model Context Protocol) 標準採用による AI Agent Workforce 基盤

- **スキル**: 07-27/08-03記録の MCP トレンドを本番採用。Claude Code / Cursor / n8n / Zapier Agents が MCP サーバー経由で複数システム横断操作。判断込み自動化（例外分岐解釈）を LLM エージェントに委任、決定論処理はコード（08-05記録のハイブリッド設計）。
- **適用**: 建設業クライアント7社の Notion / Airtable / どっと原価 / freee / マネーフォワード に MCP サーバー実装、Claude Opus 4.7 エージェントが「請求書の記載揺れ解釈」「原価科目分類」「例外承認判定」を実行。判断根拠は追記専用ログ（08-03記録）で全保存。
- **KPI**: 判断込み自動化のカバー範囲 20%→60%／例外分岐の人手判断工数 月20h→月3h／MCP経由の横断操作ジョブ数 0→50件/月。

### 4. GitHub Actions Reusable Workflows によるCI/CD自動化基盤

- **スキル**: dry-run→idempotent検証→通知設定→本番反映を全ジョブ共通の GitHub Actions Reusable Workflows として実装（06-16記録の共通化を強化）。案件は数行のYAML呼び出しだけで本番反映。
- **適用**: `.github/workflows/bo-deploy.yml` を全自動化リポジトリで参照可能に、Job matrix で7社×環境（dev/staging/prod）を並列検証。ゴールデンテストCSV（06-16記録）・件数突合ラッパー・金額レンジ検証（06-17記録）を CI に強制組み込み。
- **KPI**: 新規自動化の本番反映リードタイム 3日→当日／検証工程の実装漏れゼロ／S案件の週次リリース本数 週2件→週10件。

### 5. オブザーバビリティ基盤（Datadog / Grafana + OpenTelemetry）

- **スキル**: AIエージェント判断根拠・入力/出力・使用ツールを OpenTelemetry で構造化ログ化、Datadog Log Management（$0.10/GB）または Grafana Loki（Self-hosted）で保全。08-03記録のオブザーバビリティ要件を本番実装。
- **適用**: 全自動化ジョブに OTel SDK 組み込み、trace_id で「Sales受注ボタン→ Owl 状態遷移→ Bo handoff配布→ Finance 見積→ 現場着手」までの全社横断フローを1画面追跡。誤処理時に trace_id 1つで全経路遡及可能。
- **KPI**: 誤処理の原因特定時間 平均4h→15分／会計連携ジョブの実行証跡100%保全（07-03記録の電帳法対応）／障害の根本原因不明率 30%→5%。

### 6. Airtable Automations + Notion Automations によるノーコード業務DB化

- **スキル**: 7社別マスタ・運用台帳（06-03記録）・DLQ再処理（06-23記録）・OAuthトークン期限監視（07-01記録）・停止権限台帳（08-16記録）を Airtable/Notion で一元管理。実装エンジニア不在でも現場BO担当が編集可能。
- **適用**: `bo new <社名> <ジョブ名>` コマンド（09-01記録）が Notion Automations で運用台帳・現場向け1枚・停止手順・通知2系統設定を自動生成。四半期乖離監査（07-03記録）も Notion Automations で自動リマインド。
- **KPI**: 台帳-実装乖離ゼロ／四半期監査時間 8h→1h／現場BO担当による台帳更新率 20%→80%。

### 7. iPaaS階層戦略（Zapier=SMB / Workato=Enterprise / n8n=Self-hosted）

- **スキル**: 対象業務のトランザクション量・監査要件・課金モデル・機微データ有無で iPaaS を使い分ける階層戦略。単一ツール依存を回避し、業務特性に最適化。
- **適用**: 低頻度・簡易＝Zapier（月$30/席）、複雑ワークフロー＝Workato（月$1,000〜、Enterprise Governance）、高頻度・機微データ＝n8n Self-hosted（項目1）。ジョブ選定は「頻度×監査要件×コスト」の3軸で自動判定するフローチャートを運用台帳に固定。
- **KPI**: 自動化ROI 前年比 +40%／ジョブ選定の意思決定時間 1日→30分／単一ツール障害時の影響範囲 全体→33%以下。

### 8. Peppol / JP PINT デジタルインボイス受信基盤

- **スキル**: 08-03記録の Peppol/JP PINT トレンドを本番採用。PDF/OCR前提から構造化データ受信前提へ設計転換、電帳法完全準拠を維持しながら請求書処理を自動化。
- **適用**: 建設業クライアント7社の請求書受信を Peppol Access Point 経由の構造化データ受信に段階移行、下請け・仕入先へ Peppol 対応を Gen 経由で啓発。OCR経路は移行期間中のフォールバックとして併存、スキーマ検証（06-03記録）でフォーマット変更を自動検知。
- **KPI**: 請求書処理工数 1件8分→1件30秒／OCR起因の誤読エラー 月20件→月0件／電帳法対応の実行証跡100%保全。

### 9. Human-in-the-loop 承認UI共通部品（Retool / Appsmith）

- **スキル**: 08-03/08-05記録の Human-in-the-loop 承認UI標準化を Retool（$10/席/月）または Appsmith（Self-hosted 無料）で実装。承認待ちキュー＋ワンクリック承認/差し戻し＋監査ログを共通部品化。
- **適用**: 7社×全ジョブの承認画面を Retool の1つの App で統合、可逆性で全自動と承認要を切り分け（07-16記録）た設計を UI 側に集約。承認者・承認日時・判断根拠を追記専用ログに全保存、監査対応可。
- **KPI**: 承認UI実装工数 1ジョブ2日→30分／社別実装差分ゼロ／承認応答時間 平均24h→平均2h（モバイル対応 Retool アプリで）。

### 10. プロセスマイニング（Celonis / UiPath Process Mining）

- **スキル**: 業務ログ（システムログ・API呼び出し履歴・タイムスタンプ）から真の頻度・工数・例外パターンを実測抽出、机上推測・現場ヒアリングの恐怖バイアス（06-07記録）ゼロ化。Celonis または UiPath Process Mining を選択。
- **適用**: 建設業クライアント7社の会計・原価・請求システムのログを Celonis に接続、実際の処理経路（Happy Path/Deviation）を可視化。自動化候補スコア（工数×頻度×単純度／05-26記録）の入力を Celonis 実測値に完全置換し、Dat連携（06-04記録）を強化。
- **KPI**: 空振り自動化案件（低頻度業務への着手） 60%→10%／自動化ROI 前年比 +50%／プロセス改善提案の意思決定サイクル 四半期→月次。
