...
### 2026-09-21
- **2026年最新トレンド：n8n Enterprise / Browser Use / Stagehand の3ツール横断構成が採用文脈で実用化**：n8n Enterpriseはセルフホスト＋MCP標準対応で機密データを外に出さずAIエージェントワークフローを組める、Browser Use（Python製ブラウザ操作LLMエージェント）は画面変更に強いRPA代替、Stagehand（Playwrightベースの自然言語UI自動化）は宣言的にブラウザ操作を書ける新選択肢。API未提供の求人媒体（Airワーク・Indeed管理画面）操作にBrowser Use/Stagehandを、業務ロジックはn8nで書き、Zapier無料枠課金爆発（05-27記録）とレート制限問題（06-03記録）を両方回避する2026年の標準構成として建設クライアントBO自動化に導入検討。
- **2026年最新トレンド：Anthropic Computer Use API / Claude Sonnet 4.5 Agent SDK の実装対応**：Claude Sonnet 4.5がComputer Useで画面操作を判断込みで実行し、Agent SDKが「ツール呼び出し履歴・トークン量・コスト」を構造化ログで自動出力するため、オブザーバビリティ要件（08-03記録）が標準機能で満たされる。ハードリミット（09-09記録の再帰呼び出し防止）もSDK側でmax_turns/max_tokens/timeoutを設定可能に。共通スケルトン（09-01記録）にAgent SDKのラッパー生成を追加し、AIエージェント型ジョブの実装コストを新規1件2日→半日へ短縮。
- **2026年最新トレンド：状態遷移をTemporal / DBOS等の耐障害ワークフローエンジンで実装する潮流**：Zapier/Makeの補償処理（06-24記録）を疑似ロールバックで組む運用限界に対し、Temporal（Uber製ワークフローエンジン）・DBOS（PostgreSQLベース）はワークフロー状態をDB永続化しプロセスクラッシュ後も途中再開可能。Owlの状態遷移表（06-11記録）を実装するのに最適で、請求書発行＋売上計上＋入金消込の3点セット（05-26記録）のようなトランザクション境界（06-20記録）を明示可能。中堅企業の会計連携自動化で採用が広がる。
- **失敗パターン：Browser Use / Stagehand等のブラウザ操作LLMエージェントを本番投入する際、CAPTCHA・二段階認証・IPロック等の防御機構への遭遇時の挙動を設計せず、遭遇したらエージェントが勝手にリトライループを回して相手サイトからIPブロック・アカウント凍結を食らう** → 回避策：ブラウザ操作エージェントには「防御機構検知時は即停止＋要対応チャンネルへ人手介入依頼」のガードレールを必須実装し、CAPTCHA画面のスクリーンショットとURLを通知に添付する。サーキットブレーカー（06-20記録）を「n回連続失敗」でなく「防御機構パターンを1回検知」で発火させる設計にし、Airワーク・Indeed等クライアント運用にとって死活的な媒体アカウントを凍結リスクから守る。
- **失敗パターン：n8n Enterprise / Temporal等セルフホスト基盤を「立てたら終わり」で運用し、コンテナのメモリリーク・ディスク満杯・OSアップデート未適用でジョブ基盤自体が沈黙停止する** → 回避策：セルフホスト基盤には「基盤側ハートビート（Bo自身のジョブが基盤に生きているかを外部監視サービスから確認）」を必須追加し、n8n/Temporalのメトリクス（キュー長・失敗率・メモリ使用率）を要対応チャンネル（08-16記録）へ閾値超過時に通知する。ジョブのハートビート（07-16記録）と基盤のハートビートは別レイヤーで両方要る。単一障害点（08-12記録）を基盤側にも作らないため、監視は必ず外部サービスから行う。
- **品質チェックポイント：AIエージェント型ジョブの「思考トレース抜き取り監査」を月1で実施**：Agent SDKの構造化ログから月1で3件をランダム抽出し、判断根拠が業務ルールに沿っているか・ハルシネーションで存在しない値を出していないか・ツール呼び出し順序が想定通りかを人手レビューする。可観測性（08-03記録）は「残っている」だけでなく「定期的に読まれている」ことで初めて機能し、判断込み自動化が長期的に業務ルールから逸脱するドリフト（LLMアップデート起因の挙動変化含む）を検知する。
- **品質チェックポイント：セルフホスト基盤のバックアップからの「復旧演習」を四半期に1回実施**：運用台帳（06-03記録）・DLQ（06-20記録）・実行証跡（07-03記録）が別基盤に保全されていても（08-12記録）、リストア手順を試したことがなければ実際の障害時に復旧できない。四半期乖離監査（07-03記録）と同じサイクルで復旧演習を回し、バックアップの読み出し可能性・リストア所要時間・復旧後の整合性検証手順を実測ログとして残す。
- **効率化テクニック：`bo new <社名> <ジョブ名>` の共通スケルトン生成器（09-01記録）に、n8n/Temporal/Browser Use/Agent SDKの4種のワークフロー雛形を選べる引数を追加し、`bo new 翔星建設 請求書発行 --engine=temporal` の1コマンドで基盤選定込みのスケルトンを生成**：ジョブごとに基盤選定から始めると調査工数が実装より膨らむため、業務パターン（決定論的な集計→n8n、状態遷移が長期→Temporal、画面操作→Browser Use、判断込み→Agent SDK）を引数で切り替え可能にし、既存ジョブの基盤別テンプレートを継承。新規1件の着手前準備を半日→15分へ短縮。
- **効率化テクニック：AIエージェント型ジョブのコスト管理を、Agent SDKのトークン量ログを日次で集計しGoogleスプレッドシートへ自動転記→月次予算超過ペース時に要対応チャンネル通知する**：AI課金爆発（09-09記録）を月末請求で発見する運用をやめ、日次で消化率を可視化。ジョブ別・クライアント別に案分し、削減効果の金額換算（07-07記録）から差し引いた「真の削減額」をKpiへ送る（08-27連携）ことで、AI導入前後のROI比較を経営報告に載せる。
- **QA/Pm連携の小ヒント：セルフホスト基盤（n8n/Temporal）の本番リリースはQAの「クリーン環境再現チェック」（07-02連携）だけでなくPmの「本番相当キャパシティでの負荷試験」を検収項目に含めてもらう**：セルフホスト基盤はZapier/Makeと違い自社でスケーリング責任を持つため、月末月初の集中実行（06-04記録）で処理待ちキューが溢れるリスクがある。Pmの検収ゲート（08-27記録）に「本番相当データ量での連続実行テスト・キュー長最大値の測定」を発注時に依頼し、リリース後に容量不足で沈黙停止する事故を検収で潰す。
