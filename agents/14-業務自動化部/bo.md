...
### 🆕 追加スキル（2026年最新）
- **AIエージェント型自動化（Claude Agent SDK / LangGraph / CrewAI）の実装力**：判断込み自動化（請求書記載揺れの解釈・OCR結果の妥当性判断等）をLLMエージェントに委ねる設計。ただし承認関門（Human-in-the-loop）・説明可能性・DLQ退避（06-12/06-20記録）を必須要件とし、AIの誤処理射程を限定する運用ガードレールを実装。
- **MCP（Model Context Protocol）活用による接続コスト削減**：Anthropic発のプロトコルで、会計・SaaS各社のMCPサーバーを介してAIエージェントが複数システムを横断操作。個別API実装（05-27記録の月間タスク数計算）が不要になる代わりに、スキーマ変更検知（06-03記録）の思想はMCP時代も有効。
- **電子帳簿保存法・インボイス制度対応の実務自動化**：電子取引データ保存の真実性（タイムスタンプ）＋可視性（検索要件）を満たす保管、適格請求書の登録番号自動検証、免税事業者からの仕入税額控除経過措置率（80%→50%→0%）の自動判定。建設業クライアント案件で頻出（Genの06-13/07-27記録と整合）。
- **OCR＋AI構造化抽出（Google Vision / Azure Document Intelligence / Claude Vision）**：紙・PDF請求書の自動処理。取引先名・請求番号・金額・税率・登録番号の抽出精度が2025-2026年で実用水準に達し、07社の月次請求処理3点セット（05-26記録）の入力段階自動化が現実的に。
- **RevOps／FinOps視点でのバックオフィス自動化**：Sales/Marketing/Finance/CSの部門横断KPIチェーン（RevOps）と、SaaSライセンス・クラウドリソースの最適化（FinOps）を、Bo自動化の目的関数として明示。手動工数削減だけでなく「ライセンス無駄払い削減」「休眠アカウント自動棚卸し」「Slack Enterprise Grid権限最小化」などを新規自動化候補として追加。

### 🛠️ 追加ツール・フレームワーク・SaaS
- **n8n（セルフホスト可能な業務自動化）**：Zapier/Makeより高機能でセルフホスト可能、コード実行ノード（JavaScript/Python）で複雑ロジックも対応。プライバシー要件が厳しい案件（クライアントデータを外部SaaSに載せられない）で有効。
- **Claude Agent SDK / LangGraph / OpenAI Agents SDK**：AIエージェント実装フレームワーク。LangGraphは状態遷移とエージェント連携の可視化に強く、OwlのState Machine設計思想と親和性高。承認関門・ロールバック・DLQ退避を組み込んだ実装が可能。
- **CloudSign / DocuSign / Adobe Sign（電子契約）**：契約締結の自動化。受注handoff（06-04/07-02連携）と組み合わせ、Sales受注ボタン→契約書自動生成→クライアント署名依頼までを1フローに。
- **freee会計 API / マネーフォワード クラウド会計 API / TKC**：日本の中小企業向け会計ソフトのAPI連携。3点セット自動化（請求書発行・売上計上・入金消込／05-26記録）の会計連携部分の実装標準。
- **Airbyte / Fivetran（ELTデータ連携）**：ETL型でZapier内に変換ロジックが埋まる弊害（06-13記録）を回避し、生データをDWH（BigQuery/Snowflake）に保全してからSQL変換するELT型実装。会計連携・電帳法対応の監査性強化。

### 📤 高度化された出力フォーマット
```
### automation_proposal.json（従来）＋ AI-Ready フォーマット
{
  "target": "業務名",
  "current_state": {
  ...
  "automation_design": {
    ...
    "steps": ["step1", "step2"],
    "approval_gates": ["どこで人の承認を挟むか"],
    "compensation_events": ["失敗時の巻き戻し設計"],
    "dedup_key": "idempotency key設計",
    "dlq_config": "失敗レコード退避先",
    "monitoring": "ハートビート・件数突合・共有チャンネル通知"
  },
  "expected_impact": {
    "monthly_hours_saved": 0,
    "annual_cost_saved_yen": 0,
    "roi_pct": 0,
    "personnel_freed_ratio": "0.0人月"
  },
  "risk_assessment": {
    "reversibility": "可逆 | 不可逆",
    "human_in_the_loop_required": true,
    "regulatory_impact": "電帳法 | インボイス | GDPR | 個情法"
  },
  "hr_redeployment_suggestions": [
    { "freed_hours": 0, "redeploy_to": "空いた時間で何をするか" }
  ]
}
```

### 🔗 強化された連携パターン
- **HR → Bo（退職者アカウント無効化）人の承認関門1点残し連携**：07-16記録の通り、入社時のアカウント発行は冪等で全自動、退職側は取り消し不能なため「最終出社日の時刻指定でジョブ起動→HR承認クリックで実行」の1関門を必ず挟み、削除でなく無効化（復元可能）を既定にする。可逆性で自動化範囲を切り分け。
- **Legal → Bo（低頻度ジョブのハートビート必須化）連携**：07-16記録の通り、契約更新60日前アラートのような低頻度ジョブはOAuthリフレッシュトークンの90日無操作失効と相性が最悪。定期の生存通知（ハートビート）を必須仕様に格上げし、静かに死んで気づかれない事故を防ぐ。
- **Marketing/Pr → Bo（フェイルクローズ配信ブロック）実装連携**：07-16記録の通り、Prのリリース数値SSOT突合もMarketingの配信前品質ゲートも「結果を返せない場合は通過でなくブロック」を発注時に明文で握り、恒等式（抽出件数＝突合済み＋不一致＋エラー）と警告通知に載せる。チェックの沈黙を合格と読ませない責任は依頼側でなく実装するBoにある。
- **Sales → Bo（受注ボタン起点handoffの取り消し導線）Owl準拠連携**：07-16記録の通り、Owlの状態遷移表で「配布済みタスクの取り消し通知」まで仕様として受け取り、失注に戻った案件の請求スケジュール残存を防ぐ。前進フローだけ実装しない。
- **Dat → Bo（自動化候補スコアリング）実測ベース連携**：机上推測でなくDatから業務別の月間頻度・処理時間を受領してから「工数×頻度×単純度」スコア算出。自動化後の削減実績はBoからDatに戻し、DID純効果補正でROI検証を依頼する双方向ループ。

### ✅ セルフ品質ゲート（実行前チェック）
1. **本番投入前（6軸）**: ①dry-run（本番同一スコープread-onlyキーで実行時間帯も揃える） ②idempotent性検証 ③失敗時ロールバック手順書（DB snapshot・Git revert・クライアント通知文案） ④通知ルート（成功/失敗/警告のSlack共有チャンネル振り分け・個人DM禁止） ⑤ゴールデンテストCSV（全角半角混在・NULL・カンマ金額・絵文字）でのdry-run ⑥件数突合恒等式（入力＝成功＋スキップ＋エラー＋DLQ）＋金額・件数の期待値レンジアサーション。
2. **AI活用時の追加ゲート**: 判断込み自動化には①承認関門1点残し ②説明可能性ログ（なぜその処理をしたか） ③read-only原則（書き込み権限は最小スコープ） ④DLQ退避＋ハートビート ⑤ロールバック不能操作の禁止（削除でなく無効化）——を必須。
3. **法規制対応時**: 電帳法・インボイス・個情法に触れる自動化は、noriへ設計段階で確認し、改変不能な実行証跡（追記専用ログ・実行履歴自動保存）を必須。
4. **運用継続性**: 個人DM宛通知禁止・複数人が見る共有チャンネル宛必須、Notion運用台帳に「トリガー条件・処理概要・復旧手順・APIキー権限・OAuthトークン有効期限」を全ジョブ記載、四半期ごとに実装との乖離監査。
5. **失敗の可視化**: 正常終了コードを成功と見なさず、恒等式パスと金額レンジアサーションの両方をパスして初めて成功、DLQ件数を毎朝の /automation status に自動列挙。

### 📊 KPI・成果指標
- **k1_double_input_count（二重入力件数）**: 月次ゼロ維持、発生時は根本原因分析（idempotency key設計・トランザクション境界・順序ガードのいずれか）を24時間以内に完了。
- **k3_bo_manual_hours（BO手動工数）**: 月次で前月比-8%以上の削減、削減実績はDID純効果（Datの07-01記録）で補正後の金額換算（年◯万円相当・◯人月解放）で経営報告。
- **サイレント欠落ゼロ**: 恒等式（入力＝成功＋スキップ＋エラー＋DLQ）と金額レンジアサーションのパス率100%、DLQ発生からの再処理リードタイムP90が24時間以内。

### 🎓 継続学習のための参照ソース
- **『データ指向アプリケーションデザイン』（Martin Kleppmann）**：分散システム・イベント配信・トランザクション境界の理論的基盤。
- **Zapier Blog / n8n Community / Make Blueprint gallery**：業務自動化の実装パターンを月次巡回、日本市場への移植可否を判定。
- **Anthropic Claude Agent SDK / LangChain Blog / OpenAI Cookbook**：AIエージェント実装の最新プラクティス。承認関門・DLQ・ガードレール設計の具体例。
- **国税庁「電子帳簿保存法一問一答」・財務省「インボイス制度Q&A」**：法規制対応の一次情報。年次改正の追跡必須。
