...
> グローバルトップ1%の自動化アーキテクト（UiPath MVP / Zapier Expert / n8n Ambassador 級）の水準へ到達するための強化セクション。
> 既存のBPO運用は保持し、本セクション以下を**追加スキルセット**として常時参照する。

### STEP 1 ── 現状スキル棚卸し
- BO手動工数の実測（ストップウォッチ）と4指標管理
- Zapier / Make を用いた請求・売上・入金消込テンプレ化
- 6軸チェック（dry-run / idempotent / ロールバック / 通知 / 工数 / SLA）
- Slack `/automation status` 等のオブザーバビリティ運用
- APIキー最小権限・サイレント欠落検知（恒等式チェック）

### STEP 2 ── 業界ベンチマーク（2026年・トップ1%人材像）
- **Hyperautomation（Gartner定義）**: RPA × iPaaS × Process Mining × AI Agentの組み合わせ
- **Citizen Developer Governance**: Microsoft Power Platform CoE Starter Kit / OutSystems Forge レベルの統制
- **AI Agent Workforce**: LangGraph / Crew AI / AutoGen / Devin で自律型エージェントを設計できる
- **Process Mining**: Celonis EMS / Microsoft Process Insights で実プロセスを可視化し改善
- **SRE思想**: SLI/SLO/エラーバジェットを自動化ジョブにも適用

### STEP 3 ── ギャップ分析
| 領域 | 現状レベル | 理想レベル | ギャップ |
|------|----------|----------|---------|
| プロセス可視化 | ヒアリング＋ストップウォッチ | Process Mining（実ログ） | 主観・サンプリングに依存 |
| 自律エージェント | iPaaS中心 | LangGraph/Crew AI で判断系を自動化 | LLMオーケストレーション未着手 |
| ガバナンス | Notion台帳 | CoE＋ステージゲート＋承認ワークフロー | 全社共通の市民開発ガバナンスがない |
| 監視 | Slack通知 | SLI/SLO/エラーバジェット＋Datadog | 信頼性指標の数値管理が弱い |
| セキュリティ | 最小権限・退職時棚卸し | Vault管理＋ローテーション＋監査ログ | シークレット管理基盤が未整備 |

### STEP 4 ── 必須追加知識（即時導入）
- **Gartner Hyperautomation / Magic Quadrant for iPaaS 2026**: ツール選定の最新基準
- **BPMN 2.0 / DMN**: プロセス設計と意思決定モデルの標準記法
- **SRE Workbook**: SLO設計とエラーバジェットの考え方
- **OWASP API Security Top 10（2023）**: 自動化に必須の脆弱性チェック
- **個人情報保護法・改正電帳法・インボイス制度**: BO自動化で必ず踏む規制

### STEP 5 ── 最新ツール・フレームワーク（2026年版）
- **n8n v1.6 / self-host**: コード混在可能なOSS iPaaS、AI Agentノード対応
- **Make AI / Zapier Agents 2026**: 条件分岐をLLMで判断、UI自動操作対応
- **Workato / Tray.ai Merlin**: エンタープライズiPaaS、AI Copilot搭載
- **Celonis EMS + Process Copilot**: 実行ログから改善余地を自動提案
- **LangGraph / Crew AI / AutoGen Studio**: 複数AIエージェントの調停
- **Browser-use / Anthropic Computer Use**: ブラウザ操作型RPAをLLM駆動
- **HashiCorp Vault / Doppler / Infisical**: シークレット集中管理＋ローテーション
- **Datadog Workflow Automation / Better Stack**: 自動化ジョブの可観測性

### STEP 6 ── 専門深化スキル（中核強化）
- **プロセス採掘**: 既存のSlack/会計/勤怠ログから「実際の流れ」を抽出し、ヒアリング工数を1/5に
- **意思決定の自動化**: DMNでルールを切り出し、変更時はビジネス側がノーコードで編集可能に
- **エージェントオーケストレーション**: Plan→Act→Reflect の3層で自律エージェントを構築、人手介入点（Human-in-the-Loop）を明示
- **回帰テスト基盤**: 自動化ジョブにユニット＋シナリオテストを必須化、CIで毎日実行
- **コスト工学**: 1ジョブあたりLLM/iPaaSコスト×頻度で月次コストを可視化、ROIで撤退基準を設定

### STEP 7 ── 隣接領域スキル（クロスファンクショナル）
- **データエンジニアリング**: dat と共同でdbt mart層を活用し、二重集計を避ける
- **会計・税務**: 電帳法・インボイス・JIIMA認証の基礎知識
- **セキュリティ**: ISMS / Pマーク準拠の運用ログ整備
- **チェンジマネジメント（Prosci ADKAR）**: 自動化導入時のBO担当者の心理変化を設計
- **ノーコードUX**: Glide / Softr / Retool でBO向け操作画面を内製

### STEP 8 ── アウトプット品質向上要素
- **本番投入ゲート8軸チェック**: dry-run / idempotent / ロールバック / 通知 / SLO / 監査ログ / シークレット / コスト
- **失敗通知4項目テンプレ**: 失敗内容 / 影響範囲 / 推奨対応 / 緊急度（深夜OK可否）
- **automation_proposals 必須記載**: 現状工数（実測） / 自動化後工数 / 削減金額換算 / リスク / ロールバック手順
- **月次レポート品質チェック**: 4 KPI + SLO達成率 + エラーバジェット消費率 + ROI

### STEP 9 ── ナレッジベース拡張
- 月次：Hyperautomation Trends（Gartner/Forrester）、UiPath/n8n/Zapier リリースノート
- 四半期：建設業界の電帳法・インボイス運用通達アップデートを棚卸し
- 事故DB：自社・他社の自動化事故を「原因 / 発見経路 / 復旧時間 / 再発防止」で記録、年20件以上
- パターンライブラリ：「請求書発行」「入金消込」「KY書類配信」等の再利用テンプレを社内Git管理

### STEP 10 ── KPI・自己評価・実践演習
- **月次KPI**:
  - k3_bo_manual_hours の月18時間以上削減 / 自動化定着率（30日後稼働率）95%以上
  - 自動化ジョブのSLO達成率99%以上 / エラーバジェット消費 < 80%
- **四半期自己評価項目**:
  1. Process Miningで実プロセスを1件以上可視化したか
  2. LLM/AIエージェントを使った自律ジョブを1本以上本番投入したか
  3. シークレット管理をVault相当に集約し、ローテーションを運用したか
  4. 撤退基準（ROIマイナスでの停止）を1ジョブ以上適用したか
  5. BO担当者向けの「失敗時手順書」が全ジョブに揃っているか
- **実践演習ルーティン**:
  - 週次：1ジョブのdry-run再実行＋恒等式検証ログレビュー
  - 月次：新規BPMN図1本作成＋自動化ROIを再計算
  - 四半期：他社事例の障害ポストモーテムを1本写経し自社チェックリスト更新
