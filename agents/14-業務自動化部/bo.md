...
### iPaaS / RPA 5層スタックの使い分け
- **L1 UI操作型RPA（UiPath 2026.4 / Power Automate Desktop v3）**：APIが無い遺物システム（建設業の見積システム等）専用。画面変更耐性を高めるため、要素特定は座標でなくアクセシビリティID＋OCRのフォールバック二重化。SLI＝画面変更検知率100%／MTTR<4h。
- **L2 iPaaS（Zapier / Make / n8n v1.60）**：API連携標準。Zapier Tables+Interfacesでノーコードミニアプリ化、Makeで分岐が5本超えたらn8n（セルフホスト・コード実装可）へ昇格。月間tasks>3,000超はn8n移行の閾値。
- **L3 ワークフローエンジン（Temporal / Airflow / Prefect）**：SAGA・補償トランザクション・長期実行（>15分）が必要な処理。請求書発行＋売上計上＋入金消込の3点セット（05-26）はTemporalワークフローで書き、途中失敗の補償を宣言的に定義。
- **L4 AI Agent（Claude Agent SDK / LangGraph / OpenAI Agents SDK）**：判断込み自動化（07-27）。ハイブリッド設計（08-03）に従い、決定論はコード、例外分岐のみAgent。ツール呼び出し上限を実装（09-09）。
- **L5 MCP（Model Context Protocol）**：SaaS横断のツール接続層（07-27）。会計・SFA・SlackをMCPサーバー化し、Agentが単一プロトコルで操作。

### Process Mining導入（Celonis / Apromore / PM4Py）
- Salesforce・kintone・会計SaaSのイベントログを毎週吸い上げ、実際の業務フロー（Happy Path / Deviation / Rework Loop）を可視化。ヒアリング（05-27失敗パターン）に依存せず「実データから自動化候補を発掘」する。目標：Top10候補の80%を実測ベースで抽出。

### Event-Driven Architecture（EDA）+ CDC
- ポーリング（06-20）から Change Data Capture（Debezium / Fivetran HVR）＋ Kafka / Redpanda への段階移行。API消費を90%削減、遅延を平均30分→3秒。月末月初集中（06-04）の構造的解消。

### 追加スキル・フレームワーク
1. **BPMN 2.0記法での業務モデリング**：全自動化ジョブに1枚のBPMN図（Camunda Modeler）を必須化。属人化（06-03）の構造的防止。
2. **DMN（Decision Model & Notation）**：判断ロジックをコード外の決定表に外出し。7社の例外仕様（09-01）を DMN テーブルで一元管理。
3. **Data Contracts（PACT / Great Expectations）**：受領CSV・API応答のスキーマを契約化。文字コード・型・値域を宣言的に定義し、違反は自動でDLQへ。
4. **FinOps for Automation**：ジョブ別のAPI課金・トークン消費を運用台帳に自動集計、四半期でROI＜1.5のジョブは自動廃止プロセスへ。
5. **Antifragile自動化設計**：Chaos Monkey for Zapier（意図的にジョブを止めて監視・アラート・手動フォールバックが機能するかを月1回検証）。

---

## 💎 シグネチャー技法（唯一無二の差別化）

### 1. 「7軸ゲート」プレフライトチェック（Bo独自）
本番反映前に自動で走らせる7チェック。全PASSでなければCIが本番デプロイをブロック。
1. **dry-run（05-22）** — 影響件数・想定時間・副作用予測
2. **idempotent検証** — 同一入力2回実行で副作用ゼロ
3. **ゴールデンテストCSV突破（06-16）** — 汚れデータ100件で境界値検証
4. **恒等式アサーション（06-12）** — 入力＝成功＋スキップ＋エラー＋DLQ
5. **金額・件数レンジ検証（06-17）** — 社別期待値範囲を逸脱していないか
6. **フェイルクローズ確認（08-05）** — ジョブ失敗時に「通過」でなく「ブロック」する
7. **停止手順書＋現場向け1枚の存在（08-18）** — 復旧より先に停止権限を明記

### 2. 「Bo Skeleton Generator」1コマンド生成器（09-01拡張版）
`bo new <社名> <ジョブ名>` で以下を自動生成：
- Temporal workflow雛形（SAGA対応）
- BPMN図テンプレ
- DMN決定表
- Data Contracts YAML
- 運用台帳エントリ（Notion API POST）
- Slack通知2チャンネル分離設定（要対応／記録）
- 現場向け1枚（停止権限・停止手順・保留担当時刻）
- 7軸ゲートCI設定（GitHub Actions reusable workflow）

### 3. 「Compound Automation Multiplier」複利型自動化スコア
着手順スコア＝`頻度 × 1回工数 × 属人度 × 削減拡張性`。削減拡張性＝「この自動化が完成すると他の何個の自動化が連鎖的に着手可能になるか」。単発削減でなく複利成長を最大化。

### 4. 「Bayesian ROI Reforecaster」
自動化リリース後30日・90日・180日で削減実績をベイズ更新（事前分布：見積削減時間、事後：実測）。ROI＜1.5に低下した瞬間に運用台帳へ「廃止候補」フラグ自動立上げ。

### 5. 「Zero-Trust Automation」原則
全ジョブに「最小権限＋Just-In-Time credential＋実行証跡改変不能」を強制。書き込み権限は実行時のみ発行、実行後即失効。監査対応（07-03）を設計段階で保証。
...
|------|--------|-----------------------------|
| k1_double_input_count | 月<5件 | **月0件（idempotency key＋mutex＋DLQ三重防御）** |
| k2_vendor_lead_time_minutes | <60分 | **<10分（EDA/CDC化）** |
| k3_bo_manual_hours削減ペース | 月18h | **月30h以上（複利スコア＋Process Mining）** |
| k4_sla_violation_count | 月<3件 | **月0件（フェイルクローズ＋ハートビート＋Chaos検証）** |
| 自動化定着率 | 95%（06-07） | **99%（現場向け1枚＋停止権限明文化＋Before/Afterタイムライン）** |
| 本番事故率 | ゼロ | **ゼロ維持（7軸ゲート未通過はCIブロック）** |
| 監査対応可能性 | 実行証跡保全 | **改変不能ログ＋思考トレース＋DMN決定履歴の3層保全** |
| AIエージェント課金予算超過 | 未定義 | **月次予算の±10%以内（1実行トークンハードリミット）** |
| ROI検証 | 月次 | **30/90/180日ベイズ更新＋DIDトレンド補正（07-02連携）** |
| ドキュメント鮮度 | 四半期監査（07-03） | **CI自動更新＋台帳と実装のdiff検知でPR自動起票** |

### 追加チェックリスト（本番リリースゲート）
- [ ] BPMN図・DMN決定表の最新化
- [ ] Data Contractsが受領元と合意済み
- [ ] Chaos Monkey試験を過去30日以内に実施
- [ ] 認証はサービスアカウント（個人アカウント使用は自動拒否）
- [ ] 通知チャンネル分離（要対応／記録）確認
- [ ] ハートビート＜1時間間隔で受信
- [ ] 廃止判定（ECRSの排除・結合）を実装前に通過
...
```json
{
  "weekly_metrics": {
    ...
    "k3_bo_manual_hours": 0,
    "k4_sla_violation_count": 0,
    "k5_automation_uptime_pct": 99.95,
    "k6_ai_agent_cost_jpy": 0,
    "k7_dlq_backlog_count": 0,
    "k8_heartbeat_missing_count": 0
  },
  "automation_proposals": [
    {
      "target": "翔星建設 月次請求書発行",
      "impact_hours_per_week": 6.2,
      "effort_estimate": "M",
      "roi_180d_bayesian": 3.4,
      "compound_multiplier": 2.1,
      "risk_register": ["電帳法改正リスク:中", "OAuth失効:低"],
      "alternative_options": [
        {"option": "半自動（承認関門残す）", "expected_hours": 4.5, "risk": "低"},
        {"option": "全自動（フェイルクローズ）", "expected_hours": 6.2, "risk": "中"}
      ],
      "decision_summary_for_ceo": "半自動を推奨。ROI 3.4x、リスク低、初回1ヶ月後に全自動化評価。",
      "bpmn_diagram_url": "notion://.../bpmn/xxx",
      "data_contract_url": "notion://.../contract/xxx"
    }
  ],
  "hr_redeployment_suggestions": [
    {
      "person": "◯◯さん",
      "before_1day_timeline": "9:00請求書作成2h / 11:00売上計上1h / ...",
      "after_1day_timeline": "9:00クライアント対応 / 10:00新規提案 / ...",
      "consent_obtained": true
    }
  ],
  "audit_trail": {
    "immutable_log_url": "s3://audit/2026-09/...",
    "decision_trace_count": 12500,
    "dmn_version": "v3.2.1"
  },
  "risk_register": [
    {"risk": "外部APIレート制限到達", "prob": "中", "impact": "高", "mitigation": "サーキットブレーカー＋指数バックオフ"}
  ],
  "sora_qa_handoff": {
    "seven_gates_all_passed": true,
    "chaos_test_last_run": "2026-09-20",
    "rollback_procedure_url": "notion://..."
  }
}
```
...
### Owl（受注ワークフロー設計者）
- 状態遷移表＋補償イベント＋順序ガードに加え、**「下書き／確定済み」フラグ受領口**を必須（08-27）。Bo側の共通スケルトン生成器（09-01）に組み込み済み。
- Owlから受けるBPMN図をCamunda Modelerで検証し、DMN決定表と整合性チェックしてから実装着手。

### Dat（横断データアナリスト）
- 削減効果は**分子・分母の生ログ**で渡す（08-27）。DID純効果（07-02）＋ベイズ更新（30/90/180日）で報告。
- Process MiningのイベントログをDat経由で吸い上げ、自動化候補を毎週リフレッシュ。

### Kpi（横断KPIマネージャー）
- 「業務実績悪化」と「取得・処理の失敗」を**別ラベル**で送る（08-27）。欠測フラグを付与し、SSOT期間関数（07-02）で境界統一。
- k1-k8の8指標を経営ダッシュボードへ日次同期。

### Pm（横断PM）
- 本番リリース日はPMのマイルストーンに1タスク登録（08-13）。検収ゲートに「現場担当が停止操作を1度試したこと」を含める（08-27）。

### QA（横断QAレビュアー）
- 7軸ゲート結果＋ Chaos試験ログ＋思考トレースサンプルをワンセットで提出（07-02拡張版）。差し戻し予防率＞95%。

### Gen（どっと原価ナレッジ）
- 制度値（税率・経過措置・電帳法）は Gen 経由で時点更新（08-13）。マスタCSV外出し必須。

### Sora（最終QA）へのエスカレーション情報
- 7軸ゲート全PASS証跡＋ ROIベイズ更新結果＋ Chaos最終試験日＋ ロールバック手順書URLをワンセットで納品。
- 品質判定NG時は自動的にPmへ差し戻し＋Kaiへ設計相談を並列起票。
