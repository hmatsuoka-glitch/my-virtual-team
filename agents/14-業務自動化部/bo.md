...
サクバズ事業（SNSマーケ×採用支援）のクライアント7社バックオフィス自動化を、2026年下期の「AIエージェント×MCP」標準化を前提に格上げする。既存の「工数×頻度×単純度スコア」「6軸チェックポイント」「DLQ／恒等式／サーキットブレーカー」「立ち上げウィザード」の延長で、上位レイヤ（意思決定・監査・自然言語I/F）を積む。

### スキル拡張

1. **AI Agent Workforce オーケストレーション（LangGraph / CrewAI / Zapier Agents 型の複数エージェント連携）**
   単一Zap／単一シナリオでの直列処理から、「請求書取込エージェント × 与信判定エージェント × 会計登録エージェント」を状態機械で束ねる方式へ移行。各エージェントの責務境界（06-11記録のenum状態遷移思想を エージェント間I/Fに拡張）を明示し、失敗時は該当エージェントだけを再実行できる粒度で組む。1つの巨大ワークフローを直すたびに検証3工程（06-16）が再発する肥大化を、責務分割で解消。

2. **プロセスマイニング（Celonis / SAP Signavio / Microsoft Process Mining）による自動化候補発見の"客観化"**
   BO担当ヒアリング（07-07のNotionフォーム化）と Datの工数実測（06-04）に加え、実際の操作ログ（SaaSのAPI叩かれ順・スプレッドシート編集履歴・Slack操作履歴）からボトルネックを機械抽出する。恐怖バイアス（06-07）で現場が業務を小さく見せる問題を、ログ側の客観データで補正し、候補スコア（05-26）の第4軸「実測乖離率」を追加。

3. **ドキュメントインテリジェンス（Claude 4 Vision / Google Document AI）による請求書・領収書・契約書の構造化**
   RPAで画面OCRするレガシー方式から、LLM Visionで「表・押印位置・手書きメモ・改ざん痕跡」まで含めて抽出する方式に切替。電帳法対応（06-22の建設業ニーズ）と組み合わせ、7社の月次BO処理（05-26の3点セット）の入力側を「紙／PDF → 構造化JSON」で自動化。抽出結果はゴールデンテストCSV（06-16）に金額桁・日付境界（07-01）の期待値レンジで検証必須。

4. **MCP（Model Context Protocol）サーバー自作による社内SSOT接続**
   Notion運用台帳（06-03）・Datの実測データ（06-04）・KPI定義SSOT（07-02）を独自MCPサーバー化し、Claude Code／Cursor／Claude.aiから同一プロトコルで参照できる状態にする。エージェントが「今の削減実績は？」「このジョブの復旧手順は？」を自然言語で聞くだけで運用台帳の最新版を引く体制にし、台帳と実装の四半期乖離（07-03）を「常時同期」に格上げ。

5. **自然言語ジョブ起動 I/F（Slack `/ask-bo` + LLMルーティング）**
   `/automation status`（05-26）を土台に、「翔星建設の先月分の請求書だけ再発行して」「宮村建設のDLQ全部バックフィルして」を自然言語で受け付け、LLMが該当ジョブ＋パラメータ＋承認関門（07-16のHR無効化と同じく可逆／不可逆で分岐）に振り分ける。BO担当の"操作を覚える負担"（06-07）をゼロに近づけ、定着率を構造的に押し上げる。

### 追加ツール・手法

1. **n8n セルフホスト（またはWindmill）でのワークフロー自主管理**
   Zapier/Makeの月額課金爆発（05-27失敗パターン）と、シナリオ内変換の埋没（06-13のETL弊害）を回避するため、コード管理可能な n8n を並走。CIパイプライン（06-16のGitHub Actions reusable workflow）と統合し、S案件の週1リリース検証3工程をワークフロー定義ごとGit管理する。ノーコードの敏捷性とコードの監査性のハイブリッド運用。

2. **OpenTelemetry ベースの分散トレーシング＋Grafana／Datadog 可観測性**
   Slack通知だけの成否判定（06-17失敗パターン）を超え、全ジョブに trace_id を付与して「請求書取込 → 与信 → 会計登録 → 通知」のスパンを一気通貫で追う。DLQ件数（06-23）・OAuth失効30日前（07-01）・恒等式（06-12）を同一ダッシュボードに集約し、`/automation status` の裏付けデータを持つ。SLI／SLO（06-13）の実測もここから自動生成。

3. **AI駆動異常検知（Isolation Forest / PyOD / Amazon Lookout for Metrics）による"金額桁ズレの動的検知"**
   社別期待値レンジの台帳（07-01）を人が手動で書く運用から、過去12ヶ月の処理実績から統計的にレンジを自動学習させる方式へ格上げ。月次バッチの出力に対して「同社・同月・同項目の分布から外れたら警告」を自動発火し、ハードコードした閾値の陳腐化（07-01の1社ルール変更見落とし）を防ぐ。

### 追加出力フォーマット

**1. AI Agent Workforce 設計書 `agents/bo_automation_specialist/agent_workforce_design.md`**

```markdown
# AI Agent Workforce 設計書

## 対象業務
{{業務名（例：翔星建設 月次請求書発行～入金消込）}}

## エージェント構成
| Agent | 責務 | I/F入力 | I/F出力 | 可逆性 | 承認関門 |
|-------|------|---------|---------|--------|---------|
| DocIngestAgent | PDF/紙→JSON構造化 | S3パス | 構造化JSON+信頼度 | 可逆 | 不要 |
| ValidationAgent | 期待値レンジ検証 | 構造化JSON | OK/警告/エラー | 可逆 | 警告時のみ |
| PostingAgent | 会計登録 | 検証済みJSON | 仕訳ID | 不可逆 | HARU承認 |
| ReconAgent | 入金消込 | 仕訳ID+入金データ | 消込結果 | 可逆 | 不要 |

## 状態遷移（enum）
- INGESTED → VALIDATED → POSTED → RECONCILED
- 補償イベント: PostingReversed / IngestionRejected

## 失敗時ロールバック
{{各エージェント単位の再実行手順・DLQ退避先}}

## 観測性
- trace_id: {{命名規則}}
- SLI: 各Spanの成功率・p95レイテンシ
- SLO / SLA: {{数値}}

## MCP接続
- 参照SSOT: {{Notion運用台帳／KPI定義／Dat実測}}
```

**2. 月次オートメーション経営レポート `agents/bo_automation_specialist/monthly_exec_report.md`**

```markdown
# 業務自動化 月次経営レポート {{YYYY-MM}}

## ハイライト（3行）
- 削減工数: {{X時間}} / 金額換算: {{Y円}}（Dat DID補正後の純効果）
- 新規稼働: {{件数}} / 廃止: {{件数}} / DLQ滞留: {{件数}}
- 重大インシデント: {{件数}}（詳細は末尾）

## KPI推移（前月比・前年同月比）
| KPI | 今月 | 前月比 | 前年同月比 |
|-----|------|--------|-----------|
| k1_double_input_count | | | |
| k2_vendor_lead_time_minutes | | | |
| k3_bo_manual_hours | | | |
| k4_sla_violation_count | | | |

## クライアント別内訳（サクバズ7社）
| 社名 | 稼働ジョブ | 削減時間 | DLQ | 未対応 |
|------|-----------|---------|-----|-------|
| エスコプロモーション | | | | |
| cantera | | | | |
| ナワショウ | | | | |
...
## 次月アクション（Top3・工数×頻度×単純度＋実測乖離率スコア順）
1.
2.
3.

## 追加投資判断が必要な事項
- {{有料プラン移行／MCPサーバー増強／異常検知モデル再学習 等}}

## インシデント記録
- {{発生日／影響範囲／恒久対策／台帳更新済みか}}
```

### 成長目標

**3ヶ月（〜2026-10-20）**
- サクバズ7社の月次3点セット（請求書発行・売上計上・入金消込）を全社で「ゴールデンテストCSV＋社別期待値レンジ自動学習＋DLQ日次バックフィル」の標準セットで稼働させ、k3_bo_manual_hours を全社平均で月-20時間以上維持。
- n8n セルフホスト環境を1台立ち上げ、既存Zapier案件のうちS案件（05-27）3件を移管し、月額課金を10%削減。
- Notion運用台帳を独自MCPサーバー化し、Claude Codeから自然言語で参照できる状態を構築。

**6ヶ月（〜2027-01-20）**
- AI Agent Workforce 設計書に基づき、DocIngest→Validation→Posting→Recon の4エージェント構成を1クライアントで本番運用開始（電帳法対応込み）。Claude 4 Vision で紙・PDF請求書の構造化精度95%以上を達成。
- プロセスマイニング（無料枠のCelonis SNAP等）でBO担当のSaaS操作ログを取り込み、候補スコアに「実測乖離率」第4軸を追加、恐怖バイアス補正を運用化。
- Slack `/ask-bo` の自然言語I/Fを提供開始、BO担当の起動操作時間を10秒→3秒以下に短縮。

**12ヶ月（〜2027-07-20）**
- 7社全てで AI Agent Workforce 構成へ移行完了、月次経営レポートを HARU 宛に自動配信（Dat DID補正・KPI SSOT期間関数・金額換算まで完全自動）。
- 削減実績を年間ベースで1,500時間（人員換算0.75人月×12ヶ月）を達成、経営報告で「追加投資→さらなる削減」の再投資ループを常態化。
- 建設業DX（gen連携）と統合し、「どっと原価×AI Agent Workforce」の提案パッケージをサクバズ営業武器として ryota / 営業側に提供、新規クライアント獲得のアップセル素材とする。
