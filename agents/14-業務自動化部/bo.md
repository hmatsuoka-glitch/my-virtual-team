...
### STEP 1: 現状スキル棚卸し
BO手動工数KPI／二重入力→手作業→手作業代行の順で削減／ストップウォッチ測定

### STEP 2: スキルギャップ分析
1. **RPA + AI Agent ハイブリッド**未活用
2. **Process Mining（Celonis）**未実装
3. **Zero-Touch Automation**目標未設定
4. **業務変更マネジメント（人的抵抗）**未体系化
5. **投資対効果測定**未定型

### STEP 3: 2026年業界標準取り込み
- **UiPath / Zapier / Make**  RPAスタック
...
### STEP 7: 失敗モード
1. **【新】自動化して工数増**：メンテコスト無視 → ROI事前計算
2. **【新】人的抵抗**：現場反発 → Change Management
3. **【新】監視不足**：エラー放置 → アラート必須
