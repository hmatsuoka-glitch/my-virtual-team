...
### 2026-09-04
- **2026年業界動向：Zapier Copilot/Agents・Make AI・n8n（セルフホスト）・GAS+Gemini・Airtable AI Automationsが「自動化ツール5大選択肢」として確定し、選定はコスト構造と実行環境（SaaS／自ホスト）と機微度で切り分ける**：Zapier Agentsは判断込み処理で1シナリオあたりのtask消費が3〜5倍に膨らみ、07-27記録のAIエージェント化と05-27記録の課金爆発リスクが同時に再燃する。7社ぶんの月次バッチはZapier Agentsに全寄せせず、決定論処理（06-20記録）はMake.com/GAS、判断込み分岐（08-05記録のハイブリッド設計）のみZapier Agentsへ寄せる棲み分けを06-13記録のRPA/BPA層分けと同じ思想で明文化する。ツール選定表を運用台帳（06-03記録）に必須列として追加し、新規ジョブ着手時にkaiとツール決定を1関門で握る。
- **Kuu/Ao連携：建設クライアントの請求・原価データを扱うジョブはn8nセルフホスト（社内VPS）を第一候補にし、Zapier/MakeのSaaS依存を段階的に減らす**：08-03記録のデジタルインボイス移行と07-03記録の実行証跡保全を満たすうえで、SaaS iPaaSはデータが海外リージョンを通過し、電帳法・下請法の観点で建設経営者に説明しづらい。n8nセルフホストなら証跡を自社ストレージ（08-12記録の証跡分離）に閉じられ、Kuuにインフラ・バックアップ・バージョンアップを、Aoにn8nカスタムノード（社内API叩き）を分担してもらう構成をkaiと合意する。SaaS側に残すのは低機微・低頻度のジョブに限定し、社別データ機微度を運用台帳の必須列にしてn8n寄せ／SaaS据え置きの判断を機械化する。
- **Ryota連携：Google Workspaceに閉じるBO業務（見積書整形・議事録整理・請求書メール雛形生成）はGAS+Gemini（Workspace標準）を「第一マイル」にし、iPaaSを投入せず着手前準備を30分以下にする**：09-01記録の`bo new`生成器はiPaaS前提だが、社内・クライアント間でGoogle Workspaceに閉じる業務までiPaaSを通すのは過剰投資で、月間task消費と月額課金を無駄に食う。Ryotaが持つ7社のクライアント接点で「Workspaceに閉じる業務」を先に切り出し、GASからGeminiを叩く最小テンプレを共通スケルトンに追加する。ただしPIIマスキング（07-03記録）と決定論処理をLLMに投げない原則（08-05記録）は同一で、Gemini出力は必ず金額・件数レンジ検証（06-17記録）を通し、Sora QAに通したテンプレのみを現場に公開する。
- **RPA vs LLMワークフローの決定マトリクスを06-13記録のRPA/BPA層分けに追記し、着手前に必ず1回通す**：判定軸は「①公式APIの有無→無ければRPA／有ればBPA」「②処理の決定性→決定論なら従来コード・ルール（08-05記録）／判断込みならLLMエージェント（07-27記録）」「③例外分岐の頻度→月1回未満なら人手＋通知／月10回超ならLLM分岐＋承認関門（05-29記録）」「④PII機微度→高機微は自ホストLLMまたはマスキング前提（07-03記録）／低機微はSaaS LLM可」「⑤月額コスト上限→task/token消費見積を必須入力（05-27記録）」の5軸。マトリクス出力をNotionフォーム（07-07記録）の必須項目にし、kaiへのタスク振り分け依頼時にマトリクス通過の証跡を添付する。マトリクスが結論を出せない案件は着手せず、Owlに状態遷移表の再整理を差し戻す。
- **Slack Workflow Builder 2.0（フォーム＋LLMステップ）とChatGPT Actionsを「BO自己サービス受付口」として整備し、Sora QAを通したBo監修済みテンプレのみ現場が触れる状態にする**：現場が「小さな自動化をBo依頼せず自分で組みたい」ニーズは06-07記録の恐怖感の逆で歓迎すべきだが、無統制に組ませると08-05記録のフェイルオープンやPII直流し（07-03記録）が量産される。Slack Workflow Builder 2.0のLLMステップにBoが事前検証したプロンプト・PIIマスキング・件数突合ラッパー（06-16記録）を組み込んだテンプレを3〜5種類用意し、Sora QAの関所を通した版のみ現場に公開する。ChatGPT Actionsで社内APIを叩くCustom GPTも同じ関所ルールで、read-onlyスコープ（06-12記録）＋要対応チャンネル通知（08-16記録）を必須設定にし、現場自作の暴走を発生源で止める。
