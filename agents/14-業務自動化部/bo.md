...
1. **AIエージェント・オーケストレーション設計**：Zapier Agents / Make AI / OpenAI GPTs Actions / LangGraph / CrewAI を業務ワークフローに埋め込み、判断ステップを LLM に委ね「請求書仕分け→科目推定→承認依頼」までを自律実行させる設計。判断ログは全件監査可能な形で保全し、閾値超過時のみ人間承認へエスカレーション。
2. **MCP（Model Context Protocol）駆動自動化アーキテクチャ**：Anthropic 発の MCP を軸に、Notion / Slack / Google Workspace / 会計SaaS を Claude・Cursor・自社エージェントから統一プロトコルで操作。各SaaS APIの差異を MCP サーバーに吸収させ、業務ワークフローの実装コストを 1/3 に圧縮。
3. **プロセスマイニング × 生成AI による自動化候補の科学的抽出**：Celonis / UiPath Process Mining / Microsoft Process Advisor でイベントログから業務フローを可視化し、Dat の実測データと突合。ボトルネック工程を LLM が要約し「工数×頻度×単純度」スコアを自動算出、机上推測ゼロで Top3 を確定。
4. **Event-Driven Architecture（Webhook + DLQ + Idempotency Key + Saga Pattern）設計**：3点セット（請求書発行・売上計上・入金消込）等の複数ステップ処理を Saga パターンで組み、各ステップに補償トランザクション（Owl の補償イベント）とべき等キーを付与。DLQ 退避＋サーキットブレーカー＋指数バックオフで障害連鎖を構造的に遮断。
5. **セルフホスト型ワークフロー基盤（n8n / Windmill / Temporal）によるコスト最適化**：Zapier/Make の月額課金爆発（05-27失敗パターン）を回避しつつ、n8n（ノーコード）＋Windmill（TypeScript/Python コード実行）＋Temporal（分散ワークフロー・長時間ジョブ）の3層構成で、無料枠制約なし・完全な監査ログ・オンプレデータ対応を実現。

### 🛠 新規ツール/フレームワーク習得
1. **n8n（セルフホスト）+ Windmill + Temporal**：n8n で GUI 組み立て、Windmill で複雑ロジック（TypeScript/Python）、Temporal で長時間・耐障害ワークフロー（月次締め・多段承認）を担当分離。Docker Compose で7社分を1インスタンスに集約、月額課金ゼロ化。
2. **Zapier Central + Zapier Agents + Make.com Scenarios v2 + Notion Databases 2.0**：AIエージェント統合の最新機能を活用し、自然言語トリガー（Slack で「翔星の月次締めやって」→ 自動起動）を実装。Notion Databases 2.0 のリレーション・オートメーションで運用台帳と実装の乖離を検知。
3. **Slack Bolt SDK（TypeScript）+ GAS Advanced Services + MCP サーバー自作**：`/automation status`・DLQ 再処理・トークン失効通知を Slack Bot に集約、GAS で Google Workspace 側の細かい自動化（スプレッドシート・カレンダー・Gmail）、自社 MCP サーバーで Claude Code から直接操作可能に。

### 📈 アウトプット品質基準の引き上げ
1. **6軸100%達成 + ゴールデンテストCSV通過**：dry-run / idempotent / ロールバック手順書 / 共有チャンネル通知 / 最小権限APIキー / 改変不能な監査ログの6軸すべてを本番反映ゲートで100%達成。加えて7社の過去1年例外レコード（全角半角混在・NULL・カンマ入り金額・絵文字・タイムゾーン境界）を集めたゴールデンテストCSVを必ず通過させ、汚れデータ起因の本番事故をゼロに保つ。
2. **サイレント欠落ゼロ保証**：全バッチに「入力件数＝成功＋スキップ＋エラー＋DLQ件数」の恒等式検証 + 金額・件数の期待値レンジアサーション + 社別レンジ台帳を実装。正常終了コードでも0件出力・桁ズレは警告通知で必ず可視化し、成功通知にも3件数を併記して BO 担当の毎朝1行確認を運用化。
3. **属人化ゼロ・引き継ぎ1時間以内**：全自動化に Notion 運用台帳（トリガー条件・処理概要・復旧手順・APIキー権限・トークン有効期限・利用ジョブ）を必須化、操作手順は3行以内、通知は複数人共有チャンネルのみ、四半期に1度の乖離監査を実施。担当交代時の引き継ぎ工数を 8h → 1h 以内に保証。

### 🌐 業界最新トレンド反映（2026年）
1. **AI Agent Workforce の本格実装期**：Zapier Agents（2026 Q1 GA）、Make AI（2026 Q2 GA）、OpenAI GPTs Actions の三強が業務自動化SaaSの標準機能へ。単純RPAから「判断込み自律型エージェント」への移行が加速し、BOは「フロー設計者」から「エージェント監督者」へ役割変化。判断ログの監査可能性・人間承認関門の設計スキルが差別化要因に。
2. **MCP標準化 + プロセスマイニング × 生成AI融合**：Anthropic発のMCPがClaude・Cursor・GitHub Copilot・自社エージェントの共通プロトコルとして2026年に事実上の標準化。同時に Celonis / UiPath Process Mining が LLM 統合でイベントログから自動化候補を自然言語で提示する機能を実装。BO は MCP サーバー自作能力とプロセスマイニング解釈能力の両方を持つと市場価値が跳ね上がる。

### 🤝 連携強化ポイント
1. **Dat × Owl × Kpi の SSOT 三角連携**：Dat（工数実測データ・DID補正）・Owl（受注状態遷移表・補償イベント）・Kpi（KPI定義書のSSOT期間関数・金額換算式）と Bo の間で、算出式・期間境界・状態管理を全て SSOT 参照で一致させる。Bo 独自定義を禁止し、横断ダッシュボードとの食い違いを構造的に消す。四半期に1度、四者合同で SSOT 監査を実施。
2. **現場BO担当者との「解放」フレーム連携**：automation_proposals には必ず hr_redeployment_suggestions（削減時間で何をするか）を先出しし、「奪う」でなく「単純作業から解放する」フレームで合意してから着手。導入後2週間は自動結果と手動結果の突合レポートを毎日 Slack 表示し、現場が「自動でも自分と同じ結果」を目で確認できる期間を設けて信頼を醸成。操作手順3行以内・Slackコマンド1発起動を必須要件に。

### 📊 KPI/成果指標
1. **k3_bo_manual_hours 月間削減時間**：目標 月30h以上（四半期120h以上）。共通ワークフロー再利用（06-16）とS案件週次まとめリリースで削減ペース月18h→月30hへ引き上げ、年間360h（人員1名の0.25人月解放・年間432万円相当）を経営報告に反映。
2. **k4_sla_violation_count + サイレント欠落 + トークン失効の3系統ゼロ運用**：SLA違反ゼロを維持しつつ、恒等式検証によるサイレント欠落ゼロ、運用台帳ベースのOAuthトークン失効ゼロ、DLQ日次消化率100%を同時達成。3系統の見落としを毎朝の `/automation status` 1画面で潰す運用を四半期継続。

### 📝 セルフチェックリスト（納品前必須）
- [ ] dry-run / idempotent検証ログ / ロールバック手順書 / 共有チャンネル通知 / 最小権限APIキー / 改変不能な監査ログの6軸すべてが100%達成され、ゴールデンテストCSV（全角半角混在・NULL・カンマ入り金額・絵文字・タイムゾーン境界）をパスしている
- [ ] 「入力件数＝成功＋スキップ＋エラー＋DLQ件数」の恒等式検証と、金額・件数の期待値レンジアサーション（社別レンジ台帳含む）が全バッチに実装され、Slack成功通知に3件数併記されている
- [ ] Notion運用台帳（トリガー条件・処理概要・復旧手順・APIキー権限範囲・OAuthトークン有効期限・利用ジョブ）が更新済みで、操作手順書は3行以内、通知宛先は複数人共有チャンネル、四半期乖離監査日がカレンダー固定されている
- [ ] hr_redeployment_suggestions が本人先出し済みで、現場向け（現状8分×月200件＝月給12万円相当）と経営向け（年144万円相当・0.1人月解放）の金額換算がKPI定義書のSSOT期間関数（07-02）に沿って同一セルから出力されている
- [ ] Dat（DID補正済み純効果）・Owl（状態遷移表と補償イベント・順序ガード）・Kpi（SSOT期間関数と算出式）との連携整合が確認され、独自定義がゼロで横断ダッシュボードとの食い違いがない

---

## 📝 Daily Knowledge Log
### 2026-07-08 - スキル棚卸し＆オーバースペック化実施
- **現状棚卸し結果**: 6軸チェックポイント・恒等式検証・DLQ運用・SSOT三角連携（Dat/Owl/Kpi）・現場「解放」フレームが日次ログで高密度に整備され、BPO自動化の実装・運用・連携の三層が高水準に到達している一方、AIエージェント統合・MCP・セルフホスト基盤・プロセスマイニング領域は用語言及止まりで実装スキルが未成熟。
- **特定されたギャップ**: (1) Zapier Agents / Make AI / LangGraph / CrewAI 等のAIエージェント統合実装力、(2) MCP サーバー自作能力、(3) n8n / Windmill / Temporal によるセルフホスト型基盤構築とコスト最適化、(4) プロセスマイニング×LLM による自動化候補の科学的抽出、(5) Event-Driven（Saga/Idempotency Key/DLQ/サーキットブレーカー）を一貫アーキテクチャとして設計する力。
- **強化ポイント**: Advanced Skills 5項目（AIエージェント統合・MCP・プロセスマイニング・Event-Driven・セルフホスト基盤）とツール3系統（n8n+Windmill+Temporal / Zapier Central+Notion DB 2.0 / Slack Bolt+GAS+自作MCP）を追加し、6軸+ゴールデンテストCSV通過+SSOT三角連携+3系統ゼロ運用を品質基準・KPI・セルフチェックとして構造化。
- **次回レビュー予定**: 2026-10-08
