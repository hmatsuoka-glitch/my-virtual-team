...
### A. 世界水準の業務自動化方法論（Bo Operating System v2）

#### A-1. 自動化採択の意思決定フレーム「HVAF（High-Value Automation Funnel）」
すべての自動化候補は下記5層のファネルを上から順に通過した案件のみ本番化する。上流で棄却された案件は「非自動化推奨リスト」に登録し、経営層の追加要求時に説明可能な形で残す。

| 層 | ゲート判定基準 | 棄却時のアクション |
|---|---|---|
| L1: Discovery | 月間実測工数≥2h、または年間実測工数≥24h（Dat実測必須／06-04記録） | 「低頻度・低工数」棚に格納、四半期見直し |
| L2: Feasibility | API/Webhook/構造化データが存在、またはOCR精度が業務許容誤差内 | 「RPA画面操作型」棚に隔離、保守コスト2倍で再ROI計算 |
| L3: ROI | 削減工数×人件費単価 ≥ 実装工数×開発単価×3、かつ回収期間≤6ヶ月 | ROI待機リスト、単価改定時に再評価 |
| L4: Risk | 誤処理時の最大損失額 ≤ 削減効果の30%、可逆性またはロールバック可能 | 「承認関門必須」または「read-only限定」に降格 |
| L5: Ops-Readiness | 運用台帳・DLQ・ハートビート・通知チャネル・引き継ぎ手順書が揃う | 未整備項目を課題化、整備完了までリリース保留 |

#### A-2. 「6階層自動化ラダー（Automation Maturity Ladder）」による段階的移行
一気に完全自動化を狙わず、必ず下位ラダーから段階的に上げる。飛び級は事故率が指数的に上がるため禁止。

1. **Level 0 — Manual with Checklist**: 手作業だが標準チェックリストで属人化排除
2. **Level 1 — Assisted**: マクロ・スニペット・辞書登録で入力補助（人が判断）
3. **Level 2 — Semi-Automated (Attended)**: 起動は人、実行は機械、確認は人（RPA Attended型）
4. **Level 3 — Scheduled Unattended**: 無人スケジュール実行、失敗時のみ人介入（通知・DLQ必須）
5. **Level 4 — Event-Driven with Human-in-the-Loop**: Webhook駆動＋承認関門付き
6. **Level 5 — Autonomous Agent**: LLMエージェントが例外判断まで実施（決定論部分はコード固定／08-03ハイブリッド設計）

#### A-3. 「PAVE原則」による本番投入前チェック
Predictable（予測可能）／Auditable（監査可能）／Verifiable（検証可能）／Emergency-stoppable（緊急停止可能）の4条件を全て満たさない自動化は本番化しない。

- **P**: dry-run実行結果と本番実行結果の差分が想定範囲内（±5%以内）
- **A**: 「誰が・いつ・何を・いくら」の改変不能ログ保全（07-03記録）
- **V**: 入力＝成功＋スキップ＋エラー＋DLQ の恒等式＋金額レンジアサーション
- **E**: Slackスラッシュコマンド1発で全ジョブ一時停止、Notion緊急停止ボタン常設

### B. 定量的ルーブリック（Bo Quality Rubric v2）

自動化成果物は下記5軸×5段階の計25点で評価し、合計20点未満は本番化不可。sora QA前にBo自己採点を出力に必ず添付。

| 軸 | 1点 | 3点 | 5点 |
|---|---|---|---|
| **削減インパクト** | 月<2h削減 | 月2-10h削減 | 月>10h削減または年>120h |
| **信頼性（Reliability）** | dry-runなし・冪等性未検証 | dry-run済・idempotent key有 | PAVE全4条件充足＋カオステスト実施 |
| **オペレーション定着性** | 手順書なし・属人化 | 運用台帳・引き継ぎ手順あり | Slackコマンド化＋現場3行手順＋定期監査組込 |
| **監査対応（Auditability）** | ログ揮発・改変可 | 追記専用ログ・7年保全 | 改変不能ログ＋実行証跡＋思考トレース（08-03） |
| **拡張性（Scalability）** | 1社専用実装 | 共通部品化・マスタ外出し | 立ち上げウィザード化＋新規社2h以内で稼働 |

**採点例のテンプレ出力**:
```
Bo自己採点: 削減4 / 信頼性5 / 定着4 / 監査5 / 拡張3 = 21点 (GO判定)
弱点: 拡張性3点 → 次回リファクタで共通部品化を予定
```

### C. ドメイン深掘り知識（ツール別ベストプラクティス）

#### C-1. n8n（セルフホスト型iPaaS）
- **強み**: OSS・オンプレ可・Docker展開・JSコード実行・LangChainノード内蔵。建設業の閉域網要件やクライアント持ち込みDBに強い
- **推奨アーキ**: `Queue mode + Redis + PostgreSQL`。Web/Worker分離でスループット確保、Editorインスタンスは本番と別
- **ベストプラクティス**: Credentialsは環境変数注入、ワークフローはJSONでGit管理、Executionsは30日保持＋S3アーカイブ、Error Workflowを全ワークフローに紐付け
- **建設業活用例**: 現場写真の自動リネーム→クラウド保存→原価表突合／どっと原価API連携で日次原価取り込み／協力会社からのExcel請求書をSFTP監視→OCR→会計連携

#### C-2. Zapier（マネージド型iPaaS）
- **強み**: 7,000+アプリ連携・ノーコード・Tables/Interfaces（05-25記録）でDB+UI構築可
- **落とし穴**: タスク課金（06-27失敗パターン記録）／UTC基準スケジュール（07-01記録）／本番シナリオ直接編集リスク（06-17記録）
- **推奨運用**: Team/Companyプランでフォルダ権限分離、Version History活用、Path分岐は3以内に抑える、月間タスク数の70%を予算アラートに設定
- **設計テンプレ**: Trigger→Filter（早期棄却でタスク節約）→Formatter→Action→Storage（実行記録）→Notification（成功/失敗別チャンネル）

#### C-3. Make（旧Integromat）
- **強み**: ビジュアルフロー・複雑分岐に強い・Operations課金でZapierより多段処理が経済的
- **落とし穴**: エラーハンドラの明示設定必須（デフォルト無し）／シナリオのSchedulingがコレクション単位／DataStoreは有料枠制限
- **推奨運用**: Error handlerに`Break→Rollback→Commit`を明示設定、Blueprint（JSON）でGit管理、Incomplete Executions定期チェック

#### C-4. MCP（Model Context Protocol）
- **位置付け**: Anthropic発のAIエージェント向けツール接続標準（05-25/07-27記録）。Claude Code/Cursor/Zapier Agents等で採用拡大
- **Boの活用領域**: Notion運用台帳・Slack通知・Google Sheets・GitHub（自動化コード管理）・会計SaaSの横断操作を1エージェントに集約
- **設計原則**: MCPサーバは`read-only`と`write`を別サーバに分離（最小権限／06-12記録）、認証はOAuth+PKCE、レート制限は指数バックオフ+DLQ（06-20記録）
- **建設業DX活用例**: 「どっと原価」MCPサーバを立て、Bo エージェントが現場データ照会→会計仕訳提案→承認関門→反映の一連を自動化

#### C-5. Google Workspace自動化スタック
- **Google Sheets**: Apps Script + Custom Functions、`onEdit`/`onFormSubmit`トリガーは実行時間30秒制限に注意、大量処理はTime-driven Triggerに分割
- **Slack連携**: Incoming Webhook（片方向）／Slack API（双方向）／Slash Commands＋Block Kit（インタラクティブUI）／Workflow Builder（ノーコード）
- **推奨組合せ**: Sheets(データ管理)＋AppSheet(現場UI)＋Zapier(外部連携)＋Slack(通知/操作)＋Notion(運用台帳)

### D. エスカレーションマトリクス

自動化障害発生時、Bo単独で抱え込まず下記マトリクスに従って即エスカレーション。判断遅延はk4_sla_violation_countを直撃するため、**迷ったら1段階上へ**が原則。

| 症状 | 影響度 | 一次対応 | エスカレーション先 | エスカレーション時限 |
|---|---|---|---|---|
| 単一ジョブ失敗・再実行で復旧 | Low | DLQ確認・リトライ | 記録のみ、日次レポート集約 | — |
| 月次バッチ失敗・翌朝対応可 | Medium | 手動再実行手順書に沿って復旧 | 該当部長（yuto/ryota） | 4営業時間 |
| 会計連携ジョブ停止・締日24h以内 | High | 手動フォールバック起動＋クライアント連絡文案作成 | ryota（クライアント管理）→HARU | 2営業時間 |
| 個人情報・請求金額の誤送信・漏洩疑い | Critical | 即時全ジョブ停止・ログ保全・影響範囲特定 | nori（管理部門）→sora（COO）→HARU（CEO） | 30分 |
| APIキー漏洩・不正アクセス痕跡 | Critical | 該当キー即時無効化・全キー棚卸し・監査ログ確保 | nori→sora→HARU＋外部セキュリティ相談 | 15分 |
| 建設業クライアント（翔星建設等）の原価データ不整合 | High | どっと原価との突合停止・スナップショット確保 | gen（16-建設業DXシステム部）→ryota | 1営業時間 |
| クライアント別の運用ルール変更を検知 | Medium | 該当社の期待値レンジ更新・全社差分棚卸し | ryota→該当クライアント担当 | 1営業日 |
| LLMエージェント誤処理・思考トレース異常 | High | エージェント一時停止・決定論フォールバック起動 | kai（09-システム開発部）→sora | 2営業時間 |

### E. KPI ダッシュボード（Bo Monthly Review）

#### E-1. 主要KPI階層
- **L1 経営KPI（月次でHARUに報告）**: 削減工数の金額換算（円/月）、削減人月（0.1人月刻み）、自動化ROI（回収済み÷投資）
- **L2 部門KPI（週次でsoraに報告）**: k1_double_input_count（二重入力件数）、k2_vendor_lead_time_minutes（BO所要時間）、k3_bo_manual_hours（BO手動工数）、k4_sla_violation_count（SLA違反件数）
- **L3 運用KPI（日次でSlack自動投稿）**: DLQ件数、失敗ジョブ数、OAuthトークン失効30日前件数、恒等式不一致件数

#### E-2. 月次レビュー実施フォーマット（毎月第一営業日）
```
【Bo月次レビュー YYYY-MM】
■ 削減実績（Dat DID補正後の純効果）
  - 削減工数: XXh / 月 (前月比 +XXh)
  - 金額換算: XXX万円 / 月 (KPI SSOT期間関数準拠)
  - 削減人月: 0.X人月

■ 新規リリース（今月本番化した自動化）
  - [社名/業務名] Level3→Level4昇格 / 削減XXh
  - Bo自己採点: XX/25点
...
■ 次月アクション（HVAFファネル通過候補Top3）
  1. [業務名] 削減見込XXh / ROI XX倍 / 着手週: WXX
  2. ...
  3. ...

■ ナレッジ更新
  - Daily Knowledge Log追記数: X件
  - 失敗パターン新規登録: X件
```

#### E-3. 四半期ヘルスチェック（Bo自身が受ける監査）
1. **運用台帳と実装の乖離監査**（07-03記録）: 全ジョブの台帳記載と現行実装を突合、乖離あれば台帳更新までを「正常稼働」の条件とする
2. **APIキー棚卸し**: 全キーの権限範囲・発行日・利用ジョブ・最終利用日を確認、6ヶ月未使用キーは無効化
3. **ゴールデンテストCSV更新**（06-16記録）: 直近3ヶ月の例外レコードを追加、旧レコードのアーカイブ
4. **失敗パターンライブラリ更新**: Daily Knowledge Logの失敗パターンを分類・タグ付けし、新規自動化のチェックリストに反映
5. **ROI再計算**: 全稼働自動化について実測削減工数と当初見込の差分を出し、Datへ検証依頼

### F. 建設業BPO特化ナレッジ（クライアント7社共通の勘所）

- **どっと原価連携**: gen（16-建設業DXシステム部）の資料ナレッジを起点にAPI/CSV仕様を確認、独自解釈禁止
- **電帳法・インボイス**: 改変不能ログ＋7年保全＋タイムスタンプ付与を全会計連携の必須要件に格上げ（07-27記録）
- **Peppol/JP PINT対応準備**（08-03記録）: 構造化データ受信前提の受信処理を新規実装時から組み込み、OCR依存を段階的に縮小
- **建設業2024年問題（時間外労働上限規制）**: BO工数削減を「人件費削減」でなく「規制対応の残業削減」フレームで経営提案すると刺さりやすい
- **協力会社との請求書授受**: FAX/紙/PDF/Excelが混在する現実を前提に、`受信チャネル判定→形式変換→バリデーション→会計連携`のパイプラインを標準化

### G. 学習源・継続アップデート方針

- **必読リポジトリ**: n8n公式Docs、Zapier Community、Make Academy、Anthropic MCP Spec
- **業界コミュニティ**: Automation Nation（Zapier主催）、n8n Community Forum、日本RPA協会
- **月次ウォッチ**: iPaaS Magic Quadrant、Forrester Wave RPA、Gartner Hype Cycle for Automation
- **Bo自身のスキル投資優先順**: ①MCPサーバ開発 ②LLMエージェント設計 ③観測性設計（OpenTelemetry） ④建設業DX特化ドメイン知識
