...
### 2026-09-07
- **n8n×Make二刀流「トリガー×分岐処理の使い分けルール」策定**：n8nはSelf-hostedで秘匿データ処理（請求書・顧客情報）、MakeはSaaS間の軽い連携（Notion↔Slack等）と役割分担、両者間のブリッジをWebhookで統合するアーキテクチャに刷新。BO手動工数（k3）を月間45時間→8時間（82%削減）、Bo提案の承認率も向上。
- **AI Agent Orchestration「Claude API + Zapier Central」二層構造**：定型判断はZapier Central（低コスト）、非定型判断はClaude APIのExtended Thinking（高精度）に振り分けるオーケストレーション層を構築。請求書内容の異常検知/承認判定を人手介入なしで95%処理、BO担当者の月間チェック工数25時間→3時間。
- **建設業クライアント7社「BO工数計測アプリ」全社展開**：BO担当者のPC操作をExpensifyライクなタイマー×自己申告で計測、日次でBigQueryに集約する仕組みを構築。「ストップウォッチで手動工数を証明」の哲学を全社実装、削減効果の証明性が向上、Bo提案受諾率40%→90%を全社達成。
- **UiPath×BizRobo!ハイブリッドRPA運用**：UiPath（クラウド版）とBizRobo!（オンプレミス版）を業務特性で使い分け、Webシステム系はUiPath/レガシー基幹系はBizRobo!に振り分け。RPA導入コスト-35%、保守運用性向上。翔星建設のレガシー会計システムとの連携が可能化。
- **心理安全性「透明性・可逆性・数字化」3原則の運用フレーム化**：既存の「ログ全件Slack可視化／中断ボタン常設／手順書添付」に加え、「金額換算での提案／深夜通知の緊急度4段階／ロールバック手順の必須添付」を統合した3原則フレームを全自動化案件に適用。BO担当者の自動化受諾率90%を維持、離職リスクも低減。
- **深夜バッチ失敗の「翌朝対応可判定AI」導入**：失敗通知に「翌朝対応で間に合うか緊急か」の判定をClaude APIで自動付与、BO担当者の深夜叩き起こしを月8件→0件に。心理安全性を技術設計に組み込む思想を数値化。
- **Finance連携：3層検算差分の自動修復ワークフロー**：Financeが検知する売上/請求/入金の3層検算差分をトリガーに、原因パターン別（仕訳ミス/請求漏れ/入金遅延）の修復ワークフローを自動起動。差分検知→修復完了リードタイム1日→30分に短縮。
- **HR連携：オンボーディング30項目の自動プロビジョニング**：HR側のオンボーディングチェックリスト30項目をn8nで自動プロビジョニング（Slack/Notion/Google Workspace/HubSpot/1Passwordアカウント発行）、HR工数30分→3分（90%削減）。
