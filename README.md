...
## 👥 チーム編成（<!-- AGENTS_COUNT -->58<!-- AGENTS_COUNT_END -->人）
...
| 部署 | エージェント（役職） |
|------|------------------|
| 00-COO | deva（Devil's Advocate / 批判的検証担当） / sora（COO / クオリティアシュアランスリード） |
| 01-経営企画部 | fuca（FCビジネスアナリスト） / haruto（経営企画リード） / retri（議事録・資料リサーチャー） / sutu（イシューストラクチャラー） |
...
| 05-データ分析部 | deng（データエンジニア） / shun（データアナリスト） |
| 06-リサーチ部 | ana（アナロジー事例リサーチャー） / rui（インダストリーリサーチャー） |
| 07-LP部 | hana（CSS抽出スペシャリスト） / iro（ブランドカラー抽出スペシャリスト / LP制作係） / kaito（LP部 部長 兼 複製係 係長 / LP複製プロジェクトディレクター） / kotone（LPコピーライター / LP制作係） / mia（ビジュアルQAスペシャリスト） / nao（フロントエンド設計スペシャリスト） / ren（フロントエンド実装スペシャリスト） / saki（LP修正スペシャリスト） / sota（LPデザイン企画スペシャリスト） / tsumugi（LP制作係 係長 / LP制作プロジェクトディレクター） |
...
| 11-管理部門 | finance（経理・財務マネージャー） / hr（人事マネージャー） / legal（リーガル（法務 / 契約・知財・リスク法務担当）） / nori（リーガルチェック・コンプライアンス担当） |
| 12-営業部 | sales（営業マネージャー） |
| 13-マーケティング部 | marketing（マーケティングマネージャー） / pr（広報・PRマネージャー） |
| 14-業務自動化部 | bo（業務自動化スペシャリスト） / owl（受注ワークフロー設計者） |
| 15-横断チーム | dat（横断データアナリスト） / kpi（横断KPIダッシュボードマネージャー） / pm（横断プロジェクトマネージャー） / qa（横断QAレビュアー） |
| 16-建設業DXシステム部 | gen（建設業DXナレッジスペシャリスト） |
